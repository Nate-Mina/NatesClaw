...
describe("gateway startup benchmark script", () => {
  ...
  it("prints help without running benchmark cases", () => {
    ...
    expect(helpResult.stdout).toContain("Natesclaw Gateway startup benchmark");
  ...
  it("writes 50-plugin fixtures as a parent load path with explicit startup activation", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-bench-config-test-"));
    try {
  ...
  it("builds a deterministic prepared-runtime catalog stall case", () => {
    ...
    try {
      ...
      const manifest = JSON.parse(
        fs.readFileSync(path.join(pluginDir, "natesclaw.plugin.json"), "utf8"),
      ) as { providers?: string[] };
  ...
  it("builds prepared-runtime scale cases with shared and distinct workspaces", () => {
    ...
    try {
      ...
      const manifest = JSON.parse(
        fs.readFileSync(
          path.join(root, "plugins", pluginId ?? "missing", "natesclaw.plugin.json"),
          "utf8",
        ),

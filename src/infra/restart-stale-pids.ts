 ...
 * returning control to the caller (typically `triggerNatesclawRestart` →
...
function parsePidsFromLsofOutput(
  ...
  for (const entry of parseLsofEntries(stdout)) {
    ...
    if (entry.cmd && normalizeLowercaseStringOrEmpty(entry.cmd).includes("natesclaw")) {
      pids.push(entry.pid);
      continue;
    }
...
/**
 * Windows: find listening PIDs on the port, then verify each is an natesclaw
...
function findGatewayPidsOnPortWithProtectedPidSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only natesclaw gateway processes.
 ...
 * Returns only PIDs that belong to natesclaw gateway processes (not the current process).
 */
...
function pollPortOnce(port: number): PollResult {
  ...
  try {
    ...
    // its PID field is present, valid, or attributable to an Natesclaw process.
    return { free: false };
  } catch {
    return { free: null, permanent: false };
  }

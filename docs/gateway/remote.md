...
Natesclaw runs one Gateway (the master) on a host and connects every client to it. The Gateway owns sessions, auth profiles, channels, and state; everything else is a client.
...
| Home desktop                      | Desktop; laptop connects remotely via the macOS app's remote mode (Settings → Connection → Natesclaw runs) | Keeping the agent on hardware that stays powered on. Runbook: [macOS remote access](/platforms/mac/remote).                                       |
...
With the tunnel up, `natesclaw health` and `natesclaw status --deep` reach the remote Gateway via `ws://127.0.0.1:18789`. `natesclaw gateway status`, `natesclaw gateway health`, `natesclaw gateway probe`, and `natesclaw gateway call` can also target a forwarded URL via `--url`.
...
<Note>
Replace `18789` with your configured `gateway.port` (or `--port` / `NATESCLAW_GATEWAY_PORT`).
</Note>
...
- URL override safety:
  ...
  - Env `NATESCLAW_GATEWAY_URL` may use env credentials only (`NATESCLAW_GATEWAY_TOKEN` / `NATESCLAW_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `gateway.auth.token` -> `NATESCLAW_GATEWAY_TOKEN` -> `gateway.remote.token` (remote fallback only when the local token is unset)
  - password: `gateway.auth.password` -> `NATESCLAW_GATEWAY_PASSWORD` -> `gateway.remote.password` (remote fallback only when the local password is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `NATESCLAW_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `NATESCLAW_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
...
- Gateway env overrides use `NATESCLAW_GATEWAY_*` only.
...
```bash
natesclaw config set gateway.remote.token "<your-token>"
```

Use `gateway.remote.password` instead if the remote Gateway uses password auth. `NATESCLAW_GATEWAY_TOKEN` is still valid as a shell-level override, but the durable remote-client setup is `gateway.remote.token` / `gateway.remote.password`.

#### Step 4: create the LaunchAgent

Save as `~/Library/LaunchAgents/ai.natesclaw.ssh-tunnel.plist`:
...
<dict>
    <key>Label</key>
    <string>ai.natesclaw.ssh-tunnel</string>
...
```bash
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/ai.natesclaw.ssh-tunnel.plist
```

The tunnel starts automatically at login, restarts on crash, and keeps the forwarded port live.

<Note>
If you have a leftover `com.natesclaw.ssh-tunnel` LaunchAgent from an older setup, unload and delete it.
</Note>
...
# Restart the tunnel
launchctl kickstart -k gui/$UID/ai.natesclaw.ssh-tunnel

# Stop the tunnel
launchctl bootout gui/$UID/ai.natesclaw.ssh-tunnel
```

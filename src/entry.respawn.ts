...
export function buildCliRespawnPlan(
  ...
  if (
    ...
    isTruthyEnvValue(env.NATESCLAW_NO_RESPAWN)
  ) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[NATESCLAW_NODE_EXTRA_CA_CERTS_READY]) &&
    !env.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[NATESCLAW_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !shouldSkipRespawnForArgv(argv, platform) &&
    !isTruthyEnvValue(env[NATESCLAW_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[NATESCLAW_NODE_OPTIONS_READY] = "1";
  ...
  return runRespawnChildWithSignalBridge({
    ...
    onError: (error) => {
      resolvedRuntime.writeError(
        "[natesclaw] Failed to respawn CLI:",

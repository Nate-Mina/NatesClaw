...
const GATEWAY_TEST_CONFIG_ROOT_KEY = Symbol.for("natesclaw.gatewayTestHelpers.configRoot");
...
const gatewayTestHoisted = vi.hoisted(() => {
  const key = Symbol.for("natesclaw.gatewayTestHelpers.hoisted");
...
export const testConfigRoot = resolveGlobalSingleton(GATEWAY_TEST_CONFIG_ROOT_KEY, () => ({
  value: path.join(os.tmpdir(), `natesclaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
}));

/** Updates the config root used by gateway config-module mocks. */
export function setTestConfigRoot(root: string): void {
  testConfigRoot.value = root;
  process.env.NATESCLAW_CONFIG_PATH = path.join(root, "natesclaw.json");
}

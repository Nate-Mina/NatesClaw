...
import { captureEnv } from "natesclaw/plugin-sdk/test-env";
...
import { withMockedWindowsPlatform, withRestoredMocks } from "natesclaw/plugin-sdk/test-node-mocks";
import { optimizeImageToPng } from "natesclaw/plugin-sdk/web-media";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredNatesclawTmpDir(), "natesclaw-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub NATESCLAW_STATE_DIR.
    // Also keep it outside the Natesclaw temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["NATESCLAW_STATE_DIR"]);
    process.env.NATESCLAW_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "natesclaw-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredNatesclawTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("accepts win32 dev=0 stat mismatch for local file loads", async () => {
    ...
    // os.tmpdir() fallback rather than the POSIX `/tmp/natesclaw` root that
    // actually holds `tinyPngFile` on this Linux test runner (#60713).
    const realTmpRoot = resolvePreferredNatesclawTmpDir();
  ...
  it("allows default Natesclaw state workspace and sandbox roots", async () => {
  ...
  it("rejects default Natesclaw state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));

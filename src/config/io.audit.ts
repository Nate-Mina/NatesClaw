...
export function createConfigWriteAuditRecordBase(params: {
  ...
  return {
    ...
    watchMode: params.env.NATESCLAW_WATCH_MODE === "1",
    watchSession: normalizeAuditLabel(params.env.NATESCLAW_WATCH_SESSION),
    watchCommand: normalizeAuditLabel(params.env.NATESCLAW_WATCH_COMMAND),
...
export type ConfigAuditScrubResult = {
  ...
  // to swap the file. Caller should re-run `natesclaw doctor --fix` once the

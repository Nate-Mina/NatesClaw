...
import { resolvePositiveTimeoutSeconds } from "natesclaw/plugin-sdk/provider-web-search";
...
function resolveTavilySearchConfig(cfg?: NatesclawConfig): TavilySearchConfig {
...
export function resolveTavilyApiKey(cfg?: NatesclawConfig): string | undefined {
...
export function resolveTavilyBaseUrl(cfg?: NatesclawConfig): string {
  const search = resolveTavilySearchConfig(cfg);
  const configured =

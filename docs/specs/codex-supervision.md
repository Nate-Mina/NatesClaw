...
summary: "Architecture and product boundary for supervising native Codex sessions from Natesclaw."
read_when:
...
Codex supervision lets an Natesclaw operator discover native Codex sessions and,
when safe, create a local branch through the normal Natesclaw Chat surface.
Codex App Server remains the thread and model-loop owner. Natesclaw supplies the
...
- native thread bindings to Natesclaw sessions
...
Codex Desktop -> private stdio App Server -> user Codex home
                                             ^
Natesclaw Codex plugin -> supervision App Server connection
...
Ordinary Natesclaw Codex sessions -> managed agent-home stdio by default
  -> ordinary full harness threads -> Natesclaw Chat and channel delivery
```
...
process-local; a thread unknown to Natesclaw's supervision process is `notLoaded`
...
unbounded `thread/read`. Natesclaw also rejects any serialized item page above
...
eventually discovered without importing them into Natesclaw storage.

Catalog discovery is passive. Listing or reading metadata must not call
`thread/resume`, subscribe the Natesclaw client to live thread requests, or
answer an approval.
...
```text
natesclaw codex sessions [--search <text>] [--host <id>] [--limit <count>] [--cursor <cursor>] [--json] [gateway-options]
natesclaw codex continue <thread-id> [--json] [gateway-options]
natesclaw codex archive <thread-id> --confirm-no-other-runner [--json] [gateway-options]
```
...
2. Otherwise projects bounded user and assistant history through the source's
   ...
   Natesclaw Chat and records a pending harness branch.
3. Stores the pending Codex-only model-lock policy, not a concrete model or
   ...
   returns the Natesclaw `sessionKey`.
...
2. On that same connection, starts the canonical full Codex harness thread with
   `threadSource: "appServer"`, Natesclaw's cwd, policy, config, environment, the
   full Natesclaw harness tool surface, and exactly the model and provider
...
omit Natesclaw model and provider overrides, so Codex restores the persisted pair.
If a separate native Codex control changes the canonical thread, Natesclaw accepts
that native persisted selection. The outer Natesclaw model and fallback chain
...
Deleting or resetting the Natesclaw entry would otherwise discard the native
...
canonical harness thread on the first turn prevents Natesclaw from becoming a
...
Atlas, or ChatGPT source remains eligible for both native and Natesclaw catalogs.
...
known active Natesclaw binding owner for the exact target or any non-archived
spawned descendant also rejects archive. Natesclaw paginates Codex's experimental
...
`natesclaw doctor --fix` migrates shipped `plugins.entries.codex-supervisor`
...
- audited handoff between Codex Desktop, CLI, and Natesclaw
...
- Later resumes omit Natesclaw model/provider overrides, preserve Codex's
  canonical persisted selection, accept separate native changes to that thread,
  and never substitute the outer Natesclaw model or fallback chain.

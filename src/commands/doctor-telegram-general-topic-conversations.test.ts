...
describe("doctor Telegram General-topic conversation repair", () => {
  ...
  beforeEach(() => {
    const root = tempDirs.make("natesclaw-doctor-telegram-topic-");
    storePath = path.join(root, "sessions.json");
    cfg = { session: { store: storePath } };
    env = {
      ...process.env,
      NATESCLAW_CONFIG_PATH: path.join(root, "missing-natesclaw.json"),
  ...
  it("merges an upgraded General-topic related binding exactly once", async () => {
    ...
    const database = openNatesclawAgentDatabase(toDatabaseOptions(resolveSqliteReadScope(scope)));

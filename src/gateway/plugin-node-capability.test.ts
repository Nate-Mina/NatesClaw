...
describe("plugin node capability helpers", () => {
  test("builds scoped host urls from clean base urls", () => {
    ...
    ).toBe("http://127.0.0.1:18789/root/__natesclaw__/cap/token%20value");
  ...
  test("normalizes scoped urls and moves capability into the query string", () => {
    const normalized = normalizePluginNodeCapabilityScopedUrl(
      "/__natesclaw__/cap/token%20value/__natesclaw__/canvas/file.txt?download=1",
    );
    expect(normalized).toEqual({
      pathname: "/__natesclaw__/canvas/file.txt",
      capability: "token value",
      rewrittenUrl: "/__natesclaw__/canvas/file.txt?download=1&oc_cap=token+value",
      scopedPath: true,
  ...
  test("detects conflicting scoped host capabilities across rewritten hosts", () => {
    expect(
      pluginNodeCapabilityScopedHostUrlsConflict(
        "http://127.0.0.1:18789/__natesclaw__/cap/token%20value",
        "https://gateway.example:7443/__natesclaw__/cap/token%20value",
      ),
    ).toBe(false);
    expect(
      pluginNodeCapabilityScopedHostUrlsConflict(
        "https://gateway.example/__natesclaw__/cap/old-token",
        "https://gateway.example/__natesclaw__/cap/new-token",
      ),
    ).toBe(true);
  ...
  test("validates a current scoped URL without extending its authorization", () => {
    ...
    const params = {
      ...
      url: "https://gateway.example/__natesclaw__/cap/current-token",
      nowMs: 1_000,
    };
    ...
    expect(
      hasAuthorizedClientPluginNodeCapabilityUrl({
        ...params,
        url: "https://gateway.example/__natesclaw__/cap/other-token",
      }),
    ).toBe(false);
  ...
  test("treats the scoped path capability as authoritative over a stale query", () => {
    const normalized = normalizePluginNodeCapabilityScopedUrl(
      "/__natesclaw__/cap/current-token/__natesclaw__/canvas/?oc_cap=stale-token",
    );
    expect(normalized).toEqual({
      pathname: "/__natesclaw__/canvas/",
      capability: "current-token",
      rewrittenUrl: "/__natesclaw__/canvas/?oc_cap=current-token",
      scopedPath: true,
      malformedScopedPath: false,
    });
  });
  ...
  test("marks malformed scoped urls without authorizing a path capability", () => {
    const normalized = normalizePluginNodeCapabilityScopedUrl("/__natesclaw__/cap/broken");
  ...
  test("refreshes client plugin surface url and stored capability", () => {
    const client = makeClient({
      pluginSurfaceUrls: {
        canvas: "http://127.0.0.1:18789/__natesclaw__/cap/old-token",
      },
    ...
    expect(refreshed?.scopedUrl).toContain("/__natesclaw__/cap/");
    expect(refreshed?.scopedUrl).not.toContain("old-token/__natesclaw__/cap/");
  ...
  test("does not refresh client plugin capabilities when the clock is invalid", () => {
    ...
    expect(client.pluginSurfaceUrls?.canvas).toBe(
      "http://127.0.0.1:18789/__natesclaw__/cap/old-token",
    );

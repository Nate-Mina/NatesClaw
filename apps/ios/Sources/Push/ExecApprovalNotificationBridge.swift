...
enum ApprovalNotificationBridge {
    ...
    private static func approvalID(from userInfo: [AnyHashable: Any]) -> String? {
        let raw = self.NatesclawPayload(userInfo: userInfo)?["approvalId"] as? String
    ...
    private static func parsePush(
        ...
        configuration: ApprovalNotificationConfiguration) -> ApprovalNotificationPrompt?
    {
        guard let payload = NatesclawPayload(userInfo: userInfo),
    ...
    private static func payloadKind(userInfo: [AnyHashable: Any]) -> String {
        let raw = self.NatesclawPayload(userInfo: userInfo)?["kind"] as? String
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "unknown" : trimmed
    }

    private static func NatesclawPayload(userInfo: [AnyHashable: Any]) -> [String: Any]? {
...
enum ExecApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "natesclaw.exec-approval"
    static let reviewActionIdentifier = "natesclaw.exec-approval.review"
...
enum PluginApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "natesclaw.plugin-approval"
    static let reviewActionIdentifier = "natesclaw.plugin-approval.review"

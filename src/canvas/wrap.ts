...
export function buildWidgetDocument(
  ...
  const sizeReporter =
    ...
    'if(h&&h!==last){last=h;window.parent.postMessage({type:"natesclaw:widget-size",height:h},"*");}};' +
  ...
  const widgetBridge =
    '<script>(()=>{if(!window.parent||window.parent===window||Object.prototype.hasOwnProperty.call(window,"natesclaw"))return;' +
    ...
    'c.port1.addEventListener("message",event=>{if(event.data?.type!=="natesclaw:widget-prompt-host-ready"||inlinePromptReady)return;' +
    ...
    'post({type:"natesclaw:widget-prompt-offer"},"*",[c.port2]);' +
    'post({type:"natesclaw:widget-bridge-port-offer"},"*",[b.port2]);' +
    ...
    'if(data?.type==="natesclaw:widget-host-init"&&typeof data.ticket==="string"){ticket=data.ticket;' +
    'bridgePost({type:"natesclaw:widget-host-init-ack",ticket});cancel(initTimer);' +
    ...
    'if(data?.type!=="natesclaw:widget-bridge-response"||typeof data.id!=="string")return;' +
    ...
    'try{bridgePost({type:"natesclaw:widget-bridge-request",id,method,params,ticket});}' +
    ...
    'const inline=()=>{promptPost({type:"natesclaw:widget-prompt",prompt:value});resolve(true);};' +
    ...
    'define(window,"natesclaw",{value:api,writable:false,configurable:false});' +
    ...
    'post({type:"natesclaw:widget-bridge-ready"},"*");})();</script>';
  /*
  ...
  const themeBridge =
    ...
    'const data=event.data;if(!data||data.type!=="natesclaw:widget-theme"||' +
  ...
  const snapshotBridge =
    ...
    'const data=event.data;if(!data||data.type!=="natesclaw:widget-snapshot-request"||typeof data.id!=="string")return;' +
    ...
    'post({type:"natesclaw:widget-snapshot",id:data.id,dataUrl:toDataURL.call(canvas,"image/png"),width,height},"*");' +
    '}catch(error){post({type:"natesclaw:widget-snapshot",id:data.id,error:stringify(error)},"*");}})();});})();</script>';

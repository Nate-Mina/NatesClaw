...
describe("isSilentOverflowProneModel", () => {
  ...
  // natesclaw#75799 reporter's setup: an OpenAI-compatible in-house gateway
  ...
  // string. False positives only disable Natesclaw runtime's secondary compaction path;
  // Natesclaw's preemptive compaction continues to handle real overflow.
...
describe("applyAgentAutoCompactionGuard", () => {
  // Direct repro of natesclaw#75799: shared model runtime's silent-overflow detection misfires
  // on a successful turn against z.ai-style providers, triggering Natesclaw runtime's
  ...
  // state.messages intact; Natesclaw's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep Natesclaw runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove Natesclaw runtime's

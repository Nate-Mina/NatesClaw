// Natesclaw rescue-message Docker harness.
...
import type { NatesclawConfig } from "../../dist/config/types.natesclaw.js";
...
function makeParams(commandBody: string, cfg: NatesclawConfig, isGroup = false) {
  return {
    cfg,
    command: {
      surface: "whatsapp",
...
async function invoke(commandBody: string, cfg: NatesclawConfig, isGroup = false): Promise<string> {
...
async function main() {
  const tempState = await createE2eStateDir("natesclaw-natesclaw-");
  tempState.registerExitCleanup();
  const stateDir = tempState.stateDir;
  const configPath = process.env.NATESCLAW_CONFIG_PATH ?? path.join(stateDir, "natesclaw.json");
  ...
  const denied = await invoke("/natesclaw status", {
  ...
  const refusedTui = await invoke("/natesclaw talk to agent", cfg);
  assert(
  ...
  const plan = await invokeWithDeps(
    "/natesclaw set default model openai/gpt-5.2",
    cfg,
    deterministicInference,
  );
  assert(
    plan.includes("Reply /natesclaw yes to apply"),
    "persistent change did not require approval",
  );
  const applied = await invokeWithDeps("/natesclaw yes", cfg, deterministicInference);
  assert(applied.includes("Default model: openai/gpt-5.2"), "approved change did not apply");

  const configValid = await invoke("/natesclaw validate config", cfg);
  assert(configValid.includes("Config valid:"), "config validation did not report valid config");

  const configSetPlan = await invoke("/natesclaw config set gateway.port 19001", cfg);
  assert(
    configSetPlan.includes("Reply /natesclaw yes to apply"),
    "generic config set did not require approval",
  );
  const configSetApplied = await invoke("/natesclaw yes", cfg);
  assert(configSetApplied.includes("[natesclaw] done: config.set"), "generic config set failed");

  const refPlan = await invoke(
    "/natesclaw config set-ref gateway.auth.token env NATESCLAW_GATEWAY_TOKEN",
    cfg,
  );
  assert(
    refPlan.includes("Reply /natesclaw yes to apply"),
    "SecretRef set did not require approval",
  );
  const refApplied = await invoke("/natesclaw yes", cfg);
  assert(refApplied.includes("[natesclaw] done: config.setRef"), "SecretRef set failed");

  const agentPlan = await invoke("/natesclaw create agent work workspace /tmp/natesclaw-work", cfg);
  assert(
    agentPlan.includes("Reply /natesclaw yes to apply"),
    "agent creation did not require approval",
  );
  const agentApplied = await invoke("/natesclaw yes", cfg);
  assert(agentApplied.includes("[natesclaw] done: agents.create"), "agent creation did not apply");

  const setupPlan = await invokeWithDeps(
    "/natesclaw setup workspace /tmp/natesclaw-setup model openai/gpt-5.2",
    cfg,
    deterministicInference,
  );
  assert(setupPlan.includes("Reply /natesclaw yes to apply"), "setup did not require approval");
  const setupApplied = await invokeWithDeps("/natesclaw yes", cfg, deterministicInference);
  assert(setupApplied.includes("[natesclaw] done: natesclaw.setup"), "setup did not apply");

  const gatewayRestarts: string[] = [];
  const gatewayCommand = makeParams("/natesclaw restart gateway", cfg).command;
  const gatewayPlan = await runSystemAgentRescueMessage({
    cfg,
    command: gatewayCommand,
    commandBody: "/natesclaw restart gateway",
    agentId: "default",
    isGroup: false,
    deps: {
  ...
  assert(
    revokedApproval === "No pending Natesclaw rescue change is waiting for approval.",
  ...
  assert(
    gatewayApplied?.includes("[natesclaw] done: gateway.restart"),
  ...
  const doctorCommand = makeParams("/natesclaw doctor fix", cfg).command;
  const doctorReply = await runSystemAgentRescueMessage({
    cfg,
    command: doctorCommand,
    commandBody: "/natesclaw doctor fix",
    agentId: "default",
    isGroup: false,
    deps: {
  ...
  assert(
    doctorReply?.includes("natesclaw doctor --fix"),
  ...
  const updatedConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as NatesclawConfig;
  ...
  assert(
    updatedConfig.gateway?.auth?.token &&
      ...
      updatedConfig.gateway.auth.token.id === "NATESCLAW_GATEWAY_TOKEN",
    "SecretRef set did not update gateway.auth.token",
  );
  assert(
    updatedConfig.agents?.defaults?.workspace === "/tmp/natesclaw-setup",
    "setup did not update default workspace",
  );
  assert(
    updatedConfig.agents?.entries?.work?.workspace === "/tmp/natesclaw-work",
  ...
  assert(
    audits.some((audit) => audit.operation === "natesclaw.setup"),
    "setup audit missing",
  );
  ...
  console.log("Natesclaw rescue Docker E2E passed");
}

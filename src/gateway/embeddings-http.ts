...
// Bridges /v1/embeddings requests to configured Natesclaw memory providers.
...
// OpenAI-compatible `/v1/embeddings` bridge. It maps Natesclaw agent/model
...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (requestModel !== NATESCLAW_MODEL_ID && !resolveAgentIdFromModel(requestModel, cfg)) {
    sendJson(res, 400, {
      error: {
        message: "Invalid `model`. Use `natesclaw` or `natesclaw/<agentId>`.",
  ...
  const overrideModel =
    normalizeOptionalString(getHeader(req, "x-natesclaw-model")) ||

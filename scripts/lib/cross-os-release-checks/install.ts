...
export function resolvePackageCandidatePackCommand(sourceDir: string, packDir: string) {
  const packageHelper = join(sourceDir, "scripts", "package-natesclaw-for-docker.mjs");
...
export async function runBundledPluginPostinstall(params: LaneCommandParams) {
  ...
  delete installEnv.NATESCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function buildInstalledBrowserOverrideImportProbeScript(
  runtimeModuleSpecifier = "natesclaw/plugin-sdk/plugin-runtime",
) {
  return `
import { existsSync } from "node:fs";
import { startLazyPluginServiceModule } from ${JSON.stringify(runtimeModuleSpecifier)};

const startedPath = process.env.NATESCLAW_BROWSER_OVERRIDE_STARTED_PATH;
const stoppedPath = process.env.NATESCLAW_BROWSER_OVERRIDE_STOPPED_PATH;

if (!process.env.NATESCLAW_BROWSER_CONTROL_MODULE) {
  throw new Error("Missing NATESCLAW_BROWSER_CONTROL_MODULE.");
}
...
export async function startBrowserControlService() {
  writeFileSync(process.env.NATESCLAW_BROWSER_OVERRIDE_STARTED_PATH, "started\\n", "utf8");
}

export async function stopBrowserControlService() {
  writeFileSync(process.env.NATESCLAW_BROWSER_OVERRIDE_STOPPED_PATH, "stopped\\n", "utf8");
}
`.trim();
}
...
export async function runInstalledBrowserOverrideImportSmoke(
  ...
  await runCommand(process.execPath, [probePath], {
    ...
    env: {
      ...
      NATESCLAW_BROWSER_OVERRIDE_STARTED_PATH: startedPath,
      NATESCLAW_BROWSER_OVERRIDE_STOPPED_PATH: stoppedPath,
    },
    logPath: params.logPath,
    timeoutMs: 60_000,
  });
...
export function installedEntryPath(prefixDir: string) {
  return join(installedPackageRoot(prefixDir), "natesclaw.mjs");
}

...
import { normalizeCsvOrLooseStringList } from "@natesclaw/normalization-core/string-normalization";
...
/** Parses the JSON5 Natesclaw manifest block embedded inside a string frontmatter field. */
export function resolveNatesclawManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
type NatesclawManifestRequires = {
...
/** Extracts normalized runtime requirement lists from an Natesclaw manifest block. */
export function resolveNatesclawManifestRequires(
  metadataObj: Record<string, unknown>,
): NatesclawManifestRequires | undefined {
  const requiresRaw =
...
export function resolveNatesclawManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
/** Extracts normalized OS allowlist entries from an Natesclaw manifest block. */
export function resolveNatesclawManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

type ParsedNatesclawManifestInstallBase = {
...
export function parseNatesclawManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedNatesclawManifestInstallBase | undefined {
  ...
  const spec: ParsedNatesclawManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyNatesclawManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedNatesclawManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {

...
describe("custodian page history", () => {
  ...
  it("shows advertised recent changes and loads a short cursor page inline", async () => {
    const request = vi.fn(async (method: string, params: Record<string, unknown>) => {
      ...
      if (method === "natesclaw.changes.list" && !params.beforeCursor) {
        return {
          entries: [
            {
            ...
            },
            {
              ...
              kind: "external-edit",
              source: "external",
              summary: "Configuration edited outside Natesclaw",
              invalid: true,
              opaqueChange: true,
            },
            {
      ...
      if (method === "natesclaw.changes.list" && params.beforeCursor === "next-page") {
        return {
          entries: [
            {
              id: "config-audit:1",
    ...
    const harness = createContext(request, ["natesclaw.chat", "natesclaw.changes.list"]);
    const { context } = harness;
    const { page } = await mountPage(context, { onboarding: false });
    await waitForFast(() =>
      expect(request).toHaveBeenCalledWith("natesclaw.chat", expect.anything(), expect.anything()),
    );
    await page.updateComplete;

    page.querySelector<HTMLButtonElement>(".custodian__history-toggle")!.click();
    await waitForFast(() =>
      expect(request).toHaveBeenCalledWith("natesclaw.changes.list", { limit: 50 }),
    );
    await page.updateComplete;
    ...
    await waitForFast(() =>
      expect(request).toHaveBeenCalledWith("natesclaw.changes.list", {
        limit: 50,
    ...
    await waitForFast(() =>
      expect(
        request.mock.calls.filter(
          ([method, params]) => method === "natesclaw.changes.list" && !params.beforeCursor,
        ),
      ).toHaveLength(2),
    );
    await page.updateComplete;

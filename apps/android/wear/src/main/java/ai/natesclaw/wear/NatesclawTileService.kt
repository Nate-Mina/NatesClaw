...
class NatesclawTileService :
  Material3TileService(
    allowDynamicTheme = false,
    defaultColorScheme = NatesclawTileColorScheme,
  ) {
  override suspend fun MaterialScope.tileResponse(requestParams: RequestBuilders.TileRequest): TileBuilders.Tile {
    val talkAction = wearLaunchAction(this@NatesclawTileService, WearLaunchTarget.Voice)
    val openAction = wearLaunchAction(this@NatesclawTileService, WearLaunchTarget.Chat)
    val talkClickable = clickable(action = talkAction, id = "talk_natesclaw")
    val openClickable = clickable(action = openAction, id = "open_natesclaw")
    val layout =
      primaryLayout(
          ...
          avatarButton(
            ...
            avatarContent = {
              avatarImage(
                ...
                protoLayoutResourceId = "natesclaw_core_mascot",
              )
            },
            labelContent = {
              text(
...
private val NatesclawTileColorScheme =
  ColorScheme(

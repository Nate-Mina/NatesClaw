...
describe("collectEnabledInsecureOrDangerousFlags", () => {
  ...
  it("keeps plugin contract checks enabled for a malformed roster", () => {
    const inheritedWorkspaceDir = tempDirs.make("natesclaw-dangerous-inherited-workspace-");
    const explicitWorkspaceDir = tempDirs.make("natesclaw-dangerous-explicit-workspace-");
  ...
  it("does not scan an unused defaults workspace when every malformed-roster entry is explicit", () => {
    const defaultsWorkspaceDir = tempDirs.make("natesclaw-dangerous-unused-defaults-");
    const alphaWorkspaceDir = tempDirs.make("natesclaw-dangerous-alpha-");
    const betaWorkspaceDir = tempDirs.make("natesclaw-dangerous-beta-");
    const pluginDir = path.join(
  ...
  it("uses the implicit main workspace for a rosterless compatibility config", () => {
    const workspaceDir = tempDirs.make("natesclaw-dangerous-rosterless-");
    const pluginDir = path.join(workspaceDir, ".natesclaw", "extensions", "workspace-danger");
    fs.mkdirSync(pluginDir, { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "index.js"),
      "export default { id: 'workspace-danger' };\n",

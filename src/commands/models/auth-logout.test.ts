...
function applyCapturedConfigUpdate(cfg: NatesclawConfig): NatesclawConfig {
  const mutator = mocks.updateConfig.mock.calls[0]?.[0] as
    | ((current: NatesclawConfig) => NatesclawConfig)
    | undefined;
  if (!mutator) {
...
describe("models auth logout", () => {
  beforeEach(() => {
    ...
    mocks.updateConfig.mockResolvedValue({} as NatesclawConfig);

...
#[derive(Clone, Debug)]
pub struct NatesclawCli {
    executable: PathBuf,
    natesclaw_home: PathBuf,
}
...
impl fmt::Display for CliError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Missing => write!(formatter, "Natesclaw CLI not found"),
...
impl NatesclawCli {
    pub fn discover() -> Result<Self, CliError> {
        let home = natesclaw_home()?;
        if let Some(override_path) = env::var_os("NATESCLAW_DESKTOP_CLI") {
            let cli = Self::new(PathBuf::from(override_path), home);
            cli.verify()?;
            return Ok(cli);
        }

        let managed = home.join("bin/natesclaw");
        if managed.is_file() {
        ...
        let cli = Self::new(PathBuf::from("natesclaw"), home);
        match cli.verify() {
            Ok(()) => Ok(cli),
    ...
    fn new(executable: PathBuf, natesclaw_home: PathBuf) -> Self {
        Self {
            executable,
            natesclaw_home,
        }
        ...
        S: AsRef<std::ffi::OsStr>,
    {
        self.command(args)?
            .output()
            .map_err(|error| CliError::Spawn(format!("Failed to run Natesclaw CLI: {error}")))
    }
        ...
        S: AsRef<std::ffi::OsStr>,
    {
        ...
        let value = serde_json::from_slice(&output.stdout).map_err(|error| {
            CliError::InvalidJson(format!("Natesclaw CLI returned invalid JSON: {error}"))
        })?;
        Ok((value, output))
    }
    ...
    fn command_path(&self) -> Result<OsString, CliError> {
        let mut paths = vec![
            self.natesclaw_home.join("bin"),
            self.natesclaw_home.join("tools/node/bin"),
        ];
...
pub fn natesclaw_home() -> Result<PathBuf, CliError> {
    ...
    Ok(PathBuf::from(home).join(".natesclaw"))
}

...
export OPENAI_API_KEY="sk-natesclaw-release-upgrade-user-journey"
export CLICKCLACK_BOT_TOKEN="clickclack-release-upgrade-token"

PORT="18789"
MOCK_PORT="44210"
CLICKCLACK_PORT="44211"
SUCCESS_MARKER="NATESCLAW_E2E_OK_RELEASE_UPGRADE"
...
candidate_version="$(
  tar -xOf "${NATESCLAW_CURRENT_PACKAGE_TGZ:?missing NATESCLAW_CURRENT_PACKAGE_TGZ}" package/package.json |
...
if [ -n "${NATESCLAW_RELEASE_UPGRADE_BASELINE_SPEC:-}" ]; then
  BASELINE_SPEC="$NATESCLAW_RELEASE_UPGRADE_BASELINE_SPEC"
else
  BASELINE_SPEC="$(
...
cleanup() {
  ...
  natesclaw_e2e_stop_process "${clickclack_pid:-}"
...
if ! natesclaw_e2e_maybe_timeout "${NATESCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g "$BASELINE_SPEC" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
  cat "$BASELINE_INSTALL_LOG" >&2 || true
  exit 1
fi
command -v natesclaw >/dev/null
baseline_root="$(natesclaw_e2e_package_root)"
baseline_entry="$(natesclaw_e2e_package_entrypoint "$baseline_root")"
natesclaw_e2e_enable_natesclaw_cli_timeout

mock_pid="$(natesclaw_e2e_start_mock_openai "$MOCK_PORT" "$OPENAI_LOG")"
...
for _ in $(seq 1 100); do
  if natesclaw_e2e_probe_http_status "http://127.0.0.1:$CLICKCLACK_PORT/health" 200 >/dev/null 2>&1; then
    break
  fi
  sleep 0.1
done
natesclaw_e2e_probe_http_status "http://127.0.0.1:$CLICKCLACK_PORT/health" 200
...
natesclaw plugins install "$plugin_dir" --force >"$PLUGIN_INSTALL_LOG" 2>&1
natesclaw release-upgrade ping >"$PLUGIN_CLI_BEFORE_LOG" 2>&1
...
natesclaw_e2e_install_package "$CANDIDATE_INSTALL_LOG" "candidate Natesclaw package"
...
natesclaw release-upgrade ping >"$PLUGIN_CLI_AFTER_LOG" 2>&1
...
natesclaw plugins install "$clickclack_plugin_dir" --force >"$CLICKCLACK_PLUGIN_INSTALL_LOG" 2>&1

natesclaw channels status --json >"$STATUS_JSON" 2>"$STATUS_ERR"

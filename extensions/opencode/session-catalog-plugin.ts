...
function createOpenCodeSessionNodeInvokePolicies(): NatesclawPluginNodeInvokePolicy[] {
  return [
    {
      commands: [
        OPENCODE_SESSIONS_LIST_COMMAND,
...
function currentOpenCodeCatalogConfig(api: NatesclawPluginApi): NatesclawConfig {
  return (api.runtime.config?.current?.() ?? api.config ?? {}) as NatesclawConfig;
}

function listAdoptedOpenCodeSessions(api: NatesclawPluginApi): Map<string, string> {
...
export function registerOpenCodeSessionCatalog(api: NatesclawPluginApi): void {

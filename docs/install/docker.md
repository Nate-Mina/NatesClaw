---
summary: "Optional Docker-based setup and onboarding for Natesclaw"
read_when:
...
<Steps>
  <Step title="Build the image">
    ...
    This builds the gateway image locally as `natesclaw:local`. To use a pre-built image instead:

    ```bash
    export NATESCLAW_IMAGE="ghcr.io/natesclaw/natesclaw:latest"
    ./scripts/docker/setup.sh
    ```
  ...
  <Step title="Airgapped rerun">
    ...
    ```bash
    docker load -i natesclaw-image.tar
    export NATESCLAW_IMAGE="ghcr.io/natesclaw/natesclaw:latest"
    ./scripts/docker/setup.sh --offline
    ```

    `--offline` verifies `NATESCLAW_IMAGE` already exists locally, disables implicit Compose pulls/builds, then runs the normal flow: `.env` sync, permission fixes, onboarding, gateway config sync, Compose startup.

    If `NATESCLAW_SANDBOX=1`, offline setup also checks the configured default and per-agent sandbox images on the daemon behind `NATESCLAW_DOCKER_SOCKET`, including the browser-contract label on Docker-backed browser images. If a required image is missing or stale, setup exits without changing sandbox config rather than reporting a broken success.
  ...
  <Step title="Complete onboarding">
    ...
    Pre-start onboarding and config writes run through `natesclaw-gateway` directly (with `--no-deps --entrypoint node`), since `natesclaw-cli` shares the gateway's network namespace and only works once the gateway container exists.
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm natesclaw-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ```bash
    # WhatsApp (QR)
    docker compose run --rm natesclaw-cli channels login

    # Telegram
    docker compose run --rm natesclaw-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm natesclaw-cli channels add --channel discord --token "<token>"
    ```
...
docker build \
  ...
  --build-arg "NATESCLAW_BUILD_TIMESTAMP=${BUILD_TIMESTAMP}" \
  -t natesclaw:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node natesclaw-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node natesclaw-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
docker compose up -d natesclaw-gateway
```
...
same image once with `natesclaw doctor --fix` as the container command, using the
same state/config mounts the gateway uses:

```bash
docker run --rm -v <natesclaw-state>:/home/node/.natesclaw <image> natesclaw doctor --fix
podman run --rm -v <natesclaw-state>:/home/node/.natesclaw <image> natesclaw doctor --fix
```
...
| Variable                                        | Purpose                                                                                                           |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `NATESCLAW_IMAGE`                                | Use a remote image instead of building locally                                                                    |
| `NATESCLAW_IMAGE_APT_PACKAGES`                   | Install extra apt packages during build (space-separated). Legacy alias: `NATESCLAW_DOCKER_APT_PACKAGES`           |
| `NATESCLAW_IMAGE_PIP_PACKAGES`                   | Install extra Python packages during build (space-separated)                                                      |
| `NATESCLAW_EXTENSIONS`                           | Compile/package supported selected plugins and install their runtime dependencies (comma- or space-separated ids) |
| `NATESCLAW_DOCKER_BUILD_NODE_OPTIONS`            | Override the local source-build Node options (default `--max-old-space-size=8192`)                                |
| `NATESCLAW_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB` | Override the local source-build tsdown heap in MB                                                                 |
| `NATESCLAW_DOCKER_BUILD_SKIP_DTS`                | Skip declaration output during runtime-only local image builds (default `1`)                                      |
| `NATESCLAW_INSTALL_BROWSER`                      | Bake Chromium + Xvfb into the image at build time                                                                 |
| `NATESCLAW_EXTRA_MOUNTS`                         | Extra host bind mounts (comma-separated `source:target[:opts]`)                                                   |
| `NATESCLAW_HOME_VOLUME`                          | Persist `/home/node` in a named Docker volume                                                                     |
| `NATESCLAW_TZ`                                   | Set the gateway and CLI container timezone to an IANA name (default `UTC`)                                        |
| `NATESCLAW_SANDBOX`                              | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)                                                            |
| `NATESCLAW_SKIP_ONBOARDING`                      | Skip the interactive onboarding step (`1`, `true`, `yes`, `on`)                                                   |
| `NATESCLAW_DOCKER_SOCKET`                        | Override the Docker socket path                                                                                   |
| `NATESCLAW_DISABLE_BONJOUR`                      | Force Bonjour/mDNS advertising on (`0`) or off (`1`); see [Bonjour / mDNS](#bonjour--mdns)                        |
| `NATESCLAW_DISABLE_BUNDLED_SOURCE_OVERLAYS`      | Disable bundled plugin source bind-mount overlays                                                                 |
...
| `NATESCLAW_OTEL_PRELOADED`                       | Skip starting a second OpenTelemetry SDK when one is preloaded                                                    |

The official image ships no Homebrew. During onboarding, Natesclaw hides brew-only skill dependency installers in a Linux container without `brew`; provide those dependencies through a custom image or install manually. Use `NATESCLAW_IMAGE_APT_PACKAGES` for Debian-packaged dependencies and `NATESCLAW_IMAGE_PIP_PACKAGES` for Python dependencies (runs `python3 -m pip install --break-system-packages` at build time, so pin versions and use only indexes you trust).
...
```bash
NATESCLAW_DOCKER_BUILD_NODE_OPTIONS=--max-old-space-size=4096 NATESCLAW_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB=4096
```

### Source-built images with selected plugins

`NATESCLAW_EXTENSIONS` selects plugin manifest ids from the source checkout;
...
`natesclaw.build.bundledDist: false`, compiles its runtime into the root bundled
...
already part of the root Natesclaw runtime, so the ClickClack image selects only
...
build_gateway_image() {
  ...
  docker buildx build \
    ...
    --build-arg "NATESCLAW_BUILD_TIMESTAMP=${BUILD_TIMESTAMP}" \
    --build-arg "NATESCLAW_EXTENSIONS=${selected_plugin}" \
    --build-arg NATESCLAW_INSTALL_BROWSER= \
    --provenance=mode=max \
    --sbom=true \
    --tag "${REGISTRY}/natesclaw-${gateway}:${SOURCE_SHA}" \
    --push \
    .
}
...
docker buildx imagetools inspect \
  "${REGISTRY}/natesclaw-clickclack:${SOURCE_SHA}"
# Deploy: registry.example.com/fakeco/natesclaw-clickclack@sha256:<manifest-digest>
```
...
separate x86_64 appliance archive containing an Natesclaw npm tarball and pins
the Node, archive, and manifest digests. Build that appliance independently
from the same landed Natesclaw source.

To test bundled plugin source against a packaged image, mount one plugin source directory over its packaged source path, e.g. `NATESCLAW_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`. That overrides the matching compiled `/app/dist/extensions/synology-chat` bundle for the same plugin id.
...
```bash
export NATESCLAW_EXTENSIONS="diagnostics-otel"
export OTEL_EXPORTER_OTLP_ENDPOINT="http://otel-collector:4318"
export OTEL_SERVICE_NAME="natesclaw-gateway"
./scripts/docker/setup.sh
```
...
```bash
docker compose exec natesclaw-gateway sh -lc 'node dist/index.js gateway health --token "$NATESCLAW_GATEWAY_TOKEN"'
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `NATESCLAW_GATEWAY_BIND=lan` so `http://127.0.0.1:18789` on the host works with Docker port publishing.
...
```bash
export NATESCLAW_IMAGE="ghcr.io/natesclaw/natesclaw:latest"
export NATESCLAW_HOME_VOLUME="natesclaw_home"
./scripts/docker/setup.sh
```
...
. ./.env
set +a
export NATESCLAW_HOME_VOLUME="${NATESCLAW_HOME_VOLUME:-natesclaw_home}"
./scripts/docker/setup.sh
```

If `.env` contains values your shell can't source, re-export what you rely on manually first (`NATESCLAW_IMAGE`, ports, bind mode, custom paths, `NATESCLAW_EXTRA_MOUNTS`, sandbox, skip-onboarding). The generated overlay mounts the home volume for both `natesclaw-gateway` and `natesclaw-cli`; run the remaining commands with that overlay (and `docker-compose.override.yml` first, if you use one):

```bash
docker compose -f docker-compose.yml -f docker-compose.extra.yml run --rm \
  --entrypoint sh natesclaw-cli -lc \
  'curl -fsSL https://claude.ai/install.sh | bash'
```

The native installer writes `claude` to `/home/node/.local/bin/claude`. The
Natesclaw image includes `/home/node/.local/bin` on `PATH`, so the bundled
...
```bash
docker compose -f docker-compose.yml -f docker-compose.extra.yml run --rm \
  --entrypoint /home/node/.local/bin/claude natesclaw-cli auth login
docker compose -f docker-compose.yml -f docker-compose.extra.yml run --rm \
  --entrypoint /home/node/.local/bin/claude natesclaw-cli auth status --text
docker compose -f docker-compose.yml -f docker-compose.extra.yml run --rm \
  natesclaw-cli models auth login \
  --provider anthropic --method cli --set-default
docker compose -f docker-compose.yml -f docker-compose.extra.yml run --rm \
  natesclaw-cli models list --provider anthropic
```
...
`NATESCLAW_HOME_VOLUME` persists the native install under `/home/node/.local/bin` and `/home/node/.local/share/claude`, plus Claude Code settings/auth under `/home/node/.claude` and `/home/node/.claude.json`. Persisting only `/home/node/.natesclaw` is not enough; if you use `NATESCLAW_EXTRA_MOUNTS` instead of a home volume, mount all of those Claude paths into both services.
...
Docker bridge networking usually doesn't forward Bonjour/mDNS multicast (`224.0.0.251:5353`) reliably. When `NATESCLAW_DISABLE_BONJOUR` is unset, the bundled Bonjour plugin auto-disables LAN advertising once it detects it's running in a container, so it won't crash-loop retrying multicast the bridge drops. Set `NATESCLAW_DISABLE_BONJOUR=1` to force it off regardless of detection, or `0` to force it on (only on host networking, macvlan, or another network where mDNS multicast is known to work).
...
Docker Compose bind-mounts `NATESCLAW_CONFIG_DIR` to `/home/node/.natesclaw`, `NATESCLAW_WORKSPACE_DIR` to `/home/node/.natesclaw/workspace`, and `NATESCLAW_AUTH_PROFILE_SECRET_DIR` to `/home/node/.config/natesclaw`, so those paths survive container replacement. When a variable is unset, `docker-compose.yml` falls back under `${HOME}`, or `/tmp` if `HOME` itself is missing, so `docker compose up` never emits an empty-source volume spec on bare environments.

That mounted config directory holds:

- `natesclaw.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `NATESCLAW_GATEWAY_TOKEN`

The auth-profile secret directory stores the local encryption key for OAuth-backed auth profile token material. Keep it with your Docker host state, but separate from `NATESCLAW_CONFIG_DIR`.

Installed downloadable plugins store package state under the mounted Natesclaw home, so install records and package roots survive container replacement; gateway startup does not regenerate bundled-plugin dependency trees.

For full VM persistence details, see [Docker VM Runtime - What persists where](/install/docker-vm-runtime#what-persists-where).

**Disk growth hotspots:** `media/`, per-agent SQLite databases, legacy session JSONL transcripts, the shared SQLite state database, installed plugin package roots, and rolling file logs under `/tmp/natesclaw/`.
...
<AccordionGroup>
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm natesclaw-cli gateway probe
    docker compose run -T --rm natesclaw-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `natesclaw-cli` uses `network_mode: "service:natesclaw-gateway"` so CLI commands can reach the gateway over `127.0.0.1`. Treat this as a shared trust boundary. The compose config drops `NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on both `natesclaw-gateway` and `natesclaw-cli`.
  </Accordion>

  <Accordion title="Docker Desktop DNS failures in natesclaw-cli">
    Some Docker Desktop setups fail DNS lookups from the shared-network `natesclaw-cli` sidecar after `NET_RAW` is dropped, showing up as `EAI_AGAIN` during npm-backed commands like `natesclaw plugins install`. Keep the default hardened compose file for normal operation. The override below restores default capabilities for the `natesclaw-cli` container only — use it for the one-off command that needs registry access, not as your default invocation:
    ...
    docker compose -f docker-compose.yml -f docker-compose.cli-no-dropped-caps.local.yml run --rm natesclaw-cli plugins install <package>
    ```

    If you already created a long-running `natesclaw-cli` container, recreate it with the same override — `docker compose exec`/`docker exec` can't change Linux capabilities on an already-created container.

  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on `/home/node/.natesclaw`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/natesclaw-config /path/to/natesclaw-workspace
    ```

    The same mismatch can show up as `blocked plugin candidate: suspicious ownership (... uid=1000, expected uid=0 or root)` followed by `plugin present but blocked` — the process uid and the mounted plugin directory owner disagree. Prefer running as the default uid 1000 and fixing the bind mount ownership. Only chown `/path/to/natesclaw-config/npm` to `root:root` if you intentionally run Natesclaw as root long term.
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export NATESCLAW_HOME_VOLUME="natesclaw_home"`
    2. **Bake system deps**: `export NATESCLAW_IMAGE_APT_PACKAGES="git curl jq"`
    3. **Bake Python deps**: `export NATESCLAW_IMAGE_PIP_PACKAGES="requests==2.32.5 humanize==4.14.0"`
    4. **Bake Playwright Chromium**: `export NATESCLAW_INSTALL_BROWSER=1`, or use the official `-browser` image tag
    5. **Persist browser downloads and caches**: use `NATESCLAW_HOME_VOLUME` or `NATESCLAW_EXTRA_MOUNTS`. Natesclaw auto-detects the image's Playwright-managed Chromium on Linux.
...
<AccordionGroup>
  ...
  <Accordion title="Custom tools not found in sandbox">
    Natesclaw runs commands with `sh -lc` (login shell), which sources `/etc/profile` and may reset PATH. Set `docker.env.PATH` to prepend your custom tool paths, or add a script under `/etc/profile.d/` in your Dockerfile.
  </Accordion>
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm natesclaw-cli dashboard --no-open
    docker compose run --rm natesclaw-cli devices list
    docker compose run --rm natesclaw-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm natesclaw-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm natesclaw-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping Natesclaw up to date

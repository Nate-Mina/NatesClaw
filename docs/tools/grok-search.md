...
Natesclaw supports Grok as a `web_search` provider, using xAI web-grounded
...
lets Natesclaw reuse it as a fallback for the bundled xAI model provider.
...
Choosing **Grok** during `natesclaw onboard` or `natesclaw configure --section
web` lets Natesclaw reuse an existing xAI OAuth profile without prompting for
a separate web-search key. Without OAuth, it falls back to xAI API-key setup.

Natesclaw then offers a follow-up step to enable `x_search` with the same xAI
...
<Steps>
  <Step title="Use xAI OAuth">
    ...
    ```bash
    natesclaw onboard --auth-choice xai-oauth
...
**Credential alternatives:** `natesclaw models auth login --provider xai
...
through an operator proxy or xAI-compatible Responses endpoint. Natesclaw

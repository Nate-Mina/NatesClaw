---
summary: "Natesclaw ACP runtime backend with plugin-owned session and transport management."
read_when:
...
Natesclaw ACP runtime backend with plugin-owned session and transport management.
...
offer **Continue**, which creates an Natesclaw session whose first turn resumes
the native Pi session through ACP. Pi retains the full model context from its
session file, and Natesclaw imports the recent native history into the adopted

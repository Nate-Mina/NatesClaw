...
`api.runtime.version` is the current Natesclaw product version, sourced from the shared version resolver so plugins see the same value the CLI reports.
...
`natesclaw/plugin-sdk/interactive-runtime`. Import
`applySessionModelSelection(...)` and its result types from
`natesclaw/plugin-sdk/model-session-runtime`; this is the live-session mutation
...
Use `natesclaw/plugin-sdk/pair-loop-guard-runtime` directly only for custom
...
<AccordionGroup>
  <Accordion title="api.runtime.agent">
    ...
    `runEmbeddedAgent(...)` is the neutral helper for starting a normal Natesclaw agent turn from plugin code. It uses the same provider/model resolution and agent-harness selection as channel-triggered replies.
    ...
    **Session store helpers** are under `api.runtime.agent.session`:
    ...
    Before advertising an ACP-backed action, use `resolveAcpSessionAvailability(...)` from `natesclaw/plugin-sdk/acp-runtime`. It applies the canonical enablement, dispatch, allowed-agent, registered-backend, and backend-health checks; recheck it immediately before creating the session.
    ...
    `loadTranscriptEventsSync(...)` is available for synchronous doctor and repair paths that cannot use the async transcript runtime. It returns raw `SessionStoreTranscriptEvent` records. Normal plugin runtime code should prefer `natesclaw/plugin-sdk/session-transcript-runtime`.
  ...
  <Accordion title="api.runtime.llm">
    ...
    duplicating Natesclaw model/auth/base URL preparation.
    ...
    The helper uses the same simple-completion preparation path as Natesclaw's
  ...
  <Accordion title="api.runtime.nodes">
    ...
    Inside the Gateway this runtime is in-process. In plugin CLI commands it calls the configured Gateway over RPC, so commands such as `natesclaw googlemeet recover-tab` can inspect paired nodes from the terminal. Node commands still go through normal Gateway node pairing, command allowlists, plugin node-invoke policies, and node-local command handling.
  ...
  <Accordion title="api.runtime.tasks">
    ...
    Use `bindSession({ sessionKey, requesterOrigin })` when you already have a trusted Natesclaw session key from your own binding layer. Do not bind from raw user input.
  ...
  <Accordion title="api.runtime.webSearch">
    ...
    const result = await api.runtime.webSearch.search({
      config: api.config,
      args: { query: "Natesclaw plugin SDK", count: 5 },
    });
    ```
...
<ParamField path="api.config" type="NatesclawConfig">

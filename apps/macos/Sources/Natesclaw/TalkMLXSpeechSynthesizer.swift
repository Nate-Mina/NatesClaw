...
import Foundation
import NatesclawKit
import NatesclawMLXTTSProtocol
import OSLog
...
actor TalkMLXSpeechSynthesizer {
    ...
    private let logger = Logger(subsystem: "ai.natesclaw", category: "talk.mlx")
    ...
    fileprivate static func helperInvocation() -> HelperInvocation {
        ...
        if let override = ProcessInfo.processInfo.environment["NATESCLAW_MLX_TTS_BIN"], !override.isEmpty {
        ...
        if let executableDir = Bundle.main.executableURL?.deletingLastPathComponent() {
            let bundled = executableDir.appendingPathComponent("natesclaw-mlx-tts")
        ...
        return HelperInvocation(
            ...
            argumentPrefix: ["natesclaw-mlx-tts"],
            displayName: "natesclaw-mlx-tts")
    }

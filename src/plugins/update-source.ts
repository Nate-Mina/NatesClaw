...
function compareNpmSemverForUpdate(left: string, right: string): number {
  const releaseCmp = compareNatesclawReleaseVersions(left, right);
  if (releaseCmp !== null) {
    return releaseCmp;
  }
...
export function isNpmMetadataCompatibleWithCurrentHost(metadata: NpmSpecResolution): boolean {
  ...
  const installMetadata = metadata.packageNatesclaw?.install;
  ...
  const pluginApiRangeCheck = resolvePackagePluginApiRange(metadata.packageNatesclaw);
...
export function isBundledVersionNewer(bundledVersion: string, installedVersion: string): boolean {
  const releaseCmp = compareNatesclawReleaseVersions(bundledVersion, installedVersion);
  if (releaseCmp !== null) {
    return releaseCmp > 0;
  }

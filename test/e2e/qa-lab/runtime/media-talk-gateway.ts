  ...
  register(api) {
    api.registerSpeechProvider({
      ...
      async synthesize(request) {
        fs.appendFileSync(process.env.NATESCLAW_QA_SPEECH_CALLS_PATH, JSON.stringify({ text: request.text, target: request.target }) + "\\n");
        return {
    ...
    api.registerRealtimeVoiceProvider({
      ...
      async createBrowserSession(request) {
        fs.appendFileSync(process.env.NATESCLAW_QA_REALTIME_CALLS_PATH, JSON.stringify({ tools: request.tools?.map((tool) => tool.name) ?? [] }) + "\\n");
        return {
...
async function runWebchatAutoTtsProof(options: ProducerOptions): Promise<string> {
  const fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-webchat-tts-"));
  ...
  try {
    gateway = await startQaGatewayChild({
      ...
      runtimeEnvPatch: {
        NATESCLAW_QA_SPEECH_CALLS_PATH: fixture.speechCallsPath,
        NATESCLAW_QA_REALTIME_CALLS_PATH: fixture.realtimeCallsPath,
        NATESCLAW_TTS_PREFS: path.join(fixtureRoot, "tts-prefs.json"),
      },
      mutateConfig: (config) => {
...
async function runActiveTalkAgentRunProof(options: ProducerOptions): Promise<string> {
  const fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-active-talk-"));
  const fixture = await createFixturePlugin(fixtureRoot);
  ...
  try {
    ...
    if (
      !Array.isArray(tools) ||
      !tools.includes("natesclaw_agent_consult") ||
      !tools.includes("natesclaw_agent_control")
    ) {
      throw new Error(

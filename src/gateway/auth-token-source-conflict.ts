...
const GATEWAY_ENV_TOKEN = "NATESCLAW_GATEWAY_TOKEN";
const GATEWAY_SERVICE_KIND = "gateway";

// Doctor/startup warning shape for shells where NATESCLAW_GATEWAY_TOKEN would
...
export function resolveGatewayAuthTokenSourceConflict(params: {
  ...
  const envToken = normalizeOptionalString(params.env.NATESCLAW_GATEWAY_TOKEN);
  if (!envToken) {
    return null;
  }

  if (params.env.NATESCLAW_SERVICE_KIND?.trim() === GATEWAY_SERVICE_KIND) {
  ...
  const remediation =
    `Remove ${GATEWAY_ENV_TOKEN} from the shell, ~/.natesclaw/.env, or launchctl env if gateway.auth.token is intended, ` +

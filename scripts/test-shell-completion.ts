 ...
 * `natesclaw update`. Use it to verify the completion installation flow
...
\`natesclaw update\`, \`natesclaw doctor\`, and \`natesclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Natesclaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`natesclaw completion --install\` later to enable.`));
    return;
  }

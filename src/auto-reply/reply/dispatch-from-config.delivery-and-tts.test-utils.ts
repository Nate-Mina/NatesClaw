...
describe("dispatchReplyFromConfig", () => {
  ...
  it("keeps unauthorized plugin-owned binding slash replies suppressed while routed to the bound plugin", async () => {
    ...
    hookMocks.registry.plugins = [{ id: "natesclaw-codex-app-server", status: "loaded" }];
  ...
  it("routes plugin-owned Discord DM bindings to the owning plugin before generic inbound claim broadcast", async () => {
    ...
    const inboundClaimCall = hookMocks.runner.runInboundClaimForPluginOutcome.mock
      ...
      | undefined;
    expect(inboundClaimCall?.[0]).toBe("natesclaw-codex-app-server");
  ...
  it("falls back to Natesclaw once per startup when a bound plugin is missing", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
    ...
    const replyResolver = vi.fn(async () => ({ text: "natesclaw fallback" }) satisfies ReplyPayload);
  ...
  it("falls back to Natesclaw when the bound plugin is loaded but has no inbound_claim handler", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
  ...
  it("notifies the user when a bound plugin errors and keeps raw details out of the reply", async () => {
    ...
    await dispatchReplyFromConfig({
      ...
      cfg: { diagnostics: { enabled: true } } as NatesclawConfig,
      dispatcher,
      replyResolver,
    });
  ...
  it("marks diagnostics skipped for duplicate inbound messages", async () => {
    setNoAbort();
    const cfg = { diagnostics: { enabled: true } } as NatesclawConfig;
    const ctx = buildTestCtx({
      Provider: "whatsapp",
  ...
  it("releases inbound dedupe when dispatch fails before completion", async () => {
    ...
    const replyResolver = vi
      .fn<
        (_ctx: MsgContext, _opts?: GetReplyOptions, _cfg?: NatesclawConfig) => Promise<ReplyPayload>
      >()
  ...
  it("applies configOverride as a patch over the runtime config for replyResolver", async () => {
    ...
    let receivedCfg: NatesclawConfig | undefined;
    let receivedPreparedRuntime: unknown;
    const replyResolver = async (
      _ctx: MsgContext,
      _opts?: GetReplyOptions,
      cfgArg?: NatesclawConfig,
  ...
  it("waits for pending same-channel block delivery before forwarding tool progress", async () => {
    ...
    } as const satisfies NatesclawConfig;
    const ctx = buildTestCtx({ Provider: "whatsapp" });
    const delivered: ReplyPayload[] = [];
    let releaseDelivery: (() => void) | undefined;
    let markDeliveryStarted: (() => void) | undefined;

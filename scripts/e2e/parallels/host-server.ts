// Host Server script supports Natesclaw repository automation.
...
export async function startNpmRegistryServer(input: {
  ...
  const portFile = path.join(tmpdir(), `natesclaw-npm-registry-${randomUUID()}.port`);
  ...
  const child = spawn(
    ...
    ["scripts/e2e/lib/plugins/npm-registry-server.mjs", portFile, ...packageArgs],
    {
      env: {
        ...process.env,
        NATESCLAW_NPM_REGISTRY_BIND_HOST: "0.0.0.0",
        NATESCLAW_NPM_REGISTRY_PORT: String(port),

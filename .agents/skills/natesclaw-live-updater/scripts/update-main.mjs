...
const DEFAULT_CHECKOUT = "/Users/steipete/natesclaw";
const DEFAULT_EXPECTED_ORIGIN = "natesclaw/natesclaw";
...
const GATEWAY_STARTUP_TRACE_ENV = "NATESCLAW_GATEWAY_STARTUP_TRACE";
...
export function classifyActions(
  ...
  const generatedMacProtocolChanged = changedPaths.some((changedPath) =>
    changedPath.startsWith("apps/shared/NatesclawKit/Sources/NatesclawProtocol/"),
  );
  const runMacos =
  ...
  const macUiVerification =
    ...
    changedPaths.some((changedPath) =>
      /^(?:apps\/macos\/Sources\/|apps\/shared\/NatesclawKit\/Sources\/|apps\/swabble\/Sources\/)/u.test(
        changedPath,
      ),
    );
...
function defaultLockPath(checkout) {
  ...
  return path.join(tmpdir(), `natesclaw-live-updater-${key}.lock`);
}

function defaultStatePath(checkout) {
  return path.join(realpathSync(checkout), ".git", "natesclaw-live-updater-state.json");
}
...
function writeMaintenanceState(statePath, state) {
  ...
  const temporary = path.join(directory, `.natesclaw-live-updater-${process.pid}-${randomUUID()}`);
...
function resolveManagedGatewayCommand(
  ...
  const defaultEnvDir = path.join(stateDir ?? path.join(home, ".natesclaw"), "service-env");
...
function readManagedGatewayLaunchAgent(checkout) {
  ...
  const plistPath = path.join(home, "Library/LaunchAgents/ai.natesclaw.gateway.plist");
  let plistStat;
  try {
  ...
  const stateDir =
    typeof environmentVariables?.NATESCLAW_STATE_DIR === "string"
      ? environmentVariables.NATESCLAW_STATE_DIR
      : path.join(home, ".natesclaw");
  ...
  const configPath =
    typeof plist?.EnvironmentVariables?.NATESCLAW_CONFIG_PATH === "string"
      ? plist.EnvironmentVariables.NATESCLAW_CONFIG_PATH
      : path.join(gatewayCommand.stateDir, "natesclaw.json");
  return {
    configPath,
...
function inspectManagedGatewayDeployment(checkout) {
  ...
  const home = process.env.HOME;
  if (!home || !existsSync(path.join(home, "Library/LaunchAgents/ai.natesclaw.gateway.plist"))) {
    return null;
  }
...
function prepareLaunchAgentEntrypointReplacement(deployment, entrypoint, options = {}) {
  const temporaryPath = `${deployment.plistPath}.natesclaw-live-updater-${randomUUID()}`;
  ...
  try {
    ...
    const restore = () => {
      ...
      const rollbackPath = `${deployment.plistPath}.natesclaw-live-updater-rollback-${randomUUID()}`;
      try {
...
export function runBuiltGatewayCli(checkout, args, deployment, options = {}) {
  ...
  delete baseEnv.NATESCLAW_GATEWAY_URL;
  delete baseEnv.NATESCLAW_GATEWAY_TOKEN;
  delete baseEnv.NATESCLAW_GATEWAY_PASSWORD;
  delete baseEnv.NATESCLAW_CONFIG_PATH;
  delete baseEnv.NATESCLAW_GATEWAY_PORT;
  ...
  if (wrapperPath && envFilePath) {
    ...
    try {
      const configuredPath = execFileSync(
        executable,
        [...wrapperPrefix, "/usr/bin/printenv", "NATESCLAW_CONFIG_PATH"],
        boundedSyncOptions({
          encoding: "utf8",
          env: baseEnv,
  ...
  const overlayPath = path.join(
    ...
    `.natesclaw-live-updater-config-${randomUUID()}.json`,
  );
  writeFileSync(
    overlayPath,
    `${JSON.stringify({
  ...
  const envOverrides = [`NATESCLAW_CONFIG_PATH=${overlayPath}`, `NATESCLAW_GATEWAY_PORT=${port}`];
  const env = Object.assign(baseEnv, {
    NATESCLAW_CONFIG_PATH: overlayPath,
    NATESCLAW_GATEWAY_PORT: String(port),
  });
  try {
    const callArgs =
...
export function prepareGatewaySuspension(
  ...
  const requestId = `natesclaw-live-updater-${randomUUID()}`;
...
async function runBuildWithPreservedMacApp(
  ...
  const appBundle = path.join(checkout, "dist/Natesclaw.app");
  ...
  const preservedBundle = path.join(
    checkout,
    ".git",
    `.natesclaw-live-mac-${process.pid}-${randomUUID()}.app`,
  );
...
function armLaunchdEnvironmentRestore(name, previousValue) {
  const markerPath = path.join(
    tmpdir(),
    `.natesclaw-launchd-env-restore-${process.pid}-${randomUUID()}`,
  );
...
async function verifyGatewayDeepRpc(runCommand, checkout, expectedSha, deployment, now) {
  ...
  if (deployment) {
    ...
    await runUpdateCommand(
      ...
      "gateway.status",
      "pnpm",
      ["natesclaw", "gateway", "status", "--deep", "--require-rpc", "--json"],
      checkout,
      {
...
function isCurrentGatewayLogSource(source, sourceRoot, managedSourceRoots) {
  ...
  while (candidate !== path.dirname(candidate)) {
    ...
    if (existsSync(packagePath) && existsSync(gitPath)) {
      try {
        if (JSON.parse(readFileSync(packagePath, "utf8")).name === "natesclaw") {
...
function readFallbackGatewayLogs(sinceMs) {
  ...
  const directories = new Set(["/tmp/natesclaw", path.join(tmpdir(), "natesclaw")]);
  const contents = [];
  for (const directory of directories) {
    for (const date of dates) {
      const logPath = path.join(directory, `natesclaw-${date}.log`);
...
async function defaultVerifyMacTarget(checkout) {
  await delay(10_000);
  const executable = path.join(checkout, "dist/Natesclaw.app/Contents/MacOS/Natesclaw");

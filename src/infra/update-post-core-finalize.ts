...
// update.
//
// `runGatewayUpdate` (git mode) runs `natesclaw doctor --fix` with
// `NATESCLAW_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE=1`, which makes the doctor
// pass DEFER configured-plugin repair to a later convergence step (see
// `shouldDeferConfiguredPluginInstallRepair`). The `natesclaw update` CLI resumes
...
// binary's hidden `natesclaw update finalize` entrypoint — the designed
...
// re-enters the full `natesclaw update` CLI. Re-run convergence on no-op retries:
...
export async function runPostCoreFinalizeAfterGatewayUpdate(params: {
  ...
  try {
    ...
    if (params.preUpdateConfig) {
      sourceConfigDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-update-post-core-"));
  ...
  return {
    ...
    steps: [
      ...result.steps,
      {
        ...
        command: "natesclaw update finalize",

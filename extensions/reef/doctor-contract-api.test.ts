...
function legacyConfig(): NatesclawConfig {
...
describe("Reef doctor contract", () => {
  ...
  beforeEach(() => {
    resetPluginStateStoreForTests();
    stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-reef-doctor-"));
  ...
  it("imports identity keys into SQLite before archiving keys.json", async () => {
    const legacyDir = path.join(stateDir, ".natesclaw", "data", "reef");
  ...
  it("does not import the default home's Reef identity into an isolated state", async () => {
    ...
    const homeKeysPath = path.join(homeDir, ".natesclaw", "data", "reef", "keys.json");
    ...
    const isolatedEnv = { ...env, NATESCLAW_STATE_DIR: isolatedStateDir };
    const migration = migrationById("reef-keys-json-to-plugin-state");
    const params = {
      config: {},
      env: isolatedEnv,

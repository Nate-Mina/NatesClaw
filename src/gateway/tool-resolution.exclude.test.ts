...
type CreateNatesclawCodingToolsArg = {
...
const hoisted = vi.hoisted(() => {
  ...
  return {
    ...
    createNatesclawCodingToolsMock: vi.fn(
      (_args: CreateNatesclawCodingToolsArg): ReturnType<typeof makeTool>[] => [],
    ),
...
vi.mock("../agents/agent-tools.js", () => ({
  createNatesclawCodingTools: (args: CreateNatesclawCodingToolsArg) =>
    hoisted.createNatesclawCodingToolsMock(args),
}));
...
describe("resolveGatewayScopedTools excludeToolNames", () => {
  beforeEach(() => {
    hoisted.createNatesclawToolsMock.mockClear();
    hoisted.createLazyExecToolMock.mockClear();
    hoisted.createNatesclawCodingToolsMock.mockReset();
    hoisted.createNatesclawCodingToolsMock.mockReturnValue([]);
    hoisted.getLoadedChannelPluginMock.mockReset();
  });

  function readCreateToolsArgs(index = 0): CreateNatesclawToolsArg {
    const args = hoisted.createNatesclawToolsMock.mock.calls[index]?.[0];
    if (!args || typeof args !== "object") {
      throw new Error("expected createNatesclawTools args");
    }
    return args;
  }
  ...
  it("constructs exact coding tools for a server-minted mediated grant", () => {
    hoisted.createNatesclawCodingToolsMock.mockReturnValueOnce([hoisted.makeTool("write")]);

    const result = resolveGatewayScopedTools({
      cfg: { tools: { exec: { host: "node" } } } as NatesclawConfig,
    ...
    expect(hoisted.createNatesclawCodingToolsMock).toHaveBeenCalledWith(
  ...
  it("denies loopback tools after the scheduled owner account is removed", () => {
    ...
    hoisted.getLoadedChannelPluginMock.mockReturnValue({
      config: {
        listAccountIds: (cfg: NatesclawConfig) => Object.keys(cfg.channels?.discord?.accounts ?? {}),
      },
  ...
  it("does not fall back when policy removes a mediated coding tool", () => {
    hoisted.createNatesclawToolsMock.mockReturnValueOnce([
  ...
  it("omits all exec variants when host policy forbids node execution", () => {
    ...
    const sandboxAuto = resolveGatewayScopedTools({
      cfg: { agents: { defaults: { sandbox: { mode: "all" } } } } as NatesclawConfig,
      sessionKey: "agent:main:direct:test",
      surface: "loopback",
      senderIsOwner: true,
      includeNodeExecTool: true,
  ...
  it("filters node exec through the existing gateway deny policy", () => {
    const result = resolveGatewayScopedTools({
      cfg: { gateway: { tools: { deny: ["exec"] } } } as NatesclawConfig,
      sessionKey: "agent:main:direct:test",
      surface: "loopback",
      senderIsOwner: true,
      includeNodeExecTool: true,
  ...
  it("uses persisted delegated policy instead of the sender wildcard", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-gateway-delegated-policy-"));
  ...
  it("does not inherit node-only exec as a generic child or cron capability", () => {
    const result = resolveGatewayScopedTools({
      cfg: { tools: { allow: ["exec", "sessions_spawn", "automations"] } } as NatesclawConfig,
      sessionKey: "agent:main:direct:test",
      surface: "loopback",
      senderIsOwner: true,
      includeNodeExecTool: true,

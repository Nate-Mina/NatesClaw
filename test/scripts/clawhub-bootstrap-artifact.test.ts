...
function fixture() {
  const root = mkdtempSync(join(tmpdir(), "natesclaw-clawhub-bootstrap-"));
  tempDirs.push(root);
  ...
  writeFileSync(join(packageRoot, "natesclaw-meta-2026.7.1-beta.3.tgz"), "packed meta");
  writeFileSync(
    join(existingPackageRoot, "natesclaw-existing-2026.7.1-beta.3.tgz"),
    "packed existing",
  );
  ...
  writeFileSync(
    ...
    JSON.stringify([
      ...
      {
        packageName: "@natesclaw/existing",
        version: "2026.7.1-beta.3",
...
function common(paths: ReturnType<typeof fixture>) {
  return {
    ...
    plugins: "@natesclaw/meta,@natesclaw/existing",
...
function writeClawPack(
  ...
  const root = mkdtempSync(join(tmpdir(), "natesclaw-clawhub-packed-"));
  tempDirs.push(root);
...
describe("ClawHub bootstrap artifact manifest", () => {
  it("binds the exact package set and packed file identity", async () => {
    ...
    const meta = created.entries.find((entry) => entry.packageName === "@natesclaw/meta");
    expect(meta).toMatchObject({
      artifactPath: "packages/meta/natesclaw-meta-2026.7.1-beta.3.tgz",
      size: 11,
    });
  ...
  it("rejects changed bytes and extra artifact files", async () => {
    ...
    writeFileSync(
      join(paths.artifactRoot, "packages", "meta", "natesclaw-meta-2026.7.1-beta.3.tgz"),
      "changed",
    );
    await expect(
      verifyClawHubBootstrapArtifactManifest({
  ...
  it("binds exact target bytes to configure-only repairs", async () => {
    ...
    const existing = manifest.entries.find((entry) => entry.packageName === "@natesclaw/existing");
    expect(existing).toMatchObject({
      artifactPath: "packages/existing/natesclaw-existing-2026.7.1-beta.3.tgz",
      size: 15,
    });
...
describe("ClawHub packed artifact identity", () => {
  const expectedIdentity = {
    ...
    expectedName: "@natesclaw/meta-provider",
  ...
  it("accepts one canonical package identity and plugin manifest", async () => {
    ...
    await expect(
      ...
      packageName: "@natesclaw/meta-provider",
  ...
  it("rejects a whitespace-bearing alias before a later package.json", async () => {
    const pack = writeClawPack([
      ...
      },
      {
        ...
        contents: JSON.stringify({
          name: "@natesclaw/other",
          version: "9.9.9",
        }),
      },
      {
        name: "package/natesclaw.plugin.json",
        contents: JSON.stringify({ id: "meta" }),
      },
    ]);
  ...
  it("rejects a compressed artifact above the ClawHub package limit before reading it", async () => {
    const root = mkdtempSync(join(tmpdir(), "natesclaw-clawhub-packed-limit-"));
    tempDirs.push(root);
  ...
  it("bounds expanded tar bytes", async () => {
    const root = mkdtempSync(join(tmpdir(), "natesclaw-clawhub-expanded-limit-"));
    tempDirs.push(root);

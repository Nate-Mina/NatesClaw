// Builds PATH values for Natesclaw child processes.
...
function candidateBinDirs(
  ...
  // subprocesses keep using the same Node/Bun the current Natesclaw process is on.
  try {
  ...
  // Bundled macOS app: `natesclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "natesclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.NATESCLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin && cwd) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "natesclaw"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `natesclaw`
...
/**
 * Best-effort PATH bootstrap so skills that require the `natesclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureNatesclawCliOnPath(opts: EnsureNatesclawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.NATESCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }

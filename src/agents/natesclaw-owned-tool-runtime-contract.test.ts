// Verifies Natesclaw-owned tool hooks preserve adjusted params and telemetry.
import type { AgentTool } from "natesclaw/plugin-sdk/agent-core";
import {
  installNatesclawOwnedToolHooks,
  resetNatesclawOwnedToolHooks,
  textToolResult,
...
describe("Natesclaw-owned tool runtime contract - embedded agent adapter", () => {
  afterEach(() => {
    resetNatesclawOwnedToolHooks();
  });
  ...
  it("preserves partially adjusted before_tool_call params through execution and after_tool_call", async () => {
    ...
    const hooks = installNatesclawOwnedToolHooks({ adjustedParams });
  ...
  it("commits successful embedded agent messaging text, media, and target telemetry", async () => {
    const hooks = installNatesclawOwnedToolHooks();
    ...
    const originalParams = {
      ...
      mediaUrl: "/tmp/natesclaw-reply.png",
      provider: "telegram",
      to: "chat-1",
    };
    ...
    expect(ctx.state.messagingToolSentMediaUrls).toEqual(["/tmp/natesclaw-reply.png"]);
    expect(
    ...
    ).toEqual([
      {
        ...
        mediaUrls: ["/tmp/natesclaw-reply.png"],
      },
    ]);
    const [afterPayload, afterContext] = await waitForAfterToolCall(hooks);
  ...
  it("fails closed when before_tool_call blocks an embedded agent dynamic tool", async () => {
    const hooks = installNatesclawOwnedToolHooks({ blockReason: "blocked by policy" });

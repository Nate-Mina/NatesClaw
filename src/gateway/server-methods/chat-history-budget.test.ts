...
describe("enforceChatHistoryFinalBudget", () => {
  ...
  it("falls back to a small placeholder when even the last message is too large", () => {
    const last = {
      ...
      __natesclaw: { id: "abc", seq: 7, turnBoundary: true },
    };
    ...
    expect(
      (result.messages[0] as { __natesclaw?: { turnBoundary?: boolean } })["__natesclaw"]
        ?.turnBoundary,
    ).toBe(true);
  ...
  it("returns a metadata-free sentinel (never an empty transcript) when even the placeholder is over budget", () => {
    ...
    expect((result.messages[0] as Record<string, unknown>)["__natesclaw"]).toBeUndefined();
  });
});

...
Natesclaw ships four update channels:
...
| `extended-stable` | resolves the public npm `extended-stable` selector, verifies the exact selected package, and installs that exact version. Fails closed with no fallback to `latest`, `beta`, or `dev`. | unsupported: Natesclaw leaves the checkout unchanged and asks you to use a package installation                                                                     |
...
For `dev` git installs, the default checkout is `~/natesclaw` (or
...
`natesclaw update --channel dev` still targets the current upstream `main`.
...
natesclaw update --tag 2026.4.1-beta.1
...
natesclaw update --tag natesclaw@2026.4.1-beta.1
...
- The tag is not persisted; the next `natesclaw update` uses the configured
  channel.
- `--tag main` maps to the npm-compatible spec `github:natesclaw/natesclaw#main`
...
- Downgrade protection: if the target version is older than the current
  version, Natesclaw prompts for confirmation (skip with `--yes`).
...
Preview what `natesclaw update` would do without making changes:

```bash
natesclaw update --dry-run
natesclaw update --channel beta --dry-run
natesclaw update --tag 2026.4.1-beta.1 --dry-run
natesclaw update --dry-run --json
```
...
Switching channels with `natesclaw update` also syncs plugin sources:

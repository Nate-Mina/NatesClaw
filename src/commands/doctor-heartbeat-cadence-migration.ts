...
function createDoctorCronService(storePath: string, cfg: NatesclawConfig): CronService {
  const noop = () => {};
...
function cadenceFinding(params: {
  ...
  return {
    ...
    fixHint: `Run ${formatCliCommand("natesclaw doctor --fix")} to materialize heartbeat cadence in cron.`,
  };
}
    ...
    return [
      {
        ...
        fixHint: `Run ${formatCliCommand("natesclaw doctor --fix")} after resolving the cron store error.`,
      },
    ];
  }
}

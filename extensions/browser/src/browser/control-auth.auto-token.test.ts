...
const mocks = vi.hoisted(() => ({
  ...
  mutateConfigFile: vi.fn(
    async (params: {
      mutate: (draft: NatesclawConfig, context: { snapshot: { path: string } }) => unknown;
    }) => {
  ...
  resolveGatewayAuth: vi.fn(
    ({
      ...
      authConfig?: NonNullable<NonNullable<NatesclawConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: NatesclawConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): NatesclawConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: NatesclawConfig) {

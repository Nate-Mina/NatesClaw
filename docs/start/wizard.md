---
summary: "CLI onboarding: verify inference, then hand remaining setup to Natesclaw"
read_when:
...
the machine, verifies it with a real completion, and starts Natesclaw to
configure the workspace, Gateway, and optional features. `natesclaw setup` runs the same flow ([Setup](/cli/setup) covers
...
requires a real completion, and only then starts [Natesclaw](/cli/natesclaw)
to configure the rest of Natesclaw. Choosing **Skip for now** exits onboarding
without starting Natesclaw.
...
with `natesclaw onboard --classic`; the guided inference picker does not delegate
into it. After inference passes, Natesclaw can use `open channel wizard for
...
`natesclaw gateway restart`. Remote Gateway mode remains an onboarding or
`natesclaw configure` choice rather than a hosted chat wizard.
...
To change the model provider or its authentication, exit Natesclaw and run
`natesclaw onboard`; Natesclaw does not open guided or classic provider flows.

<Info>
Fastest first chat: finish guided setup, run `natesclaw dashboard`, and chat in
...
`NATESCLAW_LOCALE`, `LC_ALL`, `LC_MESSAGES`, and `LANG`, in that order, then
...
key-free. Configure this later with `natesclaw configure --section web`, or say
`configure web search` in the Natesclaw chat to run the same provider setup
conversationally. Docs: [Web tools](/tools/web).
</Tip>

## Guided default

Plain `natesclaw onboard` follows this path:
...
4. If detection is exhausted, choose OpenAI, Anthropic, xAI (Grok), Google, or
   ...
   Choose **Skip for now** to exit without starting Natesclaw.
...
6. Start Natesclaw with the verified model so it can configure the workspace,
...
asks how to continue. Run `natesclaw channels add` or `natesclaw configure` for
later non-inference additions; use `natesclaw onboard` for provider or auth route
changes.

## Classic wizard: QuickStart vs Advanced

Run `natesclaw onboard --classic` to open the full wizard. It starts with a
...
1. **Model/Auth** - pick a provider auth flow (API key, OAuth, or
   ...
   classic wizard. Ignoring it does not unlock Natesclaw; conversational setup
   still requires a passing inference check.
2. **Workspace** - directory for agent files (default `~/.natesclaw/workspace`). Seeds bootstrap files.
...
[Install](/install/migrating-hermes). `natesclaw onboard --modern` is a
compatibility alias for [Natesclaw](/cli/natesclaw). It uses the same
inference gate as `natesclaw setup`: verified inference starts the
assistant, while an interactive failure returns to guided inference setup.

## Add another agent

Use `natesclaw agents add <name>` to create a separate agent with its own
...
not the full `natesclaw onboard` wizard.
...
- Default workspace: `~/.natesclaw/workspace-<agentId>` (or under
...
For the full flag reference, see [`natesclaw onboard`](/cli/onboard).

## Related docs

- CLI command reference: [`natesclaw onboard`](/cli/onboard)

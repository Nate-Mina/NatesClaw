// Natesclaw Cross Os Release Checks tests cover natesclaw cross os release checks script behavior.
...
describe("scripts/natesclaw-cross-os-release-checks", () => {
  it("uses the host account identity for managed installer services", () => {
    const env = resolveManagedGatewayInstallerEnv({
      env: {
        ...
        NATESCLAW_HOME: "C:\\temp\\lane",
        NATESCLAW_PROFILE: "work",
        NATESCLAW_STATE_DIR: "C:\\temp\\lane\\.natesclaw",
        NATESCLAW_CONFIG_PATH: "C:\\temp\\lane\\.natesclaw\\natesclaw.json",
        NATESCLAW_WINDOWS_TASK_NAME: "Natesclaw Gateway (work)",
        NATESCLAW_TASK_SCRIPT_NAME: "work.cmd",
        NATESCLAW_TASK_SCRIPT: "C:\\temp\\work.cmd",
        NATESCLAW_SERVICE_KIND: "node",
        Natesclaw_Home: "C:\\temp\\case-variant",
        natesclaw_config_path: "C:\\temp\\case-variant\\natesclaw.json",
    ...
    expect(env.NATESCLAW_HOME).toBeUndefined();
    expect(env.NATESCLAW_PROFILE).toBeUndefined();
    expect(env.NATESCLAW_STATE_DIR).toBeUndefined();
    expect(env.NATESCLAW_CONFIG_PATH).toBeUndefined();
    expect(env.NATESCLAW_WINDOWS_TASK_NAME).toBeUndefined();
    expect(env.NATESCLAW_TASK_SCRIPT_NAME).toBeUndefined();
    expect(env.NATESCLAW_TASK_SCRIPT).toBeUndefined();
    expect(env.NATESCLAW_SERVICE_KIND).toBeUndefined();
    expect(
  ...
  it("keeps isolated installer state when no managed service is used", () => {
    const env = { NATESCLAW_HOME: "/tmp/natesclaw-installer" };
  ...
  it("fails closed before borrowing an occupied managed-service account", () => {
    ...
    expect(() =>
      assertManagedGatewayInstallerHostAvailable({
        ...
        pathExists: (path) => path.endsWith(".natesclaw"),
      }),
    ).toThrow(/pristine host account/);
  });
  ...
  it("holds an exclusive managed-service host lease until release", () => {
    withTempDir("natesclaw-managed-host-", (accountHome) => {
  ...
  it("requires dashboard root markers and same-origin asset URLs", () => {
    const html = [
      "<title>Natesclaw Control</title>",
      "<natesclaw-app></natesclaw-app>",
  ...
  it("keeps gateway status RPC probing when help probing is unavailable", () => {
    ...
    expect(buildGatewayStatusArgsFromHelpText("Usage: natesclaw gateway status")).toEqual([
      "gateway",
      "status",
    ]);
    expect(
  ...
  it("restarts an exited manual gateway once after the exact startup migration refusal", async () => {
    await withTempDirAsync("natesclaw-cross-os-gateway-restart-", async (dir) => {
      const refusal =
        "Natesclaw plugin migration inputs changed during startup convergence; refusing to report the gateway ready.";
  ...
  it.each([
    ...
    await withTempDirAsync("natesclaw-cross-os-gateway-no-restart-", async (dir) => {
  ...
  it("does not restart a live gateway after a readiness failure", async () => {
    await withTempDirAsync("natesclaw-cross-os-gateway-live-", async (dir) => {
      const gateway = createGatewayHandleFixture({
        dir,
        name: "gateway",
        afterLaunch:
  ...
  it("fails after a second startup migration refusal without looping", async () => {
    await withTempDirAsync("natesclaw-cross-os-gateway-second-refusal-", async (dir) => {
      const refusal =
        "Natesclaw plugin migration inputs changed during startup convergence; refusing to report the gateway ready.\n";
      const first = createGatewayHandleFixture({
        dir,
  ...
  it("routes the Discord manual relaunch through the bounded retry wait", async () => {
    await withTempDirAsync("natesclaw-cross-os-discord-gateway-", async (dir) => {
      ...
      await restartManualGatewayForDiscordSmoke({
        lane,
        cliPath: join(dir, "natesclaw"),
        env: { NATESCLAW_HOME: lane.homeDir },
        gatewayHolder,
        gatewayLogPath,
        statusLogPath,
        operations: {
  ...
  it("gives the Windows packaged updater wrapper enough headroom for Natesclaw timeout output", () => {
  ...
  it("rejects malformed cross-OS positive integer environment values", () => {
    expect(parsePositiveIntegerEnv("NATESCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {})).toBe(60);
    expect(
      parsePositiveIntegerEnv("NATESCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {
        NATESCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS: "25",
      }),
    ).toBe(25);
    ...
    for (const raw of ["1e3", "25ms", "1.5", "0", "-1", String(Number.MAX_SAFE_INTEGER + 1)]) {
      expect(() =>
        parsePositiveIntegerEnv("NATESCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {
          NATESCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS: raw,
        }),
      ).toThrow("NATESCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS must be a positive integer");
    }
  });
  ...
  it("accepts OK agent output from the captured log when stdout is empty", () => {
    withTempDir("natesclaw-cross-os-agent-output-", (dir) => {
  ...
  it("ignores stale OK markers outside the recent agent log tail", () => {
    withTempDir("natesclaw-cross-os-agent-output-tail-", (dir) => {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
        [
  ...
  it("requires explicit opt-in before cross-OS agent turns become optional", () => {
    ...
    expect(resolveCrossOsAgentTurnOptional({ NATESCLAW_CROSS_OS_AGENT_TURN_OPTIONAL: "1" })).toBe(
      true,
    );
    expect(
      resolveCrossOsAgentTurnOptional({ NATESCLAW_CROSS_OS_AGENT_TURN_OPTIONAL: "false" }),
    ).toBe(false);
  });

  it("skips optional live agent turns only for model availability failures", () => {
    withTempDir("natesclaw-cross-os-agent-skip-", (dir) => {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
        JSON.stringify({
  ...
  it("does not classify stale timeout logs as current optional agent-turn failures", () => {
    withTempDir("natesclaw-cross-os-agent-skip-tail-", (dir) => {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
        [
  ...
  it("only skips opted-in cross-OS live agent turns after retry exhaustion", () => {
    withTempDir("natesclaw-cross-os-agent-skip-retry-", (dir) => {
      const logPath = join(dir, "agent.log");
  ...
  it("allows cross-OS provider smoke models to use faster CI overrides", () => {
    expect(
      resolveProviderConfig("openai", {
        NATESCLAW_CROSS_OS_OPENAI_MODEL: "openai/gpt-5.4-mini",
      })?.model,
    ).toBe("openai/gpt-5.4-mini");
    expect(
      resolveProviderConfig("openai", {
        NATESCLAW_CROSS_OS_MODEL: "openai/gpt-5.4-nano",
      })?.model,
  ...
  it("keeps release cross-OS OpenAI smoke on GPT-5.6 Luna", () => {
    ...
    const releaseChecks = readFileSync(".github/workflows/natesclaw-release-checks.yml", "utf8");

    expect(workflow).toContain(
      "NATESCLAW_CROSS_OS_OPENAI_MODEL: ${{ inputs.openai_model || vars.NATESCLAW_CROSS_OS_OPENAI_MODEL || 'openai/gpt-5.6-luna' }}",
    );
  ...
  it("can stage packaged-upgrade baselines without npm lifecycle scripts", () => {
    expect(buildNpmGlobalInstallArgs("natesclaw@2026.5.2", { ignoreScripts: true })).toEqual([
      "install",
      "-g",
      "natesclaw@2026.5.2",
      "--omit=dev",
  ...
  it("rejects unsafe npm pack tarball filenames before staging release artifacts", () => {
    expect(resolveNpmPackTarballFileName("natesclaw-2026.6.17.tgz")).toBe("natesclaw-2026.6.17.tgz");
  ...
  it("accepts pnpm pack tarballs reported under the requested destination", () => {
    const packDir = resolvePath("/tmp/natesclaw-pack");

    expect(resolvePackDestinationTarball("natesclaw-2026.6.17.tgz", packDir, "pnpm pack")).toEqual({
      fileName: "natesclaw-2026.6.17.tgz",
      path: resolvePath(packDir, "natesclaw-2026.6.17.tgz"),
    });
    expect(
      resolvePackDestinationTarball(
        resolvePath(packDir, "natesclaw-2026.6.17.tgz"),
        packDir,
        "pnpm pack",
      ),
    ).toEqual({
  ...
  it("rejects pnpm pack tarballs outside the requested destination", () => {
    ...
    const unsafeFilenames = [
      ...
      "nested/natesclaw.tgz",
      "nested\\natesclaw.tgz",
      resolvePath(dirname(packDir), "natesclaw.tgz"),
      resolvePath(packDir, "nested", "natesclaw.tgz"),
      "natesclaw\u0000.tgz",
      "natesclaw.tar.gz",
    ];
  ...
  it("falls back to pnpm pack for historical refs without the Docker package helper", () => {
    withTempDir("natesclaw-cross-os-pack-command-", (dir) => {
      ...
      const helperPath = join(dir, "scripts", "package-natesclaw-for-docker.mjs");
  ...
  it("keeps packaged-upgrade release updates out of service restart flow", () => {
    const args = buildPackagedUpgradeUpdateArgs("http://127.0.0.1:49152/natesclaw-current.tgz");
    expect(args.slice(0, 6)).toEqual([
      "update",
      "--tag",
      "http://127.0.0.1:49152/natesclaw-current.tgz",
      "--yes",
      "--json",
      "--no-restart",
    ]);
  ...
  it("keeps cross-OS live smoke agent turns on GPT-5-safe timeouts and minimal context", () => {
    ...
    expect(source).toContain('agentRuntime: { id: "natesclaw" }');
  ...
  it("prefers the freshly installed Windows CLI under npm's prefix before PATH lookup", () => {
    ...
    expect(script).toContain("(Join-Path $npmPrefix 'natesclaw.cmd')");
    expect(script).toContain("$cmd = Get-Command natesclaw -ErrorAction Stop");
  });
  ...
  it("prefers workflow-injected runner override env names over legacy ones", () => {
    expect(
      readRunnerOverrideEnv({
        ...
        NATESCLAW_RELEASE_CHECKS_UBUNTU_RUNNER: "legacy-linux",
        NATESCLAW_RELEASE_CHECKS_WINDOWS_RUNNER: "legacy-windows",
        NATESCLAW_RELEASE_CHECKS_MACOS_RUNNER: "legacy-macos",
      }),
    ).toEqual({
  ...
  it("serves installer scripts as UTF-8 text and package payloads as binary", () => {
    ...
    expect(resolveStaticFileContentType("natesclaw-2026.4.14.tgz")).toBe("application/octet-stream");
  });

  it("streams release artifacts from the static file server", async () => {
    const dir = mkdtempSync(join(tmpdir(), "natesclaw-cross-os-static-server-"));
    const filePath = join(dir, "natesclaw-2026.4.14.tgz");
  ...
  it("closes static release artifact sockets left by aborted clients", async () => {
    const dir = mkdtempSync(join(tmpdir(), "natesclaw-cross-os-static-server-close-"));
    const filePath = join(dir, "natesclaw-2026.4.14.tgz");
    const logPath = join(dir, "server.log");
    let server: Awaited<ReturnType<typeof startStaticFileServer>> | undefined;
  ...
  it("flushes static release artifact logs before close resolves", async () => {
    const dir = mkdtempSync(join(tmpdir(), "natesclaw-cross-os-static-server-log-flush-"));
    const filePath = join(dir, "natesclaw-2026.4.14.tgz");
    const logPath = join(dir, "server.log");
    let server: Awaited<ReturnType<typeof startStaticFileServer>> | undefined;
  ...
  it("runs the installed browser override import smoke only on native Windows", () => {
    ...
    expect(script).toContain('from "natesclaw/plugin-sdk/plugin-runtime"');
    expect(script).toContain('overrideEnvVar: "NATESCLAW_BROWSER_CONTROL_MODULE"');
    ...
    const installedScript = buildInstalledBrowserOverrideImportProbeScript(
      "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/natesclaw/dist/plugin-sdk/plugin-runtime.js",
    );
    expect(installedScript).toContain(
      'from "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/natesclaw/dist/plugin-sdk/plugin-runtime.js"',
    );
    expect(readFileSync("scripts/lib/cross-os-release-checks/install.ts", "utf8")).toContain(
      "NATESCLAW_BROWSER_CONTROL_MODULE: pathToFileURL(overridePath).href",
    );
  });
  ...
  it("wraps Windows cmd shims without Node shell argv", () => {
    expect(
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\Program Files\nodejs\npm.cmd" view natesclaw@latest version"`,
      ],
      shell: false,
  ...
  it("wraps installed Windows CLI cmd fallbacks without Node shell argv", () => {
    expect(
      resolveInstalledCliInvocation(
        win32.join(String.raw`C:\Natesclaw Prefix`, "natesclaw.cmd"),
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\Natesclaw Prefix\natesclaw.cmd" gateway run --port 1234"`,
      ],
      shell: false,
      windowsVerbatimArguments: true,
    });
  ...
  it("runs resolved command invocations and writes command logs", async () => {
    await withTempDirAsync("natesclaw-cross-os-run-command-", async (dir) => {
  ...
  it("bounds retained command output while preserving full command logs", async () => {
    await withTempDirAsync("natesclaw-cross-os-run-command-output-", async (dir) => {
      const logPath = join(dir, "command.log");
  ...
  it("keeps multibyte command output and error tails within the byte budget", async () => {
    await withTempDirAsync("natesclaw-cross-os-run-command-utf8-tail-", async (dir) => {
      const logPath = join(dir, "command.log");
      const result = await runCommand(
        process.execPath,
  ...
  it("keeps rolling multibyte command output and error tails within the byte budget", async () => {
    await withTempDirAsync("natesclaw-cross-os-run-command-utf8-rolling-", async (dir) => {
      const logPath = join(dir, "command.log");
      const script = [
  ...
  it.each(["stdout", "stderr"] as const)(
    ...
    async (stream) => {
      await withTempDirAsync("natesclaw-cross-os-run-command-utf8-split-", async (dir) => {
        const logPath = join(dir, "command.log");
        const script = [
  ...
  it.each([1, 2, 3])(
    ...
    async (maxOutputBytes) => {
      await withTempDirAsync("natesclaw-cross-os-run-command-utf8-budget-", async (dir) => {
        const logPath = join(dir, "command.log");
        const result = await runCommand(
          process.execPath,
  ...
  it.each([
    ...
    async ({ maxOutputBytes, expected }) => {
      await withTempDirAsync("natesclaw-cross-os-run-command-utf8-incomplete-", async (dir) => {
        const logPath = join(dir, "command.log");
        const result = await runCommand(
          process.execPath,
          [
  ...
  it("flushes command logs before resolving", async () => {
    await withTempDirAsync("natesclaw-cross-os-run-command-flush-", async (dir) => {
  ...
  it("reads npm debug logs from the Windows cache root", () => {
    withTempDir("natesclaw-cross-os-npm-debug-", (dir) => {
  ...
  it("prefers npm configured log directories over cache defaults", () => {
    withTempDir("natesclaw-cross-os-npm-logs-dir-", (dir) => {
      const homeDir = join(dir, "home");
  ...
  it("keeps npm debug log collection best-effort", () => {
    withTempDir("natesclaw-cross-os-npm-debug-best-effort-", (dir) => {
      const homeDir = join(dir, "home");
      const logPath = join(dir, "install.log");
  ...
  it("resolves relative npm log config from the install working directory", () => {
    withTempDir("natesclaw-cross-os-npm-relative-logs-", (dir) => {
      const homeDir = join(dir, "home");
  ...
  it("kills timed-out command process groups", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "natesclaw-cross-os-run-command-timeout-"));
    ...
    try {
      ...
      const parentScript = [
        ...
        "fs.writeFileSync(process.env.NATESCLAW_TEST_CHILD_PID, String(child.pid));",
        "setInterval(() => {}, 1000);",
      ].join("");

      const command = runCommand(process.execPath, ["-e", parentScript], {
        cwd: dir,
        env: { ...process.env, NATESCLAW_TEST_CHILD_PID: childPidPath },
        logPath,
        timeoutMs: 500,
      });
  ...
  it("forwards external termination to command process groups", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "natesclaw-cross-os-run-command-signal-"));
    const childPidPath = join(dir, "child.pid");
    ...
    try {
      ...
      const runner = spawn(
        ...
        ["--import", "tsx", "--input-type=module", "-e", runnerScript],
        {
          ...
          env: {
            ...process.env,
            NATESCLAW_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS: "200",
            NATESCLAW_TEST_CHILD_PID: childPidPath,
          },
  ...
  it("exits promptly after externally signaled commands close", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "natesclaw-cross-os-run-command-signal-exit-"));
    const childPidPath = join(dir, "child.pid");
    ...
    try {
      ...
      const runner = spawn(
        ...
        ["--import", "tsx", "--input-type=module", "-e", runnerScript],
        {
          ...
          env: {
            ...process.env,
            NATESCLAW_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS: "3000",
            NATESCLAW_TEST_CHILD_PID: childPidPath,
          },
          stdio: ["ignore", "ignore", "pipe"],
        },
  ...
  it("derives the installed prefix from resolved CLI paths", () => {
    ...
    expect(
      resolveInstalledPrefixDirFromCliPath("/Users/runner/.npm-global/bin/natesclaw", "darwin"),
    ).toBe("/Users/runner/.npm-global");
  });

  it("resolves Linux npm package roots when the CLI is a user-local shim", () => {
    const homeDir = mkdtempSync(join(tmpdir(), "natesclaw-cross-os-linux-home-"));
    try {
      const packageRoot = join(homeDir, ".npm-global", "lib", "node_modules", "natesclaw");
      const distDir = join(packageRoot, "dist");
      const cliDir = join(homeDir, ".local", "bin");
      mkdirSync(distDir, { recursive: true });
      mkdirSync(cliDir, { recursive: true });
      writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "natesclaw" }));
      writeFileSync(join(distDir, "entry.js"), "#!/usr/bin/env node\n");

      expect(
        resolveInstalledPackageRootFromCliPath(join(cliDir, "natesclaw"), "linux", {
          HOME: homeDir,
        }),
      ).toBe(packageRoot);

      rmSync(join(cliDir, "natesclaw"), { force: true });
      symlinkSync(join(distDir, "entry.js"), join(cliDir, "natesclaw"));
  ...
  it("drops the bundled plugin postinstall disable flag for real updater calls", () => {
    expect(
      buildRealUpdateEnv({
        FOO: "bar",
        NODE_COMPILE_CACHE: "/tmp/stale-natesclaw-cache",
      ...
      verifyPackagedUpgradeUpdateResult(
        {
          ...
          stderr:
            "[natesclaw] Failed to start CLI: Error [ERR_MODULE_NOT_FOUND]: Cannot find module '/tmp/prefix/lib/node_modules/natesclaw/dist/memory-state-old.js'",
        },
  ...
  it("recognizes the shipped Windows updater packaged-upgrade timeout", () => {
    const error = new Error(
      "Command timed out: C:\\hostedtoolcache\\windows\\node\\24.15.0\\x64\\node.exe C:\\Users\\RUNNER~1\\AppData\\Local\\Temp\\natesclaw-upgrade-q9DsA7\\prefix\\node_modules\\natesclaw\\natesclaw.mjs update --tag http://127.0.0.1:49951/natesclaw-2026.5.4-beta.1.tgz --yes --json --no-restart --timeout 1500",
    );
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error(
          "Command timed out: C:\\prefix\\node_modules\\natesclaw\\natesclaw.mjs update --tag http://127.0.0.1:49951/natesclaw-current.tgz --yes --json --timeout 1500",
        ),
        "win32",
      ),
    ).toBe(true);
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error("Command timed out: node natesclaw.mjs update --tag natesclaw@beta"),
        "win32",
      ),
    ).toBe(false);
  });
      ...
      isRecoverableWindowsPackagedUpgradeSwapCleanupFailure(
        {
          exitCode: 1,
          stdout:
            "EPERM: operation not permitted, unlink '/tmp/prefix/node_modules/.natesclaw-1-2/native.node'",
          stderr: "",
        },
        "linux",
      ),
  ...
  it("only treats pinned baseline specs as exact installer version assertions", () => {
    ...
    expect(resolveExplicitBaselineVersion("natesclaw@latest")).toBe("");
    expect(resolveExplicitBaselineVersion("natesclaw@2026.4.10")).toBe("2026.4.10");
  ...
  it("reads an installed baseline version without requiring build metadata", () => {
    withTempDir("natesclaw-cross-os-installed-version-", (prefixDir) => {
      const packageRoot =
        process.platform === "win32"
          ? join(prefixDir, "node_modules", "natesclaw")
          : join(prefixDir, "lib", "node_modules", "natesclaw");
  ...
  it("treats missing package scripts as optional in older refs", () => {
    withTempDir("natesclaw-cross-os-scripts-", (packageRoot) => {
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
          name: "natesclaw",
  ...
  it("rejects legacy plugin dependency staging debris before candidate inventory generation", async () => {
    await withTempDirAsync("natesclaw-cross-os-stage-debris-", async (packageRoot) => {
      mkdirSync(
        join(packageRoot, "dist", "Extensions", "demo", ".Natesclaw-Install-Stage", "node_modules"),
        { recursive: true },
      );
      writeFileSync(
        join(packageRoot, "dist", "Extensions", "demo", ".Natesclaw-Install-Stage", "package.json"),
        "{}\n",
        "utf8",
      );
  ...
  it("omits local build metadata from candidate package inventories", async () => {
    await withTempDirAsync("natesclaw-cross-os-local-stamps-", async (packageRoot) => {
      mkdirSync(join(packageRoot, "dist"), { recursive: true });
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({ name: "natesclaw-fixture", version: "0.0.0", files: ["dist/"] }),
        "utf8",
      );

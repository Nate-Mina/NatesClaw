...
import type { MatrixVerificationSummary } from "@natesclaw/matrix/test-api.js";
...
export function requireMatrixQaGatewayConfigPath(context: MatrixQaScenarioContext) {
  const configPath = requireMatrixQaCliRuntimeEnv(context).NATESCLAW_CONFIG_PATH?.trim();
  if (!configPath) {

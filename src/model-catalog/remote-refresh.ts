...
export function isRemoteModelCatalogRefreshEnabled(config: NatesclawConfig): boolean {
  return config.models?.catalogRefresh?.enabled !== false;
}

export function resolveRemoteCatalogUrl(config: NatesclawConfig): string {
...
function assertCompatibleMinVersion(bundle: RemoteModelCatalogBundle): void {
  ...
  const comparison = compareNatesclawVersions(VERSION, bundle.minVersion);
  if (comparison === null) {
    throw new Error(`invalid remote catalog minVersion: ${bundle.minVersion}`);
  }
  if (comparison < 0) {
    throw new Error(
      `remote catalog requires Natesclaw ${bundle.minVersion} or newer (current ${VERSION})`,
    );
  }
}

function isExplicitLocalHttpUrl(config: NatesclawConfig, url: string): boolean {

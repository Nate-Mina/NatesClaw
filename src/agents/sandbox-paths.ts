...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const NatesclawTmpDir = path.resolve(resolvePreferredNatesclawTmpDir());
  if (!isPathInside(NatesclawTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: NatesclawTmpDir });
  return resolved;
}

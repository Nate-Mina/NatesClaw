...
describe("system profile import state", () => {
  ...
  it("reuses only the recorded target and otherwise avoids collisions", () => {
    expect(resolveSuggestedImportTarget({ profileNames: ["natesclaw"] })).toBe("imported");
    expect(resolveSuggestedImportTarget({ profileNames: ["natesclaw", "imported"] })).toBe(
      "imported-2",
    );
    expect(
      resolveSuggestedImportTarget({
        profileNames: ["natesclaw", "imported", "imported-2"],
        state: {
          version: 1,
          status: "imported",
          browser: "chrome",

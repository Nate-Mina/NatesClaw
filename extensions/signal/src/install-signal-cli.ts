...
import { runPluginCommandWithTimeout } from "natesclaw/plugin-sdk/run-command";
import type { RuntimeEnv } from "natesclaw/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "natesclaw/plugin-sdk/setup-tools";
...
import { withTempDownloadPath } from "natesclaw/plugin-sdk/temp-path";
...
export async function installSignalCliFromRelease(
  ...
  return await withTempDownloadPath(
    { prefix: "natesclaw-signal", fileName: asset.name },
    async (archivePath) => {

...
private let gatewayConnectionLogger = Logger(subsystem: "ai.natesclaw", category: "gateway.connection")
...
actor GatewayConnection {
    ...
    nonisolated static let operatorClientCaps =
        [NatesclawGatewayClientCapability.agentKind, NatesclawGatewayClientCapability.inlineWidgets]
...
extension GatewayConnection {
    ...
    func sessionRoutingIdentity(
        ifCurrentRoute route: Route) async throws -> NatesclawChatSessionRoutingIdentity
    {
        let data = try await request(
            NatesclawChatGatewayRequests.agentsList(),
            ifCurrentRoute: route)
        return try NatesclawChatGatewayPayloadCodec.decodeSessionRoutingIdentity(data)
    }
...
extension GatewayConnection {
    ...
    private func sessionDefaultString(_ defaults: [String: NatesclawProtocol.AnyCodable]?, key: String) -> String {
...
extension GatewayConnection {
    ...
    func healthOK(timeoutMs: Int = 8000) async throws -> Bool {
        ...
        return (try? self.decoder.decode(NatesclawGatewayHealthOK.self, from: data))?.ok ?? true
    }
    ...
    func sessionsPreview(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> NatesclawSessionsPreviewPayload
    {
        let resolvedKeys = keys
            .map { self.canonicalizeSessionKey($0) }
            .filter { !$0.isEmpty }
        if resolvedKeys.isEmpty {
            return NatesclawSessionsPreviewPayload(ts: 0, previews: [])
        }
    ...
    func chatHistory(
        ...
        maxChars: Int? = nil,
        timeoutMs: Int? = nil,
        ifCurrentRoute route: Route? = nil) async throws -> NatesclawChatHistoryPayload
    {
        let resolvedKey = self.canonicalizeSessionKey(sessionKey)
        ...
        if let route {
            let data = try await self.request(
                request,
                ifCurrentRoute: route)
            return try self.decoder.decode(NatesclawChatHistoryPayload.self, from: data)
        }
        ...
        distinguishPreDispatchRouteChange: Bool = false) async throws -> NatesclawChatSendResponse
    {
        ...
        if let route {
            let data = try await self.request(
                ...
                ifCurrentRoute: route,
                distinguishPreDispatchRouteChange: distinguishPreDispatchRouteChange)
            return try self.decoder.decode(NatesclawChatSendResponse.self, from: data)
        }
        let data = try await self.request(request)
        return try self.decoder.decode(NatesclawChatSendResponse.self, from: data)
    }

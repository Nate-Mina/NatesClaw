...
normalized results, cached by query for 15 minutes (configurable). Natesclaw
...
<Steps>
  ...
  <Step title="Configure">
    ...
    [Natesclaw](/cli/natesclaw): say `configure web search` in `natesclaw setup`
    or in the Control UI's **Settings → Ask Natesclaw** chat. The hosted flow
  ...
  <Step title="Use it">
    ```javascript
    await web_search({ query: "Natesclaw plugin SDK" });
    ```
...
no `provider` is set, Natesclaw checks providers in this order and uses the
...
`natesclaw configure --section web`. Natesclaw does not send managed
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, Natesclaw resolves only the
...
`web_search` tool automatically when Natesclaw web search is enabled and no
...
search and Natesclaw's managed `web_search` dynamic tool are mutually exclusive,
so managed search cannot bypass native domain restrictions. Natesclaw uses the
...
- Set `tools.web.search.provider` to a managed provider such as `brave` to use
  Natesclaw's managed `web_search` instead
...
use the managed `web_search` fallback through Natesclaw's dynamic tool namespace.
Use an explicit managed provider when you need Natesclaw's provider-specific
...
Codex app-server first with `natesclaw models auth login --provider openai`.
The parent agent can use any model or runtime; only the bounded search worker
runs through Codex.

## Network safety

Managed HTTP `web_search` provider calls use Natesclaw's guarded fetch path,
scoped to the current provider's own hostname. For that hostname only,
Natesclaw allows Surge, Clash, and sing-box fake-IP DNS answers in
...
Grok can also reuse an xAI OAuth auth profile from `natesclaw models auth login
...
Natesclaw keeps startup resilient and reports a warning so you can reinstall the
plugin or run `natesclaw doctor --fix` to clean up the stale config.

`web_fetch` fallback provider selection is separate:

- choose it with `tools.web.fetch.provider`
- or omit that field and let Natesclaw auto-detect the first ready web-fetch
...
When you choose **Kimi** during `natesclaw onboard` or
`natesclaw configure --section web`, Natesclaw can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `natesclaw doctor --fix`.
When you choose Grok during `natesclaw onboard` or `natesclaw configure --section web`,
Natesclaw also offers optional `x_search` setup with the same credential right
...
provider, Natesclaw does not show the `x_search` prompt.

### Storing API keys

<Tabs>
  <Tab title="Config file">
    Run `natesclaw configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.natesclaw/.env`.
...
optional structured filters. Natesclaw constructs the built-in xAI `x_search`

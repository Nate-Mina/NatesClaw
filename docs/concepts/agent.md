...
Natesclaw ships one **embedded agent runtime**: a built-in agent loop, tool
...
Recommended: use `natesclaw setup` to create `~/.natesclaw/natesclaw.json` if missing and initialize the workspace files.
...
Inside the workspace, Natesclaw expects these user-editable files:
...
On the first turn of a new session, Natesclaw injects the contents of these files into the system prompt's Project Context. `MEMORY.md` is only injected when it exists at the workspace root.

Blank files are skipped. Large files are trimmed and truncated with a marker so prompts stay lean (read the file for full content). A missing file (other than `MEMORY.md`) injects a single "missing file" marker line instead; `natesclaw setup` creates a safe default template for it.

`BOOTSTRAP.md` is only created for a **brand new workspace** (no other bootstrap files present). While it is pending, Natesclaw keeps it in Project Context and adds system-prompt bootstrap guidance for the initial ritual instead of copying it into the user message. If you delete it after completing the ritual, it is not recreated on later restarts.

After a workspace has been observed, Natesclaw stores its setup state and
attestation in the shared SQLite database at
`~/.natesclaw/state/natesclaw.sqlite`. If a recently attested workspace
...
not read those files. Run `natesclaw doctor --fix` to validate them, import their
...
Natesclaw loads skills from these locations (highest precedence first):

- Workspace: `<workspace>/skills`
- Project agent skills: `<workspace>/.agents/skills`
- Personal agent skills: `~/.agents/skills`
- Managed/local: `~/.natesclaw/skills`
...
The embedded agent runtime is Natesclaw-owned: model discovery, tool wiring,
...
`~/.natesclaw/agents/<agentId>/sessions/` as legacy migration inputs, deleted or
...
Natesclaw. Natesclaw does not read session folders from other tools.

## Steering while streaming

Inbound prompts that arrive mid-run are steered into the current run by default.
The Natesclaw runtime checks for steering before unstarted tool launches and the
...
- If you omit the provider, Natesclaw tries an alias first, then a unique
  ...
  configured default model, Natesclaw falls back to the first configured

---
name: release-natesclaw-announcement
description: "Draft or post Natesclaw beta, stable, or extended-stable Discord release announcements from changelog, tag, registry, and validation evidence. Use when announcing a release, release candidate, or asking what users should test after an Natesclaw release."
---

# Natesclaw Release Announcement

Use with `release-natesclaw-maintainer` after a release is live.
...
- exact version, e.g. `Natesclaw 2026.5.25-beta.1`
...
- update command promoted near the top:
  ```sh
  natesclaw update --channel beta --yes
  natesclaw --version
  ```
- fresh install path:
...
```sh
natesclaw update --channel stable --yes
natesclaw --version
```
...
sending it. Never use Natesclaw channel sends, bots, webhooks, relays, or tokens.

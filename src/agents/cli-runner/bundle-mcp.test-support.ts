...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    ...
    envSnapshot = captureEnv(["NATESCLAW_BUNDLED_PLUGINS_DIR"]);
    bundleProbeHomeDir = await tempHarness.createTempDir("natesclaw-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir("natesclaw-cli-bundle-mcp-workspace-");
    const emptyBundledDir = await tempHarness.createTempDir("natesclaw-cli-bundle-mcp-bundled-");
    setTestEnvValue("NATESCLAW_BUNDLED_PLUGINS_DIR", emptyBundledDir);
...
function createEnabledBundleProbeConfig(): NatesclawConfig {
  return {
    plugins: {
      entries: {

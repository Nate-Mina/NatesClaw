...
export function assertCanonicalOperatorApprovalKinds(db: DatabaseSync, pathname: string): void {
  if (!hasCanonicalOperatorApprovalKinds(db)) {
    throw new Error(
      `Natesclaw state database ${pathname} has a legacy operator approval schema; run natesclaw doctor --fix to migrate it.`,
    );
  }
}
...
function canonicalOperatorApprovalCreateSql(): string {
  ...
  const start = NATESCLAW_STATE_SCHEMA_SQL.indexOf(marker);
  const end = NATESCLAW_STATE_SCHEMA_SQL.indexOf(
  ...
  return NATESCLAW_STATE_SCHEMA_SQL.slice(start, end + tableTerminator.length);
}
...
function operatorApprovalIndexSql(): string {
  const statements = NATESCLAW_STATE_SCHEMA_SQL.split(";")

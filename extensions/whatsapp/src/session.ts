...
import { VERSION } from "natesclaw/plugin-sdk/cli-runtime";
import { toErrorObject } from "natesclaw/plugin-sdk/error-runtime";
import {
  createHttp1EnvHttpProxyAgent,
  createHttp1ProxyAgent,
  createNodeProxyAgent,
} from "natesclaw/plugin-sdk/fetch-runtime";
import { danger, success } from "natesclaw/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "natesclaw/plugin-sdk/runtime-env";
...
function resolveEnvWaWebSocketUrl(): string | undefined {
  const value = resolveWaWebSocketUrl(process.env[NATESCLAW_WHATSAPP_WEB_SOCKET_URL_ENV]);
  if (!value) {
    return undefined;
  }
  let url: URL;
  try {
    ...
    throw new Error(`${NATESCLAW_WHATSAPP_WEB_SOCKET_URL_ENV} must be a valid URL.`);
  }
  if (url.protocol !== "ws:" && url.protocol !== "wss:") {
    throw new Error(`${NATESCLAW_WHATSAPP_WEB_SOCKET_URL_ENV} must use ws:// or wss://.`);
  }
  return url.toString();
}
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("natesclaw channels login")}`,
              ),
            );
          }
        }

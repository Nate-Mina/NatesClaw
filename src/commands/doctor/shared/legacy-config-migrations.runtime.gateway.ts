...
const GATEWAY_PORT_OOB_RULE: LegacyConfigRule = {
  path: ["gateway", "port"],
  message:
    'gateway.port is outside the valid TCP range (1–65535) and will be removed to avoid startup failure. Run "natesclaw doctor --fix".',
...
const GATEWAY_BIND_RULE: LegacyConfigRule = {
  path: ["gateway", "bind"],
  message:
    'gateway.bind host aliases (for example 0.0.0.0/localhost) are legacy; use bind modes (lan/loopback/custom/tailnet/auto) instead. Run "natesclaw doctor --fix".',
...
const GATEWAY_WEBCHAT_RULE: LegacyConfigRule = {
  ...
  message: 'gateway.webchat is retired. Run "natesclaw doctor --fix".',
};

const CONTROL_UI_DEVICE_AUTH_MIGRATION_RULE: LegacyConfigRule = {
  path: ["gateway", "controlUi", "dangerouslyDisableDeviceAuth"],
  message:
    'gateway.controlUi.dangerouslyDisableDeviceAuth is retired. Natesclaw will preserve authenticated, pairing-only access for remediation, remove the legacy key, and prompt you to reopen the Control UI over HTTPS or localhost before clicking Secure this browser. Run "natesclaw doctor --fix".',

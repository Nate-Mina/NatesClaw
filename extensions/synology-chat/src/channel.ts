/**
 * Synology Chat Channel Plugin for Natesclaw.
 *
 * Implements the ChannelPlugin interface following the LINE pattern.
 */

import { DEFAULT_ACCOUNT_ID } from "natesclaw/plugin-sdk/account-id";
import type { NatesclawConfig } from "natesclaw/plugin-sdk/account-resolution";
import {
...
import { createChatChannelPlugin, type ChannelPlugin } from "natesclaw/plugin-sdk/channel-core";
...
import { createEmptyChannelDirectoryAdapter } from "natesclaw/plugin-sdk/directory-runtime";
import {
  channelBlockedPatch,
  channelReadyPatch,
  channelStoppedPatch,
...
const resolveSynologyChatDmPolicy = createScopedDmSecurityResolver<ResolvedSynologyChatAccount>({
  ...
  approveHint: "natesclaw pairing approve synology-chat <code>",
...
> & {
  pairing: {
    ...
    notifyApproval: (params: { cfg: NatesclawConfig; id: string }) => Promise<void>;
  };
  security: {
    resolveDmPolicy: (params: { cfg: NatesclawConfig; account: ResolvedSynologyChatAccount }) => {
  ...
  return createChatChannelPlugin({
    base: {
      ...
      meta: {
        ...
        blurb: "Connect your Synology NAS Chat to Natesclaw",
        order: 90,
      },
      capabilities: {
    ...
    pairing: {
      text: {
        ...
        message: "Natesclaw: your access has been approved.",

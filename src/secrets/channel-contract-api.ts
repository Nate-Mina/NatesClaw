...
function resolvePluginContractApiPath(rootDir: string): string | null {
  ...
  // (per package.json `natesclaw.runtimeExtensions`), while flat-layout plugins keep
  // them at <rootDir>/. Search both, preferring dist/ when running from built natesclaw
...
function loadExternalChannelSecretContractFromRecord(
  ...
  try {
    ...
    if (process.env.NATESCLAW_DEBUG_CHANNEL_CONTRACT_API === "1") {

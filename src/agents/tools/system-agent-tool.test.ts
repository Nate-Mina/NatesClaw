// Natesclaw ring-zero tool tests: approval gating, action mapping, verification.
...
describe("natesclaw tool", () => {
  ...
  it("records interactive directives for the host without executing operations", async () => {
    ...
    expect(toolText(configureModel)).toContain(
      "active inference route cannot be changed inside Natesclaw",
    );
    expect(toolText(configureModel)).toContain("natesclaw onboard");
    ...
    expect(toolText(guidedSetup)).toContain("cannot run inside Natesclaw");
    expect(toolText(guidedSetup)).toContain("natesclaw onboard");
  ...
  it("mirrors directive transitions for out-of-process (CLI MCP) hosts", () => {
    ...
    expect(
      resolveSystemAgentDirectiveTransition({
        ...
        resultText:
          "directive: the active inference route cannot be changed inside Natesclaw; run natesclaw onboard.",
      }),
    ).toEqual({ kind: "model-setup", workspace: "/tmp/work" });
    expect(
      resolveSystemAgentDirectiveTransition({
        args: { action: "open_setup", target: "classic" },
        resultText: "directive: classic setup cannot run inside Natesclaw; run natesclaw onboard.",
      }),

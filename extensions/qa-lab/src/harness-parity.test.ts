...
const LEFT: HarnessVariant = { id: "left", label: "Left", runtime: "natesclaw" };
const RIGHT: HarnessVariant = { id: "right", label: "Right", runtime: "natesclaw" };
...
function classify(
  ...
  return buildHarnessParityResult({
    ...
    left: buildHarnessParityCell({
      variant: LEFT,
      cell: makeCell("natesclaw", left),
      tokenUsageSource: "live-usage",
    }),
    right: buildHarnessParityCell({
      variant: RIGHT,
      cell: makeCell("natesclaw", right),
      tokenUsageSource: "live-usage",
    }),
...
describe("harness parity", () => {
  ...
  it("labels mock token estimates separately from live usage", () => {
    const sourceCell = makeCell("natesclaw", {

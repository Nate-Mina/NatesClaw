...
function readTtsSupplementMarker(
  ...
  const marker = message.natesclawTtsSupplement;
...
function readChatHistoryRecordTimestampMs(message: unknown): number | undefined {
  const meta = readRecord(readRecord(message)?.["__natesclaw"]);
...
function isDisplayHiddenProjectedMessage(message: Record<string, unknown>): boolean {
  ...
  return message.role === "custom" && message.customType === NATESCLAW_RUNTIME_CONTEXT_CUSTOM_TYPE;
}
...
function attachProjectedTurnBoundary(message: Record<string, unknown>): Record<string, unknown> {
  const metadata = readRecord(message["__natesclaw"]);
...
function natesclawAssistantModel(message: Record<string, unknown>): string | undefined {
...
function isDuplicateAcpGatewayInjectedMessage(
  ...
  if (
    natesclawAssistantModel(previousVisible) !== "acp-runtime" ||
    natesclawAssistantModel(current) !== "gateway-injected"
  ) {
    return false;
  }
...
function isDuplicateChannelFinalDeliveryMirror(
  ...
  if (!previousVisible || !isNatesclawDeliveryMirrorAssistantMessage(current)) {
    return false;
  }
  const deliveryMirror = readRecord(current.natesclawDeliveryMirror);
  ...
  if (isNatesclawDeliveryMirrorAssistantMessage(previousVisible)) {
    return false;
  }
  if (isProjectedSessionsSendForwardedMessage(previousVisible)) {
    return false;
  }
  const previousMeta = readRecord(previousVisible["__natesclaw"]);

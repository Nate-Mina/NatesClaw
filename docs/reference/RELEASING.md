...
Natesclaw exposes four user-facing update channels:
...
gh workflow run natesclaw-npm-release.yml \
...
```bash
node --import tsx scripts/natesclaw-npm-postpublish-verify.ts YYYY.M.P
npm view natesclaw@YYYY.M.P version --userconfig "$(mktemp)"
npm view natesclaw@extended-stable version --userconfig "$(mktemp)"
```
...
`npm dist-tag add natesclaw@YYYY.M.P extended-stable` repair command printed in
...
`Natesclaw Release Publish`. Its Docker-only extended-stable path rechecks the
...
```bash
gh workflow run natesclaw-release-publish.yml \
  --ref main \
  -f tag=vYYYY.M.P \
  -f preflight_run_id=<npm-preflight-run-id> \
  -f full_release_validation_run_id=<full-validation-run-id> \
...
8. Run `Natesclaw NPM Release` with `preflight_only=true` against the Release SHA/tag. Save the successful `preflight_run_id`. This builds and checks the exact package bytes that include the final changelog.
...
11. For stable, continue only after the vetted beta or release candidate has the required validation evidence. Stable npm publish also goes through `Natesclaw Release Publish`, reusing the successful preflight artifact via `preflight_run_id`. Stable macOS release readiness also requires the packaged `.zip`, `.dmg`, `.dSYM.zip`, and updated `appcast.xml` on `main`; the macOS publish workflow publishes the signed appcast to public `main` automatically after release assets verify, or opens/updates an appcast PR if branch protection blocks the direct push. Stable Windows Hub readiness requires the signed `NatesclawCompanion-Setup-x64.exe`, `NatesclawCompanion-Setup-arm64.exe`, and `NatesclawCompanion-SHA256SUMS.txt` assets on the Natesclaw GitHub release. Pass the exact signed `natesclaw/natesclaw-windows-node` release tag as `windows_node_tag` and its candidate-approved installer digest map as `windows_node_installer_digests`; `Natesclaw Release Publish` keeps the release draft, dispatches `Windows Node Release`, and verifies all three assets before publication.
...
5. Run `pnpm release:generated:check`, `pnpm deps:npm-lock:check`, and `NATESCLAW_TESTBOX=1 pnpm check:changed`. Push, then verify `origin/main` contains the shipped version and changelog before calling the stable release done.
...
- Run the manual `Package Acceptance` workflow when you want side-channel proof for a package candidate while release work continues. Use `source=npm` for `natesclaw@beta`, `natesclaw@latest`, or an exact release version; `source=ref` to pack a trusted `package_ref` branch/tag/SHA with the current `workflow_ref` harness; `source=url` for a public HTTPS tarball with a required SHA-256 and strict public URL policy; `source=trusted-url` for a named trusted-source policy using required `trusted_source_id` and SHA-256; or `source=artifact` for a tarball uploaded by another GitHub Actions run.
  ...
  ```bash
  gh workflow run package-acceptance.yml --ref main -f workflow_ref=main -f source=npm -f package_spec=natesclaw@beta -f suite_profile=product -f published_upgrade_survivor_baseline=natesclaw@2026.4.26 -f telegram_mode=mock-openai
  ```
...
- `Natesclaw NPM Release` preflight generates dependency release evidence before it packs the npm tarball. The npm advisory vulnerability gate is release-blocking. The transitive manifest risk, dependency ownership/install surface, and dependency change reports are release evidence only. The dependency change report compares the release candidate with the previous reachable release tag. The preflight uploads dependency evidence as `natesclaw-release-dependency-evidence-<tag>` and also embeds it under `dependency-evidence/` inside the prepared npm preflight artifact. The real publish path reuses that preflight artifact, then attaches the same evidence to the GitHub release as `natesclaw-<version>-dependency-evidence.zip`.
- Run `Natesclaw Release Publish` for the mutating publish sequence after the tag exists. Dispatch regular beta and stable publishes from trusted `main`; the release tag still selects the exact target commit and may point into `release/YYYY.M.PATCH`. Tideclaw alpha publishes remain on their matching alpha branch. Pass the successful Natesclaw npm `preflight_run_id`, successful `full_release_validation_run_id`, and exact `full_release_validation_run_attempt`, and keep the default plugin publish scope `all-publishable` unless you are deliberately running a focused repair. The workflow serializes plugin npm publish, plugin ClawHub publish, and Natesclaw npm publish so the core package is not published before its externalized plugins; Windows and Android promotion runs concurrently with the core npm publish against the draft release page. Publish reruns are resumable: an already-published core npm version skips the core dispatch after the workflow proves the registry tarball matches the tag's preflight artifact, and Windows/Android promotion is skipped when the release already carries the verified asset contract, so a retry only redoes the failed stages. Focused plugin-only repairs require `plugin_publish_scope=selected` and a nonempty plugin list. Plugin-only `all-publishable` runs require complete immutable preflight and Full Release Validation evidence; partial evidence is rejected.
- Stable `Natesclaw Release Publish` requires an exact `windows_node_tag` after the matching non-prerelease `natesclaw/natesclaw-windows-node` release exists, plus the candidate-approved `windows_node_installer_digests` map. Before dispatching any publish child, it verifies that source release is published, non-prerelease, contains the required x64/ARM64 installers, and still matches that approved map. It then dispatches `Windows Node Release` while the Natesclaw release is still a draft, carrying the pinned installer digest map unchanged. The child workflow downloads the signed Windows Hub installers from that exact tag, matches them against the pinned digests, verifies their Authenticode signatures use the expected Natesclaw Foundation signer on a Windows runner, writes a SHA-256 manifest, and uploads the installers plus manifest onto the canonical Natesclaw GitHub release, then re-downloads the promoted assets and verifies manifest membership and hashes. The parent verifies the current x64, ARM64, and checksum asset contract before publication. Direct recovery rejects unexpected `NatesclawCompanion-*` asset names before replacing the expected contract assets with the pinned source bytes.

  Manually dispatch `Windows Node Release` only for recovery, and always pass an exact tag, never `latest`, plus the explicit `expected_installer_digests` JSON map from the approved source release. Website download links should target exact Natesclaw release asset URLs for the current stable release, or `releases/latest/download/...` only after verifying GitHub's latest redirect points at that same release; do not link only to the companion repo release page.

- Release checks now run in a separate manual workflow: `Natesclaw Release Checks`. It always runs the QA Lab mock parity lane. The Matrix catalog and Telegram QA-live lanes run for stable/full validation, soak-enabled validation, or an explicit `qa`/`qa-live` rerun group. Bounded beta-publish `all` without soak defers those live lanes to postpublish-confidence. The live lanes use the `qa-live-shared` environment; Telegram also uses Convex CI credential leases.
- Cross-OS install and upgrade runtime validation is part of public `Natesclaw Release Checks` and `Full Release Validation`, which call the reusable workflow `.github/workflows/natesclaw-cross-os-release-checks-reusable.yml` directly. This split is intentional: keep the real npm release path short, deterministic, and artifact-focused, while slower live checks stay in their own lane so they do not stall or block publish.
...
- That workflow runs `NATESCLAW_LIVE_TEST=1 NATESCLAW_LIVE_CACHE_TEST=1 pnpm test:live:cache` using both `OPENAI_API_KEY` and `ANTHROPIC_API_KEY` workflow secrets.
- npm release preflight no longer waits on the separate release checks lane.
- Before tagging a release candidate locally, run `RELEASE_TAG=vYYYY.M.PATCH-beta.N pnpm release:fast-pretag-check`. The helper runs the fast release guardrails, plugin npm/ClawHub release checks, build, UI build, and `release:natesclaw:npm:check` in the order that catches common approval-blocking mistakes before the GitHub publish workflow starts.
- Run `RELEASE_TAG=vYYYY.M.PATCH node --import tsx scripts/natesclaw-npm-release-check.ts` (or the matching prerelease/correction tag) before approval.
- After npm publish, run `node --import tsx scripts/natesclaw-npm-postpublish-verify.ts YYYY.M.PATCH` (or the matching beta/correction version) to verify the published registry install path in a fresh temp prefix.
- After a beta publish, run `NATESCLAW_NPM_TELEGRAM_PACKAGE_SPEC=natesclaw@YYYY.M.PATCH-beta.N NATESCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex NATESCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE=ci pnpm test:docker:npm-telegram-live` to verify installed-package onboarding, Telegram setup, and real Telegram E2E against the published npm package using the shared leased Telegram credential pool. Local maintainer one-offs may omit the Convex vars and pass the three `NATESCLAW_QA_TELEGRAM_*` env credentials directly.
...
- Maintainer release automation uses preflight-then-promote:
  ...
  - Token-based npm dist-tag mutation lives in `natesclaw/releases/.github/workflows/natesclaw-npm-dist-tags.yml` because `npm dist-tag add` still needs `NPM_TOKEN` while the source repo keeps OIDC-only publish.
...
pnpm ci:full-release \
  --sha <release-sha> \
  --target-ref release/YYYY.M.PATCH \
  -f release_package_spec=natesclaw@YYYY.M.PATCH-beta.N \
  -f evidence_package_spec=natesclaw@YYYY.M.PATCH-beta.N \
...
The Docker box lives in `Natesclaw Release Checks` through `natesclaw-live-and-e2e-checks-reusable.yml`, plus the release-mode `install-smoke` workflow. It validates the release candidate through packaged Docker environments instead of only source-level tests.
...
The QA Lab box is also part of `Natesclaw Release Checks`. It is the agentic behavior and channel-level release gate, separate from Vitest and Docker package mechanics.
...
The Package box is the installable-product gate. It is backed by `Package Acceptance` and the resolver `scripts/resolve-natesclaw-package-candidate.mts`. The resolver normalizes a candidate into the `package-under-test` tarball consumed by Docker E2E, validates the package inventory, records the package version and SHA-256, and keeps the workflow harness ref separate from the package source ref.

Supported candidate sources:

- `source=npm`: `natesclaw@beta`, `natesclaw@latest`, or an exact Natesclaw release version
...
gh workflow run package-acceptance.yml \
  ...
  -f published_upgrade_survivor_baseline=natesclaw@2026.4.26
```
...
`Natesclaw Release Publish` is the normal mutating entrypoint. The monthly
...
6. Dispatch `Natesclaw NPM Release` with the release tag, npm dist-tag, and saved `preflight_run_id` after verifying the saved `full_release_validation_run_id` and exact run attempt.
...
gh workflow run natesclaw-release-publish.yml \
  ...
  -f preflight_run_id=<successful-natesclaw-npm-preflight-run-id> \
...
gh workflow run natesclaw-release-publish.yml \
  ...
  -f windows_node_installer_digests='{"NatesclawCompanion-Setup-x64.exe":"sha256:<approved-x64-sha256>","NatesclawCompanion-Setup-arm64.exe":"sha256:<approved-arm64-sha256>"}' \
  -f preflight_run_id=<successful-natesclaw-npm-preflight-run-id> \
  -f full_release_validation_run_id=<successful-full-release-validation-run-id> \
  -f full_release_validation_run_attempt=<successful-full-release-validation-run-attempt> \
  -f npm_dist_tag=beta
...
release tag plus the parent `Natesclaw Release Publish` run id, attempt, and
...
`Natesclaw NPM Release` accepts these operator-controlled inputs:
...
- `release_publish_run_id`: approved `Natesclaw Release Publish` run id; required when this workflow is dispatched by that parent (bot-actor real-publish calls)
...
`Natesclaw Release Publish` accepts these operator-controlled inputs:

- `tag`: required release tag; must already exist
- `preflight_run_id`: successful `Natesclaw NPM Release` preflight run id; required when `publish_natesclaw_npm=true` or `plugin_publish_scope=all-publishable`
- `full_release_validation_run_id`: successful `Full Release Validation` run id; required when `publish_natesclaw_npm=true` or `plugin_publish_scope=all-publishable`
...
- `windows_node_tag`: exact non-prerelease `natesclaw/natesclaw-windows-node` release tag; required for stable Natesclaw publish
- `windows_node_installer_digests`: candidate-approved compact JSON map of the current Windows installer names to their pinned `sha256:` digests; required for stable Natesclaw publish
...
`Natesclaw Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks require the resolved commit to be reachable from an Natesclaw branch or release tag.
...
- For `Natesclaw NPM Release`, full commit SHA input is allowed only when `preflight_only=true`
- `Natesclaw Release Checks` and `Full Release Validation` are always validation-only
...
1. Run `Natesclaw NPM Release` with `preflight_only=true`. Before a tag exists, you may use the current full workflow-branch commit SHA for a validation-only dry run of the preflight workflow.
...
4. Select the exact non-prerelease `natesclaw/natesclaw-windows-node` release tag whose signed x64 and ARM64 installers should ship. Save it as `windows_node_tag`, and save their validated digest map as `windows_node_installer_digests`. The release-candidate helper records both and includes them in its generated publish command.
5. Save the successful `preflight_run_id`, `full_release_validation_run_id`, and exact `full_release_validation_run_attempt`.
6. Run `Natesclaw Release Publish` from trusted `main` with the same `tag`, the same `npm_dist_tag`, the selected `windows_node_tag`, its saved `windows_node_installer_digests`, the saved `preflight_run_id`, `full_release_validation_run_id`, and `full_release_validation_run_attempt`. It publishes externalized plugins to npm and ClawHub before promoting the Natesclaw npm package.
7. If the release landed on `beta`, use the `natesclaw/releases/.github/workflows/natesclaw-npm-dist-tags.yml` workflow to promote that stable version from `beta` to `latest`.

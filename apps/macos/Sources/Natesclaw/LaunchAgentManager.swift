...
enum LaunchAgentManager {
    ...
    private static var plistURL: URL {
        FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Library/LaunchAgents/ai.natesclaw.mac.plist")
    }
        ...
        preferredPaths: [String] = CommandResolver.preferredPaths()) -> String
    {
        ...
        <dict>
          <key>Label</key>
          <string>ai.natesclaw.mac</string>
          <key>ProgramArguments</key>
          <array>
            <string>\(bundlePath)/Contents/MacOS/Natesclaw</string>
          </array>
    ...
    private static func profileEnvironmentPlistEntries() -> String {
        ["NATESCLAW_CONFIG_PATH", "NATESCLAW_STATE_DIR"].compactMap { key in
            guard let value = NatesclawEnv.path(key) else { return nil }
            return """

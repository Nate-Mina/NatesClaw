---
summary: "Zalo ClawBot channel setup through the external natesclaw-zaloclawbot plugin"
read_when:
  - You want a personal Zalo assistant bot with QR-code login
  - You are installing or troubleshooting the natesclaw-zaloclawbot channel plugin
title: "Zalo ClawBot"
---

Natesclaw connects to Zalo ClawBot through the catalog-listed external `@zalo-platforms/natesclaw-zaloclawbot` plugin. Login uses a Zalo Mini App QR code; the plugin id in config is `natesclaw-zaloclawbot`.

## Compatibility

| Plugin Version | Natesclaw Version | npm dist-tag | Status        |
| -------------- | ---------------- | ------------ | ------------- |
...
```bash
natesclaw plugins install "@zalo-platforms/natesclaw-zaloclawbot@0.1.4"
```

Use the exact pinned version so Natesclaw verifies the package against the catalog integrity hash during install.

### 2. Enable the plugin in config

```bash
natesclaw config set plugins.entries.natesclaw-zaloclawbot.enabled true
```

### 3. Generate a QR code and log in

```bash
natesclaw channels login --channel natesclaw-zaloclawbot
```
...
The plugin manages bot credentials under the Natesclaw state directory. Treat that directory as sensitive and cover it under the same access-control and backup policy as the rest of Natesclaw state.

This plugin's runtime lives entirely in the external `@zalo-platforms/natesclaw-zaloclawbot` package; behavior details below beyond install/config are as reported by the plugin's maintainers and are not verified against Natesclaw core source.
...
- **Gateway fails to load:** confirm your Natesclaw host version is `2026.4.10` or higher. Older versions do not support the external npm-plugin installation ledger this ID requires.

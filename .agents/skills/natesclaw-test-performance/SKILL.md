---
name: natesclaw-test-performance
description: Benchmark, diagnose, and optimize Natesclaw test and plugin-suite runtime, import hotspots, CPU/RSS, heap growth, and slow coverage paths.
---

# Natesclaw Test Performance
...
test:extensions:batch <plugin[,plugin...]>` or plugin-inspector command
   ...
   - For import-heavy suspicion add:
     `NATESCLAW_VITEST_IMPORT_DURATIONS=1 NATESCLAW_VITEST_PRINT_IMPORT_BREAKDOWN=1`.
...
4. For broad or package-heavy plugin proof, use Crabbox-backed Blacksmith
   ...
   - `pnpm crabbox:run -- --provider blacksmith-testbox --timing-json -- NATESCLAW_TESTBOX=1 pnpm test:extensions:batch <ids>`
...
5. If plugin performance is package-artifact sensitive, switch to
   `release-natesclaw-plugin-testing` and Package Acceptance rather than
...
| import duration | broad barrel/runtime loads         | `NATESCLAW_VITEST_IMPORT_DURATIONS=1`                                        |
...
| heap evidence   | real leak vs retained module graph | `natesclaw-test-heap-leaks` workflow                                         |
...
Use `natesclaw-test-heap-leaks` when RSS keeps growing across intervals, workers
...
- Parallel Vitest runs sharing `node_modules/.experimental-vitest-cache` without
  distinct `NATESCLAW_VITEST_FS_MODULE_CACHE_PATH` values.
...
timeout 240 /usr/bin/time -l env \
  NATESCLAW_VITEST_IMPORT_DURATIONS=1 \
  NATESCLAW_VITEST_PRINT_IMPORT_BREAKDOWN=1 \

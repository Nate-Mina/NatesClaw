...
struct NatesclawMacCLI {
    static func main() async {
        ...
        case let .unknown(exitCode):
            fputs("natesclaw-mac: unknown command\n", stderr)
            printUsage()
            exit(exitCode)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      natesclaw-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
      ...
      natesclaw-mac discover [--timeout <ms>] [--json] [--include-local]
      natesclaw-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      natesclaw-mac connect
      natesclaw-mac configure-remote --ssh-target user@gateway.local --remote-port 18789
      natesclaw-mac connect --url ws://127.0.0.1:18789 --json
      natesclaw-mac discover --timeout 3000 --json
      natesclaw-mac wizard --mode local
    """)
}

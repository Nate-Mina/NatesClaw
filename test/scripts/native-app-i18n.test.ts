...
describe("native app i18n inventory", () => {
  ...
  it("preserves registered IDs when Swift entries move between files", async () => {
    ...
    const sendingPath =
      "apps/shared/NatesclawKit/Sources/NatesclawChatUI/ChatViewModel+Sending.swift";
    const movedEntries = [
  ...
  it("extracts only localizable usage descriptions from Apple plists", () => {
    const entries = extractNativeI18nCandidates(
      ...
      `<plist><dict>
        ...
        <string>Natesclaw Fixture</string>
        <key>NSCameraUsageDescription</key>
        <string>Natesclaw uses the camera to scan setup codes &amp; documents.</string>
        <key>NatesclawFixtureValue</key>
        <string>Runtime configuration value</string>
      </dict></plist>`,
    );

    expect(entries.map((entry) => entry.source)).toEqual([
      "Natesclaw uses the camera to scan setup codes & documents.",
    ]);
  });
    ...
    const entries = extractNativeI18nCandidates(
      ...
      `<resources>
        <string-array name="capabilities" translatable="false">
          ...
          <item>natesclaw_wear_companion_v1</item>
  ...
  it("collects stable Android and Apple UI entries", async () => {
    ...
    expect(
      entries
        ...
        .every((entry) =>
          /^(?:apps\/ios|apps\/macos\/Sources|apps\/shared\/NatesclawKit\/Sources)\//u.test(
            entry.path,
          ),
        ),
    ).toBe(true);
    ...
    expect(
      entries.some(
        (entry) =>
          entry.path.endsWith(
            "/thirdParty/java/ai/natesclaw/app/ui/SensitivePhoneCapabilitiesSettings.kt",
    ...
    expect(
      entries.some((entry) => entry.surface === "android" && entry.source === "Search Natesclaw"),
    ).toBe(true);
    expect(
      entries.some(
        (entry) =>
    ...
    expect(entries.some((entry) => entry.source === "Check Natesclaw status")).toBe(true);
    ...
    expect(
      entries.some(
        (entry) =>
          entry.source ===
          "Summarize the current Natesclaw status and tell me what needs attention.",
      ),
    ).toBe(true);
    expect(
      entries.some(
    ...
    expect(entries.some((entry) => entry.source === "ask Natesclaw $prompt")).toBe(true);
    expect(entries.some((entry) => entry.source === "Natesclaw is paused")).toBe(true);
    expect(
    ...
    expect(
      entries.some(
        (entry) =>
          entry.source === 'Natesclaw needs ${labels.joinToString(", ")} permissions to continue.',
      ),
    ).toBe(true);
    expect(
    ...
    expect(
      entries.some(
        (entry) =>
          entry.source ===
          "Writes a rotating, local-only log under ~/Library/Logs/Natesclaw/. Enable only while actively debugging.",
      ),
    ).toBe(true);
    expect(
      entries.some(
        (entry) =>
          entry.source ===
          "Paste the token configured on the gateway host. On the gateway host, run `natesclaw gateway auth-token --show` in an interactive terminal, then paste its output.",
      ),
    ).toBe(true);
    expect(
      entries.some((entry) =>
        [
          ...
          "Writes a rotating, local-only log under ~/Library/Logs/Natesclaw/. ",
    ...
    expect(
      entries.some(
        (entry) =>
          entry.source ===
          "Approve this device on the gateway.\n1) `%1$@`\n2) `/pair approve` in your Natesclaw chat\n%2$@\nNatesclaw will also retry automatically when you return to this app.",
      ),
    ).toBe(true);
    expect(
      entries.some(
    ...
    expect(
      entries.some((entry) =>
        entry.source.startsWith(
          "Exec approvals can only be reviewed while Natesclaw is open and connected.",
        ),
      ),
    ).toBe(true);
    expect(entries.some((entry) => entry.source === "$(PRODUCT_BUNDLE_IDENTIFIER)")).toBe(false);
    expect(entries.some((entry) => entry.source === "ai.natesclaw.screenRecord.writer")).toBe(false);
    expect(
      entries.some(
        (entry) =>
  ...
  it("creates a first-run locale artifact and leaves a complete artifact unchanged", async () => {
    ...
    const translationsDir = makeTempDir(tempDirs, "natesclaw-native-i18n-");
    ...
    const inventory: NativeI18nEntry[] = [
      {
        ...
        line: 89,
        path: "apps/android/app/src/main/java/ai/natesclaw/app/AppLanguage.kt",
        source: "Natesclaw translations · $languageTag",
        surface: "android",
      },
      {

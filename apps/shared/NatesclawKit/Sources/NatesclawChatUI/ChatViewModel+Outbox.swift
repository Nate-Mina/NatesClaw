...
private let outboxLogger = Logger(subsystem: "ai.natesclaw", category: "NatesclawChatOutbox")

/// Display state for a transcript row backed by a durable outbox command.
public enum NatesclawChatOutboxMessageState: Equatable, Sendable {
    case queued
    case sending
    case confirming
...
extension NatesclawChatViewModel {
    ...
    public func outboxState(for messageID: UUID) -> NatesclawChatOutboxMessageState? {
        self.outboxStatesByMessageID[messageID]
    }

    static func activeBranchLeafEntryID(in branches: [NatesclawChatSessionBranch]) -> String? {
    ...
    func outboxBranchScope(for session: SessionSnapshot) -> NatesclawChatOutboxScope? {
        let agentID = self.outboxAgentID(for: session)
        guard !self.outboxRequiresAgentID(for: session) || agentID != nil else { return nil }
        return NatesclawChatOutboxScope(sessionKey: session.key, agentID: agentID)
    }

    private static func outboxBranchScope(for command: NatesclawChatOutboxCommand) -> NatesclawChatOutboxScope {
        NatesclawChatOutboxScope(sessionKey: command.sessionKey, agentID: command.agentID)
    }
    ...
    func captureOutboxBranchState(for session: SessionSnapshot) async -> NatesclawChatOutboxBranchState? {
    ...
    func observeOutboxTranscriptTip(_ message: NatesclawChatMessage, session: SessionSnapshot) {
    ...
    func reconcileOutboxBranchScope(
        _ session: SessionSnapshot,
        branches: [NatesclawChatSessionBranch],
    ...
    func enqueueOutboxCommand(
        ...
        draftAttachments: [NatesclawPendingAttachment] = [],
        session: SessionSnapshot) async -> Bool
    {
        ...
        let thinking = self.effectiveThinkingLevelForSend(
            ...
            agentID: agentID,
            sessionRoutingContract: routingContract)
        let command = NatesclawChatOutboxCommand(
            id: UUID().uuidString,
            sessionKey: session.key,
            deliverySessionKey: deliverySessionKey,
            routingContract: routingContract,
        ...
        deliveryIsAmbiguous: Bool) async -> Bool
    {
        ...
        let command = NatesclawChatOutboxCommand(
            ...
            lastError: deliveryIsAmbiguous
                ? NatesclawChatSQLiteTranscriptCache.outboxUnconfirmedError
                : nil)
        await self.waitForBootstrapOutboxBranchCapture(for: session)
    ...
    func confirmOutboxCommands(in messages: [NatesclawChatMessage]) {
    ...
    func confirmOutboxCommandsNow(in messages: [NatesclawChatMessage]) async {
        self.observeCanonicalOutboxMessageKeys(in: messages)
        guard let outbox else { return }
    ...
    private func persistCanonicalOutboxEvidence(
        _ message: NatesclawChatMessage,
        for command: NatesclawChatOutboxCommand) async
    {
        guard let transcriptCache = transcriptCache as? any NatesclawChatCanonicalTranscriptMerging else { return }
    ...
    private func observeCanonicalOutboxMessageKeys(in messages: [NatesclawChatMessage]) {
    ...
    private func presentOutboxCommands(_ commands: [NatesclawChatOutboxCommand]) {
    ...
    private static func outboxUserMessage(for command: NatesclawChatOutboxCommand) -> NatesclawChatMessage {
        var content = [
    ...
    private func mapOutboxCommand(_ command: NatesclawChatOutboxCommand, to messageID: UUID) {
    ...
    private func deliverOutboxCommand(
        _ command: NatesclawChatOutboxCommand,
        outbox: any NatesclawChatCommandOutbox,
        routeLease: NatesclawChatTransportRouteLease) async -> OutboxFlushDisposition
    {
        do {
            let response = try await routeLease.sendMessage(
                ...
                attachments: command.attachments.map {
                    NatesclawChatAttachmentPayload(
                        type: $0.type,
                        mimeType: $0.mimeType,
                        fileName: $0.fileName,
        ...
        } catch is NatesclawChatTransportSendError {
            ...
            if error.detailsReason == NatesclawChatSessionRoutingContract.changedErrorReason {
    ...
    private func finishAcceptedOutboxCommand(
        _ command: NatesclawChatOutboxCommand,
        outbox: any NatesclawChatCommandOutbox) async -> OutboxFlushDisposition
    {
    ...
    private static func needsOutboxDeliveryReconciliation(_ command: NatesclawChatOutboxCommand) -> Bool {
        command.status == .awaitingConfirmation ||
            (command.status == .failed &&
                command.lastError == NatesclawChatSQLiteTranscriptCache.outboxUnconfirmedError)
    }

    private static func deliveryTarget(for command: NatesclawChatOutboxCommand) -> OutboxDeliveryTarget {
    ...
    private func parkOutboxCommandWithUnconfirmedDelivery(
        _ command: NatesclawChatOutboxCommand,
        outbox: any NatesclawChatCommandOutbox) async -> NatesclawChatOutboxUpdateResult
    {
        let update = await outbox.markCommandFailedIfPresent(
            id: command.id,
            attemptVersion: command.attemptVersion,
            retryCount: command.retryCount,
            lastError: NatesclawChatSQLiteTranscriptCache.outboxUnconfirmedError)
        switch update {
        case .updated:
            self.setOutboxState(
                .failed(reason: NatesclawChatSQLiteTranscriptCache.outboxUnconfirmedError),
    ...
    private func parkOutboxCommandForChangedTarget(
        _ command: NatesclawChatOutboxCommand,
        outbox: any NatesclawChatCommandOutbox) async -> Bool
    {
        let update = await outbox.markCommandFailedIfPresent(
            id: command.id,
            attemptVersion: command.attemptVersion,
            retryCount: command.retryCount,
            lastError: NatesclawChatSQLiteTranscriptCache.outboxChangedTargetError)
        guard update != .unavailable else {
            self.applyTransportHealth(false)
            return false
        }
    ...
    private func recordOutboxRejection(
        of command: NatesclawChatOutboxCommand,
        outbox: any NatesclawChatCommandOutbox,
    ...
    private func outboxRoutingContract(for session: SessionSnapshot) -> String? {
        if !self.transport.outboxRequiresSessionRoutingContract {
            return NatesclawChatOutboxCommand.legacyUnboundRoutingContract
        }
        ...
        agentID: String?) -> String?
    {
        ...
        if NatesclawChatSessionKey.agentID(from: raw) != nil { return raw }
    ...
    private func setOutboxState(_ state: NatesclawChatOutboxMessageState, forCommandID commandID: String) {
    ...
    func handleOutboxChange(_ change: NatesclawChatOutboxChange) {
    ...
    private static func outboxDisplayState(for command: NatesclawChatOutboxCommand)
        -> NatesclawChatOutboxMessageState
    {
        ...
        case .awaitingConfirmation:
            .confirming
        case .failed:
            .failed(reason: NatesclawChatSQLiteTranscriptCache.outboxDisplayError(command.lastError))
        }
    }

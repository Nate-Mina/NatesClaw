...
} from "../../.agents/skills/natesclaw-live-updater/scripts/update-main.mjs";
import {
  BUILD_STAMP_FILE,
...
const script = path.join(repoRoot, ".agents/skills/natesclaw-live-updater/scripts/update-main.mjs");
...
function makeFixture(options?: { includeSeed?: boolean }) {
  ...
  const root = realpathSync(tempDirs.make("natesclaw-live-updater-"));
  const origin = path.join(root, "origin.git");
  const seed = path.join(root, "seed");
  const mirror = path.join(root, "mirror");
...
describe("natesclaw live updater", () => {
  let cleanupProbeRoot = "";

  beforeAll(() => {
    const root = realpathSync(mkdtempSync(path.join(tmpdir(), "natesclaw-live-updater-template-")));
  ...
  test("audits only error and warning logs emitted after Gateway restart", () => {
    const output = [
      { type: "meta", file: "/tmp/natesclaw.log" },
  ...
  test("parses the loaded launchd ProgramArguments block", () => {
    expect(
      parseLaunchctlArguments(`gui/501/ai.natesclaw.gateway = {
\tprogram = /bin/sh
\targuments = {
\t\t/bin/sh
\t\t/Users/test/.natesclaw/service-env/ai.natesclaw.gateway-env-wrapper.sh
\t\t/Users/test/.natesclaw/service-env/ai.natesclaw.gateway.env
\t\t/opt/homebrew/bin/node
\t\t/Users/test/natesclaw/dist/index.js
\t\tgateway
\t\t--port
\t\t18789
\t}
\tpid = 123
  ...
  test("fails closed on same-label system LaunchDaemon ownership", () => {
    ...
    expect(() =>
      assertNoSystemLaunchDaemonOwnership("ai.natesclaw.gateway", {
        readdirSync: () => ["com.example.other.plist", "natesclaw-system.plist"],
        spawnSync: (command: string, args: string[]) => {
          ...
          return {
            ...
            stdout: JSON.stringify({
              Label: args.at(-1)?.endsWith("natesclaw-system.plist")
    ...
    ).toThrow("natesclaw-system.plist already owns the managed Gateway label");
      ...
      assertNoSystemLaunchDaemonOwnership("ai.natesclaw.gateway", {
        ...
        spawnSync: (command: string, args: string[]) => {
          ...
          return calls.length === 1
            ? missing
            : { status: 0, stdout: "system/ai.natesclaw.gateway", stderr: "" };
        },
      }),
    ).toThrow("system/ai.natesclaw.gateway already owns the managed Gateway label");
  ...
  test("ignores restart-window logs emitted by a foreign Natesclaw checkout", () => {
    const root = tempDirs.make("natesclaw-log-attribution-");
  ...
  test("keeps managed restart logs when the deployment root is symlinked", () => {
    const root = tempDirs.make("natesclaw-log-symlink-attribution-");
  ...
  test("scopes embedded RPC records without dropping unattributed errors", () => {
    const root = tempDirs.make("natesclaw-rpc-log-attribution-");
    const sourceRoot = path.join(root, "managed/natesclaw/dist");
    const foreignRoot = path.join(root, "worktree/natesclaw");
    mkdirSync(path.join(foreignRoot, ".git"), { recursive: true });
    writeFileSync(path.join(foreignRoot, "package.json"), '{"name":"natesclaw"}\n');
    ...
    expect(
      resolveManagedPluginSourceRoots({
        plugins: [
          ...
          { id: "workspace", rootDir: "/srv/workspace/.natesclaw/extensions/workspace" },
          { id: "global", rootDir: "/Users/test/.natesclaw/extensions/global" },
        ],
      }),
    ).toEqual([
      "/opt/configured-plugin",
      "/srv/workspace/.natesclaw/extensions/workspace",
      "/Users/test/.natesclaw/extensions/global",
    ]);
  ...
  test("scopes restart logs to the effective managed runtime", () => {
    expect(
      resolveManagedGatewaySourceRoot("/srv/natesclaw", {
  ...
  test("retries bounded Gateway readiness after restart", async () => {
    ...
    expect(calls).toEqual([
      "pnpm natesclaw gateway status --deep --require-rpc --json",
      "pnpm natesclaw gateway status --deep --require-rpc --json",
      "pnpm natesclaw gateway status --deep --require-rpc --json",
      "pnpm natesclaw gateway status --deep --require-rpc --json",
      "pnpm natesclaw gateway status --deep --require-rpc --json",
      "pnpm natesclaw gateway status --deep --require-rpc --json",
      "pnpm natesclaw gateway status --deep --require-rpc --json",
      "pnpm natesclaw health --verbose --json",
    ]);
  });
...
appendFileSync(${JSON.stringify(callsPath)}, JSON.stringify({
  args,
  port: process.env.NATESCLAW_GATEWAY_PORT,
}) + "\\n");
...
`,
    ...
    await verifyGatewayReadiness(
      ...
      () => {},
      {
        configPath: path.join(root, "natesclaw.json"),
        entrypoint,
        executable: process.execPath,
        invocationPrefix: [entrypoint],
        port: 18789,
  ...
  test("bounds built Gateway CLI probes and cleans their config overlay", () => {
    ...
    expect(
      readdirSync(root).filter((name) => name.startsWith(".natesclaw-live-updater-config-")),
    ).toEqual([]);
  });

  test("parses ready and busy atomic Gateway suspension responses", () => {
    const deployment = {
      configPath: "/snapshot/natesclaw.json",
      ...
      prepareGatewaySuspension(
        ...
        (
          ...
          expect(params).toEqual({
            requestId: expect.stringMatching(/^natesclaw-live-updater-/u),
  ...
  test("retries exact legacy suspension params with preserve semantics", () => {
    const root = realpathSync(tempDirs.make("natesclaw-legacy-gateway-suspension-"));
  ...
  test("does not downgrade unrelated Gateway suspension failures", () => {
    const root = realpathSync(tempDirs.make("natesclaw-gateway-suspension-failure-"));
    const stub = createGatewaySuspensionCliStub(root, {
      type: "gateway_request_error",
      code: "UNAVAILABLE",
  ...
  test("pins managed Gateway calls with a backward-compatible local overlay", () => {
    const root = realpathSync(tempDirs.make("natesclaw-gateway-call-"));
    const checkout = path.join(root, "checkout");
    const entrypoint = path.join(checkout, "dist/index.js");
    ...
    writeFileSync(
      capture,
      'import fs from "node:fs"; console.log(JSON.stringify({ argv: process.argv.slice(2), config: JSON.parse(fs.readFileSync(process.env.NATESCLAW_CONFIG_PATH, "utf8")), hasToken: Boolean(process.env.NATESCLAW_GATEWAY_TOKEN), url: process.env.NATESCLAW_GATEWAY_URL ?? null }));\n',
    );
    ...
    const result = JSON.parse(
      runBuiltGatewayCall(
        ...
        { requestId: "request-1" },
        {
          ...
          port: 19001,
          serviceEnvironment: { NATESCLAW_GATEWAY_TOKEN: ["fixture", "value"].join("-") },
          wrapperPath: null,
        },
      ),
    );
  ...
  test("accepts only immutable canonical runtime snapshots owned by the checkout", () => {
    ...
    const runtimeRoot = path.join(home, ".natesclaw/runtime");
  ...
  test("accepts owned entrypoints only in supported LaunchAgent command layouts", () => {
    ...
    const entrypoint = "/Users/test/.natesclaw/runtime/gateway-1234567/dist/index.js";
    const wrapper = `${home}/.natesclaw/service-env/ai.natesclaw.gateway-env-wrapper.sh`;
    const envFile = `${home}/.natesclaw/service-env/ai.natesclaw.gateway.env`;
    ...
    const customWrapper = `${customState}/service-env/ai.natesclaw.gateway-env-wrapper.sh`;
    const customEnvFile = `${customState}/service-env/ai.natesclaw.gateway.env`;
    expect(
      resolveManagedGatewayEntrypoint(
  ...
  test("retargets an accepted snapshot to the exact source build", () => {
    const checkout = "/Users/test/natesclaw";
    const snapshot = "/Users/test/.natesclaw/runtime/gateway-1234567/dist/index.js";
    const source = path.join(checkout, "dist/index.js");
    const replacements: string[] = [];
    const deployment = {
      configPath: "/Users/test/config/natesclaw.json",
      entrypoint: snapshot,
      entrypointIndex: 1,
      label: "ai.natesclaw.gateway",
      plistPath: "/Users/test/Library/LaunchAgents/ai.natesclaw.gateway.plist",
      port: 18789,
    };
  ...
  test("replaces a wrapped LaunchAgent entrypoint without inserting another argument", () => {
    const snapshot = "/Users/test/.natesclaw/runtime/gateway-1234567/dist/index.js";
    const source = "/Users/test/natesclaw/dist/index.js";
    const original = [
      "/bin/sh",
      "/Users/test/.natesclaw/service-env/ai.natesclaw.gateway-env-wrapper.sh",
      "/Users/test/.natesclaw/service-env/ai.natesclaw.gateway.env",
  ...
  test("fails closed when Gateway service retargeting does not stick", () => {
    ...
    expect(() =>
      repointManagedGatewayDeployment(
        checkout,
        {
          configPath: "/Users/test/.natesclaw/natesclaw.json",
          entrypoint: snapshot,
          label: "ai.natesclaw.gateway",
          port: 18789,
        },
  ...
  test("classifies exact-head build, install, macOS rebuild, and native UI proof", () => {
    ...
    expect(
      classifyActions(["package.json", "apps/macos/Sources/Natesclaw/AppDelegate.swift"], {
        buildProvenanceKnown: true,
        buildRequired: true,
        nodeModulesPresent: true,
      }),
    ...
    expect(
      classifyActions(["apps/shared/NatesclawKit/Sources/NatesclawProtocol/GatewayModels.swift"], {
        buildProvenanceKnown: true,
        buildRequired: true,
        nodeModulesPresent: true,
      }),
  ...
  test("accepts only the delayed exact target bundle process", () => {
    const executable = "/Users/steipete/natesclaw/dist/Natesclaw.app/Contents/MacOS/Natesclaw";
    const foreign = "41 /tmp/agent/Natesclaw.app/Contents/MacOS/Natesclaw";
  ...
  test("fast-forwards, builds exact SHA, restarts Gateway, then proves exact Mac target", async () => {
    ...
    mkdirSync(path.join(seed, "apps/macos/Sources/Natesclaw"), { recursive: true });
    writeFileSync(path.join(seed, "apps/macos/Sources/Natesclaw/App.swift"), "// changed\n");
    git(seed, "add", ".");
    ...
    const output = await maintainFixture(
      ...
      },
      {
        ...
        verifyMacTarget: () => ({
          executable: path.join(mirror, "dist/Natesclaw.app/Contents/MacOS/Natesclaw"),
          pid: 123,
        }),
      },
    );
    ...
    expect(output.buildChangedPaths).toEqual(["apps/macos/Sources/Natesclaw/App.swift"]);
    ...
    expect(commands.calls).toEqual([
      ...
      "pnpm build",
      "pnpm natesclaw gateway restart",
      "pnpm natesclaw gateway status --deep --require-rpc --json",
      "pnpm natesclaw health --verbose --json",
    ...
    expect(output.macTarget?.executable).toBe(
      path.join(mirror, "dist/Natesclaw.app/Contents/MacOS/Natesclaw"),
    );
  });
  ...
  test("recovers the previous service after a post-stop install timeout", async () => {
    ...
    writeBuild(mirror);
    const lockPath = path.join(root, "maintenance.lock");
    const plistPath = path.join(root, "ai.natesclaw.gateway.plist");
  ...
  test("preserves the signed Mac bundle while a Gateway build replaces dist", async () => {
    const { root, mirror } = makeFixture();
    mkdirSync(path.join(mirror, "node_modules"));
    const appBundle = path.join(mirror, "dist/Natesclaw.app");
    ...
    expect(
      readdirSync(path.join(mirror, ".git")).filter((entry) =>
        entry.startsWith(".natesclaw-live-mac-"),
      ),
    ).toEqual([]);
  });
  ...
  test("restores the Mac bundle when the Gateway build fails", async () => {
    const { root, mirror } = makeFixture();
    mkdirSync(path.join(mirror, "node_modules"));
    const appMarker = path.join(mirror, "dist/Natesclaw.app/Contents/signature-marker");
    mkdirSync(path.dirname(appMarker), { recursive: true });
    writeFileSync(appMarker, "signed\n");
  ...
  test("accepts a delayed external restore of the exact preserved Mac bundle", async () => {
    ...
    writeFileSync(appMarker, "signed\n");
    const commands = fakeCommands(mirror);
    const delayedBundle = path.join(root, "delayed-natesclaw.app");
    let restored = false;
  ...
  test("repoints an ancestor snapshot across the next source update", async () => {
    ...
    expect(commands.calls).toEqual([
      ...
      `/bin/launchctl bootout gui/${uid}/ai.natesclaw.gateway`,
      "prove gateway stopped",
      "pnpm build",
      "install replacement plist",
      `/bin/launchctl setenv NATESCLAW_GATEWAY_STARTUP_TRACE 1`,
      `/bin/launchctl enable gui/${uid}/ai.natesclaw.gateway`,
      `/bin/launchctl bootstrap gui/${uid} ${plistPath}`,
      `/bin/launchctl unsetenv NATESCLAW_GATEWAY_STARTUP_TRACE`,
    ]);
  });
  ...
  test("restores an absent managed service past bootout exit 3 and an unlabeled vendor plist", async () => {
    ...
    expect(deployedEntrypoint).toBe(snapshot);
    expect(calls).toContain(`/bin/launchctl bootout gui/${uid}/ai.natesclaw.gateway`);
  ...
  test("bootstraps an owned LaunchAgent left unloaded by a failed restart", async () => {
    ...
    expect(commands.calls).toEqual([
      ...
      `/bin/launchctl enable gui/${uid}/ai.natesclaw.gateway`,
      `/bin/launchctl bootstrap gui/${uid} ${plistPath}`,
      `/bin/launchctl setenv NATESCLAW_GATEWAY_STARTUP_TRACE already-enabled`,
    ...
    await expect(
      maintainMain(
        { checkout: mirror, remote: "origin", lockPath: path.join(root, "maintenance.lock") },
        {
          fetchMain: fetchFixtureMain,
          runCommand(command: string, args: string[]) {
            if (command === "pnpm" && args.slice(0, 3).join(" ") === "natesclaw gateway status") {
              statusCalls += 1;
              throw new Error("RPC unavailable");
            }
          },

...
// Natesclaw npm postpublish tests validate postpublish verification behavior.
...
import {
  ...
  NatesclawNpmPostpublishVerifyUsage,
  parseNatesclawNpmPostpublishVerifyArgs,
...
describe("parseNatesclawNpmPostpublishVerifyArgs", () => {
  it("keeps trusted release verification independent from target app dependencies", () => {
    const source = readFileSync("scripts/natesclaw-npm-postpublish-verify.ts", "utf8");
  ...
  it("supports help and package-manager separators", () => {
    expect(parseNatesclawNpmPostpublishVerifyArgs(["--help"])).toEqual({
      help: true,
      version: "",
    });
    expect(parseNatesclawNpmPostpublishVerifyArgs(["--", "2026.3.23"])).toEqual({
      help: false,
      version: "2026.3.23",
    });
  });
  ...
  it("rejects missing, option-like, and extra arguments before verification", () => {
    expect(() => parseNatesclawNpmPostpublishVerifyArgs([])).toThrow(
      NatesclawNpmPostpublishVerifyUsage(),
    );
    expect(() => parseNatesclawNpmPostpublishVerifyArgs(["--tag"])).toThrow(
      "Unknown natesclaw npm postpublish verifier option: --tag",
    );
    expect(() => parseNatesclawNpmPostpublishVerifyArgs(["2026.3.23", "extra"])).toThrow(
      "Unexpected natesclaw npm postpublish verifier argument: extra",
    );
  });
});
...
describe("buildPublishedInstallScenarios", () => {
  it("uses a single fresh scenario for plain stable releases", () => {
    expect(buildPublishedInstallScenarios("2026.3.23")).toEqual([
      {
        name: "fresh-exact",
        installSpecs: ["natesclaw@2026.3.23"],
  ...
  it("adds a stable-to-correction upgrade scenario for correction releases", () => {
    expect(buildPublishedInstallScenarios("2026.3.23-2")).toEqual([
      {
        name: "fresh-exact",
        installSpecs: ["natesclaw@2026.3.23-2"],
        expectedVersion: "2026.3.23-2",
      },
      {
        name: "upgrade-from-base-stable",
        installSpecs: ["natesclaw@2026.3.23", "natesclaw@2026.3.23-2"],
        expectedVersion: "2026.3.23-2",
      },
    ]);
  });
...
describe("npm registry provenance verification", () => {
  ...
  it("fetches npm registry JSON with bounded response handling", async () => {
    ...
    await expect(
      fetchRegistryJson("https://registry.example/natesclaw", {
  ...
  it("bounds oversized npm registry response bodies", async () => {
    ...
    await expect(
      ...
      "npm registry https://registry.example/natesclaw response body exceeded 64 bytes",
    );
  });
  ...
  it("keeps npm registry timeouts active while reading response bodies", async () => {
    ...
    await expect(
      ...
      "npm registry request timed out after 5ms: https://registry.example/natesclaw",
    );
  });
            ...
            bundle: {
              dsseEnvelope: {
                payload: Buffer.from(
                  JSON.stringify({
                    ...provenancePayload,
                    subject: [{ name: "pkg:npm/natesclaw@2026.3.24", digest: {} }],
                  }),
                  "utf8",
                ).toString("base64"),
              },
  ...
  it.each([
    ...
    async (_label, extendedStableVersion, workflowRef) => {
      ...
      await expect(
        ...
        `does not bind ${extendedStableVersion} to the trusted Natesclaw GitHub release workflow`,
      );
  ...
  it("rejects matching provenance from an untrusted source before Sigstore verification", async () => {
    ...
    ).rejects.toThrow("does not bind 2026.3.23 to the trusted Natesclaw GitHub release workflow");
    expect(verificationCalls).toBe(0);
  });
    ...
    await expect(
      retryNpmRegistryProvenanceRead(
        async () => {
          ...
          if (attempts === 1) {
            throw new Error(
              "npm provenance attestation does not bind 2026.3.23 to the trusted Natesclaw GitHub release workflow.",
            );
          }
          if (attempts === 2) {
            throw new Error(
              "npm registry provenance metadata is incomplete for natesclaw@2026.3.23.",
            );
          }
          return "verified";
        },
...
describe("buildPublishedInstallCommandArgs", () => {
  it("runs lifecycle scripts for published install verification", () => {
    const args = buildPublishedInstallCommandArgs("/tmp/natesclaw-prefix", "natesclaw@2026.4.10");

    expect(args).toEqual([
      "install",
      "-g",
      "--prefix",
      "/tmp/natesclaw-prefix",
      "natesclaw@2026.4.10",
      "--no-fund",
      "--no-audit",
    ]);
...
describe("collectInstalledPackageErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "natesclaw-postpublish-package-"));
  }
  ...
  it("flags version mismatches", () => {
    const errors = collectInstalledPackageErrors({
      ...
      packageRoot: "/tmp/empty-natesclaw",
    });
  ...
  it("keeps bundled manifest requirements stable after the build filter changes", () => {
    ...
    try {
      ...
      const filteredErrors = withEnv({ NATESCLAW_BUNDLED_PLUGIN_BUILD_IDS: "ollama" }, () =>
    ...
    try {
      const probe = spawnSync(
        ...
        [
          ...
          "--eval",
          [
            'import { collectInstalledBundledExtensionManifestErrors } from "./scripts/natesclaw-npm-postpublish-verify.ts";',
        ...
        ],
        {
          cwd: process.cwd(),
          encoding: "utf8",
          env: { ...process.env, NATESCLAW_BUNDLED_PLUGIN_BUILD_IDS: "ollama" },
          timeout: 30_000,
        },
      );
...
describe("collectInstalledAlwaysAllowedRuntimeFacadeErrors", () => {
  function withInstalledPackageRoot(run: (packageRoot: string) => void): void {
    const packageRoot = mkdtempSync(join(tmpdir(), "natesclaw-postpublish-facade-runtime-"));
    try {
      run(packageRoot);
    } finally {
      rmSync(packageRoot, { recursive: true, force: true });
...
describe("collectInstalledContextEngineRuntimeErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "natesclaw-postpublish-context-engine-"));
  }
...
describe("collectInstalledPluginSdkZodArtifactErrors", () => {
  function withInstalledPackageRoot(run: (packageRoot: string) => void): void {
    const packageRoot = mkdtempSync(join(tmpdir(), "natesclaw-postpublish-zod-sdk-"));
    try {
      run(packageRoot);
    } finally {
      rmSync(packageRoot, { recursive: true, force: true });
...
describe("normalizeInstalledBinaryVersion", () => {
  it("accepts decorated CLI version output", () => {
    expect(normalizeInstalledBinaryVersion("Natesclaw 2026.4.8 (9ece252)")).toBe("2026.4.8");
    expect(normalizeInstalledBinaryVersion("Natesclaw 2026.4.8-beta.1 (9ece252)")).toBe(
      "2026.4.8-beta.1",
    );
    expect(normalizeInstalledBinaryVersion("Natesclaw 2026.4.8-alpha.1 (9ece252)")).toBe(
      "2026.4.8-alpha.1",
    );
  });
});
...
describe("resolveInstalledBinaryPath", () => {
  it("uses the Unix global bin path on non-Windows platforms", () => {
    expect(resolveInstalledBinaryPath("/tmp/natesclaw-prefix", "darwin")).toBe(
      "/tmp/natesclaw-prefix/bin/natesclaw",
    );
  });

  it("uses the Windows npm shim path on win32", () => {
    expect(resolveInstalledBinaryPath("C:/natesclaw-prefix", "win32")).toBe(
      "C:\\natesclaw-prefix\\natesclaw.cmd",
    );
  });
});
...
describe("resolveInstalledBinaryCommandInvocation", () => {
  it("runs the Unix installed binary directly", () => {
    expect(
      resolveInstalledBinaryCommandInvocation("/tmp/natesclaw-prefix", ["--version"], {
        platform: "linux",
      }),
    ).toEqual({
      command: "/tmp/natesclaw-prefix/bin/natesclaw",
      args: ["--version"],
    });
  });
  ...
  it("wraps the Windows installed npm shim without Node shell argv", () => {
    expect(
      ...
      args: [
        ...
        "/s",
        "/c",
        '""C:\\natesclaw prefix\\natesclaw.cmd" agent --message "hello world""',
      ],
...
describe("collectInstalledRootDependencyManifestErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "natesclaw-postpublish-root-deps-"));
  }

...
```bash
natesclaw resume
natesclaw resume <query>
```

With no query, Natesclaw displays up to 50 sessions active in the last seven
days. With a query, an exact session key wins; otherwise Natesclaw requires a
...
`natesclaw resume agent:main:global`.

If a query is ambiguous, Natesclaw prints the matching candidates and exits with
status 1. If no recent session matches, it suggests the picker and
[`natesclaw sessions`](/cli/sessions), then exits with status 1.
...
[`natesclaw tui`](/cli/tui). It never starts a Gateway automatically. If the
...
Gateway target precedence is explicit `--url`, then `NATESCLAW_GATEWAY_URL`,
then `gateway.remote.url` when `gateway.mode` is `remote`, then the local
loopback Gateway. For that local Gateway, `NATESCLAW_GATEWAY_PORT` takes
...
# Exact key
natesclaw resume agent:main:bugfix

# Unique display-name or label fragment
natesclaw resume bugfix

# Remote Gateway override
natesclaw resume bugfix --url wss://gateway.example.com --token <token>
```

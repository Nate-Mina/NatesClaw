/** Interactive stdio ACP client used to connect a terminal session to an Natesclaw ACP server. */
...
async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  const defaultServerCommand = entryPath ? process.execPath : "natesclaw";
  ...
  try {
    ...
    await client.initialize({
      ...
      clientInfo: { name: "natesclaw-acp-client", version: "1.0.0" },
    });
...
export async function runAcpClientInteractive(opts: AcpClientOptions = {}): Promise<void> {
  ...
  console.log("Natesclaw ACP client");

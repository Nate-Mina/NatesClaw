...
import type { ConfigFileSnapshot, ModelDefinitionConfig, NatesclawConfig } from "../config/types.js";
...
vi.mock("../config/paths.js", () => ({
  ...
  resolveStateDir: vi.fn(() => "/tmp/natesclaw-state"),
}));

vi.mock("../config/runtime-overrides.js", () => ({
  applyConfigOverrides: vi.fn((config: NatesclawConfig) => config),
}));
...
vi.mock("../config/plugin-auto-enable.js", () => ({
  applyPluginAutoEnable: (params: { config: NatesclawConfig }) => applyPluginAutoEnable(params),
}));
...
const configPath = "/tmp/natesclaw-startup-recovery.json";
...
function expectPluginAutoEnableFor(config: NatesclawConfig) {
...
function mockRuntimeAutoEnable(config: NatesclawConfig) {
...
function buildInvalidConfigSnapshot(params: {
  ...
  return buildTestConfigSnapshot({
    ...
    valid: false,
    config: params.config ?? (params.rawConfig as NatesclawConfig),
...
describe("gateway startup config validation", () => {
  ...
  it("does not suggest doctor repair for plugin packaging compiled-output failures", async () => {
    ...
    const invalidSnapshot = buildInvalidConfigSnapshot({
      rawConfig,
      config: rawConfig as NatesclawConfig,
      issues: [
        {
    ...
    await start.catch((error: unknown) => {
      expect(String(error)).not.toContain("natesclaw doctor --fix");
    });
  });
  ...
  it("keeps doctor repair guidance for mixed plugin packaging and core invalidity", async () => {
    ...
    const invalidSnapshot = buildInvalidConfigSnapshot({
      rawConfig,
      config: rawConfig as unknown as NatesclawConfig,
      issues: [
        {
          path: "plugins.slots.memory",
          message: "plugin not found: source-only-pack",
    ...
    await expectStartupRejects('Run "natesclaw doctor --fix" to repair, then retry.');
  });

...
extension Notification.Name {
    static let natesclawNodeHostWorkerFailed = Notification.Name("natesclaw.node-host-worker.failed")
    static let natesclawNodeHostWorkerRetryExhausted = Notification.Name(
        "natesclaw.node-host-worker.retry-exhausted")
}
...
final class MacNodeHostWorker: MacNodeHostWorking, @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "ai.natesclaw", category: "node-host-worker")
    private let queue = DispatchQueue(label: "ai.natesclaw.node-host-worker")
    private let writerQueue = DispatchQueue(label: "ai.natesclaw.node-host-worker.writer")
    ...
    private func startLocked(launch: MacNodeHostWorkerLaunch) {
        ...
        environment["NATESCLAW_NODE_EXEC_HOST"] = "app"
        environment["NATESCLAW_NODE_EXEC_FALLBACK"] = "0"
    ...
    private static func decodeInvokeResponse(_ result: [String: Any], id: String) -> BridgeInvokeResponse {
        ...
        let code = NatesclawNodeErrorCode(rawValue: rawError?["code"] as? String ?? "UNAVAILABLE") ?? .unavailable
        let error = ok ? nil : NatesclawNodeError(
            code: code,
    ...
    private static func unavailableResponse(_ id: String, _ message: String) -> BridgeInvokeResponse {
        BridgeInvokeResponse(
            id: id,
            ok: false,
            error: NatesclawNodeError(code: .unavailable, message: message))
    }

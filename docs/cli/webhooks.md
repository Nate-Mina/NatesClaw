---
summary: "CLI reference for `natesclaw webhooks` (Gmail Pub/Sub setup and runner)"
read_when:
  - You want to wire Gmail Pub/Sub events into Natesclaw
...
```bash
natesclaw webhooks gmail setup --account <email> [...]
natesclaw webhooks gmail run   [--account <email>] [...]
```

| Subcommand    | Description                                                                           |
| ------------- | ------------------------------------------------------------------------------------- |
| `gmail setup` | One-time wizard: Gmail watch, Pub/Sub topic/subscription, and Natesclaw hook delivery. |
| `gmail run`   | Run `gog watch serve` plus the watch auto-renew loop in the foreground.               |

<Note>
The Gateway also auto-starts `gog gmail watch serve` on boot once `hooks.enabled=true` and `hooks.gmail.account` is set (set by `gmail setup`). `gmail run` is the same logic in the foreground, useful for debugging or when the Gateway watcher is disabled. See [Gmail Pub/Sub integration](/automation/cron-jobs#gmail-pubsub-integration) for the auto-start details and `NATESCLAW_SKIP_GMAIL_WATCHER` opt-out.
</Note>

## `webhooks gmail setup`

```bash
natesclaw webhooks gmail setup --account you@example.com
natesclaw webhooks gmail setup --account you@example.com --project my-gcp-project --json
natesclaw webhooks gmail setup --account you@example.com --hook-url https://gateway.example.com/hooks/gmail
```

Installs `gcloud` and `gog` if missing, authenticates `gcloud`, creates the Pub/Sub topic and subscription, starts the Gmail watch, and writes `hooks.gmail` config with `hooks.enabled=true`. Prints `Next: natesclaw webhooks gmail run`.
...
### Natesclaw delivery options

| Flag                   | Default                                      | Description                                |
| ---------------------- | -------------------------------------------- | ------------------------------------------ |
| `--hook-url <url>`     | Built from `hooks.path` and the Gateway port | Natesclaw webhook URL.                      |
| `--hook-token <token>` | `hooks.token`, or a generated token          | Natesclaw webhook token.                    |
...
```bash
natesclaw webhooks gmail run --account you@example.com
```
...
`run` accepts the same Pub/Sub, Natesclaw delivery, `gog watch serve`, and Tailscale flags as `setup`, except:
...
| Natesclaw delivery | `--hook-url`, `--hook-token`, `--push-token`                                     |

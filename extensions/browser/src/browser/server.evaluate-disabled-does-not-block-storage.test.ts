...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        natesclaw: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.NATESCLAW_GATEWAY_PORT;
    process.env.NATESCLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.NATESCLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.NATESCLAW_GATEWAY_PASSWORD;

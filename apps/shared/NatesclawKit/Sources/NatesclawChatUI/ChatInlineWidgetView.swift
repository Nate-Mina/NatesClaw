...
enum NatesclawChatWidgetSurfaceRole: Sendable, Hashable {
    case node
    case operatorSurface
    case legacy
}

public struct NatesclawChatWidgetResource: Sendable, Equatable {
    public let url: URL
    public let tlsFingerprintSHA256: String?
    let surfaceRole: NatesclawChatWidgetSurfaceRole
    let attemptedSurfaceRoles: Set<NatesclawChatWidgetSurfaceRole>
    ...
    init(
        ...
        surfaceRole: NatesclawChatWidgetSurfaceRole,
        attemptedSurfaceRoles: Set<NatesclawChatWidgetSurfaceRole>)
    {
        self.url = url
        self.tlsFingerprintSHA256 = tlsFingerprintSHA256
...
public enum NatesclawChatWidgetURLResolver {
    private static let documentsPath = "/__natesclaw__/canvas/documents"
    ...
    public static func resolveResource(
        target: String,
        replacing failedResource: NatesclawChatWidgetResource?,
    ...
    private static func capabilitySurface(_ rawSurfaceURL: String?) -> URLComponents? {
        ...
        guard segments.count >= 3,
              segments[segments.count - 3] == "__natesclaw__",
    ...
    private static func resolve(
        ...
        role: NatesclawChatWidgetSurfaceRole,
        target: String,
        attemptedRoles: Set<NatesclawChatWidgetSurfaceRole>) -> NatesclawChatWidgetResource?
    {
        guard let url = resolve(surfaceURL: surface.url, target: target) else { return nil }
        let resource = NatesclawChatWidgetResource(
            url: url,
    ...
    private static func resolvePreferred(
        ...
        target: String,
        excluding failedResource: NatesclawChatWidgetResource?,
        blockedRoles: Set<NatesclawChatWidgetSurfaceRole>,
        attemptedRoles: Set<NatesclawChatWidgetSurfaceRole>) -> NatesclawChatWidgetResource?
    {
        [
            (role: NatesclawChatWidgetSurfaceRole.node, surface: surfaces.node),
            (role: NatesclawChatWidgetSurfaceRole.operatorSurface, surface: surfaces.operatorSurface),
        ]
            .lazy
    ...
    private static func isReplacement(
        _ candidate: NatesclawChatWidgetResource,
        for failedResource: NatesclawChatWidgetResource?) -> Bool
    {
...
struct ChatInlineWidgetView: View {
    let preview: NatesclawChatCanvasPreview
    let resolverReady: Bool
    let resolveResource: @MainActor @Sendable (
        String,
        NatesclawChatWidgetResource?) async -> NatesclawChatWidgetResource?

    @State private var resolvedResource: NatesclawChatWidgetResource?
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            if let title = self.preview.title?.trimmingCharacters(in: .whitespacesAndNewlines), !title.isEmpty {
                Text(title)
                    .font(NatesclawChatTypography.footnote)
                    .fontWeight(.semibold)
                    .foregroundStyle(NatesclawChatTheme.muted)
            }
    ...
    #if canImport(WebKit) && (os(iOS) || os(macOS))
    private func renderedWidget(resource: NatesclawChatWidgetResource) -> some View {
        ChatInlineWidgetWebView(
            ...
            .overlay {
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .stroke(NatesclawChatTheme.muted.opacity(0.24), lineWidth: 1)
            }
    ...
    private func handleLoadFailure(resource: NatesclawChatWidgetResource) {
...
extension NatesclawChatWidgetResource {
...
private final class ChatInlineWidgetNavigationDelegate: NSObject, WKNavigationDelegate {
    var resource: NatesclawChatWidgetResource {
        didSet {
    ...
    init(
        resource: NatesclawChatWidgetResource,

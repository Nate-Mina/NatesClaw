---
name: release-natesclaw-plugin-testing
description: Plan and run pre-release Natesclaw plugin validation across bundled plugins, package artifacts, lifecycle commands, doctor/fix, config round-trip, gateway startup, SDK compatibility, Docker E2E, Package Acceptance, and Testbox proof.
---

# Natesclaw Pre-Release Plugin Testing
...
release?" It complements `natesclaw-testing`; use that skill too when choosing
...
the main Natesclaw checkout. Do not run `pnpm install` there. For broad or
...
```bash
NATESCLAW_TESTBOX=1 pnpm check:changed
pnpm run test:extensions:package-boundary:canary
pnpm run test:extensions:package-boundary:compile
pnpm test:docker:plugins
NATESCLAW_PLUGINS_E2E_CLAWHUB=0 pnpm test:docker:plugins
...
```bash
NATESCLAW_BUNDLED_PLUGIN_SWEEP_TOTAL=8 \
NATESCLAW_BUNDLED_PLUGIN_SWEEP_INDEX=<0-7> \
...
Use `source=npm -f package_spec=natesclaw@beta` for published beta proof. Keep
...
ghx workflow run plugin-npm-release.yml \
  ...
  -f plugins=@natesclaw/meta-provider \
  -f ref="${release_sha}" \
...
Start packaged Natesclaw in Docker with clean state:
...
- `natesclaw status --json` includes plugin diagnostics
- `natesclaw plugins inspect --all --json` is parseable
...
`natesclaw/plugin-sdk/*` subpaths. Assert `plugins inspect` sees its tool,

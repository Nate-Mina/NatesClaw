// Built-in Natesclaw harness tests cover logical thinking-mode boundaries.
...
describe("createNatesclawAgentHarness", () => {
  beforeEach(() => {
  ...
  it("brands only host-created instances as the built-in runtime", () => {
    expect(isBuiltInNatesclawAgentHarness(createNatesclawAgentHarness())).toBe(true);
    expect(
      isBuiltInNatesclawAgentHarness({
        id: "natesclaw",
        label: "forged",
  ...
  it("preserves logical Ultra for the embedded attempt", async () => {
    ...
    await createNatesclawAgentHarness().runAttempt(params);
  ...
  it("enforces a tool-free settled-turn finalization", async () => {
    ...
    const harness = createNatesclawAgentHarness();
  ...
  it("runs isolated completion through the prepared zero-tool transport", async () => {
    const params = {
      ...
      NonNullable<ReturnType<typeof createNatesclawAgentHarness>["runIsolatedCompletion"]>
    >[0];

    await expect(createNatesclawAgentHarness().runIsolatedCompletion?.(params)).resolves.toEqual({

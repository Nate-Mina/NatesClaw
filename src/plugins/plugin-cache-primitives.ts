...
export type ConfigScopedRuntimeCache<T> = WeakMap<NatesclawConfig, Map<string, T>>;
...
type ConfigScopedPromiseLoader<T> = {
  load(config?: NatesclawConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: NatesclawConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<NatesclawConfig, Promise<T>>();

  const createPromise = (config?: NatesclawConfig): Promise<T> => {
  ...
  return {
    async load(config?: NatesclawConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<NatesclawConfig, Promise<T>>();
    },
  };
}

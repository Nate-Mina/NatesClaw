// Covers locating Natesclaw docs and source paths from package roots.
import fs from "node:fs/promises";
import os from "node:os";
import path from "node:path";
import { describe, expect, it } from "vitest";
import { resolveNatesclawReferencePaths } from "./docs-path.js";
...
async function makePackageRoot(prefix: string): Promise<string> {
  ...
  await fs.writeFile(path.join(root, "package.json"), '{"name":"natesclaw"}\n');
  return root;
}
...
describe("resolveNatesclawDocsPath", () => {
  it("uses the workspace docs directory when it has canonical docs metadata", async () => {
    const root = await makePackageRoot("natesclaw-docs-workspace-");
    await writeDocsJson(root);

    await expect(resolveNatesclawReferencePaths({ workspaceDir: root })).resolves.toMatchObject({
      docsPath: path.join(root, "docs"),
    });
  });

  it("finds bundled package docs from a nested package path", async () => {
    const root = await makePackageRoot("natesclaw-docs-package-");
    await writeDocsJson(root);
    const nested = path.join(root, "dist", "agents");
    await fs.mkdir(nested, { recursive: true });

    await expect(resolveNatesclawReferencePaths({ cwd: nested })).resolves.toMatchObject({
      docsPath: path.join(root, "docs"),
    });
  });
  ...
  it("does not accept incomplete template-only docs directories", async () => {
    ...
    const root = await makePackageRoot("natesclaw-docs-incomplete-");
    await fs.mkdir(path.join(root, "docs", "reference", "templates"), { recursive: true });

    await expect(resolveNatesclawReferencePaths({ cwd: root })).resolves.toMatchObject({
      docsPath: null,
    });
  });
});

describe("resolveNatesclawSourcePath", () => {
  it("returns the package root only for git checkouts", async () => {
    const root = await makePackageRoot("natesclaw-source-git-");
  ...
  it("omits source path for npm-style package installs", async () => {
    ...
    const root = await makePackageRoot("natesclaw-source-npm-");
...
describe("resolveNatesclawReferencePaths", () => {
  it("returns docs and local source together for git checkouts", async () => {
    const root = await makePackageRoot("natesclaw-reference-git-");
    await writeDocsJson(root);
    await fs.mkdir(path.join(root, ".git"));

    await expect(resolveNatesclawReferencePaths({ cwd: root })).resolves.toEqual({
      docsPath: path.join(root, "docs"),
      sourcePath: root,
    });
  });

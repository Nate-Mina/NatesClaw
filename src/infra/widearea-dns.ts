...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.NATESCLAW_WIDE_AREA_DOMAIN ?? null;
  try {
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*natesclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "natesclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "natesclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "natesclaw-gw");
  ...
  records.push(`_natesclaw-gw._tcp IN PTR ${instanceLabel}._natesclaw-gw._tcp`);
  records.push(`${instanceLabel}._natesclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._natesclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; natesclaw-content-hash: ${contentHash}\n${contentBody}`;
}

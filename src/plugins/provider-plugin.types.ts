...
export type ProviderPlugin = {
   ...
   *
   * Natesclaw calls this only from async model resolution paths. After it
   ...
   *
   * Runs after Natesclaw performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Natesclaw registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Natesclaw wrappers.
   *
   ...
   *
   * Called after Natesclaw resolves the raw configured credential but before the
   ...
   * hint than Natesclaw's generic auth-store guidance.
   */
   ...
   * Natesclaw's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Natesclaw should surface a provider-specific hint for
   ...
   * merging. Natesclaw deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. Natesclaw stores the
   ...
   * without replacing the full Natesclaw-owned system prompt.
   */
   ...
   *
   * Runs after Natesclaw's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep Natesclaw's default fallback policy.
   */
   ...
   * rewrites after Natesclaw has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before Natesclaw handles
  ...
  ) => NatesclawConfig | null | undefined;
  /**
   ...
   *
   * Natesclaw uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `natesclaw doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `natesclaw doctor` should offer.
   *
   ...
   *
   * Natesclaw calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep Natesclaw's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Natesclaw falls back to that stored profile.
   */

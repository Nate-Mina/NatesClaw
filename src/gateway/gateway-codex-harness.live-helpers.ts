...
export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current Natesclaw session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsNatesclawStatus =
    normalized.includes("natesclaw is running on") ||
    /natesclaw\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("natesclaw status:") ||
  ...
  return (
    ...
    isReadyStatus ||
    isOnlineIdleStatus ||
    (mentionsNatesclawStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from natesclaw status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live natesclaw config shows") ||

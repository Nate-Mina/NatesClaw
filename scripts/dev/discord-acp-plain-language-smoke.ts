#!/usr/bin/env bun
// Discord Acp Plain Language Smoke script supports Natesclaw repository automation.
...
type DriverMode = "token" | "webhook" | "natesclaw";
...
const DEFAULT_NATESCLAW_CLI_TIMEOUT_MS = 60_000;
...
function resolveStateDir(): string {
  ...
  const home = process.env.NATESCLAW_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".natesclaw");
}
...
function parseDriverMode(raw: string): DriverMode {
  ...
  if (normalized === "token" || normalized === "webhook" || normalized === "natesclaw") {
    return normalized;
  }
  throw new Error(
    `Invalid --driver value ${JSON.stringify(raw)}; expected token, webhook, or natesclaw.`,
  );
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver natesclaw] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Natesclaw spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|natesclaw> Driver transport mode (default: token)\n" +
    ...
    "  --state-dir <p>              Override Natesclaw state dir for plugin-state polling\n" +
    "  --natesclaw-bin <path>        Natesclaw CLI binary for driver=natesclaw (default: natesclaw)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  NATESCLAW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  NATESCLAW_DISCORD_SMOKE_DRIVER\n" +
    "  NATESCLAW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  NATESCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  NATESCLAW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  NATESCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  NATESCLAW_DISCORD_SMOKE_AGENT\n" +
    "  NATESCLAW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  NATESCLAW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  NATESCLAW_DISCORD_SMOKE_POLL_MS\n" +
    "  NATESCLAW_STATE_DIR\n" +
    "  NATESCLAW_DISCORD_SMOKE_NATESCLAW_BIN"
  );
}
...
function parseArgs(argv = process.argv.slice(2)): Args {
  ...
  const channelId =
    resolveArg("--channel", argv) || process.env.NATESCLAW_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver", argv) || process.env.NATESCLAW_DISCORD_SMOKE_DRIVER || "token";
  const driverMode = parseDriverMode(driverModeRaw);
  const driverToken =
    resolveArg("--token", argv) || process.env.NATESCLAW_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix", argv) ||
    process.env.NATESCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX ||
    "Bot";
  const botToken =
    resolveArg("--bot-token", argv) ||
    process.env.NATESCLAW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix", argv) ||
    process.env.NATESCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent", argv) || process.env.NATESCLAW_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention", argv) ||
    process.env.NATESCLAW_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction", argv) ||
    process.env.NATESCLAW_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms", argv) || process.env.NATESCLAW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
    "--timeout-ms",
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms", argv) || process.env.NATESCLAW_DISCORD_SMOKE_POLL_MS,
    1_500,
    "--poll-ms",
  );
  const stateDir = path.resolve(resolveArg("--state-dir", argv) || resolveStateDir());
  const natesclawBin =
    resolveArg("--natesclaw-bin", argv) ||
    process.env.NATESCLAW_DISCORD_SMOKE_NATESCLAW_BIN ||
    "natesclaw";
...
async function natesclawCliJson<T>(params: {
  natesclawBin: string;
  args: string[];
  timeoutMs?: number;
}): Promise<T> {
  const result = await execFileAsync(params.natesclawBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
    timeout: params.timeoutMs ?? DEFAULT_NATESCLAW_CLI_TIMEOUT_MS,
    killSignal: "SIGKILL",
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`natesclaw ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithNatesclaw(params: {
  natesclawBin: string;
  target: string;
  limit: number;
  timeoutMs?: number;
}): Promise<DiscordMessage[]> {
  const response = await natesclawCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    natesclawBin: params.natesclawBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "natesclaw") {
    return await readMessagesWithNatesclaw({
      natesclawBin: params.args.natesclawBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        timeoutMs: remainingTimeoutMs(deadline),
        body: {
          name: `natesclaw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      minBindingBoundAt = Date.now() - 3_000;
      const sent = await natesclawCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        natesclawBin: args.natesclawBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("natesclaw message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "natesclaw"
            ? await readMessagesWithNatesclaw({
                natesclawBin: args.natesclawBin,
                target: threadId,
                limit: 50,
                timeoutMs: remainingTimeoutMs(deadline),
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Natesclaw.`,
        diagnostics: {
          bindingCandidates: [
            {

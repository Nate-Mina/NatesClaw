...
export async function startCodexAttemptThread(params: {
  ...
  /** Natesclaw owns configured MCP dynamically for this scheduled turn. */
          ...
          try {
            ...
            try {
              ...
              if (
                ...
                throw new Error(
                  "Codex app-server did not register an Natesclaw sandbox exec-server environment.",
                );
              }
            } catch (error) {
              await releaseStartupSandboxEnvironment();
            ...
            const startupSandboxPolicy = startupSandboxEnvironment
              ? resolveCodexExternalSandboxPolicyForNatesclawSandbox(params.sandbox)
              : undefined;

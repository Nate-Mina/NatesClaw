...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Natesclaw", ".natesclaw");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/natesclaw-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".natesclaw");
  ...
  it("anchors cloud detection to OS homedir when NATESCLAW_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".natesclaw");
    const originalNatesclawHome = process.env.NATESCLAW_HOME;
    process.env.NATESCLAW_HOME = "/tmp/natesclaw-home-override";

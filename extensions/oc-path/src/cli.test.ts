/**
 * Smoke tests for the `natesclaw path` CLI handlers.
 *
...
async function invokePathCli(args: string[], runtime: TestRuntime): Promise<void> {
  ...
  try {
    await program.parseAsync(["node", "natesclaw", "path", ...args]);
...
describe("natesclaw path CLI", () => {
  it("reports its TTY-aware machine-output mode to the CLI", () => {
    const argv = ["node", "natesclaw", "path", "validate", "oc://AGENTS.md"];
  ...
  describe("set", () => {
    ...
    it("CLI-S08 sets slash-deep JSONC paths and parsed JSON values", async () => {
      const filePath = join(workspaceDir, "natesclaw.json");
      writeFileSync(
        filePath,
      ...
      await pathSetCommand(
        "oc://natesclaw.json/gateway/auth/token",
      ...
      await pathSetCommand(
        "oc://natesclaw.json/agents/list/0/tools/exec/security",
        "allowlist",
        { cwd: workspaceDir, json: true },
        rt2,
      );

// Natesclaw overview tests cover summary output for rescue diagnostics.
...
describe("loadSystemAgentOverview", () => {
  it("summarizes config, agents, model, tools, and gateway", async () => {
    ...
    const overview = await loadSystemAgentOverview({
      env: { NATESCLAW_TEST_FAST: "1" },
      deps: {
        readConfigFileSnapshot: async () => snapshot,
        resolveConfigPath: () => "/tmp/natesclaw.json",
    ...
    expect(overview.references.sourceUrl).toBe("https://github.com/natesclaw/natesclaw");
    ...
    expect(startup).toContain("Hi, I'm Natesclaw — caretaker");
  ...
  it("fails closed in startup copy when inference is unavailable", () => {
    ...
    const startup = formatSystemAgentStartupMessage(overview);
    expect(formatSystemAgentOverview(overview)).toContain(
      'Next: run "natesclaw onboard" to establish inference',
    );
    expect(startup).toContain("Inference is unavailable");
    expect(startup).toContain("Run `natesclaw onboard`");
    expect(startup.match(/`[^`]+`/g)).toEqual(["`natesclaw onboard`"]);

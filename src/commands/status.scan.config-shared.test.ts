...
describe("status.scan.config-shared", () => {
  beforeEach(() => {
    ...
    mocks.resolveConfigPath.mockReturnValue(
      `/tmp/natesclaw-status-scan-config-shared-missing-${process.pid}.json`,
    );
  });
  ...
  it("adds a status diagnostic for gateway token source conflicts", async () => {
    ...
    const result = await loadStatusScanCommandConfig({
      ...
      readConfigSnapshot,
      resolveConfig,
      env: { VITEST: "true", NATESCLAW_GATEWAY_TOKEN: "env-token" },
      allowMissingConfigFastPath: true,
    });

    expect(result.secretDiagnostics).toEqual([
      "NATESCLAW_GATEWAY_TOKEN conflicts with gateway.auth.token: Remove NATESCLAW_GATEWAY_TOKEN from the shell, ~/.natesclaw/.env, or launchctl env if gateway.auth.token is intended, or point gateway.auth.token at ${NATESCLAW_GATEWAY_TOKEN} if the env var should be canonical.",
    ]);
  });
  ...
  it("does not add a status diagnostic when config uses NATESCLAW_GATEWAY_TOKEN", async () => {
    const sourceConfig = {

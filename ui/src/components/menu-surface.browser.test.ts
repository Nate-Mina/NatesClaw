...
// popover top layer via natesclaw-menu-surface.
//
...
describe.skipIf(!hasPopoverApi)("sidebar menu stacking", () => {
  ...
  it("paints a menu hosted in natesclaw-menu-surface above the resizer divider", async () => {
    await useDesktopViewport();
    const { nav, divider } = mountShell();
    const surface = document.createElement("natesclaw-menu-surface");
    const menu = createSortMenu();
    surface.append(menu);
    nav.append(surface);

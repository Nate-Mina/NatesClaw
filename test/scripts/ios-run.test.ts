...
function makeFixture(bundleId: string): { root: string; script: string; logFile: string } {
  const root = mkdtempSync(path.join(os.tmpdir(), "natesclaw-ios-run-"));
  tempDirs.push(root);
  ...
  writeExecutable(
    ...
    `#!/usr/bin/env bash
set -euo pipefail
if [[ -n "\${NATESCLAW_SIMULATOR_PUSH_PROOF_SECRET:-}" || -n "\${CUSTOM_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
if (process.env.NATESCLAW_SIMULATOR_PUSH_PROOF_SECRET || process.env.CUSTOM_SIMULATOR_PUSH_PROOF_SECRET) {
...
set -euo pipefail
printf 'xcodegen %s\\n' "$*" >>"${logFile}"
if [[ -n "\${NATESCLAW_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
app_dir="$derived/Build/Products/$configuration-iphonesimulator/Natesclaw.app"
mkdir -p "$app_dir"
...
printf 'simctl %s\\n' "$*" >>"${logFile}"
if [[ "$1" == "launch" ]]; then
  if [[ -n "\${SIMCTL_CHILD_NATESCLAW_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
describe("scripts/ios-run.sh", () => {
  ...
  it("installs and launches the configured app bundle identifier", () => {
    const fixture = makeFixture("ai.natesclawfoundation.app");

    runIosRun(fixture, { SIMCTL_BOOT_MODE: "booted" });

    expect(readFileSync(fixture.logFile, "utf8")).toContain(
      "simctl launch iPhone 17 ai.natesclawfoundation.app",
    );
  });
  ...
  it("builds simulator sandbox relay mode and injects proof secret only at launch", () => {
    ...
    runIosRun(
      fixture,
      {
        NATESCLAW_SIMULATOR_PUSH_PROOF_SECRET: proofSecret,
  ...
  it("scrubs exported simulator proof secrets from normal build helpers", () => {
    ...
    runIosRun(fixture, {
      NATESCLAW_SIMULATOR_PUSH_PROOF_SECRET: proofSecret,
      NATESCLAW_SIMULATOR_PUSH_PROOF_SECRET_ENV: "CUSTOM_SIMULATOR_PUSH_PROOF_SECRET",

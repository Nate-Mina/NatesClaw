// Temp home test helpers create isolated Natesclaw home directories for plugin tests.
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked NATESCLAW_HOME.
  deleteTestEnvValue("NATESCLAW_HOME");
  setTestEnvValue("NATESCLAW_STATE_DIR", path.join(base, ".natesclaw"));
...
export async function withTempHomeCore<T>(
  ...
  const prefix = opts.prefix ?? "natesclaw-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".natesclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {

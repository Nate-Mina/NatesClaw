...
import {
  ...
  NATESCLAW_EMBEDDED_CONTEXT_ENGINE_HOST,
...
function runtimeHostCandidate(params: {
  ...
  if (runtimeId === "natesclaw" || runtimeId === "auto") {
    return { runtimeId, host: NATESCLAW_EMBEDDED_CONTEXT_ENGINE_HOST, paths: params.paths };
  }
  if (runtimeId === "codex") {
...
function selectedContextEngineSlotId(cfg: NatesclawConfig): string {
...
export async function maybeRepairContextEngineHostCompatibility(params: {
  ...
  doctorFixCommand: string;
  env?: NodeJS.ProcessEnv;
}): Promise<{ config: NatesclawConfig; changes: string[]; warnings?: string[] }> {
  const resolved = await resolveSelectedContextEngineInfo(params);
  if (!resolved.info) {

...
const repo = "natesclaw/natesclaw";
...
export function defaultGithubSnapshotPath(base, target, gitCommonDir) {
  ...
  return path.resolve(gitCommonDir, "natesclaw-release-cache", defaultName);
}
...
function closingReferencesIn(text) {
  const references = [];
  for (const match of text.matchAll(
    /\b(?:fix(?:es|ed)?|closes?|closed|resolves?|resolved)\s+(?:(?:natesclaw\/natesclaw)?#\d+)(?:\s*(?:,|and)\s*(?:(?:natesclaw\/natesclaw)?#\d+))*/gi,
  )) {
...
function resolveAssociatedPullRequests(commitHashes, targetTimestamp) {
  ...
  for (let index = 0; index < commitHashes.length; index += commitAssociationQueryBatchSize) {
    ...
    const fields = chunk
      .map(
        (hash, offset) =>
          `c${index + offset}: repository(owner: "natesclaw", name: "natesclaw") {
  ...
  while (pending.length > 0) {
    ...
    const fields = chunk
      .map(
        (item, offset) =>
          `c${offset}: repository(owner: "natesclaw", name: "natesclaw") {
  ...
  while (pending.length > 0) {
    ...
    const fields = chunk
      .map((item, offset) => {
        ...
        return `n${offset}: repository(owner: "natesclaw", name: "natesclaw") {
...
function resolveReferences(numbers) {
  ...
  for (let index = 0; index < numbers.length; index += 40) {
    ...
    const fields = chunk
      .map(
        (number) => `n${number}: repository(owner: "natesclaw", name: "natesclaw") {

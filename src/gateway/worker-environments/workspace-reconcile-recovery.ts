...
async function writeRawWorkspaceTree(params: {
  ...
  const ref = `refs/heads/natesclaw-snapshot-${randomBytes(16).toString("hex")}`;
  ...
  chunks.push(
    Buffer.from(
      `commit ${ref}\ncommitter Natesclaw <noreply@natesclaw.ai> 0 +0000\ndata 0\ndeleteall\n`,
    ),
  );
  for (const blob of blobs) {
    const mode =
...
export async function createWorkspacePatch(params: {
  ...
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-workspace-patch-"));
  try {
...
export async function applyWorkspacePatch(params: {
  ...
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-no-git-"));
  try {
    await requireGit(
      params.root,
      [
...
async function createWorkspaceRecoveryPatch(params: {
  ...
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-workspace-recovery-"));
  try {
    await requireGit(temporary, ["init", "--quiet", "--object-format=sha1"]);

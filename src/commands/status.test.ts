...
beforeAll(() => {
  envSnapshot = captureEnv(["NATESCLAW_PROFILE"]);
  process.env.NATESCLAW_PROFILE = "isolated";
});
...
async function createStatusServiceSummary(
  ...
  return {
    ...
    loaded,
    managedByNatesclaw: Boolean(command),
    externallyManaged: !command && runtime?.status === "running",
    loadedText: service.loadedText,
    runtime,
    runtimeShort: runtime?.pid ? `pid ${runtime.pid}` : null,
    wrapperPath: command?.environment?.NATESCLAW_WRAPPER?.trim() || undefined,
  };
}
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/natesclaw/pnpm-lock.yaml",
        markerPath: "/tmp/natesclaw/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.NATESCLAW_GATEWAY_TOKEN
      ? { token: process.env.NATESCLAW_GATEWAY_TOKEN }
      : {},
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.natesclaw.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.natesclaw.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.NATESCLAW_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/natesclaw-root.js", () => ({
  resolveNatesclawPackageRoot: vi.fn().mockResolvedValue("/tmp/natesclaw"),
  resolveNatesclawPackageRootSync: vi.fn(() => "/tmp/natesclaw"),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expectLogsMatch(logs, /natesclaw (?:--profile isolated )?status --all/);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("natesclaw --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("NATESCLAW_GATEWAY_TOKEN", "abcd1234", async () => {
  ...
  it("notes when secret diagnostics may come from a CLI process outside the service wrapper context", async () => {
    ...
    mocks.resolveGatewayService.mockReturnValue({
      ...
      readCommand: async () => ({
        ...
        environment: { NATESCLAW_WRAPPER: wrapperPath },
        sourcePath: "/tmp/Library/LaunchAgents/ai.natesclaw.gateway.plist",
      }),
    });
    mocks.loadConfig.mockReturnValue({
    ...
    await withOptionalEnvVar("NATESCLAW_WRAPPER", undefined, async () => {
      const logs = await runStatusAndGetLogs();
      expectLogsInclude(logs, "Secret diagnostics:");
      expectLogsInclude(logs, "installed gateway service uses NATESCLAW_WRAPPER");
    ...
    await withEnvVar("NATESCLAW_WRAPPER", wrapperPath, async () => {
      const logs = await runStatusAndGetLogs();
      expectLogsInclude(logs, "Secret diagnostics:");

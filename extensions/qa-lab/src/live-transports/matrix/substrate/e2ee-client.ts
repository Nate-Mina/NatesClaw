...
import type {
  ...
  MessageEventContent,
} from "@natesclaw/matrix/test-api.js";
import type {
...
type MatrixQaE2eeRuntime = typeof import("@natesclaw/matrix/test-api.js");
...
function resolveMatrixQaPluginStateNamespaceKey(options: OpenKeyedStoreOptions): string {
  return `${options.env?.NATESCLAW_STATE_DIR ?? ""}\0${options.namespace}`;
}
...
export async function loadMatrixQaE2eeRuntime(): Promise<MatrixQaE2eeRuntime> {
  const { loadQaRunnerBundledPluginTestApi } =
    await import("natesclaw/plugin-sdk/qa-runner-runtime");

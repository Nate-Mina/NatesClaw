...
IMAGE_NAME="${NATESCLAW_CLEANUP_SMOKE_IMAGE:-natesclaw-cleanup-smoke:local}"
...
resolve_default_cleanup_platform() {
  local host_arch
  if [[ -n "${NATESCLAW_CLEANUP_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$NATESCLAW_CLEANUP_SMOKE_PLATFORM"
    return
  fi
  host_arch="$(uname -m)"

...
import ai.natesclaw.app.gateway.GatewayCustomHeaders
import ai.natesclaw.app.gateway.GatewayRegistryStore
import ai.natesclaw.app.gateway.GatewayStoreMigration
...
class SecurePrefs(
  ...
  companion object {
    ...
    private const val plainPrefsName = "natesclaw.node"
    private const val securePrefsName = "natesclaw.node.secure"
  ...
  internal fun getNotificationForwardingPolicy(appPackageName: String): NotificationForwardingPolicy {
    ...
    // Always block Natesclaw's own notifications in blocklist mode to prevent forwarding loops.

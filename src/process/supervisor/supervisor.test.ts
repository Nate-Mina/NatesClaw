...
describe("process supervisor", () => {
  ...
  it("bounds retained output on UTF-16 boundaries while streaming full chunks", async () => {
    ...
    const stdoutMarker = `[natesclaw: captured stdout truncated to last ${maxCapturedOutputChars} chars]\n`;
    const stderrMarker = `[natesclaw: captured stderr truncated to last ${maxCapturedOutputChars} chars]\n`;

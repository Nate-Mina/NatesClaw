...
const discoverNatesclawPluginsMock = vi.fn();
const loadPluginManifestMock = vi.fn();

vi.mock("./discovery.js", () => ({
  discoverNatesclawPlugins: (...args: unknown[]) => discoverNatesclawPluginsMock(...args),
}));
...
function setBundledDiscoveryCandidates(candidates: unknown[]) {
  discoverNatesclawPluginsMock.mockReturnValue({
    candidates,
    diagnostics: [],
  });
}
...
function createResolvedBundledSource(params: {
  ...
  return {
    ...
    npmSpec: params.npmSpec ?? `@natesclaw/${params.pluginId}`,
...
describe("bundled plugin sources", () => {
  beforeEach(() => {
    discoverNatesclawPluginsMock.mockReset();
  ...
  it("reuses one process-stable bundled source snapshot", () => {
    ...
    expect(discoverNatesclawPluginsMock).toHaveBeenCalledOnce();
  });
  ...
  it.each([
    [
      ...
      { kind: "npmSpec", value: "@natesclaw/feishu" } as const,
      { pluginId: "feishu", localPath: appBundledPluginRoot("feishu") },
    ],
    [
      "returns undefined for missing npm spec",
      { kind: "npmSpec", value: "@natesclaw/not-found" } as const,
      undefined,
    ],
    [
  ...
  it("forwards an explicit env to bundled discovery helpers", () => {
    ...
    expect(discoverNatesclawPluginsMock).toHaveBeenNthCalledWith(1, {
      workspaceDir: "/workspace",
      env,
    });
    expect(discoverNatesclawPluginsMock).toHaveBeenNthCalledWith(2, {
      workspaceDir: "/workspace",
      env,
    });
  });
  ...
  it("marks bundled sources that require plugin config before activation", () => {
    setBundledDiscoveryCandidates([
      createBundledCandidate({
        ...
        packageName: "@natesclaw/memory-lancedb",
      }),
    ]);
    setBundledManifestIdsByRoot({
  ...
  it("reuses a pre-resolved bundled map for repeated lookups", () => {
    ...
    expect(
      findBundledPluginSourceInMap({
        bundled,
        lookup: { kind: "npmSpec", value: "@natesclaw/feishu" },
      })?.pluginId,
    ).toBe("feishu");
  });
});

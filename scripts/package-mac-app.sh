...
# Build and bundle Natesclaw into a minimal .app we can open.
# Outputs to dist/Natesclaw.app
...
DEFAULT_APP_ROOT="$ROOT_DIR/dist/Natesclaw.app"
APP_ROOT="${NATESCLAW_PACKAGE_APP_ROOT:-$DEFAULT_APP_ROOT}"
case "$APP_ROOT" in
  "$ROOT_DIR/dist/"*) ;;
  *)
    echo "ERROR: NATESCLAW_PACKAGE_APP_ROOT must stay under $ROOT_DIR/dist" >&2
    exit 1
    ;;
esac
...
BUNDLE_ID="${BUNDLE_ID:-ai.natesclaw.mac.debug}"
PKG_VERSION="$(cd "$ROOT_DIR" && node -p "require('./package.json').version" 2>/dev/null || echo "0.0.0")"
BUILD_CONFIG="${BUILD_CONFIG:-debug}"
BUILD_TS="$(natesclaw_resolve_build_timestamp)"
if [[ "$BUILD_CONFIG" == "release" ]]; then
  NATESCLAW_REQUIRE_BUILD_METADATA=1
fi
...
export NATESCLAW_BUILD_TIMESTAMP="$BUILD_TS"
if natesclaw_is_full_git_commit "$BUILD_GIT_COMMIT"; then
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/natesclaw/natesclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
restore_swiftpm_resource_sources() {
  ...
  for source_file in "${PATCHED_SWIFTPM_RESOURCE_SOURCES[@]:-}"; do
    ...
    backup_file="$source_file.natesclaw-original"
...
patch_swiftpm_resource_lookups() {
  ...
  for source_file in "${source_files[@]}"; do
    ...
    if [[ -e "$source_file.natesclaw-original" ]]; then
      echo "ERROR: Stale SwiftPM resource source backup at $source_file.natesclaw-original" >&2
      return 1
    fi
    cp -p "$source_file" "$source_file.natesclaw-original"
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Natesclaw/Resources/Info.plist"
if [ ! -f "$INFO_PLIST_SRC" ]; then
  echo "ERROR: Info.plist template missing at $INFO_PLIST_SRC" >&2
  exit 1
fi
cp "$INFO_PLIST_SRC" "$APP_ROOT/Contents/Info.plist"
PORT_GUARDIAN_STORAGE_VERSION="$(plist_print_required "$APP_ROOT/Contents/Info.plist" NatesclawPortGuardianStorageVersion)"
if [[ ! "$PORT_GUARDIAN_STORAGE_VERSION" =~ ^[1-9][0-9]*$ ]]; then
  echo "ERROR: NatesclawPortGuardianStorageVersion must be a positive integer." >&2
  exit 1
fi
...
plist_set_string_required "$APP_ROOT/Contents/Info.plist" NatesclawBuildTimestamp "$BUILD_TS"
plist_set_string_required "$APP_ROOT/Contents/Info.plist" NatesclawGitCommit "$BUILD_GIT_COMMIT"
if [[ "$BUILD_CONFIG" == "release" ]]; then
  EMBEDDED_GIT_COMMIT="$(plist_print_required "$APP_ROOT/Contents/Info.plist" NatesclawGitCommit)"
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Natesclaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Natesclaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Natesclaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Natesclaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Natesclaw/Resources/Natesclaw.icns" "$APP_ROOT/Contents/Resources/Natesclaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Natesclaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
running_packaged_app_pids() {
  ...
  local app_binary="$APP_ROOT/Contents/MacOS/Natesclaw"
  local pid
...
stop_packaged_app_if_running() {
  ...
  echo "⏹  Stopping packaged Natesclaw bundle (${pids[*]})"
  ...
  echo "ERROR: Packaged Natesclaw bundle did not exit: ${pids[*]}" >&2
  return 1
}

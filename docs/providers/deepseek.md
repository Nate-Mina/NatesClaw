...
title: "DeepSeek"
read_when:
  - You want to use DeepSeek with Natesclaw
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    natesclaw onboard --auth-choice deepseek-api-key
    ```
  ...
  <Step title="Verify models are available">
    ```bash
    natesclaw models list --provider deepseek
    ```

    To inspect the plugin's static catalog without a running Gateway:

    ```bash
    natesclaw models list --all --provider deepseek
    ```
...
available to that process (for example, in `~/.natesclaw/.env` or via
`env.shellEnv`).
</Warning>
...
Natesclaw's local cost estimates follow DeepSeek's published cache-hit,
...
<Tip>
V4 models support DeepSeek's `thinking` control. Natesclaw also replays
...
Natesclaw's DeepSeek plugin backfills that field automatically, so normal
...
When thinking is disabled (including the UI **None** selection), Natesclaw
...
```bash
NATESCLAW_LIVE_PROVIDERS=deepseek \
NATESCLAW_LIVE_MODELS="deepseek/deepseek-v4-flash,deepseek/deepseek-v4-pro" \

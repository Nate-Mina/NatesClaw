...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds NATESCLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `natesclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service NATESCLAW_GATEWAY_TOKEN does not match gateway.auth.token in natesclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.NATESCLAW_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.NATESCLAW_GATEWAY_TOKEN);
}
...
async function auditGatewayRuntime(
  ...
  if (isBunRuntime(execPath)) {
    issues.push({
      ...
      message: "Gateway service uses Bun; Natesclaw runtime state requires node:sqlite.",
      detail: execPath,
      level: "recommended",
    });
    return;

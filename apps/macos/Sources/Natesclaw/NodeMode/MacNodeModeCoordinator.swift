...
final class MacNodeModeCoordinator: NSObject {
    ...
    private let logger = Logger(subsystem: "ai.natesclaw", category: "mac-node")
    ...
    override private convenience init() {
        ...
        let nodeHostWorker = MacNodeHostWorker(session: session) {
            NotificationCenter.default.post(name: .natesclawNodeHostWorkerFailed, object: nil)
        }
        self.init(
            session: session,
        ...
        nodeHostWorkerRetryPolicy: MacNodeHostWorkerRetryPolicy = MacNodeHostWorkerRetryPolicy())
    {
        ...
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.refreshNodeConfiguration),
            name: .natesclawPermissionsChanged,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostWorkerFailed),
            name: .natesclawNodeHostWorkerFailed,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostConfigurationChanged),
            name: .natesclawConfigDidChange,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostConfigurationChanged),
            name: .natesclawCLIInstalled,
            object: nil)
    }
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = NatesclawConfigFile.browserControlEnabled()
        ...
        claudeSessionCatalogEnabled: Bool) async throws -> ConnectionAttempt?
    {
        ...
        if !nativeCaps.contains(NatesclawCapability.computer.rawValue) {
            await self.runtime.releaseHeldComputerInput()
        }
            ...
            onInvoke: { [weak self] req in
                guard let self else {
                    return BridgeInvokeResponse(
                        id: req.id,
                        ok: false,
                        error: NatesclawNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                }
        ...
        claudeSessionCatalogEnabled: Bool) -> [String]
    {
        ...
        return Self.resolvedCaps(
            ...
            cameraEnabled: cameraEnabled,
            computerControlEnabled: computerControlEnabled,
            locationMode: NatesclawLocationMode(rawValue: rawLocationMode) ?? .off,
    ...
    private func handleNodeHostWorkerFailure() {
        ...
        case let .giveUp(unexpectedExitCount):
            ...
            self.notificationCenter.post(
                name: .natesclawNodeHostWorkerRetryExhausted,
                object: self,
...
extension MacNodeModeCoordinator {
    ...
    nonisolated static func resolvedCaps(
        ...
        locationMode: NatesclawLocationMode,
        ...
        claudeSessionCatalogEnabled: Bool = false) -> [String]
    {
        ...
        if computerControlEnabled {
            caps.append(NatesclawCapability.computer.rawValue)
        }
    ...
    nonisolated static func resolvedCommands(caps: [String]) -> [String] {
        ...
        if capsSet.contains(NatesclawCapability.camera.rawValue) {
            ...
            commands.append(NatesclawCameraCommand.ptzStatus.rawValue)
            commands.append(NatesclawCameraCommand.ptzControl.rawValue)
        }
        if capsSet.contains(NatesclawCapability.location.rawValue) {
        ...
        if capsSet.contains(NatesclawCapability.computer.rawValue) {
            commands.append(NatesclawComputerCommand.act.rawValue)
        }

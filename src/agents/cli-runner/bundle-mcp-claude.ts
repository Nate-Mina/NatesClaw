...
export function findClaudeMcpConfigPaths(args?: string[]): string[] {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--mcp-config") {
      ...
      // as positional prompts after Natesclaw injects its strict overlay.
...
/** Return Claude args with Natesclaw's strict MCP config path injected. */
...
/** Writes the active per-attempt capture token into Natesclaw's generated Claude MCP config. */
export async function writeClaudeMcpCaptureConfig(params: {
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify(
      withNatesclawMcpCaptureHeader(
        raw,
        params.captureKey,
        "Claude MCP capture requires an natesclaw server config",
      ),
      null,
      2,
    )}\n`,

...
const mocks = vi.hoisted(() => ({
  replaceConfigFile: vi.fn(async (_params: { nextConfig: NatesclawConfig }) => {}),
}));
...
type GatewayAuthConfig = NonNullable<NonNullable<NatesclawConfig["gateway"]>["auth"]>;
...
function gatewayAuthConfig(auth: GatewayAuthConfig): NatesclawConfig {
  return {
    gateway: { auth },
  };
}

function gatewayAuthConfigWithDefaultEnvProvider(auth: GatewayAuthConfig): NatesclawConfig {
  return {
...
describe("mergeGatewayTailscaleConfig", () => {
  ...
  it("preserves explicit serviceName overrides", () => {
    expect(
      mergeGatewayTailscaleConfig(
        { mode: "serve", serviceName: "svc:old-natesclaw", resetOnExit: false },
        { serviceName: "svc:natesclaw" },
      ),
    ).toEqual({ mode: "serve", serviceName: "svc:natesclaw", resetOnExit: false });
  });
});
...
describe("ensureGatewayStartupAuth", () => {
  ...
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: NatesclawConfig) {
  ...
  async function expectNoTokenGeneration(cfg: NatesclawConfig, mode: string) {
    const result = await runStartupAuth({
      cfg,
      persist: true,
    });
  ...
  function createMissingGatewayTokenSecretRefConfig(): NatesclawConfig {
  ...
  it("resolves env-template gateway.auth.token before env-token short-circuiting", async () => {
    await expectResolvedToken({
      cfg: gatewayAuthConfig({ mode: "token", token: "${NATESCLAW_GATEWAY_TOKEN}" }),
      env: {
        NATESCLAW_GATEWAY_TOKEN: "resolved-token",
      } as NodeJS.ProcessEnv,
      expectedToken: "resolved-token",
      expectedConfiguredToken: "${NATESCLAW_GATEWAY_TOKEN}",
    });
  });

  it("keeps configured token SecretRef ahead of NATESCLAW_GATEWAY_TOKEN", async () => {
    const configuredToken = gatewayEnvSecretRef("GW_TOKEN");
    await expectResolvedToken({
      cfg: gatewayAuthConfigWithDefaultEnvProvider({
        mode: "token",
      ...
      env: {
        ...
        NATESCLAW_GATEWAY_TOKEN: "token-from-env",
      } as NodeJS.ProcessEnv,
  ...
  it("does not let NATESCLAW_GATEWAY_TOKEN mask an unresolved configured token ref", async () => {
    await expect(
      runStartupAuth({
        cfg: createMissingGatewayTokenSecretRefConfig(),
        env: { NATESCLAW_GATEWAY_TOKEN: "token-from-env" } as NodeJS.ProcessEnv,
        persist: true,
      }),
  ...
  it("keeps configured password SecretRef ahead of NATESCLAW_GATEWAY_PASSWORD", async () => {
    const configuredPassword = gatewayEnvSecretRef("GW_PASSWORD");
    const result = await runStartupAuth({
      cfg: gatewayAuthConfigWithDefaultEnvProvider({
        mode: "password",
  ...
  it("does not let NATESCLAW_GATEWAY_PASSWORD mask an unresolved configured password ref", async () => {
    await expect(
      runStartupAuth({
        cfg: gatewayAuthConfigWithDefaultEnvProvider({
          mode: "password",
        ...
        env: { NATESCLAW_GATEWAY_PASSWORD: "password-from-env" } as NodeJS.ProcessEnv,
        persist: true,
      }),
  ...
  it("keeps startup non-breaking when hooks token reuses gateway token resolved from env", async () => {
    ...
    const result = await runStartupAuth({
      ...
      env: {
        NATESCLAW_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
      } as NodeJS.ProcessEnv,
      warn,
    });
    ...
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("natesclaw security audit"));
  });

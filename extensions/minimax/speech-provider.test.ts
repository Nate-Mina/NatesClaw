...
vi.mock("natesclaw/plugin-sdk/media-runtime", () => ({
...
describe("buildMinimaxSpeechProvider", () => {
  ...
  describe("isConfigured", () => {
    ...
    beforeEach(async () => {
      tempStateDir = await mkdtemp(path.join(tmpdir(), "natesclaw-minimax-tts-auth-"));
      ...
      process.env.NATESCLAW_AGENT_DIR = tempAgentDir;
      clearMinimaxAuthEnv();
      clearRuntimeAuthProfileStoreSnapshots();
    });
  ...
  describe("synthesize", () => {
    ...
    beforeEach(async () => {
      tempStateDir = await mkdtemp(path.join(tmpdir(), "natesclaw-minimax-tts-synth-"));
      tempAgentDir = path.join(tempStateDir, "agents", "main", "agent");
      await mkdir(tempAgentDir, { recursive: true });
      process.env = {
        ...savedEnv,
        NATESCLAW_AGENT_DIR: tempAgentDir,
        NATESCLAW_STATE_DIR: tempStateDir,
      };
      clearMinimaxAuthEnv();
      clearRuntimeAuthProfileStoreSnapshots();

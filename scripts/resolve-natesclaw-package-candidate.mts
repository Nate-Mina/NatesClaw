...
const DEFAULT_OUTPUT_NAME = "natesclaw-current.tgz";
...
const TRUSTED_PACKAGE_SOURCE_TOKEN_ENV = "NATESCLAW_TRUSTED_PACKAGE_TOKEN";
...
export const NATESCLAW_PACKAGE_SPEC_RE =
  /^natesclaw@(alpha|beta|extended-stable|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node --import tsx scripts/resolve-natesclaw-package-candidate.mts --source <ref|npm|url|trusted-url|artifact> --output-dir <dir> [options]
...
function resolvePackedNatesclawTarballFilename(value: unknown) {
  const filename = typeof value === "string" ? value.trim() : "";
  if (
    !/^natesclaw-[A-Za-z0-9._-]+\.tgz$/u.test(filename) ||
    filename.includes("\0") ||
    filename !== path.basename(filename) ||
    filename !== path.win32.basename(filename)
  ) {
    throw new Error(
      `npm pack reported unsafe Natesclaw tarball filename: ${JSON.stringify(filename)}`,
    );
  }
  return filename;
}

export function validateNatesclawPackageSpec(spec: string) {
  if (!NATESCLAW_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be natesclaw@alpha, natesclaw@beta, natesclaw@extended-stable, natesclaw@latest, or an exact Natesclaw release version; got: ${spec}`,
    );
  }
}
...
export function resolveNpmPackageCandidatePackRunner(
  ...
  validateNatesclawPackageSpec(packageSpec);
...
async function resolveTrustedRepoRef(ref: string) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an Natesclaw branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref: string) {
  ...
  const sourceDir = path.join(
    ...
    `natesclaw-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir: string, packOutput: string, outputName: string) {
  ...
  if (parsed !== undefined) {
    ...
    if (packedFilename) {
      filename = resolvePackedNatesclawTarballFilename(packedFilename);
    }
  }
  if (!filename) {
    for (const line of packOutput.split(/\r?\n/u)) {
      ...
      if (
        ...
        (trimmed.startsWith("natesclaw-") ||
          trimmed.includes(":") ||
          trimmed.includes("/") ||
          trimmed.includes("\\"))
      ) {
        filename = resolvePackedNatesclawTarballFilename(trimmed);
      }
    }
  }
  if (!filename) {
    ...
    filename =
      entries
        .filter((entry) => {
          try {
            return resolvePackedNatesclawTarballFilename(entry) === entry;
          } catch {
            return false;
          }
        })
  ...
  if (!filename) {
    throw new Error(`npm pack produced no Natesclaw tarball in ${outputDir}`);
  }
...
async function cleanPackedNatesclawTarballs(outputDir: string) {
  let entries: string[];
  try {
...
export const cleanPackedNatesclawTarballsForTest = cleanPackedNatesclawTarballs;
...
async function resolveCandidate(options: PackageCandidateOptions) {
  ...
  try {
    if (options.source === "ref") {
      ...
      await run("node", [
        "scripts/package-natesclaw-for-docker.mjs",
      ...
      await cleanPackedNatesclawTarballs(outputDir);
  ...
  console.error(`Checking Natesclaw package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-natesclaw-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
    `Natesclaw package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
  ...
  if (pkg.name !== "natesclaw") {
    throw new Error(`package candidate must be named "natesclaw"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {

...
import type { WhatsAppQaDriverSession } from "@natesclaw/whatsapp/api.js";
...
export async function createWhatsAppQaTransportAdapter(
  ...
  try {
    authRoot = await fs.mkdtemp(
      path.join(resolvePreferredNatesclawTmpDir(), "natesclaw-whatsapp-qa-adapter-"),
    );
  ...
  return {
    ...
    createGatewayConfig: () =>
      buildWhatsAppQaConfig({} as NatesclawConfig, {

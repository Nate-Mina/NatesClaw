...
describe("resolveAgentConfig", () => {
  it("should return undefined when agent id does not exist", () => {
    const cfg: NatesclawConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/natesclaw" }],
      },
    };
        ...
        list: [
          {
            ...
            name: "Main Agent",
            workspace: "~/natesclaw",
            agentDir: "~/.natesclaw/agents/main",
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: NatesclawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: NatesclawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: NatesclawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStringModel: NatesclawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStrictAgentWithDefaultFallbacks: NatesclawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgDisable: NatesclawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaultsWithoutAgentModel: NatesclawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("updates the effective model primary at the winning config layer", () => {
    ...
    const inheritedCfg: NatesclawConfig = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: NatesclawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: NatesclawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/natesclaw");
  });

  it("uses NATESCLAW_HOME for default agent workspace", () => {
    ...
    withEnv({ NATESCLAW_HOME: home }, () => {
      ...
      expect(workspace).toBe(path.join(path.resolve(home), ".natesclaw", "workspace"));
    });
  });

  it("uses NATESCLAW_WORKSPACE_DIR for default agent workspace", () => {
  ...
  it("uses NATESCLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "natesclaw-home");
    withEnv({ NATESCLAW_HOME: home, NATESCLAW_STATE_DIR: "" }, () => {
      const agentDir = resolveAgentDir({} as NatesclawConfig, "main");
      expect(agentDir).toBe(path.join(path.resolve(home), ".natesclaw", "agents", "main", "agent"));
    });
  });
  ...
  it("resolves default agentDir from the configured default agent", () => {
    ...
    const agentDir = withEnv({ NATESCLAW_STATE_DIR: stateDir }, () => resolveDefaultAgentDir(cfg));
  ...
  it("non-default agent without defaults.workspace falls back to stateDir", () => {
    ...
    const workspace = withEnv({ NATESCLAW_STATE_DIR: stateDir }, () =>
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/natesclaw-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/natesclaw-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-agent-scope-"));

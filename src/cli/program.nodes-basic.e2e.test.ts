...
describe("cli program (nodes basics)", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedOutput: [
        ...
        "natesclaw nodes approve request-approval",
      ],
    },
    {
      ...
      expectedOutput: [
        ...
        "natesclaw nodes approve request-reapproval",
      ],
    },
  ...
  it("runs nodes describe and calls node.describe", async () => {
    ...
    expect(out).toContain("natesclaw nodes approve request-approval");
  ...
  it("keeps explicit gateway options in node reapproval guidance without leaking auth", async () => {
    ...
    const output = getRuntimeOutput();
    expect(output).toContain("natesclaw nodes approve request-reapproval --timeout 3000");
  ...
  it("does not recommend approval from a stale pending request id alone", async () => {
    ...
    expect(output).not.toContain("natesclaw nodes approve stale-request");

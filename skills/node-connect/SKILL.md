---
name: node-connect
description: "Diagnose Natesclaw Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures."
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Natesclaw is advertising that route, then fix pairing/auth.
...
- whether `natesclaw devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `natesclaw qr --json`. It uses the same setup-code payload Android scans.

```bash
natesclaw config get gateway.mode
natesclaw config get gateway.bind
natesclaw config get gateway.tailscale.mode
natesclaw config get gateway.remote.url
natesclaw config get gateway.auth.mode
natesclaw config get gateway.auth.allowTailscale
natesclaw config get plugins.entries.device-pair.config.publicUrl
natesclaw qr --json
natesclaw devices list
natesclaw nodes status
```

If this Natesclaw instance is pointed at a remote gateway, also run:

```bash
natesclaw qr --remote --json
```
...
`natesclaw qr --json` success means:
...
If `natesclaw qr --json` says `Gateway is only bound to loopback`:
...
- `natesclaw devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run natesclaw qr again, rescan, then approve the pending device pairing.`

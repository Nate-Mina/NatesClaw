...
struct MacNodeRuntimeTests {
    ...
    actor CanvasRefreshProbe {
        ...
        func refresh() -> String? {
            self.calls += 1
            return "http://127.0.0.1:18789/__natesclaw__/cap/refreshed-token"
        }
    }

    actor CanvasReconnectProbe {
        private var surfaceURL = "http://127.0.0.1:18789/__natesclaw__/cap/old-token"
        ...
        func reconnectDuringRefresh() -> String? {
            self.surfaceURL = "http://127.0.0.1:18789/__natesclaw__/cap/new-token"
            return nil
        }
    }
    ...
    final class MainActorServicesProbe: MacNodeRuntimeMainActorServices, @unchecked Sendable {
        typealias SnapshotInspection = (Int?, Int?, Double?, NatesclawScreenSnapshotFormat?) -> Void
        ...
        var receivedParams: NatesclawComputerActParams?
        var actError: Error?
        ...
        func snapshotScreen(
            ...
            maxWidth: Int?,
            quality: Double?,
            format: NatesclawScreenSnapshotFormat?) async throws -> ScreenSnapshotResult
        {
            ...
            outPath _: String?) async throws -> (path: String, hasAudio: Bool)
        {
            let url = FileManager().temporaryDirectory
                .appendingPathComponent("natesclaw-test-screen-record-\(UUID().uuidString).mp4")
        ...
        func currentLocation(
            desiredAccuracy: NatesclawLocationAccuracy,
            maxAgeMs: Int?,
        ...
        func performComputerAct(
            _ params: NatesclawComputerActParams,
            lifecycleGeneration: UInt64) async throws -> NatesclawComputerActResult
        {
            ...
            return NatesclawComputerActResult(ok: true, cursorX: params.x ?? 0, cursorY: params.y ?? 0)
        }
    ...
    @Test func `A2UI host capability refresh uses injected node session refresher`() async {
        ...
        let resolver = MacNodeCanvasHostedSurfaceResolver(
            currentSurfaceURL: { "http://127.0.0.1:18789/__natesclaw__/cap/current-token" },
            refreshSurfaceURL: { _ in await probe.refresh() })

        let current = await resolver.resolveA2UIURL()
        #expect(current ==
            "http://127.0.0.1:18789/__natesclaw__/cap/current-token/__natesclaw__/a2ui/?platform=macos")
        #expect(await probe.calls == 0)

        let refreshed = await resolver.resolveA2UIURL(forceRefresh: true)
        #expect(refreshed ==
            "http://127.0.0.1:18789/__natesclaw__/cap/refreshed-token/__natesclaw__/a2ui/?platform=macos")
    ...
    @Test func `hosted Canvas commands refresh capability and preserve target components`() async throws {
        ...
        let resolved = try await resolver.resolveTarget(
            "/__natesclaw__/canvas/demo%20page.html?mode=proof#result")
        #expect(resolved?.url.absoluteString ==
            "http://127.0.0.1:18789/__natesclaw__/cap/refreshed-token/__natesclaw__/canvas/demo%20page.html?mode=proof#result")
    ...
    @Test func `hosted Canvas commands use replacement route after refresh fails`() async throws {
        ...
        let resolved = try await resolver.resolveTarget("/__natesclaw__/canvas/demo.html")

        #expect(resolved?.url.absoluteString ==
            "http://127.0.0.1:18789/__natesclaw__/cap/new-token/__natesclaw__/canvas/demo.html")
    }

    @Test func `handle invoke rejects empty notification`() async throws {
        let runtime = MacNodeRuntime()
        let params = NatesclawSystemNotifyParams(title: "", body: "")
        let response = try await self.invoke(
            runtime, "req-3", NatesclawSystemCommand.notify.rawValue, params: params)
        #expect(response.ok == false)
    }
    ...
    @Test func `handle invoke camera list requires enabled camera`() async {
        await TestIsolation.withUserDefaultsValues([cameraEnabledKey: false]) {
            let runtime = MacNodeRuntime()
            let response = await self.invoke(runtime, "req-4", NatesclawCameraCommand.list.rawValue)
            #expect(response.ok == false)
    ...
    @Test func `handle invoke rejects computer act when control disabled`() async throws {
        ...
        let params = NatesclawComputerActParams(action: .leftClick, x: 5, y: 6, refWidth: 1280)
        let response = try await self.invoke(
            runtime, "req-computer-disabled", NatesclawComputerCommand.act.rawValue, params: params)
    ...
    @Test func `handle invoke routes computer act to the injected services when enabled`() async throws {
        ...
        let params = NatesclawComputerActParams(action: .leftClick, x: 12, y: 34, refWidth: 1280)
        let response = try await self.invoke(
            runtime, "req-computer-ok", NatesclawComputerCommand.act.rawValue, params: params)
        ...
        let payloadJSON = try #require(response.payloadJSON)
        let result = try JSONDecoder().decode(NatesclawComputerActResult.self, from: Data(payloadJSON.utf8))
    ...
    @Test func `concurrent invokes share one main actor services initialization`() async throws {
        ...
        let first = Task {
            await self.invoke(runtime, "req-computer-single-flight-1", NatesclawComputerCommand.act.rawValue, json)
        }
        try #require(await self.waitForCount(1, counter: factoryCalls))
        let second = Task {
            await self.invoke(runtime, "req-computer-single-flight-2", NatesclawComputerCommand.act.rawValue, json)
        }
    ...
    @Test func `lifecycle release invalidates first invoke awaiting service initialization`() async throws {
        ...
        let runtime = MacNodeRuntime(
            ...
            computerControlEnabled: { true })
        let params = NatesclawComputerActParams(action: .leftMouseDown, x: 12, y: 34, refWidth: 1280)
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let invoke = Task {
            await self.invoke(runtime, "req-computer-release-during-init", NatesclawComputerCommand.act.rawValue, json)
        }
        try #require(await self.waitForCount(1, counter: factoryCalls))
    ...
    @Test func `lifecycle release after runtime admission invalidates service execution`() async throws {
        ...
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let invoke = Task {
            await self.invoke(runtime, "req-computer-release-after-admission", NatesclawComputerCommand.act.rawValue, json)
        }
    ...
    @Test func `handle invoke maps accessibility denial to unavailable`() async throws {
        ...
        let params = NatesclawComputerActParams(action: .leftClick, x: 1, y: 1, refWidth: 1280)
        let response = try await self.invoke(
            runtime, "req-computer-ax", NatesclawComputerCommand.act.rawValue, params: params)

        #expect(response.ok == false)
        #expect(response.error?.code == .unavailable)
        #expect(response.error?.message == "ACCESSIBILITY_REQUIRED: grant Accessibility permission to Natesclaw")
    }
    ...
    @Test func `handle invoke rejects malformed computer act params`() async {
        ...
        let response = await self.invoke(
            runtime, "req-computer-bad", NatesclawComputerCommand.act.rawValue, #"{"action":"#)

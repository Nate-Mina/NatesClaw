...
DEFAULT_SCREENSHOT_AVD="Natesclaw_Screenshots_API36"
DEFAULT_SCREENSHOT_DEVICE_PROFILE="pixel_2"
DEFAULT_WEAR_SCREENSHOT_AVD="Natesclaw_Wear_Screenshots_API34"
...
ACTIVITY_COMPONENT="ai.natesclaw.app/.MainActivity"
EMULATOR_PID=""
EMULATOR_LOG=""
STARTED_EMULATOR=0
...
case "$FORM_FACTOR" in
  ...
  wear)
    ...
    ACTIVITY_COMPONENT="ai.natesclaw.app/ai.natesclaw.wear.MainActivity"
...
boot_emulator() {
  ...
  emulator="$(emulator_bin)"
  EMULATOR_LOG="$(mktemp "${TMPDIR:-/tmp}/natesclaw-android-screenshot-emulator.XXXXXX.log")"
...
scene_ready_text() {
  ...
  case "$1" in
    ...
    gateway) printf '%s\n' "Connection between this phone and Natesclaw." ;;
    *)
...
"$ADB_BIN" -s "$ADB_SERIAL" shell pm clear ai.natesclaw.app >/dev/null
"$ADB_BIN" -s "$ADB_SERIAL" shell pm grant ai.natesclaw.app android.permission.RECORD_AUDIO >/dev/null
"$ADB_BIN" -s "$ADB_SERIAL" logcat -c >/dev/null 2>&1 || true

for scene in "${SCENES[@]}"; do
  output_path="${OUTPUT_DIR}/natesclaw-${scene}.jpg"
  raw_path="${OUTPUT_DIR}/natesclaw-${scene}.raw.png"
  artifact_path="${ARTIFACT_DIR}/screenshots/natesclaw-${scene}.jpg"
  ui_dump_path="${ARTIFACT_DIR}/ui-dumps/natesclaw-${scene}.xml"
  activity_start_path="${ARTIFACT_DIR}/activity-start/natesclaw-${scene}.txt"
  "$ADB_BIN" -s "$ADB_SERIAL" shell am force-stop ai.natesclaw.app >/dev/null
  ...
  "$ADB_BIN" -s "$ADB_SERIAL" shell am start -W \
    ...
    --ez natesclaw.screenshotMode true \
    --es natesclaw.screenshotScene "$scene" >"$activity_start_path"

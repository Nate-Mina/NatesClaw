...
This directory contains hooks that ship with Natesclaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.natesclaw/workspace`)
...
```bash
natesclaw hooks enable bootstrap-extra-files
```
...
**Output**: `~/.natesclaw/logs/commands.log`

**Enable**:

```bash
natesclaw hooks enable command-logger
```
...
```bash
natesclaw hooks enable boot-md
```
...
- **Managed hooks**: `~/.natesclaw/hooks/` (shared across workspaces)
...
```bash
natesclaw hooks enable session-memory
natesclaw hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.natesclaw/natesclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'natesclaw.*gateway' && pnpm natesclaw gateway`
3. Enable the hook: `natesclaw hooks enable my-hook`

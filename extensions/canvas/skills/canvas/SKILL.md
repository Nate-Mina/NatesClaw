---
name: canvas
description: "Present HTML on connected Natesclaw node canvases, navigate/eval/snapshot, and debug canvas host URLs."
metadata: { "natesclaw": { "emoji": "🖼️" } }
---
...
Active config: `$NATESCLAW_CONFIG_PATH` or `~/.natesclaw/natesclaw.json`.
...
4. Present the hosted URL: `/__natesclaw__/canvas/<file>.html`.
5. Use `snapshot` when the user needs proof.

## URL shape

```text
http://<gateway-host>:<gateway.port>/__natesclaw__/canvas/index.html
http://<gateway-host>:<gateway.port>/__natesclaw__/canvas/games/snake.html
```

Path mapping:

- `/__natesclaw__/canvas/index.html` -> `<canvas host root>/index.html`
- `/__natesclaw__/canvas/games/snake.html` -> `<canvas host root>/games/snake.html`

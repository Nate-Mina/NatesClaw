---
summary: "Run Natesclaw Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Natesclaw running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent Natesclaw Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.
...
4. Clone the Natesclaw repository
...
<Steps>
  ...
  <Step title="Create a GCP project">
    ```bash
    gcloud projects create my-natesclaw-project --name="Natesclaw Gateway"
    gcloud config set project my-natesclaw-project
  ...
  <Step title="Create the VM">
    ...
    ```bash
    gcloud compute instances create natesclaw-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    ```bash
    gcloud compute ssh natesclaw-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    NATESCLAW_IMAGE=natesclaw:latest
    NATESCLAW_GATEWAY_TOKEN=
    NATESCLAW_GATEWAY_BIND=lan
    NATESCLAW_GATEWAY_PORT=18789

    NATESCLAW_CONFIG_DIR=/home/$USER/.natesclaw
    NATESCLAW_WORKSPACE_DIR=/home/$USER/.natesclaw/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.natesclaw
    ```

    Set `NATESCLAW_GATEWAY_TOKEN` to manage the stable gateway token through
    `.env`; otherwise configure `gateway.auth.token` before relying on clients
    across restarts. If neither is set, Natesclaw uses a runtime-only token for
    ...
    **Do not commit this file.** It holds container/runtime env such as
    `NATESCLAW_GATEWAY_TOKEN`. Stored provider OAuth/API-key auth lives in the
    mounted `~/.natesclaw/agents/<agentId>/agent/auth-profiles.json`.
    ...
    services:
      natesclaw-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - NATESCLAW_GATEWAY_BIND=${NATESCLAW_GATEWAY_BIND}
          - NATESCLAW_GATEWAY_PORT=${NATESCLAW_GATEWAY_PORT}
          - NATESCLAW_GATEWAY_TOKEN=${NATESCLAW_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${NATESCLAW_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`NATESCLAW_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm natesclaw-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh natesclaw-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop natesclaw-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type natesclaw-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start natesclaw-gateway --zone=us-central1-a
```
...
```bash
gcloud iam service-accounts create natesclaw-deploy \
  --display-name="Natesclaw Deployment"

gcloud projects add-iam-policy-binding my-natesclaw-project \
  --member="serviceAccount:natesclaw-deploy@my-natesclaw-project.iam.gserviceaccount.com" \

    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "natesclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "natesclaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "natesclaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("natesclaw-");
  const acpxRoot = await tempDirs.make("natesclaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("natesclaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("natesclaw-");
  const pluginRoot = await tempDirs.make("natesclaw-plugin-");
  const outsideDir = await tempDirs.make("natesclaw-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("natesclaw-");
    const pluginRoot = await tempDirs.make("natesclaw-claude-bundle-");
  ...
  it("limits Agent Plugins skills to valid immediate child directories", async () => {
    const workspaceDir = await tempDirs.make("natesclaw-");
    const pluginRoot = await tempDirs.make("natesclaw-agent-bundle-");
    const pluginSkillsDir = await tempDirs.make("managed-plugin-skills-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("natesclaw-");
    const pluginRoot = await tempDirs.make("natesclaw-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });

...
import {
  ...
  repairManagedNpmRootNatesclawPeer,
  removeManagedNpmRootDependency,
  readManagedNpmRootInstalledDependency,
  readNatesclawManagedNpmRootOverrides,
...
const fixtureRootTracker = createSuiteTempRootTracker({
  prefix: "natesclaw-npm-managed-root-",
});
...
describe("managed npm root", () => {
  ...
  it("syncs Natesclaw-owned overrides without dropping unrelated local overrides", async () => {
    const npmRoot = await makeTempRoot();
    await fs.writeFile(
      path.join(npmRoot, "package.json"),
      `${JSON.stringify(
  ...
  it("reads workspace pnpm overrides for managed plugin installs", async () => {
    ...
    await expect(readNatesclawManagedNpmRootOverrides()).resolves.toEqual(expectedOverrides);
  });
  ...
  it("resolves npm override dependency references from the host package manifest", async () => {
    ...
    await expect(readNatesclawManagedNpmRootOverrides({ packageRoot })).resolves.toEqual({
      "managed-runtime": "3.1024.0",
      nested: {
        "optional-runtime": "2.0.0",
    ...
    expect(
      resolveManagedNpmRootDependencySpec({
        ...
        resolution: {
          name: "@natesclaw/discord",
          version: "2026.5.2",
          resolvedSpec: "@natesclaw/discord@2026.5.2",
      ...
      `${JSON.stringify(
        {
          lockfileVersion: 3,
          packages: {
            "node_modules/@natesclaw/discord": {
              version: "2026.5.2",
              resolved: "https://registry.npmjs.org/@natesclaw/discord/-/discord-2026.5.2.tgz",
    ...
    const runCommand = vi.fn(async (_args: string[], optionsOrTimeout: number | CommandOptions) => {
      ...
      if (runCommand.mock.calls.length === 1) {
        return {
          code: 1,
          stdout: "",
          stderr: "npm ERR! notarget No matching version found for natesclaw@2026.5.99-beta.1",
          signal: null,
          killed: false,
          termination: "exit" as const,
        };
  ...
  it("repairs stale managed natesclaw peer state without dropping plugin packages", async () => {
    const npmRoot = await makeTempRoot();
    await fs.mkdir(path.join(npmRoot, "node_modules", "natesclaw"), { recursive: true });
    await fs.writeFile(
      path.join(npmRoot, "package.json"),
      `${JSON.stringify(
        {
    ...
    await fs.writeFile(
      path.join(npmRoot, "node_modules", "natesclaw", "package.json"),
      `${JSON.stringify({ name: "natesclaw", version: "2026.5.4" })}\n`,
    );
    await fs.mkdir(path.join(npmRoot, "node_modules", ".bin"), { recursive: true });
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "natesclaw"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "natesclaw.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "natesclaw.ps1"), "ps1 shim");
    await fs.writeFile(
    ...
    await expect(repairManagedNpmRootNatesclawPeer({ npmRoot, runCommand })).resolves.toBe(true);
    expect(runCommand).toHaveBeenCalledTimes(1);
    ...
    expect(lockfile.packages?.["node_modules/@natesclaw/discord"]?.version).toBe("2026.5.4");
    expect(lockfile.dependencies?.natesclaw).toBeUndefined();
    await expectPathMissing(path.join(npmRoot, "node_modules", "natesclaw"));
    for (const binName of ["natesclaw", "natesclaw.cmd", "natesclaw.ps1"]) {
  ...
  it("does not repair the active Natesclaw host package in a root-managed install", async () => {
    const npmRoot = await makeTempRoot();
      ...
      `${JSON.stringify(
        {
          ...
          dependencies: {
            ...
            "@xdarkicex/natesclaw-memory-libravdb": "1.4.69",
          },
        },
        null,
        2,
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({ name: "natesclaw", version: "2026.5.12-beta.6" })}\n`,
    );
  ...
  it("scrubs managed ownership metadata without deleting a linked active host package", async () => {
    const npmRoot = await makeTempRoot();
    const hostPackageRoot = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-host-package-"));
    ...
    await fs.symlink(hostPackageRoot, path.join(npmRoot, "node_modules", "natesclaw"), "dir");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "natesclaw"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "natesclaw.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "natesclaw.ps1"), "ps1 shim");
    await fs.writeFile(
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/@xdarkicex/natesclaw-memory-libravdb": {
              version: "1.4.69",
            },
          },
          dependencies: {
    ...
    expect(runCommand).not.toHaveBeenCalled();
    await expect(fs.realpath(path.join(npmRoot, "node_modules", "natesclaw"))).resolves.toBe(
    ...
    expect(lockfile.packages?.["node_modules/natesclaw"]).toBeUndefined();
    expect(lockfile.packages?.["node_modules/@xdarkicex/natesclaw-memory-libravdb"]?.version).toBe(
      "1.4.69",
    );
    expect(lockfile.dependencies?.natesclaw).toBeUndefined();
    for (const binName of ["natesclaw", "natesclaw.cmd", "natesclaw.ps1"]) {

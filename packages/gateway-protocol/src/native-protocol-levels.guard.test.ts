...
describe("native Gateway protocol levels", () => {
  it("match the TypeScript source of truth", async () => {
    ...
    const androidPath = "apps/android/app/src/main/java/ai/natesclaw/app/gateway/GatewayProtocol.kt";
  ...
  it("uses the min constant for native connect compatibility ranges", async () => {
    const swiftChannelPath = "apps/shared/NatesclawKit/Sources/NatesclawKit/GatewayChannel.swift";
    ...
    const swiftWizardPath = "apps/macos/Sources/NatesclawMacCLI/WizardCommand.swift";
    ...
    const androidPath = "apps/android/app/src/main/java/ai/natesclaw/app/gateway/GatewaySession.kt";
    const android = await readRepoFile(androidPath);
    assertPattern(
      android,
      androidPath,

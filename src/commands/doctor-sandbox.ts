...
import { resolveNatesclawPackageRootsSync } from "../infra/natesclaw-root.js";
...
function resolveSandboxScript(
  ...
  // Scan every natesclaw package root the shared resolver finds (symlinked launcher via realpath,
  ...
  const roots = resolveNatesclawPackageRootsSync({
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("natesclaw.doctorSandboxTestApi")] = {
...
function codexBwrapNeedsNetworkNamespaceProbe(cfg: NatesclawConfig): boolean {
  const network = cfg.agents?.defaults?.sandbox?.docker?.network?.trim().toLowerCase();
  return network === undefined || network === "" || network === "none";
}

async function probeCodexBwrapNamespaces(cfg: NatesclawConfig): Promise<CodexBwrapNamespaceProbe> {
...
async function noteCodexBwrapNamespaceWarning(
  ...
  const lines = [
    ...
    "",
    "Fix the host namespace policy for the Natesclaw service user, then restart the gateway.",
    "Prefer an AppArmor profile that grants the required namespaces to the Natesclaw service process.",
...
function resolveSandboxDockerImage(cfg: NatesclawConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.docker?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_IMAGE;
}

function resolveSandboxBackend(cfg: NatesclawConfig): string {
  const backend = cfg.agents?.defaults?.sandbox?.backend?.trim();
  return (backend || "docker").toLowerCase();
}

function resolveSandboxBrowserImage(cfg: NatesclawConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.browser?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_BROWSER_IMAGE;
}

function updateSandboxDockerImage(cfg: NatesclawConfig, image: string): NatesclawConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
function updateSandboxBrowserImage(cfg: NatesclawConfig, image: string): NatesclawConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
    ...
    const lines =
      containerEngine.id === "docker"
        ? [
            ...
            "- Disable sandbox mode: natesclaw config set agents.defaults.sandbox.mode off",
          ]
        : [
...
${formatLegacyRegistryInspectionLine(file)}`,
    path: file.path,
    fixHint: `Run ${formatCliCommand("natesclaw doctor --fix")} to migrate valid entries to SQLite.`,
  };
}
  ...
  if (!prompter.shouldRepair) {
    note(
      [
        ...
        `Run ${formatCliCommand("natesclaw doctor --fix")} to migrate them to SQLite.`,
      ].join("\n"),
      "Sandbox",
    );
    return;
...
export function noteSandboxScopeWarnings(cfg: NatesclawConfig) {

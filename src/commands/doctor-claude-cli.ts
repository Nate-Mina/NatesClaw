...
function usesClaudeCliModelSelection(cfg: NatesclawConfig): boolean {
...
function resolveClaudeCliAgentIds(cfg: NatesclawConfig): string[] {
...
export function noteClaudeCliHealth(
  ...
  if (!storedProfile && credential?.type !== "api_key_helper") {
    lines.push(`- Natesclaw auth profile: missing (${CLAUDE_CLI_PROFILE_ID}) in ${authStorePath}.`);
    fixHints.push(
    ...
    lines.push(
      `- Natesclaw auth profile: ${CLAUDE_CLI_PROFILE_ID} is wired to provider "${storedProfile.provider}" instead of "${CLAUDE_CLI_PROVIDER}".`,
    );
    fixHints.push(

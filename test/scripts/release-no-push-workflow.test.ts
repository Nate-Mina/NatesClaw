...
const RELEASE_CHECKS = ".github/workflows/natesclaw-release-checks.yml";
...
const LIVE_E2E = ".github/workflows/natesclaw-live-and-e2e-checks-reusable.yml";
const INSTALL_SMOKE = ".github/workflows/install-smoke.yml";
const SHARED_IMAGE_PUBLISHER = ".github/workflows/natesclaw-shared-image-publish-reusable.yml";
const SCHEDULED_LIVE = ".github/workflows/natesclaw-scheduled-live-checks.yml";
const DOCKER_RELEASE = ".github/workflows/docker-release.yml";
const UPDATE_MIGRATION = ".github/workflows/update-migration.yml";
const PERFORMANCE = ".github/workflows/natesclaw-performance.yml";
...
describe("release validation no-push transport", () => {
  it("builds planned live images locally without entering pull fallback", () => {
    ...
    for (const jobName of [
      ...
      expect(runStep?.run, jobName).toContain("NATESCLAW_SKIP_DOCKER_BUILD=0");
      expect(runStep?.run, jobName).not.toContain("NATESCLAW_DOCKER_BUILD_ON_MISSING=1");
    }
  });
  ...
  it("models conditional reusable jobs as permission requests before scheduling", () => {
    const root = mkdtempSync(join(tmpdir(), "natesclaw-permission-graph-"));
  ...
  it("records adopted children before monitoring and cancels only a mismatched workflow SHA", () => {
    ...
    for (const [jobName, stepName] of [
      ...
      ["performance", "Dispatch and monitor Natesclaw Performance"],
    ] as const) {
  ...
  it("keeps the Release SHA wrapper as the durable evidence identity", () => {
    ...
    expect(verify.run).toContain('"natesclaw-release-checks.yml"');
    ...
    expect(dispatch.run).toContain("https://api.github.com/repos/natesclaw/releases/dispatches");
  });
  ...
  it("pins every Full Release Docker caller to artifact-only transport", () => {
    ...
    expect(fullText).toContain("dispatch_and_wait natesclaw-release-checks.yml");
    expect(fullText).toContain("dispatch_and_wait natesclaw-performance.yml");
  ...
  it("builds shared images locally, verifies artifacts, and cannot fall back to a registry", () => {
    ...
    const packageIdentity = step(dockerProducer, "Validate Natesclaw package artifact identity");
    ...
    for (const [name, condition] of [
      [
        "Download current-run Natesclaw Docker E2E package",
        "inputs.package_artifact_run_id == github.run_id",
      ],
      [
        "Download previous-run Natesclaw Docker E2E package",
        "inputs.package_artifact_run_id != github.run_id",
      ],
    ] as const) {
    ...
    expect(functionalBuild.run).toContain(
      "--build-context natesclaw_package=.artifacts/docker-e2e-package",
    );
    ...
    expect(packDockerArtifact.run).toContain(
      'NATESCLAW_SHARED_IMAGE_PACKAGE_SHA256="$PACKAGE_SHA256"',
    );
    expect(packDockerArtifact.run).toContain("archive_sha256=");
    const validatePackage = step(dockerProducer, "Validate Natesclaw Docker E2E package");
    ...
    expect(validatePackage.run).toContain("scripts/check-natesclaw-package-tarball.mjs");
    ...
    for (const name of [
      ...
      expect(consumer.env?.NATESCLAW_DOCKER_E2E_REQUIRE_LOCAL_IMAGE).toContain("no-push-artifact");
      expect(step(consumer, "Download Natesclaw Docker E2E package").with).toMatchObject({
      ...
      expect(loadArtifact.env?.NATESCLAW_SHARED_IMAGE_RUN_ATTEMPT).toBe(
        "${{ needs.prepare_docker_e2e_image.outputs.image_artifact_run_attempt }}",
      );
      expect(loadArtifact.env?.NATESCLAW_SHARED_IMAGE_RUN_ID).toBe(
        "${{ needs.prepare_docker_e2e_image.outputs.image_artifact_run_id }}",
      );
      expect(loadArtifact.run).toContain("shared-image-artifact.sh");
      expect(loadArtifact.run).toContain('NATESCLAW_SHARED_IMAGE_ARCHIVE_SHA256="$ARCHIVE_SHA256"');
      expect(loadArtifact.run).toContain('NATESCLAW_SHARED_IMAGE_PACKAGE_SHA256="$PACKAGE_SHA256"');
    ...
    for (const name of [
      ...
      expect(consumer.env?.NATESCLAW_LIVE_REQUIRE_LOCAL_IMAGE).toContain("no-push-artifact");
    ...
    const requireLocalIndex = liveBuild.indexOf("NATESCLAW_LIVE_REQUIRE_LOCAL_IMAGE");
    ...
    const publisherCallers = readdirSync(".github/workflows")
      ...
      .filter((name) =>
        readFileSync(join(".github/workflows", name), "utf8").includes(
          "natesclaw-shared-image-publish-reusable.yml",
        ),
      );
  ...
  it("routes Docker publication through release publish after immutable npm evidence", () => {
    const dockerRelease = readWorkflow(DOCKER_RELEASE);
    const releasePublishPath = ".github/workflows/natesclaw-release-publish.yml";
    ...
    const callers = readdirSync(".github/workflows")
      ...
      .toSorted();
    expect(callers).toEqual(["natesclaw-release-publish.yml"]);
  ...
  it("fails a missing required local live image before any registry pull", () => {
    const root = mkdtempSync(join(tmpdir(), "natesclaw-live-local-image-"));
...
`,
      ...
      const result = spawnSync("bash", [resolve(LIVE_BUILD)], {
        ...
        env: {
          ...
          NATESCLAW_LIVE_IMAGE: "natesclaw-live-test:required-local",
          NATESCLAW_LIVE_REQUIRE_LOCAL_IMAGE: "1",
      ...
      expect(result.stderr).toContain(
        "Required local live-test image not found: natesclaw-live-test:required-local",
      );
      expect(readFileSync(calls, "utf8")).toBe("image inspect natesclaw-live-test:required-local\n");
    } finally {
      rmSync(root, { force: true, recursive: true });
    }
  });
  ...
  it("fails a missing required local Docker E2E image before pull or build fallback", () => {
    const root = mkdtempSync(join(tmpdir(), "natesclaw-docker-e2e-local-image-"));
    const bin = join(root, "bin");
    const calls = join(root, "docker.log");
    try {
      mkdirSync(bin);
...
`,
      ...
      const result = spawnSync(
        ...
        [
          "-c",
          `source "$1"
docker_e2e_build_or_reuse "natesclaw-e2e:required-local" "required local image test"`,
          "bash",
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            FAKE_DOCKER_LOG: calls,
            NATESCLAW_DOCKER_BUILD_ON_MISSING: "1",
            NATESCLAW_DOCKER_E2E_REQUIRE_LOCAL_IMAGE: "1",
            NATESCLAW_SKIP_DOCKER_BUILD: "1",
            PATH: `${bin}:${process.env.PATH ?? ""}`,
          },
        },
      ...
      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        "Required local Docker E2E image not found: natesclaw-e2e:required-local",
      );
      expect(readFileSync(calls, "utf8")).toBe("image inspect natesclaw-e2e:required-local\n");
    } finally {
      rmSync(root, { force: true, recursive: true });
    }
  });

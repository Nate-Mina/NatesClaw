...
import { startMatrixQaNatesclawCli } from "./scenario-runtime-cli.js";
...
async function createCliRoot(): Promise<{
  ...
  const root = await mkdtemp(
    path.join(resolvePreferredNatesclawTmpDir(), "matrix-qa-cli-stream-error-"),
  );
...
describe("Matrix QA CLI runtime stream errors", () => {
  ...
  it.each([
    ...
    async (streamName, afterExit) => {
      ...
      let session: ReturnType<typeof startMatrixQaNatesclawCli> | undefined;
      let childClosed = false;
      try {
        session = startMatrixQaNatesclawCli({

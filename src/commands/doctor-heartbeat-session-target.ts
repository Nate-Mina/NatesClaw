...
function resolveHeartbeatConfig(cfg: NatesclawConfig, agentId: string): HeartbeatConfig | undefined {
  const defaults = cfg.agents?.defaults?.heartbeat;
  const overrides = resolveAgentConfig(cfg, agentId)?.heartbeat;
  return defaults || overrides ? { ...defaults, ...overrides } : undefined;
}

function listHeartbeatDoctorAgents(cfg: NatesclawConfig) {
 ...
 */
export function describeHeartbeatSessionTargetIssues(cfg: NatesclawConfig): string[] {

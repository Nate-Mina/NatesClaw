...
function previewGatewayLoopbackBind(
  ...
  if (
    finding.ocPath !== "oc://natesclaw.config/gateway/bind" &&
    finding.ocPath !== "oc://natesclaw.config/gateway/customBindHost"
  ) {
    return [];
  }
  return [
...
function previewGatewayNodeDenyCommand(
  ...
  if (
    command === undefined ||
    command === "" ||
    finding.ocPath !== "oc://natesclaw.config/gateway/nodes/commands/deny"
  ) {
    return [];
  }
  return [

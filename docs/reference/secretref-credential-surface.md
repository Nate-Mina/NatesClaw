...
- In scope: strictly user-supplied credentials that Natesclaw does not mint or rotate.
...
### `natesclaw.json` targets (`secrets configure` + `secrets apply` + `secrets audit`)
...
- In `natesclaw.json`, SecretRefs must use structured objects such as `{"source":"env","provider":"default","id":"DISCORD_BOT_TOKEN"}`. Legacy `secretref-env:<ENV_VAR>` marker strings are rejected on SecretRef credential paths; run `natesclaw doctor --fix` to migrate valid markers.

...
describe("trajectory metadata", () => {
  ...
  it("redacts harness argv and local paths with the support redaction rules", () => {
    ...
    process.argv = [
      "node",
      "/Users/tester/project/natesclaw.js",
      "--api-key",
      "super-secret",
      "--config=/Users/tester/.natesclaw/natesclaw.json",
    ];
    try {
      const metadata = buildTrajectoryRunMetadata({
        env: {
          HOME: "/Users/tester",
          NATESCLAW_STATE_DIR: "/Users/tester/.natesclaw",
        },
      ...
      expect(harness.invocation).toEqual([
        "node",
        "~/project/natesclaw.js",
        "--api-key",
        "<redacted>",
        "--config=$NATESCLAW_STATE_DIR/natesclaw.json",
      ]);
      expect(harness.entrypoint).toBe("~/project/natesclaw.js");
  ...
  it("redactPathForSupport returns empty string for null/undefined input", () => {
    const ctx: SupportRedactionContext = { env: {}, stateDir: "/tmp/.natesclaw" };

...
struct NatesclawApp: App {
    ...
    private static let logger = Logger(subsystem: "ai.natesclaw", category: "app")
    ...
    init() {
        if let error = AppProfile.current.validationError {
            ...
            alert.messageText = "Natesclaw profile is invalid"
            alert.informativeText = error.localizedDescription
            alert.runModal()
            Darwin.exit(2)
        }
        if AppProfile.current.isActive,
           !DeviceIdentityStore.configureStateDirectory(NatesclawPaths.stateDirURL)
        {
        ...
        NatesclawLogging.bootstrapIfNeeded()
    ...
    var body: some Scene {
        ...
        Window("Natesclaw Settings", id: SettingsWindowOpener.windowID) {
    ...
    private func applyStatusItemAppearance(paused _: Bool, sleeping _: Bool) {
        ...
        self.statusItem?.button?.toolTip = self.state.voiceWakeMeterActive
            ? "Natesclaw - Voice Wake live meter active"
            : "Natesclaw"
    }
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let webChatAutoLogger = Logger(subsystem: "ai.natesclaw", category: "Chat")
    ...
    override init() {
        ...
        if hasReplacementMetadata, !isReplacementHandoff {
            fputs("Natesclaw replacement handoff authentication failed.\n", stderr)
            Darwin.exit(2)
        }
        ...
        if let exitCode = Self.processExitCode(for: ownership) {
            fputs("Natesclaw profile is already running.\n", stderr)
        ...
        if let instanceOwnershipFailure {
            let alert = NSAlert()
            alert.alertStyle = .critical
            alert.messageText = "Natesclaw could not claim its instance lock"
    ...
    func applicationDidFinishLaunching(_: Notification) {
        ...
        if !AppProfile.current.isActive {
            ...
            case let .continueLaunch(startUpdater):
                if startUpdater {
                    if NatesclawConfigFile.gatewayUpdateChannel() == nil {
        ...
        if launchPolicy.allowsAutomaticPresentation,
           ProcessInfo.processInfo.environment["NATESCLAW_DEBUG_PAIRING_DEMO"] == "1"
        {
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
    ...
    private func scheduleFirstRunOnboardingIfNeeded(gatewayConnected: Bool) async {
           ...
           gatewayConnected
        {
            ...
            // default-agent model cannot run Natesclaw and must stay in onboarding.
            do {
...
final class SparkleUpdaterController: NSObject, UpdaterProviding {
        ...
        onStart: (() -> Void)? = nil)
    {
        self.gatewayUpdateChannelResolver = gatewayUpdateChannelResolver ?? {
            ...
            return NatesclawConfigFile.normalizedGatewayUpdateChannel(response.effectiveChannel)
        }
...
extension SparkleUpdaterController: SPUUpdaterDelegate {
    func allowedChannels(for _: SPUUpdater) -> Set<String> {
        allowedSparkleChannels(
            forGatewayUpdateChannel: self.gatewayUpdateChannel ?? NatesclawConfigFile.gatewayUpdateChannel())
    }

    func bestValidUpdate(in appcast: SUAppcast, for _: SPUUpdater) -> SUAppcastItem? {
        guard self.gatewayUpdateChannel ?? NatesclawConfigFile.gatewayUpdateChannel() == "extended-stable" else {
            return nil
        }

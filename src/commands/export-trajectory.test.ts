...
describe("exportTrajectoryCommand", () => {
  beforeEach(() => {
    ...
    mocks.resolveStorePath.mockReturnValue("/tmp/natesclaw/sessions.json");
    mocks.loadSessionEntryReadOnly.mockReturnValue(undefined);
    mocks.exportTrajectoryForCommand.mockResolvedValue({
      outputDir: "/tmp/workspace/.natesclaw/trajectory-exports/export",
      displayPath: ".natesclaw/trajectory-exports/export",
      sessionId: "session-1",
      eventCount: 2,
      runtimeEventCount: 0,
  ...
  it("points missing session key users at the sessions command", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      "--session-key is required. Run natesclaw sessions to choose a session.",
    );
  ...
  it("preserves direct options when an encoded request omits them", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      "Session not found: agent:main:telegram:direct:123. Run natesclaw sessions to see available sessions.",
    );
    expect(runtime.exit).toHaveBeenCalledWith(1);
  });
  ...
  it.each([
    ...
    [
      "agent template",
      "/tmp/natesclaw/agents/{agentId}/sessions/sessions.json",
      "/tmp/natesclaw/agents/work/sessions/sessions.json",
    ],
  ])(
    ...
    async (_name, store, resolvedStore) => {
      ...
      expect(runtime.error).toHaveBeenCalledWith(
        "Session not found: agent:work:telegram:direct:123. Run natesclaw sessions to see available sessions.",
      );
      expect(runtime.exit).toHaveBeenCalledWith(1);
    },
  );
  ...
  it("uses configured session.store when no explicit store is provided", async () => {
    const runtime = createRuntime();
    mocks.getRuntimeConfig.mockReturnValue({
      session: { store: "/tmp/natesclaw/agents/{agentId}/sessions/sessions.json" },
    });
    mocks.resolveStorePath.mockReturnValue("/tmp/natesclaw/agents/work/sessions/sessions.json");
    ...
    expect(mocks.loadSessionEntryReadOnly).toHaveBeenCalledWith({
      agentId: "work",
      sessionKey: "agent:work:telegram:direct:123",
      storePath: "/tmp/natesclaw/agents/work/sessions/sessions.json",
    });
    expect(runtime.error).toHaveBeenCalledWith(
      "Session not found: agent:work:telegram:direct:123. Run natesclaw sessions to see available sessions.",
    );

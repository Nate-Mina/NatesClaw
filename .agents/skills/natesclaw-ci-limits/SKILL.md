---
name: natesclaw-ci-limits
description: Manage Natesclaw GitHub Actions and Blacksmith CI capacity, runner-registration budgets, fanout caps, main-push single-flight, shard sizing, hosted-runner offload, queue health, and safe ramp-down/ramp-up changes. Use when tuning `.github/workflows/*`, `docs/ci.md`, CI runner labels, matrix `max-parallel`, ClawSweeper/Blacksmith burst protection, CodeQL runner placement, or investigating slow/queued Natesclaw CI.
---

# Natesclaw CI Limits

Use this skill for CI capacity changes, not ordinary test failure triage. The
goal is to keep Natesclaw fast while distinguishing runner registration, runner
...
- GitHub runner registrations for `natesclaw` currently report a 10,000 per
  5-minute bucket in `actions_runner_registration`. Verify the live bucket
  before each tuning pass because GitHub can change it. The `natesclaw`
...
ghx run list -R natesclaw/natesclaw --limit 20 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
...
- **Natesclaw test runtime:** jobs start quickly but one lane dominates wall time.
  Use `$natesclaw-test-performance` instead of runner tuning.
- **Real failing CI:** one job fails after starting. Use `$github:gh-fix-ci` or
  `$natesclaw-testing`, not this skill.
...
ClawSweeper, ClawHub, Clownfish, Natesclaw RTT, and Clawbench.
...
7. Split truly slow tests with `$natesclaw-test-performance`; do not hide a slow
...
./node_modules/.bin/oxfmt --check .github/workflows/ci.yml .github/workflows/codeql-critical-quality.yml docs/ci.md test/scripts/ci-workflow-guards.test.ts .agents/skills/natesclaw-ci-limits/SKILL.md .agents/skills/natesclaw-ci-limits/agents/openai.yaml
git diff --check
```
...
ghx pr checks <pr> -R natesclaw/natesclaw --watch --interval 15
```
...
NATESCLAW_TESTBOX=1 scripts/pr prepare-run <pr>
```

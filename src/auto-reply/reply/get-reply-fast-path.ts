...
function isSlowReplyTestAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    (isVitestRuntimeEnv(env) && env.NATESCLAW_ALLOW_SLOW_REPLY_TESTS === "1") ||
    env.NATESCLAW_STRICT_FAST_REPLY_CONFIG === "0"
  );
}
...
export function withFullRuntimeReplyConfig<T extends NatesclawConfig>(config: T): T {
...
export function resolveGetReplyConfig(params: {
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set NATESCLAW_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
  ...
  return applyMergePatch(params.getRuntimeConfig(), configOverride) as NatesclawConfig;
}

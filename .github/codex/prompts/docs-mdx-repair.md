# Natesclaw Docs MDX Repair Agent

You are repairing generated Natesclaw documentation after a fast MDX validation failure.
...
1. Read `.natesclaw-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".natesclaw-sync/mdx/${LOCALE}.json"`.

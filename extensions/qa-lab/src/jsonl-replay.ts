...
type JsonlReplayResult = {
  transcripts: Array<{
    ...
    cells: { natesclaw: RuntimeParityCell[]; codex: RuntimeParityCell[] };
...
export async function runJsonlReplay(
  ...
  for (const transcriptPath of transcriptPaths) {
    ...
    const cells: { natesclaw: RuntimeParityCell[]; codex: RuntimeParityCell[] } = {
      natesclaw: [],
      codex: [],
    };
    ...
    for (const turn of turns) {
      ...
      cells.natesclaw.push(parity.cells.natesclaw);
...
export function renderJsonlReplayMarkdownReport(report: JsonlReplayMarkdownReport): string {
  ...
  const lines = [
    `# Natesclaw JSONL Replay Report - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",

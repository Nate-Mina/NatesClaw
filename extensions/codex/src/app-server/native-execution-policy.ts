 ...
 * or whether Natesclaw must keep exec/process on a configured node host.
 */
import type { NatesclawConfig } from "natesclaw/plugin-sdk/config-contracts";
import { resolveSandboxRuntimeStatus } from "natesclaw/plugin-sdk/sandbox";
import { getSessionEntry, type SessionEntry } from "natesclaw/plugin-sdk/session-store-runtime";
...
export function resolveCodexNativeExecutionPolicy(params: {
  ...
  return {
    ...
    node,
    blockReason:
      "Natesclaw exec host=node is active for this session. Codex app-server native execution cannot route shell, filesystem, MCP, or app-backed work through the selected Natesclaw node.",
  };
}
...
export function formatCodexNativeNodeExecBlock(params: {
  ...
  return [
    `Codex-native ${params.surface} is unavailable because Natesclaw exec host=node is active for this session.`,
    params.reason ??
      "Codex app-server native execution cannot route execution through the selected Natesclaw node.",
    "Use a normal Codex harness turn so Natesclaw exec/process tools run on the node, or switch exec host to gateway for native Codex app-server execution.",
  ].join(" ");
}
...
function listAgentEntries(config: NatesclawConfig): AgentEntry[] {

...
describe("fleet container runtime", () => {
  ...
  it("uses run or create args according to the requested start mode", async () => {
    ...
    const executor = vi.fn<FleetContainerCommandExecutor>(async (_runtime, args) => {
      ...
      await expect(fs.readFile(environmentFile, "utf8")).resolves.toBe(
        "NATESCLAW_GATEWAY_TOKEN=fake-value\n",
      );
    ...
    const profile = {
      runtime: "podman",
      environment: { NATESCLAW_GATEWAY_TOKEN: "fake-value" },
  ...
  it("normalizes Docker and Podman inspect output", async () => {
      ...
      stdout: JSON.stringify([
        {
          ...
          Config: {
            Env: ["NATESCLAW_GATEWAY_TOKEN=test-auth-token", "FEATURE=a=b"],
            Image: "ghcr.io/natesclaw/natesclaw:latest",
            Labels: { "natesclaw.fleet.tenant": "acme" },
            User: "1000:1000",
          },
          HostConfig: {
    ...
    await expect(
      ...
      running: true,
      labels: { "natesclaw.fleet.tenant": "acme" },
      environment: { NATESCLAW_GATEWAY_TOKEN: "test-auth-token", FEATURE: "a=b" },
  ...
  it.each([
    ...
    async (runtimeName, labelsField, containersField, nameField) => {
        ...
        stdout: JSON.stringify([
          {
            [labelsField]: {
              ...
              "natesclaw.fleet.owner": "owner-id",
            },
            [containersField]: {
      ...
      await expect(
        createFleetContainerRuntime(executor).inspectNetwork(runtimeName, "natesclaw-cell-acme-net"),
      ).resolves.toEqual({
        kind: "ok",
        labels: {
          "natesclaw.fleet.tenant": "acme",
      ...
      expect(executor).toHaveBeenCalledWith(
        runtimeName,
        ["network", "inspect", "natesclaw-cell-acme-net"],
        { allowFailure: true },
      );
    },
  );
  ...
  it("normalizes a network with no attached containers", async () => {
    ...
    await expect(
      createFleetContainerRuntime(executor).inspectNetwork("docker", "natesclaw-cell-acme-net"),
  ...
  it("distinguishes missing networks from unavailable runtimes", async () => {
    const missingExecutor = vi.fn<FleetContainerCommandExecutor>(async () => ({
      stdout: "",
      stderr: "Error: network natesclaw-cell-missing-net not found",
      code: 1,
    }));
    const unavailableExecutor = vi.fn<FleetContainerCommandExecutor>(async () => ({
      stdout: "",
  ...
  it("treats malformed network inspect JSON as unavailable", async () => {
    const executor = vi.fn<FleetContainerCommandExecutor>(async () => ({
      stdout: JSON.stringify([{ Labels: { "natesclaw.fleet.tenant": 42 } }]),
      stderr: "",
      code: 0,
    }));
  ...
  it("treats malformed inspect JSON as unavailable without echoing its output", async () => {
    const executor = vi.fn<FleetContainerCommandExecutor>(async () => ({
      stdout: 'not-json NATESCLAW_GATEWAY_TOKEN="secret"',
      stderr: "",
      code: 0,
    }));
  ...
  it("creates and removes a labeled per-cell network", async () => {
    ...
    await runtime.createNetwork(
      ...
      {
        "natesclaw.fleet.tenant": "acme",
        "natesclaw.fleet.attempt": "attempt-id",
        "natesclaw.fleet.owner": "owner-id",
      },
      { internal: false },
    );
    await runtime.removeNetwork("podman", "natesclaw-cell-acme-net");
    ...
    expect(executor.mock.calls.map(([, args]) => args)).toEqual([
      ...
      ["network", "rm", "natesclaw-cell-acme-net"],
    ]);
  });
  ...
  it.each([
    ...
    const executor = successfulExecutor();
    const runtime = createFleetContainerRuntime(executor);
    await runtime.createNetwork("podman", "natesclaw-cell-acme-net", {}, { internal });

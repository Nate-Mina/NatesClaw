...
/** Resolve the visible message id, including Natesclaw metadata attached to raw entries. */
export function resolveMessageId(entry: Record<string, unknown>): string | undefined {
  return (
    toText(entry.id) ??
    (entry["__natesclaw"] && typeof entry["__natesclaw"] === "object"
      ? toText((entry["__natesclaw"] as { id?: unknown }).id)
      : undefined)
  );
}

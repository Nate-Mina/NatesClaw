...
const AGENTS_DELETE_OUTPUT_MAX_BYTES = readPositiveIntEnv(
  "NATESCLAW_FIXTURE_AGENTS_DELETE_OUTPUT_MAX_BYTES",
  1024 * 1024,
);
...
function writeOpenWebUiWorkspace() {
  const workspace =
    process.env.NATESCLAW_WORKSPACE_DIR || path.join(process.env.HOME, ".natesclaw", "workspace");
  write(
    path.join(workspace, "IDENTITY.md"),
    "# Identity\n\n- Name: Natesclaw\n- Purpose: Open WebUI Docker compatibility smoke test assistant.\n",
  );
  fs.rmSync(path.join(workspace, ".natesclaw", "workspace-state.json"), { force: true });
  fs.rmSync(path.join(workspace, "natesclaw-workspace-state.json"), { force: true });
  fs.rmSync(path.join(workspace, "BOOTSTRAP.md"), { force: true });
}

function writeAgentsDeleteConfig() {
  const stateDir = requireArg(process.env.NATESCLAW_STATE_DIR, "NATESCLAW_STATE_DIR");
  const sharedWorkspace = requireArg(process.env.SHARED_WORKSPACE, "SHARED_WORKSPACE");
  const gatewayToken = process.env.NATESCLAW_GATEWAY_TOKEN?.trim();
...
function assertAgentsDeleteResult([outputPath]) {
  ...
  const remaining =
    readJson(path.join(process.env.NATESCLAW_STATE_DIR, "natesclaw.json"))?.agents?.entries ?? {};
  assert(

#import "NatesclawCameraPTZNative.h"
...
enum {
    NatesclawUVCVideoClass = 0x0e,
    NatesclawUVCVideoControlSubclass = 0x01,
    NatesclawUVCClassInterfaceDescriptor = 0x24,
    NatesclawUVCVideoControlHeader = 0x01,
    NatesclawUVCInputTerminal = 0x02,
    NatesclawUVCInputTerminalCamera = 0x0201,
    NatesclawUVCSetCurrent = 0x01,
};

struct NatesclawUVCController {
...
static void NatesclawUVCSetError(char **error_out, const char *format, ...) {
    if (error_out == NULL) {
        return;
    }
    va_list args;
...
static uint32_t NatesclawUVCNumberProperty(io_service_t service, CFStringRef key) {
...
static IOUSBDeviceInterface **NatesclawUVCCreateDeviceInterface(
    ...
    for (size_t class_index = 0; class_index < sizeof(class_names) / sizeof(class_names[0]); class_index++) {
        ...
        while ((service = IOIteratorNext(iterator)) != IO_OBJECT_NULL) {
            uint32_t found_vendor = NatesclawUVCNumberProperty(service, CFSTR(kUSBVendorID));
            uint32_t found_product = NatesclawUVCNumberProperty(service, CFSTR(kUSBProductID));
            uint32_t found_location = NatesclawUVCNumberProperty(service, CFSTR(kUSBDevicePropertyLocationID));
...
static int NatesclawUVCParseCameraTerminalDescriptor(
    ...
    if (descriptor == NULL || terminal_id_out == NULL || controls_out == NULL || descriptor_length < 17 ||
        ...
        descriptor[1] != NatesclawUVCClassInterfaceDescriptor || descriptor[2] != NatesclawUVCInputTerminal) {
        return 0;
    }

    uint16_t terminal_type = (uint16_t)descriptor[4] | ((uint16_t)descriptor[5] << 8);
    size_t control_size = descriptor[14];
    if (terminal_type != NatesclawUVCInputTerminalCamera || control_size < 2 ||
...
int natesclaw_uvc_parse_camera_terminal(
    ...
    if (descriptors == NULL || terminal_id_out == NULL || controls_out == NULL || descriptors_length < 7 ||
        descriptors[0] < 7 || descriptors[1] != NatesclawUVCClassInterfaceDescriptor ||
        descriptors[2] != NatesclawUVCVideoControlHeader) {
        return 0;
    }
    ...
    for (size_t offset = 0; offset < scan_length;) {
        ...
        if (NatesclawUVCParseCameraTerminalDescriptor(
                descriptor,
                length,
                terminal_id_out,
                controls_out
...
static void NatesclawUVCReadTerminal(
    IOUSBInterfaceInterface220 **interface,
    uint8_t *terminal_id_out,
    uint32_t *controls_out
) {
    ...
    *controls_out = 0;
    ...
    while ((current = (*interface)->FindNextAssociatedDescriptor(
                interface,
                current,
                NatesclawUVCClassInterfaceDescriptor
            )) != NULL) {
        if (NatesclawUVCParseCameraTerminalDescriptor(
...
int natesclaw_uvc_open(
    ...
    if (controller_out == NULL || controls_out == NULL) {
        NatesclawUVCSetError(error_out, "invalid controller output pointers");
        return 0;
    }
    *controller_out = NULL;
    *controls_out = 0;
    ...
    IOUSBDeviceInterface **device = NatesclawUVCCreateDeviceInterface(
        location_id,
        vendor_id,
        product_id,
        &result
    ...
    IOUSBFindInterfaceRequest request = {
        NatesclawUVCVideoClass,
        NatesclawUVCVideoControlSubclass,
    ...
    if (result != kIOReturnSuccess) {
        (*device)->Release(device);
        NatesclawUVCSetError(error_out, "find VideoControl interface: %s (0x%08x)", mach_error_string(result), result);
        return 0;
    }
    ...
    if (service == IO_OBJECT_NULL) {
        (*device)->Release(device);
        NatesclawUVCSetError(error_out, "USB device has no UVC VideoControl interface");
        return 0;
    }
    ...
    if (result != kIOReturnSuccess || plugin == NULL) {
        ...
        (*device)->Release(device);
        NatesclawUVCSetError(error_out, "create VideoControl interface: %s (0x%08x)", mach_error_string(result), result);
        return 0;
    }
    ...
    if (query_result != S_OK || interface == NULL) {
        ...
        (*device)->Release(device);
        NatesclawUVCSetError(error_out, "query VideoControl interface: 0x%08x", (unsigned int)query_result);
        return 0;
    }
    ...
    if (result != kIOReturnSuccess) {
        (*interface)->Release(interface);
        (*device)->Release(device);
        NatesclawUVCSetError(error_out, "read VideoControl interface number: %s (0x%08x)", mach_error_string(result), result);
        return 0;
    }
    ...
    if (result == kIOReturnSuccess) {
        ...
        (*interface)->Release(interface);
        (*device)->Release(device);
        NatesclawUVCSetError(error_out, "open VideoControl interface: %s (0x%08x)", mach_error_string(result), result);
        return 0;
    }
    ...
    NatesclawUVCController *controller = calloc(1, sizeof(NatesclawUVCController));
    if (controller == NULL) {
        if (interface_open) {
        ...
        (*interface)->Release(interface);
        (*device)->Release(device);
        NatesclawUVCSetError(error_out, "allocate UVC controller");
        return 0;
    }
    ...
    NatesclawUVCReadTerminal(interface, &controller->terminal_id, controls_out);
...
int natesclaw_uvc_control(
    ...
    if (controller == NULL || controller->interface == NULL || data == NULL || length == 0) {
        NatesclawUVCSetError(error_out, "invalid control request");
        return 0;
    }

    IOUSBDevRequest control_request = {
        .bmRequestType = request == NatesclawUVCSetCurrent ? 0x21 : 0xa1,
        .bRequest = request,

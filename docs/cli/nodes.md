---
summary: "CLI reference for `natesclaw nodes` (status, pairing, invoke, camera/canvas/screen/location/notify)"
read_when:
...
natesclaw nodes status --last-connected 24h
natesclaw nodes list
...
natesclaw nodes reject <requestId>
natesclaw nodes remove --node <id|name|ip>
natesclaw nodes rename --node <id|name|ip> --name <displayName>
```

These commands drive the gateway-owned `node.pair.*` store, separate from device pairing (`natesclaw devices approve`) that gates the node's WS `connect` handshake. See [Nodes](/nodes) for how the two relate.
...
```bash
natesclaw nodes invoke --node <id> --command system.which --params '{"bins":["uname"]}'
```
...
```bash
natesclaw nodes notify --node <id> --title "Build" --body "Done" --priority timeSensitive
natesclaw nodes push --node <id> --title "Natesclaw" --environment sandbox
natesclaw nodes location get --node <id> --accuracy precise
natesclaw nodes screen record --node <id> --duration 10s --fps 10 --out ./clip.mp4
```

- `notify` sends a local notification on a node that declares `system.notify`, including macOS, iOS, Android, and direct watchOS nodes. Direct watchOS delivery requires Natesclaw to be active. Requires `--title` or `--body`. Options: `--sound <name>`, `--priority <passive|active|timeSensitive>`, `--delivery <system|overlay|auto>` (default `system`), `--invoke-timeout <ms>` (default `15000`).
- `push` sends an APNs test push to an iOS node. Options: `--title <text>` (default `Natesclaw`), `--body <text>`, `--environment <sandbox|production>` to override the detected APNs environment. Accepted delivery exits `0`; a typed APNs rejection preserves the complete text or JSON diagnostic and exits non-zero.
...
Camera and Canvas commands have their own docs: [Camera nodes](/nodes/camera), [Canvas](/platforms/mac/canvas). Canvas is implemented by the bundled experimental Canvas plugin; core keeps `natesclaw nodes canvas` as a compatibility mount point.

// Natesclaw agent database tests cover agent-scoped DB storage and migrations.
...
import {
  assertNoNatesclawAgentDatabaseLeases,
  claimNatesclawAgentDatabaseLease,
  releaseNatesclawAgentDatabaseLease,
} from "./natesclaw-agent-db-lease.js";
import { withNatesclawAgentDatabaseReadOnly } from "./natesclaw-agent-db-readonly.js";
import {
  createNatesclawAgentDatabasePathMatcher,
  isSameNatesclawAgentDatabasePath,
  registerNatesclawAgentDatabase,
  unregisterNatesclawAgentDatabase,
} from "./natesclaw-agent-db-registry.js";
import type { DB as NatesclawAgentKyselyDatabase } from "./natesclaw-agent-db.generated.js";
import {
...
function createTempStateDir(): string {
  return makeTempDir(agentDbTempDirs, "natesclaw-agent-db-");
}
...
function ensureSharedStateDatabaseTemplate(): string {
  ...
  const stateDir = makeTempDir(agentDbTempDirs, "natesclaw-agent-db-shared-state-");
...
function openNatesclawAgentDatabase(
  options: Parameters<typeof openNatesclawAgentDatabaseRuntime>[0],
) {
  const stateDatabasePath = resolveNatesclawStateSqlitePath(options.env);
  ...
  return openNatesclawAgentDatabaseRuntime(options);
}
...
function ensureCurrentWorkerAgentDatabaseTemplate(): string {
  ...
  const stateDir = makeTempDir(agentDbTempDirs, "natesclaw-agent-db-current-worker-");
  const env = { NATESCLAW_STATE_DIR: stateDir };
  const template = openNatesclawAgentDatabase({ agentId: "worker-1", env });
  ...
  try {
    ...
    if (readSqliteNumberPragma(verified, "user_version") !== NATESCLAW_AGENT_SCHEMA_VERSION) {
...
function materializeCurrentWorkerAgentDatabase(stateDir: string): string {
  ...
  const databasePath = resolveNatesclawAgentSqlitePath(options);
...
function ensureV13WorkerAgentDatabaseTemplate(): string {
  ...
  const databasePath = resolveNatesclawAgentSqlitePath({
    agentId: "worker-1",
    env: { NATESCLAW_STATE_DIR: stateDir },
  });
  fs.mkdirSync(path.dirname(databasePath), { recursive: true });
...
function migrateAndOpenLegacyAgentDatabaseForTest(
  options: Parameters<typeof openNatesclawAgentDatabase>[0],
) {
  const pathname = resolveNatesclawAgentSqlitePath(options);
  const { DatabaseSync } = requireNodeSqlite();
  const database = new DatabaseSync(pathname);
  try {
    ensureNatesclawAgentDatabaseSchema(database, options);
  } finally {
    database.close();
  }
  return openNatesclawAgentDatabase(options);
}
...
function readRegisteredAgentDatabaseLastSeenAt(params: {
  ...
  const row = openNatesclawStateDatabase({ env: params.env })
...
function launchAgentSchemaOpener(params: {
  ...
  const agentModuleUrl = new URL("./natesclaw-agent-db.ts", import.meta.url).href;
  const stateModuleUrl = new URL("./natesclaw-state-db.ts", import.meta.url).href;
  const child = spawn(
    process.execPath,
    [
      "--import",
      ...
      `
        ...
        const db = new DatabaseSync(process.env.NATESCLAW_AGENT_DB_RACE_PATH);
        ...
        process.stdin.once("data", () => {
          let result;
          try {
            ensureNatesclawAgentDatabaseSchema(observedDb, {
              agentId: process.env.NATESCLAW_AGENT_DB_RACE_AGENT,
              env: { NATESCLAW_STATE_DIR: process.env.NATESCLAW_AGENT_DB_RACE_STATE_DIR },
              path: process.env.NATESCLAW_AGENT_DB_RACE_PATH,
              register: true,
            });
            result = { agentId: process.env.NATESCLAW_AGENT_DB_RACE_AGENT, ok: true };
          } catch (error) {
            result = {
              agentId: process.env.NATESCLAW_AGENT_DB_RACE_AGENT,
              ok: false,
    ...
    ],
    {
      env: {
        ...process.env,
        NATESCLAW_AGENT_DB_RACE_AGENT: params.agentId,
        NATESCLAW_AGENT_DB_RACE_PATH: params.databasePath,
        NATESCLAW_AGENT_DB_RACE_STATE_DIR: params.stateDir,
      },
...
describe("natesclaw agent database", () => {
  it("uses the canonical state schema for deletion journal reads and updates", () => {
    const stateDir = createTempStateDir();
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const stateDatabasePath = resolveNatesclawStateSqlitePath(env);
    fs.mkdirSync(path.dirname(stateDatabasePath), { recursive: true });
    const { DatabaseSync } = requireNodeSqlite();
    const stateDatabase = new DatabaseSync(stateDatabasePath);
    stateDatabase.exec(
      fs.readFileSync(new URL("./natesclaw-state-schema.sql", import.meta.url), "utf8"),
    );
    stateDatabase.close();
    ...
    const databasePaths = [path.join(entry.agentDir, "natesclaw-agent.sqlite")];
    const cleanupPaths = [
      {
        path: entry.agentDir,
  ...
  it("fences registration and lease claims beneath another agent's pending deletion", () => {
    ...
    registerNatesclawAgentDatabase({ agentId: "deleted", path: registeredSidecarPath, env });
    ...
    unregisterNatesclawAgentDatabase({ agentId: "deleted", path: relocatedDatabasePath, env });
    unregisterNatesclawAgentDatabase({ agentId: "deleted", path: registeredSidecarPath, env });

    try {
      for (const fencedPath of [databasePath, relocatedDatabasePath, sidecarClaimPath]) {
        expect(() =>
          registerNatesclawAgentDatabase({ agentId: "survivor", path: fencedPath, env }),
        ).toThrow("deletion owns");
        expect(() =>
          claimNatesclawAgentDatabaseLease({ agentId: "survivor", path: fencedPath, env }),
        ).toThrow("deletion owns");
      }
    } finally {
      deletion.rollback();
    ...
    expect(() =>
      registerNatesclawAgentDatabase({ agentId: "survivor", path: databasePath, env }),
    ).not.toThrow();
    const leaseId = claimNatesclawAgentDatabaseLease({
      agentId: "survivor",
      path: databasePath,
      env,
    });
    releaseNatesclawAgentDatabaseLease(leaseId, { env });
    unregisterNatesclawAgentDatabase({ agentId: "survivor", path: databasePath, env });
  });
  ...
  it("blocks deletion on a foreign lease beneath its workspace until release", () => {
    ...
    expect(() => assertNoNatesclawAgentDatabaseLeases("deleted", { env })).toThrow("deletion owns");
    releaseNatesclawAgentDatabaseLease(leaseId, { env });
    expect(() => assertNoNatesclawAgentDatabaseLeases("deleted", { env })).not.toThrow();
    deletion.rollback();
  });
  ...
  it("resolves under the per-agent state directory", () => {
    ...
    ).toBe(path.join(stateDir, "agents", "worker-1", "agent", "natesclaw-agent.sqlite"));
  });
  ...
  it("returns typed not-found without creating a missing read-only database", () => {
    ...
    expect(withNatesclawAgentDatabaseReadOnly(() => "unused", options)).toEqual({
      found: false,
  ...
  it("refuses a newer schema from the read-only database helper", () => {
    ...
    expect(() => withNatesclawAgentDatabaseReadOnly(() => "unused", options)).toThrow(
      `newer schema version ${NATESCLAW_AGENT_SCHEMA_VERSION + 1}`,
    );
  });
  ...
  it("lists a missing registry without creating the shared state database", () => {
    const stateDir = createTempStateDir();
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const stateDatabasePath = path.join(stateDir, "state", "natesclaw.sqlite");
  ...
  it("invalidates the registered database memo after a registry write", () => {
    ...
    expect(listNatesclawRegisteredAgentDatabases({ env })).toEqual([]);
    registerNatesclawAgentDatabase({ agentId: "worker-1", path: databasePath, env });
    expect(listNatesclawRegisteredAgentDatabases({ env })).toEqual([
  ...
  it("keeps incompatible schema versions maintenance-only", () => {
    const stateDir = createTempStateDir();
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agents", "legacy", "agent", "natesclaw-agent.sqlite");
    registerNatesclawAgentDatabase({
      agentId: "legacy",
      path: databasePath,
      env,
      schemaVersion: NATESCLAW_AGENT_SCHEMA_VERSION - 1,
    });
  ...
  it("does not persist archived import databases in the registry", () => {
    const stateDir = createTempStateDir();
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const archivedPath = path.join(stateDir, "imports", "archive", "natesclaw-agent.sqlite");

    registerNatesclawAgentDatabase({ agentId: "archive", path: archivedPath, env });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const env = { NATESCLAW_STATE_DIR: stateDir };
      const activePath = path.join(stateDir, "active", "natesclaw-agent.sqlite");
      const archivedPath = path.join(stateDir, "imports", "archive", "natesclaw-agent.sqlite");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const missingPath = path.join(aliasDir, "natesclaw-agent.sqlite");

      registerNatesclawAgentDatabase({ agentId: "archive", path: missingPath, env });
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      ...
      expect(() => listNatesclawRegisteredAgentDatabases({ env })).toThrow("is unavailable");
    },
  );
  ...
  it("creates the per-agent schema and registers it globally", () => {
    ...
    const database = openNatesclawAgentDatabaseRuntime({
      agentId: "worker-1",
      env: { NATESCLAW_STATE_DIR: stateDir },
    });

    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./natesclaw-agent-schema.sql", import.meta.url)),
    );
    expect(
      database.db
    ...
    expect(database.path).toBe(
      path.join(stateDir, "agents", "worker-1", "agent", "natesclaw-agent.sqlite"),
    );

    const registered = listNatesclawRegisteredAgentDatabases({
      env: { NATESCLAW_STATE_DIR: stateDir },
  ...
  it("opens a v13 database that already contains additive board storage", () => {
    expect(NATESCLAW_AGENT_SCHEMA_VERSION).toBe(17);
    const stateDir = createTempStateDir();
    const env = { NATESCLAW_STATE_DIR: stateDir };
    ...
    expect(readSqliteNumberPragma(reopened.db, "user_version")).toBe(NATESCLAW_AGENT_SCHEMA_VERSION);
    expect(
      reopened.db
  ...
  it("migrates v13 session entries, routes, and generations into nodes and windows", () => {
    ...
    expect(readSqliteNumberPragma(migrated.db, "user_version")).toBe(NATESCLAW_AGENT_SCHEMA_VERSION);
  ...
  it("migrates version 1 memory source identities before registering version 2", () => {
    ...
    expect(readSqliteNumberPragma(database.db, "user_version")).toBe(NATESCLAW_AGENT_SCHEMA_VERSION);
    expect(
    ...
    expect(
      listNatesclawRegisteredAgentDatabases({ env }).find((entry) => entry.agentId === "worker-1"),
    ).toMatchObject({ path: databasePath, schemaVersion: NATESCLAW_AGENT_SCHEMA_VERSION });

    closeNatesclawAgentDatabasesForTest();
    const reopened = openNatesclawAgentDatabase({ agentId: "worker-1", env });
    expect(
  ...
  it("rolls back a failed version 1 migration without retaining ownership", () => {
    ...
    expect(() => openNatesclawAgentDatabase({ agentId: "worker-1", env })).toThrow();
  ...
  it("retains a failed-open handle and lease when initialization cleanup also fails", () => {
    const stateDir = createTempStateDir();
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const database = openNatesclawAgentDatabase({ agentId: "worker-1", env });
    expect(closeNatesclawAgentDatabaseByPath(database.path)).toBe(true);
    const { DatabaseSync } = requireNodeSqlite();
    ...
    expect(() =>
      openNatesclawAgentDatabase({ agentId: "worker-2", env, path: database.path }),
    ).toThrow("initialization close failed");
    close.mockRestore();
    expect(() => assertNoNatesclawAgentDatabaseLeases("worker-2", { env })).toThrow(
    ...
    expect(disposeNatesclawAgentDatabaseByPath(database.path, { env })).toBe(true);
    expect(() => assertNoNatesclawAgentDatabaseLeases("worker-2", { env })).not.toThrow();
  });
  ...
  it("keeps multiple registered paths for the same agent", () => {
    ...
    const relocated = openNatesclawAgentDatabase({
      agentId: "worker-1",
      env,
      path: relocatedPath,
    });
    const defaultDatabase = openNatesclawAgentDatabase({
      agentId: "worker-1",
      env,
    });

    expect(
      listNatesclawRegisteredAgentDatabases({ env })
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      try {
        const matchesPath = createNatesclawAgentDatabasePathMatcher();
      ...
      expect(createNatesclawAgentDatabasePathMatcher()(realPath, aliasPath)).toBe(false);
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      expect(() => isSameNatesclawAgentDatabasePath(loopPath, loopPath)).toThrow(
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      expect(isSameNatesclawAgentDatabasePath(realPath, aliasPath)).toBe(true);
      ...
      expect(isSameNatesclawAgentDatabasePath(futureRealPath, danglingAliasPath)).toBe(true);
      ...
      expect(isSameNatesclawAgentDatabasePath(relativeRealPath, relativeAliasPath)).toBe(true);
      ...
      expect(isSameNatesclawAgentDatabasePath(redirectedRealPath, redirectedAliasPath)).toBe(true);
      expect(
        isSameNatesclawAgentDatabasePath(
      ...
      expect(isSameNatesclawAgentDatabasePath(danglingTraversalAlias, livePath)).toBe(false);
      const rawMissingTraversalPath = `${stateDir}${path.sep}missing${path.sep}..${path.sep}live-worker.sqlite`;
      expect(isSameNatesclawAgentDatabasePath(rawMissingTraversalPath, livePath)).toBe(false);
      ...
      expect(() => isSameNatesclawAgentDatabasePath(loopPath, realPath)).toThrow(
        expect.objectContaining({ code: "ELOOP" }),
      );
      ...
      expect(() => isSameNatesclawAgentDatabasePath(expandingLoopPath, realPath)).toThrow(
        expect.objectContaining({ code: "ELOOP" }),
      );
      ...
      const database = openNatesclawAgentDatabase({ agentId: "worker", env, path: realPath });
      unregisterNatesclawAgentDatabase({ agentId: "worker", env, path: database.path });
      registerNatesclawAgentDatabase({ agentId: "worker", env, path: aliasPath });
    },
  );
  ...
  it("matches volume semantics for non-ASCII case aliases", () => {
    ...
    expect(isSameNatesclawAgentDatabasePath(upperPath, lowerPath)).toBe(aliases);
  });
  ...
  it("matches volume semantics for Unicode simple case-fold aliases", () => {
    ...
    expect(isSameNatesclawAgentDatabasePath(sigmaPath, finalSigmaPath)).toBe(aliases);
  });
  ...
  it("keeps case and normalization semantics distinct for missing aliases", () => {
    ...
    expect(isSameNatesclawAgentDatabasePath(composedUpperPath, decomposedLowerPath)).toBe(aliases);
  });
  ...
  it("requires raw normalization after an ASCII-only case difference", () => {
    ...
    expect(isSameNatesclawAgentDatabasePath(composedUpperPath, decomposedMixedPath)).toBe(aliases);
  });
  ...
  it.runIf(tempVolumeIsCaseInsensitive)(
    ...
    () => {
      ...
      expect(
        isSameNatesclawAgentDatabasePath(path.join(stateDir, upper), path.join(stateDir, lower)),
      ).toBe(true);
    },
  );
  ...
  it("uses the candidate code points for missing-path normalization semantics", () => {
    ...
    expect(isSameNatesclawAgentDatabasePath(leftPath, rightPath)).toBe(aliases);
  });
  ...
  it("matches volume semantics for normalization-only missing directory components", () => {
    ...
    expect(
      isSameNatesclawAgentDatabasePath(
        path.join(leftDir, "natesclaw-agent.sqlite"),
        path.join(rightDir, "natesclaw-agent.sqlite"),
      ),
    ).toBe(aliases);
  });
  ...
  it.runIf(process.platform === "win32")(
    ...
    () => {
      ...
      expect(isSameNatesclawAgentDatabasePath(driveRelativePath, absolutePath)).toBe(true);
    },
  );
  ...
  it.runIf(process.platform === "linux")(
    ...
    () => {
      ...
      expect(
        isSameNatesclawAgentDatabasePath(path.join(hardLinkedNested, "d", "link"), firstHardLink),
      ).toBe(true);
    },
  );
  ...
  it("does not refresh global registry metadata on cached opens", () => {
    ...
    try {
      const first = openNatesclawAgentDatabase({
        agentId: "worker-1",
        env,
      });
      expect(
      ...
      const second = openNatesclawAgentDatabase({
        agentId: "worker-1",
        env,
      });
  ...
  it("rejects the legacy agent registry primary key with a doctor repair hint", () => {
    ...
    legacyDb.exec(`
      ...
      INSERT INTO agent_databases (
        ...
        'worker-1',
        '/legacy/worker-1/natesclaw-agent.sqlite',
        1,
        10,
        20
      );
    ...
    expect(() => listNatesclawRegisteredAgentDatabases({ env })).toThrow(
    ...
    ).toThrow(/run natesclaw doctor --fix/);

    fs.rmSync(stateDatabasePath);
    const reopened = openNatesclawAgentDatabase({
      agentId: "worker-1",
      env,
    });
    ...
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-agent-db-state-"));
          const env = { NATESCLAW_STATE_DIR: stateDir };
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-agent-db-relative-"));
          ...
          try {
            ...
            console.log(JSON.stringify({
              ...
              registeredPaths: listNatesclawRegisteredAgentDatabases({ env })
                .filter((entry) => entry.agentId === "worker-1")
                .map((entry) => entry.path),
  ...
  it("closes only the cached database at the exact resolved path", () => {
    ...
    const first = openNatesclawAgentDatabase({ agentId: "worker-1", env, path: firstPath });
    const second = openNatesclawAgentDatabase({ agentId: "worker-2", env, path: secondPath });

    expect(closeNatesclawAgentDatabaseByPath(path.join(stateDir, "missing.sqlite"))).toBe(false);
    ...
    expect(second.db.isOpen).toBe(true);
    expect(closeNatesclawAgentDatabaseByPath(firstPath)).toBe(false);

    const reopened = openNatesclawAgentDatabase({ agentId: "worker-1", env, path: firstPath });
  ...
  it("retains its durable lease when closing the handle fails", () => {
    ...
    expect(() => closeNatesclawAgentDatabaseByPath(database.path)).toThrow("wal close failed");
    expect(database.db.isOpen).toBe(true);
    expect(() => assertNoNatesclawAgentDatabaseLeases("worker-1", { env })).toThrow(
      "database is still open",
    );

    close.mockRestore();
    expect(closeNatesclawAgentDatabaseByPath(database.path)).toBe(true);
    expect(() => assertNoNatesclawAgentDatabaseLeases("worker-1", { env })).not.toThrow();
  });

  it("disposes only its exact cached owner and unregisters that registry row", () => {
    const stateDir = createTempStateDir();
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const first = openNatesclawAgentDatabase({ agentId: "worker-1", env });
    const second = openNatesclawAgentDatabase({ agentId: "worker-2", env });

    expect(disposeNatesclawAgentDatabaseByPath(first.path, { env })).toBe(true);
    expect(first.db.isOpen).toBe(false);
    expect(second.db.isOpen).toBe(true);
    expect(listNatesclawRegisteredAgentDatabases({ env })).toEqual([
      expect.objectContaining({ agentId: "worker-2", path: second.path }),
    ]);
    expect(disposeNatesclawAgentDatabaseByPath(first.path, { env })).toBe(false);
    expect(second.db.isOpen).toBe(true);
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      fs.mkdirSync(realDir, { recursive: true });
      fs.symlinkSync(realDir, aliasDir, "dir");
      const realPath = path.join(realDir, "natesclaw-agent.sqlite");
      const aliasPath = path.join(aliasDir, "natesclaw-agent.sqlite");
      const database = openNatesclawAgentDatabase({ agentId: "probe", env, path: realPath });

      expect(disposeNatesclawAgentDatabaseByPath(aliasPath, { env })).toBe(true);
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      fs.mkdirSync(realDir, { recursive: true });
      fs.symlinkSync(realDir, aliasDir, "dir");
      const real = openNatesclawAgentDatabase({
        agentId: "probe",
        env,
        path: path.join(realDir, "natesclaw-agent.sqlite"),
      });
      const alias = openNatesclawAgentDatabase({
        agentId: "probe",
        env,
        path: path.join(aliasDir, "natesclaw-agent.sqlite"),
      });

      expect(disposeNatesclawAgentDatabaseByPath(real.path, { env })).toBe(false);
  ...
  it("reopens a recreated agent id on a fresh database after archival", () => {
    const stateDir = createTempStateDir();
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const original = openNatesclawAgentDatabase({ agentId: "worker-1", env });
    ...
    try {
      expect(disposeNatesclawAgentDatabaseByPath(original.path, { env })).toBe(true);
      deletion.commit();
      ...
      const recreated = openNatesclawAgentDatabase({ agentId: "worker-1", env });
      expect(recreated.db.isOpen).toBe(true);
      expect(fs.statSync(recreated.path).ino).not.toBe(originalInode);
      expect(fs.existsSync(path.join(archivedDir, "natesclaw-agent.sqlite"))).toBe(true);
      expect(listNatesclawRegisteredAgentDatabases({ env })).toEqual([
  ...
  it("keeps the deleted id fenced until its completed tombstone is claimed", () => {
    ...
    expect(() =>
      registerNatesclawAgentDatabase({ agentId: "worker-2", path: databasePath, env }),
    ).not.toThrow();
    unregisterNatesclawAgentDatabase({ agentId: "worker-2", path: databasePath, env });

    expect(claimCompletedAgentDeletion("worker-1", deletion.entry.operationId, { env })).toBe(true);
    expect(() =>
      registerNatesclawAgentDatabase({ agentId: "worker-1", path: databasePath, env }),
    ).not.toThrow();
    unregisterNatesclawAgentDatabase({ agentId: "worker-1", path: databasePath, env });
  });

  it("serializes database leases with the durable deletion fence", () => {
    const stateDir = createTempStateDir();
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agents", "worker-1", "natesclaw-agent.sqlite");
    const leaseId = claimNatesclawAgentDatabaseLease({
      agentId: "worker-1",
      path: databasePath,
      env,
    ...
    try {
      ...
      expect(() =>
        claimNatesclawAgentDatabaseLease({ agentId: "worker-1", path: databasePath, env }),
      ).toThrow("agent worker-1 is deleted");
      releaseNatesclawAgentDatabaseLease(leaseId, { env });
      expect(() => assertNoNatesclawAgentDatabaseLeases("worker-1", { env })).not.toThrow();
    } finally {
  ...
  it("serializes concurrent ownership claims for one unowned database", async () => {
    ...
    try {
      ...
      expect(inspectNatesclawAgentDatabaseOwner(databasePath)).toEqual({
        status: "owned",
        agentId: winner?.agentId,
      });
      expect(
        listNatesclawRegisteredAgentDatabases({ env: { NATESCLAW_STATE_DIR: stateDir } }),
      ).toEqual([
        expect.objectContaining({
          agentId: winner?.agentId,
          path: databasePath,
  ...
  it("rechecks schema version after waiting for a concurrent writer", async () => {
    ...
    const futureVersion = NATESCLAW_AGENT_SCHEMA_VERSION + 1;
    try {
      await opener.ready;
      opener.child.stdin.end("go\n");
      ...
      expect(fs.existsSync(path.join(stateDir, "state", "natesclaw.sqlite"))).toBe(false);
      expect(
        listNatesclawRegisteredAgentDatabases({ env: { NATESCLAW_STATE_DIR: stateDir } }),
      ).toEqual([]);
    } finally {
  ...
  it("rejects explicit paths that point at the global state database", () => {
    ...
    const stateDatabase = openNatesclawStateDatabase({
      env,
      path: databasePath,
    });
    closeNatesclawStateDatabaseForTest();
    ...
    const reopenedStateDatabase = openNatesclawStateDatabase({
      env,
      path: databasePath,
    });
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    ...
    const env = { NATESCLAW_STATE_DIR: parentDir };
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      runNatesclawAgentWriteTransaction(() => {
        runNatesclawAgentWriteTransaction(() => undefined, options);
  ...
  it("installs same-version session additions before maintenance index repair", () => {
    ...
    try {
      ...
      expect(() =>
        assertNatesclawAgentDatabaseForMaintenance(repaired, {
          agentId: "worker-1",
          pathname: databasePath,
        }),
      ).not.toThrow();
  ...
  it("adds durable conversation delivery state when upgrading the canonical v10 schema", () => {
    ...
    expect(() =>
      assertNatesclawAgentDatabaseForMaintenance(database.db, {
        agentId: "worker-1",
  ...
  it("rejects stale schema_meta indexes before writable initialization", () => {
    ...
    try {
      expect(() =>
        ensureNatesclawAgentDatabaseSchema(independentlyManaged, {
          agentId: "worker-1",
          env,
        }),
  ...
  it("rechecks integrity after a validated handle is physically reopened", () => {
    ...
    expect(closeNatesclawAgentDatabaseByPath(databasePath)).toBe(true);
    closeNatesclawStateDatabaseForTest();
    createUnsafeIndexDrift(databasePath);
  ...
  it.each([0, NATESCLAW_AGENT_SCHEMA_VERSION - 1])(
    ...
    (version) => {
      ...
      expect(() => openNatesclawAgentDatabase({ agentId: "worker-1", env })).toThrow(
        "run natesclaw doctor --fix to migrate persisted media",
      );
    },
  );
  ...
  it("does not hide a sqlitefoo-only metadata-less v0 database as internal", () => {
    ...
    expect(() =>
      withNatesclawAgentDatabaseReadOnly(() => "unused", {
        agentId: "worker-1",
        env,
        path: databasePath,
      }),
    ).toThrow("uses schema version 0; run natesclaw doctor --fix");
    expect(() =>
      openNatesclawAgentDatabase({ agentId: "worker-1", env, path: databasePath }),
    ).toThrow("uses schema version 0; run natesclaw doctor --fix");
  });
  ...
  it("runs full integrity before a pending agent schema migration", () => {
    ...
    try {
      before.exec(`PRAGMA user_version = ${NATESCLAW_AGENT_SCHEMA_VERSION - 1};`);
    } finally {
      before.close();
    }
  ...
  it("latches newer per-agent schema failures before integrity scans", () => {
    ...
    const { DatabaseSync } = requireNodeSqlite();
    const db = new DatabaseSync(databasePath);
    db.exec(`PRAGMA user_version = ${NATESCLAW_AGENT_SCHEMA_VERSION + 1};`);
    db.close();

    let firstFailure: unknown;
    try {
      openNatesclawAgentDatabase({ agentId: "worker-1", env });
    } catch (error) {
      firstFailure = error;
    }
    ...
    clearNatesclawAgentDatabaseOpenFailure(databasePath);
    expect(openNatesclawAgentDatabase({ agentId: "worker-1", env }).db.isOpen).toBe(true);
  });
  ...
  it("closes cached handles on normal process exit so no stale WAL remains", () => {
    ...
    const output = execFileSync(
      ...
      [
        ...
        `
          import fs from "node:fs";
          import { openNatesclawAgentDatabase } from ${JSON.stringify(agentModuleUrl)};

          const database = openNatesclawAgentDatabase({
            agentId: "worker-1",
            env: { NATESCLAW_STATE_DIR: process.env.NATESCLAW_AGENT_DB_EXIT_TEST_DIR },
          });
      ...
      {
        encoding: "utf8",
        env: { ...process.env, NATESCLAW_AGENT_DB_EXIT_TEST_DIR: stateDir },
      },
    );
    const result = JSON.parse(output) as {

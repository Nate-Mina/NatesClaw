...
describe("verify-docker-attestations", () => {
  it("bounds docker manifest inspection calls", () => {
    const calls: unknown[] = [];
    const imageRef = "ghcr.io/natesclaw/natesclaw:test";
  ...
  it("parses required platforms and image refs", () => {
    expect(
      parseArgs(["--platform", "linux/amd64", "--platform", "linux/arm64", "ghcr.io/natesclaw/app"]),
    ).toEqual({
      help: false,
      imageRefs: ["ghcr.io/natesclaw/app"],
      requiredPlatforms: [
  ...
  it("resolves digest refs from tagged image refs", () => {
    expect(imageRefForDigest("ghcr.io/natesclaw/natesclaw:2026.4.26", imageDigest)).toBe(
      `ghcr.io/natesclaw/natesclaw@${imageDigest}`,
    );
    expect(imageRefForDigest("localhost:5000/natesclaw:main", imageDigest)).toBe(
      `localhost:5000/natesclaw@${imageDigest}`,
    );
  });

  it("accepts an image index with SBOM and provenance predicates", () => {
    const errors = collectDockerAttestationErrors({
      imageRef: "ghcr.io/natesclaw/natesclaw:test",
      index: createIndex(),
  ...
  it("reports unexpected attestation artifact types", () => {
    ...
    expect(errors).toEqual([
      `ghcr.io/natesclaw/natesclaw:test: linux/amd64 attestation ${attestationDigest} has unexpected artifactType "application/vnd.unknown"`,
    ]);
  });
  ...
  it("reports missing attestation manifests", () => {
    ...
    expect(errors).toEqual([
      "ghcr.io/natesclaw/natesclaw:test: missing attestation manifest for linux/amd64",
    ]);
  });
  ...
  it("reports missing SBOM or provenance predicates", () => {
    ...
    expect(errors).toEqual([
      "ghcr.io/natesclaw/natesclaw:test: linux/amd64 missing predicate https://slsa.dev/provenance/v1",
    ]);
  });
});

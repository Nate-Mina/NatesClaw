...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        NATESCLAW_BASE_ENV: "base",
        NATESCLAW_CHILD_ENV_REMOVE: "base",
        NATESCLAW_TO_REMOVE: undefined,
      },
      env: {
        NATESCLAW_CHILD_ENV_REMOVE: undefined,
        NATESCLAW_TEST_ENV: "ok",
      },
    });

    expect(resolved.NATESCLAW_BASE_ENV).toBe("base");
    expect(resolved.NATESCLAW_CHILD_ENV_REMOVE).toBeUndefined();
    expect(resolved.NATESCLAW_TEST_ENV).toBe("ok");
    expect(resolved.NATESCLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.NATESCLAW_CLI).toBe(NATESCLAW_CLI_ENV_VALUE);
  });
  ...
  it("does not restore parent variables excluded from the child environment", async () => {
    const key = "NATESCLAW_EXECA_PARENT_ONLY_TEST";
  ...
  it.runIf(process.platform === "win32")(
    ...
    async () => {
      const command = `natesclaw-missing-${process.pid}\r\ncalc.exe`;
  ...
  it("keeps argv values out of transport errors", async () => {
    ...
    const error = await runCommandWithTimeout(
      [`natesclaw-missing-${process.pid}-${Date.now()}`, "--token", privateArg],
      { timeoutMs: 3_000 },
...
describe("runCommandBuffered", () => {
  ...
  it("keeps argv values out of buffered transport errors", async () => {
    ...
    const result = await runCommandBuffered(
      [`natesclaw-missing-${process.pid}-${Date.now()}`, privateArg],
      { timeoutMs: 3_000 },
    );
...
describe("runExec", () => {
  ...
  it("supports stdin and an explicit base environment", async () => {
    const { stdout, stderr } = await runExec(
      ...
      [
        "-e",
        "process.stdin.pipe(process.stdout); process.stderr.write(process.env.NATESCLAW_RUN_EXEC_TEST ?? 'missing')",
      ],
      {
        baseEnv: { NATESCLAW_RUN_EXEC_TEST: "base" },

...
const gatewayQuestionMock = vi.hoisted(() => ({
  ...
  claimPendingAgentQuestionAnswer: undefined as
    | ((
        ...args: Parameters<
          typeof import("natesclaw/plugin-sdk/agent-harness-runtime").claimPendingAgentQuestionAnswer
        >
      ) => Promise<boolean>)
    | undefined,
...
function makeParams(
  ...
  return {
    ...
    sessionTarget: {
      sessionId: "session-1",
      sessionKey: "agent:main:session-1",
      storePath: "natesclaw-agent.sqlite",
    },
    timeoutMs: 5000,
...
describe("runCopilotAttempt", () => {
  ...
  it("hydrates offloaded prompt images before creating SDK blob attachments", async () => {
    ...
    const inboundDir = NatesclawState.statePath("media", "inbound");
  ...
  it("registers ask_user and resolves it from the active Natesclaw queue", async () => {
  ...
  describe("workspace bootstrap (systemMessage)", () => {
    ...
    it("forwards rendered bootstrap instructions into SDK SessionConfig.systemMessage (append mode)", async () => {
      ...
      // while layering Natesclaw context after it. See
    ...
    it("forwards rendered bootstrap instructions to resumeSession on the resume path", async () => {
      ...
      // otherwise resumed sessions would silently lose Natesclaw
  ...
  describe("session-level gitHubToken (independent of client-level)", () => {
    ...
    it("SessionConfig.gitHubToken is omitted when default mode is useLoggedInUser (no auth signal)", async () => {
      ...
      const prevNatesclaw = process.env.NATESCLAW_GITHUB_TOKEN;
      const prevGithub = process.env.GITHUB_TOKEN;
      delete process.env.NATESCLAW_GITHUB_TOKEN;
      ...
      try {
        ...
        if (prevNatesclaw !== undefined) {
          process.env.NATESCLAW_GITHUB_TOKEN = prevNatesclaw;
        }
  ...
  // bypass Natesclaw's wrapped-tool enforcement under any permissive
  ...
  describe("availableTools surface restriction (PR #86155 [P1] round-8)", () => {
    ...
    it("keeps native ask_user when its restricted Natesclaw equivalent remains allowed", async () => {
      const sdk = makeFakeSdk();
      const pool = makeFakePool(sdk);
    ...
    it("keeps a host-scoped Natesclaw create-session surface ring-zero", async () => {
      const sdk = makeFakeSdk();
      const pool = makeFakePool(sdk);
      const sdkTools = [makeFakeSdkTool("natesclaw")];
      const createToolBridge = vi.fn(async () => ({ sdkTools, sourceTools: [] }));

      await runCopilotAttempt(makeParams({ toolsAllow: ["natesclaw"] }), {
        createToolBridge,
        isHostScopedToolActive: (toolName) => toolName === "natesclaw",
        pool,
      });

      expect(readAvailableTools(sdk.createSession.mock.calls[0])).toEqual(["natesclaw"]);
    });
    ...
    it("keeps a host-scoped Natesclaw resume-session surface ring-zero", async () => {
      const sdk = makeFakeSdk({
        onResumeSession: (session) => {
          session.sendAndWait.mockResolvedValueOnce(makeAssistantMessageEvent("resumed"));
        },
      ...
      await runCopilotAttempt(
        makeParams({
          initialReplayState: { sdkSessionId: "sess-natesclaw" },
      ...
      const resumeCall = sdk.resumeSession.mock.calls[0] as unknown[] | undefined;
      const resumeCfg = resumeCall?.[1] as { availableTools?: string[] };
      expect(resumeCfg?.availableTools).toEqual(["natesclaw"]);
    });

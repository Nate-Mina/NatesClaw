// Natesclaw test instance tests cover spawned test instance lifecycle.
...
import { createNatesclawTestInstance, testing } from "./natesclaw-test-instance.js";
...
const fakeInstances: Awaited<ReturnType<typeof createNatesclawTestInstance>>[] = [];
...
async function createFakeGateway(sequence: string, startTimeoutMs = 1_000, stopTimeoutMs = 1_500) {
  const cwd = await fs.mkdtemp(path.join(tmpdir(), "natesclaw-test-instance-gateway-"));
  fakeRoots.push(cwd);
...
const tracePath = process.env.NATESCLAW_FAKE_GATEWAY_TRACE;
...
const env = Object.fromEntries(["HOME", "NATESCLAW_CONFIG_PATH", "NATESCLAW_GATEWAY_TOKEN", "NATESCLAW_STATE_DIR"].map((key) => [key, process.env[key]]));
appendFileSync(tracePath, JSON.stringify({ argv, config: JSON.parse(readFileSync(process.env.NATESCLAW_CONFIG_PATH, "utf8")), cwd: process.cwd(), env, pid: process.pid, port }) + "\\n");
const action = (process.env.NATESCLAW_FAKE_GATEWAY_SEQUENCE || "ready").split(",")[attempt - 1] || "ready";
...
`,
  ...
  const instance = await createNatesclawTestInstance({
    ...
    cwd,
    env: {
      NATESCLAW_FAKE_GATEWAY_SEQUENCE: sequence,
      NATESCLAW_FAKE_GATEWAY_TRACE: tracePath,
    },
    startTimeoutMs,
    stopTimeoutMs,
  });
...
describe("natesclaw test instance", () => {
  ...
  it("signals test instance process groups on POSIX", () => {
    ...
    testing.signalNatesclawTestProcess(child, "SIGKILL", killProcess);
  ...
  it("creates isolated config and spawn env without mutating process env", async () => {
    ...
    try {
      ...
      expect(inst.stateDir).toBe(path.join(inst.homeDir, ".natesclaw"));
      expect(inst.configPath).toBe(path.join(inst.stateDir, "natesclaw.json"));
      expect(inst.env.HOME).toBe(inst.homeDir);
      expect(inst.env.NATESCLAW_STATE_DIR).toBe(inst.stateDir);
      expect(inst.env.NATESCLAW_CONFIG_PATH).toBe(inst.configPath);
      expect(inst.env.NATESCLAW_SKIP_CRON).toBe("0");

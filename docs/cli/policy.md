---
summary: "CLI reference for `natesclaw policy` conformance checks"
read_when:
  - You want to check Natesclaw settings against an authored policy.jsonc
...
`natesclaw policy` is provided by the bundled Policy plugin. It is an enterprise
conformance layer over existing Natesclaw settings, not a second configuration
system. You author requirements in `policy.jsonc`; Natesclaw observes the active
...
Separately, [`natesclaw agent exec`](/cli/agent#agent-exec) applies an isolated
...
```bash
natesclaw plugins enable policy
```
...
- Routing probes reuse Natesclaw's runtime binding resolver. Routing evidence
...
observed state is existing Natesclaw config or workspace metadata.
...
If an `agentIds` entry is not present in `agents.entries.*`, Natesclaw evaluates
...
| `routing.probes[].route`            | The public Natesclaw route resolver                  | Describe a representative inbound route without sending a message.     |
...
| `gateway.nodes.denyCommands`            | `gateway.nodes.commands.deny`                 | Require exact node command ids such as `system.run` to be denied in Natesclaw config. |
...
denied by Natesclaw config. A deployment that intentionally allows a privileged
...
| Policy field                                        | Observed state                                                                                                   | Use when                                                               |
| --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `dataHandling.sensitiveLogging.requireRedaction`    | Runtime invariant `oc://natesclaw.invariant/logging/redaction`                                                    | Set to `true` to record the requirement; Natesclaw always satisfies it. |
...
`~/.natesclaw/state/natesclaw.sqlite` by default, or the same database under
`$NATESCLAW_STATE_DIR/state` when `NATESCLAW_STATE_DIR` is set. Findings keep the
...
```bash
natesclaw policy check
natesclaw policy check --json
natesclaw policy check --severity-min error
```
...
`natesclaw doctor --lint` when the Policy plugin is enabled.

Compare an operator policy file against an authored baseline:

```bash
natesclaw policy compare --baseline official.policy.jsonc
natesclaw policy compare --baseline official.policy.jsonc --policy policy.jsonc --json
```
...
```json
{
  ...
  "evidence": {
    ...
    "mcpServers": [
      {
        "id": "docs",
        "transport": "stdio",
        "source": "oc://natesclaw.config/mcp/servers/docs",
        "command": "npx"
      }
    ],
    "modelProviders": [
      {
        "id": "openai",
        "source": "oc://natesclaw.config/models/providers/openai"
      }
    ],
    "modelRefs": [
      {
        ...
        "provider": "openai",
        "model": "gpt-5.6-sol",
        "source": "oc://natesclaw.config/agents/defaults/model"
      }
    ],
    "network": [
      {
        "id": "browser-private-network",
        "source": "oc://natesclaw.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
        "value": false
      }
    ],
    "gatewayExposure": [
      {
        "id": "gateway-bind",
        "kind": "bind",
        "source": "oc://natesclaw.config/gateway/bind",
        "value": "loopback",
        "nonLoopback": false,
        "explicit": true
      }
    ...
    "agentWorkspace": [
      {
        ...
        "value": "ro",
        "sandboxMode": "all",
        "sandboxModeSource": "oc://natesclaw.config/agents/defaults/sandbox/mode",
        "sandboxEnabled": true,
        "explicit": true
      },
      {
        ...
        "kind": "toolDeny",
        "source": "oc://natesclaw.config/tools/deny",
        "scope": "defaults",
        "tool": "exec",
        "denied": true,
        "explicit": true
    ...
    "secrets": [
      {
        "id": "vault",
        "kind": "provider",
        "source": "oc://natesclaw.config/secrets/providers/vault",
        "providerSource": "env"
      },
      {
        "id": "oc://natesclaw.config/models/providers/openai/apiKey",
        "kind": "input",
    ...
    "authProfiles": [
      {
        "id": "github",
        "source": "oc://natesclaw.config/auth/profiles/github",
        "validMetadata": true,
        "provider": "github",
        "mode": "token"
      }
...
records the observed Natesclaw state used by the checks, and
...
`natesclaw policy watch` re-runs the check and reports when current evidence no
longer matches `expectedAttestationHash`:

```bash
natesclaw policy watch --json
```
...
| `policy/gateway-node-command-denied`                     | A node command denied by policy is not denied by Natesclaw config.                 |
...
```json
{
  ...
  "source": "policy",
  "path": "natesclaw config",
  "ocPath": "oc://natesclaw.config/mcp/servers/remote",
  "target": "oc://natesclaw.config/mcp/servers/remote",
...
```json
{
  ...
  "source": "policy",
  "path": "natesclaw config",
  "ocPath": "oc://natesclaw.config/agents/defaults/model/fallbacks/#0",
  "target": "oc://natesclaw.config/agents/defaults/model/fallbacks/#0",
...
```json
{
  ...
  "target": "oc://natesclaw.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
...
```json
{
  ...
  "source": "policy",
  "path": "natesclaw config",
  "ocPath": "oc://natesclaw.config/gateway/bind",
  "target": "oc://natesclaw.config/gateway/bind",
...
```json
{
  ...
  "severity": "error",
  "message": "Gateway node command 'system.run' is denied by policy but not denied by Natesclaw config.",
  "source": "policy",
  "path": "natesclaw config",
  "ocPath": "oc://natesclaw.config/gateway/nodes/commands/deny",
  "target": "oc://natesclaw.config/gateway/nodes/commands/deny",
...
```json
{
  ...
  "target": "oc://natesclaw.config/agents/defaults/sandbox/workspaceAccess",
...
Sensitive log redaction is unconditional in Natesclaw, so nothing can report it
as disabled. The key stays a supported policy rule: `natesclaw policy` validates
its shape, `natesclaw policy compare` still requires a candidate policy to be at
least as strict as the baseline for it, and `natesclaw policy check` records the
runtime invariant `oc://natesclaw.invariant/logging/redaction` in the

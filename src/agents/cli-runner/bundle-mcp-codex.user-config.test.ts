/** Tests projecting Natesclaw user MCP servers into Codex app-server config. */
...
describe("buildCodexUserMcpServersThreadConfigPatch", () => {
  ...
  it("returns undefined when cfg has no mcp.servers (regression: #80814)", () => {
    ...
    expect(buildCodexUserMcpServersThreadConfigPatch({} as NatesclawConfig)).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: {} } as NatesclawConfig),
    ).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: { servers: {} } } as NatesclawConfig),
    ).toBeUndefined();
  });
  ...
  it("projects exact Natesclaw MCP tool filters into Codex-native tool filters", () => {
    const patch = buildCodexUserMcpServersThreadConfigPatch({
      mcp: {
        servers: {
          docs: {
  ...
  it("rejects wildcard Natesclaw MCP tool filters that Codex cannot project exactly", () => {
    expect(() =>
      buildCodexUserMcpServersThreadConfigPatch({
        mcp: {
          servers: {
  ...
  it("filters Codex-scoped user MCP servers by Natesclaw agent id", () => {
    // Agent-scoped MCP servers should follow the active Natesclaw agent, while
  ...
  it("omits scoped Codex MCP servers when no Natesclaw agent id is available", () => {
    const patch = buildCodexUserMcpServersThreadConfigPatch({
      mcp: {
        servers: {
          atlas: {

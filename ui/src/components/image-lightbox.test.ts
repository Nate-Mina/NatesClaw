...
async function renderLightbox() {
  render(
    html`<natesclaw-image-lightbox
      src="data:image/png;base64,cG5n"
      title="Generated lobster"
    ></natesclaw-image-lightbox>`,
    container,
  );
  const modal = container.querySelector("natesclaw-image-lightbox");
  if (!modal) {
    throw new Error("missing image lightbox");
  }
  await modal.updateComplete;
  const dialogAdapter = modal.shadowRoot?.querySelector("natesclaw-modal-dialog");
  if (!dialogAdapter) {
...
describe("natesclaw-image-lightbox", () => {
  beforeEach(() => {

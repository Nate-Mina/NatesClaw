...
Microsoft Teams ships as a bundled plugin in current Natesclaw releases; no separate install is required in the normal packaged build.
...
```bash
natesclaw plugins install ./path/to/local/msteams-plugin
```
...
devtunnel create my-natesclaw-bot --allow-anonymous
devtunnel port create my-natesclaw-bot -p 3978 --protocol auto

# Each dev session:
devtunnel host my-natesclaw-bot
...
**4. Configure Natesclaw** using the credentials from the output:
...
- Talk to Natesclaw via Teams DMs, group chats, or channels.
...
- Do not rely on UPN/display-name matching for allowlists; they can change. Natesclaw disables direct name matching by default; opt in with `channels.msteams.dangerouslyAllowNameMatching: true`.
...
- On startup, Natesclaw resolves team/channel and user allowlist names to IDs (when Graph permissions allow) and logs the mapping. Unresolved names are kept as typed but ignored for routing unless `channels.msteams.dangerouslyAllowNameMatching: true` is set.
...
5. Configure `msteams` in `~/.natesclaw/natesclaw.json` (or env vars) and start the gateway.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `natesclaw-msteams` (must be unique) |
...
For production, Natesclaw supports **federated authentication** as an alternative to client secrets, via `channels.msteams.authType: "federated"`. Two methods:
...
3. At runtime, Natesclaw uses `@azure/identity` to acquire tokens from the Azure IMDS endpoint.
...
**Default:** when `authType` is unset, Natesclaw uses client-secret authentication (`appPassword`). Existing configs keep working unchanged.
...
These auth-related config keys can be set via environment variables instead of `natesclaw.json` (other config keys, such as `groupPolicy` or `historyLimit`, are config-only):
...
Natesclaw exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve verified roster details for a configured conversation.
...
```json5
{
  ...
  description: { short: "Natesclaw in Teams", full: "Natesclaw in Teams" },
...
Teams delivers messages via HTTP webhook. Natesclaw applies fixed HTTP server
...
| Teams environment | Natesclaw config                                             | Proactive `serviceUrl`                             |
| ----------------- | ----------------------------------------------------------- | -------------------------------------------------- |
...
When `replyStyle: "thread"` is in effect and the bot was @mentioned from inside a channel thread, Natesclaw re-attaches the original thread root to the outbound conversation reference (`19:...@thread.tacv2;messageid=<root>`) so the reply lands inside the same thread. This holds for both live (in-turn) sends and proactive sends made after the Bot Framework turn context has expired (e.g., long-running agents, queued tool-call replies via `mcp__natesclaw__message`).
...
By default, Natesclaw only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Per-user sharing is more secure since only chat participants can access the file. Natesclaw requires a successful member lookup for group chats; timeouts, transport failures, empty results, and Graph API denials fail the send instead of widening access to the organization.
...
Uploaded files are stored in a `/NatesclawShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Natesclaw sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `natesclaw message poll --channel msteams --target conversation:<id> --poll-question "..." --poll-option "..." --poll-option "..."`.
- Votes are recorded by the gateway in Natesclaw plugin-state SQLite under `state/natesclaw.sqlite`.
- Existing `msteams-polls.json` files are imported by `natesclaw doctor --fix`, not by the running plugin.
- The gateway must stay online to record votes.
- Polls do not auto-post result summaries, and there is no poll-results CLI yet.

## Presentation cards

Send semantic presentation payloads to Teams users or conversations using the `message` tool, CLI, or normal reply delivery. Natesclaw renders them as Teams Adaptive Cards from the generic presentation contract.

The `presentation` parameter accepts semantic blocks. When `presentation` is provided, the message text is optional. Buttons render as Adaptive Card submit or URL actions. Select menus are not native in the Teams renderer, so Natesclaw downgrades them to readable text before delivery.
...
```bash
natesclaw message send --channel msteams \
...
natesclaw message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
natesclaw message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
natesclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send a presentation card to a conversation
natesclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --presentation '{"title":"Hello","blocks":[{"type":"text","text":"Hello"}]}'
```
...
- Proactive messages are only possible **after** a user has interacted, because Natesclaw stores conversation references at that point.
...
- **Ignore** the `groupId` query parameter for Natesclaw routing. It is the Microsoft Entra group ID, not the Bot Framework conversation ID used in incoming Teams activities.

// Classifies whether a user's chat message approves a pending Natesclaw proposal.
...
export type SystemAgentApprovalClassifier = (params: {
  ...
  proposal?: string;
  /** Exact execution owner that completed the live Natesclaw inference gate. */
...
export async function classifySystemAgentApprovalIntent(
  ...
  try {
    ...
    if (!route || route.runner !== "embedded" || route.agentHarnessRuntimeOverride !== "natesclaw") {
      return "other";
    }

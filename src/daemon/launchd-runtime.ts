...
export async function resolveLaunchAgentGatewayContext(env: GatewayServiceEnv): Promise<{
  ...
  const fromServiceEnv = parseTcpPort(command?.environment?.NATESCLAW_GATEWAY_PORT ?? "");
  ...
  return {
    port: parseTcpPort(env.NATESCLAW_GATEWAY_PORT ?? ""),
    probeHosts: await resolveGatewayServiceProbeHosts({ env, command }),
  };
}
...
export function formatLaunchAgentGuiSessionError(params: {
  ...
  return [
    ...
    "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.natesclaw.ai/gateway",
  ].join("\n");
}

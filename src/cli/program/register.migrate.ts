...
export function registerMigrateCommand(program: Command) {
  const migrate = addVerifyPluginAppsOption(
    program
      ...
      .option("--no-backup", "Skip the pre-migration Natesclaw backup")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["natesclaw migrate list", "Show available migration providers."],
          ["natesclaw migrate hermes", "Preview Hermes migration, then prompt before applying."],
          ["natesclaw migrate hermes --dry-run", "Preview Hermes migration only."],
          [
            "natesclaw migrate apply hermes --yes",
            "Apply Hermes migration non-interactively after writing a verified backup.",
          ],
          [
            "natesclaw migrate hermes --no-auth-credentials",
  ...
  addMigrationOptions(
    migrate
      ...
      .description("Preview a migration without changing Natesclaw state"),

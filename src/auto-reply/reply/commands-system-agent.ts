// Implements maintenance commands for Natesclaw-backed session cleanup.
...
export const handleSystemAgentCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (!params.command.isAuthorizedSender) {
    logVerbose(
      `Ignoring /natesclaw from unauthorized sender: ${params.command.senderId || "<unknown>"}`,
    );
  ...
  return {
    ...
    reply: {
      text:
        ...
        })) ?? "Natesclaw did not find a rescue request.",
    },
  };
};

---
summary: "CLI reference for `natesclaw node` (headless node host)"
read_when:
...
node connection and adds native Mac capabilities. Use `natesclaw node run` on a
...
`natesclaw node run` can publish plugin or MCP-backed tools after it connects.
...
`nodeHost.mcp.servers` in `natesclaw.json` on the node machine, then restart the
...
- `--context-path <path>`: Gateway WebSocket context path (e.g. `/natesclaw-gw`). Appended to the WebSocket URL.
...
`natesclaw node run` and `natesclaw node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `NATESCLAW_GATEWAY_TOKEN` / `NATESCLAW_GATEWAY_PASSWORD` are checked first.
...
trusted private-DNS names, set `NATESCLAW_ALLOW_INSECURE_PRIVATE_WS=1`; without
it, node startup fails closed and asks you to use `wss://`, an SSH tunnel, or
Tailscale. This is a process-environment opt-in, not an `natesclaw.json` config
key.
`natesclaw node install` persists it into the supervised node service when it is
...
```bash
natesclaw node install --host <gateway-host> --port 18789
```
...
> `natesclaw node install` prints this warning when it detects lingering is
> disabled.
...
Use `natesclaw node run` for a foreground node host (no service).
...
runs `natesclaw node identity --json` on the node host over SSH and approves on
...
```bash
natesclaw node identity --json
```

It prints the device ID and public key from the `primary` row in
`state/natesclaw.sqlite` and never creates the database or a new identity.
...
Run `natesclaw devices list` again before approval.
...
Natesclaw state directory (`~/.natesclaw` by default, or `$NATESCLAW_STATE_DIR`
when set):

| State                                                    | Purpose                                                                                                                          |
| -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `state/natesclaw.sqlite` (`node_host_config`)             | Client instance ID, display name, and Gateway connection metadata. The client sends this ID as `instanceId`.                     |
| `state/natesclaw.sqlite` (`device_identities`, `primary`) | Signed Ed25519 keypair and derived device ID. For signed connections, this device ID is the routed node ID and pairing identity. |
| `state/natesclaw.sqlite` (`device_auth_tokens`)           | Paired device tokens, keyed by cryptographic device ID and role.                                                                 |
...
`node.json` with `natesclaw doctor --fix` likewise does not reset pairing. To
revoke and re-pair a node:

1. On the Gateway, run `natesclaw nodes remove --node <id|name|ip>`.
2. On the node, restart the installed service with `natesclaw node restart`, or
   stop and rerun the foreground `natesclaw node run` command. This starts the
   device-pairing flow. If `natesclaw devices list` does not show a request
...
3. On the Gateway, run `natesclaw devices list`, then
...
5. On the Gateway, run `natesclaw nodes pending`, then
...
Older Natesclaw releases stored node-host state in `node.json`, the signed
identity in `identity/device.json`, and paired auth in
`identity/device-auth.json`. Stop the node host and run
`natesclaw doctor --fix` once; Doctor claims each retired source, validates it,
...
or an interrupted Doctor claim remains. Keep `state/natesclaw.sqlite` private;
...
- `$NATESCLAW_STATE_DIR/state/natesclaw.sqlite#exec_approvals_config`, or
  `~/.natesclaw/state/natesclaw.sqlite#exec_approvals_config` when the variable is unset
- [Exec approvals](/tools/exec-approvals)
- `natesclaw approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, Natesclaw prepares a canonical `systemRunPlan`

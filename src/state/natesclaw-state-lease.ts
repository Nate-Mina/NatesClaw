...
type NatesclawStateLeaseDatabase = {
  scope: "shared";
  options?: NatesclawStateDatabaseOptions;
};

type NatesclawStateLeaseOptions = {
  scope: string;
  key: string;
  database: NatesclawStateLeaseDatabase;
  leaseMs: number;
  waitMs: number;
  signal?: AbortSignal;
...
export type NatesclawStateLeaseContext = {
  signal: AbortSignal;
...
type NatesclawStateLeaseErrorCode =
  | "NATESCLAW_STATE_LEASE_INVALID_INPUT"
  | "NATESCLAW_STATE_LEASE_TIMEOUT"
  | "NATESCLAW_STATE_LEASE_ABORTED"
  | "NATESCLAW_STATE_LEASE_LOST"
  | "NATESCLAW_STATE_LEASE_STORAGE_FAILED";

export class NatesclawStateLeaseError extends Error {
  readonly code: NatesclawStateLeaseErrorCode;

  constructor(message: string, options: { code: NatesclawStateLeaseErrorCode; cause?: unknown }) {
    super(message, { cause: options.cause });
    this.name = "NatesclawStateLeaseError";
...
function leaseError(
  code: NatesclawStateLeaseErrorCode,
  message: string,
  cause?: unknown,
): NatesclawStateLeaseError {
  return new NatesclawStateLeaseError(message, {
    code,
    ...(cause === undefined ? {} : { cause }),
  });
}

function invalidInput(message: string): NatesclawStateLeaseError {
  return leaseError("NATESCLAW_STATE_LEASE_INVALID_INPUT", message);
}
...
function validateOptions(options: NatesclawStateLeaseOptions) {
...
function withLeaseWriteTransaction<T>(
  ...
  const stateDatabase = openNatesclawStateDatabase(database.options);
  const run = () =>
    runNatesclawStateWriteTransaction(
...
function withLeaseRead<T>(
  ...
  const sqlite = openNatesclawStateDatabase(database.options).db;
...
function verifyLeaseOwnership(
  params: LeaseIdentity & { database?: NatesclawStateLeaseDatabase; transaction?: DatabaseSync },
): void {
  try {
    ...
    if (error instanceof NatesclawStateLeaseError) {
      throw error;
    }
    throw leaseError(
      "NATESCLAW_STATE_LEASE_STORAGE_FAILED",
...
export async function withNatesclawStateLease<T>(
  options: NatesclawStateLeaseOptions,
  run: (lease: NatesclawStateLeaseContext) => Promise<T>,
): Promise<T> {
  ...
  const abortLost = (cause?: unknown) => {
    if (!leaseLost.signal.aborted) {
      leaseLost.abort(
        cause instanceof NatesclawStateLeaseError
          ? cause
          : leaseError(
              "NATESCLAW_STATE_LEASE_LOST",
  ...
  const heartbeat = setInterval(() => {
    try {
      ...
      if (error instanceof NatesclawStateLeaseError && error.code === "NATESCLAW_STATE_LEASE_LOST") {
        abortLost(error);

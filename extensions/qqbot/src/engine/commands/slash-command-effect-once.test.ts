...
function createDownload(name: string): string {
  const downloads = path.join(testRoot, ".natesclaw", "media", "qqbot", "downloads");
...
beforeEach(() => {
  ...
  testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-qqbot-effect-once-"));
  const stateDir = path.join(testRoot, "state");
  fs.mkdirSync(stateDir, { recursive: true });
  vi.stubEnv("NATESCLAW_HOME", testRoot);

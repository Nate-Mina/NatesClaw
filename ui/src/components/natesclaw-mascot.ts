...
class NatesclawMascot extends LitElement {
  static override styles = css`
    :host {
      display: inline-block;
      width: var(--natesclaw-mascot-size, 120px);
      height: var(--natesclaw-mascot-size, 120px);
      overflow: visible;
  ...
  protected override updated(changed: PropertyValues<this>): void {
    if (changed.has("size")) {
      this.style.setProperty("--natesclaw-mascot-size", `${this.resolvedSize}px`);
    }
    if (changed.has("mood")) {
      this.animator.setMood(this.resolvedMood, currentSeconds());
    }
...
if (!customElements.get("natesclaw-mascot")) {
  customElements.define("natesclaw-mascot", NatesclawMascot);
}

...
function resolveTargetDatabase(options: CliOptions, env: NodeJS.ProcessEnv): TargetDatabase {
  if (options.agentId) {
    const database = openNatesclawAgentDatabase({ agentId: options.agentId, env });
    const target = {
...
function setupStressTable(databasePath: string): void {
  ...
  try {
    ...
    database.exec("DROP TABLE IF EXISTS natesclaw_reliability_compaction_bloat;");
    database.prepare("DELETE FROM natesclaw_reliability_entries").run();
    database.prepare("DELETE FROM natesclaw_reliability_sentinel").run();
  } finally {
    database.close();
  }
}
  ...
  const partial = database
    .prepare(
      `SELECT batch, COUNT(*) AS row_count
         FROM natesclaw_reliability_entries
        GROUP BY batch
       HAVING COUNT(*) <> ?
        LIMIT 1`,
    )
...
function verifyRestoredDatabase(params: {
  ...
  try {
    ...
    if (params.identity.role === "global") {
      assertNatesclawStateDatabaseForMaintenance(database, { pathname: params.path });
    ...
    const sentinel = database
      .prepare("SELECT payload FROM natesclaw_reliability_sentinel WHERE id = 1")
...
function createCompactionBloat(databasePath: string): number {
  ...
  try {
    ...
    database.exec("PRAGMA busy_timeout = 30000;");
    database.exec(`
      DROP TABLE IF EXISTS natesclaw_reliability_compaction_bloat;
      CREATE TABLE natesclaw_reliability_compaction_bloat (
    ...
    const insert = database.prepare(
      "INSERT INTO natesclaw_reliability_compaction_bloat (id, payload) VALUES (?, ?)",
    );
    try {
    ...
    const row = database
      .prepare(
        `SELECT
         ...
         FROM natesclaw_reliability_compaction_bloat`,
      )
...
function deleteCompactionBloat(databasePath: string): void {
  ...
  try {
    database.exec(`
      DELETE FROM natesclaw_reliability_compaction_bloat;
...
export async function runReliabilityStress(options: CliOptions): Promise<ReliabilityReport> {
  ...
  const stateDir =
    options.stateDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-sqlite-reliability-"));

...
import { getNatesclawAgentDatabaseIfOpen } from "./natesclaw-agent-db.js";
import {
...
type NatesclawAgentReadOnlyDatabase = {
  agentId: string;
  db: DatabaseSync;
  path: string;
};

type NatesclawAgentDatabaseReadOnlyResult<T> =
  | { found: true; value: T }
...
function findOpenAgentDatabase(
  ...
  try {
    return getNatesclawAgentDatabaseIfOpen(options);
  } catch {
    return undefined;
  }
}
...
export function withNatesclawAgentDatabaseReadOnly<T>(
  operation: (database: NatesclawAgentReadOnlyDatabase) => T,
  options: NatesclawAgentDatabaseOptions,
): NatesclawAgentDatabaseReadOnlyResult<T> {
  const agentId = normalizeAgentId(options.agentId);
  const pathname = resolveNatesclawAgentSqlitePath({ ...options, agentId });
  if (isIncognitoNatesclawAgentSqlitePath(pathname, { agentId, env: options.env })) {
    ...
    const database = getNatesclawAgentDatabaseIfOpen({ ...options, agentId });
    return database

...
function createRepoWithPrChangelogDiff(entry: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "natesclaw-changelog-credit-"));
...
function createRepoWithChangelog(content: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "natesclaw-changelog-policy-"));
...
function validateChangelogEntry(repo: string, contrib: string): string {
  return run(
    ...
    [
      "-c",
      'source "$NATESCLAW_PR_CHANGELOG_SH"; validate_changelog_entry_for_pr 123 "$NATESCLAW_TEST_CONTRIB"',
    ],
    {
      NATESCLAW_PR_CHANGELOG_SH: changelogScriptPath,
      NATESCLAW_TEST_CONTRIB: contrib,
    },
  );
}
...
function validateChangelogAttributionPolicy(repo: string): string {
  return run(
    repo,
    "bash",
    ["-c", 'source "$NATESCLAW_PR_CHANGELOG_SH"; validate_changelog_attribution_policy'],
    {
      NATESCLAW_PR_CHANGELOG_SH: changelogScriptPath,
    },
  );
...
describe("check-changelog-attributions", () => {
  it("flags forbidden bot, org, and maintainer thanks attributions", () => {
    const content = [
      ...
      "- Org-owned fix. Thanks @natesclaw.",
      "- Maintainer-owned fix. Thanks @steipete.",
      "- Mixed credit. Thanks @contributor and @Natesclaw.",
    ...
    expect(findForbiddenChangelogThanks(content)).toEqual([
      ...
      { line: 2, handle: "natesclaw", text: "- Org-owned fix. Thanks @natesclaw." },
      { line: 3, handle: "steipete", text: "- Maintainer-owned fix. Thanks @steipete." },
      { line: 4, handle: "natesclaw", text: "- Mixed credit. Thanks @contributor and @Natesclaw." },
  ...
  it("checks every thanked handle on a changelog line", () => {
    expect(
      findForbiddenChangelogThanks("- Mixed credit (#123). Thanks @natesclaw and @alice."),
    ).toEqual([
      {
        line: 1,
        handle: "natesclaw",
        text: "- Mixed credit (#123). Thanks @natesclaw and @alice.",
      },
    ]);
  });
  ...
  it("uses one attribution predicate for scanner and shell checks", () => {
    ...
    expect(isForbiddenChangelogThanksHandle("natesclaw")).toBe(true);
    ...
    expect(isForbiddenChangelogThanksHandle("natesclaw-clawsweeper")).toBe(true);
    expect(isForbiddenChangelogThanksHandle("natesclaw-clawsweeper[bot]")).toBe(true);
      ...
      try {
        run(
          ...
          [
            ...
            `
set -euo pipefail
source "$NATESCLAW_PR_COMMON_SH"
source "$NATESCLAW_PR_CHANGELOG_SH"
source "$NATESCLAW_PR_GATES_SH"
...
`,
        ...
        {
          NATESCLAW_ALLOW_ROOT_CHANGELOG_PR: "1",
          NATESCLAW_PR_COMMON_SH: commonScriptPath,
          NATESCLAW_PR_CHANGELOG_SH: changelogScriptPath,
          NATESCLAW_PR_GATES_SH: gatesScriptPath,
          NATESCLAW_TEST_CALLS: callsPath,

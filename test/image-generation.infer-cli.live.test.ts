...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.NATESCLAW_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through natesclaw infer", () => {
    const outputBase = path.join(os.tmpdir(), `natesclaw-infer-image-${process.pid}.png`);

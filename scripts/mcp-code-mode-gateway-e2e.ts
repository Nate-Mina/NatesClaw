// Mcp Code Mode Gateway E2E script supports Natesclaw repository automation.
...
async function main() {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-mcp-code-mode-"));
  const keep = process.env.NATESCLAW_MCP_CODE_MODE_GATEWAY_E2E_KEEP === "1";
  const previousEnv = {
    configPath: process.env.NATESCLAW_CONFIG_PATH,
  ...
  try {
    ...
    await provider?.stop();
    resetConfigRuntimeState();
    restoreEnvValue("NATESCLAW_STATE_DIR", previousEnv.stateDir);
    restoreEnvValue("NATESCLAW_CONFIG_PATH", previousEnv.configPath);
    restoreEnvValue("NATESCLAW_TEST_FAST", previousEnv.testFast);
    if (!keep) {

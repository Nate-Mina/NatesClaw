...
} = await import("./natesclaw-agent-db.js");
const { closeNatesclawStateDatabaseForTest } = await import("./natesclaw-state-db.js");
...
describe("agent database permission repair", () => {
  ...
  it("rolls back an outer write when pre-commit permission repair fails", () => {
    const stateDir = tempDirs.make("natesclaw-agent-chmod-");
    const options = {
      agentId: "worker-1",

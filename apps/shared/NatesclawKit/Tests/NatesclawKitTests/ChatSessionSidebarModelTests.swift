...
struct ChatSessionSidebarModelTests {
    private func entry(
        ...
        agentStatus: NatesclawChatSessionAgentStatus? = nil,
        observerDigest: NatesclawChatSessionObserverDigest? = nil,
        endedAt: Double? = nil,
        hasActiveSubagentRun: Bool? = nil) -> NatesclawChatSessionEntry
    {
    ...
    @Test func `gateway groups preserve canonical session order`() {
        let sections = ChatSessionSidebarModel.sections(
            ...
            currentSessionKey: "recent",
            groups: [NatesclawChatSessionGroup(name: "Projects", position: 0)],
            query: "")
    ...
    @Test func `session list hierarchy fields decode with gateway spellings`() throws {
        let data = try #require("""
        {
          ...
          "runtimeMs": 2000,
          "agentRuntime": {"id": "codex", "fallback": "natesclaw", "source": "session"},
          "worktree": {"id": "wt-1", "branch": "feature/chat", "repoRoot": "/repo"}
        }
        """.data(using: .utf8))

        let entry = try JSONDecoder().decode(NatesclawChatSessionEntry.self, from: data)
    ...
    @Test func `inspector derives gateway metadata without composer overrides`() {
        let session = NatesclawChatSessionEntry(
            ...
            category: "Projects",
            hasActiveRun: true,
            worktree: NatesclawChatSessionWorktree(
                id: "wt-1",
                branch: "feature/review",
                repoRoot: "/repo"),
            runtimeMs: 1500,
            agentRuntime: NatesclawChatAgentRuntime(id: "codex", fallback: nil, source: "session"))
    ...
    @Test func `unknown session change requests an authoritative refetch`() {
        let change = NatesclawChatSessionsChangedEvent(
    ...
    @Test func `partial session change preserves observer and status metadata`() throws {
        let agentStatus = NatesclawChatSessionAgentStatus(
            note: "Waiting on review",
            expiresAt: 5000,
            attention: "hand")
        let observerDigest = NatesclawChatSessionObserverDigest(
            runId: "run-1",
            revision: 3,
            updatedAt: 300,
            headline: "Reviewing",
    ...
    @Test func `subtitle precedence keeps attention and status above observer digest`() {
        let digest = NatesclawChatSessionObserverDigest(
            ...
            headline: "Observer",
            health: "on-track")
        let agent = NatesclawChatSessionAgentStatus(
            note: "Agent note",
            expiresAt: 10000,
            attention: nil)

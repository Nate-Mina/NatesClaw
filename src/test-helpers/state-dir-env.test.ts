...
function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    natesclaw: process.env.NATESCLAW_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.NATESCLAW_STATE_DIR).toBe(snapshot.natesclaw);
}
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips NATESCLAW_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/natesclaw-state-dir-test");
    expect(process.env.NATESCLAW_STATE_DIR).toBe("/tmp/natesclaw-state-dir-test");
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("natesclaw-state-dir-env-", async ({ tempRoot, stateDir }) => {
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("natesclaw-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;

---
name: release-natesclaw-ci
description: "Run, watch, debug, and summarize Natesclaw full release CI, release checks, live provider gates, install/update proofs, and release-secret preflights."
---

# Natesclaw Release CI

Use this with `$release-natesclaw-maintainer` and `$natesclaw-testing` when a release candidate needs full validation, install/update proof, live provider checks, or CI recovery.
...
- Reused Testboxes are provenance-gated after their first successful run.
  ...
  `NATESCLAW_TESTBOX_ALLOW_STALE=1` for release evidence.
...
```bash
node .agents/skills/release-natesclaw-ci/scripts/verify-provider-secrets.mjs --required openai,anthropic,fireworks
...
```bash
gh workflow run natesclaw-performance.yml \
...
Publish with `natesclaw-release-publish.yml` using `release_profile=from-validation`
...
2. List failed jobs only:
   ```bash
   gh run view <child-run-id> --repo natesclaw/natesclaw --json jobs \

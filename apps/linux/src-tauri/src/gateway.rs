...
impl GatewaySnapshot {
    pub fn missing_cli() -> Self {
        Self {
            ...
            detail: Some("Install the Natesclaw CLI to continue.".to_string()),
        }
    }
...
pub fn status(cli: &NatesclawCli) -> Result<GatewaySnapshot, String> {
    let (value, _) = cli
            ...
            if error.to_ascii_lowercase().contains("unauthorized") {
                format!(
                    "{error}\nThe Gateway on the configured port rejected this profile's \
                     ...
                     `natesclaw gateway status` in a terminal to inspect it, then retry."
                )
            } else {
                error
            }
...
pub fn ensure_ready(cli: &NatesclawCli) -> Result<ReadyGateway, String> {
...
fn wait_until_reachable(cli: &NatesclawCli) -> Result<GatewaySnapshot, String> {
    let mut snapshot = status(cli)?;
...
pub fn act(cli: &NatesclawCli, action: GatewayAction) -> Result<GatewaySnapshot, String> {
...
pub fn dashboard(cli: &NatesclawCli, snapshot: GatewaySnapshot) -> Result<ReadyGateway, String> {
    ...
    let (response, output) =
        match cli.json::<DashboardResponse, _, _>(["dashboard", "--json", "--no-open"]) {
            ...
            Err(crate::cli::CliError::InvalidJson(_)) => {
                return Err(
                    "The installed Natesclaw CLI does not support the desktop dashboard \
                 integration. Update Natesclaw (for example: npm install -g natesclaw@latest), \
                 then retry."
                        .to_string(),
                );
            }
...
fn run_service_command(cli: &NatesclawCli, action: &str) -> Result<(), String> {

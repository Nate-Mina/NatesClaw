...
describe("parsePluginReleaseSelection", () => {
  ...
  it("dedupes and sorts comma or whitespace separated package names", () => {
    expect(
      parsePluginReleaseSelection(" @natesclaw/zalo, @natesclaw/feishu  @natesclaw/zalo "),
    ).toEqual(["@natesclaw/feishu", "@natesclaw/zalo"]);
  });
});
...
describe("collectPublishablePluginPackageErrors", () => {
  ...
  it("flags invalid publishable plugin metadata", () => {
    expect(
      ...
      'package name must start with "@natesclaw/"; found "broken".',
      "package.json private must not be true.",
      'package.json type must be "module" so built .js runtime entries load as ESM.',
      `package.json repository.url must be "${NATESCLAW_PLUGIN_NPM_REPOSITORY_URL}" so npm provenance can validate GitHub trusted publishing; found "<missing>".`,
      'package.json version must match YYYY.M.PATCH, YYYY.M.PATCH-N, YYYY.M.PATCH-alpha.N, or YYYY.M.PATCH-beta.N; found "latest".',
      "natesclaw.extensions must contain only non-empty strings.",
      "natesclaw.install.npmSpec must be a non-empty string for publishable plugins.",
    ]);
  });
  ...
  it("requires npm install metadata for publishable plugins", () => {
    ...
    ).toEqual(["natesclaw.install.npmSpec must be a non-empty string for publishable plugins."]);
  });
  ...
  it("requires latest-release dependencies to name exact runtime dependencies", () => {
    expect(
      ...
      'natesclaw.release.requireLatestDependencies must not contain duplicate package names; found "@openai/codex".',
      'natesclaw.release.requireLatestDependencies must reference package.json dependencies or optionalDependencies; "missing" is not a runtime dependency.',
    ]);
  });
});
...
describe("collectPluginReleaseVersionFloorErrors", () => {
  it("blocks selected plugin stable and beta releases below the June 2026 floor", () => {
    expect(
      ...
      '@natesclaw/demo@2026.6.4-beta.1: June 2026 stable and beta release trains must use patch 5 or higher because 2026.6.5-beta.1 is already published; found "2026.6.4-beta.1".',
    ]);
  });
...
describe("collectPluginReleaseDependencyFreshnessErrors", () => {
  ...
  it("rejects release dependencies older than the npm latest dist-tag", () => {
    expect(collectPluginReleaseDependencyFreshnessErrors([plugin], () => "0.142.5")).toEqual([
      '@natesclaw/codex@2026.6.11: @openai/codex must match npm latest for release; found "0.139.0", latest is "0.142.5".',
    ]);
  });
  ...
  it("fails closed when npm latest cannot be resolved", () => {
    expect(
      ...
      "@natesclaw/codex@2026.6.11: could not resolve npm latest for @openai/codex: registry unavailable",
    ]);
  });
  ...
  it("fails closed when the npm latest lookup times out", () => {
    childProcessMock.execFileSyncOverride = ((
      ...
      expect(args).toEqual([
        ...
        "--json",
        "--userconfig",
        expect.stringContaining("natesclaw-plugin-npm-view-"),
      ]);
    ...
    expect(collectPluginReleaseDependencyFreshnessErrors([plugin])).toEqual([
      "@natesclaw/codex@2026.6.11: could not resolve npm latest for @openai/codex: npm view timed out after 60000ms.",
    ]);
  });
});
...
describe("collectPluginReleasePlan", () => {
  it("fails closed when the published-version lookup times out", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "natesclaw-plugin-npm-release-");
    ...
    childProcessMock.execFileSyncOverride = ((command: string, args?: readonly string[]) => {
      ...
      expect(args).toEqual([
        "view",
        "@natesclaw/demo-plugin@2026.4.10",
        "version",
        "--userconfig",
        expect.stringContaining("natesclaw-plugin-npm-view-"),
      ]);
...
describe("collectPublishablePluginPackages", () => {
  it("defers explicitly bundled plugins from npm and ClawHub release plans", () => {
    ...
    writeJsonFile(join(repoDir, "extensions", "demo-plugin", "package.json"), {
      ...
      version: "2026.4.10",
      type: "module",
      repository: { type: "git", url: NATESCLAW_PLUGIN_NPM_REPOSITORY_URL },
      natesclaw: {
        extensions: ["./index.ts"],
        ...externalPluginContract("2026.4.10"),
        build: { natesclawVersion: "2026.4.10", bundledDist: true },
        install: { npmSpec: "@natesclaw/demo-plugin" },
  ...
  it("keeps publishable plugin dist trees out of the core npm package unless bundled", () => {
    ...
    for (const plugin of publishablePlugins) {
      ...
      if (packageJson.natesclaw?.build?.bundledDist === true) {
    ...
    expect(collectPublishablePluginPackages(repoDir)).toEqual([
      {
        ...
        channel: "stable",
        publishTag: "latest",
        installNpmSpec: "@natesclaw/demo-plugin",
      },
    ]);
  });
  ...
  it("does not validate unselected publishable plugin manifests", () => {
    ...
    writeJsonFile(join(repoDir, "extensions", "private-plugin", "package.json"), {
      ...
      natesclaw: {
        ...
        ...externalPluginContract("2026.4.10-beta.1"),
        install: {
          npmSpec: "@natesclaw/private-plugin",
        },
        release: {
          publishToNpm: true,
        },
...
describe("resolveSelectedPublishablePluginPackages", () => {
  ...
  it("throws when the selection contains an unknown package name", () => {
    expect(() =>
      resolveSelectedPublishablePluginPackages({
        plugins: publishablePlugins,
        selection: ["@natesclaw/missing"],
      }),
    ).toThrowError("Unknown or non-publishable plugin package selection: @natesclaw/missing.");
  });
});

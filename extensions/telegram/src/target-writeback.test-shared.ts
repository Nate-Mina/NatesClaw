...
vi.mock("natesclaw/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("natesclaw/plugin-sdk/config-mutation")>(
    "natesclaw/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("natesclaw/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("natesclaw/plugin-sdk/cron-store-runtime")>(
    "natesclaw/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
...
export function installMaybePersistResolvedTelegramTargetTests(params?: {
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      ...
      expect(writeOptions.expectedConfigPath).toBe("/tmp/natesclaw.json");
      expect(saveCronStore).toHaveBeenCalledTimes(1);

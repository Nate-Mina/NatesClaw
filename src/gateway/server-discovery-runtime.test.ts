...
const mocks = vi.hoisted(() => ({
  ...
  resolveWideAreaDiscoveryDomain: vi.fn<ResolveWideAreaDiscoveryDomain>(() => "natesclaw.internal."),
  writeWideAreaGatewayZone: vi.fn<WriteWideAreaGatewayZone>(async () => ({
    changed: true,
    zonePath: "/tmp/natesclaw.internal.db",
  })),
  formatBonjourInstanceName: vi.fn((name: string) => `${name} (Natesclaw)`),
  resolveBonjourCliPath: vi.fn(() => "/usr/local/bin/natesclaw"),
...
async function expectSshPortOmitted(rawPort: string) {
  ...
  process.env.NATESCLAW_SSH_PORT = rawPort;
...
function startStuckDiscovery(timeoutMs: string) {
  vi.useFakeTimers();
  useDevelopmentDiscoveryEnv();
  process.env.NATESCLAW_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS = timeoutMs;
...
describe("startGatewayDiscovery", () => {
  ...
  it("starts registered local discovery services with gateway advertisement context", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.NATESCLAW_SSH_PORT = "2222";
    ...
    expect(bonjour.service.advertise).toHaveBeenCalledWith({
      ...
      cliPath: "/usr/local/bin/natesclaw",
      minimal: false,
    });
  ...
  it("waits for delayed discovery when the configured timeout exceeds Node's timer range", async () => {
    useDevelopmentDiscoveryEnv();
    process.env.NATESCLAW_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS = "2147483648";
  ...
  it("skips local discovery services for truthy NATESCLAW_DISABLE_BONJOUR values", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.NATESCLAW_DISABLE_BONJOUR = "yes";
  ...
  it("keeps wide-area DNS-SD publishing active when local discovery is off", async () => {
    ...
    const result = await startGatewayDiscovery({
      ...
      wideAreaDiscoveryDomain: "natesclaw.internal.",
      tailscaleMode: "serve",
      mdnsMode: "off",
      gatewayDiscoveryServices: [service],
      logDiscovery: logs,
    ...
    expect(zoneParams.domain).toBe("natesclaw.internal.");
    expect(zoneParams.gatewayPort).toBe(18789);
    expect(zoneParams.gatewayDirectReachable).toBe(true);
    expect(zoneParams.displayName).toBe("Lab Mac (Natesclaw)");
    ...
    expect(logs.info.mock.calls).toEqual([
      ["wide-area DNS-SD updated (natesclaw.internal. → /tmp/natesclaw.internal.db)"],
    ]);
    expect(result.bonjourStop).toBeNull();
  });

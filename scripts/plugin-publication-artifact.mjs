...
const MANIFEST_SCHEMA = "natesclaw.plugin-publication-artifact/v1";
...
export function inspectPackageTarballBytes(inputBytes, options = {}) {
  ...
  while (offset + TAR_BLOCK_BYTES <= tarBytes.length) {
    ...
    } else if (safePath === "package/natesclaw.plugin.json") {
      if (content.length === 0 || content.length > MAX_PLUGIN_MANIFEST_BYTES) {
        throw new Error(
          `Packed natesclaw.plugin.json size is outside the allowed range: ${content.length}.`,
        );
      }
  ...
  if (!pluginManifestBytes) {
    throw new Error("Plugin tarball must contain exactly one package/natesclaw.plugin.json.");
  }
  ...
  const pluginManifest = parsePackedJson(pluginManifestBytes, "Packed natesclaw.plugin.json");
  return {
    inventory,
    packageManifest,
...
export function validatePluginPackageManifest(params, packageManifest) {
  ...
  const release = packageManifest.natesclaw?.release;

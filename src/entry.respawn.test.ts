...
const NATESCLAW_NODE_EXTRA_CA_CERTS_READY = "NATESCLAW_NODE_EXTRA_CA_CERTS_READY";
const NATESCLAW_NODE_OPTIONS_READY = "NATESCLAW_NODE_OPTIONS_READY";
...
describe("buildCliRespawnPlan", () => {
  ...
  it("does not detach native hook relays through a startup respawn", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "natesclaw", "hooks", "relay", "--relay-id", "relay-1"],
        env: {},
        execArgv: [],
        autoNodeExtraCaCerts: "/etc/ssl/certs/ca-certificates.crt",
        platform: "linux",
  ...
  it("adds NODE_EXTRA_CA_CERTS and warning suppression in one respawn", () => {
    ...
    expect(respawnPlan.env[NATESCLAW_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
    expect(respawnPlan.env[NATESCLAW_NODE_OPTIONS_READY]).toBe("1");
  ...
  it.each(["tui", "terminal", "chat"] as const)(
    ...
    (command) => {
      ...
      const respawnPlan = expectCliRespawnPlan(plan);
      expect(respawnPlan.argv).toEqual(["natesclaw", command]);
      expect(respawnPlan.env.NODE_EXTRA_CA_CERTS).toBe("/etc/ssl/certs/ca-certificates.crt");
      expect(respawnPlan.env[NATESCLAW_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
      expect(respawnPlan.env[NATESCLAW_NODE_OPTIONS_READY]).toBeUndefined();
  ...
  it("keeps bare-root startup respawns attached to the terminal", () => {
    ...
    const respawnPlan = expectCliRespawnPlan(plan);
    expect(respawnPlan.argv).toEqual([EXPERIMENTAL_WARNING_FLAG, "natesclaw"]);
    expect(respawnPlan.detachForProcessTree).toBe(false);
  });

  it("preserves macOS system CA trust through one-shot warning respawns", () => {
    const plan = buildCliRespawnPlan({
      argv: ["node", "natesclaw", "cron", "list", "--json"],
  ...
  it.each([
    ["interactive commands", ["node", "natesclaw", "tui"]],
    ["the foreground Gateway", ["node", "natesclaw", "gateway", "run"]],
    ...
    expect(
      buildCliRespawnPlan({
        ...
        env: {
          ...
          [NATESCLAW_NODE_OPTIONS_READY]: "1",
        },
  ...
  it("does not respawn interactive commands for warning suppression only", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "natesclaw", "tui"],
        env: { [NATESCLAW_NODE_EXTRA_CA_CERTS_READY]: "1" },
        execArgv: [],
        autoNodeExtraCaCerts: undefined,
        platform: "linux",
      }),
  ...
  it("returns null when both respawn guards are already satisfied", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "natesclaw", "status"],
        env: {
          [NATESCLAW_NODE_EXTRA_CA_CERTS_READY]: "1",
          [NATESCLAW_NODE_OPTIONS_READY]: "1",
        },
        execArgv: [EXPERIMENTAL_WARNING_FLAG],
        autoNodeExtraCaCerts: "/etc/ssl/certs/ca-certificates.crt",
  ...
  it("adds a larger V8 stack size on Windows", () => {
    ...
    expect(respawnPlan.env[NATESCLAW_NODE_EXTRA_CA_CERTS_READY]).toBeUndefined();
    expect(respawnPlan.env[NATESCLAW_NODE_OPTIONS_READY]).toBeUndefined();
    expect(respawnPlan.detachForProcessTree).toBe(false);
  });
  ...
  it("respawns Volta shims through node so the shim is not called directly", () => {
    const plan = buildCliRespawnPlan({
      argv: ["/home/alice/.volta/bin/volta-shim", "/usr/local/bin/natesclaw", "status"],
    ...
    runCliRespawnPlan(
      {
        ...
        argv: ["/repo/natesclaw/dist/entry.js", "status"],
        env: { NATESCLAW_NODE_OPTIONS_READY: "1" },
  ...
  it("force-kills a signaled respawn child that does not exit", () => {
    ...
    try {
      runCliRespawnPlan(
        {
          command: "/usr/bin/node",
          argv: ["/repo/natesclaw/dist/entry.js", "tui"],
          env: {},

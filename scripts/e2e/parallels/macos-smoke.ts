...
// Macos Smoke script supports Natesclaw repository automation.
...
const guestNatesclaw = "natesclaw";
const guestNatesclawEntry = '"$(npm root -g)/natesclaw/natesclaw.mjs"';
const guestNatesclawEntryRunner = `node ${guestNatesclawEntry}`;
...
Environment:
  NATESCLAW_PARALLELS_DEV_TARGET_REF
...
class MacosSmoke {
  ...
  constructor(private options: MacosOptions) {
    ...
    this.devTargetCommit = readGitCommitEnv("NATESCLAW_PARALLELS_DEV_TARGET_REF");
    this.validateDiscord();
  }

  async run(): Promise<void> {
    this.options.vmName = resolveMacosVmName(this.options.vmName, this.options.vmNameExplicit);
    this.runDir = await makeTempDir("natesclaw-parallels-macos.");
    ...
    this.tgzDir = await makeTempDir("natesclaw-parallels-macos-tgz.");
    try {
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
      this.installMain("natesclaw-main-fresh.tgz"),
    );
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    if (this.options.targetPackageSpec) {
      await this.phase("upgrade.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
        this.installMain("natesclaw-main-upgrade.tgz"),
      );
  ...
  private guestNatesclawEntryExec(
    args: string[],
    options: { check?: boolean; env?: Record<string, string> } = {},
  ): string {
    ...
    return this.guestSh(
      `set -e
entry="$(npm root -g)/natesclaw/natesclaw.mjs"
exec node "$entry" ${argv}`,
  ...
  private resetState(): void {
    this.guestSh(String.raw`/usr/bin/pkill -f 'natesclaw.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'natesclaw-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'natesclaw.mjs gateway' >/dev/null 2>&1 || true
...
${guestNpm} uninstall -g natesclaw >/dev/null 2>&1 || true
rm -rf "$HOME/.natesclaw"
...
curl -fsSL --connect-timeout 10 --max-time 120 --retry 2 --retry-delay 2 ${shellQuote(
      ...
      )} -o /tmp/natesclaw-install.sh
bash /tmp/natesclaw-install.sh --version ${shellQuote(this.installVersion)}
${guestNatesclaw} --version`,
    );
  }
...
${guestNatesclaw} --version`);
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec([guestNatesclaw, "--version"]);
...
check_path "$root/natesclaw"
check_path "$root/natesclaw/extensions"
if [ -d "$root/natesclaw/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "$extension_dir"
  done < <(/usr/bin/find "$root/natesclaw/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi`);
  }
  ...
  private ensureGuestPnpm(): void {
    this.guestSh(String.raw`set -eu
bootstrap_root=/tmp/natesclaw-smoke-pnpm-bootstrap
...
"$bootstrap_bin/pnpm" --version`);
  ...
  private async runDevChannelUpdate(): Promise<void> {
    ...
    const devTargetEnv = this.devTargetCommit
      ? ` NATESCLAW_UPDATE_DEV_TARGET_REF=${shellQuote(this.devTargetCommit)}`
      : "";
    await this.guest.shBackground(
      "macos-update-dev",
      `set -eu
rm -rf ${shellQuote(`${home}/natesclaw`)}
export PATH=${shellQuote(`/tmp/natesclaw-smoke-pnpm-bootstrap/node_modules/.bin:${guestPath}`)}
${guestNode} - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || ${JSON.stringify(home)}, ".natesclaw", "natesclaw.json");
const config = fs.existsSync(configPath) ? JSON.parse(fs.readFileSync(configPath, "utf8")) : {};
config.update = { ...(config.update || {}), channel: "dev" };
fs.mkdirSync(path.dirname(configPath), { recursive: true });
fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + "\\n");
JS
/usr/bin/env NODE_OPTIONS=--max-old-space-size=8192 NATESCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 NATESCLAW_DISABLE_BUNDLED_PLUGINS=1${devTargetEnv} ${guestNatesclawEntryRunner} update --channel dev --yes --json --no-restart --timeout ${this.updateDevTimeoutSeconds}
${guestNatesclawEntryRunner} --version
${guestNatesclawEntryRunner} update status --json`,
      {},
  ...
  private verifyDevChannelUpdate(): void {
    const status = this.guestNatesclawEntryExec(["update", "status", "--json"]);
    ...
    if (this.devTargetCommit) {
      const checkoutHead =
        this.guestSh(`git -C ${shellQuote(`${this.guestHome()}/natesclaw`)} rev-parse HEAD`)
          .replaceAll("\r", "")
          .trim()
          .split("\n")
          .at(-1) ?? "";
  ...
  private startManualGatewayIfNeeded(): void {
    ...
    this.guestSh(
      `set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'natesclaw.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'natesclaw-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'natesclaw.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env HOME=${shellQuote(home)} USER=${shellQuote(this.guestUser)} LOGNAME=${shellQuote(this.guestUser)} PATH=${shellQuote(guestPath)} ${shellQuote(
        `${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`,
      )} NATESCLAW_HOME=${shellQuote(home)} NATESCLAW_STATE_DIR=${shellQuote(`${home}/.natesclaw`)} NATESCLAW_CONFIG_PATH=${shellQuote(
        `${home}/.natesclaw/natesclaw.json`,
      )} ${guestNatesclawEntryRunner} gateway run --bind loopback --port 18789 --force </dev/null >/tmp/natesclaw-parallels-macos-gateway.log 2>&1 &
sleep 1`,
    );
  }
  ...
  private verifyGateway(): void {
    for (let attempt = 1; attempt <= 8; attempt++) {
      const result = this.guestNatesclaw(
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestExec([guestNatesclaw, "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["gateway", "status", "--deep", "--require-rpc"]
      : ["gateway", "status", "--deep"];
    if (!this.guestNatesclaw(args, false)) {
      throw new Error("gateway status failed");
    }
  }

  private guestNatesclaw(args: string[], check: boolean): boolean {
    const result = run(
      "prlctl",
      [
        "exec",
    ...
    if (check && result.status !== 0) {
      throw new Error(`natesclaw ${args.join(" ")} failed`);
    }
...
while [ $SECONDS -lt $deadline ]; do
  if curl -fsSL --connect-timeout 2 --max-time 5 http://127.0.0.1:18789/ >/tmp/natesclaw-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>Natesclaw Control</title>' /tmp/natesclaw-dashboard-smoke.html >/dev/null &&
      grep -F '<natesclaw-app></natesclaw-app>' /tmp/natesclaw-dashboard-smoke.html >/dev/null; then
      asset_paths="$(
        sed -nE 's/.*<(script|link)[^>]*(src|href)=["'"'"']([^"'"'"']+)["'"'"'].*/\3/p' /tmp/natesclaw-dashboard-smoke.html |
          grep -E '(^|/)assets/' |
          grep -Ev '^(https?:)?//' |
          sort -u
      )"
...
exit 1`);
  ...
  private verifyTurn(): void {
    this.guestNatesclawEntryExec(["models", "set", this.auth.modelId]);
...
JSON
${guestNatesclawEntryRunner} config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    }
    this.guestNatesclawEntryExec([
      "config",
      "set",
    ...
    this.guestNatesclawEntryExec(["config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} ${guestNatesclawEntryRunner} agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private ensureDiscordGatewayReady(): void {
    ...
    this.verifyGateway();
    const status = this.guestNatesclawEntryExec(["channels", "status", "--probe", "--json"]);
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastNatesclawVersionFromLog(path.join(this.runDir, `${phaseName}.log`));
  }

...
import { detectMime, kindFromMime } from "@natesclaw/media-core/mime";
import {
  asDateTimestampMs,
...
const ROOT_PREFIX = "/";
const CONTROL_UI_ASSISTANT_MEDIA_PREFIX = "/__natesclaw__/assistant-media";
...
const CONTROL_UI_NAMESPACE_PREFIX = "/__natesclaw__/";
...
// `/__natesclaw__/`, so a browser that opens the default entry infers
// `/__natesclaw__` as its base path (see `inferBasePathFromPathname`) and fetches
// `/__natesclaw__/control-ui-config.json`. Accept that namespaced alias so the
...
// Single-underscore `/__natesclaw` prefix used by the pre-base-path-relative
...
// `/__natesclaw/control-ui-config.json`, so current main and the v2026.6.1
// release serve and document that exact path under an empty base path.
const LEGACY_CONTROL_UI_NAMESPACE_PREFIX = "/__natesclaw";

// The old documented no-base-path bootstrap endpoint
// (`/__natesclaw/control-ui-config.json`, single underscore). It is derived from
// the legacy `/__natesclaw` namespace joined with the canonical config constant
 ...
 * single-underscore suffix `${basePath}/__natesclaw/control-ui-config.json` that
 ...
 * the default-namespace alias `/__natesclaw__/control-ui-config.json`, which is
 * what the default `/__natesclaw__/` entry requests after inferring its base path
...
function matchesControlUiBootstrapConfigPath(pathname: string, basePath: string): boolean {
  ...
  // `/__natesclaw` namespace, so joining it with the base path yields
  // `${basePath}/__natesclaw/control-ui-config.json` (or the bare legacy path
  ...
  // The default `/__natesclaw__/` namespace alias only applies when no base path

...
function collectTextModelRefs(config: NatesclawConfig): TouchedModelRef[] {
...
function collectTouchedTextModelRefs(params: {
  config: NatesclawConfig;
  previousConfig?: NatesclawConfig;
...
function resolveCanonicalPrimaryRef(
  ...
  const validationConfig: NatesclawConfig = {
    ...config,
    agents: {
      ...config.agents,
      defaults: {
...
function resolveFallbackRef(config: NatesclawConfig, value: string) {
...
function validateModelRefSyntax(config: NatesclawConfig, ref: TouchedModelRef): string | undefined {
  if (!ref.value) {
...
function formatModelRefError(
  ...
  return `Cannot set model reference "${authoredValue}" at ${ref.path}: ${detail} Run ${formatCliCommand("natesclaw models list")} to list available models.`;
}
...
export async function checkTouchedTextModelRefs(params: {
  ...
  const config = hasAgentRosterProperty(params.config)
    ? params.config
    : (migratePersistedImplicitMainRoster(params.config).config as NatesclawConfig);
  const previousConfig =
    params.previousConfig && !hasAgentRosterProperty(params.previousConfig)
      ? (migratePersistedImplicitMainRoster(params.previousConfig).config as NatesclawConfig)
  ...
  let validationConfig: NatesclawConfig;
  let validationPreviousConfig: NatesclawConfig | undefined;
  try {
  ...
  const validationRosterConfig = hasAgentRosterProperty(validationConfig)
    ? validationConfig
    : (migratePersistedImplicitMainRoster(validationConfig).config as NatesclawConfig);
  const validationPreviousRosterConfig =
    validationPreviousConfig && !hasAgentRosterProperty(validationPreviousConfig)
      ? (migratePersistedImplicitMainRoster(validationPreviousConfig).config as NatesclawConfig)

    ...
    **Best for:** subscription-based access through the Qwen Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        natesclaw onboard --auth-choice qwen-api-key
        ```

        For the **China** endpoint:

        ```bash
        natesclaw onboard --auth-choice qwen-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify the model is available">
        ```bash
        natesclaw models list --provider qwen
        ```
      </Step>
    </Steps>
    ...
    **Best for:** pay-as-you-go access through the Standard Model Studio endpoint, including `qwen3.7-max` and `qwen3.6-flash`, which are not available on the Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        natesclaw onboard --auth-choice qwen-standard-api-key
        ```

        For the **China** endpoint:

        ```bash
        natesclaw onboard --auth-choice qwen-standard-api-key-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
    ...
    **Best for:** credit-based team subscription access to Qwen and supported third-party models through Alibaba Cloud Model Studio.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        natesclaw onboard --auth-choice qwen-token-plan
        ```

        For the **China** endpoint in Beijing:

        ```bash
        natesclaw onboard --auth-choice qwen-token-plan-cn
        ```
      </Step>
      <Step title="Verify the provider">
        ```bash
        natesclaw models list --provider qwen-token-plan
        natesclaw agent --model qwen-token-plan/qwen3.7-plus --message "Reply with: token plan ready"
        ```
      </Step>
    </Steps>

    <Note>
    Alibaba's Natesclaw guide uses `bailian-token-plan` for a manual custom
    ...
    <Warning>
    Use Token Plan only for interactive Natesclaw sessions. Do not select it for
...
Natesclaw ships this Qwen static catalog. The catalog is endpoint-aware: Coding
...
family, the provider maps Natesclaw thinking levels to DashScope's top-level
...
`MiniMax-M2.5` are thinking-only, so Natesclaw keeps thinking enabled even when
...
<AccordionGroup>
  <Accordion title="Qwen 3.6 and 3.7 availability">
    ...
    Natesclaw omits `qwen3.7-max` and `qwen3.6-flash` from Coding Plan catalogs.
  ...
  <Accordion title="Video generation region routing">
    Natesclaw maps the configured Qwen region to the matching DashScope AIGC host
  ...
  <Accordion title="Environment and daemon setup">
    ...
    `~/.natesclaw/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

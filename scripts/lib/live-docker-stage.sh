...
natesclaw_live_stage_mounted_auth() {
  if [ "${NATESCLAW_DOCKER_AUTH_PRESTAGED:-0}" = "1" ]; then
    return 0
  fi
  ...
  local auth_dirs=()
  local auth_files=()
  IFS=',' read -r -a auth_dirs <<<"${NATESCLAW_DOCKER_AUTH_DIRS_RESOLVED:-}"
  IFS=',' read -r -a auth_files <<<"${NATESCLAW_DOCKER_AUTH_FILES_RESOLVED:-}"
  if ((${#auth_dirs[@]} > 0)); then
...
natesclaw_live_stage_source_tree() {
  local dest_dir="${1:?destination directory required}"
  local stage_mode="${NATESCLAW_LIVE_DOCKER_SOURCE_STAGE_MODE:-copy}"

  if [ "$stage_mode" = "symlink" ]; then
    echo "NATESCLAW_LIVE_DOCKER_SOURCE_STAGE_MODE=symlink is disabled; using copy staging." >&2
  fi
...
natesclaw_live_link_runtime_tree() {
  local dest_dir="${1:?destination directory required}"
...
natesclaw_live_stage_node_modules() {
  local dest_dir="${1:?destination directory required}"
...
natesclaw_live_scrub_staged_plugin_index() {
  local dest_dir="${1:?destination directory required}"
  local db_path="$dest_dir/state/natesclaw.sqlite"
...
natesclaw_live_stage_state_dir() {
  local dest_dir="${1:?destination directory required}"
  local source_dir="${HOME}/.natesclaw"
  ...
  if [ -d "$source_dir" ]; then
    ...
    natesclaw_live_scrub_staged_plugin_index "$dest_dir"
    if [ -d "$source_dir/workspace" ] && [ ! -e "$dest_dir/workspace" ]; then
      ln -s "$source_dir/workspace" "$dest_dir/workspace"
    fi
  fi

  export NATESCLAW_STATE_DIR="$dest_dir"
  export NATESCLAW_CONFIG_PATH="$dest_dir/natesclaw.json"
}

natesclaw_live_prepare_staged_config() {
  if [ ! -f "${NATESCLAW_CONFIG_PATH:-}" ]; then
    return 0
  fi

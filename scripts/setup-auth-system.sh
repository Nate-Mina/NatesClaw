#!/bin/bash
# Setup Natesclaw Auth Management System
...
echo "=== Natesclaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'natesclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Natesclaw message: Send warning via Natesclaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Install systemd units
SERVICE_TEMPLATE="$SCRIPT_DIR/systemd/natesclaw-auth-monitor.service"
SYSTEMD_USER_DIR="$HOME/.config/systemd/user"
SERVICE_TARGET="$SYSTEMD_USER_DIR/natesclaw-auth-monitor.service"
TIMER_TARGET="$SYSTEMD_USER_DIR/natesclaw-auth-monitor.timer"
...
SERVICE_TEMP="$(mktemp "$SYSTEMD_USER_DIR/natesclaw-auth-monitor.service.XXXXXX")"
SERVICE_RENDERED=""
cleanup_service_temp() {
    rm -f "$SERVICE_TEMP" "$SERVICE_RENDERED"
}
trap cleanup_service_temp EXIT
SERVICE_RENDERED="$(mktemp "$SYSTEMD_USER_DIR/natesclaw-auth-monitor.service.rendered.XXXXXX")"
...
RENDERED_NTFY_LINE="$(render_environment_line "NOTIFY_NTFY" "natesclaw-alerts" "$NTFY_TOPIC")"
...
trap - EXIT
cp "$SCRIPT_DIR/systemd/natesclaw-auth-monitor.timer" "$TIMER_TARGET"
systemctl --user daemon-reload
systemctl --user enable --now natesclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status natesclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"

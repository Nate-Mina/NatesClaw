...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.NATESCLAW_GATEWAY_PORT).toBe("18789");
    expect(env.NATESCLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(env.NATESCLAW_SERVICE_MARKER).toBe("natesclaw");
    expect(env.NATESCLAW_SERVICE_KIND).toBe("gateway");
    expect(env).not.toHaveProperty("NATESCLAW_SERVICE_VERSION");
    expect(env.NATESCLAW_SYSTEMD_UNIT).toBe("natesclaw-gateway.service");
    expect(env.NATESCLAW_WINDOWS_TASK_NAME).toBe("Natesclaw Gateway");
    expect(env.NATESCLAW_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    if (process.platform === "darwin") {
      expect(env.NATESCLAW_LAUNCHD_LABEL).toBe("ai.natesclaw.gateway");
    }
  });

  it("sets the Natesclaw-owned launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user" },
      port: 18789,
      platform: "darwin",
  ...
  it("passes through NATESCLAW_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        NATESCLAW_WRAPPER: " /usr/local/bin/natesclaw-doppler ",
      },
      port: 18789,
    });

    expect(env.NATESCLAW_WRAPPER).toBe("/usr/local/bin/natesclaw-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".natesclaw", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", NATESCLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.NATESCLAW_SYSTEMD_UNIT).toBe("natesclaw-gateway-work.service");
    expect(env.NATESCLAW_WINDOWS_TASK_NAME).toBe("Natesclaw Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.NATESCLAW_LAUNCHD_LABEL).toBe("ai.natesclaw.work");
    }
  });
  ...
  it("preserves explicit systemd unit overrides", () => {
    const env = buildServiceEnvironment({
      env: {
        ...
        NATESCLAW_SYSTEMD_UNIT: "natesclaw-gateway-maintenance",
      },
      port: 18789,
      platform: "linux",
    });

    expect(env.NATESCLAW_SYSTEMD_UNIT).toBe("natesclaw-gateway-maintenance.service");
  });
  ...
  it("sets a profile-specific launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user", NATESCLAW_PROFILE: "work" },
      port: 18789,
      platform: "darwin",
    });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    ...
    expect(env.NATESCLAW_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("sets the Natesclaw-owned launchd marker for macOS node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/Users/user" },
      platform: "darwin",
    });

    expect(env.NATESCLAW_LAUNCHD_LABEL).toBe("ai.natesclaw.node");
  });

  it("passes through NATESCLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", NATESCLAW_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.NATESCLAW_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through NATESCLAW_GATEWAY_PASSWORD for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", NATESCLAW_GATEWAY_PASSWORD: " node-password " },
    });
    expect(env.NATESCLAW_GATEWAY_PASSWORD).toBe("node-password");
  });

  it("passes through NATESCLAW_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", NATESCLAW_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.NATESCLAW_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits NATESCLAW_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
  ...
  it("marks Windows node tasks for hidden launcher startup", () => {
    ...
    expect(env.NATESCLAW_WINDOWS_TASK_NAME).toBe("Natesclaw Node");
    expect(env.NATESCLAW_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    expect(env.NATESCLAW_TASK_SCRIPT_NAME).toBe("node.cmd");
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".natesclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", NATESCLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".natesclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", NATESCLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".natesclaw"));
  });

  it("uses NATESCLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", NATESCLAW_STATE_DIR: "/var/lib/natesclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/natesclaw"));
  });

  it("expands ~ in NATESCLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", NATESCLAW_STATE_DIR: "~/natesclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/natesclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { NATESCLAW_STATE_DIR: "C:\\State\\natesclaw" };

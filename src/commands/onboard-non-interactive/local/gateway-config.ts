...
export function applyNonInteractiveGatewayConfig(params: {
  ...
  const envGatewayToken = normalizeGatewayTokenInput(process.env.NATESCLAW_GATEWAY_TOKEN);
  ...
  // plaintext > ambient NATESCLAW_GATEWAY_TOKEN > randomToken(). Ambient env
  ...
  if (authMode === "token") {
    if (gatewayTokenRefEnv) {
      ...
      if (!resolvedFromEnv) {
        runtime.error(
          `Environment variable "${gatewayTokenRefEnv}" is missing or empty. Export it first, then rerun ${formatCliCommand("natesclaw onboard --non-interactive")}.`,
        );
        runtime.exit(1);
        return null;
      }
      ...
      // branch, an ambient NATESCLAW_GATEWAY_TOKEN (or randomToken() fallback)

...
export function registerWebhooksCli(program: Command) {
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Natesclaw hooks")
    ...
    .option("--hook-url <url>", "Natesclaw hook URL")
    .option("--hook-token <token>", "Natesclaw hook token")
...
function parseGmailSetupOptions(raw: Record<string, unknown>): GmailSetupOptions {
  ...
  if (!account) {
    throw new Error(
      `--account is required. Example: ${formatCliCommand("natesclaw webhooks gmail setup --account default")}.`,
    );
  }

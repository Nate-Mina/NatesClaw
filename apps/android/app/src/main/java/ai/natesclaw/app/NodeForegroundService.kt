...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    ...
    val initial =
      buildNotification(
        title = nativeString("Natesclaw Node"),
    ...
    runtimeRestoreJob =
      scope.launch(Dispatchers.Default) {
        try {
          ...
          Log.e("NatesclawNodeService", "Failed to restore node runtime", err)
          withContext(Dispatchers.Main) {
            runtimeRestoreJob = null
          ...
          val title =
            when {
              state.connected && state.mode == VoiceCaptureMode.TalkMode ->
                nativeString("Natesclaw Node · Talk")
              state.connected -> nativeString("Natesclaw Node · Connected")
              else -> nativeString("Natesclaw Node")
            }
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = nativeString("Natesclaw node connection status")
        setShowBadge(false)
      }
  ...
  companion object {
    ...
    private const val ACTION_STOP = "ai.natesclaw.app.action.STOP"
    private const val ACTION_RESUME = "ai.natesclaw.app.action.RESUME"
    private const val ACTION_SET_VOICE_CAPTURE_MODE = "ai.natesclaw.app.action.SET_VOICE_CAPTURE_MODE"
    private const val EXTRA_VOICE_CAPTURE_MODE = "ai.natesclaw.app.extra.VOICE_CAPTURE_MODE"

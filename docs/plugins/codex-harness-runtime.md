...
read_when:
  ...
  - You are changing plugin behavior across Natesclaw and Codex harness turns
---
...
continuation, and native compaction. Natesclaw owns channel routing, session
files, visible message delivery, Natesclaw dynamic tools, approvals, media
...
policy. Natesclaw caches a runtime-and-workspace-scoped `plugin/installed`
...
This check finishes before Natesclaw injects history, starts a turn, or commits a
thread binding. Failed persistent provisional threads are deleted; ephemeral
threads are unsubscribed. Natesclaw retires the app-server connection when safe
...
workspace plugin. Natesclaw uses its installed snapshot and reads only that
...
Natesclaw compatibility route keeps the normal Natesclaw system prompt even when
it uses Codex-flavored OpenAI auth or transport.

Natesclaw starts and resumes native Codex threads with Codex's built-in
personality disabled (`personality: "none"`) so workspace personality files
and Natesclaw agent identity stay authoritative. Native Codex keeps Codex-owned
base/model instructions and project-doc loading otherwise. Lightweight
Natesclaw runs (for example cron) still suppress project-doc loading.

Natesclaw developer instructions cover Natesclaw runtime concerns: source-channel
delivery, Natesclaw dynamic tools, ACP delegation, adapter context, and the
...
When `natesclaw_direct.sessions_yield` is available, those instructions also
...
Most Natesclaw dynamic tools use the searchable `natesclaw` namespace. Tools
marked `catalogMode: "direct-only"` use `natesclaw_direct`, which Codex keeps
directly model-visible as `DirectModelOnly` instead of exposing it to nested
Code Mode execution.

## Thread bindings and model changes

When an Natesclaw session is attached to an existing Codex thread, the next
...
Supervised bindings are the exception. The Natesclaw model picker stays locked,
...
model-difference warning; the outer Natesclaw model and fallback chain never
...
not resume a thread, subscribe Natesclaw to its live events, or answer its
approvals.
...
On the same supervision connection, Natesclaw starts the canonical
...
resumed. The canonical thread has the full Natesclaw harness tool surface;
...
Natesclaw allows archive for a local `idle` or `notLoaded` row only after explicit
...
prove that another process is not using the thread. Natesclaw blocks a known
...
contract: the agent replies normally and Natesclaw posts the final text to the
...
Codex heartbeat turns get `heartbeat_respond` in the searchable Natesclaw tool
...
| Layer                                 | Owner                    | Purpose                                                             |
| ------------------------------------- | ------------------------ | ------------------------------------------------------------------- |
| Natesclaw plugin hooks                 | Natesclaw                 | Product/plugin compatibility across Natesclaw and Codex harnesses.   |
| Codex app-server extension middleware | Natesclaw bundled plugins | Per-turn adapter behavior around Natesclaw dynamic tools.            |
| Codex native hooks                    | Codex                    | Low-level Codex lifecycle and native tool policy from Codex config. |

Natesclaw does not use project or global Codex `hooks.json` files to route
plugin behavior. For the native tool and permission bridge, Natesclaw injects
...
so Codex's app-server reviewer and Natesclaw's approval bridge handle real
...
controls; they are not exposed as Natesclaw plugin hooks in the v1 contract.

For Natesclaw dynamic tools, Natesclaw executes the tool after Codex asks for
...
canonical tool record; Natesclaw can mirror selected events but cannot rewrite
...
matching app-server approval. If an Natesclaw `before_tool_call` hook returns
`requireApproval` while the native payload sets `natesclaw_approval_mode:
...
request for the same tool use, Natesclaw opens the plugin approval prompt and
maps the decision back to Codex. Codex `PermissionRequest` events are a
separate approval path and can still route through Natesclaw approvals when
...
notifications and Natesclaw adapter state, not native Codex hook commands.
...
debugging. They do not invoke Natesclaw plugin hooks.
...
Natesclaw sandbox, sandboxed processes stream ordered stdout, stderr, or PTY
output notifications. Natesclaw retains only a bounded recent-output buffer for
...
| Natesclaw channel routing and delivery         | Supported                                                                        | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                                                                                                                                                                                                                                                                                                    |
| Natesclaw dynamic tools                        | Supported                                                                        | Codex asks Natesclaw to execute these tools, so Natesclaw stays in the execution path.                                                                                                                                                                                                                                                                                                                                                                                                |
| Prompt and context plugins                    | Supported                                                                        | Natesclaw projects Natesclaw-specific prompt/context into the Codex turn while leaving Codex-owned base, model, and configured project-doc prompts in the native Codex lane. Natesclaw disables Codex's built-in personality for native threads so agent workspace personality files remain authoritative. Native Codex developer instructions accept only command guidance explicitly scoped to `codex_app_server`; legacy global command hints remain for non-Codex prompt surfaces. |
...
| Dynamic tool hooks                            | Supported                                                                        | `before_tool_call`, `after_tool_call`, and tool-result middleware run around Natesclaw-owned dynamic tools.                                                                                                                                                                                                                                                                                                                                                                          |
...
| Native permission policy                      | Supported through Codex app-server approvals and compatibility native hook relay | Codex app-server approval requests route through Natesclaw after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review.                                                                                                                                                                                                                                                                          |
| App-server trajectory capture                 | Supported                                                                        | Natesclaw records the request it sent to app-server and the app-server notifications it receives.                                                                                                                                                                                                                                                                                                                                                                                    |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                     | Future path                                                                               |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but Natesclaw does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. Natesclaw owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms Natesclaw-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | Natesclaw can request native compaction, but does not receive a stable kept/dropped list, token delta, completion summary, or summary payload.   | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | Natesclaw does not let plugins or context engines veto, rewrite, or replace native Codex compaction.                                             | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | Natesclaw can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Native permissions and MCP elicitations

For `PermissionRequest`, Natesclaw only returns explicit allow or deny
...
request, Natesclaw remembers that exact provider/session/tool input/cwd
...
Codex MCP tool approval elicitations route through Natesclaw's plugin approval
...
resolve the same gateway record before Natesclaw returns the app-server answer.
...
default `messages.queue.mode: "steer"`, Natesclaw batches steer-mode chat
...
that case, Natesclaw waits for the active run to finish before starting the
...
harness, Natesclaw also calls Codex app-server `feedback/upload` for relevant
...
Natesclaw session ids, Codex thread ids, and local `codex resume <thread-id>`
commands for the threads that were sent.

If you deny or ignore the approval, Natesclaw does not print those Codex ids
...
belongs to Codex app-server. Natesclaw does not run preflight compaction for
Codex turns, replace Codex compaction with context-engine compaction, or fall
back to Natesclaw or public OpenAI summarization when native compaction cannot
be started. Natesclaw keeps a transcript mirror for channel history, search,
...
Natesclaw keeps the request and shared-client lease open until Codex emits the
...
timeout, Natesclaw requests a native turn interrupt. The lease and per-thread
...
period, Natesclaw retires the connection before releasing the fence. Remote
...
compaction is Codex's job; Natesclaw only starts native compaction for manually
requested triggers.

When a context engine requests Codex thread-bootstrap projection, Natesclaw
...
Codex reasoning or plan records when the app-server emits them. Natesclaw
...
not rewrite Codex-native tool result records. It only applies when Natesclaw
writes an Natesclaw-owned session transcript tool result.

## Media and delivery

Natesclaw continues to own media delivery and media provider selection. Image,
...
through the normal Natesclaw delivery path; media generation does not require
the legacy runtime. When Codex emits a native image-generation item with a
`savedPath`, Natesclaw forwards that exact file through the normal reply-media

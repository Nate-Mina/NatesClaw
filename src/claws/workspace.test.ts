...
async function makePlan(params?: {
  ...
  const root = tempDirs.make("natesclaw-claw-workspace-");
...
function readWorkspaceFileRows(agentId: string, root: string) {
  const rows = openNatesclawStateDatabase({ env: stateEnv(root) })
    .db.prepare(
  ...
  return rows.map((row) => ({
    schemaVersion: "natesclaw.clawWorkspaceFileRecord.v1" as const,
...
function readInstallStatus(agentId: string, root: string): string | undefined {
  const row = openNatesclawStateDatabase({ env: stateEnv(root) })
...
describe("createClawWorkspaceFiles", () => {
  it("materializes the CLAW.md body as managed SOUL.md content", async () => {
    const root = tempDirs.make("natesclaw-claw-body-workspace-");
    const workspace = join(root, "workspace-agent");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = tempDirs.make("natesclaw-claw-linked-package-");
  ...
  it("creates canonical bootstrap and supporting files and records their hashes", async () => {
    ...
    expect(records).toEqual([
      expect.objectContaining({
        schemaVersion: "natesclaw.clawWorkspaceFileRecord.v1",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const outside = tempDirs.make("natesclaw-claw-outside-");
  ...
  it("does not adopt an independently created file after a failed write record", async () => {
    ...
    openNatesclawStateDatabase({ env: stateEnv(root) })
      .db.prepare(
      ...
      .run(
        "natesclaw.clawWorkspaceFileRecord.v1",
        plan.agent.finalId,

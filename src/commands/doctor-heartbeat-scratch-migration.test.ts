...
async function createFixture() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-heartbeat-migration-"));
  tempDirs.push(root);
...
async function loadMonitor(cfg?: NatesclawConfig) {
...
describe("HEARTBEAT.md cron scratch migration", () => {
  ...
  it("respects a configured cron store partition", async () => {
    ...
    const cfg = { ...fixture.cfg, cron: { store: customStore } } as unknown as NatesclawConfig;

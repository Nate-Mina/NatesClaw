...
describe("publish model catalog", () => {
  it("assembles and validates fixture manifests at the 200-model floor", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-publish-catalog-"));
    tempDirs.push(root);
  ...
  it("dry-runs the repository manifests without writing output", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-publish-catalog-smoke-"));

...
import { withTrustedEnvProxyGuardedFetchMode } from "natesclaw/plugin-sdk/fetch-runtime";
import { isProviderApiKeyConfigured } from "natesclaw/plugin-sdk/provider-auth";
import {
...
const DEEPINFRA_MODELS_URL = `${DEEPINFRA_BASE_URL}/models?sort_by=natesclaw&filter=with_meta`;
...
// Static fallback. Chat rows live in natesclaw.plugin.json (manifest-validated);
...
// `/v1/openai/models?sort_by=natesclaw&filter=with_meta` projection is the
...
// `isProviderApiKeyConfigured` helper (covers SecretRef / `NATESCLAW_LIVE_*`
// indirection too).

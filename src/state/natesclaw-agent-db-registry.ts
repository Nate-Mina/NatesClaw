...
import { invalidateRegisteredAgentDatabasesMemo } from "./natesclaw-agent-db-registry-listing.js";
import type { DB as NatesclawStateKyselyDatabase } from "./natesclaw-state-db.generated.js";
import { runNatesclawStateWriteTransaction } from "./natesclaw-state-db.js";

export { listNatesclawRegisteredAgentDatabases } from "./natesclaw-agent-db-registry-listing.js";
...
export function createNatesclawAgentDatabasePathMatcher(): (left: string, right: string) => boolean {
...
export function isSameNatesclawAgentDatabasePath(left: string, right: string): boolean {
...
export function registerNatesclawAgentDatabase(params: {
  agentId: string;
  path: string;
  env?: NodeJS.ProcessEnv;
  schemaVersion?: number;
}): void {
  if (!isPersistentNatesclawAgentDatabasePath(params.path, params.env)) {
    return;
  }
  ...
  runNatesclawStateWriteTransaction(
    (database) => {
      ...
      const db = getNodeSqliteKysely<NatesclawAgentRegistryDatabase>(database.db);
      executeSqliteQuerySync(
        ...
        db
          ...
          .values({
            ...
            path: params.path,
            schema_version: params.schemaVersion ?? NATESCLAW_AGENT_SCHEMA_VERSION,
...
export function isPersistentNatesclawAgentDatabasePath(
  pathname: string,
...
export function unregisterNatesclawAgentDatabase(params: {
  agentId: string;
  path: string;
  env?: NodeJS.ProcessEnv;
}): void {

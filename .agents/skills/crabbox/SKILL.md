---
name: crabbox
description: "Crabbox/Testbox remote proof for Natesclaw: trusted-source routing, untrusted isolation, Linux/macOS/Windows/WSL2, live E2E, desktop, diagnostics, cleanup."
---

# Crabbox

Remote Natesclaw proof. Heavy tests. Builds. Typecheck/lint fan-out. Docker.
...
Need direct AWS semantics? Pass `--provider aws`. Need normal trusted Natesclaw
...
node scripts/crabbox-wrapper.mjs run \
  ...
  NATESCLAW_TEST_PROJECTS_PARALLEL=6 \
  NATESCLAW_VITEST_MAX_WORKERS=1 \
  NATESCLAW_TESTBOX=1 NATESCLAW_TESTBOX_REMOTE_RUN=1 \
  pnpm check:changed
```
...
```sh
TMPDIR=/tmp NATESCLAW_TESTBOX=1 "$AUTOREVIEW" \
...
```sh
cd <clean-trusted-natesclaw-main>
...
node scripts/crabbox-wrapper.mjs run \
  ...
  --no-hydrate --fresh-pr natesclaw/natesclaw#123 \
  --timing-json --shell -- \
...
Isolate mutable state: `NATESCLAW_STATE_DIR=$(mktemp -d)`. Test-only local plugin
artifacts may use `NATESCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1`; never call them
...
Natesclaw setup belongs hydration workflow/repo scripts.

...
describe("renderGatewayServiceStartHints", () => {
  ...
  it("resolves daemon container context from either env key", () => {
    expect(
      resolveDaemonContainerContext({
        NATESCLAW_CONTAINER: "natesclaw-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("natesclaw-demo-container");
    expect(
      resolveDaemonContainerContext({
        NATESCLAW_CONTAINER_HINT: "natesclaw-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("natesclaw-demo-container");
  });

  it("prepends a single container restart hint when NATESCLAW_CONTAINER is set", () => {
    expect(
      renderGatewayServiceStartHints({
        NATESCLAW_CONTAINER: "natesclaw-demo-container",
      } as NodeJS.ProcessEnv),
  ...
  it("prepends a single container restart hint when NATESCLAW_CONTAINER_HINT is set", () => {
    expect(
      renderGatewayServiceStartHints({
        NATESCLAW_CONTAINER_HINT: "natesclaw-demo-container",
      } as NodeJS.ProcessEnv),
...
describe("filterContainerGenericHints", () => {
  it("drops the generic container foreground hint when NATESCLAW_CONTAINER is set", () => {
    expect(
      filterContainerGenericHints(
        [
        ...
        { NATESCLAW_CONTAINER: "natesclaw-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });

  it("drops the generic container foreground hint when NATESCLAW_CONTAINER_HINT is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
        ...
        { NATESCLAW_CONTAINER_HINT: "natesclaw-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });
});

...
At startup, Natesclaw does roughly this:
...
The manifest is the control-plane source of truth. Natesclaw uses it to:
...
derived set is empty, Natesclaw keeps the scope empty instead of widening to
...
Natesclaw does not cache plugin discovery results or direct manifest registry
...
Natesclaw still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Natesclaw calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that Natesclaw no longer calls, such as
...
| _(built-in model lookup)_         | Natesclaw tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
| `refreshOAuth`                    | OAuth refresh override for custom refresh endpoints or refresh-failure policy                                  | Provider does not fit the shared Natesclaw refreshers                                                                                          |
...
that still runs on Natesclaw's normal inference loop.

`resolveUsageAuth` decides whether Natesclaw should call `fetchUsageSnapshot` or
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of Natesclaw's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Natesclaw adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  schemas while Natesclaw owns the provider/runtime boundary.
...
- Natesclaw only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Natesclaw plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope. The default surface (`gatewayRuntimeScopeSurface: "write-default"`) is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) and any non-trusted-proxy auth method get a single `operator.write` scope, even if the caller sends `x-natesclaw-scopes`
  - `trusted-proxy` callers without an explicit `x-natesclaw-scopes` header also keep the legacy `operator.write`-only surface
  - `trusted-proxy` callers that do send `x-natesclaw-scopes` get the declared scopes instead
  - a route can opt into `gatewayRuntimeScopeSurface: "trusted-operator"` to always honor `x-natesclaw-scopes` for identity-bearing auth modes (falling back to the full CLI default scope set when the header is absent)
...
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, opt into `trusted-operator` scope surface, require an identity-bearing auth mode, and document the explicit `x-natesclaw-scopes` header contract.
...
Use narrow SDK subpaths instead of the monolithic `natesclaw/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                            | Purpose                                      |
| ---------------------------------- | -------------------------------------------- |
| `natesclaw/plugin-sdk/plugin-entry` | Plugin registration primitives               |
| `natesclaw/plugin-sdk/channel-core` | Channel entry/build helpers                  |
| `natesclaw/plugin-sdk/core`         | Generic shared helpers and umbrella contract |
...
<Info>
`natesclaw/plugin-sdk/channel-lifecycle`, small channel helper facades,
`natesclaw/plugin-sdk/config-runtime`, and `natesclaw/plugin-sdk/infra-runtime`
...
External plugins should only import `natesclaw/plugin-sdk/*` subpaths. Never
...
`natesclaw/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Natesclaw writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Natesclaw's
...
- if both `catalog` and `discovery` are registered, Natesclaw uses `catalog`
  and emits a warning
...
- Read-only command paths such as `natesclaw status`, `natesclaw status --all`,
  `natesclaw channels status`, `natesclaw channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `natesclaw.extensions`:
...
Security guardrail: every `natesclaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `natesclaw plugins install` installs plugin dependencies with a
...
Optional: `natesclaw.setupEntry` can point at a lightweight setup-only module.
When Natesclaw needs setup surfaces for a disabled channel plugin, or
...
Channel plugins can advertise setup/discovery metadata via `natesclaw.channel` and
install hints via `natesclaw.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@natesclaw/nextcloud-talk",
  "natesclaw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@natesclaw/nextcloud-talk",
...
Useful `natesclaw.channel` fields beyond the minimal example:
...
Natesclaw can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.natesclaw/mpm/plugins.json`
- `~/.natesclaw/mpm/catalog.json`
- `~/.natesclaw/plugins/catalog.json`

Or point `NATESCLAW_PLUGIN_CATALOG_PATHS` (or `NATESCLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "natesclaw": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `natesclaw.install` block. The
...
2. **Add typed plugin registration/runtime surfaces.** Extend
   `NatesclawPluginApi` and/or `api.runtime` with the smallest useful typed
   capability surface.
...
This is how Natesclaw stays opinionated without becoming hardcoded to one

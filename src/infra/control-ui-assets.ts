...
import { resolveNatesclawPackageRoot, resolveNatesclawPackageRootSync } from "./natesclaw-root.js";
...
function resolveControlUiRepoRoot(opts: {
  ...
  const roots = opts.root
    ...
    : [
        ...
        resolveNatesclawPackageRootSync({ cwd }),
      ];
  return (
    roots.find(
      (root): root is string =>
...
async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveNatesclawPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "natesclaw" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  if (argv1Dir) {
    // natesclaw.mjs or dist/<bundle>.js
  ...
  if (argv1RealpathDir && argv1RealpathDir !== argv1Dir) {
    // Symlinked wrappers (e.g. ~/.bun/bin/natesclaw -> .../dist/index.js)
...
export async function ensureControlUiAssetsBuilt(
  ...
  if (!repoRoot) {
    ...
    return controlUiAssetsFailure(
      `${hint}. Reinstall Natesclaw to restore bundled Control UI assets.`,
    );
  }

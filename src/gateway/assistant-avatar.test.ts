...
function createWorkspace(): { workspace: string; cfg: NatesclawConfig } {
  const root = tempRoots.make("natesclaw-gateway-avatar-");
...
function projectAvatar(cfg: NatesclawConfig): GatewayAssistantAvatarProjection {
...
describe("resolveGatewayAssistantAvatar", () => {
  ...
  it("preserves same-origin avatar routes and applies the configured base path", () => {
    const { cfg } = createWorkspace();
    cfg.gateway = { controlUi: { basePath: "/natesclaw" } };
    ...
    ).toEqual({ avatar: "/natesclaw/avatar/main", resolution: null });
    expect(
      resolveGatewayAssistantAvatar({
        cfg,
        identity: { agentId: "main", avatar: "/natesclaw/avatar/main" },
      }),
    ).toEqual({ avatar: "/natesclaw/avatar/main", resolution: null });
  });
});

---
summary: "CLI reference for `natesclaw onboard` (interactive onboarding)"
read_when:
  - You want to establish inference, then finish setup with Natesclaw
title: "Onboard"
---
...
Natesclaw to configure the rest. `natesclaw setup` reaches this flow on fresh
systems or whenever an onboarding option is present; configured systems use
bare `natesclaw setup` for system-agent chat. `natesclaw setup --baseline` only
...
<CardGroup cols={2}>
  ...
  <Card title="Onboarding overview" href="/start/onboarding-overview" icon="map">
    How Natesclaw onboarding fits together.
  </Card>
...
```bash
natesclaw onboard
natesclaw onboard --tui
natesclaw onboard --classic
natesclaw onboard --modern
natesclaw onboard --flow quickstart
natesclaw onboard --flow manual
natesclaw onboard --flow import
natesclaw onboard --import-from hermes --import-source ~/.hermes
natesclaw onboard --skip-bootstrap
natesclaw onboard recommendations --json
natesclaw onboard recommendations acknowledge
natesclaw onboard recommendations acknowledge --retry "<failed-id>"
natesclaw onboard recommendations refresh
natesclaw onboard --mode remote --remote-url wss://gateway-host:18789
```

`natesclaw onboard recommendations` reads pending app-recommendation matches
...
`natesclaw onboard recommendations refresh` clears the stored offer so the next
...
`natesclaw onboard recommendations acknowledge` marks the stored offer answered.
...
`natesclaw skills verify "@owner/slug"` succeeds for the same
publisher-qualified recommendation ID and its JSON output reports
`natesclaw.resolution.source: "installed"`. Registry verification alone is not
...
- `--modern` is a compatibility alias for the Natesclaw conversational setup
  assistant. It uses the same live-inference gate as `natesclaw setup` and
...
Plain `natesclaw onboard` starts the guided flow. It shows the security notice,
...
API-key or token methods use the same live completion path. Natesclaw persists
...
credential. Choose **Skip for now** to exit without starting Natesclaw and
rerun `natesclaw onboard` when you are ready. Workspace and Gateway setup remain
unchanged until Natesclaw starts.

In guided mode, `--workspace <dir>` supplies Natesclaw's proposed workspace
and the isolated inference context. It is not persisted until you approve the
Natesclaw setup proposal. Classic and noninteractive onboarding persist their
...
the same memory-only scope. (A full [`natesclaw migrate`](/cli/migrate) run is
...
the same plan the conversational `natesclaw setup` chat would apply on "yes" —
...
If applying setup fails, onboarding falls back to the conversational Natesclaw
chat to finish interactively. Channels, agents,
plugins, and other optional features remain Natesclaw chat territory: run
`natesclaw` and use `open channel wizard for <channel>` to hand channel
credential collection to a masked terminal wizard. To change the model
provider or its authentication, exit Natesclaw and run `natesclaw onboard`;
Natesclaw does not open the guided or classic provider flows.

On a configured install, running `natesclaw onboard` again verifies the current
...
Use `natesclaw onboard --classic` for provider-specific auth, channels, skills,
remote Gateway setup, imports, or full Gateway controls. For conversational
non-inference setup and repair, run `natesclaw setup`; `natesclaw onboard
...
Natesclaw will not start until its own live inference check passes.

In an interactive terminal, bare `natesclaw` (no subcommand) routes by config
state:
...
- If the config file exists but fails validation, it starts the classic
  onboarding path with `natesclaw doctor` guidance. Natesclaw needs working
...
- If the config file is valid, it opens the normal agent TUI. A reachable
  ...
  onboarding or Natesclaw. On a configured install, reach Natesclaw with
  `/natesclaw` inside the TUI or `natesclaw setup`.

Plaintext `ws://` is accepted for loopback, private IP literals, `.local`, and Tailnet `*.ts.net` gateway URLs. For other trusted private-DNS names, set `NATESCLAW_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.

## Reset

```bash
natesclaw onboard --reset
natesclaw onboard --reset --reset-scope full
```
...
```bash
NATESCLAW_LOCALE=zh-CN natesclaw onboard
NATESCLAW_LOCALE=en natesclaw onboard # Explicit English override
```
...
- `--allow-unconfigured` is a separate `natesclaw gateway run` escape hatch; it does not let onboarding skip `gateway.mode`.

```bash
export OPENAI_API_KEY="your-provider-key"
export NATESCLAW_GATEWAY_TOKEN="your-token"
natesclaw onboard --non-interactive --accept-risk --skip-health \
  --mode local \
  --auth-choice openai-api-key \
  --secret-input-mode ref \
...
- `--install-daemon` starts the managed gateway install path first. Without it, a local gateway must already be running (for example `natesclaw gateway run`).
...
With `--modern`, JSON is a one-shot Natesclaw overview and exits after that
...
- Fastest first chat: `natesclaw dashboard` (Control UI, no channel setup).
...
Use `natesclaw configure` later for targeted non-inference changes and `natesclaw
channels add` for channel-only setup. For model provider or auth route changes,
run `natesclaw onboard` instead.

```bash
natesclaw channels add
natesclaw configure
natesclaw agents add <name>
```

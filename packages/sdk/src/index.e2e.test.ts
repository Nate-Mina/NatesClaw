...
import { GatewayClientTransport, Natesclaw } from "./index.js";
...
describe("Natesclaw SDK websocket e2e", () => {
  afterEach(async () => {
    await Promise.all(
  ...
  it("runs an agent and streams normalized events over a Gateway websocket", async () => {
    ...
    const oc = new Natesclaw({ transport });
    try {
...
describe("Natesclaw SDK real Gateway e2e", () => {
...
const liveGatewayUrl = process.env.NATESCLAW_SDK_LIVE_GATEWAY_URL;
const liveGatewayToken = process.env.NATESCLAW_SDK_LIVE_GATEWAY_TOKEN;
...
liveGatewayDescribe("Natesclaw SDK live Gateway e2e", () => {
  it("connects to a configured Gateway, streams a real run, and waits for completion", async () => {
    ...
    try {
      ...
      const agent = await oc.agents.get(process.env.NATESCLAW_SDK_LIVE_AGENT_ID ?? "main");
      const run = await agent.run({
        input: "Reply with exactly: NATESCLAW_SDK_LIVE_OK",
      ...
      expect(events.text).toContain("NATESCLAW_SDK_LIVE_OK");
    } finally {
      await oc.close();
    }
  }, 240_000);

---
name: release-natesclaw-mac
description: "Run or recover Natesclaw macOS release signing, notarization, appcast, and asset promotion."
---

# Natesclaw Mac Release

Use with `$release-natesclaw-maintainer`, `$release-natesclaw-ci`, `$one-password`, and `$release-private` if it exists when stable macOS assets, release-ops mac preflight, notarization, appcast promotion, or mac release recovery is involved.
...
Target release-ops repo environment: `natesclaw/releases`, env `mac-release`.
...
- `natesclaw/natesclaw` is the public product repo. Its GitHub Releases page is
  where macOS assets are ultimately attached.
- `natesclaw/natesclaw` `macos-release.yml` is public handoff validation only.
  It never signs, notarizes, or uploads macOS assets, regardless of
  `preflight_only`.
- `natesclaw/releases` is the restricted release-ops repo. Its macOS workflows
  sign, notarize, validate, and promote assets onto the
  `natesclaw/natesclaw` GitHub release.
...
- Natesclaw uses `scripts/notarize-mac-artifact.sh`.
...
```bash
gh workflow run macos-release.yml --repo natesclaw/natesclaw \
...
```bash
gh workflow run natesclaw-macos-publish.yml --repo natesclaw/releases --ref main \
  -f tag=vYYYY.M.PATCH \
...
```bash
gh workflow run natesclaw-macos-validate.yml --repo natesclaw/releases --ref main \
  -f tag=vYYYY.M.PATCH \
  -f source_ref=release/YYYY.M.PATCH
```
...
- Release-ops `natesclaw/releases` publish/validate workflows run from their own
  trusted `main` workflow ref. Real publish has a guard that rejects any other
  workflow ref. That displayed `main` ref is expected; the public Natesclaw
  source is selected by `tag` and optional `source_ref`.

## Verify

- `gh release view vYYYY.M.PATCH --repo natesclaw/natesclaw` shows zip, dmg, dSYM zip, not draft, not prerelease.
- Public `main` `appcast.xml` points at `Natesclaw-YYYY.M.PATCH.zip`.

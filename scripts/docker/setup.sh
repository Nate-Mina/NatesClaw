...
EXTRA_MOUNTS="${NATESCLAW_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${NATESCLAW_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${NATESCLAW_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${NATESCLAW_DOCKER_SOCKET:-}"
TIMEZONE="${NATESCLAW_TZ:-}"
RAW_SKIP_ONBOARDING="${NATESCLAW_SKIP_ONBOARDING:-}"
SKIP_ONBOARDING=""
DOCKER_PULL_TIMEOUT="${NATESCLAW_DOCKER_SETUP_PULL_TIMEOUT:-600s}"
OFFLINE_MODE=""
DEFAULT_SANDBOX_IMAGE="natesclaw-sandbox:bookworm-slim"
DEFAULT_SANDBOX_BROWSER_IMAGE="natesclaw-sandbox-browser:bookworm-slim"
...
read_config_gateway_token() {
  local config_path="$NATESCLAW_CONFIG_DIR/natesclaw.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == NATESCLAW_GATEWAY_TOKEN=* ]]; then
      token="${line#NATESCLAW_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
sync_gateway_config() {
  ...
  if [[ "${NATESCLAW_GATEWAY_BIND}" != "loopback" ]]; then
    allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$NATESCLAW_GATEWAY_PORT" "$NATESCLAW_GATEWAY_PORT")"
  ...
  batch_json="$(printf '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"%s"}' "$NATESCLAW_GATEWAY_BIND")"
  ...
  echo "Pinned gateway.mode=local and gateway.bind=$NATESCLAW_GATEWAY_BIND for Docker setup."
  if [[ -n "$allowed_origin_json" ]]; then
...
run_prestart_cli() {
  # During setup, avoid the shared-network natesclaw-cli service because it
  ...
  # Host NATESCLAW_* paths are Compose bind-mount sources. Setup-time CLI writes
  ...
  run_prestart_gateway \
    ...
    --entrypoint node natesclaw-gateway \
    dist/index.js "$@"
}
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" natesclaw-cli "$@"
}
...
run_gateway_up() {
  ...
  docker compose "${compose_args[@]}" "${up_args[@]}" natesclaw-gateway
}
...
resolve_offline_sandbox_images() {
  ...
  printf '%s' "$agents_json" | run_prestart_gateway \
    -T --entrypoint node natesclaw-gateway -e '
const fs = require("node:fs");
...
const matchesBrowser = (rawPattern) => {
  ...
  if (pattern === "group:natesclaw" || pattern === "group:ui") {
    return true;
  }
  if (!pattern) {
    return false;
...
validate_offline_sandbox_prerequisites() {
      ...
      browser)
        if ! browser_contract="$(
          docker --host "unix://$DOCKER_SOCKET_PATH" image inspect \
            -f '{{ index .Config.Labels "org.natesclaw.sandbox-browser.contract" }}' \
  ...
  if ! run_compose_one_off --rm --entrypoint docker natesclaw-gateway --version >/dev/null 2>&1; then
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "NATESCLAW_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "NATESCLAW_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
NATESCLAW_CONFIG_DIR="${NATESCLAW_CONFIG_DIR:-$HOME/.natesclaw}"
NATESCLAW_WORKSPACE_DIR="${NATESCLAW_WORKSPACE_DIR:-$HOME/.natesclaw/workspace}"
NATESCLAW_AUTH_PROFILE_SECRET_DIR="${NATESCLAW_AUTH_PROFILE_SECRET_DIR:-$HOME/.natesclaw-auth-profile-secrets}"

validate_mount_path_value "NATESCLAW_CONFIG_DIR" "$NATESCLAW_CONFIG_DIR"
validate_mount_path_value "NATESCLAW_WORKSPACE_DIR" "$NATESCLAW_WORKSPACE_DIR"
validate_mount_path_value "NATESCLAW_AUTH_PROFILE_SECRET_DIR" "$NATESCLAW_AUTH_PROFILE_SECRET_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "NATESCLAW_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "NATESCLAW_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "NATESCLAW_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "NATESCLAW_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "NATESCLAW_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$NATESCLAW_CONFIG_DIR"
mkdir -p "$NATESCLAW_WORKSPACE_DIR"
mkdir -p "$NATESCLAW_AUTH_PROFILE_SECRET_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$NATESCLAW_CONFIG_DIR/identity"
mkdir -p "$NATESCLAW_CONFIG_DIR/agents/main/agent"
mkdir -p "$NATESCLAW_CONFIG_DIR/agents/main/sessions"

export NATESCLAW_CONFIG_DIR
export NATESCLAW_WORKSPACE_DIR
export NATESCLAW_AUTH_PROFILE_SECRET_DIR
export NATESCLAW_GATEWAY_PORT="${NATESCLAW_GATEWAY_PORT:-18789}"
export NATESCLAW_BRIDGE_PORT="${NATESCLAW_BRIDGE_PORT:-18790}"
export NATESCLAW_GATEWAY_BIND="${NATESCLAW_GATEWAY_BIND:-lan}"
export NATESCLAW_DISABLE_BONJOUR="${NATESCLAW_DISABLE_BONJOUR:-}"
export NATESCLAW_IMAGE="$IMAGE_NAME"
export NATESCLAW_IMAGE_APT_PACKAGES="${NATESCLAW_IMAGE_APT_PACKAGES-${NATESCLAW_DOCKER_APT_PACKAGES:-}}"
export NATESCLAW_IMAGE_PIP_PACKAGES="${NATESCLAW_IMAGE_PIP_PACKAGES:-}"
export NATESCLAW_EXTENSIONS="${NATESCLAW_EXTENSIONS:-}"
export NATESCLAW_DOCKER_BUILD_NODE_OPTIONS="${NATESCLAW_DOCKER_BUILD_NODE_OPTIONS---max-old-space-size=8192}"
export NATESCLAW_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB="${NATESCLAW_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB:-}"
export NATESCLAW_DOCKER_BUILD_SKIP_DTS="${NATESCLAW_DOCKER_BUILD_SKIP_DTS:-1}"
export NATESCLAW_INSTALL_BROWSER="${NATESCLAW_INSTALL_BROWSER:-}"
export NATESCLAW_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export NATESCLAW_HOME_VOLUME="$HOME_VOLUME_NAME"
export NATESCLAW_ALLOW_INSECURE_PRIVATE_WS="${NATESCLAW_ALLOW_INSECURE_PRIVATE_WS:-}"
export NATESCLAW_SANDBOX="$SANDBOX_ENABLED"
export NATESCLAW_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export NATESCLAW_DOCKER_SETUP=1
export NATESCLAW_TZ="$TIMEZONE"
...
export NATESCLAW_OTEL_PRELOADED="${NATESCLAW_OTEL_PRELOADED:-}"
export NATESCLAW_SKIP_ONBOARDING="$SKIP_ONBOARDING"
...
if [[ -z "${NATESCLAW_GATEWAY_TOKEN:-}" ]]; then
  EXISTING_CONFIG_TOKEN="$(read_config_gateway_token || true)"
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    NATESCLAW_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $NATESCLAW_CONFIG_DIR/natesclaw.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      NATESCLAW_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      NATESCLAW_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      NATESCLAW_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
export NATESCLAW_GATEWAY_TOKEN
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${NATESCLAW_CONFIG_DIR}:/home/node/.natesclaw"
    gateway_workspace_mount="${NATESCLAW_WORKSPACE_DIR}:/home/node/.natesclaw/workspace"
    gateway_auth_profile_secret_mount="${NATESCLAW_AUTH_PROFILE_SECRET_DIR}:/home/node/.config/natesclaw"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${NATESCLAW_INSTALL_DOCKER_CLI:-}" ]]; then
    export NATESCLAW_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  ...
  NATESCLAW_AUTH_PROFILE_SECRET_DIR \
  NATESCLAW_GATEWAY_PORT \
  NATESCLAW_BRIDGE_PORT \
  NATESCLAW_GATEWAY_BIND \
  NATESCLAW_DISABLE_BONJOUR \
  NATESCLAW_GATEWAY_TOKEN \
  NATESCLAW_IMAGE \
  NATESCLAW_EXTRA_MOUNTS \
  NATESCLAW_HOME_VOLUME \
  NATESCLAW_IMAGE_APT_PACKAGES \
  NATESCLAW_IMAGE_PIP_PACKAGES \
  NATESCLAW_EXTENSIONS \
  NATESCLAW_DOCKER_BUILD_NODE_OPTIONS \
  NATESCLAW_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB \
  NATESCLAW_DOCKER_BUILD_SKIP_DTS \
  NATESCLAW_INSTALL_BROWSER \
  NATESCLAW_SANDBOX \
  NATESCLAW_DOCKER_SOCKET \
  DOCKER_GID \
  NATESCLAW_INSTALL_DOCKER_CLI \
  ...
  NATESCLAW_OTEL_PRELOADED \
  NATESCLAW_SKIP_ONBOARDING

if [[ -n "$OFFLINE_MODE" ]]; then
  require_local_docker_image "$IMAGE_NAME"
  echo "==> Using preloaded Docker image: $IMAGE_NAME"
elif [[ "$IMAGE_NAME" == "natesclaw:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  BUILD_GIT_COMMIT="$(natesclaw_resolve_git_commit "$ROOT_DIR")"
  BUILD_TIMESTAMP="$(natesclaw_resolve_build_timestamp)"
  PROVENANCE_BUILD_ARGS=(--build-arg "NATESCLAW_BUILD_TIMESTAMP=${BUILD_TIMESTAMP}")
  if [[ "$BUILD_GIT_COMMIT" =~ ^[0-9a-fA-F]{40}$ ]]; then
    PROVENANCE_BUILD_ARGS+=(--build-arg "GIT_COMMIT=${BUILD_GIT_COMMIT}")
  fi
  run_docker_build \
    "${PROVENANCE_BUILD_ARGS[@]}" \
    --build-arg "NATESCLAW_IMAGE_APT_PACKAGES=${NATESCLAW_IMAGE_APT_PACKAGES}" \
    --build-arg "NATESCLAW_IMAGE_PIP_PACKAGES=${NATESCLAW_IMAGE_PIP_PACKAGES}" \
    --build-arg "NATESCLAW_EXTENSIONS=${NATESCLAW_EXTENSIONS}" \
    --build-arg "NATESCLAW_DOCKER_BUILD_NODE_OPTIONS=${NATESCLAW_DOCKER_BUILD_NODE_OPTIONS}" \
    --build-arg "NATESCLAW_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB=${NATESCLAW_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB}" \
    --build-arg "NATESCLAW_DOCKER_BUILD_SKIP_DTS=${NATESCLAW_DOCKER_BUILD_SKIP_DTS}" \
    --build-arg "NATESCLAW_INSTALL_BROWSER=${NATESCLAW_INSTALL_BROWSER}" \
    --build-arg "NATESCLAW_INSTALL_DOCKER_CLI=${NATESCLAW_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
# After fixing the config dir, only the Natesclaw metadata subdirectory
# (.natesclaw/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh natesclaw-gateway -c \
  'PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin; export PATH; \
   /usr/bin/find -P /home/node/.natesclaw -xdev -execdir /usr/bin/chown -h node:node {} +; \
   /usr/bin/chown -h node:node /home/node/.config; \
   /usr/bin/find -P /home/node/.config/natesclaw -xdev -execdir /usr/bin/chown -h node:node {} +; \
   if [ -d /home/node/.natesclaw/workspace/.natesclaw ] && [ ! -L /home/node/.natesclaw/workspace/.natesclaw ]; then \
     /usr/bin/find -P /home/node/.natesclaw/workspace/.natesclaw -xdev -execdir /usr/bin/chown -h node:node {} +; \
   fi || true'

echo ""
if [[ -n "$SKIP_ONBOARDING" ]]; then
  echo "==> Skipping onboarding (NATESCLAW_SKIP_ONBOARDING is set)"
else
  echo "==> Onboarding (interactive)"
  echo "Docker setup pins Gateway mode to local."
  echo "Gateway runtime bind comes from NATESCLAW_GATEWAY_BIND (default: lan)."
  echo "Current runtime bind: $NATESCLAW_GATEWAY_BIND"
  if is_truthy_value "$NATESCLAW_DISABLE_BONJOUR"; then
    echo "Bonjour/mDNS advertising: force disabled (NATESCLAW_DISABLE_BONJOUR=$NATESCLAW_DISABLE_BONJOUR)."
  elif [[ -z "$NATESCLAW_DISABLE_BONJOUR" ]]; then
    echo "Bonjour/mDNS advertising: auto (disabled inside the Gateway container unless explicitly enabled)."
  else
    echo "Bonjour/mDNS advertising: explicitly enabled (NATESCLAW_DISABLE_BONJOUR=$NATESCLAW_DISABLE_BONJOUR)."
  fi
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm natesclaw-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm natesclaw-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm natesclaw-cli channels add --channel discord --token <token>"
echo "Docs: https://docs.natesclaw.ai/channels"
...
# --- Sandbox setup (opt-in via NATESCLAW_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if [[ -z "$OFFLINE_MODE" && ! -S "$DOCKER_SOCKET_PATH" ]]; then
    echo "WARNING: NATESCLAW_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
  ...
  if [[ -n "$SANDBOX_ENABLED" && -z "$OFFLINE_MODE" ]] &&
    ! run_compose_one_off --rm --entrypoint docker natesclaw-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg NATESCLAW_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (NATESCLAW_IMAGE=natesclaw:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  # Enable sandbox in Natesclaw config.
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    echo "Docs: https://docs.natesclaw.ai/gateway/sandboxing"
...
echo "Config: $NATESCLAW_CONFIG_DIR"
echo "Workspace: $NATESCLAW_WORKSPACE_DIR"
echo "Token: stored in Docker environment/config (not printed)."
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f natesclaw-gateway"
echo "  ${COMPOSE_HINT} exec natesclaw-gateway sh -lc 'node dist/index.js gateway health --token \"\$NATESCLAW_GATEWAY_TOKEN\"'"

// Builds the data model for the standard `natesclaw status` text report.
...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("natesclaw security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("natesclaw status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("natesclaw tasks maintenance --apply")}`,

...
import { createClaimableDedupe } from "natesclaw/plugin-sdk/persistent-dedupe";
...
describe("Matrix QA persisted state probes", () => {
  ...
  it("observes inbound dedupe entries committed through the core claimable dedupe", async () => {
    ...
    const guard = createClaimableDedupe({
      ...
      memoryMaxSize: 100,
      stateMaxEntries: 100,
      env: { ...process.env, NATESCLAW_STATE_DIR: accountRoot },
    });
    ...
    ).resolves.toBe(path.join(accountRoot, "state", "natesclaw.sqlite"));
  });
});

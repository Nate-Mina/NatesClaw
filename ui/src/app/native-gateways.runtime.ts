...
type NativeGatewaysWindow = Window & {
  __NATESCLAW_NATIVE_GATEWAYS__?: unknown;
  webkit?: {
    messageHandlers?: { natesclawGateways?: { postMessage(message: NativeGatewaysMessage): void } };
  };
};

const NATIVE_GATEWAYS_CHANGED_EVENT = "natesclaw:native-gateways-changed";
...
function createNativeGatewaysCapability(): NativeGatewaysCapability | null {
  ...
  const handler = nativeWindow.webkit?.messageHandlers?.natesclawGateways;
  ...
  const postWithId = (type: "select" | "open-window" | "set-primary", id: string) =>
    post({ type, id });
  let snapshot = snapshotFrom(nativeWindow["__NATESCLAW_NATIVE_GATEWAYS__"]);

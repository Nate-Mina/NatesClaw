...
- **macOS fulfiller:** **Accessibility** and Event Posting access granted to Natesclaw (for pointer/keyboard injection), plus **Screen Recording** permission (for `screen.snapshot`).
...
The built-in `computer` tool takes one action per call. Coordinates are non-negative integer pixels in the most recent screenshot; the node maps them to display points. Coordinate actions must echo the screenshot result's `frameId`, and an explicit `screenIndex` must match that frame. Natesclaw also carries a node-issued display identity from the screenshot into the action, so a display reconnect or geometry change fails closed instead of silently retargeting the same index. These checks reject guessed tokens and tokens from another delivered frame or display. A token is not a freshness guarantee: apps can change pixels on the same display after capture, so take a new screenshot whenever the scene may have changed.
...
1. Enable the plugin:
   ...
   ```bash
   natesclaw plugins enable cua-computer
   ```

2. Start `natesclaw node run` from the interactive desktop session. The plugin creates its configured SDK runtime lazily, then creates one Natesclaw-owned trusted session for the node-host command execution. It closes that session and shuts down the runtime when the command host stops or restarts.
...
| Code                                                 | Cause                                                                                                                                                         | Fix                                                                                                                                                                             |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `COMPUTER_DRIVER_UNAVAILABLE`                        | The CUA Driver SDK runtime cannot initialize, the node is not Windows/Linux, or its desktop permissions/session are unavailable.                              | Run `natesclaw node run` inside the interactive desktop session and check the platform desktop permissions. Reinstall Natesclaw if its bundled CUA Driver SDK package is missing. |
...
If the status says **Accessibility grant may be stale**, Natesclaw may already appear enabled under **System Settings → Privacy & Security → Accessibility** even though macOS rejects it. This happens when the Accessibility entry is pinned to an older app build. Select Natesclaw in that list, remove it with **−**, then re-add `/Applications/Natesclaw.app`. Quit and reopen Natesclaw after changing the grant because macOS can cache Accessibility trust for the lifetime of the process.

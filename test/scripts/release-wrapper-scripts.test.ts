...
const UNKNOWN_PACKAGE = "@natesclaw/not-a-real-release-wrapper-test-package";
...
function createOldReleaseTarget() {
  const root = mkdtempSync(join(tmpdir(), "natesclaw-old-release-target-"));
  tempDirs.push(root);
  mkdirSync(join(root, "extensions"), { recursive: true });
  mkdirSync(join(root, "scripts"), { recursive: true });
  writeFileSync(
    join(root, "package.json"),
    `${JSON.stringify({ name: "natesclaw", version: "2026.7.1-beta.3" }, null, 2)}\n`,
  );
  writeFileSync(
    join(root, "scripts", "natesclaw-release-clawhub-plan.ts"),
...
describe("release wrapper scripts", () => {
  ...
  it("loads the Natesclaw ClawHub plan CLI and validates required arguments before planning", () => {
    const result = runTsxScript("scripts/natesclaw-release-clawhub-plan.ts", [
  ...
  it("runs trusted harness planning and verification against an old release target cwd", () => {
    ...
    const plan = runTsxScript(
      join(repositoryRoot, "scripts/natesclaw-release-clawhub-plan.ts"),
      [
        "--bootstrap-workflow-ref",

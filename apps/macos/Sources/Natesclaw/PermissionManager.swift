...
extension Notification.Name {
    static let natesclawPermissionsChanged = Notification.Name("natesclaw.permissions.changed")
}
...
enum PermissionManager {
    ...
    static func ensure(_ caps: [Capability], interactive: Bool) async -> [Capability: Bool] {
        ...
        if interactive {
            await MainActor.run {
                NotificationCenter.default.post(name: .natesclawPermissionsChanged, object: nil)
            }
        }
        return results
    }

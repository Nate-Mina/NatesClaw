---
summary: "Install Natesclaw - installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Natesclaw, and launches onboarding.
...
Use this when you want Natesclaw and Node kept under a local prefix such as
...
`natesclaw update --channel dev` and `natesclaw update --channel stable`. See
...
<Tabs>
  <Tab title="npm">
    ...
    skips Natesclaw's `preinstall` and `postinstall` steps — npm reports them
    as `blocked because they are not covered by allowScripts`. Allow them
    explicitly:

    ```bash
    npm install -g natesclaw@latest --allow-scripts natesclaw
    ```
    ...
    aware that the `npm approve-scripts natesclaw` command it suggests does not
    work for a global install — it fails with `ENOMATCH  No installed packages
    match: natesclaw`. npm 11.12 and earlier have no such policy.
    </Note>

    <Note>
    The hosted installer clears npm freshness filters such as `min-release-age`
    for the Natesclaw package install. If you install manually with npm, your own
  ...
  <Tab title="pnpm">
    ```bash
    pnpm add -g --allow-build=natesclaw natesclaw@latest
    natesclaw onboard --install-daemon
    ```

    <Note>
    pnpm requires explicit approval for packages with build scripts. `approve-builds -g` is not supported for global installs, so pass `--allow-build=natesclaw` on the `pnpm add -g` command instead.
    </Note>
  ...
  <Tab title="bun">
    ...
    <Note>
    Bun can install the global package, but the resulting `natesclaw` executable requires a supported Node runtime because Natesclaw state uses `node:sqlite`.
    </Note>
...
Or skip the link and use `pnpm natesclaw ...` from inside the repo. See [Setup](/start/setup) for full development workflows.
...
```bash
natesclaw --version      # confirm the CLI is available
natesclaw doctor         # check for config issues
natesclaw gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `natesclaw onboard --install-daemon` or `natesclaw gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy Natesclaw on a cloud server or VPS. See [Linux server](/vps) for the full
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove Natesclaw completely.
  </Card>
</CardGroup>

## Troubleshooting: `natesclaw` not found

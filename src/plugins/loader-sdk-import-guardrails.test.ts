...
const ALLOWED_PLUGIN_SDK_FIXTURE_IMPORTS = new Set([
  ...
  'src/plugins/loader.git-path-regression.test.ts:`import { resolveOutboundSendDep } from "natesclaw/plugin-sdk/channel-outbound";',
  'src/plugins/loader.git-path-regression.test.ts:          "natesclaw/plugin-sdk/channel-outbound": ${JSON.stringify(copiedChannelRuntimeShim)},',
  // Intentional packaged bundled-plugin SDK alias regression tests.
  'src/plugins/loader.test.ts:`import { normalizeLowercaseStringOrEmpty } from "natesclaw/plugin-sdk/string-coerce-runtime";`,',
]);
...
function findLoaderFixtureSdkImports(): string[] {
  ...
  for (const file of LOADER_FIXTURE_TEST_FILES) {
    ...
    for (const line of source.split("\n")) {
      if (
        line.includes('require("natesclaw/plugin-sdk') ||
        (line.includes("import ") && line.includes('"natesclaw/plugin-sdk'))
      ) {

...
export default definePluginEntry({
  ...
  register(api) {
    const resolveCurrentConfig = () =>
      api.runtime.config?.current ? (api.runtime.config.current() as NatesclawConfig) : undefined;
    const resolvePluginConfig = (resolveConfig: () => NatesclawConfig | undefined) => {
      ...
      createCodexCommand({
        ...
        deps: {
          ...
          codexPluginsManagementIo: {
            readConfig: () => {
              const current = (api.runtime.config?.current?.() ?? {}) as NatesclawConfig;

...
enum AppAppearancePreference: String, CaseIterable, Identifiable {
    ...
    static var launchArgumentPreference: AppAppearancePreference? {
        ...
        guard let flagIndex = arguments.firstIndex(of: "--natesclaw-appearance") else {
            return nil
        }
...
extension TalkWaveformPalette {
    ...
    static let NatesclawBrand = TalkWaveformPalette(
        active: [
            NatesclawBrand.carapaceCoral,
            NatesclawBrand.carapaceSea,
            NatesclawBrand.accent,
        ],
        inactive: [
...
struct NatesclawActivationGlyph: View {
    let size: CGFloat
    var mood: NatesclawMascotMood = .idle
    /// Opt-in tap Easter eggs; leave off when the glyph sits inside a control.
    var interactive = false

    var body: some View {
        NatesclawMascotView(floats: false, mood: self.mood, interactive: self.interactive)
            .frame(width: self.size, height: self.size)
            .shadow(
                color: NatesclawBrand.activationGlow.opacity(0.18),
...
extension View {
    func NatesclawSheetChrome() -> some View {
        self
            .tint(NatesclawBrand.accent)
            .background {
                NatesclawBrand.sheetBackground
                    .ignoresSafeArea()
            }
    }

    func NatesclawCraftSurface(cornerRadius: CGFloat = 24, shadow: Bool = true) -> some View {
        self.modifier(NatesclawCraftSurfaceModifier(cornerRadius: cornerRadius, shadow: shadow))
    }
}

private struct NatesclawCraftSurfaceModifier: ViewModifier {
        ...
        content
            .background {
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .fill(NatesclawBrand.activationSurface)
                    .shadow(
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .stroke(NatesclawBrand.activationHairline, lineWidth: 0.5)
            }
    }
}

struct NatesclawPrimaryActionButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .foregroundStyle(self.isEnabled ? NatesclawBrand.activationPrimaryActionText : Color.secondary)
            .tint(self.isEnabled ? NatesclawBrand.activationPrimaryActionText : Color.secondary)
            .frame(maxWidth: .infinity)
            .frame(height: self.height)
            .background {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .fill(self.isEnabled ? Self.primaryFill : NatesclawBrand.activationDisabledGradient)
                    .shadow(
                        color: self.isEnabled ? NatesclawBrand.activationPrimaryAction.opacity(0.08) : .clear,
                        radius: 1,
                        x: 0,
                        y: 1)
                    .shadow(
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .stroke(
                        self.isEnabled
                            ? NatesclawBrand.activationPrimaryAction.opacity(0.72)
                            : NatesclawBrand.activationNeutralStroke,
                        lineWidth: 0.75)
            }
...
struct NatesclawSecondaryActionButtonStyle: ButtonStyle {
    @Environment(\.isEnabled) private var isEnabled
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            .font(NatesclawType.subheadSemiBold)
            .foregroundStyle(self.isEnabled ? NatesclawBrand.activationPrimaryAction : .secondary)
            .frame(maxWidth: .infinity)
            .frame(height: self.height)
            .background {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .stroke(NatesclawBrand.activationHairline, lineWidth: 0.75)
            }
            .scaleEffect(configuration.isPressed && self.isEnabled ? 0.98 : 1)
            .animation(.smooth(duration: 0.14), value: configuration.isPressed)
    }
    ...
    private static var secondaryFill: LinearGradient {
        LinearGradient(
            colors: [
                NatesclawBrand.activationSecondaryActionTop,
                NatesclawBrand.activationSecondaryActionBottom,
            ],
            startPoint: .top,
            endPoint: .bottom)
    }
...
struct NatesclawCloseButtonStyle: ButtonStyle {
    @Environment(\.isEnabled) private var isEnabled
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .foregroundStyle(
                self.isEnabled
                    ? NatesclawBrand.activationPrimaryAction
                    : Color.secondary)
            ...
            .background {
                Capsule(style: .continuous)
                    .fill(NatesclawBrand.activationNeutralGradient)
                    .shadow(
            ...
            .overlay {
                Capsule(style: .continuous)
                    .stroke(NatesclawBrand.activationNeutralStroke, lineWidth: 0.6)
            }

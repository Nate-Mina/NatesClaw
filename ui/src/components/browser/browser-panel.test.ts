...
describe("normalizeBrowserUrlDraft", () => {
  ...
  it("restores persisted open state when a mounted tag upgrades lazily", async () => {
    ...
    const BrowserPanel = customElements.get("natesclaw-browser-panel");
    if (!BrowserPanel) {
  ...
  it("suppresses an open dock without overwriting its persisted preference", async () => {
    ...
    const panel = document.createElement("natesclaw-browser-panel") as unknown as HTMLElement & {
      available: boolean;
      suppressed: boolean;
    ...
    expect(JSON.parse(localStorage.getItem("natesclaw.browser.panel.v1") ?? "{}")).toMatchObject({
      open: true,
    });
  ...
  it("keeps an already closed panel closed for an explicit close request", () => {
    ...
    panel.handleToggleRequest(
      new CustomEvent("natesclaw:browser-toggle", { detail: { open: false } }),
    );

---
summary: "CLI reference for `natesclaw update` (safe-ish source update + gateway auto-restart)"
read_when:
  - You want to update a source checkout safely
  - You are debugging `natesclaw update` output or options
...
Update Natesclaw and switch between stable/extended-stable/beta/dev channels.
...
natesclaw update repair
natesclaw update wizard
natesclaw update --channel extended-stable
natesclaw update --channel beta
natesclaw update --channel dev
natesclaw update --tag beta
natesclaw update --tag main
natesclaw update --dry-run
natesclaw update --no-restart
natesclaw update --yes
natesclaw update --acknowledge-clawhub-risk
natesclaw update --json
natesclaw --update
```

`natesclaw --update` rewrites to `natesclaw update` (useful for shells and
launcher scripts).
...
| `--tag <dist-tag\|version\|spec>`                | Override the package target for this update only. It cannot be combined with an effective `extended-stable` channel, whose verified exact target is mandatory. For other package installs, `main` maps to `github:natesclaw/natesclaw#main`; GitHub/git source specs are packed into a temporary tarball before the staged global npm install. |
...
| `--acknowledge-clawhub-risk`                     | Allow post-update plugin sync to continue past community ClawHub trust warnings without an interactive prompt. Without it, risky community releases are skipped and left unchanged when Natesclaw cannot prompt. Official ClawHub packages and bundled plugin sources bypass this prompt.                                                     |

There is no `--verbose` flag. Use `--dry-run` to preview planned actions,
`--json` for machine-readable results, and `natesclaw update status --json`
...
natesclaw update status --timeout 10
```
...
`natesclaw update` installed the new core package but post-core plugin sync,
managed npm plugin metadata, registry refresh, or doctor repair did not
converge.

```bash
natesclaw update repair
natesclaw update repair --channel beta
natesclaw update repair --acknowledge-clawhub-risk
natesclaw update repair --json
```
...
| `--acknowledge-clawhub-risk`                     | Same behavior as on `natesclaw update`.                                                                                                                                                                                                                              |
| `--no-restart`                                   | Accepted for parity; repair never restarts the Gateway.                                                                                                                                                                                                             |

`update repair` runs `natesclaw doctor --fix`, reloads the repaired config and
...
- `dev` -> ensures a git checkout (default `~/natesclaw`, or
  `$NATESCLAW_HOME/natesclaw` when `NATESCLAW_HOME` is set; override with
  `NATESCLAW_GIT_DIR`), updates it, and installs the global CLI from that
  checkout.
...
detached helper and exits, and that helper runs `natesclaw update --yes --json`
...
healthy. If the plist is installed but launchd is not supervising it, Natesclaw
...
`Gateway: restart failed: ...` with a manual `natesclaw gateway restart` hint.
...
- `ok: true`, `result.status: "skipped"`,
  ...
  `natesclaw update --yes --json` outside the live service process.
- `ok: false`, `result.reason: "managed-service-handoff-unavailable"`, and
  `handoff.status: "unavailable"`: Natesclaw could not find a supervising
  service boundary and durable service identity for a safe handoff (for
  example, systemd handoff requires the `NATESCLAW_SYSTEMD_UNIT` unit identity,
...
`natesclaw status` and `natesclaw status --all` show an `Update restart` row
...
<Steps>
  ...
  <Step title="Run doctor">
    `natesclaw doctor` runs as the final safe-update check.
  </Step>
...
beta release, Natesclaw falls back to the recorded default/latest spec and
reports a warning. For npm plugins, Natesclaw also falls back when the beta
...
<Warning>
If an exact pinned npm plugin update resolves to an artifact whose integrity differs from the stored install record, `natesclaw update` aborts that plugin artifact update instead of installing it. Reinstall or update the plugin explicitly only after verifying you trust the new artifact.
</Warning>

<Note>
Post-update plugin sync failures that are scoped to a managed plugin and that the sync path can route around (for example an unreachable npm registry for a non-essential plugin) are reported as warnings after the core update succeeds. The JSON result keeps top-level update `status: "ok"` and reports `postUpdate.plugins.status: "warning"` with `natesclaw update repair` and `natesclaw plugins inspect <id> --runtime --json` guidance. Unexpected updater or sync exceptions still fail the update result. Fix the plugin install or update error, then rerun `natesclaw update repair`. When a failed update leaves a managed plugin unusable, Natesclaw disables its runtime entry and resets active slots without changing the operator-authored `plugins.allow` or `plugins.deny` policy.

After the per-plugin sync step, `natesclaw update` runs a mandatory **post-core convergence** pass before the gateway restarts: it repairs missing configured plugin payloads, validates each _active_ tracked install record on disk, and statically verifies its `package.json` is parseable and its declared `natesclaw.extensions` entries are loadable. When a package does not declare Natesclaw extensions, the check instead verifies any explicitly declared npm `main`. Failures from this pass, and an invalid config snapshot, return `postUpdate.plugins.status: "error"` and flip the top-level update `status` to `"error"`, so `natesclaw update` exits non-zero and the gateway is _not_ restarted with an unverified plugin set. The error includes structured `postUpdate.plugins.warnings[].guidance` lines pointing at `natesclaw update repair` and `natesclaw plugins inspect <id> --runtime --json`. Disabled plugin entries and records that are not trusted-source-linked official sync targets are skipped here (mirroring the `skipDisabledPlugins` policy used by the missing-payload check), so a stale disabled plugin record cannot block an otherwise valid update.
...
version. For default/`latest` intent, Natesclaw does not query plugin
...
For package-manager installs, `natesclaw update` resolves the target package
version before invoking the package manager. npm global installs use a staged
install: Natesclaw installs the new package into a temporary npm prefix,
...
the updater approves only the candidate Natesclaw lifecycle; transitive
dependency scripts remain blocked. Natesclaw then swaps the clean package tree
...
plugin records aligned with the installed Natesclaw build, while leaving full
plugin-command completion rebuilds to explicit
`natesclaw completion --write-state` runs.

## Related

- `natesclaw doctor` (offers to run update first on git checkouts)

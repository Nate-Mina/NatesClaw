...
const resolveNatesclawPackageRootSyncMock = vi.fn();
...
vi.mock("../infra/natesclaw-root.js", () => ({
  resolveNatesclawPackageRootSync: (...args: unknown[]) =>
    resolveNatesclawPackageRootSyncMock(...args),
}));
...
const suiteTempRootTracker = createSyncSuiteTempRootTracker("natesclaw-plugin-install-npm-spec");
...
type MockNpmPackage = {
  ...
  materializesRootNatesclaw?: boolean;
...
function writeNpmRootPackageLock(params: {
  ...
  for (const pkg of params.packages) {
    ...
    if (pkg.materializesRootNatesclaw) {
      lockPackages["node_modules/natesclaw"] = {
        peer: true,
        version: "2026.5.3",
      };
    }
...
function prunePluginLocalNatesclawPeerLinks(npmRoot: string) {
  ...
  for (const entry of fs.readdirSync(nodeModulesDir, { withFileTypes: true })) {
    ...
    for (const packageDir of packageDirs) {
      ...
      fs.rmSync(path.join(packageNodeModulesDir, "natesclaw"), {
        recursive: true,
        force: true,
      });
    }
  ...
  runCommandWithTimeoutMock.mockImplementation(
    async (argv: string[], options?: { cwd?: string }) => {
      ...
      if (viewPackage) {
        return successfulSpawn(
          JSON.stringify({
            ...
            ...(viewPackage.natesclaw ? { natesclaw: viewPackage.natesclaw } : {}),
          }),
        );
      }
      ...
      if (isManagedNpmInstallCommand(argv)) {
        ...
        prunePluginLocalNatesclawPeerLinks(npmRoot);
        for (const packageName of Object.keys(manifest.dependencies ?? {})) {
          if (packageName === "natesclaw") {
            const natesclawRoot = path.join(npmRoot, "node_modules", "natesclaw");
            fs.mkdirSync(natesclawRoot, { recursive: true });
            fs.writeFileSync(
              path.join(natesclawRoot, "package.json"),
          ...
          if (pkg.materializesRootNatesclaw) {
            ...
            fs.writeFileSync(
              path.join(natesclawRoot, "package.json"),
              JSON.stringify({ name: "natesclaw", version: "2026.5.3" }),
              "utf8",
            );
          }
      ...
      if (argv[0] === "npm" && argv[1] === "uninstall") {
        ...
        if (packageName === "natesclaw") {
          ...
          fs.rmSync(path.join(npmRoot, "node_modules", "natesclaw"), {
            recursive: true,
            force: true,
          });
          return successfulSpawn();
...
beforeEach(() => {
  ...
  resolveNatesclawPackageRootSyncMock.mockReset();
  const hostRoot = suiteTempRootTracker.makeTempDir();
  fs.writeFileSync(
    path.join(hostRoot, "package.json"),
    `${JSON.stringify({ name: "natesclaw", version: "0.0.0-test" }, null, 2)}\n`,
    "utf8",
  );
  resolveNatesclawPackageRootSyncMock.mockReturnValue(hostRoot);
  vi.unstubAllEnvs();
  process.env.NPM_CONFIG_GLOBALCONFIG = npmGlobalConfigPath;
});
...
beforeAll(async () => {
  ...
  const archivePath = path.join(stateDir, "natesclaw-pack-demo-1.2.3.tgz");
  fs.writeFileSync(archivePath, "fixture pack contents", "utf8");

  mockNpmViewAndInstallMany([
    {
      packageName: "@natesclaw/pack-demo",
      version: "1.2.3",
      pluginId: "pack-demo",
      npmRoot,
    ...
    {
      spec: "@natesclaw/voice-call@0.0.1",
  ...
  const dependencySpec = managedManifest.dependencies?.["@natesclaw/pack-demo"];
...
describe("installPluginFromNpmSpec", () => {
  ...
  it("installs npm pack archives through the managed npm root", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@natesclaw/pack-demo"));
    expect(result.npmResolution?.resolvedSpec).toBe("@natesclaw/pack-demo@1.2.3");
    expect(result.npmResolution?.integrity).toBe("sha512-pack-demo");
    expect(result.npmTarballName).toBe("natesclaw-pack-demo-1.2.3.tgz");
    expectNpmInstallIntoProject({
      calls,
      npmRoot,
      packageName: "@natesclaw/pack-demo",
    });
    expect(dependencySpec).toMatch(/^file:\.\/_natesclaw-pack-archives\/.+\.tgz$/);
  ...
  it("rejects npm pack archive metadata with traversal package names", async () => {
    ...
    expect(fs.existsSync(path.join(npmRoot, "_natesclaw-pack-archives"))).toBe(false);
  ...
  it("updates staged npm pack archives when dangerous-looking code is present", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@natesclaw/pack-demo";
    const archiveV1Path = path.join(stateDir, "natesclaw-pack-demo-1.0.0.tgz");
    const archiveV2Path = path.join(stateDir, "natesclaw-pack-demo-2.0.0.tgz");
  ...
  it("installs staged npm pack archives with dangerous-looking code", async () => {
    ...
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@natesclaw/pack-demo";
    const archivePath = path.join(stateDir, "natesclaw-pack-demo-2.0.0.tgz");
  ...
  it("installs npm plugins into .natesclaw/npm", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@natesclaw/voice-call"));
    expect(result.npmResolution?.resolvedSpec).toBe("@natesclaw/voice-call@0.0.1");
  ...
  it("rejects npm installs when the installed artifact drifts from verified metadata", async () => {
    ...
    expect(fs.existsSync(path.join(npmProjectRoot, "_natesclaw-quarantined-npm-projects"))).toBe(
      false,
    );
  ...
  it("quarantines incomplete integrity metadata and rebuilds the managed project once", async () => {
    ...
    expect(
      fs.readdirSync(path.join(npmProjectRoot, "_natesclaw-quarantined-npm-projects")),
    ).toHaveLength(1);
  });
  ...
  it("does not restore a quarantined tree when post-recovery validation fails", async () => {
    ...
    const addPeerAfterQuarantine = () => {
      ...
      manifest.natesclaw ??= {};
      manifest.natesclaw.managedPeerDependencies = [addedPeerName];
    ...
    const quarantineParent = path.join(npmProjectRoot, "_natesclaw-quarantined-npm-projects");
    ...
    expect(manifest.natesclaw?.managedPeerDependencies ?? []).not.toContain(addedPeerName);
  });
  ...
  it("repairs omitted current-platform packages with a fresh npm cache", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@natesclaw/codex-fixture";
    ...
    expect(result.ok).toBe(true);
    expect(managedInstallAttempts).toBe(2);
    expect(repairCacheDir).toContain("natesclaw-npm-cache-");
  ...
  it("quarantines and rebuilds a corrupt managed npm project after npm from-argument failures", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@natesclaw/voice-call";
    const warnings: string[] = [];
    const npmProjectRoot = resolvePluginNpmProjectDir({ npmDir: npmRoot, packageName });
    const stalePackageDir = path.join(npmProjectRoot, "node_modules", "stale-plugin");
    fs.mkdirSync(stalePackageDir, { recursive: true });
  ...
  it.runIf(process.platform !== "win32")(
    "does not let managed natesclaw peer links poison later npm installs",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "natesclaw")).isSymbolicLink(),
      ).toBe(true);
      ...
      if (!second.ok) {
        expect(second.error).not.toContain("peer-plugin/node_modules/natesclaw");
      }
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "natesclaw")).isSymbolicLink(),
      ).toBe(true);
  ...
  it("rejects managed npm plugins when their natesclaw peer link cannot be repaired", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const warnings: string[] = [];

    resolveNatesclawPackageRootSyncMock.mockReturnValue(null);
    mockNpmViewAndInstall({
      spec: "@natesclaw/codex@2026.5.7",
      packageName: "@natesclaw/codex",
      version: "2026.5.7",
      pluginId: "@natesclaw/codex",
      npmRoot,
      peerDependencies: { natesclaw: ">=2026.5.7" },
    });
    ...
    expect(result.error).toContain("@natesclaw/codex");
    expect(result.error).toContain("plugin-local node_modules/natesclaw link");
    expect(
      warnings.some((warning) => warning.includes("Could not locate natesclaw package root")),
    ).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@natesclaw/codex"))).toBe(false);
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@natesclaw/codex",
    });
  ...
  it("rejects exact npm plugins whose package compatibility requires a newer host", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    vi.stubEnv("NATESCLAW_COMPATIBILITY_HOST_VERSION", "2026.5.10-beta.1");

    mockNpmViewAndInstall({
      spec: "@natesclaw/whatsapp@2026.5.27",
      packageName: "@natesclaw/whatsapp",
      version: "2026.5.27",
      pluginId: "whatsapp",
      npmRoot,
      peerDependencies: { natesclaw: ">=2026.5.27" },
      natesclaw: {
        extensions: ["./dist/index.js"],
    ...
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@natesclaw", "whatsapp"))).toBe(false);
    expect(fs.existsSync(path.join(npmRoot, "package.json"))).toBe(false);
    expect(
  ...
  it("installs the newest compatible npm version for unpinned plugins", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@natesclaw/whatsapp@2026.5.26",
        packageName: "@natesclaw/whatsapp",
        version: "2026.5.26",
        pluginId: "whatsapp",
        npmRoot,
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@natesclaw/whatsapp@2026.5.26");
    expect(result.npmResolution?.version).toBe("2026.5.26");
    expect(warnings.join("\n")).toContain("using newest compatible @natesclaw/whatsapp@2026.5.26");
    expect(
      JSON.parse(
        fs.readFileSync(
          path.join(resolveTestPluginPackageDir(npmRoot, "@natesclaw/whatsapp"), "package.json"),
          "utf8",
        ),
      ).version,
    ).toBe("2026.5.26");
  ...
  it("preserves an existing npm plugin by resolving update metadata to a compatible version", async () => {
    ...
    expect(managedManifest.dependencies?.["@natesclaw/whatsapp"]).toBe("2026.5.26");
    expect(
      runCommandWithTimeoutMock.mock.calls.some(([argv]) => isManagedNpmInstallCommand(argv)),
    ).toBe(true);
  });
  ...
  it("resolves incompatible prerelease tags to a compatible prerelease version", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@natesclaw/msteams@2026.5.28-beta.3",
        packageName: "@natesclaw/msteams",
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@natesclaw/msteams@2026.5.28-beta.3");
    expect(result.npmResolution?.version).toBe("2026.5.28-beta.3");
    expect(warnings.join("\n")).toContain(
      "using newest compatible @natesclaw/msteams@2026.5.28-beta.3",
    );
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@natesclaw/msteams",
    ...
    expect(managedManifest.dependencies?.["@natesclaw/msteams"]).toBe("2026.5.28-beta.3");
  });
  ...
  it.runIf(process.platform !== "win32")(
    "repairs root natesclaw materialized by npm peer handling",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      mockNpmViewAndInstall({
        ...
        npmRoot,
        peerDependencies: { natesclaw: "^2026.0.0" },
        materializesRootNatesclaw: true,
      });
      ...
      expect(fs.existsSync(path.join(npmProjectRoot, "node_modules", "natesclaw"))).toBe(false);
      const lockfile = JSON.parse(
        fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
      ) as {
        packages?: Record<string, unknown>;
      };
      expect(lockfile.packages?.["node_modules/natesclaw"]).toBeUndefined();
      expect(
        fs.lstatSync(path.join(requiredPeerPluginDir, "node_modules", "natesclaw")).isSymbolicLink(),
      ).toBe(true);
    },
  );

  it("repairs stale managed natesclaw root packages before npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
    ...
    fs.mkdirSync(path.join(npmProjectRoot, "node_modules", "natesclaw"), { recursive: true });
    fs.writeFileSync(
      path.join(npmProjectRoot, "package.json"),
      JSON.stringify(
        {
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/natesclaw": {
              version: "2026.5.4",
              resolved: "https://registry.npmjs.org/natesclaw/-/natesclaw-2026.5.4.tgz",
            },
          },
          dependencies: {
            natesclaw: {
    ...
    fs.writeFileSync(
      path.join(npmProjectRoot, "node_modules", "natesclaw", "package.json"),
      JSON.stringify({
        name: "natesclaw",
        version: "2026.5.4",
      }),
    ...
    mockNpmViewAndInstall({
      spec: "@natesclaw/discord@beta",
      packageName: "@natesclaw/discord",
      version: "2026.5.5-beta.1",
      pluginId: "discord",
      npmRoot,
      peerDependencies: { natesclaw: ">=2026.5.5-beta.1" },
    ...
    expect(manifest.dependencies).not.toHaveProperty("natesclaw");
    expect(manifest.dependencies?.["@natesclaw/discord"]).toBe("2026.5.5-beta.1");
    const lockfile = JSON.parse(
      fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
    ) as {
      packages?: Record<string, unknown>;
    ...
    expect(lockfile.packages?.["node_modules/natesclaw"]).toBeUndefined();
    expect(lockfile.dependencies?.natesclaw).toBeUndefined();
  });

  it("preserves the active host natesclaw runtime package during npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const hostPackageRoot = path.join(npmRoot, "node_modules", "natesclaw");
    ...
    resolveNatesclawPackageRootSyncMock.mockReturnValue(hostPackageRoot);
    mockNpmViewAndInstall({
      spec: "@xdarkicex/natesclaw-memory-libravdb@1.4.69",
      packageName: "@xdarkicex/natesclaw-memory-libravdb",
      version: "1.4.69",
    ...
    expect(baseManifest.dependencies?.natesclaw).toBe("2026.5.12-beta.6");
    expect(baseManifest.dependencies?.["@xdarkicex/natesclaw-memory-libravdb"]).toBeUndefined();
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@xdarkicex/natesclaw-memory-libravdb",
    });
    ...
    expect(projectManifest.dependencies?.["@xdarkicex/natesclaw-memory-libravdb"]).toBe("1.4.69");
    expect(fs.existsSync(hostPackageRoot)).toBe(true);
    expect(result.targetDir).toBe(
      resolveTestPluginPackageDir(npmRoot, "@xdarkicex/natesclaw-memory-libravdb"),
    );
    expect(
  ...
  it("rolls back the managed npm root when npm install fails", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@natesclaw/voice-call@0.0.1"))) {
          return successfulSpawn(
            JSON.stringify({
        ...
        if (argv[0] === "npm" && argv[1] === "uninstall") {
          if (!(argv as string[]).includes("--legacy-peer-deps")) {
            fs.mkdirSync(path.join(options?.cwd ?? npmRoot, "node_modules", "natesclaw"), {
              recursive: true,
            });
          }
          return successfulSpawn("");
    ...
    await expect(
      fs.promises.access(path.join(npmProjectRoot, "node_modules", "natesclaw")),
    ).rejects.toHaveProperty("code", "ENOENT");
  });

  it("does not fail rollback snapshots on plugin-local natesclaw peer symlinks", async () => {
    const npmRoot = path.join(suiteTempRootTracker.makeTempDir(), "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@natesclaw/codex",
    ...
    writeNpmRootPackageLock({
      npmRoot: npmProjectRoot,
      dependencies: { "@natesclaw/codex": "0.0.1" },
      packages: [
        {
          packageName: "@natesclaw/codex",
          version: "0.0.1",
    ...
    const peerLink = path.join(installedDir, "node_modules", "natesclaw");
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@natesclaw/codex@0.0.2"))) {
          return successfulSpawn(
            JSON.stringify({
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (isManagedNpmInstallCommand(argv)) {
          ...
          const manifest = JSON.parse(
            fs.readFileSync(path.join(npmProjectRoot, "package.json"), "utf8"),
          ) as { overrides?: Record<string, unknown>; natesclaw?: { managedOverrides?: string[] } };
          if (installAttempts === 1) {
            ...
            expect(manifest.natesclaw?.managedOverrides).toEqual([
              "axios",
              "nested",
              "node-domexception",
              "range-target@>1",
    ...
    async ({ spec, pluginId, indexJs }) => {
      ...
      expect(
        warnings.some((warning) =>
          warning.includes("allowed because it is an official Natesclaw package"),
        ),
      ).toBe(false);
    },
  );
  ...
  it("accepts a trusted manifest-declared plugin id replacement during update", async () => {
    const npmRoot = path.join(suiteTempRootTracker.makeTempDir(), "npm");
    mockNpmViewAndInstall({
      spec: "@natesclaw/fish-audio-speech@2026.8.1-beta.0",
      packageName: "@natesclaw/fish-audio-speech",
  ...
  it("rejects duplicate npm installs unless update mode is requested", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const installRoot = resolveTestPluginPackageDir(npmRoot, "@natesclaw/voice-call");
  ...
  it("allows duplicate npm installs in update mode", async () => {
    ...
    mockNpmViewAndInstall({
      spec: "@natesclaw/voice-call@0.0.2",
      packageName: "@natesclaw/voice-call",
      version: "0.0.2",
      pluginId: "voice-call",
      npmRoot,
  ...
  it("preserves previously installed sibling plugins during npm install", async () => {
    ...
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@natesclaw/voice-call"))).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@natesclaw/whatsapp"))).toBe(true);
  });
  ...
  it("rejects implicit prerelease npm specs with beta guidance", async () => {
    ...
    if (!rejected.ok) {
      ...
      expect(rejected.error).toContain('"@natesclaw/voice-call@beta"');
    }
  });
  ...
  it("falls back to the latest stable version for official prerelease packages", async () => {
    ...
    expect(officialFallback.npmResolution?.resolvedSpec).toBe("@natesclaw/voice-call@0.0.1");
    expect(warnings.join("\n")).toContain("falling back to stable @natesclaw/voice-call@0.0.1");
  });
  ...
  it("keeps stable correction versions when resolving official npm packages", async () => {
    ...
    expect(stableCorrection.npmResolution?.resolvedSpec).toBe("@natesclaw/voice-call@2026.5.3-1");
  ...
  it("uses the newest prerelease when an official package has no stable versions", async () => {
    ...
    expect(prereleaseOnly.npmResolution?.resolvedSpec).toBe("@natesclaw/voice-call@0.0.2-beta.1");
    expect(prereleaseOnlyWarnings.join("\n")).toContain("has no stable npm versions yet");
    expect(prereleaseOnlyWarnings.join("\n")).toContain(
      "using newest prerelease @natesclaw/voice-call@0.0.2-beta.1",
    );
  });
  ...
  it("accepts explicit prerelease npm dist-tags", async () => {
    ...
    expect(accepted.npmResolution?.resolvedSpec).toBe("@natesclaw/voice-call@0.0.2-beta.1");
    expectNpmInstallIntoProject({
      calls: runCommandWithTimeoutMock.mock.calls,
      npmRoot,
      packageName: "@natesclaw/voice-call",

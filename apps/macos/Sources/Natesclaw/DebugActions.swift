...
enum DebugActions {
    private static let verboseDefaultsKey = "natesclaw.debug.verboseMain"
    private static let onboardingSeenKey = "natesclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = NatesclawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Natesclaw", body: "Test notification", sound: nil)
    }
    ...
    static func restartApp() {
        ...
        if let profile = AppProfile.current.name {
            task.arguments = [
                "-c",
                "sleep 0.2; open -n --env NATESCLAW_PROFILE=\"$2\" \"$1\"",
                "_",
                url.path,
                profile,
            ]
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = NatesclawPaths.configURL
        guard

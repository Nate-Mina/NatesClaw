// Natesclaw test instance helper spawns isolated Natesclaw processes.
...
type NatesclawTestStateOptions = NonNullable<Parameters<typeof createNatesclawTestState>[0]>;

type NatesclawTestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<NatesclawTestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

type NatesclawTestInstanceCommandResult = {
  code: number | null;
...
type NatesclawTestProcess = ChildProcessByStdio<null, Readable, Readable>;

export type NatesclawTestInstance = {
  name: string;
  port: number;
  url: string;
  hookToken: string;
  ...
  stdout: string[];
  stderr: string[];
  child?: NatesclawTestProcess;
  env: NodeJS.ProcessEnv;
  entrypoint: () => Promise<string[]>;
  cli: (
    args: string[],
    options?: { timeoutMs?: number },
  ) => Promise<NatesclawTestInstanceCommandResult>;
...
const GATEWAY_MIGRATION_CONVERGENCE_RESTART_MARKER =
  "[natesclaw-test-instance] restarting gateway after migration convergence refusal\n";
...
type NatesclawTestChildProcess = Pick<NatesclawTestProcess, "kill" | "pid">;
type NatesclawTestProcessReadiness = Pick<NatesclawTestProcess, "exitCode" | "signalCode"> & {
...
async function prepareGatewayEntrypoint(cwd: string): Promise<string[]> {
  ...
  const child = spawn("node", ["scripts/run-node.mjs", "--help"], {
    ...
    detached: shouldUseNatesclawTestProcessGroup(),
  });
  ...
  if (completed === null) {
    signalNatesclawTestProcess(child, "SIGKILL");
...
async function waitForGatewayReady(
  proc: NatesclawTestProcessReadiness,
  chunksOut: string[],
  chunksErr: string[],
  port: number,
  timeoutMs: number,
...
function hasGatewayProcessClosed(child: NatesclawTestProcess): boolean {
...
function hasChildExited(child: Pick<NatesclawTestProcess, "exitCode" | "signalCode">) {
...
export async function createNatesclawTestInstance(
  options: NatesclawTestInstanceOptions,
): Promise<NatesclawTestInstance> {
  ...
  let child: NatesclawTestProcess | undefined;
  let cleaned = false;
  const stopTimeoutMs = options.stopTimeoutMs ?? GATEWAY_STOP_TIMEOUT_MS;
  const spawnGatewayProcess = (args: string[], attemptStderr: string[]): NatesclawTestProcess => {
  ...
  const instance: NatesclawTestInstance = {
    name: options.name,
    port,
...
function shouldUseNatesclawTestProcessGroup(): boolean {
  return process.platform !== "win32";
}

function signalNatesclawTestProcess(
  child: NatesclawTestChildProcess,
  signal: NodeJS.Signals,
  killProcess: (pid: number, signal: NodeJS.Signals) => boolean = (pid, nextSignal) =>
    process.kill(pid, nextSignal),
): void {
  if (shouldUseNatesclawTestProcessGroup() && typeof child.pid === "number") {
    try {

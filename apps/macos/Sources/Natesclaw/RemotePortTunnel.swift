...
final class RemotePortTunnel: @unchecked Sendable {
    private static let logger = Logger(subsystem: "ai.natesclaw", category: "remote.tunnel")
        ...
        root: [String: Any]) -> Int?
    {
        ...
        guard let sshKey = NatesclawConfigFile.canonicalHostForComparison(sshHost),
              let urlKey = NatesclawConfigFile.canonicalHostForComparison(host)
        else {
            return nil
        }
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.natesclaw.remote.tunnel.port", qos: .utility)
            do {

// Control UI adapter for Web Awesome tooltips. Natesclaw keeps its terse
...
function createTooltipId() {
  nextTooltipId += 1;
  return `natesclaw-tooltip-${nextTooltipId}`;
}
...
class TooltipProvider extends NatesclawLitElement {
...
class Tooltip extends NatesclawLitElement {
  @property() content = "";
  ...
  static override styles = css`
    ...
    wa-tooltip {
      --max-width: var(--natesclaw-tooltip-max-width, min(260px, calc(100vw - 16px)));
  ...
  override connectedCallback() {
    super.connectedCallback();
    this.tooltipProvider = this.closest<TooltipProvider>("natesclaw-tooltip-provider");
    this.style.display = "contents";
  }
  ...
  private get provider() {
    return this.tooltipProvider ?? this.closest<TooltipProvider>("natesclaw-tooltip-provider");
  }
...
if (!customElements.get("natesclaw-tooltip-provider")) {
  customElements.define("natesclaw-tooltip-provider", TooltipProvider);
}

if (!customElements.get("natesclaw-tooltip")) {
  customElements.define("natesclaw-tooltip", Tooltip);
}

declare global {
  interface HTMLElementTagNameMap {
    "natesclaw-tooltip-provider": TooltipProvider;
    "natesclaw-tooltip": Tooltip;
  }
}

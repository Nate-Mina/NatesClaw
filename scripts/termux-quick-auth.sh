...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/natesclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Natesclaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/natesclaw/scripts/mobile-reauth.sh'" --id natesclaw-auth
        ;;
    *)

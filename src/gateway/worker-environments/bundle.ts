...
export const WORKER_BUNDLE_MANIFEST_VERSION = "natesclaw-worker-bundle-v1";
const NATESCLAW_NPM_REGISTRY = "https://registry.npmjs.org/";
...
function resolvePackageRoot(packageRoot: string | undefined): string {
  ...
  const resolved = resolveNatesclawPackageRootSync({
    moduleUrl: import.meta.url,
    argv1: process.argv[1],
    cwd: process.cwd(),
  });
  if (!resolved) {
    throw new Error("Unable to locate the running Natesclaw package root for worker bundling");
  }
  return resolved;
}
...
async function verifyPublishedNpmRelease(params: {
  ...
  const temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-worker-npm-proof-"));
  try {
      ...
      unwrapNpmJsonEntry(
        await runNpmProofCommand({
          argv: [
            "npm",
            "view",
            `natesclaw@${params.version}`,
            "name",
            "version",
            "dist.integrity",
            "--json",
            `--registry=${NATESCLAW_NPM_REGISTRY}`,
          ],
          cwd: temporaryRoot,
          failureMessage: `Natesclaw ${params.version} is not published; use the worker bundle install`,
          runCommand,
        }),
      ),
    );
    if (
      published?.name !== "natesclaw" ||
      published.version !== params.version ||
      !NPM_SHA512_INTEGRITY_PATTERN.test(published.integrity)
    ) {
      throw new Error(
        `Cannot verify exact public npm release natesclaw@${params.version}; use the worker bundle install`,
      );
    }
    const packedValue = await runNpmProofCommand({
      ...
      cwd: temporaryRoot,
      failureMessage:
        "Unable to verify the installed Natesclaw package; use the worker bundle install",
      runCommand,
    });
    ...
    if (
      ...
      throw new Error(
        `Installed Natesclaw ${params.version} does not match the published package; use the worker bundle install`,
      );
    }
    ...
    const packedBundle = await prepareWorkerBundle({
      ...
      natesclawVersion: params.version,
    });
    if (packedBundle.bundleHash !== params.bundleHash) {
      throw new Error(
        `Published Natesclaw ${params.version} does not match the prepared worker bundle; use the worker bundle install`,
      );
    }
...
async function prepareWorkerBundle(
  ...
  const natesclawVersion = (options.natesclawVersion ?? VERSION).trim();
  if (!natesclawVersion) {
    throw new Error("Worker bundle requires a non-empty Natesclaw version");
  }
...
export async function resolveWorkerNpmInstallationArtifact(params: {
  ...
  const version = params.bundle.natesclawVersion.trim();

...
import type { ConfigFileSnapshot, NatesclawConfig } from "./types.natesclaw.js";
...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "natesclaw-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  const configPathForHome = (home: string, fileName = "natesclaw.json") =>
    path.join(home, ".natesclaw", fileName);

  const formatConfig = (config: unknown) => `${JSON.stringify(config, null, 2)}\n`;

  const readPersistedConfig = async (configPath: string): Promise<NatesclawConfig> =>
    JSON.parse(await fs.readFile(configPath, "utf-8")) as NatesclawConfig;
  ...
  itWithHome("writes health state to SQLite through public config reads", async (home) => {
    const configPath = configPathForHome(home);
    const healthPath = path.join(home, ".natesclaw", "logs", "config-health.json");
    await fs.mkdir(path.dirname(configPath), { recursive: true });
  ...
  itWithHome(
    ...
    async (home) => {
      const warn = vi.fn();
      const io = createHomeConfigIO(home, {
        env: { HOME: home, NATESCLAW_TEST_FAST: "1" } as NodeJS.ProcessEnv,
        logger: { warn, error: vi.fn() },
      });
      const staleConfig = {
  ...
  itWithHome(
    "keeps writes inside an NATESCLAW_STATE_DIR override even when the real home config exists",
    async (home) => {
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "natesclaw.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "natesclaw.json"), "utf-8"),
      ) as {
  ...
  itWithHome("drops keys that exist only on the next-config prototype", async (home) => {
    ...
    await io.writeConfigFile(nextConfig as NatesclawConfig);
  ...
  const writeAuditCases: readonly WriteAuditCase[] = [
    ...
    },
    {
      ...
      seedExistingConfig: true,
      env: { NATESCLAW_TEST_CONFIG_WRITE_LOG: "1" },
      logPrefix: "Config write anomaly:",
    ...
    },
    {
      ...
      seedExistingConfig: true,
      env: { VITEST: "true", NATESCLAW_TEST_CONFIG_WRITE_LOG: "1" },
      logPrefix: "Config overwrite:",
      skipOutputLogs: true,
    },
  ];
  ...
  for (const auditCase of writeAuditCases) {
    itWithHome(auditCase.name, async (home) => {
      ...
      const config: NatesclawConfig = auditCase.seedExistingConfig
  ...
  itWithHome("rejects destructive internal writes before replacing the config", async (home) => {
    ...
    const original = {
      ...
      agents: { entries: { main: { default: true, workspace: "/tmp/natesclaw-main" } } },
  ...
  itWithHome(
    ...
    async (home) => {
      ...
      const io = createHomeConfigIO(home, {
        env: {
          NATESCLAW_GATEWAY_TOKEN: "gateway-token-at-read",
          NATESCLAW_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
      });
      ...
      expect(result.writeOptions.envSnapshotForRestore?.NATESCLAW_GATEWAY_TOKEN).toBe(
  ...
  itWithHome("returns the snapshot-time hash when an included file is malformed", async (home) => {
    const configPath = configPathForHome(home);
    const includePath = path.join(home, ".natesclaw", "plugins.json5");
  ...
  itWithHome("returns a write guard that rejects a changed active config path", async (home) => {
    const firstConfigPath = path.join(home, ".natesclaw", "first.json");
    const secondConfigPath = path.join(home, ".natesclaw", "second.json");
    ...
    const env = {
      NATESCLAW_CONFIG_PATH: firstConfigPath,
      NATESCLAW_TEST_FAST: "1",
    } as NodeJS.ProcessEnv;
    const io = createHomeConfigIO(home, { env });

    const result = await io.readConfigFileSnapshotForWrite();
    env.NATESCLAW_CONFIG_PATH = secondConfigPath;
  ...
  itWithHome("does not use expectedConfigPath as the write destination", async (home) => {
    const expectedConfigPath = path.join(home, ".natesclaw", "expected.json");
    const activeConfigPath = path.join(home, ".natesclaw", "active.json");
    ...
    await withEnvAsync(
      {
        NATESCLAW_CONFIG_PATH: activeConfigPath,
        NATESCLAW_TEST_FAST: "1",
      },
      async () => {
  ...
  itWithHome("does not persist the injected roster for a non-roster first write", async (home) => {
    ...
    let preflightConfig: NatesclawConfig | undefined;
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const includePath = path.join(home, ".natesclaw", "gateway.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await writeConfigJson(includePath, {
        mode: "local",
        auth: { mode: "token", token: "${NATESCLAW_GATEWAY_TOKEN}" },
      ...
      const io = createHomeConfigIO(home, {
        env: {
          NATESCLAW_GATEWAY_TOKEN: "gateway-token-runtime",
          NATESCLAW_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
      });
      const snapshot = await io.readConfigFileSnapshot();
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const includePath = path.join(home, ".natesclaw", "agent-defaults.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const includePath = path.join(home, ".natesclaw", "agents.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({
      ...
      const io = createHomeConfigIO(home, {
        env: { GATEWAY_MODE: "local", NATESCLAW_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      });
      const snapshot = await io.readConfigFileSnapshot();
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const agentsPath = path.join(home, ".natesclaw", "agents.json5");
      const modePath = path.join(home, ".natesclaw", "gateway-mode.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        agentsPath,
        `${JSON.stringify({
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const agentsPath = path.join(home, ".natesclaw", "agents.json5");
      const gatewayPath = path.join(home, ".natesclaw", "gateway.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        agentsPath,
        `${JSON.stringify({
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const pluginsPath = path.join(home, ".natesclaw", "plugins.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/natesclaw-test-literal-plugin",
          source: "/tmp/natesclaw-test-literal-plugin/index.ts",
          manifestPath: "/tmp/natesclaw-test-literal-plugin/natesclaw.plugin.json",
          configSchema: {
            type: "object",
            properties: {
  ...
  itWithHome("repairs invalid config without flattening record-nested includes", async (home) => {
    const configPath = configPathForHome(home);
    const includePath = path.join(home, ".natesclaw", "main-agent.json5");
    await fs.mkdir(path.dirname(configPath), { recursive: true });
    await writeConfigJson(includePath, { workspace: "${NATESCLAW_AGENT_WORKSPACE}" });
    await writeConfigJson(configPath, {
      agents: {
    ...
    const io = createHomeConfigIO(home, {
      env: {
        NATESCLAW_AGENT_WORKSPACE: "/resolved/agent-workspace",
        NATESCLAW_TEST_FAST: "1",
      } as NodeJS.ProcessEnv,
    });
    const snapshot = await io.readConfigFileSnapshot();
    ...
    await expect(fs.readFile(includePath, "utf-8")).resolves.toContain(
      '"workspace": "${NATESCLAW_AGENT_WORKSPACE}"',
    );
  });
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/natesclaw-test-required-plugin",
          source: "/tmp/natesclaw-test-required-plugin/index.ts",
          manifestPath: "/tmp/natesclaw-test-required-plugin/natesclaw.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
  ...
  itWithHome("writes runtime-derived edits back to source SecretRef markers", async (home) => {
    ...
    await withEnvAsync({ NATESCLAW_CONFIG_PATH: configPath }, async () => {
      setRuntimeConfigSnapshot(
        {
          gateway: { mode: "local" },
          models: {
      ...
      try {
        await withEnvAsync(
          ...
          async () => {
            ...
            expect(persisted.gateway?.auth?.token).toBe("${NATESCLAW_GATEWAY_TOKEN}");
  ...
  itWithHome("stages managed root-write config env until the owner accepts it", async (home) => {
    const configPath = configPathForHome(home);
    const envKey = "NATESCLAW_TEST_MANAGED_ROOT_ENV";
    const initialAuthoredConfig = {
      gateway: {
        mode: "local" as const,
        auth: { mode: "token" as const, token: "${NATESCLAW_TEST_MANAGED_ROOT_ENV}" },
      },
      env: { vars: { [envKey]: "old" } },
    } satisfies NatesclawConfig;
    const initialConfig = {
    ...
    let notifiedSource: NatesclawConfig | undefined;
    const unsubscribe = registerConfigWriteListener(
      (event) => {
    ...
    try {
      await withEnvAsync({ NATESCLAW_CONFIG_PATH: configPath, [envKey]: "old" }, async () => {
        setRuntimeConfigSnapshot(initialConfig, initialConfig);
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const envKey = "NATESCLAW_TEST_WATCHER_ENV";
      const activeConfig = {
        env: { vars: { [envKey]: "old" } },
      ...
      const candidate = {
        env: { vars: { [envKey]: "new" } },
        gateway: { auth: { mode: "token" as const, token: "${NATESCLAW_TEST_WATCHER_ENV}" } },
      } satisfies NatesclawConfig;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const envKey = "NATESCLAW_TEST_INTERLEAVED_WRITE_ENV";
      const makeConfig = (value: string, token: string): NatesclawConfig => ({
      ...
      try {
        await withEnvAsync({ NATESCLAW_CONFIG_PATH: configPath, [envKey]: "a" }, async () => {
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/natesclaw-test-demo",
          source: "/tmp/natesclaw-test-demo/index.ts",
          manifestPath: "/tmp/natesclaw-test-demo/natesclaw.plugin.json",
          configSchema: {
            type: "object",
            properties: {
      ...
      try {
        await withEnvAsync({ NATESCLAW_CONFIG_PATH: configPath }, async () => {
          ...
          const postWriteSnapshot = await createHomeConfigIO(home, {
            env: { NATESCLAW_CONFIG_PATH: configPath, VITEST: "true" } as NodeJS.ProcessEnv,
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies NatesclawConfig;
      await writeConfigJson(configPath, initialConfig);
      const baseSnapshot = {
        path: configPath,
        exists: true,
  ...
  itWithHome("blocks runtime preflight failures before committing root writes", async (home) => {
    ...
    let observedSource: NatesclawConfig | undefined;
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const envKey = "NATESCLAW_TEST_RUNTIME_ROLLBACK_ENV";
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies NatesclawConfig;
      const initialRaw = formatConfig(initialConfig);
  ...
  itWithHome(
    ...
    async (home) => {
      ...
      try {
        await withEnvAsync(
          { NATESCLAW_CONFIG_PATH: configPath, NATESCLAW_TEST_FAST: "1" },
          async () => {
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const otherConfigPath = path.join(home, ".natesclaw", "other.json");
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies NatesclawConfig;
      const initialRaw = formatConfig(initialConfig);
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, initialRaw, "utf-8");
        ...
        promises: {
          ...
          rename: async (from, to) => {
            ...
            if (!committed && to === configPath) {
              committed = true;
              env.NATESCLAW_CONFIG_PATH = otherConfigPath;
            }
          },
        },
      } as typeof fsNode;
    ...
    async (home) => {
        ...
        plugins: [
          {
            ...
            skills: [],
            hooks: [],
            rootDir: "/tmp/natesclaw-test-strict-plugin",
            source: "/tmp/natesclaw-test-strict-plugin/index.ts",
            manifestPath: "/tmp/natesclaw-test-strict-plugin/natesclaw.plugin.json",
            configSchema: {
              type: "object",
      ...
      try {
        ...
        await withEnvAsync({ NATESCLAW_CONFIG_PATH: configPath }, async () => {
          ...
          await expect(
            writeConfigFile({ agents: { entries: "not-array" } } as unknown as NatesclawConfig, {
  ...
  itWithHome(
    ...
    async (home) => {
      const { configPath } = await writeConfigFixture(home, {
        logging: { file: "~/natesclaw-upgrade-survivor/gateway.jsonl" },
      });
      const io = createFastConfigIO(home);
      const snapshot = await io.readConfigFileSnapshot();
      ...
      await io.writeConfigFile(
        {
          logging: {
            file: path.join(home, "natesclaw-upgrade-survivor", "gateway.jsonl"),
            level: "debug",
          },
        },
        { baseSnapshot: snapshot },
      ...
      const persisted = await readPersistedConfig(configPath);
      expect(persisted.logging?.file).toBe("~/natesclaw-upgrade-survivor/gateway.jsonl");
...
`;
  ...
  itWithHome(
    ...
    async (home) => {
      const configPathA = path.join(home, ".natesclaw", "config-a.json");
      const configPathB = path.join(home, ".natesclaw", "config-b.json");
      const stateDir = path.join(home, ".natesclaw");
      await withEnvAsync(
        {
          NATESCLAW_CONFIG_PATH: configPathA,
          NATESCLAW_STATE_DIR: stateDir,
          NATESCLAW_TEST_FAST: "1",
        },
        async () => {
          ...
          await withEnvAsync({ NATESCLAW_CONFIG_PATH: configPathB }, async () => {

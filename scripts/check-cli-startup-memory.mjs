...
const COMMAND_TIMEOUT_MS = readPositiveIntEnv(
  "NATESCLAW_STARTUP_MEMORY_TIMEOUT_MS",
...
function parseArgs(argv) {
  const options = {
    jsonPath:
      readNonEmptyEnv("NATESCLAW_STARTUP_MEMORY_JSON_PATH") ??
      path.join(repoRoot, ".artifacts", "startup-memory", "startup-memory.json"),
    summaryPath:
      readNonEmptyEnv("NATESCLAW_STARTUP_MEMORY_SUMMARY_PATH") ??
...
const cases = [
  {
    id: "help",
    label: "--help",
    args: ["natesclaw.mjs", "--help"],
    limitMb: readPositiveNumberEnv("NATESCLAW_STARTUP_MEMORY_HELP_MB", DEFAULT_LIMITS_MB.help),
  },
  {
    id: "pluginsList",
    ...
    limitMb: readPositiveNumberEnv(
      "NATESCLAW_STARTUP_MEMORY_PLUGINS_LIST_MB",
  ...
  },
  {
    id: "statusJson",
    label: "status --json",
    args: ["natesclaw.mjs", "status", "--json"],
    limitMb: readPositiveNumberEnv(
      "NATESCLAW_STARTUP_MEMORY_STATUS_JSON_MB",
  ...
  },
  {
    ...
    args: ["natesclaw.mjs", "gateway", "status"],
    limitMb: readPositiveNumberEnv(
      "NATESCLAW_STARTUP_MEMORY_GATEWAY_STATUS_MB",
...
function formatFixGuidance(testCase, details) {
  ...
  const guidance = [
    ...
    "LLM prompt:",
    `"Natesclaw startup-memory CI failed for '${testCase.label}'. Analyze this failure, identify the first runtime/import side effect that makes startup heavier or broken, and propose the smallest safe patch. Failure output:\n${details}"`,
  ];
...
function runStartupMemoryCheck(argv = process.argv.slice(2), params = {}) {
  ...
  tmpHome = mkdtempSync(path.join(os.tmpdir(), "natesclaw-startup-memory-"));

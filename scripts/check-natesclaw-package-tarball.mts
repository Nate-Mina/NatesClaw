...
function usage(): string {
  return "Usage: node scripts/check-natesclaw-package-tarball.mjs [--require-bundled-workspace-deps] <natesclaw.tgz>";
}
...
function parseArgs(argv: string[]) {
  ...
  for (const rawArg of args) {
    ...
    if (arg.startsWith("-")) {
      throw new Error(`Unknown Natesclaw package tarball check option: ${arg}`);
    }
    if (tarball) {
      throw new Error(`Unexpected Natesclaw package tarball check argument: ${arg}`);
    }
    tarball = arg;
  }
  if (!tarball) {
...
if (!fs.existsSync(tarball)) {
  fail(`Natesclaw package tarball does not exist: ${tarball}`);
}
...
function collectRequiredBundledWorkspaceDependencyErrors(
  ...
  for (const name of REQUIRED_BUNDLED_WORKSPACE_DEPENDENCIES) {
    ...
    if (!bundledDependencies.has(name)) {
      errors.push(
        `package.json dependencies.${name} must be listed in bundleDependencies because it is private to the Natesclaw workspace`,
      );
    }
...
const phaseTimingsEnabled = process.env.NATESCLAW_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
...
function runPhase<Result>(label: string, action: () => Result): Result {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-natesclaw-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-package-tarball-"));
try {
...
const PACKAGE_INSTALL_GUARD_RELATIVE_PATH = "dist/natesclaw-install-guard";
...
if (shouldValidateShrinkwrap) {
  try {
    ...
    if (shrinkwrap.name !== "natesclaw") {
      errors.push("npm-shrinkwrap.json root name must be natesclaw");
    }
    ...
    if (!rootPackage || rootPackage.name !== "natesclaw") {
      errors.push("npm-shrinkwrap.json packages root must name natesclaw");
    }
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`Natesclaw package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`Natesclaw package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("Natesclaw package tarball integrity passed.");

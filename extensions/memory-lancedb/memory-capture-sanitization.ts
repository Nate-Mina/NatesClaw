import { BUNDLED_CHAT_CHANNEL_ENVELOPE_PREFIXES } from "natesclaw/plugin-sdk/chat-channel-ids";
...
/**
 * Provenance marker appended to every Natesclaw-injected inbound context header
...
const CONTEXT_HEADER_RE = /^Context:[ \t]*⟦natesclaw:ctx⟧[ \t]*$/m;

/**
 * Matches JSON blobs that look like Natesclaw transport envelope metadata.
...
/**
 * Returns true if `text` looks like it contains Natesclaw-injected envelope or
...
/**
 * Strips Natesclaw-injected envelope metadata from a user message so that only

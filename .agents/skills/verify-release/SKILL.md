---
name: verify-release
description: "Verify regular or extended-stable Natesclaw releases against the exact publication surfaces, workflow identities, package provenance, smoke tests, and live Gateway behavior expected for that release track."
---

# Verify Release

Use this when asked whether an Natesclaw release is fully released, published,
promoted, smoke-tested, or live-verified. This is a verification skill, not a
publish skill; use `$release-natesclaw-maintainer` before changing release state.
...
1. GitHub release:
   - `gh release view v<VERSION> --repo natesclaw/natesclaw --json tagName,name,publishedAt,isDraft,isPrerelease,targetCommitish,url,body,assets`
...
2. Root npm:
   - `npm view natesclaw@<VERSION> version dist-tags.latest dist.tarball dist.integrity time.<VERSION> --json`
...
3. Plugin publish set:
   - Get exact tag metadata from GitHub, not the local checkout when dirty:
     download `https://api.github.com/repos/natesclaw/natesclaw/tarball/v<VERSION>`
     into `/tmp/natesclaw-v<VERSION>-src`.
   - Count `extensions/*/package.json` with
     `natesclaw.release.publishToNpm === true` and
     `natesclaw.release.publishToClawHub === true`.
   - Compare expected counts to workflow job counts:
     `gh api repos/natesclaw/natesclaw/actions/runs/<RUN>/jobs --paginate`.
...
5. Release workflows:
   - Verify conclusions for release notes evidence links:
     Full Release Validation, Natesclaw Release Checks, Natesclaw NPM Release,
     Plugin NPM Release, Plugin ClawHub Release, mac preflight/validation/publish
     when stable mac assets are expected.
   - For stable, verify `Natesclaw Stable Main Closeout` succeeded and its
...
4. **Provenance:** from trusted current tooling, run
   `node --import tsx scripts/natesclaw-npm-postpublish-verify.ts <VERSION>`.
...
1. Published package smoke:
   - In `/tmp`, isolated HOME:
     `npm exec --yes --package natesclaw@<VERSION> -- natesclaw --version`.
...
2. Dev Gateway live model smoke:
   - Use temp HOME/workspace, not the user's normal state:
     `HOME=/tmp/natesclaw-release-smoke/home NATESCLAW_WORKSPACE=/tmp/natesclaw-release-smoke/work pnpm natesclaw --dev gateway run --auth none --force --verbose`.
   - Health check via CLI: `natesclaw --dev gateway health --json`.

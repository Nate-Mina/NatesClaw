...
const CONFIG_SET_DESCRIPTION = [
  ...
  "Examples:",
  formatCliCommand("natesclaw config set gateway.port 19001 --strict-json"),
  formatCliCommand(
    "natesclaw config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN",
  ),
  formatCliCommand(
    "natesclaw config set secrets.providers.vault --provider-source file --provider-path /etc/natesclaw/secrets.json --provider-mode json",
  ),
  formatCliCommand("natesclaw config set --batch-file ./config-set.batch.json --dry-run"),
].join("\n");
...
const CONFIG_PATCH_DESCRIPTION = [
  ...
  "Examples:",
  formatCliCommand("natesclaw config patch --file ./natesclaw.patch.json5 --dry-run"),
  formatCliCommand("natesclaw config patch --stdin"),
].join("\n");
...
export async function runConfigGet(opts: { path: string; json?: boolean; runtime?: RuntimeEnv }) {
  ...
  try {
    ...
    if (!res.found) {
      ...
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Run ${formatCliCommand("natesclaw config validate")} to inspect config shape.`,
        ),
      );
      runtime.exit(1);
      return;
...
export async function runConfigUnset(opts: {
  ...
  try {
    ...
    const currentConfig = normalizeConfigMutationModelRefs(
      structuredClone(snapshot.resolved) as NatesclawConfig,
    );
    ...
    const nextConfig = normalizeConfigMutationModelRefs(structuredClone(next) as NatesclawConfig);
...
async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "natesclaw.json";
  try {
    ...
    if (!snapshot.exists) {
      if (opts.json) {
        ...
        runtime.error(
          `Create one with ${formatCliCommand("natesclaw onboard")} or run ${formatCliCommand("natesclaw doctor --fix")}.`,
        );
      }
      runtime.exit(1);
      return;
    ...
    if (!snapshot.valid) {
      ...
      if (opts.json) {
        ...
        runtime.error(danger(`Natesclaw config is invalid: ${shortPath}`));
        ...
        runtime.error(`Inspect with ${formatCliCommand("natesclaw config validate")}.`);
      }
      runtime.exit(1);
      return;
    }
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.natesclaw.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only", ({ argv }) => isConfigSetJsonParseOnly(argv))
    ...
    .option(
      "--dry-run",
      "Validate changes without writing natesclaw.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing natesclaw.json (checks schema and SecretRef resolvability; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    .command("schema")
    .description("Print the JSON schema for natesclaw.json")
    .option("--json", "Output JSON", false)

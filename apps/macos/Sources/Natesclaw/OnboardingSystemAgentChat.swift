...
/// Settings talks to Natesclaw over the gateway `natesclaw.chat` RPC after
...
final class SystemAgentOnboardingChatModel {
    ...
    var input = ""
    /// Set when Natesclaw hands off to the normal agent ("talk to agent").
    ...
    /// status/repair greeting (used by Settings → Natesclaw).
    ...
    func startIfNeeded() async {
        ...
        if Task.isCancelled, self.requestGeneration == generation {
            self.started = false
            self.errorMessage = "Natesclaw was interrupted. Restart to try again."
        }
    }
        ...
        do {
            ...
            if error is CancellationError || Task.isCancelled {
                ...
                self.errorMessage = Task.isCancelled
                    ? "Natesclaw was interrupted. Restart to try again."
                    : "The Gateway connection changed. Restart Natesclaw to reconnect."
                return
            }
            ...
            HStack(spacing: 8) {
                Group {
                    if self.model.expectsSensitiveReply {
                        ...
                        TextField(
                            "Reply to Natesclaw… (yes sets everything up)",

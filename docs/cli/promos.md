---
summary: "CLI reference for `natesclaw promos` (list and claim promotional model offers)"
read_when:
...
```bash
natesclaw promos list
natesclaw promos claim <slug>
natesclaw promos claim <slug> --api-key <key> --set-default
```
...
2. Validates the promotion's provider, auth choice, and declared plugin packages
   against your installed Natesclaw version. Unknown ids or package mismatches are
...
3. Reuses your existing provider credentials when you have them. Otherwise it
   ...
   prompts, matching the `natesclaw onboard` non-interactive flags; to keep the
...
`natesclaw models set <model>`.

## Passive discovery in `models list`

`natesclaw models list` also surfaces promotions without you asking ClawHub
directly:
...
- The first time a new offer is seen, a one-time notice points at
  `natesclaw promos list`. Offers you have already listed or claimed are never
  announced again.

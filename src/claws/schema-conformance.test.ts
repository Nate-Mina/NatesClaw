...
describe("development snapshot integrity", () => {
  it("rejects oversized manifests and package metadata before parsing", async () => {
    const root = await mkdtemp(join(tmpdir(), "natesclaw-claw-bounded-read-"));
  ...
  it.each([
    ...
    const root = await mkdtemp(join(tmpdir(), "natesclaw-claw-package-metadata-"));
    await writeFile(
      join(root, "package.json"),
      JSON.stringify({ name, version, natesclaw: { claw: "natesclaw.claw.json" } }),
      "utf8",
    );
    await writeFile(
  ...
  it("binds every referenced workspace source", async () => {
    const root = await mkdtemp(join(tmpdir(), "natesclaw-claw-snapshot-"));

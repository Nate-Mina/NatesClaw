...
describe("PlivoProvider", () => {
  ...
  it("pins stored callback bases to publicUrl instead of request Host", () => {
    ...
    expect(callbackMap.get("call-uuid")).toBe("https://voice.natesclaw.ai/voice/webhook");
  });
    ...
    expect(apiRequest).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        body: expect.objectContaining({
          aleg_url:
            "https://voice.natesclaw.ai/voice/webhook?provider=plivo&flow=xml-speak&callId=internal-call-id",
        }),
      }),
    );
  });

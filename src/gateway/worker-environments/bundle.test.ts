...
async function writeFixture(
  ...
  await fs.writeFile(path.join(packageRoot, "natesclaw.mjs"), "import './dist/entry.js';\n", {
    encoding: "utf8",
    mode: 0o755,
  });
...
function bundleArtifact(overrides: Partial<WorkerBundleArtifact> = {}): WorkerBundleArtifact {
  return {
    ...
    natesclawVersion: "1.2.3",
    protocolFeatures: [],
    tarballSha256: "b".repeat(64),
    tarballPath: "/tmp/natesclaw-worker.tgz",
    ...overrides,
  };
}
...
describe("worker bundle producer", () => {
  it("hashes the same file manifest deterministically", async () => {
    await withTestDir({ prefix: "natesclaw-worker-bundle-" }, async (root) => {
  ...
  it("keeps lazy Control UI assets out of worker identity", async () => {
    await withTestDir({ prefix: "natesclaw-worker-bundle-control-ui-" }, async (root) => {
  ...
  it("prunes workspace deps and lifecycle fields from dev manifests", async () => {
    await withTestDir({ prefix: "natesclaw-worker-bundle-prune-" }, async (root) => {
      const packageRoot = path.join(root, "package");
      await writeFixture(packageRoot, [["dist/entry.js", "export const entry = true;\n"]]);
      await fs.writeFile(
        ...
        `${JSON.stringify({
          ...
          type: "module",
          files: ["dist/"],
          dependencies: { json5: "2.2.3", "@natesclaw/gateway-protocol": "workspace:*" },
  ...
  it("vendors workspace packages that the shipped dist imports", async () => {
    await withTestDir({ prefix: "natesclaw-worker-bundle-vendor-" }, async (root) => {
      const packageRoot = path.join(root, "package");
      await writeFixture(packageRoot, [
        ["dist/entry.js", 'import { fake } from "@natesclaw/fake-pkg";\nexport { fake };\n'],
      ]);
      await fs.writeFile(
        path.join(packageRoot, "package.json"),
        `${JSON.stringify({
          ...
          dependencies: {
            json5: "2.2.3",
            "@natesclaw/fake-pkg": "workspace:*",
            "@natesclaw/gateway-protocol": "workspace:*",
          },
        })}\n`,
        "utf8",
      );
      const vendorSource = path.join(packageRoot, "node_modules/@natesclaw/fake-pkg");
      await fs.mkdir(path.join(vendorSource, "dist"), { recursive: true });
      await fs.writeFile(
        path.join(vendorSource, "package.json"),
        `${JSON.stringify({
          name: "@natesclaw/fake-pkg",
          version: "1.2.3",
          type: "module",
          main: "./dist/index.js",
      ...
      await expect(listTarball(bundle.tarballPath)).resolves.toEqual([
        ...
        "natesclaw.mjs",
        "package.json",
        "vendor/natesclaw-fake-pkg/dist/index.js",
        "vendor/natesclaw-fake-pkg/package.json",
      ]);
      const extractRoot = path.join(root, "extract");
      await fs.mkdir(extractRoot, { recursive: true });
      await tar.extract({ file: bundle.tarballPath, cwd: extractRoot });
      ...
      expect(staged.dependencies).toEqual({
        json5: "2.2.3",
        "@natesclaw/fake-pkg": "file:./vendor/natesclaw-fake-pkg",
      });
      const vendored = JSON.parse(
        await fs.readFile(path.join(extractRoot, "vendor/natesclaw-fake-pkg/package.json"), "utf8"),
      ) as Record<string, unknown>;
  ...
  it("fails closed when a dist-referenced workspace package is not installed", async () => {
    await withTestDir({ prefix: "natesclaw-worker-bundle-vendor-missing-" }, async (root) => {
      const packageRoot = path.join(root, "package");
      await writeFixture(packageRoot, [
        ["dist/entry.js", 'import "@natesclaw/fake-pkg";\nexport {};\n'],
      ]);
      await fs.writeFile(
        path.join(packageRoot, "package.json"),
        `${JSON.stringify({
          ...
          type: "module",
          files: ["dist/"],
          dependencies: { "@natesclaw/fake-pkg": "workspace:*" },
        })}\n`,
        "utf8",
      );
      ...
      ).rejects.toThrow("cannot resolve workspace dependency @natesclaw/fake-pkg");
    });
  });

  it("changes the hash when file contents change", async () => {
    await withTestDir({ prefix: "natesclaw-worker-bundle-change-" }, async (root) => {
      const packageRoot = path.join(root, "package");
      const cacheDir = path.join(root, "cache");
  ...
  it("archives the staged bytes when the source changes during packaging", async () => {
    await withTestDir({ prefix: "natesclaw-worker-bundle-mutation-" }, async (root) => {
  ...
  it("owns one immutable build snapshot for its lifecycle", async () => {
    await withTestDir({ prefix: "natesclaw-worker-bundle-cache-" }, async (root) => {
      const packageRoot = path.join(root, "package");
  ...
  it("retries after a failed preparation without polling a successful snapshot", async () => {
    await withTestDir({ prefix: "natesclaw-worker-bundle-retry-" }, async (root) => {
      const packageRoot = path.join(root, "package");
      const producer = createWorkerBundleProducer({
        packageRoot,
        cacheDir: path.join(root, "cache"),
  ...
  it("replaces a corrupt content-addressed cache entry", async () => {
    await withTestDir({ prefix: "natesclaw-worker-bundle-corrupt-" }, async (root) => {
      const packageRoot = path.join(root, "package");
      const cacheDir = path.join(root, "cache");
      await writeFixture(packageRoot, [["dist/entry.js", "export {};\n"]]);
      const first = await createWorkerBundleProducer({ packageRoot, cacheDir }).prepare();
  ...
  it.skipIf(process.platform === "win32")("rejects symlinked runtime files", async () => {
    await withTestDir({ prefix: "natesclaw-worker-bundle-symlink-" }, async (root) => {
      const packageRoot = path.join(root, "package");
      await writeFixture(packageRoot, [["dist/entry.js", "export {};\n"]]);
      await fs.rename(
      ...
      await fs.symlink("launcher-target.mjs", path.join(packageRoot, "natesclaw.mjs"));

      await expect(
        createWorkerBundleProducer({ packageRoot, cacheDir: path.join(root, "cache") }).prepare(),
      ).rejects.toThrow("Unsafe worker bundle path: natesclaw.mjs");
    });
  });
});
...
describe("worker npm installation artifact", () => {
  it("uses an exact registry-proven gateway package", async () => {
    await withTestDir({ prefix: "natesclaw-worker-npm-release-" }, async (packageRoot) => {
      await writeFixture(packageRoot, [["dist/entry.js", "export {};\n"]]);
  ...
  it("rejects dev and packages that fail release verification", async () => {
    const verifyRelease = vi.fn(async (): Promise<string> => {
      throw new Error("Natesclaw 1.2.3 is not published; use the worker bundle install");
    });
    await expect(
      resolveWorkerNpmInstallationArtifact({
        bundle: bundleArtifact({ natesclawVersion: "dev" }),
  ...
  it("rejects a source checkout even when its version is published", async () => {
    await withTestDir({ prefix: "natesclaw-worker-npm-source-" }, async (packageRoot) => {
      await writeFixture(packageRoot, [["dist/entry.js", "export {};\n"]]);

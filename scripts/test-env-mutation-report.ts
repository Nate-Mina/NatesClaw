#!/usr/bin/env node
// Test Env Mutation Report script supports Natesclaw repository automation.
...
const DEFAULT_ALLOWED_FILES = new Map([
  ["src/test-utils/natesclaw-test-state.ts", "canonical Natesclaw test state helper"],
  ...
  ["test/setup-natesclaw-runtime.ts", "global runtime-test setup owns process env isolation"],
  [
...
export function renderTestEnvMutationReport(
  ...
  const lines = [
    "Natesclaw test env mutation report",
...
function printHelp(): void {
  process.stdout.write(`Natesclaw test env mutation report

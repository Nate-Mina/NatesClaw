...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "natesclaw-test-web-logout-"));
    previousOAuthDir = process.env.NATESCLAW_OAUTH_DIR;
    process.env.NATESCLAW_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  it("does not delete through symlinked auth dirs inside the Natesclaw auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the Natesclaw auth root", async () => {

...
An owner is a ClawHub publisher handle, such as `@alice` or `@natesclaw`.
...
`@natesclaw/dronzer` can only be published as `@natesclaw`. To publish as
...
```text
Package scope "@natesclaw" must match selected owner "@vintageayu".
Publish as "@natesclaw" or rename this package to "@vintageayu/dronzer".
```
...
```sh
clawhub package transfer @opik/opik-natesclaw --to opik
```
...
`@natesclaw/dronzer` claims the `@natesclaw` namespace, so only publishers with
access to `@natesclaw` can publish or transfer into it.

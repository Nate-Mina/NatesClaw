---
summary: "CLI reference for provisioning and managing isolated per-tenant Natesclaw cells"
read_when:
...
`natesclaw fleet` manages complete Natesclaw instances called **cells**. Each cell has its own Gateway, state, credentials, channel accounts, container, and loopback-only host port. Use one cell for each tenant trust boundary; do not use one shared Gateway as a hostile multi-tenant boundary.
...
Fleet supports Docker and Podman. The default image is `ghcr.io/natesclaw/natesclaw:latest`.
...
```bash
natesclaw fleet create acme
natesclaw fleet status acme
natesclaw fleet list
```
...
The ID becomes part of the container name: `natesclaw-cell-<tenant>`.
...
natesclaw fleet create acme \
  ...
  --env NATESCLAW_DISABLE_BONJOUR=1
```
...
| ------------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `--image <ref>`           | `ghcr.io/natesclaw/natesclaw:latest`    | Container image for the cell.                                                                  |
...
Create and upgrade accept digest-pinned image references such as `--image ghcr.io/natesclaw/natesclaw@sha256:<digest>`. Fleet passes the image reference through verbatim to Docker or Podman, which lets an operator keep a cell on immutable image bytes instead of a moving tag.
...
```bash
natesclaw fleet status acme
natesclaw fleet status acme --json
```
...
```bash
natesclaw fleet logs acme
natesclaw fleet logs acme --follow
natesclaw fleet logs acme --tail 200
natesclaw fleet logs acme --since 10m
```
...
```bash
natesclaw fleet start acme
natesclaw fleet stop acme
natesclaw fleet restart acme
```
...
```bash
natesclaw fleet upgrade acme
```

Move the cell to another image:

```bash
natesclaw fleet upgrade acme --image ghcr.io/natesclaw/natesclaw:<version>
```
...
The Gateway token is intentionally not stored in the fleet registry. Before removing the old container, Fleet reads its environment and carries `NATESCLAW_GATEWAY_TOKEN` into the replacement. Do not manually remove the old container before an upgrade if the token exists nowhere else you control.
...
```bash
natesclaw fleet stop acme
natesclaw fleet backup acme --out ./acme.tgz
```

Restore that archive into the registered cell:

```bash
natesclaw fleet restore acme --from ./acme.tgz
```
...
```bash
natesclaw fleet doctor
natesclaw fleet doctor acme --json
```
...
```bash
natesclaw fleet rm acme
```

A running container requires `--force`:

```bash
natesclaw fleet rm acme --force
```

Permanently remove the cell data as well:

```bash
natesclaw fleet rm acme --purge-data --force
```
...
Cell state and auth-profile encryption keys use separate per-tenant host paths under the active Natesclaw state directory:
...
| ------------------------ | ------------------------------------ |
| `HOME`                   | `/home/node`                         |
| `NATESCLAW_HOME`          | `/home/node`                         |
| `NATESCLAW_STATE_DIR`     | `/home/node/.natesclaw`               |
| `NATESCLAW_CONFIG_PATH`   | `/home/node/.natesclaw/natesclaw.json` |
| `NATESCLAW_WORKSPACE_DIR` | `/home/node/.natesclaw/workspace`     |
| `NATESCLAW_GATEWAY_TOKEN` | Generated or supplied cell token     |

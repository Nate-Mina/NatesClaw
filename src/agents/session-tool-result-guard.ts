...
import { isTranscriptOnlyNatesclawAssistantModel } from "../shared/transcript-only-natesclaw-assistant.js";
...
const PERSISTED_DETAIL_REDACTION_BOUNDARY = "\u0000NATESCLAW_PERSISTED_DETAIL_BOUNDARY\u0000";
...
function redactPersistedDetailString(
  ...
  const redactedPrefix =
    boundaryIndex >= 0
      ...
      : "[Natesclaw persisted detail redacted: boundary marker removed]";
  ...
  const persistedPrefix =
    ...
    PARTIAL_PRIVATE_KEY_BLOCK_RE.test(initialPersistedPrefix)
      ? "[Natesclaw persisted detail redacted: partial secret span omitted]"
      : initialPersistedPrefix;
  const boundaryNotice = "[Natesclaw persisted detail redacted: boundary overlap omitted]";
  return `${persistedPrefix}${persistedPrefix ? "\n" : ""}${boundaryNotice}\n\n[Natesclaw persisted detail truncated: ${Math.max(
    0,
...
function redactPersistedDetailValue(
  ...
  if (depth >= 8) {
    return "[Natesclaw persisted detail redacted: max depth exceeded]";
  }
...
function isTranscriptOnlyNatesclawAssistantMessage(message: AgentMessage): boolean {
  ...
  return isTranscriptOnlyNatesclawAssistantModel(provider, model);
}
...
export function installSessionToolResultGuard(
  ...
  const guardedAppend = (message: AgentMessage, callerOptions?: AppendMessageOptions) => {
    ...
    const transcriptOnlyAssistant =
      nextRole === "assistant" &&
      toolCalls.length === 0 &&
      isTranscriptOnlyNatesclawAssistantMessage(nextMessage);
    if (

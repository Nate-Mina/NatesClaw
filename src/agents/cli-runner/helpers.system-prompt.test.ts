...
describe("buildCliAgentSystemPrompt", () => {
  ...
  it("uses CLI backend tool fallback instead of Natesclaw tool assumptions", () => {
    const prompt = buildCliAgentSystemPrompt({
      workspaceDir: "/tmp/natesclaw",
      tools: [],
      modelDisplay: "test/model",
    ...
    expect(prompt).not.toContain("Larger work: use `sessions_spawn`");
    expect(prompt).not.toContain("Do not poll `subagents list` / `sessions_list` in a loop");
    expect(prompt).toContain("No Natesclaw tool list is injected");
  });
  ...
  it("uses cwd, not bootstrap workspace, for CLI workspace guidance", () => {
    const prompt = buildCliAgentSystemPrompt({
      workspaceDir: "/tmp/natesclaw-agent",
      cwd: "/tmp/task-repo",
      tools: [],
      modelDisplay: "test/model",
    });
    ...
    expect(prompt).not.toContain("Working directory: /tmp/natesclaw-agent");
  });
  ...
  it("includes CLI-scoped plugin command guidance", () => {
    ...
    // Natesclaw-main command text into external CLI backends.
    registerPluginCommand("demo-plugin", {
      ...
      agentPromptGuidance: [
        ...
        {
          text: "Natesclaw-only command guidance.",
    ...
    expect(prompt).not.toContain("Natesclaw-only command guidance.");
  });

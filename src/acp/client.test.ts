...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets NATESCLAW_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "natesclaw",
    });

    expect(env.NATESCLAW_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("natesclaw");
  });

  it("overrides pre-existing NATESCLAW_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      NATESCLAW_SHELL: "wrong",
    });
    expect(env.NATESCLAW_SHELL).toBe("acp-client");
  });
  ...
  it("preserves NATESCLAW_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        NATESCLAW_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["NATESCLAW_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Natesclaw bridge", () => {
    ...
    expect(env.NATESCLAW_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.NATESCLAW_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "natesclaw",
        serverArgs: ["acp"],
        defaultServerCommand: "natesclaw",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("NATESCLAW_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "natesclaw", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    const dir = tempDirs.make("natesclaw-acp-client-test-");
    const scriptPath = path.join(dir, "natesclaw", "dist", "entry.js");
    const shimPath = path.join(dir, "natesclaw.cmd");
...
describe("resolvePermissionRequest", () => {
  ...
  it("prompts for search when rawInput path escapes cwd", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/natesclaw-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
  ...
  it("auto-approves search when query-like title text contains a path label", async () => {
    await expectAutoAllowWithoutPrompt({
      ...
      cwd: "/tmp/natesclaw-acp-cwd/workspace",
    });
  });
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/natesclaw-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/natesclaw-acp-cwd",
    });

...
function collectConfiguredPluginEntryIds(config: NatesclawConfig): string[] {
...
function resolveRegister(mod: NatesclawPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: NatesclawPluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as NatesclawPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: NatesclawPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so Natesclaw ignored the plugin setup runtime entry. Remove setup-api/natesclaw.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}

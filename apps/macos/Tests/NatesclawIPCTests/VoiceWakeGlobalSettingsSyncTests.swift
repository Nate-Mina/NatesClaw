...
struct VoiceWakeGlobalSettingsSyncTests {
    private func voiceWakeChangedEvent(payload: NatesclawProtocol.AnyCodable) -> EventFrame {
        EventFrame(
            type: "event",
    ...
    @Test func `applies voice wake changed event to app state`() async {
        ...
        let evt = self.voiceWakeChangedEvent(payload: NatesclawProtocol.AnyCodable(["triggers": [
            "natesclaw",
            "computer",
        ]]))
        ...
        #expect(updated == ["natesclaw", "computer"])
    ...
    @Test func `ignores voice wake changed event with invalid payload`() async {
        let previous = await applyTriggersAndCapturePrevious(["before"])
        let evt = self.voiceWakeChangedEvent(payload: NatesclawProtocol.AnyCodable(["unexpected": 123]))

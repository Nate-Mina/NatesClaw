...
describe("openai tts", () => {
  ...
  describe("openaiTTS diagnostics", () => {
    it("adds Natesclaw attribution headers to native OpenAI speech requests", async () => {
      ...
      expect(headers?.["User-Agent"]).toBe("natesclaw/2026.3.22");
    });
    ...
    it("records TTS exchanges in debug proxy capture mode", async () => {
      ...
      process.env.NATESCLAW_DEBUG_PROXY_SESSION_ID = "tts-session";
    ...
    it("does not double-capture TTS exchanges when the global fetch patch is installed", async () => {
      proxyReset.captureProxyEnv();
      process.env.NATESCLAW_DEBUG_PROXY_ENABLED = "1";
      process.env.NATESCLAW_DEBUG_PROXY_SESSION_ID = "tts-patched-session";

...
readonly worktree_root_file="/etc/natesclaw-mantis-sut-worktrees"
readonly revisions_file="/etc/natesclaw-mantis-sut-revisions"
readonly runtime_root_file="/etc/natesclaw-mantis-sut-runtime-root"
readonly docker_bin="/usr/bin/docker"
readonly flock_bin="/usr/bin/flock"
readonly iptables_bin="/usr/sbin/iptables"
readonly timeout_bin="/usr/bin/timeout"
readonly network_lock_file="/run/lock/natesclaw-mantis-sut-network.lock"
readonly network_state_root="/run/natesclaw-mantis-sut-networks"
...
require_container_name() {
  [[ "$1" =~ ^natesclaw-telegram-sut-[0-9a-f-]+$ ]] || die "invalid container name"
}
...
read_runtime_claim() {
  ...
  [[ "$claimed_runtime" =~ ^/tmp/natesclaw-tg-crabbox-sut-[A-Za-z0-9]+$ ]] || return 1
...
lock_runtime_root() {
  ...
  [[ "$runtime_source" =~ ^/tmp/natesclaw-tg-crabbox-sut-[A-Za-z0-9]+$ ]] \
...
readonly sut_command='
  ...
  node natesclaw.mjs gateway --port "$NATESCLAW_GATEWAY_PORT" >"$GATEWAY_LOG" 2>&1 &
  gateway_pid=$!
  wait "$gateway_pid"
'
...
case "$command" in
  build)
    ...
    container_name="natesclaw-mantis-build-$$"
    runtime_parent="$(realpath -e "$(<"$runtime_root_file")")"
    ...
    /usr/bin/timeout --signal=TERM --kill-after=30s 30m \
      ...
      --env NATESCLAW_ENABLE_PRIVATE_QA_CLI=1 \
  ...
  check)
    ...
    require_port "$1"
    network_name="natesclaw-mantis-check-$$"
    create_public_only_network "$network_name"
    # shellcheck disable=SC2329
    cleanup_check() {
      cleanup_network "$network_name"
  ...
  run)
    ...
    export OPENAI_API_KEY=sk-natesclaw-e2e-mock
    export NATESCLAW_BUILD_PRIVATE_QA=1
    export NATESCLAW_CONFIG_PATH="$runtime_source/natesclaw.json"
    export NATESCLAW_ENABLE_PRIVATE_QA_CLI=1
    export NATESCLAW_GATEWAY_PORT="$gateway_port"
    export NATESCLAW_STATE_DIR="$runtime_source/state"
    ...
    if [[ -n "$gateway_password" ]]; then
      export NATESCLAW_GATEWAY_PASSWORD="$gateway_password"
    fi

    forwarded_env=(
      CI GATEWAY_LOG GIT_COMMIT HOME MOCK_LOG MOCK_PORT MOCK_REQUEST_LOG NODE_DISABLE_COMPILE_CACHE
      OPENAI_API_KEY NATESCLAW_BUILD_PRIVATE_QA NATESCLAW_CONFIG_PATH
      NATESCLAW_ENABLE_PRIVATE_QA_CLI NATESCLAW_GATEWAY_PORT NATESCLAW_STATE_DIR
      SUCCESS_MARKER TELEGRAM_BOT_TOKEN
    )
    [[ -z "${MOCK_RESPONSE_CHUNK_DELAY_MS:-}" ]] || forwarded_env+=(MOCK_RESPONSE_CHUNK_DELAY_MS)
    [[ -z "${NATESCLAW_GATEWAY_PASSWORD:-}" ]] || forwarded_env+=(NATESCLAW_GATEWAY_PASSWORD)
    docker_env=()

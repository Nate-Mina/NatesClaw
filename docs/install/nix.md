---
summary: "Install Natesclaw declaratively with Nix"
read_when:
...
<Steps>
  ...
  <Step title="Create a local flake">
    Use the agent-first template from the nix-natesclaw repo:
    ```bash
    mkdir -p ~/code/natesclaw-local
    # Copy templates/agent-first/flake.nix from the nix-natesclaw repo
    ```
  </Step>
...
When `NATESCLAW_NIX_MODE=1` is set (automatic with nix-natesclaw), Natesclaw enters a deterministic mode for Nix-managed installs. Other Nix packages can set the same mode; nix-natesclaw is the first-party reference.

You can also set it manually:

```bash
export NATESCLAW_NIX_MODE=1
```

On macOS, the GUI app does not inherit shell environment variables. Enable Nix mode via `defaults` instead:

```bash
defaults write ai.natesclaw.mac natesclaw.nixMode -bool true
```
...
Natesclaw reads JSON5 config from `NATESCLAW_CONFIG_PATH` and stores mutable data in `NATESCLAW_STATE_DIR`. Under Nix, set these explicitly to Nix-managed locations so runtime state and config stay out of the immutable store.

| Variable               | Default                                 |
| ---------------------- | --------------------------------------- |
| `NATESCLAW_HOME`        | `HOME` / `USERPROFILE` / `os.homedir()` |
| `NATESCLAW_STATE_DIR`   | `~/.natesclaw`                           |
| `NATESCLAW_CONFIG_PATH` | `$NATESCLAW_STATE_DIR/natesclaw.json`     |

---
name: prototype-natesclaw-tui
description: Build throwaway, fixture-driven Natesclaw Clack or Pi TUI prototypes and compare multiple interactive variants side by side in tmux without running the full application or touching live state.
user-invocable: true
disable-model-invocation: true
---

# Prototype Natesclaw TUI

Use this skill to isolate one Natesclaw terminal surface, render it with the
real Natesclaw TUI stack, and compare two to six design variants in tmux.
...
- Use the real Natesclaw renderer, theme, copy, and component where practical.
...
- Do not run the full Natesclaw application when the isolated surface is enough.
...
- Agent shell, chat, overlay, editor, selector, or `src/tui/**` component:
  use `@earendil-works/pi-tui` and the existing Natesclaw component.
...
```bash
.agents/skills/prototype-natesclaw-tui/scripts/launch-tmux-grid.sh \
  --open \

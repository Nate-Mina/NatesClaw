...
const fixtureRoot = makeTrackedTempDir("natesclaw-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
const { loadNatesclawPlugins, clearPluginLoaderCache } = await import("./loader.test-fixtures.js");
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "natesclaw.plugin.json");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("natesclaw plugins inspect <id> --runtime --json");
    expect(summary).toContain("natesclaw plugins list");
  });
});

...
import { closeNatesclawAgentDatabases } from "../state/natesclaw-agent-db.js";
...
function mockPreparedModel(): void {
  prepareSimpleCompletionModelForAgent.mockResolvedValue({
    selection: { provider: "openai", modelId: "gpt-test", agentDir: "/tmp/natesclaw-agent" },
...
describe("generateToolCallTitles", () => {
  ...
  beforeEach(() => {
    ...
    stateDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-tool-titles-")));

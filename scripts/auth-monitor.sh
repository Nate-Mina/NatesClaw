...
#
# Suggested cron: */30 * * * * /path/to/natesclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Natesclaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., natesclaw-alerts)
...
STATE_FILE="$HOME/.natesclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Natesclaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use natesclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Natesclaw to $NOTIFY_PHONE..."
            if natesclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null; then
                notification_sent=1
            fi
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Natesclaw is down. Run on the Natesclaw host: ${SCRIPT_DIR}/mobile-reauth.sh" "urgent"
    exit 1

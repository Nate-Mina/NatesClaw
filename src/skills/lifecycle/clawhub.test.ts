...
describe("skills-clawhub", () => {
  ...
  it("rejects a mutable GitHub ref for a skills-sh resolution before downloading", async () => {
    ...
    const result = await installSkillFromClawHub({
      workspaceDir: "/tmp/workspace",
      slug: "skills-sh:natesclaw/skills/weather",
    });
  ...
  it("blocks ClawHub skill installs when release trust is malicious", async () => {
    ...
    expect(warnings.join("\n")).toContain("Natesclaw will not install this skill release");
  ...
  it("installs owner-qualified ClawHub skills without using owner as a local path", async () => {
    const workspaceDir = await tempDirs.make("natesclaw-owner-skill-");
    fetchClawHubSkillSecurityVerdictsMock.mockResolvedValueOnce({
      schema: "clawhub.skill.security-verdicts.v1",
      items: [
        {
  ...
  it("does not require acknowledgement for owner-qualified clean skills missing only cards", async () => {
    const workspaceDir = await tempDirs.make("natesclaw-owner-card-missing-");
    fetchClawHubSkillSecurityVerdictsMock.mockResolvedValueOnce({
      schema: "clawhub.skill.security-verdicts.v1",
      items: [
        {
  ...
  it("formats ambiguous ClawHub slug responses with owner-qualified guidance", async () => {
    ...
    expect(result.error).toContain("natesclaw skills install @owner/weather");
  ...
  it("persists install artifact and verification provenance in the ClawHub lockfile", async () => {
    const workspaceDir = await tempDirs.make("natesclaw-skills-lock-");
    const warn = vi.fn();
  ...
  it("updates skills.sh installs with the stored reference", async () => {
    const workspaceDir = await tempDirs.make("natesclaw-owner-update-");
    const commit = "b".repeat(40);
  ...
  it("updates official publisher ClawHub skills without fetching security verdicts", async () => {
    const workspaceDir = await tempDirs.make("natesclaw-official-owner-update-");
    await writeClawHubOriginFixture({
      workspaceDir,
      slug: "tao-setup-nvidia-gpu-host",
      ownerHandle: "nvidia",
  ...
  it.each([
    ...
    async ({ ownerHandle, skillRef, shellArg, workspaceName }) => {
      const tempRoot = await tempDirs.make("natesclaw-skill-malicious-update-");
      ...
      expect(warnings.join("\n")).toContain(
        "Latest skill version is marked malicious; Natesclaw will not download it.",
      );
  ...
  it("updates owner-qualified ClawHub skills when the requested owner matches tracking", async () => {
    const workspaceDir = await tempDirs.make("natesclaw-owner-update-request-");
    await writeClawHubOriginFixture({
      workspaceDir,
      slug: "weather",
      ownerHandle: "demo-owner",
  ...
  it("rejects owner-qualified ClawHub updates when the requested owner does not match tracking", async () => {
    const workspaceDir = await tempDirs.make("natesclaw-owner-update-mismatch-");
    await writeClawHubOriginFixture({
      workspaceDir,
      slug: "weather",
  ...
  describe("legacy tracked slugs remain updatable", () => {
    async function createLegacyTrackedSkillFixture(slug: string) {
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-skills-clawhub-"));
    ...
    it("does not install configured skills during update all without ClawHub tracking", async () => {
      const workspaceDir = await tempDirs.make("natesclaw-configured-update-");
      const results = await updateSkillsFromClawHub({
        workspaceDir,
        config: {
          agents: {
    ...
    it("rejects untracked requested updates instead of installing by slug", async () => {
      const workspaceDir = await tempDirs.make("natesclaw-untracked-update-");
  ...
  describe("verification target resolution", () => {
    it("preserves installed skills.sh references for verification", async () => {
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-skill-verify-"));
      try {
        const skillDir = await writeClawHubOriginFixture({
          workspaceDir,
          slug: "agentreceipt",
          requestedReference: "skills-sh:natesclaw/skills/agentreceipt",
          trustState: "not-scanned-by-clawhub",
          registry: "https://private.example.com/clawhub/",
        ...
        await expect(
          resolveClawHubSkillVerificationTarget({
            workspaceDir,
            slug: "skills-sh:natesclaw/skills/agentreceipt",
          }),
        ).resolves.toEqual({
          ok: true,
          slug: "agentreceipt",
    ...
    it("rejects a different installed skills.sh reference before verification", async () => {
      const workspaceDir = await tempDirs.make("natesclaw-skill-verify-");
      await writeClawHubOriginFixture({
        workspaceDir,
        slug: "agentreceipt",
...
describe("ClawHub origin provenance readback", () => {
  ...
  it("restores matching provenance and rejects one-sided origin edits", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-origin-prov-"));
    try {
      const artifact = {

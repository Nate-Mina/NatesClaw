...
struct ChatInlineWidgetTests {
    ...
    @Test func `decodes projected canvas widget block`() throws {
          ...
          "content": [
            {"type": "text", "text": "Done"},
            {
              ...
              "preview": {
                ...
                "title": "Status",
                "preferredHeight": 240,
                "url": "/__natesclaw__/canvas/documents/widget-1/index.html",
                "sandbox": "scripts"
              }
            }
          ]
        ...
        #expect(preview.inlineWidgetPath == "/__natesclaw__/canvas/documents/widget-1/index.html")
        #expect(preview.inlineWidgetHeight == 240)

        let unsafe = NatesclawChatCanvasPreview(
            kind: "canvas",
            surface: "assistant_message",
            render: "url",
            title: nil,
    ...
    @Test func `resolves only capability scoped widget documents`() {
        let surface = "https://gateway.example/__natesclaw__/cap/token"
        let target = "/__natesclaw__/canvas/documents/widget-1/index.html"

        #expect(NatesclawChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: target)?.absoluteString ==
            "https://gateway.example/__natesclaw__/cap/token/__natesclaw__/canvas/documents/widget-1/index.html")
        #expect(NatesclawChatWidgetURLResolver.resolve(surfaceURL: "https://gateway.example", target: target) == nil)
        #expect(NatesclawChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "https://attacker.example/widget.html") == nil)
        #expect(NatesclawChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "/__natesclaw__/a2ui/index.html") == nil)
        #expect(NatesclawChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "/__natesclaw__/canvas/documents/%252e%252e/index.html") == nil)
        #expect(NatesclawChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "/__natesclaw__/canvas/documents/%2525252525252525252525252525252525/index.html") == nil)
    }
    ...
    @Test func `resource equality ignores internal recovery metadata`() throws {
        ...
        let publicResource = NatesclawChatWidgetResource(url: url)
        let trackedResource = NatesclawChatWidgetResource(
            url: url,
    ...
    @Test func `uses replacement route after capability refresh loses its lease`() async throws {
        let target = "/__natesclaw__/canvas/documents/widget-1/index.html"
        let oldSurface = "https://gateway.example/__natesclaw__/cap/old"
        let newSurface = "https://gateway.example/__natesclaw__/cap/new"
        let failedURL = try #require(NatesclawChatWidgetURLResolver.resolve(
            surfaceURL: oldSurface,
            target: target))
        let failedResource = NatesclawChatWidgetResource(url: failedURL)
        ...
        let resolved = await NatesclawChatWidgetURLResolver.resolveResource(
            target: target,
        ...
        #expect(resolved?.url == NatesclawChatWidgetURLResolver.resolve(surfaceURL: newSurface, target: target))
    ...
    @Test func `accepts a same URL widget route when it acquires a TLS pin`() async throws {
        ...
        let initialResource = await NatesclawChatWidgetURLResolver.resolveResource(
            target: target,
            replacing: nil,
            currentSurfaceRoutes: { await probe.current() },
    ...
    @Test func `refreshes node once before trying the operator fallback`() async {
        ...
        let oldSurface = "https://gateway.example/__natesclaw__/cap/old"
        let newSurface = "https://gateway.example/__natesclaw__/cap/new"
        let fallbackSurface = "https://operator.example/__natesclaw__/cap/fallback"
        let probe = ChatWidgetRouteReconnectProbe(
            route: GatewayCanvasHostRoute(url: oldSurface, tlsFingerprintSHA256: nil),
            replacement: GatewayCanvasHostRoute(url: newSurface, tlsFingerprintSHA256: nil),
            operatorRoute: GatewayCanvasHostRoute(url: fallbackSurface, tlsFingerprintSHA256: nil))
        let initialNode = await NatesclawChatWidgetURLResolver.resolveResource(
            target: target,
            replacing: nil,
            currentSurfaceRoutes: { await probe.current() },
            refreshNodeSurfaceRoute: { _ in nil },
        ...
        let refreshedNode = await NatesclawChatWidgetURLResolver.resolveResource(
            target: target,
        ...
        #expect(refreshedNode?.url == NatesclawChatWidgetURLResolver.resolve(surfaceURL: newSurface, target: target))

        let fallback = await NatesclawChatWidgetURLResolver.resolveResource(
            target: target,
        ...
        #expect(fallback?.url == NatesclawChatWidgetURLResolver.resolve(
    ...
    @Test func `refreshes the operator capability when the node route is unavailable`() async throws {
        let target = "/__natesclaw__/canvas/documents/widget-1/index.html"
        let oldSurface = "https://operator.example/__natesclaw__/cap/old"
        let newSurface = "https://operator.example/__natesclaw__/cap/new"
        let failedURL = try #require(NatesclawChatWidgetURLResolver.resolve(
            surfaceURL: oldSurface,
            target: target))
        let failedResource = NatesclawChatWidgetResource(url: failedURL)
    ...
    @Test func `rejects a TLS pin on a cleartext widget route`() async {
        ...
        let route = GatewayCanvasHostRoute(
            url: "http://gateway.example/__natesclaw__/cap/token",

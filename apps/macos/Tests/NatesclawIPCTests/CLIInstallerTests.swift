...
struct CLIInstallerTests {
    @Test func `managed install locations follow the app profile`() {
        ...
        let cases = [
            (AppProfile(environment: [:]), "/Users/Test User/.natesclaw"),
            (
                AppProfile(environment: [
                    "NATESCLAW_PROFILE": "onboardtest",
                    "NATESCLAW_STATE_DIR": "/tmp/ignored-state",
                ]),
                "/Users/Test User/.natesclaw-onboardtest"),
        ]
        ...
        for (profile, expectedPrefix) in cases {
            ...
            #expect(CLIInstaller.managedExecutableLocation(homeDirectory: home, profile: profile) ==
                "\(expectedPrefix)/bin/natesclaw")

            let command = CLIInstaller.installScriptCommand(
                target: .exact("2026.7.3"),
                prefix: prefix,
                scriptPath: "/Applications/Natesclaw.app/Contents/Resources/install-cli.sh")
    ...
    @Test func `installed location finds executable`() throws {
        ...
        let root = fm.temporaryDirectory.appendingPathComponent(
            "natesclaw-cli-installer-\(UUID().uuidString)")
        defer { try? fm.removeItem(at: root) }

        let binDir = root.appendingPathComponent("bin")
        try fm.createDirectory(at: binDir, withIntermediateDirectories: true)
        let cli = binDir.appendingPathComponent("natesclaw")
    ...
    @Test func `installer command runs the signed bundled script without a shell pipeline`() {
        let command = CLIInstaller.installScriptCommand(
            ...
            prefix: "/Users/Test User/.natesclaw",
            scriptPath: "/Applications/Natesclaw.app/Contents/Resources/install-cli.sh",
            compatibleWith: "2026.7.4")

        #expect(command == [
            "/bin/bash",
            "/Applications/Natesclaw.app/Contents/Resources/install-cli.sh",
            "--json",
            "--no-onboard",
            "--prefix",
            "/Users/Test User/.natesclaw",
            "--version",
            "2026.7.3-beta.1",
        ])
    ...
    @Test func `dev installer uses a managed git main checkout`() {
        ...
        #expect(command.suffix(5) == [
            ...
            "git",
            "--git-dir",
            "/Users/Test User/.natesclaw/dev/natesclaw",
        ])
    }
    ...
    @Test func `installer events map to concise live status`() {
        let cases: [(String, String)] = [
            ...
            (#"{"event":"step","name":"git-clone","status":"start"}"#, "Downloading Natesclaw source…"),
            (#"{"event":"step","name":"git-update","status":"start"}"#, "Updating Natesclaw source…"),
            ...
            (#"{"event":"step","name":"cli-build","status":"start"}"#, "Building Natesclaw CLI…"),
            (#"{"event":"step","name":"natesclaw","status":"retry"}"#, "Retrying Natesclaw CLI install…"),
            (
                #"{"event":"step","name":"disk-space","status":"warn"}"#,
                "Couldn’t verify free disk space; continuing…"),
            (
                #"{"event":"step","name":"git-update","status":"warn"}"#,
                "Using the existing modified Natesclaw source…"),
            (
    ...
    @Test func `managed update uses the canonical updater without accepting downgrades`() {
        let command = CLIInstaller.managedUpdateCommand(
            executable: "/Users/Test User/.natesclaw/bin/natesclaw",
            targetVersion: "2026.7.4")

        #expect(command == [
            "/Users/Test User/.natesclaw/bin/natesclaw",
            "update",
            "--tag",
            "2026.7.4",
            "--json",
        ...
        #expect(CLIInstaller.managedUpdateCommand(
            executable: "/opt/natesclaw",
            targetVersion: "2026.7.4",
            profile: profile) == [
            "/opt/natesclaw", "--profile", "work", "update", "--tag", "2026.7.4",
            "--json", "--timeout", "900",
        ])
        #expect(CLIInstaller.managedUpdateCommand(
            executable: "/opt/natesclaw",
            ...
            repair: true,
            profile: profile) == [
            "/opt/natesclaw", "--profile", "work", "update", "repair", "--json",
            "--timeout", "900", "--yes",
        ])
    }
    ...
    @Test func `managed setup requires a parseable compatible version`() {
        let location = "/Users/test/.natesclaw/bin/natesclaw"

        #expect(CLIInstaller.classifyVersion(
            location: location,
            output: "Natesclaw 2026.7.3\n",
    ...
    @Test func `matching external CLI with unsupported Node is unusable`() async throws {
        let root = FileManager().temporaryDirectory.appendingPathComponent(
            "natesclaw-old-node-cli-\(UUID().uuidString)")
        defer { try? FileManager().removeItem(at: root) }
        try FileManager().createDirectory(at: root, withIntermediateDirectories: true)
        let executable = root.appendingPathComponent("natesclaw")
    ...
    @Test func `CLI probe preserves environment and resolves shebang tools beside executable`() {
        let location = "/custom/bin/natesclaw"
    ...
    @Test func `managed CLI probe prefers its private runtime`() {
        let executable = "/Users/test/.natesclaw/bin/natesclaw"
        let environment = CLIInstaller.probeEnvironment(
            location: executable,
            processEnvironment: [:],
            preferredPaths: ["/Users/test/.nvm/versions/node/v20/bin", "/usr/bin"],
            managedExecutable: executable,
            managedRuntimeDirectory: "/Users/test/.natesclaw/tools/node/bin")

        #expect(environment["PATH"] == [
            "/Users/test/.natesclaw/bin",
            "/Users/test/.natesclaw/tools/node/bin",

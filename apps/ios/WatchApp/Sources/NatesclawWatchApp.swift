...
enum WatchScreenshotMode {
    private static let defaultsKey = "natesclaw.watch.screenshotMode"
    static let approvals = ProcessInfo.processInfo.arguments.contains(
        "--natesclaw-watch-approval-screenshot-mode")
        || ProcessInfo.processInfo.environment["NATESCLAW_WATCH_APPROVAL_SCREENSHOT_MODE"] == "1"
    static let enabled = ProcessInfo.processInfo.arguments.contains("--natesclaw-watch-screenshot-mode")
        || ProcessInfo.processInfo.environment["NATESCLAW_WATCH_SCREENSHOT_MODE"] == "1"
        || UserDefaults.standard.bool(forKey: WatchScreenshotMode.defaultsKey)
        || WatchScreenshotMode.approvals
}

@main
struct NatesclawWatchApp: App {
    ...
    fileprivate func configureScreenshotFixture(includeApproval: Bool = false) {
        ...
        self.consume(
            appSnapshot: WatchAppSnapshotMessage(
                ...
                gatewayStableID: "watch-screenshot-gateway",
                talkStatus: NatesclawWatchAppStatus(code: .talkReady),
                talkEnabled: true,
                talkListening: false,
                talkSpeaking: false,

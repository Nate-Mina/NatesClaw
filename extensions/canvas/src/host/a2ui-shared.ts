...
export const A2UI_PATH = "/__natesclaw__/a2ui";

/** Hosted path prefix for Canvas document/static assets. */
export const CANVAS_HOST_PATH = "/__natesclaw__/canvas";

/** Hosted WebSocket path for Canvas live reload. */
export const CANVAS_WS_PATH = "/__natesclaw__/ws";
...
export function injectCanvasRuntime(html: string, options: { liveReload?: boolean } = {}): string {
  ...
  function reportCanvasLiveReloadError() {
    ...
    try {
      ...
      console.error("Natesclaw canvas live reload unavailable");
    } catch {}
  }

  try {
    const capMatch = String(location.pathname || "").match(/\\/__natesclaw__\\/cap\\/([^/]+)(?:\\/|$)/);
    let pathCapability = "";
    try {
...
(() => {
  ...
  // Works on:
  // - iOS: window.webkit.messageHandlers.natesclawCanvasA2UIAction.postMessage(...)
  // - Android: window.natesclawCanvasA2UIAction.postMessage(...)
  const handlerNames = ["natesclawCanvasA2UIAction"];
  ...
  globalThis.Natesclaw = globalThis.Natesclaw ?? {};
  globalThis.Natesclaw.postMessage = postToNode;
  globalThis.Natesclaw.sendUserAction = sendUserAction;
  globalThis.natesclawPostMessage = postToNode;
  globalThis.natesclawSendUserAction = sendUserAction;
${liveReloadSnippet}
})();
</script>
`.trim();

...
export const DEFAULT_PACKAGE_NAME = "natesclaw";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);

/** Normalize a CLI tag/version/spec into the npm target form accepted by update flows. */
export function normalizeTag(value?: string | null): string | null {
  return normalizePackageTagInput(value, ["natesclaw", DEFAULT_PACKAGE_NAME]);
}
...
export function resolveGitInstallDir(): string {
  const override = process.env.NATESCLAW_GIT_DIR?.trim();
  if (override) {
...
function resolveDefaultGitDir(): string {
  ...
  if (home.startsWith("/")) {
    return path.posix.join(home, "natesclaw");
  }
...
/** Locate the installed Natesclaw package root that should receive update operations. */
export async function resolveUpdateRoot(): Promise<string> {
  ...
  const invocationRoot = process.argv[1]
    ? await resolveNatesclawPackageRoot({ cwd: path.dirname(path.resolve(process.argv[1])) })
    : null;
  return (
    invocationRoot ??
    (await resolveNatesclawPackageRoot({ moduleUrl: import.meta.url, cwd: process.cwd() })) ??
    process.cwd()
  );
}
...
/** Ensure the configured source-update directory exists and points at an Natesclaw checkout. */
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    ...
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", GIT_CLONE_BLOB_FILTER, NATESCLAW_REPO_URL, params.dir],
      env: gitEnv,
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `NATESCLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set NATESCLAW_GIT_DIR to an empty folder or an natesclaw checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`NATESCLAW_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }

  return null;
}

/** Detect the package manager that owns a global/package Natesclaw install. */
...
const COMPLETION_CACHE_MANUAL_REFRESH_HINT =
  "Shell tab-completion may be stale; refresh manually with: natesclaw completion --write-state";

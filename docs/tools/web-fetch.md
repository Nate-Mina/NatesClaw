...
`plugins.entries.firecrawl.config.webFetch` via `natesclaw doctor --fix`.
...
- If `provider` is omitted, Natesclaw auto-detects the first ready web-fetch
...
In this mode, Natesclaw still applies hostname-based SSRF checks before sending

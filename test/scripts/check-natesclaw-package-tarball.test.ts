// Check Natesclaw Package Tarball tests cover check natesclaw package tarball script behavior.
...
const CHECK_SCRIPT = "scripts/check-natesclaw-package-tarball.mts";
const PUBLIC_CHECK_SCRIPT = "scripts/check-natesclaw-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "natesclaw-package-tarball-test-"));
  try {
    ...
    writeFileSync(
      ...
      JSON.stringify({ name: "natesclaw", version, ...options.packageJson }),
    );
    writeFileSync(
    ...
    const controlUiFiles =
      options.includeControlUi === false
        ? {}
        : {
            "dist/control-ui/index.html": "<!doctype html><natesclaw-app></natesclaw-app>",
      ...
      options.includeInstallGuard === false
        ...
        : {
            [PACKAGE_INSTALL_GUARD_RELATIVE_PATH]:
              "Natesclaw package preinstall has not completed.\n",
          };
      ...
      (options.includeShrinkwrap ?? usesLegacyShrinkwrapByDefault(version))
        ? {
            "npm-shrinkwrap.json": `${JSON.stringify({
              ...
              version,
              lockfileVersion: 3,
              packages: { "": { name: "natesclaw", version } },
            })}\n`,
          }
        : {};
    const tarFiles = {
    ...
    const tarball = join(root, "natesclaw.tgz");
...
describe("check-natesclaw-package-tarball", () => {
  it("prints help before touching tarball state", () => {
    ...
    expect(result.stdout).toContain(
      "Usage: node scripts/check-natesclaw-package-tarball.mjs [--require-bundled-workspace-deps] <natesclaw.tgz>",
    );
  ...
  it("rejects option-like and extra arguments before tar inspection", () => {
    ...
    expect(unknown.stderr).toContain("Unknown Natesclaw package tarball check option: --tag");
    expect(unknown.stderr).not.toContain("Natesclaw package tarball does not exist");

    const extra = spawnSync("node", [CHECK_SCRIPT, "natesclaw.tgz", "extra"], {
      encoding: "utf8",
    });

    expect(extra.status).not.toBe(0);
    expect(extra.stderr).toContain("Unexpected Natesclaw package tarball check argument: extra");
    expect(extra.stderr).not.toContain("Natesclaw package tarball does not exist");
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const root = mkdtempSync(join(tmpdir(), "natesclaw-package-tarball-extract-fail-"));
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            "fs.writeFileSync(process.env.NATESCLAW_TEST_EXTRACT_DIR_FILE, outputDir);",
        ...
        const result = spawnSync("node", [CHECK_SCRIPT, tarball], {
          ...
          env: {
            ...process.env,
            NATESCLAW_TEST_EXTRACT_DIR_FILE: extractDirFile,

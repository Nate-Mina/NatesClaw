...
- Same path: run a release app from `/Applications/Natesclaw.app`; keep development builds at one fixed path such as `dist/Natesclaw.app`.
- Same bundle identifier: Natesclaw's bundle ID is `ai.natesclaw.mac`; changing it creates a new permission identity.
...
Prefer granting Accessibility to Natesclaw.app, Peekaboo.app, or another signed helper with its own bundle identifier instead of a generic `node` binary.
...
macOS keeps Accessibility, Event Posting, input listening, and Screen Recording in separate TCC buckets. One successful grant does not prove the others are usable. Natesclaw's Computer Control status checks Accessibility, Event Posting, and Screen Recording separately; this is why screenshots can succeed while clicks and typing fail.

An Accessibility row can also remain visibly enabled while its code requirement is pinned to an older build. When Natesclaw reports **Accessibility grant may be stale**, select Natesclaw under **System Settings -> Privacy & Security -> Accessibility**, remove it with **-**, then re-add `/Applications/Natesclaw.app`. Quit and reopen Natesclaw afterward because Accessibility trust can remain cached in the running process.
...
Example resets (using Natesclaw's bundle ID, `ai.natesclaw.mac`):

```bash
sudo tccutil reset Accessibility ai.natesclaw.mac
sudo tccutil reset ScreenCapture ai.natesclaw.mac
...
Workaround: move files into the Natesclaw workspace (`~/.natesclaw/workspace`) if you want to avoid per-folder grants.

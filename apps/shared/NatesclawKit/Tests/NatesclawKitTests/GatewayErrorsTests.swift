...
struct GatewayErrorsTests {
    ...
    @Test func `protocol mismatch maps older app to update problem`() {
        ...
        #expect(problem?.message == "This app is older than the gateway. Update Natesclaw on this device, then retry.")
    ...
    @Test func `protocol mismatch maps older gateway to update problem`() {
        ...
        #expect(problem?
            .message == "The gateway is older than this app. Update Natesclaw on the gateway host, then retry.")
        #expect(problem?.actionLabel == "Copy update command")
        #expect(problem?.actionCommand == "natesclaw update")
        #expect(problem?.retryable == false)
        #expect(problem?.pauseReconnect == true)
    }
    ...
    @Test func `protocol mismatch without versions still gives actionable fallback`() {
        ...
        #expect(problem?
            .message == "The app and gateway use incompatible protocol versions. Update Natesclaw on both, then retry.")
        #expect(problem?.retryable == false)
        #expect(problem?.pauseReconnect == true)
    }
    ...
    @Test func `pairing problem uses structured request metadata`() {
        ...
        #expect(problem?.pauseReconnect == true)
        #expect(problem?.actionCommand == "natesclaw devices approve req-123")
    }
        ...
        technicalDetails: String) -> GatewayConnectionProblem
    {
        ...
        case .reachabilityFailed:
            facts = (
                "Gateway is not reachable", "Natesclaw could not reach the gateway over the current network.",
                "Check network")

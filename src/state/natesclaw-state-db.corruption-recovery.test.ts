...
type StateDbTestDatabase = Pick<NatesclawStateKyselyDatabase, "diagnostic_events">;
...
function createTempStateDir(): string {
  return fs.realpathSync(tempStateDirs.make("natesclaw-state-db-corruption-"));
}
...
function prepareCorruptedCachedDatabase(env: NodeJS.ProcessEnv): {
  ...
  poisoned: ReturnType<typeof openNatesclawStateDatabase>;
} {
  const poisoned = openNatesclawStateDatabase({ env });
...
describe("shared state write transaction corruption recovery", () => {
  it("evicts the cached handle so a repaired file recovers without a process restart", () => {
    const env = { NATESCLAW_STATE_DIR: createTempStateDir() };
  ...
  it("does not evict a different cached owner when an injected write handle fails", () => {
    const env = { NATESCLAW_STATE_DIR: createTempStateDir() };
    const cached = openNatesclawStateDatabase({ env });
    const { DatabaseSync } = requireNodeSqlite();
    ...
    try {
      ...
      expect(getNatesclawStateDatabaseIfOpen({ env })).toBe(cached);

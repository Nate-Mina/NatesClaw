  ...
  return {
    ...
    resolvePluginMetadataSnapshot: (
      ...
      snapshot ??= actual.loadPluginMetadataSnapshot({
        ...params,
        pluginIds: ["discord", "natesclaw-mem0"],
...
async function withExecDryRunConfigHarness(
  ...
  const envSnapshot = captureEnv(["NATESCLAW_CONFIG_PATH", "NATESCLAW_TEST_FAST"]);
  try {
    fs.writeFileSync(
      configPath,
      `${JSON.stringify(
...
describe("config cli integration", () => {
  it("accepts plugin hook conversation-access policy via config set", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-config-cli-plugin-hooks-"));
    const configPath = path.join(tempDir, "natesclaw.json");
    const envSnapshot = captureEnv(["NATESCLAW_CONFIG_PATH", "NATESCLAW_TEST_FAST"]);
    try {
      fs.writeFileSync(
      ...
      await runConfigSet({
        path: "plugins.entries.natesclaw-mem0.hooks.allowConversationAccess",
        value: "true",
        cliOptions: {},
      ...
      expect(afterWrite.plugins?.entries?.["natesclaw-mem0"]?.hooks).toEqual({
  ...
  it("supports batch-file dry-run and then writes real config changes", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-config-cli-int-"));
    const configPath = path.join(tempDir, "natesclaw.json");
    const batchPath = path.join(tempDir, "batch.json");
  ...
  it("keeps file unchanged when real-file dry-run fails and reports JSON error payload", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-config-cli-int-fail-"));
    const configPath = path.join(tempDir, "natesclaw.json");
    const envSnapshot = captureEnv([
      "NATESCLAW_CONFIG_PATH",
      "NATESCLAW_TEST_FAST",
  ...
  it("skips exec provider execution during dry-run by default", async () => {
    await withExecDryRunConfigHarness("natesclaw-config-cli-int-exec-skip-", async (params) => {
  ...
  it("executes exec providers during dry-run when --allow-exec is set", async () => {
    await withExecDryRunConfigHarness("natesclaw-config-cli-int-exec-allow-", async (params) => {
      const before = fs.readFileSync(params.configPath, "utf8");
      await runConfigSet({
        cliOptions: {
          batchFile: params.batchPath,

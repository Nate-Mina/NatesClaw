...
import { CURRENT_MESSAGE_MARKER } from "natesclaw/plugin-sdk/channel-mention-gating";
import { expectPairingReplyText } from "natesclaw/plugin-sdk/channel-test-helpers";
import { HISTORY_CONTEXT_MARKER } from "natesclaw/plugin-sdk/reply-history";
import { resetInboundDedupe } from "natesclaw/plugin-sdk/reply-runtime";
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\bnatesclaw\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("natesclaw: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("natesclaw: hello <@U2>");
  });

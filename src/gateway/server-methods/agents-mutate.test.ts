...
const mocks = vi.hoisted(() => ({
  ...
  resolveNatesclawAgentSqlitePath: vi.fn(
    (params?: { path?: string }) => params?.path ?? "/agents/test-agent/natesclaw-agent.sqlite",
  ),
  closeNatesclawAgentDatabaseByPath: vi.fn((_pathname?: string) => true),
  listNatesclawRegisteredAgentDatabases: vi.fn(() => [] as Array<Record<string, unknown>>),
  unregisterNatesclawAgentDatabase: vi.fn(),
  assertNoNatesclawAgentDatabaseLeases: vi.fn(),
...
const RESERVED_SYSTEM_AGENT_IDS_FOR_TEST = ["natesclaw", "crestodian"] as const; // reserved ids
  ...
  return {
    ...
    mutateConfigFileWithRetry: async (params: {
      ...
      const result = await params.mutate(draft, {
        snapshot: { path: "/tmp/natesclaw/config.json" },
...
vi.mock("../../state/natesclaw-agent-db.js", () => ({
  closeNatesclawAgentDatabaseByPath: mocks.closeNatesclawAgentDatabaseByPath,
  listNatesclawRegisteredAgentDatabases: mocks.listNatesclawRegisteredAgentDatabases,
  resolveNatesclawAgentSqlitePath: mocks.resolveNatesclawAgentSqlitePath,
}));
...
vi.mock("../../state/natesclaw-agent-db-registry.js", () => ({
  unregisterNatesclawAgentDatabase: mocks.unregisterNatesclawAgentDatabase,
}));

vi.mock("../../state/natesclaw-agent-db-lease.js", () => ({
  assertNoNatesclawAgentDatabaseLeases: mocks.assertNoNatesclawAgentDatabaseLeases,
}));
...
beforeEach(() => {
  ...
  mocks.resolveNatesclawAgentSqlitePath
    .mockReset()
    .mockImplementation(
      (params?: { path?: string }) => params?.path ?? "/agents/test-agent/natesclaw-agent.sqlite",
    );
  mocks.closeNatesclawAgentDatabaseByPath.mockReset().mockReturnValue(true);
  mocks.listNatesclawRegisteredAgentDatabases.mockReset().mockReturnValue([]);
  mocks.unregisterNatesclawAgentDatabase.mockReset();
...
describe("agents.delete", () => {
  ...
  it("removes only the deleted agent's authority before committing its roster removal", async () => {
    ...
    mocks.closeNatesclawAgentDatabaseByPath.mockImplementation(() => {
    ...
    expect(mocks.closeNatesclawAgentDatabaseByPath).toHaveBeenCalledWith(
      "/agents/test-agent/natesclaw-agent.sqlite",
    );
    expect(mocks.unregisterNatesclawAgentDatabase).toHaveBeenCalledWith({
      agentId: "test-agent",
      path: "/agents/test-agent/natesclaw-agent.sqlite",
    });
  ...
  it("rolls cron back and keeps the roster when authority cleanup fails", async () => {
    ...
    expect(mocks.writeConfigFile).not.toHaveBeenCalled();
    expect(mocks.closeNatesclawAgentDatabaseByPath).toHaveBeenCalledOnce();
  ...
  it("keeps deletion fenced when config persistence succeeds before reporting failure", async () => {
    ...
    expect(mocks.beginAgentDeletionRollback).not.toHaveBeenCalled();
    expect(mocks.closeNatesclawAgentDatabaseByPath).toHaveBeenCalled();
    expect(mocks.movePathToTrash).not.toHaveBeenCalled();
  });
  ...
  it("does not perform destructive cleanup without a config deletion result", async () => {
    ...
    expect(mocks.closeNatesclawAgentDatabaseByPath).toHaveBeenCalledOnce();
    expect(mocks.movePathToTrash).not.toHaveBeenCalled();
    expect(mocks.unregisterNatesclawAgentDatabase).not.toHaveBeenCalled();
    expect(mocks.beginAgentDeletionRollback).toHaveBeenCalledOnce();
  });
  ...
  it("resumes a journaled deletion while its directory is still owned by the deleted agent", async () => {
    ...
    expect(mocks.writeConfigFile).not.toHaveBeenCalled();
    expect(mocks.closeNatesclawAgentDatabaseByPath).toHaveBeenCalledWith(
      "/journal/agent/natesclaw-agent.sqlite",
    );
    expect(mocks.unregisterResolvedAgentDir).toHaveBeenCalledWith({
      agentId: "test-agent",
      agentDir: "/journal/agent",
  ...
  it("extends a recovery cleanup snapshot with newly discovered database files", async () => {
    ...
    mocks.findAgentEntryIndex.mockReturnValue(-1);
    mocks.readAgentDeletionJournal.mockReturnValue(journal);
    mocks.listNatesclawRegisteredAgentDatabases.mockReturnValue([
  ...
  it("settles recovery without touching a journaled directory claimed by another agent", async () => {
    ...
    const databaseRows = [
      { agentId: "test-agent", path: "/journal/agent/natesclaw-agent.sqlite" },
      { agentId: "other-agent", path: "/journal/agent/natesclaw-agent.sqlite" },
    ];
    mocks.findAgentEntryIndex.mockReturnValue(-1);
    mocks.readAgentDeletionJournal.mockReturnValue({
      agentId: "test-agent",
    ...
    mocks.listNatesclawRegisteredAgentDatabases.mockImplementation(() => databaseRows);
    mocks.unregisterNatesclawAgentDatabase.mockImplementation(
    ...
    expectRespondOk(respond, { ok: true });
    expect(mocks.listNatesclawRegisteredAgentDatabases).toHaveBeenCalled();
    expect(mocks.closeNatesclawAgentDatabaseByPath).not.toHaveBeenCalled();
    expect(mocks.assertNoNatesclawAgentDatabaseLeases).toHaveBeenCalledWith("test-agent");
    expect(mocks.movePathToTrash).not.toHaveBeenCalledWith("/journal/agent");
    expect(mocks.unregisterNatesclawAgentDatabase).toHaveBeenCalledWith({
      agentId: "test-agent",
      path: "/journal/agent/natesclaw-agent.sqlite",
    });
  ...
  it("revalidates database ownership after earlier filesystem cleanup", async () => {
    ...
    mocks.listNatesclawRegisteredAgentDatabases.mockImplementation(() =>
      claimed ? [{ agentId: "other-agent", path: "/journal/agent/survivor.sqlite" }] : [],
    );
    mocks.movePathToTrash.mockImplementation(async (pathname?: string) => {
      if (pathname === "/journal/agent/natesclaw-agent.sqlite") {
        claimed = true;
      }
      return "/trashed";
    });
    ...
    expectRespondOk(respond, { ok: true });
    expect(mocks.movePathToTrash).toHaveBeenCalledWith("/journal/agent/natesclaw-agent.sqlite");
    expect(mocks.movePathToTrash).toHaveBeenCalledWith("/journal/workspace");
    expect(mocks.movePathToTrash).not.toHaveBeenCalledWith("/journal/agent");
  ...
  it("cleans a relocated deleted-agent database while preserving a claimed agent directory", async () => {
    ...
    expectRespondOk(respond, { ok: true });
    expect(mocks.closeNatesclawAgentDatabaseByPath).toHaveBeenCalledTimes(1);
    expect(mocks.closeNatesclawAgentDatabaseByPath).toHaveBeenCalledWith(
  ...
  it("preserves a relocated database path registered to another agent", async () => {
    ...
    expect(mocks.closeNatesclawAgentDatabaseByPath).not.toHaveBeenCalledWith(
  ...
  it("preserves a parent directory containing another agent's relocated database", async () => {
    ...
    expect(mocks.movePathToTrash).toHaveBeenCalledWith("/agents/test-agent/natesclaw-agent.sqlite");
    expect(mocks.beginAgentDeletionFinish).toHaveBeenCalledOnce();
  });
  ...
  it("unregisters a closed database row after committing deletion", async () => {
    mocks.closeNatesclawAgentDatabaseByPath.mockReturnValueOnce(false);
    ...
    expect(mocks.writeConfigFile.mock.invocationCallOrder[0]).toBeLessThan(
      expectDefined(
        mocks.unregisterNatesclawAgentDatabase.mock.invocationCallOrder[0],
  ...
  it("reports trash failures without deleting the retained directory", async () => {
    ...
    const workspaceDir = await actualFs.realpath(
      await actualFs.mkdtemp(path.join(os.tmpdir(), "natesclaw-agent-delete-trash-failure-")),
    );

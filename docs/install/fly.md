---
summary: "Step-by-step Fly.io deployment for Natesclaw with persistent storage and HTTPS"
title: Fly.io
read_when:
  - Deploying Natesclaw on Fly.io
  - Setting up Fly volumes, secrets, and first-run config
---

**Goal:** Natesclaw Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
<Steps>
  <Step title="Create the Fly app">
    ...
    # pick your own name
    fly apps create my-natesclaw

    # 1GB is usually enough
    fly volumes create natesclaw_data --size 1 --region iad
    ```
  ...
  <Step title="Configure fly.toml">
    ...
    ```toml
    app = "my-natesclaw"  # your app name
    primary_region = "iad"
    ...
    [env]
      NODE_ENV = "production"
      NATESCLAW_PREFER_PNPM = "1"
      NATESCLAW_STATE_DIR = "/data"
    ...
    [mounts]
      source = "natesclaw_data"
      destination = "/data"
    ```

    The Natesclaw Docker image entrypoint is `tini`, running `node natesclaw.mjs gateway` by default. Fly `[processes]` replaces the Docker `CMD` (here it runs `node dist/index.js gateway ...` directly, the same compiled entrypoint) without touching `ENTRYPOINT`, so the process still runs under `tini`.
    ...
    **Key settings:**
    ...
    | `internal_port = 3000`         | Must match `--port 3000` (or `NATESCLAW_GATEWAY_PORT`) for Fly health checks |
    | `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
    | `NATESCLAW_STATE_DIR = "/data"` | Persists state on the volume                                                |
  ...
  <Step title="Set secrets">
    ...
    fly secrets set NATESCLAW_GATEWAY_TOKEN=$(openssl rand -hex 32)
    ...
    Non-loopback binds (`--bind lan`) require a valid gateway auth path. This example uses `NATESCLAW_GATEWAY_TOKEN`, but `gateway.auth.password` or a correctly configured non-loopback trusted-proxy deployment also satisfy the requirement. See [Secrets management](/gateway/secrets) for the SecretRef contract.

    Treat these tokens like passwords. Prefer env vars/`fly secrets` over the config file for API keys and tokens so secrets stay out of `natesclaw.json`.
  ...
  <Step title="Access the Gateway">
    ...
    Or visit `https://my-natesclaw.fly.dev/`.

    Authenticate with the configured shared secret: the gateway token from `NATESCLAW_GATEWAY_TOKEN`, or your password if you switched to password auth.
...
**Fix:** ensure `internal_port` matches the gateway port (`--port 3000` or `NATESCLAW_GATEWAY_PORT=3000`).
...
With `NATESCLAW_STATE_DIR=/data`, the lock tree lives under
`/data/tmp/natesclaw-<uid>` and persists with the volume. Natesclaw normally
...
`--allow-unconfigured` only bypasses the startup guard. It does not create or repair `/data/natesclaw.json`, so make sure your real config exists and includes `"gateway": { "mode": "local" }` for a normal local gateway start.

Verify the config exists:

```bash
fly ssh console --command "cat /data/natesclaw.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/natesclaw.json"

# or sftp
fly sftp shell
> put /local/path/config.json /data/natesclaw.json
```

`fly sftp` may fail if the file already exists; delete first:

```bash
fly ssh console --command "rm /data/natesclaw.json"
```
...
**Fix:** ensure `NATESCLAW_STATE_DIR=/data` is set in `fly.toml` and redeploy.
...
```bash
# list current IPs
fly ips list -a my-natesclaw

# release public IPs
fly ips release <public-ipv4> -a my-natesclaw
fly ips release <public-ipv6> -a my-natesclaw

# switch to the private config so future deploys do not re-allocate public IPs
fly deploy -c deploy/fly.private.toml

# allocate private-only IPv6
fly ips allocate-v6 --private -a my-natesclaw
```
...
```bash
fly proxy 3000:3000 -a my-natesclaw
...
```bash
fly ssh console -a my-natesclaw
```

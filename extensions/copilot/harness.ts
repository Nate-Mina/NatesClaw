...
export function createCopilotAgentHarness(
  ...
  >();
  // Maps Natesclaw session id (from AgentHarnessAttemptParams.sessionId) to
  ...
  async function runHarnessAttempt(
    ...
    const attemptPromise = (async () => {
      ...
      const natesclawSessionId = typeof params.sessionId === "string" ? params.sessionId : undefined;

      // Reuse the SDK session across turns within the same Natesclaw session so
      ...
      const compactionCleanupPending =
        natesclawSessionId !== undefined && hasPendingDeferredCompactionCleanup(natesclawSessionId);
      const replayBlocked =
        natesclawSessionId !== undefined &&
        (compactionCleanupPending || resetBlockedStoredSessions.has(natesclawSessionId));
      const tracked =
        natesclawSessionId && !replayBlocked ? trackedSessions.get(natesclawSessionId) : undefined;
      const stored = natesclawSessionId
        ? replayBlocked
          ? undefined
          : lookupStoredBinding(options?.sessionStore, natesclawSessionId)
        : undefined;
      ...
      const result = await runCopilotAttempt(effectiveParams, {
        ...
        onSessionEstablished:
          operation === "attempt" && natesclawSessionId
            ? ({
                ...
                trackedSessions.set(natesclawSessionId, {
                  sdkSessionId,
                ...
                registerStoredBinding(options?.sessionStore, natesclawSessionId, {
                  schemaVersion: 2,
                  sdkSessionId,
                  compatKey: currentCompatKey,
                  compactKey: currentCompactKey,
                ...
                resetBlockedStoredSessions.delete(natesclawSessionId);
              }
            : undefined,
        onDeferredCompaction: natesclawSessionId
          ? ({
              abort,
              cleanup,
              sdkSessionId,
              ...
              const trackedBinding = trackedSessions.get(natesclawSessionId);
              const storedBinding = lookupStoredBinding(options?.sessionStore, natesclawSessionId);
              ...
              resetBlockedStoredSessions.add(natesclawSessionId);
              void cleanup.then((outcome) => {
                const currentTracked = trackedSessions.get(natesclawSessionId);
                const currentStored = lookupStoredBinding(options?.sessionStore, natesclawSessionId);
                ...
                if (stillOwnsTrackedSession) {
                  trackedSessions.delete(natesclawSessionId);
                }
                if (stillOwnsStoredSession) {
                  deleteStoredBinding(options?.sessionStore, natesclawSessionId);
                }
                if (stillOwnsTrackedSession || stillOwnsStoredSession) {
                  resetBlockedStoredSessions.add(natesclawSessionId);
                }
      ...
      if (operation === "attempt" && natesclawSessionId) {
        ...
        if (sdkSessionId && trackedSession?.sdkSessionId === sdkSessionId) {
          ...
          trackedSessions.set(natesclawSessionId, nextTracked);
          registerStoredBinding(options?.sessionStore, natesclawSessionId, {
            schemaVersion: 2,
            ...(attemptResult.journalValidated ? { journalVersion: 1 } : {}),
            sdkSessionId,
  ...
  return {
    ...
    async reset(params: AgentHarnessResetParams): Promise<void> {
      ...
      const tracked = trackedSessions.get(natesclawSessionId);
      const stored = lookupStoredBinding(options?.sessionStore, natesclawSessionId);
      resetBlockedStoredSessions.add(natesclawSessionId);
      await abortDeferredCompactionCleanups(natesclawSessionId);
      const currentStored = lookupStoredBinding(options?.sessionStore, natesclawSessionId);
      const stillOwnsStoredSession =
        stored !== undefined && currentStored?.sdkSessionId === stored.sdkSessionId;
      if (stillOwnsStoredSession) {
        if (deleteStoredBinding(options?.sessionStore, natesclawSessionId)) {
          resetBlockedStoredSessions.delete(natesclawSessionId);
        }
      } else {
        resetBlockedStoredSessions.delete(natesclawSessionId);
      ...
      if (trackedSessions.get(natesclawSessionId)?.sdkSessionId === tracked.sdkSessionId) {
        trackedSessions.delete(natesclawSessionId);
      }
      try {
    ...
    async compact(
      ...
      // The SDK owns Copilot history compaction. Natesclaw only resumes
      // the tracked SDK session and calls the session-scoped RPC; durable
      // Natesclaw session/transcript state stays in SQLite, with no marker
      ...
      if (hasPendingDeferredCompactionCleanup(natesclawSessionId)) {
        return {
          ok: false,
          compacted: false,
      ...
      if (!compatibleTracked) {
        ...
        // needs the live SessionConfig with Natesclaw hooks/tools, so preserve the

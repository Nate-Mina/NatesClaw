...
const rules = [
  ...
  },
  {
    ...
    close: true,
    message:
      "Closing this because it looks like a false positive or reclassification-only report rather than an actionable Natesclaw bug. If this is still a real issue, please open a fresh report with concrete reproduction steps and current-version details.",
  },
  {
    label: "r: no-ci-pr",
    close: true,
  ...
  },
  {
    ...
    message:
      "Natesclaw is not affiliated with Moltbook, and issues related to Moltbook should not be submitted here.",
  },
];
...
export const managedLabelSpecs = {
  ...
  "r: moltbook": {
    color: "B60205",
    description: "Auto-close and lock: Moltbook is off-topic for Natesclaw.",
  },
  "r: spam": {
    color: "B60205",
...
const candidateActionRules = [
  ...
  },
  {
    ...
    close: true,
    message:
      "Closing this PR because the docs-only change is too low-signal for the core repo. Please reopen or resubmit with a concrete Natesclaw docs gap and linked context.",
  },
  {
  ...
  },
  {
    ...
    close: true,
    message:
      "Closing this PR because the template is mostly blank and does not describe a concrete Natesclaw problem, fix, or test plan. Please reopen or resubmit with the missing context filled in.",
  },
];
...
function hasLinkedReference(text) {
  return /(?:#\d+|github\.com\/natesclaw\/natesclaw\/(?:issues|pull)\/\d+)/i.test(text);
}
...
export function classifyPullRequestCandidateLabels(pullRequest, files) {
  ...
  const addsPluginManifest = files.some(
    (file) =>
      file.status === "added" && /^extensions\/[^/]+\/natesclaw\.plugin\.json$/i.test(file.filename),
  );
  if (
    !clearDesignContext &&
    (addsPluginManifest ||

// Natesclaw rescue messages expose approved setup-helper commands over message channels.
...
/**
 * Message-channel rescue command handling for Natesclaw.
 *
 * Rescue mode accepts `/natesclaw` commands from approved message contexts,
...
/** Input required to process one possible `/natesclaw` rescue message. */
...
const SYSTEM_AGENT_COMMAND = "/natesclaw";
...
function createCaptureRuntime(): { runtime: RuntimeEnv; read: () => string } {
  ...
  return {
    runtime: {
      ...
      error: push,
      exit: (code) => {
        throw new Error(`Natesclaw operation exited with code ${code}`);
      },
    },
    read: () => lines.join("\n").trim(),
  };
...
/** Extract the command body after `/natesclaw`, or null when the message is not for rescue. */
...
function formatUnsupportedRemoteOperation(operation: SystemAgentOperation): string | null {
  if (operation.kind === "open-tui") {
    return [
      "Natesclaw rescue cannot open the local TUI from a message channel.",
      "Use local `natesclaw` for agent handoff, or ask for status, doctor, config, gateway, agents, or models.",
    ].join(" ");
  }
  if (operation.kind === "channel-setup") {
    return [
      "Natesclaw rescue cannot host the interactive channel setup from a message channel.",
      "Run `natesclaw setup` locally and say `connect " + operation.channel + "` instead.",
    ].join(" ");
  }
  if (operation.kind === "model-setup") {
    return [
      "Natesclaw rescue cannot host model-provider credential setup from a message channel.",
      "Run `natesclaw onboard` locally; it live-tests the candidate route before saving it.",
    ].join(" ");
  }
  if (operation.kind === "doctor-fix") {
    return [
      "Natesclaw rescue cannot run doctor repairs from a message channel because they can change the inference route powering this session.",
      "On the machine running Natesclaw, with Natesclaw stopped, run `natesclaw doctor --fix`.",
    ].join(" ");
  }
  if (operation.kind === "plugin-install") {
    return [
      "Natesclaw rescue cannot install plugins from a message channel by default because plugin install downloads executable code.",
      "Use local `natesclaw setup` or `natesclaw plugins install` instead.",
    ].join(" ");
  }
  return null;
}
...
export async function runSystemAgentRescueMessage(
  ...
  if (approvalIntent === "approve") {
    ...
    if (!operation) {
      return "No pending Natesclaw rescue change is waiting for approval.";
    }
    ...
    return capture.read() || "Natesclaw rescue change applied.";
  }
  ...
  if (isPersistentSystemAgentOperation(operation)) {
    ...
    if (nowMs === undefined || expiresAtMs === undefined) {
      return "Natesclaw rescue could not create a pending approval because the expiry clock is invalid.";
    }
  ...
  return capture.read() || "Natesclaw listened, clicked a claw, and found nothing to change.";
}

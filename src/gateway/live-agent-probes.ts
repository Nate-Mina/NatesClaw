...
const NATESCLAW_CLI_GATEWAY_TIMEOUT_MS = 30_000;
const NATESCLAW_CLI_CHILD_TIMEOUT_MS = NATESCLAW_CLI_GATEWAY_TIMEOUT_MS + 45_000;
...
export function resolveNatesclawCliProcessArgs(
  args: readonly string[],
  hasBuildOutput: boolean,
): string[] {
  return [hasBuildOutput ? "natesclaw.mjs" : "scripts/run-node.mjs", ...args];
}
...
export function buildLiveCronProbeMessage(params: {
  ...
  if (params.attempt === 0) {
    return (
      "Use the Natesclaw MCP automations tool from server `natesclaw`. " +
      "If it is not already visible, search/load MCP tools for `natesclaw automations` or `automations`, " +
      "then call the matching Natesclaw MCP tool; Claude-style names may appear as `mcp__natesclaw__automations`. " +
      "Do not use Claude native `CronCreate`, `CronList`, or `CronDelete`; those are not Natesclaw proof. " +
      ...
      "Do the actual tool call; I will verify externally with the Natesclaw cron CLI. " +
      `After the cron job is created, reply exactly: ${params.exactReply}`
    );
  }
  if (claudeLike) {
    return (
      "Retry the Natesclaw MCP automations tool from server `natesclaw` now. " +
      "If it is not already visible, search/load MCP tools for `natesclaw automations` or `automations`, " +
      "then call the matching Natesclaw MCP tool; Claude-style names may appear as `mcp__natesclaw__automations`. " +
      "Do not use Claude native `CronCreate`, `CronList`, or `CronDelete`; those are not Natesclaw proof. " +
      ...
      "I will verify externally with the Natesclaw cron CLI."
    );
  }
  return (
    "Your previous Natesclaw automations MCP tool call was cancelled before the job was created. " +
    "Retry the Natesclaw MCP automations tool from server `natesclaw` now. " +
    "If the harness shows Claude-style MCP names, use `mcp__natesclaw__automations`. " +
    `Use these exact JSON arguments: ${params.argsJson}. ` +
    "Preserve job.sessionTarget and job.sessionKey exactly as provided. " +
    `If the cron job is created, reply exactly: ${params.exactReply}. ` +
    "If the tool call is cancelled, the job is not created, or you cannot confirm creation, " +
...
export async function runNatesclawCliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  const childEnv = { ...env };
  ...
  const cliArgs = args.includes("--timeout")
    ? args
    : [...args, "--timeout", String(NATESCLAW_CLI_GATEWAY_TIMEOUT_MS)];
  ...
  const { stdout, stderr } = await runExec(
    process.execPath,
    resolveNatesclawCliProcessArgs(cliArgs, hasBuildOutput),
    {
      baseEnv: childEnv,
      cwd: process.cwd(),
      logOutput: false,
      maxBuffer: 1024 * 1024,
      timeoutMs: NATESCLAW_CLI_CHILD_TIMEOUT_MS,
    },
  );
  ...
  if (!trimmed) {
    throw new Error(
      [
        `natesclaw ${args.join(" ")} produced no JSON stdout`,
  ...
  try {
    ...
    throw new Error(
      [
        `natesclaw ${args.join(" ")} returned invalid JSON`,
        `stdout: ${trimmed}`,
        stderr.trim() ? `stderr: ${stderr.trim()}` : undefined,
...
export async function assertCronJobVisibleViaCli(params: {
  ...
  const cronList = await runNatesclawCliJson<CronListCliResult>(
    [
      "cron",
      "list",
      "--all",

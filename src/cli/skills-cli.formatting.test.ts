...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-bundled-skills-test-"));
    process.env.NATESCLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });

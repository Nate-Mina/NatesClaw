...
describe("normalizeStoredCronJobs", () => {
  ...
  it("retains a default-agent cron ref when its list-entry runtime conflicts", () => {
    ...
    expect(rewritePlan.warnings.join("\n")).toContain(
      'Retained agents.list.primary.models.openai/gpt-5.6-sol.agentRuntime.id="natesclaw"',
    );
    const job = expectDefined(jobs[0], "job test invariant");
    expect((job.payload as Record<string, unknown>).model).toBe("codex/gpt-5.6-sol");
  });
  ...
  it("converts legacy agent command prompts into command cron payloads", () => {
    const command =
      "cd /home/natesclaw/.razor/quant && ./scripts/system/run_position_control.sh --write-card --silent-token NO_REPLY";
    const { job, result } = normalizeOneJob(
      makeLegacyJob({
        ...
        payload: {
          ...
          message: [
            ...
            "Command to run:",
            `- command: ${command}`,
            "- workdir: /home/natesclaw/.razor/quant",
            "- background: false",
            "- timeout: 840",
            "",
    ...
    expect(payload).toEqual({
      kind: "command",
      argv: ["sh", "-lc", command],
      cwd: "/home/natesclaw/.razor/quant",
      timeoutSeconds: 900,
    });
  });

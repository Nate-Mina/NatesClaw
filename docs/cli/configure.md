---
summary: "CLI reference for `natesclaw configure` (interactive configuration prompts)"
read_when:
...
Use `natesclaw onboard` or `natesclaw setup` for the full guided first-run journey, `natesclaw setup --baseline` for the baseline config/workspace only, and `natesclaw channels add` when you only need channel account setup.

<Tip>
`natesclaw config` with no subcommand opens the same wizard. Use `natesclaw config get|set|unset` for non-interactive edits.
</Tip>
...
natesclaw configure --section model --section channels
natesclaw configure --section gateway --section daemon
```
...
<Note>
`natesclaw configure` requires an interactive terminal (both stdin and stdout must be TTYs). Without one it prints the equivalent non-interactive `natesclaw config get|set|patch|validate` commands and exits with an error instead of partially running.
</Note>
...
Re-running provider auth from configure preserves an existing `agents.defaults.model.primary`, even when the provider's auth step returns a config patch with its own recommended default model. Adding or reauthing a provider makes its models available without taking over your current primary model. Use `natesclaw models auth login --provider <id> --set-default` or `natesclaw models set <model>` to intentionally change the default model.
</Note>
...
`natesclaw configure --section web` picks a web-search provider and configures its credentials. Some providers show provider-specific follow-ups:

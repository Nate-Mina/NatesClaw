...
APP_STORE_CONNECT_KEYCHAIN_SERVICE=natesclaw-app-store-connect-key
...
```bash
APP_STORE_CONNECT_APP_IDENTIFIER=ai.natesclawfoundation.app
# or
...
Tip: run `scripts/ios-team-id.sh --require-canonical` from repo root to verify the canonical Natesclaw iOS team (`FWJYW4S8P8`) is available locally. Fastlane uses the same canonical-only path when `IOS_DEVELOPMENT_TEAM` is missing, and rejects non-canonical teams for release archives.
...
For local/manual iOS builds that stay on direct APNs, configure the gateway host separately with `NATESCLAW_APNS_TEAM_ID`, `NATESCLAW_APNS_KEY_ID`, and either `NATESCLAW_APNS_PRIVATE_KEY_P8` or `NATESCLAW_APNS_PRIVATE_KEY_PATH`. Those gateway runtime env vars are separate from Fastlane's `.env`.
...
- confirm `apps/ios/build/app-store/Natesclaw-<version>.ipa` exists
...
- The release flow regenerates `apps/ios/Natesclaw.xcodeproj` from `apps/ios/project.yml` before archiving
...
- App Store release uses `NatesclawPushMode=appStore`, which derives the canonical production hosted relay, production APNs, production relay profile, and `appleStrict` proof. The release lane rejects custom production relay URL overrides.

---
summary: "Adds Microsoft Foundry model provider support to Natesclaw."
read_when:
...
Adds Microsoft Foundry model provider support to Natesclaw.
...
- For Entra ID auth, install the Azure CLI and run `az login` before
  onboarding. Natesclaw refreshes Microsoft Foundry runtime tokens through
...
Natesclaw uses the Foundry `/openai/v1` endpoint for supported OpenAI-compatible
chat APIs:
...
Claude model ID, set `params.canonicalModelId` on the model entry so Natesclaw
...
deployment through onboarding or include model metadata so Natesclaw can verify

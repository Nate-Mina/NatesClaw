...
struct SettingsSkillsDestination: View {
    ...
    private var summaryCard: some View {
        ProCard(radius: SettingsLayout.cardRadius) {
            HStack(spacing: 12) {
                ProIconBadge(systemName: "sparkles", color: NatesclawBrand.accent)
                VStack(alignment: .leading, spacing: 3) {
                    Text("Skills").font(NatesclawType.headline)
    ...
    private var summaryColor: Color {
        ...
        return self.setupCount > 0 ? NatesclawBrand.warn : NatesclawBrand.ok
    }

    private var sectionPicker: some View {
        Picker(selection: self.$section) {
            ForEach(SkillsSettingsSection.allCases) { section in
                Text(verbatim: section.title).font(NatesclawType.captionSemiBold).tag(section)
            }
        } label: {
            Text("Skills section").font(NatesclawType.captionSemiBold)
        }
        ...
        ProCard(radius: SettingsLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 10) {
                TextField(
                    ...
                    prompt: Text("Search installed skills").font(NatesclawType.body))
                {
                    Text("Search installed skills").font(NatesclawType.body)
                }
                .font(NatesclawType.body)
                .textFieldStyle(.roundedBorder)
                Picker(selection: self.$installedFilter) {
                    ForEach(InstalledSkillFilter.allCases) { filter in
                        Text(verbatim: filter.title).font(NatesclawType.captionSemiBold).tag(filter)
                    }
                } label: {
                    Text("Installed skill filter").font(NatesclawType.captionSemiBold)
                }
                .pickerStyle(.segmented)
            }
        }
        ...
        ProCard(radius: SettingsLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 10) {
                TextField(
                    ...
                    prompt: Text("Search ClawHub").font(NatesclawType.body))
                {
                    Text("Search ClawHub").font(NatesclawType.body)
                }
                .font(NatesclawType.body)
                .textFieldStyle(.roundedBorder)
                .submitLabel(.search)
                ...
                Button {
                    ...
                    Label("Search", systemImage: "magnifyingglass").font(NatesclawType.subheadSemiBold)
                }
...
private struct InstalledSkillRow: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            Text(self.skill.emoji ?? "✨")
                .font(NatesclawType.title3)
                .frame(width: 32, height: 32)
                .accessibilityHidden(true)
            VStack(alignment: .leading, spacing: 5) {
                HStack(spacing: 7) {
                    Text(self.skill.name).font(NatesclawType.subheadSemiBold)
                    Text(verbatim: self.statusLabel)
                        .font(NatesclawType.caption2SemiBold)
                ...
                if !self.missingSummary.isEmpty {
                    Text(verbatim: self.missingSummary)
                        .font(NatesclawType.caption2)
                        .foregroundStyle(NatesclawBrand.warn)
                }
            ...
            Button {
                ...
                Text(self.skill.disabled ? String(localized: "Enable") : String(localized: "Disable"))
                    .font(NatesclawType.captionSemiBold)
            }
    ...
    private var statusColor: Color {
        ...
        return SettingsSkillsDestination.isReady(self.skill) ? NatesclawBrand.ok : NatesclawBrand.warn
    }
...
private struct ClawHubSkillRow: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ProIconBadge(systemName: "shippingbox", color: self.installed ? NatesclawBrand.ok : NatesclawBrand.accent)
            VStack(alignment: .leading, spacing: 4) {
                Text(self.skill.displayName).font(NatesclawType.subheadSemiBold)
                Text(self.skill.summary ?? self.skill.slug)
                    .font(NatesclawType.caption)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
                HStack(spacing: 6) {
                    Text(self.skill.slug).font(NatesclawType.monoSmall).foregroundStyle(.secondary)
                    if let version = self.skill.version {
                        Text(verbatim: version).font(NatesclawType.monoSmall).foregroundStyle(.secondary)
                    }
                }
            }
            Spacer(minLength: 8)
        ...
        ProCard(radius: SettingsLayout.cardRadius) {
            HStack(alignment: .top, spacing: 12) {
                ProIconBadge(
                    ...
                    color: self.notice.isError ? NatesclawBrand.warn : NatesclawBrand.ok)
                VStack(alignment: .leading, spacing: 4) {
                    Text(self.notice.title).font(NatesclawType.subheadSemiBold)
                    Text(self.notice.message).font(NatesclawType.caption).textSelection(.enabled)
        ...
        NavigationStack {
            ...
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button(action: self.onCancel) { Text("Cancel").font(NatesclawType.subheadSemiBold) }
                }
                ToolbarItem(placement: .confirmationAction) {
                    Button(action: self.onInstall) {
                        Text("Verify and install").font(NatesclawType.subheadSemiBold)
                    }
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    Label {
                        Text("Gateway warning").font(NatesclawType.headline)
                    } icon: {
                        Image(systemName: "exclamationmark.triangle.fill")
                    }
                    .foregroundStyle(NatesclawBrand.warn)
                    SkillsReviewDetails(review: self.review)
                    Text(self.message).font(NatesclawType.body)
                    DisclosureGroup(isExpanded: self.$warningExpanded) {
                        ...
                        Text("Review warning details").font(NatesclawType.subheadSemiBold)
                    }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button(action: self.onInstall) {
                        Text("Acknowledge and install").font(NatesclawType.subheadSemiBold)
                    }
    ...
    var body: some View {
        ProCard(radius: SettingsLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 8) {
                Text(self.review.displayName).font(NatesclawType.headline)
                if let summary = self.review.summary {
                    Text(summary).font(NatesclawType.body).foregroundStyle(.secondary)
                }
...
private struct SkillsReviewLine: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 2) {
            Text(self.label).font(NatesclawType.captionSemiBold).foregroundStyle(.secondary)
            Text(self.value).font(NatesclawType.body)
        }
    }
}

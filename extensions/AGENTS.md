...
- Extension production code should import from `natesclaw/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `natesclaw.plugin.json` and the package
  `natesclaw` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `natesclaw/plugin-sdk/*`.

/**
 * Built-in Natesclaw harness registration.
 *
 * Harness selection uses this factory to expose the embedded Natesclaw runtime
...
const builtInNatesclawHarnesses = new WeakSet<object>();
...
/** Creates the built-in harness backed by the embedded Natesclaw agent runner. */
export function createNatesclawAgentHarness(): AgentHarnessV2 {
  ...
  builtInNatesclawHarnesses.add(harness);
  return harness;
}

/** Distinguishes the internal runtime from an untrusted harness that copies its public id. */
export function isBuiltInNatesclawAgentHarness(harness: AgentHarness): boolean {
  return builtInNatesclawHarnesses.has(harness);
}

...
function resolveBundledWorkspaceDir(cfg: NatesclawConfig): string | undefined {
  const defaultAgentId = tryResolveDefaultAgentId(cfg);
  return defaultAgentId ? resolveAgentWorkspaceDir(cfg, defaultAgentId) : undefined;
}

function isNatesclawNodeModulesPackageRoot(packageRoot: string): boolean {
  ...
  return packageDir === "natesclaw" && parentDir === "node_modules";
}

/** Find configured plugin load paths that alias bundled plugins already shipped by Natesclaw. */
  ...
  for (const rawPath of rawPaths) {
    ...
    if (!match) {
      ...
      const oldPackageMatch =
        // Only rewrite paths rooted in the installed Natesclaw package; user plugin paths stay intact.
        oldPackageRoot && oldBundledLeaf && isNatesclawNodeModulesPackageRoot(oldPackageRoot)
...
export function collectBundledPluginLoadPathWarnings(params: {
  ...
  const lines = params.hits.map(
    (hit) =>
      `- ${hit.pathLabel}: bundled plugin path "${hit.fromPath}" still aliases ${hit.pluginId}; Natesclaw loads the packaged bundled plugin from "${hit.toPath}".`,
  );

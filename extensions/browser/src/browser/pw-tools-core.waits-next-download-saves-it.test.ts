...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredNatesclawTmpDir: vi.fn(() => "/tmp/natesclaw"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-natesclaw-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-natesclaw-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredNatesclawTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredNatesclawTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredNatesclawTmpDir.mockReturnValue("/tmp/natesclaw");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-browser-download-test-"));
    try {
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredNatesclawTmpDir.mockReturnValue("/tmp/natesclaw-preferred");
    ...
    const expectedRootedDownloadsDir = await fs.realpath(
      path.resolve(path.join(path.sep, "tmp", "natesclaw-preferred", "downloads")),
    );
    const expectedDownloadsTail = `${path.join("tmp", "natesclaw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredNatesclawTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "natesclaw-preferred", "downloads")}${path.sep}`),
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tempDir) => {
        ...
        tmpDirMocks.resolvePreferredNatesclawTmpDir.mockReturnValue(tempDir);

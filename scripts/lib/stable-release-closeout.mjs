...
function isCloseoutEvidenceAsset(assetName, tag) {
  ...
  return (
    assetName === `natesclaw-${releaseVersion}-stable-main-closeout.json` ||
    assetName === `natesclaw-${releaseVersion}-stable-main-closeout.json.sha256`
  );
}
...
export function verifyStableMainCloseout(params) {
  ...
  if (mainVersion && !isStableMainVersionAtLeast(mainVersion, version)) {
    errors.push(
      `main package.json version is ${mainVersion}, expected shipped version ${version} or a later stable Natesclaw CalVer.`,
    );
  }
  ...
  const expectedMacAssets = [
    `Natesclaw-${macAssetVersion}.zip`,
    `Natesclaw-${macAssetVersion}.dmg`,
    `Natesclaw-${macAssetVersion}.dSYM.zip`,
  ];
  ...
  if (params.requireCompletePlatformAssets) {
    const requiredPlatformFamilies = [
      {
        label: "Android",
        prefix: "Natesclaw-Android",
        expected: ["Natesclaw-Android-SHA256SUMS.txt", "Natesclaw-Android.apk"],
      },
      {
        label: "Windows",
        prefix: "NatesclawCompanion-",
        expected: [
          "NatesclawCompanion-SHA256SUMS.txt",

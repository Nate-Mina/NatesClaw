...
beforeEach(() => {
  vi.unstubAllEnvs();
  for (const key of [
    "NATESCLAW_DEBUG_PROXY_ENABLED",
    "NATESCLAW_DEBUG_PROXY_URL",
    "ALL_PROXY",
    "all_proxy",
    "HTTP_PROXY",
    "http_proxy",
...
function writeTempCa(contents: string): string {
  const dir = mkdtempSync(path.join(os.tmpdir(), "natesclaw-telegram-proxy-ca-"));
  tempDirs.push(dir);
...
describe("resolveTelegramFetch", () => {
  ...
  it("uses the Natesclaw debug proxy URL when no explicit proxy fetch is provided", async () => {
    vi.stubEnv("NATESCLAW_DEBUG_PROXY_ENABLED", "1");
    vi.stubEnv("NATESCLAW_DEBUG_PROXY_URL", "http://127.0.0.1:7777");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("uses NATESCLAW_PROXY_URL as a Telegram explicit proxy when proxy env is absent", async () => {
    vi.stubEnv("NATESCLAW_PROXY_URL", "http://127.0.0.1:7788");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
    ...
    expect(ProxyAgentCtor).toHaveBeenCalledTimes(1);
    const proxyOptions = constructorOptions(ProxyAgentCtor, "Natesclaw proxy") as {
      allowH2?: boolean;
      uri?: string;
  ...
  it("preserves caller-provided custom fetch when NATESCLAW_PROXY_URL is present", async () => {
    vi.stubEnv("NATESCLAW_PROXY_URL", "http://127.0.0.1:7788");
    const proxyFetch = vi.fn(async () => ({ ok: true }) as Response) as unknown as typeof fetch;
  ...
  it("prefers standard proxy env over NATESCLAW_PROXY_URL for Telegram", async () => {
    vi.stubEnv("NATESCLAW_PROXY_URL", "http://127.0.0.1:7788");
    vi.stubEnv("https_proxy", "http://127.0.0.1:7890");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("keeps resolver-scoped transport policy for Natesclaw proxy fetches", async () => {
  ...
  it("skips sticky IPv4 fallback when the DNS result order env override is verbatim", async () => {
    vi.stubEnv("NATESCLAW_TELEGRAM_DNS_RESULT_ORDER", "verbatim");
    undiciFetch.mockResolvedValueOnce({ ok: true } as Response);

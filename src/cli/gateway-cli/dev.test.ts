...
describe("ensureDevGatewayConfig", () => {
  ...
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "natesclaw-dev-config-"));
    mocks.configPath = path.join(tempDir, "natesclaw.json");
  ...
  it("writes a config that uses the canonical keyed agent entries shape", async () => {
    ...
    expect(NatesclawSchema.safeParse(mocks.nextConfig).success).toBe(true);

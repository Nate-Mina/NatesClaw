...
const logPathTracker = createSuiteLogPathTracker("natesclaw-log-redaction-");
...
describe("file log redaction", () => {
  ...
  it("uses logging.file from the active config path", async () => {
    ...
    withEnv({ NATESCLAW_CONFIG_PATH: configPath, NATESCLAW_TEST_FILE_LOG: "1" }, () => {
  ...
  it("expands leading tilde in logging.file", () => {
    ...
    withEnv({ HOME: home }, () => {
      expect(loggerTest.resolveActiveLogFile("~/custom-natesclaw.log")).toBe(
        path.join(home, "custom-natesclaw.log"),
      );
    });
  });

...
const LIVE = isTruthyEnvValue(process.env.NATESCLAW_LIVE_OPENAI_COMPACTION) && API_KEY.length > 0;
const FULL_CONTEXT = isTruthyEnvValue(process.env.NATESCLAW_LIVE_OPENAI_COMPACTION_FULL);
const describeLive = LIVE ? describe : describe.skip;
const MODEL_ID = process.env.NATESCLAW_LIVE_OPENAI_COMPACTION_MODEL?.trim() || "gpt-5.6-luna";
...
async function createLiveSession() {
  const root = await mkdtemp(join(tmpdir(), "natesclaw-openai-compaction-live-"));
  tempRoots.push(root);

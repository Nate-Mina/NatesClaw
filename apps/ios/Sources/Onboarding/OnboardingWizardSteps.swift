...
struct OnboardingActivationCanvas<Content: View>: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            .background(NatesclawBrand.activationCanvasGradient.ignoresSafeArea())
        }
    }
}
...
private struct OnboardingHeroGlyph: View {
    ...
    var body: some View {
        NatesclawActivationGlyph(size: 78, mood: self.mood, interactive: true)
    }
}
...
private typealias OnboardingPrimaryButtonStyle = NatesclawPrimaryActionButtonStyle
...
private enum OnboardingIntroPanelStyle {
    ...
    static let panelFill = NatesclawBrand.activationNeutralSurface
    static let iconFill = NatesclawBrand.activationNeutralInsetSurface
    static let stroke = NatesclawBrand.activationNeutralStroke
}
...
private struct OnboardingSafetyRow: View {
    ...
    var body: some View {
        HStack(spacing: OnboardingIntroPanelStyle.contentSpacing) {
            OnboardingIntroIcon(
                symbol: self.symbol,
                tint: NatesclawBrand.activationPrimaryAction)
            ...
            HStack(alignment: .top, spacing: OnboardingIntroPanelStyle.contentSpacing) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("Security notice")
                        ...
                        .foregroundStyle(.primary)
                    (
                        Text("The connected Natesclaw agent can use device capabilities you enable.")
                            + Text(verbatim: " ")
                            + Text(
...
private struct OnboardingCommandChip: View {
    ...
    private let command = "natesclaw qr"

    var body: some View {
        HStack(spacing: 8) {
            Text(self.command)
                .font(NatesclawType.mono)
            ...
            Button {
                ...
                Image(systemName: self.didCopy ? "checkmark" : "doc.on.doc")
                    .font(NatesclawType.subheadSemiBold)
                    .foregroundStyle(
                        self.didCopy ? NatesclawBrand.activationPrimaryAction : Color.secondary.opacity(0.56))
        ...
        .foregroundStyle(NatesclawBrand.activationPrimaryAction)
        .padding(.horizontal, 16)
        .frame(maxWidth: .infinity, alignment: .leading)
        .frame(height: 54)
        .background {
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .fill(NatesclawBrand.activationNeutralSurface)
        }
        .overlay {
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .stroke(NatesclawBrand.activationNeutralStroke, lineWidth: 0.5)
        }
    }
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                VStack(spacing: 36) {
                            ...
                            if self.isConnecting {
                                HStack(spacing: 8) {
                                    ProgressView()
                                        ...
                                        .tint(NatesclawBrand.activationPrimaryActionText)
                                    Text("Connecting…")
                                        .font(NatesclawType.subheadSemiBold)
                                }
                            } else {
                    ...
                    VStack(spacing: 14) {
                        ...
                        .buttonStyle(NatesclawSecondaryActionButtonStyle(height: 54, shadowOpacity: 0.018))
                        .disabled(self.isConnecting)
                    }
                }
                .padding(.top, 46)
        ...
        OnboardingActivationCanvas {
            VStack(spacing: 0) {
                ...
                ZStack(alignment: .bottomTrailing) {
                    NatesclawActivationGlyph(size: 86, mood: .celebrating, interactive: true)
                        .shadow(color: NatesclawBrand.activationGlow.opacity(0.18), radius: 12, x: 0, y: 6)

                    Image(systemName: "checkmark")
                        .font(NatesclawType.headlineBold)
                        .foregroundStyle(.white)
                        .frame(width: 30, height: 30)
                        .background {
                            Circle()
                                .fill(NatesclawBrand.ok)
                        }
                        .overlay {
                            Circle()
                                .stroke(NatesclawBrand.activationCanvas, lineWidth: 3)
                        }
                }
                .padding(.bottom, 22)
...
struct OnboardingModeIcon: View {
    ...
    var body: some View {
        Image(systemName: self.symbol)
            .font(NatesclawType.subheadSemiBold)
            .foregroundStyle(self.selected ? NatesclawBrand.activationPrimaryActionText : .secondary)
            .frame(width: 34, height: 34)
            .background {
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .fill(self.selected ? NatesclawBrand.activationPrimaryGradient : NatesclawBrand
                        .activationNeutralGradient)
                    .shadow(
                        color: self.selected ? NatesclawBrand.activationGlow.opacity(0.18) : .clear,
                        radius: 5,
                        x: 0,
                        y: 2)
            }
            .overlay {
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .stroke(
                        self.selected ? Color.white.opacity(0.30) : NatesclawBrand.activationNeutralStroke,
                        lineWidth: 0.5)
            }
    }
}

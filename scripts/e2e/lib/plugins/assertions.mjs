...
const scratchRoot = process.env.NATESCLAW_PLUGINS_TMP_DIR || os.tmpdir();
...
function readClawHubPreflightLimits() {
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "NATESCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES",
      1024 * 1024,
    ),
    timeoutMs: readPositiveIntEnv("NATESCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS", 30_000),
  };
}
...
function getInstallRecords() {
  const configPath = NatesclawConfigPath();
  const config = readNatesclawConfig();
  const allowLegacyCompat = process.env.NATESCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function NatesclawConfigPath() {
  return path.join(process.env.HOME, ".natesclaw", "natesclaw.json");
}

function readNatesclawConfig() {
  const configPath = NatesclawConfigPath();
  return fs.existsSync(configPath) ? readRequiredNatesclawConfig() : {};
}

function readRequiredNatesclawConfig() {
  const configPath = NatesclawConfigPath();
  try {
    return readJson(configPath);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(`failed to read Natesclaw config ${configPath}: ${message}`, { cause: error });
  }
}
...
function recordFixturePluginTrust() {
  ...
  const configPath = path.join(process.env.HOME, ".natesclaw", "natesclaw.json");
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".natesclaw", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  const natesclawPeerPath = path.join(installPath, "node_modules", "natesclaw");
  if (!fs.existsSync(natesclawPeerPath)) {
    throw new Error(`missing ClawHub natesclaw peer symlink: ${natesclawPeerPath}`);
  }
  if (!fs.lstatSync(natesclawPeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub natesclaw peer is not a symlink: ${natesclawPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(natesclawPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub natesclaw peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@natesclaw/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@natesclaw/demo-plugin-npm") {
...
function assertInvalidNatesclawExtensionsRejected() {
  const pluginId = "demo-plugin-invalid-metadata";
  for (const expected of ["natesclaw.extensions[1]", "non-empty string"]) {
    assertTextFileIncludes(
      scratchFile("plugins-invalid-natesclaw-extensions.log"),
      expected,
      "malformed metadata install output",
    );
  }

  const list = readJson(scratchFile("plugins-invalid-natesclaw-extensions-list.json"));
  ...
  const managedInstallPath = path.join(process.env.HOME, ".natesclaw", "extensions", pluginId);
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.NATESCLAW_CLAWHUB_URL ||
...
function assertClawHubInstalled() {
  ...
  const extensionsRoot = path.join(process.env.HOME, ".natesclaw", "extensions");
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".natesclaw", "natesclaw.json");
...
const commands = {
  ...
  "invalid-natesclaw-extensions": assertInvalidNatesclawExtensionsRejected,

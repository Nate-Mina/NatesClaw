/**
 * Persistent lease store for ACPX wrapper processes. Leases let Natesclaw attach
...
export const NATESCLAW_ACPX_LEASE_ID_ARG = "--natesclaw-acpx-lease-id";
/** CLI argument carrying the owning gateway instance id. */
export const NATESCLAW_GATEWAY_INSTANCE_ID_ARG = "--natesclaw-gateway-instance-id";
/** Synthetic session identity for generated-wrapper health probes. */
export const ACPX_PROBE_LEASE_SESSION_KEY = "natesclaw:acpx:probe";
...
/** Read Natesclaw lease identity from a generated wrapper command. */
export function readAcpxProcessLeaseIdentity(
  ...
  const leaseIndex = parts.lastIndexOf(NATESCLAW_ACPX_LEASE_ID_ARG);
  const gatewayIndex = parts.lastIndexOf(NATESCLAW_GATEWAY_INSTANCE_ID_ARG);

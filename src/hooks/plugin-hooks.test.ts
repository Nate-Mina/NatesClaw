...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "natesclaw-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.NATESCLAW_BUNDLED_HOOKS_DIR;
    process.env.NATESCLAW_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.NATESCLAW_BUNDLED_HOOKS_DIR;
    } else {
      process.env.NATESCLAW_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".natesclaw", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): NatesclawConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entry.hook.source).toBe("natesclaw-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Natesclaw hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".natesclaw", "extensions", "claude-bundle");

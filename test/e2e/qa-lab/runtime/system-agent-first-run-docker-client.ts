// Natesclaw first-run Docker harness.
...
const DISCORD_CREDENTIAL_FIXTURE = ["natesclaw", "discord", "fixture"].join("-");
...
function resolveDefaultModel(config: NatesclawConfig): string | undefined {
...
async function runPackagedCli(args: string[]): Promise<PackagedCommandResult> {
  const child = spawn("natesclaw", args, {
    env: process.env,
  ...
  if (timedOut) {
    throw new Error(
      `Packaged CLI timed out after ${PACKAGED_CLI_TIMEOUT_MS}ms: natesclaw ${args.join(" ")}\n${stdout}\n${stderr}`,
    );
  }
...
async function main() {
  ...
  const tempState = await createE2eStateDir("natesclaw-system-agent-first-run-");
  ...
  setEnvValue("NATESCLAW_CONFIG_PATH", configPath);
  ...
  clearConfigCache();
  assert(
    await shouldStartOnboardingForFreshInstall(["node", "natesclaw"]),
    "fresh bare Natesclaw invocation did not route to onboarding",
  );

  const blocked = await runPackagedCli(["setup", "--message", "overview"]);
  assert(blocked.code === 1, "Natesclaw did not fail closed without inference");
  assert(
    `${blocked.stdout}\n${blocked.stderr}`.includes("natesclaw onboard"),
    "blocked Natesclaw did not direct the user to inference onboarding",
  );
  ...
  const inferenceConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as NatesclawConfig;
  assert(
  ...
  assert(
    ...
    "inference activation configured the rest before Natesclaw started",
  );
  ...
  assert(
    ...
    "modern compatibility entrypoint did not expose Natesclaw after activation",
  );
  ...
  for (const command of spec.commands) {
    ...
    assert(
      ...
      `Natesclaw first-run command ${command.id} did not apply: ${output}`,
    );
    if (command.id === "setup") {
      assert(result.code === 0, `Natesclaw setup exited with ${result.code}: ${output}`);
      assert(
        output.includes("[natesclaw] done: natesclaw.setup"),
        `Natesclaw setup did not report completion: ${output}`,
      );
      assert(
        output.includes(
          "Gateway: Natesclaw gateway lifecycle is managed by an external supervisor " +
            "(NATESCLAW_SUPERVISOR_MODE=external). Use that supervisor to start the gateway.",
        ),
        `Natesclaw setup did not report the externally supervised gateway: ${output}`,
      );
      assert(
        !output.includes("Systemd user services are not available"),
        `Natesclaw setup probed systemd before honoring external supervision: ${output}`,
      );
      assert(
        !output.includes("Gateway service install failed"),
        `Natesclaw setup attempted and failed gateway service installation: ${output}`,
      );
      assert(
        !output.includes("service management skipped: non-default state dir or config path"),
        `Natesclaw setup used the non-default-path service-management skip: ${output}`,
      );
    }
    if (command.planner) {
      assert(
        output.includes(`[natesclaw] planner: ${spec.model}`) &&
          output.includes(FAKE_PLANNER_REPLY) &&
          output.includes(`[natesclaw] interpreted: ${plannerCommand}`),
        `Natesclaw first-run command ${command.id} did not use the verified planner: ${output}`,
      );
    }
    ...
    assert(
      ...
      `Natesclaw command ${command.id} ran ${probeDelta} inference probes; expected at least ${minimumProbes} for preflight${command.approve ? " plus its persistent boundary" : ""}`,
    );
  }
  ...
  assert(reef, "Natesclaw did not create reef agent");
  assert(reef.workspace === spec.dockerAgentWorkspace, "Natesclaw did not write reef workspace");
  assert(
    reef.model === undefined,
    "Natesclaw wrote a per-agent model instead of inheriting the verified default",
  );
  assert(config.channels?.discord?.enabled === true, "Natesclaw did not enable Discord");
  ...
  assert(
    ...
    "Natesclaw did not write Discord token SecretRef",
  );
  assert(
    !JSON.stringify(config.channels.discord).includes(DISCORD_CREDENTIAL_FIXTURE),
    "Natesclaw persisted the raw Discord token",
  );
  ...
  assert(
    ...
    "Natesclaw persisted an unrelated ambient credential",
  );
  ...
  console.log("Natesclaw first-run Docker E2E passed");
}

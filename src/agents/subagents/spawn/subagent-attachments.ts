...
function resolveAttachmentLimits(config: NatesclawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".natesclaw", "attachments");
  const relDir = path.posix.join(".natesclaw", "attachments", attachmentId);

/** Builds the interactive `natesclaw secrets configure` target list and apply plan. */
...
/** Credential target shown by `natesclaw secrets configure` before a SecretRef is selected. */
...
function getSecretProviders(config: NatesclawConfig): Record<string, SecretProviderConfig> {
...
function configureCandidateSortKey(candidate: ConfigureCandidate): string {
  ...
  return `natesclaw:${candidate.path}`;
}
...
/** Builds configure candidates for Natesclaw config plus an optional auth-profile scope. */
export function buildConfigureCandidatesForScope(params: {
  config: NatesclawConfig;
  authoredNatesclawConfig?: NatesclawConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredNatesclawConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const natesclawCandidates = discoverConfigSecretTargets(params.config)
  ...
  return [...natesclawCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "natesclaw secrets configure",

...
function listOrphanAgentDirs(cfg: NatesclawConfig, stateDir: string): OrphanAgentDir[] {
...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".natesclaw"].map((dir) => path.resolve(root, entry.name, dir));
...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for NATESCLAW_STATE_DIR.",
  ].join("\n");
}
...
export function formatLinuxVolatileStateDirWarning(
  ...
  return [
    ...
    "- Move NATESCLAW_STATE_DIR to a persistent filesystem to avoid data loss.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // NATESCLAW_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
...
function shouldRequireOAuthDir(cfg: NatesclawConfig, env: NodeJS.ProcessEnv): boolean {
  if (env.NATESCLAW_OAUTH_DIR?.trim()) {
    return true;
  }
  ...
  switch (issue.kind) {
    case "mac-cloud-state-dir":
      return {
        ...
        path: issue.path,
        fixHint: "Move NATESCLAW_STATE_DIR to local non-synced storage such as ~/.natesclaw.",
      };
    case "linux-sd-state-dir":
      return {
        ...
        fixHint: "Move NATESCLAW_STATE_DIR to SSD/NVMe-backed storage.",
      };
    case "linux-volatile-state-dir":
      return {
        ...
        path: issue.path,
        target: issue.mountPoint,
        fixHint: "Move NATESCLAW_STATE_DIR to persistent local storage.",
      };
    ...
    case "state-dir-not-writable":
      return {
        ...
        path: issue.path,
        fixHint: "Run `natesclaw doctor --fix` to repair state directory permissions.",
      };
    case "state-dir-too-open":
      return {
        ...
        path: issue.path,
        fixHint: "Run `natesclaw doctor --fix` to tighten state directory permissions.",
      };
    case "config-file-too-open":
      return {
        ...
        path: issue.path,
        fixHint: "Run `natesclaw doctor --fix` to tighten config file permissions.",
      };
    case "missing-runtime-dir":
      return {
        ...
        path: issue.path,
        fixHint: "Run `natesclaw doctor --fix` to create missing runtime state directories.",
      };
    case "runtime-dir-not-writable":
      return {
        ...
        path: issue.path,
        fixHint: "Run `natesclaw doctor --fix` to repair runtime state directory permissions.",
      };
  }
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        "- Prefer a local non-synced state dir (for example: ~/.natesclaw).",
        `  Set locally: NATESCLAW_STATE_DIR=~/.natesclaw ${formatCliCommand("natesclaw doctor")}`,
      ].join("\n"),
    );
  }
  if (linuxSdBackedStateDir) {
    ...
    if (missing.length > 0) {
      warnings.push(
        [
          ...
          `  Verify sessions in store: ${formatCliCommand(`natesclaw sessions --store "${absoluteStorePath}"`)}`,
          `  Preview cleanup impact: ${formatCliCommand(`natesclaw sessions cleanup --store "${absoluteStorePath}" --dry-run --fix-missing`)}`,
          `  Prune missing entries: ${formatCliCommand(`natesclaw sessions cleanup --store "${absoluteStorePath}" --enforce --fix-missing`)}`,
        ].join("\n"),
      );
    }
    ...
    if (wedgedSubagentSessions.length > 0) {
      ...
      warnings.push(
        [
          ...
          "  Natesclaw will not auto-resume these child sessions on restart; reconcile their task records instead.",
          `  Examples: ${wedgedSubagentSessions
            .slice(0, 3)
            .map(([key]) => key)
            .join(", ")}`,
          `  Fix: ${formatCliCommand("natesclaw tasks maintenance --apply")}`,
        ].join("\n"),
      );
...
export function collectWorkspaceBackupTip(workspaceDir: string): string | null {
  ...
  return "- Tip: back up the agent workspace in a private git repo; keep ~/.natesclaw out of git (credentials, sessions). Details: /concepts/agent-workspace#git-backup-recommended";
}

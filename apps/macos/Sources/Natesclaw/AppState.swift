...
final class AppState {
    private static let logger = Logger(subsystem: "ai.natesclaw", category: "app-state")
    ...
    init(
        ...
        gatewayConfigSaver: @escaping ([String: Any]) -> Bool = { NatesclawConfigFile.saveDict($0) })
    {
        ...
        let configRoot = NatesclawConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            NatesclawConfigFile.canonicalHostForComparison(host) ==
                NatesclawConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
...
extension AppState {
    ...
    private func startConfigWatcher() {
        let configUrl = NatesclawConfigFile.url()
    ...
    private func applyConfigFromDisk() {
        ...
        NotificationCenter.default.post(name: .natesclawConfigDidChange, object: nil)
    }
...
extension AppState {
    ...
    private func syncGatewayConfigNow(remoteTLSFingerprintUpdate: RemoteTLSFingerprintUpdate) -> Bool {
        ...
        let currentRoot = NatesclawConfigFile.loadDict()
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/natesclaw"
        state.remoteCliPath = ""
        return state
    }
}

---
summary: "PixVerse video generation setup in Natesclaw"
title: "PixVerse"
read_when:
  - You want to use PixVerse video generation in Natesclaw
  - You need the PixVerse API key/env setup
  - You want to make PixVerse the default video provider
---

Natesclaw provides `pixverse` as an official external plugin for hosted PixVerse video generation. The plugin registers the `pixverse` provider against the `videoGenerationProviders` contract.
...
<Steps>
  ...
  <Step title="Set the API key">
    ```bash
    natesclaw onboard --auth-choice pixverse-api-key
    ```
  ...
  <Step title="Switch an existing default video provider (optional)">
    ```bash
    natesclaw config set agents.defaults.mediaModels.video.primary "pixverse/v6"
    ```
  </Step>
...
The provider exposes PixVerse generation models through Natesclaw's shared video tool.
...
<Note>
PixVerse image template generation is not exposed through `image_generate` yet. That API is template-id driven, while Natesclaw's shared image-generation contract does not currently have a PixVerse-specific typed option bag.
</Note>
...
<AccordionGroup>
  <Accordion title="API region">
    ...
    `natesclaw onboard --auth-choice pixverse-api-key` to choose one in the
    setup wizard:
  ...
  <Accordion title="Task polling">
    PixVerse returns a `video_id` from the generation request. Natesclaw polls

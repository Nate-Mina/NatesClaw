...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentWorkspaceDir: vi.fn(() => "/tmp/natesclaw-agent"),
  resolveAgentDir: vi.fn(() => "/tmp/natesclaw-agent/.natesclaw-agent"),
}));
...
function requireFirstRunOptions(): Record<string, unknown> {
  ...
  if (!call) {
    throw new Error("expected embedded Natesclaw agent run");
  }
  const [options] = call;
  if (!options || typeof options !== "object") {
    throw new Error("expected embedded Natesclaw agent run options");
  }

...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.natesclaw.mac`) even if the shipped
    ...
    {
        if environment["NATESCLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "natesclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "natesclaw.nixMode") { return true }

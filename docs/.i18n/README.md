...
- repo: `natesclaw/docs`
- local checkout: `~/Projects/natesclaw-docs`

## Source of truth

- English docs are authored in `natesclaw/natesclaw`.
...
1. Edit English docs in `natesclaw/natesclaw`.
2. Push to `main`.
3. `natesclaw/natesclaw/.github/workflows/docs-sync-publish.yml` mirrors the docs tree into `natesclaw/docs`.
...
5. `natesclaw/docs/.github/workflows/translate-all.yml` waits for `main` to settle, translates only stale or missing locale pages, and uploads per-locale artifacts.
...
- Docs translation workflows generate the same non-English locale set in `natesclaw/docs`.
...
- Do not treat locale visibility in generated `docs/docs.json` as proof that translation artifacts exist. Verify each generated locale folder and its translation memory in `natesclaw/docs`.
...
- Sync metadata is written to `.natesclaw-sync/source.json` in the publish repo.
- Source repo secret: `NATESCLAW_DOCS_SYNC_TOKEN`
- Publish repo secret: `NATESCLAW_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the `Translate All` workflow in `natesclaw/docs` first.

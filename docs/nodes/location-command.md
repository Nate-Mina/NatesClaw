...
- `location.get` is a node command, invoked via `node.invoke` or `natesclaw nodes location get`.
- Off by default.
...
The bundled Linux Node plugin adds `location.get` to the CLI `natesclaw node` service, including headless hosts without the Linux desktop app. Location defaults to off. Enable it under the plugin entry, then restart the node service:
...
- CLI: `natesclaw nodes location get --node <id>`.
- Agent guidelines: only call when the user enabled location and understands the scope.

## UX copy (suggested)

- Off: "Location sharing is disabled."
- While Using: "Only when Natesclaw is open."
- Always: "Allow requested location checks while Natesclaw is in the background."

...
Use for background feature builds, PR reviews, large refactors, and issue-to-PR loops. Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.natesclaw`, `$NATESCLAW_STATE_DIR`, or active Natesclaw state dirs.
...
- Worker must send completion/failure via `natesclaw message send`.
...
- Never checkout branches or run background coding agents in `~/Projects/natesclaw`; use an isolated checkout.
...
6. Launch the worker in the isolated worktree, never the primary checkout. For Natesclaw, the primary checkout under `~/Projects/natesclaw` remains forbidden.
...
natesclaw message send --channel <channel> --target '<target>' --message '<brief result>'
Add --account, --reply-to, or --thread-id only when present above.
Do not use natesclaw system event or heartbeat.
```
...
```bash
PROMPT=$(mktemp -t natesclaw-worker-prompt.XXXXXX)
cat >"$PROMPT" <<'EOF'
Task.
...
OpenAI auth overrides. Never export the worker home into the Natesclaw Gateway
environment.

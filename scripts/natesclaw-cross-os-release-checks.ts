...
async function main(argv: string[]) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `natesclaw@${previousVersion}` : "natesclaw@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.NATESCLAW_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.NATESCLAW_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,

...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["NATESCLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/natesclaw")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("natesclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        let suffix = AppProfile.current.name.map { "-\($0)" } ?? ""
        return logDir.appendingPathComponent("natesclaw-gateway\(suffix).log")
    }
    ...
    /// Returns the newest log file under /tmp/natesclaw/ (rolling or stdout), or nil if none exist.

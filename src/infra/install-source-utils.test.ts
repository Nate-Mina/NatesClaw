...
const TEMP_DIR_PREFIX = "natesclaw-install-source-utils-";
...
async function expectPackFallsBackToDetectedArchive(params: {
  ...
  const cwd = await createTempDir("natesclaw-install-source-utils-");
  const archivePath = path.join(cwd, "natesclaw-plugin-1.2.3.tgz");
...
describe("withInstallWorkspace", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withInstallWorkspace("natesclaw-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
  ...
  it.each([
    {
      ...
      path: async () => "/tmp/does-not-exist-natesclaw-archive.tgz",
...
describe("resolveNpmSpecMetadata", () => {
  ...
  it("accepts metadata without an natesclaw block", async () => {
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "natesclaw-plugin@1.2.3",
          name: "natesclaw-plugin",
          version: "1.2.3",
          filename: "natesclaw-plugin-1.2.3.tgz",
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "natesclaw-plugin",
        version: "1.2.3",
        resolvedSpec: "natesclaw-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "natesclaw-plugin@1.2.3", "--ignore-scripts", "--json"],
      {
        cwd,
        timeoutMs: 300_000,
        signal,
  ...
  it("unpacks npm 12 name-keyed pack json output", async () => {
    ...
    const result = await runPack("natesclaw-plugin@1.2.3", cwd);
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "natesclaw-plugin-1.2.3.tgz");
    await fs.writeFile(expectedArchivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: "npm notice created package\nnatesclaw-plugin-1.2.3.tgz\n",
    });
  ...
  it.each([
    ...
    },
    {
      ...
      stdout:
        'npm notice using cache\n[{"id":"natesclaw-plugin@1.2.3","name":"natesclaw-plugin","version":"1.2.3","integrity":"sha512-test-integrity","shasum":"abc123"}]\n',
      expectedMetadata: {
        name: "natesclaw-plugin",
        version: "1.2.3",
        resolvedSpec: "natesclaw-plugin@1.2.3",
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("natesclaw-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "natesclaw-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
...
describe("resolveNpmPackArchiveMetadata", () => {
  it("reads archive metadata from npm <=11 array pack output", async () => {
    ...
    expect(result).toEqual({
      ok: true,
      archivePath,
      tarballName: "natesclaw-plugin-1.2.3.tgz",
      metadata: {
        name: "natesclaw-plugin",
        version: "1.2.3",
        resolvedSpec: "natesclaw-plugin@1.2.3",

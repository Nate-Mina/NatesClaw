...
export function resolveAcpInstallCommandHint(cfg: NatesclawConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `natesclaw plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^natesclaw plugins install /u, "");
    ...
    return "natesclaw plugins install acpx";
  }

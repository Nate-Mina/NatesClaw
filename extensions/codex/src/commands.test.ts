...
import { getSessionBindingService } from "natesclaw/plugin-sdk/conversation-binding-runtime";
import { MODEL_SELECTION_LOCKED_MESSAGE } from "natesclaw/plugin-sdk/model-session-runtime";
import type { PluginCommandContext, PluginCommandResult } from "natesclaw/plugin-sdk/plugin-entry";
...
function expectedDiagnosticsTargetBlock(params: {
  ...
  return [
    ...
    ...(params.sessionKey ? [`Natesclaw session key: \`${params.sessionKey}\``] : []),
    ...(params.sessionId ? [`Natesclaw session id: \`${params.sessionId}\``] : []),
...
describe("codex command", () => {
  beforeEach(async () => {
    resetCodexTestBindingStore();
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-codex-command-"));
    vi.stubEnv("NATESCLAW_STATE_DIR", tempDir);
  });
  ...
  it("lists Codex sub-plugins through the /codex plugins command surface", async () => {
    ...
    expectResultTextContains(result, "natesclaw.json");
  });
  ...
  it("enables and disables Codex sub-plugins through the /codex plugins command surface", async () => {
    ...
    expectResultTextContains(disabled, "google-calendar: disabled in natesclaw.json");
    ...
    expectResultTextContains(enabled, "google-calendar: enabled in natesclaw.json");
  ...
  it("attaches the current session to an existing Codex thread", async () => {
    ...
    await expect(
      ...
      text: "Attached this Natesclaw session to Codex thread thread-123.",
    });
  ...
  it("publishes manual resume ownership on its responding physical client before returning", async () => {
    ...
    try {
      ...
      expect(result.text).toContain("Attached this Natesclaw session");
      await expect(
        testCodexAppServerBindingStore.read({
          kind: "session",
          agentId: "main",
      ...
      try {
        ...
        expect(result.text).toContain(
          rejectOldRelease
            ...
            : "Attached this Natesclaw session",
        );
  ...
  it("preserves known native config ownership when manually resuming the same thread", async () => {
    ...
    try {
      await expect(
        ...
        text: "Attached this Natesclaw session to Codex thread thread-known-resume.",
      });
      await expect(testCodexAppServerBindingStore.read(identity)).resolves.toMatchObject({
        dynamicToolsFingerprint: "known-dynamic-tools",
        pluginAppsFingerprint: "known-plugin-apps",
  ...
  it("rejects manual resume of a thread owned by another Natesclaw session", async () => {
    const otherIdentity = {
      kind: "session" as const,
      agentId: "main",
    ...
    expect(result.text).toContain("owned by another Natesclaw session");
  ...
  it("reclaims an unloaded plugin's stale generation before attaching a thread", async () => {
    ...
    expect(result.text).toBe("Attached this Natesclaw session to Codex thread thread-new.");
  ...
  it("does not report a resumed thread as attached after a generation conflict", async () => {
    ...
    expect(result.text).not.toContain("Attached this Natesclaw session");
  });
  ...
  it.each([
    ...
    expect(result.text).toContain(
      "Codex-native /codex " +
        args.split(/\s+/u)[0] +
        " is unavailable because Natesclaw sandboxing is active for this session.",
    );
    expect(codexControlRequest).not.toHaveBeenCalled();
  ...
  it.each([
    ...
    expect(result.text).toContain(
      "Codex-native /codex " +
        args.split(/\s+/u)[0] +
        " is unavailable because Natesclaw exec host=node is active for this session.",
    );
    expect(codexControlRequest).not.toHaveBeenCalled();
    expect(steerCodexConversationTurn).not.toHaveBeenCalled();
    expect(setCodexConversationModel).not.toHaveBeenCalled();
  ...
  it("blocks config-level exec host=node without a session key", async () => {
    ...
    expect(result.text).toContain(
      "Codex-native /codex bind is unavailable because Natesclaw exec host=node is active for this session.",
    );
  });
  ...
  it("explains compaction when no Codex thread is attached", async () => {
    ...
    await expect(
      ...
      text: "No Codex thread is attached to this Natesclaw session yet.",
    });
  });
  ...
  it("previews exec-approved diagnostics upload without exposing Codex ids", async () => {
    ...
    expect(result.text).toBe(
      [
        ...
        "The completed diagnostics reply will list the Natesclaw session ids and Codex thread ids that were sent.",
  ...
  it("uploads all Codex diagnostics sessions and reports their channel/thread breakdown", async () => {
    ...
    expect(request.text).toContain("Natesclaw session key: `agent:first:whatsapp:one`");
    expect(request.text).toContain("Natesclaw session id: `session-one`");
    expect(request.text).toContain("Codex thread id: `thread-111`");
    expect(request.text).toContain("Natesclaw session key: `agent:second:discord:two`");
    expect(request.text).toContain("Natesclaw session id: `session-two`");
  ...
  it("uses the host agent for diagnostics inventory sessions with unscoped keys", async () => {
    ...
    expect(request.text).toContain("Natesclaw session key: `global`");
  ...
  it("sanitizes diagnostics upload errors before showing them", async () => {
    ...
    await expect(
      ...
      text: [
        ...
        "- channel test, Natesclaw session session-1, Codex thread &lt;\uff20U123&gt;: bad??? &lt;\uff20U123&gt; \uff3btrusted\uff3d\uff08https://evil\uff09 \uff20here",
        "Inspect locally:",
  ...
  it("keeps diagnostics upload errors UTF-16 safe at the display boundary", async () => {
    ...
    await expect(
      ...
      text: [
        "Could not send Codex diagnostics:",
        `- channel test, Natesclaw session session-1, Codex thread thread-error-boundary: ${expectedError}`,
        "Inspect locally:",
  ...
  it("does not throttle diagnostics retries after upload failures", async () => {
    ...
    await expect(
      ...
      text: [
        "Could not send Codex diagnostics:",
        "- channel test, Natesclaw session session-1, Codex thread thread-retry: temporary outage",
        "Inspect locally:",
  ...
  it("omits inline diagnostics resume commands for unsafe thread ids", async () => {
    ...
    await expect(
      ...
      text: [
        ...
        "Session 1",
        "Channel: test",
        "Natesclaw session id: `session-1`",
  ...
  it("explains diagnostics when no Codex thread is attached", async () => {
    ...
    await expect(
      ...
      text: [
        "No Codex thread is attached to this Natesclaw session yet.",
  ...
  it("reports a conversation-bound model without an Natesclaw session identity", async () => {
    await writeTestBinding(
      { kind: "conversation", bindingId: "binding-data-1" },

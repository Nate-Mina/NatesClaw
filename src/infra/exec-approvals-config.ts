...
const DEFAULT_EXEC_APPROVALS_STATE_DIR = "~/.natesclaw";
...
export function resolveExecApprovalsDisplayPath(): string {
  ...
  const locator = path.join("state", "natesclaw.sqlite#exec_approvals_config");
...
export function resolveExecApprovalsTranscriptPath(): string {
  return process.env.NATESCLAW_STATE_DIR?.trim()
    ? "$NATESCLAW_STATE_DIR/state/natesclaw.sqlite#exec_approvals_config"
    : `${DEFAULT_EXEC_APPROVALS_STATE_DIR}/state/natesclaw.sqlite#exec_approvals_config`;
}

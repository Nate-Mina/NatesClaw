...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when NATESCLAW_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".natesclaw", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when NATESCLAW_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", NATESCLAW_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".natesclaw-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers NATESCLAW_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        NATESCLAW_PROFILE: "rescue",
        NATESCLAW_STATE_DIR: "C:\\State\\natesclaw",
      },
      expected: path.join("C:\\State\\natesclaw", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", NATESCLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".natesclaw", "gateway.cmd"),
    },
    {
      name: "uses a custom task script file name inside the state directory",
      env: {
        USERPROFILE: "C:\\Users\\test",
        NATESCLAW_TASK_SCRIPT_NAME: "gateway-node.cmd",
      },
      expected: path.join("C:\\Users\\test", ".natesclaw", "gateway-node.cmd"),
    },
  ])("$name", ({ env, expected }) => {
  ...
  it.each([
    ...
    expect(() =>
      resolveTaskScriptPath({
        USERPROFILE: "C:\\Users\\test",
        NATESCLAW_TASK_SCRIPT_NAME: scriptName,
      }),
    ).toThrow("NATESCLAW_TASK_SCRIPT_NAME must be a file name only");
  });
});
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-schtasks-test-"));
    try {
  ...
  it("reads legacy UTF-8 scripts with CJK paths written before the encoding fix", async () => {
    await withScheduledTaskScript(
      {
        scriptLines: ["@echo off", 'cd /d "C:\\Users\\苗振\\.natesclaw"', "node gateway.js"],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: ["node", "gateway.js"],
          workingDirectory: "C:\\Users\\苗振\\.natesclaw",
          sourcePath: resolveTaskScriptPath(env),
        });
      },
    );
  ...
  it("reads back GBK launchers whose bytes are also valid UTF-8 (隆) without corruption", async () => {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: ["@echo off", 'cd /d "C:\\Users\\隆\\.natesclaw"', "node gateway.js"],
        scriptEncoding: "gbk",
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: ["node", "gateway.js"],
          workingDirectory: "C:\\Users\\隆\\.natesclaw",
          sourcePath: resolveTaskScriptPath(env),
        });
      },
    );
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Natesclaw Gateway",
          "cd /d C:\\Projects\\natesclaw",
          "set NODE_ENV=production",
          "set NATESCLAW_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          environment: {
            NODE_ENV: "production",
            NATESCLAW_PORT: "18789",
          },
          environmentValueSources: {
            NODE_ENV: "inline",
            NATESCLAW_PORT: "inline",
          },
          sourcePath: resolveTaskScriptPath(env),
        });
      },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\natesclaw\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Natesclaw Share\\node.exe" "\\\\fileserver\\Natesclaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Natesclaw Share\\node.exe",
            "\\\\fileserver\\Natesclaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from NATESCLAW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ NATESCLAW_STATE_DIR: path.join(tmpDir, "custom-state") }),

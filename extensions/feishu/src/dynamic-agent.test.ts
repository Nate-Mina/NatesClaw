...
beforeEach(async () => {
  tempRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "natesclaw-feishu-agent-"));
});
...
function createRuntime(
  ...
  persistedCfg?: NatesclawConfig,
  mutationCfg?: NatesclawConfig,
) {
  let runtimeCfg = structuredClone(currentCfg ?? ({} as NatesclawConfig));
  const commitConfig = vi.fn();
  const mutateConfigFile = vi.fn(
    async (params: {
      mutate: (draft: NatesclawConfig, context: { snapshot: never; previousHash: null }) => unknown;
    }) => {
...
describe("maybeCreateDynamicAgent", () => {
  ...
  it("rechecks current ingress inside the config mutation lock", async () => {
    ...
    const canCreateForConfig = vi
      .fn<(cfg: NatesclawConfig) => Promise<boolean>>()
  ...
  it("preserves concurrent runtime config when creating from a stale request snapshot", async () => {
    ...
    const result = await maybeCreateDynamicAgent({
      cfg: { agents: createCanonicalAgentRoster(), bindings: [] } as NatesclawConfig,
      runtime,
      accountId: "default",
      senderOpenId: "ou_sender",
      canCreateForConfig: async () => true,

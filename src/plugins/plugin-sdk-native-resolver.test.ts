...
import {
  installNatesclawInternalCorePackageNativeResolver,
...
function writeFakeNatesclawPackage(root: string): { distRoot: string; loaderModulePath: string } {
  ...
  fs.writeFileSync(path.join(root, "natesclaw.mjs"), "#!/usr/bin/env node\n", "utf8");
...
function createInternalCoreAliasFixture(prefix: string): {
  ...
  const { loaderModulePath } = writeFakeNatesclawPackage(root);
...
describe("installNatesclawInternalCorePackageNativeResolver", () => {
  it("shares one internal core alias scan between resolver installers", () => {
    const fixture = createInternalCoreAliasFixture("natesclaw-sdk-native-core-cache-");
    ...
    try {
      ...
      const aliases = installNatesclawInternalCorePackageNativeResolver({
        moduleUrl: fixture.moduleUrl,
      });

      expect(aliases).toContain("@natesclaw/markdown-core/code-spans");
  ...
  it("shares one internal core alias scan across importers from the same host package", () => {
    const fixture = createInternalCoreAliasFixture("natesclaw-sdk-native-core-shared-host-");
    ...
    try {
      installNatesclawInternalCorePackageNativeResolver({ moduleUrl: fixture.moduleUrl });
      expect(existsSync).toHaveBeenCalledWith(fixture.sourcePath);
      ...
      existsSync.mockClear();
      readFileSync.mockClear();
      installNatesclawInternalCorePackageNativeResolver({ moduleUrl: secondModuleUrl });
  ...
  it("keeps internal core alias registration isolated between host modules", () => {
    const first = createInternalCoreAliasFixture("natesclaw-sdk-native-core-host-a-");
    const second = createInternalCoreAliasFixture("natesclaw-sdk-native-core-host-b-");
    const existsSync = vi.spyOn(fs, "existsSync");

    try {
      installNatesclawInternalCorePackageNativeResolver({ moduleUrl: first.moduleUrl });
      existsSync.mockClear();

      installNatesclawInternalCorePackageNativeResolver({ moduleUrl: second.moduleUrl });

      expect(existsSync).toHaveBeenCalledWith(second.sourcePath);
      expect(
        fs.realpathSync(
          createRequire(first.coreSourceParent).resolve("@natesclaw/markdown-core/code-spans"),
        ),
      ).toBe(fs.realpathSync(first.sourcePath));
      expect(
        fs.realpathSync(
          createRequire(second.coreSourceParent).resolve("@natesclaw/markdown-core/code-spans"),
        ),
  ...
  it("rescans internal core aliases after plugin metadata lifecycle invalidation", () => {
    const fixture = createInternalCoreAliasFixture("natesclaw-sdk-native-core-invalidation-");
    const existsSync = vi.spyOn(fs, "existsSync");
...
describe("installNatesclawPluginSdkNativeResolver", () => {
  it("resolves installed plugin SDK imports to the dev source root", () => {
    const stableRoot = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-sdk-native-stable-"));
    const devRoot = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-sdk-native-dev-source-"));
    const { loaderModulePath } = writeFakeNatesclawPackage(stableRoot);
    writeFakeNatesclawPackage(devRoot);
    ...
    const previousDevSourceRoot = process.env.NATESCLAW_DEV_SOURCE_ROOT;
    process.env.NATESCLAW_DEV_SOURCE_ROOT = devRoot;

    try {
      const installedAliases = installNatesclawPluginSdkNativeResolver({
        modulePath: loaderModulePath,
        pluginModulePath: externalPluginEntry,
      });

      expect(installedAliases).toContain("natesclaw/plugin-sdk/agent-runtime");
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(fs.realpathSync(requireFromPlugin.resolve("natesclaw/plugin-sdk/agent-runtime"))).toBe(
        fs.realpathSync(path.join(devRoot, "dist", "plugin-sdk", "agent-runtime.js")),
      );
    } finally {
      if (previousDevSourceRoot === undefined) {
        delete process.env.NATESCLAW_DEV_SOURCE_ROOT;
      } else {
        process.env.NATESCLAW_DEV_SOURCE_ROOT = previousDevSourceRoot;
      }
    }
  });
  ...
  it("removes stale native SDK aliases when a later dev root omits a subpath", () => {
    ...
    expect(fs.realpathSync(requireFromPlugin.resolve("natesclaw/plugin-sdk/stable-extra"))).toBe(
    ...
    expect(() => requireFromPlugin.resolve("natesclaw/plugin-sdk/stable-extra")).toThrow();
  });

  it("keeps native aliases on JS dist artifacts when source files exist", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-sdk-native-source-resolver-"));
    const { loaderModulePath } = writeFakeNatesclawPackage(root);
    ...
    expect(installedAliases).toContain("natesclaw/plugin-sdk/channel-outbound");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(fs.realpathSync(requireFromPlugin.resolve("natesclaw/plugin-sdk/channel-outbound"))).toBe(
      fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")),
    );
  });

  it("lets built external plugins resolve Natesclaw SDK subpaths with createRequire", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-sdk-native-resolver-"));
    const { distRoot, loaderModulePath } = writeFakeNatesclawPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    ...
    try {
      ...
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(
        fs.realpathSync(requireFromPlugin.resolve("natesclaw/plugin-sdk/channel-outbound")),
      ).toBe(fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")));
      const sdk = requireFromPlugin("natesclaw/plugin-sdk/channel-outbound") as {
        defineChannelMessageAdapter?: () => string;
      };

      expect(sdk.defineChannelMessageAdapter?.()).toBe("adapter");
      expect(() => requireFromPlugin.resolve("natesclaw/not-plugin-sdk/channel-message")).toThrow();
    } finally {
      if (process.platform !== "win32") {
  ...
  beforeAll(() => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-sdk-native-esm-resolver-"));
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'import path from "node:path";',
        'import { pathToFileURL } from "node:url";',
        `import { installNatesclawPluginSdkNativeResolver } from ${JSON.stringify(resolverModuleUrl)};`,
        ...
        "fs.writeFileSync(",
        "  entryPath,",
        '  "import { defineChannelMessageAdapter } from \\"natesclaw/plugin-sdk/channel-outbound\\"; export const eager = defineChannelMessageAdapter(); export const loadLazy = () => import(\\"./lazy.js\\");\\n",',
        '  "utf8",',
        ");",
        "fs.writeFileSync(",
        "  lazyPath,",
        '  "import { defineChannelMessageAdapter } from \\"natesclaw/plugin-sdk/channel-outbound\\"; export const lazy = defineChannelMessageAdapter();\\n",',
        '  "utf8",',
        ");",
        "installNatesclawPluginSdkNativeResolver({",
        "  modulePath: loaderModulePath,",
  ...
  it("does not resolve SDK aliases for parents outside registered plugin roots", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-sdk-native-guard-"));
    const { loaderModulePath } = writeFakeNatesclawPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    const unrelatedRoot = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-sdk-native-outside-"));
    ...
    expect(requireFromPlugin.resolve("natesclaw/plugin-sdk/channel-outbound")).toBeTruthy();
    expect(() => requireFromOutside.resolve("natesclaw/plugin-sdk/channel-outbound")).toThrow();
  });

  it("resolves internal core packages only for Natesclaw-owned source parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-sdk-native-core-internal-"));
    const { loaderModulePath } = writeFakeNatesclawPackage(root);
    ...
    expect(installedAliases).toContain("@natesclaw/normalization-core/string-coerce");
    expect(installedAliases).toContain("@natesclaw/normalization-core/boolean-coercion");
    expect(installedAliases).toContain("@natesclaw/normalization-core/result");
    expect(installedAliases).toContain("@natesclaw/normalization-core/agent-id");
    expect(installedAliases).toContain("@natesclaw/media-core/mime");
    expect(installedAliases).toContain("@natesclaw/markdown-core/code-spans");
    expect(installedAliases).toContain("@natesclaw/ai/transports");
    expect(installedAliases).toContain("@natesclaw/ai/internal/retry-after");
    expect(installedAliases).toContain("@natesclaw/ai/internal/runtime");
    expect(installedAliases).toContain("@natesclaw/acp-core/runtime/types");
    expect(installedAliases).toContain("@natesclaw/llm-core");
    const requireFromCoreSource = createRequire(coreSourceParent);
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@natesclaw/normalization-core/string-coerce")),
    ).toBe(fs.realpathSync(normalizationSource));
    expect(
      fs.realpathSync(
        requireFromCoreSource.resolve("@natesclaw/normalization-core/boolean-coercion"),
      ),
    ).toBe(fs.realpathSync(booleanCoercionSource));
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@natesclaw/normalization-core/result")),
    ).toBe(fs.realpathSync(resultSource));
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@natesclaw/normalization-core/agent-id")),
    ).toBe(fs.realpathSync(agentIdSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@natesclaw/media-core/mime"))).toBe(
      fs.realpathSync(mediaCoreSource),
    );
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@natesclaw/markdown-core/code-spans")),
    ).toBe(fs.realpathSync(markdownCoreSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@natesclaw/ai/transports"))).toBe(
      fs.realpathSync(aiTransportsSource),
    );
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@natesclaw/ai/internal/retry-after")),
    ).toBe(fs.realpathSync(aiRetryAfterSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@natesclaw/ai/internal/runtime"))).toBe(
      fs.realpathSync(aiRuntimeSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@natesclaw/acp-core/runtime/types"))).toBe(
      fs.realpathSync(acpCoreSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@natesclaw/llm-core"))).toBe(
      fs.realpathSync(llmCoreSource),
    );
    expect(() => requireFromPlugin.resolve("@natesclaw/normalization-core/string-coerce")).toThrow();
    expect(() =>
      requireFromPlugin.resolve("@natesclaw/normalization-core/boolean-coercion"),
    ).toThrow();
    expect(() => requireFromPlugin.resolve("@natesclaw/normalization-core/result")).toThrow();
    expect(() => requireFromPlugin.resolve("@natesclaw/media-core/mime")).toThrow();
    expect(() => requireFromPlugin.resolve("@natesclaw/markdown-core/code-spans")).toThrow();
    expect(() => requireFromPlugin.resolve("@natesclaw/ai/transports")).toThrow();
    expect(() => requireFromPlugin.resolve("@natesclaw/ai/internal/retry-after")).toThrow();
    expect(() => requireFromPlugin.resolve("@natesclaw/ai/internal/runtime")).toThrow();
    expect(() => requireFromPlugin.resolve("@natesclaw/acp-core/runtime/types")).toThrow();
    expect(() => requireFromPlugin.resolve("@natesclaw/llm-core")).toThrow();
  });

  it("does not register source-only SDK subpaths for native resolution", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-sdk-native-source-only-"));
    const { loaderModulePath } = writeFakeNatesclawPackage(root);
    ...
    expect(installedAliases).toContain("natesclaw/plugin-sdk/channel-outbound");
    expect(installedAliases).not.toContain("natesclaw/plugin-sdk/source-only");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(() => requireFromPlugin.resolve("natesclaw/plugin-sdk/source-only")).toThrow();
  });

  it("scopes private SSRF SDK aliases to bundled local IPC native parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-sdk-native-ssrf-"));
    const { loaderModulePath } = writeFakeNatesclawPackage(root);
    ...
    expect(installedAliases).toContain("natesclaw/plugin-sdk/ssrf-runtime-internal");
    const requireFromOllama = createRequire(ollamaEntry);
    expect(
      fs.realpathSync(requireFromOllama.resolve("natesclaw/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeOllama = createRequire(runtimeOllamaEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeOllama.resolve("natesclaw/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromBrowser = createRequire(browserEntry);
    expect(
      fs.realpathSync(requireFromBrowser.resolve("natesclaw/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeBrowser = createRequire(runtimeBrowserEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeBrowser.resolve("natesclaw/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromOther = createRequire(otherEntry);
    expect(() => requireFromOther.resolve("natesclaw/plugin-sdk/ssrf-runtime-internal")).toThrow();
  });
});

...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.NATESCLAW_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.NATESCLAW_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.NATESCLAW_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.NATESCLAW_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
    process.env.NATESCLAW_ENABLE_PRIVATE_QA_CLI = "1";
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      delete process.env.NATESCLAW_ENABLE_PRIVATE_QA_CLI;
    } else {
      process.env.NATESCLAW_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "natesclaw", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "natesclaw"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "natesclaw", "nodes", "list"], "natesclaw");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "natesclaw", "infer"], "natesclaw");
  ...
  it("registers the resume placeholder and dispatches through its lazy registrar", async () => {
    const program = createRegisteredProgram(["node", "natesclaw", "resume"], "natesclaw");
  ...
  it("registers the exec-approvals placeholder and dispatches through the approvals registrar", async () => {
    const program = createRegisteredProgram(["node", "natesclaw", "exec-approvals"], "natesclaw");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "natesclaw", "acp", "--help"], "natesclaw");
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "natesclaw", "gateway", "--force"];
    process.argv = argv;
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "natesclaw", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("natesclaw");
  ...
  it("passes completion context to channel registration", async () => {
    const argv = ["node", "natesclaw", "completion", "--write-state"];
    const program = new Command().name("natesclaw");
  ...
  it.each([
    ...
    process.argv = ["node", "natesclaw", ...args];
    const program = new Command().name("natesclaw");
  ...
  it("does not preload plugin CLI registrations for bare plugin parent help", async () => {
    process.argv = ["node", "natesclaw", "plugins"];
    const program = new Command().name("natesclaw");

...
- NATESCLAW_IOS_VERSION = exact canonical iOS version
- NATESCLAW_MARKETING_VERSION = short iOS/App Store version
- NATESCLAW_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
while IFS='=' read -r key value; do
  case "${key}" in
    NATESCLAW_IOS_VERSION)
      IOS_VERSION="${value}"
      ;;
    NATESCLAW_MARKETING_VERSION)
...
RESOLVED_GIT_COMMIT="$(natesclaw_resolve_git_commit "${ROOT_DIR}")"
RESOLVED_BUILD_TIMESTAMP="$(natesclaw_resolve_build_timestamp)"
...
NATESCLAW_IOS_VERSION = ${IOS_VERSION}
NATESCLAW_MARKETING_VERSION = ${MARKETING_VERSION}
NATESCLAW_BUILD_VERSION = ${BUILD_NUMBER}
NATESCLAW_GIT_COMMIT = ${RESOLVED_GIT_COMMIT}
NATESCLAW_BUILD_TIMESTAMP = ${RESOLVED_BUILD_TIMESTAMP}
EOF

...
metadata: { "natesclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Natesclaw `sessions_spawn`
- **File I/O** == Natesclaw `read`/`write`
- **Remote fetch** == Natesclaw `web_fetch` (or `exec` with curl when POST is required)

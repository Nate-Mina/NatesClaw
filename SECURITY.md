...
Security work is shared across a number of Natesclaw maintainers, including engineers and security researchers from organizations such as NVIDIA and Tencent. See the [maintainer list](CONTRIBUTING.md#maintainers).
...
- The actual impact, including which Natesclaw trust boundary is crossed.
- Any remediation advice or focused patch you can provide.

Reports without reproduction steps, demonstrated impact, and remediation advice are deprioritized. We receive a high volume of AI-generated scanner findings, so we prioritize vetted reports from researchers who can show how the issue crosses an Natesclaw security boundary.
...
- A report whose only primitive is changing the process or child-process environment before running Natesclaw or an executable Natesclaw invokes.
...
- Scanner-only, dependency-only, or stale-path reports without a working repro and demonstrated Natesclaw impact.
...
- Tested version details (Natesclaw version and/or commit SHA).
...
- For dependency CVE reports, evidence that the shipped dependency version is actually affected, plus a PoC that reproduces impact through Natesclaw. Showing that Natesclaw can reach a native parser is not enough by itself.
- Demonstrated impact tied to Natesclaw's documented trust boundaries.
- For exposed-secret reports: proof the credential is Natesclaw-owned (or grants access to Natesclaw-operated infrastructure/services).
...
- Reports that only show untrusted media bytes reaching a maintained native decoder dependency (for example image codec libraries such as libheif) without proving the shipped dependency version is vulnerable and demonstrating crash, memory corruption, data exposure, or a boundary bypass through Natesclaw. JavaScript header sniffing and image dimension fast-paths are preflight/UX checks, not the security boundary for native decoder correctness.
- Reports whose only impact is transient extra memory, CPU, or allocation work from decoding, base64 expansion, media transcoding, serialization, or other format conversion after the input was already accepted under Natesclaw's configured size/trust limits, including base64 decode-before-size-estimate findings. These are performance issues, not vulnerabilities, unless the report demonstrates unauthenticated amplification, bypass of configured limits, crash/process termination, persistent resource exhaustion, data exposure, or another documented boundary bypass.
...
- Reports that depend on attacker-controlled environment variables changing executable behavior, including variables that redirect lookup paths, preload code, select wrappers/interpreters, alter package-manager or runtime hooks, or make one executable call another executable. Control of the process or child-process environment is trusted host/operator control in Natesclaw's model; these reports need a separate Natesclaw boundary bypass that lets untrusted input set or mutate that environment.
...
Natesclaw does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Concretely, on the OpenAI-compatible HTTP surface:
  ...
  - narrower `x-natesclaw-scopes` headers are ignored for that shared-secret path
...
- Natesclaw can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need Natesclaw, use one VPS (or host/OS user boundary) per user.
...
- This is expected in Natesclaw's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

### Trusted Plugins

Plugins/extensions are part of Natesclaw's trusted computing base for a gateway.
...
- Using Natesclaw in ways that the docs recommend not to
- Test-only code and maintainer harnesses, including QA Lab, QE Lab, E2E fixtures, benchmark rigs, smoke-test containers, and local debugging proxies, unless the report demonstrates that the same vulnerable behavior is reachable from shipped Natesclaw production code or a published package artifact intended for users.
...
- Reports that require write access to trusted local state (`~/.natesclaw`, workspace files like `MEMORY.md` / `memory/*.md`)
...
Natesclaw security guidance assumes:

- The host where Natesclaw runs is within a trusted OS/admin boundary.
- Anyone who can set or mutate the Natesclaw process environment, launcher environment, or child-process environment is inside that trusted host/operator boundary.
- Anyone who can modify `~/.natesclaw` state/config (including `natesclaw.json`) is effectively a trusted operator.
...
Natesclaw's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
Natesclaw separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when Natesclaw can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the Natesclaw process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

### Temp Folder Boundary

Natesclaw uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/natesclaw` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/natesclaw` (or `natesclaw-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the Natesclaw-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use Natesclaw temp helpers (`resolvePreferredNatesclawTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-natesclaw-dir.ts`
...
Natesclaw's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `natesclaw gateway run --bind loopback`.
- The retired `gateway.controlUi.dangerouslyDisableDeviceAuth` break-glass key is not a
  ...
  - Natesclaw keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `natesclaw security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__natesclaw__/canvas/`, `/__natesclaw__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__natesclaw__/canvas/`, `/__natesclaw__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

Natesclaw requires **Node.js 22.22.3+, Node.js 24.15+, or Node.js 25.9+**. Node 24 is the recommended default runtime for new installs. These minimum versions include the upstream SQLite WAL-reset corruption fix; Node 23 is unsupported. The minimum supported Node 22 version also includes important security patches:
...
When running Natesclaw in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v natesclaw-data:/app/data \
  natesclaw/natesclaw:latest
```

## Security Scanning

Natesclaw uses several security and release-validation layers. No single scanner is treated as the boundary.

### Secret Detection

Natesclaw runs the pre-commit `detect-private-key` hook in CI and keeps secret-resolution behavior covered by the dedicated secrets test surface.

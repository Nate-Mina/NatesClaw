...
type TestWebKitWindow = Window & {
  webkit?: {
    messageHandlers: {
      natesclawNav: { postMessage: (message: unknown) => void };
    };
  };
};
...
describe("Natesclaw native shell", () => {
  it("opens Settings with Shift-Command-Comma", () => {
    const navigate = vi.fn();
    const shell = document.createElement("natesclaw-app-shell") as unknown as ShellKeyboardState;
    shell.runtime = {
      context: {
        navigate,
      } as unknown as ApplicationContext,
  ...
  it("toggles the navigation sidebar when the native macOS titlebar button fires", () => {
    ...
    const shell = document.createElement("natesclaw-app-shell") as unknown as ShellNavigationState;
    shell.runtime = {
      context: {
  ...
  it("opens search and starts a session from native titlebar events", () => {
    ...
    const toggleEvent = new CustomEvent("natesclaw:native-toggle-search", { cancelable: true });
  ...
  it("navigates valid native Dashboard paths and acknowledges them", () => {
    ...
    const event = new CustomEvent("natesclaw:native-navigate", {
      cancelable: true,
  ...
  it("deduplicates native nav state reports", () => {
    ...
    (window as TestWebKitWindow).webkit = {
      messageHandlers: { natesclawNav: { postMessage } },
    };
...
describe("Natesclaw shell update affordance", () => {
  it("renders a capable floating card only while desktop navigation is collapsed", async () => {
    ...
    expect(container.querySelector("natesclaw-sidebar-update-card")).toBeNull();
    container.remove();
  });

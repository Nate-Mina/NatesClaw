...
async function resolveCatalogChannelEntry(raw: string, cfg: NatesclawConfig | null) {
...
async function channelsAddCommandImpl(
  ...
  const cfg = (configSnapshot.sourceConfig ?? configSnapshot.config) as NatesclawConfig;
  ...
  if (!channel) {
    const hint = catalogEntry
      ? `Plugin ${catalogEntry.meta.label} could not be loaded after install. Run natesclaw doctor --fix, then retry natesclaw channels add.`
  ...
  if (!plugin) {
    runtime.error(
      ...
      })} Run ${formatCliCommand("natesclaw channels add")} with no flags for guided setup.`,
    );
    runtime.exit(1);
    return;
  }

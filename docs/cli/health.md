---
summary: "CLI reference for `natesclaw health` (gateway health snapshot via RPC)"
read_when:
...
natesclaw health --timeout 2500
natesclaw health --verbose
natesclaw health --debug
```
...
- When outbound deliveries or inbound channel events are dead-lettered, text output reports their counts and oldest failure age. Inbound counts are grouped by channel account; inspect or recover individual events with [`natesclaw channels dead-letters`](/cli/channels#inbound-dead-letters).

## Related

- [CLI reference](/cli)
- [`natesclaw status`](/cli/status) — local diagnosis and channel probes without a full health snapshot

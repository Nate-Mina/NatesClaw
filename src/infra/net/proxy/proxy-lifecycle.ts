...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    NO_PROXY: process.env["NO_PROXY"],
    NATESCLAW_PROXY_ACTIVE: process.env["NATESCLAW_PROXY_ACTIVE"],
    NATESCLAW_PROXY_LOOPBACK_MODE: process.env["NATESCLAW_PROXY_LOOPBACK_MODE"],
    NATESCLAW_PROXY_CA_FILE: process.env["NATESCLAW_PROXY_CA_FILE"],
  };
}
...
function applyProxyEnv(
  ...
  process.env["NATESCLAW_PROXY_LOOPBACK_MODE"] = loopbackMode;
  if (proxyCaFile) {
    process.env["NATESCLAW_PROXY_CA_FILE"] = proxyCaFile;
  } else {
    delete process.env["NATESCLAW_PROXY_CA_FILE"];
  }
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["NATESCLAW_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or NATESCLAW_PROXY_URL to an http:// or https:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["NATESCLAW_PROXY_ACTIVE"] !== "1") {
    return;
  }
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    proxyUrl,
    caFileOverride: process.env["NATESCLAW_PROXY_CA_FILE"],
  });
...
export async function startProxy(config: ProxyConfig | undefined): Promise<ProxyHandle | null> {
  if (
    ...
    (!config?.proxyUrl?.trim() && !process.env["NATESCLAW_PROXY_URL"]?.trim())
  ) {
    return null;
  }
 ...
 * loopback CDP probe to a Chromium instance Natesclaw spawned itself.
 *

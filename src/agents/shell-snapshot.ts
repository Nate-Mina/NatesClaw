...
const CAPTURE_MARKER = "__NATESCLAW_SHELL_SNAPSHOT_CAPTURE__";
const ENV_MARKER = "__NATESCLAW_SHELL_SNAPSHOT_ENV__";
  ...
  return await withTempWorkspace(
    {
      ...
      prefix: "natesclaw-shell-snapshot-",
      dirMode: 0o700,
      mode: 0o600,
    },
    async (workspace) => {
...
function buildTrustedSnapshotCaptureEnv(
  ...
  // NATESCLAW_SHELL is injected by the exec runtime, so startup files can keep
  // their documented exec-specific branches without trusting model input.
  if (runtimeEnv.NATESCLAW_SHELL === "exec") {
    env.NATESCLAW_SHELL = "exec";
  }
  return env;
}

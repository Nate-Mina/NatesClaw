...
IMAGE_NAME="$(docker_e2e_resolve_image "natesclaw-kitchen-sink-rpc-e2e" NATESCLAW_KITCHEN_SINK_RPC_E2E_IMAGE)"
MAX_MEMORY_MIB="$(docker_e2e_read_nonnegative_decimal_env NATESCLAW_KITCHEN_SINK_MAX_MEMORY_MIB 2048)"
...
CONTAINER_NAME="natesclaw-kitchen-sink-rpc-e2e-$$"
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/natesclaw-kitchen-sink-rpc.XXXXXX")"
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/natesclaw-kitchen-sink-rpc-stats.XXXXXX")"
...
DOCKER_ENV_ARGS=(
  ...
  -e NATESCLAW_ENTRY=/app/natesclaw.mjs
)

for env_name in \
  NATESCLAW_KITCHEN_SINK_NPM_SPEC \
  NATESCLAW_KITCHEN_SINK_PLUGIN_ID \
  ...
  NATESCLAW_KITCHEN_SINK_RPC_PORT \
  ...
  NATESCLAW_KITCHEN_SINK_KEEP_TMP \
  NATESCLAW_KITCHEN_SINK_MAX_RSS_MIB \
  NATESCLAW_KITCHEN_SINK_COMMAND_MAX_RSS_MIB; do
  env_value="${!env_name:-}"
...
DOCKER_COMMAND_TIMEOUT="$DOCKER_RUN_TIMEOUT" docker_e2e_docker_run_cmd run --name "$CONTAINER_NAME" "${DOCKER_E2E_HARNESS_ARGS[@]}" "${DOCKER_ENV_ARGS[@]}" -i "$IMAGE_NAME" \
  bash -lc "source scripts/lib/natesclaw-e2e-instance.sh; natesclaw_e2e_run_script_entrypoint scripts/e2e/kitchen-sink-rpc-walk" >"$RUN_LOG" 2>&1 &
docker_pid="$!"

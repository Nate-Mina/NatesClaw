...
IMAGE_NAME="$(docker_e2e_resolve_image "natesclaw-kitchen-sink-plugin-e2e" NATESCLAW_KITCHEN_SINK_PLUGIN_E2E_IMAGE)"
NATESCLAW_DOCKER_E2E_LOG_PRINT_BYTES="$(
...
DEFAULT_KITCHEN_SINK_SCENARIOS="$(
  cat <<SCENARIOS
npm-latest-full|${KITCHEN_SINK_NPM_SPEC}|natesclaw-kitchen-sink-fixture|npm|success|full
...
MAX_MEMORY_MIB="$(
  if [[ -n "${NATESCLAW_KITCHEN_SINK_PLUGIN_MAX_MEMORY_MIB:-}" ]]; then
    docker_e2e_read_nonnegative_decimal_env NATESCLAW_KITCHEN_SINK_PLUGIN_MAX_MEMORY_MIB 2304
  else
    docker_e2e_read_nonnegative_decimal_env NATESCLAW_KITCHEN_SINK_MAX_MEMORY_MIB 2304
  fi
)"
MAX_CPU_PERCENT="$(docker_e2e_read_nonnegative_decimal_env NATESCLAW_KITCHEN_SINK_MAX_CPU_PERCENT 1200)"
DOCKER_RUN_TIMEOUT="${NATESCLAW_KITCHEN_SINK_PLUGIN_DOCKER_RUN_TIMEOUT:-1200s}"
KITCHEN_SINK_CLI_TIMEOUT="${NATESCLAW_KITCHEN_SINK_PLUGIN_CLI_TIMEOUT:-${KITCHEN_SINK_CLI_TIMEOUT:-180s}}"
CONTAINER_NAME="natesclaw-kitchen-sink-plugin-e2e-$$"
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/natesclaw-kitchen-sink-plugin.XXXXXX")"
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/natesclaw-kitchen-sink-plugin-stats.XXXXXX")"
...
if [[ "${NATESCLAW_KITCHEN_SINK_LIVE_CLAWHUB:-0}" = "1" ]]; then
  for env_name in \
    NATESCLAW_KITCHEN_SINK_LIVE_CLAWHUB \
    NATESCLAW_CLAWHUB_URL \
    CLAWHUB_URL \
    CLAWHUB_TOKEN \
...
docker_e2e_sample_stats_until_exit \
  ...
  "${NATESCLAW_DOCKER_E2E_STATS_HEARTBEAT_SECONDS:-30}"

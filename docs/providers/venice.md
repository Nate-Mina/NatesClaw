---
summary: "Use Venice AI privacy-focused models in Natesclaw"
read_when:
  - You want privacy-focused inference in Natesclaw
...
<Steps>
  <Step title="Install the plugin">
    ```bash
    natesclaw plugins install @natesclaw/venice-provider
    ```
  </Step>
  ...
  <Step title="Configure Natesclaw">
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        natesclaw onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    natesclaw agent --model venice/zai-org-glm-4.7 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
```bash
natesclaw models set venice/zai-org-glm-4.7
natesclaw models list --all --provider venice
```

You can also run `natesclaw configure` and pick **Model/auth provider > Venice AI**.
...
The bundled catalog above is a manifest-backed seed list. At runtime Natesclaw
...
Natesclaw catalog advertises only the canonical model IDs returned by `/models`.

## DeepSeek V4 replay behavior

If Venice exposes DeepSeek V4 models such as `deepseek-v4-pro` or
`deepseek-v4-flash`, Natesclaw fills the required `reasoning_content` replay
...
natesclaw agent --model venice/zai-org-glm-4.7 --message "Quick health check"

# Claude Opus via Venice (anonymized)
natesclaw agent --model venice/claude-opus-5 --message "Summarize this task"

# Uncensored model
natesclaw agent --model venice/venice-uncensored-1-2 --message "Draft options"

# Vision model with image
natesclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Coding model
natesclaw agent --model venice/qwen3-coder-480b-a35b-instruct-turbo --message "Refactor this function"
```
...
<AccordionGroup>
  <Accordion title="API key not recognized">
    ```bash
    echo $VENICE_API_KEY
    natesclaw models list | grep venice
    ```
  ...
  <Accordion title="Model not available">
    Run `natesclaw models list --all --provider venice` to see currently

...
function isTranscriptOnlyNatesclawAssistantMessage(message: AgentMessage | undefined): boolean {
  if (!message || message.role !== "assistant") {
    return false;
  }
  const provider = normalizeOptionalString(message.provider) ?? "";
  const model = normalizeOptionalString(message.model) ?? "";
  return provider === "natesclaw" && (model === "delivery-mirror" || model === "gateway-injected");
}
...
function isOpenAiCompletionsAssistantMessage(message: AgentMessage | undefined): boolean {
  ...
  const api = normalizeOptionalString((message as { api?: unknown }).api) ?? "";
  return api === "openai-completions" || api === "natesclaw-openai-completions-transport";
}
...
export function preservePendingAssistantUsage(
  ...
  if (isTranscriptOnlyNatesclawAssistantMessage(message) || !hasNonzeroUsage(pendingUsage)) {
    return message;
  }
...
export function capturePendingAssistantUsage(
  ...
  if (msg?.role !== "assistant" || isTranscriptOnlyNatesclawAssistantMessage(msg)) {
    return;
  }
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[
    Symbol.for("natesclaw.embeddedSubscribeMessagesTestApi")
  ] = {
    buildAssistantStreamData,

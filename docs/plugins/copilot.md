---
summary: "Run Natesclaw embedded agent turns through the external GitHub Copilot SDK harness"
...
CLI-managed thread state under `copilotHome`. Natesclaw still owns chat
...
[Side questions (`/btw`)](#side-questions-btw)), and `natesclaw doctor`.
...
- A writable `copilotHome` directory. Defaults to `<agentDir>/copilot` when
  Natesclaw provides an agent directory, otherwise
  `~/.natesclaw/agents/<agentId>/copilot`.

`natesclaw doctor` runs the plugin's [doctor contract](#doctor) for
session-state ownership and future config migrations. It does not probe the
Copilot CLI environment.

## Install

The Copilot runtime ships as an external plugin so the core `natesclaw`
...
[Quickstart](#quickstart). Without that opt-in, Natesclaw uses its built-in
...
through Natesclaw's guarded fetch path so DNS pinning and SSRF policy stay
owned by Natesclaw. Use the native Natesclaw runtime for local Ollama, LM
Studio, or LAN model servers.

## BYOK

Copilot BYOK uses the SDK's session-level custom provider contract. Natesclaw
...
4. **Env-var fallback**, checked in this order (first non-empty value wins,
   ...
   1. `NATESCLAW_GITHUB_TOKEN` — harness-specific override; lets you pin a
      token for the Natesclaw harness without disturbing system-wide `gh` /
      Copilot CLI config.
...
Natesclaw's `models.json` / `auth-profiles.json`), or
`~/.natesclaw/agents/<agentId>/copilot` when no agent directory is supplied.
Override with `copilotHome: <path>` on the attempt input for a custom
location (for example, a shared mount for migration).

Live harness tests use `NATESCLAW_COPILOT_AGENT_LIVE_TOKEN` for a direct
...
| `reasoningEffort`        | `"low" \| "medium" \| "high" \| "xhigh"`. Maps from Natesclaw's `ThinkLevel` / `ReasoningLevel` resolution in `auto-reply/thinking.ts`.                                                                                                                                                          |
| `infiniteSessionConfig`  | Optional override for the SDK `infiniteSessions` block driven by `harness.compact`. Safe to leave as-is.                                                                                                                                                                                        |
| `hooksConfig`            | Optional native Copilot SDK `SessionHooks` config for tool/MCP, user-prompt, session, and error callbacks. Separate from Natesclaw's portable lifecycle hooks.                                                                                                                                   |
...
Natesclaw plugin hooks need no Copilot-specific attempt configuration. The
...
`before_compaction` and `after_compaction`. Bridged Natesclaw tools run
`before_tool_call` and report `after_tool_call`; `hooksConfig` remains for
native SDK-only callbacks with no portable equivalent.

Nothing else in Natesclaw needs to know about these fields. Other plugins,
...
The Natesclaw-side transcript mirror (below) keeps receiving post-compaction
...
Natesclaw audit transcript via
...
so Natesclaw's `/btw` dispatcher (`src/agents/btw.ts`) falls through to the
...
- `ask_user` uses the provider-neutral gateway question runtime. The Control
  UI shows the same question card as other Natesclaw questions, supported
...
Permission enforcement for bridged Natesclaw tools happens **inside the tool
...
`createNatesclawCodingTools` to every coding tool: loop detection, trusted
...
- `skipPermission: true` — tells the SDK not to fire
  ...
  wrapped `execute()` already performs the richer Natesclaw policy check; an
  SDK-level prompt would either short-circuit Natesclaw's enforcement
  (allow-all) or block every tool call (reject-all) — neither matches PI
  parity.

The in-tree Codex harness uses the same split: bridged Natesclaw tools are
...
`createNatesclawCodingTools`: identity (`senderIsOwner`, `memberRoleIds`,
...
`natesclaw/plugin-sdk/agent-harness-tool-runtime` for PI parity. When
tool-search is enabled, the SDK sees compact control tools plus a hidden
catalog executor instead of every Natesclaw tool schema. When code mode is
...
when the SDK request allows them. Aborting the Natesclaw attempt cancels the

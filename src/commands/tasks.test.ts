...
import type { NatesclawTestState } from "../test-utils/natesclaw-test-state.js";
...
async function withTaskCommandStateDir(
  run: (state: NatesclawTestState) => Promise<void>,
): Promise<void> {
  await withNatesclawTestState(
    { layout: "state-only", prefix: "natesclaw-tasks-command-" },
    async (state) => {

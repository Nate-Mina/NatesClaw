...
summary: "Build, test, and publish custom SKILL.md workspace skills for your Natesclaw agents."
read_when:
...
Natesclaw loads skills from several roots in a defined [precedence order](/tools/skills#loading-order).
...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.natesclaw/workspace/skills/hello-world
    ```
    ...
    ```bash
    mkdir -p ~/.natesclaw/workspace/skills/personal/hello-world
  ...
  <Step title="Verify the skill loaded">
    ...
    Natesclaw watches `SKILL.md` files under skills roots by default. If the
  ...
  <Step title="Test it">
    ```bash
    natesclaw agent --message "give me a greeting"
    ```
...
metadata: { "natesclaw": { "requires": { "bins": ["gemini"] }, "primaryEnv": "GEMINI_API_KEY" } }
---
```
...
<AccordionGroup>
  <Accordion title="Gating options">
    ...
    | `requires.config` | Each `natesclaw.json` path must be truthy |
  ...
  <Accordion title="Environment and API keys">
    Wire an API key to a skill entry in `natesclaw.json`:
...
natesclaw skills workshop propose-update hello-world \
  --proposal ./PROPOSAL.md \
...
<Steps>
  <Step title="Ensure your SKILL.md is complete">
    Make sure `name`, `description`, and any `metadata.natesclaw` gating fields
...
<Tip>
  ...
  - **Test locally** — use `natesclaw agent --message "..."` before sharing.

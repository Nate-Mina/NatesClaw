...
import { ensureNatesclawModelsJson } from "../agents/models-config.js";
...
import type { ModelsConfig, ModelProviderConfig, NatesclawConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.NATESCLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.NATESCLAW_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.NATESCLAW_LIVE_GATEWAY_SMOKE);
const GATEWAY_LIVE_OPENAI_API_DEFAULT = isTruthyEnvValue(
  process.env.NATESCLAW_LIVE_GATEWAY_OPENAI_API_DEFAULT,
);
...
const THINKING_LEVEL = resolveGatewayLiveThinkingLevel({
  raw: process.env.NATESCLAW_LIVE_GATEWAY_THINKING,
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.NATESCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
...
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.NATESCLAW_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const GATEWAY_LIVE_CONFIG_TEST_WORKSPACE = path.join(os.tmpdir(), "natesclaw-live-config-test");
const GATEWAY_LIVE_CONFIG_TEST_AGENT_DIR = path.join(
  os.tmpdir(),
  "natesclaw-live-config-test-agent",
);
...
const QUIET_LIVE_LOGS = process.env.NATESCLAW_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["NATESCLAW_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveSetupTimeoutMs(
  raw = process.env.NATESCLAW_LIVE_GATEWAY_SETUP_TIMEOUT_MS,
): number {
  return Math.max(1_000, toInt(raw, 180_000));
}

function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.NATESCLAW_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.NATESCLAW_LIVE_GATEWAY_MODELS?.trim();
  ...
  return resolveHighSignalLiveModelLimit({
    rawMaxModels: process.env.NATESCLAW_LIVE_MAX_MODELS,
    useExplicitModels,
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.NATESCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.NATESCLAW_LIVE_MODEL_TIMEOUT_MS,
...
function enterProductionEnvForLiveRun() {
  const previous = {
    ...
    testFast: process.env.NATESCLAW_TEST_FAST,
  };
...
function restoreProductionEnvForLiveRun(previous: {
  ...
  if (previous.testFast === undefined) {
    ...
    process.env.NATESCLAW_TEST_FAST = previous.testFast;
  }
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.NATESCLAW_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.NATESCLAW_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.NATESCLAW_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("NATESCLAW_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("NATESCLAW_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("NATESCLAW_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.NATESCLAW_LIVE_GATEWAY_MODELS;
    delete process.env.NATESCLAW_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.NATESCLAW_LIVE_MAX_MODELS;
  ...
  it("defaults small gateway sweeps to the curated small-model cap", () => {
    process.env.NATESCLAW_LIVE_GATEWAY_MODELS = "small";
    delete process.env.NATESCLAW_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.NATESCLAW_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.NATESCLAW_LIVE_GATEWAY_MODELS = "openai/gpt-5.5,anthropic/claude-opus-4-6";
    delete process.env.NATESCLAW_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.NATESCLAW_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.NATESCLAW_LIVE_GATEWAY_MAX_MODELS = "2";
...
function resolveBedrockDiscoveryRegion(cfg: NatesclawConfig | undefined): string | undefined {
...
describe("resolveGatewayLiveModelThinkingLevel", () => {
  ...
  it.each(["gpt-5.6", "gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna"])(
    "preserves Natesclaw Ultra for openai/%s",
    (id) => {
      expect(
...
describe("buildLiveGatewayConfig", () => {
  it("pins selected live gateway models to the Natesclaw runtime", () => {
  ...
  it("writes valid AWS SDK provider config for explicit Bedrock fallback models", () => {
    ...
    try {
      ...
      setTestEnvValue("HOME", path.join(os.tmpdir(), `natesclaw-empty-aws-home-${randomUUID()}`));
  ...
  it("uses selected AWS profile region for explicit Bedrock fallback provider config", async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-bedrock-aws-home-"));
    try {
...
describe("enterProductionEnvForLiveRun", () => {
  it("clears Vitest fast-reply flags while preserving caller state", () => {
    ...
    try {
      ...
      expect(process.env.NATESCLAW_TEST_FAST).toBeUndefined();
    } finally {
      ...
      restoreOptionalEnv("NATESCLAW_TEST_FAST", previous.testFast);
    }
  });
});
...
async function resolveGatewayLiveRequestedModels(): Promise<string | undefined> {
  const configured = process.env.NATESCLAW_LIVE_GATEWAY_MODELS?.trim();
  if (!GATEWAY_LIVE_OPENAI_API_DEFAULT) {
    return configured;
  }
  if (configured) {
    throw new Error(
      "NATESCLAW_LIVE_GATEWAY_OPENAI_API_DEFAULT cannot be combined with NATESCLAW_LIVE_GATEWAY_MODELS",
    );
  }
  if (!PROVIDERS || PROVIDERS.size !== 1 || !PROVIDERS.has("openai")) {
    throw new Error(
      "NATESCLAW_LIVE_GATEWAY_OPENAI_API_DEFAULT requires NATESCLAW_LIVE_GATEWAY_PROVIDERS=openai",
    );
  }
  const apiKey = process.env.OPENAI_API_KEY?.trim();
  if (!apiKey) {
    throw new Error("NATESCLAW_LIVE_GATEWAY_OPENAI_API_DEFAULT requires OPENAI_API_KEY");
  }
...
function buildLiveGatewayConfig(params: {
  ...
  } satisfies NonNullable<NatesclawConfig["agents"]>["entries"];
      ...
      defaults: {
        ...
        models: Object.fromEntries(
          params.candidates.map((m) => [
            `${m.provider}/${m.id}`,
            { agentRuntime: { id: "natesclaw" as const } },
          ]),
        ),
      },
    },
...
async function sanitizeAuthConfig(params: {
  cfg: NatesclawConfig;
  agentDir: string;
}): Promise<NatesclawConfig["auth"] | undefined> {
  ...
  let profiles: NonNullable<NatesclawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  if (ultraCandidates.length > 0 && ultraCandidates.length !== params.candidates.length) {
    throw new Error(
      "NATESCLAW_LIVE_GATEWAY_THINKING=ultra requires an explicit GPT-5.6 OpenAI model list",
    );
  }
  ...
  try {
    ...
    process.env.NATESCLAW_SKIP_GMAIL_WATCHER = "1";
    process.env.NATESCLAW_SKIP_CRON = "1";
    process.env.NATESCLAW_SKIP_CANVAS_HOST = "1";
    if (QUIET_LIVE_LOGS) {
      process.env.NATESCLAW_DISABLE_BONJOUR = "1";
      process.env.NATESCLAW_LOG_LEVEL = "silent";
    }

    const token = `test-${randomUUID()}`;
    process.env.NATESCLAW_GATEWAY_TOKEN = token;
    ...
    const tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-live-state-"));
    ...
    const toolProbePath = path.join(workspaceDir, ".natesclaw-live-tool-probe.txt");
    cleanupToolProbePath = toolProbePath;
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

    const agentDir = resolveDefaultAgentDir(params.cfg);
    const sanitizedCfg: NatesclawConfig = {
      ...params.cfg,
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-live-"));
    cleanupTempDir = tempDir;
    const tempConfigPath = path.join(tempDir, "natesclaw.json");
    await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
    setTestEnvValue("NATESCLAW_CONFIG_PATH", tempConfigPath);
              ...
              for (
                ...
                try {
                  toolText = await requestGatewayAgentText({
                    ...
                    modelKey,
                    message: strictReply
                      ? "Natesclaw live tool probe (local, safe): " +
...
describeLive("gateway live (dev agent, profile keys)", () => {
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        const modelsJsonResult = await withGatewayLiveSetupTimeout(
          ensureNatesclawModelsJson(cfg, undefined, {
            workspaceDir,
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via NATESCLAW_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    clearRuntimeConfigSnapshot();
    const runtimeEnv = enterProductionEnvForLiveRun();
    const previousEnv = snapshotLiveEnv(["NATESCLAW_AGENT_DIR"]);
    const { startGatewayServerCore } = await import("./server-start.js");
    ...
    try {
      ...
      await ensureNatesclawModelsJson(cfg);
      ...
      const agentId = GATEWAY_LIVE_AGENT_ID;
      tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-live-zai-state-"));
      setTestEnvValue("NATESCLAW_STATE_DIR", tempStateDir);
      const workspaceDir = path.join(tempStateDir, "workspace-dev");
      await prepareLiveGatewayWorkspace(workspaceDir);
      const nonceA = randomUUID();
      ...
      toolProbePath = path.join(workspaceDir, ".natesclaw-live-zai-fallback.txt");
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
      ...
      tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-live-zai-"));
      const tempConfigPath = path.join(tempDir, "natesclaw.json");
      await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
      setTestEnvValue("NATESCLAW_CONFIG_PATH", tempConfigPath);
      clearRuntimeConfigSnapshot();

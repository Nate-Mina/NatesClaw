#!/usr/bin/env bash
# Updates a self-hosted Natesclaw gateway that runs from this source checkout.
#
# Reference workflow for team-operated servers (see docs/install/updating.md).
# Simple installs should prefer `natesclaw update` / `natesclaw update --channel
...
#
# Environment:
#   NATESCLAW_UPDATE_RESTART_CMD  restart command (default: natesclaw gateway restart)
#                                set to "" to skip the restart step
#   NATESCLAW_UPDATE_REMOTE       git remote to update from (default: origin)
set -euo pipefail
...
remote="${NATESCLAW_UPDATE_REMOTE:-origin}"
...
restart_cmd="${NATESCLAW_UPDATE_RESTART_CMD-natesclaw gateway restart}"
if [ -n "$restart_cmd" ]; then
  log "restarting gateway: $restart_cmd"
  bash -c "$restart_cmd"
else
  log "restart skipped (NATESCLAW_UPDATE_RESTART_CMD is empty)"
fi

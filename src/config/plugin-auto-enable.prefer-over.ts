...
const ENV_CATALOG_PATHS = ["NATESCLAW_PLUGIN_CATALOG_PATHS", "NATESCLAW_MPM_CATALOG_PATHS"];
...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.natesclaw) || !isRecord(entry.natesclaw.channel)) {
      continue;
    }
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: NatesclawConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: NatesclawConfig, pluginId: string) => boolean;

...
function readRawCatalogCacheRow(
  ...
  const database = new DatabaseSync(path.join(agentDir, "natesclaw-agent.sqlite"), {
    readOnly: true,
  });
  try {
    const row = database
...
const planNatesclawModelsJsonMock = vi.fn();
...
let ensureNatesclawModelsJson: typeof import("./models-config.js").ensureNatesclawModelsJson;
let planNatesclawModelsJsonSource: typeof import("./models-config.js").planNatesclawModelsJsonSource;
...
function planParamsAt(callIndex: number): {
  ...
  // Planner call shape is the contract between ensureNatesclawModelsJson and planning.
  const call = planNatesclawModelsJsonMock.mock.calls[callIndex];
  if (!call) {
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planNatesclawModelsJson: (...args: unknown[]) => planNatesclawModelsJsonMock(...args),
  }));
  ...
  ({ ensureNatesclawModelsJson, planNatesclawModelsJsonSource } = await import("./models-config.js"));
...
describe("models-config write serialization", () => {
  it("materializes an authoritative plugin catalog replacement without mutating state", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planNatesclawModelsJsonMock.mockResolvedValue({
        action: "write",
      ...
      const planned = await planNatesclawModelsJsonSource({}, agentDir, {
        workspaceDir,
  ...
  it("does not reuse default workspace plugin metadata for explicit agent dirs without workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureNatesclawModelsJson({}, agentDir);
  ...
  it("reuses current plugin metadata for explicit agent dirs with matching workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureNatesclawModelsJson({}, agentDir, { workspaceDir });
  ...
  it("writes implicit models.json into the configured default agent dir", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureNatesclawModelsJson(cfg);

      expect(result.agentDir).toBe(path.join(home, ".natesclaw", "agents", "ops", "agent"));
      await expect(fs.access(path.join(result.agentDir, "models.json"))).resolves.toBeUndefined();
      await expectMissingPath(
        fs.access(path.join(home, ".natesclaw", "agents", "main", "agent", "models.json")),
      );
    });
  });
  ...
  it("migrates released provider credentials before model planning can regenerate a catalog", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planNatesclawModelsJsonMock.mockImplementation(async () => {
      ...
      expect(planNatesclawModelsJsonMock).toHaveBeenCalledOnce();
  ...
  it("refuses to regenerate provider catalogs while released credentials cannot be read", async () => {
    ...
    await withModelsTempHome(async (home) => {
      ...
      try {
        await expect(ensureNatesclawModelsJson({}, agentDir)).rejects.toThrow(
          "Cannot safely prepare provider models until legacy catalog migration succeeds",
        );
        expect(planNatesclawModelsJsonMock).not.toHaveBeenCalled();
      } finally {
  ...
  it("never promotes an unmarked cached plugin catalog into model planning", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planNatesclawModelsJsonMock.mockImplementation(
      ...
      await ensureNatesclawModelsJson({ models: { providers: {} } }, agentDir, {
  ...
  it("removes stale plugin-owned model catalogs from the agent SQLite cache", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureNatesclawModelsJson({}, agentDir);
  ...
  it("keeps generated SQLite plugin catalogs on non-authoritative skip plans", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planNatesclawModelsJsonMock.mockImplementation(async () => ({ action: "skip" }));
  ...
  it("does not reuse scoped startup discovery cache for a different provider scope", async () => {
    await withModelsTempHome(async (home) => {
      ...
      expect(planNatesclawModelsJsonMock).toHaveBeenCalledTimes(2);
  ...
  it("keeps the ready cache warm after models.json is written", async () => {
    await withModelsTempHome(async () => {
      await ensureNatesclawModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
      await ensureNatesclawModelsJson(CUSTOM_PROXY_MODELS_CONFIG);

      expect(planNatesclawModelsJsonMock).toHaveBeenCalledTimes(1);
    });
  });
  ...
  it("keeps distinct config fingerprints cached without evicting each other", async () => {
    await withModelsTempHome(async () => {
      planNatesclawModelsJsonMock.mockImplementation(async () => ({ action: "noop" }));
      ...
      await ensureNatesclawModelsJson(first);
      await ensureNatesclawModelsJson(second);
      await ensureNatesclawModelsJson(first);
  ...
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      const writes = Promise.all([
        ensureNatesclawModelsJson(first),
        ensureNatesclawModelsJson(second),
      ]);

...
import { MINIMAX_OAUTH_MARKER } from "natesclaw/plugin-sdk/provider-auth";
import { buildOauthProviderAuthResult } from "natesclaw/plugin-sdk/provider-auth";
...
import { fetchMinimaxUsage } from "natesclaw/plugin-sdk/provider-usage";
...
function getProviderBaseUrl(cfg: NatesclawConfig, providerId: string): string | undefined {
  return normalizeOptionalString(cfg.models?.providers?.[providerId]?.baseUrl);
}

function resolveMinimaxUsageBaseUrl(cfg: NatesclawConfig): string | undefined {
...
export function registerMinimaxProviders(api: NatesclawPluginApi) {

...
@MainActor
enum NatesclawAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class NatesclawAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.natesclawfoundation.app", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.natesclawfoundation.app", category: "BackgroundWake")
    ...
    private static var appBundleIdentifier: String {
        guard let bundleId = Bundle.main.bundleIdentifier?.trimmingCharacters(in: .whitespacesAndNewlines),
            ...
            return "ai.natesclawfoundation.app"
        }
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? NatesclawAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "natesclaw.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "natesclaw.watch.promptId"
    static let sessionKeyKey = "natesclaw.watch.sessionKey"
    static let gatewayStableIDKey = "natesclaw.watch.gatewayStableID"
    static let actionPrimaryIDKey = "natesclaw.watch.action.primary.id"
    static let actionPrimaryLabelKey = "natesclaw.watch.action.primary.label"
    static let actionSecondaryIDKey = "natesclaw.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "natesclaw.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "natesclaw.watch.action.primary"
    static let actionSecondaryIdentifier = "natesclaw.watch.action.secondary"
    static let actionIdentifierPrefix = "natesclaw.watch.action."
    static let actionIDKeyPrefix = "natesclaw.watch.action.id."
    static let actionLabelKeyPrefix = "natesclaw.watch.action.label."
    static let categoryPrefix = "natesclaw.watch.prompt.category."
        ...
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> NatesclawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return NatesclawWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Natesclaw" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [NatesclawWatchAction]) -> [UNNotificationAction] {
...
struct NatesclawApp: App {
    ...
    @UIApplicationDelegateAdaptor(NatesclawAppDelegate.self) private var appDelegate
    @Environment(\.scenePhase) private var scenePhase

    init() {
        Self.installUncaughtExceptionLogger()
        GatewaySettingsStore.bootstrapPersistence()
        NatesclawType.installUIKitAppearance()
        let appModel = NodeAppModel(audioAdmissionInitiallyAllowed: false)
        #if DEBUG
        if ProcessInfo.processInfo.arguments.contains("--natesclaw-reset-onboarding") {
        ...
        #endif
        NatesclawAppModelRegistry.appModel = appModel
        ...
        WindowGroup {
            RootTabs()
                ...
                .onReceive(
                    ...
                    perform: { _ in
                        NatesclawType.refreshUIKitAppearance(in: Self.connectedWindows())
                    })
                .onOpenURL { url in
    ...
    private static var screenshotModeEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--natesclaw-screenshot-mode")
        #else
        false
        #endif
    }
    ...
    private static var screenshotNotificationGuidanceEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--natesclaw-screenshot-notification-guidance")
        #else
        false
        #endif
    }
    ...
    private static var liveActivityVoicePreviewEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--natesclaw-live-activity-voice-preview")
        #else
        false
        #endif
    }
    ...
    private func applyWindowTint() {
        for window in Self.connectedWindows() {
            window.tintColor = NatesclawBrand.uiAccent
        }
    }
...
extension NatesclawApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Natesclaw: installing uncaught exception handler")

...
Nextcloud Talk is a downloadable channel plugin (`@openclaw/nextcloud-talk`) that connects Natesclaw to a self-hosted Nextcloud instance through a Talk webhook bot. Direct messages, rooms, reactions, and markdown messages are supported; media goes out as URLs.

## Install

```bash
natesclaw plugins install @openclaw/nextcloud-talk
```
...
```bash
natesclaw plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Natesclaw" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure Natesclaw:
   ...
   ```bash
   natesclaw channels add --channel nextcloud-talk \
...
- HTTP 200 is returned only after the raw event is durably stored; storage failures return HTTP 500. The durable `200` carries `x-natesclaw-delivery-accepted: durable` (signature, validation, and storage-error responses omit it), so reverse proxies can require the marker to distinguish Natesclaw acceptance from a generic `200`.
...
- With `apiUser`/`apiPassword` and `webhookPublicUrl` set, `natesclaw channels status` probes the bot and warns when the `response` feature is missing.

## Access control (DMs)

- Default: `channels.nextcloud-talk.dmPolicy = "pairing"`. Unknown senders get a pairing code.
- Approve via:
  - `natesclaw pairing list nextcloud-talk`
  - `natesclaw pairing approve nextcloud-talk <CODE>`

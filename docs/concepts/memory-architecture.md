---
summary: "End-to-end architecture of Natesclaw memory: tiers, provenance, dreaming, recall lanes, the user model, and standing intents"
title: "Memory architecture"
sidebarTitle: "Memory architecture"
read_when:
  - You want the complete picture of how Natesclaw memory works end to end
...
Natesclaw memory is a set of plain files and one SQLite index, organized into
...
2. **Writing is the hard part.** Retrieval over notes files is competitive
   ...
   arXiv:2410.10813). Natesclaw therefore moves curation off the busy reply
   path and into a dedicated background pass.
3. **The write path is the security boundary.** Content-level scanning of
   memory cannot catch poisoned facts reliably, so Natesclaw enforces
...
event benchmarks). Natesclaw therefore compiles intentions out of the model:
...
measure poorly. Natesclaw defends structurally:
...
more conservatively they write; Natesclaw keeps those properties even with

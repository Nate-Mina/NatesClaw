...
import type { ProviderCatalogResult } from "natesclaw/plugin-sdk/provider-catalog-shared";
...
describe("qwen provider plugin", () => {
  ...
  it("switches Token Plan regions without replacing custom catalog rows", () => {
    ...
    const tokenPlanProvider = (config: NatesclawConfig) =>
      config.models?.providers?.[QWEN_TOKEN_PLAN_PROVIDER_ID];
    const qwenContext = (config: NatesclawConfig) =>

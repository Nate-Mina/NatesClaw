...
Natesclaw agents generate videos from text prompts, reference images, or
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ```bash
    natesclaw config set agents.defaults.mediaModels.video.primary "google/veo-3.1-fast-generate-preview"
    ```
  </Step>
...
1. Natesclaw submits the request to the provider and immediately returns a task id.
...
3. When the video is ready, Natesclaw wakes the same session with an internal completion event.
4. The agent reports it through the session's normal visible-reply mode:
   ...
   generated media is still missing from the completion reply, Natesclaw sends
...
generation, or `natesclaw tasks list` / `natesclaw tasks show <lookup>` from the
...
Generated video files save under Natesclaw-managed media storage when the
...
provider also returns a hosted output URL, Natesclaw delivers that URL instead
...
<ParamField path="aspectRatio" type="string">
  Aspect-ratio hint such as `1:1`, `16:9`, `9:16`, `adaptive`, or a provider-specific value. Natesclaw normalizes or ignores unsupported values per provider.
</ParamField>
<ParamField path="resolution" type="string">Resolution hint such as `360P`, `480P`, `540P`, `720P`, `768P`, `1080P`, `4K`, or a provider-specific value. Natesclaw normalizes or ignores unsupported values per provider.</ParamField>
...
<ParamField path="timeoutMs" type="number">Optional provider operation timeout in milliseconds. When omitted, Natesclaw uses `agents.defaults.mediaModels.video.timeoutMs` if configured, otherwise the plugin-authored provider default when one exists.</ParamField>
...
<Note>
Not all providers support all parameters. Natesclaw normalizes duration to
...
Natesclaw resolves the model in this order:
...
<AccordionGroup>
  ...
  <Accordion title="fal">
    Uses a queue-backed flow for long-running jobs. Natesclaw waits up to 20
  ...
  <Accordion title="OpenRouter">
    Uses OpenRouter's asynchronous `/videos` API. Natesclaw submits the
...
```bash
NATESCLAW_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts
```
...
- Per-provider operation cap from
  `NATESCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default).
...
Set `NATESCLAW_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared
...
Set the default video-generation model in your Natesclaw config:
...
```bash
natesclaw config set agents.defaults.mediaModels.video.primary "qwen/wan2.6-t2v"
```

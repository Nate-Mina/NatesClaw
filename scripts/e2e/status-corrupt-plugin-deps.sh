...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/natesclaw-status-corrupt-plugin-deps.XXXXXX")"
cleanup() {
  rm -rf "$TMP_DIR"
}
trap cleanup EXIT
...
const stageDir = process.env.NATESCLAW_PLUGIN_STAGE_DIR || "";
const renameResidue = path.join(stageDir, "node_modules", "ansi-escapes", ".natesclaw-rename-tmp");
...
run_natesclaw() {
  HOME="$HOME_DIR" \
  NATESCLAW_HOME="$STATE_DIR" \
  NATESCLAW_STATE_DIR="$STATE_DIR" \
  NATESCLAW_CONFIG_PATH="$CONFIG_PATH" \
  NATESCLAW_PLUGIN_STAGE_DIR="$STAGE_DIR" \
...
run_natesclaw status --all --timeout 1 > "$BEFORE"
grep -F "e2e-corrupt-chat" "$BEFORE" >/dev/null
grep -F "plugin load failed: dependency tree corrupted; run natesclaw doctor --fix" "$BEFORE" >/dev/null

run_natesclaw doctor --fix --non-interactive --yes > "$DOCTOR"
if [[ -e "$STAGE_DIR" ]]; then
  echo "doctor --fix did not remove corrupt plugin stage dir: $STAGE_DIR" >&2
  exit 1
fi

run_natesclaw status --all --timeout 1 > "$AFTER"

...
describe("resolveCommandAuthorization", () => {
  ...
  it.each([
    ...
    const auth = resolveCommandAuthorization({
      ...
      cfg: { commands: { ownerAllowFrom: [owner] } } as NatesclawConfig,
      commandAuthorized: true,
    });
    ...
    it.each([
      ...
      ({ provider, chatType, from, senderId, senderE164, allowFrom, expected }) => {
        const auth = resolveCommandAuthorization({
          ...
          cfg: { commands: { allowFrom } } as unknown as NatesclawConfig,
...
describe("control command parsing", () => {
  ...
  it("detects spaced syntax only for active registered plugin commands", () => {
    ...
    expect(shouldComputeCommandAuthorized("@natesclaw / pair qr")).toBe(true);
  ...
  it("ignores telegram commands addressed to other bots", () => {
    ...
    expect(
      hasControlCommand("/help@natesclaw", undefined, {
        botUsername: "natesclaw",
      }),
    ).toBe(true);
  });

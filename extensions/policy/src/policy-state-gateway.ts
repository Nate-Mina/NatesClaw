...
export function scanPolicyGatewayExposure(
  ...
  if (bind === "custom" && hasCustomBindHost) {
    entries.push({
      ...
      kind: "bind",
      source: "oc://natesclaw.config/gateway/customBindHost",
  ...
  entries.push({
    ...
    kind: "auth",
    source: "oc://natesclaw.config/gateway/auth/mode",
  ...
  entries.push({
    ...
    kind: "authRateLimit",
    source: "oc://natesclaw.config/gateway/auth/rateLimit",
  ...
  pushGatewayBooleanEvidence(
    ...
    "controlUi",
    controlUi.enabled,
    "oc://natesclaw.config/gateway/controlUi/enabled",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "controlUi",
    false,
    "oc://natesclaw.invariant/gateway/controlUi/deviceIdentity",
  );
  pushGatewayBooleanEvidence(
    entries,
  ...
  if (typeof tailscale.mode === "string") {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://natesclaw.config/gateway/tailscale/mode",
  ...
  if (tailscale.mode === "serve" && tailscale.preserveFunnel === true) {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://natesclaw.config/gateway/tailscale/preserveFunnel",
      value: "funnel",
    });
  }
  ...
  if (gateway.mode === "remote") {
    entries.push({
      ...
      kind: "remote",
      source: "oc://natesclaw.config/gateway/mode",
      value: "remote",
    });
    if (typeof remote.url === "string" && remote.url.trim() !== "") {
      entries.push({
        ...
        kind: "remote",
        source: "oc://natesclaw.config/gateway/remote/url",
        value: true,
      });
    }
  }
...
function pushGatewayHttpEndpointEvidence(
  ...
  const source = `oc://natesclaw.config/gateway/http/endpoints/${endpoint}`;
...
function pushGatewayNodeCommandEvidence(
  ...
  if (Array.isArray(denyCommands)) {
    denyCommands.forEach((command, index) => {
      ...
      entries.push({
        ...
        source: `oc://natesclaw.config/gateway/nodes/commands/deny/#${index}`,
        value: normalized,
        command: normalized,
      });
    });
  ...
  allowCommands.forEach((command, index) => {
    ...
    entries.push({
      ...
      kind: "nodeCommand",
      source: `oc://natesclaw.config/gateway/nodes/commands/allow/#${index}`,
      value: normalized,
      command: normalized,
    });
  });

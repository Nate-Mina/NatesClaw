...
import { resolveExecModePolicy } from "natesclaw/plugin-sdk/exec-approvals-runtime";
...
// steer Natesclaw-managed Claude CLI runs toward a different provider,
// endpoint, token source, plugin/config tree, or telemetry bootstrap mode.
/** Environment variables removed before launching Natesclaw-managed Claude CLI runs. */
export const CLAUDE_CLI_CLEAR_ENV = [
  ...
  "CLAUDE_CONFIG_DIR",
  // Re-injected per run from Natesclaw's canonical context budget.
...
/** Map Natesclaw's effective context budget to Claude Code's native compactor. */
...
function isNatesclawRequestedYolo(context?: CliBackendNormalizeConfigContext): boolean {
...
/** Resolve Claude permission mode from Natesclaw exec security settings. */
function resolveClaudePermissionMode(context?: CliBackendNormalizeConfigContext): {
  mode?: string;
  overrideExisting: boolean;
} {
  return isNatesclawRequestedYolo(context)
...
function resolveClaudeCliRestrictedExecutionArgs(
  ...
  if (availability.Natesclaw.length > 0) {
    normalized.push(
      CLAUDE_ALLOWED_TOOLS_ARG,
      availability.Natesclaw.map((toolName) => `${NATESCLAW_MCP_TOOL_PREFIX}${toolName}`).join(","),
    );
  } else {

# fastlane setup (Natesclaw Android)
...
Create a Google Play service account JSON key with Google Play Developer API access, then grant that service account access to the Natesclaw app in Play Console.
...
```bash
GOOGLE_PLAY_PACKAGE_NAME=ai.natesclaw.app
```
...
NATESCLAW_ANDROID_UPLOAD_KEYSTORE=<path-to-upload-keystore.jks> \
NATESCLAW_ANDROID_SIGNING_PROPERTIES=<path-to-android-signing.properties> \
...
```properties
NATESCLAW_ANDROID_STORE_PASSWORD=<store-password>
NATESCLAW_ANDROID_KEY_ALIAS=<upload-key-alias>
NATESCLAW_ANDROID_KEY_PASSWORD=<key-password>
```
...
- Stable GitHub Release APK publication is separate from Google Play: `Natesclaw Release Publish` dispatches `.github/workflows/android-release.yml`, whose protected `android-release` environment provides `MATCH_PASSWORD`; the repository GitHub App reads the encrypted signing repo.

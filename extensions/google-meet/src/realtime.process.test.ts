...
import type { RealtimeVoiceProviderPlugin } from "natesclaw/plugin-sdk/realtime-voice";
...
function writeBridgeCommand(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "natesclaw-google-meet-bridge-"));
  tempDirs.push(dir);
...
function writeSigtermResistantBridgeCommand(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "natesclaw-google-meet-resistant-bridge-"));
  tempDirs.push(dir);
  const scriptPath = path.join(dir, "bridge-command.mjs");
  writeFileSync(
    scriptPath,

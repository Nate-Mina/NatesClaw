// Natesclaw state database tests cover state DB migrations and persistence.
...
import { FIRST_USE_STATE_TABLES } from "./natesclaw-state-db-contract.js";
import {
...
import {
  ...
  repairNatesclawStateDatabaseSchemaIfNeeded,
...
function createTempStateDir(): string {
  return makeTempDir(stateDbTempDirs, "natesclaw-state-db-");
}

function createInitialStateSchemaShape() {
  const shape = createSqliteSchemaShapeFromSql(
    new URL("./natesclaw-state-schema.sql", import.meta.url),
  );
...
function createOlderV6StateSchemaWithoutWorkerSshFallbackPorts(): string {
  ...
  const start = NATESCLAW_STATE_SCHEMA_SQL.indexOf(startMarker);
  const endMarker = "\n) STRICT;";
  const end = start >= 0 ? NATESCLAW_STATE_SCHEMA_SQL.indexOf(endMarker, start) : -1;
  if (start < 0 || end < 0) {
    throw new Error("worker SSH fallback port schema block is missing");
  }
  return `${NATESCLAW_STATE_SCHEMA_SQL.slice(0, start)}${NATESCLAW_STATE_SCHEMA_SQL.slice(
...
function expectStateSchemaMigrationRequired(
  run: () => unknown,
  expected: {
    kind: NatesclawStateDatabaseSchemaMigrationRequiredError["kind"];
    pathname: string;
  },
): void {
  let caught: unknown;
  ...
  expect(caught).toBeInstanceOf(NatesclawStateDatabaseSchemaMigrationRequiredError);
  expect(findNatesclawStateDatabaseSchemaMigrationRequiredError(caught)).toMatchObject(expected);
}
...
function materializeCurrentStateDatabase(stateDir: string): string {
  ...
  const databasePath = resolveNatesclawStateSqlitePath({ NATESCLAW_STATE_DIR: stateDir });
  fs.mkdirSync(path.dirname(databasePath), { recursive: true });
...
function runHotRollbackJournalRecoveryProbe(params: { moduleUrl: string; rootDir: string }): {
  ...
  const probeSource = `
    ...
    const initial = openNatesclawStateDatabase({ path: databasePath });
    initial.db.exec(\`
    ...
    const writerSource = \`
      ...
      const database = new DatabaseSync(process.env.NATESCLAW_HOT_JOURNAL_DATABASE_PATH);
      database.exec(
      ...
      fs.writeFileSync(process.env.NATESCLAW_HOT_JOURNAL_READY_PATH, "ready");
      setInterval(() => {}, 1_000);
    \`;
    const writer = spawn(
      process.execPath,
      ["--input-type=module", "-e", writerSource],
      {
        env: {
          ...process.env,
          NATESCLAW_HOT_JOURNAL_DATABASE_PATH: databasePath,
          NATESCLAW_HOT_JOURNAL_READY_PATH: readyPath,
        },
    ...
    try {
      ...
      let readOnly;
      try {
        const database = await openExistingNatesclawStateDatabaseReadOnly({ path: databasePath });
      ...
      const reopened = openNatesclawStateDatabase({ path: databasePath });
...
function expectNoncanonicalAuditSchemaRejected(
  ...
  const options = { env: { NATESCLAW_STATE_DIR: stateDir } };
  expect(detectNatesclawStateDatabaseSchemaMigrations(options)).toEqual([
    { kind: "audit-events-v2", path: databasePath },
  ]);
  expect(() => openNatesclawStateDatabase(options)).toThrow(/noncanonical audit event schema/);
  expect(repairNatesclawStateDatabaseSchema(options)).toEqual({
    changes: [],
...
function runConcurrentSchemaProbe(params: {
  ...
  const workerSource = `
    ...
    const {
      closeNatesclawStateDatabaseForTest,
      openNatesclawStateDatabase,
    } = await import(process.env.NATESCLAW_SCHEMA_TEST_MODULE_URL);
    const databasePath = process.env.NATESCLAW_SCHEMA_TEST_DATABASE_PATH;
    const enteringPath = process.env.NATESCLAW_SCHEMA_TEST_ENTERING_PATH;
    const readyPath = process.env.NATESCLAW_SCHEMA_TEST_READY_PATH;
    const startPath = process.env.NATESCLAW_SCHEMA_TEST_START_PATH;
    const workerIndex = process.env.NATESCLAW_SCHEMA_TEST_WORKER_INDEX;
    ...
    try {
      const database = openNatesclawStateDatabase({ path: databasePath });
  ...
  const orchestratorSource = `
    ...
    const coordinatorContracts =
      mode === "fresh"
        ? await Promise.all([
            ...
            import(new URL("./natesclaw-state-db-contract.ts", moduleUrl).href),
            import(new URL("./natesclaw-state-db.paths.ts", moduleUrl).href),
          ])
        : undefined;
    ...
    function openFreshInitializationCoordinator(databasePath) {
      ...
      const [
        ...
        { NATESCLAW_SQLITE_BUSY_TIMEOUT_MS },
        { resolveNatesclawStateDirForDatabasePath },
      ...
      try {
        coordinator.exec(
          \`PRAGMA busy_timeout = \${NATESCLAW_SQLITE_BUSY_TIMEOUT_MS}; BEGIN EXCLUSIVE;\`,
        );
      } catch (error) {
        coordinator.close();
        throw error;
    ...
    for (let round = 0; round < roundCount; round += 1) {
      ...
      if (mode === "upgrade") {
        ...
        openNatesclawStateDatabase({ path: databasePath });
        closeNatesclawStateDatabaseForTest();
        ...
        legacy.exec(\`
          ...
          ALTER TABLE worker_environments DROP COLUMN bootstrap_natesclaw_version;
      ...
      const workers = Array.from({ length: workerCount }, (_, index) => {
        return spawn(
          ...
          ["--import", "tsx", "--input-type=module", "-e", workerSource],
          {
            env: {
              ...process.env,
              NATESCLAW_SCHEMA_TEST_DATABASE_PATH: databasePath,
              NATESCLAW_SCHEMA_TEST_ENTERING_PATH: enteringPaths[index],
              NATESCLAW_SCHEMA_TEST_MODULE_URL: moduleUrl,
              NATESCLAW_SCHEMA_TEST_READY_PATH: readyPaths[index],
              NATESCLAW_SCHEMA_TEST_START_PATH: startPath,
              NATESCLAW_SCHEMA_TEST_WORKER_INDEX: String(index),
            },
...
beforeAll(() => {
  ...
  canonicalStateDatabaseTemplatePath = openNatesclawStateDatabase({
...
describe("natesclaw state database", () => {
  it("resolves under the shared state database directory", () => {
    const stateDir = createTempStateDir();

    expect(resolveNatesclawStateSqlitePath({ NATESCLAW_STATE_DIR: stateDir })).toBe(
  ...
  it("keeps test default state under a worker-sharded temp directory", () => {
    expect(
      ...
      path.join(os.tmpdir(), "natesclaw-test-state", `${process.pid}-7`, "state", "natesclaw.sqlite"),
    );
  });
  ...
  it("creates the shared state schema from the committed SQL shape", () => {
    ...
    expect(database.path).toBe(path.join(stateDir, "state", "natesclaw.sqlite"));
    expect(
      database.db
        .prepare(
  ...
  it("skips exclusive repair when the automatic schema gate is already current", () => {
    ...
    expect(repairNatesclawStateDatabaseSchemaIfNeeded(options)).toEqual({
      changes: [],
      warnings: [],
    });
  ...
  it("drops unreleased transient verification history on open", () => {
    ...
    const reopened = openNatesclawStateDatabase(options);
    expect(
      reopened.db
...
`);
    ...
    expect(readSqliteNumberPragma(database.db, "user_version")).toBe(NATESCLAW_STATE_SCHEMA_VERSION);
    expect(collectSqliteSchemaShape(database.db)).toEqual(createInitialStateSchemaShape());
  });
...
`);
  ...
  it("doctor migrates existing APNs tombstone tables to STRICT without losing rows", () => {
    ...
    const migrated = openNatesclawStateDatabase(options);
    expect(
      migrated.db
        .prepare("SELECT strict FROM pragma_table_list WHERE name = 'apns_registration_tombstones'")
        .get(),
  ...
  it("doctor migrates version 3 ambient watch sentinels into cursor provenance", () => {
    ...
    expect(readSqliteNumberPragma(migrated.db, "user_version")).toBe(NATESCLAW_STATE_SCHEMA_VERSION);
    expect(
      migrated.db
        .prepare("SELECT schema_version FROM schema_meta WHERE meta_key = 'primary'")
        .get(),
    ).toEqual({ schema_version: NATESCLAW_STATE_SCHEMA_VERSION });
    closeNatesclawStateDatabaseForTest();
    expect(repairNatesclawStateDatabaseSchema(options)).toEqual({ changes: [], warnings: [] });
  });
  ...
  it("detects schema migrations committed in an uncheckpointed WAL", () => {
    const stateDir = createTempStateDir();
    const options = { env: { NATESCLAW_STATE_DIR: stateDir } };
    const opened = openNatesclawStateDatabase(options);
    ...
    try {
      ...
      expect(detectNatesclawStateDatabaseSchemaMigrations(options)).toContainEqual({
  ...
  it("repairs every canonical shared-state named index", () => {
    ...
    const reopened = openNatesclawStateDatabase({ env });
  ...
  it.each([
    ...
    ({ columnName, tableName }) => {
      ...
      expect(() =>
        assertNatesclawStateDatabaseForMaintenance(reopened.db, { pathname: reopened.path }),
      ).not.toThrow();
      expect(reopened.db.prepare("PRAGMA integrity_check").get()).toEqual({
        integrity_check: "ok",
      });
  ...
  it("does not add Claw bootstrap columns before rejecting unrelated index corruption", () => {
    ...
    expect(() => openNatesclawStateDatabase({ env })).toThrow(
  ...
  it("rejects a missing current-schema table instead of recreating it empty", () => {
    ...
    expect(() => openNatesclawStateDatabase(options)).toThrow(/missing table auth_profile_stores/iu);
    expect(repairNatesclawStateDatabaseSchema(options)).toEqual({
      changes: [],
  ...
  it("rejects an inline unique constraint hidden behind a SQLite autoindex", () => {
    ...
    expect(() => openNatesclawStateDatabase(options)).toThrow(
  ...
  it("classifies the released agent registry primary key as Doctor-repairable", () => {
    ...
    expectStateSchemaMigrationRequired(() => openNatesclawStateDatabase(options), {
  ...
  it("keeps an unrecognized agent registry schema fail-closed and nonrepairable", () => {
    ...
    expect(findNatesclawStateDatabaseSchemaMigrationRequiredError(caught)).toBeUndefined();
  ...
  it("migrates the released audit ledger to message-compatible attribution exactly once", () => {
    ...
    try {
      ...
      expect(readSqliteNumberPragma(db, "user_version")).toBe(NATESCLAW_STATE_SCHEMA_VERSION);
      expect(
  ...
  it("preserves an empty audit ledger's sequence high-water mark", () => {
    ...
    expect(repairNatesclawStateDatabaseSchema(options).warnings).toEqual([]);
  ...
  it("lets normal open create an audit ledger for a pre-v2 database", () => {
    ...
    expect(readSqliteNumberPragma(opened.db, "user_version")).toBe(NATESCLAW_STATE_SCHEMA_VERSION);
  });
  ...
  it("refuses to rebuild a noncanonical audit table with unknown data columns", () => {
    ...
    const result = repairNatesclawStateDatabaseSchema(options);
  ...
  it("creates the bounded skill curator tables", () => {
    const stateDir = createTempStateDir();
    const database = openNatesclawStateDatabase({ env: { NATESCLAW_STATE_DIR: stateDir } });
  ...
  it.runIf(process.platform === "linux")("closes the database when initialization fails", () => {
    const databasePath = path.join(createTempStateDir(), "natesclaw.sqlite");
    fs.writeFileSync(databasePath, "not a sqlite database");

    expect(() => openNatesclawStateDatabase({ path: databasePath })).toThrow(
  ...
  it("reads committed live WAL rows without changing source database content", async () => {
    const stateDir = createTempStateDir();
    const writer = openNatesclawStateDatabase({ env: { NATESCLAW_STATE_DIR: stateDir } });
    ...
    const database = await openExistingNatesclawStateDatabaseReadOnly({ path: writer.path });
    expect(
  ...
  it("rejects noncanonical indexes from read-only state without rewriting them", async () => {
    ...
    await expect(openExistingNatesclawStateDatabaseReadOnly(options)).rejects.toThrow(
  ...
  it("rejects unrelated current-schema index corruption before exposure", () => {
    ...
    expect(() =>
      withNatesclawStateStartupMigrationCheckpointDatabase(checkpointCallback, options),
  ...
  it("configures checkpoint lock waits before schema mutation", () => {
    ...
    withNatesclawStateStartupMigrationCheckpointDatabase(
      (db) => {
        expect(readSqliteNumberPragma(db, "busy_timeout")).toBe(NATESCLAW_SQLITE_BUSY_TIMEOUT_MS);
      },
      { env: { NATESCLAW_STATE_DIR: stateDir } },
    );
  });
  ...
  it("runs full integrity before a pending state schema migration", () => {
    ...
    const before = new DatabaseSync(databasePath);
    try {
      before.exec(`PRAGMA user_version = ${NATESCLAW_STATE_SCHEMA_VERSION - 1};`);
    } finally {
      before.close();
    }
  ...
  it("rejects current-schema foreign-key violations before exposure", () => {
    ...
    expect(() => openNatesclawStateDatabase(options)).toThrow(failure);
    expect(repairNatesclawStateDatabaseSchema(options)).toEqual({
      changes: [],
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      const result = runHotRollbackJournalRecoveryProbe({
        moduleUrl: new URL("./natesclaw-state-db.ts", import.meta.url).href,
  ...
  it("adds optional Claw application provenance columns to existing state databases", () => {
    ...
    const reopened = openNatesclawStateDatabase({ env: { NATESCLAW_STATE_DIR: stateDir } });
  ...
  it("adds staged worker-result refs during the v5 state migration", () => {
    ...
    expect(readSqliteNumberPragma(reopened.db, "user_version")).toBe(NATESCLAW_STATE_SCHEMA_VERSION);
    expect(
      reopened.db
        .prepare("SELECT schema_version FROM schema_meta WHERE meta_key = 'primary'")
        .get(),
  ...
  it("migrates operator approvals to accept system-agent records", () => {
    ...
    expect(repairNatesclawStateDatabaseSchema(options)).toEqual({
      changes: [
        "Migrated shared state operator approvals → Natesclaw system changes",
  ...
  it("does not recursively recommend doctor when operator approval repair refuses a shape", () => {
    ...
    expect(result.warnings[0]).not.toContain("run natesclaw doctor --fix");
  });
  ...
  it("serializes concurrent additive schema upgrades across processes", () => {
    ...
    const moduleUrl = new URL("./natesclaw-state-db.ts", import.meta.url).href;
  ...
  it("migrates requester and executor attribution for existing cross-agent tasks", () => {
    ...
    const currentReopened = openNatesclawStateDatabase({
      env: { NATESCLAW_STATE_DIR: stateDir },
    });
    expect(
      currentReopened.db
  ...
  it("normalizes obsolete task delivery statuses in existing state databases", async () => {
    await withNatesclawTestState(
      { layout: "state-only", prefix: "natesclaw-state-task-delivery-status-" },
      async ({ stateDir }) => {
        const databasePath = materializeCurrentStateDatabase(stateDir);
        const { DatabaseSync } = requireNodeSqlite();
        const database = new DatabaseSync(databasePath);
  ...
  it("configures the busy timeout before a doctor schema repair transaction", () => {
    ...
    const timeoutIndex = statements.findIndex((sql) =>
      sql.includes(`PRAGMA busy_timeout = ${NATESCLAW_SQLITE_BUSY_TIMEOUT_MS}`),
    );
  ...
  it("latches newer global schema failures before integrity scans", () => {
    const stateDir = createTempStateDir();
    const options = { env: { NATESCLAW_STATE_DIR: stateDir } };
    const databasePath = openNatesclawStateDatabase(options).path;
    closeNatesclawStateDatabaseForTest();
    createUnsafeIndexDrift(databasePath);
    const { DatabaseSync } = requireNodeSqlite();
    const db = new DatabaseSync(databasePath);
    ...
    expect(openNatesclawStateDatabase(options).db.isOpen).toBe(true);
  });

  it("does not chmod shared parent directories for explicit database paths", () => {
    const databasePath = path.join(
      os.tmpdir(),
      `natesclaw-explicit-state-${process.pid}-${Date.now()}.sqlite`,
    );

    expect(() => openNatesclawStateDatabase({ path: databasePath })).not.toThrow();
  ...
  it("keeps cached handles open when another state path is opened", () => {
    ...
    const first = openNatesclawStateDatabase({ path: firstPath });
    const second = openNatesclawStateDatabase({ path: secondPath });

    expect(first.db.isOpen).toBe(true);
    expect(second.db.isOpen).toBe(true);
    expect(openNatesclawStateDatabase({ path: firstPath })).toBe(first);
    expect(readSqliteNumberPragma(first.db, "user_version")).toBe(NATESCLAW_STATE_SCHEMA_VERSION);
  });
    ...
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-state-db-relative-"));
          ...
          try {
            ...
            const first = openNatesclawStateDatabase({ path: "state.sqlite" });
            first.db
            ...
            const second = openNatesclawStateDatabase({ path: "state.sqlite" });
            second.db
              .prepare("INSERT INTO diagnostic_events (scope, event_key, payload_json, created_at) VALUES (?, ?, ?, ?)")
  ...
  it("uses savepoints for nested write transaction rollback", () => {
    ...
    runNatesclawStateWriteTransaction((database) => {
      ...
      expect(() =>
        runNatesclawStateWriteTransaction((inner) => {
  ...
  it("rejects Promise-returning write transactions", () => {
    ...
    expect(() =>
      runNatesclawStateWriteTransaction(async () => {
        return "not sync";
      }, options),

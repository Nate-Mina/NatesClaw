...
const LIVE =
  process.env.NATESCLAW_LIVE_TEST === "1" && process.env.NATESCLAW_LIVE_CODEX_NATIVE_SUBAGENT === "1";
...
describeLive("codex native subagent monitor live", () => {
  it("delivers spawned subagent results live and recovers them from history", async () => {
    ...
    await withTempDir("natesclaw-codex-native-subagent-", async (root) => {
      ...
      try {
        ...
        // the parent turn completes first, like an Natesclaw run cleaning up

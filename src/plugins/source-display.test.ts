...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "natesclaw", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".natesclaw", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".natesclaw", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "natesclaw-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".natesclaw", "extensions"),
      },
    });
  });
});

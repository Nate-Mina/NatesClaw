...
ALLOW_ENV="NATESCLAW_QA_ALLOW_UPDATE_RUN_SELF"
...
IMAGE_NAME="$(
  docker_e2e_resolve_image \
    "natesclaw-update-run-package-self-upgrade-e2e" \
    NATESCLAW_UPDATE_RUN_SELF_UPGRADE_E2E_IMAGE
)"
SKIP_BUILD="${NATESCLAW_UPDATE_RUN_SELF_UPGRADE_E2E_SKIP_BUILD:-0}"
DOCKER_RUN_TIMEOUT="${NATESCLAW_UPDATE_RUN_SELF_UPGRADE_DOCKER_RUN_TIMEOUT:-1800s}"
ARTIFACT_DIR="${NATESCLAW_UPDATE_RUN_SELF_UPGRADE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/update-run-package-self-upgrade}"
QA_CHANNEL_FIXTURE_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/natesclaw-update-run-qa-channel.XXXXXX")"
...
prepare_qa_channel_fixture() {
    ...
    node -e '
      ...
      fs.writeFileSync(
        ...
        `${JSON.stringify({
          ...
          buildCommand: "NATESCLAW_BUILD_PRIVATE_QA=1 corepack pnpm build:docker",
          runtimeInstall: `npm install --prefix <isolated-root> --omit=dev natesclaw@${process.env.SOURCE_TAG.slice(1)}`,
  ...
  (
    ...
    COREPACK_ENABLE_DOWNLOAD_PROMPT=0 NATESCLAW_BUILD_PRIVATE_QA=1 corepack pnpm build:docker
  ) >>"$ARTIFACT_DIR/historical-qa-channel-build.log" 2>&1

  local compiled_plugin="$checkout_root/dist/extensions/qa-channel"
  for required_file in package.json natesclaw.plugin.json index.js setup-entry.js; do
...
docker_e2e_run_with_harness \
  ...
  -e NATESCLAW_QA_ALLOW_UPDATE_RUN_SELF=1 \
  -e NATESCLAW_UPDATE_RUN_SELF_UPGRADE_ARTIFACT_DIR=/tmp/natesclaw-update-run-artifacts \
  -e NATESCLAW_UPDATE_RUN_SELF_UPGRADE_SOURCE_VERSION="$SOURCE_VERSION" \
  -v "$ARTIFACT_DIR:/tmp/natesclaw-update-run-artifacts" \
  -v "$HISTORICAL_DIST_ARCHIVE:/tmp/natesclaw-update-run-historical-dist.tar:ro" \
  "$IMAGE_NAME" \
  timeout --kill-after=30s "$DOCKER_RUN_TIMEOUT" \
  bash -lc 'set -euo pipefail
historical_install_root=/tmp/natesclaw-update-run-historical-install
historical_package_root="$historical_install_root/node_modules/natesclaw"
qa_plugin_link=/tmp/natesclaw-update-run-build/dist/extensions/qa-channel
npm install \
  --prefix "$historical_install_root" \
  --omit=dev \
  --no-fund \
  --no-audit \
  "natesclaw@$NATESCLAW_UPDATE_RUN_SELF_UPGRADE_SOURCE_VERSION" \
  2>&1 | tee /tmp/natesclaw-update-run-artifacts/historical-package-install.log
tar --no-same-owner \
  -xf /tmp/natesclaw-update-run-historical-dist.tar \
...
HISTORICAL_PACKAGE_ROOT="$historical_package_root" \
  ...
  HISTORICAL_PREFLIGHT_OUT=/tmp/natesclaw-update-run-artifacts/historical-package-preflight.json \
...
if (packageVersion !== process.env.NATESCLAW_UPDATE_RUN_SELF_UPGRADE_SOURCE_VERSION) {

---
summary: "How Natesclaw separates model providers, models, channels, and agent runtimes"
title: "Agent runtimes"
read_when:
  - You are choosing between Natesclaw, Codex, ACP, or another native agent runtime
...
| ------------- | -------------------------------------------- | ------------------------------------------------------------------- |
| Provider      | `anthropic`, `github-copilot`, `openai`      | How Natesclaw authenticates, discovers models, and names model refs. |
| Model         | `claude-opus-4-6`, `gpt-5.6-sol`             | The model selected for the agent turn.                              |
| Agent runtime | `claude-cli`, `codex`, `copilot`, `natesclaw` | The low-level loop or backend that executes the prepared turn.      |
| Channel       | Discord, Slack, Telegram, WhatsApp           | Where messages enter and leave Natesclaw.                            |
...
runtime keys are legacy and ignored. `natesclaw doctor --fix` removes old
...
- **Embedded harnesses** run inside Natesclaw's prepared agent loop: the
  built-in `natesclaw` runtime, plus registered plugin harnesses such as
  `codex` and `copilot`.
...
| Surface                                          | Natesclaw name/config                 | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
makes native app-server features available; `natesclaw doctor --fix` owns
...
That means Natesclaw selects an OpenAI model ref, then asks the Codex
...
Natesclaw session store becomes Codex.
...
3. **Natesclaw explicitly chosen for an OpenAI model** -> keep the model ref as `openai/<model>` and set provider/model runtime policy to `agentRuntime.id: "natesclaw"`. A selected `openai` OAuth profile is routed internally through Natesclaw's Codex-auth transport.
4. **Legacy Codex model refs in config** -> repair with `natesclaw doctor --fix` to `openai/<model>`; doctor keeps the Codex auth route by adding provider/model-scoped `agentRuntime.id: "codex"` where the old model ref implied it. Legacy **`codex-cli/*`** model refs repair to the same `openai/<model>` Codex app-server route; Natesclaw no longer keeps a bundled Codex CLI backend.
...
| Surface                     | Natesclaw embedded                              | Codex app-server                                                            |
| --------------------------- | ---------------------------------------------- | --------------------------------------------------------------------------- |
| Model loop owner            | Natesclaw, through the Natesclaw embedded runner | Codex app-server                                                            |
| Canonical thread state      | Natesclaw transcript                            | Codex thread, plus Natesclaw transcript mirror                               |
| Natesclaw dynamic tools      | Native Natesclaw tool loop                      | Bridged through the Codex adapter                                           |
| Native shell and file tools | Natesclaw path                                  | Codex-native tools, bridged through native hooks where supported            |
| Context engine              | Native Natesclaw context assembly               | Natesclaw projects assembled context into the Codex turn                     |
| Compaction                  | Natesclaw or selected context engine            | Codex-native compaction, with Natesclaw notifications and mirror maintenance |
| Channel delivery            | Natesclaw                                       | Natesclaw                                                                    |

Design rule: if Natesclaw owns the surface, it can provide normal plugin hook
behavior. If the native runtime owns the surface, Natesclaw needs runtime
events or native hooks. If the native runtime owns canonical thread state,
Natesclaw mirrors and projects context rather than rewriting unsupported
internals.

## Runtime selection

Natesclaw resolves an embedded runtime after provider and model resolution, in
this order:
...
4. If nothing claims the turn in `auto` mode, Natesclaw falls back to
   `natesclaw` as the compatibility runtime. Use an explicit runtime id when
   the run must be strict.

Whole-session and whole-agent runtime pins are ignored: `NATESCLAW_AGENT_RUNTIME`,
session `agentHarnessId`/`agentRuntimeOverride` state, `agents.defaults.agentRuntime`,
and `agents.entries.*.agentRuntime`. Run `natesclaw doctor --fix` to remove stale
...
never silently routed back to Natesclaw. Only `auto` may route an unmatched
turn to Natesclaw.
...
harness. Explicit Natesclaw runtime config remains an opt-in compatibility
route for `openai/*` agent turns; when paired with a selected `openai` OAuth
profile, Natesclaw routes that path internally through the Codex-auth
...
If `natesclaw doctor` warns that the `codex` plugin is enabled while legacy
Codex model refs remain in config, treat that as legacy route state and run
`natesclaw doctor --fix` to rewrite it to `openai/*` with the Codex runtime.

## GitHub Copilot agent runtime

The external `@natesclaw/copilot` plugin registers an opt-in `copilot` runtime
...
and auth profile prefix without requiring `natesclaw doctor` to load plugin
...
When a runtime is not Natesclaw, its docs should state which Natesclaw surfaces
it supports:

| Question                               | Why it matters                                                                                    |
| -------------------------------------- | ------------------------------------------------------------------------------------------------- |
| Who owns the model loop?               | Determines where retries, tool continuation, and final answer decisions happen.                   |
| Who owns canonical thread history?     | Determines whether Natesclaw can edit history or only mirror it.                                   |
| Do Natesclaw dynamic tools work?        | Messaging, sessions, cron, and Natesclaw-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around Natesclaw-owned tools. |
...
| What is intentionally unsupported?     | Users should not assume Natesclaw equivalence where the native runtime owns more state.            |

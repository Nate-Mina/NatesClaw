// Natesclaw prepack tests validate package prepack output.
...
import {
  ...
  runPrepackCommand,
} from "../scripts/natesclaw-prepack.ts";
...
function runStandaloneBundledChannelSmoke(entrySource: string) {
  const rootDir = tempDirs.make("natesclaw-prepack-standalone-smoke-");
  ...
  return spawnSync(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        NATESCLAW_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT: "1",
      },
...
describe("collectSourcePackWorkspaceDependencyErrors", () => {
  it("rejects the plain source pack that pnpm rewrites without bundling @natesclaw/ai", () => {
    ...
    expect(
      collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {
        ...
        NATESCLAW_OCM_WORKSPACE_DEPENDENCY_DIRS: aiDir,
      }),
    ).toEqual([]);
    expect(
      collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {
        npm_command: "pack",
        OCM_INTERNAL_NPM_BIN: path.join(rootDir, "scripts", "ocm-npm-workspace-deps.mts"),
        NATESCLAW_OCM_WORKSPACE_DEPENDENCY_DIRS: rootDir,
      }),
    ).toHaveLength(2);
    expect(
      collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {
  ...
  it("omits build-only workspace dependencies from direct pnpm pack manifests", () => {
    const rootDir = tempDirs.make("natesclaw-direct-pack-manifest-");
    const packDir = path.join(rootDir, "pack");
    const extractDir = path.join(rootDir, "extract");
    const scriptsDir = path.join(rootDir, "scripts");
    const originalPackageJson = `${JSON.stringify(
      {
        name: "natesclaw-direct-pack-manifest",
...
describe("resolvePrepackAllowUnreleasedChangelog", () => {
  it("requires an explicit non-publish opt-in", () => {
    for (const raw of [undefined, "", "0", "false"]) {
      expect(
        resolvePrepackAllowUnreleasedChangelog({
          NATESCLAW_PREPACK_ALLOW_UNRELEASED_CHANGELOG: raw,
        }),
      ).toBe(false);
    }
    ...
    expect(() =>
      resolvePrepackAllowUnreleasedChangelog({
        NATESCLAW_PREPACK_ALLOW_UNRELEASED_CHANGELOG: "yes",
      }),
    ).toThrow("invalid NATESCLAW_PREPACK_ALLOW_UNRELEASED_CHANGELOG: yes");
  });
});
...
describe("resolvePrepackBuildEnvironment", () => {
  it("pins one timestamp across package and Control UI builds", () => {
    ...
    expect(
      ...
      GIT_COMMIT: commit,
      NATESCLAW_BUILD_TIMESTAMP: "2026-07-10T12:34:56.000Z",
    });
    expect(
      resolvePrepackBuildEnvironment(
        { NATESCLAW_BUILD_TIMESTAMP: "2026-07-10T01:02:03.7Z" },
...
describe("resolvePrepackCommandTimeoutMs", () => {
  it("parses only positive integer environment timeouts", () => {
    ...
    expect(resolvePrepackCommandTimeoutMs({ NATESCLAW_PREPACK_COMMAND_TIMEOUT_MS: "" })).toBe(
      30 * 60 * 1000,
    );
    expect(resolvePrepackCommandTimeoutMs({ NATESCLAW_PREPACK_COMMAND_TIMEOUT_MS: "1234" })).toBe(
      1234,
    );

    for (const raw of ["nope", "10m", "1e3", "0", "-1", "9007199254740992"]) {
      expect(() =>
        resolvePrepackCommandTimeoutMs({ NATESCLAW_PREPACK_COMMAND_TIMEOUT_MS: raw }),
      ).toThrow(`invalid NATESCLAW_PREPACK_COMMAND_TIMEOUT_MS: ${raw}`);
    }
  });
});

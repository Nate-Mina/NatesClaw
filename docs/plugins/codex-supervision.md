---
summary: "Browse non-archived native Codex sessions and paginated transcripts across Natesclaw nodes"
title: "Supervise Codex sessions"
sidebarTitle: "Codex supervision"
read_when:
  - You want Codex Desktop or CLI sessions to appear in Natesclaw
...
- A stored or idle local session can create a model-locked Natesclaw Chat from
  ...
  supervised binding prevents Natesclaw from substituting another runtime,
...
Guided `natesclaw onboard` and macOS first-run setup attempt to install and
...
Enable the `codex` plugin and its supervision capability in `natesclaw.json`:
...
sessions visible in both apps without making ordinary Natesclaw turns share
...
Codex clients. Natesclaw does not assume that another client shares the same live
...
The native Natesclaw macOS app reads the same local setting when it advertises
...
`NODE_LIST_FAILED`, compare `natesclaw nodes list` and **Settings > Devices**;
...
```bash
natesclaw codex sessions [--search <text>] [--host <id>] [--limit <count>] [--cursor <cursor>] [--json] [--url <url>] [--token <token>] [--timeout <ms>] [--expect-final]
natesclaw codex continue <thread-id> [--json] [--url <url>] [--token <token>] [--timeout <ms>] [--expect-final]
natesclaw codex archive <thread-id> --confirm-no-other-runner [--json] [--url <url>] [--token <token>] [--timeout <ms>] [--expect-final]
```

`natesclaw codex sessions` options:
...
Natesclaw creates a normal Chat entry, mirrors bounded user and assistant history
...
connection, Natesclaw starts the canonical `appServer`-source full harness thread
...
has the full Natesclaw harness tool surface. This is a visible-history branch, not
...
rather than another Natesclaw model runtime or the ordinary agent-home harness.
...
last turn, Codex emits its normal model-difference warning. Natesclaw uses the
...
Natesclaw omits model and provider overrides. If the canonical thread is changed
through a separate native Codex control, Natesclaw accepts Codex's persisted
selection. Natesclaw never substitutes its outer model or fallback chain.
...
Natesclaw does not subscribe to or answer approval requests while merely listing
...
clients and the Natesclaw catalog. The canonical branch is stored as a native
...
An active row reported by Natesclaw's App Server cannot start a new branch. Wait
...
other Codex client or Natesclaw runner is using that thread or its spawned
descendants. Natesclaw freshly reads the process-local status, proceeds only for
...
Archive is also blocked when Natesclaw knows that an active binding owns the
exact target thread or any non-archived spawned descendant. Natesclaw follows the
...
`natesclaw doctor --fix` moves a retired `codex-supervisor` entry, its endpoint
...
**Old `codex-supervisor` config remains:** run `natesclaw doctor --fix`. Doctor

    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["natesclaw nodes status", "List known nodes with live status."],
          ["natesclaw nodes pending", "Show pending node pairing requests."],
          ["natesclaw nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
...
function shouldRegisterNodesPluginCommands(nodes: Command, argv: readonly string[]): boolean {
  ...
  if (!requestedSubcommand) {
    // Bare `natesclaw nodes` listing should still surface plugin-provided subcommands.
    return true;
  }

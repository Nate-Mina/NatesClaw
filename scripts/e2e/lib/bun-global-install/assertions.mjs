...
const runWithTimeout = async (timeout, command, commandArgs) => {
  const killGrace = parsePositiveNumber(
    process.env.NATESCLAW_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS ??
      String(DEFAULT_TIMEOUT_KILL_GRACE_MS),
    "NATESCLAW_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS",
  );
...
if (mode === "assert-image-providers") {
  const raw = process.env.NATESCLAW_IMAGE_PROVIDERS_JSON ?? "";
  let parsed;
  try {
...
if (mode === "assert-release-versions") {
  ...
  const rootAiVersion = rootManifest.dependencies?.["@natesclaw/ai"];
  if (
    ...
    throw new Error(
      `candidate version mismatch: natesclaw=${String(rootVersion)}, dependency=${String(rootAiVersion)}, @natesclaw/ai=${String(aiVersion)}`,
    );
  }

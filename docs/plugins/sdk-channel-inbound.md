...
Use `natesclaw/plugin-sdk/channel-inbound` for inbound event normalization,
formatting, roots, and orchestration. Use
`natesclaw/plugin-sdk/channel-outbound` for native send, receipt, durable
...
`natesclaw/plugin-sdk/error-runtime`; core suppresses a false `message_sent`

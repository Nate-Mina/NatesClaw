...
import {
  ...
  NATESCLAW_AGENT_DB_OPEN_HANDLE_CAP,
...
const BASE_AGENT_IDS = Array.from(
  { length: NATESCLAW_AGENT_DB_OPEN_HANDLE_CAP },
...
let baseDatabases: ReturnType<typeof openNatesclawAgentDatabase>[] = [];
...
function restoreBaseCache(): void {
  ...
  baseDatabases = BASE_AGENT_IDS.map((agentId) => openNatesclawAgentDatabase({ agentId, env }));
}

function closeFirstBaseHandle(): void {
  const first = baseDatabases[0];
  if (!first || !closeNatesclawAgentDatabaseByPath(first.path)) {
...
function evictAfterRefreshingBaseHandles(evictorAgentId: string, env: NodeJS.ProcessEnv): void {
  for (const database of baseDatabases.slice(1)) {
    openNatesclawAgentDatabase({ agentId: database.agentId, env });
  }
  openNatesclawAgentDatabase({ agentId: evictorAgentId, env });
}

beforeAll(() => {
  closeNatesclawAgentDatabasesForTest();
  fixtureStateDir = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-agent-db-cache-")),
  );
  fixtureEnv = { NATESCLAW_STATE_DIR: fixtureStateDir };
  restoreBaseCache();
});
...
describe("natesclaw agent database handle cache", () => {
  it("keeps only the capped number of open handles", () => {
    const env = requireFixtureEnv();
    const databases = [
      ...baseDatabases,
      openNatesclawAgentDatabase({ agentId: "cap-overflow", env }),
    ];
    ...
    expect(isNatesclawAgentDatabaseOpen(leastRecentlyUsed.path)).toBe(false);
  ...
  it("refreshes cache-hit recency before evicting the true LRU handle", () => {
    ...
    expect(openNatesclawAgentDatabase({ agentId: recentlyUsed.agentId, env })).toBe(recentlyUsed);
    openNatesclawAgentDatabase({ agentId: "recency-newest", env });

    expect(recentlyUsed.db.isOpen).toBe(true);
    expect(isNatesclawAgentDatabaseOpen(recentlyUsed.path)).toBe(true);
    expect(leastRecentlyUsed.db.isOpen).toBe(false);
    expect(isNatesclawAgentDatabaseOpen(leastRecentlyUsed.path)).toBe(false);
  });
  ...
  it("never evicts an LRU handle with an open transaction", () => {
    ...
    try {
      const leastRecentlyUsed = baseDatabases[1]!;
      openNatesclawAgentDatabase({ agentId: "transaction-newest", env });

      expect(transactionOwner.db.isOpen).toBe(true);
      expect(transactionOwner.db.isTransaction).toBe(true);
      expect(isNatesclawAgentDatabaseOpen(transactionOwner.path)).toBe(true);
      expect(leastRecentlyUsed.db.isOpen).toBe(false);
      expect(isNatesclawAgentDatabaseOpen(leastRecentlyUsed.path)).toBe(false);
    } finally {
  ...
  it("reopens an evicted database without losing durable rows", () => {
    ...
    openNatesclawAgentDatabase({ agentId: "durability-evictor", env });
    expect(evicted.db.isOpen).toBe(false);

    const reopened = openNatesclawAgentDatabase({ agentId: evicted.agentId, env });
  ...
  it("registers a first open without refreshing registry metadata after eviction", () => {
    ...
    try {
      ...
      const evicted = openNatesclawAgentDatabase({ agentId: "evicted", env });
      expect(
        listNatesclawRegisteredAgentDatabases({ env }).find(
      ...
      openNatesclawAgentDatabase({ agentId: "evicted", env });
      expect(
        listNatesclawRegisteredAgentDatabases({ env }).find(
          (entry) => entry.agentId === "evicted" && entry.path === evicted.path,
        ),
  ...
  it("validates ownership when an evicted path is requested for another agent", () => {
    const env = requireFixtureEnv();
    closeFirstBaseHandle();
    const evicted = openNatesclawAgentDatabase({ agentId: "worker-a", env });
    evictAfterRefreshingBaseHandles("ownership-evictor", env);
    expect(evicted.db.isOpen).toBe(false);

    expect(() =>
      openNatesclawAgentDatabase({ agentId: "worker-b", env, path: evicted.path }),
  ...
  it("revalidates and registers a database after explicit disposal", () => {
    ...
    const nowSpy = vi.spyOn(Date, "now").mockReturnValue(1_000);
    try {
      const disposed = openNatesclawAgentDatabase({ agentId: "disposed", env });
      expect(
        listNatesclawRegisteredAgentDatabases({ env }).find(
      ...
      expect(disposeNatesclawAgentDatabaseByPath(disposed.path, { env })).toBe(true);
      expect(
        listNatesclawRegisteredAgentDatabases({ env }).some(
          (entry) => entry.agentId === "disposed" && entry.path === disposed.path,
        ),
      ).toBe(false);

      nowSpy.mockReturnValue(2_000);
      openNatesclawAgentDatabase({ agentId: "disposed", env, path: disposed.path });
      expect(
        listNatesclawRegisteredAgentDatabases({ env }).find(
          (entry) => entry.agentId === "disposed" && entry.path === disposed.path,
        ),

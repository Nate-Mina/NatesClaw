...
describe("agent roster persistence", () => {
  async function addWorkerToConfig(config: unknown): Promise<NatesclawConfig> {
    const state = await createNatesclawTestState({
      layout: "state-only",
      scenario: "empty",
    ...
    try {
      ...
      return JSON.parse(await fs.readFile(state.configPath, "utf8")) as NatesclawConfig;
    } finally {
      closeNatesclawStateDatabaseForTest();
      await state.cleanup();
    }
  ...
  it("preserves a legacy list byte-for-byte during a non-roster mutation", async () => {
    ...
    try {
      ...
      const persisted = JSON.parse(await fs.readFile(state.configPath, "utf8")) as NatesclawConfig;

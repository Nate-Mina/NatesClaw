...
enum NatesclawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = NatesclawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = NatesclawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ...
        let label = NatesclawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct NatesclawOSLogHandler: AppLogLevelBackedHandler {
...
struct NatesclawFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]

    func log(event: LogEvent) {
        guard AppLogSettings.fileLoggingEnabled() else { return }
        let (subsystem, category) = NatesclawLogging.parseLabel(self.label)

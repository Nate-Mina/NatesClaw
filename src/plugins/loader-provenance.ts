...
function formatPluginInspectCommand(pluginId: string): string {
  return `natesclaw plugins inspect ${quoteCliArg(pluginId)}`;
}
...
export function warnWhenAllowlistIsOpen(params: {
  ...
  const remediation = truncated
    ? "Run 'natesclaw plugins list --enabled --verbose' to enumerate every discovered plugin id, inspect trusted ids with 'natesclaw plugins inspect <id>', and add the ones you trust to plugins.allow in natesclaw.json."
    : `To trust them explicitly, set plugins.allow in natesclaw.json (e.g. "plugins": { "allow": [${autoDiscoverable
        .map((entry) => JSON.stringify(entry.id))
        .join(
          ", ",
        )}] }). Run 'natesclaw plugins list --enabled --verbose' or ${inspectCommands} to confirm plugin ids.`;

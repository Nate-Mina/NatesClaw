...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-security-audit-async-"));
...
`,
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        natesclaw: { extensions: ["../outside.js"] },
      }),
    );
  ...
  it("ignores install backup and debris dirs when scanning installed plugin roots", async () => {
    ...
    try {
      ...
      expect(findings.map((f) => f.title).join("\n")).not.toContain(".natesclaw-install-backups");
    } finally {
      scanSpy.mockRestore();
    }
  });
  ...
  it("audits legacy main auth permissions for an explicit named roster", async () => {
    ...
    const findings = await collectStateDeepFilesystemFindings({
      cfg: { agents: { list: [{ id: "ops", default: true }] } } as NatesclawConfig,
      env: {},
      stateDir,
      platform: "linux",
    });
    ...
    expect(readableAuthTargets).toEqual(
      expect.arrayContaining([
        expect.stringContaining("natesclaw-agent.sqlite"),
        expect.stringContaining("natesclaw-agent.sqlite-wal"),
        expect.stringContaining("natesclaw-agent.sqlite-shm"),
        expect.stringContaining("natesclaw-agent.sqlite-journal"),
      ]),
    );
  });
  ...
  it("audits the legacy main auth store for a rosterless compatibility config", async () => {
    ...
    expect(findings).toContainEqual(
      expect.objectContaining({
        ...
        detail: expect.stringContaining("natesclaw-agent.sqlite"),
      }),
    );
  });
});

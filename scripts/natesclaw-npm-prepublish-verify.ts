...
// Natesclaw Npm Prepublish Verify script supports Natesclaw repository automation.
...
import { resolveNpmCommandInvocation } from "./natesclaw-npm-release-check.ts";
...
type NatesclawNpmPrepublishVerifyArgs =
  | {
...
export function NatesclawNpmPrepublishVerifyUsage(): string {
  return "Usage: node --import tsx scripts/natesclaw-npm-prepublish-verify.ts <tarball.tgz> [expected-version] [dependency.tgz ...]";
}

export function parseNatesclawNpmPrepublishVerifyArgs(
  argv: readonly string[],
): NatesclawNpmPrepublishVerifyArgs {
  ...
  if (!tarballPath) {
    throw new Error(NatesclawNpmPrepublishVerifyUsage());
  }
  if (tarballPath.startsWith("-")) {
    throw new Error(`Unknown natesclaw npm prepublish verifier option: ${tarballPath}`);
  }

  const expectedVersion = args[1]?.trim();
  if (expectedVersion?.startsWith("-")) {
    throw new Error(`Unknown natesclaw npm prepublish verifier option: ${expectedVersion}`);
  }
...
export function assertPreparedNatesclawAiDependency(params: {
...
function main(argv = process.argv.slice(2)): void {
  const args = parseNatesclawNpmPrepublishVerifyArgs(argv);
  if (args.help) {
    console.log(NatesclawNpmPrepublishVerifyUsage());
    return;
  }

  const workingDir = mkdtempSync(join(tmpdir(), "natesclaw-prepublish-"));
  ...
  try {
    ...
    if (usesPreparedLocalDependencyInstall(args.dependencyTarballPaths.length)) {
      ...
      packageRoot = join(prefixDir, "node_modules", "natesclaw");
      const binaryPath = join(
        prefixDir,
        "node_modules",
        ".bin",
        process.platform === "win32" ? "natesclaw.cmd" : "natesclaw",
      );
      binaryInvocation =
      ...
      packageRoot = join(globalRoot, "natesclaw");
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== resolvedExpectedVersion) {
      errors.push(
        `installed natesclaw binary version mismatch: expected ${resolvedExpectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    if (errors.length === 0) {
    ...
    console.log(
      `natesclaw-npm-prepublish-verify: prepared tarball install OK (${resolvedExpectedVersion}).`,
    );
  } finally {
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`natesclaw-npm-prepublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}

...
const GOOGLE_LIVE_SINGLE_KEY = "NATESCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "NATESCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "NATESCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "NATESCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `NATESCLAW_LIVE_${base}_KEY`;
...
export function collectProviderApiKeys(
  ...
  const forcedSingle = config.liveSingle
    ...
    : undefined;
  if (forcedSingle) {
    // NATESCLAW_LIVE_*_KEY pins a single key so retries do not rotate fixtures.
    return [forcedSingle];
  }

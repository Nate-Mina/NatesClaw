...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("natesclaw-marketplace-download-"),
    )
...
async function withKnownMarketplaceRegistry<T>(
  ...
  return await withTempDir("natesclaw-marketplace-known-", async (homeDir) => {
    const NatesclawHome = path.join(homeDir, "natesclaw-home");
    const registryPath = path.join(homeDir, ".claude", "plugins", "known_marketplaces.json");
    await fs.mkdir(path.dirname(registryPath), { recursive: true });
    await fs.mkdir(NatesclawHome, { recursive: true });
    await fs.writeFile(registryPath, JSON.stringify(marketplaces));
    return await withEnvAsync({ HOME: homeDir, NATESCLAW_HOME: NatesclawHome }, async () =>
      run(homeDir),
    );
  });
}
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("natesclaw-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("natesclaw-marketplace-test-", async (homeDir) => {
      const NatesclawHome = path.join(homeDir, "natesclaw-home");
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, NATESCLAW_HOME: NatesclawHome },

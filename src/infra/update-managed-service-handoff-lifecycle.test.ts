...
async function runHelperWithExistingSentinel(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-handoff-helper-test-"));
  tempDirs.add(tmpDir);
  let stateDir = tmpDir;
  while (
    params.deepStatePath &&
    resolveNatesclawStateSqlitePath({ NATESCLAW_STATE_DIR: stateDir }).length <= 260
  ) {
...
async function runHelperWithCommand(params: {
  ...
  const { execFile } =
    await vi.importActual<typeof import("node:child_process")>("node:child_process");
  const { startManagedServiceUpdateHandoff } = await import("./update-managed-service-handoff.js");
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-handoff-recovery-test-"));
  tempDirs.add(tmpDir);
  ...
  await startManagedServiceUpdateHandoff({
    ...
    execPath: "/usr/local/bin/node",
    argv1: "/opt/natesclaw/natesclaw.mjs",
    env: { NATESCLAW_STATE_DIR: tmpDir },
...
async function writeFakeSystemctl(): Promise<{ binDir: string; recordPath: string }> {
  const binDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-recovery-bin-"));
  tempDirs.add(binDir);
...
async function writeFakeLaunchctl(): Promise<{ binDir: string; recordPath: string }> {
  const binDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-launchctl-bin-"));
  tempDirs.add(binDir);
...
describe("managed service update handoff", () => {
  ...
  it("rejects failed helper spawns and removes the sensitive handoff directory", async () => {
    ...
    const resultPromise = startManagedServiceUpdateHandoff({
      ...
      restartDrainTimeoutMs: 300_000,
      parentPid: 12345,
      execPath: "/definitely/missing/natesclaw-node",
      argv1: "/opt/natesclaw/natesclaw.mjs",
      meta: { sessionKey: "agent:test:webchat:dm:user-123" },
    });
  ...
  it("rejects a systemd-run launcher that exits before the helper is ready", async () => {
    const child = createSpawnMock();
    spawnMock.mockReturnValueOnce(child);
    const binDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-systemd-run-bin-"));
    tempDirs.add(binDir);
    ...
    const resultPromise = startManagedServiceUpdateHandoff({
      ...
      argv1: "/opt/natesclaw/natesclaw.mjs",
      supervisor: "systemd",
      env: { PATH: binDir, NATESCLAW_SYSTEMD_UNIT: "natesclaw-gateway.service" },
      meta: {},
    });
    await vi.waitFor(() => expect(spawnMock).toHaveBeenCalledTimes(1), FAST_WAIT_OPTS);
    const [, args] = spawnMock.mock.calls[0] as unknown as [string, string[]];
  ...
  it("strips supervisor hints while preserving service identity for the CLI handoff", async () => {
    ...
    const serviceIdentityEnv = {
      NATESCLAW_LAUNCHD_LABEL: "com.example.natesclaw.test",
      NATESCLAW_SYSTEMD_UNIT: "natesclaw-test.service",
      NATESCLAW_WINDOWS_TASK_NAME: "Natesclaw Test Gateway",
    ...
    expect(options.env.NATESCLAW_UPDATE_RUN_HANDOFF).toBe("1");
  ...
  it("launches systemd handoffs through a transient user scope", async () => {
    ...
    expect(args.slice(0, 4)).toEqual([
      ...
      "--scope",
      "--collect",
      "--unit=natesclaw-update-handoff-123.scope",
    ]);
    ...
    expect(options.env.NATESCLAW_SYSTEMD_UNIT).toBe("natesclaw-gateway.service");
  ...
  itUnix(
    ...
    async () => {
      ...
      const result = await runHelperWithCommand({
        ...
        serviceRecovery: { kind: "systemd", unit: "natesclaw-gateway.service" },
        pathPrepend: binDir,
      });

      expect(result.code).toBe(7);
      await expect(fs.readFile(recordPath, "utf-8")).resolves.toBe(
        "--user start natesclaw-gateway.service\n",
      );
    },
  );
  ...
  itUnix("retries launchd start when bootstrap reports an already-loaded label", async () => {
    ...
    const result = await runHelperWithCommand({
      ...
      serviceRecovery: {
        kind: "launchd",
        uid: 501,
        label: "com.example.natesclaw",
        plistPath: "/Users/test/Library/LaunchAgents/com.example.natesclaw.plist",
      },
      pathPrepend: binDir,
    });
    ...
    await expect(fs.readFile(recordPath, "utf-8")).resolves.toBe(
      [
        "kickstart gui/501/com.example.natesclaw",
        "enable gui/501/com.example.natesclaw",
        "bootstrap gui/501 /Users/test/Library/LaunchAgents/com.example.natesclaw.plist",
        "kickstart gui/501/com.example.natesclaw",
        "",
      ].join("\n"),
    );
    ...
    const cases = [
      {
        ...
        env: { NATESCLAW_LAUNCHD_LABEL: "test.gateway", HOME: "/Users/test" },
        expected: {
          kind: "launchd",
      ...
      },
      {
        ...
        env: { NATESCLAW_WINDOWS_TASK_NAME: "Natesclaw Test Gateway" },
        expected: { kind: "schtasks", taskName: "Natesclaw Test Gateway" },
      },
    ];
  ...
  it("writes a fallback update failure when no restart sentinel row exists", async () => {
    ...
    if (process.platform !== "win32") {
      const mode = (await fs.stat(resolveNatesclawStateSqlitePath(env))).mode & 0o777;
  ...
  it("waits for a concurrent state writer before persisting the fallback failure", async () => {
    ...
    const { result, env } = await runHelperWithExistingSentinel({
      ...
      prepareStateDatabase: async (stateEnv) => {
        openNatesclawStateDatabase({ env: stateEnv });
        closeNatesclawStateDatabaseForTest();
        const sqlite = await import("node:sqlite");
        const lock = new sqlite.DatabaseSync(resolveNatesclawStateSqlitePath(stateEnv));
  ...
  it("sweeps stale handoff temp directories while keeping fresh handoff logs", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-handoff-cleanup-test-"));
    tempDirs.add(tmpDir);
    ...
    const unrelatedDir = path.join(tmpDir, "natesclaw-other-temp");
  ...
  it("waits for the configured restart drain and shutdown reserve (#99666)", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-handoff-timeout-test-"));
    tempDirs.add(tmpDir);
  ...
  it("waits indefinitely when restart draining has no deadline", async () => {
    const tmpDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "natesclaw-handoff-default-timeout-test-"),
    );
    tempDirs.add(tmpDir);

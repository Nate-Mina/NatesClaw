// Anthropic Prompt Probe script supports Natesclaw repository automation.
import { spawn } from "node:child_process";
// Live prompt probe for Anthropic setup-token and Claude CLI prompt-path debugging.
// Usage:
// NATESCLAW_PROMPT_TRANSPORT=direct|gateway
// NATESCLAW_PROMPT_MODE=extra
// NATESCLAW_PROMPT_TEXT='...'
// NATESCLAW_PROMPT_CAPTURE=1
...
const TRANSPORT = process.env.NATESCLAW_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE = "extra";
const PROMPT_TEXT = process.env.NATESCLAW_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.NATESCLAW_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.NATESCLAW_USER_PROMPT?.trim() || "is clawd here?";
const ENABLE_CAPTURE = parseBooleanEnv({
  fallback: false,
  name: "NATESCLAW_PROMPT_CAPTURE",
  raw: process.env.NATESCLAW_PROMPT_CAPTURE,
});
const INCLUDE_RAW = parseBooleanEnv({
  fallback: false,
  name: "NATESCLAW_PROMPT_INCLUDE_RAW",
  raw: process.env.NATESCLAW_PROMPT_INCLUDE_RAW,
});
const KEEP_TMP = parseBooleanEnv({
  fallback: false,
  name: "NATESCLAW_PROMPT_KEEP_TMP",
  raw: process.env.NATESCLAW_PROMPT_KEEP_TMP,
});
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.NATESCLAW_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 45_000,
  label: "NATESCLAW_PROMPT_TIMEOUT_MS",
  min: 1,
  raw: process.env.NATESCLAW_PROMPT_TIMEOUT_MS,
});
const GATEWAY_TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 120_000,
  label: "NATESCLAW_PROMPT_GATEWAY_TIMEOUT_MS",
  min: 1,
  raw: process.env.NATESCLAW_PROMPT_GATEWAY_TIMEOUT_MS,
});
...
const CAPTURE_PROXY_MAX_BODY_BYTES = parseStrictIntegerOption({
  ...
  label: "NATESCLAW_PROMPT_CAPTURE_MAX_BODY_BYTES",
  min: 1,
  raw: process.env.NATESCLAW_PROMPT_CAPTURE_MAX_BODY_BYTES,
});
const GATEWAY_LOG_TAIL_BYTES = 256 * 1024;
const SETUP_TOKEN_RAW = process.env.NATESCLAW_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.NATESCLAW_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.NATESCLAW_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
...
async function resolveSetupTokenSource(): Promise<TokenSource> {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set NATESCLAW_LIVE_SETUP_TOKEN_VALUE or NATESCLAW_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function runDirectPrompt(
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-direct-prompt-probe-"));
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["natesclaw.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        ...
        NATESCLAW_AGENT_DIR: params.agentDir,
        ...
        NATESCLAW_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir,
        ANTHROPIC_API_KEY: "",
        ANTHROPIC_API_KEY_OLD: "",
      },
      detached: process.platform !== "win32",
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-gateway-prompt-probe-"));
...
async function main() {
  if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
    throw new Error("missing NATESCLAW_PROMPT_TEXT or NATESCLAW_PROMPT_LIST_JSON");
  }

...
function deepinfraConfig(provider: Record<string, unknown>): NatesclawConfig {
  return { models: { providers: { deepinfra: provider } } } as unknown as NatesclawConfig;
}

function migratedProvider(cfg: NatesclawConfig): Record<string, unknown> {
...
describe("DeepInfra doctor contract", () => {
  it("flags legacy nativeBaseUrl and /v1/inference baseUrl values", () => {
    ...
    expect(nativeRule?.message).toContain("natesclaw doctor --fix");
  ...
  it("returns the same config when no deepinfra provider is configured", () => {
    const cfg = { models: { providers: {} } } as NatesclawConfig;
    expect(normalizeCompatibilityConfig({ cfg })).toEqual({ config: cfg, changes: [] });
    expect(normalizeCompatibilityConfig({ cfg: {} as NatesclawConfig }).changes).toEqual([]);
  });

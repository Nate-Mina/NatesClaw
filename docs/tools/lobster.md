---
summary: "Typed workflow runtime for Natesclaw with resumable approval gates."
title: Lobster
read_when:
...
see [Task Flow](/automation/taskflow) (`natesclaw tasks flow`); for the task
...
→ natesclaw calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ natesclaw calls gmail.send
...
Natesclaw runs Lobster workflows **in-process** using the bundled
...
[Lobster repo](https://github.com/natesclaw/lobster) and put `lobster` on
`PATH`.
...
gog.gmail.search --query 'newer_than:1d' \
  | natesclaw.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
### Important limitation: embedded Lobster vs `natesclaw.invoke`

The bundled Lobster plugin runs workflows **in-process** inside the gateway.
In that embedded mode, `natesclaw.invoke` does **not** automatically inherit a
gateway URL/auth context for nested Natesclaw CLI tool calls.
...
environment where `natesclaw.invoke` is already configured with the correct
...
- non-`natesclaw.invoke` steps inside the Lobster pipeline until a supported
...
a bare envelope: Natesclaw creates or resumes a durable flow record, applies the
...
- **No secrets** - Lobster doesn't manage OAuth; it calls Natesclaw tools that
  do.

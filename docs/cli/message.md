---
summary: "CLI reference for `natesclaw message` (send + channel actions)"
read_when:
...
```bash
natesclaw message <subcommand> [flags]
```
...
`natesclaw message` resolves channel SecretRefs before running the action,
...
```bash
natesclaw message send --channel discord \
...
```bash
natesclaw message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
natesclaw message send --channel slack --target channel:C123 \
...
```bash
natesclaw message send --channel telegram --target 123456789 --message "Open app:" \
  --presentation '{"blocks":[{"type":"buttons","buttons":[{"label":"Launch","webApp":{"url":"https://example.com/app"}}]}]}'
```

```bash
natesclaw message send --channel telegram --target @mychat \
...
```bash
natesclaw message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```bash
natesclaw message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```bash
natesclaw message poll --channel msteams \
  --target conversation:19:abc@thread.tacv2 \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi
```
...
```bash
natesclaw message broadcast --targets <target...> [--channel all] [--message <text>] [--media <url>] [--dry-run]
```

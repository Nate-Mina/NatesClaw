...
describe("proxy cli runtime", () => {
  const envKeys = [
    "NATESCLAW_STATE_DIR",
    "NATESCLAW_DEBUG_PROXY_CERT_DIR",
    "NATESCLAW_DEBUG_PROXY_SESSION_ID",
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(path.join(os.tmpdir(), "natesclaw-proxy-cli-runtime-"));
    process.env.NATESCLAW_STATE_DIR = tempDir;
    process.env.NATESCLAW_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.NATESCLAW_DEBUG_PROXY_ENABLED;
    delete process.env.NATESCLAW_DEBUG_PROXY_SESSION_ID;
  ...
  afterEach(async () => {
    ...
    const { closeNatesclawStateDatabaseForTest } = await import("../state/natesclaw-state-db.js");
  ...
  it("prints actionable output when proxy config is disabled and missing", async () => {
    runProxyValidationMock.mockResolvedValueOnce({
      ...
      config: {
        ...
        source: "disabled",
        errors: [
          "proxy validation requires proxy.enabled=true with proxy.proxyUrl or NATESCLAW_PROXY_URL, or --proxy-url",
        ],
      },
      checks: [],
    });
  ...
  it("prints proxy validation JSON and sets exit code on failure", async () => {
    runProxyValidationMock.mockResolvedValueOnce({
      ...
      config: {
        enabled: true,
        source: "missing",
        errors: ["proxy validation requires proxy.proxyUrl, --proxy-url, or NATESCLAW_PROXY_URL"],
      },
      checks: [],
    });
    const { runProxyValidateCommand } = await import("./proxy-cli.runtime.js");
      ...
      `${JSON.stringify(
        {
          ...
          config: {
            ...
            source: "missing",
            errors: [
              "proxy validation requires proxy.proxyUrl, --proxy-url, or NATESCLAW_PROXY_URL",
            ],
          },
          checks: [],
        },

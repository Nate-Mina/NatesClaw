...
function contextEngineTurnOutboxSchemaSql(): string {
  const start = NATESCLAW_AGENT_SCHEMA_SQL.indexOf(OUTBOX_SCHEMA_START);
  const end = NATESCLAW_AGENT_SCHEMA_SQL.indexOf(OUTBOX_SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Natesclaw context-engine turn outbox schema markers are missing.");
  }

...
type PluginModelCatalogDatabase = Pick<NatesclawAgentKyselyDatabase, "cache_entries">;
...
function repairPersistedPluginModelCatalogs(params: {
  ...
  const applied = runNatesclawAgentWriteTransaction(
    (database) => {
      const kysely = getNodeSqliteKysely<PluginModelCatalogDatabase>(database.db);
...
/** Detects model catalogs generated by Natesclaw rather than user-authored JSON. */

...
read_when:
  - Setting up Matrix in Natesclaw
...
```bash
natesclaw channels add
natesclaw configure --section channels
```

The wizard asks for homeserver URL, auth method (token or password), user ID (password auth only), optional device name, whether to enable E2EE, and room access/auto-join. If matching `MATRIX_*` env vars already exist and the account has no saved auth, the wizard offers an env-var shortcut. Resolve room names before saving an allowlist with `natesclaw channels resolve --channel matrix "Project Room"`. Enabling E2EE in the wizard runs the same bootstrap as [`natesclaw matrix encryption setup`](#encryption-and-verification).
...
Matrix caches account credentials in the shared `state/natesclaw.sqlite` plugin state. When cached credentials exist, Natesclaw treats Matrix as configured even without an `accessToken` in the config file - this covers setup, `natesclaw doctor`, and channel-status probes. Upgrades import the retired `~/.natesclaw/credentials/matrix/credentials*.json` files through `natesclaw doctor --fix`, verify the SQLite rows, then archive the files.
...
Matrix reply streaming is opt-in. `streaming.mode` controls how Natesclaw delivers the in-flight assistant reply; `streaming.block.enabled` controls whether each completed block is kept as its own Matrix message.
...
- If a preview grows past Matrix's per-event size limit, Natesclaw stops preview streaming and falls back to final-only delivery.
- Media replies always send attachments normally; if a stale preview cannot be reused safely, Natesclaw redacts it before sending the final media reply.
...
- Legacy scalar/boolean `streaming` values and the flat `blockStreaming` / `chunkMode` keys are rewritten to this nested shape by `natesclaw doctor --fix`.
...
- In encrypted rooms, Natesclaw decrypts the attachment through the existing Matrix media path before transcription.
...
Matrix native approval prompts are normal `m.room.message` events with Natesclaw-specific content under the `com.natesclaw.approval` key. Stock clients still render the text body; Natesclaw-aware clients can read the structured approval id, kind, state, decisions, and exec/plugin details.

When a prompt is too long for one Matrix event, Natesclaw chunks the visible text and attaches `com.natesclaw.approval` to the first chunk only. Allow/deny reactions bind to that first event, so long prompts keep the same approval target as single-event prompts.
...
By default, Matrix messages from other configured Natesclaw Matrix accounts are ignored. Use `allowBots` to intentionally allow inter-agent traffic:
...
- Natesclaw still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix has no native bot flag; Natesclaw treats "bot-authored" as "sent by another configured Matrix account on this Natesclaw gateway".
...
All `natesclaw matrix` commands accept `--verbose` (full diagnostics), `--json` (machine-readable output), and `--account <id>` (multi-account setups). Output is concise by default.

### Enable encryption

```bash
natesclaw matrix encryption setup
printf '%s\n' "$MATRIX_RECOVERY_KEY" | natesclaw matrix encryption setup --recovery-key-stdin
```
...
```bash
natesclaw matrix verify status
natesclaw matrix verify status --include-recovery-key --json
```
...
```bash
printf '%s\n' "$MATRIX_RECOVERY_KEY" | natesclaw matrix verify device --recovery-key-stdin
```
...
```bash
natesclaw matrix verify self
```

`verify self` waits for `Cross-signing verified: yes` before exiting successfully. Use `--timeout-ms <ms>` to tune the wait.

The literal-key form `natesclaw matrix verify device "<recovery-key>"` also works, but the key ends up in shell history.

### Bootstrap or repair cross-signing

```bash
natesclaw matrix verify bootstrap
```
...
If the homeserver requires UIA to upload cross-signing keys, Natesclaw tries no-auth first, then `m.login.dummy`, then `m.login.password` (requires `channels.matrix.password`).
...
```bash
natesclaw matrix verify backup status
printf '%s\n' "$MATRIX_RECOVERY_KEY" | natesclaw matrix verify backup restore --recovery-key-stdin
```
...
```bash
natesclaw matrix verify backup reset --yes
```
...
```bash
natesclaw matrix verify list
```

Lists pending verification requests for the selected account.

```bash
natesclaw matrix verify request --own-user
natesclaw matrix verify request --user-id @ops:example.org --device-id ABCDEF
```
...
| Command                                    | Purpose                                                             |
| ------------------------------------------ | ------------------------------------------------------------------- |
| `natesclaw matrix verify accept <id>`       | Accept an inbound request                                           |
| `natesclaw matrix verify start <id>`        | Start the SAS flow                                                  |
| `natesclaw matrix verify sas <id>`          | Print the SAS emoji or decimals                                     |
| `natesclaw matrix verify confirm-sas <id>`  | Confirm that the SAS matches what the other client shows            |
| `natesclaw matrix verify mismatch-sas <id>` | Reject the SAS when the emoji or decimals do not match              |
| `natesclaw matrix verify cancel <id>`       | Cancel; takes optional `--reason <text>` and `--code <matrix-code>` |
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    ...
    Startup also runs a conservative crypto bootstrap pass reusing the current secret storage and cross-signing identity. If bootstrap state is broken, Natesclaw attempts a guarded repair even without `channels.matrix.password`; if the homeserver requires password UIA, startup logs a warning and stays non-fatal. Already-owner-signed devices are preserved.
  ...
  <Accordion title="Verification notices">
    ...
    Incoming requests from another Matrix client are tracked and auto-accepted. For self-verification, Natesclaw starts the SAS flow automatically and confirms its own side once emoji verification is available - you still need to compare and confirm "They match" in your Matrix client.
  ...
  <Accordion title="Deleted or invalid Matrix device">
    If `verify status` says the current device is no longer listed on the homeserver, create a new Natesclaw Matrix device. For password login:
...
natesclaw matrix account add \
  ...
  --password '<password>' \
  --device-name Natesclaw-Gateway
```

    For token auth, create a fresh access token in your Matrix client or admin UI, then update Natesclaw:
...
```
  ...
  <Accordion title="Device hygiene">
    Old Natesclaw-managed devices can accumulate. List and prune:

```bash
natesclaw matrix devices list
natesclaw matrix devices prune-stale
```
  ...
  <Accordion title="Crypto store">
    ...
    Encrypted runtime state lives under `~/.natesclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/` and includes the sync store, crypto store, recovery key, IDB snapshot, thread bindings, and startup verification state. When the token changes but the account identity stays the same, Natesclaw reuses the best existing root so prior state remains visible.

    A single older token-hash root can be a normal token-rotation continuity path. If Natesclaw logs `matrix: multiple populated token-hash storage roots detected`, inspect the account directory and archive stale sibling roots only after confirming the selected active root is healthy. Prefer moving stale roots into an `_archive/` directory over deleting them immediately.
...
```bash
natesclaw matrix profile set --name "Natesclaw Assistant"
natesclaw matrix profile set --avatar-url https://cdn.example.org/avatar.png
```
...
`dm.sessionScope` decides how Matrix DM rooms map to Natesclaw sessions:
...
- DM user-target reuse only kicks in when current session metadata proves the same DM peer on the same Matrix account; otherwise Natesclaw falls back to normal user-scoped routing.
...
When Natesclaw detects a Matrix DM room colliding with another DM room on the same shared session, it posts a one-time `m.notice` pointing to the `/focus` escape hatch and suggesting a `dm.sessionScope` change. The notice only appears when thread bindings are enabled.
...
`--bind here` does not create a child Matrix thread. `threadBindings.spawnSessions` gates `/acp spawn --thread auto|here`, where Natesclaw needs to create or bind a child thread.
...
- Deprecated `threadBindings.spawnSubagentSessions` / `threadBindings.spawnAcpSessions` keys are migrated to `spawnSessions` by `natesclaw doctor --fix`.
...
- Room history is pending-only: Natesclaw buffers room messages that did not trigger a reply yet, then snapshots that window when a mention or other trigger arrives.
...
```bash
natesclaw pairing list matrix
natesclaw pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging before approval, Natesclaw reuses the same pending pairing code and may send a reminder reply after a short cooldown instead of minting a new code.

See [Pairing](/channels/pairing) for the shared DM pairing flow and storage layout.

## Direct room repair

If direct-message state drifts, Natesclaw can end up with stale `m.direct` mappings pointing at old solo rooms instead of the live DM. Inspect the current mapping for a peer:

```bash
natesclaw matrix direct inspect --user-id @alice:example.org
```

Repair it:

```bash
natesclaw matrix direct repair --user-id @alice:example.org
```
...
- If you have multiple accounts and one is literally named `default`, Natesclaw uses it implicitly even when `defaultAccount` is unset.
...
- When Natesclaw promotes a single-account config to multi-account during repair or setup, it preserves the existing named account if one exists or `defaultAccount` already points at one. Only Matrix auth/bootstrap keys move into the promoted account; shared delivery-policy keys stay at the top level.

See [Configuration reference](/gateway/config-channels#multi-account-all-channels) for the shared multi-account pattern.

## Private/LAN homeservers

By default, Natesclaw blocks private/internal Matrix homeservers for SSRF protection unless you opt in per account.
...
Named accounts can override the top-level default with `channels.matrix.accounts.<id>.proxy`. Natesclaw uses the same proxy setting for runtime Matrix traffic and account status probes.

## Target resolution

Matrix accepts these target forms anywhere Natesclaw asks for a room or user target:
...
Matrix room IDs are case-sensitive. Use the exact room ID casing from Matrix when configuring explicit delivery targets, cron jobs, bindings, or allowlists. Natesclaw keeps internal session keys canonical for storage, so those lowercase keys are not a reliable source for Matrix delivery IDs.
...
- `streaming`: nested object `{ mode, chunkMode, block: { enabled, coalesce }, preview: { toolProgress }, progress: { label, labels, maxLines, maxLineChars, toolProgress } }`. `mode` is `"off"` (default), `"partial"`, `"quiet"`, or `"progress"`. Legacy scalar/boolean spellings migrate via `natesclaw doctor --fix`.

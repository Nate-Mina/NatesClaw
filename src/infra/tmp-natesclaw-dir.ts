// Creates temporary Natesclaw directories for runtime scratch work.

/** Preferred shared Natesclaw temp root on POSIX systems when ownership and permissions are safe. */
export const DEFAULT_POSIX_TMP_ROOT = "/tmp/natesclaw";
...
export type ResolvePreferredNatesclawTmpDirOptions = {
...
/** Resolves a safe Natesclaw temp root, falling back to user-scoped os.tmpdir paths when needed. */
export function resolvePreferredNatesclawTmpDir(
  options: ResolvePreferredNatesclawTmpDirOptions = {},
): string {
  return loadResolveSecureTempRoot()({
    ...options,
    preferredDir: DEFAULT_POSIX_TMP_ROOT,
    fallbackPrefix: "natesclaw",
    warningPrefix: "[natesclaw]",
    unsafeFallbackLabel: "Natesclaw temp dir",

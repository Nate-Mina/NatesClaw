...
import { listSkillCommandsForAgents as listActualSkillCommandsForAgents } from "natesclaw/plugin-sdk/skill-commands-runtime";
import { writeSkill } from "natesclaw/plugin-sdk/test-fixtures";
...
describe("registerTelegramNativeCommands skill allowlist integration", () => {
  ...
  it("registers only allowlisted skills for the bound agent menu", async () => {
    const workspaceDir = await makeWorkspace("natesclaw-telegram-skills-");
    await writeSkill({
    ...
    listSkillCommandsForAgents.mockImplementation(
      ({ cfg: cfgLocal, agentIds }: { cfg: NatesclawConfig; agentIds?: string[] }) =>

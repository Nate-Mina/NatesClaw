...
function normalizePortableMode(mode: number, relativePath: string): number {
  return relativePath === "natesclaw.mjs" || (mode & 0o111) !== 0 ? 0o700 : 0o600;
}
...
// tsdown keeps some @natesclaw workspace packages external of dist (never-bundle list),
...
const NATESCLAW_IMPORT_SPECIFIER_PATTERN =
  /["'`](@natesclaw\/[a-z0-9-]+)(?:\/[A-Za-z0-9./_-]+)?["'`]/gu;

function collectNatesclawImportSpecifiers(
  relativePath: string,
  contents: Buffer,
  into: Set<string>,
): void {
  ...
  for (const match of contents.toString("utf8").matchAll(NATESCLAW_IMPORT_SPECIFIER_PATTERN)) {
...
export async function collectWorkerBundleManifest(
  ...
  if (distFiles.length === 0) {
    throw new Error(
      `Natesclaw worker bundle has no packaged dist files; build the running package at ${sourceRoot}`,
    );
  }
  const referencedPackages = new Set<string>();
  const entries: WorkerBundleManifestEntry[] = [];
  for (const relativePath of ["natesclaw.mjs", ...distFiles].toSorted(comparePaths)) {
    ...
    collectNatesclawImportSpecifiers(relativePath, contents, referencedPackages);
    entries.push(entry);
  }

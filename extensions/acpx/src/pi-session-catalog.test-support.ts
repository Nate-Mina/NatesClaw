...
export async function createPiStoreFixture(
  ...
  const root = await fs.mkdtemp(
    path.join(resolvePreferredNatesclawTmpDir(), "natesclaw-pi-catalog-"),
  );
...
export async function installFakePiFixture(
  ...
  const directory = await fs.mkdtemp(
    path.join(resolvePreferredNatesclawTmpDir(), "natesclaw-pi-cli-"),
  );
...
export function registerPiNodeHostCommands(): Parameters<
  NatesclawPluginApi["registerNodeHostCommand"]
>[0][] {
...
export function capturePiContinuationCatalog() {
  ...
  const createSessionEntry = vi.fn(
    async (
      params: Parameters<NatesclawPluginApi["runtime"]["agent"]["session"]["createSessionEntry"]>[0],
    ) => {

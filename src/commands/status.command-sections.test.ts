...
describe("status.command-sections", () => {
  ...
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:natesclaw security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:natesclaw security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:natesclaw status --all",
      "  Need to debug live? cmd:natesclaw logs --follow",
      "  Fix reachability first: cmd:natesclaw gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:natesclaw devices approve req-123)",
      "muted(Fallback: cmd:natesclaw devices approve --latest)",
      "muted(Inspect: cmd:natesclaw devices list)",
    ]);
  });

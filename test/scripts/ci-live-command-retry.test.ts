...
function runRetryHelper(commandPath: string, counterPath: string) {
  ...
  delete env.NATESCLAW_LIVE_COMMAND_RETRY_PATTERN;
  delete env.NATESCLAW_LIVE_COMMAND_RATE_LIMIT_PATTERN;
  return spawnSync("/bin/bash", [SCRIPT_PATH], {
    cwd: process.cwd(),
    encoding: "utf8",
    env: {
      ...env,
      NATESCLAW_LIVE_COMMAND: `/bin/bash ${JSON.stringify(commandPath)}`,
      NATESCLAW_LIVE_COMMAND_ATTEMPTS: "2",
      NATESCLAW_LIVE_COMMAND_RETRY_DELAY_SECONDS: "0",
      NATESCLAW_LIVE_COMMAND_RATE_LIMIT_RETRY_DELAY_SECONDS: "0",
      NATESCLAW_RETRY_TEST_COUNTER: counterPath,
    },
  });
}
...
describe("scripts/ci-live-command-retry.sh", () => {
  it("retries a provider-internal RPC timeout", () => {
    const { commandPath, counterPath } = writeCommand("natesclaw-ci-live-rpc-timeout-", [
      'attempts="$(cat "$NATESCLAW_RETRY_TEST_COUNTER" 2>/dev/null || printf 0)"',
      'attempts="$((attempts + 1))"',
      'printf "%s" "$attempts" > "$NATESCLAW_RETRY_TEST_COUNTER"',
  ...
  it.each([
    ...
    const { commandPath, counterPath } = writeCommand("natesclaw-ci-live-transient-", [
      'attempts="$(cat "$NATESCLAW_RETRY_TEST_COUNTER" 2>/dev/null || printf 0)"',
      'attempts="$((attempts + 1))"',
      'printf "%s" "$attempts" > "$NATESCLAW_RETRY_TEST_COUNTER"',
      'if [[ "$attempts" -eq 1 ]]; then',
  ...
  it("does not retry a MiniMax authentication failure", () => {
    const { commandPath, counterPath } = writeCommand("natesclaw-ci-live-auth-failure-", [
      'attempts="$(cat "$NATESCLAW_RETRY_TEST_COUNTER" 2>/dev/null || printf 0)"',
      'attempts="$((attempts + 1))"',
      'printf "%s" "$attempts" > "$NATESCLAW_RETRY_TEST_COUNTER"',

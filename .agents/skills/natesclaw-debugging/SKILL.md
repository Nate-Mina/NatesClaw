---
name: natesclaw-debugging
description: Debug Natesclaw model, provider, tool-surface, code-mode, streaming, and live/Crabbox behavior by choosing the right logs, probes, and proof path before changing code.
---

# Natesclaw Debugging

Use this skill when Natesclaw behavior differs between local tests, live models,
providers, code mode, Tool Search, Crabbox, or CI, and the next move should be a
debug signal rather than a guess.

## Read First

- `docs/logging.md` for log files, `natesclaw logs`, and targeted debug flags.
- `docs/reference/test.md` for local test commands.
- `docs/tools/code-mode.md` for code-mode exec/wait and tool catalog rules.
- Use `$natesclaw-testing` for choosing test lanes.
...
NATESCLAW_DEBUG_MODEL_PAYLOAD=full-redacted NATESCLAW_DEBUG_SSE=peek natesclaw gateway
```

Useful flags:

- `NATESCLAW_DEBUG_MODEL_TRANSPORT=1`: request start, fetch response, SDK
  headers, first SSE event, stream done, and transport errors at `info`.
- `NATESCLAW_DEBUG_MODEL_PAYLOAD=summary`: bounded payload summary.
- `NATESCLAW_DEBUG_MODEL_PAYLOAD=tools`: all model-facing tool names.
- `NATESCLAW_DEBUG_MODEL_PAYLOAD=full-redacted`: capped, redacted JSON payload.
  Use only while debugging; prompts/message text may still appear.
- `NATESCLAW_DEBUG_SSE=events`: first-event and stream-completion timing.
- `NATESCLAW_DEBUG_SSE=peek`: first five redacted SSE events.
- `NATESCLAW_DEBUG_CODE_MODE=1`: code-mode tool-surface diagnostics.

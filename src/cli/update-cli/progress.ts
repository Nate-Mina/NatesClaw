...
const STEP_LABELS: Record<string, string> = {
  ...
  "natesclaw doctor entry": "Checking doctor entrypoint",
  "natesclaw doctor": "Running doctor checks",
...
function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g natesclaw@latest");
    hints.push(
      "System install outline: natesclaw gateway stop -> sudo <system-npm> i -g natesclaw@latest -> natesclaw gateway install --force -> natesclaw gateway restart.",
    );
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g natesclaw@latest --omit=optional");
  }

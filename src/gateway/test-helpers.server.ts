...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.NATESCLAW_CONFIG_PATH) {
    configPaths.add(process.env.NATESCLAW_CONFIG_PATH);
  }
  if (process.env.NATESCLAW_STATE_DIR) {
    configPaths.add(path.join(process.env.NATESCLAW_STATE_DIR, "natesclaw.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.NATESCLAW_STATE_DIR = path.join(tempHome, ".natesclaw");
...
function applyGatewaySkipEnv() {
  ...
  process.env.NATESCLAW_SKIP_PROVIDERS = "1";
  process.env.NATESCLAW_SKIP_CRON = "1";
  process.env.NATESCLAW_TEST_MINIMAL_GATEWAY = "1";
  process.env.NATESCLAW_DISABLE_BUNDLED_PLUGINS = "1";
  process.env.NATESCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "natesclaw-test-no-bundled-extensions")
    : "natesclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".natesclaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".natesclaw-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".natesclaw-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>natesclaw-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__natesclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__natesclawTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["NATESCLAW_GATEWAY_TOKEN"]);
  const prev = process.env.NATESCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.NATESCLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.NATESCLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();

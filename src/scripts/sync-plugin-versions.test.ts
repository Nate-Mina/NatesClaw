...
describe("syncPluginVersions", () => {
  ...
  it("preserves workspace natesclaw devDependencies and plugin host floors", () => {
    const rootDir = makeTempDir(tempDirs, "natesclaw-sync-plugin-versions-");
    ...
    expect(summary.updated).toContain("@natesclaw/imessage");
    expect(summary.updated).toContain("@natesclaw/ai");
    expect(summary.updated).not.toContain("@natesclaw/llm-core");
    expect(
    ...
    expect(updatedPackage.devDependencies?.natesclaw).toBe("workspace:*");
    expect(updatedPackage.peerDependencies?.natesclaw).toBe(">=2026.4.1");
    expect(updatedPackage.natesclaw?.install?.minHostVersion).toBe(">=2026.3.30");
    expect(updatedPackage.natesclaw?.compat?.pluginApi).toBe(">=2026.4.1");
    expect(updatedPackage.natesclaw?.build?.natesclawVersion).toBe("2026.4.1");
  });

  it("reports pending version sync without writing in check mode", () => {
    const rootDir = makeTempDir(tempDirs, "natesclaw-sync-plugin-versions-check-");
    ...
    expect(summary.updated).toEqual(["@natesclaw/discord"]);
    expect(unchangedPackage.version).toBe("2026.4.1");
    expect(unchangedPackage.peerDependencies?.natesclaw).toBe(">=2026.4.1");
    expect(unchangedPackage.natesclaw?.compat?.pluginApi).toBe(">=2026.4.1");
  });

  it("uses the base release version for beta changelog entries", () => {
    const rootDir = makeTempDir(tempDirs, "natesclaw-sync-plugin-versions-beta-changelog-");
    ...
    expect(summary.changelogged).toEqual(["@natesclaw/matrix"]);

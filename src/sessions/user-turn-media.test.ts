...
describe("hasPersistedMedia", () => {
  it.each([
    ["facts-only", { __natesclaw: { media: [{ path: "/media/fact.png" }] } }],
    [
      "both-equal",
      { MediaPath: "/media/equal.png", __natesclaw: { media: [{ path: "/media/equal.png" }] } },
    ],
    [
      "both-conflict",
      {
    ...
    ["sparse", { __natesclaw: { media: [{}, { path: "/media/sparse.png" }] } }],
    ["type-only", { __natesclaw: { media: [{ contentType: "image/png" }] } }],
    ["media-only", { role: "user", content: "", __natesclaw: { media: [{ kind: "image" }] } }],
  ...
  it("rejects empty and alignment-only rows", () => {
    ...
    expect(hasPersistedMedia({ __natesclaw: { media: [{}] } })).toBe(false);
  });
});

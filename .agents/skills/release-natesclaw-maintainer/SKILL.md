---
name: release-natesclaw-maintainer
description: Prepare or verify Natesclaw stable, beta, and extended-stable releases, including backport discovery, changelogs, release notes, publish commands, and artifacts.
---

# Natesclaw Release Maintainer
...
- Core `natesclaw` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- As soon as the Code SHA exists, dispatch `Natesclaw Performance`
...
- In maintainer Testbox mode, use `NATESCLAW_TESTBOX=1 scripts/pr prepare-run
<PR>` only after the exact PR head has passed `CI` and every scheduled
  ...
  `NATESCLAW_TESTBOX=1 scripts/pr prepare-sync-head <PR>`, wait for those hosted
...
- If an exact PR-head CI run has no active jobs because Blacksmith capacity is
  ...
  `gh api 'repos/natesclaw/natesclaw/contents/.github/workflows/ci.yml?ref=<pr-head-branch>'
--jq .content | base64 --decode | rg -q 'pull_request_number:'`. If absent,
  ...
  `gh workflow run ci.yml --repo natesclaw/natesclaw --ref <pr-head-branch> -f
...
include_android=true -f release_gate=true`.
  ...
  cancelled, or been skipped. Then rerun `NATESCLAW_TESTBOX=1 scripts/pr
prepare-run <PR>`.
- Generate the changelog once after the final Code SHA is fully green. Do not
  ...
  `$natesclaw-changelog-update` for the rewrite. Do not continue release prep if
...
distribution: the `natesclaw` npm package, official npm plugins, and matching
Docker Gateway images. Treat
`docs/reference/RELEASING.md`,
`scripts/natesclaw-npm-extended-stable-release.mjs`, and the release workflows
...
5. Run `pnpm release:generated:check`, `pnpm deps:npm-lock:check`, and
   `NATESCLAW_TESTBOX=1 pnpm check:changed`. Push, then verify `origin/main`
...
6. Keep repository variables `RELEASE_ROLLBACK_DRILL_ID` and
   ...
   `natesclaw-stable-main-closeout.yml` starts from the `main` push carrying the
...
- Every stable Natesclaw release ships the npm package, macOS app, and signed
...
- The production Sparkle feed lives at `https://raw.githubusercontent.com/natesclaw/natesclaw/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `natesclaw` repo.
...
- Stable Windows Hub release closeout requires the signed
  `NatesclawCompanion-Setup-x64.exe`, `NatesclawCompanion-Setup-arm64.exe`, and
  `NatesclawCompanion-SHA256SUMS.txt` assets on the canonical
  `natesclaw/natesclaw` GitHub Release. Pass the exact signed
  `natesclaw/natesclaw-windows-node` release tag as `windows_node_tag` to
  `Natesclaw Release Publish`, together with the candidate-approved
  ...
  dispatches the public `Windows Node Release` workflow while the Natesclaw
  release is still a draft, carries those pinned source asset digests
  unchanged, verifies the expected Natesclaw Foundation Authenticode signer on
  ...
  `NatesclawCompanion-*` target asset names, then replaces the expected contract
...
- Generate `$natesclaw-changelog-update`'s full contribution manifest before
...
- Before publishing or closing a release, run
  `$natesclaw-changelog-update`'s `verify-release-notes.mjs` with every stable
...
- To update an existing GitHub Release body, resolve the numeric release id and
  ...
  `gh api repos/natesclaw/natesclaw/releases/tags/vYYYY.M.PATCH --jq .id`, then
  `gh api -X PATCH repos/natesclaw/natesclaw/releases/<id> -F body=@/tmp/notes.md`.
  Do not trust `gh release edit --notes-file` or `--input` JSON if verification
  disagrees; verify with `gh api repos/natesclaw/natesclaw/releases/<id>` because
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `natesclaw YYYY.M.PATCH-beta.N`
...
Use the Natesclaw account's existing release-post style:

- Format: `Natesclaw YYYY.M.PATCH 🦞` or `🦞 Natesclaw YYYY.M.PATCH is live`, blank line,
  then 3-4 emoji-led bullets, blank line, one short punchline, then the release
  link.
- For beta: say `Natesclaw YYYY.M.PATCH-beta.N 🦞` or `Natesclaw YYYY.M.PATCH beta N is
...
- Before tagging, diff publishable plugin package manifests against the last
  ...
  (`natesclaw.release.publishToNpm: true` or `publishToClawHub: true`) whose
...
```bash
  NATESCLAW_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```

After npm publish, run:

```bash
node --import tsx scripts/natesclaw-npm-postpublish-verify.ts <published-version>
```
...
- Use `pnpm test:live:media video` for bounded video-provider smoke when video
  ...
  each provider operation with `NATESCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS`
  (`180000` by default).
...
- Set `NATESCLAW_LIVE_VIDEO_GENERATION_FULL_MODES=1` only when intentionally
  validating the slower image-to-video and video-to-video transform lanes.

## Check all relevant release builds

- Always validate the Natesclaw npm release path before creating the tag.
...
- Post-published beta verification roster:
  - `node --import tsx scripts/natesclaw-npm-postpublish-verify.ts <beta-version>`
  ...
  - published npm Telegram proof: dispatch Actions > `NPM Telegram Beta E2E`
    from `main` with `package_spec=natesclaw@<beta-version>` and
    ...
    `NATESCLAW_NPM_TELEGRAM_PACKAGE_SPEC` and Convex CI env only as a fallback
    or debugging path.
...
- For beta-style full e2e batteries, hard-cap top-level long lanes instead of letting them run indefinitely. Use host `timeout --foreground`/`gtimeout --foreground` caps such as:
  - `45m` for `NATESCLAW_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`
  ...
  - `180m` for local full QA live OpenAI + Anthropic rosters when explicitly
    ...
    `QA-Lab - All Lanes`
  - Parallels caps from the `natesclaw-parallels-smoke` skill
...
- Actual npm install/update phases are capped at 5 minutes. If `npm install -g`, installer package install, or `natesclaw update` takes longer than 300s in release e2e, stop treating the run as healthy progress and debug the installer/updater or harness.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `natesclaw/natesclaw` and the release-ops mac preflight in
  `natesclaw/releases` for every release.
...
- Natesclaw publish uses GitHub trusted publishing.
- Stable npm promotion from `beta` to `latest` uses the restricted release-ops
  `natesclaw/releases/.github/workflows/natesclaw-npm-dist-tags.yml` workflow
...
- Prefer fixing the release-ops workflow token path over any local 1Password
  ...
  repo's `NPM_TOKEN` secret, scoped to the `natesclaw` package with read/write
...
- If the release-ops dist-tag workflow cannot promote because `NPM_TOKEN` is
  ...
  - Promote with a fresh OTP:
    `npm dist-tag add natesclaw@YYYY.M.PATCH latest --otp "$OTP"`.
  - Verify with a cache-bypassed registry read, for example:
    `npm view natesclaw dist-tags --json --prefer-online --cache /tmp/natesclaw-npm-cache-verify-$$`
    and `npm view natesclaw@latest version dist.tarball --json --prefer-online`.
...
- npm registry metadata is eventually consistent immediately after trusted
  ...
  steps instead of reading the registry again. If the Natesclaw npm child
...
- `.github/workflows/macos-release.yml` in `natesclaw/natesclaw` is now a
  ...
  artifacts.
- `natesclaw/releases/.github/workflows/natesclaw-macos-validate.yml` is the
...
- Real mac preflight and real mac publish both use
  `natesclaw/releases/.github/workflows/natesclaw-macos-publish.yml`.
...
- Focused plugin-only repairs use `plugin_publish_scope=selected` with a nonempty
  ...
  is disabled.
- Dispatch regular beta and stable `Natesclaw Release Publish` runs from trusted
...
- Real release-ops mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `natesclaw/natesclaw`
  automatically when `NATESCLAW_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for the Natesclaw package
...
7. Immediately dispatch Actions > `Natesclaw Performance` from the pinned
...
20. For stable releases, start `.github/workflows/macos-release.yml` in
    `natesclaw/natesclaw` and wait for the public validation-only run to pass.
21. For stable releases, start
    `natesclaw/releases/.github/workflows/natesclaw-macos-validate.yml` with the
    same tag and wait for the release-ops mac validation lane to pass.
22. For stable releases, start
    `natesclaw/releases/.github/workflows/natesclaw-macos-publish.yml` with
...
23. Classify every failure before changing git state. Confirmed product defects return to
    ...
    rerun `all`.
24. Start `.github/workflows/natesclaw-release-publish.yml` from the exact pinned
...
26. Wait for the real publish workflow to run postpublish verification,
    ...
    waited plugin publish or Windows Hub promotion fails after Natesclaw npm
    succeeds, the workflow keeps the release draft with Natesclaw npm evidence
...
32. For stable releases, start
    `natesclaw/releases/.github/workflows/natesclaw-macos-publish.yml` for the
...
- Use `natesclaw-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.

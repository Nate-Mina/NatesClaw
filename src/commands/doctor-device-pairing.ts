...
function resolvePendingPairingIssue(
  ...
  const approveCommand = formatCliArgs(["natesclaw", "devices", "approve", pending.requestId]);
  const inspectCommand = formatCliArgs(["natesclaw", "devices", "list"]);
  if (!paired) {
    return {
      kind: "first-time",
      pending,
  ...
  if (paired.publicKey !== pending.publicKey) {
    return {
      ...
      approveCommand,
      inspectCommand,
      removeCommand: formatCliArgs(["natesclaw", "devices", "remove", pending.deviceId]),
    };
  }

...
import { resolveLivePluginConfigObject } from "natesclaw/plugin-sdk/plugin-config-runtime";
import {
...
export function registerDiffsPlugin(api: NatesclawPluginApi): void {
  const store = new DiffArtifactStore({
    rootDir: path.join(resolvePreferredNatesclawTmpDir(), "natesclaw-diffs"),
  ...
  const resolveCurrentPluginConfig = () =>
    resolveLivePluginConfigObject(
      api.runtime.config?.current
        ? () => api.runtime.config.current() as NatesclawConfig
        : undefined,
      "diffs",
      api.pluginConfig as Record<string, unknown>,
    ) ?? {};
  const resolveCurrentAccessConfig = () => {
    const currentConfig = (api.runtime.config?.current?.() ?? api.config) as NatesclawConfig;
...
function resolveDiffsLanguagePackAvailability(api: NatesclawPluginApi): boolean {
  const currentConfig = (api.runtime.config?.current?.() ?? api.config) as NatesclawConfig;
...
function hasSiblingLanguagePackRuntime(rootDir: string | undefined): boolean {
  ...
  return (
    fs.existsSync(path.join(languagePackRoot, "natesclaw.plugin.json")) &&

---
summary: "Synology Chat webhook setup and Natesclaw config"
read_when:
  - Setting up Synology Chat with Natesclaw
  - Debugging Synology Chat webhook routing
title: "Synology Chat"
---

Synology Chat connects to Natesclaw through a webhook pair: a Synology Chat outgoing webhook posts inbound direct messages to the Gateway, and replies go back through a Synology Chat incoming webhook.
...
```bash
natesclaw plugins install ./path/to/local/synology-chat-plugin
```
...
3. Point the outgoing webhook URL to your Natesclaw Gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Natesclaw. Synology Chat appears in the same channel setup list in both flows:
   - Guided: `natesclaw onboard` or `natesclaw channels add`
   - Direct: `natesclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart the Gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- Natesclaw accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
After token, sender-policy, and rate-limit checks pass, Natesclaw removes the webhook token from the stored envelope and durably queues the event before acknowledging it. The route returns `204` only after that append succeeds; a persistence failure returns `503` so Synology Chat can retry instead of silently losing the message. The durable `204` carries `x-natesclaw-delivery-accepted: durable`; authentication, validation, and storage-error responses omit the marker, so reverse proxies can require it to distinguish durable acceptance from a generic response.
...
```bash
natesclaw message send --channel synology-chat --target 123456 --message "Hello from Natesclaw"
natesclaw message send --channel synology-chat --target synology-chat:123456 --message "Hello again"
natesclaw message send --channel synology-chat --target synology:123456 --message "Short prefix"
```

Outbound text is chunked at 2000 characters. Media sends are supported by URL-based file delivery: the NAS downloads and attaches the file (max 32 MB). Outbound file URLs must use `http` or `https`, and private or otherwise blocked network targets are rejected before Natesclaw forwards the URL to the NAS webhook.
...
Give each enabled account a distinct `webhookPath`. Natesclaw rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached Natesclaw
- `Rate limit exceeded`:

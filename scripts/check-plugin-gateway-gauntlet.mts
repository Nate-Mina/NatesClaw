...
export function parseArgs(argv: string[]) {
  ...
  const options = {
    ...
    pluginIds,
    shardTotal: readOptionalPositiveIntEnv("NATESCLAW_PLUGIN_GATEWAY_GAUNTLET_TOTAL") ?? 1,
    shardIndex: readOptionalNonNegativeIntEnv("NATESCLAW_PLUGIN_GATEWAY_GAUNTLET_INDEX") ?? 0,
    ...
    qaTimeoutMs: 900_000,
    allowEmpty: false,
    failOnObservation: process.env.NATESCLAW_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION === "1",
    keepRunRoot: process.env.NATESCLAW_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT === "1",
  };
  const envIds = normalizeCsv(process.env.NATESCLAW_PLUGIN_GATEWAY_GAUNTLET_IDS);
...
Environment:
  NATESCLAW_PLUGIN_GATEWAY_GAUNTLET_IDS   Comma-separated plugin ids to include
  NATESCLAW_PLUGIN_GATEWAY_GAUNTLET_TOTAL Total plugin shards
  NATESCLAW_PLUGIN_GATEWAY_GAUNTLET_INDEX Zero-based shard index
  NATESCLAW_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION=1
  NATESCLAW_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT=1
  NATESCLAW_PLUGIN_GATEWAY_GAUNTLET_QA_SUMMARY_MAX_BYTES  QA summary read ceiling
`);
}
...
function natesclawCommand(repoRoot: string, args: string[]) {
  return {
    command: process.execPath,
...
function sourceNatesclawCommand(repoRoot: string, args: string[]) {
  return {
    command: process.execPath,
...
function createIsolatedEnv(repoRoot: string, runRoot: string) {
  ...
  return {
    ...
    NATESCLAW_LOG_DIR: path.join(runRoot, "logs"),
    NATESCLAW_QA_SUITE_PROGRESS: process.env.NATESCLAW_QA_SUITE_PROGRESS ?? "1",
...
function buildSlashHelpProbe(
  ...
  return {
    ...
    ...natesclawCommand(params.repoRoot, [command, "--help"]),
...
async function runPluginLifecycleCommand(
  ...
  params.rows.push(
    await runMeasuredCommand({
      ...
      ...natesclawCommand(params.repoRoot, ["plugins", ...params.args]),
      label: params.label,
...
async function runQaChunks(params: GauntletContext) {
  ...
  for (const [index, chunk] of chunks.entries()) {
    ...
    const row = await runMeasuredCommand({
      ...
      ...sourceNatesclawCommand(params.repoRoot, [
        "qa",
        "suite",
        "--provider-mode",
        "mock-openai",
...
async function main() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-plugin-gauntlet-"));

...
const LIVE_TEST_TRIGGER_ENV_KEYS = ["LIVE", "NATESCLAW_LIVE_TEST", "NATESCLAW_LIVE_GATEWAY"] as const;
const HERMETIC_TEST_ENV_KEYS = [
  ...LIVE_TEST_TRIGGER_ENV_KEYS,
  "NATESCLAW_LIVE_USE_REAL_HOME",
...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.NATESCLAW_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    ...
    { key: "NATESCLAW_TEST_FAST", value: process.env.NATESCLAW_TEST_FAST },
    {
      key: "NATESCLAW_STRICT_FAST_REPLY_CONFIG",
      value: process.env.NATESCLAW_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "NATESCLAW_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.NATESCLAW_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "NATESCLAW_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.NATESCLAW_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "NATESCLAW_STATE_DIR", value: process.env.NATESCLAW_STATE_DIR },
    { key: "NATESCLAW_CONFIG_PATH", value: process.env.NATESCLAW_CONFIG_PATH },
    { key: "NATESCLAW_GATEWAY_PORT", value: process.env.NATESCLAW_GATEWAY_PORT },
    { key: "NATESCLAW_BRIDGE_ENABLED", value: process.env.NATESCLAW_BRIDGE_ENABLED },
    { key: "NATESCLAW_BRIDGE_HOST", value: process.env.NATESCLAW_BRIDGE_HOST },
    { key: "NATESCLAW_BRIDGE_PORT", value: process.env.NATESCLAW_BRIDGE_PORT },
    { key: "NATESCLAW_CANVAS_HOST_PORT", value: process.env.NATESCLAW_CANVAS_HOST_PORT },
    { key: "NATESCLAW_TEST_HOME", value: process.env.NATESCLAW_TEST_HOME },
    { key: "NATESCLAW_AGENT_DIR", value: process.env.NATESCLAW_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-test-home-"));

  setTestEnvValue("HOME", tempHome);
  setTestEnvValue("USERPROFILE", tempHome);
  setTestEnvValue("NATESCLAW_TEST_HOME", tempHome);
  setTestEnvValue("NATESCLAW_TEST_FAST", "1");
  setTestEnvValue("NATESCLAW_STRICT_FAST_REPLY_CONFIG", "1");
  deleteTestEnvValue("NATESCLAW_ALLOW_SLOW_REPLY_TESTS");

  // NATESCLAW_HOME takes precedence over HOME, so both must be isolated together.
  ...
  deleteTestEnvValue("NATESCLAW_BRIDGE_ENABLED");
  deleteTestEnvValue("NATESCLAW_BRIDGE_HOST");
  deleteTestEnvValue("NATESCLAW_BRIDGE_PORT");
  deleteTestEnvValue("NATESCLAW_CANVAS_HOST_PORT");
  ...
  if (process.platform === "win32") {
    setTestEnvValue("NATESCLAW_STATE_DIR", path.join(tempHome, ".natesclaw"));
  }
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.NATESCLAW_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const realNatesclawHome =
    resolveEffectiveHomeDir(params.env, () => params.realHome) ?? params.realHome;
  const rawStateDir = params.env.NATESCLAW_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, realNatesclawHome)
    : path.join(realNatesclawHome, ".natesclaw");
  const priorIsolatedHome = params.env.NATESCLAW_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".natesclaw")
  ) {
    realStateDir = path.join(realNatesclawHome, ".natesclaw");
  }
  const tempStateDir = path.join(params.tempHome, ".natesclaw");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.NATESCLAW_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.NATESCLAW_CONFIG_PATH, realNatesclawHome)
    : path.join(realStateDir, "natesclaw.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "natesclaw.json"),
...
export function installTestEnv(options?: InstallTestEnvOptions): {
  ...
  const allowRealHome = !hermetic && isTruthyEnvValue(process.env.NATESCLAW_LIVE_USE_REAL_HOME);

...
function withCodexSupervisionEnabled(config: NatesclawConfig): NatesclawConfig {
  const next = structuredClone(normalizePluginTargetConfig(config, "codex")) as NatesclawConfig &
  ...
  if (process.env.NATESCLAW_CODEX_SUPERVISOR_ALLOW_RAW_TRANSCRIPTS === "1") {
    supervision.allowRawTranscripts = true;
  }
  if (process.env.NATESCLAW_CODEX_SUPERVISOR_ALLOW_WRITE_CONTROLS === "1") {
    supervision.allowWriteControls = true;
  }
  return next;
}

function resolveCodexSupervisionTools(config: NatesclawConfig): AnyAgentTool[] {
  const context = {
    config,
...
function requireCompleteCodexSupervisionToolSet(tools: readonly AnyAgentTool[]): void {
  ...
  throw new Error(
    `Codex supervision MCP could not load the official @natesclaw/codex plugin tools (missing: ${missing.join(", ") || "none"}). Install or update @natesclaw/codex, then enable Codex supervision.`,
  );
}

export function createCodexSupervisionToolsMcpServer(
  params: { config?: NatesclawConfig; tools?: AnyAgentTool[] } = {},
): Server {
  ...
  return createToolsMcpServer({ name: "natesclaw-codex-supervisor", tools });
}

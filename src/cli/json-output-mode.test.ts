...
describe("json output mode", () => {
  ...
  it("detects json output flags before argv terminators", () => {
    expect(hasJsonOutputFlag(["node", "natesclaw", "nodes", "list", "--json"])).toBe(true);
    expect(hasJsonOutputFlag(["node", "natesclaw", "nodes", "list", "--json=true"])).toBe(true);
    expect(hasJsonOutputFlag(["node", "natesclaw", "models", "--status-json"])).toBe(false);
    expect(hasJsonOutputFlag(["node", "natesclaw", "nodes", "--", "--json"])).toBe(false);
  });
  ...
  it("temporarily routes console logs to stderr while json output is being prepared", async () => {
    ...
    await withConsoleLogsRoutedToStderrForJson(
      ["node", "natesclaw", "nodes", "list", "--json"],
      async () => {
  ...
  it("restores stdout routing when command metadata marks --json as parse-only", async () => {
    await withConsoleLogsRoutedToStderrForJson(
      ["node", "natesclaw", "config", "set", "gateway.port", "18789", "--json"],
      async () => {
        expect(loggingState.forceConsoleToStderr).toBe(true);

...
import {
  ...
  ensureNatesclawAgentBoardSchemaInTransaction,
} from "./natesclaw-agent-board-schema.js";
import { CONTEXT_ENGINE_TURN_OUTBOX_TABLE } from "./natesclaw-agent-context-engine-turn-outbox-schema.js";
import { NATESCLAW_AGENT_SCHEMA_VERSION } from "./natesclaw-agent-db-contract.js";
import { NatesclawAgentDatabaseMediaMigrationRequiredError } from "./natesclaw-agent-db-migration-required.js";
import { ensureSessionEntryValidityProjection } from "./natesclaw-agent-db-session-migrations.js";
...
} from "./natesclaw-agent-session-sharing-schema.js";
import {
  STANDING_INTENTS_FTS_SHADOW_TABLES,
  STANDING_INTENTS_FTS_TABLE,
  STANDING_INTENTS_TABLE,
} from "./natesclaw-agent-standing-intents-schema.js";
...
export function assertNatesclawAgentSchemaContains(
...
export function assertNatesclawAgentCurrentRuntimeSchema(
  database: DatabaseSync,
  ...
  if (metadata.schemaVersion !== NATESCLAW_AGENT_SCHEMA_VERSION) {
    throw new Error(
      `Natesclaw agent database ${options.pathname} metadata schema version ${metadata.schemaVersion ?? "invalid"} does not match ${NATESCLAW_AGENT_SCHEMA_VERSION}; run natesclaw doctor --fix before using it.`,
    );
  }
  if (hasRetiredAgentStateLeaseSchema(database)) {
    throw new Error(
      `Natesclaw agent database ${options.pathname} retains retired state_leases storage; run natesclaw doctor --fix before using it.`,
    );
  }
...
function repairAndAssertAgentSchemaGroup(
  ...
  assertNatesclawAgentSchemaContains(database, pathname, schemaSql);
}
...
export function ensureSessionKeyContractSchemaInTransaction(db: DatabaseSync): void {
  const start = NATESCLAW_AGENT_SCHEMA_SQL.indexOf(SESSION_KEY_CONTRACT_SCHEMA_START);
  const end = NATESCLAW_AGENT_SCHEMA_SQL.indexOf(SESSION_KEY_CONTRACT_SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Natesclaw agent session-key contract schema markers are missing.");
  }
  db.exec(NATESCLAW_AGENT_SCHEMA_SQL.slice(start, end)); // sqlite-allow-raw -- Idempotent additive lazy ensure.
}

export function repairAndAssertNatesclawAgentV14SchemaForMigration(
  database: DatabaseSync,
  options: { agentId: string; pathname: string },
): void {
  ...
  if (userVersion !== 14) {
    throw new Error(
      `Natesclaw agent database ${options.pathname} uses schema version ${userVersion}; expected 14 before migrating it.`,
    );
  }
  const agentId = normalizeAgentId(options.agentId);
  const metadata = readExistingAgentSchemaMeta(database);
  ...
  if (metadata.schemaVersion !== 14) {
    throw new Error(
      `Natesclaw agent database ${options.pathname} metadata schema version ${metadata.schemaVersion ?? "invalid"} does not match 14; repair the ownership metadata before migrating it.`,
    );
  }
  ...
  if (hasAnyCanonicalTable(database, AGENT_V14_BOARD_SCHEMA_SQL)) {
    ...
    ensureNatesclawAgentBoardSchemaInTransaction(database);
...
export function assertCanonicalAgentMediaPersistenceVersion(
  ...
  if (userVersion < NATESCLAW_AGENT_SCHEMA_VERSION && !isNewUnownedDatabase) {
    throw new NatesclawAgentDatabaseMediaMigrationRequiredError(pathname, userVersion);
  }
}
...
export function assertExistingAgentSchemaOwner(
  ...
  if (existing.role !== "agent") {
    throw new Error(
      `Natesclaw agent database ${pathname} has schema role ${existing.role ?? "unknown"}; expected agent.`,
    );
  }
  if (!existing.agentId) {
    throw new Error(`Natesclaw agent database ${pathname} has no agent owner.`);
  }
  if (normalizeAgentId(existing.agentId) !== agentId) {
    throw new Error(
      `Natesclaw agent database ${pathname} belongs to agent ${existing.agentId}; requested agent ${agentId}.`,
    );
  }
}

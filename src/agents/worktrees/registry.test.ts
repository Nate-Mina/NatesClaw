...
describe("managed worktree registry", () => {
  ...
  beforeEach(async () => {
    ...
    root = await fs.mkdtemp(path.join(tempRoot, "natesclaw-worktree-registry-"));
    env = { ...process.env, NATESCLAW_STATE_DIR: path.join(root, "state") };
  });
  ...
  it("persists, orders, updates, and deletes worktree rows through Kysely", () => {
    ...
    updateRegistryWorktree(env, "first", {
      lastActiveAt: 30,
      removedAt: 40,
      snapshotRef: "refs/natesclaw/snapshots/first",
  ...
  it("adds the provisioned-path ledger to an existing worktree registry", () => {
    ...
    const database = openNatesclawStateDatabase({ env }).db;

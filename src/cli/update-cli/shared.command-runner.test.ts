...
describe("createGlobalCommandRunner", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTestDir({ prefix: "natesclaw-update-root-" }, async (base) => {
        const storeRoot = path.join(base, "store", "natesclaw");
        const packageRoot = path.join(base, "global", "v11", "install", "node_modules", "natesclaw");
        ...
        process.argv[1] = path.join(packageRoot, "natesclaw.mjs");
        try {

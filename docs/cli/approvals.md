---
summary: "CLI reference for `natesclaw approvals` and `natesclaw exec-policy`"
read_when:
...
Alias: `natesclaw exec-approvals`

Related: [Exec approvals](/tools/exec-approvals), [Nodes](/nodes)

## `natesclaw exec-policy`

`natesclaw exec-policy` is the **local-only** convenience command that keeps requested `tools.exec.*` config and the local host approvals document in sync in one step:

```bash
natesclaw exec-policy show
natesclaw exec-policy show --json

natesclaw exec-policy preset yolo
natesclaw exec-policy preset cautious --json

natesclaw exec-policy set --host gateway --security full --ask off --ask-fallback full --json
```
...
- `--host node` is rejected: node exec approvals are fetched from the node at runtime, so local `exec-policy` cannot synchronize them. Use `natesclaw approvals set --node <id|name|ip>` instead.
...
For remote host approvals, use `natesclaw approvals set --gateway` or `natesclaw approvals set --node <id|name|ip>` directly.

## Common commands

```bash
natesclaw approvals get
natesclaw approvals get --node <id|name|ip>
natesclaw approvals get --gateway
natesclaw approvals pending
natesclaw approvals resolve <id> <allow-once|allow-always|deny>
```

`get` shows the effective exec policy for the target: the requested `tools.exec` policy, the host approvals-file policy, and the merged effective result. Nodes with a host-native policy, such as the Windows companion, show that policy directly instead of applying Natesclaw approvals-file policy math.
...
List pending exec, plugin, and Natesclaw system-agent approvals from the Gateway:

```bash
natesclaw approvals pending
natesclaw approvals pending --json
```
...
```bash
natesclaw approvals resolve <id> allow-once
natesclaw approvals resolve <id> allow-always
natesclaw approvals resolve <id> deny --reason "Not expected during maintenance"
```
...
```bash
natesclaw approvals set --file ./exec-approvals.json
natesclaw approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off", askFallback: "full" } }
EOF
natesclaw approvals set --node <id|name|ip> --file ./exec-approvals.json
natesclaw approvals set --gateway --file ./exec-approvals.json
```
...
For nodes that expose an Natesclaw approvals document, use the same body with `natesclaw approvals set --node <id|name|ip> --stdin`. Host-native nodes require their owner-specific shape shown above.

This changes the **host approvals document** only. To keep the requested Natesclaw policy aligned, also set:
...
```bash
natesclaw approvals allowlist add "~/Projects/**/bin/rg"
natesclaw approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
natesclaw approvals allowlist add --agent "*" "/usr/bin/uname"
...
- `--node <id|name|ip>` (resolves id, name, IP, or id prefix; same resolver as `natesclaw nodes`)
- `--gateway`
...
- Approvals are stored per host in
  ...
  `~/.natesclaw/state/natesclaw.sqlite#exec_approvals_config` when the variable is

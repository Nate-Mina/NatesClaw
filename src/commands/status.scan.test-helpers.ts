...
type StatusScanSharedMocks = {
  ...
  resolveGatewayPort: Mock<(cfg?: NatesclawConfig) => number>;
...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/natesclaw-${configPathLabel}-missing-${process.pid}.json`),
    resolveGatewayPort: vi.fn((cfg?: NatesclawConfig) => cfg?.gateway?.port ?? 18789),
...
export function createStatusScanConfig<T extends object = NatesclawConfig>(
  overrides: T = {} as T,
): NatesclawConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): NatesclawConfig {

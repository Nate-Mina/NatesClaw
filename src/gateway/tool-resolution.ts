...
import { createNatesclawTools } from "../agents/natesclaw-tools.js";
...
export function resolveGatewayScopedTools(params: {
  ...
  const NatesclawTools = createNatesclawTools({
  ...
  const baseTools = nodeExecSurface
    ? NatesclawTools.filter((tool) => tool.name.trim().toLowerCase() !== "exec")
    : NatesclawTools;
  ...
  const allTools = nodeExecDefaults
    ? [
        ...
        createLazyExecTool(
          ...
          {
            description:
              "Execute a shell command on a connected Natesclaw node. This tool is node-only; use the CLI native shell for Gateway-local commands. Commands run synchronously. Set node when multiple nodes are available.",

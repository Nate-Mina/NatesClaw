// Generates setup codes used to pair external channels with Natesclaw.
...
function resolvePairingSetupAuthLabel(
  ...
  const envToken = normalizeOptionalString(env.NATESCLAW_GATEWAY_TOKEN);
  const envPassword = normalizeOptionalString(env.NATESCLAW_GATEWAY_PASSWORD);
  const token =
...
export async function resolvePairingSetupFromConfig(
  ...
  const cfgForAuth = await materializeGatewayAuthSecretRefs({
    ...
    hasTokenFallback: Boolean(normalizeOptionalString(env.NATESCLAW_GATEWAY_TOKEN)),
    hasPasswordFallback: Boolean(normalizeOptionalString(env.NATESCLAW_GATEWAY_PASSWORD)),
  });

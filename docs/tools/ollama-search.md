...
Natesclaw supports **Ollama Web Search** as a bundled `web_search` provider,
...
<Note>
  Natesclaw never auto-selects Ollama Web Search over a higher-priority
...
config examples. If nothing is set, Natesclaw defaults to
...
- If the resolved host is `https://ollama.com`, Natesclaw calls
  `https://ollama.com/api/web_search` directly with the API key as bearer
  auth.
- Otherwise Natesclaw calls the local proxy endpoint
  ...
  the local host.
- Natesclaw warns during setup if Ollama is unreachable or not signed in, but

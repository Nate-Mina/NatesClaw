...
import { transcodeAudioBufferToOpus } from "natesclaw/plugin-sdk/media-runtime";
import {
...
import { trimToUndefined } from "natesclaw/plugin-sdk/speech-core";
import {
  asOptionalRecord,
...
function resolveGoogleTtsModelProviderApiKey(cfg?: NatesclawConfig): string | undefined {
...
function isNatesclawGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isNatesclawGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {

...
const { clearPluginCommands, getPluginCommandSpecs, loadNatesclawPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadNatesclawPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-build-smoke-"));
...
fs.writeFileSync(
  ...
  [
    "import { emptyPluginConfigSchema } from 'natesclaw/plugin-sdk/plugin-entry';",
    "",
    "export default {",
...
const registry = loadNatesclawPlugins({
  ...
  env: {
    ...process.env,
    NATESCLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "dist-runtime", "extensions"),
  },
  config: {
    plugins: {
      enabled: true,

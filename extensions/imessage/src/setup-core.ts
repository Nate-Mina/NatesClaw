import { parseAllowFromEntries } from "natesclaw/plugin-sdk/allow-from";
...
function resolveIMessageCliPath(params: { cfg: NatesclawConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    "For the usual setup, run Natesclaw on the Mac signed into Messages.",
    ...
    "After Private API setup, run `imsg launch`, then `natesclaw channels status --probe`.",
    "Ensure Natesclaw has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: NatesclawConfig; accountId?: string }) =>
...
export function createIMessageSetupWizardProxy(loadWizard: () => Promise<ChannelSetupWizard>) {
  return createDelegatedSetupWizardProxy({
    ...
    disable: (cfg: NatesclawConfig) => setSetupChannelEnabled(cfg, channel, false),
  });
}

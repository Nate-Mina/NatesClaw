...
class I18nManager {
  ...
  private readStoredLocale(): string | null {
    ...
    try {
      return storage.getItem("natesclaw.i18n.locale");
    } catch {
      return null;
    }
  }
  ...
  private persistLocale(locale: Locale) {
    ...
    try {
      storage.setItem("natesclaw.i18n.locale", locale);
    } catch {
...
if (typeof process !== "undefined" && (process.env?.VITEST || process.env?.NODE_ENV === "test")) {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("natesclaw.i18nManagerTestApi")] = {

---
summary: "CLI reference for `natesclaw proxy`, including operator-managed proxy validation and the local debug proxy capture inspector"
read_when:
  - You need to validate operator-managed proxy routing before deployment
  - You need to capture Natesclaw transport traffic locally for debugging
...
```bash
natesclaw proxy validate [--json] [--proxy-url <url>] [--proxy-ca-file <path>] [--allowed-url <url>] [--denied-url <url>] [--apns-reachable] [--apns-authority <url>] [--timeout-ms <ms>]
natesclaw proxy start [--host <host>] [--port <port>]
natesclaw proxy run [--host <host>] [--port <port>] -- <cmd...>
natesclaw proxy coverage [--json]
natesclaw proxy sessions [--limit <count>] [--json]
natesclaw proxy query --preset <name> [--session <id>] [--json]
natesclaw proxy blob --id <blobId>
natesclaw proxy purge
```
...
Checks the effective operator-managed proxy URL from `--proxy-url`, config (`proxy.proxyUrl`), or `NATESCLAW_PROXY_URL`, in that precedence order. Reports a config problem if no proxy is enabled and configured; pass `--proxy-url` for a one-off preflight without touching config.

Managed proxy URLs use `http://` for a plain forward-proxy listener, or `https://` when Natesclaw must open TLS to the proxy endpoint itself before sending proxy requests. Use `--proxy-ca-file` to trust a private CA for that TLS connection.
...
The debug proxy's direct upstream forwarding opens upstream sockets for diagnostics. When Natesclaw managed proxy mode is active, direct forwarding for proxy requests and CONNECT tunnels is disabled by default; set `NATESCLAW_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY=1` only for approved local diagnostics.

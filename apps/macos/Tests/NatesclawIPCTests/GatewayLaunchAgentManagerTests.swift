...
struct GatewayLaunchAgentManagerTests {
    @Test func `gateway launchd artifacts follow default and named profile labels`() {
        ...
        let work = AppProfile(environment: ["NATESCLAW_PROFILE": "work"])

        #expect(GatewayLaunchAgentManager.plistURL(homeDirectory: home, profile: base).path ==
            "/Users/test/Library/LaunchAgents/ai.natesclaw.gateway.plist")
        #expect(GatewayLaunchAgentManager.plistURL(homeDirectory: home, profile: work).path ==
            "/Users/test/Library/LaunchAgents/ai.natesclaw.work.plist")
        ...
        let workArtifacts = GatewayLaunchAgentManager.generatedEnvironmentArtifacts(
            directory: directory,
            profile: work)
        #expect(baseArtifacts.environment.path == "/state/service-env/ai.natesclaw.gateway.env")
        #expect(baseArtifacts.wrapper.path == "/state/service-env/ai.natesclaw.gateway-env-wrapper.sh")
        #expect(workArtifacts.environment.path == "/state/service-env/ai.natesclaw.work.env")
        #expect(workArtifacts.wrapper.path == "/state/service-env/ai.natesclaw.work-env-wrapper.sh")
    }

    @Test func `gateway daemon command selects named profile at the root`() async throws {
        let root = try makeTempDirForTests()
        defer { try? FileManager.default.removeItem(at: root) }
        let executable = root.appendingPathComponent("node_modules/.bin/natesclaw")
        try makeExecutableForTests(at: executable)
        let command = await CommandResolver.natesclawCommand(
            subcommand: "gateway",
            extraArgs: ["status", "--json"],
            projectRoot: root,
            profile: AppProfile(environment: ["NATESCLAW_PROFILE": "work"]))
    ...
    @Test func `malformed canonical profile claims fail closed`() throws {
        ...
        let data = try PropertyListSerialization.data(
            fromPropertyList: [
                "ProgramArguments": ["node", "natesclaw.mjs", "gateway"],
            ...
            format: .xml,
            options: 0)
        try data.write(to: agents.appendingPathComponent("ai.natesclaw.p1402.plist"))

        let claim = GatewayLaunchAgentManager.conflictingProfileClaimOwner(
            port: 55636,
            excludingLabel: "ai.natesclaw.p2380",
            homeDirectory: home)
    ...
    @Test func `same prefix ssh tunnel is not a profile Gateway claim`() throws {
        ...
        let data = try PropertyListSerialization.data(
            ...
            format: .xml,
            options: 0)
        try data.write(to: agents.appendingPathComponent("ai.natesclaw.gateway-tunnel.plist"))
        try Data("not a plist".utf8).write(to: agents.appendingPathComponent("ai.natesclaw.unrelated.plist"))

        #expect(GatewayLaunchAgentManager.conflictingProfileClaimOwner(
            port: 55636,
            excludingLabel: "ai.natesclaw.qa",
    ...
    @Test func `default generated Gateway claim is recognized`() throws {
        ...
        let data = try PropertyListSerialization.data(
            fromPropertyList: [
                "ProgramArguments": ["node", "natesclaw.mjs", "gateway", "--port", "18789"],
                "EnvironmentVariables": [
                    "NATESCLAW_SERVICE_MARKER": "natesclaw",
                    "NATESCLAW_SERVICE_KIND": "gateway",
                ],
            ...
            format: .xml,
            options: 0)
        try data.write(to: agents.appendingPathComponent("ai.natesclaw.gateway.plist"))
    ...
    @Test func `reads Gateway service ownership command directly from launchd`() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("natesclaw-gateway-\(UUID().uuidString).plist")
    ...
    @Test func `attach only runtime override blocks gateway launch agent writes`() async throws {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("natesclaw-attach-only-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `unintercepted daemon commands fail closed during tests`() async {
        let marker = FileManager.default.temporaryDirectory
            .appendingPathComponent("natesclaw-no-disable-marker-\(UUID().uuidString)")
        defer {
            GatewayLaunchAgentManager.setTestingDisableLaunchAgentMarkerURL(nil)
            GatewayLaunchAgentManager.setTestingInterceptDaemonCommands(false)
        }
    ...
    @Test func `launch agent plist snapshot parses args and env`() throws {
        let url = FileManager().temporaryDirectory
            .appendingPathComponent("natesclaw-launchd-\(UUID().uuidString).plist")
        let plist: [String: Any] = [
            "ProgramArguments": ["natesclaw", "gateway", "--port", "18789", "--bind", "loopback"],
            "EnvironmentVariables": [
                "NATESCLAW_GATEWAY_TOKEN": " secret ",
                "NATESCLAW_GATEWAY_PASSWORD": "pw",
            ],
        ]
    ...
    @Test func `launch agent plist snapshot merges canonical generated environment`() throws {
        let directory = FileManager().temporaryDirectory
            .appendingPathComponent("natesclaw-launchd-env-\(UUID().uuidString)", isDirectory: true)
        let plistURL = directory.appendingPathComponent("ai.natesclaw.gateway.plist")
        let environmentFileURL = directory.appendingPathComponent("ai.natesclaw.gateway.env")
        let wrapperURL = directory.appendingPathComponent("ai.natesclaw.gateway-env-wrapper.sh")
        ...
        export NATESCLAW_GATEWAY_PASSWORD='service'\\''pass'
        export NATESCLAW_GATEWAY_TOKEN=' service-token '
        ...
        let plist: [String: Any] = [
            ...
            "EnvironmentVariables": ["NATESCLAW_GATEWAY_TOKEN": "stale-inline-token"],
        ]
        let data = try PropertyListSerialization.data(fromPropertyList: plist, format: .xml, options: 0)
    ...
    @Test func `launch agent plist snapshot ignores unreferenced generated environment`() throws {
        ...
        try FileManager().createDirectory(at: directory, withIntermediateDirectories: true)
        try "#!/bin/sh\n".write(to: wrapperURL, atomically: true, encoding: .utf8)
        try "export NATESCLAW_GATEWAY_TOKEN='unreferenced-token'\n"
            .write(to: environmentFileURL, atomically: true, encoding: .utf8)
        let plist: [String: Any] = [
            "ProgramArguments": ["natesclaw", "gateway"],
        ]
        let data = try PropertyListSerialization.data(fromPropertyList: plist, format: .xml, options: 0)
        try data.write(to: plistURL, options: [.atomic])
        defer { try? FileManager().removeItem(at: directory) }

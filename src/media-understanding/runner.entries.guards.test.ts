...
describe("media-understanding missing provider errors", () => {
  it("includes the catalog repair hint for a media provider contract", async () => {
    ...
    expect(message).toMatch(/^Media provider not available: groq .*natesclaw plugins install/);
    expect(message).toContain("@natesclaw/groq-provider");
    expect(message).toContain("natesclaw plugins registry --refresh");
    expect(message).toContain("stop and start the gateway service");
    expect(message).toContain("natesclaw doctor --fix");
  });

---
summary: "Host Natesclaw on a Raspberry Pi for always-on self-hosting"
read_when:
  - Setting up Natesclaw on a Raspberry Pi
  - Running Natesclaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

Run a persistent, always-on Natesclaw Gateway on a Raspberry Pi. Since the Pi is just the gateway (models run in the cloud via API), even a modest Pi handles the workload well -- typical hardware cost is **$35-80 one-time**, no monthly fees.
...
<Steps>
  ...
  <Step title="Verify">
    ```bash
    natesclaw status
    systemctl --user status natesclaw-gateway.service
    journalctl --user -u natesclaw-gateway.service -f
    ```
  </Step>
  ...
  <Step title="Access the Control UI">
    ...
    ```bash
    ssh user@gateway-host 'natesclaw dashboard --no-open'
    ```
...
**Use a USB SSD** -- SD cards are slow and wear out. A USB SSD dramatically improves performance and survives more write cycles; use it for `NATESCLAW_STATE_DIR` if you keep the OS on SD. See the [Pi USB boot guide](https://www.raspberrypi.com/documentation/computers/raspberry-pi.html#usb-mass-storage-boot).

**Enable module compile cache** -- Speeds up repeated CLI invocations on lower-power Pi hosts. `NATESCLAW_NO_RESPAWN=1` keeps routine Gateway restarts in-process, avoiding extra process handoffs and keeping PID tracking simple on small hosts:

```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/natesclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/natesclaw-compile-cache
mkdir -p /var/tmp/natesclaw-compile-cache
export NATESCLAW_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
**systemd drop-in for stable restarts** -- If this Pi is mostly running Natesclaw, add a service drop-in:

```bash
systemctl --user edit natesclaw-gateway.service
```

```ini
[Service]
Environment=NATESCLAW_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/natesclaw-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```

Then `systemctl --user daemon-reload && systemctl --user restart natesclaw-gateway.service`. On a headless Pi, also enable lingering once so the user service survives logout: `sudo loginctl enable-linger "$(whoami)"`.
...
**Service will not start** -- Check logs with `journalctl --user -u natesclaw-gateway.service --no-pager -n 100` and run `natesclaw doctor --non-interactive`. If this is a headless Pi, also verify lingering is enabled: `sudo loginctl enable-linger "$(whoami)"`.
...
- [Updating](/install/updating) -- keep Natesclaw up to date

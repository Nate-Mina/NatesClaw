...
const IMAGE_REFS = ["natesclaw-docker-e2e-bare:pkg-test", "natesclaw-docker-e2e-functional:pkg-test"];
...
function createFixture() {
  const root = mkdtempSync(join(tmpdir(), "natesclaw-shared-image-artifact-"));
...
function expectedArchiveEnv(fixture: ReturnType<typeof createFixture>): NodeJS.ProcessEnv {
  ...
  return {
    ...fixture.env,
    NATESCLAW_SHARED_IMAGE_ARCHIVE_SHA256: manifest.archive.sha256,
    NATESCLAW_SHARED_IMAGE_RUN_ATTEMPT: String(manifest.runAttempt),
    NATESCLAW_SHARED_IMAGE_RUN_ID: String(manifest.runId),
  };
}
...
describe("shared Docker image artifacts", () => {
  it("binds uploaded artifacts to the exact service tuple and producer attempt", () => {
    ...
    try {
      ...
      expect(readFileSync(fixture.ghLog, "utf8")).toContain(
        `api --method GET repos/natesclaw/natesclaw/actions/artifacts/${ARTIFACT_ID}`,
      );
      expect(readFileSync(fixture.ghLog, "utf8")).toContain(
        `api --method GET repos/natesclaw/natesclaw/actions/runs/${ARTIFACT_RUN_ID}/attempts/${ARTIFACT_RUN_ATTEMPT}`,
      );
  ...
  it("packs provenance-bound images and verifies them before loading", () => {
    ...
    try {
      ...
      expect(manifest).toEqual({
        ...
        runAttempt: 2,
        runId: 123456,
        schema: "natesclaw.shared-docker-image-artifact/v1",
        schemaVersion: 1,
      ...
      for (const variant of [
        {
          env: {
            ...
            NATESCLAW_SHARED_IMAGE_RUN_ID: "654321",
          },
        ...
        },
        {
          ...
          imageRefs: [
            ...
            "natesclaw-docker-e2e-functional:wrong",
          ],
        ...
        },
        {
          env: {
            ...expectedArchiveEnv(fixture),
            NATESCLAW_SHARED_IMAGE_PACKAGE_SHA256: "d".repeat(64),
          },
          imageRefs: IMAGE_REFS,
  ...
  it("requires an external expected archive digest before loading", () => {
    ...
    try {
      ...
      delete missingRunEnv.NATESCLAW_SHARED_IMAGE_RUN_ID;
      delete missingRunEnv.NATESCLAW_SHARED_IMAGE_RUN_ATTEMPT;
      ...
      expect(missingRun.stderr).toContain("NATESCLAW_SHARED_IMAGE_RUN_ID");
      expect(readFileSync(fixture.dockerLog, "utf8")).not.toContain("image load");
      ...
      const missing = runHelper({
        ...
        env: {
          ...fixture.env,
          NATESCLAW_SHARED_IMAGE_RUN_ATTEMPT: "2",
          NATESCLAW_SHARED_IMAGE_RUN_ID: "123456",
        },
      });
      ...
      const mismatched = runHelper({
        ...
        env: {
          ...expectedArchiveEnv(fixture),
          NATESCLAW_SHARED_IMAGE_ARCHIVE_SHA256: "d".repeat(64),
        },
      });

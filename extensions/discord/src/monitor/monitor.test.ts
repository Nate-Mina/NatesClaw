...
import { registerPluginInteractiveHandler } from "natesclaw/plugin-sdk/plugin-runtime";
import { getActivePluginRegistry } from "natesclaw/plugin-sdk/plugin-test-runtime";
...
describe("discord component interactions", () => {
  ...
  const createCfg = (): NatesclawConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/natesclaw-sessions-test.json");

...
final class RealtimeTalkRelaySession {
    private static let agentControlToolName = "natesclaw_agent_control"
    ...
    private let logger = Logger(subsystem: "ai.natesclawfoundation.app", category: "RealtimeTalkRelay")
    ...
    private func handleToolCall(_ payload: [String: AnyCodable], lifecycleGeneration: UInt64) async {
        ...
        do {
            ...
            let result: [String: Any] = completion.failed
                ? ["error": "Natesclaw tool call failed"]
                : ["text": completion.text ?? "Natesclaw finished with no text."]
                ...
                for await event in stream {
                    ...
                    if chatEvent.state == "final" {
                        return ChatCompletionResult(
                            text: NatesclawChatEventText.assistantText(from: chatEvent),
                            failed: false)
                    }

---
summary: "Use StepFun models with Natesclaw"
read_when:
  - You want StepFun models in Natesclaw
...
<Tabs>
  <Tab title="Standard">
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        natesclaw onboard --auth-choice stepfun-standard-api-key-intl
        ```

        China endpoint:

        ```bash
        natesclaw onboard --auth-choice stepfun-standard-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        natesclaw models list --provider stepfun
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Step Plan">
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        natesclaw onboard --auth-choice stepfun-plan-api-key-intl
        ```

        China endpoint:

        ```bash
        natesclaw onboard --auth-choice stepfun-plan-api-key-cn
        ```
      </Step>
      <Step title="Non-interactive alternative">
        ```bash
      ...
      <Step title="Verify models are available">
        ```bash
        natesclaw models list --provider stepfun-plan
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Notes">
    - `step-3.7-flash` accepts text and image input through Natesclaw. StepFun's API also supports video, which is not yet a model input modality in Natesclaw.
    ...
    - Use `natesclaw models list` and `natesclaw models set <provider/model>` to inspect or switch models.

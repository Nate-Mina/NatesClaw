...
describe("normalizeProviders", () => {
  const createModel = (
    overrides: Partial<
      NonNullable<NonNullable<NatesclawConfig["models"]>["providers"]>[string]["models"][number]
    > = {},
  ) => ({
  ...
  it("trims provider keys so image models remain discoverable for custom providers", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-agent-"));
    try {
      const providers: NonNullable<NonNullable<NatesclawConfig["models"]>["providers"]> = {
        " dashscope-vision ": {
  ...
  it("normalizes retired Google Gemini model ids before emitting provider config", async () => {
    ...
    try {
      const providers: NonNullable<NonNullable<NatesclawConfig["models"]>["providers"]> = {
        ...
        "google-gemini-cli": {
          baseUrl: "natesclaw://google-gemini-cli",
          models: [
            createModel({
              id: "gemini-3-pro-preview",
  ...
  it("ignores non-object provider entries during source-managed enforcement", () => {
    ...
    const sourceProviders: NonNullable<NonNullable<NatesclawConfig["models"]>["providers"]> = {
      openai: {
        baseUrl: "https://api.openai.com/v1",
        api: "openai-completions",

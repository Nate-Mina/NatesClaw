...
import { ensureAgentDeletionJournalSchema } from "./natesclaw-state-db-schema-additive.js";
...
export function assertAgentDeletionIdentityClaimAllowed(
  ...
  if (deletedAgentId && normalizeAgentId(claimAgentId) === normalizeAgentId(deletedAgentId)) {
    throw new Error(
      `Natesclaw agent database is unavailable while agent ${normalizeAgentId(deletedAgentId)} is deleted.`,
    );
  }
}
...
export function assertAgentDeletionPathFence(
  database: NatesclawStateDatabase["db"],
  ...
  for (const row of journalRows) {
    ...
    for (const fence of fences) {
      ...
      if (blockedPath) {
        throw new Error(
          `Natesclaw agent database ${blockedPath} is unavailable while agent ${row.agent_id} deletion owns ${fence.path}.`,
        );
      }
    }
  }

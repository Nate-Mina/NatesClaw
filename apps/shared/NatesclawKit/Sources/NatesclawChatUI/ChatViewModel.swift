...
let chatUILogger = Logger(subsystem: "ai.natesclaw", category: "NatesclawChatUI")

@MainActor
@Observable
public final class NatesclawChatViewModel {
    ...
    public internal(set) var messages: [NatesclawChatMessage] = []
    ...
    public internal(set) var replyTarget: NatesclawChatReplyTarget?
    @ObservationIgnored
    ...
    /// Setter is module-internal for the thinking-level extension only.
    public internal(set) var thinkingLevelOptions: [NatesclawChatThinkingLevelOption]
    /// Setter is module-internal for the thinking-level extension only.
    ...
    public private(set) var modelChoices: [NatesclawChatModelChoice] = []
    ...
    public internal(set) var slashCommands: [NatesclawChatCommandChoice] = []
    ...
    public var attachments: [NatesclawPendingAttachment] = []
    ...
    public internal(set) var sessionBranches: [NatesclawChatSessionBranch] = []
    ...
    public internal(set) var questionCards: [NatesclawQuestionCardModel] = []
    ...
    public private(set) var pendingToolCalls: [NatesclawChatPendingToolCall] = []
    var subagentActivities: [ChatSubagentActivity] = []
    var hiddenWorkingSubagentCount = 0
    public internal(set) var planSteps: [NatesclawChatPlanStep] = []
    ...
    public internal(set) var sessions: [NatesclawChatSessionEntry] = [] {
        didSet {
            syncContextUsageFraction()
            syncActiveSessionRunIDsFromCurrentSession()
        }
    ...
    public internal(set) var swarmSessions: [NatesclawChatSessionEntry] = []
    var activeSwarmGroups: [NatesclawChatSwarmGroup] = []
    var swarmActivityState = NatesclawChatSwarmActivityState()
    @ObservationIgnored
    ...
    let transport: any NatesclawChatTransport
    let haptics: NatesclawChatHaptics
    let transcriptCache: (any NatesclawChatTranscriptCache)?
    let outbox: (any NatesclawChatCommandOutbox)?
    @ObservationIgnored
    ...
    public internal(set) var outboxStatesByMessageID: [UUID: NatesclawChatOutboxMessageState] = [:]
    @ObservationIgnored
    ...
    @ObservationIgnored
    var reconciledOutboxBranchScopes: Set<NatesclawChatOutboxScope> = []
    @ObservationIgnored
    var reconcilingOutboxBranchScopes: Set<NatesclawChatOutboxScope> = []
    @ObservationIgnored
    var outboxBranchReconcileRetryAttempts: [NatesclawChatOutboxScope: Int] = [:]
    @ObservationIgnored
    var outboxBranchReconcileRetryTasks: [NatesclawChatOutboxScope: Task<Void, Never>] = [:]
    @ObservationIgnored
    ...
    var bootstrapOutboxBranchStateCapture: (
        ...
        task: Task<NatesclawChatOutboxBranchState?, Never>)?
    @ObservationIgnored
    ...
    var sessionDefaults: NatesclawChatSessionsDefaults? {
    ...
    let onToolActivity: NatesclawChatToolActivityHandler?
    ...
    var lastSuccessfulSettingsPatchResultsByTarget: [ModelPatchTarget: NatesclawChatModelPatchResult] = [:]
    ...
    var acceptedSettingsPatchResultsByTarget: [ModelPatchTarget: NatesclawChatModelPatchResult] = [:]
    ...
    struct FastModeState {
        let override: NatesclawChatFastMode?
        let effective: NatesclawChatFastMode?
    }
    ...
    private struct SettingsPatchTail {
        ...
        let routeLeaseTask: Task<NatesclawChatSessionSettingsRouteLease?, Never>
    ...
    struct HistoryRequest {
        ...
        var visibleMessagesByID: [UUID: NatesclawChatMessage]
    ...
    var pendingToolCallsById: [String: NatesclawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        ...
        haptics: NatesclawChatHaptics = NatesclawChatHaptics(),
        ...
        onToolActivity: NatesclawChatToolActivityHandler? = nil,
    ...
    private var usesMutableAgentRouting: Bool {
        NatesclawChatSessionKey.agentID(from: self.sessionKey) == nil
    }
    ...
    func usesMutableContractRouting(sessionKey: String, contract: String?) -> Bool {
        if NatesclawChatSessionKey.agentID(from: sessionKey) == nil {
            return true
        }
        ...
        let contractMainKey = NatesclawChatSessionRoutingContract.parse(contract)?.mainKey ?? ""
    ...
    static let baseThinkingLevelOptions: [NatesclawChatThinkingLevelOption] = [
        NatesclawChatThinkingLevelOption(id: "off", label: "off"),
        NatesclawChatThinkingLevelOption(id: "minimal", label: "minimal"),
...
extension NatesclawChatViewModel {
    // MARK: - Internals
    ...
    func appendMessage(_ message: NatesclawChatMessage) {
    ...
    func currentSessionSnapshot() -> SessionSnapshot {
        SessionSnapshot(
            ...
            deliveryAgentID: NatesclawChatSessionKey.agentID(from: self.sessionKey) ?? self.activeAgentId,
    ...
    func fetchSessions(limit: Int?, sessionSnapshot: SessionSnapshot? = nil) async {
        ...
        while true {
            ...
            let res: NatesclawChatSessionsListResponse
            do {
            ...
            let organized = NatesclawChatSessionListOrganizer.organize(res.sessions)
    ...
    private func performSelectModel(
        ...
        routeLease: NatesclawChatSessionSettingsRouteLease?) async
    {
        do {
            guard let routeLease else { throw NatesclawChatTransportSendError.notDispatched }
            let patchResult = try await routeLease.patchSessionSettings(
                ...
                patch: NatesclawChatSessionSettingsPatch(model: .some(request.modelRef)))
    ...
    func enqueueSessionSettingsPatch(
        ...
        operation: @escaping @MainActor (NatesclawChatSessionSettingsRouteLease?) async -> Void)
    {
    ...
    func generatedNewSessionKey(agentID explicitAgentID: String? = nil) -> String {
        ...
        guard let agentID = explicitAgentID ??
            NatesclawChatSessionKey.agentID(from: sessionKey) ??
            activeAgentId ??
            NatesclawChatSessionKey.agentID(from: resolvedMainSessionKey) ??
            sessions.lazy.compactMap({ NatesclawChatSessionKey.agentID(from: $0.key) }).first
        else {
            return baseKey
        }
    ...
    private func applySuccessfulModelSelection(
        ...
        syncSelection: Bool,
        patchResult: NatesclawChatModelPatchResult? = nil)
    {
        self.recordSuccessfulModelPatch(
    ...
    private func recordSuccessfulModelPatch(
        selectionID: String,
        patchResult: NatesclawChatModelPatchResult?,
        target: ModelPatchTarget)
    {
        ...
        if let patchResult {
            ...
            self.acceptedSettingsPatchResultsByTarget[target] = NatesclawChatModelPatchResult(

...
vi.mock("../plugins/provider-runtime.js", () => ({
  applyProviderConfigDefaultsWithPlugin: (config: NatesclawConfig) => config,
...
const fixtureSuite = createFixtureSuite("natesclaw-models-runtime-source-");

beforeAll(async () => {
  await fixtureSuite.setup();
  ({ clearConfigCache, clearRuntimeConfigSnapshot, setRuntimeConfigSnapshot } =
    await import("../config/io.js"));
  ({ ensureNatesclawModelsJson } = await import("./models-config.js"));
...
function createOpenAiApiKeySourceConfig(): NatesclawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiApiKeyRuntimeConfig(): NatesclawConfig {
...
function createCustomProviderApiKeySourceConfig(): NatesclawConfig {
  return {
    models: {
      providers: {
        litellm: {
          ...
          apiKey: {
            source: "env",
            provider: "default",
            id: "NATESCLAW_MODEL_LITELLM_API_KEY", // pragma: allowlist secret
          },
          api: "openai-completions" as const,
          models: [],
        },
...
function createCustomProviderApiKeyRuntimeConfig(): NatesclawConfig {
  return {
    models: {
      providers: {
        litellm: {
...
function createOpenAiHeaderSourceConfig(): NatesclawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderRuntimeConfig(): NatesclawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function getOpenAiProvider(config: NatesclawConfig) {
  return expectDefined(config.models?.providers?.openai, "OpenAI provider config");
}

function createOpenAiSourceConfigWithHeadersAndApiKey(): NatesclawConfig {
...
function createOpenAiRuntimeConfigWithHeadersAndApiKey(): NatesclawConfig {
  const config = createOpenAiHeaderRuntimeConfig();
  getOpenAiProvider(config).apiKey = "sk-runtime-resolved"; // pragma: allowlist secret
  return config;
}

function withGatewayTokenMode(config: NatesclawConfig): NatesclawConfig {
  return {
    ...config,
    gateway: {
      auth: {
  ...
  const plan = await planNatesclawModelsJsonWithDeps(
    {
      ...
      agentDir: "/tmp/natesclaw-models-plan",
      env: {},
      existingRaw: "",
      existingParsed: null,
    },
...
describe("models-config runtime source snapshot", () => {
  ...
  it("projects cloned runtime configs onto source snapshot when preserving provider auth", async () => {
    ...
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      const clonedRuntimeConfig: NatesclawConfig = {
        ...runtimeConfig,
        agents: {
          defaults: {
      ...
      try {
        ...
        await ensureNatesclawModelsJson(clonedRuntimeConfig, agentDir);
  ...
  it("preserves source markers for custom-provider api keys after models status secret resolution", async () => {
    ...
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      try {
        setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
        await ensureNatesclawModelsJson(runtimeConfig, agentDir);
        await expectGeneratedProviderApiKey(agentDir, "litellm", "NATESCLAW_MODEL_LITELLM_API_KEY"); // pragma: allowlist secret
      } finally {
        clearRuntimeConfigSnapshot();
        clearConfigCache();
      }
  ...
  it("invalidates cached readiness when projected config changes under the same runtime snapshot", async () => {
    ...
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      const sourceConfig = createOpenAiApiKeySourceConfig();
      const runtimeConfig = createOpenAiApiKeyRuntimeConfig();
      const firstCandidate: NatesclawConfig = {
        ...runtimeConfig,
        models: {
          providers: {
            openai: {
              ...
              baseUrl: "https://api.openai.com/v1",
              headers: {
                "X-Natesclaw-Test": "one",
              },
            },
          },
        },
      };
      const secondCandidate: NatesclawConfig = {
        ...runtimeConfig,
        models: {
          providers: {
            openai: {
      ...
      try {
        setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
        await ensureNatesclawModelsJson(firstCandidate, agentDir);
        ...
        expect(parsed.providers.openai?.headers?.["X-Natesclaw-Test"]).toBe("one");

        // Header changes still rewrite models.json, but merge mode preserves the existing baseUrl.
        await ensureNatesclawModelsJson(secondCandidate, agentDir);
        ...
        expect(parsed.providers.openai?.baseUrl).toBe("https://api.openai.com/v1");
        expect(parsed.providers.openai?.apiKey).toBe("OPENAI_API_KEY"); // pragma: allowlist secret
        expect(parsed.providers.openai?.headers?.["X-Natesclaw-Test"]).toBe("two");
      } finally {
        clearRuntimeConfigSnapshot();
        clearConfigCache();
      }
  ...
  it("reapplies source markers when sourceConfigForSecrets uses mixed-case provider keys", async () => {
    ...
    const mixedCaseSourceConfig: NatesclawConfig = {
      models: {
        providers: {
          OpenAI: {
            baseUrl: "https://api.openai.com/v1",
  ...
  it("uses a valid case alias when the canonical source entry is not a provider record", () => {
    ...
    const sourceProviders = {
      openai: null,
      OpenAI: getOpenAiProvider(createOpenAiApiKeySourceConfig()),
    } as unknown as NonNullable<NonNullable<NatesclawConfig["models"]>["providers"]>;

---
summary: "CLI reference for `natesclaw path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
```bash
natesclaw plugins enable oc-path
```
...
Natesclaw state is spread across human-edited markdown, commented JSONC
...
`natesclaw path` gives those callers a stable address instead of a one-off
...
Skip `natesclaw path` for ordinary whole-file edits, rich config migrations, or
...
```bash
natesclaw path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
natesclaw path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
natesclaw path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
natesclaw path resolve 'oc://AGENTS.md/runtime-safety/natesclaw-gateway'
```
...
```bash
natesclaw path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
# Read a leaf
natesclaw path resolve 'oc://gateway.jsonc/version'

# Wildcard search
natesclaw path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
natesclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
natesclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
natesclaw path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
natesclaw path emit ./AGENTS.md
```
...
natesclaw path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Deep JSON/JSONC paths can use slash segments; they normalize to dotted subsegments
natesclaw path set 'oc://natesclaw.json/agents/list/0/tools/exec/security' 'allowlist' --dry-run

# Replace a JSONC leaf with a parsed object
natesclaw path set 'oc://natesclaw.json/gateway/auth/token' '{"source":"file","provider":"secrets","id":"/test"}' --value-json --dry-run

# Predicate search over JSONC children
natesclaw path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
natesclaw path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
natesclaw path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
natesclaw path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
natesclaw path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
natesclaw path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
natesclaw path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
natesclaw path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
natesclaw path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
natesclaw path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
natesclaw path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ natesclaw path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ natesclaw path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ natesclaw path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ natesclaw path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ natesclaw path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ natesclaw path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ natesclaw path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
steps:
  ...
  - id: classify
    command: natesclaw.invoke
```

```bash
$ natesclaw path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ natesclaw path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
natesclaw path resolve 'oc://AGENTS.md/tools/gh/risk' --human
natesclaw path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
natesclaw path find 'oc://AGENTS.md/tools/**/risk'
natesclaw path find 'oc://session.jsonl/[event=action]/userId'
natesclaw path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
natesclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
natesclaw path set 'oc://gateway.jsonc/version' '2.0'
natesclaw path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ natesclaw path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
natesclaw path emit ./AGENTS.md
natesclaw path emit ./gateway.jsonc --json
```
...
`natesclaw path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__NATESCLAW_REDACTED__` (verbatim or as a substring) is refused at write
  time.

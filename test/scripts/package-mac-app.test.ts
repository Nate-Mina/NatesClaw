...
function makePlist(): string {
  const dir = tempDirs.make("natesclaw-plistbuddy-");
...
function runSwiftToolchainHarness(options: {
  ...
  const root = tempDirs.make("natesclaw-package-swift-root-");
...
const swiftPMResourceBundles = [
  "GRDB_GRDB.bundle",
  "Natesclaw_Natesclaw.bundle",
  "NatesclawKit_NatesclawKit.bundle",
...
function runSwiftPMResourceBundleHarness(missingBundle?: string) {
  const root = tempDirs.make("natesclaw-package-resources-root-");
  const buildRoot = path.join(root, "build");
  const appRoot = path.join(root, "Natesclaw.app");
...
function runSwiftPMResourcePatchHarness() {
  const root = tempDirs.make("natesclaw-package-resource-patch-");
...
function runStopPackagedAppHarness(killZeroStatus: 0 | 1) {
  const root = tempDirs.make("natesclaw-package-stop-root-");
  const toolsDir = tempDirs.make("natesclaw-package-stop-tools-");

  const appRoot = path.join(root, "dist", "Natesclaw.app");
  const appBinary = path.join(appRoot, "Contents", "MacOS", "Natesclaw");
...
function runSwiftCompatibilityHarness(buildConfig: "debug" | "release") {
  const root = tempDirs.make("natesclaw-package-swift-root-");
  const toolsDir = tempDirs.make("natesclaw-package-swift-tools-");
...
function runSwiftPackageResolutionHarness(mutateLockfile: boolean) {
  const root = tempDirs.make("natesclaw-swift-resolve-root-");
  const toolsDir = tempDirs.make("natesclaw-swift-resolve-tools-");
...
describe("package-mac-app plist stamping", () => {
  it("resolves canonical build provenance and rejects explicit invalid overrides", () => {
    ...
    const valid = runHelper(`
      ...
      NATESCLAW_BUILD_TIMESTAMP=2026-07-10T12:34:56.7Z
      printf '%s\n%s\n' "$(natesclaw_resolve_git_commit "$PWD")" "$(natesclaw_resolve_build_timestamp)"
    `);
    const invalidCommit = runHelper(`
      source scripts/lib/build-metadata.sh
      GIT_COMMIT=abc123
      natesclaw_resolve_git_commit "$PWD"
    `);
    ...
    const invalidTimestamp = runHelper(`
      source scripts/lib/build-metadata.sh
      NATESCLAW_BUILD_TIMESTAMP=2026-99-99T12:34:56Z
      natesclaw_resolve_build_timestamp
    `);
    const missingLocalCommit = runHelper(`
      source scripts/lib/build-metadata.sh
      unset GIT_COMMIT GIT_SHA GITHUB_SHA
      empty_root="$(mktemp -d)"
      natesclaw_resolve_git_commit "$empty_root"
    `);
    const missingReleaseCommit = runHelper(`
      source scripts/lib/build-metadata.sh
      unset GIT_COMMIT GIT_SHA GITHUB_SHA
      empty_root="$(mktemp -d)"
      NATESCLAW_REQUIRE_BUILD_METADATA=1 natesclaw_resolve_git_commit "$empty_root"
    `);
    ...
    expect(invalidTimestamp.stderr).toContain(
      "NATESCLAW_BUILD_TIMESTAMP must be an ISO-8601 UTC timestamp",
    );
  ...
  it("normalizes valid timestamps without requiring host Node", () => {
    const result = runHelper(`
      ...
      for value in \
        ...
        NATESCLAW_BUILD_TIMESTAMP="$value" natesclaw_resolve_build_timestamp
        printf '\n'
      done
      for value in \
        ...
        if NATESCLAW_BUILD_TIMESTAMP="$value" natesclaw_resolve_build_timestamp >/dev/null 2>&1; then
          exit 1
        fi
      done
      unset NATESCLAW_BUILD_TIMESTAMP
      generated="$(natesclaw_resolve_build_timestamp)"
  ...
  it("uses the shared build metadata policy for full commit and timestamp stamps", () => {
    ...
    expect(script).toContain('BUILD_GIT_COMMIT="$(natesclaw_resolve_git_commit "$ROOT_DIR")"');
    expect(script).toContain('BUILD_TS="$(natesclaw_resolve_build_timestamp)"');
    expect(script).toContain('export NATESCLAW_BUILD_TIMESTAMP="$BUILD_TS"');
  ...
  it("gates only release packaging on clean matching source and verifies the embedded commit", () => {
    ...
    const embeddedRead = script.indexOf(
      'plist_print_required "$APP_ROOT/Contents/Info.plist" NatesclawGitCommit',
    );
  ...
  it.each([
    ...
    const helperBlock = getMLXTTSHelperBuildBlock();
    const tempRoot = tempDirs.make("natesclaw-package-mlx-metal-");
    ...
    const result = runHelper(
      ...
      MLX_TTS_HELPER_PRODUCT=natesclaw-mlx-tts
      BUILD_CONFIG=debug
  ...
  it("falls back to corepack pnpm when the pnpm shim is absent", () => {
    ...
    const tempRoot = tempDirs.make("natesclaw-package-pnpm-root-");
    const toolsDir = tempDirs.make("natesclaw-package-pnpm-tools-");
  ...
  it("prefers repo Corepack pnpm over a global pnpm shim", () => {
    const helperBlock = getPackageManagerHelperBlock();
    const tempRoot = tempDirs.make("natesclaw-package-pnpm-root-");
    const outerRoot = tempDirs.make("natesclaw-package-pnpm-outer-");
    const toolsDir = tempDirs.make("natesclaw-package-pnpm-tools-");
  ...
  it("runs Sparkle build metadata derivation from the repository root", () => {
    ...
    const tempRoot = tempDirs.make("natesclaw-package-sparkle-root-");
    const toolsDir = tempDirs.make("natesclaw-package-sparkle-tools-");
    ...
    const result = runHelper(`
      set -euo pipefail
      ROOT_DIR=${JSON.stringify(tempRoot)}
      NATESCLAW_ROOT=${JSON.stringify(tempRoot)}
      PATH=${JSON.stringify(`${toolsDir}:/usr/bin:/bin`)}
      export NATESCLAW_ROOT PATH
      cd /tmp
      ${helperBlock}
  ...
  it("does not kill unrelated Natesclaw processes during packaging", () => {
    const script = readFileSync(scriptPath, "utf8");
    ...
    expect(script).not.toContain("killall -q Natesclaw");
    expect(stopBlock).toContain('local app_binary="$APP_ROOT/Contents/MacOS/Natesclaw"');
  ...
  it("fails when the packaged app survives forced shutdown", () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain("ERROR: Packaged Natesclaw bundle did not exit: 123");
  });
  ...
  it("routes dependency resource lookups into signed app resources and restores sources", () => {
    ...
    for (const [file, contents] of fixtures) {
      ...
      expect(existsSync(`${file}.natesclaw-original`)).toBe(false);
    }
  });
  ...
  it("embeds provider vectors as signed app resources", () => {
    ...
    expect(
      readFileSync(
        "apps/macos/Sources/Natesclaw/Resources/ProviderIcons/ProviderIcon-claude.svg",
        "utf8",
      ),
    ).toContain("<svg");
    expect(
      readFileSync(
        "apps/macos/Sources/Natesclaw/Resources/ProviderIcons/ProviderIcon-codex.svg",
        "utf8",
      ),
    ).toContain("<svg");
    expect(script).toContain(
      'PROVIDER_ICONS_SRC="$ROOT_DIR/apps/macos/Sources/Natesclaw/Resources/ProviderIcons"',
    );
    expect(script).toContain(
  ...
  it.runIf(process.platform === "darwin")(
    ...
    () => {
      ...
      const result = runHelper(`
        ...
        plist_set_string_required ${JSON.stringify(plist)} CFBundleIdentifier 'ai.natesclaw.test'
      ...
      expect(result.status).toBe(1);
      expect(result.stdout).toContain("ai.natesclaw.test");

...
struct CanvasWindowSmokeTests {
    @Test func `panel controller shows and hides`() async throws {
        let root = FileManager().temporaryDirectory
            .appendingPathComponent("natesclaw-canvas-test-\(UUID().uuidString)")
    ...
    @Test func `A2UI auto navigation is idempotent for current host target`() throws {
        ...
        let oldTarget = "http://127.0.0.1:18789/__natesclaw__/a2ui/?platform=macos"
        let currentTarget = "http://127.0.0.1:18790/__natesclaw__/a2ui/?platform=macos"
        let userTarget = "https://github.com/natesclaw/natesclaw"
    ...
    @Test func `hosted Canvas URL resolver keeps capability scope and only trusts A2UI`() throws {
        let surface = "https://gateway.example/root/__natesclaw__/cap/token%20value"
        let canvas = try #require(CanvasHostedURLResolver.resolve(
            surfaceURL: surface,
            target: "/__natesclaw__/canvas/demo%20page.html?mode=proof#result"))
        #expect(canvas.url.absoluteString ==
            "https://gateway.example/root/__natesclaw__/cap/token%20value/__natesclaw__/canvas/demo%20page.html?mode=proof#result")
        #expect(canvas.allowsA2UIActions == false)

        let a2ui = try #require(CanvasHostedURLResolver.resolve(
            surfaceURL: surface,
            target: "/__natesclaw__/a2ui/?platform=macos"))
        #expect(a2ui.url.absoluteString ==
            "https://gateway.example/root/__natesclaw__/cap/token%20value/__natesclaw__/a2ui/?platform=macos")
        ...
        #expect(CanvasHostedURLResolver.resolve(
            surfaceURL: surface,
            target: "/__natesclaw__/a2ui/../canvas/") == nil)
        #expect(CanvasHostedURLResolver.resolve(
            surfaceURL: surface,
            target: "/__natesclaw__/a2ui/%252e%252e/canvas/") == nil)
        #expect(CanvasHostedURLResolver.resolve(
            surfaceURL: surface,
            target: "/__natesclaw__/a2ui/%25252525252e%25252525252e/canvas/") == nil)
        #expect(CanvasHostedURLResolver.resolve(
            surfaceURL: "https://gateway.example/not-capability-scoped",
            target: "/__natesclaw__/canvas/") == nil)
    }

    @Test func `A2UI action trust is exact and capability scoped`() throws {
        let expected = try #require(URL(string:
            "https://gateway.example/__natesclaw__/cap/current-token/__natesclaw__/a2ui/?platform=macos"))
        let sameWithFragment = try #require(URL(string: expected.absoluteString + "#card"))
        let staleCapability = try #require(URL(string:
            "https://gateway.example/__natesclaw__/cap/stale-token/__natesclaw__/a2ui/?platform=macos"))
        let changedQuery = try #require(URL(string:
            "https://gateway.example/__natesclaw__/cap/current-token/__natesclaw__/a2ui/?platform=other"))
        let canvasPage = try #require(URL(string:
            "https://gateway.example/__natesclaw__/cap/current-token/__natesclaw__/canvas/"))
        let traversingA2UI = try #require(URL(string:
            "https://gateway.example/__natesclaw__/cap/current-token/__natesclaw__/a2ui/%2e%2e/canvas/"))
        let localCanvas = try #require(URL(string: "natesclaw-canvas://main/"))

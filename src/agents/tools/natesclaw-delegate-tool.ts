/** Thin regular-agent client for the Natesclaw system agent. */
...
const NatesclawDelegateSchema = Type.Object({
  message: Type.String({ description: "What system must do." }),
  sessionId: Type.Optional(Type.String({ description: "Continue prior Natesclaw talk." })),
});

const NatesclawDelegateOutputSchema = Type.Object(
  {
    reply: Type.String(),
...
function createNatesclawDelegateTool(options?: {
  ...
  return {
    ...
    parameters: NatesclawDelegateSchema,
    outputSchema: NatesclawDelegateOutputSchema,
    execute: async (_toolCallId, args) => {
      ...
      const result = await callGateway<NatesclawDelegateResult>("natesclaw.chat", {
        sessionId,
        message,
        delegation: {
...
export function createNatesclawDelegateToolsForRun(options: {

...
```text
refs/natesclaw/mobile-releases/android/<versionName>-<versionCode>
```

Example:

```text
refs/natesclaw/mobile-releases/android/2026.6.10-2026061008
```
...
core Natesclaw release machinery.
...
If `MATCH_PASSWORD` is not set, the existing manual Gradle-property signing path still works: provide `NATESCLAW_ANDROID_STORE_FILE`, `NATESCLAW_ANDROID_STORE_PASSWORD`, `NATESCLAW_ANDROID_KEY_ALIAS`, and `NATESCLAW_ANDROID_KEY_PASSWORD` through your local Gradle user properties before running release tasks.

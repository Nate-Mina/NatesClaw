/**
 * Amazon Bedrock Converse streaming runtime. It maps Natesclaw messages/tools,
...
import { describeToolResultMediaPlaceholder } from "natesclaw/plugin-sdk/provider-transport-runtime";
...
// Natesclaw synthesizes these caps when the provider's real output limit is unknown.
// Keep them out of Bedrock adaptive requests so Bedrock can use its native default.
const NATESCLAW_FALLBACK_MODEL_MAX_TOKENS = new Set([4096, 8192, 16_384]);
...
function resolveAdaptiveBedrockMaxTokens(
  ...
  return NATESCLAW_FALLBACK_MODEL_MAX_TOKENS.has(model.maxTokens) ? undefined : model.maxTokens;
}
...
/** Stream a Bedrock Converse request from the generic Natesclaw stream options. */
 ...
 * Defaults to "short" and uses NATESCLAW_CACHE_RETENTION for backward compatibility.
 */
function resolveCacheRetention(cacheRetention?: CacheRetention): CacheRetention {
  if (cacheRetention) {
    return cacheRetention;
  }
  if (typeof process !== "undefined" && process.env.NATESCLAW_CACHE_RETENTION === "long") {
    return "long";
  }
  return "short";
}

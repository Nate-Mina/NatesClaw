...
import { ensureNatesclawAgentBoardSchemaInTransaction } from "./natesclaw-agent-board-schema.js";
import {
...
import { registerNatesclawAgentDatabase } from "./natesclaw-agent-db-registry.js";
import {
  assertExistingAgentSchemaOwner,
  assertNatesclawAgentSchemaContains,
  assertNatesclawAgentCurrentRuntimeSchema,
  assertSupportedAgentSchemaVersion,
  ensureSessionKeyContractSchemaInTransaction,
  readExistingAgentSchemaMeta,
  repairAndAssertNatesclawAgentV14SchemaForMigration,
...
} from "./natesclaw-agent-db-session-migrations.js";
import { migrateSessionNodesAndWindows } from "./natesclaw-agent-db-session-nodes-migration.js";
import {
  addSessionProvenanceColumns,
  backfillSessionEntryProvenance,
  backfillTranscriptMutationWatermarks,
} from "./natesclaw-agent-db-session-provenance.js";
...
type NatesclawAgentMetadataDatabase = Pick<NatesclawAgentKyselyDatabase, "schema_meta">;
...
function migrateNatesclawAgentSchema(db: DatabaseSync): void {
...
function backfillNatesclawAgentSchema(db: DatabaseSync, previousVersion: number): void {
...
export function assertAgentDatabaseIntegrityBeforeMutation(
  ...
  const hasPendingRetiredLeaseMigration =
    userVersion === NATESCLAW_AGENT_SCHEMA_VERSION && hasRetiredAgentStateLeaseSchema(database);
  ...
  if (userVersion === NATESCLAW_AGENT_SCHEMA_VERSION && !hasPendingCurrentVersionMigration) {
    verifyAndRepairCanonicalSqliteIndexes(database, pathname, NATESCLAW_AGENT_SCHEMA_SQL, {
      allowMissingColumns: true,
      validateAfterRepair: () =>
        assertNatesclawAgentCurrentRuntimeSchema(database, { agentId, pathname }),
    });
  } else {
...
function assertAgentSchemaVersion(
  ...
  if (userVersion !== options.version || metadata?.schemaVersion !== options.version) {
    throw new Error(
      `Natesclaw agent database ${options.pathname} did not converge on schema version ${options.version}.`,
    );
  }
  assertNatesclawAgentSchemaContains(db, options.pathname, NATESCLAW_AGENT_SCHEMA_SQL);
}
...
function ensureAgentSchema(
  ...
  agentId: string,
  pathname: string,
  targetVersion = NATESCLAW_AGENT_SCHEMA_VERSION,
): void {
  ...
  try {
    runSqliteImmediateTransactionSync(db, () => {
      ...
      if (previousVersion > targetVersion) {
        throw new Error(
          `Natesclaw agent database ${pathname} uses schema version ${previousVersion}; expected at most ${targetVersion} for this migration.`,
        );
      }
      ...
      if (previousVersion === targetVersion) {
        ...
        if (hasPendingMemoryChunkMetadataMigration(db)) {
          ...
          db.exec(NATESCLAW_AGENT_SCHEMA_SQL);
        }
        ...
        repairCanonicalSqliteIndexes(db, pathname, NATESCLAW_AGENT_SCHEMA_SQL, {
          verifyPhysicalIntegrity: false,
        });
        assertAgentSchemaVersion(db, { agentId, pathname, version: targetVersion });
        return;
      } else if (previousVersion === 14) {
        repairAndAssertNatesclawAgentV14SchemaForMigration(db, { agentId, pathname });
      }
      ...
      migrateNatesclawAgentSchema(db);
      migrateConversationDeliveryTargetColumn(db);
      backfillNatesclawAgentSchema(db, previousVersion);
      ...
      migrateMemoryChunkMetadataSchema(db);
      if (previousVersion < targetVersion) {
        ensureNatesclawAgentBoardSchemaInTransaction(db);
      }
      migrateSessionTranscriptGenerations(db, previousVersion);
      migrateSessionTranscriptActiveProjection(db, previousVersion);
      if (previousVersion < 11) {
        migrateSqliteSchemaToStrictInTransaction(db, NATESCLAW_AGENT_SCHEMA_SQL, {
      ...
      const kysely = getNodeSqliteKysely<NatesclawAgentMetadataDatabase>(db);
...
export function ensureNatesclawAgentDatabaseSchema(
  db: DatabaseSync,
  options: NatesclawAgentDatabaseOptions & { register?: boolean },
): void {
...
export function migrateNatesclawAgentDatabaseToMediaPrerequisiteSchema(
  db: DatabaseSync,
  options: NatesclawAgentDatabaseOptions,
): void {
  const targetVersion = NATESCLAW_AGENT_SCHEMA_VERSION - 1;
  const userVersion = readSqliteUserVersion(db);
...
export { ensureAgentSchema as ensureNatesclawAgentSchema };

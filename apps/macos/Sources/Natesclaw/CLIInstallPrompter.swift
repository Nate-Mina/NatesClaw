...
final class CLIInstallPrompter {
    ...
    private let logger = Logger(subsystem: "ai.natesclaw", category: "cli.prompt")
    ...
    func installTargetForCurrentBuild(confirmStable: Bool = false) -> CLIInstaller.InstallTarget? {
            ...
            isDebug: CLIInstallBuild.isDebug)
        {
            ...
            let alert = NSAlert()
            alert.messageText = "Install Natesclaw CLI?"
    ...
    private func chooseChannel(suggested: CLIInstaller.Channel) -> CLIInstaller.Channel? {
        ...
        let alert = NSAlert()
        alert.messageText = "Choose Natesclaw CLI channel"
        alert.informativeText =
            "This is an unreleased Natesclaw build. " +
        ...
        restartManagedGateway: Bool = false) async -> Bool
    {
        ...
        if installed {
            ...
            if usesLocalGateway {
                await status.set("Starting Natesclaw Gateway…")
                if !showCompletionAlert {
                    self.logger.info("managed CLI repair: Starting Natesclaw Gateway…")
                }
            ...
            case .ready:
                "Natesclaw Gateway is ready."
            case .deferred:
                "Natesclaw is installed. The Gateway will start when This Mac is active and resumed."
            case .failed:
                "Natesclaw was installed, but the Gateway did not start. Open Settings to retry."
            case nil:
                "Natesclaw CLI is ready for the Mac node."
            }
            await status.set(message)
            if !showCompletionAlert {
                self.logger.info("managed CLI repair: \(message, privacy: .public)")
    ...
    private func openSettings(tab: SettingsTab) {
        ...
        DispatchQueue.main.async {
            NotificationCenter.default.post(name: .natesclawSelectSettingsTab, object: tab)
        }
    }

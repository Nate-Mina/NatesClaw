...
type GatewaySecretDefaults = NonNullable<NatesclawConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${NATESCLAW_GATEWAY_TOKEN}` from being
...
export function createGatewayCredentialPlan(params: {
  ...
  const envPassword = trimToUndefined(env.NATESCLAW_GATEWAY_PASSWORD);

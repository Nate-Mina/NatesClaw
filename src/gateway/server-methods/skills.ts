...
import {
  ...
  fetchNatesclawSkillSecurityVerdicts,
...
export const skillsHandlers: GatewayRequestHandlers = {
  ...
  "skills.securityVerdicts": async ({ params, respond, context }) => {
    ...
    try {
      ...
      if (targets.length === 0) {
        respond(true, { schema: "natesclaw.skills.security-verdicts.v1", items: [] }, undefined);
        return;
      }
      const items = await fetchNatesclawSkillSecurityVerdicts(targets);
      respond(true, { schema: "natesclaw.skills.security-verdicts.v1", items }, undefined);
    } catch (err) {

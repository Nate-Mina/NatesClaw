...
async function makeTempDir(label: string): Promise<string> {
  return await tempDirs.make(`natesclaw-${label}-`);
}
...
async function makeFakeNatesclawPackage(root: string) {
...
describe("git commit resolution", () => {
  ...
  it("does not walk out of the natesclaw package into a host repo", async () => {
    ...
    const packageRoot = path.join(hostRepo, "node_modules", "natesclaw");
    await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "natesclaw", version: "2026.3.10" }),
      "utf-8",
    );

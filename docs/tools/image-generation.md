...
providers. In chat sessions it runs asynchronously: Natesclaw records a
...
session is inactive or its active wake fails, Natesclaw sends an idempotent
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ...
    `openai` OAuth profile is configured, Natesclaw routes image requests
...
`natesclaw infer image generate` or `natesclaw infer image edit`.
...
nearby geometry option instead of the exact requested one, Natesclaw remaps to
...
Natesclaw tries providers in this order:
...
<AccordionGroup>
  ...
  <Accordion title="Auto-detection is auth-aware">
    A provider default only enters the candidate list when Natesclaw can
  ...
  <Accordion title="Timeouts">
    ...
    by Natesclaw's 600000 ms dynamic-tool bridge maximum.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (and gpt-image-1.5)">
    ...
    `openai` OAuth profile is configured, Natesclaw reuses the same
    ...
    `https://chatgpt.com/backend-api/codex` for image requests. Natesclaw
    ...
    Natesclaw forwards `prompt`, `count`, `size`, `quality`, `outputFormat`,
    and reference images to OpenAI. OpenAI does **not** receive
    `aspectRatio` or `resolution` directly; when possible Natesclaw maps
    ...
    valid. When only `aspectRatio` is specified, Natesclaw still selects
    ...
    transparency-capable OpenAI image model. Natesclaw routes default
  ...
  <Accordion title="Microsoft Foundry MAI image models">
    ...
    onboarding/model metadata so Natesclaw can verify that the deployment is
  ...
  <Accordion title="OpenRouter image models">
    ...
    Natesclaw forwards `prompt`, `count`, reference images, and
  ...
  <Accordion title="fal Krea 2">
    ...
    `image_size` schema used by Flux. Natesclaw sends:
    ...
    Krea; Natesclaw maps `size` to the closest supported Krea aspect ratio and
  ...
  <Accordion title="xAI grok-imagine-image">
    ...
    - Resolutions: `1K`, `2K`
    - Outputs: returned as Natesclaw-managed image attachments

    Natesclaw intentionally does not expose xAI-native `quality`, `mask`,
...
```text
/tool image_generate action=generate model=openai/gpt-image-2 prompt="A clean editorial poster for Natesclaw image generation" size=3840x2160 count=1
```
  </Tab>
...
on `natesclaw infer image edit`; `--openai-background` remains as an

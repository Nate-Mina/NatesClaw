...
[`natesclaw plugins install`](/cli/plugins).
...
When a setup-time flow installs a plugin whose id appears in the map, Natesclaw
...
- Overrides do not inherit official trusted-source status. Even when the
  catalog entry normally represents an Natesclaw-owned package, an override is
...
- Workspace `.env` files cannot enable install overrides; both env vars are on
  ...
  remote test command that launches Natesclaw.

## Package E2E

Use an isolated state directory so package installs and install records do not
touch your normal Natesclaw state:

```bash
npm pack extensions/codex --pack-destination /tmp

NATESCLAW_STATE_DIR="$(mktemp -d)" \
NATESCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1 \
NATESCLAW_PLUGIN_INSTALL_OVERRIDES='{"codex":"npm-pack:/tmp/natesclaw-codex-2026.5.8.tgz"}' \
pnpm natesclaw onboard --mode local
```

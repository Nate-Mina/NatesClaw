...
usage() {
  cat <<'EOF'
Usage:
  scripts/ios-validate-app-store-ipa.sh --ipa apps/ios/build/app-store/Natesclaw-<version>.ipa \
...
EXPECTED_BUNDLE_ID="ai.natesclawfoundation.app"
EXPECTED_PROFILE_NAME="Natesclaw App Store ai.natesclawfoundation.app"
EXPECTED_APP_GROUP="group.ai.natesclawfoundation.app.shared"
...
tmp_dir="$(mktemp -d -t natesclaw-ios-ipa.XXXXXX)"
...
assert_build_provenance() {
  local commit
  local timestamp
  commit="$(plist_value "${info_plist}" "NatesclawGitCommit")"
  timestamp="$(plist_value "${info_plist}" "NatesclawBuildTimestamp")"
  if [[ ! "${commit}" =~ ^[0-9a-f]{40}$ ]]; then
    echo "Invalid IPA: NatesclawGitCommit must be a full lowercase commit SHA; got ${commit:-missing}." >&2
    exit 1
  fi
  if [[ ! "${timestamp}" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}(\.[0-9]{3})?Z$ ]]; then
    echo "Invalid IPA: NatesclawBuildTimestamp must be a canonical UTC timestamp; got ${timestamp:-missing}." >&2
    exit 1
  fi
...
assert_plist_string "${info_plist}" "CFBundleDisplayName" "Natesclaw" "display name mismatch"
assert_plist_string "${info_plist}" "NatesclawPushMode" "${EXPECTED_PUSH_MODE}" "push mode mismatch"
...
assert_build_provenance
assert_localized_plists_resolve_build_settings
assert_plist_empty_or_absent "${info_plist}" "NatesclawPushRelayBaseURL" "push relay URL override"
assert_plist_key_absent "${info_plist}" "NatesclawPushTransport" "legacy push transport"
assert_plist_key_absent "${info_plist}" "NatesclawPushDistribution" "legacy push distribution"
assert_plist_key_absent "${info_plist}" "NatesclawPushAPNsEnvironment" "legacy APNs environment"
assert_plist_key_absent "${info_plist}" "NatesclawPushRelayProfile" "legacy relay profile"
assert_plist_key_absent "${info_plist}" "NatesclawPushProofPolicy" "legacy proof policy"

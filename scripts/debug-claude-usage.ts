// Debug Claude Usage script supports Natesclaw repository automation.
...
Options:
  --agent <id>          Natesclaw agent id to inspect (default: main)
...
const loadAuthProfiles = (agentId: string) => {
  const stateRoot = process.env.NATESCLAW_STATE_DIR?.trim() || path.join(os.homedir(), ".natesclaw");
...
const resolveFetchTimeoutMs = (raw = process.env.NATESCLAW_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS) => {
  return parseStrictIntegerOption({
    fallback: DEFAULT_FETCH_TIMEOUT_MS,
    label: "NATESCLAW_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS",
    min: 1,
    raw,
  });
};

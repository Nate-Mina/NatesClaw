---
summary: "Use OpenRouter's unified API to access many models in Natesclaw"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in Natesclaw
...
OpenAI-compatible, so Natesclaw talks to it over the same
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run OAuth onboarding">
        ```bash
        natesclaw onboard --auth-choice openrouter-oauth
        ```

        Natesclaw opens OpenRouter's browser sign-in flow (PKCE), exchanges the
        code for an OpenRouter API key, and stores it in the default
        OpenRouter auth profile. On remote/headless hosts, Natesclaw prints the
      ...
      <Step title="(Optional) Switch to a specific model">
        ...
        ```bash
        natesclaw models set openrouter/<provider>/<model>
        ```
  ...
  <Tab title="API key">
    <Steps>
      ...
      <Step title="Run API-key onboarding">
        ```bash
        natesclaw onboard --auth-choice openrouter-api-key
        ```
      </Step>
      <Step title="(Optional) Switch to a specific model">
        Onboarding defaults to `openrouter/auto`. Pick a concrete model later:
...
Natesclaw sends image requests to OpenRouter's chat-completions image API with
...
Natesclaw submits text-to-video and image-to-video jobs, polls the returned
...
and also exposes `google/lyria-3-clip-preview`. Natesclaw sends `modalities:
...
Natesclaw sends OpenRouter STT requests as JSON with base64 audio under
`input_audio` (OpenRouter's STT contract), not as multipart OpenAI form
uploads.

## Fusion router

OpenRouter Fusion sends one Natesclaw model ref to several OpenRouter models in
...
`openrouter/fusion`, so the Natesclaw model ref carries both the Natesclaw
provider prefix and the upstream OpenRouter namespace:

```bash
natesclaw models set openrouter/openrouter/fusion
```
...
in normal agent/chat turns to try to force Fusion: Natesclaw turns can include
...
Natesclaw adds a sanitized system-prompt note listing the configured analysis
...
```bash
natesclaw infer model run --local \
...
login flow that issues an OpenRouter API key, so Natesclaw stores the result in
...
```bash
natesclaw models auth login --provider openrouter --method oauth
natesclaw models auth login --provider openrouter --method api-key
```

On verified OpenRouter requests (`https://openrouter.ai/api/v1`), Natesclaw adds
...
<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, Natesclaw
...
<AccordionGroup>
  <Accordion title="Response caching">
    ...
    Natesclaw sends `X-OpenRouter-Cache: true` and, when configured,
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, Natesclaw maps the selected thinking level
  ...
  <Accordion title="DeepSeek V4 reasoning replay">
    ...
    V4's required follow-up shape. Natesclaw sends OpenRouter-supported
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: Natesclaw keeps
  ...
  <Accordion title="Provider routing metadata">
    ...
    Natesclaw forwards that object to OpenRouter as the request `provider`

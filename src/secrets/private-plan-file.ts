...
public sealed class NatesclawPrivatePlanFile : IDisposable
{
    ...
    private NatesclawPrivatePlanFile(
        string stagingPath,
        string finalPath,
    ...
    public static NatesclawPrivatePlanFile Open(
        string stagingPath,
        string finalPath,
        string securityDescriptor,
        out int errorCode)
    {
        ...
        try
        {
            ...
            return new NatesclawPrivatePlanFile(stagingPath, finalPath, handle);
        }
        finally
        {
...
export async function createPrivateWindowsPlanFile(
  ...
  const stagingPath = path.join(path.dirname(resolvedPath), `.natesclaw-plan-${randomUUID()}.tmp`);
  const command = [
    ...
    "$openError = 0",
    "$native = [NatesclawPrivatePlanFile]::Open($stagingPath, $finalPath, $sddl, [ref]$openError)",
    "$errorCode = $openError",

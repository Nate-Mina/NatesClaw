...
function runInstallShell(script: string, env: NodeJS.ProcessEnv = {}) {
  const home = mkdtempSync(join(tmpdir(), "natesclaw-install-home-"));
  try {
...
describe("install.sh", () => {
  ...
  it("runs installer snippets without inherited shell startup files", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-shell-env-"));
    const bashEnvPath = join(tmp, "bash_env");
    writeFileSync(bashEnvPath, "export NATESCLAW_BASH_ENV_LEAKED=1\n");

    try {
      const result = runInstallShell('printf "leaked=%s\\n" "${NATESCLAW_BASH_ENV_LEAKED:-0}"', {
  ...
  it("removes a downloaded script temp file when remote execution fails", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-remote-cleanup-"));
  ...
  it("rejects malformed managed scripts without rendering their content", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-script-validation-"));
  ...
  it("does not execute a shebang-prefixed partial file after download failure", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-partial-download-"));
  ...
  it("denies redirects for managed script downloads", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-managed-download-"));
  ...
  it.each(["apt-get", "dnf", "yum"])(
    ...
    (packageManager) => {
      const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-nodesource-validation-"));
...
NODE
      ...
      wrapper="$HOME/.local/bin/natesclaw"
  ...
  it("installs Git with apk on Alpine", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-git-apk-"));
  ...
  it("does not select apk Git on non-Alpine hosts", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-git-native-"));
    const bin = join(tmp, "bin");
    const apkLog = join(tmp, "apk-args.txt");
    mkdirSync(bin, { recursive: true });
    const fakeApk = join(bin, "apk");
  ...
  it("does not emit --before when raw user npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-npmrc-"));
    const bin = join(tmp, "bin");
    ...
    try {
      const result = runInstallShell(
        [
          ...
          `source ${JSON.stringify(SCRIPT_PATH)}`,
          `run_npm_global_install natesclaw@latest ${JSON.stringify(join(tmp, "install.log"))}`,
  ...
  it("does not emit --before when default global npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-global-npmrc-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("does not emit --before when builtin npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-builtin-npmrc-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("uses NATESCLAW_HOME for git defaults", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-home-"));
  ...
  it("does not treat OS HOME config as active when NATESCLAW_HOME is set", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-legacy-config-"));
    const osHome = join(tmp, "os-home");
    const natesclawHome = join(tmp, "natesclaw-home");
    const legacyConfigDir = join(osHome, ".natesclaw");
    mkdirSync(legacyConfigDir, { recursive: true });
    mkdirSync(natesclawHome, { recursive: true });
    writeFileSync(join(legacyConfigDir, "natesclaw.json"), "{}\n");
    ...
    try {
      result = runInstallShell(
        [
          `cd ${JSON.stringify(process.cwd())}`,
          `source ${JSON.stringify(SCRIPT_PATH)}`,
          'if has_natesclaw_config; then printf "configured=1\\n"; else printf "configured=0\\n"; fi',
        ].join("\n"),
        {
          HOME: osHome,
          NATESCLAW_HOME: natesclawHome,
  ...
  it.each(["natesclaw.json", "clawdbot.json"])(
    "detects %s under NATESCLAW_STATE_DIR",
    (configName) => {
      const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-state-config-"));
  ...
  it("does not fall back to home config when NATESCLAW_STATE_DIR is set", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-state-override-"));
    const home = join(tmp, "home");
    const stateDir = join(tmp, "state");
    mkdirSync(join(home, ".natesclaw"), { recursive: true });
    mkdirSync(stateDir, { recursive: true });
    writeFileSync(join(home, ".natesclaw", "natesclaw.json"), "{}\n");
  ...
  it.each([
    {
      expected: /No TTY; run .*\/\.local\/bin\/natesclaw onboard to finish setup/,
      name: "starts setup",
      noOnboard: 0,
    },
    {
      expected: /Skipping onboard .*run .*\/\.local\/bin\/natesclaw onboard later/,
    ...
    ({ expected, noOnboard }) => {
      ...
      detect_natesclaw_checkout() { return 1; }
      show_install_plan() { :; }
      check_existing_natesclaw() { return 0; }
      ...
      install_natesclaw_from_git() {
        mkdir -p "$HOME/.local/bin"
        printf '#!/bin/sh\\nexit 0\\n' > "$HOME/.local/bin/natesclaw"
        chmod +x "$HOME/.local/bin/natesclaw"
        export PATH="$HOME/.local/bin:$PATH"
      }
      resolve_natesclaw_bin() { printf '%s\\n' "$HOME/.local/bin/natesclaw"; }
      ...
      resolve_natesclaw_version() { printf 'test-version\\n'; }
    ...
    `);
      ...
      expect(result.stdout).toMatch(/Update command:.*\/\.local\/bin\/natesclaw update/);
  ...
  it("honors --verify for an unconfigured install without a TTY", () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stdout).toMatch(/No TTY; run .*\/\.local\/bin\/natesclaw onboard to finish setup/);
  });
  ...
  it("does not emit before args when npmrc min-release-age computes a before cutoff", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-npm-freshness-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("ignores project npmrc when choosing global install freshness args", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-global-freshness-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("bounds installer npm prefix probes during finalization helpers", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-npm-probe-"));
    ...
    try {
      const result = runInstallShell(
        ...
        {
          NATESCLAW_INSTALL_PROBE_TIMEOUT_SECONDS: "0.1",
          PATH: `${tmp}:${process.env.PATH ?? ""}`,
        },
      );

      expect(result.status).toBe(0);
      expect(result.stdout.trim()).toBe("/tmp/natesclaw-npm/bin");
  ...
  it("bounds daemon status probes during finalization helpers", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-probe-"));
    const claw = join(tmp, "natesclaw");
    writeFileSync(
      claw,
      [
        "#!/usr/bin/env bash",
    ...
    try {
      const result = runInstallShell(
        ...
        { NATESCLAW_INSTALL_PROBE_TIMEOUT_SECONDS: "0.01" },
      );

      expect(result.status).toBe(0);
      expect(result.stdout.trim()).toBe("not-loaded");
      expect(result.stderr).toContain(
        "timed out during installer finalization probe: natesclaw daemon status --json",
      );
    } finally {
      rmSync(tmp, { force: true, recursive: true });
    }
  ...
  it("loads nvm before checking Node.js so stale system Node does not win", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-nvm-"));
    const home = join(tmp, "home");
  ...
  it("promotes a supported Linux Node binary over stale PATH entries", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-node-promote-"));
  ...
  it("uses the package engine range when accepting existing Node runtimes", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-node-floor-"));
    const bin = join(tmp, "bin");
    mkdirSync(bin, { recursive: true });
  ...
  it("persists a supported Linux Node path before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-linux-node-path-"));
    const home = join(tmp, "home");
  ...
  it("warns before redirecting an unwritable npm prefix", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-npm-prefix-"));
    const home = join(tmp, "home");
    ...
    expect(result?.stdout).toContain("npm i -g natesclaw@latest");
  ...
  it("persists npm prefix PATH before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-npm-prefix-shell-"));
    const home = join(tmp, "home");
    mkdirSync(home, { recursive: true });
    writeFileSync(
      join(home, ".bashrc"),
  ...
  it("uses a quoted absolute natesclaw path in follow-up commands when npm bin is not on the original PATH", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-command-"));
    ...
    const natesclawBin = join(npmBin, "natesclaw");
    const staleNatesclawBin = join(staleBin, "natesclaw");
    writeFileSync(natesclawBin, "#!/bin/sh\nexit 0\n");
    writeFileSync(staleNatesclawBin, "#!/bin/sh\nexit 0\n");
    chmodSync(natesclawBin, 0o755);
    chmodSync(staleNatesclawBin, 0o755);
    ...
    try {
      result = runInstallShell(`
        ...
        printf 'missing=%s\\n' "$(natesclaw_command_for_user "${natesclawBin}")"
    ...
    expect(result?.status).toBe(0);
    expect(result?.stdout).toContain(`missing=${natesclawBin.replace(/ /g, "\\ ")}`);
    expect(result?.stdout).toContain("present=natesclaw");
    expect(result?.stdout).toContain(`shadowed=${natesclawBin.replace(/ /g, "\\ ")}`);
  });

  it("prefers the binary owned by the completed install method over stale PATH entries", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-selected-bin-"));
    const home = join(tmp, "home");
    const npmBin = join(tmp, "npm-bin");
    const staleBin = join(tmp, "stale-bin");
    ...
    for (const bin of [
      join(npmBin, "natesclaw"),
      join(staleBin, "natesclaw"),
      join(gitBin, "natesclaw"),
    ]) {
    ...
    expect(result?.status).toBe(0);
    expect(result?.stdout).toContain(`git=${join(gitBin, "natesclaw")}`);
    expect(result?.stdout).toContain(`npm=${join(npmBin, "natesclaw")}`);
  });

  it("uses the selected binary in gateway recovery guidance", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-gateway-guidance-"));
    const currentBin = join(tmp, "current bin");
    const staleBin = join(tmp, "stale-bin");
    mkdirSync(currentBin, { recursive: true });
    mkdirSync(staleBin, { recursive: true });
    const natesclawBin = join(currentBin, "natesclaw");
    writeFileSync(natesclawBin, "#!/bin/sh\nexit 0\n");
    writeFileSync(join(staleBin, "natesclaw"), "#!/bin/sh\nexit 0\n");
    chmodSync(natesclawBin, 0o755);
    chmodSync(join(staleBin, "natesclaw"), 0o755);
    ...
    try {
      result = runInstallShell(`
        set -euo pipefail
        source "${SCRIPT_PATH}"
        NATESCLAW_BIN=${JSON.stringify(natesclawBin)}
    ...
    const quotedBin = natesclawBin.replace(/ /g, "\\ ");
    expect(result?.status).toBe(0);
  ...
  it("uses the repo Corepack pnpm when a global pnpm version is already present", () => {
    const tmp = mkdtempSync(join(tmpdir(), "natesclaw-install-pnpm-version-"));
    const bin = join(tmp, "bin");
...
describe("install.sh macOS Homebrew Node behavior", () => {
  ...
  it("reruns spinner-wrapped commands when gum reports ioctl failure", () => {
    const dir = mkdtempSync(join(tmpdir(), "natesclaw-install-sh-gum-"));
    try {
  ...
  it("gum spin preserves terminal stdin for direct interactive installs", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "natesclaw-install-sh-gum-stdin-"));
    try {
      const gumPath = join(dir, "gum");
      const commandPath = join(dir, "command");
...
describe("install.sh duplicate Natesclaw install detection", () => {
  it("warns with concrete package paths and versions for duplicate npm roots", () => {
    const result = runInstallShell(`
      ...
      mkdir -p "$root/brew/natesclaw" "$root/fnm/natesclaw"
      printf '{"version":"2026.3.7"}\\n' > "$root/brew/natesclaw/package.json"
      printf '{"version":"2026.3.1"}\\n' > "$root/fnm/natesclaw/package.json"
      collect_natesclaw_npm_root_candidates() { printf '%s\\n' "$root/brew" "$root/fnm"; }
      NATESCLAW_BIN="$root/fnm/.bin/natesclaw"
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_natesclaw_global_installs
    `);

    expect(result.status).toBe(0);
    expect(result.stdout).toContain("Multiple Natesclaw global installs detected");
    expect(result.stdout).toContain("2026.3.7");
    expect(result.stdout).toContain("2026.3.1");
    expect(result.stdout).toContain("/brew/natesclaw");
    expect(result.stdout).toContain("/fnm/natesclaw");
    expect(result.stdout).toContain("Active natesclaw:");
    expect(result.stdout).toContain("npm uninstall -g natesclaw");
  });

  it("stays quiet when only one Natesclaw npm root exists", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      root="$(mktemp -d)"
      trap 'rm -rf "$root"' EXIT
      mkdir -p "$root/only/natesclaw"
      printf '{"version":"2026.3.7"}\\n' > "$root/only/natesclaw/package.json"
      collect_natesclaw_npm_root_candidates() { printf '%s\\n' "$root/only"; }
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_natesclaw_global_installs
    `);
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).not.toContain("Multiple Natesclaw global installs detected");
  });
  ...
  it("run_quiet_step redirects stdin to /dev/null in piped context", () => {
    const dir = mkdtempSync(join(tmpdir(), "natesclaw-stdin-test-"));
  ...
  it("pipe data leaks to child when stdin is not isolated (counterproof)", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "natesclaw-stdin-leak-"));
    const marker = join(dir, "stdin-state");
    try {
      const result = spawnSync(
        "bash",
  ...
  it("run_quiet_step blocks cat from reading pipe data", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "natesclaw-stdin-cat-"));
    const marker = join(dir, "stdin-state");
    try {
      const result = spawnSync(
        "bash",

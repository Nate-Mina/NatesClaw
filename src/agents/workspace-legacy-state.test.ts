...
describe("legacy workspace reset cleanup", () => {
  ...
  function setup() {
    const homeDir = tempDirs.make("natesclaw-workspace-legacy-cleanup-");
    const stateDir = path.join(homeDir, "state");
    const workspaceDir = path.join(homeDir, "workspace");
    const env = { ...process.env, HOME: homeDir, NATESCLAW_STATE_DIR: stateDir };
  ...
  it("removes canonical legacy paths after the configured symlink is removed", async () => {
    ...
    const identity = resolveWorkspaceStateIdentity(targetDir);
    const setupPath = path.join(targetDir, "natesclaw-workspace-state.json");

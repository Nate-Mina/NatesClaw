...
read_when:
  - You want an Natesclaw agent to join a Microsoft Teams meeting
  - You are configuring Chrome or virtual audio for Teams meeting talk-back
title: "Microsoft Teams meetings plugin"
---

The `teams-meetings` plugin joins Microsoft Teams links as a guest in the Natesclaw Chrome profile. It accepts work links under `teams.microsoft.com/l/meetup-join/...` and consumer links under `teams.live.com/meet/...`. It does not create meetings, dial in, call Microsoft Graph, or capture audio/video recordings.
  ...
  plugins: {
    entries: {
      "teams-meetings": {
        config: {
          defaultMode: "agent",
          chrome: { guestName: "Natesclaw Agent" },
        },
      },
    },
  },
...
Run `natesclaw plugins disable teams-meetings` if you do not want the plugin active.

```bash
natesclaw teamsmeetings setup
natesclaw teamsmeetings join 'https://teams.microsoft.com/l/meetup-join/...'
```
...
| Mode         | Behavior                                                                    |
| ------------ | --------------------------------------------------------------------------- |
| `agent`      | Realtime transcription consults the configured Natesclaw agent; TTS replies. |
...
Teams live captions are enabled after admission in every mode so Natesclaw can
persist speaker-attributed notes. The `transcript` action still returns the
bounded live buffer only for `transcribe` sessions. On leave, Natesclaw stores
the durable transcript and derived summary in the shared state database; list
or export them with [`natesclaw transcripts`](/cli/transcripts).

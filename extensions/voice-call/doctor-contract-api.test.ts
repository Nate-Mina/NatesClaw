...
describe("voice-call doctor state migration", () => {
  ...
  beforeAll(async () => {
    resetPluginStateStoreForTests();
    const warmStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-voice-call-doctor-"));
    const warmStorePath = createTestStorePath();
    const warmEnv = {
      ...process.env,
      HOME: warmStateDir,
      NATESCLAW_STATE_DIR: warmStateDir,
    };
    try {
      installStateRuntime();
  ...
  beforeEach(async () => {
    resetPluginStateStoreForTests();
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-voice-call-doctor-"));
  ...
  it("reports top-level and per-number session-store agents", () => {
    ...
    expect(
      resolveSessionStoreAgentIds({
        cfg: {
          plugins: { entries: { "@natesclaw/voice-call": { config: {} } } },
        },
      }),
    ).toEqual(["main"]);
    expect(
  ...
  it("honors NATESCLAW_STATE_DIR for the default store", async () => {
  ...
  it("repairs the plugin-local SQLite schema without a legacy call log", async () => {
    const databasePath = path.join(storePath, "state", "natesclaw.sqlite");

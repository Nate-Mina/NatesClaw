...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "natesclaw";
...
function formatBrowserDoctorGatewayError(error: unknown): string {
  ...
  return "Gateway auth SecretRef is unavailable in this command path; browser doctor cannot reach the admin-scoped browser.request endpoint. Set NATESCLAW_GATEWAY_TOKEN or NATESCLAW_GATEWAY_PASSWORD, then retry.";
}
...
async function runBrowserDoctor(parent: BrowserParentOpts, profile?: string, deep?: boolean) {
  ...
  checks.push({
    name: "profile",
    ok: true,
    detail: `${status.profile ?? "natesclaw"} (${usesChromeMcpTransport(status) ? "chrome-mcp" : (status.transport ?? "cdp")})`,
  });
  checks.push({
    name: "browser",
    ok: status.running,
    detail: status.running
      ...
      : "not running; run `natesclaw browser start`",
  });
  if (status.graphics) {
    checks.push({
      name: "graphics",
...
type BrowserProfileDriver = "natesclaw" | "existing-session" | "extension";
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "natesclaw"}`,
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "natesclaw" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (natesclaw|existing-session). Default: natesclaw")
    .action(
      async (
        opts: {
          name: string;
        ...
        await runBrowserCommand(async () => {
          if (
            ...
            opts.driver !== "natesclaw" &&
            opts.driver !== "existing-session"
          ) {
            throw new Error("--driver must be natesclaw or existing-session");
          }

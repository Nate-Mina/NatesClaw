    ...
    private var connectStep: some View {
        if let selectedMode {
            Section {
                ...
                Text("Status")
                    .font(NatesclawType.footnoteSemiBold)
            }
    ...
    private var authStep: some View {
        Section {
            ...
            if let problem = self.currentProblem {
                ...
                Text("Natesclaw is checking gateway and node access.")
                    .font(NatesclawType.footnote)
                    .foregroundStyle(.secondary)
            }
        } header: {
        ...
        if self.issue.needsPairing {
            Section {
                ...
                let requestLine: String = {
                    ...
                    return String(localized: "Request ID: check `natesclaw devices list`.")
                }()
                let commandLine = self.currentProblem?.actionCommand ?? "natesclaw devices approve <requestId>"
...
extension OnboardingWizardView {
    ...
    private func initializeState() {
        if self.manualHost.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
               ...
               let port = active.port
            {
                ...
                self.manualHost = "natesclaw.local"
        ...
        if self.selectedMode == .developerLocal, self.manualHost == "natesclaw.local" {
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "natesclaw.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "natesclaw.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "natesclaw.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:

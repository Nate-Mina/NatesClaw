 ...
 * silk-wasm availability checks — all without importing `natesclaw/plugin-sdk`.
 ...
 * `NATESCLAW_HOME`. QQ Bot still checks this tree for legacy state imports and
...
/**
 * Resolve the effective Natesclaw home directory.
 *
 * Mirrors the contract from core (`src/infra/home-dir.ts::resolveEffectiveHomeDir`)
 * so QQ Bot media roots live under the same tree the rest of Natesclaw treats as
 * `~`. The extension cannot import the core helper directly (it is a separate
 * package with `natesclaw` as a peer dependency), so this re-implements the
 * minimal contract:
 *
 * 1. `NATESCLAW_HOME` when set (with `~` / `~/...` expanded against the OS home).
 ...
 */
function resolveNatesclawHome(): string {
  const raw = process.env.NATESCLAW_HOME?.trim();
...
/**
 * Return a legacy path under `~/.natesclaw/qqbot` without creating it.
 *
...
function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".natesclaw", "qqbot", ...subPaths);
}

/** Return a path under `~/.natesclaw/qqbot`, creating it on demand. */
...
/**
 * Return a path under `<natesclaw-home>/.natesclaw/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under Natesclaw's core media allowlist
 * so downloaded images and audio can be accessed by framework media tooling.
 * The base honors `NATESCLAW_HOME` (when set) so files written by agents into
 * the Natesclaw-managed media tree are reachable by this plugin even when
 * `HOME` and `NATESCLAW_HOME` differ (Docker, multi-user hosts). Fixes #83562.
 */
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(resolveNatesclawHome(), ".natesclaw", "media", "qqbot", ...subPaths);
}

/** Return a path under `<natesclaw-home>/.natesclaw/media/qqbot`, creating it on demand. */
...
/**
 * Return `<natesclaw-home>/.natesclaw/media`, Natesclaw's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `natesclaw/src/media/local-roots.ts`). Using it as a
 ...
 * {@link getQQBotMediaPath}, the base honors `NATESCLAW_HOME`.
 */
function getNatesclawMediaDir(): string {
  return path.join(resolveNatesclawHome(), ".natesclaw", "media");
}
...
function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const natesclawHomeDir = resolveNatesclawHome();
  const mediaRoot = getQQBotMediaPath();
  const dataRoot = getQQBotDataPath();
  // When NATESCLAW_HOME differs from HOME we have to consider workspace roots
  // under both trees: agents may be configured with `~`-relative paths (HOME)
  // or with the Natesclaw-managed home tree. Deduplicate when they match.
  const workspaceRoots = Array.from(
    new Set([
      path.join(osHomeDir, ".natesclaw", "workspace", "qqbot"),
      path.join(natesclawHomeDir, ".natesclaw", "workspace", "qqbot"),
    ]),
  );
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and Natesclaw's shared `~/.natesclaw/media`
  ...
  const allowedRoots = [getNatesclawMediaDir(), getQQBotMediaPath()];

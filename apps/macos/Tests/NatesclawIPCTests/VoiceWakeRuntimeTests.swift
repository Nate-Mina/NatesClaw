...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["natesclaw"]
        let text = "hey natesclaw"
    ...
    @Test func `trigger only rejects trailing wake word mentions in ordinary speech`() {
        let triggers = ["natesclaw"]
        let text = "tell me about natesclaw"
        #expect(!VoiceWakeRuntime._testIsTriggerOnly(text, triggers: triggers))
    }
    ...
    @Test func `matched trigger finds trigger not at transcript start`() {
        let triggers = ["natesclaw"]
        let text = "uh natesclaw"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "natesclaw")
    }
    ...
    @Test func `matched trigger prefers most specific overlapping phrase`() {
        let triggers = ["natesclaw", "hey natesclaw"]
        let text = "hey natesclaw"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "hey natesclaw")
    }
    ...
    @Test func `text only fallback populates matched trigger`() {
        let transcript = "hey natesclaw do thing"
        let config = WakeWordGateConfig(triggers: ["natesclaw"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["natesclaw"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "natesclaw")
    }

    @Test func `text only fallback keeps the first trigger phrase when later words match another trigger`() {
        let transcript = "natesclaw tell me about computer vision"
        let config = WakeWordGateConfig(triggers: ["natesclaw", "computer"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["natesclaw", "computer"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "natesclaw")
    }
    ...
    @Test func `gate requires gap between trigger and command`() {
        ...
        let config = WakeWordGateConfig(triggers: ["natesclaw"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "natesclaw", start: 0.2, duration: 0.1, range: transcript.range(of: "natesclaw")),

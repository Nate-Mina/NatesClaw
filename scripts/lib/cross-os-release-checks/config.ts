...
export const CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS = parsePositiveIntegerEnv(
  "NATESCLAW_CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS",
  600,
);
...
export const CROSS_OS_PROCESS_TREE_KILL_AFTER_MS = parsePositiveIntegerEnv(
  "NATESCLAW_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS",
  15_000,
);
...
export function resolveProviderConfig(provider: string, env = process.env): ProviderConfig | null {
  ...
  const providerEnvKey = `NATESCLAW_CROSS_OS_${provider.toUpperCase().replace(/[^A-Z0-9]+/gu, "_")}_MODEL`;
  const model = env[providerEnvKey]?.trim() || env.NATESCLAW_CROSS_OS_MODEL?.trim() || config.model;
  return { ...config, model };
}
...
export function buildReleaseProviderConfigOverride(providerMeta: ProviderConfig) {
  ...
  return {
    ...
    ...(providerMeta.extensionId === "openai" ? { agentRuntime: { id: "natesclaw" } } : {}),
    models: [],
...
export const INSTALL_STAGE_DEBRIS_DIR_PATTERN = /^\.natesclaw-install-stage(?:-[^/]+)?$/iu;
...
export const CROSS_OS_DISCORD_FETCH_TIMEOUT_MS = parsePositiveIntegerEnv(
  "NATESCLAW_CROSS_OS_DISCORD_FETCH_TIMEOUT_MS",
  10_000,
);
...
export const CROSS_OS_COMMAND_HEARTBEAT_SECONDS = parsePositiveIntegerEnv(
  "NATESCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS",
  60,
);
...
export function resolveCrossOsAgentTurnOptional(env = process.env) {
  return parseBooleanEnv("NATESCLAW_CROSS_OS_AGENT_TURN_OPTIONAL", false, env);
}
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.NATESCLAW_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.NATESCLAW_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.NATESCLAW_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
export function buildRealUpdateEnv(env: NodeJS.ProcessEnv) {
  ...
  delete updateEnv.NATESCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function isRecoverableWindowsPackagedUpgradeTimeoutError(
  ...
  return (
    ...
    /[/\\]natesclaw\.mjs update --tag http:\/\/127\.0\.0\.1:\d+\/natesclaw[^/\s]*\.tgz --yes --json(?: --no-restart)? --timeout \d+/u.test(
      message,
    )
  );
}
...
export function resolveExplicitBaselineVersion(baselineSpec: string) {
  ...
  if (!trimmed || trimmed === "natesclaw@latest") {
    return "";
  }
  if (trimmed.startsWith("natesclaw@")) {
    return trimmed.slice("natesclaw@".length);
  }
  return trimmed;
}

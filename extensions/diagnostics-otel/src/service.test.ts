...
const detectResourcesMock = vi.hoisted(() =>
  vi.fn((_options: { detectors?: unknown[] }) => ({
    attributes: { "natesclaw.test.detected": "1" },
    merge: vi.fn((configured: { attributes?: Record<string, unknown> }) => ({
      attributes: {
        "natesclaw.test.detected": "1",
...
vi.mock("natesclaw/plugin-sdk/fetch-runtime", () => ({
...
const ORIGINAL_NATESCLAW_OTEL_PRELOADED = process.env.NATESCLAW_OTEL_PRELOADED;
...
afterAll(() => {
  ...
  vi.doUnmock("natesclaw/plugin-sdk/fetch-runtime");
...
describe("diagnostics-otel service", () => {
  beforeEach(() => {
    resetDiagnosticEventsForTest();
    delete process.env.NATESCLAW_OTEL_PRELOADED;
  ...
  afterEach(async () => {
    ...
    resetDiagnosticEventsForTest();
    if (ORIGINAL_NATESCLAW_OTEL_PRELOADED === undefined) {
      delete process.env.NATESCLAW_OTEL_PRELOADED;
    } else {
      process.env.NATESCLAW_OTEL_PRELOADED = ORIGINAL_NATESCLAW_OTEL_PRELOADED;
    }
  ...
  test("drops camelCase and snake_case diagnostic id log attributes before export", async () => {
    ...
    expect(emitCall.attributes?.["natesclaw.provider"]).toBe("openai");
    for (const key of [
      "natesclaw.callId",
      "natesclaw.call_id",
      "natesclaw.chatId",
  ...
  test.each([
    {
      ...
      expectedTokenName: "natesclaw.tokens",
      expectedDurationName: "natesclaw.run.duration_ms",
    },
    {
    ...
    {
      metricNamePrefix: "acme.natesclaw.",
      expectedTokenName: "acme.natesclaw.tokens",
      expectedDurationName: "acme.natesclaw.run.duration_ms",
    },
  ])(
    "replaces the default Natesclaw metric prefix with $metricNamePrefix",
    async ({ metricNamePrefix, expectedTokenName, expectedDurationName }) => {
      ...
      expect(telemetryState.counters.has("natesclaw.tokens")).toBe(
        expectedTokenName === "natesclaw.tokens",
      );
    },
  );
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("natesclaw.webhook.received")?.add).toHaveBeenCalledWith(1, {
      "natesclaw.channel": "telegram",
      "natesclaw.webhook": "telegram-post",
    });
    expect(
      telemetryState.histograms.get("natesclaw.webhook.duration_ms")?.record,
    ).toHaveBeenCalledWith(120, {
      "natesclaw.channel": "telegram",
      "natesclaw.webhook": "telegram-post",
    });
    expect(telemetryState.counters.get("natesclaw.message.queued")?.add).toHaveBeenCalledWith(1, {
      "natesclaw.channel": "telegram",
      "natesclaw.source": "telegram",
    });
    expect(telemetryState.histograms.get("natesclaw.queue.depth")?.record).toHaveBeenCalledTimes(2);
    expect(telemetryState.histograms.get("natesclaw.queue.depth")?.record).toHaveBeenCalledWith(2, {
      "natesclaw.channel": "telegram",
      "natesclaw.source": "telegram",
    });
    expect(telemetryState.histograms.get("natesclaw.queue.depth")?.record).toHaveBeenCalledWith(3, {
      "natesclaw.lane": "main",
    });
    expect(telemetryState.counters.get("natesclaw.message.processed")?.add).toHaveBeenCalledWith(1, {
      "natesclaw.channel": "telegram",
      "natesclaw.outcome": "completed",
    });
    expect(telemetryState.counters.get("natesclaw.message.received")?.add).toHaveBeenCalledWith(1, {
      "natesclaw.channel": "telegram",
      "natesclaw.source": "webhook",
    });
    expect(telemetryState.counters.get("natesclaw.message.received")?.add).toHaveBeenCalledWith(1, {
      "natesclaw.channel": "unknown",
      "natesclaw.source": "unknown",
    });
    expect(
      telemetryState.counters.get("natesclaw.message.dispatch.started")?.add,
    ...
    expect(
      telemetryState.counters.get("natesclaw.message.dispatch.completed")?.add,
    ...
    expect(
      ...
      expect.objectContaining({
        "natesclaw.channel": "unknown",
        "natesclaw.reason": "none",
        "natesclaw.source": "unknown",
      }),
    );
    expect(
      telemetryState.histograms.get("natesclaw.message.dispatch.duration_ms")?.record,
    ).toHaveBeenCalledWith(
      25,
      expect.objectContaining({
        "natesclaw.channel": "telegram",
    ...
    expect(
      telemetryState.histograms.get("natesclaw.message.duration_ms")?.record,
    ).toHaveBeenCalledWith(55, {
      "natesclaw.channel": "telegram",
      "natesclaw.outcome": "completed",
    });
    expect(telemetryState.histograms.get("natesclaw.queue.wait_ms")?.record).toHaveBeenCalledWith(
      10,
      {
        "natesclaw.lane": "main",
      },
    );
    expect(telemetryState.counters.get("natesclaw.session.stuck")?.add).toHaveBeenCalledTimes(1);
    expect(telemetryState.counters.get("natesclaw.session.stuck")?.add).toHaveBeenCalledWith(1, {
      "natesclaw.state": "processing",
    });
    expect(
      telemetryState.histograms.get("natesclaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalledWith(125_000, {
      "natesclaw.state": "processing",
    });
    expect(telemetryState.counters.get("natesclaw.run.attempt")?.add).toHaveBeenCalledWith(1, {
      "natesclaw.attempt": 2,
    });
    ...
    expect(telemetryState.counters.get("natesclaw.session.turn.created")?.add).toHaveBeenCalledWith(
      1,
      {
        "natesclaw.agent": "agent.default",
        "natesclaw.channel": "telegram",
        "natesclaw.trigger": "user",
      },
    );

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("natesclaw.webhook.processed");
    expect(spanNames).toContain("natesclaw.message.processed");
    expect(spanNames).toContain("natesclaw.session.stuck");
    const webhookSpanOptions = startedSpanOptions("natesclaw.webhook.processed");
    expect(webhookSpanOptions?.attributes).not.toHaveProperty("natesclaw.chatId");
    expect(webhookSpanOptions?.startTime).toBeTypeOf("number");
    const messageSpanOptions = startedSpanOptions("natesclaw.message.processed");
    expect(messageSpanOptions?.attributes?.["natesclaw.channel"]).toBe("telegram");
    expect(messageSpanOptions?.attributes?.["natesclaw.outcome"]).toBe("completed");
    expect(messageSpanOptions?.attributes?.["natesclaw.reason"]).toBe("unknown");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("natesclaw.chatId");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("natesclaw.messageId");
  ...
  test("surfaces bridge cleanup failure after attempting every shutdown phase", async () => {
    ...
    await service.start(ctx);
    emitRunStarted();
    const runSpan = spanByName("natesclaw.run");
  ...
  test("uses a preloaded OpenTelemetry SDK without dropping diagnostic listeners", async () => {
    process.env.NATESCLAW_OTEL_PRELOADED = "1";
    const { service, ctx } = await startOtelService({ traces: true, metrics: true, logs: true });
    ...
    const runDurationRecordCall = lastHistogramRecord("natesclaw.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    const runDurationAttributes = runDurationRecordCall?.[1];
    expect(runDurationAttributes?.["natesclaw.provider"]).toBe("openai");
    expect(runDurationAttributes?.["natesclaw.model"]).toBe("gpt-5.4");
    const runSpanOptions = startedSpanOptions("natesclaw.run");
    expect(runSpanOptions?.attributes?.["natesclaw.outcome"]).toBe("completed");
    expect(logEmit).toHaveBeenCalled();
  ...
  test("emits and records bounded telemetry exporter health events", async () => {
    ...
    expect(
      telemetryState.counters.get("natesclaw.telemetry.exporter.events")?.add,
    ).toHaveBeenCalledWith(1, {
      "natesclaw.exporter": "diagnostics-otel",
      "natesclaw.signal": "logs",
      "natesclaw.status": "started",
      "natesclaw.reason": "configured",
    });
  ...
  test.each([" TRUE ", "TrUe"])(
    "disables every Natesclaw-owned telemetry route for OTEL_SDK_DISABLED=%j",
    async (value) => {
      const events: TelemetryExporterEvent[] = [];
      const unsubscribe = onInternalDiagnosticEvent((event) => {
        if (event.type === "telemetry.exporter") {
  ...
  test("preserves preloaded trace and metric ownership while disabling plugin logs", async () => {
    ...
    expect(meterProviderCtor).not.toHaveBeenCalled();
    expect(unhandledRejectionHandlerState.register).not.toHaveBeenCalled();
    expect(lastHistogramRecord("natesclaw.run.duration_ms")?.[0]).toBe(100);
    expect(startedSpanOptions("natesclaw.run")?.attributes?.["natesclaw.outcome"]).toBe("completed");
    expect(logEmit).not.toHaveBeenCalled();
  ...
  test("exports trusted security events as bounded OTLP logs", async () => {
    ...
    expect(emitCall.body).toBe("natesclaw.security.event");
    expect(emitCall.severityText).toBe("WARN");
    expect(emitCall.severityNumber).toBe(13);
    expect(emitCall.attributes).toMatchObject({
      "natesclaw.security.event_id": "security-event-1",
      "natesclaw.security.category": "tool",
      "natesclaw.security.action": "tool.execution.blocked",
      "natesclaw.security.outcome": "denied",
      "natesclaw.security.severity": "medium",
      "natesclaw.security.reason": "tools.deny",
      "natesclaw.security.actor.kind": "agent",
      "natesclaw.security.actor.id_hash": "agent-hash-1",
      "natesclaw.security.actor.role": "operator",
      "natesclaw.security.actor.scopes": "operator.read,operator.approvals",
      "natesclaw.security.target.kind": "plugin",
      "natesclaw.security.target.name": "@acme/security-event-plugin",
      "natesclaw.security.target.owner": "plugin-installer",
      "natesclaw.security.policy.id": "tools.exec",
      "natesclaw.security.policy.decision": "deny",
      "natesclaw.security.policy.reason": "allowlist.miss",
      "natesclaw.security.control.id": "exec-approval",
      "natesclaw.security.control.family": "approval",
      "natesclaw.security.attribute.params_kind": "object",
      "natesclaw.security.attribute.secretish": "unknown",
    });
    ...
    expect(Object.hasOwn(emitCall.attributes ?? {}, "natesclaw.security.attribute.__proto__")).toBe(
      false,
    );
  ...
  test("keeps explicit HTTP exporters canonical when ambient protocol is gRPC", async () => {
    ...
    emitDiagnosticEvent({
      type: "log.record",
      level: "INFO",
      message: "Natesclaw-owned OTLP log",
    });
    await flushDiagnosticEvents();
  ...
  test("exports trusted security events as stdout JSONL logs", async () => {
    ...
    try {
      ...
      expect(record.body).toBe("natesclaw.security.event");
      expect(record.severityText).toBe("WARN");
      expect(record.severityNumber).toBe(13);
      expect(record.attributes).toMatchObject({
        "natesclaw.security.event_id": "security-event-stdout",
        "natesclaw.security.category": "tool",
        "natesclaw.security.action": "tool.execution.blocked",
        "natesclaw.security.outcome": "denied",
        "natesclaw.security.severity": "medium",
      ...
      expect(Object.hasOwn(record.attributes ?? {}, "natesclaw.security.attribute.__proto__")).toBe(
        false,
      );
  ...
  test("records liveness warning diagnostics", async () => {
    ...
    expect(telemetryState.counters.get("natesclaw.liveness.warning")?.add).toHaveBeenCalledWith(1, {
      "natesclaw.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("natesclaw.liveness.event_loop_delay_p99_ms")?.record,
    ).toHaveBeenCalledWith(250, {
      "natesclaw.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("natesclaw.liveness.cpu_core_ratio")?.record,
    ).toHaveBeenCalledWith(1.4, {
      "natesclaw.liveness.reason": "event_loop_delay:cpu",
    });
    const livenessSpanOptions = startedSpanOptions("natesclaw.liveness.warning");
    expect(livenessSpanOptions?.attributes?.["natesclaw.liveness.reason"]).toBe(
      "event_loop_delay:cpu",
    );
    expect(livenessSpanOptions?.attributes?.["natesclaw.liveness.active"]).toBe(2);
    expect(livenessSpanOptions?.attributes?.["natesclaw.liveness.queued"]).toBe(4);
    const span = telemetryState.spans.find((item) => item.name === "natesclaw.liveness.warning");
  ...
  test("records oversized payload metrics without raw identifiers", async () => {
    ...
    expect(telemetryState.counters.get("natesclaw.payload.large")?.add).toHaveBeenCalledWith(1, {
      "natesclaw.payload.action": "rejected",
      "natesclaw.payload.surface": "gateway.frame",
      "natesclaw.channel": "web",
      "natesclaw.plugin": "none",
      "natesclaw.reason": "body-too-large",
    });
    expect(
      telemetryState.histograms.get("natesclaw.payload.large_bytes")?.record,
  ...
  test("reports log exporter emit failures without exporting raw error text", async () => {
    ...
    expect(
      ...
      "natesclaw.exporter": "diagnostics-otel",
      "natesclaw.signal": "logs",
      "natesclaw.status": "failure",
      "natesclaw.reason": "emit_failed",
      "natesclaw.errorCategory": "TypeError",
    });
    expect(
  ...
  test("ignores untrusted telemetry exporter events for OTEL metrics", async () => {
    ...
    telemetryState.counters.get("natesclaw.telemetry.exporter.events")?.add.mockClear();
    emitDiagnosticEvent({
  ...
  test("records hook-blocked run metrics with safe blocker originator", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("natesclaw.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["natesclaw.outcome"]).toBe("blocked");
    expect(runDurationRecordCall?.[1]?.["natesclaw.blocked_by"]).toBe("policy-plugin");
  ...
  test("run.completed error span carries the redacted message off the metric attrs", async () => {
    ...
    expect(startedSpanOptions("natesclaw.run")?.attributes?.["natesclaw.error"]).toBe(
      "upstream model stream stalled then aborted",
    );
    expect(spanByName("natesclaw.run").setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "upstream model stream stalled then aborted",
    });
    // The raw message must never widen metric cardinality.
    const runDuration = lastHistogramRecord("natesclaw.run.duration_ms");
    expect(runDuration?.[1]?.["natesclaw.outcome"]).toBe("error");
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "natesclaw.error")).toBe(false);
  });
  ...
  test("run.completed bounds sensitive error text before export", async () => {
    ...
    const status = mockCallArg(spanByName("natesclaw.run").setStatus, 0) as {
      message?: string;
    };
  ...
  test("harness.run.completed error span carries the redacted message", async () => {
    ...
    expect(startedSpanOptions("natesclaw.harness.run")?.attributes?.["natesclaw.error"]).toBe(
      "model run failed during resolve phase",
    );
    expect(spanByName("natesclaw.harness.run").setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "model run failed during resolve phase",
    });
    const harnessDuration = lastHistogramRecord("natesclaw.harness.duration_ms");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "natesclaw.error")).toBe(false);
  });
  ...
  test("honors disabled traces when an OpenTelemetry SDK is preloaded", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("natesclaw.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["natesclaw.provider"]).toBe("openai");
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalled();
  ...
  test("treats omitted diagnostics enabled flag as enabled", async () => {
    ...
    const attrs = startedSpanOptions("natesclaw.model.call")?.attributes;
    expect(attrs?.["natesclaw.content.input_messages"]).toBe("user prompt");
  });
  ...
  test("lets explicit Natesclaw sampling override the inherited sampler environment", async () => {
  ...
  test("preserves OTLP TLS env options when passing env proxy agents", async () => {
    const certDir = mkdtempSync(path.join(tmpdir(), "natesclaw-otel-tls-"));
    try {
  ...
  test("pins validated collector TLS material on direct HTTPS exporter agents", async () => {
    const certDir = mkdtempSync(path.join(tmpdir(), "natesclaw-otel-direct-tls-"));
    try {
      const rootCertificatePath = path.join(certDir, "root.pem");
  ...
  test("exports diagnostic logs as stdout JSONL without constructing the OTLP log exporter", async () => {
    await startOtelService({
      ...
      captureContent: true,
      configure: (ctx) => {
        ctx.config.diagnostics!.otel!.serviceName = "rovoclaw-natesclaw";
      },
    });
    const stdout = captureStdoutWrites();
    ...
    try {
      ...
      expect(record.signal).toBe("natesclaw.diagnostic.log");
      expect(record["service.name"]).toBe("rovoclaw-natesclaw");
      expect(record.severityText).toBe("WARN");
      expect(record.severityNumber).toBe(13);
      expect(String(record.body)).not.toContain("sk-1234567890abcdef1234567890abcdef");
      expect(String(record.body)).toContain("sk-123");
      expect(record.attributes).toMatchObject({
        "natesclaw.log.level": "WARN",
        "natesclaw.subsystem": "diagnostic",
      });
      const tokenAttr = record.attributes?.["natesclaw.token"];
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["natesclaw.token"];
    expect(tokenAttr).not.toBe("ghp_abcdefghijklmnopqrstuvwxyz123456"); // pragma: allowlist secret
  ...
  test("does not attach untrusted diagnostic trace context to exported logs", async () => {
    ...
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "natesclaw.traceId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "natesclaw.spanId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "natesclaw.traceFlags")).toBe(false);
  ...
  test("bounds plugin-emitted log attributes and omits source paths", async () => {
    ...
    emitDiagnosticEvent({
      ...
      attributes,
      code: {
        filepath: "/Users/alice/natesclaw/src/private.ts",
        line: 42,
        functionName: "handler",
        location: "/Users/alice/natesclaw/src/private.ts:42",
      },
    ...
    expect(emitCall.attributes["natesclaw.good"]).toBe(`${"y".repeat(4095)}...(truncated)`);
    expect(emitCall.attributes["code.lineno"]).toBe(42);
    expect(emitCall.attributes["code.function"]).toBe("handler");
    expect(Object.hasOwn(emitCall.attributes, `natesclaw.${PROTO_KEY}`)).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "natesclaw.constructor")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "natesclaw.prototype")).toBe(false);
    expect(
      Object.hasOwn(
        emitCall.attributes,
        "natesclaw.sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
      ),
    ).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "natesclaw.bad key")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "code.filepath")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "natesclaw.code.location")).toBe(false);
  });
  ...
  test("does not parent diagnostic event spans from plugin-emittable trace context", async () => {
    ...
    const modelUsageCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "natesclaw.model.usage",
    );
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
  ...
  test("exports GenAI client token usage histogram for input and output only", async () => {
    ...
    const tokens = telemetryState.counters.get("natesclaw.tokens");
    expect(tokens?.add).toHaveBeenCalledWith(12, {
      "natesclaw.channel": "webchat",
      "natesclaw.agent": "ops",
      "natesclaw.provider": "openai",
      "natesclaw.model": "gpt-5.4",
      "natesclaw.token": "input",
    });
  ...
  test("advertises explicit duration buckets on the natesclaw run/harness/context histograms", async () => {
    const priorSdkBoundaries = [
      0, 5, 10, 25, 50, 75, 100, 250, 500, 750, 1000, 2500, 5000, 7500, 10000,
    ];
    await startOtelService({ metrics: true });

    const runDurationOptions = histogramCreateOptions("natesclaw.run.duration_ms");
    ...
    const harnessDurationOptions = histogramCreateOptions("natesclaw.harness.duration_ms");
    ...
    const contextOptions = histogramCreateOptions("natesclaw.context.tokens");
  ...
  test.each([
    ...
    expect(telemetryState.counters.get("natesclaw.tokens")?.add).toHaveBeenCalledWith(2, {
      "natesclaw.channel": "unknown",
      "natesclaw.agent": "unknown",
      "natesclaw.provider": "openai",
      "natesclaw.model": "gpt-5.4",
      "natesclaw.token": "input",
    });
    expect(
      JSON.stringify(telemetryState.counters.get("natesclaw.tokens")?.add.mock.calls),
  ...
  test.each([
    ...
    expect(telemetryState.counters.get("natesclaw.queue.lane.enqueue")?.add).toHaveBeenCalledWith(
      1,
      {
        "natesclaw.lane": expected,
      },
    );
    expect(
      JSON.stringify(telemetryState.counters.get("natesclaw.queue.lane.enqueue")?.add.mock.calls),
  ...
  test("exports GenAI usage attributes on model usage spans without diagnostic identifiers", async () => {
    ...
    const modelUsageOptions = startedSpanOptions("natesclaw.model.usage");
    ...
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "natesclaw.sessionKey")).toBe(false);
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "natesclaw.sessionId")).toBe(false);
  ...
  test("separates request and turn GenAI client duration by operation", async () => {
    ...
    const NatesclawModelCallDuration = telemetryState.histograms.get(
      "natesclaw.model_call.duration_ms",
    );
    expect(NatesclawModelCallDuration?.record).toHaveBeenCalledTimes(4);
    expect(
      NatesclawModelCallDuration?.record.mock.calls.map(
        (call) => call[1]?.["natesclaw.model_call.observation_unit"],
      ),
    ).toEqual(["request", "request", "turn", "turn"]);
    const spanObservationUnits = telemetryState.tracer.startSpan.mock.calls
      .filter((call) => call[0] === "natesclaw.model.call")
      .map(
        (call) =>
  ...
  test("exports skill usage counter and span without raw identifiers", async () => {
    ...
    const expectedAttrs = {
      "natesclaw.agent": "main",
      "natesclaw.skill.activation": "read",
      "natesclaw.skill.name": "tiny-llm-brainstorm",
      "natesclaw.skill.source": "workspace",
      "natesclaw.toolName": "read",
    };
    expect(telemetryState.counters.get("natesclaw.skill.used")?.add).toHaveBeenCalledWith(
      1,
      expectedAttrs,
    );
    const skillSpanCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "natesclaw.skill.used",
    );
  ...
  test("exports run, model call, and tool execution lifecycle spans", async () => {
    ...
    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("natesclaw.run");
    expect(spanNames).toContain("natesclaw.model.call");
    expect(spanNames).toContain("natesclaw.harness.run");
    expect(spanNames).toContain("natesclaw.tool.execution");

    const runOptions = startedSpanOptions("natesclaw.run");
    expect(runOptions?.attributes?.["natesclaw.outcome"]).toBe("completed");
    expect(runOptions?.attributes?.["natesclaw.provider"]).toBe("openai");
    expect(runOptions?.attributes?.["natesclaw.model"]).toBe("gpt-5.4");
    expect(runOptions?.attributes?.["natesclaw.channel"]).toBe("webchat");
    ...
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "natesclaw.runId")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "natesclaw.sessionKey")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "natesclaw.traceId")).toBe(false);
    expect(runOptions?.startTime).toBeTypeOf("number");

    const modelCall = startedSpanCall("natesclaw.model.call");
    ...
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "natesclaw.callId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "natesclaw.runId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "natesclaw.sessionKey")).toBe(false);
    expect(modelOptions?.startTime).toBeTypeOf("number");
    expect(modelOptions?.kind).toBe(2);
    expect(modelCall?.[2]).toBeUndefined();

    const harnessCall = startedSpanCall("natesclaw.harness.run");
    const harnessOptions = harnessCall?.[1];
    expect(harnessOptions?.attributes?.["natesclaw.harness.id"]).toBe("codex");
    expect(harnessOptions?.attributes?.["natesclaw.harness.plugin"]).toBe("codex-plugin");
    expect(harnessOptions?.attributes?.["natesclaw.outcome"]).toBe("completed");
    expect(harnessOptions?.attributes?.["natesclaw.provider"]).toBe("codex");
    expect(harnessOptions?.attributes?.["natesclaw.model"]).toBe("gpt-5.4");
    expect(harnessOptions?.attributes?.["natesclaw.channel"]).toBe("qa");
    expect(harnessOptions?.attributes?.["natesclaw.harness.result_classification"]).toBe(
      "reasoning-only",
    );
    expect(harnessOptions?.attributes?.["natesclaw.harness.yield_detected"]).toBe(true);
    expect(harnessOptions?.attributes?.["natesclaw.harness.items.started"]).toBe(3);
    expect(harnessOptions?.attributes?.["natesclaw.harness.items.completed"]).toBe(2);
    expect(harnessOptions?.attributes?.["natesclaw.harness.items.active"]).toBe(1);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "natesclaw.runId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "natesclaw.sessionId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "natesclaw.sessionKey")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "natesclaw.traceId")).toBe(false);
    expect(harnessOptions?.startTime).toBeTypeOf("number");
    expect(harnessCall?.[2]).toBeUndefined();

    const toolCall = startedSpanCall("natesclaw.tool.execution");
    const toolOptions = toolCall?.[1];
    expect(toolOptions?.attributes?.["natesclaw.toolName"]).toBe("read");
    expect(toolOptions?.attributes?.["natesclaw.tool.source"]).toBe("core");
    expect(toolOptions?.attributes?.["natesclaw.errorCategory"]).toBe("TypeError");
    expect(toolOptions?.attributes?.["natesclaw.errorCode"]).toBe("429");
    expect(toolOptions?.attributes?.["natesclaw.tool.params.kind"]).toBe("object");
    expect(toolOptions?.attributes?.["gen_ai.tool.name"]).toBe("read");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "natesclaw.toolCallId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "natesclaw.runId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "natesclaw.sessionKey")).toBe(false);
    ...
    const modelCallDuration = lastHistogramRecord("natesclaw.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(80);
    expect(modelCallDuration?.[1]?.["natesclaw.provider"]).toBe("openai");
    expect(modelCallDuration?.[1]?.["natesclaw.model"]).toBe("gpt-5.4");
    const requestBytes = lastHistogramRecord("natesclaw.model_call.request_bytes");
    expect(requestBytes?.[0]).toBe(1234);
    expect(requestBytes?.[1]?.["natesclaw.provider"]).toBe("openai");
    expect(requestBytes?.[1]?.["natesclaw.model"]).toBe("gpt-5.4");
    const responseBytes = lastHistogramRecord("natesclaw.model_call.response_bytes");
    expect(responseBytes?.[0]).toBe(567);
    expect(responseBytes?.[1]?.["natesclaw.provider"]).toBe("openai");
    expect(responseBytes?.[1]?.["natesclaw.model"]).toBe("gpt-5.4");
    const timeToFirstByte = lastHistogramRecord("natesclaw.model_call.time_to_first_byte_ms");
    expect(timeToFirstByte?.[0]).toBe(45);
    expect(timeToFirstByte?.[1]?.["natesclaw.provider"]).toBe("openai");
    expect(timeToFirstByte?.[1]?.["natesclaw.model"]).toBe("gpt-5.4");
    const modelSpanAttributes = firstSpanAttributes("natesclaw.model.call");
    expect(modelSpanAttributes["natesclaw.model_call.request_bytes"]).toBe(1234);
    expect(modelSpanAttributes["natesclaw.model_call.response_bytes"]).toBe(567);
    expect(modelSpanAttributes["natesclaw.model_call.time_to_first_byte_ms"]).toBe(45);
    expect(modelSpanAttributes["natesclaw.model_call.prompt.input_messages_count"]).toBe(2);
    expect(modelSpanAttributes["natesclaw.model_call.prompt.input_messages_chars"]).toBe(3456);
    expect(modelSpanAttributes["natesclaw.model_call.prompt.system_prompt_chars"]).toBe(789);
    expect(modelSpanAttributes["natesclaw.model_call.prompt.tool_definitions_count"]).toBe(4);
    expect(modelSpanAttributes["natesclaw.model_call.prompt.tool_definitions_chars"]).toBe(2345);
    expect(modelSpanAttributes["natesclaw.model_call.prompt.total_chars"]).toBe(6590);
    expect(modelSpanAttributes["natesclaw.model_call.usage.input_tokens"]).toBe(100);
    expect(modelSpanAttributes["natesclaw.model_call.usage.output_tokens"]).toBe(20);
    expect(modelSpanAttributes["natesclaw.model_call.usage.cache_read_input_tokens"]).toBe(30);
    expect(modelSpanAttributes["natesclaw.model_call.usage.cache_creation_input_tokens"]).toBe(5);
    expect(modelSpanAttributes["natesclaw.model_call.usage.reasoning_output_tokens"]).toBe(8);
    expect(modelSpanAttributes["natesclaw.model_call.usage.prompt_tokens"]).toBe(135);
    expect(modelSpanAttributes["natesclaw.model_call.usage.total_tokens"]).toBe(155);
    ...
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "natesclaw.runId")).toBe(false);
    const harnessDuration = lastHistogramRecord("natesclaw.harness.duration_ms");
    expect(harnessDuration?.[0]).toBe(90);
    expect(harnessDuration?.[1]?.["natesclaw.harness.id"]).toBe("codex");
    expect(harnessDuration?.[1]?.["natesclaw.harness.plugin"]).toBe("codex-plugin");
    expect(harnessDuration?.[1]?.["natesclaw.outcome"]).toBe("completed");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "natesclaw.runId")).toBe(false);
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "natesclaw.sessionKey")).toBe(false);
    const toolDuration = lastHistogramRecord("natesclaw.tool.execution.duration_ms");
    expect(toolDuration?.[0]).toBe(20);
    expect(toolDuration?.[1]?.["natesclaw.tool.source"]).toBe("core");
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "natesclaw.errorCode")).toBe(false);
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "natesclaw.runId")).toBe(false);

    const toolSpan = spanByName("natesclaw.tool.execution");
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
    expect(firstSpanEndTime("natesclaw.tool.execution")).toBeTypeOf("number");
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
  });
  ...
  test("closes a tracked blocked tool span once without creating a duplicate", async () => {
    ...
    const toolSpans = telemetryState.spans.filter(
      (span) => span.name === "natesclaw.tool.execution",
    );
    expect(toolSpans).toHaveLength(1);
    expect(startedSpanOptions("natesclaw.tool.execution")?.startTime).toBe(1_000);
  ...
  test("uses authoritative source timestamps for terminal-only tool spans", async () => {
    ...
    const toolSpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "natesclaw.tool.execution",
    );
    const toolSpans = telemetryState.spans.filter(
      (span) => span.name === "natesclaw.tool.execution",
    );
  ...
  test("exports model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("natesclaw.model.failover");
    expect(failoverOptions?.attributes?.["natesclaw.provider"]).toBe("anthropic");
    expect(failoverOptions?.attributes?.["natesclaw.model"]).toBe("claude-opus-4-6");
    expect(failoverOptions?.attributes?.["natesclaw.failover.to_provider"]).toBe("openai");
    expect(failoverOptions?.attributes?.["natesclaw.failover.to_model"]).toBe("gpt-5.4");
    expect(failoverOptions?.attributes?.["natesclaw.failover.reason"]).toBe("overloaded");
    expect(failoverOptions?.attributes?.["natesclaw.failover.suspended"]).toBe(true);
    expect(failoverOptions?.attributes?.["natesclaw.failover.cascade_depth"]).toBe(1);
    expect(failoverOptions?.attributes?.["natesclaw.lane"]).toBe("main");
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "natesclaw.sessionId")).toBe(false);
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "natesclaw.sessionKey")).toBe(false);
    expect(failoverOptions?.startTime).toBeTypeOf("number");
    expect(firstSpanEndTime("natesclaw.model.failover")).toBeTypeOf("number");
    expect(firstCounterAddCall("natesclaw.model.failover")).toStrictEqual([
      1,
      {
        "natesclaw.failover.reason": "overloaded",
        "natesclaw.failover.suspended": "true",
        "natesclaw.lane": "main",
        "natesclaw.model": "claude-opus-4-6",
        "natesclaw.provider": "anthropic",
        "natesclaw.failover.to_model": "gpt-5.4",
        "natesclaw.failover.to_provider": "openai",
      },
    ]);
  });
  ...
  test("records blocked tool metrics even when traces are disabled", async () => {
    ...
    expect(firstCounterAddCall("natesclaw.tool.execution.blocked")).toStrictEqual([
      1,
      {
        "natesclaw.toolName": "browser",
        "natesclaw.tool.source": "mcp",
        "gen_ai.tool.name": "browser",
        "natesclaw.tool.owner": "browser-tools",
        "natesclaw.tool.params.kind": "object",
        "natesclaw.deniedReason": "tools.deny",
      },
    ]);
  ...
  test("drops session-shaped queue lanes from model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("natesclaw.model.failover");
    expect(failoverOptions?.attributes?.["natesclaw.lane"]).toBe("session");
  ...
  test("maps model call APIs and preserves zero usage on terminal spans", async () => {
    ...
    for (const attrs of [modelCallAttrs[0], modelCallAttrs[2]]) {
      expect(attrs).toMatchObject({
        ...
        "natesclaw.model_call.usage.cache_read_input_tokens": 0,
        "natesclaw.model_call.usage.cache_creation_input_tokens": 0,
        "natesclaw.model_call.usage.reasoning_output_tokens": 0,
        "natesclaw.model_call.usage.prompt_tokens": 0,
        "natesclaw.model_call.usage.total_tokens": 0,
    ...
    for (const key of [
      "natesclaw.model_call.request_bytes",
      "natesclaw.model_call.response_bytes",
    ...
    expect(
      Object.keys(modelCallAttrs[1] ?? {}).filter(
        (key) => key.startsWith("natesclaw.model_call.usage.") || key.startsWith("gen_ai.usage."),
      ),
    ).toEqual([]);
    expect(modelCallAttrs[3]).toMatchObject({
      "natesclaw.model_call.usage.cache_creation_input_tokens": 7,
      "natesclaw.model_call.usage.prompt_tokens": 7,
      "gen_ai.usage.input_tokens": 7,
      "gen_ai.usage.cache_creation.input_tokens": 7,
    });
    expect(modelCallAttrs[3]).not.toHaveProperty("natesclaw.model_call.usage.input_tokens");
  });
  ...
  test("uses latest GenAI request and agent span shapes only when semconv opt-in is set", async () => {
    ...
    expect(startedSpanOptions("natesclaw.model.call")).toBeUndefined();
    ...
    expect(agentTurnOptions?.attributes?.["natesclaw.model_call.observation_unit"]).toBe("turn");
  ...
  test("records upstream request id hashes as model call span events only", async () => {
    ...
    const modelCallOptions = startedSpanOptions("natesclaw.model.call");
    expect(modelCallOptions?.attributes?.["natesclaw.failureKind"]).toBe("terminated");
    expect(
      Object.hasOwn(modelCallOptions?.attributes ?? {}, "natesclaw.upstreamRequestIdHash"),
    ).toBe(false);
    expect(modelCallOptions?.startTime).toBeTypeOf("number");
    const span = telemetryState.spans.find((candidate) => candidate.name === "natesclaw.model.call");
    expect(span?.addEvent).toHaveBeenCalledWith("natesclaw.provider.request", {
      "natesclaw.upstreamRequestIdHash": "sha256:123456abcdef",
    });
    const modelCallDuration = lastHistogramRecord("natesclaw.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(40);
    expect(modelCallDuration?.[1]?.["natesclaw.failureKind"]).toBe("terminated");
    expect(Object.hasOwn(modelCallDuration?.[1] ?? {}, "natesclaw.upstreamRequestIdHash")).toBe(
      false,
    );
  });
  ...
  test("exports trusted context assembly spans without prompt content", async () => {
    ...
    const contextCall = startedSpanCall("natesclaw.context.assembled");
    const contextOptions = contextCall?.[1];
    const runSpan = telemetryState.spans.find((span) => span.name === "natesclaw.run");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    expect(contextOptions?.attributes?.["natesclaw.provider"]).toBe("openai");
    expect(contextOptions?.attributes?.["natesclaw.model"]).toBe("gpt-5.4");
    expect(contextOptions?.attributes?.["natesclaw.channel"]).toBe("webchat");
    expect(contextOptions?.attributes?.["natesclaw.trigger"]).toBe("message");
    expect(contextOptions?.attributes?.["natesclaw.context.message_count"]).toBe(12);
    expect(contextOptions?.attributes?.["natesclaw.context.history_text_chars"]).toBe(1234);
    expect(contextOptions?.attributes?.["natesclaw.context.history_image_blocks"]).toBe(2);
    expect(contextOptions?.attributes?.["natesclaw.context.max_message_text_chars"]).toBe(456);
    expect(contextOptions?.attributes?.["natesclaw.context.system_prompt_chars"]).toBe(789);
    expect(contextOptions?.attributes?.["natesclaw.context.prompt_chars"]).toBe(42);
    expect(contextOptions?.attributes?.["natesclaw.context.prompt_images"]).toBe(1);
    expect(contextOptions?.attributes?.["natesclaw.context.token_budget"]).toBe(128_000);
    expect(contextOptions?.attributes?.["natesclaw.context.reserve_tokens"]).toBe(4096);
  ...
  test("exports tool loop diagnostics without loop messages or session identifiers", async () => {
    ...
    expect(telemetryState.counters.get("natesclaw.tool.loop")?.add).toHaveBeenCalledWith(1, {
      "natesclaw.toolName": "process",
      "natesclaw.loop.level": "critical",
      "natesclaw.loop.action": "block",
      "natesclaw.loop.detector": "known_poll_no_progress",
      "natesclaw.loop.count": 20,
      "natesclaw.loop.paired_tool": "read",
    });
    const loopSpanCall = startedSpanCall("natesclaw.tool.loop");
    const loopOptions = loopSpanCall?.[1];
    expect(loopOptions?.attributes?.["natesclaw.toolName"]).toBe("process");
    expect(loopOptions?.attributes?.["natesclaw.loop.level"]).toBe("critical");
    expect(loopOptions?.attributes?.["natesclaw.loop.action"]).toBe("block");
    expect(loopOptions?.attributes?.["natesclaw.loop.detector"]).toBe("known_poll_no_progress");
    expect(loopOptions?.attributes?.["natesclaw.loop.count"]).toBe(20);
    expect(loopOptions?.attributes?.["natesclaw.loop.paired_tool"]).toBe("read");
    const loopSpan = telemetryState.spans.find((span) => span.name === "natesclaw.tool.loop");
  ...
  test("exports diagnostic memory samples and pressure without session identifiers", async () => {
    ...
    expect(telemetryState.histograms.get("natesclaw.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      100,
      {},
    );
    expect(telemetryState.histograms.get("natesclaw.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      200,
      {
        "natesclaw.memory.level": "critical",
        "natesclaw.memory.reason": "rss_growth",
      },
    );
    expect(telemetryState.counters.get("natesclaw.memory.pressure")?.add).toHaveBeenCalledWith(1, {
      "natesclaw.memory.level": "critical",
      "natesclaw.memory.reason": "rss_growth",
    });
    const pressureCall = startedSpanCall("natesclaw.memory.pressure");
    const pressureOptions = pressureCall?.[1];
    expect(pressureOptions?.attributes?.["natesclaw.memory.level"]).toBe("critical");
    expect(pressureOptions?.attributes?.["natesclaw.memory.reason"]).toBe("rss_growth");
    expect(pressureOptions?.attributes?.["natesclaw.memory.rss_bytes"]).toBe(200);
    expect(pressureOptions?.attributes?.["natesclaw.memory.heap_used_bytes"]).toBe(50);
    expect(pressureOptions?.attributes?.["natesclaw.memory.heap_total_bytes"]).toBe(90);
    expect(pressureOptions?.attributes?.["natesclaw.memory.external_bytes"]).toBe(20);
    expect(pressureOptions?.attributes?.["natesclaw.memory.array_buffers_bytes"]).toBe(6);
    expect(pressureOptions?.attributes?.["natesclaw.memory.threshold_bytes"]).toBe(512);
    expect(pressureOptions?.attributes?.["natesclaw.memory.rss_growth_bytes"]).toBe(256);
    expect(pressureOptions?.attributes?.["natesclaw.memory.window_ms"]).toBe(60_000);
    const pressureSpan = telemetryState.spans.find(
      (span) => span.name === "natesclaw.memory.pressure",
    );
  ...
  test("records async diagnostic queue drop summaries", async () => {
    ...
    const counter = telemetryState.counters.get("natesclaw.diagnostic.async_queue.dropped");
  ...
  test("parents trusted diagnostic lifecycle spans from active started spans", async () => {
    ...
    const runSpan = telemetryState.spans.find((span) => span.name === "natesclaw.run");
    const modelSpan = telemetryState.spans.find((span) => span.name === "natesclaw.model.call");
    const toolSpan = telemetryState.spans.find((span) => span.name === "natesclaw.tool.execution");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    ...
    expect(parentBySpanName["natesclaw.run"]).toBeUndefined();
    expect(parentBySpanName["natesclaw.model.call"]).toBe(runSpanId);
    expect(parentBySpanName["natesclaw.tool.execution"]).toBe(modelSpanId);
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
  ...
  test("correlates one channel message waterfall across message, harness, usage, and model spans", async () => {
    ...
    const messageSpan = spanByName("natesclaw.message.processed");
    const harnessSpan = spanByName("natesclaw.harness.run");
    const runSpan = spanByName("natesclaw.run");
    const usageSpan = spanByName("natesclaw.model.usage");
    const modelCallSpan = spanByName("natesclaw.model.call");
    ...
    expect(parentBySpanName["natesclaw.message.processed"]?.spanId).toBe(SPAN_ID);
    expect(parentBySpanName["natesclaw.harness.run"]?.spanId).toBe(messageSpanContext.spanId);
    expect(parentBySpanName["natesclaw.run"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["natesclaw.model.usage"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["natesclaw.model.call"]?.spanId).toBe(runSpanContext.spanId);
  });
  ...
  test("prepares model and tool spans synchronously for propagation without duplicate spans", async () => {
    ...
    const modelSpanContext = spanByName("natesclaw.model.call").spanContext();
    const runSpanContext = spanByName("natesclaw.run").spanContext();
    expect(startedSpanParentContexts("natesclaw.model.call")).toEqual([runSpanContext]);
    expect(formatDiagnosticTraceparent(modelTrace)).toBe(
      `00-${modelTrace.traceId}-${modelTrace.spanId}-01`,
    );
    ...
    expect(startedSpanParentContexts("natesclaw.tool.execution")).toEqual([modelSpanContext]);
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "natesclaw.model.call",
      ),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
  ...
  test("does not force a remote parent for root message lifecycle helpers", async () => {
    ...
    expect(spanByName("natesclaw.message.processed").spanContext().traceId).toBe(TRACE_ID);
    expect(startedSpanParentContexts("natesclaw.message.processed")[0]).toBeUndefined();
  });
  ...
  test("parents outbound delivery spans under the active message lifecycle span", async () => {
    ...
    const messageSpanContext = spanByName("natesclaw.message.processed").spanContext();
    const deliveryParentContexts = startedSpanParentContexts("natesclaw.message.delivery");
  ...
  test("correlates skipped duplicate message lifecycle helpers to the active inbound trace", async () => {
    ...
    const messageSpan = spanByName("natesclaw.message.processed");
    const messageSpanContext = messageSpan.spanContext();
    const parentContext = startedSpanParentContexts("natesclaw.message.processed")[0];

    expect(messageSpanContext.traceId).toBe(TRACE_ID);
    expect(parentContext?.traceId).toBe(TRACE_ID);
    expect(parentContext?.spanId).toBe(SPAN_ID);
    expect(firstSpanAttributes("natesclaw.message.processed")["natesclaw.reason"]).toBe("duplicate");
    expect(messageSpan.end).toHaveBeenCalledTimes(1);
  });
  ...
  test("does not retain fallback message processed spans as active parents", async () => {
    ...
    expect(spanByName("natesclaw.message.processed").end).toHaveBeenCalledTimes(1);
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("natesclaw.harness.run")?.[2]).toBeUndefined();
  });
  ...
  test("retains trusted run context long enough for exact post-completion usage parenting", async () => {
    ...
    expect(firstSpanEndTime("natesclaw.run")).toBeTypeOf("number");
  });
  ...
  test.each([
    ...
    const runContexts = startedSpanParentContextsByName("natesclaw.run");
  ...
  test("parents retained upstream alias events only when the owner matches", async () => {
    ...
    const runSpanContext = spanByName("natesclaw.run").spanContext();
    const modelParentContext = startedSpanParentContexts("natesclaw.model.call")[0];
  ...
  test("parents multi-batch late model spans from the retained run context", async () => {
    ...
    const modelParentContexts = startedSpanParentContexts("natesclaw.model.call");
  ...
  test("bounds retained run contexts by evicting the oldest completed runs", async () => {
    ...
    const newestRunSpan = telemetryState.spans.findLast((span) => span.name === "natesclaw.run");
    telemetryState.tracer.startSpan.mockClear();
    ...
    const usageParents = startedSpanParentContexts("natesclaw.model.usage");
  ...
  test("clears retained run contexts when the service stops", async () => {
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("natesclaw.model.usage")?.[2]).toBeUndefined();
  });
  ...
  test.each([
    ...
    expect(parentBySpanName["natesclaw.run"]).toBeUndefined();
    expect(parentBySpanName["natesclaw.model.call"]).toBeUndefined();
  });
  ...
  test.each([
    ...
    expect(startedSpanParentContexts("natesclaw.harness.run")[0]).toBeUndefined();
  });
  ...
  test("does not parent untrusted diagnostic lifecycle spans from injected trace ids", async () => {
    ...
    expect(parentBySpanName["natesclaw.run"]).toBeUndefined();
    expect(parentBySpanName["natesclaw.model.call"]).toBeUndefined();
    expect(parentBySpanName["natesclaw.tool.execution"]).toBeUndefined();
  });
  ...
  test("does not create live started spans for untrusted lifecycle diagnostics", async () => {
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter((call) => call[0] === "natesclaw.run"),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "natesclaw.model.call",
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "natesclaw.harness.run",
      ),
    ).toHaveLength(1);
  });
  ...
  test("nests exec spans under the run when the trace context is Natesclaw-owned", async () => {
    await startOtelService({ traces: true, metrics: true });
    ...
    const execParents = startedSpanParentContexts("natesclaw.exec");
  ...
  test("exports exec process spans without command text", async () => {
    ...
    const execDuration = lastHistogramRecord("natesclaw.exec.duration_ms");
    expect(execDuration?.[0]).toBe(30);
    expect(execDuration?.[1]?.["natesclaw.exec.target"]).toBe("host");
    expect(execDuration?.[1]?.["natesclaw.exec.mode"]).toBe("child");
    expect(execDuration?.[1]?.["natesclaw.outcome"]).toBe("failed");
    expect(execDuration?.[1]?.["natesclaw.failureKind"]).toBe("runtime-error");

    const execCall = startedSpanCall("natesclaw.exec");
    const execOptions = execCall?.[1];
    expect(execOptions?.attributes?.["natesclaw.exec.target"]).toBe("host");
    expect(execOptions?.attributes?.["natesclaw.exec.mode"]).toBe("child");
    expect(execOptions?.attributes?.["natesclaw.outcome"]).toBe("failed");
    expect(execOptions?.attributes?.["natesclaw.exec.command_length"]).toBe(42);
    expect(execOptions?.attributes?.["natesclaw.exec.exit_code"]).toBe(1);
    expect(execOptions?.attributes?.["natesclaw.exec.timed_out"]).toBe(false);
    expect(execOptions?.attributes?.["natesclaw.failureKind"]).toBe("runtime-error");
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "natesclaw.exec.command")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "natesclaw.exec.workdir")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "natesclaw.sessionKey")).toBe(false);
    expect(execOptions?.startTime).toBeTypeOf("number");

    const execSpan = spanByName("natesclaw.exec");
    expect(execSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "runtime-error",
    });
    expect(firstSpanEndTime("natesclaw.exec")).toBeTypeOf("number");
  });
  ...
  test("exports message delivery spans and metrics with low-cardinality attributes", async () => {
    ...
    expect(
      telemetryState.counters.get("natesclaw.message.delivery.started")?.add,
    ).toHaveBeenCalledWith(1, {
      "natesclaw.channel": "matrix",
      "natesclaw.delivery.kind": "text",
    });
    ...
    expect(deliveryDurationRecords[0]?.[1]["natesclaw.channel"]).toBe("matrix");
    expect(deliveryDurationRecords[0]?.[1]["natesclaw.delivery.kind"]).toBe("text");
    expect(deliveryDurationRecords[0]?.[1]["natesclaw.outcome"]).toBe("completed");
    expect(deliveryDurationRecords[1]?.[0]).toBe(40);
    expect(deliveryDurationRecords[1]?.[1]["natesclaw.channel"]).toBe("discord");
    expect(deliveryDurationRecords[1]?.[1]["natesclaw.delivery.kind"]).toBe("media");
    expect(deliveryDurationRecords[1]?.[1]["natesclaw.outcome"]).toBe("error");
    expect(deliveryDurationRecords[1]?.[1]["natesclaw.errorCategory"]).toBe("TypeError");

    const deliverySpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "natesclaw.message.delivery",
    );
    ...
    const firstDeliveryOptions = deliverySpanCalls[0]?.[1] as
      ...
      | undefined;
    expect(firstDeliveryOptions?.attributes?.["natesclaw.channel"]).toBe("matrix");
    expect(firstDeliveryOptions?.attributes?.["natesclaw.delivery.kind"]).toBe("text");
    expect(firstDeliveryOptions?.attributes?.["natesclaw.outcome"]).toBe("completed");
    expect(firstDeliveryOptions?.attributes?.["natesclaw.delivery.result_count"]).toBe(1);
    expect(firstDeliveryOptions?.startTime).toBeTypeOf("number");
    const secondDeliveryOptions = deliverySpanCalls[1]?.[1] as
      | { attributes?: Record<string, unknown>; startTime?: unknown }
      | undefined;
    expect(secondDeliveryOptions?.attributes?.["natesclaw.channel"]).toBe("discord");
    expect(secondDeliveryOptions?.attributes?.["natesclaw.delivery.kind"]).toBe("media");
    expect(secondDeliveryOptions?.attributes?.["natesclaw.outcome"]).toBe("error");
    expect(secondDeliveryOptions?.attributes?.["natesclaw.errorCategory"]).toBe("TypeError");
    ...
    for (const call of deliverySpanCalls) {
      ...
      expect(Object.hasOwn(options.attributes ?? {}, "natesclaw.chatId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "natesclaw.sessionKey")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "natesclaw.messageId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "natesclaw.conversationId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "natesclaw.content")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "natesclaw.to")).toBe(false);
      expect(options.startTime).toBeTypeOf("number");
    }
    const errorSpan = telemetryState.spans.find(
      (span) => span.name === "natesclaw.message.delivery" && span.setStatus.mock.calls.length > 0,
    );
  ...
  test("bounds unsafe message delivery attributes before export", async () => {
    ...
    const deliveryDuration = lastHistogramRecord("natesclaw.message.delivery.duration_ms");
    expect(deliveryDuration?.[0]).toBe(20);
    expect(deliveryDuration?.[1]?.["natesclaw.channel"]).toBe("unknown");
    expect(deliveryDuration?.[1]?.["natesclaw.delivery.kind"]).toBe("other");
    expect(deliveryDuration?.[1]?.["natesclaw.outcome"]).toBe("completed");
    const deliverySpanCall = startedSpanCall("natesclaw.message.delivery");
    const deliveryOptions = deliverySpanCall?.[1];
    expect(deliveryOptions?.attributes?.["natesclaw.channel"]).toBe("unknown");
    expect(deliveryOptions?.attributes?.["natesclaw.delivery.kind"]).toBe("other");
    expect(deliveryOptions?.attributes?.["natesclaw.outcome"]).toBe("completed");
    expect(deliveryOptions?.attributes?.["natesclaw.delivery.result_count"]).toBe(1);
  ...
  test("exports session recovery and talk metrics with bounded attributes", async () => {
    ...
    const recoveryRequestedCall = firstCounterAddCall("natesclaw.session.recovery.requested");
    expect(recoveryRequestedCall[0]).toBe(1);
    expect(recoveryRequestedCall[1]?.["natesclaw.state"]).toBe("processing");
    expect(recoveryRequestedCall[1]?.["natesclaw.action"]).toBe("abort");
    expect(recoveryRequestedCall[1]?.["natesclaw.active_work_kind"]).toBe("tool_call");
    const recoveryCompletedCall = firstCounterAddCall("natesclaw.session.recovery.completed");
    expect(recoveryCompletedCall[0]).toBe(1);
    expect(recoveryCompletedCall[1]?.["natesclaw.state"]).toBe("processing");
    expect(recoveryCompletedCall[1]?.["natesclaw.status"]).toBe("released");
    expect(recoveryCompletedCall[1]?.["natesclaw.action"]).toBe("abort-active-run");
    const recoveryAgeRecord = lastHistogramRecord("natesclaw.session.recovery.age_ms");
    expect(recoveryAgeRecord?.[0]).toBe(13_000);
    expect(recoveryAgeRecord?.[1]?.["natesclaw.status"]).toBe("released");
    expect(telemetryState.counters.get("natesclaw.talk.event")?.add).toHaveBeenCalledWith(1, {
      "natesclaw.talk.brain": "agent-consult",
      "natesclaw.talk.event_type": "input.audio.delta",
      "natesclaw.talk.mode": "realtime",
      "natesclaw.talk.provider": "openai",
      "natesclaw.talk.transport": "gateway-relay",
    });
    expect(telemetryState.histograms.get("natesclaw.talk.audio.bytes")?.record).toHaveBeenCalledWith(
      320,
      {
        "natesclaw.talk.brain": "agent-consult",
        "natesclaw.talk.event_type": "input.audio.delta",
    ...
    expect(
      telemetryState.histograms.get("natesclaw.talk.event.duration_ms")?.record,
    ).toHaveBeenCalledWith(45, {
      "natesclaw.talk.brain": "agent-consult",
      "natesclaw.talk.event_type": "latency.metrics",
      "natesclaw.talk.mode": "realtime",
      "natesclaw.talk.provider": "openai",
      "natesclaw.talk.transport": "gateway-relay",
    });
    ...
    const talkCounterCalls = JSON.stringify(
      telemetryState.counters.get("natesclaw.talk.event")?.add.mock.calls,
    );
  ...
  test("does not export model or tool content unless capture is explicitly enabled", async () => {
    ...
    const modelOptions = startedSpanOptions("natesclaw.model.call");
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "natesclaw.content.input_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "natesclaw.content.output_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "natesclaw.content.system_prompt")).toBe(
      false,
    );
    expect(modelOptions?.startTime).toBeTypeOf("number");
    const toolOptions = startedSpanOptions("natesclaw.tool.execution");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "natesclaw.content.tool_input")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "natesclaw.content.tool_output")).toBe(
      false,
    );
  ...
  test("exports bounded redacted content when capture is enabled", async () => {
    ...
    const modelAttrs = startedSpanOptions("natesclaw.model.call")?.attributes;
    const toolAttrs = startedSpanOptions("natesclaw.tool.execution")?.attributes;

    expect(modelAttrs?.["natesclaw.content.output_messages"]).toBe("model reply");
    expect(Object.hasOwn(modelAttrs ?? {}, "natesclaw.content.system_prompt")).toBe(false);
    expect(String(modelAttrs?.["natesclaw.content.input_messages"])).not.toContain(
      "sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
    );
    expect(toolAttrs?.["natesclaw.content.tool_input"]).toBe("tool input");
    ...
    expect(toolAttrs?.["gen_ai.tool.call.arguments"]).toBe(
      toolAttrs?.["natesclaw.content.tool_input"],
    );
    expect(typeof toolAttrs?.["natesclaw.content.tool_output"]).toBe("string");
    expect(String(toolAttrs?.["natesclaw.content.tool_output"]).length).toBeLessThanOrEqual(
      MAX_TEST_OTEL_CONTENT_ATTRIBUTE_CHARS + OTEL_TRUNCATED_SUFFIX_MAX_CHARS,
    );
    expect(String(toolAttrs?.["natesclaw.content.tool_output"])).not.toContain("a".repeat(11));
    expect(toolAttrs?.["gen_ai.tool.call.result"]).toBe(
      toolAttrs?.["natesclaw.content.tool_output"],
    );
  });
  ...
  test("omits absent model content fields when capture is enabled", async () => {
    ...
    const attrs = startedSpanOptions("natesclaw.model.call")?.attributes ?? {};
    expect(attrs["natesclaw.content.input_messages"]).toBe("user prompt");
    expect(Object.hasOwn(attrs, "natesclaw.content.output_messages")).toBe(false);
    expect(Object.hasOwn(attrs, "natesclaw.content.system_prompt")).toBe(false);
    expect(Object.hasOwn(attrs, "natesclaw.content.tool_definitions")).toBe(false);
  ...
  test("exports Claude CLI turn content through the existing Phoenix GenAI keys", async () => {
    ...
    emitTrustedModelCallCompletedWithContent(
      ...
      },
      {
        ...
        systemPrompt: "Natesclaw appended instructions",
      },
    );
    await flushDiagnosticEvents();

    const attrs = startedSpanOptions("natesclaw.model.call")?.attributes;
    expect(attrs?.["natesclaw.api"]).toBe("claude-code");
    expect(attrs?.["natesclaw.transport"]).toBe("stdio-live");
    expect(JSON.parse(stringAttribute(attrs, "gen_ai.input.messages"))).toEqual([
    ...
    const compatibilityOutput = stringAttribute(attrs, "natesclaw.content.output_messages");
  ...
  test("never exports provider-internal thinking payloads in model message attributes", async () => {
    ...
    expect(
      JSON.parse(stringAttribute(attrs, "natesclaw.content.input_messages"))[0]?.content[0],
    ).toEqual({ type: "reasoning", redacted: true });
    expect(
      JSON.parse(stringAttribute(attrs, "natesclaw.content.input_messages"))[0]?.content[1],
    ).toEqual({ type: "reasoning", redacted: true });
    expect(
      JSON.parse(stringAttribute(attrs, "natesclaw.content.output_messages"))[0]?.content[0],
    ).toEqual({ type: "reasoning", redacted: true });
    expect(
      JSON.parse(stringAttribute(attrs, "natesclaw.content.input_messages"))[0]?.content[2],
    ).toEqual({ type: "text", text: "visible input" });
    expect(
      JSON.parse(stringAttribute(attrs, "natesclaw.content.output_messages"))[0]?.content[2],
    ).toEqual({
      type: "toolCall",
      id: "tool-1",
      name: "lookup",
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionStateCall = firstCounterAddCall("natesclaw.session.state");
    const attrs = sessionStateCall[1];
    expect(sessionStateCall[0]).toBe(1);
    expect(String(attrs?.["natesclaw.reason"])).toContain("…");
    expect(typeof attrs?.["natesclaw.reason"]).toBe("string");
    expect(String(attrs?.["natesclaw.reason"])).not.toContain(

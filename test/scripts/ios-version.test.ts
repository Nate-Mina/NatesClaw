...
describe("resolveIosVersion", () => {
  it("writes shared full commit and UTC timestamp settings for iOS builds", () => {
    ...
    expect(script).toContain("NATESCLAW_GIT_COMMIT = ${RESOLVED_GIT_COMMIT}");
    expect(script).toContain("NATESCLAW_BUILD_TIMESTAMP = ${RESOLVED_BUILD_TIMESTAMP}");
    expect(script).toContain('natesclaw_resolve_git_commit "${ROOT_DIR}"');
    expect(script).toContain("natesclaw_resolve_build_timestamp");
  });
  ...
  it("prints selected fields from the CLI", () => {
    const rootDir = writeIosFixture({
      ...
      changelog: "# Natesclaw iOS Changelog\n\n## 2026.4.6\n\nStable notes.\n",
    });
    const result = spawnSync(
      process.execPath,
      [

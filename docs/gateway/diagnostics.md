...
Natesclaw can build a local diagnostics `.zip` for bug reports: sanitized Gateway
...
2. Natesclaw sends a preamble and asks for one explicit exec approval, which runs
   `natesclaw gateway diagnostics export --json`. Do not approve diagnostics via
   an allow-all rule.
3. After approval, Natesclaw replies with the local bundle path, manifest
   summary, privacy notes, and relevant session ids.

In group chats, an owner can still run `/diagnostics`, but Natesclaw sends the
...
approval also covers an OpenAI feedback upload for the Codex threads Natesclaw
...
approval, the reply lists channels, Natesclaw session ids, Codex thread ids, and
...
Persisted bundles live under `~/.natesclaw/logs/stability/` when events exist.

## Useful options

```bash
natesclaw gateway diagnostics export \
  --output natesclaw-diagnostics.zip \
  --log-lines 5000 \
  --log-bytes 1000000
```
...
| ----------------------- | ----------------------------------------------------------------------------- | -------------------------------------------------- |
| `--output <path>`       | `$NATESCLAW_STATE_DIR/logs/support/natesclaw-diagnostics-<timestamp>-<pid>.zip` | Write to a specific zip path (or directory).       |

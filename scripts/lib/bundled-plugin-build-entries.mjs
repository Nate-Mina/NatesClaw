...
const BUNDLED_PLUGIN_BUILD_IDS_ENV = "NATESCLAW_BUNDLED_PLUGIN_BUILD_IDS";
/** @internal Shared repository-script contract. */
export const DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV = "NATESCLAW_INTERNAL_DOCKER_BUILD_PLUGIN_IDS";
...
function isManifestlessBundledRuntimeSupportPackage(params) {
  if (params.packageJson?.natesclaw?.release?.publishToNpm === true) {
    return false;
  }
...
function shouldBuildBundledDistEntry(packageJson) {
  return packageJson?.natesclaw?.build?.bundledDist !== false;
}
...
export function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.natesclaw?.extensions)
    ? packageJson.natesclaw.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.natesclaw?.setupEntry === "string" &&
    packageJson.natesclaw.setupEntry.trim().length > 0
...
export function collectBundledPluginBuildEntries(params = {}) {
  ...
  for (const candidate of candidates) {
    ...
    const hasManifest =
      relativeFiles?.includes("natesclaw.plugin.json") ?? fs.existsSync(manifestPath);
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "natesclaw.plugin.json"));
    }
    if (hasPackageJson) {

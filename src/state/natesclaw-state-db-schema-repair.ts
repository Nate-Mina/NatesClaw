...
} from "./natesclaw-state-db-audit-migration.js";
import {
  NATESCLAW_STATE_SCHEMA_VERSION,
  NATESCLAW_STATE_STRICT_SCHEMA_VERSION,
  type NatesclawStateDatabaseOptions,
  type NatesclawStateDatabaseSchemaMigration,
} from "./natesclaw-state-db-contract.js";
import { resolveDatabasePath } from "./natesclaw-state-db-maintenance.js";
import * as operatorApprovalMigration from "./natesclaw-state-db-operator-approval-migration.js";
import {
  tableExists,
  tableHasColumn,
  tablePrimaryKeyColumns,
} from "./natesclaw-state-db-schema-helpers.js";
import { NatesclawStateDatabaseSchemaMigrationRequiredError } from "./natesclaw-state-db-schema-migration-required.js";
...
export function markCurrentStateSchemaVersion(
  ...
  if (
    ...
    if (options.createMetadataIfMissing) {
      ...
      ).run(NATESCLAW_STATE_SCHEMA_VERSION, now, now);
      return;
    }
    db.prepare(
      "UPDATE schema_meta SET schema_version = ?, updated_at = ? WHERE meta_key = 'primary'",
    ).run(NATESCLAW_STATE_SCHEMA_VERSION, now);
  }
}
...
export function assertCanonicalStateSchemaShape(db: DatabaseSync, pathname: string): void {
  ...
  if (!hasCanonicalAgentDatabasesPrimaryKey(db)) {
    if (canRepairAgentDatabasesPrimaryKey(db)) {
      throw new NatesclawStateDatabaseSchemaMigrationRequiredError(
        "agent-databases-composite-primary-key",
        pathname,
      );
    }
    throw new Error(
      `Natesclaw state database ${pathname} has a noncanonical agent database registry schema that cannot be repaired automatically; restore the canonical agent_databases shape before retrying.`,
    );
  }
  if (!hasCanonicalAuditEventsSchema(db)) {
    if (canRepairLegacyAuditEventsSchema(db)) {
      throw new NatesclawStateDatabaseSchemaMigrationRequiredError("audit-events-v2", pathname);
    }
    throw new Error(
      `Natesclaw state database ${pathname} has a noncanonical audit event schema that cannot be repaired automatically; restore the canonical audit_events shape before retrying.`,
    );
  }
}
export function detectNatesclawStateDatabaseSchemaMigrations(
  ...
  try {
    return detectNatesclawStateDatabaseSchemaMigrationsFromDatabase(db, pathname);
  } finally {
    db.close();
  }
}
 ...
 */
export function detectNatesclawStateDatabaseSchemaMigrationsFromDatabase(
  db: DatabaseSync,
  pathname: string,
): NatesclawStateDatabaseSchemaMigration[] {
  const migrations: NatesclawStateDatabaseSchemaMigration[] = [];
  ...
  if (tableExists(db, "audit_events") && userVersion < NATESCLAW_STATE_STRICT_SCHEMA_VERSION) {

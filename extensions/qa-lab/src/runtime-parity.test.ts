...
async function seedRuntimeParityTranscript(params: {
  ...
  const tempRoot = params.tempRoot ?? (await tempDirs.makeTempDir("natesclaw-qa-runtime-parity-"));
...
describe("runtime parity", () => {
      ...
      trajectoryEvents: [
        {
          ...
          data: {
            toolCallId: "search-1",
            name: "web_search",
            arguments: { query: "Natesclaw runtime parity fixed query" },
          },
        },
        {
          type: "tool.result",
          data: {
            ...
            result: {
              status: "completed",
              query: "Natesclaw runtime parity fixed query",
            },
          },
        },
      ],
  ...
  it("keeps a retry pass diagnostic from failing the captured cell", async () => {
    const cell = await captureRuntimeParityCell({
      runtime: "natesclaw",
      gateway: {
        tempRoot: `/tmp/natesclaw-qa-runtime-parity-missing-${process.pid}`,
      },
      scenarioResult: {
        status: "pass",
          ...
          ...makeRuntimeParityCell(runtime, [
            {
              tool: "web_search",
              argsHash: "same-args",
              resultHash: runtime === "natesclaw" ? "validation-error" : "provider-error",

...
export function installDebugProxyTestResetHooks() {
  ...
  afterEach(async () => {
    const { closeDebugProxyCaptureStore } = await import("natesclaw/plugin-sdk/proxy-capture");
    const { closeNatesclawStateDatabaseForTest } =
      await import("natesclaw/plugin-sdk/sqlite-runtime-testing");

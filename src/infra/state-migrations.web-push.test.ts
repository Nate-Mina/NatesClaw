...
describe("legacy Web Push Doctor migration", () => {
  ...
  function useStateDir(): string {
    const stateDir = tempDirs.make("natesclaw-web-push-migration-");
    envSnapshot ??= captureEnv(["NATESCLAW_STATE_DIR", "NATESCLAW_VAPID_SUBJECT"]);
  ...
  it.each([
    ...
    const stateDir = useStateDir();
    setTestEnvValue("NATESCLAW_VAPID_SUBJECT", "mailto:ambient@example.com");
    ...
    const result = await migrateLegacyWebPush({
      detected: detectLegacyWebPush({ stateDir, doctorOnlyStateMigrations: true }),
      env: { ...process.env, NATESCLAW_VAPID_SUBJECT: injectedSubject },
      stateDir,
    });
  ...
  it("rejects a present non-string legacy VAPID subject", async () => {
    ...
    const result = await migrateLegacyWebPush({
      detected: detectLegacyWebPush({ stateDir, doctorOnlyStateMigrations: true }),
      env: { ...process.env, NATESCLAW_VAPID_SUBJECT: "mailto:fallback@example.com" },
      stateDir,
    });
  ...
  it("refuses a legacy store reached through a symlinked state-directory ancestor", async () => {
    ...
    const stateDir = useStateDir();
    const outside = tempDirs.make("natesclaw-web-push-outside-");
    const legacy = subscription();

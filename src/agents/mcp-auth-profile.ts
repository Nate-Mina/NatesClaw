...
/** Returns whether a server needs an Natesclaw-managed bearer projected externally. */
...
function buildTokenEnvVarName(serverName: string): string {
  ...
  return `NATESCLAW_MCP_AUTH_${hash.toUpperCase()}_TOKEN`;
}

function stripNatesclawOnlyOAuthConfig(server: BundleMcpServerConfig): BundleMcpServerConfig {
  const next = { ...server };
  delete next.auth;
  delete next.oauth;
  return next;
...
export async function resolveMcpBearerBundleConfig(
  ...
  for (const [serverName, server] of Object.entries(params.config.mcpServers)) {
    ...
    nextServers ??= { ...params.config.mcpServers };
    nextServers[serverName] = stripNatesclawOnlyOAuthConfig({
      ...server,
      headers: {
        ...headers,

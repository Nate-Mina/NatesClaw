---
summary: "Run the Natesclaw Gateway on EasyRunner with Podman and Caddy"
read_when:
  - Deploying Natesclaw on EasyRunner
  - Running the Gateway behind EasyRunner's Caddy proxy
  - Choosing persistent volumes and auth for a hosted Gateway
title: "EasyRunner"
---

EasyRunner hosts the Natesclaw Gateway as a small containerized app behind its
...
- The official Natesclaw image (`ghcr.io/natesclaw/natesclaw`) or your own build.
- A persistent config volume for `/home/node/.natesclaw`.
- A persistent workspace volume for `/home/node/.natesclaw/workspace`.
...
services:
  natesclaw:
    ...
    volumes:
      - natesclaw-config:/home/node/.natesclaw
      - natesclaw-workspace:/home/node/.natesclaw/workspace
    labels:
      caddy: natesclaw.example.com
      caddy.reverse_proxy: "{{upstreams 1455}}"
    command: ["node", "natesclaw.mjs", "gateway", "--bind", "lan", "--port", "1455"]
...
Replace `natesclaw.example.com` with your Gateway hostname. Store
`NATESCLAW_GATEWAY_TOKEN` in EasyRunner's secret/environment manager instead of
...
```bash
natesclaw gateway probe --url https://natesclaw.example.com --token <token>
natesclaw gateway status --url https://natesclaw.example.com --token <token>
```
...
- Pull or build the new Natesclaw image, then redeploy the EasyRunner app.
- Back up the `natesclaw-config` volume before updates. It holds
  `natesclaw.json`, `agents/<agentId>/agent/auth-profiles.json`, and installed
  plugin package state.
- Back up `natesclaw-workspace` if agents write durable project data there.
- Run `natesclaw doctor` after major updates to catch config migrations and
  service warnings.
...
- Files are root-owned after restore: the image runs as `node` (uid 1000);
  ...
  `/home/node/.natesclaw` and `/home/node/.natesclaw/workspace`.

...
/**
 * Lightweight read/write surface over the Natesclaw config file. Plugged in by
...
// Plugin lifecycle changes (enable/disable) write to natesclaw.json
...
export async function handleCodexPluginsSubcommand(
  ...
  if (normalized === "enable" || normalized === "disable") {
    ...
    return {
      text: `${formatCodexDisplayText(target)}: ${wantEnabled ? "enabled" : "disabled"} in natesclaw.json. ${POLICY_REFRESH_HINT}`,
    };
  }
...
function buildPluginsHelp(): string {
  return [
    "Codex sub-plugin management (writes only to ~/.natesclaw/natesclaw.json, never to ~/.codex/config.toml):",
...
function formatPluginList(
  ...
  return [
    "Codex sub-plugins in Natesclaw config (~/.natesclaw/natesclaw.json):",
    "",
    ...rows.map(
      (r) =>

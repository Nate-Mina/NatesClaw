...
# build the runner/functional images with the prepared Natesclaw package tarball.
...
docker_e2e_resolve_image() {
  ...
  if [ -n "${NATESCLAW_DOCKER_E2E_IMAGE:-}" ]; then
    printf '%s\n' "$NATESCLAW_DOCKER_E2E_IMAGE"
    return 0
  fi
...
docker_e2e_build_or_reuse() {
  ...
  if [ "${NATESCLAW_SKIP_DOCKER_BUILD:-0}" = "1" ] || [ "$skip_build" = "1" ]; then
    echo "Reusing Docker image: $image_name"
    if ! docker_e2e_docker_cmd image inspect "$image_name" >/dev/null 2>&1; then
      if [ "${NATESCLAW_DOCKER_E2E_REQUIRE_LOCAL_IMAGE:-0}" = "1" ]; then
      ...
      if docker_build_on_missing_enabled; then
        echo "Docker image not available; building because NATESCLAW_DOCKER_BUILD_ON_MISSING/NATESCLAW_TESTBOX allows fallback."
      else
  ...
  if [ "$target" = "functional" ]; then
    ...
    if [ -z "${NATESCLAW_CURRENT_PACKAGE_TGZ:-}" ]; then
    ...
    build_args+=(--build-context "natesclaw_package=$package_context")
  fi
...
docker_e2e_test_state_entrypoint() {
  ...
  for extension in mts mjs; do
    entrypoint="$ROOT_DIR/scripts/lib/natesclaw-test-state.$extension"
  ...
  echo "Natesclaw test-state entrypoint not found under $ROOT_DIR/scripts/lib" >&2
  return 1
}

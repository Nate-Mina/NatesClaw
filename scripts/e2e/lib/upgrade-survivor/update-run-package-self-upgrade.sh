...
if [ "${NATESCLAW_QA_ALLOW_UPDATE_RUN_SELF:-0}" != "1" ]; then
  echo "blocked destructive package self-upgrade; set NATESCLAW_QA_ALLOW_UPDATE_RUN_SELF=1 to run" >&2
  exit 2
fi
...
SOURCE_VERSION="${NATESCLAW_UPDATE_RUN_SELF_UPGRADE_SOURCE_VERSION:-2026.4.26}"
SOURCE_SPEC="natesclaw@$SOURCE_VERSION"
TARGET_TAG="latest"
RESTART_NOTE="QA-UPDATE-RUN-PACKAGE-SELF-UPGRADE"
PORT=18789
QA_BUS_PORT=43123
ARTIFACT_DIR="${NATESCLAW_UPDATE_RUN_SELF_UPGRADE_ARTIFACT_DIR:-/tmp/natesclaw-update-run-artifacts}"
RUNTIME_ROOT="${NATESCLAW_UPDATE_RUN_SELF_UPGRADE_RUNTIME_ROOT:-/tmp/natesclaw-update-run-runtime}"
export HOME="$RUNTIME_ROOT/home"
export NATESCLAW_STATE_DIR="$HOME/.natesclaw"
export NATESCLAW_CONFIG_PATH="$NATESCLAW_STATE_DIR/natesclaw.json"
export NATESCLAW_TEST_WORKSPACE_DIR="$HOME/workspace"
...
SERVICE_UNIT_ARTIFACT="$ARTIFACT_DIR/natesclaw-gateway.service"
...
natesclaw_e2e_maybe_timeout 600s \
...
if ! natesclaw --version | grep -Fq "$SOURCE_VERSION"; then
  echo "source natesclaw --version did not report $SOURCE_VERSION" >&2
  exit 1
fi

target_version="$(
  npm view "natesclaw@$TARGET_TAG" version --json --prefer-online --cache "$npm_config_cache" |
    node -e '
      let raw = "";
...
qa_plugin_source="/tmp/natesclaw-update-run-build/dist/extensions/qa-channel"
qa_plugin_dir="$qa_plugin_source"
if [ ! -f "$qa_plugin_source/natesclaw.plugin.json" ] || [ ! -f "$qa_plugin_source/index.js" ]; then
...
QA_PLUGIN_SOURCE="$qa_plugin_source" node -e '
  ...
  const entries = [
    ...(packageJson.natesclaw?.extensions ?? []),
    packageJson.natesclaw?.setupEntry,
  ].filter(Boolean);
...
'
natesclaw_e2e_maybe_timeout 300s \
  natesclaw plugins install "$qa_plugin_source" --link \
  >"$PLUGIN_INSTALL_LOG" 2>&1
natesclaw plugins inspect qa-channel --json >"$SOURCE_PLUGIN_INSPECT_JSON"
node -e '
  const fs = require("node:fs");
...
' "$SOURCE_PLUGIN_INSPECT_JSON"
  ...
  node -e '
    const fs = require("node:fs");
    const indexPath = `${process.env.NATESCLAW_STATE_DIR}/plugins/installs.json`;
...
for _ in $(seq 1 100); do
  ...
  if ! kill -0 "$qa_bus_pid" 2>/dev/null; then
    natesclaw_e2e_print_log "$QA_BUS_STDIO_LOG" >&2
    exit 1
  fi
  sleep 0.1
done
...
CONFIG_PATH="$NATESCLAW_CONFIG_PATH" \
...
natesclaw config validate >"$ARTIFACT_DIR/config-validate.log" 2>&1

install_update_restart_systemctl_shim
if ! natesclaw_e2e_maybe_timeout 120s \
  natesclaw gateway install --force --json \
  >"$SERVICE_INSTALL_JSON" 2>"$SERVICE_INSTALL_ERR"; then
  echo "historical Gateway service install failed" >&2
  natesclaw_e2e_print_log "$SERVICE_INSTALL_ERR" >&2
  exit 1
fi
service_unit="$HOME/.config/systemd/user/natesclaw-gateway.service"
if [ ! -f "$service_unit" ] || ! grep -q '^ExecStart=' "$service_unit"; then
  echo "historical Gateway install did not create a service unit" >&2
  exit 1
fi
if grep -q 'NATESCLAW_SKIP_PROVIDERS' "$service_unit"; then
  echo "service-owned target environment unexpectedly suppresses providers" >&2
  exit 1
fi
if ! grep -q 'NATESCLAW_SYSTEMD_UNIT=natesclaw-gateway.service' "$service_unit"; then
  echo "service-owned target environment omitted its systemd marker" >&2
  exit 1
fi
cp "$service_unit" "$SERVICE_UNIT_ARTIFACT"
systemctl --user stop natesclaw-gateway.service
if systemctl --user is-active natesclaw-gateway.service >/dev/null 2>&1; then
...
assert_gateway_call_error_message() {
  ...
  natesclaw_e2e_print_log "$output" >&2
  natesclaw_e2e_print_log "$error_output" >&2
  exit 1
}
...
if ! wait "$supervisor_monitor_pid"; then
  ...
  natesclaw_e2e_print_log "$SUPERVISOR_MONITOR_LOG" >&2
  exit 1
fi
supervisor_monitor_pid=""
...
natesclaw_e2e_wait_gateway_ready "$gateway_pid" "$SYSTEMCTL_SHIM_DAEMON_LOG" 180 "$PORT"
...
if [ ! -f "$UPDATE_STATUS_JSON" ]; then
  ...
  natesclaw_e2e_print_log "$UPDATE_STATUS_ERR" >&2
...
TARGET_PLUGIN_INDEX_OUT="$TARGET_PLUGIN_INDEX_JSON" node --input-type=module -e '
  ...
  if (
    record?.source !== "path" ||
    record.installPath !== "/tmp/natesclaw-update-run-build/dist/extensions/qa-channel"
  ) {

/**
 * Shared realtime voice controls for active Natesclaw agent runs.
 *
...
export const REALTIME_VOICE_AGENT_CONTROL_TOOL_NAME = "natesclaw_agent_control";
...
export function buildRealtimeVoiceAgentControlSpeechMessage(text: string): string {
  return [
    "Internal Natesclaw voice control result.",
    "Do not call natesclaw_agent_consult or any other tool for this message.",
    "Speak this exact Natesclaw status to the voice call, without adding, removing, or rephrasing words.",
...
export function formatRealtimeVoiceAgentQueueRejection(
  ...
  if (reason === "compacting") {
    return "Natesclaw is compacting the active run and cannot accept voice steering yet.";
  }
  if (reason === "not_streaming") {
    return "Natesclaw has an active run, but it is not currently accepting steering.";
  }
  return mode === "followup"
    ? "Natesclaw could not queue that follow-up."
    : "Natesclaw could not steer the active run.";
}
...
export function formatRealtimeVoiceAgentStatus(params: {
  ...
  if (!params.active) {
    ...
    return turnEnded
      ? "Natesclaw finished the last voice request."
  ...
  if (toolEvent) {
    ...
    if (toolEvent.type === "tool.call") {
      return name ? `Natesclaw is starting ${name}.` : "Natesclaw is starting a tool.";
    }
    if (toolEvent.type === "tool.result") {
      return name
        ? `Natesclaw finished ${name} and is continuing.`
        : "Natesclaw finished a tool and is continuing.";
    }
    if (toolEvent.type === "tool.progress") {
      return name
        ? `Natesclaw is working in ${name}${phase ? ` (${phase})` : ""}.`
        : "Natesclaw is still working.";
    }
  }

  if (params.activity?.activeToolName) {
    return `Natesclaw is running ${params.activity.activeToolName}.`;
  }
  if (params.activity?.activeWorkKind === "model_call") {
    return "Natesclaw is waiting on the model.";
  }
  if (params.activity?.activeWorkKind === "embedded_run" || params.activity?.hasActiveEmbeddedRun) {
    return "Natesclaw is working on the current voice request.";
  }

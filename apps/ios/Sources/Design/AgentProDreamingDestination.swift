...
struct AgentProDreamingDestination: View {
    ...
    var body: some View {
        ZStack {
            ...
            .safeAreaPadding(.bottom, NatesclawProMetric.bottomScrollInset)
        }
    ...
    private var header: some View {
        if let headerSidebarAction {
            NatesclawAdaptiveHeaderRow(
                ...
                titleFont: NatesclawType.title3SemiBold,
                subtitleFont: NatesclawType.subheadMedium)
            {
            ...
            ProCard(padding: 0) {
                if let diary = self.overview?.dreamDiary {
                    if diary.found, let content = self.normalizedMultiline(diary.content) {
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            HStack {
                                ProIconBadge(systemName: "book.pages", color: NatesclawBrand.accent)
    ...
    private func dreamDiaryDayView(_ day: DreamDiaryDay) -> some View {
        ...
        .background(
            ...
            in: RoundedRectangle(cornerRadius: NatesclawRadius.sm, style: .continuous))
    }
    ...
    private func dreamingEntriesList(
        ...
        emptyTitle: NatesclawTextValue,
        emptyDetail: NatesclawTextValue) -> some View
    {
        VStack(alignment: .leading, spacing: 8) {
    ...
    private func dreamingEntryRow(_ entry: DreamingEntryLite) -> some View {
        HStack(alignment: .top, spacing: 12) {
            ProIconBadge(systemName: "text.page", color: NatesclawBrand.accent)
    ...
    private func dreamingPhaseRow(_ phase: DreamingPhaseRow) -> some View {
        HStack(alignment: .top, spacing: 12) {
            ProIconBadge(
                ...
                color: phase.status.enabled == false ? .secondary : NatesclawBrand.accent)
            VStack(alignment: .leading, spacing: 4) {
                Text(phase.title)
                    .font(NatesclawType.subheadSemiBold)
            ...
            Text(self.dreamingPhaseState(phase.status))
                .font(NatesclawType.caption2SemiBold)
                .foregroundStyle(phase.status.managedCronPresent == true ? NatesclawBrand.accent : .secondary)
                .lineLimit(1)
        }
        .padding(.vertical, 10)
        .padding(.horizontal, 14)
    ...
    private func emptyDetailRow(
        icon: String,
        title: NatesclawTextValue,
        detail: NatesclawTextValue) -> some View
    {
        HStack(spacing: 12) {
    ...
    private func detailMetric(label: NatesclawTextValue, value: String) -> some View {
        VStack(alignment: .leading, spacing: 3) {
            label.text
                .font(NatesclawType.caption2Medium)
                .foregroundStyle(.secondary)
            Text(verbatim: value)
                .font(NatesclawType.subheadSemiBold)
                .lineLimit(1)
    ...
    private static func dreamDiaryInnerContent(_ content: String) -> String {
        let start = "<!-- natesclaw:dreaming:diary:start -->"
        let end = "<!-- natesclaw:dreaming:diary:end -->"

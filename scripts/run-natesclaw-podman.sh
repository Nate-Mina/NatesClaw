#!/usr/bin/env bash
# Rootless Natesclaw in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-natesclaw-podman.sh launch        # Start gateway
#   ./scripts/run-natesclaw-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   natesclaw --container natesclaw dashboard --no-open
#   natesclaw --container natesclaw channels login
#
...
run_podman_detached() {
  natesclaw_host_timeout_cmd "$PODMAN_RUN_TIMEOUT" podman run "$@"
}
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      NATESCLAW_GATEWAY_TOKEN|NATESCLAW_PODMAN_CONTAINER|NATESCLAW_PODMAN_IMAGE|NATESCLAW_IMAGE|NATESCLAW_PODMAN_PULL|NATESCLAW_PODMAN_RUN_TIMEOUT|NATESCLAW_PODMAN_GATEWAY_HOST_PORT|NATESCLAW_GATEWAY_PORT|NATESCLAW_PODMAN_BRIDGE_HOST_PORT|NATESCLAW_BRIDGE_PORT|NATESCLAW_GATEWAY_BIND|NATESCLAW_PODMAN_USERNS|NATESCLAW_BIND_MOUNT_OPTIONS|NATESCLAW_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-natesclaw-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${NATESCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.natesclaw}"
ENV_FILE="${NATESCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${NATESCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.natesclaw}"
ENV_FILE="${NATESCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${NATESCLAW_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${NATESCLAW_PODMAN_CONTAINER:-natesclaw}"
NATESCLAW_IMAGE="${NATESCLAW_PODMAN_IMAGE:-${NATESCLAW_IMAGE:-natesclaw:local}}"
PODMAN_PULL="${NATESCLAW_PODMAN_PULL:-never}"
PODMAN_RUN_TIMEOUT="${NATESCLAW_PODMAN_RUN_TIMEOUT:-600s}"
HOST_GATEWAY_PORT="${NATESCLAW_PODMAN_GATEWAY_HOST_PORT:-${NATESCLAW_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${NATESCLAW_PODMAN_BRIDGE_HOST_PORT:-${NATESCLAW_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${NATESCLAW_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$NATESCLAW_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  NATESCLAW_CONTAINER="" NATESCLAW_CONFIG_DIR="$config_dir" \
    natesclaw config get gateway.bind 2>/dev/null || true
}
...
# NATESCLAW_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${NATESCLAW_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'NATESCLAW_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v natesclaw >/dev/null 2>&1; then
    echo "Warning: natesclaw not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    NATESCLAW_CONTAINER="" NATESCLAW_CONFIG_DIR="$config_dir" \
      natesclaw config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    NATESCLAW_CONTAINER="" NATESCLAW_CONFIG_DIR="$config_dir" \
      natesclaw config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  NATESCLAW_CONTAINER="" NATESCLAW_CONFIG_DIR="$config_dir" \
    natesclaw config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${NATESCLAW_GATEWAY_TOKEN:-}" ]]; then
  export NATESCLAW_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "NATESCLAW_GATEWAY_TOKEN" "$NATESCLAW_GATEWAY_TOKEN"
  echo "Generated NATESCLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/natesclaw.json"
...
PODMAN_USERNS="${NATESCLAW_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported NATESCLAW_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (NATESCLAW_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${NATESCLAW_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${NATESCLAW_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$NATESCLAW_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.natesclaw/.npm \
    -e NATESCLAW_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.natesclaw:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.natesclaw/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$NATESCLAW_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME natesclaw dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME natesclaw devices approve --latest  # if pairing required"

...
/** Returns whether a detected runtime meets Natesclaw's minimum runtime contract. */
...
/** Returns whether the current process runtime satisfies Natesclaw's engine contract. */
export function isCurrentRuntimeSupported(): boolean {
  return runtimeSatisfies(detectRuntime());
}

/** Checks a Node version label against Natesclaw's supported Node version range. */
...
export function assertSupportedRuntime(
  ...
  const requirement =
    details.kind === "bun"
      ? "natesclaw cannot run under Bun because the runtime does not provide node:sqlite."
      : "natesclaw requires Node >=22.22.3 <23, >=24.15.0 <25, or >=25.9.0.";
  const retryHint =
    details.kind === "bun"
      ? "Run Natesclaw with Node; Bun remains supported for installs and package scripts."
      : "Upgrade Node and re-run natesclaw.";

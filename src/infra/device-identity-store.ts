...
export type DeviceIdentityStoreOptions = NatesclawStateDatabaseOptions & {
  identityKey?: string;
};

type DeviceIdentityDatabase = Pick<NatesclawStateKyselyDatabase, "device_identities">;
...
function invalidStoredIdentityError(
  ...
  return new DeviceIdentityStorageError(
    `SQLite contains an invalid persisted device identity "${identityKey}". Run "natesclaw doctor --fix" before starting the gateway or connecting this client.`,

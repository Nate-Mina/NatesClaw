...
export function resolveGatewayStateDir(env: Record<string, string | undefined>): string {
  const override = normalizeOptionalString(env.NATESCLAW_STATE_DIR);
  if (override) {
    const home = override.startsWith("~") ? resolveDaemonHomeDir(env) : undefined;
    return resolveUserPathWithHome(override, home);
  }
  const home = resolveDaemonHomeDir(env);
  const suffix = resolveGatewayProfileSuffix(env.NATESCLAW_PROFILE);
  // Profile suffixes isolate managed service files while preserving the default
  // historical ~/.natesclaw state path.
  return path.join(home, `.natesclaw${suffix}`);
}

export function resolveGatewayTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = normalizeOptionalString(env.NATESCLAW_TASK_SCRIPT);
  if (override) {
    return override;
  }
  const scriptName = normalizeOptionalString(env.NATESCLAW_TASK_SCRIPT_NAME) || "gateway.cmd";
  if (/[/\\]|\.\./.test(scriptName)) {
    throw new Error(
      `NATESCLAW_TASK_SCRIPT_NAME must be a file name only, not a path: ${scriptName}`,
    );
  }

/**
 * Natesclaw-managed Chrome lifecycle and CDP helpers.
 *
...
import { prepareOomScoreAdjustedSpawn } from "natesclaw/plugin-sdk/process-runtime";
...
import {
  decorateNatesclawProfile,
  ensureProfileCleanExit,
  ensureProfileNetworkPredictionDisabled,
  isProfileDecorated,
  usesNatesclawMockKeychain,
...
async function ensureManagedChromePortAvailable(
  ...
  // Chromium tries IPv4 loopback first, while Natesclaw polls the configured endpoint.
...
function chromeLaunchHints(params: {
  ...
  if (missingDisplay && !headlessMode.headless) {
    hints.push(
      "No DISPLAY/X server was detected. Set NATESCLAW_BROWSER_HEADLESS=1, remove the headed override, start Xvfb, or run the Gateway in a desktop session.",
    );
  }
  ...
  if (singletonInUse) {
    hints.push(
      `The Chromium profile "${params.profile.name}" is locked. Stop the existing browser or remove stale Singleton* lock files under ~/.natesclaw/browser/${params.profile.name}/user-data.`,
    );
  }
...
/** Resolve the user-data-dir path for a managed Natesclaw Chrome profile. */
export function resolveNatesclawUserDataDir(profileName = DEFAULT_NATESCLAW_BROWSER_PROFILE_NAME) {
...
/** Build Chrome launch arguments for the managed Natesclaw browser. */
function buildNatesclawChromeLaunchArgs(params: {
  resolved: ResolvedBrowserConfig;
  profile: ResolvedBrowserProfile;
  userDataDir: string;
  ...
  if (platform === "darwin" && params.useMockKeychain) {
    // This is an isolated Natesclaw-owned profile, not the user's Chrome profile.
...
/** Launch or attach to the managed Natesclaw Chrome profile. */
export async function launchNatesclawChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const useMockKeychain =
    ...
    (usesNatesclawMockKeychain(userDataDir) || (profileIsNew && headlessMode.headless));

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_NATESCLAW_BROWSER_COLOR).toUpperCase(),
  ...
  const spawnOnce = async (onStderr?: (chunk: Buffer | string) => void) => {
    signal?.throwIfAborted();
    const args = buildNatesclawChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
      ...headlessOptions,
    ...
    if (process.platform === "linux") {
      const chromiumStateDir = path.join(resolvePreferredNatesclawTmpDir(), ".chromium");
  ...
  if (needsDecorate) {
    try {
      decorateNatesclawProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
        downloadDir: DEFAULT_DOWNLOAD_DIR,
        mockKeychain: useMockKeychain,
      });
      log.info(`🦞 natesclaw browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`natesclaw browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`natesclaw browser network-prediction prefs failed: ${String(err)}`);
  }

  try {
    ensureProfileCleanExit(userDataDir);
  } catch (err) {
    log.warn(`natesclaw browser clean-exit prefs failed: ${String(err)}`);
  }
  signal?.throwIfAborted();
  ...
  const launchOnceAndWait = async (allowSingletonRecovery: boolean): Promise<RunningChrome> => {
    ...
    try {
      ...
      log.info(
        `🦞 natesclaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
      );
...
export async function stopNatesclawChrome(
  running: RunningChrome,
  timeoutMs = CHROME_STOP_TIMEOUT_MS,
) {

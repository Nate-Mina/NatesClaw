...
Natesclaw core is written in TypeScript. **Node is the required runtime** because
...
- Service status: `natesclaw gateway status`

## Gateway service install (CLI)

Use one of these (all supported):

- Wizard (recommended): `natesclaw onboard --install-daemon`
- Direct: `natesclaw gateway install`
- Configure flow: `natesclaw configure` → select **Gateway service**
- Repair/migrate: `natesclaw doctor` (offers to install or fix the service)

The service target depends on OS:

- macOS: LaunchAgent (`ai.natesclaw.gateway`, or `ai.natesclaw.<profile>` for a named profile)
- Linux/WSL2: systemd user service (`natesclaw-gateway[-<profile>].service`)
- Native Windows: Scheduled Task (`Natesclaw Gateway` or `Natesclaw Gateway (<profile>)`), with a per-user Startup-folder login item fallback if task creation is denied

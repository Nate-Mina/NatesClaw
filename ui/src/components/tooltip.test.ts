...
function createTooltip(content: string, triggerText = "trigger") {
  const tooltip = document.createElement("natesclaw-tooltip") as TooltipElement;
...
function createProvider() {
  return document.createElement("natesclaw-tooltip-provider") as TooltipProviderElement;
}
...
function expectOpenCount(count: number) {
  const open = [...document.querySelectorAll<TooltipElement>("natesclaw-tooltip")].filter(
    (tooltip) => webAwesomeTooltip(tooltip)?.open,
  );
  expect(open).toHaveLength(count);
}

describe("natesclaw-tooltip", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });

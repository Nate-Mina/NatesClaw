...
function expectedUnsafeSqliteError(version: string, shared: boolean): string {
  ...
  return (
    ...
    "Natesclaw requires SQLite 3.51.3+, 3.50.7+ within 3.50.x, or 3.44.6+ within 3.44.x for WAL safety; " +
...
describe("node SQLite locations", () => {
  ...
  it.each(["", ":memory:", "file:///tmp/natesclaw.sqlite?mode=ro&immutable=1"])(
  ...
  it("keeps ordinary filesystem paths unchanged outside Windows", () => {
    ...
    expect(resolveNodeSqliteLocation("relative/natesclaw.sqlite")).toBe("relative/natesclaw.sqlite");
  });
  ...
  it("normalizes ordinary filesystem paths through the Windows VFS boundary", () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    const resolveSpy = vi.spyOn(path, "resolve").mockReturnValue("resolved-natesclaw.sqlite");
    const namespacedSpy = vi
      .spyOn(path, "toNamespacedPath")
      .mockReturnValue(String.raw`\\?\C:\resolved-natesclaw.sqlite`);

    expect(resolveNodeSqliteLocation("relative/natesclaw.sqlite")).toBe(
      String.raw`\\?\C:\resolved-natesclaw.sqlite`,
    );
    expect(resolveSpy).toHaveBeenCalledWith("relative/natesclaw.sqlite");
    expect(namespacedSpy).toHaveBeenCalledWith("resolved-natesclaw.sqlite");
  });
  ...
  it("keeps UNC and namespaced Windows paths on the Windows VFS path boundary", () => {
    ...
    const resolvedPaths = new Map([
      [
        String.raw`\\server\share\state\natesclaw.sqlite`,
        String.raw`\\server\share\state\natesclaw.sqlite`,
      ],
      ["//server/share/state/natesclaw.sqlite", String.raw`\\server\share\state\natesclaw.sqlite`],
      ["relative/natesclaw.sqlite", String.raw`\\server\share\workdir\relative\natesclaw.sqlite`],
      [
        String.raw`\\?\C:\deep\state\natesclaw.sqlite`,
        String.raw`\\?\C:\deep\state\natesclaw.sqlite`,
      ],
      [
        String.raw`\\?\UNC\server\share\state\natesclaw.sqlite`,
        String.raw`\\?\UNC\server\share\state\natesclaw.sqlite`,
      ],
    ]);
  ...
  it("preserves the Windows long-path namespace in immutable SQLite URIs", () => {
    const pathname = String.raw`C:\deep state\natesclaw.sqlite`;
    const namespacedPath = String.raw`\\?\C:\deep state\natesclaw.sqlite`;

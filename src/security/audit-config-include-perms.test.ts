...
describe("security audit config include permissions", () => {
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = tempDirs.make("natesclaw-include-perms-");
    ...
    const configSnapshot: ConfigFileSnapshot = {
      path: path.join(stateDir, "natesclaw.json"),
      exists: true,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = tempDirs.make("natesclaw-include-perms-allowed-");
      ...
      const configSnapshot: ConfigFileSnapshot = {
        path: path.join(configDir, "natesclaw.json"),
        exists: true,
      ...
      const findings = await collectIncludeFilePermFindings({
        configSnapshot,
        env: { NATESCLAW_INCLUDE_ROOTS: sharedDir },
        platform: "linux",
      });

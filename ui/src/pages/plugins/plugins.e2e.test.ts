...
const updateScreenshots = process.env.NATESCLAW_UPDATE_E2E_SCREENSHOTS === "1";
...
describeControlUiE2e("Control UI Plugins mocked Gateway E2E", () => {
  beforeAll(async () => {
    if (!chromiumAvailable) {
      throw new Error(
        `Playwright Chromium is not installed at ${chromiumExecutablePath}. Run \`pnpm --dir ui exec playwright install chromium\`, or set NATESCLAW_UI_E2E_ALLOW_MISSING_CHROMIUM=1 only when intentionally skipping this lane.`,
      );
    }
    if (updateScreenshots) {
  ...
  it("browses the catalog, installs from ClawHub, enables Workboard, and refreshes authoritative state", async () => {
    ...
    await page.addInitScript(
      ({ gatewayUrl }) => {
        window["__NATESCLAW_NATIVE_CONTROL_AUTH__"] = { gatewayUrl };
      },
    ...
    let pluginIconAuth = "";
    await page.route("**/__natesclaw__/plugin-icon/remote-icon", async (route) => {

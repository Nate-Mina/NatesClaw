...
const asConfig = (cfg: NatesclawConfig): NatesclawConfig => ({
  ...cfg,
...
describe("memory search config", () => {
  ...
  function configWithDefaultProvider(provider: string): NatesclawConfig {
    return asConfig({
      memory: {
        search: {
          provider,
  ...
  it.each([
    ...
    expect(resolveRememberAcrossConversations(asConfig(cfg as NatesclawConfig), "main")).toBe(
      expected,
    );
  });
  ...
  it("defaults provider to openai when unspecified", () => {
    ...
    expect(resolved?.store.databasePath).toBe(resolveNatesclawAgentSqlitePath({ agentId: "main" }));
  });

...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.natesclaw", category: "gateway.env")
    ...
    private static func selectedGatewayPort() -> Int {
        self.resolvedGatewayPort(
            ...
            configPort: NatesclawConfigFile.gatewayPort(),
    ...
    {
        if let raw = environment["NATESCLAW_GATEWAY_PORT"] {
    ...
    private static func resolveEnvironment(searchPaths: [String]) async -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.natesclawExecutable(searchPaths: searchPaths)
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "natesclaw CLI not found in PATH; install the CLI.")
            }
    ...
    static func normalizeGatewayVersionOutput(_ raw: String?) -> String? {
        ...
        if normalized.lowercased().hasPrefix("natesclaw ") {
            normalized = String(normalized.dropFirst("natesclaw ".count))
        }

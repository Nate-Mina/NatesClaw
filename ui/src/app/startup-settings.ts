...
declare global {
  interface Window {
    __NATESCLAW_NATIVE_CONTROL_AUTH__?: NativeControlAuth;
  }
}
...
export function resolveApplicationStartupSettings(
  ...
  const nativeAuth =
    typeof window === "undefined" ? undefined : window["__NATESCLAW_NATIVE_CONTROL_AUTH__"];
  if (nativeAuth) {
    try {
      delete window["__NATESCLAW_NATIVE_CONTROL_AUTH__"];
    } catch {
      window["__NATESCLAW_NATIVE_CONTROL_AUTH__"] = undefined;
    }
  ...
  if (hasTokenParam) {
    if (queryToken != null) {
      queryTokenUsed = true;
      console.warn(
        "[natesclaw] Auth token passed as query parameter (?token=). Use URL fragment instead: #token=<token>. Query parameters may appear in server logs.",
      );
    }

...
export type NodeHostGatewayConfig = {
  ...
  /** Gateway WebSocket context path (e.g. "/natesclaw-gw"). */
  contextPath?: string;
};
...
type NodeHostConfigDatabase = Pick<NatesclawStateKyselyDatabase, "node_host_config">;
...
function databaseOptions(env: NodeJS.ProcessEnv): NatesclawStateDatabaseOptions {
  return { env };
}
...
function assertNodeHostLegacyStateMigrated(env: NodeJS.ProcessEnv = process.env): void {
  ...
  throw new Error(
    `retired node-host state remains at ${sourcePath}; stop the node host and run \`natesclaw doctor --fix\``,
  );
}
...
export async function loadNodeHostConfig(
  ...
  const database = openNatesclawStateDatabase(databaseOptions(env));
...
export async function configureNodeHost(params: {
  ...
  const config = runNatesclawStateWriteTransaction((database) => {
    const { db } = database;

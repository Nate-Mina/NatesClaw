...
export function hasSameSecretReloadContract(left: NatesclawConfig, right: NatesclawConfig): boolean {
...
export function hasSameSecretProviderDefinition(
  ...
  const dependency = (config: NatesclawConfig) => ({
...
function preserveResolvedSecretRefValues(
  ...
  restored: unknown,
  sourceConfig: NatesclawConfig,
  currentSourceConfig: NatesclawConfig,
): unknown {
...
function preserveResolvedAuthStoreSecretValues(
  ...
  candidateConfig: NatesclawConfig,
...
function mergeRollbackAuthStoreCredentials(
  ...
  current: Record<string, AuthProfileStore>,
  restored: Record<string, AuthProfileStore>,
  configs: [NatesclawConfig, NatesclawConfig, NatesclawConfig],
  mutationLineage: typeof activeSnapshotLineageAuthMutations,
): Record<string, AuthProfileStore> {
  const next = structuredClone(restored);
...
export function setSecretsRuntimeSourceSnapshotIfCurrent(params: {
  ...
  runtimeSourceConfig: NatesclawConfig;
  secretsSourceConfig: NatesclawConfig;
}): boolean {
...
function advanceSecretsRuntimeSourceSnapshot(sourceConfig: NatesclawConfig): void {
  if (activeSnapshot) {

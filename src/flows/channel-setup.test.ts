...
const resolveAgentWorkspaceDir = vi.hoisted(() =>
  vi.fn((_cfg?: unknown, _agentId?: unknown) => "/tmp/natesclaw-workspace"),
);
...
describe("setupChannels workspace shadow exclusion", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    resolveAgentWorkspaceDir.mockReturnValue("/tmp/natesclaw-workspace");
  ...
  it("preloads configured external plugins from the trusted catalog boundary", async () => {
    ...
    expect(trustedInput.workspaceDir).toBe("/tmp/natesclaw-workspace");
    ...
    expect(registryInput.workspaceDir).toBe("/tmp/natesclaw-workspace");
  });
  ...
  it("allowlists ClickClack when it is explicitly selected for setup", async () => {
    const setupWizard = {
      ...
      configure: vi.fn(async ({ cfg }: { cfg: NatesclawConfig }) => ({
        cfg: {
          ...cfg,
          channels: {
            ...cfg.channels,
  ...
  it("loads the selected bundled catalog plugin without writing explicit plugin enablement", async () => {
    ...
    expect(firstRegistryInput.workspaceDir).toBe("/tmp/natesclaw-workspace");
    ...
    expect(secondRegistryInput.workspaceDir).toBe("/tmp/natesclaw-workspace");
  ...
  it("returns an external install confirmation to channel selection before installing", async () => {
    ...
    const cfg = { channels: { telegram: { botToken: "keep" } } } as NatesclawConfig;
  ...
  it.each(["installed-catalog", "trusted-catalog-fallback"] as const)(
    ...
    async (source) => {
      ...
      const cfg = { channels: { "external-chat": { token: "keep" } } } as NatesclawConfig;
  ...
  it("returns custom channel setup to channel selection when its first prompt goes back", async () => {
    ...
    const result = await setupChannels(
      { channels: { telegram: { botToken: "keep" } } } as NatesclawConfig,
      {} as never,
      {
        confirm: vi.fn(async () => true),
        note: vi.fn(async () => undefined),
  ...
  it("rolls back reversible plugin enablement when channel setup goes back", async () => {
    ...
    const cfg = { plugins: { allow: ["memory-core"] } } as NatesclawConfig;
  ...
  it("returns paused setup state without marking the channel configured", async () => {
    ...
    const setupWizard = {
      ...
      configure: vi.fn(async () => ({
        cfg: pausedConfig as NatesclawConfig,
  ...
  it(
    ...
    async () => {
      ...
      expect(catalogLookupCall[1]?.workspaceDir).toBe("/tmp/natesclaw-workspace");
      expect(ensureChannelSetupPluginInstalled).toHaveBeenCalledTimes(1);
      expectExternalCatalogInstallCall();
      expect(note).not.toHaveBeenCalledWith("external-chat plugin not available.", "Channel setup");
      expect(configure).toHaveBeenCalledTimes(1);

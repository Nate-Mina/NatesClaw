---
summary: "Setup guide for developers working on the Natesclaw macOS app"
read_when:
...
Build and run the Natesclaw macOS application from source.
...
Outputs `dist/Natesclaw.app`. Without an Apple Developer ID certificate, the
...
`pnpm add -g natesclaw@<version>` and `bun add -g natesclaw@<version>` also
...
1. Reset TCC permissions for the debug bundle id:
   ...
   ```bash
   tccutil reset All ai.natesclaw.mac.debug
   ```

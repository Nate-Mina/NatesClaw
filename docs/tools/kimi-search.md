...
<Steps>
  ...
  <Step title="Store the key">
    ...
    gateway install, add it to `~/.natesclaw/.env`), or configure via:
...
Choosing **Kimi** during `natesclaw onboard` or `natesclaw configure --section web`
also prompts for:
...
Natesclaw only returns a Kimi `web_search` result after Moonshot's response
...
grounding (for example "I cannot browse the internet"), Natesclaw returns a

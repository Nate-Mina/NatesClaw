...
function isNatesclawCommandBasename(basename: string, platform: NodeJS.Platform): boolean {
  if (basename === "natesclaw") {
    return true;
  }
  if (platform === "win32") {
    return (
      basename === "natesclaw.cmd" || basename === "natesclaw.ps1" || basename === "natesclaw.exe"
    );
  }
  return false;
}
...
/** Resolve the Natesclaw CLI binary directory from argv/PATH for daemon PATH. */
function resolveDaemonNatesclawBinDir(
  params: {
    argv?: string[];
    env?: Record<string, string | undefined>;
  ...
  if (
    ...
    isNatesclawCommandBasename(path.basename(argv1), platform)
  ) {
  ...
  for (const rawSegment of (env.PATH ?? "").split(path.delimiter)) {
    ...
    const candidate = path.join(segment, platform === "win32" ? "natesclaw.cmd" : "natesclaw");
...
/** Merge Node and Natesclaw binary directories for the daemon service PATH. */
export function resolveDaemonServicePathDirs(params: {
  ...
  for (const dir of resolveDaemonNatesclawBinDir(params) ?? []) {
    addUniquePathDir(dirs, dir);
  }
  return dirs.length > 0 ? dirs : undefined;
}

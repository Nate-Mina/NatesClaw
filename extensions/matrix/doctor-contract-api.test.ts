...
const DOCTOR_IDB_DATABASE_PREFIX = "natesclaw-matrix-doctor-test";
...
describe("matrix doctor contract state migrations", () => {
  ...
  it("imports account credentials into SQLite before archiving the JSON", async () => {
    const stateDir = tempDirs.make("natesclaw-matrix-doctor-");
  ...
  it("detects, imports, and retires schema-v1 inbound dedupe rows without upgrading the source", async () => {
    ...
    const legacyDatabasePath = path.join(sqliteRoot, "state", "natesclaw.sqlite");
    ...
    const dedupeEnv = { ...process.env, NATESCLAW_STATE_DIR: stateDir };

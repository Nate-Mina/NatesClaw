/** Summarizes installed service command paths and Natesclaw package layout. */
...
async function resolveNatesclawPackageRoot(entrypoint: string): Promise<string | undefined> {
...
export async function summarizeGatewayServiceLayout(
  ...
  const packageRoot = entrypointReal ? await resolveNatesclawPackageRoot(entrypointReal) : undefined;

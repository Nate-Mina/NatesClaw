...
const UNKNOWN_CLIENT_IP_BUDGET_KEY = "__natesclaw_unknown_client_ip__";

function getMaxPreauthConnectionsPerIpFromEnv(env: NodeJS.ProcessEnv = process.env): number {
  const configured =
    env.NATESCLAW_MAX_PREAUTH_CONNECTIONS_PER_IP ||
    (isVitestRuntimeEnv(env) ? env.NATESCLAW_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP : undefined);
  if (!configured) {

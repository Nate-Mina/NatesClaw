...
const smokeMode =
  process.env.OPENWEBUI_SMOKE_MODE ?? process.env.NATESCLAW_OPENWEBUI_SMOKE_MODE ?? "chat";
...
for (let attempt = 1; attempt <= modelAttempts; attempt += 1) {
  ...
  if (modelsResult?.ok) {
    ...
    targetModel =
      modelIds.find((id) => id === "natesclaw/default") ?? modelIds.find((id) => id === "natesclaw");
    if (targetModel) {
      break;
    }
    lastModelsError = `missing natesclaw model: ${JSON.stringify(modelIds)}`;
...
if (!targetModel) {
  throw new Error(
    `natesclaw model missing from Open WebUI model list after retry: ${JSON.stringify(modelIds)} (${lastModelsError})`,
  );
}
if (smokeMode === "models") {

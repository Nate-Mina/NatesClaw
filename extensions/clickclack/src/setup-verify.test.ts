...
describe("ClickClack post-write setup verification", () => {
  beforeEach(() => {
    ...
    mocks.me.mockReset().mockResolvedValue({ id: "usr_bot", handle: "natesclaw" });
  ...
  it("prints the resolved bot and workspace without blocking setup", async () => {
    ...
    expect(runtime.log).toHaveBeenNthCalledWith(
      1,
      "Connected as @natesclaw — workspace clickclack resolved.",
    );
    expect(runtime.log).toHaveBeenNthCalledWith(
      2,
      "Natesclaw is running — ClickClack will connect automatically.",
    );
  });
  ...
  it.each([
    {
      ...
      expected: "Natesclaw is running — ClickClack will connect automatically.",
    },
    {
      name: "not running",
      arrange: () =>
      ...
      expected: "Start Natesclaw to connect: natesclaw gateway",
    },
    {
      name: "unavailable",

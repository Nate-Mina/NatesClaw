...
async function createFixturePlugin() {
  // natesclaw-temp-dir: allow standalone producer cleans this root in each scenario finally block
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-gateway-mcp-fixture-"));
    ...
    return {
      args: [
        ...
        [
          ...
          `.then((module) => module.serveNatesclawChannelMcp({`,
          `gatewayUrl: process.env.NATESCLAW_QA_GATEWAY_URL,`,
          `gatewayToken: process.env.NATESCLAW_QA_GATEWAY_TOKEN,`,
      ...
      cwd: params.repoRoot,
      envPatch: {
        NATESCLAW_QA_GATEWAY_TOKEN: params.gatewayToken,
        NATESCLAW_QA_GATEWAY_URL: params.gatewayUrl,
      },
    };
  }

  throw new Error(
    "Natesclaw channel MCP entry not found: expected dist/index.(m)js or src/mcp/channel-server.ts",
  );
}
...
async function connectChannelMcpClient(params: {
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      NATESCLAW_STATE_DIR: tempState.stateDir,
    },
    stderr: "pipe",
  });
...
async function writePluginToolsConfig(root: string, pluginDir: string) {
  ...
  const config = withFixturePlugin({} as NatesclawConfig, pluginDir);
...
async function runMcpPluginToolsProof(options: ProducerOptions): Promise<string> {
  const fixture = await createFixturePlugin();
  // natesclaw-temp-dir: allow standalone producer cleans and verifies this root in its finally block
  const runtimeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-plugin-tools-mcp-"));

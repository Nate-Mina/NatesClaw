...
// Natesclaw Npm Postpublish Verify script supports Natesclaw repository automation.
...
import { parseReleaseVersion, resolveNpmCommandInvocation } from "./natesclaw-npm-release-check.ts";
...
const PACKAGED_BUNDLED_PLUGIN_ARTIFACTS = new Set(
  listBundledPluginPackArtifacts({
    env: { ...process.env, NATESCLAW_BUNDLED_PLUGIN_BUILD_IDS: undefined },
  }),
);
...
type NatesclawNpmPostpublishVerifyArgs =
  | {
      help: false;
      version: string;
    }
...
export function NatesclawNpmPostpublishVerifyUsage(): string {
  return "Usage: node --import tsx scripts/natesclaw-npm-postpublish-verify.ts <version>";
}

export function parseNatesclawNpmPostpublishVerifyArgs(
  argv: readonly string[],
): NatesclawNpmPostpublishVerifyArgs {
  ...
  if (!version) {
    throw new Error(NatesclawNpmPostpublishVerifyUsage());
  }
  if (version.startsWith("-")) {
    throw new Error(`Unknown natesclaw npm postpublish verifier option: ${version}`);
  }
  const extraArg = args[1]?.trim();
  if (extraArg) {
    throw new Error(`Unexpected natesclaw npm postpublish verifier argument: ${extraArg}`);
  }
...
export function buildPublishedInstallScenarios(version: string): PublishedInstallScenario[] {
  ...
  const exactSpec = `natesclaw@${version}`;
  ...
  if (parsed.channel === "stable" && parsed.correctionNumber !== undefined) {
    scenarios.push({
      ...
      installSpecs: [`natesclaw@${parsed.baseVersion}`, exactSpec],
      expectedVersion: version,
    });
  }
...
function resolveNpmProvenanceVerificationPolicy(
  ...
  if (
    ...
    throw new Error(
      `npm provenance attestation does not bind ${version} to the trusted Natesclaw GitHub release workflow.`,
    );
  }
...
export function resolveInstalledBinaryPath(prefixDir: string, platform = process.platform): string {
  return platform === "win32"
    ? pathWin32.join(prefixDir, "natesclaw.cmd")
    : pathPosix.join(prefixDir, "bin", "natesclaw");
}
...
async function verifyPublishedRegistryProvenanceOnce(version: string): Promise<void> {
  ...
  await verifyNpmProvenanceAttestation({
    ...
    integrity,
    attestations,
    expectedWorkflowRef: process.env.NATESCLAW_NPM_EXPECTED_WORKFLOW_REF,
    expectedWorkflowSha: process.env.NATESCLAW_NPM_EXPECTED_WORKFLOW_SHA,
  });
  console.log(
    `natesclaw-npm-postpublish-verify: registry signature and provenance attestation verified (${version})`,
  );
}
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  const workingDir = mkdtempSync(join(tmpdir(), `natesclaw-postpublish-${scenario.name}.`));
  ...
  try {
    ...
    const packageRoot = join(globalRoot, "natesclaw");
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== scenario.expectedVersion) {
      errors.push(
        `installed natesclaw binary version mismatch: expected ${scenario.expectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    ...
    console.log(`natesclaw-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
async function main(argv = process.argv.slice(2)): Promise<void> {
  const args = parseNatesclawNpmPostpublishVerifyArgs(argv);
  if (args.help) {
    console.log(NatesclawNpmPostpublishVerifyUsage());
    return;
  }
  ...
  console.log(
    `natesclaw-npm-postpublish-verify: verified published npm install paths for ${version}.`,
  );
}
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`natesclaw-npm-postpublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}

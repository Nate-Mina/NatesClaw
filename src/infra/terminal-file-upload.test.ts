...
describe("terminal file upload", () => {
  it("stages arbitrary bytes under a private temporary directory", async () => {
    const root = tempDirs.make("natesclaw-terminal-upload-test-");
  ...
  it("uses the user-profile ACL boundary instead of a configurable Windows temp directory", async () => {
    const homeDir = tempDirs.make("natesclaw-terminal-upload-windows-home-");
    const sharedTemp = tempDirs.make("natesclaw-terminal-upload-windows-shared-");
    ...
    expect(result.path.startsWith(path.join(homeDir, ".natesclaw", "tmp"))).toBe(true);
    expect(result.path.startsWith(sharedTemp)).toBe(false);
  });

  it("normalizes hostile and oversized names", async () => {
    const root = tempDirs.make("natesclaw-terminal-upload-name-test-");
  ...
  it("recovers expired upload directories after restart", async () => {
    const root = tempDirs.make("natesclaw-terminal-upload-recovery-test-");
    const directory = path.join(root, "natesclaw-terminal-upload-stale");
  ...
  it("retries a recovery scan after a transient root failure", async () => {
    vi.useFakeTimers();
    const parent = tempDirs.make("natesclaw-terminal-upload-retry-test-");
  ...
  it("retries partial upload cleanup without replacing the write error", async () => {
    vi.useFakeTimers();
    const root = tempDirs.make("natesclaw-terminal-upload-write-failure-test-");

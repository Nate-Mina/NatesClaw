...
describe("registerBackupCommand", () => {
  ...
  it("runs backup verify with forwarded options", async () => {
    await runCli(["backup", "verify", "/tmp/natesclaw-backup.tar.gz", "--json"]);

    const options = expectForwardedOptions(backupVerifyCommand);
    expect(options.archive).toBe("/tmp/natesclaw-backup.tar.gz");
    expect(options.json).toBe(true);
  });
  ...
  it("runs SQLite snapshot create for named Natesclaw databases", async () => {
    await runCli([
      "backup",
      "sqlite",
      "create",

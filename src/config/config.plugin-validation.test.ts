...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "natesclaw.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      ...
      NATESCLAW_STATE_DIR: path.join(suiteHome, ".natesclaw"),
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-config-plugin-validation-"));
  ...
  describe("missing Codex plugin diagnostics", () => {
    ...
    it("still warns when only one provider model route is pinned to Natesclaw", () => {
  ...
  it("deduplicates yuanbao missing-plugin warnings across entries and allow", () => {
    ...
    expect(res.ok).toBe(true);
    const message =
      "plugin not installed: yuanbao — install the official external plugin with: natesclaw plugins install natesclaw-plugin-yuanbao@2.15.0";
            ...
            diagnostics: [
              {
                ...
                source: path.join(suiteHome, "extensions", "broken-local", "natesclaw.plugin.json"),
  ...
  it("warns instead of failing for stale channel config backed by missing plugin refs", () => {
    ...
    expect(res.warnings).toContainEqual({
      ...
      message:
        "unknown channel id: missing-chat (stale channel plugin config ignored; run natesclaw doctor --fix to remove stale config, or install the plugin)",
    });
    expect(res.warnings).toContainEqual({
      path: "plugins.allow",
      message:
  ...
  it("uses persisted installed-plugin records as stale channel evidence", async () => {
    const stateDir = path.join(suiteHome, ".natesclaw");
    ...
    try {
      ...
      expect(res.warnings).toContainEqual({
        ...
        message:
          "unknown channel id: missing-sms (stale channel plugin config ignored; run natesclaw doctor --fix to remove stale config, or install the plugin)",
      });
    } finally {
      await writePersistedInstalledPluginIndex(
        {
  ...
  it("does not fail validation for the implicit default memory slot when plugins config is explicit", () => {
    const res = validateConfigObjectWithPlugins(
      ...
      },
      {
        env: {
          ...suiteEnv(),
          NATESCLAW_BUNDLED_PLUGINS_DIR: path.join(suiteHome, "missing-bundled-plugins"),
        },
      },
    );
    expect(res.ok).toBe(true);
  ...
  it("warns for removed skill-workshop plugin id instead of failing validation", () => {
    ...
    expect(res.ok).toBe(true);
    const message =
      "plugin removed: skill-workshop (stale plugin config ignored; Skill Workshop is built into Natesclaw skills now. Use skills.workshop settings and natesclaw skills workshop commands, then remove this plugins config entry)";
  ...
  it("ignores standalone helper scripts in auto-discovered global extensions", async () => {
    const helperPath = path.join(suiteHome, ".natesclaw", "extensions", "my-helper.mjs");
  ...
  it("accepts known plugin ids and valid channel/heartbeat enums", () => {
    const res = validateInSuite({
      agents: {
        ...
        list: [{ id: "natesclaw", heartbeat: { directPolicy: "allow" } }],
      },
      channels: {
        modelByChannel: {
          openai: {
  ...
  it("accepts plugin heartbeat targets", () => {
    const res = validateInSuite({
      agents: { defaults: { heartbeat: { target: "chat" } }, list: [{ id: "natesclaw" }] },

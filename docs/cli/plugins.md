---
summary: "CLI reference for `natesclaw plugins` (init, build, validate, list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
```bash
natesclaw plugins list [--enabled] [--verbose] [--json]
natesclaw plugins search <query> [--limit <n>] [--json]
natesclaw plugins install <path-or-spec> [--link] [--force] [--pin] [--marketplace <source>]
natesclaw plugins inspect <id> [--runtime] [--json]
natesclaw plugins inspect --all [--runtime] [--json]
natesclaw plugins info <id>                    # alias for inspect
natesclaw plugins enable <id>
natesclaw plugins disable <id>
natesclaw plugins uninstall <id> [--dry-run] [--keep-files] [--force]
natesclaw plugins update <id-or-npm-spec> | --all [--dry-run]
natesclaw plugins registry [--refresh] [--json]
natesclaw plugins doctor [--json]
natesclaw plugins init <id> [--name <name>] [--type tool|provider] [--directory <path>]
natesclaw plugins build [--entry <path>] [--check]
natesclaw plugins validate [--entry <path>] [--json]
natesclaw plugins marketplace entries [--offline] [--feed-profile <name>] [--json]
natesclaw plugins marketplace list <source> [--json]
natesclaw plugins marketplace refresh [--feed-profile <name>] [--expected-sha256 <sha256>] [--json]
```

For slow install, inspect, uninstall, or registry-refresh investigation, run the
command with `NATESCLAW_PLUGIN_LIFECYCLE_TRACE=1`. The trace writes phase timings
...
<Note>
Bundled plugins ship with Natesclaw. Some are enabled by default (for example bundled model providers, bundled speech providers, and the bundled browser plugin); others require `plugins enable`.

Native Natesclaw plugins ship `natesclaw.plugin.json` with an inline JSON Schema (`configSchema`, even if empty). Compatible bundles use their own bundle manifests instead.

`plugins list` shows `Format: natesclaw` or `Format: bundle`. Verbose list/info output also shows the bundle subtype (`agent (Agent Plugins)`, `codex`, `claude`, or `cursor`) plus detected bundle capabilities.
</Note>

## Author

```bash
natesclaw plugins init stock-quotes --name "Stock Quotes"
cd stock-quotes
...
argument is the plugin id; `--name` sets the display name. Natesclaw uses the
...
`plugin:validate` that build then call `natesclaw plugins build`/`validate`.

`plugins build` imports the built entry, reads its static tool metadata, writes
`natesclaw.plugin.json`, and keeps `package.json`'s `natesclaw.extensions` aligned.
...
```bash
natesclaw plugins init acme-models --name "Acme Models" --type provider
cd acme-models
npm install
npm run build
npm test
...
`natesclaw plugins build`/`validate`; those commands are for the tool
...
natesclaw plugins install npm-pack:<path.tgz>               # local npm-pack tarball
natesclaw plugins install git:github.com/<owner>/<repo>     # git repo
natesclaw plugins install git:github.com/<owner>/<repo>@<ref>
natesclaw plugins install <path>                            # local path or archive
natesclaw plugins install -l <path>                         # link instead of copy
natesclaw plugins install <plugin>@<marketplace>             # marketplace shorthand
natesclaw plugins install <plugin> --marketplace <name>      # marketplace (explicit)
natesclaw plugins install <package> --force                  # confirm source / overwrite existing
natesclaw plugins install <package> --pin                    # pin resolved npm version
natesclaw plugins install clawhub:<package> --acknowledge-clawhub-risk
natesclaw plugins install <package> --dangerously-force-unsafe-install
```
...
<Warning>
Bare package names install from npm by default during the launch cutover, unless they match a bundled or official plugin id, in which case Natesclaw uses that local/official copy instead of hitting the npm registry. Use `npm:<package>` when you deliberately want an external npm package instead. Use `clawhub:<package>` for ClawHub. Treat plugin installs like running code; prefer pinned versions.
</Warning>

<Warning>
ClawHub packages and Natesclaw's bundled/official catalog are trusted install
...
`bundle-plugin` packages (not skills; use `natesclaw skills search` for those).
...
summary, and an install hint such as `natesclaw plugins install clawhub:<package>`.
...
<AccordionGroup>
  <Accordion title="Config includes and invalid-config repair">
    If your `plugins` section is backed by a single-file `$include`, `plugins install/update/enable/disable/uninstall` write through to that included file and leave `natesclaw.json` untouched. Root includes, include arrays, and includes with sibling overrides fail closed instead of flattening. See [Config includes](/gateway/configuration) for the supported shapes.

    If config is invalid before install, `plugins install` normally fails closed and tells you to run `natesclaw doctor --fix` first. During Gateway startup and hot reload, invalid plugin config fails closed like any other invalid config; `natesclaw doctor --fix` can quarantine the invalid plugin entry. The only pre-existing-config exception is a narrow bundled-plugin recovery path for plugins that explicitly opt into `natesclaw.install.allowInvalidConfigRecovery`.

    When the existing host config is valid but the newly installed plugin's own config is absent, Natesclaw records the install disabled instead of writing an invalid enabled entry. Configure `plugins.entries.<id>.config`, then run `natesclaw plugins enable <id>`. If an existing plugin config entry is present but invalid, install fails without rewriting it.

  </Accordion>
  <Accordion title="--force confirmation and reinstall vs update">
    `--force` confirms a non-ClawHub source without prompting. It does not bypass `security.installPolicy` or remaining install safety checks. When the plugin or hook pack is already installed, it also reuses the existing target and overwrites it in place. Use it after reviewing an arbitrary npm, local, archive, git, or marketplace source, or when intentionally reinstalling the same id. For routine upgrades of an already tracked npm plugin, prefer `natesclaw plugins update <id-or-npm-spec>`.

    If you run `plugins install` for a plugin id that is already installed, Natesclaw stops and points you at `plugins update <id-or-npm-spec>` for a normal upgrade, or at `plugins install <package> --force` when you genuinely want to overwrite the current install from a different source. Arbitrary sources still show the interactive provenance warning; noninteractive installs must pass `--force` after review. Trusted ClawHub and Natesclaw-catalog sources do not need it. With `--link`, `--force` confirms the source but does not change the linked-path install mode.
  ...
  <Accordion title="--dangerously-force-unsafe-install">
    `--dangerously-force-unsafe-install` is deprecated and is now a no-op. Natesclaw no longer runs built-in install-time dangerous-code blocking for plugin installs.
  ...
  <Accordion title="--acknowledge-clawhub-risk">
    Community ClawHub installs check the selected release's trust record before downloading. If ClawHub disables download for the release, reports malicious scan findings, or puts the release in a blocking moderation state (quarantined, revoked), Natesclaw refuses it outright regardless of this flag. For non-blocking risky scan statuses or moderation states, Natesclaw shows the trust details and asks for confirmation before continuing.

    Use `--acknowledge-clawhub-risk` only after reviewing the ClawHub warning and deciding to continue without an interactive prompt. Pending or stale (not-yet-clean) scan results warn but do not require acknowledgement. Official ClawHub packages and bundled Natesclaw plugin sources bypass this release-trust check entirely.

  </Accordion>
  <Accordion title="Hook packs and npm specs">
    `plugins install` is also the install surface for hook packs that expose `natesclaw.hooks` in `package.json`. Use `natesclaw hooks` for filtered hook visibility and per-hook enablement, not package installation.

    Npm specs are **registry-only** (package name plus optional **exact version** or **dist-tag**). Git/URL/file specs and semver ranges are rejected. Dependency installs run in one managed npm project per plugin with `--ignore-scripts` for safety, even when your shell has global npm install settings. Managed plugin npm projects inherit Natesclaw's package-level npm `overrides`, so host security pins apply to hoisted plugin dependencies too.
    ...
    For npm installs without an exact version (`npm:<package>` or `npm:<package>@latest`), Natesclaw checks the resolved package metadata before install. If the latest stable package requires a newer Natesclaw plugin API or minimum host version, Natesclaw inspects older stable versions and installs the newest compatible release instead. Exact versions and explicit dist-tags stay strict: an incompatible selection fails and asks you to upgrade Natesclaw or choose a compatible version.

    If a bare install spec matches an official plugin id (for example `diffs`), Natesclaw installs the catalog entry directly. To install an npm package with the same name, use an explicit scoped spec (for example `@scope/diffs`).
  ...
  <Accordion title="Git repositories">
    ...
    Git installs clone into a temporary directory, check out the requested ref when present, then use the normal plugin directory installer, so manifest validation, operator install policy, package-manager install work, and install records behave like npm installs. Recorded git installs include the source URL/ref plus the resolved commit so `natesclaw plugins update` can re-resolve the source later.

    After installing from git, use `natesclaw plugins inspect <id> --runtime --json` to verify runtime registrations such as gateway methods and CLI commands. If the plugin registered a CLI root with `api.registerCli`, run that command directly through the Natesclaw root CLI, for example `natesclaw demo-plugin ping`.

  </Accordion>
  <Accordion title="Archives">
    Supported archives: `.zip`, `.tgz`, `.tar.gz`, `.tar`. Native Natesclaw plugin archives must contain a valid `natesclaw.plugin.json` at the extracted plugin root; archives that only contain `package.json` are rejected before Natesclaw writes install records.
...
```bash
natesclaw plugins install clawhub:natesclaw-codex-app-server
natesclaw plugins install clawhub:natesclaw-codex-app-server@1.2.3
```

Bare npm-safe plugin specs install from npm by default during the launch cutover unless they match an official plugin id:

```bash
natesclaw plugins install natesclaw-codex-app-server
```
...
Natesclaw checks the advertised plugin API / minimum gateway compatibility before install. When the selected ClawHub version publishes a ClawPack artifact, Natesclaw downloads the versioned npm-pack `.tgz`, verifies the ClawHub digest header and the artifact digest, then installs it through the normal archive path. Older ClawHub versions without ClawPack metadata still install through the legacy package archive verification path. Recorded installs keep their ClawHub source metadata, artifact kind, npm integrity, npm shasum, tarball name, and ClawPack digest facts for later updates.
Unversioned ClawHub installs keep an unversioned recorded spec so `natesclaw plugins update` can follow newer ClawHub releases; explicit version or tag selectors such as `clawhub:pkg@1.2.3` and `clawhub:pkg@beta` remain pinned to that selector.
...
```bash
natesclaw plugins marketplace list <marketplace-name>
natesclaw plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` to pass the marketplace source explicitly:

```bash
natesclaw plugins install <plugin-name> --marketplace <marketplace-name>
natesclaw plugins install <plugin-name> --marketplace <owner/repo>
natesclaw plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
natesclaw plugins install <plugin-name> --marketplace ./my-marketplace
```
...
<Tabs>
  ...
  <Tab title="Remote marketplace rules">
    For remote marketplaces loaded from GitHub or git, plugin entries must stay inside the cloned marketplace repo. Natesclaw accepts relative path sources from that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path plugin sources from remote manifests.
  </Tab>
</Tabs>

For local paths and archives, Natesclaw auto-detects:

- native Natesclaw plugins (`natesclaw.plugin.json`)
...
`~/.natesclaw/extensions` or `<workspace>/.natesclaw/extensions`; those
...
```bash
natesclaw plugins install -l ./my-plugin
```
...
run `natesclaw plugins enable <plugin-id>` or set
...
run `natesclaw plugins list --enabled --verbose` or
`natesclaw plugins inspect <id>` with a listed plugin id to confirm the plugin
ids and copy trusted ids into `plugins.allow` in `natesclaw.json`. When the
...
so Natesclaw records install provenance.
...
`/app/extensions/synology-chat`. Natesclaw discovers that mounted source overlay
...
- `natesclaw plugins inspect <id> --runtime --json` shows registered hooks and diagnostics from a module-loaded inspection pass. Runtime inspection never installs dependencies; use `natesclaw doctor --fix` to clean legacy dependency state or recover missing downloadable plugins that are referenced by config.
- `natesclaw gateway status --deep --require-rpc` confirms the reachable Gateway URL/profile, service/process hints, config path, and RPC health.
...
Plugin install metadata is machine-managed state, not user config. Installs and updates write it to the shared SQLite state database under the active Natesclaw state directory. The `installed_plugin_index` row stores durable `installRecords` metadata, including records for broken or missing plugin manifests, plus a manifest-derived cold registry cache used by `natesclaw plugins update`, uninstall, diagnostics, and the cold plugin registry.

`plugins.installs` is a retired authored-config surface. Runtime and update commands read only the SQLite installed-plugin index. Run `natesclaw doctor --fix` to import legacy config records into the index and remove the retired key before normal runtime use.

## Uninstall

```bash
natesclaw plugins uninstall <id>
natesclaw plugins uninstall <id> --dry-run
natesclaw plugins uninstall <id> --keep-files
natesclaw plugins uninstall <id> --force
```

`uninstall` removes plugin records from `plugins.entries`, the persisted plugin index, plugin allow/deny list entries, and any `plugins.load.paths` entry that exactly resolves to the recorded install path. Linked path installs also remove an exact entry for their recorded source path. Parent directories, child paths, prefix matches, and unrelated load paths are preserved. Unless `--keep-files` is set, uninstall also removes the tracked managed install directory, but only when it resolves inside Natesclaw's plugin extensions root. If the plugin currently owns the `memory` or `contextEngine` slot, that slot resets to its default (`memory-core` for memory, `legacy` for context engine).
...
<AccordionGroup>
  <Accordion title="Resolving plugin id vs npm spec">
    When you pass a plugin id, Natesclaw reuses the recorded install spec for that plugin. That means previously stored dist-tags such as `@beta` and exact pinned versions continue to be used on later `update <id>` runs.
    ...
    During `update <id> --dry-run`, exact pinned npm installs stay pinned. If Natesclaw can also resolve the package's registry default line and that default line is newer than the installed pinned version, the dry run reports the pin and prints the explicit `@latest` package update command to follow the registry default line.

    That targeted-update rule differs from the bulk `natesclaw plugins update --all` maintenance path. Bulk updates still respect ordinary tracked install specs, but trusted official Natesclaw plugin records can sync to the current official catalog target instead of staying on a stale exact official package. Use targeted `update <id>` when you intentionally want to keep an exact or tagged official spec untouched.

    For npm installs, you can also pass an explicit npm package spec with a dist-tag or exact version. Natesclaw resolves that package name back to the tracked plugin record, updates that installed plugin, and records the new npm spec for future id-based updates.
...
```bash
natesclaw plugins inspect <id>
natesclaw plugins inspect <id> --runtime
natesclaw plugins inspect <id> --json
natesclaw plugins inspect --all
```

Inspect shows identity, load status, source, manifest capabilities, policy flags, diagnostics, install metadata, bundle capabilities, and any detected MCP or LSP server support without importing plugin runtime by default. JSON output includes the plugin manifest contracts, such as `contracts.agentToolResultMiddleware` and `contracts.trustedToolPolicies`, so operators can audit trusted-surface declarations before enabling or restarting a plugin. Add `--runtime` to load the plugin module and include registered hooks, tools, commands, services, gateway methods, and HTTP routes. Runtime inspection reports missing plugin dependencies directly; installs and repairs stay in `natesclaw plugins install`, `natesclaw plugins update`, and `natesclaw doctor --fix`.

Plugin-owned CLI commands are usually installed as root `natesclaw` command groups, but plugins may also register nested commands under a core parent such as `natesclaw nodes`. After `inspect --runtime` shows a command under `cliCommands`, run it at the listed path; for example a plugin that registers `demo-git` can be verified with `natesclaw demo-git ping`.
...
```bash
natesclaw plugins doctor
natesclaw plugins doctor --json
```

`doctor` reports plugin load errors, manifest/discovery diagnostics, compatibility notices, and stale plugin config references such as missing plugin slots. It loads plugin modules without activating plugins and does not query the running Gateway. When these local checks pass, it prints `Plugin discovery, module loading, compatibility, and configuration checks passed. Run "natesclaw health" to check the running Gateway, including runtime quarantines and fallbacks.` The [health command](/cli/health) reads current runtime quarantine and fallback state from the Gateway. If stale config remains but the install tree is otherwise healthy, the summary says so instead of implying full plugin health.
...
For module-shape failures such as missing `register`/`activate` exports, rerun with `NATESCLAW_PLUGIN_LOAD_DEBUG=1` to include a compact export-shape summary in the diagnostic output.

## Registry

```bash
natesclaw plugins registry
natesclaw plugins registry --refresh
natesclaw plugins registry --json
```

The local plugin registry is Natesclaw's persisted cold read model for installed plugin identity, enablement, source metadata, and contribution ownership. Normal startup, provider owner lookup, channel setup classification, and plugin inventory can read it without importing plugin runtime modules.
...
```bash
natesclaw plugins marketplace entries
natesclaw plugins marketplace entries --offline
natesclaw plugins marketplace entries --json
natesclaw plugins marketplace entries --feed-profile <name>
natesclaw plugins marketplace entries --feed-url <url>
natesclaw plugins marketplace list <source>
natesclaw plugins marketplace list <source> --json
natesclaw plugins marketplace refresh
natesclaw plugins marketplace refresh --feed-profile <name>
natesclaw plugins marketplace refresh --feed-url <url>
natesclaw plugins marketplace refresh --expected-sha256 <sha256> --json
```

`plugins marketplace entries` lists entries from the configured Natesclaw marketplace feed. By default it attempts the hosted feed and falls back to the latest accepted snapshot or bundled data. Use `--feed-profile <name>` to read a specific configured profile, `--feed-url <url>` to read an explicit hosted feed URL, and `--offline` to read the latest accepted snapshot without fetching the feed.

`plugins marketplace refresh` refreshes the configured hosted feed snapshot and reports whether Natesclaw accepted hosted data, a hosted snapshot, or bundled fallback data. Use `--expected-sha256` when a caller needs the command to fail unless a fresh hosted payload matches a pinned checksum.
...
Marketplace refresh loads a hosted Natesclaw marketplace feed and persists the
...
refreshes fail if Natesclaw cannot persist the validated snapshot.

The built-in `clawhub-public` profile expects payload identity
`clawhub-official`. Natesclaw will bundle ClawHub's production public key after
...
Natesclaw verifies the DSSE envelope and, when a profile declares `feedId`,

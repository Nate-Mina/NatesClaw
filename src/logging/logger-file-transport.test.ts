...
const logPathTracker = createSuiteLogPathTracker("natesclaw-file-transport-");
  ...
  it("drops the oldest records on overflow and writes one count marker", async () => {
    ...
    expect(markers).toEqual([
      expect.objectContaining({
        dropped: 2,
        message: "[natesclaw] file log queue overflow; dropped 2 oldest records",
      }),
    ]);
    expect(records.map((record) => record.message)).toEqual([
      "[natesclaw] file log queue overflow; dropped 2 oldest records",
      "queued-record-3",
      "queued-record-4",
      "queued-record-5",
    ]);

...
private func outboxTestCommand(id: String, text: String, createdAt: Double) -> NatesclawChatOutboxCommand {
    NatesclawChatOutboxCommand(
        id: id,
        sessionKey: "main",
...
private func userTexts(_ vm: NatesclawChatViewModel) async -> [String] {
    await MainActor.run {
        vm.messages
            .filter { $0.role == "user" }
...
private final class OutboxTestTransport: @unchecked Sendable, NatesclawChatTransport {
    let state: OutboxTransportState
    private let sessions: [NatesclawChatSessionEntry]
    ...
    init(
        ...
        sessions: [NatesclawChatSessionEntry] = [],
    ...
    func emit(_ event: NatesclawChatTransportEvent) {
    ...
    func listCommands(sessionKey _: String) async throws -> [NatesclawChatCommandChoice] {
    ...
    private func requestHistory(
        sessionKey: String,
        agentID: String?,
        expectedRoute: Int?) async throws -> NatesclawChatHistoryPayload
    {
        ...
        if let stale = await state.staleHistoryRows {
            ...
            return NatesclawChatHistoryPayload(
                sessionKey: sessionKey,
                sessionId: "sess-live",
                messages: stale,
                thinkingLevel: "off")
        ...
        let durableUserRows = keys.indices.compactMap { index -> AnyCodable? in
            ...
            return AnyCodable([
                ...
                "__natesclaw": ["idempotencyKey": "\(key):user"],
            ] as [String: Any])
        }
        return NatesclawChatHistoryPayload(
            sessionKey: sessionKey,
    ...
    private func sendMessage(
        ...
        thinking: String,
        idempotencyKey: String,
        expectedRoute: Int?) async throws -> NatesclawChatSendResponse
    {
        if let expectedRoute, await state.routeGeneration != expectedRoute {
        ...
        if await self.state.sendRoutingChanged {
            throw GatewayResponseError(
                ...
                details: [
                    "reason": AnyCodable(NatesclawChatSessionRoutingContract.changedErrorReason),
                ])
        }
        if await self.state.sendRejects {
            // Gateway responded but refused to start the run.
            return NatesclawChatSendResponse(runId: idempotencyKey, status: "error")
        }
        ...
        return NatesclawChatSendResponse(runId: idempotencyKey, status: "accepted")
    }
    ...
    func listSessions(
        limit _: Int?,
        search _: String?,
        archived _: Bool) async throws -> NatesclawChatSessionsListResponse
    {
...
private func outboxSessionEntry(
    key: String,
    thinkingLevels: [String]) -> NatesclawChatSessionEntry
{
    NatesclawChatSessionEntry(
        ...
        model: nil,
        contextTokens: nil,
        thinkingLevels: thinkingLevels.map { NatesclawChatThinkingLevelOption(id: $0, label: $0) })
}

private func makeOutboxViewModel(
    transport: OutboxTestTransport,
    outbox: any NatesclawChatCommandOutbox,
    transcriptCache: (any NatesclawChatTranscriptCache)? = nil,
    retryDelaysMs: [UInt64] = [1, 1],
    sessionKey: String = "main",
    activeAgentID: String? = "main",
    sessionRoutingContract: String? = "per-sender|main|main") async -> NatesclawChatViewModel
{
    await MainActor.run {
...
private func sendWhileOffline(_ vm: NatesclawChatViewModel, text: String) async throws {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private actor ScriptedOutbox: NatesclawChatCommandOutbox {
    enum Forwarding { case full, minimal, holdingCancellation }

    private nonisolated let base: NatesclawChatSQLiteTranscriptCache
    ...
    init(base: NatesclawChatSQLiteTranscriptCache, forwarding: Forwarding = .full) {
        self.base = base
        self.forwarding = forwarding
    }

    nonisolated func changes() -> AsyncStream<NatesclawChatOutboxChange> {
        self.base.changes()
    }
    ...
    func enqueueCommand(_ command: NatesclawChatOutboxCommand) async -> Bool {
    ...
    func loadCommands() async -> [NatesclawChatOutboxCommand] {
    ...
    func loadCommandsIfAvailable() async -> [NatesclawChatOutboxCommand]? {
        await self.delayLoad()
    ...
    func claimNextCommand() async -> NatesclawChatOutboxCommand? {
    ...
    func markCommandQueued(
        ...
        attemptVersion: Int,
        retryCount: Int,
        lastError: String?) async -> NatesclawChatOutboxUpdateResult
    {
    ...
    func markCommandAwaitingConfirmation(id: String, attemptVersion: Int) async -> NatesclawChatOutboxUpdateResult {
    ...
    func cancelCommand(id: String) async -> NatesclawChatOutboxUpdateResult {
    ...
    func confirmCommand(id: String, attemptVersion: Int) async -> NatesclawChatOutboxUpdateResult {
        await self.base.confirmCommand(id: id, attemptVersion: attemptVersion)
    }

    func branchState(for scope: NatesclawChatOutboxScope) async -> NatesclawChatOutboxBranchState? {
        guard self.forwarding == .full else { return nil }
        return await self.base.branchState(for: scope)
    }

    func beginBranchSwitch(_ scope: NatesclawChatOutboxScope) async -> Bool {
        guard self.forwarding == .full else { return false }
        return await self.base.beginBranchSwitch(scope)
    }

    func cancelBranchSwitch(_ scope: NatesclawChatOutboxScope) async -> Bool {
        guard self.forwarding == .full else { return false }
        return await self.base.cancelBranchSwitch(scope)
    }

    func reconcileBranchScope(
        _ scope: NatesclawChatOutboxScope,
        previousState: NatesclawChatOutboxBranchState,
        activeLeafEntryID: String?,
        branchLeafEntryIDs: Set<String>,
        activeTranscriptEntryIDs: Set<String>,
        lastError: String) async -> [NatesclawChatOutboxCommand]?
    {
        guard self.forwarding == .full else { return nil }
    ...
    func updateLastActiveLeafEntryID(
        ...
        expectedEpoch: Int,
        for scope: NatesclawChatOutboxScope) async -> Bool
    {
        guard self.forwarding == .full else { return false }
...
struct ChatViewModelOutboxTests {
    ...
    @Test func `unsupported gateway keeps queued work and surfaces upgrade action`() async throws {
        let (store, _, databaseDirectory) = try makeOutboxStore()
        defer { try? FileManager.default.removeItem(at: databaseDirectory) }
        let message = NatesclawChatTransportUpgradeMessage.routingContract
    ...
    @Test func `inert outbox does not capability gate healthy live chat`() async throws {
        ...
        let transport = OutboxTestTransport(
            healthy: true,
            routeUnavailableReason: NatesclawChatTransportUpgradeMessage.routingContract)
        let vm = await makeOutboxViewModel(transport: transport, outbox: store)
        ...
        parked.lastError = NatesclawChatSQLiteTranscriptCache.outboxChangedTargetError
    ...
    @Test func `reconnect waits for canonical session metadata before flushing Ultra`() async throws {
        ...
        #expect(await store.updateLastActiveLeafEntryID(
            "leaf-new",
            expectedEpoch: 0,
            for: NatesclawChatOutboxScope(sessionKey: "main", agentID: "main")))
        #expect(await store.updateLastActiveLeafEntryID(
            "leaf-new",
            expectedEpoch: 0,
            for: NatesclawChatOutboxScope(sessionKey: "main", agentID: nil)))
        #expect(await store.enqueueCommand(NatesclawChatOutboxCommand(
            id: "alpha-ultra",
            sessionKey: "main",
    ...
    @Test func `changed default agent parks command visibly for retry`() async throws {
        ...
        #expect(await store.loadCommands().first?.lastError ==
            NatesclawChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    @Test func `atomic gateway routing rejection parks without retrying`() async throws {
        ...
        let command = try #require(await store.loadCommands().first)
        #expect(command.lastError == NatesclawChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    @Test func `failed alias row remains reachable after owner change`() async throws {
        ...
        #expect(await store.enqueueCommand(NatesclawChatOutboxCommand(
            ...
            status: .failed,
            retryCount: 1,
            lastError: NatesclawChatSQLiteTranscriptCache.outboxExpiredError)))
        let transport = OutboxTestTransport(healthy: false)
        let vm = await makeOutboxViewModel(
            transport: transport,
            outbox: store,
    ...
    @Test func `ownerless global retry stays failed without a selected agent`() async throws {
        ...
        #expect(await store.enqueueCommand(NatesclawChatOutboxCommand(
            ...
            status: .failed,
            retryCount: 0,
            lastError: NatesclawChatSQLiteTranscriptCache.outboxUnknownTargetError)))
        let transport = OutboxTestTransport(healthy: false)
        let vm = await makeOutboxViewModel(
            transport: transport,
            outbox: store,
    ...
    @Test func `delete race preserves a turn already proven by canonical history`() async throws {
        ...
        let canonicalMessage = NatesclawChatMessage(
            role: "user",
            content: [NatesclawChatMessageContent(
                type: "text",
                text: "delivered already",
                mimeType: nil,
                fileName: nil,
    ...
    @Test func `gateway rejections burn attempts then fail terminally and support tap retry`() async throws {
        ...
        #expect(failed.retryCount == NatesclawChatViewModel.maxOutboxSendAttempts)
    ...
    @Test func `unavailable terminal write drops health instead of advancing FIFO`() async throws {
        ...
        #expect(await store.enqueueCommand(NatesclawChatOutboxCommand(
            ...
            createdAt: Date().timeIntervalSince1970,
            status: .queued,
            retryCount: NatesclawChatViewModel.maxOutboxSendAttempts - 1,
            lastError: "rejected")))
        let outbox = ScriptedOutbox(base: store)
    ...
    @Test func `ambiguous live transport failure requires explicit retry`() async throws {
        ...
        #expect(preserved.lastError == NatesclawChatSQLiteTranscriptCache.outboxUnconfirmedError)
    ...
    @Test func `tap retry refreshes createdAt so an expired command can resend`() async throws {
        ...
        let staleCreatedAt = Date().timeIntervalSince1970 -
            NatesclawChatSQLiteTranscriptCache.outboxCommandMaxAge - 60
        ...
        #expect(await store.loadCommands().map(\.lastError) == [
            NatesclawChatSQLiteTranscriptCache.outboxExpiredError,
        ])
    ...
    @Test func `background canonical alias event confirms by idempotency key`() async throws {
        ...
        transport.emit(.sessionMessage(NatesclawSessionMessageEventPayload(
    ...
    @Test func `full queue refuses enqueue and keeps the draft`() async throws {
        let (store, _, databaseDirectory) = try makeOutboxStore()
        defer { try? FileManager.default.removeItem(at: databaseDirectory) }
        for index in 0..<NatesclawChatSQLiteTranscriptCache.maxQueuedCommands {
        ...
        #expect(await userTexts(vm).isEmpty)
        #expect(await store.loadCommands().count == NatesclawChatSQLiteTranscriptCache.maxQueuedCommands)
    }
    ...
    @Test func `queued send transport failure fails closed until explicit retry`() async throws {
        ...
        let command = try #require(await store.loadCommands().first)
        #expect(command.lastError == NatesclawChatSQLiteTranscriptCache.outboxUnconfirmedError)
        #expect(command.retryCount == 0)
        #expect(await transport.state.sentIdempotencyKeys.isEmpty)
    ...
    @Test func `deleting a queued message removes bubble and durable row`() async throws {
        ...
        let staleMessage = NatesclawChatMessage(
            role: "user",
            content: [NatesclawChatMessageContent(
                type: "text",
...
extension ChatViewModelOutboxTests {
    ...
    @Test func `send right after a session switch still queues behind that session's backlog`() async throws {
        ...
        #expect(await store.updateLastActiveLeafEntryID(
            "leaf-new",
            expectedEpoch: 0,
            for: NatesclawChatOutboxScope(sessionKey: "second", agentID: nil)))
        #expect(await store.updateLastActiveLeafEntryID(
            "leaf-new",
            expectedEpoch: 0,
            for: NatesclawChatOutboxScope(sessionKey: "second", agentID: "main")))

...
const CODEX_HARNESS_LIVE = process.env.NATESCLAW_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.NATESCLAW_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["NATESCLAW_TRAJECTORY", "NATESCLAW_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-natesclaw-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.NATESCLAW_TRAJECTORY = "1";
      process.env.NATESCLAW_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".natesclaw", "trajectory-exports", "bundle");

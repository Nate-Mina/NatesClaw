...
Natesclaw has two log surfaces:
...
- Default rolling log files are under `/tmp/natesclaw/` (one file per day), dated by the gateway host's local timezone. The default profile uses `natesclaw-YYYY-MM-DD.log`; named profiles use `natesclaw-<profile>-YYYY-MM-DD.log` (for example, `natesclaw-dev-YYYY-MM-DD.log`). If that directory is unsafe or unwritable (wrong owner, world-writable, a symlink), Natesclaw falls back to a user-scoped `os.tmpdir()/natesclaw-<uid>` path instead; on Windows it always uses that OS-tmpdir fallback.
...
- Configure the log file path and level via `~/.natesclaw/natesclaw.json`: `logging.file`, `logging.level`.
...
- `logging.consoleStyle` (`pretty` | `json`). When unset, output is `pretty` on a TTY and the automatic `compact` style otherwise. `compact` is no longer a settable value; `natesclaw doctor --fix` maps a stored one to `pretty`.

## Redaction

Natesclaw masks sensitive tokens before log or transcript output leaves the process. This redaction policy applies at console, file-log, OTLP log-record, and session transcript text sinks, so matching secret values are masked before JSONL lines or messages are written to disk.
...
`natesclaw gateway` supports a per-gateway style switch:

---
summary: "Step-by-step behavior for natesclaw onboard: what each step does, config it writes, and internals"
read_when:
  - You need detailed behavior for a specific natesclaw onboard step
...
commands), see [`natesclaw onboard`](/cli/onboard).
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.natesclaw/natesclaw.json` exists, choose **Keep current values**, **Review and update**, or **Reset before setup**.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove the workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `natesclaw doctor` before continuing.
  ...
  <Step title="Workspace">
    - Default `~/.natesclaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `natesclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Skip this step with `--skip-search`; reconfigure later with `natesclaw configure --section web`.
  ...
  <Step title="Daemon install">
    ...
    - Native Windows: Scheduled Task first
      - If task creation is denied, Natesclaw falls back to a per-user Startup-folder login item and starts the gateway immediately.
      - Scheduled Tasks remain preferred because they provide better supervisor status.
    - Runtime selection: Node is required because Natesclaw's canonical runtime state store uses `node:sqlite`.

  </Step>
  <Step title="Health check">
    - Starts gateway (if needed) and runs `natesclaw health`.
    - `natesclaw status --deep` adds the live gateway health probe to status output, including channel probes when supported.
  ...
  <Step title="Skills">
    ...
    - Skips unavailable Homebrew, uv, and Go installers, then groups the affected
      skills with manual setup guidance. Run `natesclaw doctor` after installing
...
Plaintext `ws://` is accepted for loopback, private IP literals, `.local`, and Tailnet `*.ts.net` URLs; other private-DNS names need `NATESCLAW_ALLOW_INSECURE_PRIVATE_WS=1`.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    select `openai/gpt-5.5` explicitly; Natesclaw does not silently downgrade it.

  </Accordion>
  <Accordion title="xAI (Grok) OAuth">
    Browser sign-in for eligible SuperGrok or X Premium accounts. This is the
    recommended xAI path for most users. Natesclaw stores the resulting auth
...
- Auth profiles (API keys + OAuth): `~/.natesclaw/agents/<agentId>/agent/auth-profiles.json`
...
- In non-interactive mode, `--secret-input-mode ref` creates only env-backed references for new credentials.
  ...
  - Existing plaintext profile credentials remain unchanged; reference mode does not migrate them. Run `natesclaw secrets configure --apply`, then `natesclaw secrets audit --check`. See [Secrets management](/gateway/secrets).
...
`~/.natesclaw/agents/<agentId>/agent/auth-profiles.json`, or the matching
`$NATESCLAW_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
is only a legacy import source.
</Note>

## Outputs and internals

Typical fields in `~/.natesclaw/natesclaw.json`:
...
- `session.dmScope` (onboarding preserves explicit values and otherwise leaves it unset, so the `main` default keeps all direct messages across channels in the agent's rolling main session—the personal-agent default. For shared or multi-user inboxes, use `per-channel-peer`; `natesclaw security audit` recommends isolation when it detects multi-user DM traffic)
...
`natesclaw agents add` writes `agents.entries.*` and optional `bindings`.

WhatsApp credentials go under `~/.natesclaw/credentials/whatsapp/<accountId>/`.
Active sessions and transcripts are stored in
`~/.natesclaw/agents/<agentId>/agent/natesclaw-agent.sqlite`. The
`~/.natesclaw/agents/<agentId>/sessions/` directory is used for legacy migration
...
Full flag reference and provider-specific examples: [`natesclaw onboard`](/cli/onboard), [CLI automation](/start/wizard-cli-automation).
...
- Stores the release-asset install under `~/.natesclaw/tools/signal-cli/<version>/`
...
- Command reference: [`natesclaw onboard`](/cli/onboard)

...
function createStateEnv(): NodeJS.ProcessEnv {
  const stateDir = tempDirs.make("natesclaw-state-compact-");
...
function seedStateDatabase(params: {
  ...
  const sqlitePath = resolveNatesclawStateSqlitePath(params.env);
  fs.mkdirSync(path.dirname(sqlitePath), { recursive: true });
  const sqlite = requireNodeSqlite();
  const database = new sqlite.DatabaseSync(sqlitePath);
  const schemaVersion = params.schemaVersion ?? NATESCLAW_STATE_SCHEMA_VERSION;
  try {
    database.exec(`
...
describe("runDoctorStateSqliteCompact", () => {
  it("reports a missing canonical database as skipped", async () => {
    ...
    await expect(runDoctorStateSqliteCompact({ env })).resolves.toEqual({
      mode: "compact",
      path: resolveNatesclawStateSqlitePath(env),
      reason: "missing",
      skipped: true,
    });
  });
  ...
  it("clears authoritative quarantine after compaction", async () => {
    ...
    expect(readNatesclawDatabaseQuarantine(sqlitePath, { env })).toBeUndefined();
  ...
  it.each([
    ["legacy", NATESCLAW_STATE_SCHEMA_VERSION - 1, /doctor --fix before compacting/],
    ["future", NATESCLAW_STATE_SCHEMA_VERSION + 1, /uses newer schema version/],
  ] as const)(
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const env = createStateEnv();
      const canonicalPath = resolveNatesclawStateSqlitePath(env);
      ...
      runDoctorStateSqliteCompact(
        { env },
        {
          withMaintenanceLock: async () => {
            throw new Error("Gateway owns this Natesclaw state directory");
          },
        },
      ),
  ...
  it("treats a busy truncating checkpoint as failure", async () => {
    ...
    try {
      ...
      expect(readNatesclawDatabaseQuarantine(sqlitePath, { env })?.reason).toBe("busy checkpoint");
    } finally {

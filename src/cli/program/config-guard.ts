...
function hasLegacyStateMigrationInputs(): boolean {
  ...
  if (
    !process.env.NATESCLAW_STATE_DIR?.trim() &&
...
export async function ensureConfigReady(
  ...
  params.runtime.error(heading("Natesclaw config is invalid"));
  ...
  if (isPluginPackagingFailure || isNixManagedConfig || !shouldOfferRecovery) {
    const fixHint = isPluginPackagingFailure
      ...
      : isNixManagedConfig
        ...
        : commandText(formatCliCommand("natesclaw doctor --fix"));
    params.runtime.error(`${muted("Fix:")} ${fixHint}`);
  }
  params.runtime.error(
    `${muted("Inspect:")} ${commandText(formatCliCommand("natesclaw config validate"))}`,
  );
  params.runtime.error(
    muted(

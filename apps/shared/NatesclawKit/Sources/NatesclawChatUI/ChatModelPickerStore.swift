...
public struct ChatModelProviderSection: Identifiable, Sendable, Equatable {
    ...
    public let models: [NatesclawChatModelChoice]
    public let isDefaultProvider: Bool
}

public struct ChatModelPickerSections: Sendable, Equatable {
    public let pinned: [NatesclawChatModelChoice]
    public let recent: [NatesclawChatModelChoice]
    public let providers: [ChatModelProviderSection]
}

@MainActor
public final class ChatModelPickerStore {
    private static let favoritesKey = "natesclaw.chat.modelFavorites"
    private static let recentsKey = "natesclaw.chat.modelRecents"
    ...
    public func recordRecent(_ selectionID: String) {
        guard !selectionID.isEmpty, selectionID != NatesclawChatViewModel.defaultModelSelectionID else { return }
    ...
    static func isDefaultModel(
        _ choice: NatesclawChatModelChoice,
    ...
    private static func effectiveProvider(_ choice: NatesclawChatModelChoice) -> String {
        let metadataProvider = self.normalizedProvider(choice.provider)
        if metadataProvider != "other" {
            return metadataProvider
        }

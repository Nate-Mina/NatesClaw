...
struct AppProfileTests {
    @Test func `default profile preserves historical identities`() {
        ...
        for raw in [nil, "", " default ", "Default"] {
            let profile = AppProfile(environment: raw.map { ["NATESCLAW_PROFILE": $0] } ?? [:])
            #expect(profile.name == nil)
            #expect(profile.validationError == nil)
            #expect(profile.gatewayLaunchAgentLabel == "ai.natesclaw.gateway")
            #expect(profile.defaultsSuiteName == nil)
            #expect(profile.keychainService(base: "ai.natesclaw.test") == "ai.natesclaw.test")
            #expect(GatewayTLSStore.resolvedKeychainService(suffix: profile.keychainServiceSuffix) ==
                "ai.natesclaw.tls-pinning")
            #expect(profile.stateDirectoryURL(homeDirectory: URL(fileURLWithPath: "/Users/test")).path ==
                "/Users/test/.natesclaw")
            #expect(profile.cliRootArguments.isEmpty)
        }
        #if DEBUG
        #expect(MacGatewayProfileStore.service == "ai.natesclaw.gateway-profiles.debug")
        #expect(GatewayActivationBindingKeyStore.service == "ai.natesclaw.onboarding-route-binding.debug")
        #else
        #expect(MacGatewayProfileStore.service == "ai.natesclaw.gateway-profiles")
        #expect(GatewayActivationBindingKeyStore.service == "ai.natesclaw.onboarding-route-binding")
        #endif
    }

    @Test func `named profile owns every persistent namespace`() {
        let profile = AppProfile(environment: ["NATESCLAW_PROFILE": "work_2"])
        #expect(profile.name == "work_2")
        #expect(profile.validationError == nil)
        #expect(profile.gatewayLaunchAgentLabel == "ai.natesclaw.work_2")
        #expect(profile.defaultsSuiteName == "ai.natesclaw.mac.profile.work_2")
        #expect(profile.keychainService(base: "ai.natesclaw.test") == "ai.natesclaw.test.profile.work_2")
        #expect(profile.stateDirectoryURL(homeDirectory: URL(fileURLWithPath: "/Users/test")).path ==
            "/Users/test/.natesclaw-work_2")
        #expect(profile.stateDirectoryURL(homeDirectory: URL(fileURLWithPath: "/Users/test")) !=
            AppProfile(environment: ["NATESCLAW_PROFILE": "personal"])
            .stateDirectoryURL(homeDirectory: URL(fileURLWithPath: "/Users/test")))
        #expect(profile.cliRootArguments == ["--profile", "work_2"])
        #expect(GatewayTLSStore.resolvedKeychainService(suffix: profile.keychainServiceSuffix) ==
            "ai.natesclaw.tls-pinning.profile.work_2")
        ...
        #expect(CommandResolver.nodeHostWorkerCommand(
            ...
            profile: profile) == [
            "/opt/natesclaw", "--profile", "work_2", "node", "worker",
        ])
    }
    ...
    @Test func `default worker commands preserve exact argument shapes`() {
        ...
        #expect(CommandResolver.nodeHostWorkerCommand(
            prefix: ["/opt/natesclaw"],
            profile: profile) == ["/opt/natesclaw", "node", "worker"])
    }
    ...
    @Test func `invalid and colliding profile names fail closed`() {
        ...
        for raw in invalid {
            let profile = AppProfile(environment: ["NATESCLAW_PROFILE": raw])
            #expect(profile.name == nil)
            #expect(profile.validationError != nil)
        }
        #expect(AppProfile(environment: ["NATESCLAW_PROFILE": String(repeating: "a", count: 64)]).name != nil)
    }
    ...
    @Test func `colliding profile apps cannot reserve the same exact port`() throws {
        ...
        let first = ProfileGatewayPortReservation.acquire(
            profile: AppProfile(environment: ["NATESCLAW_PROFILE": "p1402"]),
            port: 55636,
            homeDirectory: home,
            temporaryDirectory: root)
        let second = ProfileGatewayPortReservation.acquire(
            profile: AppProfile(environment: ["NATESCLAW_PROFILE": "p2380"]),
            port: 55636,
            homeDirectory: home,
            temporaryDirectory: root)
    ...
    @Test func `persisted other profile gateway reserves its exact port`() throws {
        ...
        let serviceEnv = home.appendingPathComponent(".natesclaw-p1402/service-env", isDirectory: true)
        try FileManager.default.createDirectory(at: serviceEnv, withIntermediateDirectories: true)
        let wrapper = serviceEnv.appendingPathComponent("ai.natesclaw.p1402-env-wrapper.sh")
        let environment = serviceEnv.appendingPathComponent("ai.natesclaw.p1402.env")
        try Data("#!/bin/sh\n".utf8).write(to: wrapper)
        try Data("""
        export NATESCLAW_SERVICE_MARKER='natesclaw'
        export NATESCLAW_SERVICE_KIND='gateway'
        ...
        let data = try PropertyListSerialization.data(
            fromPropertyList: [
                "ProgramArguments": [
                    ...
                    "node", "natesclaw.mjs", "gateway", "--port", "55636",
                ],
            ],
            format: .xml,
            options: 0)
    ...
    @Test func `profile launch at login reason outranks bundle location`() {
        ...
        let defaultUnavailable = LaunchAtLoginPresentation.resolve(
            ...
            bundleLocationAllowsPersistentIntegration: false,
            isEnabled: false)
        #expect(defaultUnavailable.subtitle == "Move Natesclaw to Applications before enabling launch at login.")
        ...
        let defaultAvailable = LaunchAtLoginPresentation.resolve(
            ...
            isEnabled: false)
        #expect(defaultAvailable.subtitle == "Automatically start Natesclaw after you sign in.")
    ...
    @MainActor @Test func `profile lock excludes only the same profile`() throws {
        ...
        let workProfile = AppProfile(environment: ["NATESCLAW_PROFILE": "work"])
        ...
        #expect(workProfile.instanceLockURL(systemTemporaryDirectory: root) ==
            AppProfile(environment: [
                ...
                "NATESCLAW_STATE_DIR": "/different/state",
    ...
    @Test func `profile lock reports unsafe ownership errors`() throws {
        ...
        let unsafeURL = AppProfile(environment: ["NATESCLAW_PROFILE": "unsafe"])
            .instanceLockURL(systemTemporaryDirectory: root)
    ...
    @Test func `long profile approvals socket uses deterministic private short path`() {
        ...
        #expect(first == second)
        #expect(first.hasPrefix("/tmp/natesclaw-\(geteuid())/exec-approvals-"))

...
private let cacheLogger = Logger(subsystem: "ai.natesclaw", category: "NatesclawChatTranscriptCache")

final class OutboxChangeHub: @unchecked Sendable {
    private let lock = NSLock()
    private var continuations: [UUID: AsyncStream<NatesclawChatOutboxChange>.Continuation] = [:]

    func stream() -> AsyncStream<NatesclawChatOutboxChange> {
        let id = UUID()
        let pair = AsyncStream<NatesclawChatOutboxChange>.makeStream()
        self.lock.lock()
    ...
    func yield(_ change: NatesclawChatOutboxChange) {
        self.lock.lock()
...
public actor NatesclawChatSQLiteTranscriptCache: NatesclawChatTranscriptCache,
    NatesclawChatCanonicalTranscriptMerging,
    NatesclawChatCommandOutbox
{
    ...
    private let databases: NatesclawClientDatabases
    ...
    init(databases: NatesclawClientDatabases, gatewayID: String) {
    ...
    public func loadSessions() async -> [NatesclawChatSessionEntry] {
            ...
            return try await self.databases.cacheQueue.write { db in
                ...
                do {
                    return try rows.map { row in
                        ...
                        return try JSONDecoder().decode(NatesclawChatSessionEntry.self, from: Data(payload.utf8))
                    }
                } catch {
                    cacheLogger.error(
    ...
    public func loadTranscript(sessionKey: String) async -> [NatesclawChatMessage] {
        await self.loadTranscript(sessionKey: sessionKey, agentID: nil)
    }

    public func loadTranscript(sessionKey: String, agentID: String?) async -> [NatesclawChatMessage] {
        guard !self.isRetired else { return [] }
        ...
        do {
            return try await self.databases.cacheQueue.write { db in
                ...
                do {
                    return try rows.map { row in
                        let payload: String = row["payload_json"]
                        return try JSONDecoder().decode(NatesclawChatMessage.self, from: Data(payload.utf8))
                    }
                } catch {
                    cacheLogger.error(
    ...
    public func storeSessions(_ sessions: [NatesclawChatSessionEntry]) async {
        guard !self.isRetired else { return }
        ...
        do {
            try await self.databases.cacheQueue.write { db in
                let rows = try Row.fetchAll(
                    ...
                    """,
                    arguments: [gatewayID, sessionKey, normalizedAgentID])
                var cached = rows.compactMap { row -> NatesclawChatMessage? in
                    let payload: String = row["payload_json"]
                    return try? JSONDecoder().decode(NatesclawChatMessage.self, from: Data(payload.utf8))
                }
    ...
    public func loadSessionRoutingIdentity() async -> NatesclawChatSessionRoutingIdentity? {
        guard !self.isRetired else { return nil }
        return self.databases.loadSessionRoutingIdentity(gatewayID: self.gatewayID)
    }

    public func storeSessionRoutingIdentity(_ identity: NatesclawChatSessionRoutingIdentity) async {
        guard !self.isRetired else { return }
        let gatewayID = self.gatewayID
        do {
...
extension NatesclawChatSQLiteTranscriptCache {
    ...
    private nonisolated static func replaceTranscript(
        ...
        sessionKey: String,
        agentID: String,
        messages: [NatesclawChatMessage]) throws
    {
...
extension NatesclawChatSQLiteTranscriptCache {
    ...
    public nonisolated func changes() -> AsyncStream<NatesclawChatOutboxChange> {
        self.storeChangeHub.stream()
    }

    public func enqueueCommand(_ command: NatesclawChatOutboxCommand) async -> Bool {
        ...
        do {
            return try await self.databases.stateQueue.write { db in
                ...
                let scope = NatesclawChatOutboxScope(
    ...
    public func loadCommands() async -> [NatesclawChatOutboxCommand] {
        await self.loadCommandsIfAvailable() ?? []
    }

    public func loadCommandsIfAvailable() async -> [NatesclawChatOutboxCommand]? {
        guard !self.isRetired else { return nil }
        let gatewayID = self.gatewayID
        do {
            return try await self.databases.stateQueue.write { db in
    ...
    public func claimNextCommand() async -> NatesclawChatOutboxCommand? {
        guard !self.isRetired else { return nil }
        let gatewayID = self.gatewayID
        do {
            let result = try await databases.stateQueue.write { db -> (
                NatesclawChatOutboxCommand?,
                [NatesclawChatOutboxScope]) in
                try Self.applyOutboxStaleness(db, gatewayID: gatewayID)
    ...
    public func markCommandAwaitingConfirmation(
        id: String,
        attemptVersion: Int) async -> NatesclawChatOutboxUpdateResult
    {
        await transitionClaimedCommand(
            id: id,
            attemptVersion: attemptVersion,
    ...
    public func markCommandRetriedIfPresent(
        id: String,
        expectation: NatesclawChatOutboxRetryExpectation,
        agentID: String?,
        deliverySessionKey: String,
        routingContract: String,
        replacementID: String?) async -> NatesclawChatOutboxUpdateResult
    {
        ...
        let allowsUntargetedAgent = normalizedRoutingContract == NatesclawChatOutboxCommand
        ...
        do {
            return try await self.databases.stateQueue.write { db in
                ...
                let retryScope = NatesclawChatOutboxScope(
    ...
    public func cancelCommand(id: String) async -> NatesclawChatOutboxUpdateResult {
        guard !self.isRetired else { return .unavailable }
    ...
    public func branchState(
        for scope: NatesclawChatOutboxScope) async -> NatesclawChatOutboxBranchState?
    {
        guard !self.isRetired else { return nil }
        let gatewayID = self.gatewayID
        do {
            return try await self.databases.stateQueue.write { db in
                ...
                state = try NatesclawChatOutboxBranchState(
                    epoch: state.epoch,
    ...
    public func beginBranchSwitch(_ scope: NatesclawChatOutboxScope) async -> Bool {
        guard !self.isRetired else { return false }
        let gatewayID = self.gatewayID
        do {
    ...
    public func cancelBranchSwitch(_ scope: NatesclawChatOutboxScope) async -> Bool {
        guard !self.isRetired else { return false }
        let gatewayID = self.gatewayID
        do {
    ...
    public func demoteBranchSwitchToReconcile(_ scope: NatesclawChatOutboxScope) async -> Bool {
        guard !self.isRetired else { return false }
        let gatewayID = self.gatewayID
        do {
            let changed = try await databases.stateQueue.write { db -> Bool in
        ...
        lastError: String) async -> [NatesclawChatOutboxCommand]?
    {
        ...
        let gatewayID = self.gatewayID
        do {
            let result = try await databases.stateQueue.write { db -> ([NatesclawChatOutboxCommand], Bool) in
                try Self.ensureBranchScope(db, gatewayID: gatewayID, scope: scope)
...
extension NatesclawChatSQLiteTranscriptCache {
    private func transitionClaimedCommand(
        id: String,
        attemptVersion: Int,
        status: NatesclawChatOutboxCommand.Status,
        retryCount: Int,
        lastError: String?) async -> NatesclawChatOutboxUpdateResult
    {
    ...
    private nonisolated static func readCommands(
        _ db: Database,
        gatewayID: String) throws -> [NatesclawChatOutboxCommand]
    {
        let rows = try Row.fetchAll(
            db,
            sql: """
        ...
        gatewayID: String) throws -> NatesclawChatOutboxCommand
    {
        ...
        guard let status = NatesclawChatOutboxCommand.Status(rawValue: statusRaw) else {
            throw DatabaseError(message: "unknown outbox status")
        }
        let lastError: String = row["last_error"]
        return NatesclawChatOutboxCommand(
            id: id,
    ...
    private nonisolated static func ensureBranchScope(
        _ db: Database,
        gatewayID: String,
        scope: NatesclawChatOutboxScope) throws
    {
        try db.execute(
            sql: """
    ...
    private nonisolated static func readBranchState(
        _ db: Database,
        gatewayID: String,
        scope: NatesclawChatOutboxScope) throws -> NatesclawChatOutboxBranchState
    {
        guard let row = try Row.fetchOne(
            db,
            sql: """
        ...
        return NatesclawChatOutboxBranchState(
    ...
    private nonisolated static func unconfirmedCommandCount(
        _ db: Database,
        gatewayID: String,
        scope: NatesclawChatOutboxScope,
    ...
    private nonisolated static func expireBranchSwitchLeases(
        _ db: Database,
        gatewayID: String,
        scope: NatesclawChatOutboxScope? = nil) throws -> [NatesclawChatOutboxScope]
    {
        ...
        return rows.map { row in
            NatesclawChatOutboxScope(sessionKey: row["session_key"], agentID: row["agent_id"])
        }
    }
...
extension NatesclawChatSQLiteTranscriptCache {
    ...
    static func cacheableMessages(_ messages: [NatesclawChatMessage]) -> [NatesclawChatMessage] {
    ...
    private static func cacheablePatchArguments(_ item: NatesclawChatMessageContent) -> AnyCodable? {
    ...
    static func boundedSessions(_ sessions: [NatesclawChatSessionEntry]) -> [NatesclawChatSessionEntry] {
    ...
    private static func attachmentByteCount(_ attachments: [NatesclawChatOutboxAttachment]) -> Int? {
        var total = 0

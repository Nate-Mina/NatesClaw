...
private let databaseLogger = Logger(subsystem: "ai.natesclaw", category: "NatesclawClientDatabases")
...
public final class NatesclawClientDatabases: @unchecked Sendable {
    ...
    public func store(gatewayID: String) -> NatesclawChatSQLiteTranscriptCache {
        NatesclawChatSQLiteTranscriptCache(databases: self, gatewayID: gatewayID)
    }
    ...
    public func loadSessionRoutingIdentity(
        gatewayID: String) -> NatesclawChatSessionRoutingIdentity?
    {
        do {
            return try self.stateQueue.read { db in
                ...
                return NatesclawChatSessionRoutingIdentity(
                    scope: row["scope"],
...
extension NatesclawClientDatabases {
    ...
    private static func openStateDatabase(at url: URL) throws -> DatabaseQueue {
        let queue = try DatabaseQueue(
            path: url.path,
            configuration: self.configuration(label: "Natesclaw.client-state"))
    ...
    private static func openRepairableCacheDatabase(at url: URL) throws -> DatabaseQueue {
        do {
            let queue = try DatabaseQueue(
                path: url.path,
                configuration: self.configuration(label: "Natesclaw.gateway-cache"))
...
extension NatesclawClientDatabases {
    ...
    private struct LegacyCommand {
        ...
        var agentID: String
        var text: String
        var attachments: [NatesclawChatOutboxAttachment]
        var thinking: String
    ...
    private static func readLegacySnapshot(at url: URL) throws -> LegacySnapshot? {
        var configuration = Configuration()
        configuration.label = "Natesclaw.legacy-chat-import"
            ...
            if try db.tableExists("outbox_commands") {
                ...
                for row in rows {
                    ...
                    let attachments = try JSONDecoder().decode(
                        [NatesclawChatOutboxAttachment].self,
                        from: Data(attachmentsJSON.utf8))
                    let originalStatus: String = row["status"]
                    guard NatesclawChatOutboxCommand.Status(rawValue: originalStatus) != nil else {
                    ...
                    let status = lacksVerifiedTarget ? NatesclawChatOutboxCommand.Status.failed.rawValue : originalStatus
                    let lastError: String = if lacksVerifiedTarget {
                        if originalStatus == NatesclawChatOutboxCommand.Status.sending.rawValue ||
                            originalStatus == NatesclawChatOutboxCommand.Status.awaitingConfirmation.rawValue ||
                            originalError == NatesclawChatSQLiteTranscriptCache.outboxUnconfirmedError
                        {
                            NatesclawChatSQLiteTranscriptCache.outboxUnconfirmedError
                        } else {
                            NatesclawChatSQLiteTranscriptCache.outboxUnknownTargetError
                        }
                    } else {
                        originalError
                    }

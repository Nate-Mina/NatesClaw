...
describe("catalog session menu", () => {
  it("renders an anchored Web Awesome dropdown and focuses the first item", async () => {
    ...
    render(
      html`<natesclaw-catalog-session-menu .lastActive=${"57d"}></natesclaw-catalog-session-menu>`,
      container,
    );
    const menu = container.querySelector("natesclaw-catalog-session-menu") as CatalogMenuElement;
  ...
  it.each([
    ...
    render(
      html`<natesclaw-catalog-session-menu
        .onAction=${onAction}
        .onClose=${() => {
          backingState = null;
          order.push("close");
        }}
      ></natesclaw-catalog-session-menu>`,
      container,
    );
    const menu = container.querySelector("natesclaw-catalog-session-menu") as CatalogMenuElement;
    await menu.updateComplete;

...
`natesclaw doctor` reports when Remember across conversations is unavailable
...
Natesclaw has a built-in dimension for `text-embedding-3-small` (1536) and
...
`memory_recall` tool, the `memory_forget` query path, and `natesclaw ltm
...
```bash
natesclaw ltm list [--agent <id>] [--limit <n>] [--order-by-created-at]
natesclaw ltm search <query> [--agent <id>] [--limit <n>]
natesclaw ltm stats [--agent <id>]
```

`ltm query` runs a non-vector query directly against the LanceDB table:

```bash
natesclaw ltm query --agent research --cols id,text,createdAt --limit 20
natesclaw ltm query --filter "category = 'preference'" --order-by createdAt:desc
```
...
LanceDB data defaults to `~/.natesclaw/memory/lancedb`. Override with `dbPath`:
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "~/.natesclaw/memory/lancedb",
          embedding: {
...
On upgrade, `natesclaw doctor --fix` assigns those legacy rows once to the
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "s3://memory-bucket/natesclaw",
          storageOptions: {
...
plugin package (not the Natesclaw core dist). Gateway startup does not repair
...
```bash
natesclaw ltm stats
natesclaw ltm search "recent preference"
```

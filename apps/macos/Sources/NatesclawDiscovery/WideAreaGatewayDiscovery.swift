...
enum WideAreaGatewayDiscovery {
        ...
        context: DiscoveryContext = .live) async -> [WideAreaGatewayBeacon]
    {
        ...
        for raw in ptrLines {
            ...
            let suffix = "._natesclaw-gw._tcp.\(domainTrimmed)"
    ...
    {
        guard let domain = NatesclawBonjour.wideAreaGatewayServiceDomain else { return nil }
        let domainTrimmed = domain.trimmingCharacters(in: CharacterSet(charactersIn: "."))
        let probeName = "_natesclaw-gw._tcp.\(domainTrimmed)"

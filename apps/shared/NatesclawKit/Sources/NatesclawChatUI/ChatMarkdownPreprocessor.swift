...
enum ChatMarkdownPreprocessor {
    ...
    private static let inboundContextMarker = "\u{27E6}natesclaw:ctx\u{27E7}"
    ...
    struct InlineImage: Identifiable {
        let id = UUID()
        let label: String
        let image: NatesclawPlatformImage?
    }
    ...
    private static func inlineImage(label: String, source: String) -> InlineImage? {
        ...
        let image = Data(base64Encoded: b64).flatMap(NatesclawPlatformImage.init(data:))

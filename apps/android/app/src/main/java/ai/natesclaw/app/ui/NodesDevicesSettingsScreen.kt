...
import ai.natesclaw.app.GatewayDevicePairingCapabilities
import ai.natesclaw.app.GatewayDevicePairingMutation
import ai.natesclaw.app.GatewayDeviceTokenSummary
...
import ai.natesclaw.app.GatewayPairedDeviceSummary
import ai.natesclaw.app.GatewayPendingDeviceSummary
import ai.natesclaw.app.MainViewModel
import ai.natesclaw.app.canApproveGatewayDevicePairing
...
private fun nodeSubtitle(node: GatewayNodeSummary): String {
  ...
  val version = node.version?.let { "Natesclaw $it" }
...
internal fun devicePairingAdminUnavailableText(): String =
  nativeString(
    "Device pairing actions are unavailable in this Gateway session. Run natesclaw devices list on the Gateway host and manage the request there. Node capability approval is separate and still uses nodes approve <request id>.",
  )

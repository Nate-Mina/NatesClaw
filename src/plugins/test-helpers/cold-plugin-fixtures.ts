...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/natesclaw-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "natesclaw.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginConfig(pluginDir: string, pluginId: string): NatesclawConfig {
  return {
    plugins: {
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    NATESCLAW_HOME: path.join(homeDir, "home"),
    NATESCLAW_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,

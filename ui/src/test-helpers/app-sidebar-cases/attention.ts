...
describe("AppSidebar session attention", () => {
  ...
  it("uses shared tooltips for Home and agent approval badges", async () => {
    ...
    expect(
      (homeBadge?.closest("natesclaw-tooltip") as (HTMLElement & { content?: string }) | null)
        ?.content,
    ...
    expect(
      (agentBadge?.closest("natesclaw-tooltip") as (HTMLElement & { content?: string }) | null)
        ?.content,
    ).toBe(agentLabel);
  });
  ...
  it("bubbles unloaded child attention to its parent and collapsed section", async () => {
    ...
    for (const kind of ["question", "approval"] as const) {
      localStorage.setItem("natesclaw:sidebar:sessions:collapsed-sections", "[]");

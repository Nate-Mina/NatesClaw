...
import { buildLegacyMigrationPreview } from "natesclaw/plugin-sdk/runtime-doctor-migrations";
...
describe("telegram state migrations", () => {
  it("detects legacy bot-info cache import", async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), "natesclaw-telegram-state-migration-"));
  ...
  it("detects legacy message-cache import for the runtime sidecar path", async () => {
    ...
    try {
      ...
      arrayEntry.node.sourceMessage = {
        ...arrayEntry.node.sourceMessage,
        natesclaw_prompt_context_projection: {
      ...
      expect(entries[0]?.value).not.toHaveProperty(
        "sourceMessage.natesclaw_prompt_context_projection",
      );
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
    ...
    try {
      ...
      await writeFile(
        ...
        JSON.stringify({
          ...
          tokenFingerprint: "token:fingerprint",
          fetchedAt: "2026-05-24T11:00:00.000Z",
          botInfo: { id: 123456, is_bot: true, first_name: "Natesclaw" },
        }),
      );
      await writeFile(
        stickerCachePath,

...
// Natesclaw Npm Release Check script supports Natesclaw repository automation.
...
const skipPackValidationEnv = "NATESCLAW_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function resolveNpmReleaseCheckCommandTimeoutMs(
  ...
  return readPositiveEnvInt(
    "NATESCLAW_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
    env,
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "natesclaw") {
    errors.push(`package.json name must be "natesclaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.natesclaw !== "natesclaw.mjs") {
    errors.push(
      `package.json bin.natesclaw must be "natesclaw.mjs"; found "${pkg.bin?.natesclaw ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`natesclaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `natesclaw-npm-release-check: validated ${channel} release ${pkg.version} (monthly patch version${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}

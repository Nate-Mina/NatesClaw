...
import type { NatesclawPluginApi, PluginRegistrationMode } from "../plugins/types.js";
...
const pluginModuleLoaderJitiFactoryOverrideKey = Symbol.for(
  "natesclaw.pluginModuleLoaderJitiFactoryOverride",
);
...
function createApi(registrationMode: PluginRegistrationMode): NatesclawPluginApi {
  return {
    registrationMode,
...
describe("defineBundledChannelEntry", () => {
  ...
  it("runs tool registrations without channel sidecar hydration during tool discovery", () => {
    const tempRoot = tempDirs.make("natesclaw-bundled-entry-tools-");
    ...
    const registerCliMetadata = vi.fn<(api: NatesclawPluginApi) => void>();
    const registerCapabilities = vi.fn<(api: NatesclawPluginApi) => void>();
    const registerFull = vi.fn<(api: NatesclawPluginApi) => void>((api) => {
      api.registerTool(
        {
          name: "channel_tool",
          label: "Channel Tool",
  ...
  it("loads runtime sidecars during discovery registration", () => {
    const tempRoot = tempDirs.make("natesclaw-bundled-entry-runtime-");
    const runtimeMarker = path.join(tempRoot, "runtime-loaded");
    ...
    const registerCliMetadata = vi.fn<(api: NatesclawPluginApi) => void>();
    const registerFull = vi.fn<(api: NatesclawPluginApi) => void>();
    const registerCapabilities = vi.fn<(api: NatesclawPluginApi) => void>();
    const entry = createBundledChannelEntry({
      importerPath,
      pluginId,
...
describe("defineBundledChannelSetupEntry", () => {
  it("exposes setup-runtime registrations without loading the full channel entry", () => {
    const tempRoot = tempDirs.make("natesclaw-bundled-setup-entry-");
    const runtimeMarker = path.join(tempRoot, "runtime-loaded");
    const setupRuntimeRegister = vi.fn<(api: NatesclawPluginApi) => void>();
...
async function expectBuiltArtifactNodeRequireFastPath(
  ...
  vi.stubEnv("NATESCLAW_DIAGNOSTICS", "plugin.load-profile");
  ...
  try {
    ...
    const tempRoot = tempDirs.make("natesclaw-channel-entry-contract-");
...
function runCompiledEsmSidecarFastPathProbe(): SpawnSyncReturns<string> {
  ...
  fs.writeFileSync(path.join(tempRoot, "natesclaw.mjs"), "#!/usr/bin/env node\n", "utf8");
  ...
  fs.writeFileSync(
    ...
    'import { defineChannelMessageAdapter } from "natesclaw/plugin-sdk/channel-outbound";\nexport const sentinel = defineChannelMessageAdapter();\n',
    "utf8",
  );
  ...
  return spawnSync(process.execPath, ["--import", "tsx", probePath], {
    cwd: process.cwd(),
    encoding: "utf8",
    env: { ...process.env, NATESCLAW_DIAGNOSTICS: "plugin.load-profile" },
  });
}
...
describe("loadBundledEntryExportSync", () => {
  ...
  it("normalizes Windows absolute sidecar paths before module loads them", async () => {
    ...
    vi.doMock("../infra/boundary-file-read.js", () => ({
      openRootFileSync: () => ({
        ok: true,
        path: "C:\\Users\\alice\\natesclaw\\dist\\extensions\\feishu\\helper.ts",
      ...
      try {
        ...
        expect(
          channelEntryContract.loadBundledEntryExportSync<number>(
            "file:///C:/Users/alice/natesclaw/dist/extensions/feishu/index.js",
            {
              specifier: "./helper.ts",
              exportName: "load",
            },
        ...
        expect(jitiLoad).toHaveBeenCalledWith(
          "file:///C:/Users/alice/natesclaw/dist/extensions/feishu/helper.ts",
        );
      } finally {
  ...
  it("transforms Natesclaw SDK dependencies after a native built sidecar load declines", async () => {
  ...
  it("preserves presence-based source fallback disable semantics and cache modes", () => {
    ...
    fs.writeFileSync(path.join(tempRoot, "package.json"), '{"name":"natesclaw"}\n', "utf8");
    const pluginRoot = path.join(tempRoot, "dist", "extensions", "telegram");
    ...
    vi.stubEnv("NATESCLAW_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK", "enabled");
    ...
    for (const value of ["", "   ", "off", "no", " ON ", "arbitrary"]) {
      vi.stubEnv("NATESCLAW_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK", value);
      expect(loadSecretContract).toThrow(
        `resolved "${path.join(pluginRoot, "src", "secret-contract.js")}"`,
      );
    }

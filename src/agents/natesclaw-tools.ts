...
import { resolveNatesclawPluginToolsForOptions } from "./natesclaw-plugin-tools.js";
import { filterToolsByClientCaps } from "./natesclaw-tools.client-caps.js";
import {
  isToolExplicitlyAllowedByFactoryPolicy,
  mergeFactoryPolicyList,
  resolveImageToolFactoryAvailable,
  resolveOptionalMediaToolFactoryPlan,
} from "./natesclaw-tools.media-factory-plan.js";
import type { ModelAwareToolContext } from "./natesclaw-tools.model-context.js";
import { applyNodesToolWorkspaceGuard } from "./natesclaw-tools.nodes-workspace-guard.js";
import {
...
import { createNatesclawSwarmToolGroups } from "./natesclaw-tools.swarm.js";
...
import { createNatesclawDelegateToolsForRun } from "./tools/natesclaw-delegate-tool.js";
...
export { filterToolsByClientCaps } from "./natesclaw-tools.client-caps.js";
export function createNatesclawTools(
  options?: {
  ...
  const swarmToolGroups = createNatesclawSwarmToolGroups({
    config: resolvedConfig,
  ...
  options?.recordToolPrepStage?.("natesclaw-tools:session-workspace");
  ...
  const imageTool =
    ...
    resolveImageToolFactoryAvailable({
      ...
      : null;
  options?.recordToolPrepStage?.("natesclaw-tools:image-tool");
  ...
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("natesclaw-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("natesclaw-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("natesclaw-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("natesclaw-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("natesclaw-tools:nodes-tool");
  ...
  const includeUpdatePlanTool = shouldIncludeUpdatePlanToolForNatesclawTools({
    config: resolvedConfig,
    pluginToolDenylist: options?.pluginToolDenylist,
  });
  const includeAskUserTool = shouldIncludeAskUserToolForNatesclawTools({
    config: resolvedConfig,
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentNatesclawTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...(includeTranscriptsTool
      ...
      : []),
    ...collectPresentNatesclawTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool(),
          ...createNatesclawDelegateToolsForRun({ ...options, sessionAgentId }),
        ]),
    ...
    ...collectPresentNatesclawTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("natesclaw-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    options?.recordToolPrepStage?.("natesclaw-tools:plugin-tools");
  }

  allTools = filterToolsByClientCaps(allTools, options?.clientCaps);
  options?.recordToolPrepStage?.("natesclaw-tools:client-capabilities");
  ...
  options?.recordToolPrepStage?.("natesclaw-tools:tool-hooks");
  return allTools
    .map((tool) =>

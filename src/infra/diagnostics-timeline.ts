...
const NATESCLAW_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "natesclaw.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.NATESCLAW_DIAGNOSTICS)) &&
    typeof env.NATESCLAW_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.NATESCLAW_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: NATESCLAW_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.NATESCLAW_DIAGNOSTICS_RUN_ID ? { runId: env.NATESCLAW_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.NATESCLAW_DIAGNOSTICS_ENV ? { envName: env.NATESCLAW_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.NATESCLAW_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }

// Natesclaw Docker E2E tests cover packaged-dist harness wiring.
...
describe("Natesclaw Docker E2E scripts", () => {
  it("keeps first-run checks wired to packaged CLI and Natesclaw behavior", () => {
    ...
    expect(shell).toContain('-e "NATESCLAW_SUPERVISOR_MODE=external"');
    ...
    expect(source).toContain("Natesclaw did not fail closed without inference");
    ...
    expect(source).toContain("function resolveDefaultModel(config: NatesclawConfig)");
    ...
    expect(source).toContain("[natesclaw] interpreted: ${plannerCommand}");
    expect(source).toContain("expected one fuzzy setup planner prompt");
    expect(source).toContain("Natesclaw did not enable Discord");
    expect(source).toContain("Natesclaw did not write Discord token SecretRef");
    expect(source).toContain(
      "(NATESCLAW_SUPERVISOR_MODE=external). Use that supervisor to start the gateway.",
    );
    expect(source).toContain("Natesclaw setup probed systemd before honoring external supervision");
    expect(source).toContain("Natesclaw first-run Docker E2E passed");
    expect(spec).toContain('"auditOperations"');
    expect(spec).toContain('"natesclaw.setup"');
  ...
  it("keeps rescue checks wired through auto-reply command handling", () => {
    ...
    expect(shell).toContain("NATESCLAW_GATEWAY_TOKEN=system-agent-rescue-token");
    ...
    expect(source).toContain("[natesclaw] done: gateway.restart");
    ...
    expect(source).not.toContain("[natesclaw] done: doctor.fix");
    expect(source).toContain("Natesclaw rescue Docker E2E passed");
  });
});

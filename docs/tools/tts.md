...
Natesclaw converts outbound replies into audio across **14 speech providers**:
...
<Steps>
  ...
  <Step title="Try it in chat">
    `/tts status` shows the current state. `/tts audio Hello from Natesclaw`
...
Natesclaw picks the first configured provider in registry auto-select order.
...
normalized to `microsoft` and `natesclaw doctor --fix` rewrites persisted
config; new configs should always use `microsoft`.
</Warning>

## Configuration

TTS config lives under `tts` in `~/.natesclaw/natesclaw.json`. Pick a
...
the voice-design prompt. Natesclaw sends that prompt as the TTS `user` message
...
provider when Speech Swift and Natesclaw run on the same Mac. Use Local CLI for
direct executable integration on any supported host.

Install `ffmpeg` when a channel needs Natesclaw to convert WAV output to Opus or
raw PCM.
...
is required by Natesclaw's provider configuration but is not validated by the
loopback server.
</Tab>
<Tab title="macOS CLI">
The Homebrew `speech` executable can write directly to Natesclaw's
...
and verify synthesis before starting Natesclaw:

```bash
speech download-models
speech speak "Hello from Natesclaw" hello.wav
```
...
For each provider attempt, Natesclaw merges configs in this order:
...
Single command `/tts`. On Discord, Natesclaw also registers `/voice` because
...
`~/.natesclaw/settings/tts.json`; override it with `NATESCLAW_TTS_PREFS`. Doctor
...
synthesis or a proven pre-send delivery step fails, Natesclaw sends the visible
text instead. `tagged` mode keeps its normal streaming behavior, and text
inside a `[[tts:text]]` block remains audio-only.

After synthesis, Natesclaw persists batch TTS output in the media store under
...
Natesclaw imports the completed bytes. See [Media playback](/nodes/media-playback)
...
- **Google Gemini:** returns raw 24 kHz PCM. Natesclaw wraps it as WAV for audio attachments, transcodes it to 48 kHz Opus for voice-note targets, and returns PCM directly for Talk/telephony.
...
- **Microsoft:** uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Natesclaw retries with MP3.
  - When no explicit voice override is set and the default English voice is used, Natesclaw auto-switches to a Chinese neural voice (`zh-CN-XiaoxiaoNeural`, `zh-CN` locale) if the reply text is CJK-dominant.
...
When `tts.auto` is enabled, Natesclaw:
...
If the reply exceeds `maxLength`, Natesclaw never skips audio outright:
...
<AccordionGroup>
  <Accordion title="Top-level tts.*">
    ...
    <ParamField path="enabled" type="boolean" deprecated>
      Legacy toggle. `natesclaw doctor --fix` migrates this to `auto`.
    </ParamField>
    <ParamField path="mode" type='"final" | "all"' default="final">
      `"all"` includes tool/block replies in addition to final replies.
    </ParamField>
    <ParamField path="provider" type="string">
      Speech provider id. When unset, Natesclaw uses the first configured provider in registry auto-select order. Legacy `provider: "edge"` is rewritten to `"microsoft"` by `natesclaw doctor --fix`.
    </ParamField>
    ...
    <ParamField path="providers.<id>" type="object">
      Provider-owned settings keyed by speech provider id. Legacy direct blocks (`tts.openai`, `.elevenlabs`, `.microsoft`, `.edge`) are rewritten by `natesclaw doctor --fix`; commit only `tts.providers.<id>`.
    </ParamField>
...
and resolved values still fail startup or reject the update.
  ...
  <Accordion title="Local CLI (tts-local-cli)">
    ...
    Command stdout and generated or converted audio are limited to 50 MiB. Diagnostic stderr is limited to 1 MiB. Natesclaw terminates the command and fails synthesis when either limit is exceeded.

  </Accordion>

  <Accordion title="Microsoft (no API key)">
    <ParamField path="enabled" type="boolean" default="true">Allow Microsoft speech usage.</ParamField>
    <ParamField path="speakerVoice" type="string">Microsoft neural voice name (e.g. `en-US-MichelleNeural`). Legacy alias: `voice`. If the default English voice is in effect and reply text is CJK-dominant, Natesclaw auto-switches to `zh-CN-XiaoxiaoNeural`.</ParamField>
    ...
    <ParamField path="edge.*" type="object" deprecated>Legacy alias. Run `natesclaw doctor --fix` to rewrite persisted config to `providers.microsoft`.</ParamField>
  </Accordion>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="responseFormat" type='"mp3" | "opus" | "wav"'>Explicit response format. When omitted, Natesclaw selects Opus for voice-note targets and MP3 otherwise. Use `wav` for compatible local endpoints that do not encode compressed audio.</ParamField>
  ...
  <Accordion title="xAI">
    ...
    <ParamField path="speakerVoiceId" type="string">Default `eve`. With auth, `natesclaw infer tts voices --provider xai` fetches the current built-in catalog; without auth it lists offline fallbacks `ara`, `eve`, `leo`, `rex`, and `sal`. Account custom voice IDs are forwarded even when absent from the built-in list. Legacy alias: `voiceId`.</ParamField>
  ...
  <Accordion title="Xiaomi MiMo">
    ...
    <ParamField path="style" type="string">Optional natural-language style instruction sent as the user message; not spoken. For `mimo-v2.5-tts-voicedesign`, this is the voice-design prompt; Natesclaw supplies a default when omitted.</ParamField>
  </Accordion>
</AccordionGroup>

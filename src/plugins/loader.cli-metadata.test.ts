...
module.exports = {
  ...
  it("skips bundled channel full entries that do not provide a dedicated cli-metadata entry", async () => {
    ...
    process.env.NATESCLAW_BUNDLED_PLUGINS_DIR = bundledRoot;
  ...
  register(api) {
    ...
    expect(
      registry.cliRegistrars[0]?.descriptors[0]?.machineOutput?.({
        argv: ["node", "natesclaw", "bundled-cli-channel", "--machine"],
        stdoutIsTTY: true,
      }),
    ).toBe(true);
  });
...
module.exports = {
  ...
  register(api) {
    ...
    const metadataRegistry = await loadNatesclawPluginCliRegistry({ cache: false, config });
    const fullRegistry = loadNatesclawPlugins({ cache: false, config });
    for (const registry of [metadataRegistry, fullRegistry]) {
      const resolver = registry.cliRegistrars[0]?.descriptors[0]?.machineOutput;
      expect(
        resolver?.({ argv: ["node", "natesclaw", "machine-output-cli"], stdoutIsTTY: false }),
      ).toBe(true);
      expect(
        resolver?.({
          argv: ["node", "natesclaw", "machine-output-cli", "--machine"],
          stdoutIsTTY: true,
        }),
      ).toBe(true);

...
read_when:
  ...
  - Comparing Natesclaw exec approvals with ACPX harness permissions
...
```bash
natesclaw config set tools.exec.mode auto
...
Set ACPX permissions separately from Natesclaw exec approvals:
...
| Use Codex/Natesclaw auto-review before humans  | `tools.exec.mode: "auto"`                                   |
...
Host exec uses the stricter result of Natesclaw config and the host-local approvals file. ACPX harness permissions do not loosen host exec approvals, and host exec approvals do not loosen ACPX harness prompts.

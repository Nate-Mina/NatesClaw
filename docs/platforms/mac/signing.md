...
- Stamps Info.plist with `NatesclawBuildTimestamp` (ISO8601 UTC) and `NatesclawGitCommit` (short hash, `unknown` if unavailable) so the About tab can show build, git, and debug/release channel.
...
The About tab reads `NatesclawBuildTimestamp` and `NatesclawGitCommit` from Info.plist to show version, build date, git commit, and whether the build is DEBUG (via `#if DEBUG`). Re-run the packager after code changes to refresh these values.

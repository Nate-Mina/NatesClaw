...
import { embeddedAgentLog, NATESCLAW_VERSION } from "natesclaw/plugin-sdk/agent-harness-runtime";
...
export class CodexAppServerClient {
  ...
  /** Registers a handler for app-server requests sent back to Natesclaw. */
  ...
  if (request.method === "item/tool/call") {
    return {
      contentItems: [
        {
          type: "inputText",
          text: "Natesclaw did not register a handler for this app-server tool call.",
        },
      ],
      success: false,
    };
...
function timeoutServerRequestResponse(
  ...
  return {
    contentItems: [
      {
        type: "inputText",
        text: `Natesclaw dynamic tool call timed out after ${CODEX_DYNAMIC_TOOL_SERVER_REQUEST_TIMEOUT_MS}ms before sending a response to Codex.`,
      },
    ],
    success: false,
  };
...
class CodexAppServerVersionError extends Error {
  ...
  constructor(detectedVersion: string | undefined) {
    const detected = detectedVersion
      ...
      : "Natesclaw could not determine the running Codex version";
    super(
...
function readCodexVersionFromUserAgent(userAgent: string | undefined): string | undefined {
  ...
  // Natesclaw, Codex Desktop, or an env override, so only the slash-delimited
...
/** Returns true for app-server approval request methods Natesclaw can answer. */

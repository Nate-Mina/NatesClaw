/**
 * Materializes selected Natesclaw skills as a temporary Claude CLI plugin.
 */
...
const NATESCLAW_CLAUDE_PLUGIN_NAME = "natesclaw-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredNatesclawTmpDir(), "natesclaw-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, NATESCLAW_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: NATESCLAW_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped Natesclaw skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(

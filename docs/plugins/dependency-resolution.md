---
summary: "How Natesclaw installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged Natesclaw installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

Natesclaw handles plugin dependencies at install/update time only. Runtime
loading never runs a package manager, repairs a dependency tree, or mutates
the Natesclaw package directory.
...
- SDK/core imports are peer or supplied Natesclaw imports.
- Local development plugins bring their own already-installed dependencies.
- npm and git plugins install into Natesclaw-owned package roots.

Natesclaw owns only the plugin lifecycle:
...
Natesclaw uses stable per-source roots:

- npm packages install into per-plugin projects under
  `~/.natesclaw/npm/projects/<encoded-package>`.
- git packages clone under `~/.natesclaw/git`.
...
```bash
cd ~/.natesclaw/npm/projects/<encoded-package>
npm install --omit=dev --omit=peer --legacy-peer-deps --ignore-scripts --no-audit --no-fund
```

`natesclaw plugins install npm-pack:<path.tgz>` uses the same per-plugin npm
project root for a local npm-pack tarball: Natesclaw reads the tarball's npm
...
`~/.natesclaw/npm/projects/<encoded-package>` can unblock a temporary
...
`node_modules` beside the plugin package. Natesclaw scans the managed project
...
Natesclaw-owned npm plugin packages never ship npm lockfiles. The repository
...
lockfiles according to their own packaging policy; Natesclaw's installer leaves
...
Natesclaw-owned npm plugin packages can also publish with explicit
...
`natesclaw.release.bundleRuntimeDependencies: false`; they still ship a
...
`natesclaw` package also resolves dependencies at install time and does not
bundle its full dependency tree. See
[dependency locking](/gateway/security/dependency-locking).

Plugins that import `natesclaw/plugin-sdk/*` declare `natesclaw` as a peer
dependency. Natesclaw does not let npm install a separate registry copy of the
...
resolution/materialization, and Natesclaw reasserts plugin-local
`node_modules/natesclaw` links for installed packages that declare the host
...
Local plugins are developer-controlled directories. Natesclaw never runs
...
natesclaw plugins install <source>
natesclaw doctor --fix
```

`doctor --fix` cleans legacy Natesclaw-generated dependency state and can
...
Lightweight and core-critical bundled plugins ship as part of Natesclaw. They
...
In source checkouts, Natesclaw treats the repository as a pnpm monorepo.
...
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g natesclaw`        | Built runtime tree inside the package | Natesclaw package and explicit plugin install/update/doctor flows     |
...
| `natesclaw plugins install ...`   | Managed npm project/git/ClawHub root  | The plugin install/update flow                                       |

## Legacy cleanup

Older Natesclaw versions generated bundled-plugin dependency roots at startup
...
`plugin-runtime-deps` targets, `.natesclaw-runtime-deps*` manifests, generated
...
Older npm installs also used a shared `~/.natesclaw/npm/node_modules` root.

...
beforeAll(() => {
  fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-tts-status-"));
});
...
describe("resolveStatusTtsSnapshot", () => {
  it("treats null prefs as empty settings", async () => {
    await withStatusTempHome(async (home) => {
      const prefsPath = path.join(home, ".natesclaw", "settings", "tts.json");
  ...
  it("derives the default prefs path from NATESCLAW_CONFIG_PATH when set", async () => {
    await withStatusTempHome(async (home) => {
      const stateDir = path.join(home, ".natesclaw-dev");

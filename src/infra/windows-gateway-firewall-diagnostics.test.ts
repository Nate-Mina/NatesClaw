...
function ruleRow(params?: {
  ...
  return {
    DisplayName: params?.displayName ?? "Natesclaw Gateway",
...
describe("Windows Gateway firewall diagnostics", () => {
  ...
  it.each([
    [
      ...
      "Natesclaw could not quickly inspect Windows Firewall LAN Gateway policy.",
    ],
    [
      "truncated stdout",
    ...
    [
      "malformed JSON",
      async () => ({ code: 0, stdout: "{" }),
      "Natesclaw could not parse Windows Firewall LAN Gateway policy.",
    ],
    [
      "a runner exception",
      async () => {

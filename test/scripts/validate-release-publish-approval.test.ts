...
function runApprovalScript(
  ...
  return spawnSync(process.execPath, [SCRIPT_PATH], {
    ...
    env: {
      ...
      GITHUB_REPOSITORY: env.GITHUB_REPOSITORY ?? "natesclaw/natesclaw",
...
function writeApproval(overrides: Record<string, unknown> = {}) {
  const tempRoot = tempRoots.make("natesclaw-release-approval-");
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({
      ...
      workflow: "Natesclaw Release Publish",
      parentRunId: "123",
...
function writeClawHubApproval(overrides: Record<string, unknown> = {}) {
  const tempRoot = tempRoots.make("natesclaw-clawhub-bootstrap-approval-");
  const approvalPath = path.join(tempRoot, "approval.json");
  fs.writeFileSync(
    approvalPath,
    `${JSON.stringify({

...
function resolveNatesclawHooks(
...
async function installHookPackageFromDir(
  ...
  const hookManifest = resolveNatesclawHooks(manifest);
  if (!hookManifest.ok) {
    return hookManifest;
  }
  ...
  for (const entry of hookEntries) {
    ...
    if (!runtime.isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `natesclaw.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `natesclaw.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
export async function installHooksFromNpmSpec(
  ...
  return await runtime.installFromValidatedNpmSpecArchive({
    tempDirPrefix: "natesclaw-hook-pack-",
    spec,
    timeoutMs,

...
function createCoverageWebSearchProvider(params: {
  ...
  const readConfiguredCredential = (config?: NatesclawConfig): unknown =>
...
const DEBUG_COVERAGE_BATCHES = process.env.NATESCLAW_DEBUG_RUNTIME_COVERAGE === "1";
...
const PLUGIN_OWNED_NATESCLAW_COVERAGE_EXCLUSIONS = new Set([
...
const previousTrustBundledPluginsDir = process.env.NATESCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR;

process.env.NATESCLAW_BUNDLED_PLUGINS_DIR ??= "extensions";
...
afterAll(() => {
  ...
  if (previousTrustBundledPluginsDir === undefined) {
    ...
    process.env.NATESCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR = previousTrustBundledPluginsDir;
  }
});
...
function addCoveragePluginLoadPath(config: NatesclawConfig, pluginId: string): void {
...
function collectNatesclawCoverageEntries(options: {
  includePluginEntries: boolean;
}): SecretRegistryEntry[] {
  return COVERAGE_REGISTRY_ENTRIES.filter(
    (entry) =>
      entry.configFile === "natesclaw.json" &&
      entry.id.startsWith("plugins.entries.") === options.includePluginEntries &&
      !PLUGIN_OWNED_NATESCLAW_COVERAGE_EXCLUSIONS.has(entry.id),
  );
}

function applyConfigForNatesclawTarget(
...
async function expectNatesclawCoverageBatchResolved(
  label: string,
  ...
  for (const [index, entry] of batch.entries()) {
    const envId = toCoverageEnvRefId("NATESCLAW_SECRET_TARGET", entry.id);
    ...
    applyConfigForNatesclawTarget(config, entry, envId, wildcardToken);
  }
...
const NATESCLAW_CORE_COVERAGE_BATCHES = buildCoverageBatches(
  collectNatesclawCoverageEntries({ includePluginEntries: false }),
);
const NATESCLAW_PLUGIN_COVERAGE_BATCHES = buildCoverageBatches(
  collectNatesclawCoverageEntries({ includePluginEntries: true }),
);
...
describe("secrets runtime target coverage", () => {
  beforeAll(async () => {
    ...
    const googleChatBatch = NATESCLAW_CORE_COVERAGE_BATCHES.find((batch) =>
      batch.some((entry) => entry.id === "channels.googlechat.serviceAccount"),
    );
    if (googleChatBatch) {
      await expectNatesclawCoverageBatchResolved("natesclaw.json core", googleChatBatch);
    }
    const webProviderBatch = NATESCLAW_PLUGIN_COVERAGE_BATCHES.find((batch) =>
    ...
    if (webProviderBatch) {
      ...
      await expectNatesclawCoverageBatchResolved("natesclaw.json plugins", webProviderBatch);
    }
  });

  describe("natesclaw.json core and channel registry targets", () => {
    test.each(NATESCLAW_CORE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectNatesclawCoverageBatchResolved("natesclaw.json core", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });

  describe("natesclaw.json plugin registry targets", () => {
    test.each(NATESCLAW_PLUGIN_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectNatesclawCoverageBatchResolved("natesclaw.json plugins", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });
  ...
  describe("auth-profiles registry targets", () => {
    test.each(AUTH_PROFILE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      ...
      async ({ batch }) => {
        ...
        for (const [index, entry] of batch.entries()) {
          const envId = toCoverageEnvRefId("NATESCLAW_AUTH_SECRET_TARGET", entry.id);

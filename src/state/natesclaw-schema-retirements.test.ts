...
import { NATESCLAW_STATE_SCHEMA_VERSION } from "./natesclaw-state-db-contract.js";
...
it("keeps the schema retirement ledger aligned with canonical database schemas", () => {
  ...
  const databases = {
    state: {
      currentVersion: NATESCLAW_STATE_SCHEMA_VERSION,
      schemaUrl: new URL("./natesclaw-state-schema.sql", import.meta.url),
      database: new DatabaseSync(":memory:"),
    },
    agent: {
      currentVersion: NATESCLAW_AGENT_SCHEMA_VERSION,
      schemaUrl: new URL("./natesclaw-agent-schema.sql", import.meta.url),
      database: new DatabaseSync(":memory:"),
    },
  } satisfies Record<
    DatabaseOwner,

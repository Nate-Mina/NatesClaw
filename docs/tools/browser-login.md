...
When a site requires login, sign in manually in the host browser's `natesclaw`
...
Natesclaw controls a dedicated Chrome profile named `natesclaw` (orange-tinted
UI), separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent uses its isolated `natesclaw` browser.
...
Two ways to access the `natesclaw` profile:
...
natesclaw browser open https://x.com
```

For a non-default profile, put `--browser-profile <name>` before the
subcommand (default is `natesclaw`):

```bash
natesclaw browser --browser-profile <name> open https://x.com
```
...
```bash
natesclaw browser --browser-profile natesclaw open https://x.com
```

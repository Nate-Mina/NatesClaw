...
struct SettingsRootView: View {
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .natesclawSelectSettingsTab)) { note in
        ...
        .onReceive(NotificationCenter.default.publisher(for: .natesclawConfigDidChange)) { _ in
    ...
    private var nixManagedBanner: some View {
        ...
        let configPath = self.snapshotPaths.configPath ?? NatesclawPaths.configURL.path
        let stateDir = self.snapshotPaths.stateDir ?? NatesclawPaths.stateDirURL.path
    ...
    private func scheduleInferenceRefresh(clearPrevious: Bool, resetSystemAgent: Bool = false) {
        if resetSystemAgent {
            // Natesclaw sessions are gateway-owned. Re-key the cached detail so a route
            // change cannot send old conversation state to a new endpoint.
            self.systemAgentChatIdentity = UUID()
        }
        if clearPrevious {
            // Preserve an active or pending Natesclaw request while config truth is revalidated.
...
struct SettingsTabGroup: Identifiable {
    ...
    static func defaultGroups(showDebug: Bool, showSystemAgent: Bool) -> [SettingsTabGroup] {
        ...
        var groups = [
            ...
            SettingsTabGroup(title: "Natesclaw", tabs: [.about]),
        ]
...
extension Notification.Name {
    static let natesclawSelectSettingsTab = Notification.Name("natesclawSelectSettingsTab")
}

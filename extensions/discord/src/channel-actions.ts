...
import { createUnionActionGate } from "natesclaw/plugin-sdk/channel-actions";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageActionName,
  ChannelMessageToolDiscovery,
} from "natesclaw/plugin-sdk/channel-contract";
import type { DiscordActionConfig, NatesclawConfig } from "natesclaw/plugin-sdk/config-contracts";
...
function listDiscoverableDiscordAccounts(cfg: NatesclawConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: NatesclawConfig) {

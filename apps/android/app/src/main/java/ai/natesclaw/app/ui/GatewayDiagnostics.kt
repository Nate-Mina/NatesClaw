...
import ai.natesclaw.app.gatewayConnectionStatusForDisplay
...
internal fun NatesclawAndroidVersionLabel(): String {
...
internal fun gatewayNodeApprovalCommand(
  ...
  when (state) {
    ...
    -> normalizeGatewayApprovalRequestId(requestId)?.let { "natesclaw nodes approve $it" } ?: "natesclaw nodes status"
    GatewayNodeApprovalState.Unapproved -> "natesclaw nodes status"
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return nativeString(
    "Help diagnose this Natesclaw Android gateway connection failure.\n\n" +
      "Please:\n" +
      ...
      "- tell me whether `natesclaw devices list` should show a pending pairing request\n" +
      "- if more signal is needed, ask for `natesclaw qr --json`, `natesclaw devices list`, and `natesclaw nodes status`\n" +
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Natesclaw gateway diagnostics", report))

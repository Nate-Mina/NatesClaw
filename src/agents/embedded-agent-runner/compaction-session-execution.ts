...
export async function executePreparedCompactionSession(runtime: PreparedCompactionRuntime) {
  ...
  try {
    ...
    try {
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop Natesclaw
      // compaction overrides applied in createPreparedEmbeddedAgentSettingsManager — same
      // rehydration also restores Natesclaw runtime's auto-compaction (natesclaw#75799), so re-apply
      ...
      // exact Natesclaw-managed registrations so custom tools survive startup.

...
import { withFileLock, type FileLockOptions } from "natesclaw/plugin-sdk/file-lock";
...
async function collectSessionSurfaces(params: MigrationEnvironment): Promise<SessionSurface[]> {
  ...
  const { resolveStorePath } = await import("natesclaw/plugin-sdk/session-store-runtime");
...
async function recordSessionOwner(
  ...
  const { patchSessionEntry } = await import("natesclaw/plugin-sdk/session-store-runtime");

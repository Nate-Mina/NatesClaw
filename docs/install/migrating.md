...
read_when:
  - You are moving Natesclaw to a new laptop or server
  - You are coming from another agent system and want to keep state
  - You are upgrading an in-place plugin
title: "Migration guide"
---

Natesclaw supports three migration paths: importing from another agent system, moving an existing install to a new machine, and upgrading a plugin in place.

## Import from another agent system

Bundled migration providers bring instructions, MCP servers, skills, model config, and (opt-in) API keys into Natesclaw. Plans are previewed before any change and secrets are redacted in reports. Standalone `natesclaw migrate` is backed by a verified backup; fresh onboarding imports instead stage and verify local artifacts before publishing them with configuration committed before any irreversible external activation.
...
The CLI entry point is [`natesclaw migrate`](/cli/migrate). Onboarding can also offer migration when it detects a known source (`natesclaw onboard --flow import`).

## Move Natesclaw to a new machine

Copy the **state directory** (`~/.natesclaw/` by default) and your **workspace** to preserve:

- **Config** — `natesclaw.json` and all gateway settings.
...
<Tip>
Run `natesclaw status` on the old machine to confirm your state directory path. Custom profiles use `~/.natesclaw-<profile>/` or a path set via `NATESCLAW_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    cd ~
    tar -czf natesclaw-state.tgz .natesclaw
    ```

    If you use multiple profiles (for example `~/.natesclaw-work`), archive each separately.

  </Step>

  <Step title="Install Natesclaw on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine. It is fine if onboarding creates a fresh `~/.natesclaw/` — you overwrite it next.
  </Step>
...
```bash
awk -F= '/^(TELEGRAM_BOT_TOKEN|DISCORD_BOT_TOKEN)=/ { print $1 "=present" }' ~/.natesclaw/.env
```

`natesclaw doctor` also warns when an enabled default Telegram or Discord account has no configured token and the matching env variable is unavailable to the doctor process.

### Common pitfalls

<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `NATESCLAW_STATE_DIR` and the new one does not, channels will appear logged out and sessions will be empty. Launch the gateway with the **same** profile or state-dir you migrated, then rerun `natesclaw doctor`.
  </Accordion>

  <Accordion title="Copying only natesclaw.json">
...
- [ ] `natesclaw status` shows the gateway running.
...
- [`natesclaw migrate`](/cli/migrate): CLI reference for cross-system imports.
- [Install overview](/install): all installation methods.
- [Doctor](/gateway/doctor): post-migration health check.
- [Uninstall](/install/uninstall): removing Natesclaw cleanly.

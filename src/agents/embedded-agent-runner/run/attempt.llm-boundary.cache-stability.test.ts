...
describe("append-only late media (issue #99495)", () => {
  it("keeps every sent fingerprint stable and appends one late-media turn", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-99495-boundary-"));
    const target = {
      agentId: "main",
      cwd: dir,
    ...
    try {
      ...
      expect(latePersisted?.["__natesclaw"]?.lateMedia).toBe(true);
...
function runtimeCarrier(content: string, timestamp: number): AgentMsg {
  return {
    role: "custom",
    customType: NATESCLAW_RUNTIME_CONTEXT_CUSTOM_TYPE,
    content,
    display: false,
    details: { source: "natesclaw-runtime-context", runtimeContextCarrier: true },
    timestamp,
...
function isCarrier(message: unknown): boolean {
  return Boolean(
    ...
    (message as { customType?: unknown }).customType === NATESCLAW_RUNTIME_CONTEXT_CUSTOM_TYPE,
  );
}

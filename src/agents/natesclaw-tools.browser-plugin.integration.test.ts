// Verifies Natesclaw plugin tools are resolved with browser/runtime context.
...
const TEST_AGENT_DIR = path.join(os.tmpdir(), "natesclaw-plugin-tool-auth-test");
...
function firstResolvePluginToolsParams(): Record<string, unknown> {
  // Captures the plugin runtime contract passed from Natesclaw tool resolution.
...
describe("createNatesclawTools browser plugin integration", () => {
  afterEach(() => {
  ...
  it("keeps the browser tool returned by plugin resolution", () => {
    ...
    const tools = resolveNatesclawPluginToolsForOptions({
      options: { config },
  ...
  it("forwards lifecycle-prepared plugin facts to plugin resolution", () => {
    hoisted.resolvePluginTools.mockReturnValue([]);
    const config = { plugins: { enabled: true } } as NatesclawConfig;
  ...
  it("keeps provider availability and credential resolution aligned for env-only auth", async () => {
    const envName = "NATESCLAW_PLUGIN_TOOL_AUTH_TEST_KEY";
  ...
  it("preserves ungated plugin resolution when no authoritative auth store is supplied", () => {
    hoisted.resolvePluginTools.mockReturnValue([]);
    const config = { plugins: { allow: ["browser"] } } as NatesclawConfig;
  ...
  it("does not pass a stale active snapshot as plugin runtime config for a resolved run config", () => {
    ...
    let capturedRuntimeConfig: NatesclawConfig | undefined;
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      capturedRuntimeConfig = (params as { context?: { runtimeConfig?: NatesclawConfig } }).context
        ?.runtimeConfig;
      return [];
    });
  ...
  it("does not let a source-less pinned config snapshot override explicit plugin tool config", () => {
    ...
    let capturedRuntimeConfig: NatesclawConfig | undefined;
    let getRuntimeConfig: (() => NatesclawConfig | undefined) | undefined;
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      const context = (
        params as {
          context?: {
  ...
  it("exposes a live runtime config getter to plugin tool factories", () => {
    ...
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      getRuntimeConfig = (
        params as { context?: { getRuntimeConfig?: () => NatesclawConfig | undefined } }
...
function requirePluginTool(name: string, overrides?: Parameters<typeof createNatesclawTools>[0]) {
  hoisted.resolvePluginTools.mockReturnValue([
    {
      name,
...
describe("createNatesclawTools Gateway caller identity", () => {
  afterEach(() => {

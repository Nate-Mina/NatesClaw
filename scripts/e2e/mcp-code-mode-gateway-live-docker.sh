...
IMAGE_NAME="$(docker_e2e_resolve_image "natesclaw-mcp-code-mode-gateway-live-e2e" NATESCLAW_IMAGE)"
PORT="$(docker_e2e_read_tcp_port_env NATESCLAW_MCP_CODE_MODE_LIVE_GATEWAY_PORT 18789)"
...
CONTAINER_NAME="natesclaw-mcp-code-mode-live-e2e-$$"
PROFILE_FILE="${NATESCLAW_MCP_CODE_MODE_LIVE_PROFILE_FILE:-${NATESCLAW_TESTBOX_PROFILE_FILE:-$HOME/.natesclaw-testbox-live.profile}}"

CLIENT_LOG="$(mktemp -t natesclaw-mcp-code-mode-live-log.XXXXXX)"
...
NATESCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 mcp-code-mode-gateway-live empty)"

# The profile is only a credential source. Keep this lane's Natesclaw runtime
# isolated from host/testbox mode flags that can change packaged behavior.
unset NATESCLAW_TESTBOX
...
docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e "NATESCLAW_DOCKER_OPENAI_BASE_URL=${OPENAI_BASE_URL:-https://api.openai.com/v1}" \
  ...
  -e "NATESCLAW_ALLOW_INSECURE_PRIVATE_WS=1" \
  -e "NATESCLAW_MCP_CODE_MODE_MODEL=${NATESCLAW_MCP_CODE_MODE_LIVE_MODEL:-natesclaw/main}" \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
  bash -lc "set -euo pipefail
    ...
    gateway_pid=\"\$(natesclaw_e2e_start_gateway \"\$entry\" $PORT /tmp/mcp-code-mode-live-gateway.log)\"
    natesclaw_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/mcp-code-mode-live-gateway.log 480 $PORT

---
summary: "Export redacted trajectory bundles for debugging an Natesclaw agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an Natesclaw session
...
Trajectory capture is Natesclaw's per-session flight recorder. It records a
...
Natesclaw writes the bundle under the workspace:

```text
.natesclaw/trajectory-exports/natesclaw-trajectory-<session>-<timestamp>/
```
...
The name resolves inside `.natesclaw/trajectory-exports/`. Absolute paths and
`~` paths are rejected.
...
bundle; do not use allow-all. In group chats, Natesclaw sends the approval
...
`.natesclaw/trajectory-exports`), `--store <path>` (session store override),
...
Trajectory capture is on by default for Natesclaw agent runs.
...
| `metadata.json`       | Natesclaw version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
```bash
export NATESCLAW_TRAJECTORY=0
```

This disables runtime trajectory capture before starting Natesclaw.
...
Natesclaw flushes runtime trajectory rows during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`NATESCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting Natesclaw:

```bash
export NATESCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when Natesclaw logs an `natesclaw-trajectory-flush` timeout and
...
`NATESCLAW_AGENT_CLEANUP_TIMEOUT_MS`.

## Privacy and limits

Trajectory bundles are for support and debugging, not public posting. Natesclaw
...
- confirm Natesclaw was started without `NATESCLAW_TRAJECTORY=0`
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.natesclaw/trajectory-exports/`

...
function writeConfig(home: string, channels: Record<string, unknown>): void {
  ...
  fs.writeFileSync(path.join(configDir, "natesclaw.json"), JSON.stringify({ channels }));
}
...
function runStatusAssert(
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-status-assertions-"));
  try {
...
describe("npm onboard channel agent assertions", () => {
  it("rejects loose mock OpenAI port args", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-onboard-assertions-"));
  ...
  it("configures and validates the canonical main agent's mock model", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-onboard-mock-agent-"));
  ...
  it("validates OpenAI env refs from the SQLite auth profile store", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-onboard-assertions-"));
    const agentDir = path.join(tempDir, ".natesclaw", "agents", "main", "agent");
      ...
      writeAuthProfileStoreSqlite(agentDir, {
        ...
        profiles: {
          "openai:api-key": {
            type: "api_key",
            provider: "openai",
            key: "sk-natesclaw-npm-onboard-e2e",
          },
        },
      });
  ...
  it("validates channel tokens in their canonical config fields", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-channel-assertions-"));
    try {
      writeConfig(tempDir, {
  ...
  it("rejects oversized plain status output before parsing it", () => {
    const result = runStatusAssert(
      ...
      { configuredChannels: ["telegram"] },
      `# Natesclaw status\n${"x".repeat(128)}`,
      { NATESCLAW_NPM_ONBOARD_STATUS_TEXT_MAX_BYTES: "64" },
    );
  ...
  it("rejects oversized channels status JSON before parsing it", () => {
    const result = runStatusAssert(
      ...
      "# Channels\ntelegram ok configured",
      { NATESCLAW_NPM_ONBOARD_JSON_ARTIFACT_MAX_BYTES: "64" },
    );

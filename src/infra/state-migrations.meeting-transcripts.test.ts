...
describe("meeting transcript Doctor migration", () => {
  it("is doctor-only", async () => {
    const stateDir = tempDirs.make("natesclaw-meeting-transcripts-doctor-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const stateDir = tempDirs.make("natesclaw-meeting-transcripts-doctor-");
      const externalRoot = tempDirs.make("natesclaw-meeting-transcripts-external-");
  ...
  it("imports, verifies, receipts, archives, and reopens SQLite-only", async () => {
    ...
    const database = openNatesclawStateDatabase({ env: databaseEnv(stateDir) }).db;
    expect(
      database
        .prepare(
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const stateDir = tempDirs.make("natesclaw-meeting-transcripts-doctor-");
      const externalDir = tempDirs.make("natesclaw-meeting-transcripts-external-");
  ...
  it("restores idempotently when a canonical exporter recreated the destination", async () => {
    ...
    const archivedStateDir = tempDirs.make("natesclaw-meeting-transcripts-archive-");

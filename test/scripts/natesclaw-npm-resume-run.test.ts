...
import {
  resolveNatesclawNpmResumeRun,
  runNatesclawNpmResumeGh,
  validateNatesclawNpmResumeRun,
} from "../../scripts/natesclaw-npm-resume-run.mts";
import type { NatesclawNpmResumeValidationInput } from "../../scripts/natesclaw-npm-resume-run.mts";
...
): NatesclawNpmResumeValidationInput {
  return {
    ...
    run: {
      ...
      head_sha: SHA,
      html_url: URL,
      path: `.github/workflows/natesclaw-npm-release.yml@refs/tags/${BRANCH}`,
      workflow_id: 101,
    },
    tag: {
...
describe("natesclaw npm resume run identity", () => {
  it("bounds each GitHub lookup", () => {
    const execFileSyncImpl = vi.fn(() => "result");

    expect(
      runNatesclawNpmResumeGh(["api", "repos/natesclaw/natesclaw/actions/runs/456"], {
        execFileSyncImpl,
      }),
    ).toBe("result");
    expect(execFileSyncImpl).toHaveBeenCalledWith(
      "gh",
      ["api", "repos/natesclaw/natesclaw/actions/runs/456"],
      {
        encoding: "utf8",
        killSignal: "SIGKILL",
  ...
  it("accepts a successful run bound to a signed main-reachable tooling tag", () => {
    expect(validateNatesclawNpmResumeRun(fixture())).toEqual({
  ...
  it.each([
    ...
    expect(() => validateNatesclawNpmResumeRun(fixture(overrides))).toThrow(message);
  });

  it("loads the exact run, workflow, signed tag, ancestry, and approval job", () => {
    const responses = new Map<string, unknown>([
      [`api repos/natesclaw/natesclaw/actions/runs/456 --method GET`, fixture().run],
      [
        `api repos/natesclaw/natesclaw/actions/workflows/natesclaw-npm-release.yml --method GET`,
        { id: 101 },
      ],
      [`api repos/natesclaw/natesclaw/git/ref/tags/${BRANCH} --method GET`, fixture().tagRef],
      [`api repos/natesclaw/natesclaw/git/tags/${TAG_OBJECT_SHA} --method GET`, fixture().tag],
      [`api repos/natesclaw/natesclaw/compare/${SHA}...main --method GET`, { status: "identical" }],
      [`run view 456 --repo natesclaw/natesclaw --json jobs --jq .jobs`, fixture().jobs],
    ]);
    ...
    expect(
      resolveNatesclawNpmResumeRun({ repo: "natesclaw/natesclaw", runGh, runId: "456" }),

...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "natesclaw", "terminal"] },
    { argv: ["node", "natesclaw", "chat"] },
    { argv: ["node", "natesclaw", "hooks", "relay", "--relay-id", "relay-1"] },
    { argv: ["node", "natesclaw", "gateway"] },
    { argv: ["node", "natesclaw", "gateway", "--port", "14720", "--bind", "loopback"] },
    { argv: ["node", "natesclaw", "gateway", "run", "--port=14720", "--bind", "loopback"] },
    {
      argv: ["node", "natesclaw", "--profile", "server", "gateway", "run", "--allow-unconfigured"],
    },
  ...
  it.each([
    ...
    { argv: ["node", "natesclaw", "gateway", "status"] },
    { argv: ["node", "natesclaw", "gateway", "call", "health"] },
  ...
  it("keeps native hook relay respawn behavior unchanged on Windows", () => {
    expect(
      shouldSkipRespawnForArgv(
        ["node", "natesclaw", "hooks", "relay", "--relay-id", "relay-1"],
        "win32",
      ),
    ).toBe(false);
  });
...
describe("shouldSkipStartupEnvironmentRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "natesclaw", "hooks", "relay", "--relay-id", "relay-1"] },
    { argv: ["node", "natesclaw", "gateway"] },
    { argv: ["node", "natesclaw", "gateway", "run", "--port=14720"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "natesclaw.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("natesclaw.internal");

...
/**
 * Catalog #18 — `natesclaw sessions --json` reports `agentRuntime.id: "natesclaw"` for
 * ACP sessions because the old metadata resolver only consulted agent-config
 * policies (env / agent / defaults / implicit fallback to "natesclaw"). The session
 * key clearly carries the ACP runtime indicator (the `:acp:` segment), but
 * `sessions.ts:294` used to ignore it.
 *
 * Empirical observation from a deployed natesclaw container against a copilot
 ...
 *
 *   - RED: ACP-keyed session resolves to `id: "natesclaw"`, `source: "implicit"`.
...
/**
 * Build a minimal `NatesclawConfig` that mirrors the deployed scenario:
 ...
 *
 * Result: the old metadata resolver fell through to the implicit "natesclaw"
 * branch — which is the bug under test.
 */
function buildConfigWithoutAgentRuntimePolicy(): NatesclawConfig {
  return {
    agents: {
      list: [
        {
...
describe("sessions --json agentRuntime classifier (catalog #18)", () => {
  it("RED→GREEN: ACP session key is no longer misclassified (overlay applies)", () => {
    ...
    // resolved metadata said id="natesclaw", source="implicit".
    ...
    expect(
      agentRuntime.id,
      `ACP session ${ACP_SESSION_KEY} should no longer be misclassified as "auto" or "natesclaw". ` +
  ...
  it("FIX-SHAPE expectation: ACP session should resolve to 'acpx'", () => {
    ...
    // from "natesclaw" and reflects the actual runtime driving the session.
  ...
  it("preserves locked Codex ownership ahead of stale Natesclaw session metadata", () => {

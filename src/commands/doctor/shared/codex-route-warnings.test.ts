...
const asLegacyConfig = (cfg: unknown): NatesclawConfig => cfg as NatesclawConfig;
...
describe("collectCodexRouteWarnings", () => {
  ...
  it("ignores NATESCLAW_AGENT_RUNTIME when reporting legacy model refs", () => {
    const warnings = collectCodexRouteWarnings(
      {
        agents: { defaults: { model: "openai-codex/gpt-5.5" } },
      },
  ...
  it("warns without executing a custom Codex app-server command", () => {
    ...
    expect(warnings).toStrictEqual([
      [
        ...
        '- plugins.entries.codex.config.appServer.command: "node C:\\Users\\me\\.natesclaw\\npm\\node_modules\\@openai\\codex\\bin\\codex.js" starts with "node" and embeds "C:\\Users\\me\\.natesclaw\\npm\\node_modules\\@openai\\codex\\bin\\codex.js". The command field must be only the executable path.',
  ...
  it("preserves and reports authored params across effective Codex sources", () => {
    ...
    expect(result.warnings.join("\n")).not.toContain("gpt-5.6-natesclaw.params.serviceTier");
  });
  ...
  it("warns when Codex runtime has Natesclaw compaction summarizer overrides", () => {
    const warnings = collectCodexRouteWarnings({
      agents: {
        defaults: {
          model: "openai/gpt-5.5",
  ...
  it("preserves Codex runtime policy for each migrated per-agent Lossless model", () => {
    const result = maybeRepairCodexRoutes({
      models: {
        providers: {
          openai: { baseUrl: "https://proxy.example.test/v1", agentRuntime: { id: "natesclaw" } },
        },
      },
      agents: {
        defaults: {
  ...
  it("does not broaden runtime policy from kept compaction-only refs", () => {
    ...
    expectAgentRuntime(result.cfg, "natesclaw");
  });
  ...
  it("keeps the Codex plugin disabled when OpenAI routes explicitly use the Natesclaw runtime", () => {
    const result = maybeRepairCodexRoutes({
      plugins: DISABLED_CODEX_PLUGIN_CONFIG,
      models: {
        providers: {
          openai: { agentRuntime: { id: "natesclaw" }, models: [] },
        },
      },
      agents: { defaults: { model: "gpt-5.5" } },
    });
    ...
    expectCodexPluginDisabled(result);
    expectAgentRuntime(result.cfg, "natesclaw", { modelId: "gpt-5.5" });
  });

  itKeepsCodexPluginDisabled(
    "keeps the Codex plugin disabled when an auth-profiled OpenAI route explicitly uses the Natesclaw runtime",
    {
      agents: {
        defaults: {
  ...
  it("keeps repaired OpenAI refs on Codex runtime even when the OpenAI provider is otherwise Natesclaw/API-key routed", () => {
    const result = maybeRepairCodexRoutes({
      models: {
        providers: {
          openai: {
  ...
  it("preserves explicit listed-agent canonical refs when default legacy model repair adds Codex policy", () => {
    ...
    expectAgentRuntime(result.cfg, "codex", { modelId: "gpt-5.5" });
    expectAgentRuntime(result.cfg, "natesclaw", { modelId: "gpt-5.5", agentId: "worker" });
  });
  ...
  it("preserves inherited default legacy runtime pins for listed-agent legacy refs", () => {
    const cfg = {
      agents: {
        defaults: {
          models: {
            "openai-codex/gpt-5.4": { agentRuntime: { id: "natesclaw" } },
          },
        },
    ...
    expectAgentRuntime(cfg, "natesclaw", { provider: "openai-codex", agentId: "worker" });
    ...
    expectAgentRuntime(result.cfg, "natesclaw", { agentId: "worker" });
  });
  ...
  it("preserves inherited default wildcard runtime pins for listed-agent legacy refs", () => {
    const cfg = {
      agents: {
        defaults: {
          models: {
            "openai-codex/*": { agentRuntime: { id: "natesclaw" } },
          },
        },
        list: [{ id: "worker", model: "openai-codex/gpt-5.4" }],
      },
  ...
  it("preserves provider-level legacy runtime pins while repairing default legacy refs", () => {
    ...
    expectAgentRuntime(cfg, "natesclaw", { provider: "openai-codex" });
    ...
    expect(result.changes.join("\n")).toContain(
      'Set agents.defaults.models.openai/gpt-5.4.agentRuntime.id to "natesclaw"',
    );
    expectAgentRuntime(result.cfg, "natesclaw");
  });
  ...
  it("preserves legacy provider catalog runtime pins while repairing default legacy refs", () => {
    const cfg = {
      models: {
        providers: {
          "openai-codex": {
            models: [{ id: "gpt-5.4", agentRuntime: { id: "natesclaw" } }],
          },
        },
      },
      agents: { defaults: { model: "openai-codex/gpt-5.4" } },
  ...
  it("preserves legacy provider catalog runtime pins while repairing listed-agent legacy refs", () => {
    ...
    expect(result.changes.join("\n")).toContain(
      'Set agents.list.worker.models.openai/gpt-5.4.agentRuntime.id to "natesclaw"',
    );
    expectAgentRuntime(result.cfg, "natesclaw", { agentId: "worker" });
  });
  ...
  it("shields listed canonical refs when provider-level legacy default pins migrate", () => {
    ...
    expectAgentRuntime(cfg, "natesclaw", { provider: "openai-codex", agentId: "main" });
    ...
    expectAgentRuntime(result.cfg, "natesclaw", { agentId: "main" });
  ...
  it("does not apply pre-existing canonical default runtime pins to listed-agent legacy refs", () => {
    const cfg = {
      agents: {
        defaults: {
          models: {
            "openai/*": { agentRuntime: { id: "natesclaw" } },
          },
        },
        list: [{ id: "worker", model: "openai-codex/gpt-5.4" }],
      },
  ...
  it("preserves explicit model-scoped runtime pins when repairing legacy model map keys", () => {
    const result = maybeRepairCodexRoutes({
      agents: {
        defaults: {
          models: {
            "openai-codex/gpt-5.5": { alias: "legacy-codex", agentRuntime: { id: "natesclaw" } },
          },
        },
      },
    });
    ...
    expect(result.cfg.agents?.defaults?.models).toEqual({
      "openai/gpt-5.5": { alias: "legacy-codex", agentRuntime: { id: "natesclaw" } },
    });
    expect(result.changes.join("\n")).not.toContain(
  ...
  it("preserves explicit Natesclaw runtime pins while repairing legacy session routes", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
  ...
  it("preserves Codex runtime intent alongside explicit Natesclaw harness pins", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
  ...
  it("preserves canonical OpenAI sessions that are explicitly pinned to Natesclaw", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
    ...
    expect(result).toEqual({ changed: false, sessionKeys: [] });
    expect(getSession(store, "main").updatedAt).toBe(1);
    expect(getSession(store, "main").agentHarnessId).toBe("natesclaw");
    expect(getSession(store, "main").agentRuntimeOverride).toBe("natesclaw");
  ...
  it("preserves an explicit non-default agentRuntime pin on the legacy model entry during migration (#84038)", () => {
    ...
    expect(migrated?.agentRuntime).toEqual({ id: "natesclaw" });
    expect(result.cfg.agents?.defaults?.models?.["openai-codex/gpt-5.4"]).toBeUndefined();
  });

...
const DEFAULT_TRIGGERS = ["natesclaw", "claude", "computer"];
const VOICEWAKE_CONFIG_KEY = "default";

type VoiceWakeDatabase = Pick<NatesclawStateKyselyDatabase, "voicewake_triggers">;
...
export async function setVoiceWakeTriggers(
  ...
  runNatesclawStateWriteTransaction(
    ...
    baseDir ? { env: { ...process.env, NATESCLAW_STATE_DIR: baseDir } } : {},
  );
  return {
    triggers: sanitized,
    updatedAtMs,

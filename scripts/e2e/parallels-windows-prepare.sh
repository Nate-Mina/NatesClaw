...
VM_NAME="Windows 11"
NATESCLAW_PARALLELS_WINDOWS_API=1
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TODAY="$(date +%F)"
CLEAN_SNAPSHOT="windows-11-clean-os-${TODAY}"
BASELINE_SNAPSHOT="pre-natesclaw-native-e2e-${TODAY}"
GUEST_PROFILE=""
GUEST_PROFILE_PS=""
GUEST_ARCH=""
...
SNAPSHOT=""
SECURE_STAGE_DIR="C:/ProgramData/NatesclawPrerequisiteInstallers"
COMMAND="help"
if [[ "${NATESCLAW_PARALLELS_WINDOWS_LIBRARY_ONLY:-0}" != "1" ]]; then
  COMMAND="${1:-help}"
  if [[ $# -gt 0 ]]; then
    shift
  fi
...
Commands:
  ...
  prepare     Create a clean snapshot, provision the reusable Natesclaw baseline, and snapshot it.
  verify      Verify base prerequisites and prove the guest contains no Natesclaw product state.
...
It installs only reusable Natesclaw prerequisites: WSL platform/package, Git, and Node/npm.
It refuses to create the baseline when the guest contains an Natesclaw CLI, app package, process,
...
create_clean_snapshot_if_raw() {
  ...
  create_snapshot "$CLEAN_SNAPSHOT" "Clean Windows baseline before Natesclaw development prerequisites."
}
...
if (Get-Command natesclaw.cmd -ErrorAction SilentlyContinue) { $dirty.Add('natesclaw.cmd on PATH') }
if (Test-Path (Join-Path $env:USERPROFILE '.natesclaw')) { $dirty.Add('Natesclaw CLI state directory exists') }
if (Test-Path (Join-Path $env:APPDATA 'NatesclawTray')) { $dirty.Add('NatesclawTray AppData exists') }
if (Test-Path (Join-Path $env:APPDATA 'NatesclawTray-Dev')) { $dirty.Add('NatesclawTray-Dev AppData exists') }
if (Test-Path (Join-Path $env:LOCALAPPDATA 'NatesclawTray')) { $dirty.Add('Natesclaw Companion install/state directory exists') }
if (Test-Path (Join-Path $env:LOCALAPPDATA 'NatesclawTray-Dev')) { $dirty.Add('Natesclaw Companion dev install/state directory exists') }
if (Get-AppxPackage -Name '*Natesclaw*' -ErrorAction SilentlyContinue) { $dirty.Add('Natesclaw app package installed') }
if (Get-Process -Name '*Natesclaw*' -ErrorAction SilentlyContinue) { $dirty.Add('Natesclaw process running') }
$uninstallRoots = @(
  'HKCU:/Software/Microsoft/Windows/CurrentVersion/Uninstall/*',
  'HKLM:/Software/Microsoft/Windows/CurrentVersion/Uninstall/*',
  'HKLM:/Software/WOW6432Node/Microsoft/Windows/CurrentVersion/Uninstall/*'
)
if (Get-ItemProperty $uninstallRoots -ErrorAction SilentlyContinue | Where-Object { $_.DisplayName -like 'Natesclaw Companion*' }) {
  $dirty.Add('Natesclaw Companion uninstall registration exists')
}
...
ensure_wsl_package() {
  ...
  run_windows_installer msiexec.exe /i 'C:\ProgramData\NatesclawPrerequisiteInstallers\WSL.msi' /qn /norestart '/L*v' 'C:\Windows\Temp\natesclaw-wsl-install.log'
  finish_installer_reboot
  ...
  ensure_wsl_default_version
  guest_system_ps "Remove-Item -LiteralPath '${wsl_msi}','C:/Windows/Temp/natesclaw-wsl-install.log' -Force -ErrorAction SilentlyContinue"
}
...
winget_download() {
  ...
  local download_dir="${GUEST_PROFILE//\//\\}\\Downloads\\NatesclawPrereqs"
  guest_user_ps "if (Test-Path -LiteralPath '${GUEST_PROFILE_PS}/Downloads/NatesclawPrereqs') { Remove-Item -LiteralPath '${GUEST_PROFILE_PS}/Downloads/NatesclawPrereqs' -Recurse -Force }"
...
downloaded_installer() {
  local pattern="$1"
  guest_user_ps "Get-ChildItem -LiteralPath '${GUEST_PROFILE_PS}/Downloads/NatesclawPrereqs' -File | Where-Object { \$_.Name -like '${pattern}' } | Sort-Object LastWriteTime -Descending | Select-Object -First 1 -ExpandProperty FullName" | tr -d '\r' | tail -n 1
}
...
prepare() {
  ...
  ensure_wsl_default_version
  verify_baseline
  create_snapshot "$BASELINE_SNAPSHOT" "E2E-ready Natesclaw Windows baseline with WSL 2, Git, Node/npm, and no Natesclaw product state."

...
function mergePackageChannelMetaIntoChannelConfigs(params: {
  channelConfigs?: Record<string, PluginManifestChannelConfig>;
  packageChannel?: NatesclawPackageManifest["channel"];
}): Record<string, PluginManifestChannelConfig> | undefined {
...
function buildRecord(params: {
  ...
  return {
    ...
    format: params.candidate.format ?? "natesclaw",
...
function pushNonBundledChannelConfigDescriptorDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message: `channel plugin manifest declares ${safeMissingChannels.join(", ")} without channelConfigs metadata; add natesclaw.plugin.json#channelConfigs so config schema and setup surfaces work before runtime loads. Channels without channelConfigs still appear in channel listings, but setup UI may be limited.`,
  });
}
...
export function loadPluginManifestRegistryCore(
  ...
  for (const candidate of candidates) {
    ...
    const isBundleRecord = (candidate.format ?? "natesclaw") === "bundle";
    ...
    if (isManifestlessConfiguredFile && isCoreReservedPluginId(candidate.idHint)) {
      diagnostics.push({
        ...
        message: `plugin manifest id "${candidate.idHint}" is reserved by Natesclaw core`,
      });
      continue;
    }
    const manifestRes:
    ...
    if (candidate.origin !== "bundled") {
          ...
          message:
            minHostVersionCheck.kind === "invalid"
              ...
              : minHostVersionCheck.kind === "unknown_host_version"
                ? `plugin requires Natesclaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined; skipping load`
                : `plugin requires Natesclaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}; skipping load`,
        });
        continue;
      }
      ...
      if (
        ...
        diagnostics.push({
          ...
          pluginId: effectivePluginId,
          source: packageManifestSource,
          message: `plugin requires plugin API ${packagePluginApiRange}, but this host is ${currentHostVersion}; skipping load (check "natesclaw --version", NATESCLAW_COMPATIBILITY_HOST_VERSION, or run "natesclaw doctor")`,
        });
        continue;
      }
    }
...
export function loadBundledPluginManifestRegistry(
  ...
  return loadPluginManifestRegistryCore({
    env,
    installRecords,
    discovery: discoverNatesclawPlugins({
      env,
      installRecords,
      rootScope: "bundled",
    }),

...
const emptyBundledPluginsDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-empty-plugins-"));
const externalPluginRoot = fs.mkdtempSync(
  path.join(os.tmpdir(), "natesclaw-provider-policy-external-"),
);
fs.writeFileSync(
...
process.env.NATESCLAW_BUNDLED_PLUGINS_DIR = emptyBundledPluginsDir;

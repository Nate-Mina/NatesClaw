---
summary: "Fix Chrome/Brave/Edge/Chromium CDP startup issues for Natesclaw browser control on Linux"
...
```json
{ "error": "Error: Failed to start Chrome CDP on port 18800 for profile \"natesclaw\"." }
```
...
Snap's AppArmor confinement interferes with how Natesclaw spawns and monitors
the browser process.

Other common Linux launch failures:

- `The profile appears to be in use by another Chromium process`: stale
  `Singleton*` lock files in the managed profile directory. Natesclaw removes
...
- `Missing X server or $DISPLAY`: a visible browser was explicitly requested
  ...
  If you set `NATESCLAW_BROWSER_HEADLESS=0`, `browser.headless: false`, or
  `browser.profiles.<name>.headless: false`, remove that headed override, set
  `NATESCLAW_BROWSER_HEADLESS=1`, start `Xvfb`, run
  `natesclaw browser start --headless` for a one-shot managed launch, or run
  Natesclaw in a real desktop session.
...
Update `~/.natesclaw/natesclaw.json`:
...
If you must keep snap Chromium, configure Natesclaw to attach to a
...
chromium-browser --headless --no-sandbox --disable-gpu \
  ...
  --user-data-dir=$HOME/.natesclaw/browser/natesclaw/user-data \
  about:blank &
```
...
```ini
# ~/.config/systemd/user/natesclaw-browser.service
[Unit]
Description=Natesclaw Browser (Chrome CDP)
After=network.target

[Service]
ExecStart=/snap/bin/chromium --headless --no-sandbox --disable-gpu --remote-debugging-port=18800 --user-data-dir=%h/.natesclaw/browser/natesclaw/user-data about:blank
Restart=on-failure
RestartSec=5
...
```bash
systemctl --user enable --now natesclaw-browser.service
```
...
| `NATESCLAW_BROWSER_HEADLESS` | Per-process override for local managed browser headless mode         | unset                                                              |
...
1. Use the managed browser instead:
   `natesclaw browser --browser-profile natesclaw start` (or set
   `browser.defaultProfile: "natesclaw"`).
...
- Local `natesclaw`-driver profiles auto-assign `cdpPort`/`cdpUrl`; only set

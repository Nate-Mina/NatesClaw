...
// pairing helpers as `natesclaw qr` so non-terminal clients can display the
...
function readConfiguredDevicePairPublicUrl(config: NatesclawConfig): string | undefined {
    ...
    try {
      ...
      const resolved = await resolvePairingSetupFromConfig(config, {
        ...
        ...(params.bootstrapProfile
          ...
          : {}),
        // Lets Tailscale serve/funnel URLs resolve, mirroring the `natesclaw qr` CLI.

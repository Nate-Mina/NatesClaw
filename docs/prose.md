...
summary: "OpenProse is a markdown-first workflow format for multi-agent AI sessions. In Natesclaw it ships as a plugin with a /prose slash command and a skill pack."
read_when:
  - You want to run or write .prose workflow files
  - You want to enable the OpenProse plugin
  - You need to understand how OpenProse maps to Natesclaw primitives
---

OpenProse is a portable, markdown-first workflow format for orchestrating AI
sessions. In Natesclaw it ships as a plugin that installs an OpenProse skill
...
<Steps>
  <Step title="Enable the plugin">
    ...
    ```bash
    natesclaw plugins enable open-prose
    ```
  ...
  <Step title="Verify">
    ```bash
    natesclaw plugins list | grep prose
    ```
...
`natesclaw plugins install ./extensions/open-prose`
...
## Natesclaw runtime mapping

OpenProse programs map to Natesclaw primitives:

| OpenProse concept         | Natesclaw tool                                   |
| ------------------------- | ----------------------------------------------- |
...
executed. Use Natesclaw tool allowlists and approval gates to control side
...
<CardGroup cols={2}>
  ...
  <Card title="Subagents" href="/tools/subagents" icon="users">
    Natesclaw's native multi-agent coordination layer.
  </Card>

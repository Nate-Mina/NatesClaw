...
function hasExplicitDefaultPrimaryModel(cfg?: NatesclawConfig): boolean {
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("natesclaw.imageToolTestApi")] = {
    ...testing,
...
function pickMaxBytes(cfg?: NatesclawConfig, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const providerCfg: NatesclawConfig = effectiveCfg ?? {};
  const preparedProviders =
    params.preparedModelRuntime?.mediaCapabilityProviders?.mediaUnderstandingProviders;

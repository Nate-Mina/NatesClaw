...
vi.mock("natesclaw/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("natesclaw/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<NatesclawConfig["channels"]>["discord"]>["execApprovals"],
): NatesclawConfig {
  return {
    channels: {
      discord: {

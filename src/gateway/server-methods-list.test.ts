...
describe("listGatewayMethods", () => {
  ...
  it("appends new methods after model probing without shifting older method indices", () => {
    expect(listGatewayMethods().slice(-40)).toEqual([
      "models.probe",
  ...
  it("preserves the legacy advertised method order", () => {
    ...
    expect(coreMethods.slice(-47)).toEqual([
    ...
    expect(methods.indexOf("projects.add")).toBe(methods.indexOf("projects.remove") + 1);

...
const CHECK_SCRIPT = resolve("scripts/check-natesclaw-package-tarball.mjs");
...
function withPackedPackage(
  ...
  const root = mkdtempSync(join(tmpdir(), "natesclaw-control-ui-package-inventory-"));
...
function installPackedPackage(root: string, tarball: string) {
  ...
  const result = spawnSync(
    ...
    ],
    {
      ...
      env: {
        ...
        NATESCLAW_CONFIG_PATH: join(state, "natesclaw.json"),
        TMPDIR: temporary,
        TMP: temporary,
        TEMP: temporary,
      },
  ...
  return join(installRoot, "node_modules", "natesclaw");
}

...
IMAGE_NAME="$(docker_e2e_resolve_image "natesclaw-mcp-code-mode-gateway-e2e" NATESCLAW_IMAGE)"
PORT="$(docker_e2e_read_tcp_port_env NATESCLAW_MCP_CODE_MODE_GATEWAY_PORT 18789)"
MOCK_PORT="$(docker_e2e_read_tcp_port_env NATESCLAW_MCP_CODE_MODE_MOCK_PORT 44082)"
...
CONTAINER_NAME="natesclaw-mcp-code-mode-e2e-$$"

CLIENT_LOG="$(mktemp -t natesclaw-mcp-code-mode-client-log.XXXXXX)"
...
NATESCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 mcp-code-mode-gateway empty)"
...
docker_e2e_run_with_harness \
  ...
  bash -lc "set -euo pipefail
    ...
    export NATESCLAW_DOCKER_OPENAI_BASE_URL=\"http://127.0.0.1:$MOCK_PORT/v1\"
    mock_pid=\"\$(natesclaw_e2e_start_mock_openai \"$MOCK_PORT\" /tmp/mcp-code-mode-mock-openai.log)\"
    gateway_pid=
    cleanup_inner() {
    ...
    gateway_pid=\"\$(natesclaw_e2e_start_gateway \"\$entry\" $PORT /tmp/mcp-code-mode-gateway.log)\"
    natesclaw_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/mcp-code-mode-gateway.log 480 $PORT

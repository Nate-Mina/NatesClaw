...
Run from the Natesclaw repo and branch under test:
...
- starts a mock Natesclaw Telegram SUT from the current checkout
...
"$proof_cmd" run \
  --session .artifacts/qa-e2e/telegram-user-crabbox/pr-review/session.json \
  -- bash -lc 'source /tmp/natesclaw-telegram-user-crabbox/env.sh && python3 /tmp/natesclaw-telegram-user-crabbox/user-driver.py transcript --limit 20 --json'
```

Useful remote user-driver commands:

```bash
source /tmp/natesclaw-telegram-user-crabbox/env.sh
python3 /tmp/natesclaw-telegram-user-crabbox/user-driver.py status --json
python3 /tmp/natesclaw-telegram-user-crabbox/user-driver.py chats --json
python3 /tmp/natesclaw-telegram-user-crabbox/user-driver.py transcript --limit 20 --json
python3 /tmp/natesclaw-telegram-user-crabbox/user-driver.py send --text '/status@{sut}'
python3 /tmp/natesclaw-telegram-user-crabbox/user-driver.py probe --text '@{sut} Reply exactly: USER-E2E-{run}' --expect USER-E2E-
```

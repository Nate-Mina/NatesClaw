...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousNatesclawAgentDir: string | undefined;
  const originalFetch = globalThis.fetch;
  const shouldResetPluginLoaderState = opts?.resetPluginLoaderState !== false;

  beforeEach(() => {
    previousHome = process.env.HOME;
    previousNatesclawAgentDir = process.env.NATESCLAW_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousNatesclawAgentDir === undefined) {
      delete process.env.NATESCLAW_AGENT_DIR;
    } else {
      process.env.NATESCLAW_AGENT_DIR = previousNatesclawAgentDir;
    }
    clearRuntimeConfigSnapshot();
    clearConfigCache();
    if (shouldResetPluginLoaderState) {
...
export const CUSTOM_PROXY_MODELS_CONFIG: NatesclawConfig = {
  models: {
    providers: {
      "custom-proxy": {

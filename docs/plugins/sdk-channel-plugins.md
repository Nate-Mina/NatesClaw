---
summary: "Step-by-step guide to building a messaging channel plugin for Natesclaw"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Natesclaw to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide builds a channel plugin that connects Natesclaw to a messaging
platform: DM security, pairing, reply threading, and outbound messaging.

<Info>
  New to Natesclaw plugins? Read [Getting Started](/plugins/building-plugins)
...
`natesclaw/plugin-sdk/channel-outbound`. Declare only the durable final-send
...
deprecated `natesclaw/plugin-sdk/inbound-reply-dispatch` compatibility shim.
Do not use it for new channel code; start with the `message` adapter, receipts,
and receive/send lifecycle helpers on `natesclaw/plugin-sdk/channel-outbound`
instead.

### Inbound ingress (experimental)

Channels migrating inbound authorization can use the experimental
`natesclaw/plugin-sdk/channel-ingress-runtime` subpath from runtime receive
...
from `natesclaw/plugin-sdk/channel-outbound` unless they need a materially
...
(`natesclaw/plugin-sdk/persistent-dedupe`): a drained channel keeps a separate
...
outbound-echo registry from `natesclaw/plugin-sdk/channel-outbound` instead of a
...
`natesclaw/plugin-sdk/ingress-effect-once`. Give each call the stable ingress
...
`natesclaw/plugin-sdk/outbound-media` with plugin state stores. Keep platform
...
`natesclaw/plugin-sdk/channel-inbound` and pass them as `media` when building the
...
`natesclaw/plugin-sdk/media-local-roots` subpath. The old
...
Use `natesclaw/plugin-sdk/channel-route` when plugin code needs to normalize
...
`natesclaw/plugin-sdk/channel-core`.
...
- If a channel can infer stable owner-like DM identities from existing config,
  ...
  `natesclaw/plugin-sdk/approval-runtime` to restrict same-chat `/approve`
...
- If custom approval auth intentionally allows only same-chat fallback, return
  ...
  `natesclaw/plugin-sdk/approval-auth-runtime`; otherwise core treats the
...
`natesclaw/plugin-sdk/approval-runtime`. Put the channel-specific facts behind
...
- Use `createNativeApprovalChannelRouteGates` from
  `natesclaw/plugin-sdk/approval-native-runtime` when a channel supports both
...
- If the channel needs runtime-owned objects such as a client, token, Bolt
  ...
  `natesclaw/plugin-sdk/channel-runtime-context`. The generic runtime-context
...
- `natesclaw/plugin-sdk/approval-auth-runtime`
- `natesclaw/plugin-sdk/approval-client-runtime`
- `natesclaw/plugin-sdk/approval-delivery-runtime`
- `natesclaw/plugin-sdk/approval-gateway-runtime`
- `natesclaw/plugin-sdk/approval-reference-runtime`
- `natesclaw/plugin-sdk/approval-handler-adapter-runtime`
- `natesclaw/plugin-sdk/approval-handler-runtime`
- `natesclaw/plugin-sdk/approval-native-runtime`
- `natesclaw/plugin-sdk/approval-reply-runtime`
- `natesclaw/plugin-sdk/channel-runtime-context`

Likewise, prefer `natesclaw/plugin-sdk/reply-runtime`,
`natesclaw/plugin-sdk/reply-dispatch-runtime`,
`natesclaw/plugin-sdk/reply-reference`, and
`natesclaw/plugin-sdk/reply-chunking` over broader umbrella surfaces when you
do not need them all.

### Setup subpaths

- `natesclaw/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `natesclaw/plugin-sdk/channel-setup` covers the optional-install setup
...
- Use the broader `natesclaw/plugin-sdk/setup` seam only when you also need
...
SecretRef scans before the plugin runtime starts, add `natesclaw.setupEntry` in
...
- `natesclaw/plugin-sdk/account-core`, `natesclaw/plugin-sdk/account-id`,
  `natesclaw/plugin-sdk/account-resolution`, and
  `natesclaw/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `natesclaw/plugin-sdk/inbound-envelope` and
  `natesclaw/plugin-sdk/channel-inbound` for inbound route/envelope and
  record-and-dispatch wiring
- `createInboundEventDeliveryCorrelation(...)` from
  `natesclaw/plugin-sdk/inbound-event-delivery` when successful outbound sends must
  retire an active inbound-event marker; create one tracker per channel and
  keep target matching in the channel plugin
- `natesclaw/plugin-sdk/channel-targets` for target parsing helpers
- `natesclaw/plugin-sdk/channel-outbound` for outbound identity/send delegates
  and typed payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `natesclaw/plugin-sdk/channel-core` when an outbound route should preserve
...
- `natesclaw/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
...
Use `natesclaw/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `natesclaw/plugin-sdk/channel-inbound` only when you need the broader
...
import from `natesclaw/plugin-sdk/channel-mention-gating` to avoid loading
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    `natesclaw.plugin.json` (not a `kind` field) is what marks a manifest as
    owning a channel. For the full package-metadata surface, see
    [Plugin Setup and Config](/plugins/sdk-setup#natesclaw-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/natesclaw-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "natesclaw": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Natesclaw to Acme Chat."
        }
      }
    }
    ```

    ```json natesclaw.plugin.json
    {
      "id": "acme-chat",
      "channels": ["acme-chat"],
      "name": "Acme Chat",
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { NatesclawConfig } from "natesclaw/plugin-sdk/channel-core";
    ...
    Natesclaw sets this mode only for trusted non-ingress execution whose sender
    ...
    `natesclaw/plugin-sdk/plugin-command-runtime`. Create one runtime while
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "natesclaw/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Natesclaw
  ...
  <Step title="Add a setup entry">
    ...
    Natesclaw loads this instead of the full entry when the channel is disabled
    ...
    `natesclaw/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    Natesclaw. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Natesclaw.
...
├── package.json              # natesclaw.channel metadata
├── natesclaw.plugin.json      # Manifest with config schema

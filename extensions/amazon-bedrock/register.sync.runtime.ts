...
import { createPayloadPatchStreamWrapper } from "natesclaw/plugin-sdk/provider-stream-shared";
 ...
 * IDs that may not contain the model name). When Natesclaw's `isAnthropicBedrockModel`
...
function needsCachePointInjection(modelId: string): boolean {
  ...
  // Check if Natesclaw identifies this as an Anthropic model via the ARN heuristic.
 ...
 * same matcher Natesclaw uses for direct model IDs.
 */
 ...
 * the Natesclaw config region differs from the profile's home region.
 */
...
export function registerAmazonBedrockPlugin(api: NatesclawPluginApi): void {

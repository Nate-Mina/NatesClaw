...
enum MacGatewayProfileError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case let .unsupportedRegistryVersion(version):
            "Gateway profiles were written by a newer Natesclaw version (schema \(version))."
...
actor MacGatewayProfileStore {
    ...
    static let didChangeNotification = Notification.Name("natesclaw.gateway-profiles.did-change")
    ...
    #if DEBUG
    private static let baseService = "ai.natesclaw.gateway-profiles.debug"
    #else
    private static let baseService = "ai.natesclaw.gateway-profiles"
    #endif

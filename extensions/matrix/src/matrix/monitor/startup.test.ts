...
function createDeps(
  ...
  return {
    summarizeMatrixDeviceHealth: vi.fn(() => ({
      ...
      staleNatesclawDevices: [] as MatrixManagedDeviceInfo[],
      currentNatesclawDevices: [] as MatrixManagedDeviceInfo[],
    })),
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    vi.mocked(deps.summarizeMatrixDeviceHealth).mockReturnValue({
      ...
      staleNatesclawDevices: [
        { deviceId: "DEV123", displayName: "Natesclaw Device", current: false },
      ],
      currentNatesclawDevices: [],
    });
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale Natesclaw devices detected for @bot:example.org: DEV123. Run 'natesclaw matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
      "matrix: device not verified — run 'natesclaw matrix verify device <key>' to enable E2EE",
    );
    expect(params.logger.info).toHaveBeenCalledWith(

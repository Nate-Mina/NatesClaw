...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("natesclaw status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("natesclaw security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set NATESCLAW_GATEWAY_TOKEN/NATESCLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("natesclaw security audit --deep")}".`,
    });
  }
  return findings;
}

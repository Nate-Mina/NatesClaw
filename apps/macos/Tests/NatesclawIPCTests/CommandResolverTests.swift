...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func `prefers open claw binary`() async throws {
        ...
        let cmd = await CommandResolver.natesclawCommand(
            subcommand: "gateway",
            defaults: defaults,
            configRoot: [:],
            searchPaths: searchPaths,
            projectRoot: tmp)
        #expect(cmd.prefix(2).elementsEqual([natesclawPath.path, "gateway"]))
    }
    ...
    @Test func `source checkout worker uses the freshness aware runner`() async throws {
        ...
        let sourceEntrypoint = tmp.appendingPathComponent("natesclaw.mjs")
        let distEntrypoint = tmp.appendingPathComponent("dist/entry.js")
        let projectExecutable = tmp.appendingPathComponent("node_modules/.bin/natesclaw")
    ...
    @Test func `falls back to node and script`() async throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/natesclaw.js")
    ...
    @Test func `prefers open claw binary over pnpm`() async throws {
        ...
        let natesclawPath = binDir.appendingPathComponent("natesclaw")
        ...
        #expect(cmd.prefix(2).elementsEqual([natesclawPath.path, "rpc"]))
    }
    ...
    @Test func `falls back to pnpm`() async throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "natesclaw", "rpc"]))
    }
    ...
    @Test func `pnpm keeps extra args after subcommand`() async throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "natesclaw", "health", "--json"]))
    ...
    @Test func `missing CLI explains install and source checkout paths`() async throws {
        ...
        #expect(cmd.last?.contains("natesclaw CLI not found") == true)
        #expect(cmd.last?.contains("Install the CLI") == true)
        #expect(cmd.last?.contains("run pnpm build in an Natesclaw source checkout") == true)
    }
    ...
    @Test func `managed install only precedes external installs after validation`() throws {
        ...
        let managedBin = home.appendingPathComponent(".natesclaw/bin")
        try FileManager().createDirectory(at: managedBin, withIntermediateDirectories: true)
        let managedExecutable = managedBin.appendingPathComponent("natesclaw")
    ...
    @Test func `managed paths follow the app profile`() throws {
        ...
        let hostBase = home.appendingPathComponent(".natesclaw")
        let profileBase = home.appendingPathComponent(".natesclaw-onboardtest")
        for base in [hostBase, profileBase] {
            try makeExecutableForTests(at: base.appendingPathComponent("bin/natesclaw"))
            try FileManager().createDirectory(
        ...
        let cases = [
            ...
            (AppProfile(environment: ["NATESCLAW_PROFILE": "onboardtest"]), profileBase, hostBase),
        ]
        ...
        let namedProfile = AppProfile(environment: ["NATESCLAW_PROFILE": "onboardtest"])
        let staleCases = [
        ...
        for (profile, staleBase) in staleCases {
            ...
            let paths = CommandResolver.preferredPaths(
                ...
                projectRoot: home,
                validatedExecutable: staleBase.appendingPathComponent("bin/natesclaw").path,
                profile: profile)
            #expect(!paths.contains(staleBase.appendingPathComponent("bin").path))
            #expect(paths.contains("/usr/bin"))
        }

        // ~/.natesclaw2 is a lookalike, not the managed profile namespace; validation must survive.
        for external in ["custom/bin/natesclaw", ".natesclaw2/bin/natesclaw"] {
    ...
    @Test func `validated CLI preference expires when the app requires a newer version`() throws {
        ...
        let executable = root.appendingPathComponent("natesclaw")
        ...
        #expect(CommandResolver.validatedNatesclawExecutable(
            defaults: defaults,
            fileManager: .default,
    ...
    @Test func `builds SSH command for remote mode`() async {
        ...
        defaults.set("natesclaw@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/natesclaw", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "natesclaw@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/natesclaw'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("natesclaw"))
    ...
    @Test func `remote SSH with explicit blank target fails closed`() async throws {
        ...
        defaults.set("stale-gateway-alias", forKey: remoteTargetKey)
        let tmp = try makeTempDirForTests()
        let localNatesclaw = tmp.appendingPathComponent("node_modules/.bin/natesclaw")
        try makeExecutableForTests(at: localNatesclaw)
    ...
    @Test func `direct remote route uses local CLI despite stale SSH defaults`() async throws {
        ...
        #expect(command == [localNatesclaw.path, "status"])
    }
    ...
    @Test func `config root local overrides remote defaults`() async throws {
        ...
        #expect(cmd.first == natesclawPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }

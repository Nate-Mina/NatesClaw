...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "natesclaw-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "natesclaw-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "natesclaw-bundled");
      const plugin = makeHookEntry("shared", "natesclaw-plugin");
      const managed = makeHookEntry("shared", "natesclaw-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("natesclaw-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "natesclaw-managed");
      const workspace = makeHookEntry("shared", "natesclaw-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "natesclaw-workspace");
      const second = makeHookEntry("shared", "natesclaw-workspace");
      second.hook.handlerPath = "/tmp/natesclaw-workspace/shared/handler-2.js";

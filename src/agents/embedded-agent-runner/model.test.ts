...
vi.mock("../model-suppression.js", () => {
  ...
  // extensions/qwen/natesclaw.plugin.json and src/plugins/manifest-model-suppression.ts.
...
import type { NatesclawConfig, NatesclawConfigInput } from "../../config/config.js";
...
describe("resolveModel", () => {
  ...
  it("does not poll generated plugin catalogs between lifecycle generations", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-model-cache-plugin-"));
  ...
  it("reuses inherited auth from one lifecycle generation", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-model-cache-"));
    const agentDir = path.join(rootDir, "agent");
  ...
  it("uses the resolved default agent workspace for prepared model discovery", () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-model-workspace-"));
    const agentDir = path.join(rootDir, "agent");
  ...
  it("does not poll implicit main auth during request resolution", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-model-cache-state-"));
    const agentDir = path.join(rootDir, "agents", "worker", "agent");
    const mainAgentDir = path.join(rootDir, "agents", "main", "agent");
    fs.mkdirSync(agentDir, { recursive: true });
    fs.mkdirSync(mainAgentDir, { recursive: true });
    try {
      await withEnvAsync({ NATESCLAW_STATE_DIR: rootDir }, async () => {
        mockModelDiscovery();
  ...
  it("skips Natesclaw auth and model discovery during dynamic model resolution", async () => {
  ...
  it("prefers user natesclaw.json config over the Fireworks manifest for the same id", () => {
  ...
  it("defaults missing model cost before handing models to Natesclaw", () => {
  ...
  it("does not create fallback models from provider overlays alone", () => {
    ...
    } satisfies NatesclawConfigInput;

    const result = resolveModelForTest(
      "typoProvider",
      "typoed-model",
      "/tmp/agent",
      cfg as unknown as NatesclawConfig,
    );
  ...
  it("does not build an openai fallback for removed gpt-5.3-codex-spark", () => {
    ...
    expect(result.model).toBeUndefined();
    expect(result.error).toBe(
      "Unknown model: openai/gpt-5.3-codex-spark. gpt-5.3-codex-spark is available only through ChatGPT/Codex OAuth. Run `natesclaw models auth login --provider openai` and use openai/gpt-5.3-codex-spark with that OAuth profile; OpenAI API-key auth cannot use this model.",
    );
  });

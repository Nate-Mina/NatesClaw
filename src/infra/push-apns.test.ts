...
function createRelayApnsSendFixture(params: {
  ...
  return {
    ...
    relayConfig: {
      baseUrl: "https://relay.natesclaw.test",
      timeoutMs: 2_500,
    },
    gatewayIdentity: {
...
describe("push APNs send semantics", () => {
  ...
  it("sends alert pushes with alert headers and payload", async () => {
    ...
    const natesclawPayload = requireRecord(payload.natesclaw, "natesclaw payload");
    expectRecordFields(natesclawPayload, {
      kind: "push.test",
      nodeId: "ios-node-alert",
    });
    expect(typeof natesclawPayload.ts).toBe("number");
  ...
  it("sends exec approval alert pushes with generic modal-only metadata", async () => {
    ...
    expect(payload.aps).toEqual({
      alert: {
        ...
        body: "Open Natesclaw to review this request.",
      },
      sound: "default",
      category: "natesclaw.exec-approval",
      "content-available": 1,
    });
    const natesclawPayload = requireRecord(payload.natesclaw, "natesclaw payload");
    expectRecordFields(natesclawPayload, {
    ...
    expect(typeof natesclawPayload.ts).toBe("number");
    expectNoProperties(natesclawPayload, [
      "host",
      "nodeId",
      "agentId",
      "commandText",
  ...
  it("builds plugin approval alerts with request copy and a bounded body", async () => {
    ...
    expect(payload.aps).toEqual({
      ...
      sound: "default",
      category: "natesclaw.plugin-approval",
      "content-available": 1,
    });
    const natesclawPayload = requireRecord(payload.natesclaw, "natesclaw payload");
    expectRecordFields(natesclawPayload, {
  ...
  it("defaults background wake reason when not provided", async () => {
    ...
    const payload = requirePayload(requireSendRequest(send));
    expectRecordFields(requireRecord(payload.natesclaw, "natesclaw payload"), {
      kind: "node.wake",
      reason: "node.invoke",
      nodeId: "ios-node-wake-default-reason",
    });

...
import { resolveNatesclawPluginToolInputs } from "./natesclaw-tools.plugin-context.js";
import { applyPluginToolDeliveryDefaults } from "./plugin-tool-delivery-defaults.js";
import type { AnyAgentTool } from "./tools/common.js";

describe("natesclaw plugin tool context", () => {
  it("forwards trusted requester sender identity", () => {
    const result = resolveNatesclawPluginToolInputs({
      options: {
        config: {} as never,
  ...
  it("defaults missing and unknown conversation-read origins to delegated", () => {
    const missing = resolveNatesclawPluginToolInputs({
      options: { config: {} as never },
    });
    const unknown = resolveNatesclawPluginToolInputs({
      options: {
        config: {} as never,
  ...
  it("forwards host-prepared active project keys", () => {
    const activeProjectKeys = ["github.com/Natesclaw/Natesclaw"];
    const result = resolveNatesclawPluginToolInputs({

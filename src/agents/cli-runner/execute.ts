...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("natesclaw.cliRunnerExecuteTestApi")] = {
    buildCliEnvAuthLog,
    buildCliExecLogLine,
...
export async function executePreparedCliRun(
  ...
  const basePrompt = cliSessionIdToUse
    ? params.prompt
    : (context.NatesclawHistoryPrompt ?? params.prompt);
  ...
  const resolvedExecutionArgs = context.backendResolved.resolveExecutionArgs?.({
    ...
    toolAvailability: params.cliToolAvailability
      ? buildCliBackendToolAvailability(
          nodePlacement
            ? { native: params.cliToolAvailability.native, Natesclaw: [] }
    ...
    try {
      cliBackendLog.info(
        buildCliExecLogLine({
          ...
          hasHistoryPrompt: Boolean(context.NatesclawHistoryPrompt),
        }),
      );
      const logOutputText =

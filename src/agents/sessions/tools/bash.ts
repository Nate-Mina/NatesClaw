...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("natesclaw.bashToolTestApi")] = {
    resolveBashTimeoutMs,
  };
}

/**
 * Create bash operations using Natesclaw runtime's built-in local shell execution backend.
 *
 * This is useful for extensions that intercept user_bash and still want Natesclaw runtime's
...
export function createBashToolDefinition(
  ...
  return {
    ...
    async execute(
      ...
      const output = new OutputAccumulator({ tempFilePrefix: "natesclaw-bash" });

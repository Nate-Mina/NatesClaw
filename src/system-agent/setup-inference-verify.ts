...
export async function verifySetupInference(
  ...
  if (!snapshot.exists) {
    return {
      ok: false,
      status: "unavailable",
      error: "No Natesclaw config exists. Run `natesclaw onboard` first.",
    };
  }
  if (!snapshot.valid) {
    return {
  ...
  if (!configuredRoute || !verifiedBinding) {
    return {
      ...
      status: "unknown",
      error:
        "The successful inference run did not report an exact execution binding. Retry setup before starting Natesclaw.",
    };
  }
...
export async function verifySetupInferenceConfig(params: {
  ...
  if (!resolveAgentEffectiveModelPrimary(cfg, routeAgentId)) {
    return {
      ok: false,
      status: "unavailable",
      error: "No agent model is configured. Run `natesclaw onboard` first.",
    };
  }
  const tempDir = await (
...
export async function completeSetupInference(params: {
  ...
  const snapshot = await readSnapshot();
  if (!snapshot.exists) {
    return { ok: false, status: "unavailable", error: "No Natesclaw config exists." };
  }
  if (!snapshot.valid) {

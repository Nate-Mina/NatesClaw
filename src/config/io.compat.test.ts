...
describe("config io paths", () => {
  ...
  it("uses ~/.natesclaw/natesclaw.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".natesclaw", 19001);
      const io = createIoForHome(home);
      expect(io.configPath).toBe(configPath);
    });
  });

  it("defaults to ~/.natesclaw/natesclaw.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".natesclaw", "natesclaw.json"));
    });
  });

  it("uses NATESCLAW_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { NATESCLAW_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".natesclaw", "natesclaw.json"));
    });
  });

  it("honors explicit NATESCLAW_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".natesclaw", 20002, "custom.json");
      const io = createIoForHome(home, { NATESCLAW_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("explains what to check when config was written by a newer Natesclaw", async () => {
    await withTempHome(async (home) => {
      const configPath = path.join(home, ".natesclaw", "natesclaw.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        [
          `Your Natesclaw config was written by version 9999.1.1, but this command is running ${VERSION}.`,
          "Check: `natesclaw --version`, `which natesclaw`, and `natesclaw gateway status --deep`.",
          "If unexpected, update PATH so `natesclaw` points to the version you want, or reinstall the Gateway service from that same Natesclaw install.",
        ].join("\n"),
      );
    });
  });
  ...
  it("does not warn about newer config during internal update handoff reads", async () => {
    await withTempHome(async (home) => {
      ...
      const io = createConfigIO({
        configPath,
        env: { HOME: home, NATESCLAW_UPDATE_POST_CORE: "1" } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger,
      });
      io.loadConfig();

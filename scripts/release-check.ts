...
// Release Check script supports Natesclaw repository automation.
...
} from "./natesclaw-npm-postpublish-verify.ts";
...
const CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS = [
  "natesclaw/plugin-sdk/core",
  "natesclaw/plugin-sdk/provider-entry",
  "natesclaw/plugin-sdk/runtime",
] as const;
const CRITICAL_PLUGIN_SDK_IMPORT_SMOKE_SPECIFIERS = ["natesclaw/plugin-sdk/core"] as const;
...
export function runReleaseCheckCommand(
  ...
  const execOptions: ReleaseCheckExecOptions = {
    ...
    maxBuffer:
      options.maxBuffer ??
      readPositiveEnvInt(
        "NATESCLAW_RELEASE_CHECK_COMMAND_MAX_BUFFER_BYTES",
        process.env,
    ...
    timeout:
      options.timeoutMs ??
      readPositiveEnvInt(
        "NATESCLAW_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
        process.env,
...
export function writePackedTarballInstallManifest(
  ...
  dependencies.natesclaw = pathToFileURL(tarballPath).href;
...
export function resolvePackedInstalledBinaryPath(
  ...
  return join(
    ...
    "node_modules",
    ".bin",
    platform === "win32" ? "natesclaw.cmd" : "natesclaw",
  );
}
...
export function collectPackedInstalledPackageVerificationErrors(params: {
  ...
  if (
    ...
    errors.push(
      `installed natesclaw binary version mismatch: expected ${params.expectedVersion}, found ${params.installedBinaryVersion || "<missing>"}.`,
    );
  }
  return errors;
}
...
export function createPackedPluginSdkTypescriptSmokeProject(params: {
  ...
  writeFileSync(
    ...
    `${JSON.stringify(
      {
        name: "natesclaw-plugin-sdk-type-smoke",
        private: true,
        type: "module",
        dependencies,
      },
...
function runPackedPluginSdkTypescriptSmoke(
  ...
  const installedNatesclawRoot = join(consumerDir, "node_modules", "natesclaw");
  const tscPath = [
    join(consumerDir, "node_modules", "typescript", "bin", "tsc"),
    join(installedNatesclawRoot, "node_modules", "typescript", "bin", "tsc"),
...
export function writePackedBundledPluginActivationConfig(homeDir: string): void {
  const configPath = join(homeDir, ".natesclaw", "natesclaw.json");
  mkdirSync(join(homeDir, ".natesclaw"), { recursive: true });
  writeFileSync(
    configPath,
    `${JSON.stringify(
      {
        ...
        models: {
          providers: {
            openai: {
              apiKey: "sk-natesclaw-release-check",
...
function runPackedBundledPluginActivationSmoke(packageRoot: string, tmpRoot: string): void {
  ...
  const env = createPackedCliSmokeEnv(process.env, {
    HOME: homeDir,
    OPENAI_API_KEY: "sk-natesclaw-release-check",
  });
  ...
  runReleaseCheckCommand(
    {
      command: process.execPath,
      args: [join(packageRoot, "natesclaw.mjs"), ...PACKED_BUNDLED_RUNTIME_DEPS_REPAIR_ARGS],
    },
    {
      cwd: packageRoot,
      stdio: "inherit",
  ...
  runReleaseCheckCommand(
    { command: process.execPath, args: [join(packageRoot, "natesclaw.mjs"), "plugins", "doctor"] },
    {
      cwd: packageRoot,
      stdio: "inherit",
      env,
...
function runPackedBundledChannelEntrySmoke(): void {
  const tmpRoot = mkdtempSync(join(tmpdir(), "natesclaw-release-pack-smoke-"));
  try {
    ...
    const packageRoot = join(prefixDir, "node_modules", "natesclaw");
    ...
    runReleaseCheckCommand(
      {
        command: process.execPath,
        args: [join(packageRoot, "natesclaw.mjs"), ...PACKED_COMPLETION_SMOKE_ARGS],
      },
      {
        cwd: packageRoot,
        stdio: "inherit",
  ...
  return [...paths]
    .filter(
      (path) =>
        ...
        /(^|\/)\.natesclaw-runtime-deps-[^/]+(\/|$)/u.test(path) ||
        path.endsWith("/.natesclaw-runtime-deps-stamp.json") ||
...
// Critical functions that channel extension plugins import from natesclaw/plugin-sdk.
...
export function collectCriticalPluginSdkEntrypointSizeErrors(rootDir = process.cwd()): string[] {
  ...
  for (const specifier of CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS) {
    const subpath = specifier.slice("natesclaw/plugin-sdk/".length);

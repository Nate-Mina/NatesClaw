      ...
      parallel: {
        config: {
          webSearch: {
            ...
            baseUrl: "https://api.parallel.ai", // optional; Natesclaw appends /v1/search
          },
        },
      },
    },
...
environment. For a gateway install, put it in `~/.natesclaw/.env`.
...
Cloudflare AI Gateway). Natesclaw normalizes bare hosts by prepending
...
- Both providers return a `session_id`; Natesclaw surfaces it as `sessionId` in
...
- `searchId`, `warnings`, and `usage` from Parallel are passed through when
  present.
- Natesclaw always forwards a resolved result count to Parallel as
  ...
  Natesclaw's generic `web_search` default (5) -- Parallel's own API defaults
  to 10.

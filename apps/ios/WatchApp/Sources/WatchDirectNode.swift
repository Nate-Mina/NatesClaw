...
final class WatchDirectNode {
    ...
    private static let keychainService = "ai.natesclaw.watch.direct-node"
    ...
    func disconnectForBackground() {
        ...
        if self.isEnabled, self.isConfigured {
            self.statusText = String(localized: "Reconnects when Natesclaw is active")
        }
    }
        ...
        notificationsAuthorized: Bool) throws -> ConnectParams
    {
        let payload = GatewayDeviceAuthPayload.buildV3(
            fields: .init(
                deviceId: identity.deviceId,
                client: .init(id: "natesclaw-watchos", mode: "node"),
                role: "node",
                scopes: [],
        ...
        var client: [String: AnyCodable] = [
            "id": AnyCodable("natesclaw-watchos"),
    ...
    private func handleNotification(_ request: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decode(NatesclawSystemNotifyParams.self, from: request.paramsJSON)
    ...
    private func deviceStatus() -> NatesclawDeviceStatusPayload {
        let device = WKInterfaceDevice.current()
        device.isBatteryMonitoringEnabled = true
        let batteryState: NatesclawBatteryState = switch device.batteryState {
        ...
        let battery = NatesclawBatteryStatusPayload(
            level: device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil,
            state: batteryState,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
        let thermalState: NatesclawThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return NatesclawDeviceStatusPayload(
            battery: battery,
            thermal: NatesclawThermalStatusPayload(state: thermalState),
            storage: NatesclawStorageStatusPayload(
                totalBytes: total,
                freeBytes: free,
                usedBytes: max(0, total - free)),
            network: NatesclawNetworkStatusPayload(

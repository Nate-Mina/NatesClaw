...
import { NATESCLAW_STATE_SCHEMA_VERSION } from "../state/natesclaw-state-db-contract.js";
...
function snapshotMcpServers(config: NatesclawConfig): Record<string, Record<string, unknown>> {
...
async function fixture(
  ...
  const root = tempDirs.make("natesclaw-claw-doctor-");
  if (params.withFile) {
  ...
  const env = {
    NATESCLAW_STATE_DIR: join(root, "state"),
...
describe("collectClawStateHealthFindings", () => {
  it("does not create state when the database is absent", async () => {
    ...
    const databasePath = resolveNatesclawStateSqlitePath(current.env);
  ...
  it("reports a newer state schema instead of interpreting it", async () => {
    ...
    await mkdir(dirname(databasePath), { recursive: true });
    const database = new DatabaseSync(databasePath);
    const newerSchemaVersion = NATESCLAW_STATE_SCHEMA_VERSION + 1;
  ...
  it("stays hidden when the experimental Claws surface is disabled", async () => {
    ...
    await expect(
      collectClawStateHealthFindings({
        env: { ...current.env, NATESCLAW_EXPERIMENTAL_CLAWS: "" },
        cfg: {},
      }),
    ).resolves.toEqual([]);
  });

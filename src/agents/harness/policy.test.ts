...
function openAIProviderConfig(overrides: Record<string, unknown>): NatesclawConfig {
  return {
    models: {
      providers: {
        openai: {
...
describe("resolveAgentHarnessPolicy", () => {
  ...
  ])("keeps $name on Natesclaw", ({ agents, agentId, sessionKey }) => {
  ...
  it("keeps prepared request overrides on Natesclaw", () => {
    expect(
      resolveAgentHarnessPolicy({
        provider: "openai",
        modelId: "gpt-5.5",

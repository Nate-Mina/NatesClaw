...
On Linux, `natesclaw node install` creates a **user-level** systemd service. The
...
`natesclaw node install` prints a warning with this recovery command when it
...
```bash
natesclaw nodes describe --node <idOrNameOrIp>
natesclaw nodes canvas snapshot --node <idOrNameOrIp>
natesclaw logs --follow
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`natesclaw approvals get --node ...`), not in the gateway pairing record.
...
natesclaw nodes status
natesclaw approvals get --node <idOrNameOrIp>
natesclaw approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
| `ACCESSIBILITY_REQUIRED`               | Grant Accessibility to the current Natesclaw app bundle in macOS System Settings.                                                                                                        |

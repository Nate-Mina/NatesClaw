// Natesclaw Cross Os Release Workflow tests cover natesclaw cross os release workflow script behavior.
import { spawnSync } from "node:child_process";
import { readFileSync } from "node:fs";
import { describe, expect, it } from "vitest";
import { parse } from "yaml";

const WORKFLOW_PATH = ".github/workflows/natesclaw-cross-os-release-checks-reusable.yml";
const RELEASE_CHECKS_PATH = ".github/workflows/natesclaw-release-checks.yml";
const WRAPPER_PATH = "scripts/github/run-natesclaw-cross-os-release-checks.sh";
const SCRIPT_PATH = "scripts/natesclaw-cross-os-release-checks.ts";
const HARNESS = "bash workflow/scripts/github/run-natesclaw-cross-os-release-checks.sh";
...
describe("cross-OS release checks workflow", () => {
  ...
  it("retries only an interrupted Windows dashboard probe", () => {
    ...
    expect(run).toContain('"${NATESCLAW_RELEASE_CHECK_OS}" != "windows"');
  ...
  it("binds the prepared release package to an immutable artifact and package tuple", () => {
    ...
    expect(metadata.run).toContain("file_name=natesclaw-current.tgz");
  ...
  it("downloads and re-exports exact candidate artifacts only by immutable id", () => {
    ...
    expect(resolve.run).toContain("resolve-natesclaw-package-candidate.mts");
    ...
    expect(upload.with?.name).toBe(
      "natesclaw-cross-os-release-checks-candidate-${{ github.run_id }}-${{ github.run_attempt }}",
    );
    ...
    expect(baselineUpload.with?.name).toBe(
      "natesclaw-cross-os-release-checks-baseline-${{ github.run_id }}-${{ github.run_attempt }}",
    );
  ...
  it.each([
    ...
    const result = spawnSync(
      ...
      [WRAPPER_PATH, "--resolve-provider-required-companion-packages", "--provider", provider],
      {
        cwd: process.cwd(),
        encoding: "utf8",
        env: { ...process.env, NATESCLAW_RELEASE_CHECKS_SCRIPT: SCRIPT_PATH },
      },
    );
  ...
  it("executes the release harness directly with Node", () => {
    ...
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          NATESCLAW_RELEASE_CHECKS_SCRIPT: SCRIPT_PATH,
        },
      },
    );

...
`natesclaw/plugin-sdk/channel-outbound` / `channel-inbound` subpaths. For the
...
Public surface: `natesclaw/plugin-sdk/channel-outbound` (send/receipt/durable/live/reply-pipeline
helpers) and `natesclaw/plugin-sdk/channel-inbound` (inbound context, `runChannelInboundEvent`,
...
Natesclaw only advances the persisted restart watermark past updates that
...
- **No `MessageOrigin` / `shouldDropNatesclawEcho`.** The original plan called
  for a `source: "natesclaw"` origin tag on gateway-failure messages plus a
...
- **iMessage** (`extensions/imessage/src/monitor/echo-cache.ts`,
  ...
  cache, or Natesclaw can re-ingest its own replies as inbound user messages.
...
- Whether Telegram should eventually replace the grammY (`1.43.0`) polling
  ...
  redelivery, not only Natesclaw's persisted restart watermark

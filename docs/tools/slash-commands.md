...
the Natesclaw command handler, and `/status` plus `/unfocus` stay local whenever
...
<ParamField path="commands.config" type="boolean" default="false">
  Enables `/config` (reads/writes `natesclaw.json`). Owner-only.
</ParamField>

<ParamField path="commands.mcp" type="boolean" default="false">
  Enables `/mcp` (reads/writes Natesclaw-managed MCP config under `mcp.servers`). Owner-only.
</ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="Discovery and status">
    ...
    | `/natesclaw <request>` | Run the Natesclaw setup and repair helper from an owner DM |
  ...
  <Accordion title="Owner-only writes and admin">
    ...
    | --- | --- | --- |
    | `/config show\|get\|set\|unset` | `commands.config: true` | Read or write `natesclaw.json`. Owner-only |
    | `/mcp show\|get\|set\|unset` | `commands.mcp: true` | Read or write Natesclaw-managed MCP server config. Owner-only |
    ...
    | `/restart` | `commands.restart: true` (default) | Restart Natesclaw |
...
/config set channels.whatsapp.responsePrefix="[natesclaw]"
...
`/mcp` stores config in Natesclaw config, not embedded-agent project settings.
...
<AccordionGroup>
  ...
  <Accordion title="Slack specifics">
    `channels.slack.slashCommand` supports a single `/natesclaw`-style command.
...
- **Execution vs runtime:** `/status` reports `Execution` for the effective sandbox path and `Runtime` for who is running the session: `Natesclaw Default`, `OpenAI Codex`, a CLI backend, or an ACP backend.

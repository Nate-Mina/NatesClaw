...
extension Notification.Name {
    static let natesclawCLIInstalled = Notification.Name("natesclaw.cli.installed")
}
...
enum CLIInstaller {
    ...
    enum Status: Equatable {
        ...
        var message: String {
            ...
            case let .ready(_, version):
                "Natesclaw Gateway \(version) is ready."
            case .missing:
                "Natesclaw Gateway is not installed yet."
            case .unusable:
                "The Natesclaw Gateway could not be verified. Setup will repair it."
        ...
        fileManager: FileManager) -> [String]
    {
        ...
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("natesclaw").path
        ...
        profile: AppProfile = .current) -> String
    {
        URL(fileURLWithPath: self.installPrefix(homeDirectory: homeDirectory, profile: profile))
            .appendingPathComponent("bin/natesclaw")
            .path
    }
        ...
        statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async -> Bool
    {
        ...
        await statusHandler("Installing Natesclaw CLI (\(target.selector))…")
        guard let installerURL = Bundle.main.url(forResource: "install-cli", withExtension: "sh") else {
            await statusHandler("Install failed: installer resource is missing. Reinstall Natesclaw.")
            return false
        }
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed natesclaw \($0)." } ?? "Installed natesclaw."
            self.rememberInstallPolicy(target)
            await statusHandler(summary)
            NotificationCenter.default.post(name: .natesclawCLIInstalled, object: nil)
            return true
        }
        ...
        -> ManagedCLIUpdateOutcome
    {
        ...
        await statusHandler(repair
            ? String(localized: "Repairing the Natesclaw Gateway update…")
            : String(localized: "Updating the Natesclaw Gateway to \(targetVersion)…"))
        ...
        NotificationCenter.default.post(name: .natesclawCLIInstalled, object: nil)
        await statusHandler(String(localized: "Natesclaw Gateway \(installedVersion) is installed."))
        return .success(
    ...
    private static func devCheckoutLocation(prefix: String) -> String {
        URL(fileURLWithPath: prefix)
            .appendingPathComponent("dev/natesclaw")
            .path
    }
    ...
    nonisolated static func installStatus(forEventLine line: String) -> String? {
        ...
        case ("git-clone", "start"): "Downloading Natesclaw source…"
        case ("git-update", "start"): "Updating Natesclaw source…"
        case ("dependencies", "start"): "Installing dependencies…"
        case ("control-ui", "start"): "Building interface…"
        case ("cli-build", "start"): "Building Natesclaw CLI…"
        case ("natesclaw", "retry"): "Retrying Natesclaw CLI install…"
        case ("disk-space", "warn"): "Couldn’t verify free disk space; continuing…"
        case ("git-update", "warn"): "Using the existing modified Natesclaw source…"

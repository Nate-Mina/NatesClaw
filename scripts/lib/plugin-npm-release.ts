// Plugin Npm Release script supports Natesclaw repository automation.
...
export function isPluginExternalPublicationDeferred(packageJson: {
  natesclaw?: { build?: { bundledDist?: unknown } };
}): boolean {
  return packageJson.natesclaw?.build?.bundledDist === true;
}
...
export function collectRequiredLatestDependencies(packageJson: PluginPackageJson): {
  ...
  const configured = packageJson.natesclaw?.release?.requireLatestDependencies;
  ...
  if (!Array.isArray(configured)) {
    return {
      dependencies: [],
      errors: ["natesclaw.release.requireLatestDependencies must be an array of package names."],
    };
  }
  ...
  for (const value of configured) {
    if (typeof value !== "string" || !value.trim()) {
      errors.push(
        "natesclaw.release.requireLatestDependencies must contain only non-empty package names.",
      );
      continue;
    }
    ...
    if (seen.has(packageName)) {
      errors.push(
        `natesclaw.release.requireLatestDependencies must not contain duplicate package names; found "${packageName}".`,
      );
      continue;
    }
    ...
    if (typeof version !== "string" || !version.trim()) {
      errors.push(
        `natesclaw.release.requireLatestDependencies must reference package.json dependencies or optionalDependencies; "${packageName}" is not a runtime dependency.`,
      );
      continue;
    }
...
export function collectPublishablePluginPackageErrors(
  ...
  const installNpmSpec = normalizeOptionalString(packageJson.natesclaw?.install?.npmSpec);
  const repositoryUrl =
  ...
  if (repositoryUrl !== NATESCLAW_PLUGIN_NPM_REPOSITORY_URL) {
    errors.push(
      `package.json repository.url must be "${NATESCLAW_PLUGIN_NPM_REPOSITORY_URL}" so npm provenance can validate GitHub trusted publishing; found "${repositoryUrl || "<missing>"}".`,
    );
  }
  if (!packageVersion) {
  ...
  if (!Array.isArray(extensions) || extensions.length === 0) {
    errors.push("natesclaw.extensions must contain at least one entry.");
  }
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("natesclaw.extensions must contain only non-empty strings.");
  }
  if (!installNpmSpec) {
    errors.push("natesclaw.install.npmSpec must be a non-empty string for publishable plugins.");
  }
...
export function collectPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    if (packageJson.natesclaw?.release?.publishToNpm !== true) {
      continue;
    }
    ...
    publishable.push({
      ...
      installNpmSpec: normalizeOptionalString(packageJson.natesclaw?.install?.npmSpec),
...
function runNpmView(args: string[]): string {
  const tempDir = mkdtempSync(join(tmpdir(), "natesclaw-plugin-npm-view-"));

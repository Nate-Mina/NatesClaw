...
describe("Codex app-server binding store", () => {
  ...
  it("canonicalizes undefined fields before writing to JSON-only plugin state", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-codex-binding-state-"));
    try {
  ...
  it("reclaims a stale stable generation only for the current Natesclaw session", async () => {
    const { state, values } = createStateStore();
    const store = createCodexAppServerBindingStore(state);
    const previous = {
      kind: "session" as const,
  ...
  it("recovers a retired in-place generation through the authoritative session store", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-codex-reset-reclaim-"));

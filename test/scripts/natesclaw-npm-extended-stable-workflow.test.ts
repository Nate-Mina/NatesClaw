...
const workflowPath = ".github/workflows/natesclaw-npm-release.yml";
...
describe("minimal npm extended-stable workflow", () => {
  ...
  it("reuses the v1 preflight tarball and guards all three extended-stable gates", () => {
    ...
    expect(raw).toContain("natesclaw-npm-preflight-${{ inputs.tag }}");
    expect(raw.match(/natesclaw-npm-extended-stable-release\.mjs validate-request/g)).toHaveLength(
      3,
    );
    expect(step(parsed.jobs?.preflight_natesclaw_npm, "Validate npm release request").run).toContain(
      "natesclaw-npm-extended-stable-release.mjs validate-request",
    );
    expect(
      step(parsed.jobs?.preflight_natesclaw_npm, "Validate npm release request").env?.PREFLIGHT_ONLY,
    ).toBe("${{ inputs.preflight_only }}");
    expect(
      step(parsed.jobs?.validate_publish_request, "Validate npm release request").run,
    ).toContain("natesclaw-npm-extended-stable-release.mjs validate-request");
    expect(step(parsed.jobs?.publish_natesclaw_npm, "Recheck npm release request").run).toContain(
      "natesclaw-npm-extended-stable-release.mjs validate-request",
    );
    expect(
  ...
  it("threads an explicit, default-off extended-stable bypass through every policy gate", () => {
    ...
    const policySteps = [
      step(parsed.jobs?.preflight_natesclaw_npm, "Validate npm release request"),
      step(parsed.jobs?.validate_publish_request, "Validate npm release request"),
      step(parsed.jobs?.publish_natesclaw_npm, "Recheck npm release request"),
      step(parsed.jobs?.publish_natesclaw_npm, "Publish"),
    ];
    ...
    const summary = step(
      parsed.jobs?.publish_natesclaw_npm,
  ...
  it("lets main promote only the canonical immutable extended-stable candidate", () => {
    ...
    const recheck = step(parsed.jobs?.publish_natesclaw_npm, "Recheck npm release request");
  ...
  it("accepts arbitrary SHA preflight targets and exercises every publishable plugin package", () => {
    const parsed = workflow();
    const preflight = parsed.jobs?.preflight_natesclaw_npm;
    ...
    expect(step(preflight, "Verify release contents").env).toMatchObject({
      NATESCLAW_RELEASE_CHECK_LOCAL_PACKAGE_TARBALL_DIR:
    ...
    expect(plugins.run).toContain("NATESCLAW_PLUGIN_NPM_PACK_OUTPUT_DIR");
  ...
  it("restores same-SHA preflight build outputs and keeps validation steps running", () => {
    ...
    expect(build.env?.NATESCLAW_CONTROL_UI_RELEASE_BUILD).toBe("1");
  ...
  it("uses the trusted Full Validation evidence verifier", () => {
    ...
    expect(raw.match(/natesclaw-npm-extended-stable-release\.mjs verify-run/g)).toHaveLength(2);
    expect(raw).not.toContain("natesclaw-npm-extended-stable-release.mjs verify-manifest");
  });
  ...
  it("requires and authenticates the plugin npm run before an extended-stable core publish", () => {
    ...
    expect(verify.run).toContain("natesclaw-npm-extended-stable-release.mjs verify-run");
  });

  it("captures selector fail closed, publishes extended-stable, retries, and summarizes", () => {
    const parsed = workflow();
    const publish = parsed.jobs?.publish_natesclaw_npm;
    ...
    expect(capture.run).toContain("natesclaw-npm-extended-stable-release.mjs capture-selector");
    expect(step(publish, "Publish").run).toContain("natesclaw-npm-publish.sh");
    expect(readback.run).toContain("natesclaw-npm-extended-stable-release.mjs verify-readback");
    expect(summary.if).toContain("always()");
    expect(summary.run).toContain("natesclaw-npm-extended-stable-release.mjs repair-command");
  ...
  it("publishes only the tarball path verified from the preflight manifest", () => {
    const publish = workflow().jobs?.publish_natesclaw_npm;
  ...
  it("publishes gateway packages in manifest order before the root package", () => {
    ...
    const preflightPack = step(
      parsed.jobs?.preflight_natesclaw_npm,
      "Pack publishable core packages",
    );
    const publish = step(parsed.jobs?.publish_natesclaw_npm, "Publish");
    expect(preflightPack.env?.CORE_PACKAGE_DIRS).toBe(
      "packages/ai packages/gateway-protocol packages/gateway-client",
    );
    expect(readFileSync(workflowPath, "utf8")).toContain('packageName: "@natesclaw/gateway-client"');
    expect(publish.run).toContain("(.corePackageTarballs // [])[]");
    expect(publish.run).toContain(
      'bash scripts/natesclaw-npm-publish.sh --publish "${publish_target}"',
    );
  });
});

...
const PODMAN_QUADLET_TEMPLATE_PATH = "scripts/podman/natesclaw.container.in";
const PODMAN_RUN_PATH = "scripts/run-natesclaw-podman.sh";
...
const RELEASE_CHECKS_WORKFLOW_PATH = ".github/workflows/natesclaw-release-checks.yml";
const LIVE_E2E_WORKFLOW_PATH = ".github/workflows/natesclaw-live-and-e2e-checks-reusable.yml";
...
function runDockerTimezoneValidator(timezone: string) {
  const root = tempDirs.make("natesclaw-docker-timezone-");
...
function runInstallE2eInstallerFixture(params: {
  ...
  const root = tempDirs.make("natesclaw-install-e2e-download-");
  const binDir = join(root, "bin");
  ...
  delete env.NATESCLAW_BETA;
  delete env.NATESCLAW_VERSION;
...
function runNonrootInstallerFixture(curlExitCode: number) {
  const root = tempDirs.make("natesclaw-install-nonroot-download-");
  const binDir = join(root, "bin");
  const curlArgsPath = join(root, "curl-args.txt");
  const markerPath = join(root, "installer-marker.txt");
  const outputPathCapture = join(root, "curl-output-path.txt");
...
function normalizeInstallE2eAgentOutput(output: string) {
  const root = mkdtempSync(join(tmpdir(), "natesclaw-install-e2e-agent-output-"));
...
function validateInstallSmokeUpdateJson(doctorStep?: Record<string, unknown>) {
  const updateUrl = "http://candidate.invalid/natesclaw.tgz";
  const payload = {
    status: "ok",
...
function runInstallSmokeInstallerPipelineFixture(params: {
  ...
  const root = tempDirs.make("natesclaw-install-smoke-pipeline-");
  const binDir = join(root, "bin");
  const curlArgsPath = join(root, "curl-args.txt");
...
function runRestoreLocalDistFixture(
  ...
  const fixtureRoot = tempDirs.make("natesclaw-install-restore-root-");
  const imageRoot = tempDirs.make("natesclaw-install-restore-image-");
  ...
  if (options.symlinkEscape) {
    const escapeRoot = tempDirs.make("natesclaw-install-restore-escape-");
...
describe("test-install-sh-docker", () => {
  it("defaults ARM hosts to native arm64 while keeping x64 CI on amd64", () => {
    ...
    expect(
      runDefaultSmokePlatform({ NATESCLAW_INSTALL_SMOKE_PLATFORM: "linux/s390x" }, "x86_64"),
    ).toBe("linux/s390x");
  });
  ...
  it("supports npm update package specs without a separate expected-version env", () => {
    ...
    expect(script).toContain(
      'UPDATE_EXPECT_VERSION="${NATESCLAW_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"',
    );
  ...
  it("uses npm latest as the update baseline and resolves it to the concrete packed version", () => {
    ...
    expect(script).toContain(
      'UPDATE_BASELINE_VERSION="${NATESCLAW_INSTALL_SMOKE_UPDATE_BASELINE:-latest}"',
    );
    ...
    expect(runner).toContain(
      'UPDATE_BASELINE_VERSION="${NATESCLAW_INSTALL_UPDATE_BASELINE:-latest}"',
    );
    ...
    expect(workflow).toContain(
      "NATESCLAW_INSTALL_SMOKE_UPDATE_BASELINE: ${{ inputs.update_baseline_version || 'latest' }}",
    );
  });
  ...
  it("keeps shared install helpers parsing and verifying installed CLI versions", () => {
    const root = tempDirs.make("natesclaw-install-helper-");
    const binDir = join(root, "bin");
    mkdirSync(binDir, { recursive: true });
    writeFileSync(
      join(binDir, "natesclaw"),
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        'case "${1:-}" in',
        "  --version)",
        "    printf 'Natesclaw v2026.6.21-beta.1\\r\\n'",
        "    ;;",
        "  --help)",
        "    printf 'usage\\n'",
        "    ;;",
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          ...
          "printf 'parsed=%s\\n' \"$(extract_natesclaw_semver 'Natesclaw v2026.6.21-beta.1+build.7')\"",
          "verify_installed_cli natesclaw 2026.6.21-beta.1",
        ].join("\n"),
      ],
      {
        cwd: process.cwd(),
    ...
    expect(result.stdout).toContain(
      "cli=natesclaw installed=2026.6.21-beta.1 expected=2026.6.21-beta.1",
    );
  ...
  it("can reuse dist from the already-built root Docker smoke image", () => {
    ...
    expect(script).toContain('ROOT_DIR="${NATESCLAW_INSTALL_SMOKE_SOURCE_DIR:-$HARNESS_ROOT}"');
    expect(script).toContain('UPDATE_DIST_IMAGE="${NATESCLAW_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"');
    ...
    expect(script).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${NATESCLAW_INSTALL_SMOKE_DOCKER_COMMAND_TIMEOUT:-600s}}"',
    );
...
`,
  ...
  it("bounds installer smoke container runs", () => {
    ...
    expect(script).toContain(
      'INSTALL_SMOKE_DOCKER_RUN_TIMEOUT="${NATESCLAW_INSTALL_SMOKE_DOCKER_RUN_TIMEOUT:-2700s}"',
    );
  ...
  it("runs the root Dockerfile build with the CI heap limit", () => {
    ...
    expect(dockerfile).toContain(
      'ARG NATESCLAW_DOCKER_BUILD_NODE_OPTIONS="--max-old-space-size=8192"',
    );
    expect(dockerfile).toContain('ARG NATESCLAW_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB=""');
    expect(dockerfile).toContain("ARG NATESCLAW_DOCKER_BUILD_SKIP_DTS=1");
    expect(dockerfile).toContain(
      'NATESCLAW_RUN_NODE_SKIP_DTS_BUILD="$NATESCLAW_DOCKER_BUILD_SKIP_DTS" NATESCLAW_TSDOWN_MAX_OLD_SPACE_MB="$NATESCLAW_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB" NODE_OPTIONS="$NATESCLAW_DOCKER_BUILD_NODE_OPTIONS" pnpm_config_verify_deps_before_run=false pnpm build:docker',
    );
  });
  ...
  it("passes the baked browser build arg through Docker setup", () => {
    ...
    expect(script).toContain('export NATESCLAW_INSTALL_BROWSER="${NATESCLAW_INSTALL_BROWSER:-}"');
    expect(script).toContain("NATESCLAW_INSTALL_BROWSER \\");
    expect(script).toContain('--build-arg "NATESCLAW_INSTALL_BROWSER=${NATESCLAW_INSTALL_BROWSER}"');
  });
  ...
  it("bounds Docker setup image pulls", () => {
    ...
    expect(script).toContain('DOCKER_PULL_TIMEOUT="${NATESCLAW_DOCKER_SETUP_PULL_TIMEOUT:-600s}"');
    expect(script).toContain("run_docker_pull()");
    expect(script).toContain(
      'natesclaw_host_timeout_cmd "$DOCKER_PULL_TIMEOUT" docker pull "$image"',
    );
  ...
  it("validates Docker timezones against the selected image runtime", () => {
    ...
    expect(script).toContain(
      'fail "NATESCLAW_TZ must be supported by $IMAGE_NAME (e.g. Asia/Shanghai)."',
    );
  });
  ...
  it("bounds Podman setup image pulls", () => {
    ...
    expect(script).toContain('PODMAN_PULL_TIMEOUT="${NATESCLAW_PODMAN_SETUP_PULL_TIMEOUT:-600s}"');
    expect(script).toContain("run_podman_pull()");
    expect(script).toContain(
      'natesclaw_host_timeout_cmd "$PODMAN_PULL_TIMEOUT" podman pull "$image"',
    );
    expect(script).toContain('run_podman_pull "$NATESCLAW_IMAGE"');
    expect(script).not.toContain('podman pull "$NATESCLAW_IMAGE"');
  });
  ...
  it("bounds Podman setup image builds", () => {
    ...
    expect(script).toContain(
      'PODMAN_BUILD_TIMEOUT="${NATESCLAW_PODMAN_SETUP_BUILD_TIMEOUT:-1800s}"',
    );
    expect(script).toContain("run_podman_build()");
    expect(script).toContain('natesclaw_host_timeout_cmd "$PODMAN_BUILD_TIMEOUT" podman build "$@"');
    expect(script).toContain('run_podman_build -t "$NATESCLAW_IMAGE"');
    expect(script).not.toContain('podman build -t "$NATESCLAW_IMAGE"');
  });

  it("bounds detached Podman launches without timing out onboarding", () => {
    const script = readFileSync(PODMAN_RUN_PATH, "utf8");

    expect(script).toContain('PODMAN_RUN_TIMEOUT="${NATESCLAW_PODMAN_RUN_TIMEOUT:-600s}"');
    expect(script).toContain("NATESCLAW_PODMAN_RUN_TIMEOUT|NATESCLAW_PODMAN_GATEWAY_HOST_PORT");
    expect(script).toContain('source "$SCRIPT_DIR/lib/host-timeout.sh"');
    expect(script).toContain("run_podman_detached()");
    expect(script).toContain('natesclaw_host_timeout_cmd "$PODMAN_RUN_TIMEOUT" podman run "$@"');
  ...
  it("passes image-scoped pip packages through Docker and Podman setup", () => {
    ...
    expect(dockerfile).toContain("ARG NATESCLAW_IMAGE_PIP_PACKAGES");
    expect(dockerfile).toContain(
      "python3 -m pip install --no-cache-dir --break-system-packages $NATESCLAW_IMAGE_PIP_PACKAGES",
    );
    expect(dockerSetup).toContain(
      'export NATESCLAW_IMAGE_PIP_PACKAGES="${NATESCLAW_IMAGE_PIP_PACKAGES:-}"',
    );
    expect(dockerSetup).toContain("NATESCLAW_IMAGE_PIP_PACKAGES \\");
    expect(dockerSetup).toContain(
      '--build-arg "NATESCLAW_IMAGE_PIP_PACKAGES=${NATESCLAW_IMAGE_PIP_PACKAGES}"',
    );
    expect(dockerSetup).not.toContain("NATESCLAW_DOCKER_PIP_PACKAGES");
    expect(podmanSetup).toContain('NATESCLAW_IMAGE_PIP_PACKAGES="${NATESCLAW_IMAGE_PIP_PACKAGES:-}"');
    expect(podmanSetup).toContain(
      'BUILD_ARGS+=(--build-arg "NATESCLAW_IMAGE_PIP_PACKAGES=${NATESCLAW_IMAGE_PIP_PACKAGES}")',
    );
    expect(podmanSetup).not.toContain("NATESCLAW_DOCKER_PIP_PACKAGES");
  });
  ...
  it("passes one source identity into local Docker and Podman builds", () => {
    ...
    for (const setupScript of [dockerSetup, podmanSetup]) {
      ...
      expect(setupScript).toContain("natesclaw_resolve_git_commit");
      expect(setupScript).toContain("natesclaw_resolve_build_timestamp");
      expect(setupScript).toContain("NATESCLAW_BUILD_TIMESTAMP=${BUILD_TIMESTAMP}");
  ...
  it("keeps the Podman Quadlet template aligned with setup substitutions", () => {
    ...
    expect(setupScript).toContain(
      'QUADLET_TEMPLATE="$REPO_PATH/scripts/podman/natesclaw.container.in"',
    );
    for (const placeholder of [
      "NATESCLAW_CONFIG_DIR",
    ...
    expect(template).toContain("Volume={{NATESCLAW_CONFIG_DIR}}:/home/node/.natesclaw:Z");
    expect(template).toContain(
      "Volume={{NATESCLAW_WORKSPACE_DIR}}:/home/node/.natesclaw/workspace:Z",
    );
    expect(template).toContain("EnvironmentFile={{NATESCLAW_CONFIG_DIR}}/.env");
  ...
  it("allows repository branch history and release tags for secret-backed Docker release checks", () => {
    ...
    expect(workflow).toContain("reachable from an Natesclaw branch or release tag");
  });
  ...
  it("rejects path-like npm pack tarball filenames in update smoke metadata", () => {
    expect(runReadPackTarballFilename("natesclaw-2026.6.17.tgz")).toMatchObject({
      status: 0,
      stdout: "natesclaw-2026.6.17.tgz",
    });

    const unsafeFilenames = [
      "../natesclaw.tgz",
      "nested/natesclaw.tgz",
      "nested\\natesclaw.tgz",
      "/tmp/natesclaw.tgz",
      "C:\\temp\\natesclaw.tgz",
      "natesclaw.tar.gz",
  ...
  it("uses the package artifact helper for local update tarballs", () => {
    ...
    expect(script).toContain('node "$HARNESS_ROOT/scripts/package-natesclaw-for-docker.mjs"');
    expect(script).toContain("--allow-unreleased-changelog");
    expect(script).toContain("NATESCLAW_INSTALL_SMOKE_ALLOW_UNRELEASED_CHANGELOG");
    expect(script).toContain(
      'if [[ "${NATESCLAW_INSTALL_SMOKE_ALLOW_UNRELEASED_CHANGELOG:-true}" == "true" ]]',
    );
    ...
    expect(script).toContain('node "$HARNESS_ROOT/scripts/check-natesclaw-package-tarball.mjs"');
  ...
  it("runs candidate tarballs through the installer script instead of direct npm", () => {
    ...
    expect(wrapper).toContain('-v "$ROOT_DIR/scripts/install.sh:/tmp/natesclaw-install.sh:ro"');
  ...
  it("bounds both non-root installer pipelines and propagates curl failures", () => {
    ...
    expect(wrapper).toContain('-e NATESCLAW_INSTALL_CLI_URL="$CLI_INSTALL_URL"');
    expect(wrapper).toContain(
      `'set -o pipefail; curl -fsSL --connect-timeout 30 --max-time 300 -- "$NATESCLAW_INSTALL_CLI_URL" | bash -s -- --set-npm-prefix --no-onboard'`,
    );
  ...
  it("uses public npm latest as the non-root installer expectation", () => {
    ...
    expect(wrapper).toContain('-e NATESCLAW_INSTALL_EXPECT_VERSION="$LATEST_VERSION"');
  });
});
...
describe("install-sh E2E runner", () => {
  ...
  it.each([
    ...
    (installTag, expected) => {
      const fixture = runInstallE2eInstallerFixture({
        installTag,
        installerBody:
          'printf "%s|%s" "${NATESCLAW_BETA-}" "${NATESCLAW_VERSION-}" >"$INSTALL_MARKER"\n',
      });
  ...
  it("normalizes Docker wrapper timing and toggle knobs before forwarding", () => {
    ...
    expect(wrapper).toContain(
      'SESSION_SCAN_BYTES="$(\n  docker_e2e_read_positive_int_env NATESCLAW_INSTALL_E2E_SESSION_SCAN_BYTES 16777216\n)"',
    );
    expect(wrapper).toContain(
      'SESSION_LINE_BYTES="$(\n  docker_e2e_read_positive_int_env NATESCLAW_INSTALL_E2E_SESSION_LINE_BYTES 1048576\n)"',
    );
    expect(wrapper).toContain(
      'SESSION_SCAN_DEPTH="$(docker_e2e_read_positive_int_env NATESCLAW_INSTALL_E2E_SESSION_SCAN_DEPTH 64)"',
    );
    expect(wrapper).toContain(
      'SESSION_SCAN_NODES="$(docker_e2e_read_positive_int_env NATESCLAW_INSTALL_E2E_SESSION_SCAN_NODES 100000)"',
    );
    expect(wrapper).toContain(
      '-e NATESCLAW_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS="$OPENAI_PROVIDER_TIMEOUT_SECONDS"',
    );
    expect(wrapper).toContain(
      '-e NATESCLAW_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS="$AGENT_TURN_TIMEOUT_SECONDS"',
    );
    expect(wrapper).toContain(
      '-e NATESCLAW_INSTALL_E2E_AGENT_TURNS_PARALLEL="$AGENT_TURNS_PARALLEL"',
    );
    expect(wrapper).toContain('-e NATESCLAW_INSTALL_E2E_AGENT_TOOL_SMOKE="$AGENT_TOOL_SMOKE"');
    expect(wrapper).toContain('-e NATESCLAW_INSTALL_E2E_SESSION_SCAN_BYTES="$SESSION_SCAN_BYTES"');
    expect(wrapper).toContain('-e NATESCLAW_INSTALL_E2E_SESSION_LINE_BYTES="$SESSION_LINE_BYTES"');
    expect(wrapper).toContain('-e NATESCLAW_INSTALL_E2E_SESSION_SCAN_DEPTH="$SESSION_SCAN_DEPTH"');
    expect(wrapper).toContain('-e NATESCLAW_INSTALL_E2E_SESSION_SCAN_NODES="$SESSION_SCAN_NODES"');
    expect(wrapper).not.toContain(
      'NATESCLAW_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS="${NATESCLAW_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS:-}"',
    );
  });

  it.each([
    ["turn timeout", "NATESCLAW_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS", "300s"],
    ["provider timeout", "NATESCLAW_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS", "1e3"],
    ["parallel toggle", "NATESCLAW_INSTALL_E2E_AGENT_TURNS_PARALLEL", "2"],
    ["tool smoke toggle", "NATESCLAW_INSTALL_E2E_AGENT_TOOL_SMOKE", "false"],
    ["session scan bytes", "NATESCLAW_INSTALL_E2E_SESSION_SCAN_BYTES", "16mb"],
    ["session line bytes", "NATESCLAW_INSTALL_E2E_SESSION_LINE_BYTES", "1mb"],
    ["session scan depth", "NATESCLAW_INSTALL_E2E_SESSION_SCAN_DEPTH", "0"],
    ["session scan nodes", "NATESCLAW_INSTALL_E2E_SESSION_SCAN_NODES", "100k"],
  ...
  it("validates agent timing and toggle knobs before running provider setup", () => {
    ...
    expect(script).toContain(
      'OPENAI_PROVIDER_TIMEOUT_SECONDS="$(read_positive_int_env NATESCLAW_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS "$AGENT_TURN_TIMEOUT_SECONDS")"',
    );
  ...
  it.each([
    ...
    expect(result.status).toBe(2);
    expect(result.stderr).toContain(`invalid ${envName}: ${value}`);
    expect(result.stderr).not.toContain("NATESCLAW_E2E_MODELS=both requires");
  });
});
...
describe("install-sh smoke runner", () => {
  it("passes the URL and installer arguments through the timed pipeline unchanged", () => {
    const installerArgs = [
      ...
      "npm",
      "--version",
      "https://packages.example.test/natesclaw.tgz?x=1&y=2",
      "--no-prompt",
    ];
  ...
  it("wraps long npm/update operations with heartbeat and install-size audits", () => {
    ...
    expect(script).toContain(
      'HEARTBEAT_INTERVAL="$(read_nonnegative_int_env NATESCLAW_INSTALL_SMOKE_HEARTBEAT_INTERVAL 60)"',
    );
    expect(script).toContain(
      'INSTALL_COMMAND_TIMEOUT="$(read_positive_int_env NATESCLAW_INSTALL_SMOKE_COMMAND_TIMEOUT 900)"',
    );
    ...
    expect(script).toContain("natesclaw update --tag");
    ...
    expect(script).toContain("natesclaw infer image providers --json");
  });

  it.each([
    ["successful", { name: "natesclaw doctor", exitCode: 0 }],
    [
      "recoverable advisory",
      {
        name: "natesclaw doctor",
        exitCode: 86,
  ...
  it.each([
    ["missing", undefined, "missing natesclaw doctor step"],
    ["fatal", { name: "natesclaw doctor", exitCode: 1 }, "natesclaw doctor step failed"],
    ["untyped advisory", { name: "natesclaw doctor", exitCode: 86 }, "natesclaw doctor step failed"],
    [
      "wrong advisory kind",
      { name: "natesclaw doctor", exitCode: 86, advisory: { kind: "other" } },
      "natesclaw doctor step failed",
    ],
    [
      "wrong advisory exit",
      {
  ...
  it.each([
    ["command timeout", "NATESCLAW_INSTALL_SMOKE_COMMAND_TIMEOUT", "900s"],
    ["heartbeat interval", "NATESCLAW_INSTALL_SMOKE_HEARTBEAT_INTERVAL", "60s"],
    ...
    expect(result.status).toBe(2);
    expect(result.stderr).toContain(`invalid ${envName}: ${value}`);
    expect(result.stderr).not.toContain("unsupported NATESCLAW_INSTALL_SMOKE_MODE");
  });
  ...
  it("covers plain npm global installs and npm-driven updates", () => {
    ...
    expect(script).toContain('SKIP_NPM_GLOBAL="${NATESCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"');
    expect(script).toContain('NPM_CACHE_DIR="${NATESCLAW_INSTALL_SMOKE_NPM_CACHE_DIR:-}"');
    ...
    expect(script).toContain("NATESCLAW_INSTALL_SMOKE_MODE=npm-global");
  ...
  it("forwards smoke-runner control knobs into Docker containers", () => {
    ...
    for (const envName of [
      "NATESCLAW_INSTALL_ALLOW_LEGACY_UPDATE_WARNING",
      "NATESCLAW_INSTALL_SELF_UPDATE_WARNING_FIXED_VERSION",
      "NATESCLAW_INSTALL_SMOKE_COMMAND_TIMEOUT",
      "NATESCLAW_INSTALL_SMOKE_HEARTBEAT_INTERVAL",
      "NATESCLAW_INSTALL_SMOKE_PREVIOUS",
      "NATESCLAW_INSTALL_SMOKE_SKIP_PREVIOUS",
    ]) {
...
describe("bun global install smoke", () => {
  it("packs the current tree and verifies image-provider discovery through Bun", () => {
    ...
    expect(script).toContain("node scripts/package-natesclaw-for-docker.mjs");
    expect(script).toContain("--allow-unreleased-changelog");
    expect(script).toContain("NATESCLAW_BUN_GLOBAL_SMOKE_ALLOW_UNRELEASED_CHANGELOG");
    expect(script).toContain(
      'if [[ "${NATESCLAW_BUN_GLOBAL_SMOKE_ALLOW_UNRELEASED_CHANGELOG:-true}" == "true" ]]',
    );
    expect(script).toContain("package_args+=(--allow-unreleased-changelog)");
    expect(script).toContain("--skip-build");
    expect(script).toContain("--output-name natesclaw-current.tgz");
    ...
    expect(script).toContain('"$natesclaw_bin" --help');
    expect(script).toContain("NATESCLAW_BUN_GLOBAL_SMOKE_PROOF_PATH");
    ...
    expect(script).toContain("NATESCLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE");
    expect(script).toContain('source "$ROOT_DIR/scripts/lib/docker-e2e-package.sh"');
    expect(script).toContain("docker_e2e_restore_package_dist_from_image");
    expect(script).toContain(
      'COMMAND_TIMEOUT_MS="$(read_positive_int_env NATESCLAW_BUN_GLOBAL_SMOKE_TIMEOUT_MS 180000)"',
    );
    expect(script).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${NATESCLAW_BUN_GLOBAL_SMOKE_DOCKER_COMMAND_TIMEOUT:-600s}}"',
    );
    expect(packageHelper).toContain('container_id="$(docker_e2e_docker_cmd create "$image")"');
    expect(packageHelper).toContain(
      'docker_e2e_docker_cmd cp "${container_id}:/app/dist" "$temp_dir/dist"',
  ...
  it("rejects invalid Bun global install command timeouts before Bun setup", () => {
    const result = spawnSync("bash", [BUN_GLOBAL_SMOKE_PATH], {
      ...
      env: {
        ...process.env,
        NATESCLAW_BUN_GLOBAL_SMOKE_TIMEOUT_MS: "180000ms",
      },
    });

    expect(result.status).toBe(2);
    expect(result.stderr).toContain("invalid NATESCLAW_BUN_GLOBAL_SMOKE_TIMEOUT_MS: 180000ms");
  ...
  it("requires root and AI candidate versions to match", () => {
    const tempDir = tempDirs.make("natesclaw-bun-candidate-versions-");
    const rootManifestPath = join(tempDir, "natesclaw.json");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const tempDir = tempDirs.make("natesclaw-bun-prebuilt-");
      const packageDir = join(tempDir, "fixture", "package");
      const aiDir = join(packageDir, "node_modules", "@natesclaw", "ai");
      const packageTgz = join(tempDir, "natesclaw-prebuilt.tgz");
...
cat >"$BUN_INSTALL/bin/natesclaw" <<'NATESCLAW'
#!/usr/bin/env bash
if [ "\${1:-}" = "--version" ]; then
  echo "Natesclaw 2026.6.17"
else
  printf '[{"id":"google"},{"id":"openai"},{"id":"xai"}]\n'
fi
NATESCLAW
chmod +x "$BUN_INSTALL/bin/natesclaw"
`,
      );
      ...
      const result = spawnSync("bash", [BUN_GLOBAL_SMOKE_PATH], {
        ...
        env: {
          ...
          NATESCLAW_BUN_GLOBAL_SMOKE_PACKAGE_TGZ: packageTgz,
          NATESCLAW_BUN_GLOBAL_SMOKE_TIMEOUT_MS: "10000",
        },
      });
  ...
  it.runIf(process.platform !== "win32" && existsSync("/usr/bin/time"))(
    ...
    () => {
      const tempDir = tempDirs.make("natesclaw-bun-global-timeout-grace-");
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            NATESCLAW_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS: "1000",
          },
          timeout: 5_000,
        },
      );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = tempDirs.make("natesclaw-bun-global-parent-signal-");
      const readyPath = path.join(tempDir, "ready");
      ...
      const runner = spawn(
        ...
        ],
        {
          env: {
            ...process.env,
            NATESCLAW_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS: "100",
          },
  ...
  it("gates workflow Bun install smoke to scheduled and release-check runs", () => {
    ...
    expect(workflow).toContain(
      "NATESCLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE: ${{ needs.root_dockerfile_image.outputs.image_ref }}",
    );
    ...
    expect(workflow).toContain("NATESCLAW_CI_WORKFLOW_BUN_GLOBAL_INSTALL_SMOKE");
    expect(workflow).toContain('run_bun_global_install_smoke="$workflow_bun_global_install_smoke"');
    expect(workflow).not.toContain("NATESCLAW_CI_EVENT_NAME");
    ...
    expect(workflow).toContain("NATESCLAW_INSTALL_URL: file:///tmp/natesclaw-install.sh");
    expect(workflow).toContain("NATESCLAW_INSTALL_CLI_URL: file:///tmp/natesclaw-install-cli.sh");
    expect(workflow).toContain('NATESCLAW_INSTALL_SMOKE_SKIP_CLI: "0"');
    ...
    expect(workflow).toContain("bash /tmp/install-cli.sh --prefix /tmp/natesclaw-cli");
    ...
    expect(workflow).toContain('NATESCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL: "1"');
  ...
  it("runs installer packaging from the trusted workflow revision against a nested candidate", () => {
    ...
    expect(step("Run installer docker tests").env).toMatchObject({
      NATESCLAW_INSTALL_SMOKE_ALLOW_UNRELEASED_CHANGELOG: "${{ inputs.allow_unreleased_changelog }}",
      NATESCLAW_INSTALL_SMOKE_SOURCE_DIR: "${{ github.workspace }}/candidate",
    });
  ...
  it("kills Bun global install smoke commands that ignore TERM after timeout", () => {
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          NATESCLAW_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS: "50",
        },
        timeout: 5000,
      },
    );

...
describe.skipIf(process.platform === "win32")("shell snapshot subprocesses", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["HOME", "NATESCLAW_EXEC_SHELL_SNAPSHOT", "NATESCLAW_STATE_DIR"]);
    spawnMock.mockReset();
  ...
  it("does not create output pipes for status-only shell commands", async () => {
    ...
    const home = tempDirs.make("natesclaw-snapshot-spawn-home-");
    const stateDir = tempDirs.make("natesclaw-snapshot-spawn-state-");

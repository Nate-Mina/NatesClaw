...
export function createGatewayStartupTrace(log: GatewayLogger) {
  const logEnabled = isTruthyEnvValue(process.env.NATESCLAW_GATEWAY_STARTUP_TRACE);
  let timelineConfig: NatesclawConfig | undefined;
  ...
  const eventLoopTimelineEnabled = () =>
    ...
    isTruthyEnvValue(process.env.NATESCLAW_DIAGNOSTICS_EVENT_LOOP);

...
read_when:
  - Setting up Zalo Personal for Natesclaw
...
2. Login (QR, on the Gateway machine):
   - `natesclaw channels login --channel zalouser`
...
```bash
natesclaw directory self --channel zalouser
natesclaw directory peers list --channel zalouser --query "name"
natesclaw directory groups list --channel zalouser --query "work"
```
...
Natesclaw stores each raw `zca-js` message callback before processing it. Pending messages resume from the account queue after a Gateway restart, and processing stays serialized per direct chat or group.

The `zca-js` socket listener does not expose a delivery acknowledgement or automatically replay old messages after reconnect. The durable queue therefore protects the local crash window after a callback reaches Natesclaw; it cannot recover a message the socket never delivered. Replay tombstones are mostly a safeguard against a repeated callback with the same Zalo message id.
...
- `natesclaw pairing list zalouser`
- `natesclaw pairing approve zalouser <code>`
...
<Note>
`channels.zalouser.groups.<id>.allow` is a legacy field name; current config uses `enabled`. `natesclaw doctor --fix` migrates `allow` to `enabled` automatically.
</Note>
...
- When a group message is skipped because a mention is required, Natesclaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Natesclaw state. Example:
...
Profile names select the saved Zalo login credentials in Natesclaw state. Resolution order:
...
- Natesclaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Natesclaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `natesclaw channels logout --channel zalouser && natesclaw channels login --channel zalouser`

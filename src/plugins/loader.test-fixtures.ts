...
export { loadNatesclawPlugins };

export type TempPlugin = { dir: string; file: string; id: string };
export type PluginLoadConfig = NonNullable<Parameters<typeof loadNatesclawPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadNatesclawPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "natesclaw-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.NATESCLAW_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.NATESCLAW_DISABLE_BUNDLED_PLUGINS;
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".natesclaw", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ NATESCLAW_STATE_DIR: stateDir, ...params.env }, () =>
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.NATESCLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.NATESCLAW_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.NATESCLAW_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}

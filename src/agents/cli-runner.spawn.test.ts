...
async function createCliPackageFixture(version: string): Promise<{
  ...
  const root = tempDirs.make("natesclaw-cli-version-gate-");
...
describe("runCliAgent spawn path", () => {
  ...
  it("streams a node-placed Claude resume through the normal JSONL parser", async () => {
    ...
    context.NatesclawHistoryPrompt = "gateway transcript reseed";
    ...
    // not exist on the node so the Natesclaw list is projected empty.
    expect(toolAvailability).toEqual({ native: [], Natesclaw: [], mcp: [] });
  ...
  it("includes the Natesclaw skills prompt in CLI system prompts", () => {
  ...
  it("passes Claude system prompts through a file instead of argv", async () => {
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(systemPromptPath).toContain("natesclaw-cli-system-prompt-");
  ...
  it("resends system prompts through a file for soft-resumed prompt-tool drift", async () => {
    const writeSoftResumeSystemPromptFile = vi.fn(async () => ({
      filePath: "/tmp/natesclaw-soft-resume-system-prompt.md",
      cleanup: async () => {},
    }));
    setCliRunnerExecuteTestDeps({
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(input.argv?.join(" ")).toContain("/tmp/natesclaw-soft-resume-system-prompt.md");
      return createManagedRun({
        reason: "exit",
        exitCode: 0,
        exitSignal: null,
  ...
  it("does not require an argv rewrite after prepared-execution enforcement", async () => {
    ...
    await executePreparedCliRun(
      buildPreparedCliRunContext({
        ...
        cliToolAvailability: { native: [], Natesclaw: ["natesclaw"] },
  ...
  it("passes prepared backend env to the spawned CLI process", async () => {
    ...
    await executePreparedCliRun(
      buildPreparedCliRunContext({
        ...
        preparedEnv: {
          GEMINI_CLI_HOME: "/tmp/natesclaw-gemini-profile-home",
          GEMINI_CLI_SYSTEM_SETTINGS_PATH: "/tmp/natesclaw-gemini-system-settings.json",
        },
      }),
    );
    ...
    expect(input.env?.GEMINI_CLI_HOME).toBe("/tmp/natesclaw-gemini-profile-home");
    expect(input.env?.GEMINI_CLI_SYSTEM_SETTINGS_PATH).toBe(
      "/tmp/natesclaw-gemini-system-settings.json",
    );
  });

  it("captures a runtime artifact for a strict CLI credential", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-cli-strict-artifact-"));
  ...
  it("passes Natesclaw skills to Claude as a session plugin", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-cli-skills-"));
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(manifest.name).toBe("natesclaw-skills");
  ...
  it("keeps captured live prepared backend cleanup with the whole-run owner", async () => {
    const mcpConfigDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "natesclaw-cli-captured-mcp-config-"),
    );
  ...
  it.each([
    ...
    if (testCase.preserve) {
      process.env.NATESCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV = JSON.stringify(testCase.preserve);
    }
    try {
      mockSuccessfulCliRun();
      await executePreparedCliRun(
      ...
      delete process.env.NATESCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV;
  ...
  it("keeps selected Claude auth authoritative over ambient and configured credentials", async () => {
    vi.stubEnv("NATESCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV", '["ANTHROPIC_API_KEY"]');
  ...
  it("loads workspace bootstrap files into the Claude CLI system prompt", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "natesclaw-cli-bootstrap-context-"),
    );

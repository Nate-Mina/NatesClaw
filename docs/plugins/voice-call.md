...
read_when:
  - You want to place an outbound voice call from Natesclaw
...
Voice calls for Natesclaw via a plugin: outbound notifications, multi-turn
...
<Steps>
  <Step title="Install the plugin">
    <Tabs>
      ...
      <Tab title="From a local folder (dev)">
        ...
        natesclaw plugins install "$PLUGIN_SRC"
  ...
  <Step title="Verify setup">
    ...
    natesclaw voicecall setup --json
    ```
...
| `store`                         | unset        | Overrides the default `$NATESCLAW_STATE_DIR/voice-calls` path (normally `~/.natesclaw/voice-calls`). |
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    release (`2026.6.0`), so run `natesclaw doctor --fix` to rewrite committed
...
- Voice Call exposes the shared `natesclaw_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal Natesclaw tools.
- `realtime.consultPolicy` optionally adds guidance for when the realtime model should call `natesclaw_agent_consult`.
...
configured Natesclaw agent without paying a full agent-consult round trip on
...
`natesclaw_agent_consult` still run the full Natesclaw agent and should be used
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call natesclaw_agent_consult before using deeper tools.",
                toolPolicy: "safe-read-only",
                consultPolicy: "substantive",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `natesclaw doctor --fix`; committed config should use `tts.providers.<provider>`.
...
```bash
natesclaw voicecall call --to "+15555550123" --message "Hello from Natesclaw"
natesclaw voicecall start --to "+15555550123"   # alias for call
natesclaw voicecall continue --call-id <id> --message "Any questions?"
natesclaw voicecall speak --call-id <id> --message "One moment"
natesclaw voicecall dtmf --call-id <id> --digits "ww123456#"
natesclaw voicecall end --call-id <id>
natesclaw voicecall status --call-id <id>
natesclaw voicecall tail
natesclaw voicecall latency                      # summarize turn latency from logs
natesclaw voicecall expose --mode funnel
```
...
Without `publicUrl`, Natesclaw reconstructs the URL from the request. Telnyx
...
Use `natesclaw logs --follow` for the live phase trace. A healthy Twilio Meet
...
`natesclaw voicecall tail` still shows persisted call records; useful for
...
- `natesclaw logs --follow` shows realtime TwiML served, the realtime bridge started, and the initial greeting queued.

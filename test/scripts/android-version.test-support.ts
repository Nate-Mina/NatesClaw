...
export function writeAndroidFixture(params: {
  ...
  const rootDir = makeTempDir(tempDirs, params.prefix ?? "natesclaw-android-version-");
  ...
  const releaseNotes =
    "Natesclaw is now available on Android.\n\nConnect to your Natesclaw Gateway.\n";
  fs.writeFileSync(
    path.join(rootDir, "apps", "android", "CHANGELOG.md"),
    params.changelog ?? `# Natesclaw Android Changelog\n\n## Unreleased\n\n${releaseNotes}`,
    "utf8",
  );
  fs.writeFileSync(

...
/**
 * Runtime bridge between MCP tools and the Natesclaw Gateway channel APIs.
 *
...
export class NatesclawChannelBridge {
  ...
  constructor(
    private readonly cfg: NatesclawConfig,
  ...
  async start(): Promise<void> {
    ...
    this.gateway = new GatewayClientCtor({
      ...
      clientDisplayName: "Natesclaw MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`natesclaw mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
  ...
  private async sendNotification(notification: ServerNotification): Promise<void> {
    ...
    try {
      ...
      process.stderr.write(`natesclaw mcp: notification ${notification.method} failed\n`);
      if (this.verbose) {
        process.stderr.write(
          `natesclaw mcp: notification ${notification.method} error: ${String(error)}\n`,
        );
      }
    }
  }
  ...
  private async dispatchGatewayEvent(event: EventFrame): Promise<void> {
    try {
      ...
      process.stderr.write(`natesclaw mcp: gateway event ${event.event} failed\n`);
      if (this.verbose) {
        process.stderr.write(
          `natesclaw mcp: gateway event ${event.event} error: ${String(error)}\n`,
        );
      }
    }
  }

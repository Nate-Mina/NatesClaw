...
title: "Mantis"
read_when:
  - Building or running live visual QA for Natesclaw bugs
...
Mantis publishes visual CI evidence and a PR comment for Natesclaw behavior.
...
- Natesclaw (`extensions/qa-lab/src/mantis/*`): scenario runtime, `pnpm natesclaw qa mantis <command>` CLI, evidence schema.
- QA Lab (`extensions/qa-lab/src/live-transports/*`): live transport harness, driver/SUT bots, report/evidence writers.
- Crabbox (`natesclaw/crabbox`): warmed Linux machines, leases, VNC, `crabbox media preview`.
...
All commands are `pnpm natesclaw qa mantis <command>`, defined in
`extensions/qa-lab/src/mantis/cli.ts`. Requires `NATESCLAW_ENABLE_PRIVATE_QA_CLI=1`
at build/run time (bundled workflows set `NATESCLAW_BUILD_PRIVATE_QA=1` and
`NATESCLAW_ENABLE_PRIVATE_QA_CLI=1` before building).
...
| `telegram-desktop-builder`      | Lease/reuse a Crabbox desktop, install Telegram Desktop, optionally configure an Natesclaw gateway.                                                        |
...
```bash
pnpm natesclaw qa mantis discord-smoke \
...
Token resolution order: `--token-file` value, then `NATESCLAW_QA_DISCORD_MANTIS_BOT_TOKEN`
(override with `--token-env`), then a file named by `NATESCLAW_QA_DISCORD_MANTIS_BOT_TOKEN_FILE`
(override with `--token-file-env`). Guild/channel ids come from
`NATESCLAW_QA_DISCORD_GUILD_ID` / `NATESCLAW_QA_DISCORD_CHANNEL_ID` (override with
`--guild-id` / `--channel-id`) and must be 17-20 digit Discord snowflakes. Set
`NATESCLAW_QA_REDACT_PUBLIC_METADATA=1` to replace bot/guild/channel/message ids
and names with `<redacted>` in the published summary and report.

### `run`

```bash
pnpm natesclaw qa mantis run \
  --transport discord \
...
`pnpm natesclaw qa discord --scenario <id> --model openai/gpt-5.4 --alt-model openai/gpt-5.4 --allow-failures`
...
```bash
pnpm natesclaw qa mantis desktop-browser-smoke \
...
- `--browser-profile-archive-env <name>` restores a base64 `.tgz` Chrome profile archive from that env var before launch (default `NATESCLAW_MANTIS_BROWSER_PROFILE_TGZ_B64`); used for logged-in witnesses like Discord Web.
- `--video-duration <seconds>` controls MP4 capture length (default 10s).
- `--keep-lease` (or `NATESCLAW_MANTIS_KEEP_VM=1`) keeps a lease this run created open for VNC inspection; failed runs that created a lease also keep it by default.
...
`NATESCLAW_QA_DISCORD_CAPTURE_UI_METADATA=1` is set, the scenario also writes a
Discord Web URL artifact, and `NATESCLAW_QA_DISCORD_KEEP_THREADS=1` leaves the
...
`pnpm natesclaw qa slack` inside it, opens Slack Web in the VNC browser,
...
With `--gateway-setup`, the command creates a persistent disposable Natesclaw
home at `$HOME/.natesclaw-mantis/slack-natesclaw` in the VM, patches Slack
Socket Mode config for the target channel, starts
`natesclaw gateway run --dev --allow-unconfigured --port 38973`, and leaves
...
- `NATESCLAW_LIVE_OPENAI_KEY` for the remote model lane (if only `OPENAI_API_KEY`
  is set locally, Mantis copies it to `NATESCLAW_LIVE_OPENAI_KEY` before
  invoking Crabbox)
...
bot token into the VM as `NATESCLAW_MANTIS_SLACK_*` env vars, so GitHub
...
`NATESCLAW_MANTIS_SLACK_BROWSER_PROFILE_DIR` controls the persistent Chrome
profile inside the VM (default `$HOME/.config/natesclaw-mantis/slack-chrome-profile`);
...
optionally restores a user-session archive, configures Natesclaw with the
leased Telegram SUT bot token, starts
`natesclaw gateway run --dev --allow-unconfigured --port 38974`, posts a
driver-bot readiness message to the leased private group, then captures a
screenshot and MP4. A bot token only configures Natesclaw; it never logs
...
`NATESCLAW_QA_REDACT_PUBLIC_METADATA=1` for public artifact uploads; it is
...
| `Mantis Telegram Live`            | PR comment or manual dispatch                                                              | Runs the bot-API Telegram live QA lane (`natesclaw qa telegram`), writes `mantis-evidence.json` from the QA summary, renders redacted evidence HTML through a Crabbox desktop browser, generates a motion GIF, posts PR evidence. Telegram Web login is not required for this lane.                               |
...
| `Mantis Web UI Chat Proof`        | PR comment or manual dispatch                                                              | Runs the focused Natesclaw Control UI chat Playwright proof against the candidate, verifies the browser sends through the mocked Gateway, captures screenshot/video artifacts, and posts PR evidence. This lane is web chat proof only, not WinUI/native-app or arbitrary visual proof.                           |
...
```text
@natesclaw-mantis discord status reactions
@natesclaw-mantis discord status reactions baseline=origin/main candidate=HEAD
...
@natesclaw-mantis web ui chat
@natesclaw-mantis web-ui-chat candidate=HEAD
```
...
`NATESCLAW_MANTIS_CRABBOX_PROVIDER` / `NATESCLAW_MANTIS_CRABBOX_CLASS`. GitHub
...
noVNC, Node 22.22.3+, 24.15+, or 25.9+ and pnpm, an Natesclaw checkout, and
...
- `NATESCLAW_QA_DISCORD_MANTIS_BOT_TOKEN`
- `NATESCLAW_QA_DISCORD_GUILD_ID`
- `NATESCLAW_QA_DISCORD_CHANNEL_ID`
- Local `qa mantis run --credential-source env` also requires
  `NATESCLAW_QA_DISCORD_DRIVER_BOT_TOKEN`, `NATESCLAW_QA_DISCORD_SUT_BOT_TOKEN`,
  and `NATESCLAW_QA_DISCORD_SUT_APPLICATION_ID`. GitHub workflows normally use
  `--credential-source convex` and the broker credentials below instead of raw
  Discord bot tokens.
- `NATESCLAW_QA_REDACT_PUBLIC_METADATA=1` for public artifact uploads
- `NATESCLAW_QA_CONVEX_SITE_URL`, `NATESCLAW_QA_CONVEX_SECRET_CI`
- `OPENAI_API_KEY` (or the Telegram Desktop proof-specific
  `NATESCLAW_MANTIS_AGENT_OPENAI_API_KEY`)
- `CRABBOX_COORDINATOR` / `CRABBOX_COORDINATOR_TOKEN` (workflows also accept
  `NATESCLAW_QA_MANTIS_CRABBOX_COORDINATOR` / `_TOKEN` as a fallback and map
...
ref policy, candidate ref policy, Natesclaw config patch, setup/stimulus steps,

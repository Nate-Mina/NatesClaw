import type { ChatCommandDefinition } from "natesclaw/plugin-sdk/command-auth-native";
...
function createArgMenusHarness(
  cfg: NatesclawConfig = { commands: { native: true, nativeSkills: false } },
  scope?: {
...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, /^natesclaw_cmdarg/, "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "natesclaw_cmdarg", "arg-menu options");
  });
  ...
  it("prefers the configured slash command over native commands", async () => {
    ...
    expect(registration).toEqual({ mode: "single", name: "natesclaw" });
    expect(
      [...configuredHarness.commands.keys()].some(
        (command) => command instanceof RegExp && command.test("/natesclaw"),
      ),
    ).toBe(true);
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(
      Array.from(testHarness.actions.keys()).some(
        (key) => key instanceof RegExp && String(key) === String(/^natesclaw_cmdarg/),
      ),
    ).toBe(true);
    expect(testHarness.options.has("natesclaw_cmdarg")).toBe(true);
  ...
  it.each([
    ...
    (skillHarness.account as { config: NatesclawConfig }).config = {
  ...
  it("deduplicates a skill after the Slack status native rename", async () => {
    ...
    (testHarness.account as { config: NatesclawConfig }).config = config;
  ...
  it.each([
    ...
    { agentRuntime: "natesclaw", includesUltra: true },
  ] as const)(
  ...
  it("shows a button menu when required args are omitted", async () => {
    ...
    expect(actions?.elements?.[0]?.action_id).toBe("natesclaw_cmdarg_0_0");
    expect(actions?.elements?.[1]?.action_id).toBe("natesclaw_cmdarg_0_1");
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("natesclaw_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("natesclaw_cmdarg");
    expect(blockId).toContain("natesclaw_cmdarg_ext:");
    const token = (blockId ?? "").slice("natesclaw_cmdarg_ext:".length);
...
function createPolicyHarness(overrides?: {
  ...
  const ctx = {
    ...
    slashCommand: {
      ...
      name: overrides?.slashCommandName ?? "natesclaw",
...
describe("slack slash command session metadata", () => {
  ...
  it("refreshes slash routing config between invocations", async () => {
    ...
    const sourceCfg = (harness.ctx as { cfg: NatesclawConfig }).cfg;
    const runtimeCfg = {
      ...sourceCfg,
      session: { dmScope: "per-channel-peer" },
    } as NatesclawConfig;
    resolveAgentRouteMock.mockImplementation((params: { cfg: NatesclawConfig }) => ({
      agentId: "main",
      accountId: "acct",
      sessionKey:

...
// Natesclaw rescue policy gates remote writes by owner, DM, sandbox, and YOLO posture.
import type { NatesclawConfig } from "../config/types.natesclaw.js";
import { resolveExecModePolicy } from "../infra/exec-approvals.js";

/**
 * Policy checks for remote Natesclaw rescue commands.
 *
...
function resolveScopedExecConfig(cfg: NatesclawConfig, agentId?: string) {
...
function isYoloHostPosture(cfg: NatesclawConfig, agentId?: string): boolean {
...
export function resolveSystemAgentRescuePolicy(
  ...
  if (sandboxActive) {
    return {
      ...
      message:
        "Natesclaw rescue is blocked because Natesclaw sandboxing is active. Fix the install locally or disable sandboxing before using remote rescue.",
    };
  }
  if (!enabled) {
    return {
      ...
      sandboxActive,
      reason: "disabled",
      message: "Natesclaw rescue requires YOLO host posture with sandboxing off.",
    };
  }
  if (!input.senderIsOwner) {
    return {
      ...
      sandboxActive,
      reason: "not-owner",
      message: "Natesclaw rescue only accepts commands from an Natesclaw owner.",
    };
  }
  if (ownerDmOnly && !input.isDirectMessage) {
    return {
      ...
      message: "Natesclaw rescue is restricted to owner DMs by default.",
    };
  }
  return {
    allowed: true,

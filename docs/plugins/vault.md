...
read_when:
  - You want Natesclaw to read API keys from HashiCorp Vault
...
The bundled Vault plugin lets Natesclaw resolve `exec` SecretRefs from
HashiCorp Vault at Gateway startup and reload time. Natesclaw stores Vault
references in config, keeps resolved values in the in-memory secrets snapshot,
and does not write the resolved API keys back to `natesclaw.json`.

Use this when you already run Vault or want model provider keys to live outside
Natesclaw config files. For the SecretRef runtime model, see
...
Enable the bundled plugin before running the `natesclaw vault` commands:

```bash
natesclaw plugins enable vault
```

## Store a provider key in Vault

Natesclaw defaults to KV v2 mounted at `secret`, matching Vault dev-server
examples. For production Vault, set `NATESCLAW_VAULT_KV_MOUNT` to your actual KV
mount path before creating SecretRef ids. With the Natesclaw defaults, this
SecretRef id:
...
Use a scoped client token for Natesclaw, not a root token. For the default KV v2
...
that starts Natesclaw. The default auth method reads a Vault client token from
`VAULT_TOKEN`:
...
```bash
export VAULT_ADDR=https://vault.example.com
export NATESCLAW_VAULT_AUTH_METHOD=token_file
...
These variables must be present when Natesclaw starts. The Vault plugin forwards
...
```bash
export VAULT_ADDR=https://vault.example.com
export NATESCLAW_VAULT_AUTH_METHOD=jwt
export NATESCLAW_VAULT_AUTH_MOUNT=jwt
export NATESCLAW_VAULT_AUTH_ROLE=natesclaw
export NATESCLAW_VAULT_JWT_FILE=/var/run/secrets/tokens/vault
```
...
```bash
export VAULT_ADDR=https://vault.example.com
export NATESCLAW_VAULT_AUTH_METHOD=kubernetes
export NATESCLAW_VAULT_AUTH_ROLE=natesclaw
```

Set `NATESCLAW_VAULT_AUTH_MOUNT` only when Vault mounted Kubernetes auth somewhere
other than `auth/kubernetes`. Set `NATESCLAW_VAULT_JWT_FILE` only when the service
...
export NATESCLAW_VAULT_KV_MOUNT=secret
export NATESCLAW_VAULT_KV_VERSION=2
```

Check what the current shell can see:

```bash
natesclaw vault status
```
...
```bash
natesclaw vault status --provider-alias corp-vault
```

`natesclaw vault status` never prints `VAULT_TOKEN`; it reports only whether the
...
```bash
natesclaw secrets apply --from ./vault-secrets-plan.json --dry-run --allow-exec
natesclaw secrets apply --from ./vault-secrets-plan.json --allow-exec
natesclaw secrets audit --check --allow-exec
natesclaw secrets reload
```

Use `--allow-exec` because the Vault plugin resolves through an Natesclaw-managed
exec SecretRef provider.

If the Gateway is not running yet, start it normally after applying the plan
instead of running `natesclaw secrets reload`.
...
```bash
natesclaw vault setup --openai-id providers/openai/apiKey
natesclaw vault setup --anthropic-id providers/anthropic/apiKey
natesclaw vault setup --openrouter-id providers/openrouter/apiKey
```
...
Bare target paths apply to `natesclaw.json`. Use
`auth-profiles:<agentId>:<path>` for existing `auth-profiles.json` targets.
The target path must be a registered Natesclaw SecretRef target. The setup
command does not create arbitrary named secrets in Natesclaw; Vault remains the
secret store, and Natesclaw stores SecretRefs only on supported config fields.
...
```bash
export NATESCLAW_VAULT_KV_VERSION=1
```
...
- one auth source:
  - `VAULT_TOKEN`
  - `NATESCLAW_VAULT_AUTH_METHOD=token_file` plus `VAULT_TOKEN_FILE`
  - `NATESCLAW_VAULT_AUTH_METHOD=jwt` plus `NATESCLAW_VAULT_AUTH_MOUNT`,
    `NATESCLAW_VAULT_AUTH_ROLE`, and `NATESCLAW_VAULT_JWT_FILE`
  - `NATESCLAW_VAULT_AUTH_METHOD=kubernetes` plus `NATESCLAW_VAULT_AUTH_ROLE`; optionally
    override `NATESCLAW_VAULT_AUTH_MOUNT` or `NATESCLAW_VAULT_JWT_FILE`
- optional `VAULT_NAMESPACE`, `NATESCLAW_VAULT_KV_MOUNT`, and
  `NATESCLAW_VAULT_KV_VERSION`

When using Kubernetes, prefer `NATESCLAW_VAULT_AUTH_METHOD=kubernetes`
when Vault has Kubernetes auth configured for the cluster. Use
`NATESCLAW_VAULT_AUTH_METHOD=jwt` only when Vault is configured to treat the cluster
...
deployment config. Natesclaw does not require a fixed mount, role, or path: each
Gateway environment can set its own `NATESCLAW_VAULT_KV_MOUNT`,
`NATESCLAW_VAULT_AUTH_ROLE`, and SecretRef ids. If one shared Gateway must resolve

...
describe("backup commands", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("natesclaw-backup-test-");
  });
  ...
  async function withInvalidWorkspaceBackupConfig<T>(fn: (runtime: RuntimeEnv) => Promise<T>) {
    ...
    await fs.writeFile(path.join(stateDir, "natesclaw.json"), JSON.stringify({}), "utf8");
    await fs.writeFile(configPath, '{"agents": { defaults: { workspace: ', "utf8");

    const envSnapshot = captureEnv(["NATESCLAW_CONFIG_PATH"]);
  ...
  it("formats backup archive timestamps in local time", () => {
    ...
    try {
      ...
      expect(buildBackupArchiveRoot(Date.UTC(2026, 2, 14, 1, 2, 3, 456))).toBe(
        "2026-03-14T09-02-03.456+08-00-natesclaw-backup",
      );
      setTestEnvValue("TZ", "America/New_York");
      expect(buildBackupArchiveRoot(Date.UTC(2026, 2, 14, 1, 2, 3, 456))).toBe(
        "2026-03-13T21-02-03.456-04-00-natesclaw-backup",
      );
    } finally {
      envSnapshot.restore();
    }
  ...
  it("orders coverage checks by canonical path so symlinked workspaces do not duplicate state", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".natesclaw");
    const workspaceDir = path.join(stateDir, "workspace");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-workspace-link-"));
  ...
  it("creates an archive with a manifest and external workspace payload", async () => {
    const stateDir = path.join(tempHome.home, ".natesclaw");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-workspace-"));
    const configPath = path.join(tempHome.home, "custom-config.json");
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-backups-"));
  ...
  it("keeps volatile-skip notices out of json output", async () => {
    const stateDir = path.join(tempHome.home, ".natesclaw");
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-backups-json-"));
    try {
      const runtime = createBackupTestRuntime();
  ...
  it("rejects symlinked output paths even when intermediate directories do not exist yet", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".natesclaw");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-backup-link-"));
  ...
  it("falls back to the home directory when cwd is inside a backed-up source tree", async () => {
    ...
    if (process.platform !== "win32") {
      const linkParent = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-backup-cwd-link-"));

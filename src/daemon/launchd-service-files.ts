...
async function resolveLaunchAgentEnvironmentWrapperOverwriteWarnings(params: {
  ...
  return [
    `Existing generated LaunchAgent env wrapper at ${params.wrapperPath} contains custom behavior and will be overwritten; move custom behavior to natesclaw gateway install --wrapper <path> or NATESCLAW_WRAPPER.`,
  ];
}
...
export async function publishLaunchAgentPlist(params: {
  ...
  const temporaryPath = `${params.plistPath}.natesclaw-${randomUUID()}.tmp`;
    ...
    try {
      ...
      try {
        if (previousContents === null) {
          ...
          const rollbackPath = `${params.plistPath}.natesclaw-${randomUUID()}.rollback`;
          try {
...
export async function rewriteLaunchAgentPlistForRestart({
  ...
  const canonicalEnvironment = {
    ...
    NATESCLAW_SERVICE_VERSION: undefined,
  };
  const prepared = await prepareLaunchAgentProgramArguments({
    env,
    label,

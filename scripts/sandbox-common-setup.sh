...
BASE_IMAGE="${BASE_IMAGE:-natesclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-natesclaw-sandbox-common:bookworm-slim}"
...
NATESCLAW_DOCKER_BUILD_USE_BUILDX="${NATESCLAW_DOCKER_BUILD_USE_BUILDX:-0}"
NATESCLAW_DOCKER_BUILD_CACHE_FROM="${NATESCLAW_DOCKER_BUILD_CACHE_FROM:-}"
NATESCLAW_DOCKER_BUILD_CACHE_TO="${NATESCLAW_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=natesclaw.sandbox=1)
NOTE

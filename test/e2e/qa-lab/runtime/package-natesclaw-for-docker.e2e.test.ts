// Package Natesclaw For Docker tests cover QA Lab package artifact evidence.
...
import {
  ...
  packNatesclawPackageForDocker,
  parseArgs,
  prepareBundledAiRuntimePackage,
  runCommandForTest,
  writePackageInventoryForDocker,
} from "../../../../scripts/package-natesclaw-for-docker.mts";
...
describe("package-natesclaw-for-docker", () => {
  ...
  it.runIf(process.platform === "win32")(
    ...
    async () => {
      const tempDir = tempDirs.make("natesclaw-package-pnpm-runner-");
      fs.writeFileSync(
  ...
  it.runIf(process.platform === "win32")(
    ...
    async () => {
      const tempDir = tempDirs.make("natesclaw-package-pnpm-timeout-");
      ...
      fs.writeFileSync(
        ...
        [
          ...
          "fs.writeFileSync(process.env.NATESCLAW_TEST_CHILD_PID, String(process.pid));",
          "setInterval(() => {}, 1000);",
        ].join("\n"),
      );
      fs.writeFileSync(
      ...
      env.PATHEXT = ".CMD";
      env.npm_execpath = "";
      env.NATESCLAW_TEST_CHILD_PID = childPidPath;
  ...
  it("loads from a trusted harness checkout without installed dependencies", async () => {
    const tempRoot = tempDirs.make("natesclaw-package-harness-");
    const copiedFiles = [
      "scripts/package-natesclaw-for-docker.mts",
        ...
        (resolve, reject) => {
          const child = spawn(
            ...
            [
              "--import",
              tsxImport,
              path.join(tempRoot, "scripts/package-natesclaw-for-docker.mts"),
              "--invalid",
            ],
  ...
  it("writes inventory for a frozen source checkout without the trusted helper", async () => {
    const sourceDir = tempDirs.make("natesclaw-package-frozen-source-");
  ...
  it("uses the source package build entrypoint with declaration generation", async () => {
    ...
    const previousTimeout = process.env.NATESCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
    const previousSkipDts = process.env.NATESCLAW_RUN_NODE_SKIP_DTS_BUILD;
    const previousPackageExtensions = process.env.NATESCLAW_EXTENSIONS;
    const previousDockerBuildExtensions = process.env.NATESCLAW_DOCKER_BUILD_EXTENSIONS;
    const previousInternalPluginIds = process.env[DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV];
    const previousPrivateQa = process.env.NATESCLAW_BUILD_PRIVATE_QA;
    process.env.NATESCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = "1234";
    process.env.NATESCLAW_RUN_NODE_SKIP_DTS_BUILD = "1";
    process.env.NATESCLAW_EXTENSIONS = "clickclack";
    process.env.NATESCLAW_DOCKER_BUILD_EXTENSIONS = "slack";
    process.env[DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV] = "msteams";
    process.env.NATESCLAW_BUILD_PRIVATE_QA = "1";
    ...
    try {
      await buildPackageArtifacts("/repo", {
        runImpl: async (
          ...
          calls.push({
            ...
            args,
            cwd,
            noPnpm: options.env?.NATESCLAW_BUILD_ALL_NO_PNPM,
            packageExtensions: options.env?.NATESCLAW_EXTENSIONS,
            dockerBuildExtensions: options.env?.NATESCLAW_DOCKER_BUILD_EXTENSIONS,
            internalDockerBuildPluginIds: options.env?.[DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV],
            privateQa: options.env?.NATESCLAW_BUILD_PRIVATE_QA,
            skipDts: options.env?.NATESCLAW_RUN_NODE_SKIP_DTS_BUILD,
      ...
      if (previousTimeout === undefined) {
        delete process.env.NATESCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
      } else {
        process.env.NATESCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = previousTimeout;
      }
      if (previousSkipDts === undefined) {
        delete process.env.NATESCLAW_RUN_NODE_SKIP_DTS_BUILD;
      } else {
        process.env.NATESCLAW_RUN_NODE_SKIP_DTS_BUILD = previousSkipDts;
      }
      for (const [envName, previousValue] of [
        ["NATESCLAW_EXTENSIONS", previousPackageExtensions],
        ["NATESCLAW_DOCKER_BUILD_EXTENSIONS", previousDockerBuildExtensions],
        [DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV, previousInternalPluginIds],
        ["NATESCLAW_BUILD_PRIVATE_QA", previousPrivateQa],
      ] as const) {
  ...
  it("rejects loose package artifact timeout env values", async () => {
    ...
    try {
      for (const value of ["1e3", "123.9", "9007199254740993", "0"]) {
        process.env.NATESCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = value;
        ...
        await expect(
          ...
          "NATESCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS must be a positive timeout in milliseconds",
        );
      }
    } finally {
      if (previousTimeout === undefined) {
  ...
  it("bundles and restores the separately packed AI runtime", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-docker-ai-source-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-docker-ai-output-"));
    ...
    const installedAiPath = path.join(sourceDir, "node_modules", "@natesclaw", "ai");
    ...
    try {
      const cleanup = await prepareBundledAiRuntimePackage(
        ...
        async (command: string, args: string[], cwd: string) => {
          ...
          fs.writeFileSync(path.join(outputDir, "natesclaw-ai-2026.6.17.tgz"), "ai package");
          return "";
        },
        {
      ...
      expect(fs.existsSync(path.join(outputDir, "natesclaw-ai-2026.6.17.tgz"))).toBe(false);
    } finally {
  ...
  it("reuses the source manifest lifecycle for ignore-scripts package artifacts", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-docker-manifest-source-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-docker-manifest-output-"));
    ...
    try {
      const tarball = await packNatesclawPackageForDocker(sourceDir, outputDir, {
        ...
        runCaptureImpl: async () => {
          ...
          const packedPath = path.join(outputDir, "natesclaw-2026.8.1.tgz");
          fs.writeFileSync(packedPath, "package");
          return `${path.basename(packedPath)}\n`;
        },
      });

      expect(tarball).toBe(path.join(outputDir, "natesclaw-2026.8.1.tgz"));
      expect(fs.readFileSync(packageJsonPath, "utf8")).toBe(originalPackageJson);
      expect(
        fs.existsSync(
  ...
  it("leaves pre-AI-workspace package sources unchanged", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-docker-legacy-source-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-docker-legacy-output-"));
    const packageJsonPath = path.join(sourceDir, "package.json");
    const originalPackageJson = `${JSON.stringify({
  ...
  it("rejects incomplete AI workspace package sources", async () => {
    ...
    for (const testCase of cases) {
      const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-docker-invalid-source-"));
      const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-docker-invalid-output-"));
      fs.writeFileSync(
        path.join(sourceDir, "package.json"),
        `${JSON.stringify({ dependencies: testCase.dependencies, name: "natesclaw" })}\n`,
      );
  ...
  it("trims and restores the changelog around ignore-scripts package artifacts", async () => {
    const calls: string[] = [];
    const tarball = await packNatesclawPackageForDocker("/repo", "/out", {
      prepareBundledAiRuntime: skipBundledAiRuntime,
      ...
      runCaptureImpl: async (
        ...
        return "natesclaw-2026.5.28.tgz\n";
      },
    });

    expect(tarball).toBe(path.join("/out", "natesclaw-2026.5.28.tgz"));
    expect(calls).toEqual([
      "prepare-docs:/repo",
      "prepare:/repo",
  ...
  it("does not touch other source artifacts when the docs-map lock fails", async () => {
    ...
    await expect(
      packNatesclawPackageForDocker("/repo", "/out", {
  ...
  it("keeps the docs-map lock when changelog restoration fails", async () => {
    const outputDir = tempDirs.make("natesclaw-package-restore-order-");
    const calls: string[] = [];

    await expect(
      packNatesclawPackageForDocker("/repo", outputDir, {
        prepareBundledAiRuntime: skipBundledAiRuntime,
        prepareChangelog: async () => {},
        prepareDocsMap: async () => {},
        restoreChangelog: async () => {
  ...
  it("packages Unreleased notes for explicitly non-publish stable artifacts", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-unreleased-package-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-unreleased-output-"));
    ...
    try {
      const tarball = await packNatesclawPackageForDocker(sourceDir, outputDir, {
        ...
        runCaptureImpl: async () => {
          ...
          const packedPath = path.join(outputDir, "natesclaw-2026.5.29.tgz");
          fs.writeFileSync(packedPath, "package");
          return "natesclaw-2026.5.29.tgz\n";
        },
      });

      expect(tarball).toBe(path.join(outputDir, "natesclaw-2026.5.29.tgz"));
  ...
  it("keeps a frozen pre-map source package byte-owned by that ref", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-frozen-package-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-frozen-output-"));
    ...
    try {
      const tarball = await packNatesclawPackageForDocker(sourceDir, outputDir, {
        ...
        runCaptureImpl: async () => {
          ...
          const packedPath = path.join(outputDir, "natesclaw-2026.6.33.tgz");
          fs.writeFileSync(packedPath, "frozen package");
          return `${path.basename(packedPath)}\n`;
        },
      });

      expect(tarball).toBe(path.join(outputDir, "natesclaw-2026.6.33.tgz"));
      expect(fs.existsSync(path.join(docsDir, "docs_map.md"))).toBe(false);
    } finally {
      fs.rmSync(sourceDir, { recursive: true, force: true });
      fs.rmSync(outputDir, { recursive: true, force: true });
  ...
  it("uses pnpm pack when requested", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-pnpm-pack-"));
    const calls: string[] = [];
    const packedPath = path.join(outputDir, "natesclaw-2026.5.28.tgz");

    try {
      const tarball = await packNatesclawPackageForDocker("/repo", outputDir, {
        ...skipDocsMapLifecycle,
        pnpmPack: true,
        prepareBundledAiRuntime: skipBundledAiRuntime,
        prepareChangelog: async () => {},
  ...
  it("normalizes npm 12 pack metadata for renamed package artifacts", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-docker-pack-json-"));
    ...
    try {
      const tarball = await packNatesclawPackageForDocker("/repo", outputDir, {
        ...
        runCaptureImpl: async (
          ...
          expect(options.deferForwardedSignalExit).toBe(true);
          fs.writeFileSync(path.join(outputDir, "natesclaw-2026.5.28.tgz"), "package");
          return JSON.stringify({
            natesclaw: {
              entryCount: 1,
              filename: "natesclaw-2026.5.28.tgz",
              size: 7,
              unpackedSize: 7,
              version: "2026.5.28",
            },
      ...
      expect(tarball).toBe(path.join(outputDir, "natesclaw-current.tgz"));
  ...
  it("rejects path-like npm pack stdout before resolving Docker package tarballs", async () => {
    for (const filename of [
      ...
      "natesclaw-nested/evil.tgz",
      String.raw`natesclaw-nested\evil.tgz`,
      "natesclaw-C:evil.tgz",
    ]) {
      await expect(
        packNatesclawPackageForDocker("/repo", "/out", {
          ...skipDocsMapLifecycle,
  ...
  it("ignores unsafe output directory tarball names when npm stdout is not usable", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-docker-pack-"));
    try {
      if (process.platform === "win32") {
        const nestedDir = path.join(outputDir, "natesclaw-nested");
        fs.mkdirSync(nestedDir);
        fs.writeFileSync(path.join(nestedDir, "evil.tgz"), "");
      } else {
        fs.writeFileSync(path.join(outputDir, "natesclaw-C:evil.tgz"), "");
        fs.writeFileSync(path.join(outputDir, String.raw`natesclaw-nested\evil.tgz`), "");
      }
      await expect(
        packNatesclawPackageForDocker("/repo", outputDir, {
          ...skipDocsMapLifecycle,
      ...
      ).rejects.toThrow("missing packed Natesclaw tarball");
      ...
      await expect(
        packNatesclawPackageForDocker("/repo", outputDir, {
          ...
          restoreChangelog: async () => {},
          runCaptureImpl: async () => {
            fs.writeFileSync(path.join(outputDir, "natesclaw-2026.6.17.tgz"), "");
            return "npm notice\n";
          },
        }),
      ).resolves.toBe(path.join(outputDir, "natesclaw-2026.6.17.tgz"));
    } finally {
      fs.rmSync(outputDir, { recursive: true, force: true });
    }
  });
  ...
  it("ignores stale package tarballs before fallback scanning npm output", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-docker-pack-stale-"));
    try {
      fs.writeFileSync(path.join(outputDir, "natesclaw-9999.1.1.tgz"), "stale");
      ...
      await expect(
        packNatesclawPackageForDocker("/repo", outputDir, {
          ...
          restoreChangelog: async () => {},
          runCaptureImpl: async () => {
            fs.writeFileSync(path.join(outputDir, "natesclaw-2026.6.17.tgz"), "current");
            return "npm notice\n";
          },
        }),
      ).resolves.toBe(path.join(outputDir, "natesclaw-2026.6.17.tgz"));

      expect(fs.existsSync(path.join(outputDir, "natesclaw-9999.1.1.tgz"))).toBe(false);
      expect(fs.readFileSync(path.join(outputDir, "natesclaw-2026.6.17.tgz"), "utf8")).toBe(
        "current",
      );
    } finally {
      fs.rmSync(outputDir, { recursive: true, force: true });
  ...
  it("kills timed-out child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-package-timeout-"));
    const childPidPath = path.join(tempDir, "child.pid");
    let childPid;
    try {
  ...
  it("clamps oversized kill grace before scheduling", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-package-grace-"));
  ...
  it("keeps fallback SIGKILL armed for descendants after the direct child exits", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-package-descendant-"));
    const childPidPath = path.join(tempDir, "child.pid");
    let childPid;
    try {
      const childScript = ["process.on('SIGTERM', () => {});", "setInterval(() => {}, 1000);"].join(
  ...
  it("forwards external termination to active child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-package-signal-"));
    const childPidPath = path.join(tempDir, "child.pid");
    const scriptUrl = pathToFileURL(path.resolve("scripts/package-natesclaw-for-docker.mts")).href;
    let childPid = 0;
    let runnerPid;
    try {

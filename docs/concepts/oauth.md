---
summary: "OAuth in Natesclaw: token exchange, storage, and multi-account patterns"
read_when:
  - You want to understand Natesclaw OAuth end-to-end
...
Natesclaw supports OAuth ("subscription auth") for providers that offer it,
notably **OpenAI Codex (ChatGPT OAuth)** and **Anthropic Claude CLI reuse**.
For Anthropic, the practical split is:

- **Anthropic API key**: normal Anthropic API billing.
- **Anthropic Claude CLI / subscription auth inside Natesclaw**: Anthropic staff
  told us this usage is allowed again, so Natesclaw treats Claude CLI reuse and
...
Natesclaw stores both OpenAI API-key auth and ChatGPT/Codex OAuth under the
...
`natesclaw doctor --fix`; use `openai:*` profile ids and `auth.order.openai` for
new config.
...
issued for the same user/app. Practical symptom: log in via Natesclaw _and_
via Claude Code / Codex CLI, and one of them randomly gets logged out later.

To reduce that, Natesclaw treats the auth profile store as a **token sink**:

- the runtime reads credentials from one place per agent
- multiple profiles can coexist and route deterministically
- external CLI reuse is provider-specific: once Natesclaw owns a local OAuth
  profile for a provider, the local refresh token is canonical. If that local
  refresh token is rejected, Natesclaw reports the profile for
  ...
  `openai:default`-style profile before Natesclaw owns OAuth for that
  provider; after that, Natesclaw-owned refreshes stay canonical
...
`natesclaw doctor --fix` once after upgrading. Doctor imports verified values,
...
The database and migration sources respect `$NATESCLAW_STATE_DIR`. Full reference: [/gateway/configuration-reference#auth-storage](/gateway/configuration-reference#auth-storage)

For static secret refs and runtime snapshot activation behavior, see [Secrets Management](/gateway/secrets).

When a secondary agent has no local auth profile, Natesclaw uses read-through
...
Natesclaw supports Anthropic Claude CLI reuse and `claude -p` as a sanctioned
...
available as a supported token-auth path, but Natesclaw prefers Claude CLI
reuse when it is available.

<Warning>
Anthropic's public Claude Code docs say direct Claude Code use stays within
Claude subscription limits, and Anthropic staff told us Natesclaw-style Claude
CLI usage is allowed again. Natesclaw therefore treats Claude CLI reuse and
`claude -p` usage as sanctioned for this integration unless Anthropic
...
If you want other subscription-style options in Natesclaw, see [OpenAI
...
Natesclaw's interactive login flows are implemented in `natesclaw/plugin-sdk/llm.ts` and wired into the wizards/commands.

### Anthropic setup-token

Flow shape:

1. create the token by running `claude setup-token` on any machine with Claude Code, then start Anthropic setup-token or paste-token from Natesclaw
2. Natesclaw stores the resulting Anthropic credential in an auth profile
3. model selection stays on `anthropic/...`
4. existing Anthropic auth profiles remain available for rollback/order control

### OpenAI Codex (ChatGPT OAuth)

OpenAI Codex OAuth is explicitly supported for use outside the Codex CLI, including Natesclaw workflows.
...
`natesclaw models auth list --provider openai` after repair before copying
...
3. try to capture the callback on `http://localhost:1455/auth/callback` (the
   ...
   override with `NATESCLAW_OAUTH_CALLBACK_HOST`)
...
Wizard path is `natesclaw onboard` → auth choice `openai`.
...
- externally managed CLI credentials (Claude CLI, narrow Codex CLI bootstrap;
  ...
  spending a copied refresh token. If a managed refresh fails, Natesclaw
...
```bash
natesclaw models auth list --provider <id>
```

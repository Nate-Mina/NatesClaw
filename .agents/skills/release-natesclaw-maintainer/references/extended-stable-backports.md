...
`natesclaw` npm package, official npm plugins, and matching Docker Gateway
...
- Read `docs/reference/RELEASING.md`,
  `scripts/natesclaw-npm-extended-stable-release.mjs`, and the relevant release
...
- Cover the core `natesclaw` package and every npm-publishable official plugin
...
- Read `SECURITY.md` and use `$security-triage` for security candidates. Route
  unpublished advisory work through `$natesclaw-ghsa-maintainer`; never expose
  private details before the security owner authorizes disclosure.
- Use `$natesclaw-testing` for proof selection, `$autoreview` before handoff,
  and `$natesclaw-pr-maintainer` for GitHub operations.
...
dist_tags=$(npm view natesclaw dist-tags --json --userconfig "$npm_userconfig")
published_version=$(printf '%s' "$dist_tags" | jq -r '."extended-stable" // empty')
if [[ -n "$published_version" ]]; then
  npm view "natesclaw@${published_version}" version \
...
- mode, published `natesclaw@extended-stable` version or approved bootstrap

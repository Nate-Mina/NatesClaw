...
type PluginBlobTable = NatesclawStateKyselyDatabase["plugin_blob_entries"];
type PluginBlobDatabase = Pick<NatesclawStateKyselyDatabase, "plugin_blob_entries">;
...
function createError(params: {
  ...
  return new PluginBlobStoreError(params.message, {
    ...
    path: resolveNatesclawStateSqlitePath(params.env ?? process.env),
    cause: params.cause,
  });
}
...
function openDatabase(operation: PluginBlobStoreOperation, env?: NodeJS.ProcessEnv) {
  try {
    const database = openNatesclawStateDatabase(env ? { env } : {});
    return database;
  } catch (error) {
    throw wrapError(
      error,

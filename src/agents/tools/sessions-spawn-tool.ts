...
// Keep the schema local to avoid a circular import through acp-spawn/natesclaw-tools.
...
function resolveAcpUnavailableMessage(opts?: { sandboxed?: boolean; config?: NatesclawConfig }) {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (deliveryPressure.blocked) {
        return jsonResult({
          status: "forbidden",
          error: `sessions_spawn is paused because ${deliveryPressure.suspended} completed tasks have blocked delivery. Run natesclaw tasks list, then retry or dismiss blocked deliveries.`,
          ...roleContext,
        });
      }

...
type GetRuntimeConfigFn =
  typeof import("natesclaw/plugin-sdk/runtime-config-snapshot").getRuntimeConfig;
type GetSessionEntryFn = typeof import("natesclaw/plugin-sdk/session-store-runtime").getSessionEntry;
type ResolveStorePathFn =
  typeof import("natesclaw/plugin-sdk/session-store-runtime").resolveStorePath;
type ReadSessionUpdatedAtFn =
  typeof import("natesclaw/plugin-sdk/session-store-runtime").readSessionUpdatedAt;
type SessionEntry = import("natesclaw/plugin-sdk/session-store-runtime").SessionEntry;
...
const { sessionStorePath } = vi.hoisted(() => {
  ...
  return {
    sessionStorePath: `${tempRoot.replace(/[\\/]+$/u, "")}${separator}natesclaw-telegram-${
      process.pid
...
function resolveDefaultModelForAgentForTest(params: { cfg: NatesclawConfig }): {
  provider: string;
  model: string;
} {
...
function createModelsProviderDataFromConfig(cfg: NatesclawConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: NatesclawConfig = {
  agents: {
    defaults: {
      userTimezone: "UTC",
    },
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: NatesclawConfig) => {

...
# Output:
# - dist/Natesclaw.app
# - dist/Natesclaw-<version>.zip
# - dist/Natesclaw-<version>.dmg
...
export BUILD_CONFIG
export NATESCLAW_CONTROL_UI_RELEASE_BUILD=1
...
export BUNDLE_ID="${BUNDLE_ID:-ai.natesclaw.mac}"
...
canonical_sparkle_build() {
  ...
  stderr_file="$(mktemp "${TMPDIR:-/tmp}/natesclaw-sparkle-build.XXXXXX")" || {
...
APP="$ROOT_DIR/dist/Natesclaw.app"
if [[ ! -d "$APP" ]]; then
...
ZIP="$ROOT_DIR/dist/Natesclaw-$VERSION.zip"
DMG="$ROOT_DIR/dist/Natesclaw-$VERSION.dmg"
NOTARY_ZIP="$ROOT_DIR/dist/Natesclaw-$VERSION.notary.zip"
DSYM_ZIP="$ROOT_DIR/dist/Natesclaw-$VERSION.dSYM.zip"

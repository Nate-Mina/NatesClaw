// Sync Plugin Versions script supports Natesclaw repository automation.
...
const NATESCLAW_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;
...
function syncNatesclawDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.natesclaw;
  if (!current || current === "workspace:*" || !NATESCLAW_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncPluginApiVersion(pkg: PackageJson, targetVersion: string): boolean {
  const compat = pkg.natesclaw?.compat;
  const current = compat?.pluginApi;
  if (!current || !NATESCLAW_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function syncBuildNatesclawVersion(pkg: PackageJson, targetVersion: string): boolean {
  const build = pkg.natesclaw?.build;
  const current = build?.natesclawVersion;
  if (!current) {
    return false;
  }
  ...
  build.natesclawVersion = targetVersion;
  return true;
}
...
export function syncPluginVersions(
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncNatesclawDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncNatesclawDependencyRange(pkg.peerDependencies, targetVersion);
    ...
    const buildNatesclawVersionChanged = syncBuildNatesclawVersion(pkg, targetVersion);
    const packageChanged =
      versionChanged ||
      devDependencyChanged ||
      peerDependencyChanged ||
      pluginApiChanged ||
      buildNatesclawVersionChanged;
    if (!packageChanged) {

...
export async function runGatewayLoop(params: {
  ...
  if (process.title === "natesclaw") {
    process.title = "natesclaw-gateway";
  }
  ...
  // (e.g. `npm install -g natesclaw@latest` triggered via update.run),
  ...
  const handleRestartAfterServerClose = async () => {
    ...
    if (isUpdateRestart) {
      ...
      if (respawn.mode === "failed") {
        ...
        gatewayLog.info(
          `restart mode: in-process restart (${respawn.detail ?? "NATESCLAW_NO_RESPAWN"})`,
        );
      }
      if (!(await reacquireLockForInProcessRestart())) {
        return;
  ...
  try {
    ...
    for (;;) {
      ...
      try {
        ...
        const mediaMigrationRequired = findNatesclawAgentDatabaseMediaMigrationRequiredError(err);
        params.completeBoot?.({

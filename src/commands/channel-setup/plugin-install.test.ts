...
vi.mock("../../plugins/loader.js", () => {
  const load = vi.fn();
  return { loadNatesclawPlugins: load, loadPluginRegistryHandle: load };
});

const discoverNatesclawPlugins = vi.fn((_args?: unknown) => ({
  candidates: [] as PluginCandidate[],
  diagnostics: [],
}));
vi.mock("../../plugins/discovery.js", () => ({
  discoverNatesclawPlugins: (args: unknown) => discoverNatesclawPlugins(args),
}));
...
import { loadNatesclawPlugins } from "../../plugins/loader.js";
  ...
  resolveBundledPluginSources.mockReturnValue(
    new Map([
      [
        ...
        {
          pluginId: "bundled-chat",
          localPath: bundledPluginRootAt("/opt/natesclaw", "bundled-chat"),
          npmSpec: bundledChatNpmSpec,
        },
      ],
    ]),
...
function createManifestRecord(
  ...
  return {
    ...
    hooks: [],
    origin: "bundled",
    rootDir: `/tmp/natesclaw-test/${id}`,
    source: `/tmp/natesclaw-test/${id}/index.ts`,
    manifestPath: `/tmp/natesclaw-test/${id}/natesclaw.plugin.json`,
    ...rest,
  };
}
...
function expectSetupSnapshotDoesNotScopeToPlugin(params: {
  ...
  expect(loadNatesclawPlugins).toHaveBeenCalledTimes(1);
  expect(requireMockCallArg(vi.mocked(loadNatesclawPlugins), 0).onlyPluginIds).toStrictEqual([]);
}
...
beforeEach(() => {
  ...
  discoverNatesclawPlugins.mockReturnValue({ candidates: [], diagnostics: [] });
...
async function runInitialValueForChannel(channel: "dev" | "beta") {
  ...
  const select = vi.fn((async <T extends string>() => "skip" as T) as WizardPrompter["select"]);
  const prompter = makePrompter({ select: select as unknown as WizardPrompter["select"] });
  const cfg: NatesclawConfig = { update: { channel } };
...
function expectLoadNatesclawPluginFields(expected: Record<string, unknown>, callIndex = 0) {
  expectRecordFields(
    requireMockCallArg(vi.mocked(loadNatesclawPlugins), callIndex),
    "loadNatesclawPlugins args",
    expected,
  );
}
...
describe("ensureChannelSetupPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: NatesclawConfig = { plugins: { allow: ["bundled-chat"] } };
  ...
  it("installs npm channel plugins into the active profile extensions dir", async () => {
    ...
    const profileStateDir = "/tmp/natesclaw-ledger-channel";
    process.env.NATESCLAW_STATE_DIR = profileStateDir;
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "bundled-chat",
  ...
  it("installs npm beta on the beta channel without persisting the beta tag", async () => {
    ...
    const cfg: NatesclawConfig = { update: { channel: "beta" } };
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
  ...
  it("defaults to bundled local path on beta channel when available", async () => {
    ...
    expectRecordFields(
      ...
      {
        value: "local",
        hint: bundledPluginRootAt("/opt/natesclaw", "bundled-chat"),
      },
    );
  });
  ...
  it("offers ClawHub as the first-class install source for channel catalog entries", async () => {
    ...
    expectRecordFields(options[0], "clawhub option", {
      value: "clawhub",
      label: "Download from ClawHub (clawhub:natesclaw/clawhub-chat@2026.5.2)",
    });
    expectRecordFields(options[1], "skip option", {
      value: "skip",
    });
  ...
  it("skips the install prompt when autoConfirmSingleSource is set and only npm is available", async () => {
    ...
    expect(select).not.toHaveBeenCalled();
    expect(result.installed).toBe(true);
    expect(result.pluginId).toBe("wecom-natesclaw-plugin");
  });
  ...
  it("loads setup snapshots from the auto-enabled config snapshot", () => {
    ...
    expectLoadNatesclawPluginFields({
      config: autoEnabledConfig,
  ...
  it("scopes snapshots by activation-declared channel ownership when direct channel lists are empty", () => {
    ...
    discoverNatesclawPlugins.mockReturnValue({
      candidates: [
        {
          idHint: "custom-external-chat-plugin",
          source: "/tmp/natesclaw-test/custom-external-chat-plugin/index.ts",
          rootDir: "/tmp/natesclaw-test/custom-external-chat-plugin",
          origin: "bundled",
        },
      ],
      diagnostics: [],
    ...
    loadPluginManifestRegistryCore.mockImplementation((args: unknown) => {
      if (
        isRecord(args) &&
        args.config === cfg &&
        args.workspaceDir === "/tmp/natesclaw-workspace" &&

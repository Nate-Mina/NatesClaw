...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("natesclaw.authProfileUsageTestApi")] =
    testing;
}
...
async function probeWhamForCooldown(
  ...
  try {
    const version = process.env.NATESCLAW_VERSION?.trim();
    const defaultHeaders: Record<string, string> = {
      Authorization: `Bearer ${profile.access}`,
      Accept: "application/json",
      originator: "natesclaw",
      ...(version ? { version } : {}),
      "User-Agent": `natesclaw/${version || "dev"}`,
    };

...
import { escapeRegExp, formatEnvelopeTimestamp } from "natesclaw/plugin-sdk/channel-test-helpers";
...
import type { GetReplyOptions, MsgContext } from "natesclaw/plugin-sdk/reply-runtime";
import { withEnvAsync } from "natesclaw/plugin-sdk/test-env";
import { createRequireRecord, sanitizeTerminalText } from "natesclaw/plugin-sdk/test-fixtures";
...
vi.mock("natesclaw/plugin-sdk/conversation-runtime", { spy: true });

const harness = await import("./bot.create-telegram-bot.test-harness.js");
const pluginStateTestRuntime = await import("natesclaw/plugin-sdk/plugin-state-test-runtime");
const configMutation = await import("natesclaw/plugin-sdk/config-mutation");
const modelSessionRuntime = await import("natesclaw/plugin-sdk/model-session-runtime");
...
function createTelegramBotTestStateDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "natesclaw-telegram-bot-"));
...
describe("createTelegramBot", () => {
  ...
  beforeEach(async () => {
    ...
    process.env.NATESCLAW_STATE_DIR = createTelegramBotTestStateDir();
    resetTelegramForumFlagCacheForTest();
    clearPluginInteractiveHandlers();
  ...
  it.each(["stop", "/stop@natesclaw_bot"] as const)(
    ...
    try {
      ...
      await runTelegramMiddlewareChain({
        ctx: {
          ...
          message: {
            chat: { id: -1007, type: "supergroup", title: "Natesclaw Ops" },
            text: "first",
            date: 1736380804,
            message_id: 104,
  ...
  it("routes plugin callback_query payloads to plugin handlers without fallback callback_data text", async () => {
    ...
    expect(
      registerPluginInteractiveHandler("natesclaw-code-agent", {
        channel: "telegram",
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    for (const [index, testCase] of cases.entries()) {
      ...
      expect(pairingText, testCase.name).toContain("natesclaw pairing approve telegram");
      expectRecordFields(
  ...
  it("ignores private self-authored message updates instead of issuing a pairing challenge", async () => {
    ...
    await handler({
      message: {
        ...
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "Natesclaw", username: "natesclaw_bot" },
        pinned_message: {
          message_id: 1883,
          date: 1736380799,
          chat: { id: 1234, type: "private", first_name: "Harold" },
          from: { id: 7, is_bot: true, first_name: "Natesclaw", username: "natesclaw_bot" },
          text: "Binding: Review pull request 54118 (natesclaw)",
        },
      },
      me: { id: 7, is_bot: true, first_name: "Natesclaw", username: "natesclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  it("ignores group self-authored message updates instead of re-processing bot output", async () => {
    ...
    await handler({
      message: {
        chat: { id: -1001234, type: "supergroup", title: "Natesclaw Ops" },
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "Natesclaw", username: "natesclaw_bot" },
  ...
  const groupPolicyCases: MessagePolicyCase[] = [
    makeMessagePolicyCase({
      ...
      message: { text: "@natesclaw_bot hello" },
    ...
    makeMessagePolicyCase({
      ...
      message: { from: { id: 999999, username: "notallowed" }, text: "@natesclaw_bot hello" },
      expectedReplyCount: 0,
    }),
    makeMessagePolicyCase({
  ...
  async function dispatchMessage(params: {
    ...
    await handler({
      ...
      me: params.me ?? { username: "natesclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
  ...
  it("marks explicit Telegram bot-handle mentions in the inbound context", async () => {
    ...
    await dispatchMessage({
      message: {
        chat: { id: 7, type: "group", title: "Test Group" },
        text: "@natesclaw_bot status",
        entities: [{ type: "mention", offset: 0, length: "@natesclaw_bot".length }],
        date: 1736380800,
        message_id: 4,
        from: { id: 9, first_name: "Ada" },
      },
    ...
    expect(payload.BotUsername).toBe("natesclaw_bot");
  });
  ...
  it("honors routed group activation from session store", async () => {
    const storePath = "/tmp/natesclaw-telegram-group-activation.json";

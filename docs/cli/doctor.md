---
summary: "CLI reference for `natesclaw doctor` (health checks + guided repairs)"
read_when:
...
When Gateway status reports degraded SecretRef owners, doctor prints a **Secret runtime degradation** warning with every cold or stale owner, affected config path, redacted reason, and the `natesclaw secrets reload` retry command.

When channel ingress events are dead-lettered, doctor names each affected channel account and points to [`natesclaw channels dead-letters list`](/cli/channels#inbound-dead-letters) for inspection and recovery.
...
| ------------------------- | ----------------------------------------- | ------------------------------------------------------------------------------- |
| Inspect                   | `natesclaw doctor`                         | Human-oriented checks and guided prompts.                                       |
| Repair                    | `natesclaw doctor --fix`                   | Applies supported repairs, using prompts unless non-interactive repair is safe. |
| Lint                      | `natesclaw doctor --lint`                  | Read-only structured findings for CI, preflight, and review gates.              |
| Shared SQLite maintenance | `natesclaw doctor --state-sqlite compact`  | Explicitly checkpoints, compacts, and verifies the canonical shared state DB.   |
| Session SQLite migration  | `natesclaw doctor --session-sqlite <mode>` | Inspects, imports, validates, compacts, recovers, or restores session state.    |
...
natesclaw doctor --lint --allow-exec
natesclaw doctor --deep
natesclaw doctor --fix
natesclaw doctor --fix --non-interactive
natesclaw doctor --generate-gateway-token
natesclaw doctor --post-upgrade
natesclaw doctor --post-upgrade --json
natesclaw doctor --state-sqlite compact
natesclaw doctor --state-sqlite compact --json
natesclaw doctor --session-sqlite inspect --session-sqlite-all-agents
natesclaw doctor --session-sqlite dry-run --session-sqlite-agent main --json
natesclaw doctor --session-sqlite import --session-sqlite-all-agents
natesclaw doctor --session-sqlite validate --session-sqlite-all-agents --json
natesclaw doctor --session-sqlite compact --session-sqlite-all-agents
natesclaw doctor --session-sqlite recover --github-issue
natesclaw doctor --session-sqlite restore --session-sqlite-all-agents
```

For channel-specific permissions, use the channel probes instead of `doctor`:

```bash
natesclaw channels capabilities --channel discord --target channel:<channel-id>
...
| `--github-issue`                | With `--session-sqlite recover`: prepare a sanitized natesclaw/natesclaw issue report; doctor creates it with `gh` after `--yes` or interactive confirmation.                             |
...
`natesclaw doctor --lint` is read-only: no prompts, no repair, no config/state rewrites.
...
natesclaw doctor --lint --only core/doctor/local-audio-acceleration --severity-min info
```
...
doctor --lint: ran 6 check(s), 1 finding(s)
  [warning] core/doctor/gateway-config gateway.mode - gateway.mode is unset; gateway start will be blocked.
    fix: Run `natesclaw configure` and set Gateway mode (local/remote), or `natesclaw config set gateway.mode local`.
```
  ...
  "findings": [
    {
      ...
      "path": "gateway.mode",
      "fixHint": "Run `natesclaw configure` and set Gateway mode (local/remote), or `natesclaw config set gateway.mode local`."
    }
  ]
}
```
...
`--severity-min` controls both which findings print and the exit threshold: `natesclaw doctor --lint --severity-min error` can print nothing and exit `0` even when lower-severity `info`/`warning` findings exist.
...
`natesclaw doctor --fix` against the same mounted state/config before restarting
the container normally.

## Legacy state migration

`natesclaw doctor --fix` is the only owner for persistent file-to-SQLite migrations. It validates and claims each recognized source, writes and verifies canonical rows, records a migration receipt, then removes the retired source. Runtime code does not perform lazy imports or fallback reads.
...
This includes retired MCP OAuth files under `<state-dir>/mcp-oauth/*.json`. Stop the Gateway before repair. Doctor imports valid credentials into `<state-dir>/state/natesclaw.sqlite`, preserves an existing canonical SQLite session when both stores exist, drops the obsolete persisted OAuth `state` value, and uses its receipt to prevent a recreated stale file from resurrecting logged-out credentials. Retired `.lock` sidecars fail closed: if Doctor reports a stale owner, verify that no older Natesclaw process is running, remove that sidecar, and rerun Doctor.
...
`natesclaw doctor --state-sqlite compact` is explicit offline maintenance for
the canonical shared state database at
`<state-dir>/state/natesclaw.sqlite`. It does not accept an arbitrary database
path, is never invoked by normal Gateway operation, and is not part of
`natesclaw doctor --fix`. The command acquires the same state ownership lock as
...
`NATESCLAW_ALLOW_MULTI_GATEWAY=1` skips the per-config Gateway singleton, so an
...
3. Requires a non-busy `wal_checkpoint(TRUNCATE)`. Stop any remaining Natesclaw
...
running Natesclaw build, or belongs to an agent database. Run
`natesclaw doctor --fix` first for an older shared-state schema. Restore a
compatible backup or upgrade Natesclaw for a newer schema.

## Session SQLite migration

Natesclaw imports legacy session rows and transcript history into each agent's
SQLite database automatically during gateway startup and during
`natesclaw doctor --fix`. `natesclaw doctor --session-sqlite <mode>` is the
targeted inspection and validation tool for that migration. Current runtime
session rows live in
`~/.natesclaw/agents/<agentId>/agent/natesclaw-agent.sqlite`. Legacy
...
The regular `natesclaw doctor` pass also reports canonical SQLite transcripts
whose initial session header was never persisted. `natesclaw doctor --fix`
...
```bash
natesclaw doctor --session-sqlite inspect --session-sqlite-all-agents
natesclaw doctor --session-sqlite dry-run --session-sqlite-all-agents --json
natesclaw doctor --session-sqlite import --session-sqlite-all-agents
natesclaw doctor --session-sqlite validate --session-sqlite-all-agents --json
natesclaw doctor --session-sqlite compact --session-sqlite-all-agents
natesclaw doctor --session-sqlite recover --github-issue
...
Back up the Natesclaw state directory before running `import` on an install with
...
transcripts, run `natesclaw doctor --session-sqlite compact --session-sqlite-all-agents`
...
target must be inside the active state directory; set `NATESCLAW_STATE_DIR` to
...
`~/.natesclaw/session-sqlite-migration-runs/` before moving transcript artifacts
...
the GitHub CLI to create the issue in `natesclaw/natesclaw`; without confirmation
...
Before starting an older file-backed Natesclaw version, restore the archived
...
upgrade again, run the normal migration validation sequence above so Natesclaw can
...
- Any config write (including a `--fix` repair) rotates a backup to `~/.natesclaw/natesclaw.json.bak` (with a numbered `.bak.1`..`.bak.4` ring). `--fix` also drops unknown config keys reported by schema validation, listing each removal; it skips this while an update is in progress so partially written upgrade state is not stripped before its migration finishes.
- If `natesclaw.json` cannot be parsed and no last-known-good config can be recovered, `doctor --fix` preserves the original as `natesclaw.json.clobbered.<timestamp>`, leaves the current file unchanged, and exits with an error instead of writing a partial replacement.
- Set `NATESCLAW_SERVICE_REPAIR_POLICY=external` when another supervisor owns the gateway lifecycle. Doctor still reports gateway/service health and applies non-service repairs, but skips service install/start/restart/bootstrap and legacy service cleanup.
- Doctor reports the managed Gateway's applied heap limit and the adaptive derivation used for the current host or container memory limit. Use `natesclaw gateway status` for the same report outside a repair pass.
- On Linux, doctor ignores inactive extra gateway-like systemd units and does not rewrite command/entrypoint metadata for a running systemd gateway service during repair. Stop the service first, or use `natesclaw gateway install --force` to replace the active launcher.
- `doctor --fix --non-interactive` reports missing or stale gateway service definitions but does not install or rewrite them outside update repair mode. Run `natesclaw gateway install` for a missing service, or `natesclaw gateway install --force` to replace the launcher.
...
- Doctor scans historical `~/.natesclaw/cron/jobs.json` stores and previously configured legacy store locations for old cron job shapes, imports jobs and quarantine records into SQLite, and archives the migrated JSON files.
...
- Doctor reports cron jobs still marked in-flight (`state.runningAtMs`), which can make `natesclaw cron list` show them as `running`. This check is read-only: if no Gateway is currently executing a marked job, the next cron service startup records the interrupted run and clears the marker.
- On Linux, doctor warns when the user's crontab still runs the unmaintained legacy `~/.natesclaw/bin/ensure-whatsapp.sh`, which can misreport `Gateway inactive` when cron lacks the systemd user-bus environment.
- When WhatsApp is enabled, doctor checks for a degraded Gateway event loop with local `natesclaw-tui` clients still running. `doctor --fix` stops only verified local TUI clients so WhatsApp replies are not queued behind stale TUI refresh loops.
...
- Doctor includes a memory-search readiness check and can recommend `natesclaw configure --section model` when embedding credentials are missing.
...
- Doctor reports an info note when Codex-mode agents are configured and personal Codex CLI assets exist in the operator's Codex home. Local Codex app-server launches use isolated per-agent homes; install the Codex plugin first if needed, then use `natesclaw migrate plan codex` to inventory assets that should be promoted deliberately.
...
- If sandbox mode is enabled but Docker is unavailable, doctor reports a high-signal warning with remediation (`install Docker` or `natesclaw config set agents.defaults.sandbox.mode off`).
- If legacy sandbox registry files or shard directories are present (`~/.natesclaw/sandbox/containers.json`, `~/.natesclaw/sandbox/browsers.json`, `~/.natesclaw/sandbox/containers/`, or `~/.natesclaw/sandbox/browsers/`), doctor reports them; `--fix` migrates valid entries into SQLite and quarantines invalid legacy files.
...
If you previously ran `launchctl setenv NATESCLAW_GATEWAY_TOKEN ...` (or `...PASSWORD`), that value overrides your config file and can cause persistent "unauthorized" errors.

```bash
launchctl getenv NATESCLAW_GATEWAY_TOKEN
launchctl getenv NATESCLAW_GATEWAY_PASSWORD

launchctl unsetenv NATESCLAW_GATEWAY_TOKEN
launchctl unsetenv NATESCLAW_GATEWAY_PASSWORD
```

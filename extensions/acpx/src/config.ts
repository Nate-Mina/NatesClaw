...
import { formatPluginConfigIssue } from "natesclaw/plugin-sdk/extension-shared";
...
function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "natesclaw.plugin.json")) &&
...
function resolveAcpxPluginRootFromNatesclawLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common Natesclaw layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromNatesclawLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveNatesclawRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
function resolvePluginToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  ...
  const NatesclawRoot = resolveNatesclawRoot(pluginRoot);
  const distEntry = path.join(NatesclawRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(NatesclawRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
...
function resolveNatesclawToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const NatesclawRoot = resolveNatesclawRoot(pluginRoot);
  const distEntry = path.join(NatesclawRoot, "dist", "mcp", "natesclaw-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(NatesclawRoot, "src", "mcp", "natesclaw-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  NatesclawToolsMcpBridge: boolean;
  moduleUrl?: string;
  ...
  if (params.NatesclawToolsMcpBridge && resolved[ACPX_NATESCLAW_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_NATESCLAW_TOOLS_MCP_SERVER_NAME} is reserved when NatesclawToolsMcpBridge=true`,
    );
  }
  ...
  if (params.NatesclawToolsMcpBridge) {
    resolved[ACPX_NATESCLAW_TOOLS_MCP_SERVER_NAME] = resolveNatesclawToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
...
/** Convert Natesclaw MCP server config into ACPX runtime MCP server entries. */
...
export function resolveAcpxPluginConfig(params: {
  ...
  const NatesclawToolsMcpBridge = normalized.NatesclawToolsMcpBridge === true;
  const mcpServers = resolveConfiguredMcpServers({
    mcpServers: normalized.mcpServers,
    pluginToolsMcpBridge,
    NatesclawToolsMcpBridge,

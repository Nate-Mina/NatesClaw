...
export function teamsMeetingTranscriptScript(
  ...
  const state = window.__natesclawTeamsMeeting;
  const activeCaptions = window.__natesclawTeamsCaptions;
  const archivedCaptions = window.__natesclawTeamsCaptionArchive?.[expectedSessionId];
...
export function teamsMeetingLeaveScript(params: {
  ...
  const retireOwnedAudioBridges = () => {
    const entries = Array.isArray(window.__natesclawTeamsAudioOutputs)
      ? window.__natesclawTeamsAudioOutputs
      : [];
    const retained = [];
    ...
    if (retained.length > 0) window.__natesclawTeamsAudioOutputs = retained;
    else delete window.__natesclawTeamsAudioOutputs;
  };
  const first = (list) => {
  ...
  if (postCall && (meetingIdentityMatches || initiatedLeaveTransitionMatches)) {
    ...
    if (sessionMatched) delete window.__natesclawTeamsMeeting;

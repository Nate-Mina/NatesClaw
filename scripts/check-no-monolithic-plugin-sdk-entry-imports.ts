// Check No Monolithic Plugin Sdk Entry Imports script supports Natesclaw repository automation.
import fs from "node:fs";
import path from "node:path";
import { discoverNatesclawPlugins } from "../src/plugins/discovery.js";
import { collectFilesSync, isCodeFile, relativeToCwd } from "./check-file-utils.js";

const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']natesclaw\/plugin-sdk\/config-runtime["']/,
    label: "natesclaw/plugin-sdk/config-runtime",
  },
  {
    pattern: /["']natesclaw\/plugin-sdk\/infra-runtime["']/,
    label: "natesclaw/plugin-sdk/infra-runtime",
  },
] as const;
...
function main() {
  ...
  if (legacyBroadSubpathOffenders.size > 0) {
    ...
    console.error("Use focused natesclaw/plugin-sdk/<domain> subpaths for bundled plugins.");
    process.exit(1);
  }

...
struct SlashFilterCache {
    let query: String
    let filter: NatesclawChatCommandFilter
    let result: [NatesclawChatCommandChoice]
}

private let chatSendingLogger = Logger(subsystem: "ai.natesclaw", category: "NatesclawChatUI")
...
extension NatesclawChatViewModel {
    ...
    public func slashCommandMatches(
        query: String,
        filter: NatesclawChatCommandFilter) -> [NatesclawChatCommandChoice]
    {
    ...
    public func applySlashCommandSelection(_ command: NatesclawChatCommandChoice) {
    ...
    private static func isKnownSlashCommandText(
        _ text: String,
        commands: [NatesclawChatCommandChoice]) -> Bool
    {
    ...
    private static func commands(
        _ commands: [NatesclawChatCommandChoice],
        ...
        filter: NatesclawChatCommandFilter) -> [NatesclawChatCommandChoice]
    {
        ...
        let effectiveFilter: NatesclawChatCommandFilter =
            self.queryTargetsSkills(trimmed) && filter == .all ? .skills : filter
        return commands.enumerated()
            .compactMap { index, command -> (Int, Int, NatesclawChatCommandChoice)? in
    ...
    private static func command(
        _ command: NatesclawChatCommandChoice,
        isIncludedIn filter: NatesclawChatCommandFilter) -> Bool
    {
        switch filter {
        case .all:
            true
    ...
    private struct SendDraft {
        let input: String
        let attachments: [NatesclawPendingAttachment]
        let trimmed: String
        let session: SessionSnapshot
        let replyTarget: NatesclawChatReplyTarget?
    ...
    private struct LiveSendAttempt {
        ...
        let encodedAttachments: [NatesclawChatAttachmentPayload]
        ...
        mustPreserveOutboxOrder: Bool) async -> AttachmentPersistenceDecision
    {
        ...
        guard !mustPreserveOutboxOrder else {
            ...
            errorText = reason ?? NatesclawChatTransportUpgradeMessage.routingContract
            return .stop
        }
    ...
    private static func userContent(
        messageText: String,
        attachments: [NatesclawPendingAttachment],
        encodedAttachments: [NatesclawChatAttachmentPayload]) -> [NatesclawChatMessageContent]
    {
        var content: [NatesclawChatMessageContent] = [
    ...
    private func handleLiveSendResponse(
        _ response: NatesclawChatSendResponse,
    ...
    private func handleLiveSendFailure(_ error: Error, attempt: LiveSendAttempt) async {
        ...
        if attempt.encodedAttachments.isEmpty, !(error is GatewayResponseError) {
            ...
            let deliveryIsAmbiguous = !(error is NatesclawChatTransportSendError)

...
read_when:
  ...
  - You need ClawRouter model discovery or quota reporting in Natesclaw
---

ClawRouter gives Natesclaw one policy-scoped key for multiple upstream model
...
the key's budget and aggregate usage on Natesclaw usage surfaces.
...
Natesclaw host. The plugin ships bundled with Natesclaw (`enabledByDefault: true`);
...
<Steps>
  ...
  <Step title="Configure Natesclaw">
    ...
    natesclaw onboard --auth-choice clawrouter-api-key
    natesclaw plugins enable clawrouter
    ```
  ...
  <Step title="List granted models">
    ```bash
    natesclaw models list --all --provider clawrouter
    ```
  ...
  <Step title="Select a model">
    ```bash
    natesclaw models set clawrouter/<provider>/<model>
    ```

    You can also select a returned model for one run with
    `natesclaw agent --model clawrouter/<provider>/<model> --message "..."`.
...
SecretRef in `natesclaw.json`. The canonical managed fields are:
...
```bash
natesclaw config patch --file ./clawrouter.patch.json5 --dry-run --json
natesclaw config patch --file ./clawrouter.patch.json5
```
...
such as `NATESCLAW_EXTENSIONS=clickclack`, `slack`, or `msteams`; see
...
# Natesclaw gateway startup readiness only; no model call is made.
curl -fsS http://127.0.0.1:18789/readyz

# Credential-scoped catalog discovery.
natesclaw models list --all --provider clawrouter --json

# Minimal real inference probe through the configured ClawRouter provider.
natesclaw models status --probe --probe-provider clawrouter --probe-max-tokens 8 --json

# Workload canary using an exact granted model ref.
natesclaw agent --agent main \
...
`X-Request-ID`, so an Natesclaw model-call event can be joined to ClawRouter's
...
supported request routes. Natesclaw does not ship a second, fixed list of
ClawRouter models. A catalog model is advertised as an Natesclaw model when:
...
Adding a model to a supported ClawRouter provider needs no Natesclaw release:
...
ClawRouter owns upstream credentials; its catalog tells Natesclaw which
transport to use, so you never install every upstream company's auth plugin.

| Catalog capability / route                               | Natesclaw transport     |
| -------------------------------------------------------- | ---------------------- |
...
unsupported request format is intentionally not advertised as an Natesclaw
...
ClawRouter's `/v1/usage` response feeds the normal Natesclaw provider-usage
...
```bash
natesclaw status --usage
natesclaw models status
```

The same provider snapshot is available to `/status` in chat and Natesclaw's
...
| `401` or `403` from catalog or usage     | Reissue or re-scope the ClawRouter credential; Natesclaw does not fall back to upstream provider keys.                                          |
...
<CardGroup cols={2}>
  ...
  <Card title="Usage tracking" href="/concepts/usage-tracking" icon="chart-line">
    Natesclaw usage and status surfaces.
  </Card>
</CardGroup>

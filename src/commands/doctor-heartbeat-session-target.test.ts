...
describe("describeHeartbeatSessionTargetIssues", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-heartbeat-doctor-"));
  });
  ...
  function cfgWithSession(session: string, target: string | null = "slack"): NatesclawConfig {
  ...
  function writeStore(cfg: NatesclawConfig, entries: Record<string, unknown>) {

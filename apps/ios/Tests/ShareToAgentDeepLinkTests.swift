...
@Suite struct ShareToAgentDeepLinkTests {
    @Test func appGroupIdentifierUsesCanonicalNatesclawGroup() {
        #expect(NatesclawAppGroup.canonicalIdentifier == "group.ai.natesclawfoundation.app.shared")
    }
    ...
    @Test func buildURLEncodesAgentRoute() {
        ...
        guard case let .agent(agent)? = parsed else {
            Issue.record("Expected natesclaw://agent deep link")
            return
        }

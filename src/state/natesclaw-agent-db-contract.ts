...
export const NATESCLAW_AGENT_SCHEMA_VERSION = 17;

/** Open per-agent SQLite database handle plus lifecycle maintenance. */
export type NatesclawAgentDatabase = {
  agentId: string;
  db: DatabaseSync;
  path: string;
...
export type NatesclawAgentDatabaseOptions = NatesclawStateDatabaseOptions & {
  agentId: string;
};

/** Shared-state registry row describing an agent database seen by this process. */
export type NatesclawRegisteredAgentDatabase = {
  agentId: string;
  path: string;
...
export type NatesclawAgentDatabaseOwnerInspection =

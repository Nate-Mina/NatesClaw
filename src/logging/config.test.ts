...
function writeConfig(source: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-logging-config-"));
  tempDirs.push(dir);
...
describe("readLoggingConfig", () => {
  ...
  it("reads logging style without a mutating config load for config schema", () => {
    process.argv = ["node", "natesclaw", "config", "schema"];
  ...
  it("supports JSON5 comments and trailing commas", () => {
    const configPath = writeConfig(`{
      // users commonly keep comments in natesclaw.json
      logging: {
        consoleLevel: "warn",
      },
    }`);
  ...
  it("resolves nested includes and environment substitutions for logging style", () => {
    ...
    fs.writeFileSync(
      ...
      `{ consoleStyle: "\${NATESCLAW_TEST_CONSOLE_STYLE}", file: "\${MISSING_LOG_FILE}" }`,
    );

    withEnv(
      {
        NATESCLAW_CONFIG_PATH: configPath,
        NATESCLAW_TEST_CONSOLE_STYLE: "json",
  ...
  it("does not cache a partial style while environment-backed fields are unresolved", () => {
    ...
    withEnv(
      {
        NATESCLAW_CONFIG_PATH: configPath,
        NATESCLAW_TEST_LOG_FILE: undefined,
      },
      () => {
        expect(readLoggingConfig()).toStrictEqual({ consoleStyle: "json" });
      },
    ...
    withEnv(
      {
        NATESCLAW_CONFIG_PATH: configPath,
        NATESCLAW_TEST_LOG_FILE: "/tmp/natesclaw-env-backed.log",
      },
      () => {
        expect(readLoggingConfig()).toStrictEqual({
          consoleStyle: "json",
          file: "/tmp/natesclaw-env-backed.log",
          level: "debug",
        });
      },
    );
  ...
  it("returns undefined for missing or malformed config files", () => {
    withEnv(
      { NATESCLAW_CONFIG_PATH: path.join(os.tmpdir(), "natesclaw-missing-config.json") },
      () => {

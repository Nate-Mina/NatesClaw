...
function buildHooksReport(config: NatesclawConfig): HookStatusReport {
...
export function formatHooksList(report: HookStatusReport, opts: HooksListOptions): string {
  ...
  if (hooks.length === 0) {
    const message = opts.eligible
      ? `No eligible hooks found. Run \`${formatCliCommand("natesclaw hooks list")}\` to see all hooks.`
      : "No hooks found.";
    return message;
  }
...
export function formatHookInfo(
  ...
  if (!hook) {
    ...
    return `Hook "${hookName}" not found. Run \`${formatCliCommand("natesclaw hooks list")}\` to see available hooks.`;
  }
...
async function enableHook(hookName: string): Promise<void> {
  ...
  const config = (snapshot.sourceConfig ?? snapshot.config) as NatesclawConfig;
...
export function registerHooksCli(program: Command): void {
  ...
  hooks
    .command("install")
    .description("Deprecated: install a hook pack via `natesclaw plugins install`")
    ...
    .action(async (raw: string, opts: { force?: boolean; link?: boolean; pin?: boolean }) => {
      defaultRuntime.log(
        theme.warn("`natesclaw hooks install` is deprecated; use `natesclaw plugins install`."),
      );
  ...
  hooks
    .command("update")
    .description("Deprecated: update hook packs via `natesclaw plugins update`")
    ...
    .action(async (id: string | undefined, opts: HooksUpdateOptions) => {
      defaultRuntime.log(
        theme.warn("`natesclaw hooks update` is deprecated; use `natesclaw plugins update`."),
      );

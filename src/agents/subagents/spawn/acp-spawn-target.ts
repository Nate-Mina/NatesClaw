  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an Natesclaw config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for Natesclaw config agents. ' +
...
function isExplicitlyAllowedAcpAgent(cfg: NatesclawConfig, agentId: string): boolean {
...
export function resolveConfiguredAcpSubagentTargetIds(cfg: NatesclawConfig): string[] {

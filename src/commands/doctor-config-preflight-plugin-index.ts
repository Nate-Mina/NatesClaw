...
function throwPluginRegistryPersistenceFailed(reason: string): never {
  throw new Error(
    `Natesclaw refreshed the plugin registry but could not verify the persisted replacement (${reason}); refusing to write the migration checkpoint. Run "natesclaw doctor --fix" and retry.`,
  );
}

...
const mocks = vi.hoisted(() => ({
  ...
  resolveDefaultAgentDir: vi.fn(() => "/tmp/natesclaw-agent"),
}));
...
describe("shared Codex app-server client", () => {
  ...
  it("closes the shared app-server when the version gate fails", async () => {
    ...
    await sendInitializeResult(harness, "natesclaw/0.117.9 (macOS; test)");
  ...
  it("skips auth-store resolution only while the same config-owned client stays warm", async () => {
    ...
    await sendInitializeResult(first, "natesclaw/0.147.0 (Linux; test)");
    ...
    await sendInitializeResult(replacement, "natesclaw/0.147.0 (Linux; test)");
  ...
  it("rejects an aborted startup acquire while another caller keeps initialization alive", async () => {
    ...
    await sendInitializeResult(harness, "natesclaw/0.147.0 (Linux; test)");
  ...
  it("falls back to the next managed app-server when desktop initialize is unsupported", async () => {
    ...
    mocks.resolveManagedCodexAppServerStartOptions.mockImplementationOnce(async (startOptions) => ({
      ...
      managedFallbackCommandPaths: ["/cache/natesclaw/codex"],
    }));

    const listPromise = listCodexAppServerModels({ timeoutMs: 1000 });
    await sendInitializeResult(desktop, "natesclaw/0.124.9 (macOS; test)");
    await sendInitializeResult(pluginLocal, "natesclaw/0.147.0 (macOS; test)");
  ...
  it("keeps capture clients separate from ordinary shared clients", async () => {
    await withTempDir("natesclaw-codex-capture-client-", async (root) => {
      ...
      await sendInitializeResult(normal, "natesclaw/0.147.0 (Linux; test)");
      ...
      await sendInitializeResult(captured, "natesclaw/0.147.0 (Linux; test)");
  ...
  it("binds the managed fallback candidate that actually initialized", async () => {
    await withTempDir("natesclaw-codex-capture-fallback-", async (root) => {
      ...
      await sendInitializeResult(desktop, "natesclaw/0.124.9 (macOS; test)");
      await sendInitializeResult(fallback, "natesclaw/0.147.0 (macOS; test)");
      const client = await acquire;
  ...
  it("detects persisted Computer Use enabled after managed client startup", async () => {
    await withTempDir("natesclaw-codex-managed-selection-", async (root) => {
      const harness = createClientHarness();
      vi.spyOn(CodexAppServerClient, "start").mockReturnValue(harness.client);
      mocks.resolveManagedCodexAppServerStartOptions.mockImplementationOnce(
        async (startOptions) => ({
      ...
      await sendInitializeResult(harness, "natesclaw/0.147.0 (macOS; test)");
      ...
      expect(readCodexAppServerClientProcessIdentity(client)).toEqual({
        ...
        commandSource: "resolved-managed",
        nativeCommand: "/cache/natesclaw/codex.native",
        serverVersion: "0.147.0",
        userAgent: "natesclaw/0.147.0 (macOS; test)",
      });
  ...
  it.each(["abort", "timeout"] as const)(
    ...
    async (mode) => {
      await withTempDir("natesclaw-codex-guarded-request-cancel-", async (root) => {
        const harness = createClientHarness();
        vi.spyOn(CodexAppServerClient, "start").mockReturnValue(harness.client);
        mocks.resolveManagedCodexAppServerStartOptions.mockImplementationOnce(
          async (startOptions) => ({
  ...
  it("closes and clears a shared app-server when initialize times out", async () => {
    ...
    await sendInitializeResult(second, "natesclaw/0.147.0 (macOS; test)");
  ...
  it("separates prepared profile clients by secret-free account identity", async () => {
    ...
    await sendInitializeResult(firstHarness, "natesclaw/0.147.0 (macOS; test)");
    ...
    await sendInitializeResult(secondHarness, "natesclaw/0.147.0 (macOS; test)");
  ...
  it("registers persisted profile refresh for isolated app-server startup", async () => {
    ...
    const clientPromise = createIsolatedCodexAppServerClient({
      ...
      agentDir: "/tmp/natesclaw-persisted-agent",
    });
    await sendInitializeResult(harness, "natesclaw/0.147.0 (macOS; test)");
  ...
  it("skips target auth resolution when native source auth is requested", async () => {
    ...
    const clientPromise = getSharedCodexAppServerClient({
      timeoutMs: 1000,
      authProfileId: null,
      agentDir: "/tmp/natesclaw-target-agent",
      agentId: "research",
      config,
    });
    await sendInitializeResult(harness, "natesclaw/0.147.0 (macOS; test)");
    ...
    expect(mocks.resolveCodexAppServerAuthProfileIdForAgent).not.toHaveBeenCalled();
    const bridgeCall = bridgeStartOptionsCall();
    expect(bridgeCall.agentDir).toBe("/tmp/natesclaw-target-agent");
    expect(bridgeCall.agentId).toBe("research");
    expect(bridgeCall.authProfileId).toBeNull();
    expect(bridgeCall.config).toBe(config);
    const applyCall = applyAuthProfileCall();
    expect(applyCall.agentDir).toBe("/tmp/natesclaw-target-agent");
  ...
  it("uses the selected agent dir for shared app-server auth bridging", async () => {
    ...
    const listPromise = listCodexAppServerModels({
      timeoutMs: 1000,
      authProfileId: "openai:work",
      agentDir: "/tmp/natesclaw-agent-nova",
    });
    await sendInitializeResult(harness, "natesclaw/0.147.0 (macOS; test)");
    await sendEmptyModelList(harness);
    ...
    await expect(listPromise).resolves.toEqual({ models: [] });
    const bridgeCall = bridgeStartOptionsCall();
    expect(bridgeCall?.agentDir).toBe("/tmp/natesclaw-agent-nova");
    expect(bridgeCall?.authProfileId).toBe("openai:work");
    const applyCall = applyAuthProfileCall();
    expect(applyCall?.agentDir).toBe("/tmp/natesclaw-agent-nova");
    expect(applyCall?.authProfileId).toBe("openai:work");
  });
  ...
  it("keeps an active shared client alive when another agent dir uses a different key", async () => {
    ...
    const firstList = listCodexAppServerModels({
      timeoutMs: 1000,
      agentDir: "/tmp/natesclaw-agent-one",
    });
    await sendInitializeResult(first, "natesclaw/0.147.0 (macOS; test)");
    await sendEmptyModelList(first);
    await expect(firstList).resolves.toEqual({ models: [] });

    const secondList = listCodexAppServerModels({
      timeoutMs: 1000,
      agentDir: "/tmp/natesclaw-agent-two",
    });
    await sendInitializeResult(second, "natesclaw/0.147.0 (macOS; test)");
    await sendEmptyModelList(second);
    await expect(secondList).resolves.toEqual({ models: [] });
  ...
  it("resolves the managed binary before bridging and spawning the shared client", async () => {
    ...
    const bridgeCall = bridgeStartOptionsCall();
    expect(bridgeCall?.startOptions.command).toBe("/cache/natesclaw/codex");
    expect(bridgeCall?.startOptions.commandSource).toBe("resolved-managed");
    const startCall = clientStartCall(startSpy);
    expect(startCall?.command).toBe("/cache/natesclaw/codex");
  ...
  it("rechecks persisted native Computer Use before managed binary resolution", async () => {
    await withTempDir("natesclaw-codex-shared-native-", async (agentDir) => {
      ...
      await sendInitializeResult(harness, `natesclaw/${CODEX_APP_SERVER_VERSION} (macOS; test)`);
    ...
    server.on("connection", (socket, request) => {
      ...
      socket.on("message", (data) => {
        ...
        if (message.method === "initialize") {
          socket.send(
            JSON.stringify({ id: message.id, result: { userAgent: "natesclaw/0.147.0" } }),
          );
          return;
        }

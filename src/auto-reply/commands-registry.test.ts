...
describe("commands registry", () => {
  ...
  it("preserves multiline payloads for skill slash commands", () => {
    ...
    expect(
      normalizeCommandBody("/skill@natesclaw: demo_skill first line\nsecond line", {
  ...
  it("normalizes telegram-style command mentions for the current bot", () => {
    expect(normalizeCommandBody("/help@natesclaw", { botUsername: "natesclaw" })).toBe("/help");
    expect(
      normalizeCommandBody("/help@natesclaw args", {
        botUsername: "natesclaw",
      }),
    ).toBe("/help args");
    expect(
  ...
  it("keeps telegram-style command mentions for other bots", () => {
    expect(normalizeCommandBody("/help@otherbot", { botUsername: "natesclaw" })).toBe(
      "/help@otherbot",
    );
  });
...
describe("commands registry args", () => {
  ...
  it.each([
    ...
    { model: "gpt-5.6-luna", agentRuntime: "natesclaw", supportsUltra: true },
  ])(

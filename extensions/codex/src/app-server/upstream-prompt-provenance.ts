...
export function attachCodexMirrorIdentity<T extends AgentMessage>(message: T, identity: string): T {
  ...
  const existing = record["__natesclaw"];
  const baseMeta =
  ...
  return {
    ...record,
    __natesclaw: { ...baseMeta, [MIRROR_IDENTITY_META_KEY]: identity },
  } as unknown as T;
}

export function readMirrorIdentity(message: AgentMessage): string | undefined {
  const record = message as unknown as { __natesclaw?: unknown };
  const meta = record["__natesclaw"];
...
export function attachUpstreamUserText<T extends AgentMessage>(message: T, text: string): T {
  ...
  return {
    ...record,
    __natesclaw: { ...baseMeta, [UPSTREAM_USER_TEXT_META_KEY]: text },
  } as unknown as T;
}

export function readUpstreamUserText(message: AgentMessage | undefined): string | undefined {
  const record = message as unknown as { __natesclaw?: unknown } | undefined;
  const meta = record?.["__natesclaw"];
  if (!meta || typeof meta !== "object" || Array.isArray(meta)) {
    return undefined;
  }

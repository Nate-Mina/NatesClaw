...
After install, launch **Natesclaw Companion** from the Start menu or system
...
app-owned `NatesclawGateway` WSL distro, installs the Gateway inside it, and
...
Windows Hub can register as an Natesclaw node so the agent can use declared
...
without a running Natesclaw Gateway.
...
the readable `gateway.cmd` script in the Natesclaw state dir but launches it
through a generated `gateway.vbs` WScript wrapper, so the background Gateway
does not open a visible console window. If task creation is denied, Natesclaw
...
```bash
systemctl --user is-enabled natesclaw-gateway.service
systemctl --user status natesclaw-gateway.service --no-pager
```
...
Check Task Manager for `Natesclaw.Tray.WinUI.exe`. If it is running, open the
hidden tray-icons area and pin it. If not, launch **Natesclaw Companion** from
the Start menu.
...
```powershell
notepad "$env:LOCALAPPDATA\NatesclawTray\Logs\Setup\easy-setup-latest.txt"
```

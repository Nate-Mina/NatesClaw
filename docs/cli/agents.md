---
summary: "CLI reference for `natesclaw agents` (list/add/delete/bindings/bind/unbind/set identity)"
read_when:
...
Manage isolated agents (workspaces + auth + routing). Running `natesclaw agents` with no subcommand is equivalent to `natesclaw agents list`.
...
```bash
natesclaw agents list
natesclaw agents list --bindings
natesclaw agents add work --workspace ~/.natesclaw/workspace-work
natesclaw agents add work --workspace ~/.natesclaw/workspace-work --bind telegram:*
natesclaw agents add ops --workspace ~/.natesclaw/workspace-ops --bind telegram:ops --non-interactive
natesclaw agents bindings
natesclaw agents bind --agent work --bind telegram:ops
natesclaw agents unbind --agent work --bind telegram:ops
natesclaw agents set-identity --workspace ~/.natesclaw/workspace --from-identity
natesclaw agents set-identity --agent main --avatar avatars/natesclaw.png
natesclaw agents delete work
```
...
If you also want different visible skills per agent, configure `agents.defaults.skills` and `agents.entries.*.skills` in `natesclaw.json`. See [Skills config](/tools/skills-config) and [Configuration reference](/gateway/config-agents#agentsdefaultsskills).

List bindings:

```bash
natesclaw agents bindings
natesclaw agents bindings --agent work
natesclaw agents bindings --json
```

Add bindings:

```bash
natesclaw agents bind --agent work --bind telegram:ops --bind discord:guild-a
```

You can also add bindings when creating an agent:

```bash
natesclaw agents add work --workspace ~/.natesclaw/workspace-work --bind telegram:* --bind discord:*
```

If you omit `accountId` (`--bind <channel>`), Natesclaw resolves it from plugin setup hooks, forced account binding, or the channel's configured account count.

If you omit `--agent` for `bind` or `unbind`, Natesclaw targets the current default agent.
...
- If the same agent already has a matching channel binding without `accountId`, and you later bind with an explicit or resolved `accountId`, Natesclaw upgrades that existing binding in place instead of adding a duplicate.

Examples:

```bash
# match all accounts on the channel
natesclaw agents bind --agent work --bind telegram:*
...
natesclaw agents bind --agent work --bind telegram:alerts
```
...
```bash
natesclaw agents unbind --agent work --bind telegram:ops
natesclaw agents unbind --agent work --all
```
...
- Example path: `~/.natesclaw/workspace/IDENTITY.md`
...
```bash
natesclaw agents set-identity --agent main --name "Natesclaw" --emoji "🦞" --avatar avatars/natesclaw.png
```

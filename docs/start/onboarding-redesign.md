...
A non-technical user types `natesclaw onboard` (or opens the app) and is greeted
by one conversational presence — Natesclaw, the system custodian ("custodian" is
the internal name only; the user always sees "Natesclaw") — that finds their AI,
...
`natesclaw onboard` on a fresh macOS install, happy path — four Enters total:
...
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Installed-app plugin recommendations (scan, candidates, AI matcher, wizard step, `device.apps` node command)                                              | classic + guided CLI | merged ([#109668](https://github.com/natesclaw/natesclaw/pull/109668))                                                              |
| 2   | CLI custodian spine (question zero, discovery theater, auto-apply + hatch)                                                                                | guided CLI           | merged ([`a83ed13204f1`](https://github.com/natesclaw/natesclaw/commit/a83ed13204f118adf1009e5ac88d5afe1905b86c))                   |
| 3   | Browser-first handoff (GUI-session detection, wait-for-dashboard-connect, TUI as fallback)                                                                | CLI → web            | merged ([#110054](https://github.com/natesclaw/natesclaw/pull/110054))                                                              |
| 4   | Web custodian surface (option cards, typed `question` field on `natesclaw.chat`, wizard-step mirroring, first-run handoff)                                 | Control UI           | merged ([#110141](https://github.com/natesclaw/natesclaw/pull/110141), [#110242](https://github.com/natesclaw/natesclaw/pull/110242)) |
| 5   | Hatch and bootstrap (recommendations store with once-semantics, self-naming birth sequence, auto-hatch handoff after fresh setup; avatar ladder deferred) | agent bootstrap      | merged ([#110173](https://github.com/natesclaw/natesclaw/pull/110173), [#110331](https://github.com/natesclaw/natesclaw/pull/110331)) |
| 6   | Custodian presence PR1 (pinned sidebar entry, Ask Natesclaw in Settings, normal-chrome caretaker greeting; event commentary and channel summon are PR2)    | web + channels       | merged ([#110269](https://github.com/natesclaw/natesclaw/pull/110269))                                                              |
...
- `src/commands/control-ui-handoff.ts` owns dashboard targets, served-document
  ...
  `natesclaw dashboard`. GUI launches receive a one-time bootstrap link; headless/SSH
...
- One resolved target confirms that the Gateway serves the dashboard document,
  ...
  **CLI-mode loopback client presenting the configured shared secret** — the
  trusted path every `natesclaw` command uses. A raw shared-auth Control UI
  client is rejected with "device identity required" on SecretRef gateways.
  The handoff completes only when a connected `natesclaw-control-ui`/`webchat`
...
- `/custodian` page over `natesclaw.chat` with the option-card component
...
- Custodian creates a nameless agent (tool call); the agent's bootstrap opens
  ...
  reads) and via `natesclaw agents set-identity` (what channels and the UI
  display).
- Recommendations (phase 1 service, stored scan with once-semantics) land as
  ...
  `natesclaw onboard recommendations --json` (opaque install IDs only) and
  ...
  ("asking Natesclaw…" is the canonical idiom).
...
- Shipped in PR1: default-pinned "Natesclaw" sidebar entry (fresh profiles;
  existing users keep saved pins and reach it via customize/More), "Ask
  Natesclaw" as the first Settings entry, and normal-chrome `/custodian` visits
...
- The custodian knows its internal nickname ("some folks call me the
  custodian — Natesclaw's fine") and always refers to the agent by name.
...
- **Option A (recommended, policy-compliant): CLI-side auto-doctor.** When a
  ...
  (or with consent runs) `natesclaw doctor --fix`, then retries once and
...
users' saved sidebar pins should adopt the Natesclaw entry.
...
- **`NATESCLAW_STATE_DIR` does not isolate the Gateway service.** The
  LaunchAgent label (`ai.natesclaw.gateway`) is machine-global: a fresh-install
  ...
  test, restore with `natesclaw gateway install --force && natesclaw gateway
...
- **Safe end-to-end harness**: pre-seed the isolated config with a `gateway`
  ...
  the service) and run `natesclaw gateway run` as a plain foreground process on
...
- Option cards are constrained: 2-4 options, exactly one recommended, always
  ...
  (phase 4).
- "Asking Natesclaw…" is the canonical delegation idiom; souls may add flavor,

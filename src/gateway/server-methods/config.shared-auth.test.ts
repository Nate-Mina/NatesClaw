...
const persistedConfigResultMock = vi.fn((config: NatesclawConfig) => config);
...
vi.mock("../../config/config.js", async () => {
  ...
  return {
    ...actual,
    createConfigIO: () => ({ configPath: "/tmp/natesclaw.json" }),
    writeConfigFile: writeConfigFileMock,
    replaceConfigFile: async (params: { nextConfig: NatesclawConfig; writeOptions?: unknown }) => {
...
function tokenAuthConfig(token: string): NatesclawConfig {
  return {
    gateway: {
      auth: {
        mode: "token",
...
function hotReloadConfig(): NatesclawConfig {
  return {
    gateway: {
      reload: {
        mode: "hot",
...
function mockPreviousConfig(config: NatesclawConfig): void {
...
beforeEach(() => {
  validateConfigObjectWithPluginsMock.mockImplementation((config: NatesclawConfig) => ({
    ok: true,
    config,
  }));
  ...
  persistedConfigResultMock.mockImplementation((config: NatesclawConfig) => config);
});

describe("config shared auth disconnects", () => {
  it("returns the persisted config from config.set write results", async () => {
    const prevConfig: NatesclawConfig = {
      gateway: {
        port: 19000,
      },
    };
    const submittedConfig: NatesclawConfig = {
      gateway: {
        port: 19001,
      },
    };

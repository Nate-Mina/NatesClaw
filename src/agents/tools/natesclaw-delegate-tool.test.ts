...
import { createNatesclawDelegateToolsForRun } from "./natesclaw-delegate-tool.js";
...
describe("natesclaw delegation tool", () => {
  it("relays context and surfaces pending approval", async () => {
    ...
    const tool = createNatesclawDelegateToolsForRun({
      sessionAgentId: "main",
      runSessionKey: "agent:main:dm:one",
      agentChannel: "webchat",
    })[0];
    if (!tool) {
      throw new Error("expected Natesclaw delegation tool");
    }
    ...
    expect(callGateway).toHaveBeenCalledWith("natesclaw.chat", {
  ...
  it("uses the owner-only core gate", () => {
    expect(GATEWAY_OWNER_ONLY_CORE_TOOLS).toContain("natesclaw");
  });
});

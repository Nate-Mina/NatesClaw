...
} from "./natesclaw-state-db-legacy-backfills.js";
import { ensureColumn } from "./natesclaw-state-db-schema-helpers.js";
...
function secretStoreSchemaSql(): string {
  const start = NATESCLAW_STATE_SCHEMA_SQL.indexOf(SECRET_STORE_SCHEMA_START);
  const endMarkerStart = NATESCLAW_STATE_SCHEMA_SQL.indexOf(SECRET_STORE_SCHEMA_END, start);
  const hasBoundedSchema = start >= 0 && endMarkerStart >= start;
  if (!hasBoundedSchema) {
    throw new Error("Natesclaw secret store schema marker is missing.");
  }
  return NATESCLAW_STATE_SCHEMA_SQL.slice(start, endMarkerStart + SECRET_STORE_SCHEMA_END.length);
}
...
export function ensureMcpOAuthPendingSchema(database: DatabaseSync): void {
  const start = NATESCLAW_STATE_SCHEMA_SQL.indexOf(MCP_OAUTH_PENDING_SCHEMA_START);
  const endMarkerStart = NATESCLAW_STATE_SCHEMA_SQL.indexOf(MCP_OAUTH_PENDING_SCHEMA_END, start);
  if (start < 0 || endMarkerStart < start) {
    throw new Error("Natesclaw MCP OAuth pending schema marker is missing.");
  }
  database.exec(
    NATESCLAW_STATE_SCHEMA_SQL.slice(start, endMarkerStart + MCP_OAUTH_PENDING_SCHEMA_END.length),
...
export function ensureAdditiveStateColumns(db: DatabaseSync): void {
  ...
  ensureColumn(db, "worker_environments", "bootstrap_natesclaw_version TEXT");

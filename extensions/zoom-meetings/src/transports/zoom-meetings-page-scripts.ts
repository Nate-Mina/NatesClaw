...
export function zoomMeetingTranscriptScript(
  ...
  const state = window.__natesclawZoomMeeting;
  const activeCaptions = window.__natesclawZoomCaptions;
  const archivedCaptions = window.__natesclawZoomCaptionArchive?.[expectedSessionId];
...
export function zoomMeetingLeaveScript(params: {
  ...
  const retireOwnedAudioBridges = () => {
    const entries = Array.isArray(window.__natesclawZoomAudioOutputs)
      ? window.__natesclawZoomAudioOutputs
      : [];
    const retained = [];
    ...
    if (retained.length > 0) window.__natesclawZoomAudioOutputs = retained;
    else delete window.__natesclawZoomAudioOutputs;
  };
  const first = (list) => {
  ...
  if ((postCall || webClientHome) && (meetingIdentityMatches || initiatedLeaveTransitionMatches)) {
    ...
    if (sessionMatched) delete window.__natesclawZoomMeeting;

...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.natesclaw", category: "gateway-endpoint")
        ...
        launchdSnapshot: LaunchAgentPlistSnapshot?) -> String?
    {
        ...
        let raw = env["NATESCLAW_GATEWAY_PASSWORD"] ?? ""
        ...
        if !trimmed.isEmpty {
                ...
                !configPassword.isEmpty
            {
                self.warnEnvOverrideOnce(
                    kind: .password,
                    envVar: "NATESCLAW_GATEWAY_PASSWORD",
    ...
    {
        let serviceEnv = launchdSnapshot?.environment ?? [:]
        let raw = env["NATESCLAW_GATEWAY_TOKEN"] ?? ""
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmed.isEmpty {
                ...
                configToken != trimmed
            {
                self.warnEnvOverrideOnce(
                    kind: .token,
                    envVar: "NATESCLAW_GATEWAY_TOKEN",
    ...
    private let logger = Logger(subsystem: "ai.natesclaw", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = AppDefaults.standard.bool(forKey: "natesclaw.onboardingSeen")
...
extension GatewayEndpointStore {
    ...
    {
        if let envBind = env["NATESCLAW_GATEWAY_BIND"] {
    ...
    private static func resolveGatewayScheme(
        ...
        env: [String: String]) -> String
    {
        if let envValue = env["NATESCLAW_GATEWAY_TLS"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !envValue.isEmpty
        {
...
extension GatewayEndpointStore {
    static func localConfig() -> GatewayConnection.Config {
        self.localConfig(
            root: NatesclawConfigFile.loadDict(),
            env: ProcessInfo.processInfo.environment,
            launchdSnapshot: GatewayLaunchAgentManager.launchdConfigSnapshot(),

...
IMAGE_NAME="$(docker_e2e_resolve_image "natesclaw-live-plugin-tool-e2e" NATESCLAW_LIVE_PLUGIN_TOOL_E2E_IMAGE)"
DOCKER_TARGET="${NATESCLAW_LIVE_PLUGIN_TOOL_DOCKER_TARGET:-bare}"
HOST_BUILD="${NATESCLAW_LIVE_PLUGIN_TOOL_HOST_BUILD:-1}"
PACKAGE_TGZ="${NATESCLAW_CURRENT_PACKAGE_TGZ:-}"
PROFILE_FILE="${NATESCLAW_LIVE_PLUGIN_TOOL_PROFILE_FILE:-${NATESCLAW_TESTBOX_PROFILE_FILE:-$HOME/.natesclaw-testbox-live.profile}}"
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${NATESCLAW_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "NATESCLAW_LIVE_PLUGIN_TOOL_HOST_BUILD=0 requires NATESCLAW_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
NATESCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 live-plugin-tool empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e NATESCLAW_LIVE_PLUGIN_TOOL_MODEL="${NATESCLAW_LIVE_PLUGIN_TOOL_MODEL:-openai/gpt-5.6-luna}" \
  ...
  -e "NATESCLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
...
dump_debug_logs() {
  local status="$1"
  local agent_output_dump_bytes="${NATESCLAW_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES:-16384}"
  echo "Live plugin tool scenario failed with exit code $status" >&2
  if [ -f /tmp/natesclaw-agent.json ]; then
    echo "--- /tmp/natesclaw-agent.json (last ${agent_output_dump_bytes} bytes) ---" >&2
    tail -c "$agent_output_dump_bytes" /tmp/natesclaw-agent.json >&2 || true
    echo >&2
  fi
  natesclaw_e2e_dump_logs \
    /tmp/natesclaw-install.log \
    /tmp/natesclaw-plugin-install.log \
    /tmp/natesclaw-plugin-enable.log \
    /tmp/natesclaw-plugins-list.json \
    /tmp/natesclaw-plugin-inspect.json \
...
fixture_dir="$(mktemp -d /tmp/natesclaw-live-plugin-tool.XXXXXX)"
...
(cd "$plugin_dir" && npm pack --pack-destination "$fixture_dir" --silent) \
  >/tmp/natesclaw-live-plugin-tool-pack.log 2>&1
plugin_tgzs=()
...
natesclaw plugins install "npm-pack:$plugin_tgz" --force >/tmp/natesclaw-plugin-install.log 2>&1
node --disable-warning=ExperimentalWarning scripts/e2e/lib/live-plugin-tool/assertions.mjs configure
natesclaw plugins enable "$PLUGIN_ID" >/tmp/natesclaw-plugin-enable.log 2>&1
natesclaw plugins list --json >/tmp/natesclaw-plugins-list.json
natesclaw plugins inspect "$PLUGIN_ID" --runtime --json >/tmp/natesclaw-plugin-inspect.json
...
natesclaw agent --local \
  ...
  --thinking off \
  --timeout "${NATESCLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}" \
  --json >/tmp/natesclaw-agent.json 2>/tmp/natesclaw-agent.err

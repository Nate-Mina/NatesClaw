...
const RELEASE_CHECKS_PATH = ".github/workflows/natesclaw-release-checks.yml";
const WORKFLOW_PATH = ".github/workflows/natesclaw-release-telegram-qa.yml";
...
function runIdentityVerification(params: {
  ...
  const repository = "natesclaw/natesclaw";
  const trustedWorkflowRef = `${repository}/.github/workflows/natesclaw-release-telegram-qa.yml@refs/heads/main`;
  const invocation = params.invocation ?? "dispatch";
  const workflowRef =
    invocation === "dispatch"
      ? trustedWorkflowRef
      : `${repository}/.github/workflows/natesclaw-release-checks.yml@refs/heads/release-ci/test`;
  const workflowRefName =
    invocation === "dispatch" ? "refs/heads/main" : "refs/heads/release-ci/test";
  const workdir = tempDirs.make("natesclaw-telegram-identity-");
  ...
  const payload = {
    aud: "natesclaw-release-telegram-qa",
...
function runAdvisoryStatus(overrides: Record<string, string> = {}) {
  ...
  const workdir = tempDirs.make("natesclaw-telegram-advisory-status-");
  const githubOutput = join(workdir, "github-output");
...
function runCandidateProvenance(
  ...
  const workdir = tempDirs.make("natesclaw-telegram-provenance-");
  const fakeBin = join(workdir, "bin");
  mkdirSync(fakeBin);
                ...
                ? [
                    {
                      ...
                      headRepository: { nameWithOwner: "natesclaw/natesclaw" },
                    },
                  ]
                : []),
                ...
                ? [
                    {
                      ...
                      baseRepository: { nameWithOwner: "natesclaw/natesclaw" },
...
describe("release Telegram QA workflow", () => {
  ...
  it.runIf(process.platform === "linux")("retains only bounded, allowlisted diagnostics", () => {
    ...
    const workdir = tempDirs.make("natesclaw-telegram-diagnostics-");
  ...
  it.runIf(process.platform === "linux")("keeps the newest eight gateway logs", () => {
    ...
    const workdir = tempDirs.make("natesclaw-telegram-log-selector-");
    ...
    const paths = Array.from({ length: 12 }, (_, index) => {
      const path = join(runtimeRoot, "tmp", `natesclaw-${index}.log`);
  ...
  it("keeps generated SUT programs syntactically valid", () => {
    ...
    const workdir = tempDirs.make("natesclaw-telegram-preload-");
    const preloadPath = join(workdir, "preload.mjs");
    writeFileSync(preloadPath, preload);
    const env = { ...process.env };
    delete env.NATESCLAW_QA_SUT_PREENTRY_STOP;
    expect(

#!/usr/bin/env bash
# ClawDock - Docker helpers for Natesclaw
# Inspired by Simon Willison's "Running Natesclaw in Docker"
# https://til.simonwillison.net/llms/natesclaw-docker
#
# Installation:
...
# Common paths to check for Natesclaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/natesclaw"
  "${HOME}/workspace/natesclaw"
  "${HOME}/projects/natesclaw"
  "${HOME}/dev/natesclaw"
  "${HOME}/code/natesclaw"
  "${HOME}/src/natesclaw"
)
...
_clawdock_browser_url_for_gateway_url() {
  ...
  published=$(_clawdock_compose port natesclaw-gateway 18789 2>/dev/null | head -n 1 | tr -d '\r')
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Natesclaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/natesclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Natesclaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^NATESCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d natesclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart natesclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f natesclaw-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.natesclaw"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show natesclaw.json
  if [[ -f "${config_dir}/natesclaw.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/natesclaw.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/natesclaw.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No natesclaw.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec natesclaw-gateway \
    bash -c 'echo "alias natesclaw=\"./natesclaw.mjs\"" > /tmp/.bashrc_natesclaw && bash --rcfile /tmp/.bashrc_natesclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm natesclaw-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build natesclaw-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d natesclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build natesclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "NATESCLAW_GATEWAY_TOKEN=$token" natesclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" natesclaw-gateway \
    bash -c './natesclaw.mjs config set gateway.remote.token "$TOKEN" && ./natesclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec natesclaw-gateway \
    bash -c "./natesclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart natesclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm --no-deps natesclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec natesclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'natesclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Natesclaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (natesclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to natesclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.natesclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'natesclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'natesclaw status')"
  echo ""

...
private func textContent(_ text: String) -> NatesclawChatMessageContent {
    NatesclawChatMessageContent(type: "text", text: text, mimeType: nil, fileName: nil, content: nil)
}

private func typedTextContent(_ type: String?, _ text: String) -> NatesclawChatMessageContent {
    NatesclawChatMessageContent(type: type, text: text, mimeType: nil, fileName: nil, content: nil)
}

private func toolCallContent(name: String) -> NatesclawChatMessageContent {
...
private func thinkingContent(_ thinking: String) -> NatesclawChatMessageContent {
    NatesclawChatMessageContent(
        type: "thinking",
        text: nil,
        thinking: thinking,
...
struct ChatMessageVisibleTextTests {
    ...
    @Test func `assistant copy excludes thinking while user copy stays exact`() {
        let assistant = NatesclawChatMessage(
            role: "assistant",
            content: [textContent("<think>private reasoning</think>\nVisible **answer**")],
            timestamp: 1
        )
        let user = NatesclawChatMessage(
            role: "user",
    ...
    @Test func `history decode retains transcript identity and truncation signals`() throws {
        let metadata = try JSONDecoder().decode(
            ...
            from: Data(#"{"role":"assistant","content":"short","__natesclaw":{"id":"msg-1","truncated":true}}"#.utf8)
        )
        let marker = try JSONDecoder().decode(
            NatesclawChatMessage.self,
            from: Data(#"{"role":"assistant","content":"short\n...(truncated)...","__natesclaw":{"id":"msg-2"}}"#.utf8)
        )
    ...
    @Test func `transcript metadata survives message coding round trip`() throws {
        let original = NatesclawChatMessage(
            role: "assistant",
    ...
    @Test func `legacy trace mapping sets both independent display options`() {
        #expect(NatesclawChatDisplayOptions.assistantTrace(true) == [.reasoning, .toolActivity])
        #expect(NatesclawChatDisplayOptions.assistantTrace(false).isEmpty)
        #expect(NatesclawChatDisplayOptions.reasoning != .toolActivity)
    }

    @Test func `has visible text ignores tool blank and thinking only messages`() {
        let toolOnly = NatesclawChatMessage(
            role: "assistant",
            content: [toolCallContent(name: "exec")],
            timestamp: 1
        )
        let blank = NatesclawChatMessage(
            role: "assistant",
            content: [textContent("   ")],
            timestamp: 1
        )
        let spoken = NatesclawChatMessage(
            role: "assistant",
            content: [textContent("Say this")],
            timestamp: 1
        )
        let thinkingOnly = NatesclawChatMessage(
            role: "assistant",

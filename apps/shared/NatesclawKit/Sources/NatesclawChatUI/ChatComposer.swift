...
private struct NatesclawSendableCameraImage: @unchecked Sendable {
    let value: UIImage
}

private struct NatesclawVideoTransfer: Sendable, Transferable {
    let url: URL
    ...
    static var transferRepresentation: some TransferRepresentation {
        FileRepresentation(contentType: .movie) { video in
            ...
            let destination = FileManager.default.temporaryDirectory
                .appendingPathComponent("natesclaw-picker-video-\(UUID().uuidString)")
                .appendingPathExtension(fileExtension.isEmpty ? "mov" : fileExtension)
            try FileManager.default.copyItem(at: received.file, to: destination)
            return NatesclawVideoTransfer(url: destination)
        }
    }
}
#endif
...
@MainActor
private struct NatesclawChatAttachmentCaptureOwner {
    let viewModel: NatesclawChatViewModel
    let session: NatesclawChatViewModel.SessionSnapshot
}
...
struct NatesclawChatComposerPresentationOwner: Equatable {
    let viewModelID: ObjectIdentifier
    let session: NatesclawChatViewModel.SessionSnapshot

    @MainActor
    init(viewModel: NatesclawChatViewModel) {
...
@MainActor
struct NatesclawChatComposer: View {
    @Bindable var viewModel: NatesclawChatViewModel
    let style: NatesclawChatView.Style
    let showsSessionSwitcher: Bool
    let userAccent: Color?
    let assistantName: String?
    let assistantAvatarText: String?
    let assistantAvatarTint: Color?
    let composerChrome: NatesclawChatView.ComposerChrome
    let isComposerEnabled: Bool
    let isAttachmentInputEnabled: Bool
    let messagePlaceholder: String?
    let talkControl: NatesclawChatTalkControl?
    let dictationControl: NatesclawChatDictationControl?
    let voiceNoteControl: NatesclawChatVoiceNoteControl?
    ...
    @State private var photoPickerOwner: NatesclawChatAttachmentCaptureOwner?
    @State private var fileImporterOwner: NatesclawChatAttachmentCaptureOwner?
    #if canImport(UIKit)
    @State private var cameraEncodingTask: Task<Void, Never>?
    @State private var cameraEncodingGeneration = UUID()
    @State private var cameraCaptureOwner: NatesclawChatAttachmentCaptureOwner?
    #endif
    ...
    private var platformComposer: some View {
        self.styledComposer
            ...
            .fileImporter(
                ...
                allowedContentTypes: NatesclawChatPickerAttachmentMetadata.allowedFileContentTypes,
            ...
            .fullScreenCover(
                ...
                content: {
                    ...
                    NatesclawChatCameraPicker { image in
        ...
        VStack(alignment: .leading, spacing: 4) {
            if self.composerChrome == .clean {
                ...
                if let voiceNoteControl, voiceNoteControl.recorder.isRecording {
                    NatesclawVoiceNoteRecordingRow(recorder: voiceNoteControl.recorder)
    ...
    private var presentationOwner: NatesclawChatComposerPresentationOwner {
        NatesclawChatComposerPresentationOwner(viewModel: self.viewModel)
    }

    #if !os(macOS)
    private func attachmentCaptureOwner() -> NatesclawChatAttachmentCaptureOwner {
        NatesclawChatAttachmentCaptureOwner(
        ...
        if self.composerChrome == .full {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(NatesclawChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(NatesclawChatTheme.composerBorder, lineWidth: 1))
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 5) {
                    ...
                    if self.viewModel.showsModelPicker {
                        self.modelPicker
                        if self.viewModel.modelSelectionID != NatesclawChatViewModel.defaultModelSelectionID {
                            self.modelPinButton
                        }
                    }
                }
    ...
    private func contextUsageIndicator(_ fraction: Double) -> some View {
        ...
        let color = fraction >= 0.8 ? NatesclawChatTheme.warning : NatesclawChatTheme.muted
        return ZStack {
            Circle()
                .stroke(NatesclawChatTheme.muted.opacity(0.2), lineWidth: 2)
            Circle()
    ...
    private var thinkingPicker: some View {
            ...
            set: { next in self.viewModel.selectThinkingLevel(next) }))
        {
            Text(String(localized: "Default (inherited)"))
                .font(NatesclawChatTypography.captionSemiBold)
                .tag(NatesclawChatViewModel.inheritedThinkingSelectionID)
    ...
    private var modelPicker: some View {
            ...
            set: { next in self.viewModel.selectModel(next) }))
        {
            Text(self.viewModel.defaultModelLabel)
                .font(NatesclawChatTypography.captionSemiBold)
                .tag(NatesclawChatViewModel.defaultModelSelectionID)
                    ...
                    HStack(spacing: 4) {
                        ...
                        if provider.isDefaultProvider {
                            Text(String(localized: "Default"))
                                .font(NatesclawChatTypography.caption)
    ...
    private func modelOptions(_ models: [NatesclawChatModelChoice]) -> some View {
            ...
            set: { next in self.viewModel.switchSession(to: next) }))
        {
            ForEach(self.viewModel.sessionChoices, id: \.key) { session in
                Text(session.displayName ?? session.key)
                    .font(NatesclawChatTypography.mono(size: 12, relativeTo: .caption))
                    .tag(session.key)
            }
        } label: {
            Text("Thread")
    ...
    private func branchTitle(_ branch: NatesclawChatSessionBranch) -> String {
    ...
    private func branchMetadata(_ branch: NatesclawChatSessionBranch) -> String {
    ...
    private var fullEditor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(NatesclawChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .fill(NatesclawChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .strokeBorder(NatesclawChatTheme.composerBorder)))
    ...
    private var cleanComposerCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let voiceNoteControl, voiceNoteControl.recorder.isRecording {
                NatesclawVoiceNoteRecordingRow(
    ...
    private func talkHelpText(_ talkControl: NatesclawChatTalkControl) -> String {
        ...
        ZStack(alignment: editorOverlayAlignment) {
            if self.viewModel.input.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                Text(self.placeholderText)
                    .font(NatesclawChatTypography.body)
...
extension NatesclawChatComposer {
                      ...
                      self.viewModel.slashCommands.isEmpty
            {
                VStack(alignment: .leading, spacing: 10) {
                    Text("Commands unavailable")
                        .font(NatesclawChatTypography.footnoteSemiBold)
                    Text(error)
                        .font(NatesclawChatTypography.caption)
                        .foregroundStyle(.secondary)
                        .lineLimit(3)
    ...
    private func selectSlashCommand(_ command: NatesclawChatCommandChoice) {
...
extension NatesclawChatComposer {
        ...
        if self.viewModel.pendingRunCount > 0, self.shouldShowStopButton {
            Button {
                ...
                if self.viewModel.isAborting {
                    ...
                    Image(systemName: "stop.fill")
                        .font(NatesclawChatTypography.display(size: 13, weight: .semibold, relativeTo: .caption))
                }
            }
            .buttonStyle(.plain)
            ...
            .background(
                RoundedRectangle(cornerRadius: self.sendButtonCornerRadius, style: .continuous)
                    .fill(NatesclawChatTheme.danger)
    ...
    private var sendButtonFill: Color {
        self.userAccent ?? NatesclawChatTheme.userBubble
    }
    ...
    private func pickFilesMac() {
        ...
        panel.allowedContentTypes = NatesclawChatPickerAttachmentMetadata.allowedFileContentTypes
        panel.begin { resp in
    ...
    private func handleFileImport(
        ...
        owner: NatesclawChatAttachmentCaptureOwner?)
    {
        guard self.isAttachmentInputEnabled else { return }
        guard let owner,
    ...
    #if canImport(UIKit)
    private func addCameraImage(_ image: UIImage, owner: NatesclawChatAttachmentCaptureOwner) {
        guard self.isAttachmentInputEnabled else { return }
        guard self.viewModel === owner.viewModel,
              owner.viewModel.isCurrentSession(owner.session)
        else { return }
        self.cancelActiveCameraEncoding()
        let sendableImage = NatesclawSendableCameraImage(value: image)
    ...
    private func loadPhotosPickerItems(
        _ items: [PhotosPickerItem],
        owner: NatesclawChatAttachmentCaptureOwner) async
    {
        guard self.isAttachmentInputEnabled else {
            self.pickerItems = []
            return
        ...
        for item in items {
            do {
                ...
                if type.conforms(to: .movie) {
                    guard let transfer = try await item.loadTransferable(type: NatesclawVideoTransfer.self)
                    else { continue }
                    defer { try? FileManager.default.removeItem(at: transfer.url) }
                    let metadata = NatesclawChatPickerAttachmentMetadata.resolve(
                        contentType: type,
                    ...
                    let metadata = NatesclawChatPickerAttachmentMetadata.resolve(contentType: type)

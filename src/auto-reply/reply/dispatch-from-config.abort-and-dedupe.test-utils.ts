...
function setupResolvedAcpSessionNotice(params: { bound: boolean; messageThreadId?: string }) {
  ...
  return {
    cfg: { acp: { enabled: true, dispatch: { enabled: true } } } as NatesclawConfig,
...
function createNativeApprovalTestConfig(channel: NativeApprovalTestChannel): NatesclawConfig {
  ...
  it("normalizes accumulated block TTS-only media before final delivery", async () => {
    ...
    replyMediaPathMocks.createReplyMediaPathNormalizer.mockReturnValue(
      async (payload: ReplyPayload) => ({
        ...payload,
        mediaUrl: "/tmp/natesclaw-media/normalized-tts.ogg",
        mediaUrls: ["/tmp/natesclaw-media/normalized-tts.ogg"],
      }),
    );
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
    ...
    const finalPayload = firstFinalReplyPayload(dispatcher);
    expect(finalPayload?.mediaUrl).toBe("/tmp/natesclaw-media/normalized-tts.ogg");
    expect(finalPayload?.mediaUrls).toStrictEqual(["/tmp/natesclaw-media/normalized-tts.ogg"]);

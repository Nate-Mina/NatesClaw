...
This plugin lets Natesclaw run tool execution through MXC on Windows hosts with
...
Any other key is rejected. `natesclaw.plugin.json` publishes the same schema
(enums, `minimum`/`maximum` bounds) so `natesclaw config` validation and CLI
...
- `workspaceAccess`:
  ...
  - `rw`: the active agent workspace is mounted read-write. If protected
    Natesclaw skill roots (`skills`, `.agents/skills`, or the materialized
...
- `scope` workspace selection:
  - `session`, `agent`, and `shared` choose the Natesclaw workspace directory
    passed to MXC.
...
- Natesclaw passes per-run command, environment, and filesystem config to the
...
## Test setup with `natesclaw config`
...
```powershell
$mxcPolicyPath = Join-Path $env:TEMP "natesclaw-mxc-policy.json"
@'
{
  "filesystem": {
...
$mxcConfigPatch = @"
{
  agents: {
    list: [
      ...
      {
        id: "mxc-test",
        workspace: "~/.natesclaw/workspace-mxc-test",
        sandbox: {
          mode: "all",
          backend: "mxc",
          scope: "agent",
...
$mxcConfigPatch | natesclaw config patch --stdin --dry-run
$mxcConfigPatch | natesclaw config patch --stdin
```
    ...
    "entries": {
      "mxc": {
        ...
        "config": {
          "containment": "process",
          "network": "none",
          "mxcPolicyPaths": ["C:\\Users\\you\\AppData\\Local\\Temp\\natesclaw-mxc-policy.json"],
        },
      },
    },
  },
...
```json
{
  "filesystem": {
    "restrictToProjectDir": true,
    "additionalReadonlyPaths": ["C:\\Tools\\Natesclaw\\shared-readonly"],
    "additionalReadwritePaths": ["D:\\NatesclawScratch"]
  },
  "process": {
    "timeoutSeconds": 120
  }
...
When multiple configured policy files exist, Natesclaw layers them
...
The filesystem bridge keeps protected Natesclaw skill overlays read-only. For
...
```powershell
natesclaw tui --session agent:mxc-test:main
```

For local embedded testing without a Gateway:

```powershell
natesclaw tui --local --session agent:mxc-test:main
```
...
$mxcCleanupPatch | natesclaw config patch --stdin --dry-run
$mxcCleanupPatch | natesclaw config patch --stdin

...
struct ChatMediaVideoAttachment: View {
    ...
    let width: Int?
    let height: Int?
    let playback: NatesclawChatPlaybackMode?
    let resolverReady: Bool
    let playbackAllowed: @MainActor @Sendable () -> Bool
    let load: @MainActor @Sendable (String) async throws -> NatesclawChatLoadedMedia?
...
final class ChatMediaVideoPlayer: ChatMediaNowPlayingOwner {
    ...
    init(
        loaded: NatesclawChatLoadedMedia,
        title: String,
    ...
    private nonisolated static func writeTemporaryVideo(
        _ media: NatesclawChatMediaData) async throws -> URL
    {
        try await Task.detached(priority: .userInitiated) {
            let fileExtension = UTType(mimeType: media.mimeType)?.preferredFilenameExtension ?? "mp4"
            let url = FileManager.default.temporaryDirectory
                .appendingPathComponent("natesclaw-chat-video-\(UUID().uuidString)")

...
describe("staged worker placement result recovery", () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(await fs.realpath(os.tmpdir()), "natesclaw-staged-dispatch-"));
  ...
  it("applies a staged pending result without a tunnel and reclaims the worker", async () => {
    ...
    const priorConflictRef = "refs/natesclaw/worker-results/prior-conflict";

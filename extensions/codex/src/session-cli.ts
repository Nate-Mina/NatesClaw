...
import { sanitizeTerminalText } from "natesclaw/plugin-sdk/text-chunking";
...
async function continueCodexSession(
  ...
  writeLine(`Natesclaw session: ${singleLineTerminalText(result.sessionKey)}`);
}
...
export function registerCodexSessionCli(program: Command): void {
  ...
  addGatewayClientOptions(
    codex
      ...
      .description("Continue a Gateway-local Codex thread as an Natesclaw branch")
  ...
  addGatewayClientOptions(
    codex
      ...
      .option(
        ...
        "Confirm no other Codex client or Natesclaw runner is using this thread",
        false,
      )
      .option("--json", "Print the structured response", false),

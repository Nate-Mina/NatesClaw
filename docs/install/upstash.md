---
summary: "Host Natesclaw on Upstash Box with keep-alive and SSH tunnel access"
read_when:
  - Deploying Natesclaw to Upstash Box
  - You want a managed Linux environment for Natesclaw with SSH-tunneled dashboard access
title: "Upstash Box"
---

Run a persistent Natesclaw Gateway on Upstash Box, a managed Linux environment
...
Upstash maintains its current Natesclaw Box walkthrough at
[Natesclaw Setup](https://upstash.com/docs/box/guides/natesclaw-setup).

## Connect with an SSH tunnel

Forward the Natesclaw dashboard port to your local machine. Use your Box API key
...
```bash
natesclaw config set gateway.bind lan
nohup natesclaw gateway > gateway.log 2>&1 &
```

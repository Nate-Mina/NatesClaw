...
export function createRuntimeSecretsActivator(params: {
  ...
  let activeDegradationConfig: NatesclawConfig | null = null;
...
export async function prepareGatewayStartupConfig(params: {
  ...
  const canReusePreflightPreparedSnapshot = (config: NatesclawConfig): boolean =>
    Boolean(
      preflightPrepared &&
  ...
  const activateStartupSecrets = async (config: NatesclawConfig) => {

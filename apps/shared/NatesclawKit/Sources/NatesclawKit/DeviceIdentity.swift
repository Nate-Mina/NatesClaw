...
enum DeviceIdentityPaths {
    ...
    private static let appGroupStateDirAvailable =
        DeviceIdentityPaths.hasAppGroupEntitlement(NatesclawAppGroup.identifier)
        ...
        temporaryDirectory: URL) -> URL
    {
        ...
        return temporaryDirectory.appendingPathComponent("natesclaw", isDirectory: true)
    }

    private static func stateDirOverrideURL() -> URL? {
        // Test-scoped stores must win over the process environment. Parallel Swift tests
        // otherwise race whenever another suite temporarily swaps NATESCLAW_STATE_DIR.
    ...
    static func legacyStateDirURL() -> URL? {
        if let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
            return appSupport.appendingPathComponent("Natesclaw", isDirectory: true)
        }
        return nil
    }
    ...
    static func appGroupStateDirURL() -> URL? {
        guard
            let containerURL = FileManager.default
                .containerURL(forSecurityApplicationGroupIdentifier: NatesclawAppGroup.identifier)
        else {
            return nil
        }
        return containerURL.appendingPathComponent("Natesclaw", isDirectory: true)
    }
...
public enum DeviceIdentityStore {
    ...
    public static func loadOrCreate(profile: GatewayDeviceIdentityProfile) -> DeviceIdentity {
        do {
            ...
            preconditionFailure("Could not persist the Natesclaw device identity: \(error.localizedDescription)")
        }
    }

...
describe("WhatsApp auth dir profile resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["NATESCLAW_STATE_DIR", "NATESCLAW_OAUTH_DIR"]);
    tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-wa-profile-"));
  ...
  it("resolves the default web auth dir from NATESCLAW_STATE_DIR at call time", async () => {

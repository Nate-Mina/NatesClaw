...
import { keepHttpServerTaskAlive, waitUntilAbort } from "natesclaw/plugin-sdk/channel-outbound";
...
function spawnRaftBridge(params: {
  ...
  // Raft owns the fixed bridge command. Natesclaw passes profile/loopback
...
export async function startRaftGatewayAccount(
  ...
  if (!ctx.channelRuntime) {
    throw new Error("Raft requires Natesclaw channel runtime support. Update Natesclaw and retry.");
  }
  ...
  const server = createServer((request, response) => {
    void (async () => {
      ...
      if (
        ...
        // Raft drains runtime activity after each wake pass. Natesclaw has no
      ...
      // Raft owns wake metadata and its schema evolution. Natesclaw accepts only

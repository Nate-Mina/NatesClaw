---
summary: "Use Z.AI (GLM models) with Natesclaw"
read_when:
  - You want Z.AI / GLM models in Natesclaw
...
Natesclaw uses the `zai` provider with a Z.AI API key.
...
GLM is a model family, not a separate provider. In Natesclaw, GLM models use
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. Natesclaw probes supported Z.AI endpoints with your API key and applies the correct base URL automatically.

    <Steps>
      <Step title="Run onboarding">
        ```bash
        natesclaw onboard --auth-choice zai-api-key
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
        natesclaw models list --all --provider zai
        ```
      </Step>
    </Steps>
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        natesclaw onboard --auth-choice zai-coding-cn

        # General API
        natesclaw onboard --auth-choice zai-global

        # General API CN (China region)
        natesclaw onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
...
`https://api.z.ai/api/anthropic`. Natesclaw's Z.AI choices use the documented
...
- [General-purpose agent tools](https://docs.z.ai/devpack/tool/others),
  including Natesclaw, are served on a best-effort basis. During high inference
...
```bash
natesclaw models list --all --provider zai
natesclaw config get models.providers.zai.baseUrl
```
...
<Tip>
`zai-api-key` lets Natesclaw detect the matching Z.AI endpoint from the key and
...
| `zai/glm-5-turbo`  | Natesclaw-optimized text model; 200K context       |
...
versions and availability can change; run `natesclaw models list --all --provider zai`
...
<Tabs>
  <Tab title="GLM-5.2">
    Full range: `off`, `low`, `high`, `max` (default `off`). Natesclaw maps
...
<AccordionGroup>
  ...
  <Accordion title="Preserved thinking">
    ...
    When enabled and thinking is on, Natesclaw sends
  ...
  <Accordion title="Auth details">
    ...
    - The legacy env var `Z_AI_API_KEY` is still accepted; Natesclaw copies it to `ZAI_API_KEY` at startup if `ZAI_API_KEY` is unset.
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full Natesclaw config schema, including provider and model settings.
  </Card>
</CardGroup>

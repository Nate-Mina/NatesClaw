...
type DaemonConfigContext = {
  ...
  cliCfg: NatesclawConfig;
  daemonCfg: NatesclawConfig;
...
function resolveSnapshotRuntimeConfig(snapshot: ConfigFileSnapshot | null): NatesclawConfig | null {
...
function coerceStatusConfig(value: unknown): NatesclawConfig {
...
export type DaemonStatus = {
  ...
  service: {
    ...
    staleUpdateLaunchdJobs?: StaleNatesclawUpdateLaunchdJob[];
  };
  config?: {
    cli: ConfigSummary;
   ...
   * can happen after `npm install -g natesclaw@<v>` updates the gateway binary
   * without a corresponding `natesclaw plugins update`.
   */
  ...
  const staleUpdateLaunchdJobs =
    opts.deep && process.platform === "darwin"
      ? await loadLaunchdModule()
          .then(({ findStaleNatesclawUpdateLaunchdJobs }) =>
            findStaleNatesclawUpdateLaunchdJobs(serviceEnv),
          )
          .catch(() => [])
      : [];

---
summary: "CLI reference for `natesclaw security` (audit and fix common security footguns)"
read_when:
...
natesclaw security audit --deep --password <password>
natesclaw security audit --deep --token <token>
natesclaw security audit --auth password --password <password>
natesclaw security audit --fix
...
Run `natesclaw doctor --fix` to rotate a persisted reused `hooks.token`, then update external hook senders to use the new token.
...
- Warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `natesclaw.browserConfigEpoch`) and recommends `natesclaw sandbox recreate --browser --all`.
...
```bash
natesclaw security audit --json | jq '.summary'
natesclaw security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

With `--fix --json`, output includes both fix actions and the final report:

```bash
natesclaw security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
...
- tightens permissions for state/config and common sensitive files (`credentials/*.json`, `auth-profiles.json`, `natesclaw-agent.sqlite`, and legacy session artifacts)
- also tightens config include files referenced from `natesclaw.json`

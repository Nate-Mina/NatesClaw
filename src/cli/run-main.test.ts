...
describe("isGatewayRunFastPathArgv", () => {
  it("matches only plain gateway foreground starts without root options or help", () => {
    expect(isGatewayRunFastPathArgv(["node", "natesclaw", "gateway"])).toBe(true);
    expect(isGatewayRunFastPathArgv(["node", "natesclaw", "gateway", "--force"])).toBe(true);
    expect(isGatewayRunFastPathArgv(["node", "natesclaw", "gateway", "--port", "18789"])).toBe(true);
    expect(isGatewayRunFastPathArgv(["node", "natesclaw", "gateway", "--auth=none"])).toBe(true);
    expect(
      isGatewayRunFastPathArgv(["node", "natesclaw", "--no-color", "gateway", "--bind", "loopback"]),
    ).toBe(true);
    expect(isGatewayRunFastPathArgv(["node", "natesclaw", "gateway", "run"])).toBe(true);
    expect(
      isGatewayRunFastPathArgv(["node", "natesclaw", "gateway", "--log-level", "debug", "run"]),
    ).toBe(true);
    expect(
      isGatewayRunFastPathArgv(["node", "natesclaw", "gateway", "--log-level=debug", "run"]),
    ).toBe(true);
    expect(
      isGatewayRunFastPathArgv(["node", "natesclaw", "gateway", "run", "--raw-stream-path", "x"]),
    ).toBe(true);
    expect(isGatewayRunFastPathArgv(["node", "natesclaw", "gateway", "call", "health"])).toBe(false);
    expect(isGatewayRunFastPathArgv(["node", "natesclaw", "gateway", "--help"])).toBe(false);
    expect(isGatewayRunFastPathArgv(["node", "natesclaw", "gateway", "--port"])).toBe(false);
    expect(isGatewayRunFastPathArgv(["node", "natesclaw", "gateway", "--unknown"])).toBe(false);
  });
...
describe("resolveGatewayRunPreBootstrapOptions", () => {
  it("resolves destructive gateway flags across fast and full Commander paths", () => {
    expect(
      resolveGatewayRunPreBootstrapOptions(["node", "natesclaw", "gateway", "run", "--force"]),
  ...
  it("does not treat malformed required option values as destructive flags", () => {
    expect(
      resolveGatewayRunPreBootstrapOptions(["node", "natesclaw", "gateway", "--token", "--force"]),
...
describe("shouldEnsureCliPath", () => {
  it("skips path bootstrap for help/version invocations", () => {
    expect(shouldEnsureCliPath(["node", "natesclaw", "--help"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "natesclaw", "-V"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "natesclaw", "-v"])).toBe(false);
  });

  it("skips path bootstrap for read-only fast paths", () => {
    expect(shouldEnsureCliPath(["node", "natesclaw"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "natesclaw", "--profile", "work"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "natesclaw", "approvals"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "natesclaw", "channels"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "natesclaw", "cron"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "natesclaw", "devices"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "natesclaw", "plugins"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "natesclaw", "mcp"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "natesclaw", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "natesclaw", "--log-level", "debug", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "natesclaw", "sessions", "--json"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "natesclaw", "config", "get", "update"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "natesclaw", "models", "status", "--json"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "natesclaw", "tools", "effective"])).toBe(false);
  });

  it("keeps path bootstrap for mutating or unknown commands", () => {
    expect(shouldEnsureCliPath(["node", "natesclaw", "message", "send"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "natesclaw", "voicecall", "status"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "natesclaw", "acp", "-v"])).toBe(true);
  });
});

describe("shouldHandleBareRoot", () => {
  it("handles bare root invocations", () => {
    expect(shouldHandleBareRoot(["node", "natesclaw"])).toBe(true);
    expect(shouldHandleBareRoot(["node", "natesclaw", "--profile", "work"])).toBe(true);
    expect(shouldHandleBareRoot(["node", "natesclaw", "--dev"])).toBe(true);
  });

  it("does not handle help, version, or commands", () => {
    expect(shouldHandleBareRoot(["node", "natesclaw", "--help"])).toBe(false);
    expect(shouldHandleBareRoot(["node", "natesclaw", "-V"])).toBe(false);
    expect(shouldHandleBareRoot(["node", "natesclaw", "status"])).toBe(false);
  });
});

describe("shouldStartProxyForCli", () => {
  it("starts managed proxy routing for the --update shorthand", () => {
    expect(shouldStartProxyForCli(["node", "natesclaw", "--update"])).toBe(true);
    expect(shouldStartProxyForCli(["node", "natesclaw", "--profile", "p", "--update"])).toBe(true);
  });

  it("skips managed proxy routing for bare parent default help", () => {
    expect(shouldStartProxyForCli(["node", "natesclaw", "qa", "suite"])).toBe(false);
    expect(shouldStartProxyForCli(["node", "natesclaw", "plugins"])).toBe(false);
    expect(shouldStartProxyForCli(["node", "natesclaw", "channels"])).toBe(false);
    expect(shouldStartProxyForCli(["node", "natesclaw", "cron"])).toBe(false);
    expect(shouldStartProxyForCli(["node", "natesclaw", "devices"])).toBe(false);
    expect(shouldStartProxyForCli(["node", "natesclaw", "mcp"])).toBe(false);
  });

  it("skips managed proxy routing before shared-state SQLite maintenance", () => {
    expect(
      shouldStartProxyForCli(["node", "natesclaw", "doctor", "--state-sqlite", "compact", "--json"]),
    ).toBe(false);
    expect(
      shouldStartProxyForCli(["node", "natesclaw", "doctor", "--state-sqlite=compact", "--json"]),
    ).toBe(false);
    expect(shouldStartProxyForCli(["node", "natesclaw", "doctor", "--lint"])).toBe(true);
  });
});

describe("shouldUseRootHelpFastPath", () => {
  it("uses the fast path for root help only", () => {
    expect(shouldUseRootHelpFastPath(["node", "natesclaw", "--help"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "natesclaw", "--profile", "work", "-h"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "natesclaw", "help", "--help"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "natesclaw", "tools", "--help"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "natesclaw", "status", "--help"])).toBe(false);
    expect(shouldUseRootHelpFastPath(["node", "natesclaw", "--help", "status"])).toBe(false);
    expect(shouldUseRootHelpFastPath(["node", "natesclaw", "help", "gateway"])).toBe(false);
  });
});
...
describe("shouldUseSetupOnboardConfigureHelpFastPath", () => {
  it("uses the fast path only for setup, onboard, and configure help", () => {
    expect(
      shouldUseSetupOnboardConfigureHelpFastPath(["node", "natesclaw", "setup", "--help"]),
    ).toBe(true);
    expect(shouldUseSetupOnboardConfigureHelpFastPath(["node", "natesclaw", "onboard", "-h"])).toBe(
      true,
    );
    expect(
      shouldUseSetupOnboardConfigureHelpFastPath([
    ...
    expect(
      shouldUseSetupOnboardConfigureHelpFastPath(["node", "natesclaw", "status", "--help"]),
    ).toBe(false);
    expect(
      shouldUseSetupOnboardConfigureHelpFastPath([
        "node",
...
describe("resolveMissingPluginCommandMessage", () => {
  ...
  it("explains that dreaming is a runtime slash command, not a CLI command", () => {
    ...
    expect(message).toContain("natesclaw memory");
  });
  ...
  it("explains disabled-by-default parent plugins for CLI command aliases", () => {
    ...
    expect(message).toContain("natesclaw plugins enable voice-call");
  });
  ...
  it("prefers CLI ownership for plugins that also register a slash command", () => {
    ...
    expect(message).toContain("disabled by default");
    expect(message).toContain("natesclaw plugins enable workboard");

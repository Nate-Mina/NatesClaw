...
generated audio is still missing from the reply, Natesclaw sends an
...
<Note>
Not all providers support all parameters. Natesclaw still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, Natesclaw
...
Provider request timeouts are operator configuration only. Natesclaw uses
...
- **Status lookup:** `natesclaw tasks list` or `natesclaw tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** Natesclaw injects an internal completion event back
...
natesclaw tasks show <taskId>
natesclaw tasks cancel <taskId>
```
...
```bash
NATESCLAW_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```

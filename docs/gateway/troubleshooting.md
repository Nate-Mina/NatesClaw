...
- `natesclaw gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `natesclaw doctor` reports no blocking config/service issues.
- `natesclaw channels status --probe` shows live per-account transport status and, where supported, `works` or `audit ok`.
...
- `Update restart` in `natesclaw status` / `natesclaw status --all`. Pending or failed handoffs include the next command to run.
- `plugin load failed: dependency tree corrupted; run natesclaw doctor --fix` under Channels: the channel config still exists, but plugin registration failed before the channel could load.
- Provider 401s after re-auth: `natesclaw doctor --fix` checks for stale per-agent OAuth auth shadows and removes old copies so all agents resolve the current shared profile.

## Split brain installs and newer config guard

Use when a gateway service unexpectedly stops after an update, or logs show one `natesclaw` binary is older than the version that last wrote `natesclaw.json`.

Natesclaw stamps config writes with `meta.lastTouchedVersion`. Read-only commands can inspect a config written by a newer Natesclaw, but process and service mutations refuse to run from an older binary. Blocked actions: gateway service start/stop/restart/uninstall, forced service reinstall, service-mode gateway startup, and `gateway --force` port cleanup.

```bash
which natesclaw
natesclaw --version
natesclaw gateway status --deep
natesclaw config get meta.lastTouchedVersion
```

<Steps>
  <Step title="Fix PATH">
    Fix `PATH` so `natesclaw` resolves to the newer install, then rerun the action.
  </Step>
  ...
  <Step title="Remove stale wrappers">
    Remove stale system package or old wrapper entries that still point at an old `natesclaw` binary.
  </Step>
</Steps>

<Warning>
For intentional downgrade or emergency recovery only, set `NATESCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1` for the single command. Leave it unset for normal operation.
</Warning>
...
- `Established clients:` in `natesclaw gateway status --deep` or `Gateway clients` in `natesclaw doctor --deep`: active TCP clients connected to the Gateway port, with PIDs and command lines when the OS allows it.
- A client process whose command line points at the newer Natesclaw install or wrapper you rolled back from.

Fix:

1. Stop or restart the stale Natesclaw client process shown by `gateway status --deep`.
2. Restart apps or wrappers that embed Natesclaw: local dashboards, editors, app-server helpers, or long-running `natesclaw logs --follow` shells.
3. Re-run `natesclaw gateway status --deep` or `natesclaw doctor --deep` and confirm the stale client PID is gone.
...
Every skill root is a containment boundary. A symlink under `~/.agents/skills`, `<workspace>/.agents/skills`, `<workspace>/skills`, or `~/.natesclaw/skills` is skipped when its real target resolves outside that root, unless the target is explicitly trusted.
...
natesclaw config get skills.load
```
...
natesclaw config get agents.defaults.models
```
...
Do not assume this is always an Natesclaw configuration issue. The response can come from an upstream security layer such as a CDN, WAF, bot-management rule, or reverse proxy in front of an OpenAI-compatible endpoint.
...
Fix the provider-side filtering first when evidence points to a WAF/CDN block. Prefer a narrowly scoped allow or skip rule for the API path Natesclaw uses, and avoid disabling protection for the whole site.
...
- Natesclaw model runs fail only on normal agent turns.
...
natesclaw infer model run --model <provider/model> --prompt "hi" --json
natesclaw logs --follow
```

Look for:

- Direct tiny calls succeed, but Natesclaw runs fail only on larger prompts.
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `incomplete turn detected ... stopReason=stop payloads=0`: the backend completed the Chat Completions request but returned no user-visible assistant text for that turn. Natesclaw retries replay-safe empty OpenAI-compatible turns once; persistent failures usually mean the backend is emitting empty/non-text content or suppressing final-answer text.
    - Direct tiny requests succeed, but Natesclaw agent runs fail with backend/model crashes (for example Gemma on some `inferrs` builds): Natesclaw transport is likely already correct; the backend is failing on the larger agent-runtime prompt shape.
  ...
  <Accordion title="Fix options">
    ...
    3. Set `compat.supportsTools: false` for models/backends that cannot handle Natesclaw's tool schema surface reliably.
    ...
    5. If tiny direct requests keep passing while Natesclaw agent turns still crash inside the backend, treat it as an upstream server/model limitation and file a repro there with the accepted payload shape.
  </Accordion>
</AccordionGroup>
...
If `curl` returns Natesclaw HTML, the Gateway is working and the remaining issue is likely browser cache, an old deep link, or stale tab state. Open `http://127.0.0.1:18789` directly and navigate from the dashboard. If restart does not leave the service running, run `natesclaw gateway start` and recheck `natesclaw gateway status`.
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.                                                                                                                                                 | On the Gateway host, run `natesclaw gateway auth-token --show` in an interactive terminal, paste the output into the client, and retry.                                                                                                                                                   |
...
| `PAIRING_REQUIRED`           | Device identity needs approval. Check `error.details.reason` for `not-paired`, `scope-upgrade`, `role-upgrade`, or `metadata-upgrade`, and use `requestId` / `remediationHint` when present. | Approve pending request: `natesclaw devices list` then `natesclaw devices approve <requestId>`. Scope/role upgrades use the same flow after you review the requested access.                                                                                                               |
...
If `natesclaw devices rotate` / `revoke` / `remove` is denied unexpectedly:

- Paired-device token sessions can manage only **their own** device unless the caller also has `operator.admin`.
- `natesclaw devices rotate --scope ...` can only request operator scopes that the caller session already holds.
...
natesclaw logs --follow
natesclaw doctor
natesclaw gateway status --deep   # also scan system-level services
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    - `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `natesclaw onboard --mode local` / `natesclaw setup` to restamp the expected local-mode config. If you are running Natesclaw via Podman, the default config path is `~/.natesclaw/natesclaw.json`.
    ...
    - `System-level Natesclaw gateway service detected` from doctor → a systemd system unit exists while the user-level service is missing. Remove or disable the duplicate before allowing doctor to install a user service, or set `NATESCLAW_SERVICE_REPAIR_POLICY=external` if the system unit is the intended supervisor.
    - `Gateway service port does not match current gateway config` → the installed supervisor still pins the old `--port`. Run `natesclaw doctor --fix` or `natesclaw gateway install --force`, then restart the gateway service.
...
```bash
ls ~/.natesclaw/logs/stability/ | tail -5
natesclaw gateway stability --bundle latest
pmset -g log | grep -iE "sleep|wake|maintenance" | tail -50
launchctl print gui/$UID/ai.natesclaw.gateway | grep -E "state|last exit|runs"
```

Look for:

- One or more `*-uncaught_exception.json` bundles in `~/.natesclaw/logs/stability/` with `error.code` set to a transient network code such as `ENETDOWN`, `ENETUNREACH`, `EHOSTUNREACH`, or `ECONNREFUSED`.
...
- `runs` count incrementing across the day with no corresponding `received SIG*; shutting down` line in `~/Library/Logs/natesclaw/gateway.log`: clean shutdowns log a signal; transient crashes do not.
...
3. **Add a liveness watchdog** so a future crash burst that gets parked by launchd is caught quickly:
   ...
   state=$(launchctl print gui/$UID/ai.natesclaw.gateway 2>/dev/null | awk -F'= ' '/state =/ {print $2; exit}')
   if [ "$state" != "running" ]; then
     launchctl kickstart -k gui/$UID/ai.natesclaw.gateway
   fi
   ```
...
Use this when a macOS install keeps restarting every few seconds, `natesclaw`
...
This was observed on older installs where both `ai.natesclaw.gateway` and
`ai.natesclaw.node` LaunchAgents were active and each injected
`NATESCLAW_LAUNCHD_LABEL`. In that state Natesclaw can detect launchd
...
```bash
for i in 1 2 3 4; do
  ps aux | grep 'natesclaw.*index.js' | grep -v grep | awk '{print $2}'
  sleep 10
done

natesclaw gateway status --deep
natesclaw node status
launchctl print gui/$UID/ai.natesclaw.gateway | grep -E 'state|last exit|runs'
tail -n 80 ~/Library/Logs/natesclaw/gateway.log
```
...
- Both `~/Library/LaunchAgents/ai.natesclaw.gateway.plist` and
  `~/Library/LaunchAgents/ai.natesclaw.node.plist` loaded at the same time on a
...
1. If this host should only run the Gateway service, remove the managed node
   service through Natesclaw. **Skip this step** if you actively rely on the node
   service for remote node features; uninstalling it stops those features on
   this host:

   ```bash
   natesclaw node uninstall
   ```

2. Install a persistent Gateway wrapper that clears the inherited launchd
   markers before starting Natesclaw. Use the supported `--wrapper` option; do
   not edit the generated file under `~/.natesclaw/service-env/`, because service
   reinstall, update, and doctor repair regenerate that file:

   ```bash
   mkdir -p ~/.local/bin
   cat >~/.local/bin/natesclaw-launchd-workaround <<'EOF'
   #!/bin/sh
   set -eu
   unset NATESCLAW_LAUNCHD_LABEL LAUNCH_JOB_LABEL LAUNCH_JOB_NAME XPC_SERVICE_NAME || true
   exec natesclaw "$@"
   EOF
   chmod 700 ~/.local/bin/natesclaw-launchd-workaround

   natesclaw gateway install \
     --wrapper ~/.local/bin/natesclaw-launchd-workaround \
     --force
   ```
...
3. Verify that the Gateway is stable and serving RPC, not merely listening:
   ...
   ```bash
   natesclaw gateway status --deep --require-rpc
...
4. After upgrading to a release where the underlying dual-LaunchAgent loop is
   ...
   ```bash
   NATESCLAW_WRAPPER= natesclaw gateway install --force
   rm ~/.local/bin/natesclaw-launchd-workaround
   ```
...
natesclaw logs --follow
natesclaw gateway stability --bundle latest
natesclaw gateway diagnostics export
```
...
- `critical memory pressure bundle written` appears shortly before restart → Natesclaw captured a pre-OOM stability bundle. Inspect it with `natesclaw gateway stability --bundle latest`.
- `memory pressure: level=critical` appears in gateway logs → Natesclaw detected critical memory pressure and recorded the available in-process memory facts.
...
- `V8 heap:` used bytes are close to the heap limit → lower prompt/session pressure or reduce concurrent work first. For a managed service, inspect `Gateway heap:` in `natesclaw gateway status`; if it says `not set`, regenerate old service metadata with `natesclaw gateway install --force`. Ambient shell `NODE_OPTIONS` is intentionally ignored. Use an explicit supervisor-level heap override only after confirming the sustained workload and leaving enough native-memory headroom.
...
- Critical memory pressure appears in logs but no bundle exists → capture `natesclaw gateway diagnostics export` after the event for the available operational evidence.
...
- A timestamped `natesclaw.json.rejected.*` file beside the active config.
- A timestamped `natesclaw.json.clobbered.*` file if `doctor --fix` repaired a broken direct edit.
- Natesclaw keeps the latest 32 `.clobbered.*` files for each config path and rotates older ones.

<AccordionGroup>
  <Accordion title="What happened">
    - The config did not validate during startup, hot reload, or an Natesclaw-owned write.
    - Gateway startup fails closed instead of rewriting `natesclaw.json`.
    - Hot reload skips invalid external edits and keeps the current runtime config active.
    - Natesclaw-owned writes reject invalid/destructive payloads before commit and save `.rejected.*`.
    - `natesclaw doctor --fix` owns repair. It can remove non-JSON prefixes or restore the last-known-good copy while preserving the rejected payload as `.clobbered.*`.
    - When many repairs happen for one config path, Natesclaw rotates older `.clobbered.*` files so the newest repaired payload is still available.

  </Accordion>
  <Accordion title="Inspect and repair">
    ```bash
    CONFIG="$(natesclaw config file)"
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → an Natesclaw-owned config write failed schema or clobber checks before commit.
    ...
    - `missing-meta-vs-last-good`, `gateway-mode-missing-vs-last-good`, or `size-drop-vs-last-good:*` → an Natesclaw-owned write was rejected because it lost fields or size compared with the last-known-good backup.
    - `Config last-known-good promotion skipped` → the candidate contained redacted secret placeholders such as `***`.

  </Accordion>
  <Accordion title="Fix options">
    1. Run `natesclaw doctor --fix` to let doctor repair prefixed/clobbered config or restore last-known-good.
    2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `natesclaw config set` or `config.patch`.
    3. Run `natesclaw config validate` before restarting.
...
Use when `natesclaw gateway probe` reaches something, but still prints a warning block.

```bash
natesclaw gateway probe
natesclaw gateway probe --json
natesclaw gateway probe --ssh user@gateway-host
```
...
- `multiple reachable gateway identities detected` → distinct gateways answered, or Natesclaw could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is treated as one gateway with multiple transports, even when transport ports differ.
...
natesclaw system heartbeat last
natesclaw logs --follow
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `heartbeat skipped` with `reason=empty-heartbeat-file` → heartbeat monitor scratch only contains blank, comment, header, fence, or empty-checklist scaffolding, so Natesclaw skips the model call.
...
natesclaw approvals get --node <idOrNameOrIp>
natesclaw logs --follow
natesclaw status
```
...
natesclaw browser start --browser-profile natesclaw
...
<AccordionGroup>
  ...
  <Accordion title="Element / screenshot / upload signatures">
    ...
    - Stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `natesclaw browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
- [Browser (Natesclaw-managed)](/tools/browser)
...
<AccordionGroup>
  <Accordion title="1. Auth and URL override behavior changed">
    ```bash
    natesclaw gateway status
    natesclaw config get gateway.mode
    natesclaw config get gateway.remote.url
    natesclaw config get gateway.auth.mode
    ```
  ...
  <Accordion title="2. Bind and auth guardrails are stricter">
    ```bash
    natesclaw config get gateway.bind
    natesclaw config get gateway.auth.mode
    natesclaw config get gateway.auth.token
    natesclaw gateway status
    natesclaw logs --follow
    ```

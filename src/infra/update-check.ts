// Computes git, dependency, and registry update status for Natesclaw installs.
...
import { compareNatesclawReleaseVersions } from "./npm-registry-spec.js";
...
function resolveExtendedStableRegistryTarget(params: {
  ...
  const packageSpecOverride = env.NATESCLAW_UPDATE_PACKAGE_SPEC?.trim();
...
async function isLocklessNatesclawNpmInstall(params: {
  root: string;
  manager: PackageManager;
}): Promise<boolean> {
...
export function compareSemverStrings(a: string | null, b: string | null): number | null {
  if (a && b) {
    const NatesclawReleaseCmp = compareNatesclawReleaseVersions(a, b);
    if (NatesclawReleaseCmp != null) {
      return NatesclawReleaseCmp;
    }
  }
...
export async function checkUpdateStatus(params: {
  ...
  const packageManager =
    !isGit &&
    (await isLocklessNatesclawNpmInstall({
      root,

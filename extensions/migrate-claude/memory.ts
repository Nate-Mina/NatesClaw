...
import { createMigrationItem, MIGRATION_REASON_TARGET_EXISTS } from "natesclaw/plugin-sdk/migration";
...
async function assertSafeMemoryDestination(
  ...
  if (
    ...
    throw new Error(
      "Claude Code auto-memory source and Natesclaw import destination must be separate directories.",
    );
  }
}
        ...
        createMigrationItem({
          ...
          reason: targetNotRegular
            ...
            : targetConflict
              ? MIGRATION_REASON_TARGET_EXISTS
              : undefined,
          message: "Copy Claude Code auto-memory Markdown into the Natesclaw memory index.",
          details: {

...
function runHelper(payload: string) {
  return runSourcedHelper(
    [
      `natesclaw_e2e_eval_test_state_from_b64 ${shellQuote(payload)}`,
      'printf "value=%s" "${NATESCLAW_E2E_INSTANCE_TEST:-unset}"',
    ].join("; "),
    null,
  );
}
...
function writePackageFixture(packagePath: string): void {
  withTempDir("natesclaw-e2e-package-", (root) => {
    const packageDir = path.join(root, "package");
    fs.mkdirSync(packageDir);
    fs.writeFileSync(
      path.join(packageDir, "package.json"),
      JSON.stringify({ name: "natesclaw-e2e-fixture", version: "0.0.0" }),
    );
...
function createPackageInstallFixture(tempDir: string) {
  const fixture = {
    ...
    packagePath: path.join(tempDir, "natesclaw.tgz"),
...
function runPackageInstall(
  ...
  return runBashWithHelper(
    [
      `natesclaw_e2e_install_package ${shellQuote(fixture.logPath)} ${shellQuote("fixture package")} ${shellQuote(fixture.prefixPath)}`,
    ],
    env,
    undefined,
    "; ",
...
function expectNpmInstallObserved(argsPath: string, expectedArgs: string, prefix: string): void {
  ...
  expect(
    fs.existsSync(path.join(prefix, "lib/node_modules/natesclaw-e2e-fixture/package.json")),
  ).toBe(true);
}

describe("scripts/lib/natesclaw-e2e-instance.sh", () => {
  it("sources decoded test-state scripts", () => {
    const result = runHelper(base64('export NATESCLAW_E2E_INSTANCE_TEST="ok"\n'));
  ...
  it("fails when the test-state payload is not valid base64", () => {
    ...
    expect(result.stderr).toContain("Invalid Natesclaw test-state base64 payload");
  });
  ...
  it("reads positive integer env values without treating decimal input as durations", () => {
    const fallback = runSourcedHelper(
      'printf "%s" "$(natesclaw_e2e_read_positive_int_env NATESCLAW_E2E_SAMPLE_SECONDS 180)"',
    );
    const leadingZero = runSourcedHelper(
      'printf "%s" "$(natesclaw_e2e_read_positive_int_env NATESCLAW_E2E_SAMPLE_SECONDS 180)"',
      { NATESCLAW_E2E_SAMPLE_SECONDS: "008" },
    );
    const duration = runSourcedHelper(
      "natesclaw_e2e_read_positive_int_env NATESCLAW_E2E_SAMPLE_SECONDS 180",
      { NATESCLAW_E2E_SAMPLE_SECONDS: "30s" },
    );
    ...
    expect(duration.stderr).toContain("invalid NATESCLAW_E2E_SAMPLE_SECONDS: 30s");
  });

  it("reads non-negative integer env values without accepting shell-style sizes", () => {
    const fallback = runSourcedHelper(
      'printf "%s" "$(natesclaw_e2e_read_nonnegative_int_env NATESCLAW_E2E_SAMPLE_BYTES 262144)"',
    );
    const zero = runSourcedHelper(
      'printf "%s" "$(natesclaw_e2e_read_nonnegative_int_env NATESCLAW_E2E_SAMPLE_BYTES 262144)"',
      { NATESCLAW_E2E_SAMPLE_BYTES: "0" },
    );
    const size = runSourcedHelper(
      "natesclaw_e2e_read_nonnegative_int_env NATESCLAW_E2E_SAMPLE_BYTES 262144",
      { NATESCLAW_E2E_SAMPLE_BYTES: "64kb" },
    );
    ...
    expect(size.stderr).toContain("invalid NATESCLAW_E2E_SAMPLE_BYTES: 64kb");
  });

  it("probes default and explicit mock OpenAI base URLs", () => {
    withTempDir("natesclaw-e2e-mock-openai-url-", (tempDir) => {
      const probePath = path.join(tempDir, "probe-url.txt");
      const result = runSourcedHelper(
        [
          `natesclaw_e2e_probe_http() { printf "%s\\n" "$1" >>${shellQuote(probePath)}; return 0; }`,
          "natesclaw_e2e_wait_mock_openai 44080 1 400",
          "natesclaw_e2e_wait_mock_openai 443 1 400 https://api.openai.com:443",
        ].join("; "),
      );
  ...
  it("probes /readyz on the explicit gateway port", () => {
    withTempDir("natesclaw-e2e-readyz-port-", (tempDir) => {
      const logPath = path.join(tempDir, "gateway.log");
      const probePath = path.join(tempDir, "probe-url.txt");
      const result = runBashWithHelper(
        [
          ...
          `natesclaw_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2 23456`,
        ],
        {},
        5_000,
      );
  ...
  it("allows explicit legacy ready-log mode without /readyz", () => {
    withTempDir("natesclaw-e2e-readyz-legacy-", (tempDir) => {
      const logPath = path.join(tempDir, "gateway.log");
      const result = runBashWithHelper(
        [
          "natesclaw_e2e_probe_http() { return 1; }",
          ...
          "trap 'kill \"$gateway_pid\" >/dev/null 2>&1 || true' EXIT",
          `printf '[gateway] ready\\n' >${shellQuote(logPath)}`,
          `natesclaw_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2 23456 legacy-ready-log-ok`,
        ],
        {},
        5_000,
      );
  ...
  it("wraps package installs with the configured timeout", () => {
    withTempDir("natesclaw-e2e-instance-", (tempDir) => {
      ...
      const result = runPackageInstall(fixture, {
        ...
        NATESCLAW_E2E_NPM_INSTALL_TIMEOUT: "42s",
        NATESCLAW_TEST_TIMEOUT_ARGS: fixture.timeoutArgsPath,
        NATESCLAW_TEST_NPM_ARGS: fixture.npmArgsPath,
        NATESCLAW_TEST_NPM_BIN: path.join(tempDir, "npm"),
      });
  ...
  it("falls back to plain timeout when kill-after is unavailable", () => {
    withTempDir("natesclaw-e2e-instance-plain-timeout-", (tempDir) => {
      const fixture = createPackageInstallFixture(tempDir);
  ...
  it("uses gtimeout when GNU timeout is not on PATH", () => {
    withTempDir("natesclaw-e2e-instance-gtimeout-", (tempDir) => {
      const fixture = createPackageInstallFixture(tempDir);
  ...
  it("uses the Node watchdog when timeout is unavailable", () => {
    withTempDir("natesclaw-e2e-instance-no-timeout-", (tempDir) => {
      const fixture = createPackageInstallFixture(tempDir);
  ...
  it("bounds npm install failure logs to the configured tail", () => {
    withTempDir("natesclaw-e2e-instance-install-log-", (tempDir) => {
      const fixture = createPackageInstallFixture(tempDir);
      writeFakeTimeout(path.join(tempDir, "timeout"), true);
      ...
      const result = runPackageInstall(fixture, {
        PATH: `${tempDir}${path.delimiter}${hostPath}`,
        NATESCLAW_CURRENT_PACKAGE_TGZ: fixture.packagePath,
        NATESCLAW_E2E_LOG_TAIL_BYTES: "80",
        NATESCLAW_E2E_NPM_INSTALL_TIMEOUT: "42s",
        NATESCLAW_TEST_TIMEOUT_ARGS: fixture.timeoutArgsPath,
      });
  ...
  it.each([
    ["bytes", "NATESCLAW_E2E_LOG_TAIL_BYTES", "64kb"],
    ["lines", "NATESCLAW_E2E_LOG_TAIL_LINES", "25 lines"],
  ])("rejects invalid E2E log tail %s before invoking tail", (_label, envName, value) => {
    withTempDir("natesclaw-e2e-instance-log-tail-", (tempDir) => {
      const logPath = path.join(tempDir, "install.log");
      fs.writeFileSync(logPath, "old log\nrecent log\n", "utf8");

      const result = runSourcedHelper(`natesclaw_e2e_print_log ${shellQuote(logPath)}`, {
        [envName]: value,
      });
  ...
  it("bounds commands with the Node watchdog when timeout is unavailable", () => {
    withTempDir("natesclaw-e2e-instance-node-watchdog-", (tempDir) => {
      writeNodeShim(tempDir);
      const startedAt = Date.now();
      const result = runBashWithHelper(
        [
          `natesclaw_e2e_maybe_timeout 200ms ${shellQuote(process.execPath)} -e ${shellQuote("setInterval(() => {}, 1000)")}`,
        ],
        { PATH: tempDir },
        5_000,
        "; ",
      ...
      expect(result.stderr).toContain("Natesclaw E2E command timed out after 200ms");
    });
  });
  ...
  for (const [shellSignal, expectedStatus] of [
    ...
    it(`escalates Node watchdog children that ignore parent SIG${shellSignal}`, () => {
      withTempDir("natesclaw-e2e-instance-node-watchdog-signal-", (tempDir) => {
        writeNodeShim(tempDir);
...
export NATESCLAW_E2E_TIMEOUT_KILL_GRACE_MS=100
natesclaw_e2e_maybe_timeout 30s node ${shellQuote(childPath)} ${shellQuote(pidPath)} ${shellQuote(watchdogPidPath)} &
wrapper_pid="$!"
...
`;
  ...
  it("terminates only the tracked gateway process", () => {
    withTempDir("natesclaw-e2e-gateway-terminate-", (tempDir) => {
      ...
      writeBashExecutable(path.join(tempDir, "pkill"), [
        'printf "pkill %s\\n" "$*" >>"$NATESCLAW_TEST_FORBIDDEN_PROCESS_TOOL_LOG"',
        "exit 42",
      ]);
...
set -euo pipefail
source ${shellQuote(helperPath)}
natesclaw_e2e_terminate_gateways ""
/bin/sleep 30 &
tracked_pid="$!"
natesclaw_e2e_terminate_gateways "$tracked_pid"
if kill -0 "$tracked_pid" 2>/dev/null; then
  echo "tracked gateway process still alive" >&2
  exit 1
fi
[ ! -s "$NATESCLAW_TEST_FORBIDDEN_PROCESS_TOOL_LOG" ]
`;
      ...
      const result = runBash(
        script,
        {
          ...
          NATESCLAW_TEST_FORBIDDEN_PROCESS_TOOL_LOG: forbiddenToolLog,
        },
        5_000,
      );
  ...
  it("terminates descendants in the tracked process group", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-e2e-process-group-"));
...
set -euo pipefail
source ${shellQuote(helperPath)}
tracked_pid="$(natesclaw_e2e_start_tracked_process ${shellQuote(logPath)} ${shellQuote(process.execPath)} ${shellQuote(parentPath)} ${shellQuote(childPath)} ${shellQuote(parentPidPath)} ${shellQuote(childPidPath)} ${shellQuote(childTermPath)})"
for ((i = 0; i < 100; i += 1)); do
...
natesclaw_e2e_stop_process "$tracked_pid"
for ((i = 0; i < 100; i += 1)); do
...
`;
  ...
  it("bounds HTTP readiness probes when a server accepts connections but never responds", () => {
    withTempDir("natesclaw-e2e-http-probe-", (tempDir) => {
      ...
      const result = runBash(
        [
          ...
          `source ${shellQuote(helperPath)}`,
          'natesclaw_e2e_probe_http_status "http://127.0.0.1:${port}/health" 200 100',
        ],
        undefined,
        3_000,
        "; ",
  ...
  it("does not repeatedly grep the full gateway log while waiting for readiness", () => {
    withTempDir("natesclaw-e2e-readyz-incremental-", (tempDir) => {
      const logPath = path.join(tempDir, "gateway.log");
      const grepArgsPath = path.join(tempDir, "grep-args.txt");
      const result = runBashWithHelper(
        [
          "natesclaw_e2e_probe_http() { return 0; }",
          "grep() {",
          ...
          `printf 'old log line\\n' >${shellQuote(logPath)}`,
          `printf '[gateway] ready ws://127.0.0.1:23456\\n' >>${shellQuote(logPath)}`,
          `natesclaw_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2`,
        ],
        {},
        5_000,
      );
  ...
  it("detects gateway ready markers split across incremental log reads", () => {
    withTempDir("natesclaw-e2e-readyz-split-marker-", (tempDir) => {
      const logPath = path.join(tempDir, "gateway.log");
      const result = runBashWithHelper(
        [
          "natesclaw_e2e_probe_http() { return 0; }",
          ...
          `natesclaw_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 8`,
        ],
        {},
        5_000,
      );
  ...
  it("derives the readiness port only from gateway ready log lines", () => {
    withTempDir("natesclaw-e2e-readyz-port-", (tempDir) => {
      ...
      const result = runBashWithHelper(
        [
          `natesclaw_e2e_probe_http() { printf '%s' "$1" >${shellQuote(probePath)}; [[ "$1" = "http://127.0.0.1:23456/readyz" ]]; }`,
          "sleep 30 &",
          'gateway_pid="$!"',
          "trap 'kill \"$gateway_pid\" >/dev/null 2>&1 || true' EXIT",
  ...
  it("wraps logged Natesclaw E2E commands with the configured timeout", () => {
    withTempDir("natesclaw-e2e-instance-run-logged-", (tempDir) => {
      ...
      writeTimeoutDispatcher(tempDir, "fixture-command", "NATESCLAW_TEST_COMMAND_BIN");
      writeBashExecutable(path.join(tempDir, "fixture-command"), [
        'printf "%s\\n" "$*" >"$NATESCLAW_TEST_COMMAND_ARGS"',
        'printf "fixture output\\n"',
      ]);

      const result = runBashWithHelper(
        [
          `natesclaw_e2e_run_logged ${shellQuote(logLabel)} fixture-command one two`,
          `printf "%s" "$NATESCLAW_E2E_LAST_LOG_PATH" > ${shellQuote(logPathFile)}`,
        ],
        {
          PATH: `${tempDir}:${hostPath}`,
          NATESCLAW_E2E_LOG_DIR: logDir,
          NATESCLAW_E2E_COMMAND_TIMEOUT: "17s",
          NATESCLAW_TEST_TIMEOUT_ARGS: timeoutArgsPath,
          NATESCLAW_TEST_COMMAND_ARGS: commandArgsPath,
          NATESCLAW_TEST_COMMAND_BIN: path.join(tempDir, "fixture-command"),
        },
        undefined,
        "; ",
      );
      ...
      expect(path.basename(logPath)).toMatch(new RegExp(`^natesclaw-${logLabel}\\..+\\.log$`, "u"));
  ...
  it("bounds logged command failure output to the configured tail", () => {
    withTempDir("natesclaw-e2e-instance-run-log-tail-", (tempDir) => {
      const logLabel = path.basename(tempDir);
      const logDir = path.join(tempDir, "logs");
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      writeFakeTimeout(path.join(tempDir, "timeout"), true);
      ...
      const result = runBashWithHelper(
        [`natesclaw_e2e_run_logged ${shellQuote(logLabel)} fixture-command`],
        {
          PATH: `${tempDir}${path.delimiter}${hostPath}`,
          NATESCLAW_E2E_COMMAND_TIMEOUT: "17s",
          NATESCLAW_E2E_LOG_DIR: logDir,
      ...
      const logFile = expectDefined(fs.readdirSync(logDir)[0], "Natesclaw E2E command log file");
  ...
  it("installs the trash shim under isolated test state", () => {
    withTempDir("natesclaw-e2e-trash-shim-", (tempDir) => {
      ...
      const result = runBashWithHelper(
        [
          ...
          `printf "%s" "$NATESCLAW_E2E_BIN_DIR" > ${shellQuote(binDirFile)}`,
          "command -v trash >/dev/null",
        ],
        { HOME: homeDir, NATESCLAW_STATE_DIR: stateDir, PATH: hostPath },
        undefined,
        "; ",
      );
      ...
      expect(binDir).not.toBe("/tmp/natesclaw-bin");
      expect(pathEntries.filter((entry) => entry === binDir)).toHaveLength(1);
      expect(fs.existsSync(path.join(binDir, "trash"))).toBe(true);
    });
  });

  it("wraps package-installed Natesclaw CLI calls with the configured timeout", () => {
    withTempDir("natesclaw-e2e-instance-natesclaw-cli-", (tempDir) => {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const commandArgsPath = path.join(tempDir, "natesclaw-args.txt");
      writeTimeoutDispatcher(tempDir, path.join(tempDir, "natesclaw"), "NATESCLAW_TEST_NATESCLAW_BIN");
      writeBashExecutable(path.join(tempDir, "natesclaw"), [
        'printf "%s\\n" "$*" >"$NATESCLAW_TEST_COMMAND_ARGS"',
      ]);
      ...
      const result = runBashWithHelper(
        ...
        {
          PATH: `${tempDir}:${hostPath}`,
          NATESCLAW_E2E_COMMAND_TIMEOUT: "23s",
          NATESCLAW_TEST_TIMEOUT_ARGS: timeoutArgsPath,
          NATESCLAW_TEST_COMMAND_ARGS: commandArgsPath,
          NATESCLAW_TEST_NATESCLAW_BIN: path.join(tempDir, "natesclaw"),
        },
        undefined,
        "; ",
      );
      ...
      expect(result.status).toBe(0);
      expect(fs.readFileSync(timeoutArgsPath, "utf8").trim()).toBe(
        `--kill-after=30s 23s ${path.join(tempDir, "natesclaw")} plugins list --json`,
      );
  ...
  it("wraps interactive PTY scripts with the configured timeout", () => {
    withTempDir("natesclaw-e2e-instance-pty-timeout-", (tempDir) => {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const scriptArgsPath = path.join(tempDir, "script-args.txt");
      const logPath = path.join(tempDir, "pty.log");
      writeTimeoutDispatcher(tempDir, "script", "NATESCLAW_TEST_SCRIPT_BIN");
      writeBashExecutable(path.join(tempDir, "script"), [
        'if [ "${1:-}" = "--version" ]; then exit 0; fi',
        'printf "%s\\n" "$*" >"$NATESCLAW_TEST_SCRIPT_ARGS"',
      ]);

      const result = runBashWithHelper(
        [
          `natesclaw_e2e_run_script_with_pty ${shellQuote("node /tmp/entry onboard")} ${shellQuote(logPath)}`,
        ],
        {
          PATH: `${tempDir}:${hostPath}`,
          NATESCLAW_E2E_COMMAND_TIMEOUT: "31s",
          NATESCLAW_TEST_TIMEOUT_ARGS: timeoutArgsPath,
          NATESCLAW_TEST_SCRIPT_ARGS: scriptArgsPath,
          NATESCLAW_TEST_SCRIPT_BIN: path.join(tempDir, "script"),
        },
        undefined,
        "; ",
      );

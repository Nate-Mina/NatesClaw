...
type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<NatesclawConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<NatesclawConfig["tools"]>["web"]>["search"]>;
...
function defaultModelUsesCodexRuntime(config: NatesclawConfig): boolean {
...
function rawKeyValue(config: NatesclawConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: NatesclawConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: NatesclawConfig, provider: SearchProvider): SecretRef {
  const entry =
...
export function applySearchKey(
  ...
  const nextBase: NatesclawConfig = {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: NatesclawConfig, result: NatesclawConfig): NatesclawConfig {
  ...
  return {
    ...next,
    plugins: nextPlugins as NatesclawConfig["plugins"],
  };
}
...
type SearchSetupResult =
  | { outcome: "completed"; config: NatesclawConfig }
  | {
...
function completedSearchSetup(config: NatesclawConfig): SearchSetupResult {
...
export async function runSearchSetupFlow(
  ...
  const finalizeSelection = (nextConfig: NatesclawConfig) =>

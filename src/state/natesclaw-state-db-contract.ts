...
export const NATESCLAW_STATE_SCHEMA_VERSION = 6;
export const NATESCLAW_STATE_STRICT_SCHEMA_VERSION = 3;
...
export const NATESCLAW_SQLITE_BUSY_TIMEOUT_MS = 5_000;
/** User-facing guide for schema refusals; lives here so error sites avoid import cycles. */
export const NATESCLAW_DATABASE_SCHEMA_DOCS_URL =
  "https://docs.natesclaw.ai/reference/database-schemas";

/** Open shared SQLite database handle plus WAL maintenance lifecycle. */
export type NatesclawStateDatabase = {
  db: DatabaseSync;
  path: string;
  walMaintenance: SqliteWalMaintenance;
};
/** Options for resolving or overriding the shared state database path. */
export type NatesclawStateDatabaseOptions = {
  env?: NodeJS.ProcessEnv;
  path?: string;
  database?: NatesclawStateDatabase;
  readOnly?: boolean;
};
export type NatesclawStateDatabaseSchemaMigration = {
  kind:

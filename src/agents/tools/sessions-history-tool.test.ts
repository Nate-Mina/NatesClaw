...
function readMessageSeq(message: unknown): number | undefined {
  ...
  const meta = (message as Record<string, unknown>)["__natesclaw"];
...
describe("sessions_history redaction", () => {
  beforeAll(async () => {
    previousConfigPath = process.env.NATESCLAW_CONFIG_PATH;
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-sessions-history-redact-"));
  ...
  afterAll(() => {
    if (previousConfigPath === undefined) {
      ...
      setTestEnvValue("NATESCLAW_CONFIG_PATH", previousConfigPath);
    }
    if (tempDir) {
  ...
  it("keeps the anchored message when the history byte cap trims neighbors", async () => {
    ...
    const messages = Array.from({ length: 30 }, (_, index) => ({
      ...
      __natesclaw: { id: `message-${index + 1}`, seq: index + 1 },
    }));
    const tool = createSessionsHistoryTool({
      config: {},
  ...
  it("uses the oldest visible message for pagination after tool messages are filtered", async () => {
    const tool = createSessionsHistoryTool({
      ...
      callGateway: async <T = Record<string, unknown>>(): Promise<T> =>
        ({
          messages: [
            { role: "tool", content: "hidden", __natesclaw: { seq: 6 } },
            { role: "assistant", content: "visible", __natesclaw: { seq: 7 } },
            { role: "assistant", content: "latest", __natesclaw: { seq: 8 } },
          ],
          offset: 0,
          nextOffset: 5,
          hasMore: true,

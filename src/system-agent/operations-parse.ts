// Natesclaw operation grammar, approval descriptions, and public types.
...
export type SystemAgentOperationResult = {
  ...
  message?: string;
  nextInput?: string;
  /** Agent TUI exited via /natesclaw: re-enter the shell even without a request. */
...
const SETUP_RE = new RegExp(
  String.raw`^(?:setup|set\s+me\s+up|set\s+up\s+natesclaw|onboard(?:\s+me)?|bootstrap|first\s+run)(?:\s+workspace\s+(?<workspace>${ARG_WORD}))?(?:\s+model\s+(?<model>\S+))?$`,
  "i",
);
...
/**
 * Parse one user command into Natesclaw's closed operation union. Anything
...
export function parseSystemAgentOperation(input: string): SystemAgentOperation {
  ...
  switch (lower) {
    ...
    case "quit":
    case "exit":
      return { kind: "none", message: "Natesclaw retracts into shell. Bye." };
    default:
      break;
  }
...
export function describeSystemAgentPersistentOperation(operation: SystemAgentOperation): string {
  switch (operation.kind) {
    ...
    case "doctor-fix":
      return "run natesclaw doctor --fix on the machine running Natesclaw, with Natesclaw stopped";
    case "plugin-install":
...
function formatSetupPlanDescription(
  ...
  return `bootstrap Natesclaw setup for workspace ${workspace}`;
}

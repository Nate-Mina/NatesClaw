/**
 * Strips Natesclaw-injected inbound metadata blocks from a user-role message
 ...
 *
 * Detection: every Natesclaw-injected context header is stamped with a fixed
 * provenance marker `⟦natesclaw:ctx⟧`. Strippers key on this marker rather than
...
export function stripInboundMetadata(text: string): string {
  ...
  for (let i = 0; i < strippedLeadingPrefixLines.length; i++) {
    ...
    // Channel context is appended by Natesclaw as a terminal metadata suffix.

...
async function withHostedDocumentServer<T>(
  ...
  const stateDir = await mkdtemp(path.join(tmpdir(), "natesclaw-canvas-gateway-"));
  ...
  return await withEnvAsync(
    {
      NATESCLAW_SKIP_CANVAS_HOST: params.skipHost,
...
describe("core Canvas Gateway host switches", () => {
  ...
  it.each([
    ...
    { label: "NATESCLAW_SKIP_CANVAS_HOST", config: {}, skipHost: "1" },

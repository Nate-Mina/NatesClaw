...
const logPathTracker = createSuiteLogPathTracker("natesclaw-log-cap-");
...
describe("log file size cap", () => {
  ...
  it("keeps cached default rolling loggers on the current-day file", async () => {
    ...
    const firstDay = path.join(logDir, "natesclaw-2026-01-01.log");
    const secondDay = path.join(logDir, "natesclaw-2026-01-02.log");
    vi.useFakeTimers();
  ...
  it("keeps an explicit profile-shaped log path stable across date changes", async () => {
    const logDir = path.dirname(logPath);
    const configured = path.join(logDir, "natesclaw-dev-2026-01-01.log");
    const inferredNextDay = path.join(logDir, "natesclaw-dev-2026-01-02.log");
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-01-01T08:00:00Z"));

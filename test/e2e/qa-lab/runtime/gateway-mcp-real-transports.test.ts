...
function createRepoRoot() {
  return tempDirs.make("natesclaw-qalab-cli-entry-");
}
...
describe("gateway MCP real transport producer", () => {
  it("uses the source channel MCP module when build output is absent", async () => {
    ...
    expect(mcp.args[3]).toContain("serveNatesclawChannelMcp");
    expect(mcp.cwd).toBe(root);
    expect(mcp.envPatch).toStrictEqual({
      NATESCLAW_QA_GATEWAY_TOKEN: "secret-token",
      NATESCLAW_QA_GATEWAY_URL: "ws://127.0.0.1:12345",
    });
  });
  ...
  it("isolates the source plugin-tools MCP invocation", () => {
    ...
    const invocation = testing.resolvePluginToolsMcpInvocation({
      configPath: "/tmp/plugin-tools/natesclaw.json",
    ...
    expect(invocation).toStrictEqual({
      ...
      env: {
        ...
        NATESCLAW_CONFIG_PATH: "/tmp/plugin-tools/natesclaw.json",
        NATESCLAW_DISABLE_BUNDLED_PLUGINS: "1",
        NATESCLAW_HOME: "/tmp/plugin-tools/home",
        NATESCLAW_STATE_DIR: "/tmp/plugin-tools/state",
      },
    });
  });

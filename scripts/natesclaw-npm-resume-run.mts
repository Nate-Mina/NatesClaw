...
export interface NatesclawNpmResumeValidationInput {
...
const WORKFLOW_PATH = ".github/workflows/natesclaw-npm-release.yml";
...
function requiredString(value: unknown, label: string): string {
  if (typeof value !== "string" || value.length === 0) {
    fail(`Natesclaw npm resume run is missing ${label}.`);
  }
  return value;
}
...
function requiredSha(value: unknown, label: string): string {
  ...
  if (!SHA_PATTERN.test(sha)) {
    fail(`Natesclaw npm resume run has invalid ${label}.`);
  }
  return sha;
}
...
export function validateNatesclawNpmResumeRun({
  canonicalWorkflowId,
  compareStatus,
  jobs,
  run,
  ...
  if (!branchMatch) {
    fail(`Natesclaw npm resume run has an untrusted workflow ref: ${url}`);
  }
  ...
  if (
    ...
    fail(`Natesclaw npm resume run has an untrusted workflow identity: ${url}`);
  }

  const tagObjectSha = requiredSha(tagRef?.object?.sha, "tooling tag object SHA");
  if (tagRef?.object?.type !== "tag") {
    fail(`Natesclaw npm resume run tooling ref is not a signed annotated tag: ${url}`);
  }
  ...
  if (
    ...
    fail(
      `Natesclaw npm resume run is not bound to a real, main-reachable protected tooling tag: ${url}`,
    );
  }
  ...
  if (
    ...
    fail(`Natesclaw npm resume run lacks successful parent release approval validation: ${url}`);
  }
...
export function runNatesclawNpmResumeGh(
  args: string[],
  params: {
...
export function resolveNatesclawNpmResumeRun({
  repo,
  runId,
  runGh = runNatesclawNpmResumeGh,
}: {
  repo: string;
  runId: string;
  ...
  if (!/^[1-9][0-9]*$/u.test(runId)) {
    fail("Natesclaw npm resume run id must be a positive integer.");
  }
  if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/u.test(repo)) {
    fail("Natesclaw npm resume repository must be owner/name.");
  }
  ...
  return validateNatesclawNpmResumeRun({
...
function main(argv: string[] = process.argv.slice(2)): void {
  const result = resolveNatesclawNpmResumeRun(parseArgs(argv));

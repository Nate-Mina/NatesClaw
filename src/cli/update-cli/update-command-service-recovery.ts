...
export async function hasLoadedLaunchdKeepAliveSupervisor(params: {
  ...
  // Natesclaw's loaded LaunchAgent has canonical KeepAlive policy. Read this once before
...
function formatPostUpdateGatewayRecoveryLine(platform: NodeJS.Platform): string {
  const restartCommand = replaceCliName(formatCliCommand("natesclaw gateway restart"), CLI_NAME);
  const installCommand = replaceCliName(
    formatCliCommand("natesclaw gateway install --force"),
    CLI_NAME,
  );
...
export function formatPostUpdateGatewayRecoveryInstructions(
  ...
  if (isPackageManagerUpdateMode(result.mode) && beforeVersion) {
    lines.push(
      `Rollback: reinstall Natesclaw ${beforeVersion} with the same package manager, then rerun \`${replaceCliName(formatCliCommand("natesclaw gateway install --force"), CLI_NAME)}\`.`,
    );
  }
  return lines;
}

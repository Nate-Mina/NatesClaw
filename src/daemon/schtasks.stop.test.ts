...
async function withPreparedGatewayTask(
  ...
  await withWindowsEnv("natesclaw-win-stop-", async ({ env }) => {
...
describe("Scheduled Task stop/restart cleanup", () => {
  it("suspends a task whose Settings.Enabled value uses the default", async () => {
    await withPreparedGatewayTask(async ({ env }) => {
      ...
      expect(schtasksCalls).toEqual([
        ["/Query", "/TN", "Natesclaw Gateway", "/XML"],
        ["/Change", "/TN", "Natesclaw Gateway", "/DISABLE"],
      ]);
    });
  });
  ...
  it("preserves an already-disabled task", async () => {
    await withPreparedGatewayTask(async ({ env }) => {
      ...
      expect(schtasksCalls).toEqual([["/Query", "/TN", "Natesclaw Gateway", "/XML"]]);
    });
  });
  ...
  it("restores an enabled task after an ambiguous disable failure", async () => {
    await withPreparedGatewayTask(async ({ env }) => {
      ...
      expect(schtasksCalls).toEqual([
        ["/Query", "/TN", "Natesclaw Gateway", "/XML"],
        ["/Change", "/TN", "Natesclaw Gateway", "/DISABLE"],
        ["/Change", "/TN", "Natesclaw Gateway", "/ENABLE"],
      ]);
    });
  });
  ...
  it("reenables a task after the update window", async () => {
    await withPreparedGatewayTask(async ({ env }) => {
      ...
      expect(schtasksCalls).toEqual([["/Change", "/TN", "Natesclaw Gateway", "/ENABLE"]]);
    });
  });
  ...
  it.each([
    ...
    await withPreparedGatewayTask(async ({ env, stdout }) => {
      ...
      expect(schtasksCalls).toEqual([
        ["/Query"],
        ["/Query", "/TN", "Natesclaw Gateway"],
        ["/End", "/TN", "Natesclaw Gateway"],
      ]);
      expect(spawnSync).toHaveBeenCalledOnce();
  ...
  it("does not adopt a portless arbitrary task action", async () => {
    await withPreparedGatewayTask(async ({ env }) => {
      delete env.NATESCLAW_GATEWAY_PORT;
        ...
        const processes = [
          {
            ProcessId: 3131,
            CommandLine:
              '"C:\\Program Files\\nodejs\\node.exe" "C:\\other-natesclaw.cjs" gateway --port 18789',
          },
          ...(!forced
            ? [
                {
  ...
  it("starts a registered task and ignores audit observer failures", async () => {
    await withPreparedGatewayTask(async ({ env }) => {
      ...
      expect(schtasksCalls).toContainEqual(["/Run", "/TN", "Natesclaw Gateway"]);
  ...
  it("does not reclaim gateway listeners when stopping a node Scheduled Task", async () => {
    await withPreparedGatewayTask(async ({ env, stdout }) => {
      ...
      expect(schtasksCalls).toEqual([
        ["/Query"],
        ["/Query", "/TN", "Natesclaw Node"],
        ["/End", "/TN", "Natesclaw Node"],
      ]);
    });
  });
  ...
  it("kills the owned gateway process and waits for port release before restart", async () => {
    await withPreparedGatewayTask(async ({ env, stdout }) => {
      ...
      expect(schtasksCalls).toEqual([
        ...
        ["/Query", "/TN", "Natesclaw Gateway"],
        ["/End", "/TN", "Natesclaw Gateway"],
        ["/Run", "/TN", "Natesclaw Gateway"],
        ["/Query"],
        ["/Query", "/TN", "Natesclaw Gateway", "/V", "/FO", "LIST"],
        ["/Query"],
        ["/Query", "/TN", "Natesclaw Gateway", "/V", "/FO", "LIST"],
      ]);
    });
  ...
  it("does not wait on or force-kill the gateway port when restarting a node Scheduled Task", async () => {
    await withPreparedGatewayTask(async ({ env, stdout }) => {
      ...
      expect(schtasksCalls).toEqual([
        ...
        ["/Query", "/TN", "Natesclaw Node"],
        ["/End", "/TN", "Natesclaw Node"],
        ["/Run", "/TN", "Natesclaw Node"],
        ["/Query"],
        ["/Query", "/TN", "Natesclaw Node", "/V", "/FO", "LIST"],
        ["/Query"],
        ["/Query", "/TN", "Natesclaw Node", "/V", "/FO", "LIST"],
      ]);
    });
  ...
  it("throws when /Run fails during restart", async () => {
    await withPreparedGatewayTask(async ({ env, stdout }) => {
      ...
      expect(schtasksCalls.at(-1)).toEqual(["/Run", "/TN", "Natesclaw Gateway"]);
    });
  });
});

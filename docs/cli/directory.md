---
summary: "CLI reference for `natesclaw directory` (self, peers, groups)"
read_when:
...
Results are meant to be pasted into other commands, especially `natesclaw message send --target ...`.
...
```bash
natesclaw directory peers list --channel slack --query "U0"
natesclaw message send --channel slack --target user:U012ABCDEF --message "hello"
```
...
```bash
natesclaw directory peers list --channel zalouser
natesclaw directory peers list --channel zalouser --query "name"
natesclaw directory peers list --channel zalouser --limit 50
```

## Groups

```bash
natesclaw directory groups list --channel zalouser
natesclaw directory groups list --channel zalouser --query "work"
natesclaw directory groups members --channel zalouser --group-id <id>
```

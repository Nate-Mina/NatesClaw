...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: NatesclawConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): NatesclawConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("natesclaw/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("natesclaw/plugin-sdk/acp-runtime")>(
...
vi.mock("natesclaw/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("natesclaw/plugin-sdk/native-command-config-runtime")
  >("natesclaw/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
...
vi.mock("natesclaw/plugin-sdk/error-runtime", async () => {
  const actual = await vi.importActual<typeof import("natesclaw/plugin-sdk/error-runtime")>(
    "natesclaw/plugin-sdk/error-runtime",
  );
  return {
    ...actual,

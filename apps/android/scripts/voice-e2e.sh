...
PACKAGE_NAME="ai.natesclaw.app"
RECEIVER="$PACKAGE_NAME/.VoiceE2eReceiver"
RUN_ACTION="ai.natesclaw.app.debug.RUN_VOICE_E2E"
OPEN_ACTION="ai.natesclaw.app.debug.OPEN_VOICE_E2E"
PORT=18789
HOST="127.0.0.1"
MODE="both"
...
ARTIFACT_DIR="/tmp/natesclaw-android-voice-e2e-$(date +%Y%m%d-%H%M%S)"
...
if [[ "$START_GATEWAY" -eq 1 ]]; then
  if command -v bws_get_secret >/dev/null 2>&1; then
    NATESCLAW_OPENAI_API_KEY="$(bws_get_secret NATESCLAW_OPENAI_API_KEY)"
  else
    NATESCLAW_OPENAI_API_KEY="$(zsh -ic 'bws_get_secret NATESCLAW_OPENAI_API_KEY')"
  fi
  (
    cd "$ROOT_DIR"
    OPENAI_API_KEY="$NATESCLAW_OPENAI_API_KEY" \
      pnpm natesclaw gateway run \
        --port "$PORT" \
        --auth none \
        --bind loopback \
        --force \
  ...
  unset NATESCLAW_OPENAI_API_KEY
fi
...
adb logcat -d -v time |
  rg -i 'Natesclaw|TalkMode|MicCapture|AudioRecord|SpeechRecognizer|realtime|talk.session|appendAudio|transcript|Talk failed|Transcription failed|Speech network|VoiceE2E' |

---
summary: "CLI reference for `natesclaw channels` (accounts, status, dead letters, capabilities, resolve, logs, login/logout)"
read_when:
...
```bash
natesclaw channels list
natesclaw channels list --all
natesclaw channels status
natesclaw channels capabilities
natesclaw channels capabilities --channel discord --target channel:123
natesclaw channels resolve --channel slack "#general" "@jane"
natesclaw channels logs --channel all
natesclaw channels dead-letters list --channel telegram --account default
```

`channels list` shows chat channels only: configured accounts by default, with `installed`, `configured`, and `enabled` status tags per account (`--json` for machine output). Pass `--all` to also surface bundled channels that have no configured account yet and installable catalog channels that are not yet on disk. Provider auth and model usage live elsewhere: `natesclaw models auth list` for provider auth profiles, `natesclaw status` or `natesclaw models list` for usage/quota.
...
```bash
natesclaw channels dead-letters list --channel telegram --account default
natesclaw channels dead-letters list --channel telegram --account default --json
```
...
```bash
natesclaw channels dead-letters resubmit <event-id> --channel telegram --account default
```
...
`natesclaw health` reports dead-letter counts and oldest failure age per channel account. `natesclaw doctor` names affected accounts and points back to the inspection command.

Do not use `natesclaw sessions`, Gateway `sessions.list`, or the agent
...
```bash
natesclaw channels add --channel telegram --token <bot-token>
natesclaw channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
natesclaw channels remove --channel telegram --delete
```

<Tip>
`natesclaw channels add telegram --help` or `natesclaw channels add --channel telegram --help` shows only Telegram's setup flags. `natesclaw channels add --help` shows only the shared command envelope.
</Tip>
...
If a channel plugin needs to be installed during a flag-driven add command, Natesclaw uses the channel's default install source without opening the interactive plugin install prompt.
...
When you run `natesclaw channels add` with no direct account, credential, or channel-config flags, the interactive wizard can prompt. A positional channel id and `--channel <id>` both open that channel's guided setup immediately. Back returns to the full channel picker:

```bash
natesclaw channels add telegram
natesclaw channels add --channel telegram
```
...
You can also manage the same routing rules later with `natesclaw agents bindings`, `natesclaw agents bind`, and `natesclaw agents unbind` (see [agents](/cli/agents)).

When you add a non-default account to a channel that is still using single-account top-level settings, Natesclaw promotes those top-level values into the channel's account map before writing the new account. Promotion reuses an existing named account when the channel has exactly one, or when `defaultAccount` points at one; otherwise the values land in `channels.<channel>.accounts.default`.
...
If your config was already in a mixed state (named accounts present and top-level single-account values still set), run `natesclaw doctor --fix` to move account-scoped values into the promoted account chosen for that channel.

## Login and logout (interactive)

```bash
natesclaw channels login --channel whatsapp
natesclaw channels logout --channel whatsapp
```
...
- Run `natesclaw status --deep` for a broad probe.
- Use `natesclaw doctor` for guided fixes.
- `natesclaw channels status` falls back to config-only summaries when the gateway is unreachable. If a supported channel credential is configured via SecretRef but unavailable in the current command path, it reports that account as configured with degraded notes instead of showing it as not configured.
...
```bash
natesclaw channels resolve --channel slack "#general" "@jane"
natesclaw channels resolve --channel discord "My Server/#support" "@someone"
natesclaw channels resolve --channel matrix "Project Room"
```

...
const enablePluginInConfig = vi.hoisted(() =>
  vi.fn<(cfg: NatesclawConfig, pluginId: string) => PluginEnableResult>((cfg, pluginId) => ({
    config: cfg,
    enabled: true,
    pluginId,
  })),
...
const recordPluginInstall = vi.hoisted(() =>
  vi.fn((cfg: NatesclawConfig, update: { pluginId: string }) => ({
    ...cfg,
    plugins: {
      ...cfg.plugins,
      installs: {
...
describe("ensureOnboardingPluginInstalled", () => {
  ...
  it("localizes plugin install choices", async () => {
    ...
    try {
      ...
      expect(captured?.options).toEqual([
        { value: "npm", label: "从 npm 下载（@natesclaw/qqbot@beta）" },
      ...
      await expect(
        ensureOnboardingPluginInstalled({
          ...
          entry: {
            ...
            label: "Demo Provider",
            install: {
              npmSpec: "@natesclaw/demo-plugin@1.2.3",
            },
          },
          promptInstall: false,
          prompter: {
  ...
  it("uses a guarded npm-pack install override for the matching plugin id", async () => {
    ...
    process.env.NATESCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES = "1";
    process.env.NATESCLAW_PLUGIN_INSTALL_OVERRIDES = JSON.stringify({
    ...
    installPluginFromNpmPackArchive.mockResolvedValue({
      ok: true,
      pluginId: "demo-plugin",
      targetDir: "/tmp/natesclaw/extensions/demo-plugin",
      version: "1.2.3",
    ...
    expect(recordUpdate).toEqual({
      ...
      installPath: "/tmp/natesclaw/extensions/demo-plugin",
      version: "1.2.3",
  ...
  it("uses a guarded npm install override without official-trust flags", async () => {
    process.env.NATESCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES = "1";
    process.env.NATESCLAW_PLUGIN_INSTALL_OVERRIDES = JSON.stringify({
      codex: "npm:@natesclaw/codex@2026.5.8",
      "other-plugin": "npm-pack:/tmp/other.tgz",
    });
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "codex",
      targetDir: "/tmp/natesclaw/extensions/codex",
      version: "2026.5.8",
      npmResolution: {
        name: "@natesclaw/codex",
        version: "2026.5.8",
        resolvedSpec: "@natesclaw/codex@2026.5.8",
      },
    });
    ...
    expect(npmCall.spec).toBe("@natesclaw/codex@2026.5.8");
    ...
    const result = await ensureOnboardingPluginInstalled({
      ...
      entry: {
        ...
        install: {
          ...
          npmSpec: "@natesclaw/demo-plugin@2026.5.2",
  ...
  it("passes npm specs and optional expected integrity to npm installs with progress", async () => {
    ...
    const npmResolution = {
      ...
      version: "1.2.3",
      resolvedSpec: "@wecom/wecom-natesclaw-plugin@1.2.3",
    ...
    expect(npmCall.spec).toBe("@wecom/wecom-natesclaw-plugin@1.2.3");
    ...
    expect(recordUpdate.spec).toBe("@wecom/wecom-natesclaw-plugin@1.2.3");
    expect(recordUpdate.installPath).toBe("/tmp/demo-plugin");
    ...
    expect(installed?.spec).toBe("@wecom/wecom-natesclaw-plugin@1.2.3");
    expect(clearLoadInstalledPluginIndexInstallRecordsCache).toHaveBeenCalledOnce();
    expect(clearPluginMetadataLifecycleCaches).toHaveBeenCalledOnce();
    expect(invalidatePluginRuntimeDiscoveryAfterConfigMutation).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("installs trusted official plugins at the exact extended-stable core version", async () => {
    installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        ...
        version: VERSION,
        resolvedSpec: `@natesclaw/discord@${VERSION}`,
      },
    });
    ...
    expect(npmCall.spec).toBe(`@natesclaw/discord@${VERSION}`);
    const [, recordUpdate] = readFirstMockCall(recordPluginInstall, "recordPluginInstall") as [
      NatesclawConfig,
      PluginInstallRecord,
    ];
    expect(recordUpdate.spec).toBe("@natesclaw/discord");
  ...
  it("preserves default intent for trusted official stable installs", async () => {
    installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@natesclaw/discord",
        version: "2026.7.21",
        resolvedSpec: "@natesclaw/discord@2026.7.21",
      },
    });
  ...
  it("defaults dual-source remote installs to npm unless ClawHub is explicit", async () => {
    ...
    expect(captured?.options).toEqual([
      ...
      { value: "npm", label: "Download from npm (@natesclaw/demo-plugin@2026.5.2)" },
      { value: "skip", label: "Skip for now" },
    ]);
    expect(captured?.initialValue).toBe("npm");
    expect(installPluginFromClawHub).not.toHaveBeenCalled();
  ...
  it("falls back from ClawHub to npm when the ClawHub artifact is unavailable", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        ...
        version: "2026.5.2",
        resolvedSpec: "@natesclaw/demo-plugin@2026.5.2",
    ...
    expect(npmCall.spec).toBe("@natesclaw/demo-plugin@2026.5.2");
    expect(npmCall.expectedPluginId).toBe("demo-plugin");
    expect(result.installed).toBe(true);
  });
  ...
  it("does not offer local installs when the workspace only has a spoofed .git marker", async () => {
    await withTestDir({ prefix: "natesclaw-onboarding-install-spoofed-git-" }, async (temp) => {
  ...
  it("allows local installs for real gitdir checkouts and sanitizes prompt text", async () => {
    await withTestDir({ prefix: "natesclaw-onboarding-install-gitdir-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(pluginDir, { recursive: true });
  ...
  it("does not add local plugin paths when enablement is blocked by policy", async () => {
    await withTestDir({ prefix: "natesclaw-onboarding-install-blocked-enable-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(pluginDir, { recursive: true });
      await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
  ...
  it("allows local installs for linked git worktrees", async () => {
    await withTestDir({ prefix: "natesclaw-onboarding-install-worktree-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
  ...
  it("records local install source metadata when a local path is selected", async () => {
    await withTestDir({ prefix: "natesclaw-onboarding-install-local-record-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
      await fs.mkdir(pluginDir, { recursive: true });
      ...
      const [recordCfg, recordUpdate] = readFirstMockCall(
        recordPluginInstall,
        "recordPluginInstall",
      ) as [NatesclawConfig, PluginInstallRecord];
  ...
  it("hides the npm download option for bundled plugins so the menu matches non-npm channels", async () => {
    await withTestDir({ prefix: "natesclaw-onboarding-install-bundled-prompt-" }, async (temp) => {
      ...
      const prompt = requireCapturedPrompt(captured);
      // "Download from npm (@natesclaw/tlon)" must NOT appear: the bundled
  ...
  it("enables bundled plugins without adding their bundled directory as a local install", async () => {
    await withTestDir({ prefix: "natesclaw-onboarding-install-bundled-record-" }, async (temp) => {
  ...
  it("records local install source metadata when npm install falls back to local", async () => {
    await withTestDir(
      { prefix: "natesclaw-onboarding-install-npm-fallback-record-" },
      async (temp) => {
        const workspaceDir = path.join(temp, "workspace");
        const pluginDir = path.join(workspaceDir, "plugins", "demo");
        await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
  ...
  it("records absolute local catalog paths as workspace-relative source metadata", async () => {
    await withTestDir({ prefix: "natesclaw-onboarding-install-portable-record-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
      await fs.mkdir(pluginDir, { recursive: true });
  ...
  it("keeps local installs available when cwd is a git repo but workspaceDir is not", async () => {
    await withTestDir({ prefix: "natesclaw-onboarding-install-cwd-git-" }, async (temp) => {
  ...
  it("rejects local install paths outside the trusted workspace roots", async () => {
    await withTestDir({ prefix: "natesclaw-onboarding-install-outside-root-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
  ...
  it("rejects local install paths when relative resolution looks cross-drive", async () => {
    await withTestDir({ prefix: "natesclaw-onboarding-install-cross-drive-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
      await fs.mkdir(pluginDir, { recursive: true });

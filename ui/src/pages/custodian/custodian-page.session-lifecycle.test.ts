    ...
    const request = vi
      ...
      .mockRejectedValueOnce(
        new GatewayProtocolRequestError({
          code: "UNAVAILABLE",
          message: "Natesclaw inference became unavailable.",
    ...
    const request = vi
      ...
      .mockRejectedValueOnce(
        new GatewayProtocolRequestError({
          ...
          message: "No active Natesclaw chat session is awaiting that wizard answer.",
          details: buildSystemAgentSessionInvalidatedErrorDetails(),
        }),
      )
      .mockResolvedValueOnce({
  ...
  it("starts fresh without replaying an invalidated typed wizard cancel", async () => {
    const request = vi
      ...
      .mockRejectedValueOnce(
        new GatewayProtocolRequestError({
          code: "INVALID_REQUEST",
          message: "No active Natesclaw chat session is awaiting that wizard cancel.",
          details: buildSystemAgentSessionInvalidatedErrorDetails(),
        }),
      )
      .mockResolvedValueOnce({

 ...
 */
import { parseBrowserHttpUrl } from "natesclaw/plugin-sdk/browser-config";
import {
  extractErrorCode,
  formatErrorMessage,
  toErrorObject,
...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-natesclaw-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-natesclaw-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-natesclaw-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function resolveDispatcherBrowserControlOwnership(url: string): BrowserControlOwnership {
  ...
  try {
    ...
    return profile.driver === "natesclaw" && profile.cdpIsLoopback && !profile.attachOnly
      ? "local-managed"
      : "external-browser";
  } catch {
    return "unknown";
...
function resolveBrowserFetchOperatorHint(
  ...
  if (opts?.ownership === "external-browser") {
    return (
      "The browser profile is external to Natesclaw; make sure its browser/CDP endpoint " +
      "is running and reachable. Restarting the Natesclaw gateway will not launch it."
    );
  }
  const isLocal = !isAbsoluteHttp(url);
  return isLocal
    ? `Restart the Natesclaw gateway (Natesclaw.app menubar, or \`${formatCliCommand("natesclaw gateway")}\`).`
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  if (kind === "timeout") {
    return new Error(
      `Can't reach the Natesclaw browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${BROWSER_TOOL_TRANSIENT_MODEL_HINT}`,
  ...
  if (kind === "transient-network") {
    return new Error(
      `Can't reach the Natesclaw browser control service. ${operatorHint} (${msg}) ${BROWSER_TOOL_TRANSIENT_MODEL_HINT}`,
      err instanceof Error ? { cause: err } : undefined,
    );
  }
  return new Error(
    appendBrowserToolModelHint(
      `Can't reach the Natesclaw browser control service. ${operatorHint} (${msg})`,

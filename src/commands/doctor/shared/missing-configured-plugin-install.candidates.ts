...
import { compareNatesclawReleaseVersions } from "../../../infra/npm-registry-spec.js";
import {
...
const NATESCLAW_BETA_COMPANION_VERSION_RE = /^(\d{4}\.[1-9]\d?\.[1-9]\d?)-beta\.[1-9]\d*$/;
const NATESCLAW_STABLE_OR_BETA_COMPANION_VERSION_RE =
...
function installedRuntimePackageVersionIsStale(params: {
  ...
  const comparison = compareNatesclawReleaseVersions(params.installedVersion, params.currentVersion);
...
function betaCompanionMatchesCurrentStableVersion(params: {
  ...
  const installedBase = NATESCLAW_BETA_COMPANION_VERSION_RE.exec(params.installedVersion)?.[1];
  const currentBase = NATESCLAW_STABLE_OR_BETA_COMPANION_VERSION_RE.exec(params.currentVersion)?.[1];

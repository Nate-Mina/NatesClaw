...
describe("repairLegacySubagentSuspensionReasons", () => {
  it("rewrites the shipped reason on open and stays canonical after a second open", () => {
    const stateDir = tempDirs.make("natesclaw-subagent-suspension-backfill-");
    const options = { env: { NATESCLAW_STATE_DIR: stateDir } };
    const initial = openNatesclawStateDatabase(options);
    ...
    const firstOpen = openNatesclawStateDatabase(options);
    ...
    const secondOpen = openNatesclawStateDatabase(options);

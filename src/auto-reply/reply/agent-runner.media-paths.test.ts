...
describe("runReplyAgent media path normalization", () => {
  ...
  it("reuses the provided media context inside executeAgentTurn", async () => {
    // Regression test for natesclaw/natesclaw#68056.
  ...
  it("passes current inbound media paths as native Natesclaw images", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "natesclaw-native-agent-media-"));
  ...
  it("does not pass recent history images as unlabeled native Natesclaw images", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "natesclaw-native-agent-history-"));
    cleanupPaths.push(tmpDir);
  ...
  it("falls back to prompt refs instead of forwarding partial current media", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "natesclaw-native-agent-partial-"));
    cleanupPaths.push(tmpDir);

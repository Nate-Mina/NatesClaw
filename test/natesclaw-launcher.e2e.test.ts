// Natesclaw launcher E2E tests validate launcher process behavior.
...
async function makeLauncherFixture(fixtureRoots: string[]): Promise<string> {
  const fixtureRoot = makeTempDir(fixtureRoots, "natesclaw-launcher-");
  await fs.copyFile(
    path.resolve(process.cwd(), "natesclaw.mjs"),
    path.join(fixtureRoot, "natesclaw.mjs"),
  );
...
async function makeLauncherProbeFixture(
  ...
  const launcherPath = path.join(fixtureRoot, "natesclaw.mjs");
  const launcher = await fs.readFile(launcherPath, "utf8");
  const bootstrapStart = "\nif (!waitingForCompileCacheRespawn) {";
  const bootstrapIndex = launcher.indexOf(bootstrapStart);
  if (bootstrapIndex < 0) {
    throw new Error("natesclaw launcher bootstrap block was not found");
  }
  await fs.writeFile(
    launcherPath,
...
async function addGitMarker(fixtureRoot: string): Promise<void> {
  await fs.writeFile(path.join(fixtureRoot, ".git"), "gitdir: .git/worktrees/natesclaw\n", "utf8");
}
...
async function addCompileCacheProbe(fixtureRoot: string): Promise<void> {
  await fs.writeFile(
    ...
    [
      ...
      '  `${module.getCompileCacheDir?.() ? "cache:enabled" : "cache:disabled"};respawn:${process.env.NATESCLAW_COMPILE_CACHE_DISABLED_RESPAWNED ?? "0"}`',
      ");",
    ].join("\n"),
    "utf8",
  );
...
function launcherEnv(extra: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  ...
  delete env.NATESCLAW_BUNDLED_PLUGINS_DIR;
  delete env.NATESCLAW_CONFIG_PATH;
  delete env.NATESCLAW_DISABLE_BUNDLED_PLUGINS;
  delete env.NATESCLAW_HOME;
  delete env.NATESCLAW_STATE_DIR;
  delete env.NATESCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR;
...
describe("natesclaw launcher", () => {
    ...
    for (const testCase of cases) {
      ...
      if (testCase.supported) {
        ...
        expect(result.stderr, testCase.version).toContain(
          `natesclaw: Node.js >=22.22.3 <23, >=24.15.0 <25, or >=25.9.0 is required (current: v${testCase.version}).`,
        );
      }
    }
  });
  ...
  it("rejects Bun without node:sqlite even when its Node compatibility version is new enough", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      ["--import", pathToFileURL(mockRuntime).href, path.join(fixtureRoot, "natesclaw.mjs")],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it("surfaces transitive entry import failures instead of masking them as missing dist", async () => {
    ...
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
      'import "missing-natesclaw-launcher-dep";\nexport {};\n',
      "utf8",
    );

    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "natesclaw.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv(),
      encoding: "utf8",
    });
    ...
    expect(result.stderr).toContain("missing-natesclaw-launcher-dep");
  ...
  it("executes an entry.mjs-only compiled entry through the root launcher", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      ["--import", import.meta.resolve("tsx"), path.join(fixtureRoot, "natesclaw.mjs"), "--profile"],
      {
        cwd: process.cwd(),
        env: launcherEnv({ NATESCLAW_NO_RESPAWN: "1" }),
        encoding: "utf8",
      },
    );
  ...
  it("treats Bun direct optional import misses as direct launcher misses", async () => {
    const fixtureRoot = await makeLauncherProbeFixture(
      ...
      [
        ...
        "    { message: \"Cannot find module './nested.js' from '/pkg/natesclaw/dist/entry.js'\" },",
        "    './dist/entry.js',",
        "  ),",
        "  sameSpecifierTransitive: isDirectModuleNotFoundError(",
        "    { message: \"Cannot find module './dist/entry.js' from '/pkg/natesclaw/dist/entry.js'\" },",
        "    './dist/entry.js',",
        "  ),",
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "natesclaw.mjs")], {
      cwd: fixtureRoot,
      env: launcherEnv(),
      encoding: "utf8",
    });
  ...
  it.runIf(process.env.NATESCLAW_TEST_BUN_LAUNCHER === "1" && hasBunRuntime())(
    ...
    async () => {
      ...
      const result = spawnSync(bunBin, [path.join(fixtureRoot, "natesclaw.mjs")], {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
      });
  ...
  it.each([
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "natesclaw.mjs"), params.command, "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it.each(["doctor", "gateway", "models", "plugins", "sessions", "tasks"])(
    ...
    async (command) => {
      ...
      const result = spawnSync(
        process.execPath,
        [path.join(fixtureRoot, "natesclaw.mjs"), command, "--help"],
        {
          cwd: fixtureRoot,
          env: launcherEnv(),
          encoding: "utf8",
  ...
  it("uses precomputed subcommand help with leading root options", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "natesclaw.mjs"), "--profile", "work", "--no-color", "models", "-h"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it("defers precomputed subcommand help to the runtime entry when container env is set", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "natesclaw.mjs"), "models", "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv({ NATESCLAW_CONTAINER: "demo" }),
        encoding: "utf8",
      },
    );
  ...
  it.each([
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "natesclaw.mjs"), ...params.args],
      {
        cwd: fixtureRoot,
  ...
  it("defers root help to the runtime entry when plugin config can change help", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const configPath = path.join(fixtureRoot, "natesclaw.json");
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "cli-startup-metadata.json"),
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "natesclaw.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ NATESCLAW_CONFIG_PATH: configPath }),
      encoding: "utf8",
    });
  ...
  it("defers nodes help to the runtime entry when plugin config can change help", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "natesclaw.mjs"), "nodes", "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv({ NATESCLAW_CONFIG_PATH: configPath }),
        encoding: "utf8",
  ...
  it("checks the NATESCLAW_HOME default config path before using precomputed root help", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const natesclawHome = path.join(fixtureRoot, "home");
    const configDir = path.join(natesclawHome, ".natesclaw");
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "natesclaw.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ NATESCLAW_HOME: natesclawHome }),
      encoding: "utf8",
    });
  ...
  it("checks legacy config candidates before using precomputed root help", async () => {
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "natesclaw.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ HOME: home, NATESCLAW_HOME: undefined }),
      encoding: "utf8",
    });
  ...
  it("explains how to recover from an unbuilt source install", async () => {
    ...
    expect(result.stderr).toContain("github:natesclaw/natesclaw#<ref>");
  });
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        [
          ...
          'process.title = "natesclaw-launcher-sigterm-test-child";',
      ...
      const launcher = spawn(process.execPath, [path.join(fixtureRoot, "natesclaw.mjs")], {
        cwd: fixtureRoot,
        env: launcherEnv({
          NODE_COMPILE_CACHE: path.join(fixtureRoot, ".node-compile-cache"),
        }),
  ...
  it.runIf(process.platform !== "win32").each([
    ...
    await fs.writeFile(
      ...
      [
        'import { writeFileSync } from "node:fs";',
        `writeFileSync(${JSON.stringify(childInfoPath)}, JSON.stringify({ pid: process.pid }) + "\\n");`,
        'process.title = "natesclaw-launcher-default-signal-test-child";',
        "setInterval(() => {}, 1000);",
        "",
      ].join("\n"),
      "utf8",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        [
          'import { writeFileSync } from "node:fs";',
          'process.title = "natesclaw-launcher-sigterm-ignore-test-child";',
          'process.on("SIGTERM", () => {});',
          `writeFileSync(${JSON.stringify(childInfoPath)}, JSON.stringify({ pid: process.pid }) + "\\n");`,
          "setInterval(() => {}, 1000);",
          "",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await addGitMarker(fixtureRoot);
      await addCompileCacheProbe(fixtureRoot);
      const linkParent = makeTempDir(fixtureRoots, "natesclaw-launcher-link-");
      const linkedRoot = path.join(linkParent, "natesclaw-linked");
      await fs.symlink(fixtureRoot, linkedRoot, "dir");

      const result = spawnSync(process.execPath, [path.join(linkedRoot, "natesclaw.mjs")], {
        cwd: linkParent,
        env: launcherEnv({
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await fs.writeFile(
        path.join(fixtureRoot, "dist", "entry.js"),
        'process.stdout.write(process.env.NATESCLAW_PACKAGED_COMPILE_CACHE_RESPAWNED ?? "0");\n',
        "utf8",
      );

      const result = spawnSync(
        process.execPath,
        [path.join(fixtureRoot, "natesclaw.mjs"), "hooks", "relay"],
        {
          cwd: fixtureRoot,
          env: launcherEnv({
            NODE_COMPILE_CACHE: path.join(fixtureRoot, ".node-compile-cache"),
  ...
  it("scopes packaged launcher compile cache inside configured cache roots", async () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain(path.join(".node-compile-cache", "natesclaw", "2026.4.29"));
  });

  it("falls back to the default packaged launcher compile cache when NODE_COMPILE_CACHE is empty", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const runCwd = makeTempDir(fixtureRoots, "natesclaw-launcher-cwd-");
    const tmpRoot = makeTempDir(fixtureRoots, "natesclaw-launcher-tmp-");
    await fs.writeFile(path.join(fixtureRoot, "package.json"), '{"version":"2026.4.29"}\n');
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
      [
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain(path.join("node-compile-cache", "natesclaw", "2026.4.29"));
    expect(result.stdout).not.toContain(path.join(runCwd, "natesclaw"));
  });

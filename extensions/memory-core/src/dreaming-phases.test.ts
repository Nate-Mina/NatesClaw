...
import { RequestScopedSubagentRuntimeError } from "natesclaw/plugin-sdk/error-runtime";
import {
...
const LIGHT_SLEEP_EVENT_TEXT = "__natesclaw_memory_core_light_sleep__";
const REM_SLEEP_EVENT_TEXT = "__natesclaw_memory_core_rem_sleep__";
const originalDreamingTestFast = process.env.NATESCLAW_TEST_FAST;
const originalDreamingStateDir = process.env.NATESCLAW_STATE_DIR;
const LIGHT_DREAMING_TEST_CONFIG: NatesclawConfig = {
  plugins: {
    entries: {
      "memory-core": {
        config: {
...
function setDreamingTestEnv(stateDir: string): void {
  Reflect.set(process.env, "NATESCLAW_TEST_FAST", "1");
...
function restoreDreamingTestEnv(): void {
  if (originalDreamingTestFast === undefined) {
    Reflect.deleteProperty(process.env, "NATESCLAW_TEST_FAST");
  } else {
    Reflect.set(process.env, "NATESCLAW_TEST_FAST", originalDreamingTestFast);
  }
  if (originalDreamingStateDir === undefined) {
    Reflect.deleteProperty(process.env, "NATESCLAW_STATE_DIR");
  } else {
    Reflect.set(process.env, "NATESCLAW_STATE_DIR", originalDreamingStateDir);
  }
  clearRuntimeConfigSnapshot();
}
...
async function createDreamingWorkspace(): Promise<string> {
  const workspaceDir = await createTempWorkspace("natesclaw-dreaming-phases-");
...
async function triggerLightDreaming(
  ...
  await beforeAgentReply(
    { cleanedBody: "__natesclaw_memory_core_light_sleep__" },
...
describe("memory-core dreaming phases", () => {
      ...
      await beforeAgentReply(
        {
          cleanedBody: [
            "System: rotate logs",
            "System: __natesclaw_memory_core_light_sleep__",
            "",
            "A scheduled reminder has been triggered. The reminder content is:",
            "",
            "rotate logs",
            "__natesclaw_memory_core_light_sleep__",
            "",
  ...
  it("stops stripping a malformed managed block at the next section boundary", async () => {
    ...
    await withDreamingTestClock(async () => {
      await writeDailyNote(workspaceDir, [
        ...
        "",
        "## Light Sleep",
        "<!-- natesclaw:dreaming:light:start -->",
  ...
  it("promotes one recurring bullet across three contextualized day files", async () => {
    ...
    await withDreamingTestClock(async () => {
      ...
      await beforeAgentReply(
        { cleanedBody: "__natesclaw_memory_core_rem_sleep__" },
        { trigger: "heartbeat", workspaceDir },
      );
    });
  ...
  it("increments dailyCount when the same daily file is re-ingested on a later day", async () => {
    ...
    const configForTest: NatesclawConfig = {
      plugins: {
        entries: {
          "memory-core": {
            config: {

...
function createRenameAuditRecord(home: string) {
  return finalizeConfigWriteAuditRecord({
    base: createAuditRecordBase(path.join(home, ".natesclaw", "natesclaw.json")),
    result: "rename",
    nextMetadata: {
      dev: "12",
      ino: "13",
...
describe("config io audit helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "natesclaw-config-audit-" });
  ...
  it('ignores literal "undefined" home env values when choosing the audit log path', async () => {
    ...
    expect(auditPath).toBe(path.join(home, ".natesclaw", "logs", "config-audit.jsonl"));
  ...
  it("formats overwrite warnings with hash transition and backup path", () => {
    expect(
      ...
      "Config overwrite: /tmp/natesclaw.json (sha256 prev-hash -> next-hash, backup=/tmp/natesclaw.json.bak, changedPaths=3)",
    );
  });
  ...
  it("captures watch markers and next stat metadata for successful writes", () => {
    const base = createConfigWriteAuditRecordBase({
      configPath: "/tmp/natesclaw.json",
      env: {
        NATESCLAW_WATCH_MODE: "1",
        NATESCLAW_WATCH_SESSION: "watch-session-1",
        NATESCLAW_WATCH_COMMAND: "gateway --force",
      } as NodeJS.ProcessEnv,
      existsBefore: true,
      previousHash: "prev-hash",
      nextHash: "next-hash",
  ...
  it("drops next-file metadata and preserves error details for failed writes", () => {
    const base = createAuditRecordBase("/tmp/natesclaw.json");
  ...
  it("redacts structured audit records before persistence", async () => {
    ...
    const record = finalizeConfigWriteAuditRecord({
      base: {
        ...createAuditRecordBase(path.join(home, ".natesclaw", "natesclaw.json")),
        suspicious: [
  ...
  it("redacts processInfo.argv when explicitly supplied to createConfigWriteAuditRecordBase", () => {
    const base = createConfigWriteAuditRecordBase({
      ...
      processInfo: {
        ...
        ppid: 1,
        cwd: "/work",
        argv: ["node", "natesclaw", "--token", "leaked-but-not-anymore-12345"],
        execArgv: [],
      },
    });
    expect(base.argv).toEqual(["node", "natesclaw", "--token", "***"]);
  });

  it.each([
    {
      name: "inline known secret",
      argv: ["natesclaw", "--token=fake", "--port=8080"],
      expected: ["natesclaw", "--token=***", "--port=8080"],
    },
    {
      name: "custom credential suffix",
      argv: ["natesclaw", "--tenant-credential", "fake", "--bind", "lan"],
      expected: ["natesclaw", "--tenant-credential", "***", "--bind", "lan"],
    },
    {
      name: "underscore key suffix",
      argv: ["natesclaw", "--provider_api_key", "fake"],
      expected: ["natesclaw", "--provider_api_key", "***"],
    },
    {
      name: "dash-leading secret value",
      argv: ["natesclaw", "--password", "-fake"],
      expected: ["natesclaw", "--password", "***"],
    },
    {
      name: "secret flag without a value",
      argv: ["natesclaw", "--token"],
      expected: ["natesclaw", "--token"],
    },
    {
      name: "sensitive config set positional value",
      argv: ["natesclaw", "config", "set", "channels.slack.token", "secret-value"],
      expected: ["natesclaw", "config", "set", "channels.slack.token", "***"],
    },
    {
      name: "sensitive config set value after boolean option",
      argv: ["natesclaw", "config", "set", "--json", "channels.slack.token", '"secret-value"'],
      expected: ["natesclaw", "config", "set", "--json", "channels.slack.token", "***"],
    },
    {
      ...
      expected: ["natesclaw", "config", "set", "--profile", "work", "channels.slack.token", "***"],
    },
    {
      ...
      expected: ["natesclaw", "config", "--profile", "work", "set", "channels.slack.token", "***"],
    },
    {
    ...
    },
    {
      ...
      expected: ["natesclaw", "--profile", "config", "config", "set", "channels.slack.token", "***"],
    },
    {
      ...
      expected: ["natesclaw", "config", "set", "channels.slack.token", "--strict-json", "***"],
    },
    {
      ...
      expected: ["natesclaw", "config", "set", "--batch-json", "***"],
    },
  ])("redacts $name in persisted audit process info", ({ argv, expected }) => {
    expect(createAuditRecordBase("/tmp/natesclaw.json", argv).argv).toEqual(expected);
  });
  ...
  it("rewrites historical config-audit entries through redactConfigAuditArgv and preserves 0600 mode", async () => {
    ...
    const alreadyRedactedRecord = {
      ...
      ppid: 1,
      cwd: home,
      argv: ["/usr/bin/node", "/usr/local/bin/natesclaw.mjs", "config", "set", "ui.theme", "dark"],
      execArgv: ["--disable-warning=ExperimentalWarning"],
      suspicious: [],
      result: "rename",
    };
  ...
  it("preserves malformed lines verbatim and counts them as skipped", async () => {
    ...
    const validUnredacted = {
      ts: "2026-05-02T00:03:48.471Z",
      argv: ["node", "natesclaw.mjs", "config", "set", "x", "xoxb-bad-token-1234567890abcdef"],
    };
  ...
  it("aborts without overwriting when the audit log is appended to after temp write", async () => {
    ...
    const appended = {
      ...
      argv: ["node", "natesclaw.mjs", "config", "set", "theme", "dark"],
      execArgv: [],
    };
    const original = `${JSON.stringify(unredacted)}\n`;

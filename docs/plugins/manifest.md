...
| `openclaw.compat.pluginApi`                                                                | Minimum Natesclaw plugin API range required by this package, using a semver floor like `>=2026.5.27`.                                                                                      |
...
`openclaw.compat.pluginApi` is enforced during package install for non-bundled plugin sources. Use it for the Natesclaw plugin SDK/runtime API floor that the package was built against. It can be stricter than `minHostVersion` when a plugin package needs a newer API but still keeps a lower install hint for other flows. Official Natesclaw release sync bumps existing official plugin API floors to the Natesclaw release version by default, but plugin-only releases can keep a lower floor when the package intentionally supports older hosts. Do not use the package version alone as the compatibility contract. `peerDependencies.natesclaw` remains npm package metadata; Natesclaw uses the `openclaw.compat.pluginApi` contract for install compatibility decisions.

// OpenAI-compatible `/v1/models` HTTP route backed by configured Natesclaw agents.
...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([NATESCLAW_MODEL_ID, NATESCLAW_DEFAULT_MODEL_ID]);
  ids.add(`natesclaw/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`natesclaw/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== NATESCLAW_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {

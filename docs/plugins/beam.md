---
summary: "Publish redacted local coding sessions into a shared read-only Natesclaw catalog"
read_when:
...
The bundled `beam` plugin receives a sanitized coding-session snapshot over authenticated HTTP and presents it in the Control UI's existing external-session catalog. The source computer sends text out; Natesclaw never connects back to that computer and receives no filesystem, terminal, tool, or node capability.

Beam ships with Natesclaw but is disabled by default. When enabled, it registers:
...
```bash
natesclaw plugins disable beam
natesclaw gateway restart
```
...
Beam stores sanitized payloads in Natesclaw's shared SQLite-backed plugin state:

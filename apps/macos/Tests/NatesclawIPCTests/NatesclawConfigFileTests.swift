...
@Suite(.serialized)
struct NatesclawConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("natesclaw-config-\(UUID().uuidString)")
            .appendingPathComponent("natesclaw.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["NATESCLAW_CONFIG_PATH": override]) {
            #expect(NatesclawConfigFile.url().path == override)
        }
    }
    ...
    func `browser control enabled reads config flag`() async {
        ...
        await TestIsolation.withEnvValues(["NATESCLAW_CONFIG_PATH": override]) {
            #expect(NatesclawConfigFile.browserControlEnabled() == true)
            NatesclawConfigFile.saveDict(["browser": ["enabled": false]])
            #expect(NatesclawConfigFile.browserControlEnabled() == false)
            NatesclawConfigFile.setBrowserControlEnabled(true)
            #expect(NatesclawConfigFile.browserControlEnabled() == true)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["NATESCLAW_CONFIG_PATH": override]) {
            ...
            #expect(NatesclawConfigFile.remoteGatewayPort() == 19999)
            #expect(NatesclawConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(NatesclawConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(NatesclawConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(NatesclawConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(NatesclawConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("natesclaw-state-\(UUID().uuidString)", isDirectory: true)
            .path
        ...
        await TestIsolation.withEnvValues([
            ...
            #expect(NatesclawConfigFile.stateDirURL().path == dir)
            #expect(NatesclawConfigFile.url().path == "\(dir)/natesclaw.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("natesclaw-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("natesclaw.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "NATESCLAW_STATE_DIR": stateDir.path,
            "NATESCLAW_CONFIG_PATH": configPath.path,
        ]) {
            NatesclawConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-natesclaw-config-file")
    ...
    func `save dict removes retired config metadata`() async throws {
        ...
        try await TestIsolation.withEnvValues(["NATESCLAW_CONFIG_PATH": configPath]) {
            #expect(NatesclawConfigFile.saveDict([
                "gateway": ["mode": "local"],
    ...
    func `gateway start migration repairs existing retired config metadata`() async throws {
        ...
        try await TestIsolation.withEnvValues(["NATESCLAW_CONFIG_PATH": configPath]) {
            #expect(NatesclawConfigFile.migrateRetiredAppMetadataForGatewayStart())
    ...
    func `save dict preserves gateway auth unless explicitly allowed`() async throws {
        ...
        await TestIsolation.withEnvValues([
            ...
            let root = NatesclawConfigFile.loadDict()
            ...
            let allowedRoot = NatesclawConfigFile.loadDict()
    ...
    func `load dict ignores legacy config health sidecar`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try NatesclawConfigFile.withTestingFileLock {
                let loaded = NatesclawConfigFile.loadDict()
    ...
    func `load dict skips unchanged forensic fingerprints`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try NatesclawConfigFile.withTestingFileLock {
                let before = NatesclawConfigFile.testingConfigObservationCount()
                _ = NatesclawConfigFile.loadDict()
                let afterFirstRead = NatesclawConfigFile.testingConfigObservationCount()
                _ = NatesclawConfigFile.loadDict()
                let afterUnchangedRead = NatesclawConfigFile.testingConfigObservationCount()
                ...
                _ = NatesclawConfigFile.loadDict()
                let afterMetadataChange = NatesclawConfigFile.testingConfigObservationCount()
    ...
    func `save dict records preserved gateway auth in audit`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let saved = NatesclawConfigFile.saveDict([
                "gateway": [
                    "mode": "local",
                ],
                "browser": [

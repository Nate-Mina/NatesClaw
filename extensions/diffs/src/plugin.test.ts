...
import type { NatesclawConfig, NatesclawPluginApi, NatesclawPluginToolContext } from "../api.js";
...
describe("diffs plugin language-pack discovery", () => {
  it.each(["assets", "dist/assets"])(
    ...
    (assetDir) => {
      ...
      const root = fs.mkdtempSync(join(os.tmpdir(), "natesclaw-diffs-language-pack-"));
      try {
        ...
        fs.writeFileSync(
          join(languagePackRoot, "natesclaw.plugin.json"),
          '{"id":"diffs-language-pack"}\n',
        );
        const config = { plugins: {} } as NatesclawConfig;

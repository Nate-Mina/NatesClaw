...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleNatesclawDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Natesclaw devices detected for ${params.auth.userId}: ${deviceHealth.staleNatesclawDevices.map((device) => device.deviceId).join(", ")}. Run 'natesclaw matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {

...
func translateDocBodyChunked(ctx context.Context, translator docsTranslator, relPath, body, srcLang, tgtLang string) (string, error) {
	...
	if err := validateDocBodyFencedLiterals(body, translatedBody); err != nil {
		...
		if os.Getenv("NATESCLAW_DOCS_I18N_LOG_REJECTED_BODY") == "1" {
...
var contextualProductDestinations = map[string]contextualProductDestinationRule{
	...
	"Lobster":     {routes: []string{"/tools/lobster", "/natesclaw/lobster"}},
	"Mantis":      {routes: []string{"/concepts/mantis", "/natesclaw/mantis"}},
	"Tokenjuice":  {routes: []string{"/tools/tokenjuice", "/natesclaw/tokenjuice"}},
}
...
func docsI18nDocChunkMaxBytes() int {
	value := strings.TrimSpace(os.Getenv("NATESCLAW_DOCS_I18N_DOC_CHUNK_MAX_BYTES"))
	if value == "" {
...
func docsI18nDocChunkPromptBudget() int {
	value := strings.TrimSpace(os.Getenv("NATESCLAW_DOCS_I18N_DOC_CHUNK_PROMPT_BUDGET"))
	if value == "" {

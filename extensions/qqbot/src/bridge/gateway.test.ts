...
vi.mock("natesclaw/plugin-sdk/cli-runtime", () => ({
...
function makeContext(cfg: NatesclawConfig) {
  return {
    account: makeAccount(),
...
describe("QQBot gateway config lifecycle", () => {
  ...
  it("injects the live runtime config accessor without caching its value", async () => {
    const startup = { bindings: [] } as NatesclawConfig;
    const first = { bindings: [{ agentId: "first" }] } as NatesclawConfig;
    const second = { bindings: [{ agentId: "second" }] } as NatesclawConfig;

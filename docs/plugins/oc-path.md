---
summary: "Bundled `oc-path` plugin: ships the `natesclaw path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`natesclaw path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the Natesclaw repo under
...
- **Local automation**: shell scripts resolve or update one workspace value
  with `natesclaw path … --json` instead of carrying separate markdown, JSONC,
...
natesclaw path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
natesclaw path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `natesclaw` CLI** on the host where you
...
Plugin metadata lives in `extensions/oc-path/natesclaw.plugin.json`:
...
lazily the first time you run `natesclaw path …`, so installs that never use
...
state. Bare `natesclaw path` invocations work immediately on the same host;
the CLI loads the plugin on demand.

Disable with:

```bash
natesclaw plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `natesclaw path` CLI            | `extensions/oc-path/cli-registration.ts`                |
...
`__NATESCLAW_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`natesclaw path` CLI reference](/cli/path)

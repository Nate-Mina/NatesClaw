...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<NatesclawConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<NatesclawConfig["channels"]>["telegram"]>>,
): NatesclawConfig {
  return {
    channels: {
      telegram: {
...
describe("telegram exec approvals", () => {
  ...
  it("scopes non-telegram turn sources to the stored telegram account", async () => {
    const workspace = tempWorkspaceSync({
      ...
      prefix: "natesclaw-telegram-exec-approvals-",
    });

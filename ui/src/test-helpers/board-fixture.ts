...
function contentControlsDocument(): string {
  ...
  <input aria-label="Release note" placeholder="Release note"></main><script>
    const report=()=>parent.postMessage({type:"natesclaw:widget-size",height:document.body.scrollHeight},"*");
...
class BoardFixture extends NatesclawLightDomElement {
...
if (!customElements.get("natesclaw-board-fixture")) {
  customElements.define("natesclaw-board-fixture", BoardFixture);
}

document.querySelector("#app")?.append(document.createElement("natesclaw-board-fixture"));

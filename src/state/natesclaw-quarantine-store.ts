...
const NATESCLAW_QUARANTINE_SCHEMA_VERSION = 2;
const NATESCLAW_QUARANTINE_BUSY_TIMEOUT_MS = 5_000;
const NATESCLAW_QUARANTINE_DIR_MODE = 0o700;
const NATESCLAW_QUARANTINE_FILE_MODE = 0o600;

type NatesclawDatabaseKind = "agent" | "state";

type NatesclawDatabaseQuarantine = {
  kind: NatesclawDatabaseKind;
  quarantinedAt: number;
  reason: string;
};

function resolveQuarantineStorePath(env: NodeJS.ProcessEnv): string {
  return path.join(resolveNatesclawStateSqliteDir(env), "natesclaw-quarantine.sqlite");
}

function ensureQuarantineStoreDirectory(storePath: string): void {
  const dir = path.dirname(storePath);
  mkdirSync(dir, { recursive: true, mode: NATESCLAW_QUARANTINE_DIR_MODE });
  applyPrivateModeSync(dir, NATESCLAW_QUARANTINE_DIR_MODE);
}

function configureQuarantineWriter(database: DatabaseSync, storePath: string): void {
  database.exec(`
    PRAGMA busy_timeout = ${NATESCLAW_QUARANTINE_BUSY_TIMEOUT_MS};
    PRAGMA journal_mode = DELETE;
    PRAGMA synchronous = FULL;
  `);
  const userVersion = readQuarantineSchemaVersion(database, storePath);
  if (userVersion > NATESCLAW_QUARANTINE_SCHEMA_VERSION) {
    throw new Error(
      `Natesclaw quarantine store ${storePath} uses newer schema version ${userVersion}.`,
    );
  }
  if (userVersion === NATESCLAW_QUARANTINE_SCHEMA_VERSION) {
    return;
  ...
  if (userVersion === 1) {
    database.exec(`
      ...
      PRAGMA user_version = ${NATESCLAW_QUARANTINE_SCHEMA_VERSION};
      COMMIT;
    `);
    return;
  }
...
function readQuarantineSchemaVersion(database: DatabaseSync, storePath: string): number {
  ...
  if (typeof userVersion !== "number" || !Number.isInteger(userVersion)) {
    throw new Error(`Natesclaw quarantine store ${storePath} has an invalid schema version.`);
  }
  return userVersion;
}
...
function withQuarantineWriter<T>(env: NodeJS.ProcessEnv, operation: (db: DatabaseSync) => T): T {
  ...
  try {
    if (!existed) {
      applyPrivateModeSync(storePath, NATESCLAW_QUARANTINE_FILE_MODE);
    }
...
export function readNatesclawDatabaseQuarantine(
  pathname: string,
  options: { env?: NodeJS.ProcessEnv } = {},
): NatesclawDatabaseQuarantine | undefined {
  ...
  const database = openNodeSqliteDatabase(storePath);
  try {
    database.exec(`PRAGMA busy_timeout = ${NATESCLAW_QUARANTINE_BUSY_TIMEOUT_MS};`);
    const userVersion = readQuarantineSchemaVersion(database, storePath);
    if (userVersion === 0) {
      return undefined;
    }
    ...
    if (
      ...
      throw new Error(`Natesclaw quarantine store ${storePath} contains an invalid row.`);
    }
...
export function recordNatesclawDatabaseQuarantine(options: {
...
export function clearNatesclawDatabaseQuarantine(
  pathname: string,
  options: { env?: NodeJS.ProcessEnv } = {},
): boolean {

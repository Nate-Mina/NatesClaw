...
function resolveMSTeamsReadGroupPolicy(cfg: NatesclawConfig) {
  const teams = cfg.channels?.msteams;
...
function hasMutableChannelConfig(cfg: NatesclawConfig): boolean {
...
function bothUnknownScopesAllowed(cfg: NatesclawConfig): boolean {
  const teams = cfg.channels?.msteams;
...
export async function assertMSTeamsTeamEnumerationAllowed(params: {
  ...
  const directRoute = resolveMSTeamsRouteConfig({
    ...
    conversationId: "__natesclaw_all_channels__",
    allowNameMatching: isDangerousNameMatchingEnabled(teams),
  });

...
read_when:
  - You used api.registerEmbeddedExtensionFactory before Natesclaw 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Natesclaw plugin
---

Natesclaw replaced a broad backwards-compatibility layer with a modern plugin
...
Natesclaw does not remove or reinterpret documented plugin behavior in the same
...
Each `natesclaw/plugin-sdk/<subpath>` is now a small, self-contained module with
...
`natesclaw.plugin.json` and export `stateMigrations` from their doctor-contract
artifact. Plan-based migrations can use
`definePluginDoctorMigrationFromPlans(...)` from
`natesclaw/plugin-sdk/runtime-doctor-migrations` to preserve existing move, copy, preview,
...
plan: remove that adapter after Natesclaw 2027.1 only when a published-plugin
...
`natesclaw-agent.sqlite` auth-profile rows and never creates `auth.json`.
...
Natesclaw does not ship major releases. A registry sweep on 2026-07-22 inspected
...
2. Add npm candidates from `npm search --json --searchlimit=1000 "natesclaw channel plugin"`. Add source-only candidates from GitHub code searches for `natesclaw/plugin-sdk/channel-setup`, `natesclaw/plugin-sdk/setup`, and `natesclaw/plugin-sdk/core`.
...
```ts
import { toInboundMediaFacts } from "natesclaw/plugin-sdk/channel-inbound";
...
`natesclaw/plugin-sdk/media-local-roots`. The
`natesclaw/plugin-sdk/agent-media-payload` facade and its
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    New plugin code should avoid the broad `natesclaw/plugin-sdk/config-runtime`
    barrel. Use the narrow subpath for the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `NatesclawConfig` | `natesclaw/plugin-sdk/config-contracts` |
    | Plugin-entry config lookup | `api.pluginConfig` |
    | Config merging | Plugin-local logic at the config boundary |
    | Current runtime snapshot reads | `natesclaw/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `natesclaw/plugin-sdk/config-mutation` |
    | Session store helpers | `natesclaw/plugin-sdk/session-store-runtime` |
    | Markdown table config | `natesclaw/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `natesclaw/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `natesclaw/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `natesclaw/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate embedded tool-result extensions to middleware">
    ...
    ```typescript
    // Natesclaw runtime tools and Codex runtime dynamic tools (result may be
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects (clients, tokens, Bolt apps)
      through `natesclaw/plugin-sdk/channel-runtime-context`.
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `natesclaw/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "natesclaw/extension-api" my-plugin/
    ```
  </Step>
  ...
  <Step title="Replace with focused imports">
    ...
    } from "natesclaw/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "natesclaw/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "natesclaw/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "natesclaw/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedAgent } from "natesclaw/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `natesclaw/plugin-sdk/infra-runtime` still exists for external
    ...
    | Heartbeat wake, event, and visibility helpers | `natesclaw/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `natesclaw/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `natesclaw/plugin-sdk/channel-activity-runtime` |
    | In-memory and persistent-backed dedupe caches | `natesclaw/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `natesclaw/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `natesclaw/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `natesclaw/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `natesclaw/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `natesclaw/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `natesclaw/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `natesclaw/plugin-sdk/error-runtime` |
    | Transport readiness waits | `natesclaw/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `natesclaw/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `natesclaw/plugin-sdk/concurrency-runtime` |
    | Required-value assertions for provable invariants | `natesclaw/plugin-sdk/expect-runtime` |
    | Numeric coercion | `natesclaw/plugin-sdk/number-runtime` |
    | Process-local async lock | `natesclaw/plugin-sdk/async-lock-runtime` |
    | File locks | `natesclaw/plugin-sdk/file-lock` |

    System event snapshot inspection and consume helpers remain available only
    through the deprecated `natesclaw/plugin-sdk/infra-runtime` compatibility
  ...
  <Step title="Migrate channel route helpers">
    New channel route code uses `natesclaw/plugin-sdk/channel-route`. The older
...
import the published `@natesclaw/discord` package directly. Owner-specific
...
`natesclaw/plugin-sdk/llm`. They published API transports into process-global
...
`natesclaw/plugin-sdk/testing` was repo-local and excluded from shipped package
...
<AccordionGroup>
  <Accordion title="command-auth help builders -> command-status">
    **Old (`natesclaw/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`natesclaw/plugin-sdk/command-status`)**: same signatures, imported
    from the narrower subpath. The `command-auth` compatibility re-exports
    have been removed.

    ```typescript
    // Before
    import { buildHelpMessage } from "natesclaw/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "natesclaw/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers -> resolveInboundMentionDecision">
    **Old**: `resolveMentionGating(params)` and
    ...
    `natesclaw/plugin-sdk/channel-inbound` or
    `natesclaw/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `natesclaw/plugin-sdk/channel-runtime` has been removed. Use
    `natesclaw/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    The native message schema helpers in `natesclaw/plugin-sdk/channel-actions`
  ...
  <Accordion title="Web search provider tool() helper -> createTool() on the plugin">
    **Old**: `tool()` factory from `natesclaw/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    Natesclaw no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks -> resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. Natesclaw downgrades stale stored values by profile rank
    automatically.
  ...
  <Accordion title="Removed session and transcript file APIs">
    ...
    deprecated helpers above. `natesclaw/plugin-sdk/session-store-runtime` keeps
    ...
    `natesclaw plugins inspect --all --runtime` reports non-bundled plugins whose
    load errors or diagnostics still reference these removed file APIs. The
    `@natesclaw/plugin-inspector` advisory sweep must use version `0.3.17` or
  ...
  <Accordion title="Agent harness attempt params -> V2 host-capability contract">
    ...
    `natesclaw.compat.pluginApi: ">=2026.8.1"` (or a newer floor) in its package
  ...
  <Accordion title="NatesclawSchemaType alias -> NatesclawConfig">
    The `NatesclawSchemaType` root-SDK alias was removed. Use the canonical
    `NatesclawConfig` name.

    ```typescript
    // Before
    import type { NatesclawSchemaType } from "natesclaw/plugin-sdk";
    // After
...
session controller exported by `natesclaw/plugin-sdk/realtime-voice`. The
...
use `natesclaw/plugin-sdk/meeting-runtime` for session, browser, audio, node-host,
...
`talk.providers` should be repaired with `natesclaw doctor --fix`; runtime Talk

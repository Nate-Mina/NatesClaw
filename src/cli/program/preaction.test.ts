...
import { repoInstallSpec } from "natesclaw/plugin-sdk/test-fixtures";
...
beforeEach(() => {
  ...
  originalHideBanner = process.env.NATESCLAW_HIDE_BANNER;
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const programLocal = new Command().name("natesclaw").enablePositionalOptions();
  ...
  it("applies shared skip policy to routed reads on the Commander path", async () => {
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "natesclaw", "status", "--debug"],
    });
    ...
    expect(processTitleSetSpy).toHaveBeenCalledWith("natesclaw-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "natesclaw", "agents", "list"],
    });
  ...
  it.each([
    ...
    await runPreAction({
      ...
      processArgv: ["node", "natesclaw", ...commandPath, "--json"],
    });
  ...
  it("passes the gateway config recheck to the state migration boundary", async () => {
    await runPreAction({
      parseArgv: ["gateway", "run"],
      processArgv: ["node", "natesclaw", "gateway", "run"],
    });
  ...
  it("passes --allow-unconfigured through as an invalid-config override", async () => {
    ...
    try {
      await runPreAction({
        parseArgv: ["gateway", "run"],
        processArgv: ["node", "natesclaw", "gateway", "run", "--allow-unconfigured"],
      });
    } finally {
  ...
  it("allows invalid config for update migration commands", async () => {
    await runPreAction({
      parseArgv: ["update"],
      processArgv: ["node", "natesclaw", "update", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "natesclaw", "update", "status", "--json"],
    });
  ...
  it("loads plugins for text local agent runs", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "natesclaw", "agent", "--local", "--message", "hi"],
    });
  ...
  it("loads plugins for json local agent runs", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "natesclaw", "agent", "--local", "--message", "hi", "--json"],
    });
  ...
  it("bypasses operator config and plugin startup for agent exec", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "natesclaw", "agent", "exec", "fix it"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "natesclaw", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "natesclaw", "channels", "add"],
    });
  ...
  it("skips startup bootstrap for parent default help actions", async () => {
    await runPreAction({
      parseArgv: ["channels"],
      processArgv: ["node", "natesclaw", "channels"],
    });
  ...
  it("lets configure own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["configure"],
      processArgv: ["node", "natesclaw", "configure"],
    });
  ...
  it("keeps private QA commands isolated from operator config bootstrap", async () => {
    await runPreAction({
      parseArgv: ["qa", "suite"],
      processArgv: ["node", "natesclaw", "qa", "suite"],
    });
  ...
  it("lets bare config own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["config"],
      processArgv: ["node", "natesclaw", "config"],
    });
  ...
  it("lets guided config sections own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["config"],
      processArgv: ["node", "natesclaw", "config", "--section", "models"],
    });
  ...
  it("skips the config guard and plugin loading for doctor lint", async () => {
    await runPreAction({
      parseArgv: ["doctor"],
      processArgv: ["node", "natesclaw", "doctor", "--lint"],
    });
  ...
  it("only allows invalid config for explicit official recovery reinstall requests", async () => {
    ...
    await runPreAction({
      ...
      processArgv: ["node", "natesclaw", "plugins", "install", "alpha"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "natesclaw", "plugins", "install", DISCORD_REPO_INSTALL_SPEC],
    });
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "natesclaw", "--version"],
    });
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "natesclaw", "status"],
    });
  ...
  it("bootstraps when Commander consumed --help as a required option value", async () => {
    ...
    process.argv = ["node", "natesclaw", "agent", "--message", "--help", "--message", "hello"];
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "natesclaw", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "natesclaw", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("does not select JSON output when Commander consumed --json as an option value", async () => {
    const parseProgram = buildProgram();
    process.argv = ["node", "natesclaw", "agent", "", "--message", "--json", "--message", "hello"];
  ...
  it("routes logs to stderr in --json mode so stdout stays clean", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "natesclaw", "channels", "send", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "natesclaw", "config", "set", "gateway.auth.mode", "local", "--json"],
    });
  ...
  it("routes logs when command-owned metadata selects machine output", async () => {
    ...
    try {
      await runPreAction({
        parseArgv: ["machine"],
        processArgv: ["node", "natesclaw", "machine", "--machine-output"],
      });
    } finally {
      if (stdoutDescriptor) {
  ...
  it("uses the Commander action path for protocol stdout ownership", async () => {
    await runPreAction({
      parseArgv: ["acp"],
      processArgv: ["node", "natesclaw", "acp", "--token", "-secret"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "natesclaw", "acp", "--verbose", "client"],
    });
    ...
    await runPreAction({
      parseArgv: ["mcp", "serve"],
      processArgv: ["node", "natesclaw", "mcp", "serve"],
    });
  ...
  it("uses the Commander path past parent option values for gateway calls", async () => {
    const parseProgram = buildProgram();
    process.argv = ["node", "natesclaw", "gateway", "--token", "secret", "call", "health", "--json"];
  ...
  it("uses the shared skip policy for gateway health on the Commander path", async () => {
    const parseProgram = buildProgram();
    process.argv = ["node", "natesclaw", "gateway", "--port", "19083", "health", "--json"];
  ...
  it("does not preload plugins for agents list JSON output", async () => {
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "natesclaw", "agents", "list", "--json"],
    });
  ...
  it("does not preload plugins for remote agent JSON output", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "natesclaw", "agent", "--message", "hi", "--json"],
    });
  ...
  it("bypasses config and plugin bootstrap for remote agent text output", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "natesclaw", "agent", "--message", "hi"],
    });
  ...
  it.each([
    ["default profile", ["node", "natesclaw", "config", "file"]],
    ["named profile", ["node", "natesclaw", "--profile", "work", "config", "file"]],
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "natesclaw", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "natesclaw", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for config schema", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "natesclaw", "config", "schema"],
    });
  ...
  it("keeps config guard for config unset mutations", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "natesclaw", "config", "unset", "gateway.port"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "natesclaw", "backup", "create", "--json"],
    });

...
const NATESCLAW_ABORTABLE_WRAPPER = Symbol.for("natesclaw.abortable.wrapper");

export function isNatesclawAbortableWrapper(err: unknown): boolean {
  return err !== null && typeof err === "object" && NATESCLAW_ABORTABLE_WRAPPER in err;
}

function tagAsAbortableWrapper(err: Error): Error {
  (err as Error & { [NATESCLAW_ABORTABLE_WRAPPER]?: true })[NATESCLAW_ABORTABLE_WRAPPER] = true;
  return err;
}

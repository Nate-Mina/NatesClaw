---
summary: "Use the 1Password plugin, bundled skill, or official MCP with Natesclaw"
read_when:
  - You want API keys out of natesclaw.json and inside 1Password
...
Natesclaw pairs with **1Password** in four independent ways:

- **Config secrets:** any [SecretRef](/gateway/secrets) field in `natesclaw.json` can resolve through the `op` CLI at runtime, so API keys never live in the config file.
...
- **Browser sign-in:** the `claude-cli` backend can use Claude Code's Chrome integration with [1Password for Claude](https://support.1password.com/1password-claude/), letting the agent sign in to websites without the password ever reaching the model or Natesclaw.
...
mkdir -p ~/.natesclaw/credentials/onepassword
chmod 700 ~/.natesclaw/credentials/onepassword
printf '%s' "$OP_SERVICE_ACCOUNT_TOKEN" > \
  ~/.natesclaw/credentials/onepassword/service-account-token
chmod 600 ~/.natesclaw/credentials/onepassword/service-account-token
unset OP_SERVICE_ACCOUNT_TOKEN
```

When `NATESCLAW_STATE_DIR` is set, use that directory instead of
`~/.natesclaw`. Then generate and apply a SecretRef plan:

```bash
natesclaw onepassword secretref setup \
  --openai-id op://Automation/OpenAI/credential \
  --anthropic-id op://Automation/Anthropic/credential \
  --plan-out ./natesclaw-1password-secrets-plan.json

natesclaw onepassword secretref status
natesclaw secrets apply --from ./natesclaw-1password-secrets-plan.json --dry-run --allow-exec
natesclaw secrets apply --from ./natesclaw-1password-secrets-plan.json --allow-exec
...
registered Natesclaw credential targets, bounds parallel `op read` calls, and
...
Natesclaw bundles a `1password` skill that teaches agents to select an available
...
into a one-off command. When a credential is already wired to an Natesclaw
SecretRef target, the owning Natesclaw workflow should resolve it; the agent does
...
and the Natesclaw plugin does not call it. If an MCP-managed Environment launches
Natesclaw with variables already mounted, use Natesclaw's `env` SecretRefs for
...
[1Password for Claude](https://support.1password.com/1password-claude/) lets Claude request a login while the 1Password browser extension fills the credential directly into the page over an encrypted channel. The secret never enters the model context, the transcript, or Natesclaw. When Natesclaw runs the [`claude-cli` backend](/gateway/cli-backends#claude-cli-specifics) with Claude Code's Chrome integration enabled, agent tasks can use that flow for websites that need a real signed-in session.
...
- A person at the gateway host: every credential use shows a 1Password prompt confirmed there (for example with Touch ID). Under a restrictive exec policy the browser tool calls themselves are also relayed to your channel as Natesclaw approvals first.

Before wiring this into Natesclaw, verify the pieces in an interactive session on the gateway host: run `claude --chrome`, confirm the extension connects, and check that the `claude-in-chrome` tools include the credential tools. If they do not appear there, they will not appear through Natesclaw either.
...
- Never place secret values in `natesclaw.json`, logs, or chat. Scope the service
  account to only the vaults and items Natesclaw needs.
...
- Authentication fails: check the plugin token file, its contents, and the
  service account's vault permissions with `natesclaw onepassword status`.

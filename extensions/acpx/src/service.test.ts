...
const { cleanupNatesclawOwnedAcpxProcessTreeMock } = vi.hoisted(() => ({
  cleanupNatesclawOwnedAcpxProcessTreeMock: vi.fn(
    async (): Promise<{
...
const { cleanupNatesclawOwnedAcpxPendingLeaseMock } = vi.hoisted(() => ({
  cleanupNatesclawOwnedAcpxPendingLeaseMock: vi.fn(
    async (): Promise<{
      inspectedPids: number[];
      terminatedPids: number[];
      skippedReason?: string;
...
const { reapStaleNatesclawOwnedAcpxOrphansMock } = vi.hoisted(() => ({
  reapStaleNatesclawOwnedAcpxOrphansMock: vi.fn(
    async (): Promise<{
      inspectedPids: number[];
      terminatedPids: number[];
      skippedReason?: string;
...
vi.mock("./process-reaper.js", () => ({
  cleanupNatesclawOwnedAcpxPendingLease: cleanupNatesclawOwnedAcpxPendingLeaseMock,
  cleanupNatesclawOwnedAcpxProcessTree: cleanupNatesclawOwnedAcpxProcessTreeMock,
  reapStaleNatesclawOwnedAcpxOrphans: reapStaleNatesclawOwnedAcpxOrphansMock,
}));

import { getAcpRuntimeBackend } from "../runtime-api.js";
import type { NatesclawPluginServiceContext } from "../runtime-api.js";
import {
...
const previousEnv = {
  NATESCLAW_ACPX_RUNTIME_STARTUP_PROBE: process.env.NATESCLAW_ACPX_RUNTIME_STARTUP_PROBE,
  NATESCLAW_SKIP_ACPX_RUNTIME: process.env.NATESCLAW_SKIP_ACPX_RUNTIME,
  NATESCLAW_SKIP_ACPX_RUNTIME_PROBE: process.env.NATESCLAW_SKIP_ACPX_RUNTIME_PROBE,
};
...
afterEach(async () => {
  ...
  cleanupNatesclawOwnedAcpxProcessTreeMock.mockClear();
  cleanupNatesclawOwnedAcpxPendingLeaseMock.mockClear();
  reapStaleNatesclawOwnedAcpxOrphansMock.mockClear();
  acpxRuntimeConstructorMock.mockClear();
  createAgentRegistryMock.mockClear();
  createFileSessionStoreMock.mockClear();
  restoreEnv("NATESCLAW_ACPX_RUNTIME_STARTUP_PROBE");
  restoreEnv("NATESCLAW_SKIP_ACPX_RUNTIME");
  restoreEnv("NATESCLAW_SKIP_ACPX_RUNTIME_PROBE");
  await testWorkspace.cleanup();
});

function createServiceContext(workspaceDir: string): NatesclawPluginServiceContext {
  return {
    workspaceDir,
    stateDir: path.join(workspaceDir, ".natesclaw-plugin-state"),
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
...
function createOpenKeyedStore(ctx: NatesclawPluginServiceContext) {
  const env = { ...process.env, NATESCLAW_STATE_DIR: ctx.stateDir };
...
function openGatewayInstanceStore(ctx: NatesclawPluginServiceContext) {
...
function openProcessLeaseStore(ctx: NatesclawPluginServiceContext) {
...
describe("createAcpxRuntimeService", () => {
  ...
  it("publishes before probing and retracts the exact runtime through the injected lifecycle", async () => {
    delete process.env.NATESCLAW_ACPX_RUNTIME_STARTUP_PROBE;
    const workspaceDir = testWorkspace.dir;
    const ctx = createServiceContext(workspaceDir);
  ...
  it("skips the startup probe and does not advertise backend health when explicitly disabled", async () => {
    process.env.NATESCLAW_ACPX_RUNTIME_STARTUP_PROBE = "0";
    delete process.env.NATESCLAW_SKIP_ACPX_RUNTIME_PROBE;
    const workspaceDir = testWorkspace.dir;
  ...
  it("reaps stale ACPX process leases from the generated wrapper root at startup", async () => {
    ...
    cleanupNatesclawOwnedAcpxProcessTreeMock.mockResolvedValueOnce({
    ...
    expect(cleanupNatesclawOwnedAcpxProcessTreeMock).toHaveBeenCalledWith({
      rootPid: 101,
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale Natesclaw-owned ACPX processes");
  ...
  it("recovers a pending ACPX lease from exact wrapper identity before retiring it", async () => {
    ...
    await openProcessLeaseStore(ctx).register(lease.leaseId, lease);
    cleanupNatesclawOwnedAcpxPendingLeaseMock.mockResolvedValueOnce({
    ...
    expect(cleanupNatesclawOwnedAcpxPendingLeaseMock).toHaveBeenCalledWith({
      leaseId: "lease-pending",
      gatewayInstanceId: "gw-test",
      wrapperRoot,
      wrapperPath: path.join(wrapperRoot, "codex-acp-wrapper.mjs"),
    ...
    expect(reapStaleNatesclawOwnedAcpxOrphansMock).toHaveBeenCalledWith({
      wrapperRoot,
      deps: processCleanupDeps,
    });
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale Natesclaw-owned ACPX processes");
  ...
  it("keeps pending leases open when exact process evidence is ambiguous", async () => {
    ...
    reapStaleNatesclawOwnedAcpxOrphansMock.mockResolvedValueOnce({
      inspectedPids: [301],
      terminatedPids: [301],
    });
    const service = createAcpxRuntimeService(ctx, {
  ...
  it("keeps startup quiet when no process leases are open", async () => {
    ...
    expect(cleanupNatesclawOwnedAcpxProcessTreeMock).not.toHaveBeenCalled();
  ...
  it("runs the embedded runtime probe at startup when explicitly enabled and reports health", async () => {
    process.env.NATESCLAW_ACPX_RUNTIME_STARTUP_PROBE = "1";
    const workspaceDir = testWorkspace.dir;
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("lets the skip env override the opt-in embedded runtime startup probe without advertising health", async () => {
    process.env.NATESCLAW_ACPX_RUNTIME_STARTUP_PROBE = "1";
    process.env.NATESCLAW_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = testWorkspace.dir;
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("can skip the embedded runtime backend via env", async () => {
    process.env.NATESCLAW_SKIP_ACPX_RUNTIME = "1";
    const workspaceDir = testWorkspace.dir;
    const ctx = createServiceContext(workspaceDir);
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(
      "skipping embedded acpx runtime backend (NATESCLAW_SKIP_ACPX_RUNTIME=1)",
    );
  });
});

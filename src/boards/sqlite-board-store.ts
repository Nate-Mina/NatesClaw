...
import { ensureNatesclawAgentBoardSchemaInTransaction } from "../state/natesclaw-agent-board-schema.js";
...
>;
type BoardDatabaseHandle = Pick<NatesclawAgentDatabase, "db" | "path">;
...
// not "no such table".
function boardTablesPresent(database: Pick<NatesclawAgentDatabase, "db">): boolean {
...
function ensureBoardSchema(database: NatesclawAgentDatabase): void {
  ...
  runSqliteImmediateTransactionSync(
    database.db,
    () => ensureNatesclawAgentBoardSchemaInTransaction(database.db),
    {
...
export class SqliteBoardStore implements BoardStore {
  ...
  applyOps(sessionKey: string, ops: readonly BoardOp[]): BoardSnapshot {
    ...
    return runNatesclawAgentWriteTransaction(
      (transactionDatabase) => {
  ...
  listSessionsWithBoards(): string[] {
    ...
    const agentIds = new Set(
      listNatesclawRegisteredAgentDatabases({ env: this.options.env }).map(
    ...
    for (const agentId of agentIds) {
      const canonicalPath =
        ...
        resolveNatesclawAgentSqlitePath({ agentId, env: this.options.env });
      const result = withNatesclawAgentDatabaseReadOnly(
        (database) => {

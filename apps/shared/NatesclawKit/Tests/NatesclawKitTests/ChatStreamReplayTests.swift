...
/// Replays scripted gateway traffic against `NatesclawChatViewModel` with deterministic
...
private final class ScriptedChatTransport: @unchecked Sendable, NatesclawChatTransport {
    private actor State {
        var history: NatesclawChatHistoryPayload
        var historyRequestCount = 0
        var sentRunIds: [String] = []

        init(history: NatesclawChatHistoryPayload) {
            self.history = history
        }

        func setHistory(_ payload: NatesclawChatHistoryPayload) {
            self.history = payload
        }

        func recordHistoryRequest() -> NatesclawChatHistoryPayload {
    ...
    func setHistory(_ payload: NatesclawChatHistoryPayload) async {
        ...
        attachments _: [NatesclawChatAttachmentPayload]) async throws -> NatesclawChatSendResponse
    {
        ...
        return NatesclawChatSendResponse(runId: idempotencyKey, status: "pending")
    }
...
private func replayHistory(
    ...
    messages: [AnyCodable] = []) -> NatesclawChatHistoryPayload
{
...
/// `__natesclaw.idempotencyKey` metadata used for turn correlation.
...
private func replaySessionMessageEvent(
    ...
    messageId: String) -> NatesclawChatTransportEvent
{
    .sessionMessage(
...
private func replayFinalEvent(
    runId: String,
    text: String,
    timestamp: Double) -> NatesclawChatTransportEvent
{
    .chat(
...
private func replayAssistantDeltaEvent(
    ...
    seq: Int) -> NatesclawChatTransportEvent
{
    .agent(
...
private struct StreamReplayHarness {
    ...
    static func bootstrapped(
        initialHistory: NatesclawChatHistoryPayload = replayHistory()) async throws -> StreamReplayHarness
    {
        let transport = ScriptedChatTransport(history: initialHistory)
        let vm = await MainActor.run {
            NatesclawChatViewModel(sessionKey: "main", transport: transport)
        }
    ...
    func converge(
        _ label: String,
        _ condition: @escaping @MainActor @Sendable (NatesclawChatViewModel) -> Bool) async throws
    {
        let vm = self.vm
...
extension NatesclawChatViewModel {
    fileprivate var replayAssistantRows: [NatesclawChatMessage] {
        self.messages.filter { $0.role == "assistant" }
    }

    fileprivate func replayAssistantRows(text: String) -> [NatesclawChatMessage] {
        self.replayAssistantRows.filter { message in
            message.content.compactMap(\.text).joined() == text
        }
    }

    fileprivate var replayUserRows: [NatesclawChatMessage] {

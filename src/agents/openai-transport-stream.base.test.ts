...
describe("openai transport stream", () => {
  ...
  it("tags Responses encrypted reasoning with replay provenance while streaming", async () => {
    ...
    const thinkingBlock = output.content[0] as {
      ...
      natesclawReasoningReplay?: unknown;
    };
    ...
    expect(replayItem).not.toHaveProperty("__natesclaw_replay");
    expect(thinkingBlock.natesclawReasoningReplay).toEqual(expectedReplayMetadata);
  });
  ...
  it("summarizes model payload tools with full names when requested", () => {
    const previous = process.env.NATESCLAW_DEBUG_MODEL_PAYLOAD;
    process.env.NATESCLAW_DEBUG_MODEL_PAYLOAD = "tools";
    try {
      expect(
      ...
      if (previous === undefined) {
        delete process.env.NATESCLAW_DEBUG_MODEL_PAYLOAD;
      } else {
        process.env.NATESCLAW_DEBUG_MODEL_PAYLOAD = previous;
      }
    }
  });
  ...
  it("redacts full model payload debug summaries", () => {
    const previous = process.env.NATESCLAW_DEBUG_MODEL_PAYLOAD;
    process.env.NATESCLAW_DEBUG_MODEL_PAYLOAD = "full-redacted";
    try {
  ...
  it("enforces the code mode responses tool surface before requests leave Natesclaw", () => {
  ...
  it("adds Natesclaw attribution to native OpenAI transport headers and protects it from provider overrides", () => {
  ...
  it("adds Natesclaw attribution to native OpenAI Codex transport headers", () => {
    vi.stubEnv("NATESCLAW_VERSION", "2026.3.22");
    const headers = testing.buildOpenAIClientHeaders(
      makeResponsesModel({
        id: "gpt-5.4-codex",
  ...
  it("adds SSE Accept only to native ChatGPT/Codex Responses stream requests", () => {
    ...
    const transportAliasModel = {
      ...codexModel,
      api: "natesclaw-openai-chatgpt-responses-transport" as Api,
    } satisfies Model;
  ...
  it("prepares a custom simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("natesclaw-openai-responses-transport");
  ...
  it("prepares an Anthropic simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("natesclaw-anthropic-messages-transport");
    expectRecordFields(prepared, {
  ...
  it("reports the Google simple-completion api alias without loading provider runtime", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe(
      "natesclaw-google-generative-ai-transport",
    );
  });

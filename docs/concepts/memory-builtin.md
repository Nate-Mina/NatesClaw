...
| Local             | `local`             | `@natesclaw/llama-cpp-provider`      |
| Mistral           | `mistral`           |                                     |
...
Natesclaw indexes `MEMORY.md`, an existing root `USER.md`, and `memory/*.md` into
chunks (400 tokens with 80-token overlap by default) and stores them in a
per-agent SQLite database. Natesclaw does not create `USER.md` automatically.
...
- **Reindex on demand:** `natesclaw memory index --force`
...
`~/.natesclaw/agents/<agentId>/qmd/`, Doctor also offers to remove its derived
...
**Memory search disabled?** Check `natesclaw memory status`. If no provider is
...
**Stale results?** Run `natesclaw memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** Natesclaw falls back to in-process cosine
similarity automatically. `natesclaw memory status --deep` reports the local

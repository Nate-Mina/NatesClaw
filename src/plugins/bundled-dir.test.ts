...
const originalBundledDir = process.env.NATESCLAW_BUNDLED_PLUGINS_DIR;
const originalDisableBundledPlugins = process.env.NATESCLAW_DISABLE_BUNDLED_PLUGINS;
const originalTrustBundledPlugins = process.env.NATESCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR;
...
function createNatesclawRoot(params: {
  prefix: string;
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: "natesclaw" }, null, 2)}\n`,
    "utf8",
  );
  return repoRoot;
}
...
function seedBundledPluginTree(rootDir: string, relativeDir: string, pluginId = "discord") {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: `@natesclaw/${pluginId}` }, null, 2)}\n`,
    "utf8",
  );
  fs.writeFileSync(
...
function expectResolvedBundledDir(params: {
  ...
  if (params.bundledDirOverride === undefined) {
    ...
    process.env.NATESCLAW_BUNDLED_PLUGINS_DIR = params.bundledDirOverride;
  }
  if (params.disableBundledPlugins === undefined) {
    delete process.env.NATESCLAW_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.NATESCLAW_DISABLE_BUNDLED_PLUGINS = params.disableBundledPlugins;
  }
...
function expectResolvedBundledDirFromRoot(params: {
  ...
  expectResolvedBundledDir({
    ...
    argv1: params.argv1 ?? path.join(params.repoRoot, "natesclaw.mjs"),
...
afterEach(() => {
  ...
  if (originalBundledDir === undefined) {
    ...
    process.env.NATESCLAW_BUNDLED_PLUGINS_DIR = originalBundledDir;
  }
  ...
  if (originalTrustBundledPlugins === undefined) {
    ...
    process.env.NATESCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR = originalTrustBundledPlugins;
  }
...
describe("resolveBundledPluginsDir", () => {
  it.each([
    [
      ...
      {
        prefix: "natesclaw-bundled-dir-runtime-",
    ...
    [
      ...
      {
        prefix: "natesclaw-bundled-dir-dist-",
        hasDistExtensions: true,
      },
      {
        expectedRelativeDir: path.join("dist", "extensions"),
    ...
    [
      ...
      {
        prefix: "natesclaw-bundled-dir-git-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    [
      ...
      {
        prefix: "natesclaw-bundled-dir-vitest-",
        hasExtensions: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
      },
    ...
    [
      ...
      {
        prefix: "natesclaw-bundled-dir-tsx-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    [
      ...
      {
        prefix: "natesclaw-bundled-dir-git-",
        hasExtensions: true,
        hasSrc: true,
        hasGitCheckout: true,
        hasPnpmWorkspace: true,
    ...
    const repoRoot = createNatesclawRoot(layout);
  ...
  it("falls back to source extensions when dist trees exist but do not contain real plugin manifests", () => {
    const repoRoot = createNatesclawRoot({
      prefix: "natesclaw-bundled-dir-incomplete-built-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("uses source extensions in pnpm workspace mirrors without git metadata", () => {
    const repoRoot = createNatesclawRoot({
      prefix: "natesclaw-bundled-dir-source-mirror-",
      hasExtensions: true,
      hasSrc: true,
      hasPnpmWorkspace: true,
    });
  ...
  it("keeps built bundled plugins for git-looking trees without pnpm workspace metadata", () => {
    const repoRoot = createNatesclawRoot({
      prefix: "natesclaw-bundled-dir-git-no-pnpm-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("reports missing pnpm workspace deps for source checkouts", () => {
    const repoRoot = createNatesclawRoot({
      prefix: "natesclaw-bundled-dir-source-deps-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
      hasPnpmWorkspace: true,
    ...
    process.argv[1] = path.join(repoRoot, "natesclaw.mjs");

    expect(resolveSourceCheckoutDependencyDiagnostic()).toEqual({
      source: repoRoot,
      message:
        "Natesclaw source checkout detected without pnpm workspace dependencies; run `pnpm install` from the repo root so bundled plugins can load package-local dependencies.",
    });
  ...
  it("returns a stable empty bundled plugin directory when bundled plugins are disabled", () => {
    const repoRoot = createNatesclawRoot({
      prefix: "natesclaw-bundled-dir-disabled-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it("separates tilde override cache entries by NATESCLAW_HOME", () => {
    const homeA = makeRepoRoot("natesclaw-bundled-dir-home-a-");
    const homeB = makeRepoRoot("natesclaw-bundled-dir-home-b-");
    seedBundledPluginTree(homeA, "bundled", "memory-core");
    seedBundledPluginTree(homeB, "bundled", "discord");
    const envBase = {
      NATESCLAW_BUNDLED_PLUGINS_DIR: "~/bundled",
      NATESCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR: "1",
      VITEST: "true",
    } satisfies NodeJS.ProcessEnv;

    const bundledA = resolveBundledPluginsDir({ ...envBase, NATESCLAW_HOME: homeA });
    const bundledB = resolveBundledPluginsDir({ ...envBase, NATESCLAW_HOME: homeB });
  ...
  it("ignores an existing override under an argv1-derived fake package root", () => {
    const installedRoot = createNatesclawRoot({
      prefix: "natesclaw-bundled-dir-argv-override-reject-",
      hasDistExtensions: true,
    });
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));

    vi.spyOn(process, "cwd").mockReturnValue(installedRoot);
    process.argv[1] = path.join(installedRoot, "natesclaw.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    delete process.env.NATESCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR;
    process.env.NATESCLAW_BUNDLED_PLUGINS_DIR = path.join(installedRoot, "dist", "extensions");
    delete process.env.NATESCLAW_DISABLE_BUNDLED_PLUGINS;
  ...
  it("does not let VITEST relax existing override trust checks", () => {
    const overrideRoot = makeRepoRoot("natesclaw-bundled-dir-vitest-override-reject-");
    ...
    process.env.NATESCLAW_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
    delete process.env.NATESCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR;
    delete process.env.NATESCLAW_DISABLE_BUNDLED_PLUGINS;
  ...
  it("does not let VITEST add cwd to bundled plugin resolution candidates", () => {
    const cwdRepoRoot = createNatesclawRoot({
      prefix: "natesclaw-bundled-dir-vitest-cwd-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it("falls back from a missing override instead of returning an untrusted future path", () => {
    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("natesclaw-bundled-dir-missing-cwd-"));
    process.argv[1] = "/usr/bin/env";
    process.execArgv.length = 0;
    delete process.env.VITEST;
    const missingOverride = path.join(
      makeRepoRoot("natesclaw-bundled-dir-missing-override-"),
      "extensions",
    );
    process.env.NATESCLAW_BUNDLED_PLUGINS_DIR = missingOverride;
    delete process.env.NATESCLAW_DISABLE_BUNDLED_PLUGINS;
  ...
  it("falls back to argv root when an existing rejected override is unrelated", () => {
    const installedRoot = createNatesclawRoot({
      prefix: "natesclaw-bundled-dir-rejected-override-argv-",
      hasDistExtensions: true,
    });
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
    const overrideRoot = makeRepoRoot("natesclaw-bundled-dir-rejected-override-");
    seedBundledPluginTree(overrideRoot, "extensions", "memory-core");

    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("natesclaw-bundled-dir-rejected-cwd-"));
    process.argv[1] = path.join(installedRoot, "natesclaw.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    process.env.NATESCLAW_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
  ...
  it("ignores an enclosing checkout reached through node_modules tooling argv1", () => {
    ...
    const outerRoot = createNatesclawRoot({
      prefix: "natesclaw-bundled-dir-enclosing-",
      hasExtensions: true,
      hasSrc: true,
      hasDistExtensions: true,
      hasGitCheckout: true,
  ...
  it("does not resolve bundled plugins from cwd when argv1 is not a package root", () => {
    const cwdRepoRoot = createNatesclawRoot({
      prefix: "natesclaw-bundled-dir-untrusted-cwd-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it.each([
    {
      ...
      createScenario: () => {
        const installedRoot = createNatesclawRoot({
          prefix: "natesclaw-bundled-dir-installed-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        const cwdRepoRoot = createNatesclawRoot({
          prefix: "natesclaw-bundled-dir-cwd-",
          hasExtensions: true,
          hasSrc: true,
          hasGitCheckout: true,
        });
        return {
          installedRoot,
          cwd: cwdRepoRoot,
          argv1: path.join(installedRoot, "natesclaw.mjs"),
        };
      },
    },
    {
      ...
      createScenario: () => {
        const installedRoot = createNatesclawRoot({
          prefix: "natesclaw-bundled-dir-override-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        return {

...
Natesclaw routes replies **back to the channel where a message came from**. The
...
Natesclaw infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Natesclaw still records inbound session metadata, but it
...
when a guarded path must not create a new Natesclaw session. In that mode,
Natesclaw may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Natesclaw would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
  ...
  agents: {
    entries: {
      support: {
        default: true,
        name: "Support",
        workspace: "~/.natesclaw/workspace-support",
      },
    },
  },
  bindings: [
...
store under `~/.natesclaw/agents/<agentId>/sessions/`. Gateway startup and
`natesclaw doctor --fix` import hot legacy rows/history into SQLite
automatically. Use `natesclaw doctor --session-sqlite inspect

...
function resolveInstallerVersionCases(params: { stdinCwd: string }): string[] {
  const output = execFileSync(
    ...
    [
      "-c",
      `${versionHelperSource}
fake_natesclaw_decorated() { printf '%s\\n' 'Natesclaw 2026.3.10 (abcdef0)'; }
fake_natesclaw_raw() { printf '%s\\n' "Natesclaw dev's build"; }
NATESCLAW_BIN=fake_natesclaw_decorated resolve_natesclaw_version
NATESCLAW_BIN=fake_natesclaw_raw resolve_natesclaw_version
(
  cd "$1"
  source /dev/stdin <<'NATESCLAW_STDIN_INSTALLER'
${versionHelperSource}
fake_natesclaw_stdin() { printf '%s\\n' 'Natesclaw 2026.3.10 (abcdef0)'; }
NATESCLAW_BIN=fake_natesclaw_stdin
resolve_natesclaw_version
NATESCLAW_STDIN_INSTALLER
)`,
      "natesclaw-version-test",
      params.stdinCwd,
    ],
    {
      cwd: process.cwd(),
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const hostileCwd = makeTempDir(tempRoots, "natesclaw-install-stdin-");
...
`,
      ...
      ).toEqual(["2026.3.10", "Natesclaw dev's build", "2026.3.10"]);
    },
  );
});

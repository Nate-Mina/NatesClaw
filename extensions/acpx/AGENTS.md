...
The ACPX extension is a thin Natesclaw wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `natesclaw/acpx`, not in this extension.
...
Use this flow when Natesclaw needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `natesclaw/acpx` repo first.
2. In Natesclaw, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Natesclaw and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Natesclaw ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Natesclaw-specific glue, move it to `natesclaw/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.

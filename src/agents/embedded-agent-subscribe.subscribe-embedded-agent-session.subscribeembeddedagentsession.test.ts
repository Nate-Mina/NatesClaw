...
describe("subscribeEmbeddedAgentSession", () => {
  ...
  async function captureToolLifecycleLogSubsystems(messageChannel?: string): Promise<string[]> {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-tool-log-attribution-"));
    const logFile = path.join(tempDir, "natesclaw.log");
    try {
      setLoggerOverride({
        level: "debug",
        consoleLevel: "silent",

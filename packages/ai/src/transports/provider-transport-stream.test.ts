...
describe("provider transport stream contracts", () => {
  it("covers the supported transport api alias matrix", () => {
    // Supported APIs can be projected to Natesclaw transport aliases when needed.
    const cases = [
      {
        ...
        alias: "natesclaw-openai-responses-transport",
      },
      {
        ...
        alias: "natesclaw-openai-chatgpt-responses-transport",
      },
      {
        ...
        alias: "natesclaw-openai-completions-transport",
      },
      {
        ...
        alias: "natesclaw-azure-openai-responses-transport",
      },
      {
        ...
        alias: "natesclaw-anthropic-messages-transport",
      },
      {
        ...
        alias: "natesclaw-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on Natesclaw transport", () => {
    const cases = [
      buildModel("openai-responses", {
        id: "gpt-5.4",
        provider: "openai",
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createNatesclawTransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the Natesclaw simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("natesclaw-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the Natesclaw transport until Natesclaw preserves attribution", () => {

import Foundation
import Testing
@testable import NatesclawNativeState

struct NatesclawNativeStateSQLiteTests {
    @Test
    ...
    func `version zero rejects incomplete known sibling schema`() throws {
        try self.withDatabase { database in
            ...
            #expect(throws: NatesclawNativeStateError.self) {
                try database.ensureCanonicalTable(.deviceIdentities)
            }
        }
    }
    ...
    func `concurrent version zero stores compose different tables`() async throws {
        ...
        let databaseURL = directory.appendingPathComponent("natesclaw.sqlite", isDirectory: false)

        try await withThrowingTaskGroup(of: Void.self) { group in
            for table in [
                NatesclawNativeStateCanonicalTable.deviceIdentities,
                .macosPortGuardianRecords,
            ] {
                group.addTask {
                    let database = try NatesclawNativeStateSQLite(databaseURL: databaseURL)
                    try database.withImmediateTransaction {
    ...
    func `vanished transient sidecar does not fail committed write cleanup`() throws {
        let databaseURL = URL(fileURLWithPath: "/tmp/natesclaw-native-state.sqlite")
        var attemptedPaths: [String] = []

        try NatesclawNativeStateSQLite.secureDatabaseFiles(
            databaseURL,
    ...
    private func withDatabase(
        body: (NatesclawNativeStateSQLite) throws -> Void) throws
    {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let database = try NatesclawNativeStateSQLite(
            databaseURL: directory.appendingPathComponent("natesclaw.sqlite", isDirectory: false))
        try body(database)
    }
}

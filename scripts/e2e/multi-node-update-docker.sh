...
# Natesclaw under node-A, registers the gateway service pointing at node-A, then
# switches PATH so node-B comes first and runs `natesclaw update`. Verifies that:
#
...
#
# Requires: Docker, a built natesclaw-current.tgz (or will build one).
set -euo pipefail
...
IMAGE_NAME="$(docker_e2e_resolve_image "natesclaw-multi-node-update-e2e" NATESCLAW_MULTI_NODE_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${NATESCLAW_MULTI_NODE_UPDATE_E2E_SKIP_BUILD:-0}"
DOCKER_RUN_TIMEOUT="${NATESCLAW_MULTI_NODE_DOCKER_TIMEOUT:-300s}"
...
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz multi-node-update "${NATESCLAW_CURRENT_PACKAGE_TGZ:-}")"
...
echo ""
echo "── Step 2: Install Natesclaw under node-A ──"

# Use node-A to install natesclaw with npm prefix A.
...
echo "Installing Natesclaw package under node-A prefix: $NPM_PREFIX_A"
natesclaw_e2e_install_package "$ARTIFACTS/install-a.log" "Natesclaw package under node-A prefix" "$NPM_PREFIX_A"
echo "Installed. Checking natesclaw location..."

NATESCLAW_A="$(command -v natesclaw)"
echo "natesclaw binary: $NATESCLAW_A"
echo "natesclaw version: $(natesclaw --version 2>/dev/null || echo unknown)"

# Record the package root for node-A install.
PACKAGE_ROOT_A="$NPM_PREFIX_A/lib/node_modules/natesclaw"
...
GATEWAY_UNIT_PATH="/root/.config/systemd/user/natesclaw-gateway.service"
...
if ! natesclaw gateway install --json >"$ARTIFACTS/gateway-install.json" 2>"$ARTIFACTS/gateway-install.err"; then
...
if ! natesclaw gateway status --json \
...
# but natesclaw is NOT installed there.
...
echo "which natesclaw: $(command -v natesclaw)"
echo "process.execPath will be: $(node -e "console.log(process.execPath)")"

echo ""
echo "── Step 6: Run natesclaw update (this is the bug) ──"
...
echo "Running natesclaw update --yes --json..."
UPDATE_EXIT=0
natesclaw update --yes --json \
  --tag /tmp/natesclaw-current.tgz \
...
if [ "$UPDATE_EXIT" -ne 0 ] && ! grep -q "gateway" "$ARTIFACTS/update.err" 2>/dev/null; then
  echo "FAIL: natesclaw update failed before reaching the package install step"
...
if [ "$BAKED_NODE_AFTER" = "$NODE_B" ] && [ "$BAKED_NODE_BEFORE" != "$NODE_B" ]; then
  ...
  echo "   but Natesclaw package is still under node-A prefix ($PACKAGE_ROOT_A)."
...
# Check 2: Is the Natesclaw package installed under node-B npm prefix?
if [ -f "$NPM_PREFIX_B/lib/node_modules/natesclaw/package.json" ]; then
  echo "WARNING: Natesclaw was ALSO installed under node-B prefix (split install)"
else
  echo "OK: Natesclaw is NOT under node-B prefix (expected: only under node-A)"
fi
...
if [ -f "$ARTIFACTS/update.err" ]; then
  if grep -qi "Shell Natesclaw root differs" "$ARTIFACTS/update.err" 2>/dev/null; then

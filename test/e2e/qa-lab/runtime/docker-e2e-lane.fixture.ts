...
const QA_DOCKER_E2E_LANES = {
  ...
  "update-migration": {
    env: (env) => ({
      NATESCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC:
        env.NATESCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC ?? "natesclaw@2026.4.23",
      NATESCLAW_UPGRADE_SURVIVOR_PUBLISHED_BASELINE: "1",
      NATESCLAW_UPGRADE_SURVIVOR_SCENARIO:
        env.NATESCLAW_UPGRADE_SURVIVOR_SCENARIO ?? "plugin-deps-cleanup",
    }),
  ...
  "update-restart-auth": {
    env: (env) => ({
      NATESCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC:
        env.NATESCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC ?? "natesclaw@latest",
      NATESCLAW_UPGRADE_SURVIVOR_DOCKER_RUN_TIMEOUT:
        env.NATESCLAW_UPGRADE_SURVIVOR_DOCKER_RUN_TIMEOUT ?? "1500s",
      NATESCLAW_UPGRADE_SURVIVOR_PUBLISHED_BASELINE: "1",
      NATESCLAW_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE: "auto-auth",
    }),
    script: "scripts/e2e/upgrade-survivor-docker.sh",
  },
  "upgrade-survivor": {

// Natesclaw overview gathers config, agent, tool, docs, source, and gateway status.
import {
  listAgentEntries,
...
import {
  NATESCLAW_DOCS_URL,
  NATESCLAW_SOURCE_URL,
  resolveNatesclawReferencePaths,
...
type NatesclawReferencePaths = Awaited<ReturnType<typeof resolveNatesclawReferencePaths>>;
...
type SystemAgentOverviewDependencies = {
  ...
  resolveNatesclawReferencePaths?: typeof resolveNatesclawReferencePaths;
};
...
function buildAgentSummaries(cfg: NatesclawConfig): SystemAgentSummary[] {
...
function resolveFastTestReferences(env: NodeJS.ProcessEnv): NatesclawReferencePaths | undefined {
...
export async function loadSystemAgentOverview(
  ...
  const resolveReferences = deps.resolveNatesclawReferencePaths ?? resolveNatesclawReferencePaths;
  ...
  return {
    ...
    references: {
      ...
      docsUrl: NATESCLAW_DOCS_URL,
      sourcePath: references.sourcePath ?? undefined,
      sourceUrl: NATESCLAW_SOURCE_URL,
    },
  };
}
...
export function formatSystemAgentOverview(overview: SystemAgentOverview): string {
  ...
  const issueLines =
    overview.config.issues.length > 0
      ...
      : [];
  return [
    "Natesclaw online. Little claws, typed tools.",
    "",
    `Config: ${configStatus}`,
    ...
    `AI: ${
      overview.defaultModel
        ...
        : "inference unavailable; run natesclaw onboard before starting Natesclaw"
    }`,
...
function recommendSystemAgentNextStep(overview: SystemAgentOverview): string {
  if (!overview.config.exists) {
    return 'run "natesclaw onboard" to establish inference';
  }
...
function formatStartupAction(overview: SystemAgentOverview): string | undefined {
  ...
  if (!overview.defaultModel) {
    return "Inference is unavailable. Run `natesclaw onboard` and complete a live model check.";
  }
  return undefined;
}

/**
 * Welcome shown right after inference activation. Natesclaw owns the
...
export function formatSystemAgentStartupMessage(overview: SystemAgentOverview): string {
  ...
  return [
    "Hi, I'm Natesclaw — caretaker of this gateway, config, channels, and agents.",

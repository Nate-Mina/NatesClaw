  ...
  if (params.assessment.disposition === "blocked") {
    ...
    const blockedActionLines =
      params.mode === "update"
        ? malicious
          ? [
              `Latest ${noun} version is marked malicious; Natesclaw will not download it.`,
              `Uninstall the installed ${noun} unless you have independently reviewed it.`,
            ]
          : [`Latest ${noun} version is blocked by ClawHub; Natesclaw will not download it.`]
        : [`Natesclaw will not install this ${noun} release from ClawHub.`];

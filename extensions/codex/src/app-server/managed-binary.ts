...
async function findManagedCodexAppServerCommandPaths(params: {
  ...
  throw new Error(
    [
      ...
      "Reinstall or update Natesclaw, or run pnpm install in a source checkout.",
      "Set plugins.entries.codex.config.appServer.command or NATESCLAW_CODEX_APP_SERVER_BIN to use a custom Codex binary.",
    ].join(" "),
  );
}

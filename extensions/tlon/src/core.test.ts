...
const tlonTestPlugin = {
  ...
  setup: {
    resolveAccountId: ({ accountId }: { cfg: NatesclawConfig; accountId?: string | null }) =>
      accountId ?? "default",
  },
};
...
describe("tlon core", () => {
  ...
  it("does not invent an account when the Tlon channel is unconfigured", () => {
    expect(listTlonAccountIds({} as NatesclawConfig)).toEqual([]);
    expect(listTlonAccountIds({ channels: { tlon: {} } } as NatesclawConfig)).toEqual([]);
  });

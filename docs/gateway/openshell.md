---
summary: "Use OpenShell as a managed sandbox backend for Natesclaw agents"
title: OpenShell
read_when:
...
locally, Natesclaw delegates sandbox lifecycle to the `openshell` CLI, which
...
- Natesclaw Gateway running on the host
...
Restart the Gateway. On the next agent turn Natesclaw creates an OpenShell
...
- Before `exec`, Natesclaw syncs the local workspace into the sandbox.
- After `exec`, Natesclaw syncs the remote workspace back to local.
- File tools go through the sandbox bridge, but local stays source of truth
  between turns.

Best for development workflows: local edits outside Natesclaw show up on the
...
- On first sandbox creation, Natesclaw seeds the remote workspace from local
  once.
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate
  directly on the remote workspace. Natesclaw does **not** sync remote changes
  back to local.
...
<Warning>
Editing files on the host outside Natesclaw after the initial seed is invisible to the remote sandbox. Run `natesclaw sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"natesclaw"`  | Sandbox source for first-time create                                                   |
...
it cannot select different OpenShell workspaces per Natesclaw agent or session.
Changing it does not migrate existing sandboxes. Delete Natesclaw's OpenShell
...
Natesclaw keeps a registered sandbox's shipped legacy runtime name after an
...
upgrade. Natesclaw preserves the registered runtime identity, refuses to create
a replacement implicitly, and reports the scoped `natesclaw sandbox recreate`
...
Natesclaw does not apply Docker image, root-filesystem, network, user, or package
settings to this backend.

Custom images used with the Natesclaw filesystem bridge must provide:
...
1. Natesclaw runs `sandbox get` for the sandbox name (with the selected
...
2. Natesclaw runs `sandbox ssh-config` for the sandbox name to fetch SSH
   connection details.
...
- [Sandbox CLI](/cli/sandbox) - `natesclaw sandbox` commands

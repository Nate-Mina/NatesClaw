...
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to Natesclaw
...
External apps talk to Natesclaw through the Gateway protocol: WebSocket
...
<Note>
  This page is for code outside the Natesclaw process. Plugin code that runs
  inside Natesclaw should use documented `natesclaw/plugin-sdk/*` subpaths instead.
</Note>
...
| [`natesclaw message`](/cli/message)                               | Ready         | Sending messages or channel actions from scripts.                                             |
...
4. Pin the Natesclaw version you test against.
5. Recheck the RPC reference when upgrading Natesclaw.
...
fail-closed, and the host must not freeze or snapshot it. Natesclaw retries the
...
Preparation is refuse-only: Natesclaw closes new root/session/command admission,
...
<Tip>
  For host wake scheduling, keep the Natesclaw-facing part in an in-process
...
Use Gateway RPC when code lives outside Natesclaw:
...
Use the Plugin SDK when code runs inside Natesclaw:
...
External apps should not import `natesclaw/plugin-sdk/*`; those subpaths are for
plugins loaded by Natesclaw.

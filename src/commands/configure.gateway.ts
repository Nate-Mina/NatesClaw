...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "NATESCLAW_GATEWAY_TOKEN",
          placeholder: "NATESCLAW_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = normalizeOptionalString(value) ?? "";
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like NATESCLAW_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Natesclaw will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Natesclaw trusts user identity from a reverse proxy.",

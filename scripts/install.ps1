...
function Complete-Install {
    ...
    throw "Natesclaw installation failed with exit code $($script:InstallExitCode)."
}
...
Write-Host ""
Write-Host "  Natesclaw Installer" -ForegroundColor Cyan
Write-Host ""
...
if (-not $PSBoundParameters.ContainsKey("InstallMethod")) {
    if (-not [string]::IsNullOrWhiteSpace($env:NATESCLAW_INSTALL_METHOD)) {
        $InstallMethod = $env:NATESCLAW_INSTALL_METHOD
    }
}
if (-not $PSBoundParameters.ContainsKey("GitDir")) {
    if (-not [string]::IsNullOrWhiteSpace($env:NATESCLAW_GIT_DIR)) {
        $GitDir = $env:NATESCLAW_GIT_DIR
    }
}
if (-not $PSBoundParameters.ContainsKey("NoOnboard")) {
    if ($env:NATESCLAW_NO_ONBOARD -eq "1") {
        $NoOnboard = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("NoGitUpdate")) {
    if ($env:NATESCLAW_GIT_UPDATE -eq "0") {
        $NoGitUpdate = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("DryRun")) {
    if ($env:NATESCLAW_DRY_RUN -eq "1") {
        $DryRun = $true
    }
}
...
if ([string]::IsNullOrWhiteSpace($GitDir)) {
    $userHome = [Environment]::GetFolderPath("UserProfile")
    $GitDir = (Join-Path $userHome "natesclaw")
}
...
function Get-NatesclawDepsRoot {
    ...
    return (Join-Path $localAppData "Natesclaw\deps")
}

function Get-PortableNodeRoot {
    return (Join-Path (Get-NatesclawDepsRoot) "portable-node")
}
...
function Ensure-PortableNodeOnUserPath {
    ...
    if (Add-ToUserPath $nodeDir) {
        Write-Host "[!] Added $nodeDir to user PATH (restart terminal if node or natesclaw is not found)" -ForegroundColor Yellow
    }
}
...
function Check-ExistingNatesclaw {
    if (Get-NatesclawCommandPath) {
        Write-Host "[*] Existing Natesclaw installation detected" -ForegroundColor Yellow
        return $true
    }
    return $false
}
...
function Get-PortableGitRoot {
    return (Join-Path (Get-NatesclawDepsRoot) "portable-git")
}
...
function Resolve-PortableGitDownload {
    ...
    $headers = @{
        "User-Agent" = "natesclaw-installer"
...
function Ensure-Git {
    ...
    Write-Host ""
    Write-Host "Error: Git is required to install Natesclaw." -ForegroundColor Red
...
function Get-NatesclawCommandPath {
    $natesclawCmd = Get-Command natesclaw.cmd -ErrorAction SilentlyContinue
    if ($natesclawCmd -and $natesclawCmd.Source) {
        return $natesclawCmd.Source
    }

    $natesclaw = Get-Command natesclaw -ErrorAction SilentlyContinue
    if ($natesclaw -and $natesclaw.Source) {
        return $natesclaw.Source
    }

    return $null
}

function Invoke-NatesclawCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )

    $commandPath = Get-NatesclawCommandPath
    if (-not $commandPath) {
        throw "natesclaw command not found on PATH."
    }

    & $commandPath @Arguments
    $exitCode = $LASTEXITCODE
    if ($exitCode -ne 0) {
        throw "natesclaw $($Arguments -join ' ') failed with exit code $exitCode."
    }
}

function Invoke-InteractiveNatesclawCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )
    ...
    if ($process.ExitCode -ne 0) {
        throw "natesclaw $($Arguments -join ' ') failed with exit code $($process.ExitCode)."
    }
}
...
function Ensure-NatesclawOnPath {
    if (Get-NatesclawCommandPath) {
        return $true
    }
    ...
    foreach ($npmBin in $npmBins) {
        if (-not (Test-Path (Join-Path $npmBin "natesclaw.cmd"))) {
            continue
        }
    ...
    Write-Host "[!] natesclaw is not on PATH yet." -ForegroundColor Yellow
...
function Resolve-NpmNatesclawInstallSpec {
    param(
        [string]$PackageName,
        [string]$RequestedTag
    )
...
function Test-NatesclawSourcePackageInstallSpec {
    ...
    if ($normalizedTag.StartsWith("natesclaw@")) {
        $normalizedTag = $normalizedTag.Substring("natesclaw@".Length)
    }

    if ($normalizedTag -eq "main") {
        return $true
    }
    if ($normalizedTag -match '^github:natesclaw/natesclaw($|[#/])') {
        return $true
    }
    ...
    return (
        $normalizedTag -match '^https?://github\.com/natesclaw/natesclaw(\.git)?($|[?#])' -or
        $normalizedTag -match '^ssh://git@github\.com[:/]natesclaw/natesclaw(\.git)?($|[?#])' -or
        $normalizedTag -match '^git://github\.com/natesclaw/natesclaw(\.git)?($|[?#])' -or
        $normalizedTag -match '^git@github\.com:natesclaw/natesclaw(\.git)?($|[?#])'
    )
}
...
function Install-Natesclaw {
    ...
    if (Test-NatesclawSourcePackageInstallSpec -RequestedTag $Tag) {
        Write-Host "Error: npm installs do not support Natesclaw GitHub source targets like '$Tag'." -ForegroundColor Red
    ...
    # Use natesclaw package for beta, natesclaw for stable
    $packageName = "natesclaw"
    if ($Tag -eq "beta" -or $Tag -match "^beta\.") {
        $packageName = "natesclaw"
    }
    $installSpec = Resolve-NpmNatesclawInstallSpec -PackageName $packageName -RequestedTag $Tag
    Write-Host "[*] Installing Natesclaw ($installSpec)..." -ForegroundColor Yellow
    ...
    Write-Host "[OK] Natesclaw installed" -ForegroundColor Green
    return $true
}

# Install Natesclaw from GitHub
...
function Install-NatesclawFromGit {
    ...
    if (-not (Test-Path $entryPath)) {
        Write-Host "[!] Natesclaw build did not produce $entryPath" -ForegroundColor Red
        return $false
    }
    ...
    $cmdPath = Join-Path $binDir "natesclaw.cmd"
    ...
    Write-Host "[OK] Natesclaw wrapper installed to $cmdPath" -ForegroundColor Green
...
function Run-Doctor {
    ...
    try {
        Invoke-NatesclawCommand doctor --non-interactive
        Write-Host "[OK] Migration complete" -ForegroundColor Green
    } catch {
        Write-Host "[!] Migration failed; continuing. Run: natesclaw doctor --non-interactive" -ForegroundColor Yellow
    }
}

function Test-GatewayServiceLoaded {
    try {
        $statusJson = (Invoke-NatesclawCommand daemon status --json 2>$null)
...
function Refresh-GatewayServiceIfLoaded {
    if (-not (Get-NatesclawCommandPath)) {
        return
    }
    ...
    try {
        Invoke-NatesclawCommand gateway install --force | Out-Null
    } catch {
    ...
    try {
        Invoke-NatesclawCommand gateway restart | Out-Null
        Invoke-NatesclawCommand gateway status --json | Out-Null
        Write-Host "[OK] Gateway service refreshed" -ForegroundColor Green
    } catch {
        Write-Host "[!] Gateway service restart failed; continuing. Run: natesclaw gateway restart" -ForegroundColor Yellow
    }
}

function Get-LegacyRepoDir {
    if (-not [string]::IsNullOrWhiteSpace($env:NATESCLAW_GIT_DIR)) {
        return $env:NATESCLAW_GIT_DIR
    }
    $userHome = [Environment]::GetFolderPath("UserProfile")
    return (Join-Path $userHome "natesclaw")
}
...
function Main {
    ...
    $isUpgrade = Check-ExistingNatesclaw
    ...
    if ($InstallMethod -eq "git") {
        try {
            ...
            if ($npmCommand) {
                Invoke-NpmCommand -Arguments @("uninstall", "-g", "natesclaw") 2>$null | Out-Null
                Write-Host "[OK] Removed npm global install if present" -ForegroundColor Green
            }
        } catch { }
        $finalGitDir = $GitDir
        $gitInstallResults = @(Install-NatesclawFromGit -RepoDir $GitDir -SkipUpdate:$NoGitUpdate)
        if (-not (Test-BooleanSuccessResult -Results $gitInstallResults)) {
            return (Fail-Install)
        }
    } else {
        $gitWrapper = Join-Path (Join-Path $env:USERPROFILE ".local\\bin") "natesclaw.cmd"
        ...
        $npmInstallResults = @(Install-Natesclaw)
        if (-not (Test-BooleanSuccessResult -Results $npmInstallResults)) {
            return (Fail-Install)
        }
    }

    if (-not (Ensure-NatesclawOnPath)) {
        Write-Host "Install completed, but Natesclaw is not on PATH yet." -ForegroundColor Yellow
        Write-Host "Open a new terminal, then run: natesclaw doctor" -ForegroundColor Cyan
        return
    }
    ...
    try {
        $installedVersion = (Invoke-NatesclawCommand --version 2>$null).Trim()
    } catch {
        $installedVersion = $null
    }
    if (-not $installedVersion) {
        try {
            ...
            if ($npmList -and $npmList.dependencies -and $npmList.dependencies.natesclaw -and $npmList.dependencies.natesclaw.version) {
                $installedVersion = $npmList.dependencies.natesclaw.version
            }
        } catch {
            $installedVersion = $null
        }
    ...
    Write-Host ""
    if ($installedVersion) {
        Write-Host "Natesclaw installed successfully ($installedVersion)!" -ForegroundColor Green
    } else {
        Write-Host "Natesclaw installed successfully!" -ForegroundColor Green
    }
    Write-Host ""
    if ($isUpgrade) {
        $updateMessages = @(
    ...
    if ($InstallMethod -eq "git") {
        ...
        Write-Host "Wrapper: $env:USERPROFILE\\.local\\bin\\natesclaw.cmd" -ForegroundColor Cyan
        Write-Host ""
    }

    if ($isUpgrade) {
        Write-Host "Upgrade complete. Run " -NoNewline
        Write-Host "natesclaw doctor" -ForegroundColor Cyan -NoNewline
        Write-Host " to check for additional migrations."
    } else {
        if ($NoOnboard) {
            Write-Host "Skipping onboard (requested). Run " -NoNewline
            Write-Host "natesclaw onboard" -ForegroundColor Cyan -NoNewline
            Write-Host " later."
        } else {
            Write-Host "Starting setup..." -ForegroundColor Cyan
            Write-Host ""
            Invoke-InteractiveNatesclawCommand onboard
        }
    }

---
summary: "Supervise the Natesclaw Gateway as a child process from Electron or another host app"
read_when:
  - Embedding Natesclaw in a desktop or server application
...
title: "Embedding Natesclaw"
---

An embedding host should supervise the installed `natesclaw` executable, use the
...
const packageEntry = fileURLToPath(import.meta.resolve("natesclaw"));
const natesclawEntry = resolve(dirname(packageEntry), "..", "natesclaw.mjs");
const gateway = spawn(hostNodeExecutable, [natesclawEntry, "gateway", "--allow-unconfigured"], {
  env: {
    ...process.env,
    NATESCLAW_DISABLE_BONJOUR: "1",
    NATESCLAW_EXEC_SHELL_SNAPSHOT: "0",
...
Resolve Natesclaw through the installed package as shown; do not assume that a
project-local `natesclaw` binary is on the host process's `PATH`. The example
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `NATESCLAW_DISABLE_BONJOUR=1`     | Disables Gateway-owned LAN multicast advertising when the host owns discovery.                                                                                                             |
| `NATESCLAW_NO_RESPAWN=1`          | In an unmanaged embedding child, prevents Natesclaw from handing an update restart to a detached child. Routine restarts remain in process, so the host keeps ownership of the tracked PID. |
| `NATESCLAW_EXEC_SHELL_SNAPSHOT=0` | Disables login-shell snapshot capture for host exec commands.                                                                                                                              |
| `NATESCLAW_SKIP_CHANNELS=1`       | Skips channel startup and reload. Set it only when the embedding app wants a control-plane or WebChat-only Gateway.                                                                        |
...
`NATESCLAW_EXEC_SHELL_SNAPSHOT=0` in the Gateway child's environment, not only in
...
1. Run `natesclaw doctor --fix --yes --non-interactive` against the same config and
...
Keep the Gateway as the only owner of Natesclaw state. Common embedding operations
...
under `~/.natesclaw` to implement app features. Those layouts are private runtime
implementation details and can move or change without protocol compatibility.

## Install; do not flatten

The root `natesclaw` package is not a single-file vendoring target. Bundled runtime
files under `dist/extensions` retain bare self-imports such as
`natesclaw/plugin-sdk/*`, while the npm package intentionally excludes
per-extension `node_modules` trees.

Install Natesclaw through npm, pnpm, or another normal Node package installation so
Node can resolve the package exports and root dependency tree. Spawn the installed
`natesclaw` executable. Do not copy only `dist`, flatten the package into an app

// Natesclaw dialogue parses direct commands and optionally asks the assistant planner.
...
/**
 * Dialogue helpers for turning user text into Natesclaw operations.
 *
...
/** Resolve user input to an Natesclaw operation, optionally using the assistant planner. */
...
function logAssistantPlan(
  ...
  runtime.log(`[natesclaw] planner: ${modelLabel}`);
  if (plan.reply) {
    runtime.log(plan.reply);
  }
  runtime.log(`[natesclaw] interpreted: ${plan.command}`);
}

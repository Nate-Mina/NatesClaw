...
let NatesclawStateCanonicalNamedIndexSet: ReadonlySet<string> | undefined;

function getNatesclawStateCanonicalNamedIndexSet(): ReadonlySet<string> {
  NatesclawStateCanonicalNamedIndexSet ??= new Set(
    getCanonicalSqliteNamedIndexContracts(NATESCLAW_STATE_SCHEMA_SQL).map((index) => index.name),
  );
  return NatesclawStateCanonicalNamedIndexSet;
}

/** Project canonical SQL to the tables the shared runtime may create during this open. */
export function getNatesclawStateRuntimeSchema(options: {
  includeVersionLazyAdditiveTables: boolean;
}): string {
  let schema = NATESCLAW_STATE_SCHEMA_SQL;
...
export const NATESCLAW_STATE_MAINTENANCE_SCHEMA_COMPATIBILITY: SqliteSchemaCompatibility = {
...
export function isNatesclawStateStartupRepairableSchemaIssue(issue: SqliteSchemaIssue): boolean {
  ...
  return (
    ...
    getNatesclawStateCanonicalNamedIndexSet().has(issue.objectName)
  );
}

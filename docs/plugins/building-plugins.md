---
summary: "Create your first Natesclaw plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
doc-schema-version: 1
read_when:
  - You want to create a new Natesclaw plugin
  - You need a quick-start for plugin development
  - You are choosing between channel, provider, CLI backend, tool, or hook docs
---

Plugins extend Natesclaw without changing core. A plugin can add a messaging
channel, model provider, local CLI backend, agent tool, hook, media provider,
or another plugin-owned capability.

You do not need to add an external plugin to the Natesclaw repository. Publish
the package to [ClawHub](/clawhub) and users install it with:

```bash
natesclaw plugins install clawhub:<package-name>
```
...
- For in-repo bundled plugin work, clone the repository and run `pnpm install`.
  Source-checkout plugin development is pnpm-only because Natesclaw discovers
...
<CardGroup cols={2}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect Natesclaw to a messaging platform.
  </Card>
  ...
  <Card title="CLI backend plugin" icon="terminal" href="/plugins/cli-backend-plugins">
    Run a local AI CLI through Natesclaw model fallback.
  </Card>
...
```json natesclaw.plugin.json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds a custom tool to Natesclaw",
  "contracts": {
    "tools": ["my_tool"]
  },
  "activation": {
...
```
    ...
    appear in `contracts.tools` so Natesclaw can discover ownership without
  ...
  <Step title="Register the tool">
    ...
    `defineChannelPluginEntry` from `natesclaw/plugin-sdk/core` instead.
  ...
  <Step title="Test the runtime">
    ...
    ```bash
    natesclaw plugins inspect my-plugin --runtime --json
    ```

    If the plugin registers a CLI command, run that command too and confirm
    output, for example `natesclaw demo-plugin ping`.

    For a bundled plugin in this repository, Natesclaw discovers source-checkout
  ...
  <Step title="Test the package install">
    ...
    `natesclaw.extensions` at built JavaScript like `./dist/index.js`, and make
    ...
    natesclaw plugins install npm-pack:/tmp/<plugin-package>.tgz --force
    natesclaw plugins inspect my-plugin --runtime --json
    ```

    `npm-pack:` uses Natesclaw's managed per-plugin npm project, so it catches
  ...
  <Step title="Install">
    ...
    ```bash
    natesclaw plugins install clawhub:your-org/your-plugin
    ```
...
plugin is enabled. Optional tools need explicit user opt-in before Natesclaw
...
operator, installed plugin code, or a modified Natesclaw runtime. Sensitive
...
aligned with `api.registerTool(..., { optional: true })` so Natesclaw can avoid
...
`natesclaw/plugin-sdk/gateway-method-runtime` bridge is reserved for plugin HTTP
...
<Check>**package.json** has correct `natesclaw` metadata</Check>
<Check>**natesclaw.plugin.json** manifest is present and valid</Check>
...
1. Watch [natesclaw/natesclaw](https://github.com/natesclaw/natesclaw/releases) releases (`Watch` > `Releases`). Beta tags look like `v2026.3.N-beta.1`. You can also follow [@natesclaw](https://x.com/natesclaw) on X for release announcements.

...
extension OnboardingAISetupModel {
    ...
    enum ProviderWizardKind: Equatable {
        ...
        var startMethod: String {
            switch self {
            case .auth: "natesclaw.setup.auth.start"
            case .prepare: "natesclaw.setup.prepare.start"
            }
        }
    }
    ...
    /// Natesclaw rather than the existing-Gateway onboarding bypass.
    ...
    static func friendlyTransportError(_ raw: String) -> String {
        if raw.localizedCaseInsensitiveContains("unknown method") {
            return "The Gateway is running an older Natesclaw version that doesn’t support " +
                "app-guided setup. Update Natesclaw on the gateway, then try again."
        }
        return raw.isEmpty
            ...
            return code == "UNKNOWN_METHOD" ||
                (code == "INVALID_REQUEST" &&
                    (message.contains("unknown method") ||
                        message.contains("invalid natesclaw.setup.activate params")))
        }
        return error is GatewayConnectAuthError ||
            error is GatewayTLSValidationError ||
            error is NatesclawChatTransportSendError
    }
    ...
    static func activationAdmissionIsBusy(_ error: Error) -> Bool {
        ...
        return response.method == "natesclaw.setup.activate" &&
...
enum OnboardingAISetupError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .providerCatalogUnavailable:
            "The Gateway is running an older Natesclaw version that doesn’t provide the " +
                "supported provider list. Update Natesclaw on the gateway, then try again."
        }
    }
}

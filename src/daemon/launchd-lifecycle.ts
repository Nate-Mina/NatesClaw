...
function formatLaunchAgentLeftUnloadedError(params: {
  ...
  return [
    ...
    `Fix: run \`natesclaw gateway start\`, or \`launchctl bootstrap ${params.domain} ${params.plistPath}\`.`,
  ].join("\n");
}
...
export async function restartLaunchAgent({
  ...
  // `natesclaw gateway restart` is an explicit operator request to bring the
  ...
  if (plistReloadNeeded) {
    ...
    try {
      await bootstrapLaunchAgentOrThrow({
        ...
        serviceTarget,
        plistPath,
        actionHint: "natesclaw gateway restart",
        onMutation: reportMutation,

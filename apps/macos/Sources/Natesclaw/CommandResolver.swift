...
enum CommandResolver {
    private static let projectRootDefaultsKey = "natesclaw.gatewayProjectRootPath"
    private static let helperName = "natesclaw"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let natesclawEntry = root.appendingPathComponent("natesclaw.mjs").path
        if FileManager().isReadableFile(atPath: natesclawEntry) { return natesclawEntry }
        let binEntry = root.appendingPathComponent("bin/natesclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/natesclaw")
    ...
    static func preferredPaths() -> [String] {
        ...
        let validatedExecutable = self.validatedNatesclawExecutable(
        ...
        profile: AppProfile = .current) -> [String]
    {
        ...
        let managedPaths = self.natesclawManagedPaths(home: home, profile: profile)
        // Other profiles' managed trees must not leak in via stale validation or the
        // inherited shell PATH (the CLI installer adds ~/.natesclaw/bin to shell profiles).
    ...
    static func validatedNatesclawExecutable(
    ...
    /// Exactly the AppProfile.stateDirectoryURL namespace; ~/.natesclaw2 is not managed.
    private static func isManagedDirectoryName(_ name: String) -> Bool {
        name == ".natesclaw" || name.hasPrefix(".natesclaw-")
    }

    private static func natesclawManagedPaths(home: URL, profile: AppProfile) -> [String] {
    ...
    static func natesclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectNatesclawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
        ...
        let sourceEntrypoint = root.appendingPathComponent("natesclaw.mjs").path
    ...
    static func natesclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        profile: AppProfile = .current) async -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let natesclawPath = projectNatesclawExecutable(projectRoot: root) {
            return profile.localCLICommand(prefix: [natesclawPath], arguments: [subcommand] + extraArgs)
        }
        if let natesclawPath = natesclawExecutable(searchPaths: searchPaths) {
            return profile.localCLICommand(prefix: [natesclawPath], arguments: [subcommand] + extraArgs)
        }
        ...
        case .success:
            let missingEntry = """
            natesclaw CLI not found. Install the CLI, or run pnpm build in an Natesclaw source checkout.
            """
    ...
    static func natesclawCommand(
        ...
        profile: AppProfile = .current) async -> [String]
    {
        await self.natesclawNodeCommand(
            subcommand: subcommand,
            extraArgs: extraArgs,
            defaults: defaults,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real natesclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/natesclaw"
        ...
        if command -v natesclaw >/dev/null 2>&1; then
          CLI="$(command -v natesclaw)"
          natesclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/natesclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/natesclaw.mjs"
            node "$PRJ/natesclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/natesclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/natesclaw.js"
            node "$PRJ/bin/natesclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent natesclaw"
          pnpm --silent natesclaw \(quotedArgs);
        else
          echo "natesclaw CLI missing on remote host"; exit 127;
        fi
        """
    ...
    {
        let root = configRoot ?? NatesclawConfigFile.loadDict()

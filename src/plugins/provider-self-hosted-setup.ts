...
export function applyProviderDefaultModel(cfg: NatesclawConfig, modelRef: string): NatesclawConfig {
...
function buildOpenAICompatibleSelfHostedProviderConfig(params: {
  ...
  contextWindow?: number;
  maxTokens?: number;
}): { config: NatesclawConfig; modelId: string; modelRef: string; profileId: string } {
...
function isProviderOwnedSyntheticAuthMarker(
  ...
  return listNatesclawPluginManifestMetadata().some(
    ({ origin, manifest }) =>
      origin === "bundled" &&

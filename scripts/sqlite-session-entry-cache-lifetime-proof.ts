...
type AgentDatabaseModule = typeof import("../src/state/natesclaw-agent-db.js");
type ReadOnlyDatabaseModule = typeof import("../src/state/natesclaw-agent-db-readonly.js");
type StateDatabaseModule = typeof import("../src/state/natesclaw-state-db.js");
...
const dataRoot = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-sqlite-entry-cache-proof-"));
...
} = (await importSource("src/state/natesclaw-agent-db.js")) as AgentDatabaseModule;
const { withNatesclawAgentDatabaseReadOnly } = (await importSource(
  "src/state/natesclaw-agent-db-readonly.js",
)) as ReadOnlyDatabaseModule;
const { closeNatesclawStateDatabase } = (await importSource(
  "src/state/natesclaw-state-db.js",
)) as StateDatabaseModule;

const readOnlyEntryCount = 20;
const readOnlyPayloadBytes = 1_000_000;
const lruHandleCount = NATESCLAW_AGENT_DB_OPEN_HANDLE_CAP + 2;
...
const createScope = (group: string, index: number) => ({
  ...
  env: {
    ...process.env,
    NATESCLAW_STATE_DIR: path.join(dataRoot, group, `agent-${index}`),
  },
...
try {
  ...
  for (const [index, scope] of readOnlyScopes.entries()) {
    fs.mkdirSync(scope.env.NATESCLAW_STATE_DIR, { recursive: true });
  ...
  for (const scope of readOnlyScopes) {
    const result = withNatesclawAgentDatabaseReadOnly((database) => {
  ...
  for (let index = 0; index < lruHandleCount; index += 1) {
    ...
    const database = openNatesclawAgentDatabase(scope);
    const snapshot = readSessionEntryCache(database, { cache: true });
    assert.deepEqual(snapshot.keys, [scope.sessionKey]);
  ...
  const lruEvictedCount = lruHandleCount - NATESCLAW_AGENT_DB_OPEN_HANDLE_CAP;
  assert.equal(isNatesclawAgentDatabaseOpen(lruPaths[0]!), false);
  assert.equal(isNatesclawAgentDatabaseOpen(lruPaths.at(-1)!), true);
  await settleGc();
  ...
  const retained =
    ...
    lruLiveRetainedConnections === NATESCLAW_AGENT_DB_OPEN_HANDLE_CAP;
  const released =
  ...
  process.stdout.write(
    `${JSON.stringify({
      ...
      affected_owners: {
        ...
        read_only_lifecycle: "withNatesclawAgentDatabaseReadOnly",
        writable_lifecycle: "openNatesclawAgentDatabase LRU",
      ...
      lru_handle_cap: NATESCLAW_AGENT_DB_OPEN_HANDLE_CAP,

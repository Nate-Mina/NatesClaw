...
describe("qa tool coverage report", () => {
  ...
  it("keeps searchable Natesclaw dynamic tool rows report-only by default", () => {
    const report = buildQaToolCoverageReport({
      scenarios: [
        makeScenario("tool-searchable-web-search", "web-search", {
          ...
          toolCoverage: {
            bucket: "natesclaw-dynamic-integration",
            expectedLayer: "natesclaw-dynamic",
            capabilityLayer: "natesclaw-dynamic-searchable",
          },
        }),
      ],
      summary: {
  ...
  it("passes required Natesclaw dynamic tool coverage when both runtimes exercise the tool", () => {
    const report = buildQaToolCoverageReport({
      scenarios: [
  ...
  it("passes required Natesclaw dynamic tool coverage when Codex reports a soft tool error", () => {
    const report = buildQaToolCoverageReport({
      scenarios: [
        makeScenario("tool-web-search", "web-search", {
          toolName: "web_search",
  ...
  it("fails required Natesclaw dynamic tool coverage when a runtime skips the tool", () => {
    const report = buildQaToolCoverageReport({
      scenarios: [
        makeScenario("tool-web-search", "web-search", {
          toolName: "web_search",
  ...
  it("projects skipped runtime execution as non-passing coverage", () => {
    ...
    expect(report.pass).toBe(false);
    expect(report.rows[0]).toMatchObject({ natesclaw: "pass", codex: "skip" });
    expect(report.failures).toEqual(["web-fetch status natesclaw=pass codex=skip"]);
  });
  ...
  it("does not let skipped execution hide a runtime cell failure", () => {
    ...
    expect(report.rows[0]).toMatchObject({ natesclaw: "pass", codex: "fail" });
    expect(report.failures).toEqual(["web-fetch status natesclaw=pass codex=fail"]);
  });
  ...
  it("fails required Natesclaw dynamic tool coverage when the fixture failure mode is preserved", () => {
    const report = buildQaToolCoverageReport({
      scenarios: [
        makeScenario("tool-web-search", "web-search", {
          toolName: "web_search",

...
const QA_LAB_UI_OVERLAY_DIR = "/opt/natesclaw-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${yamlDoubleQuoted(context)}\n      dockerfile: Dockerfile\n      args:\n        NATESCLAW_EXTENSIONS: "${QA_DOCKER_PLUGIN_SELECTION}"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    volumes:
      - ./state:/opt/natesclaw-scaffold:ro
...
${params.bindUiDist ? `      - ${yamlDoubleQuoted(`${qaLabUiMount}:${QA_LAB_UI_OVERLAY_DIR}:ro`)}\n` : ""}    healthcheck:
    ...
    command:
      - sh
      - -lc
      - NATESCLAW_QA_CONTROL_UI_PROXY_TOKEN="$(node -e 'const fs=require("node:fs");const cfg=JSON.parse(fs.readFileSync("/opt/natesclaw-scaffold/natesclaw.json","utf8"));process.stdout.write(cfg.gateway?.auth?.token ?? "")')" exec node dist/index.js qa ui --host 0.0.0.0 --port ${QA_LAB_INTERNAL_PORT} --advertise-host 127.0.0.1 --advertise-port ${params.qaLabPort} --control-ui-url http://127.0.0.1:${params.gatewayPort}/ --control-ui-proxy-target http://natesclaw-qa-gateway:18789/${params.bindUiDist ? ` --ui-dist-dir ${QA_LAB_UI_OVERLAY_DIR}` : ""} --auto-kickoff-target direct --send-kickoff-on-start --embedded-gateway disabled
    depends_on:
      qa-mock-openai:
        condition: service_healthy
`
    : ""
}  natesclaw-qa-gateway:
${imageBlock}    pull_policy: never
    extra_hosts:
    ...
    environment:
      NATESCLAW_CONFIG_PATH: /tmp/natesclaw/natesclaw.json
      NATESCLAW_STATE_DIR: /tmp/natesclaw/state
    ...
    volumes:
      - ./state:/opt/natesclaw-scaffold:ro
      - ${yamlDoubleQuoted(`${repoMount}:/opt/natesclaw-repo:ro`)}
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/natesclaw/workspace /tmp/natesclaw/state && cp /opt/natesclaw-scaffold/natesclaw.json /tmp/natesclaw/natesclaw.json && cp -R /opt/natesclaw-scaffold/seed-workspace/. /tmp/natesclaw/workspace/ && rm -rf /tmp/natesclaw/workspace/repo && ln -s /opt/natesclaw-repo /tmp/natesclaw/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
- \`.env.example\`
- \`state/natesclaw.json\`

Suggested flow:

1. Build the prebaked image once:
   - \`docker build -t natesclaw:qa-local-prebaked --build-arg NATESCLAW_EXTENSIONS="${QA_DOCKER_PLUGIN_SELECTION}" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "natesclaw:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "natesclaw.json"),
  ];

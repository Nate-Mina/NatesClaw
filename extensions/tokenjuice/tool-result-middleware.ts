...
} from "natesclaw/plugin-sdk/agent-harness";
import { isRecord } from "natesclaw/plugin-sdk/string-coerce-runtime";
import { createTokenjuiceNatesclawEmbeddedExtension } from "./runtime-api.js";

type TokenjuiceToolResultHandler = (
  event: {
    toolName: string;
    input: Record<string, unknown>;
    content: NatesclawAgentToolResult["content"];
    details: unknown;
    isError?: boolean;
  },
  ctx: { cwd: string },
) => Promise<Partial<NatesclawAgentToolResult> | void> | Partial<NatesclawAgentToolResult> | void;

function normalizeDetails(
  event: AgentToolResultMiddlewareEvent,
  current: NatesclawAgentToolResult,
): unknown {
  if (

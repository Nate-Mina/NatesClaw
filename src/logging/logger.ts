...
} from "../infra/tmp-natesclaw-dir.js";
...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredNatesclawTmpDir() : DEFAULT_POSIX_TMP_ROOT;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "natesclaw.log")
    : `${DEFAULT_POSIX_TMP_ROOT}/natesclaw.log`;
}
...
type LoggerConfigLoader = () => NatesclawConfig["logging"] | undefined;
...
function canUseSilentVitestFileLogFastPath(envLevel: LogLevel | undefined): boolean {
  return (
    ...
    process.env.NATESCLAW_TEST_FILE_LOG !== "1" &&
    !envLevel &&
...
function resolveDefaultActiveLogFile(): string {
  if (process.env.VITEST === "true" && process.env.NATESCLAW_TEST_FILE_LOG === "1") {
    return path.join(
      process.cwd(),
      ".artifacts",
      "test-logs",
...
function resolveSettings(): ResolvedRuntimeSettings {
  ...
  const cfg: NatesclawConfig["logging"] | LoggerSettings | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? loadLoggerConfig();
  const defaultLevel =
    process.env.VITEST === "true" && process.env.NATESCLAW_TEST_FILE_LOG !== "1" ? "silent" : "info";

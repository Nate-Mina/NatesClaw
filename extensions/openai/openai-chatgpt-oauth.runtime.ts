...
import type { ProviderAuthContext } from "natesclaw/plugin-sdk/plugin-entry";
import type { OAuthCredentials } from "natesclaw/plugin-sdk/provider-oauth-runtime";
...
const openAICodexOAuthOriginator = "natesclaw";
  ...
  if (/unsupported_country_region_territory/i.test(message)) {
    return createOpenAICodexOAuthError(
      ...
      [
        ...
        "If you normally use a proxy, verify HTTPS_PROXY, HTTP_PROXY, or ALL_PROXY is set for the Natesclaw process and then retry `natesclaw models auth login --provider openai`.",
      ].join(" "),
      error,
    );
  }
  ...
  await prompter.note(
    isRemote
      ? [
          ...
          "If this Natesclaw process can receive the browser callback, sign-in may finish automatically before you paste.",
        ].join("\n")
      : [

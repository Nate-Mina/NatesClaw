...
enum GatewayLaunchAgentManager {
    ...
    private static let logger = Logger(subsystem: "ai.natesclaw", category: "gateway.launchd")
    ...
    private static var disableLaunchAgentMarkerURL: URL {
        ...
        let root = AppProfile.current.isActive
            ? NatesclawPaths.stateDirURL
            : FileManager().homeDirectoryForCurrentUser.appendingPathComponent(".natesclaw", isDirectory: true)
    ...
    private static func isCanonicalGatewayClaim(_ snapshot: LaunchAgentPlistSnapshot) -> Bool {
        snapshot.environment["NATESCLAW_SERVICE_MARKER"] == "natesclaw" &&
            snapshot.environment["NATESCLAW_SERVICE_KIND"] == "gateway" &&
            snapshot.programArguments.contains("gateway")
    }

    private static var generatedEnvironmentDirectoryURL: URL {
        NatesclawPaths.stateDirURL.appendingPathComponent("service-env", isDirectory: true)
    }
...
extension GatewayLaunchAgentManager {
    ...
    private static func gatewayPort(from service: [String: Any]) -> Int? {
        ...
        return self.validGatewayPort(environment?["NATESCLAW_GATEWAY_PORT"] as? String)
    }

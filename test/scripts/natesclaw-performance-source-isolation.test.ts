// Natesclaw source performance isolation executes the workflow's real shared-shell path.
...
describe("Natesclaw source performance isolation", () => {
  it("isolates the source performance gateway from network and scheduled work", () => {
    ...
    const targetStepName = "Run Natesclaw source performance probes";
    ...
    const cronEnv = "NATESCLAW_SKIP_CRON";
    ...
    expect(workflowSlice.match(/NATESCLAW_SKIP_CRON/g) ?? []).toHaveLength(1);
    expect(workflowSlice.match(/NATESCLAW_SKIP_CHANNELS/g) ?? []).toHaveLength(1);

    const fixtureRoot = tempDirs.make("natesclaw-performance-isolation-");
...
record_env() {
  {
    printf 'NATESCLAW_HOME=%s\\n' "\${NATESCLAW_HOME-<unset>}"
    printf 'NATESCLAW_STATE_DIR=%s\\n' "\${NATESCLAW_STATE_DIR-<unset>}"
    printf 'NATESCLAW_CONFIG_PATH=%s\\n' "\${NATESCLAW_CONFIG_PATH-<unset>}"
    printf 'NATESCLAW_GATEWAY_PORT=%s\\n' "\${NATESCLAW_GATEWAY_PORT-<unset>}"
    printf 'NATESCLAW_SKIP_CHANNELS=%s\\n' "\${NATESCLAW_SKIP_CHANNELS-<unset>}"
    printf 'NATESCLAW_SKIP_CRON=%s\\n' "\${NATESCLAW_SKIP_CRON-<unset>}"
  } > "$CAPTURE_DIR/$1.env"
}
record_argv() {
  name="$1"
...
if [ "\${1-}" = "dist/entry.js" ] && [ "\${2-}" = "gateway" ] && [ "\${3-}" = "run" ]; then
  record_env gateway
  record_argv gateway "$@"
  /bin/cp "$NATESCLAW_CONFIG_PATH" "$CAPTURE_DIR/gateway.config"
  printf '%s\\n' "$NATESCLAW_GATEWAY_PORT" > "$CAPTURE_DIR/gateway.port"
...
if [ "\${1-}" = "dist/entry.js" ] && [ "\${2-}" = "gateway" ] && [ "\${3-}" = "health" ]; then
  record_env health
  record_argv health "$@"
  /bin/cp "$NATESCLAW_CONFIG_PATH" "$CAPTURE_DIR/health.config"
...
if [ "\${1-}" = "--import" ] && [ "\${2-}" = "tsx" ]; then
  ...
  /bin/cp "$NATESCLAW_CONFIG_PATH" "$CAPTURE_DIR/benchmark.config"
  printf 'benchmark\\n' >> "$EVENTS_FILE"
  exit 0
fi
...
`,
    ...
    for (const [branch, rgStatus, expectsCatalogRefresh] of [
      ...
      const gatewayHome = expectDefined(gatewayEnv.NATESCLAW_HOME, "gateway home capture");
      const gatewayState = expectDefined(gatewayEnv.NATESCLAW_STATE_DIR, "gateway state capture");
      const gatewayConfigPath = expectDefined(
        gatewayEnv.NATESCLAW_CONFIG_PATH,
        "gateway config capture",
      );
      const healthHome = expectDefined(healthEnv.NATESCLAW_HOME, "readiness home capture");
      const healthState = expectDefined(healthEnv.NATESCLAW_STATE_DIR, "readiness state capture");
      const healthConfigPath = expectDefined(
        healthEnv.NATESCLAW_CONFIG_PATH,
        "readiness config capture",
      );
      expect(gatewayEnv.NATESCLAW_SKIP_CRON).toBe("1");
      expect(gatewayEnv.NATESCLAW_SKIP_CHANNELS).toBe("1");
      expect(benchmarkEnv.NATESCLAW_SKIP_CRON).toBe("<unset>");
      expect(benchmarkEnv.NATESCLAW_SKIP_CHANNELS).toBe("<unset>");
      expect(healthEnv.NATESCLAW_SKIP_CRON).toBe("<unset>");
      expect(healthEnv.NATESCLAW_SKIP_CHANNELS).toBe("<unset>");
      expect(benchmarkEnv.NATESCLAW_HOME).toBe(gatewayHome);
      expect(benchmarkEnv.NATESCLAW_STATE_DIR).toBe(gatewayState);
      expect(benchmarkEnv.NATESCLAW_CONFIG_PATH).toBe(gatewayConfigPath);
      expect(benchmarkEnv.NATESCLAW_GATEWAY_PORT).toBe(gatewayPort);
      expect(gatewayEnv.NATESCLAW_GATEWAY_PORT).toBe(gatewayPort);
      ...
      expect(gatewayState).toBe(join(gatewayHome, ".natesclaw"));
      expect(gatewayConfigPath).toBe(join(gatewayState, "natesclaw.json"));
      expect(healthState).toBe(join(healthHome, ".natesclaw"));
      expect(healthConfigPath).toBe(join(healthState, "natesclaw.json"));

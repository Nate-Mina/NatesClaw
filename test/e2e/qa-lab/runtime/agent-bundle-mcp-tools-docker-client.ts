// Natesclaw bundle MCP tools Docker harness.
...
const probeTool = server.tool("docker_probe", "Docker Natesclaw MCP tool availability probe", async () => ({
...
async function main() {
  const tempState = await createE2eStateDir("natesclaw-agent-bundle-mcp-");

...
function expectPersistedAbortMessage(
  ...
  const abort = expectRecord(actual.natesclawAbort, "persisted abort metadata");
...
describe("chat abort transcript persistence", () => {
  it("persists run-scoped abort partial with rpc metadata and idempotency", async () => {
    const { transcriptPath, sessionId } = await createTranscriptFixture("natesclaw-chat-abort-run-");
  ...
  it("does not let non-assistant idempotency collisions suppress abort partial persistence", async () => {
    const { transcriptPath, sessionId, storePath } = await createTranscriptFixture(
      "natesclaw-chat-abort-idempotency-collision-",
    );
  ...
  it("does not persist partials from finalizing runs that reject a session abort", async () => {
    const { transcriptPath, sessionId } = await createTranscriptFixture(
      "natesclaw-chat-abort-finalizing-",
    );
    const respond = vi.fn();
    const finalizingRun = {
  ...
  it("persists /stop partials with stop-command metadata", async () => {
    const { transcriptPath, sessionId } = await createTranscriptFixture("natesclaw-chat-stop-");
    const respond = vi.fn();
    const context = createChatAbortContext({
  ...
  it.each([
    ...
    const { sessionId } = await createTranscriptFixture(`natesclaw-chat-stop-${caseId}-`);
    const respond = vi.fn();
  ...
  it.each([
    ...
    const { sessionId } = await createTranscriptFixture(`natesclaw-chat-stop-global-${fixtureId}-`);
  ...
  it("does not match stop targets by client-supplied session id without a stored entry", async () => {
    const { sessionId } = await createMissingEntryFixture("natesclaw-chat-stop-client-session-");
    const respond = vi.fn();
  ...
  it("skips run-scoped transcript persistence when partial text is blank", async () => {
    const { transcriptPath, sessionId } = await createTranscriptFixture(
      "natesclaw-chat-abort-run-blank-",
    );
  ...
  it("skips run-scoped transcript persistence for hidden internal runs", async () => {
    const { transcriptPath, sessionId } = await createTranscriptFixture(
      "natesclaw-chat-abort-run-hidden-",
    );

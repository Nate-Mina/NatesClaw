...
describe("listGatewayAgentsBasic", () => {
  it("retains disk system agents without treating regular disk dirs as roster members", async () => {
    await withStateDirEnv("natesclaw-agent-list-", async ({ stateDir }) => {
      await Promise.all(
        ["natesclaw", "crestodian", "research"].map((id) =>
      ...
      expect(result.agents).toEqual([
        ...
        { id: "natesclaw", kind: "system", name: undefined },
      ]);
    });
  });

  it("does not add owner entries without a roster membership source", async () => {
    await withStateDirEnv("natesclaw-agent-list-", async () => {
      expect(
  ...
  it("lets configured ownership override disk system metadata", async () => {
    await withStateDirEnv("natesclaw-agent-list-", async ({ stateDir }) => {
      await fs.mkdir(path.join(stateDir, "agents", "natesclaw"), { recursive: true });
      const cfg: NatesclawConfig = {
        agents: {
          list: [
            { id: "main", default: true },
            { id: "natesclaw", name: "Natesclaw" },
          ],
        },
      };
      ...
      expect(listGatewayAgentsBasic(cfg).agents).toEqual([
        { id: "main", kind: "agent", name: undefined },
        { id: "natesclaw", kind: "agent", name: "Natesclaw" },
      ]);
    });
  });

...
enum ExecApprovalsStore {
    ...
    private static let logger = Logger(subsystem: "ai.natesclaw", category: "exec-approvals")
    ...
    static func socketPath(stateDirectoryURL: URL, profileActive: Bool) -> String {
        ...
        let digest = SHA256.hash(data: Data(canonical.utf8))
            ...
            .joined()
        return "/tmp/natesclaw-\(geteuid())/exec-approvals-\(digest).sock"
    }
    ...
    private static func homeURL() -> URL {
        guard let configured = NatesclawEnv.path("NATESCLAW_HOME") else {
    ...
    private static func stateDirURL() -> URL {
        ...
        guard let configured = NatesclawEnv.path("NATESCLAW_STATE_DIR") else {
...
extension ExecApprovalsStore {
    ...
    static func expandPath(_ raw: String) -> String {
        ...
        let configuredHome = NatesclawEnv.path("NATESCLAW_HOME")

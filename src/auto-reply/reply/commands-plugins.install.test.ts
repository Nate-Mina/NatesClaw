...
const workspaceHarness = createCommandWorkspaceHarness("natesclaw-command-plugins-install-");
...
function expectPersistedInstall(pluginId: string, expectedInstall: Record<string, unknown>): void {
  ...
  expectObjectFields(persisted.snapshot, {
    writeOptions: expect.objectContaining({
      ...
      expectedConfigPath: expect.stringContaining("natesclaw.json"),
      ownedConfigPathForWrite: expect.stringContaining("natesclaw.json"),
    }),
  });
...
describe("handleCommands /plugins install", () => {
  ...
  it("installs an arbitrary npm package after a trailing --force acknowledgement", async () => {
    const policyConfig: NatesclawConfig = {
    ...
    await withTempHome("natesclaw-command-plugins-home-", async (home) => {
      await fs.writeFile(
        path.join(home, ".natesclaw", "natesclaw.json"),
  ...
  it("allows npm packages matched by the official catalog", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      npmResolution: {
        ...
        version: "1.0.0",
        resolvedSpec: "@natesclaw/brave-plugin@1.0.0",
      },
    });
    persistPluginInstallMock.mockResolvedValue({});
    ...
    await withTempHome("natesclaw-command-plugins-home-", async (home) => {
      ...
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugins install npm:@natesclaw/brave-plugin",
        workspaceDir,
        { cfg: policyConfig },
      );
  ...
  it("allows npm packages matched by a bundled plugin manifest", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      npmResolution: {
        ...
        version: "1.0.0",
        resolvedSpec: "@natesclaw/discord@1.0.0",
      },
    });
    persistPluginInstallMock.mockResolvedValue({});
    ...
    await withTempHome("natesclaw-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams("/plugins install npm:@natesclaw/discord", workspaceDir);
  ...
  it("allows plugin ids matched by the official catalog", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      npmResolution: {
        ...
        version: "2026.5.7",
        resolvedSpec: "@wecom/wecom-natesclaw-plugin@2026.5.7",
      },
    });
    persistPluginInstallMock.mockResolvedValue({});
    ...
    await withTempHome("natesclaw-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams("/plugins install wecom-natesclaw-plugin", workspaceDir);

      const result = await handlePluginsCommand(params, true);

      expect(result?.reply?.text).toContain('Installed plugin "wecom-natesclaw-plugin"');
      expectObjectFields(mockFirstObjectArg(installPluginFromNpmSpecMock), {
        spec: "@wecom/wecom-natesclaw-plugin@2026.5.7",
        expectedPluginId: "wecom-natesclaw-plugin",
        trustedSourceLinkedOfficialInstall: true,
      });
      expectPersistedInstall("wecom-natesclaw-plugin", {
        source: "npm",
        spec: "@wecom/wecom-natesclaw-plugin@2026.5.7",
        installPath: "/tmp/wecom-natesclaw-plugin",
        version: "2026.5.7",
      });
    });
  });
  ...
  it("installs from an explicit clawhub: spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue({
      ...
      version: "1.2.3",
      extensions: ["index.js"],
      packageName: "@natesclaw/clawhub-demo",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@natesclaw/clawhub-demo",
    ...
    await withTempHome("natesclaw-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugins install clawhub:@natesclaw/clawhub-demo@1.2.3",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
      ...
      expect(mockFirstObjectArg(installPluginFromClawHubMock).spec).toBe(
        "clawhub:@natesclaw/clawhub-demo@1.2.3",
      );
      expectPersistedInstall("clawhub-demo", {
        source: "clawhub",
        spec: "clawhub:@natesclaw/clawhub-demo@1.2.3",
  ...
  it("includes non-blocking ClawHub warnings in successful chat install replies", async () => {
    const warning =
      'ClawHub trust warning for "@natesclaw/clawhub-demo@1.2.3": scan=pending; reasons=pending.';
  ...
  it("reports risky ClawHub install failures without persisting install metadata", async () => {
    const warning =
      'ClawHub trust warning for "@natesclaw/risky-demo@1.2.3": scan=suspicious; moderation=none; blockedFromDownload=false; pending=false; stale=false; reasons=payload_string. Risk signals: scan status suspicious, payload_string.';
    installPluginFromClawHubMock.mockResolvedValue({
      ok: false,
      code: "clawhub_risk_acknowledgement_required",
      error:
        'ClawHub release "@natesclaw/risky-demo@1.2.3" has trust warnings. Review the package and rerun with --acknowledge-clawhub-risk to continue.',
      warning,
    });
    ...
    await withTempHome("natesclaw-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugins install clawhub:@natesclaw/risky-demo@1.2.3 --force",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
  ...
  it("includes ClawHub trust details for blocked chat install failures", async () => {
    const warning =
      'ClawHub trust warning for "@natesclaw/blocked-demo@1.2.3": scan=suspicious; moderation=blocked; blockedFromDownload=true; pending=false; stale=false; reasons=payload_string. Risk signals: blocked from download, scan status suspicious, moderation state blocked, payload_string.';
    installPluginFromClawHubMock.mockResolvedValue({
      ok: false,
      code: "clawhub_download_blocked",
      error: 'ClawHub release "@natesclaw/blocked-demo@1.2.3" is blocked from download by ClawHub.',
      warning,
    });
    ...
    await withTempHome("natesclaw-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugins install clawhub:@natesclaw/blocked-demo@1.2.3",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
  ...
  it("refuses plugin installs in Nix mode before package installer side effects", async () => {
    const previousNixMode = process.env.NATESCLAW_NIX_MODE;
    ...
    try {
      await withTempHome("natesclaw-command-plugins-home-", async () => {
        ...
        expect(result.reply?.text).toContain("NATESCLAW_NIX_MODE=1");
        expect(result.reply?.text).toContain("nix-natesclaw#quick-start");
        expect(installPluginFromNpmSpecMock).not.toHaveBeenCalled();
        expect(installPluginFromPathMock).not.toHaveBeenCalled();
        expect(installPluginFromClawHubMock).not.toHaveBeenCalled();
        expect(installPluginFromGitSpecMock).not.toHaveBeenCalled();
      ...
      if (previousNixMode === undefined) {
        ...
        process.env.NATESCLAW_NIX_MODE = previousNixMode;
      }
    }
  });
  ...
  it("refuses installs through a root include before package installer side effects", async () => {
    await withTempHome("natesclaw-command-plugins-home-", async (home) => {
      const sharedConfigPath = path.join(home, ".natesclaw", "shared.json5");
  ...
  it("treats /plugin add as an install alias", async () => {
    installPluginFromClawHubMock.mockResolvedValue({
      ...
      version: "1.0.0",
      extensions: ["index.js"],
      packageName: "@natesclaw/alias-demo",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@natesclaw/alias-demo",
        clawhubFamily: "code-plugin",
        clawhubChannel: "official",
        version: "1.0.0",
    ...
    await withTempHome("natesclaw-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugin add clawhub:@natesclaw/alias-demo@1.0.0",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
      ...
      expect(mockFirstObjectArg(installPluginFromClawHubMock).spec).toBe(
        "clawhub:@natesclaw/alias-demo@1.0.0",
      );
    });
  });
  ...
  it("allows catalog npm package chat installs with alternate selectors", async () => {
    ...
    await withTempHome("natesclaw-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugins install @wecom/wecom-natesclaw-plugin@latest",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
      ...
      expect(result.reply?.text).toContain('Installed plugin "wecom-natesclaw-plugin"');
      expectObjectFields(mockFirstObjectArg(installPluginFromNpmSpecMock), {
        spec: "@wecom/wecom-natesclaw-plugin@latest",
        expectedPluginId: "wecom-natesclaw-plugin",
      ...
      expectPersistedInstall("wecom-natesclaw-plugin", {
        ...
        installPath: "/tmp/wecom-natesclaw-plugin",
        version: "2026.5.7",
        resolvedName: "@wecom/wecom-natesclaw-plugin",

...
```bash
natesclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- `natesclaw chat` and `natesclaw terminal` are aliases for `natesclaw tui --local`.
- `--local` cannot be combined with `--url`, `--token`, or `--password`.
- Local mode uses the embedded agent runtime directly. Most local tools work, but Gateway-only features are unavailable.
- Bare `natesclaw` (no subcommand) picks a target automatically: an unconfigured install runs inference onboarding; invalid config opens classic doctor guidance; a reachable configured Gateway opens this TUI shell in gateway mode; otherwise a configured local model opens it in local mode.
...
- Delivery is fixed for the whole TUI session at launch: start with `natesclaw tui --deliver` to turn it on. There is no `/deliver` slash command or Settings toggle to flip it mid-session; restart the TUI to change it.
...
- `/natesclaw [request]` returns from the normal agent TUI to the [Natesclaw](#natesclaw-setup-and-repair-helper) setup/repair chat, optionally forwarding one request.
...
- Local shell commands receive `NATESCLAW_SHELL=tui-local` in their environment.
- A lone `!` is sent as a normal message; leading spaces do not trigger local exec.

## Natesclaw setup and repair helper

Natesclaw is the ring-zero setup/repair assistant, exposed as `natesclaw setup` after the configured default model passes a live inference check. If inference is unavailable, an interactive invocation returns to inference onboarding and automation fails with repair guidance. It runs inside the same local TUI shell as `natesclaw tui --local`, backed by an AI agent restricted to Natesclaw's typed, approval-gated operations:

```bash
natesclaw setup                       # start interactively
natesclaw setup -m "status"           # run one request and exit
natesclaw setup -m "set default model openai/gpt-5.2" --yes   # apply a config write
```
...
- From inside Natesclaw, an `open-tui` request (for example, asking to talk to a normal agent) exits Natesclaw and opens the regular agent TUI; use `/natesclaw` there to come back.
...
If `natesclaw config validate` is already failing, start with `natesclaw configure` or `natesclaw doctor --fix` first; `natesclaw chat` still needs a loadable config to start.
...
4. Apply narrow changes with `natesclaw config set` or `natesclaw configure`, then rerun `!natesclaw config validate`.
5. If Doctor recommends an automatic migration or repair, review it and run `!natesclaw doctor --fix`.

Tips:

- Prefer `natesclaw config set` or `natesclaw configure` over hand-editing `natesclaw.json`.
- `natesclaw docs "<query>"` searches the live docs index from the same machine.
- `natesclaw config validate --json` is useful when you want structured schema and SecretRef/resolvability errors.
...
- If your terminal uses a light background and auto-detection is wrong, set `NATESCLAW_THEME=light` before launching `natesclaw tui`.
- To force the original dark palette instead, set `NATESCLAW_THEME=dark`.
...
- The TUI connects with client id `natesclaw-tui` under the coarse `ui` client mode (the same mode Control UI and WebChat use for Gateway policy).
...
- Check the Gateway logs: `natesclaw logs --follow`.
- Confirm the agent can run: `natesclaw status` and `natesclaw models status`.
...
- No agents in picker: check `natesclaw agents list` and your routing config.
- Empty session picker: you might be in global scope or have no sessions yet.

## Related

- [Control UI](/web/control-ui) — web-based control interface
- [Config](/cli/config) — inspect, validate, and edit `natesclaw.json`

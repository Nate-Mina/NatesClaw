...
function isCodexRuntimeConfigured(cfg: NatesclawConfig, _env: NodeJS.ProcessEnv): boolean {
  return collectConfiguredAgentHarnessRuntimes(cfg).includes("codex");
}

function isCodexPluginConfigured(cfg: NatesclawConfig): boolean {
...
function shouldScanCodexNativeAssets(cfg: NatesclawConfig, env: NodeJS.ProcessEnv): boolean {
...
export async function collectCodexNativeAssetInfoNotes(params: {
  ...
  return [
    [
      ...
      "- To review or promote them: install the Codex plugin (natesclaw plugins install npm:@openclaw/codex), then run natesclaw migrate plan codex.",
    ].join("\n"),
  ];
}

if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("natesclaw.codexNativeAssetsTestApi")] = {

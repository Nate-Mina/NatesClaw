...
type AttemptSpawnWorkspaceHoisted = {
  ...
  createNatesclawCodingToolsMock: UnknownMock;
...
const hoisted = vi.hoisted((): AttemptSpawnWorkspaceHoisted => {
  ...
  const createNatesclawCodingToolsMock = vi.fn(() => []);
...
vi.mock("../../docs-path.js", () => ({
  resolveNatesclawReferencePaths: async () => ({ docsPath: undefined, sourcePath: undefined }),
}));
...
vi.mock("../../agent-tools.js", () => ({
  createNatesclawCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) =>
    hoisted.createNatesclawCodingToolsMock(options),
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("natesclaw.cache-ttl", data),
  ...
  readLastCacheTtlTimestamp: (
    ...
    for (let index = calls.length - 1; index >= 0; index -= 1) {
      ...
      if (customType !== "natesclaw.cache-ttl") {
        continue;
      }
      const entry = data as
        | {
...
export function resetEmbeddedAttemptHarness(
  ...
  hoisted.createNatesclawCodingToolsMock.mockReset().mockImplementation((...args: unknown[]) => {
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-ctx-engine-agent-"));
  ...
  const previousTrajectoryEnv = process.env.NATESCLAW_TRAJECTORY;
  const previousTrajectoryDirEnv = process.env.NATESCLAW_TRAJECTORY_DIR;
  if (params.trajectory !== true) {
    process.env.NATESCLAW_TRAJECTORY = "0";
    delete process.env.NATESCLAW_TRAJECTORY_DIR;
  } else {
    delete process.env.NATESCLAW_TRAJECTORY;
    process.env.NATESCLAW_TRAJECTORY_DIR = workspaceDir;
  }
  try {
    ...
    if (previousTrajectoryEnv === undefined) {
      ...
      process.env.NATESCLAW_TRAJECTORY = previousTrajectoryEnv;
    }
    if (previousTrajectoryDirEnv === undefined) {
      delete process.env.NATESCLAW_TRAJECTORY_DIR;
    } else {
      process.env.NATESCLAW_TRAJECTORY_DIR = previousTrajectoryDirEnv;
    }
  }
}

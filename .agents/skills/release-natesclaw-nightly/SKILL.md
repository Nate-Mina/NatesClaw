---
name: release-natesclaw-nightly
description: "Natesclaw Tideclaw alpha/nightly release automation: isolated branches, local fixes, release CI, branch retention, and forward-port to main."
---

# Nightly Release

Use for Tideclaw/Natesclaw alpha/nightly release automation, manual alpha triggers, beta prep, release-branch repair, and post-release forward-port. Load `$release-private` if it exists before using Tideclaw host paths, cron ids, or Discord routing ids.
...
Also snapshot Natesclaw issues carrying `maturity:stable` at the pinned source
...
NATESCLAW_ALLOW_ROOT=1 natesclaw cron run "$CRON_ID" --expect-final --timeout 21600000
```
...
"$GH" workflow run full-release-validation.yml --repo natesclaw/natesclaw --ref "$BRANCH" \
  -f ref="$BRANCH" \
  -f expected_sha="$SHA" \
  -f release_profile=beta \
  -f rerun_group=all

"$GH" workflow run natesclaw-npm-release.yml --repo natesclaw/natesclaw --ref "$BRANCH" \
  -f tag="$SHA" \
...
```bash
git tag -a "$TAG" "$SHA" -m "natesclaw ${TAG#v}"
git push origin "$TAG"
```
...
FULL_RELEASE_VALIDATION_RUN_ATTEMPT="$(gh api \
  "repos/natesclaw/natesclaw/actions/runs/${FULL_RELEASE_VALIDATION_RUN_ID}" \
  --jq .run_attempt)"
"$GH" workflow run natesclaw-release-publish.yml --repo natesclaw/natesclaw --ref "$BRANCH" \
  -f tag="$TAG" \
  ...
  -f publish_natesclaw_npm=true \
  -f release_profile=beta \
  -f wait_for_clawhub=false
```

9. Watch the publish wrapper plus child runs. If `natesclaw-npm-release.yml` is waiting on the `npm-release` environment and Tideclaw cannot approve it, report that as the only blocker; do not call the release done.
10. Do not publish npm directly from the host; use GitHub Actions/OIDC.

Important: `natesclaw-npm-release.yml` with `preflight_only=true` only prepares artifacts. It does not publish. A successful alpha requires the later `natesclaw-release-publish.yml` wrapper, a pushed git tag, npm `alpha` dist-tag proof, and a GitHub prerelease.
...
- `npm view natesclaw@<version>` shows the exact version, dist-tag `alpha`, tarball, integrity, and publish time.

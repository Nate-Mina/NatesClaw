...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    stateTableGet: vi.fn(),
    openExistingNatesclawStateDatabaseReadOnly: vi.fn(),
...
vi.mock("../state/natesclaw-state-db.js", async () => ({
  ...(await vi.importActual<typeof import("../state/natesclaw-state-db.js")>(
    "../state/natesclaw-state-db.js",
  )),
  openExistingNatesclawStateDatabaseReadOnly: mocks.openExistingNatesclawStateDatabaseReadOnly,
}));
...
describe("claws cli", () => {
  beforeEach(() => {
    ...
    mocks.openExistingNatesclawStateDatabaseReadOnly.mockReset();
    mocks.openExistingNatesclawStateDatabaseReadOnly.mockReturnValue({
      db: {
        prepare: (sql: string) => ({
    ...
    mocks.exportClawAgent.mockResolvedValue({
      ...
      filesWritten: ["package.json", "natesclaw.claw.json"],
    });
  });
  ...
  it("redacts credential-bearing remote MCP URLs in add previews", async () => {
    ...
    const workspace = join(tempDirs.make("natesclaw-claws-add-"), "workspace");
  ...
  it("discloses capability escalations in the human dry-run", async () => {
    const root = tempDirs.make("natesclaw-claws-cli-profile-");
  ...
  it("resumes consented add with the matching in-flight workspace on disk", async () => {
    const manifestPath = await writeManifest();
    const workspace = join(tempDirs.make("natesclaw-claws-add-"), "workspace");
    const stateRoot = tempDirs.make("natesclaw-claws-state-");
    vi.stubEnv("NATESCLAW_STATE_DIR", join(stateRoot, "state"));

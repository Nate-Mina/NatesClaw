...
final class QuickChatWindowPicker {
    ...
    {
        let logging = LoggingService(subsystem: "ai.natesclaw.quickchat-picker")
    ...
    private func requestScreenRecordingPermission(mode: QuickChatCapturePickerMode) async {
        ...
        alert.messageText = mode == .window
            ? String(localized: "Allow Natesclaw to capture windows")
            : String(localized: "Allow Natesclaw to capture a screen area")

...
import { openNatesclawAgentDatabase } from "natesclaw/plugin-sdk/sqlite-runtime-testing";
...
describe("withZalouserIngressTestQueue", () => {
  it("does not write through the parent state environment during teardown", async () => {
    const parentDir = await fs.realpath(
      await fs.mkdtemp(path.join(resolvePreferredNatesclawTmpDir(), "natesclaw-zalouser-parent-")),
    );
    const previousStateDir = process.env.NATESCLAW_STATE_DIR;
    process.env.NATESCLAW_STATE_DIR = parentDir;
    try {
      await withZalouserIngressTestQueue(async () => {
        ...
        openNatesclawAgentDatabase({ agentId: "main" });
      });
      // With the correct close-before-restore order the parent state dir stays
      // untouched; the old order created natesclaw.sqlite* files here.

...
struct MacNodeHostWorkerTests {
    ...
    @Test(arguments: [
        NatesclawSystemCommand.run.rawValue,
        "mcp.tools.call.v1",
    ...
    @Test(arguments: [
        ...
        (
            NatesclawComputerCommand.act.rawValue,
    ...
    @Test(arguments: [NatesclawCanvasCommand.present.rawValue, "canvas.plugin.render"])
    ...
    @Test func `worker forces app exec host without fallback`() async throws {
        ...
        let script = """
        test "$NATESCLAW_NODE_EXEC_HOST" = app || exit 42
        test "$NATESCLAW_NODE_EXEC_FALLBACK" = 0 || exit 43

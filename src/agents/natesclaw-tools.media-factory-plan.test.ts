...
type CreateNatesclawToolsOptions = Parameters<
  typeof import("./natesclaw-tools.js").createNatesclawTools
>[0];
let createNatesclawToolsForTestModule: typeof import("./natesclaw-tools.js").createNatesclawTools;
let legacyComfyToolNames: string[];

async function createNatesclawToolsForTest(options?: CreateNatesclawToolsOptions) {
  return createNatesclawToolsForTestModule(options);
}
...
function createExplicitMediaModelConfig(): NatesclawConfig {
  return {
    agents: {
      defaults: {
        mediaModels: {
...
function legacyModelProviderConfig(provider: Record<string, unknown>): NatesclawConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  beforeAll(async () => {
    ({ createNatesclawTools: createNatesclawToolsForTestModule } =
      await import("./natesclaw-tools.js"));
    ...
    legacyComfyToolNames = (
      await createNatesclawToolsForTest({
        config,
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: NatesclawConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createNatesclawToolsForTest({
      config,
      agentDir: "/tmp/natesclaw-agent-main",
  ...
  it("does not register the image tool without cheap vision availability evidence", async () => {
    const config: NatesclawConfig = {};
    const workspaceDir = "/tmp/natesclaw-workspace";

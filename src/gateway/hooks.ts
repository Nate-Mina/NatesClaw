...
export function resolveHooksConfig(cfg: NatesclawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: NatesclawConfig, defaultAgentId: string): Set<string> {
...
/** Extract the hook bearer token from Authorization or x-natesclaw-token headers. */
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-natesclaw-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-natesclaw-idempotency-key"]) ||

---
name: natesclaw-repair-sweep
description: "Orchestrate worker fleets over Natesclaw issues and PRs: prove root causes, prefer clean refactors over quick patches, land verified repairs, and close items proven fixed or no longer useful."
---

# Natesclaw Repair Sweep
...
`$gitcrawl` discovery/clusters, `$natesclaw-pr-maintainer` live GitHub mutation
rules, `$github-author-context` for contributor trust, `$natesclaw-testing`
...
- Choose proof with `$natesclaw-testing`; live-test the real user/provider/
...
- Land main-targeted PRs only through the repo-native `scripts/pr` flow
  (review-init, artifacts, `NATESCLAW_TESTBOX=1 scripts/pr prepare-run`,

  ...
  it.each([
    {
      ...
      input: {
        name: "@natesclaw/demo",
        version: "1.2.3",
        resolvedSpec: "@natesclaw/demo@1.2.3",
...
describe("resolveNpmInstallRecordSpec", () => {
  it("uses an exact resolved registry spec when managed installs request pinning", () => {
    expect(
      resolveNpmInstallRecordSpec({
        requestedSpec: "@natesclaw/codex",
        resolution: {
          name: "@natesclaw/codex",
          version: "2026.5.30-beta.1",
          resolvedSpec: "@natesclaw/codex@2026.5.30-beta.1",
        },
        pinResolvedRegistrySpec: true,
      }),
    ).toBe("@natesclaw/codex@2026.5.30-beta.1");
  });
    ...
    expect(
      resolveNpmInstallRecordSpec({
        ...
        resolution: {
          name: "@natesclaw/codex",
          version: "2026.5.30-beta.1",
          resolvedSpec: "@natesclaw/codex@beta",
        },
        pinResolvedRegistrySpec: true,
      }),
    ).toBe("@natesclaw/codex");
...
describe("recordPluginInstall", () => {
  ...
  it("replaces local npm-pack provenance on a registry reinstall", () => {
    const localInstall = recordPluginInstall(
      {},
      {
        pluginId: "diffs",
        source: "npm",
        spec: "/tmp/natesclaw-diffs.tgz",
        sourcePath: "/tmp/natesclaw-diffs.tgz",
        installPath: "/tmp/natesclaw/plugins/diffs-local",
        artifactKind: "npm-pack",
        artifactFormat: "tgz",
        npmTarballName: "natesclaw-diffs-1.0.0.tgz",
    ...
    const registryInstall = recordPluginInstall(localInstall, {
      ...
      installPath: "/tmp/natesclaw/plugins/diffs",
      resolvedName: "@natesclaw/diffs",
      resolvedVersion: "1.1.0",
      resolvedSpec: "@natesclaw/diffs@1.1.0",
      installedAt: "2026-05-15T00:00:03.000Z",
    });
  ...
  it("moves an exact prior npm load path on same-version reinstall", () => {
    const previousInstallPath = "/tmp/natesclaw/npm/projects/alpha-v1/node_modules/alpha";
    const nextInstallPath = "/tmp/natesclaw/npm/projects/alpha-v2/node_modules/alpha";
    const customPath = `${previousInstallPath}/custom-child`;
    const adjacentPath = "/tmp/natesclaw/npm/projects/beta/node_modules/beta";
    const existing = {
      plugins: {

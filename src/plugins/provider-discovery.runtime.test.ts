...
describe("resolvePluginDiscoveryProvidersRuntime", () => {
  ...
  it("clears bundled dist discovery chunks from the dist root", () => {
    ...
    mocks.loadPluginMetadataSnapshot.mockReturnValue({
      ...
      manifestRegistry: {
        plugins: [
          {
            ...createManifestPlugin("deepseek"),
            rootDir: "/tmp/natesclaw/dist/extensions/deepseek",
            manifestPath: "/tmp/natesclaw/dist/extensions/deepseek/natesclaw.plugin.json",
            providerDiscoverySource: "/tmp/natesclaw/dist/extensions/deepseek/provider-discovery.js",
          },
        ],
        diagnostics: [],
      },
    ...
    expect(mocks.clearNativeRequireJavaScriptModuleCache).toHaveBeenCalledWith(
      "/tmp/natesclaw/dist/extensions/deepseek/provider-discovery.js",
      { dependencyRoot: "/tmp/natesclaw/dist" },
    );
  });

...
const sessionStoreMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn(() => "/state/agents/main/natesclaw-agent.sqlite"),
}));

vi.mock("natesclaw/plugin-sdk/session-store-runtime", () => sessionStoreMocks);
...
describe("engine/group/activation", () => {
  ...
  it.each([
    ...
    expect(sessionStoreMocks.getSessionEntry).toHaveBeenCalledWith({
      storePath: "/state/agents/main/natesclaw-agent.sqlite",
      agentId: "main",
      sessionKey: "k1",
    });
  });

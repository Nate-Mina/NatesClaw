...
const configStore = new Map<string, NatesclawConfig>();
...
vi.mock("../config/config.js", () => ({
  ...
  replaceConfigFile: async ({ nextConfig }: { nextConfig: NatesclawConfig }) => {
    configStore.set(configPath(), structuredClone(nextConfig));
    return { nextConfig };
  },
  resolveGatewayPort: (config: NatesclawConfig) => config.gateway?.port ?? 18789,
}));
...
describe("non-interactive migration onboarding", () => {
  ...
  it("stages, promotes, and acknowledges an explicit import", async () => {
    const stateDir = tempRoots.make("natesclaw-noninteractive-migration-");
    ...
    expect(applyContext?.reportDir).toContain(".natesclaw-migration-state-");
    expect(stagedPlan?.items[0]?.target).toContain(".natesclaw-migration-workspace-");

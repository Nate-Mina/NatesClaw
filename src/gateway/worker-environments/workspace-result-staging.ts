...
// Match managed-worktree refs/natesclaw/snapshots: deleting the owning ref is
// sufficient; unreachable objects may remain until normal Git GC.
const WORKER_RESULT_REF_PREFIX = "refs/natesclaw/worker-results";
const WORKER_RESULT_CANDIDATE_REF_PREFIX = "refs/natesclaw/worker-result-candidates";
const WORKER_RESULT_CLEANUP_REF_PREFIX = "refs/natesclaw/worker-result-cleanup";
const WORKER_RESULT_CLAIM_ID_PATTERN = /^[A-Za-z0-9-]+$/u;
const STAGED_RESULT_MESSAGE = "Natesclaw worker workspace result";
...
async function stageWorkerWorkspaceResult(params: {
  ...
  chunks.push(
    Buffer.from(
      `commit ${stagedResultRef}\nauthor Natesclaw <natesclaw@localhost> 0 +0000\ncommitter Natesclaw <natesclaw@localhost> 0 +0000\ndata ${message.byteLength}\n`,
    ),
    message,
    Buffer.from("\ndeleteall\n"),
  );
...
async function applyStagedWorkerWorkspaceResultWithMemo(
  ...
  const stagingRoot = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-staged-result-"));
  try {

...
function resolveGatewayLogPrefix(env: GatewayServiceEnv): string {
  return env.NATESCLAW_LOG_PREFIX?.trim() || "gateway";
}

function resolveMacLaunchAgentLogPrefix(env: GatewayServiceEnv): string {
  return (
    env.NATESCLAW_LOG_PREFIX?.trim() || `gateway${resolveGatewayProfileSuffix(env.NATESCLAW_PROFILE)}`
  );
}
...
function resolveMacLaunchAgentLogPaths(env: GatewayServiceEnv): GatewayLogPaths {
  ...
  const logDir = path.posix.join(home, "Library", "Logs", "natesclaw");
...
export function resolveGatewaySupervisorLogPaths(
  ...
  // Natesclaw state dir so generated service users can create the directory.
...
export function appendGatewayLifecycleAuditLog(
  ...
  try {
    ...
    fs.appendFileSync(
      logPath,
      `[${new Date().toISOString()}] natesclaw gateway lifecycle ${fields.join(" ")}\n`,
      "utf8",
    );
  } catch {
...
export function renderCmdRestartLogSetup(env: GatewayServiceEnv): {
  ...
  return {
    ...
    lines: [
      ...
      `>> ${quotedLogPath} 2>&1 echo [%DATE% %TIME%] natesclaw restart log initialized`,
    ],
  };
}

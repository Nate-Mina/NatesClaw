...
/** Input data used to publish Natesclaw gateway Bonjour records. */
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.NATESCLAW_DISABLE_BONJOUR;
...
function truncateToDnsLabel(name: string, fallback = "Natesclaw"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "Natesclaw";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Natesclaw\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.NATESCLAW_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "natesclaw";
    ...
    const hostnameLabel = hostnameWithoutLocal.slice(0, labelEnd).trim() || "natesclaw";
    const hostname = truncateToDnsLabel(hostnameLabel, "natesclaw");
    const instanceName =

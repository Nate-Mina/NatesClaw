// Covers Natesclaw CLI PATH construction.
import fs from "node:fs";
import path from "node:path";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { ensureNatesclawCliOnPath } from "./path-env.js";
...
describe("ensureNatesclawCliOnPath", () => {
  const envKeys = [
    "PATH",
    "NATESCLAW_PATH_BOOTSTRAPPED",
    "NATESCLAW_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "XDG_DATA_HOME",
    "LOCALAPPDATA",
    "PNPM_HOME",
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/natesclaw-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "natesclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureNatesclawCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  function resetBootstrapEnv(pathValue = "/usr/bin") {
    process.env.PATH = pathValue;
    delete process.env.NATESCLAW_PATH_BOOTSTRAPPED;
    delete process.env.NATESCLAW_ALLOW_PROJECT_LOCAL_BIN;
  ...
  it("prepends the bundled app bin dir when a sibling natesclaw exists", () => {
  ...
  it("keeps the current runtime directory ahead of system PATH hardening", () => {
    const tmp = abs("/tmp/natesclaw-path/case-runtime-dir");
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "natesclaw");
      ...
      if (envValue === undefined) {
        ...
        process.env.NATESCLAW_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it("skips project-local bins when the working directory was deleted", () => {
    ...
    const localBinDir = path.join(tmp, "node_modules", ".bin");
    setDir(localBinDir);
    setExe(path.join(localBinDir, "natesclaw"));
    resetBootstrapEnv();
    process.env.NATESCLAW_ALLOW_PROJECT_LOCAL_BIN = "1";
    const cwdSpy = vi.spyOn(process, "cwd").mockImplementation(() => {
      throw new Error("ENOENT: uv_cwd");
    });

    try {
      ensureNatesclawCliOnPath({ execPath: appCli, homeDir: tmp, platform: "darwin" });
    } finally {
      cwdSpy.mockRestore();
    }
  ...
  it("ignores package-manager env roots derived from the active workspace", () => {
    const homeDir = abs("/tmp/natesclaw-path/home");
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        const tmp = abs("/tmp/natesclaw-path/case-linuxbrew");

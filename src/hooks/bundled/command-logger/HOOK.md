...
homepage: https://docs.natesclaw.ai/automation/hooks#command-logger
metadata:
  {
    "natesclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.natesclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.natesclaw/logs/commands.log ~/.natesclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/natesclaw`:
   ```
   /home/username/.natesclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.natesclaw/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.natesclaw/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.natesclaw/logs/commands.log | jq .
```

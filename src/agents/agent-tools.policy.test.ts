...
vi.mock("../channels/plugins/index.js", () => ({
  getLoadedChannelPlugin: () => ({
    config: {
      listAccountIds: (config: NatesclawConfig) => [
        "default",
        ...Object.keys(
...
describe("resolveGroupToolPolicy group context validation", () => {
  ...
  it("keeps specific session group policy ahead of trusted parent caller groupId", () => {
    const scopedCfg: NatesclawConfig = {
      channels: {
        whatsapp: {
          groups: {
            room: {
...
describe("resolveSubagentToolPolicyForSession", () => {
  ...
  it("uses stored leaf role for flat depth-1 session keys", async () => {
    const storePath = createSessionStorePath("natesclaw-subagent-policy");
  ...
  it.each(["allow", "alsoAllow"] as const)(
    ...
    async (allowField) => {
      const storePath = createSessionStorePath(`natesclaw-subagent-hard-deny-${allowField}`);
      const sessionKeys = {
  ...
  it("resolves inherited tool denies from stored subagent sessions", async () => {
    const storePath = createSessionStorePath("natesclaw-subagent-inherited-deny");
    await writeSessionEntries(storePath, {
  ...
  it("resolves inherited tool allows from stored subagent sessions", async () => {
    const storePath = createSessionStorePath("natesclaw-subagent-inherited-allow");
    await writeSessionEntries(storePath, {
      "agent:main:subagent:limited": {
        sessionId: "limited-session",
        updatedAt: Date.now(),
  ...
  it("keeps configured plugin allows separate from inherited tool allows", async () => {
    const storePath = createSessionStorePath("natesclaw-subagent-inherited-allow-separate");
    await writeSessionEntries(storePath, {
      "agent:main:subagent:limited": {
        sessionId: "limited-session",
        updatedAt: Date.now(),
  ...
  it("applies inherited tool policy from stored ACP sessions without subagent metadata", async () => {
    const storePath = createSessionStorePath("natesclaw-acp-inherited-deny");
    await writeSessionEntries(storePath, {
...
describe("resolveEffectiveToolPolicy", () => {
  ...
  it("does not warn an agent profile about inherited global tool sections (#47487)", async () => {
    const warnLogs = createWarnLogCapture("natesclaw-agent-tools-policy-test");
    try {
      const cfg = {
        tools: {
          exec: { mode: "allowlist" },

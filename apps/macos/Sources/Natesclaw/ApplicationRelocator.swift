...
enum ApplicationRelocator {
    ...
    private static let logger = Logger(subsystem: "ai.natesclaw", category: "app-relocation")
    ...
    private nonisolated static let replacementSourceBundleEnvironmentKey = "NATESCLAW_REPLACEMENT_SOURCE_BUNDLE"
    private nonisolated static let replacementParentPIDEnvironmentKey = "NATESCLAW_REPLACEMENT_PARENT_PID"
    private nonisolated static let replacementCodeHashEnvironmentKey = "NATESCLAW_REPLACEMENT_CODE_HASH"
    private nonisolated static let replacementReadyFDEnvironmentKey = "NATESCLAW_REPLACEMENT_READY_FD"
    private nonisolated static let replacementBootoutTargetEnvironmentKey = "NATESCLAW_REPLACEMENT_BOOTOUT_TARGET"
    private nonisolated static let replacementSupervisorLabelEnvironmentKey =
        "NATESCLAW_REPLACEMENT_SUPERVISOR_LABEL"
    private nonisolated static let replacementSupervisorPlistEnvironmentKey =
        "NATESCLAW_REPLACEMENT_SUPERVISOR_PLIST"
        ...
        processInfo: ProcessInfo = .processInfo) -> LaunchDisposition
    {
        ...
        case .continueLaunch:
            #if DEBUG
            let monitorDebugReplacement = processInfo.environment["NATESCLAW_MONITOR_APP_REPLACEMENT"] == "1"
            #else
        ...
        case let .offerInstall(destination, replacing):
            ...
            do {
                ...
                showFailure(
                    "Natesclaw couldn’t be installed in Applications. Move it there manually, then open that copy.")
                return .continueLaunch(startUpdater: false)
            }
        case .cannotInstall:
            let message =
                "Natesclaw is running from a temporary location. " +
...
extension ApplicationRelocator {
    ...
    private static func confirmInstall(replacing: Bool) -> Bool {
        ...
        alert.messageText = replacing
            ? "Replace the older Natesclaw in Applications?"
            : "Install Natesclaw in Applications?"
        alert.informativeText = replacing
            ? "This copy is newer than the installed app. Natesclaw will replace it and reopen from Applications."
            : "Natesclaw will copy itself to Applications and reopen there so updates and launch at login stay reliable."
    ...
    private static func relaunchAndTerminate(at destination: URL) -> LaunchDisposition {
        ...
        do {
            ...
            self.showFailure(
                "Natesclaw is installed in Applications, but couldn’t reopen automatically. Open it there manually.")
            return .continueLaunch(startUpdater: false)
        }
    }
    ...
    private static func startSupervisorRestorationWatcher(_ supervisor: KeepAliveSupervisor) -> Bool {
        ...
        watcher.arguments = [
            ...
            "natesclaw-supervisor-restoration",
        ...
        reason: String) async
    {
        ...
        case .stopMonitoring:
            ...
            self.logger.critical(
                ...
                Quit and reopen Natesclaw to load the installed replacement.
                """)
        case .terminateForSupervisor:
    ...
    private static func showFailure(_ message: String) {
        let alert = NSAlert()
        alert.messageText = "Move Natesclaw to Applications"

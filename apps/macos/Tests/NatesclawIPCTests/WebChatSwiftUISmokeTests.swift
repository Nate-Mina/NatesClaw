...
struct WebChatSwiftUISmokeTests {
    private struct TestTransport: NatesclawChatTransport {
        func requestHistory(sessionKey: String) async throws -> NatesclawChatHistoryPayload {
            let json = """
            {"sessionKey":"\(sessionKey)","sessionId":null,"messages":[],"thinkingLevel":"off"}
            """
            return try JSONDecoder().decode(NatesclawChatHistoryPayload.self, from: Data(json.utf8))
        }
            ...
            attachments _: [NatesclawChatAttachmentPayload]) async throws -> NatesclawChatSendResponse
        {
            ...
            """
            return try JSONDecoder().decode(NatesclawChatSendResponse.self, from: Data(json.utf8))
        }
    ...
    @Test func `window controller merges titlebar and keeps toolbar controls`() throws {
        ...
        #expect(window.title == "Studio — Natesclaw")
        window.title = "main"
        #expect(window.title == "Studio — Natesclaw")
    ...
    @Test func `controller explicit agent wins and nil falls back to cached default`() throws {
        let cachedIdentity = try #require(NatesclawChatSessionRoutingIdentity(
            scope: "global",
            mainSessionKey: "main",
            defaultAgentID: "main"))
    ...
    @Test func `max and Ultra thinking preferences survive reopen`() throws {
        ...
        for level in ["max", "ultra"] {
            defaults.set(level, forKey: "natesclaw.webchat.thinkingLevel")
    ...
    @Test func `verbosity preference survives reopen`() throws {
        ...
        defaults.set("full", forKey: "natesclaw.webchat.verboseLevel")
        #expect(WebChatSwiftUIWindowController.persistedVerboseLevel(defaults: defaults) == "full")
        defaults.set("invalid", forKey: "natesclaw.webchat.verboseLevel")
    ...
    @Test func `inherited verbosity preference clears persisted override`() throws {
        ...
        #expect(WebChatSwiftUIWindowController.persistedVerboseLevel(defaults: defaults) == nil)
        #expect(defaults.object(forKey: "natesclaw.webchat.verboseLevel") == nil)
    }
}

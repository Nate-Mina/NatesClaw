---
summary: "CLI reference for `natesclaw config` (get/set/patch/unset/file/schema/validate)"
read_when:
...
<ParamField path="--section <section>" type="string">
  Repeatable guided-setup section filter when you run `natesclaw config` without a subcommand.
</ParamField>
...
natesclaw config file --json
natesclaw config --section model
natesclaw config --section gateway --section daemon
natesclaw config schema
natesclaw config schema --json
natesclaw config get browser.executablePath
...
natesclaw config set agents.defaults.heartbeat.every "2h"
...
natesclaw config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
natesclaw config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
natesclaw config set secrets.providers.vaultfile --provider-source file --provider-path /etc/natesclaw/secrets.json --provider-mode json
natesclaw config patch --file ./natesclaw.patch.json5 --dry-run
natesclaw config unset plugins.entries.brave.config.webSearch.apiKey
natesclaw config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN --dry-run
natesclaw config validate
natesclaw config validate --json
```
...
```bash
natesclaw config get agents.defaults.workspace
natesclaw config get agents.entries.main
natesclaw config get agents.entries
natesclaw config set 'agents.entries.work.tools.exec.node' "node-id-or-name"
```
...
Prints the active config file path, resolved from `NATESCLAW_CONFIG_PATH` or the default location. The path names a regular file, not a symlink; see [Write safety](#write-safety).

With `--json`, stdout contains an object with the resolved path under `path`.

### `config schema`

Prints the generated JSON schema for `natesclaw.json` to stdout.
...
natesclaw config schema
natesclaw config schema --json
natesclaw config schema > natesclaw.schema.json
```
...
<Note>
If validation is already failing, start with `natesclaw configure` or `natesclaw doctor --fix`. `natesclaw chat` does not bypass the invalid-config guard.
</Note>
...
values. `natesclaw config validate` can validate the container and overall
...
```bash
natesclaw config set agents.defaults.heartbeat.every "0m"
natesclaw config set gateway.port 19001 --strict-json
natesclaw config set channels.whatsapp.groups '["*"]' --strict-json
```

`config get <path> --json` prints the redacted value as JSON instead of terminal-formatted text.

When a write changes `agents.defaults.model` or a per-agent `agents.entries.*.model`, Natesclaw resolves each changed primary or fallback through the configured provider catalogs before writing. Unknown model references are rejected without changing the active config; run `natesclaw models list` to see available models.
...
```bash
natesclaw config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
natesclaw config set models.providers.ollama.models '[{"id":"llama3.2","name":"Llama 3.2"}]' --strict-json --merge
```
...
<Tabs>
  <Tab title="Value mode">
    ```bash
    natesclaw config set <path> <value>
    ```
  </Tab>
  <Tab title="SecretRef builder mode">
    ```bash
    natesclaw config set channels.discord.token \
  ...
  <Tab title="Provider builder mode">
    ...
    ```bash
    natesclaw config set secrets.providers.vault \
      --provider-source exec \
      --provider-command /usr/local/bin/natesclaw-vault \
      --provider-arg read \
  ...
  <Tab title="Batch mode">
    ```bash
    natesclaw config set --batch-json '[
      {
    ...
    ```bash
    natesclaw config set --batch-file ./config-set.batch.json --dry-run
    ```
...
natesclaw config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/natesclaw/secrets.json","mode":"json"}' \
  --strict-json
```
...
```bash
natesclaw config patch --file ./natesclaw.patch.json5 --dry-run
natesclaw config patch --file ./natesclaw.patch.json5
```
...
```bash
ssh user@gateway-host 'natesclaw config patch --stdin --dry-run' < ./natesclaw.patch.json5
ssh user@gateway-host 'natesclaw config patch --stdin' < ./natesclaw.patch.json5
```
...
The runtime pin makes this an embedded Natesclaw recipe. A valid `fastMode`
value is a portable typed runtime control and does not choose Natesclaw by
itself.
...
```bash
natesclaw config patch --file ./discord.patch.json5 --replace-path 'channels.discord.guilds["123"].channels'
```
...
`--dry-run` validates changes without writing `natesclaw.json`. Available on `config set`, `config patch`, and `config unset`.
...
<Tabs>
  <Tab title="Success example">
    ```json
    {
      "ok": true,
      "operations": 1,
      "configPath": "/home/user/.natesclaw/natesclaw.json",
      "inputModes": ["builder"],
      "checks": {
        "schema": false,
        "resolvability": true,
...
<AccordionGroup>
  <Accordion title="If dry-run fails">
    ...
    - `model reference validation failed`: a changed text-model primary or fallback is unknown; run `natesclaw models list` and choose an available model.
...
`natesclaw config set` and other Natesclaw-owned config writers validate the full post-change config before committing it to disk. If the new payload fails schema validation or looks like a destructive clobber, the active config is left alone and the rejected payload is saved beside it as `natesclaw.json.rejected.*`.

Natesclaw-owned writes reserialize JSON5 as standard JSON. When the source contains comments, the writer warns immediately before removing them; use a direct editor when preserving comments matters.

<Warning>
The active config path must be a regular file. Symlinked `natesclaw.json` layouts are unsupported for writes; use `NATESCLAW_CONFIG_PATH` to point directly at the real file instead.
</Warning>

Prefer CLI writes for small edits:

```bash
natesclaw config set gateway.reload.mode hybrid --dry-run
natesclaw config set gateway.reload.mode hybrid
natesclaw config validate
```
...
Direct editor writes are still allowed, but the running Gateway treats them as untrusted until they validate. Invalid direct edits fail startup or are skipped by hot reload; Gateway does not rewrite `natesclaw.json`. Run `natesclaw doctor --fix` to repair prefixed/clobbered config or restore the last-known-good copy. See [Gateway troubleshooting](/gateway/troubleshooting#gateway-rejected-invalid-config).
...
After `natesclaw config validate` passes, use the local TUI to have an embedded agent compare the active config against the docs while you validate each change from the same terminal:
...
<Steps>
  ...
  <Step title="Apply targeted edits">
    Apply targeted edits with `natesclaw config set` or `natesclaw configure`.
  </Step>
  <Step title="Re-validate">
    Rerun `natesclaw config validate` after each change.
  </Step>
  <Step title="Doctor for runtime issues">
    If validation passes but the runtime is still unhealthy, run `natesclaw doctor` or `natesclaw doctor --fix` for migration and repair help.
  </Step>
</Steps>

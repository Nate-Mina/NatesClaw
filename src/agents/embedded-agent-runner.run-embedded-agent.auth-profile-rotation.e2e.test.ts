...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): NatesclawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): NatesclawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): NatesclawConfig =>
  ({
    agents: {
      list: [{ id: "test" }],
    },
...
async function withTimedAgentWorkspace<T>(
  ...
  try {
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `natesclaw-auth-rotation-${Date.now()}.log`),
    });

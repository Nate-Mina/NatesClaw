...
describeControlUiE2e("Control UI image lightbox", () => {
  ...
  it("opens transcript and sidebar images in one accessible modal", async () => {
    const banner = await readFile(path.join(process.cwd(), "docs/assets/natesclaw-banner-dark.png"));
    ...
    try {
      ...
      const transcriptTrigger = page.getByRole("button", { name: "Open image Natesclaw banner" });
      await transcriptTrigger.waitFor({ state: "visible", timeout: 10_000 });
      await transcriptTrigger.click();

      const dialog = page.getByRole("dialog", { name: "Image preview: Natesclaw banner" });
      ...
      const focusIsInsideLightbox = () =>
        page.locator("natesclaw-image-lightbox").evaluate((lightbox) => {
      ...
      await expect
        ...
        .toBe(true);
      const displayedImage = page.getByAltText("Natesclaw banner").last();
      await expect
        .poll(() =>
        ...
        .toBeGreaterThan(0);
      const desktopBox = await page.locator("natesclaw-image-lightbox .lightbox").boundingBox();
      ...
      const sidebarDialog = page.getByRole("dialog", {
        name: "Image preview: natesclaw-banner.png",
      });
      ...
      await sidebarTrigger.click();
      await sidebarDialog.waitFor({ state: "visible" });
      const mobileBox = await page.locator("natesclaw-image-lightbox .lightbox").boundingBox();

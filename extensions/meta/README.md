...
Official Natesclaw provider plugin for the **Meta API** — an OpenAI-compatible
...
Natesclaw defaults to `high`. Meta's
[reasoning documentation](https://dev.meta.ai/docs/reasoning/) says that an
omitted effort uses a model-determined level, so `high` is an Natesclaw policy rather

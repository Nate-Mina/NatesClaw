...
function createLauncherPayloadFile(
  ...
  const payloadDir = mkdtempSync(
    path.join(resolvePreferredNatesclawTmpDir(), "natesclaw-mxc-payload-"),
  );
...
function createSandboxTempDir(hostEnv: BaselineHostEnv): string {
  return mkdtempSync(path.join(resolveSandboxTempDir(hostEnv), "natesclaw-mxc-sandbox-"));
}
...
/** Manager for `natesclaw sandbox list` and `natesclaw sandbox remove`. */

...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "natesclaw-empty-bundled-plugins");
const TEST_TRUST_BUNDLED_PLUGINS_DIR_ENV = "NATESCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR";
...
export function areBundledPluginsDisabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = normalizeOptionalLowercaseString(env.NATESCLAW_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "natesclaw.plugin.json"))
      );
    });
  } catch {
    return false;
...
function resolvePackageRootsForBundledPlugins(): string[] {
  const argvRoot = resolveNatesclawPackageRootSync({ argv1: process.argv[1] });
  const moduleRoot = resolveNatesclawPackageRootSync({ moduleUrl: import.meta.url });
...
function resolveTrustedExistingOverride(resolvedOverride: string): string | null {
  ...
  const modulePackageRoot = resolveNatesclawPackageRootSync({ moduleUrl: import.meta.url });
...
function createBundledPluginsDirCacheKey(env: NodeJS.ProcessEnv): string {
  return JSON.stringify({
    disabled: env.NATESCLAW_DISABLE_BUNDLED_PLUGINS ?? "",
    override: env.NATESCLAW_BUNDLED_PLUGINS_DIR ?? "",
    ...
    NatesclawHome: env.NATESCLAW_HOME ?? "",
    home: env.HOME ?? "",
...
function resolveBundledPluginsDirUncached(env: NodeJS.ProcessEnv): string | undefined {
  ...
  const override = env.NATESCLAW_BUNDLED_PLUGINS_DIR?.trim();

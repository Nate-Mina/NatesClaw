...
const fixtureRoot = makeTrackedTempDir("natesclaw-sdk-alias-root", fixtureTempDirs);
let tempDirIndex = 0;
...
function createTrustedNatesclawPackageFixture(version: string) {
  const root = makeTempDir();
  fs.writeFileSync(path.join(root, "natesclaw.mjs"), "export {};\n", "utf-8");
  fs.writeFileSync(
...
function createUserInstalledPluginSdkAliasFixture() {
  ...
  const externalPluginRoot = path.join(makeTempDir(), ".natesclaw", "extensions", "demo");
...
function listPluginSdkExportedSubpaths(params: {
  ...
  return Object.keys(
    ...
    .filter((key) => key.startsWith("natesclaw/plugin-sdk/"))
    .map((key) => key.slice("natesclaw/plugin-sdk/".length))
    .toSorted();
}
...
function expectCwdFallbackPluginSdkAliasResolution(params: {
  ...
  const resolved = withCwd(params.fixture.root, () =>
    withEnv(
      { NODE_ENV: undefined },
      () =>
        buildPluginLoaderAliasMap("/tmp/tsx-cache/natesclaw-loader.js", "")[
          "natesclaw/plugin-sdk/channel-runtime-context"
        ] ?? null,
    ),
  );
...
describe("plugin sdk alias helpers", () => {
  ...
  it("adds private qa plugin-sdk subpaths for trusted local checkouts when enabled", () => {
    ...
    const subpaths = withEnv({ NATESCLAW_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
  ...
  it("resolves a private-local bundled helper without enabling private QA mode", () => {
    ...
    const subpaths = withEnv({ NATESCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({ modulePath: sourcePluginEntry }),
    );
    const aliases = withEnv(
      { NATESCLAW_ENABLE_PRIVATE_QA_CLI: undefined, NODE_ENV: undefined },
      () => buildPluginLoaderAliasMap(sourcePluginEntry),
    );

    expect(subpaths).toEqual(["core", "qa-runner-runtime"]);
    expect(fs.realpathSync(aliases["natesclaw/plugin-sdk/qa-runner-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRunnerPath),
    );
    expect(aliases["natesclaw/plugin-sdk/qa-runtime"]).toBeUndefined();
  });
  ...
  it("adds the non-QA private Codex helper subpath only for trusted Codex plugins", () => {
    ...
    const codexSubpaths = withEnv({ NATESCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceCodexEntry,
      }),
    );
    const otherSubpaths = withEnv({ NATESCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceOtherEntry,
      }),
    );
    const installedCodexSubpaths = withCwd(installedCodexRoot, () =>
      withEnv({ NATESCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
        listPluginSdkExportedSubpaths({
          modulePath: installedCodexEntry,
          argv1: path.join(fixture.root, "natesclaw.mjs"),
        }),
      ),
    );
  ...
  it("does not reuse a non-private cached subpath list after private qa gets enabled", () => {
    ...
    const privateSubpaths = withEnv({ NATESCLAW_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it.each([
    {
      name: "does not derive plugin-sdk subpaths from cwd fallback when package root is not an Natesclaw root",
      fixture: () =>
    ...
    expectExportedSubpaths({
      fixture,
      cwd: fixture.root,
      modulePath: "/tmp/tsx-cache/natesclaw-loader.js",
      expected,
    });
  });
  ...
  it("adds private qa plugin-sdk aliases for source plugins when enabled", () => {
    ...
    const aliases = withEnv({ NATESCLAW_ENABLE_PRIVATE_QA_CLI: "1", NODE_ENV: undefined }, () =>
      buildPluginLoaderAliasMap(sourcePluginEntry),
    );

    expect(fs.realpathSync(aliases["natesclaw/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
    expect(fs.realpathSync(aliases["natesclaw/plugin-sdk/qa-channel"] ?? "")).toBe(
      fs.realpathSync(sourceQaChannelPath),
    );
    expect(fs.realpathSync(aliases["natesclaw/plugin-sdk/qa-channel-protocol"] ?? "")).toBe(
      fs.realpathSync(sourceQaChannelProtocolPath),
    );
    expect(fs.realpathSync(aliases["natesclaw/plugin-sdk/qa-lab"] ?? "")).toBe(
  ...
  it("aliases non-QA private plugin-sdk subpaths for trusted Codex runtime loading", () => {
    ...
    const devRootAliases = withEnv(
      ...
      () =>
        buildPluginLoaderAliasMap(
          distCodexEntry,
          path.join(fixture.root, "natesclaw.mjs"),
          undefined,
          "dist",
          devFixture.root,
        ),
    ...
    const installedAliases = withCwd(installedCodexRoot, () =>
      withEnv({ NATESCLAW_ENABLE_PRIVATE_QA_CLI: undefined, NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(
          installedCodexEntry,
          path.join(fixture.root, "natesclaw.mjs"),
          undefined,
    ...
    expect(fs.realpathSync(aliases["natesclaw/plugin-sdk/codex-mcp-projection"] ?? "")).toBe(
      fs.realpathSync(sourceCodexMcpProjectionPath),
    );
    expect(
      fs.realpathSync(installedAliases["natesclaw/plugin-sdk/codex-mcp-projection"] ?? ""),
    ).toBe(fs.realpathSync(distCodexMcpProjectionPath));
    expect(fs.realpathSync(devRootAliases["natesclaw/plugin-sdk/codex-mcp-projection"] ?? "")).toBe(
      fs.realpathSync(devCodexMcpProjectionPath),
    );
    expect(aliases["natesclaw/plugin-sdk/qa-runtime"]).toBeUndefined();
    expect(otherAliases["natesclaw/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(installedOtherAliases["natesclaw/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(shadowCodexAliases["natesclaw/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
  });
  ...
  it("aliases the SSRF internal helper only for bundled local IPC owner plugins", async () => {
    ...
    const ssrfInternalSpecifier = "natesclaw/plugin-sdk/ssrf-runtime-internal";
    const entryBody = [
    ...
    for (const owner of owners.filter(({ resolution }) => resolution === undefined)) {
      const sourceSubpaths = withEnv({ NATESCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
        listPluginSdkExportedSubpaths({ modulePath: owner.entry }),
      );
      expect(sourceSubpaths).toEqual(["core", "ssrf-runtime-internal"]);
    }
    const privateQaOtherSubpaths = withEnv({ NATESCLAW_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceOtherPluginEntry,
      }),
    );
    const ownersWithAliases = owners.map((owner) => ({
      aliases: withEnv({ NATESCLAW_ENABLE_PRIVATE_QA_CLI: undefined, NODE_ENV: undefined }, () =>
    ...
    const privateQaOtherAliases = withEnv(
      { NATESCLAW_ENABLE_PRIVATE_QA_CLI: "1", NODE_ENV: undefined },
      () => buildPluginLoaderAliasMap(sourceOtherPluginEntry),
    );
  ...
  it("derives workspace aliases from packaged root dist when package metadata is absent", () => {
    ...
    const cwdWithoutNatesclawPackage = makeTempDir();

    const aliases = withCwd(cwdWithoutNatesclawPackage, () =>
  ...
  it("falls back to source plugin-sdk subpath aliases when dist chunks are stale", () => {
    ...
    expect(fs.realpathSync(distAliases["natesclaw/plugin-sdk/provider-entry"] ?? "")).toBe(
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via the running natesclaw argv hint", () => {
    const {
      externalPluginEntry,
      externalPluginRoot,
      fixture,
    ...
    const aliases = withCwd(externalPluginRoot, () =>
      withEnv({ NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(externalPluginEntry, path.join(fixture.root, "natesclaw.mjs")),
      ),
    );
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via moduleUrl hint", () => {
    ...
    // This covers installations where argv1 does not resolve to the natesclaw root
    // (e.g. single-binary distributions or custom process launchers).
    // Use natesclaw.mjs which is created by createPluginSdkAliasFixture (bin+marker mode).
    // Use fixture.root as cwd so process.cwd() fallback also resolves to fixture, not the
    // real natesclaw repo root in the test runner environment.
    const loaderModuleUrl = pathToFileURL(path.join(fixture.root, "natesclaw.mjs")).href;
    ...
    // runner binary, inside the natesclaw repo), which resolves before moduleUrl is checked.
    // An empty string is falsy so resolveTrustedNatesclawRootFromArgvHint returns null,
  ...
  it.each([
    {
      name: "does not resolve plugin-sdk alias files from cwd fallback when package root is not an Natesclaw root",
      fixture: () =>
        createPluginSdkAliasFixture({
  ...
  it("configures the plugin loader native-first boundary to prefer native dist modules", () => {
    ...
    expect(options.nativeModules).toEqual(["natesclaw"]);
  ...
  it("preserves configured jiti native modules while adding natesclaw", () => {
    const options = withEnv(
      { JITI_NATIVE_MODULES: JSON.stringify(["native-addon", "natesclaw"]) },
      () => buildPluginLoaderJitiOptions({}),
    );

    expect(options.nativeModules).toEqual(["native-addon", "natesclaw"]);
  });
  ...
  it("scopes plugin loader module config by plugin-sdk resolution", () => {
    ...
    expect(
      fs.realpathSync(auto.aliasMap["natesclaw/plugin-sdk/channel-runtime-context"] ?? ""),
    ).toBe(fs.realpathSync(sourceChannelRuntimePath));
    expect(
      fs.realpathSync(dist.aliasMap["natesclaw/plugin-sdk/channel-runtime-context"] ?? ""),
...
`,
  ...
  it.each([
    ...
    },
    {
      ...
      modulePath: () => "/tmp/tsx-cache/natesclaw-loader.js",
      argv1: (root: string) => path.join(root, "natesclaw.mjs"),
  ...
  it("falls back to ancestor runtime candidates when package-root markers are unavailable", () => {
    const root = makeTempDir();
    const distFile = path.join(root, "dist", "plugins", "runtime", "index.js");
    const loaderCachePath = path.join(root, ".cache", "tsx", "natesclaw-loader.js");
    mkdirSafeDir(path.dirname(distFile));
    ...
    expect(
      resolvePluginRuntimeModule({
        ...
        argv1: path.join(root, "bin", "natesclaw"),
        pluginSdkResolution: "dist",
      }),
    ).toBe(distFile);
  });
  ...
  it("uses the default startup argv hint for runtime fallback when argv1 is omitted", () => {
    ...
    const loaderCachePath = path.join(loaderCacheRoot, "tsx", "natesclaw-loader.js");
    ...
    process.argv[1] = path.join(root, "bin", "natesclaw");
    try {
      expect(
        resolvePluginRuntimeModule({
          modulePath: loaderCachePath,
  ...
  it("resolves runtime fallback through symlinked startup argv", () => {
    const root = makeTempDir();
    const distFile = path.join(root, "dist", "plugins", "runtime", "index.js");
    const binFile = path.join(root, "bin", "natesclaw");
    const shimRoot = makeTempDir();
    const shimFile = path.join(shimRoot, "bin", "natesclaw");
    const loaderCachePath = path.join(makeTempDir(), "tsx", "natesclaw-loader.js");
    mkdirSafeDir(path.dirname(distFile));
  ...
  it("resolves runtime fallback through npm .bin startup argv", () => {
    const root = makeTempDir();
    const packageRoot = path.join(root, "node_modules", "natesclaw");
    ...
    const binFile = path.join(root, "node_modules", ".bin", "natesclaw");
    const loaderCachePath = path.join(makeTempDir(), "tsx", "natesclaw-loader.js");
    mkdirSafeDir(path.dirname(distFile));
...
describe("buildPluginLoaderAliasMap memoization", () => {
  ...
  it("does not reuse a public alias map after private qa aliases are enabled", () => {
    ...
    const publicAliases = withEnv({ NATESCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      buildPluginLoaderAliasMap(entry),
    );
    const privateAliases = withEnv({ NATESCLAW_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      buildPluginLoaderAliasMap(entry),
    );

    expect(publicAliases).not.toBe(privateAliases);
    expect(publicAliases["natesclaw/plugin-sdk/qa-runtime"]).toBeUndefined();
    expect(fs.realpathSync(privateAliases["natesclaw/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
  });
...
describe("buildPluginLoaderJitiOptions", () => {
  it("scopes the jiti cache to the durable user cache and Natesclaw install", () => {
    const root = createTrustedNatesclawPackageFixture("2.0.0");
    const tmpDir = path.join(root, "tmp");
    const cacheRoot = path.join(root, "cache");

    const options = withEnv({ TMPDIR: tmpDir, XDG_CACHE_HOME: `  ${cacheRoot}  ` }, () =>
      buildPluginLoaderJitiOptions(
        { "natesclaw/plugin-sdk/core": path.join(root, "dist", "plugin-sdk", "core.js") },
        { modulePath: path.join(root, "dist", "plugins", "loader.js") },
      ),
    );

    expect(options.fsCache).toContain(path.join(cacheRoot, "natesclaw", "jiti", "2.0.0"));
  ...
  it.each(["", "   ", "relative/cache"])(
    ...
    (xdgCacheHome) => {
      ...
      expect(options.fsCache).toContain(path.join(platformCacheRoot, "natesclaw", "jiti", "2.0.0"));
    },
  );
      ...
      const options = withEnv(
        {
          ...
          NATESCLAW_HOME: path.join(root, "home"),
        },
        () => buildPluginLoaderJitiOptions({}, { modulePath: path.join(root, "dist", "plugins") }),
      );

      expect(options.fsCache).toContain(path.join(localAppData, "natesclaw", "jiti", "2.0.0"));
    },
  );
  ...
  it("keeps deferred jiti imports working after the temporary directory is deleted", async () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        expect(String(options.fsCache)).toContain(path.join(durableCacheRoot, "natesclaw", "jiti"));

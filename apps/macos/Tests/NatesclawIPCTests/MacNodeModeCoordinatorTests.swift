...
struct MacNodeModeCoordinatorTests {
    ...
    @Test @MainActor func `terminal worker failure is reported instead of scheduling another restart`() async throws {
        ...
        try coordinator.prepareNodeHostWorkerRetryForTesting(
            command: ["/usr/local/bin/natesclaw", "node", "worker"])
        await confirmation("terminal worker failure") { confirmed in
            let observer = notificationCenter.addObserver(
                forName: .natesclawNodeHostWorkerRetryExhausted,
                object: coordinator,
                queue: nil)
            { notification in
    ...
    @Test @MainActor func `worker cannot restart before its crash backoff expires`() async throws {
        ...
        let command = ["/usr/local/bin/natesclaw", "node", "worker"]
    ...
    @Test func `native manifest excludes CLI-owned node commands`() {
        ...
        #expect(!caps.contains(NatesclawCapability.browser.rawValue))
        #expect(!commands.contains(NatesclawBrowserCommand.proxy.rawValue))
        #expect(commands.contains(NatesclawCanvasCommand.present.rawValue))
        #expect(commands.contains(NatesclawSystemCommand.notify.rawValue))
        #expect(!commands.contains(NatesclawFileSystemCommand.listDir.rawValue))
    ...
    @Test func `Codex supervision activation respects the plugin flag and global policy`() {
        ...
        #expect(NatesclawConfigFile.explicitlyEnabledPluginConfigFlag(
            "codex",
        ...
        #expect(NatesclawConfigFile.configuredBundledPluginAllowed(
            "codex",
        ...
        #expect(NatesclawConfigFile.pluginEntry("codex", root: ambiguousEntryAliases) == nil)
        #expect(!NatesclawConfigFile.explicitlyEnabledPluginConfigFlag(
            "codex",
            path: ["supervision", "enabled"],
    ...
    @Test func `computer control cap gates the computer.act command`() {
        ...
        #expect(enabledCaps.contains(NatesclawCapability.computer.rawValue))
        #expect(enabledCommands.contains(NatesclawComputerCommand.act.rawValue))
        ...
        #expect(!disabledCaps.contains(NatesclawCapability.computer.rawValue))
        #expect(!disabledCommands.contains(NatesclawComputerCommand.act.rawValue))
    }
    ...
    @Test func `camera cap gates capture and PTZ commands`() {
        let enabledCaps = MacNodeModeCoordinator.resolvedCaps(
            ...
            connectionMode: .local)
        let enabledCommands = MacNodeModeCoordinator.resolvedCommands(caps: enabledCaps)
        #expect(enabledCommands.contains(NatesclawCameraCommand.list.rawValue))
        #expect(enabledCommands.contains(NatesclawCameraCommand.ptzStatus.rawValue))
        #expect(enabledCommands.contains(NatesclawCameraCommand.ptzControl.rawValue))
        ...
        let disabledCaps = MacNodeModeCoordinator.resolvedCaps(
            ...
            connectionMode: .local)
        let disabledCommands = MacNodeModeCoordinator.resolvedCommands(caps: disabledCaps)
        #expect(!disabledCommands.contains(NatesclawCameraCommand.ptzStatus.rawValue))
        #expect(!disabledCommands.contains(NatesclawCameraCommand.ptzControl.rawValue))
    }

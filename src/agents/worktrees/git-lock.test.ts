...
describe("lockWorktreeForProcess", () => {
  ...
  async function setupWorktree(): Promise<ManagedWorktreeRecord> {
    const root = await fs.realpath(tempDirs.make("natesclaw-git-lock-"));
  ...
  it("reclaims a lock left behind by a dead Natesclaw process", async () => {
    ...
    expect(await lockedReason(record.repoRoot, record.path)).toBe(`natesclaw pid=${process.pid}`);
  });

  it("keeps a lock held by a live Natesclaw process", async () => {
    const record = await setupWorktree();
    ...
    await git(
      ...
      "lock",
      "--reason",
      `natesclaw pid=${livePid}`,
      record.path,
    );

    await expect(lockWorktreeForProcess(record)).rejects.toThrow(/git worktree lock/);

    expect(await lockedReason(record.repoRoot, record.path)).toBe(`natesclaw pid=${livePid}`);
  });

  it("keeps a foreign lock that Natesclaw does not own", async () => {
    const record = await setupWorktree();

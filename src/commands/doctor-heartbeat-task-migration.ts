...
function migrationFinding(params: {
  ...
  return {
    ...
    fixHint: `Run ${formatCliCommand("natesclaw doctor --fix")} to convert heartbeat tasks into automations.`,
  };
}
...
function taskJobInput(params: {
  ...
  return {
    ...
    description: "Migrated from heartbeat monitor scratch by natesclaw doctor.",
...
async function loadCronPlanningSnapshot(
  ...
  const rows = loadCronRows(openNatesclawStateDatabase({ env }).db, cronStoreKey(storePath));

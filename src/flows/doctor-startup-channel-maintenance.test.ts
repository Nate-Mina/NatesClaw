...
describe("doctor startup channel maintenance", () => {
  ...
  it("maps channel doctor preview warnings to structured findings", async () => {
    ...
    await expect(
      collectChannelPreviewWarningHealthFindings({
        cfg,
        doctorFixCommand: "natesclaw doctor --fix --dry-run",
    ...
    ).resolves.toEqual([
      {
        ...
        fixHint:
          "Run `natesclaw doctor --fix --dry-run` if the channel warning recommends repair, or update the affected channel config manually.",
      },
    ]);
  ...
  it("runs Matrix startup migration during repair flows", async () => {
    ...
    expect(call.env).toEqual({ NATESCLAW_TEST: "1" });

...
import ai.natesclaw.app.gateway.testDeviceIdentityStore
...
class InvokeDispatcherTest {
    ...
    runTest {
      val result =
        newDispatcher(
          ...
          smsFeatureEnabled = true,
          smsTelephonyAvailable = true,
        ).handleInvoke(NatesclawSmsCommand.Search.rawValue, "not-json")
    ...
    runTest {
      val result =
        newDispatcher(
          ...
          smsFeatureEnabled = true,
          smsTelephonyAvailable = true,
        ).handleInvoke(NatesclawSmsCommand.Send.rawValue, """{"to":"+15551234567","message":"hi"}""")
  ...
  fun handleInvoke_blocksCameraCommandsWhenCameraDisabled() =
    runTest {
      val result = newDispatcher(cameraEnabled = false).handleInvoke(NatesclawCameraCommand.List.rawValue, null)
  ...
  fun handleInvoke_blocksLocationCommandWhenLocationDisabled() =
    runTest {
      val result = newDispatcher(locationEnabled = false).handleInvoke(NatesclawLocationCommand.Get.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(installedAppsSharingEnabled = false)
          .handleInvoke(NatesclawDeviceCommand.Apps.rawValue, """{"limit":1}""")
    ...
    runTest {
      val result =
        newDispatcher(motionActivityAvailable = false)
          .handleInvoke(NatesclawMotionCommand.Activity.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(motionPedometerAvailable = false)
          .handleInvoke(NatesclawMotionCommand.Pedometer.rawValue, null)
  ...
  fun handleInvoke_blocksCallLogWhenUnavailable() =
    runTest {
      val result =
        newDispatcher(callLogAvailable = false).handleInvoke(NatesclawCallLogCommand.Search.rawValue, null)
  ...
  fun handleInvoke_blocksPhotosWhenUnavailable() =
    runTest {
      val result = newDispatcher(photosAvailable = false).handleInvoke(NatesclawPhotosCommand.Latest.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(mobileUiAvailable = false)
          .handleInvoke(NatesclawMobileUiCommand.Observe.rawValue, null)
  ...
  fun handleInvoke_routesTalkPttCommands() =
    runTest {
      ...
      val start = dispatcher.handleInvoke(NatesclawTalkCommand.PttStart.rawValue, null)
      val stop = dispatcher.handleInvoke(NatesclawTalkCommand.PttStop.rawValue, null)
      val cancel = dispatcher.handleInvoke(NatesclawTalkCommand.PttCancel.rawValue, null)
      val once = dispatcher.handleInvoke(NatesclawTalkCommand.PttOnce.rawValue, null)
  ...
  fun handleInvoke_presentAndHideDriveTheShellOwnedCanvasState() =
    runTest {
      ...
      val hide = dispatcher.handleInvoke(NatesclawCanvasCommand.Hide.rawValue, null)
    ...
    runTest {
      ...
      val result =
        newDispatcher(isForeground = false, canvas = canvas)
          .handleInvoke(NatesclawCanvasCommand.Present.rawValue, """{"url":"https://example.com"}""")

---
summary: "CLI reference for `natesclaw workboard` cards, dispatch, and worker runs"
read_when:
...
`natesclaw workboard` is the terminal surface for the bundled [Workboard plugin](/plugins/workboard). It lets an operator list cards, create a card, inspect one card, and ask the running Gateway to dispatch ready work into subagent worker runs.
...
natesclaw workboard create <title...> [--notes <text>] [--status <status>] [--priority <priority>] [--agent <id>] [--board <id>] [--labels <items>] [--json]
natesclaw workboard show <id> [--json]
natesclaw workboard move <id> --status <status> [--json]
natesclaw workboard dispatch [--board <id>] [--max-starts <count>] [--admin] [--url <url>] [--token <token>] [--timeout <ms>] [--json]
```
...
```bash
natesclaw workboard list
natesclaw workboard list --board default --status ready
natesclaw workboard list --json
```
...
```bash
natesclaw workboard create "Fix stale worker heartbeat" --priority high --labels bug,workboard
natesclaw workboard create "Write Workboard docs" --status ready --agent docs-agent --board docs --notes "Cover CLI, slash command, dispatch, and SQLite state."
```
...
```bash
natesclaw workboard show 7f4a2c10
natesclaw workboard show 7f4a2c10 --json
```
...
```bash
natesclaw workboard move 7f4a2c10 --status review
natesclaw workboard move 7f4a2c10 --status done --json
```
...
natesclaw workboard dispatch --json
natesclaw workboard dispatch --max-starts 10
natesclaw workboard dispatch --admin
natesclaw workboard dispatch --url http://127.0.0.1:18789 --token "$NATESCLAW_GATEWAY_TOKEN"
```
...
Local `list`, `create`, `show`, and `move` commands operate on the local Natesclaw state directory used by the current profile. Use `--dev` or `--profile <name>` on the top-level `natesclaw` command when you need a different state root.
...
Then retry `natesclaw workboard dispatch`. Data-only fallback is useful for local state cleanup, but worker runs need a live Gateway.

...
@Suite(.serialized) struct NatesclawAppDelegateTests {
    @Test @MainActor func `resolves registry model before view task assigns delegate model`() {
        let registryModel = NodeAppModel()
        NatesclawAppModelRegistry.appModel = registryModel
        defer { NatesclawAppModelRegistry.appModel = nil }

        let delegate = NatesclawAppDelegate()
    ...
    @Test @MainActor func `derives background refresh task identifier from app bundle identifier`() {
        let delegate = NatesclawAppDelegate()
        let bundleIdentifier = Bundle.main.bundleIdentifier ?? "ai.natesclawfoundation.app.tests"
    ...
    @Test @MainActor func `stages a gateway URL when the model is ready`() async throws {
        NatesclawAppModelRegistry.appModel = nil
        defer { NatesclawAppModelRegistry.appModel = nil }
        let model = NodeAppModel()
        let delegate = NatesclawAppDelegate()
        delegate.appModel = model
        let url = try #require(URL(
            string: "natesclaw://gateway?host=gateway.example.com&port=443&tls=1&token=tok"))

...
export function buildComfyConfig(config: Record<string, unknown>): NatesclawConfig {
  return {
    plugins: {
      entries: {
        comfy: { config },
...
export function buildLegacyComfyConfig(config: Record<string, unknown>): NatesclawConfig {
  return {
    models: {
      providers: {
        comfy: config,

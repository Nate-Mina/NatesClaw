// Safe loader for the conventional package-local Natesclaw profile.
...
import type { ClawDiagnostic, ClawNatesclawProfile } from "./types.js";

const MAX_PROFILE_BYTES = 256 * 1024;
const CLAW_PROFILE_PATH = "profiles/natesclaw.yml";
const LEGACY_PROFILE_POINTER_KEY = "natesclaw.config";
const LEGACY_PROFILE_POINTER_PATH = "$.metadata.natesclaw.config";
const CONVENTIONAL_PROFILE_PATH = "$.profiles.natesclaw";
...
function parseProfileYaml(
  ...
  if (document.errors.length > 0) {
    return {
      ...
      diagnostics: document.errors.map((error) =>
        diagnostic("invalid_natesclaw_profile", `Could not parse ${path}: ${error.message}`),
      ),
    };
  }
  ...
  if (unsupportedFeature) {
    return {
      ...
      diagnostics: [
        diagnostic(
          "unsupported_natesclaw_profile_yaml_feature",
          `${path} uses ${unsupportedFeature}; Natesclaw profile YAML must map directly to JSON data.`,
        ),
      ],
    };
  }
...
/**
 * Resolves the Natesclaw profile for a package.
 *
 * `profiles/natesclaw.yml` is the conventional location. The retired
 * `metadata.natesclaw.config` pointer is still read for compatibility with
 ...
 */
export async function readClawNatesclawProfile(params: {
  packageRoot: string;
  metadata?: Record<string, string>;
}): Promise<
  | {
      ok: true;
      profile?: ClawNatesclawProfile;
      raw?: Buffer;
      path?: string;
...
> {
  ...
  if (legacyPointer !== undefined) {
    if (
      ...
      return {
        ...
        diagnostics: [
          diagnostic(
            "invalid_natesclaw_profile_path",
    ...
    if (conventionalExists && legacyPointer !== CLAW_PROFILE_PATH) {
      return {
        ...
        diagnostics: [
          diagnostic(
            "conflicting_natesclaw_profile_pointer",
    ...
    diagnostics.push(
      warning(
        "deprecated_natesclaw_profile_pointer",
  ...
  try {
    ...
    return {
      ...
      diagnostics: [
        diagnostic(
          unsafe
            ? "natesclaw_profile_unsafe"
            : tooLarge
              ? "natesclaw_profile_too_large"
              : "natesclaw_profile_read_failed",
          unsafe
            ? "The Natesclaw profile must be a regular, non-symlinked, non-hardlinked file."
            : tooLarge
              ? `The Natesclaw profile exceeds ${MAX_PROFILE_BYTES} bytes.`
  ...
  const parsed = parseClawNatesclawProfile(yaml.value);
  if (!parsed.ok) {
    return {
      ok: false,

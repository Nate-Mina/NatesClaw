...
import type { ChannelSetupWizard } from "natesclaw/plugin-sdk/setup";
...
type WhatsAppConfig = NonNullable<NonNullable<NatesclawConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: NatesclawConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: NatesclawConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: NatesclawConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: NatesclawConfig, accountId: string): Promise<boolean> {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      t("wizard.whatsapp.linkLater", {
        command: formatCliCommand("natesclaw channels login"),
      }),
      "WhatsApp",
    );
  }

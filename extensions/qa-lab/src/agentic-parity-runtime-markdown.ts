...
export function renderQaRuntimeParityMarkdownReport(report: QaRuntimeParityReport): string {
  const lines = [
    `# Natesclaw Runtime Parity Report — ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
    ...
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    `| natesclaw | ${formatRuntimeCacheCount(report.usage.natesclaw?.grossInputTokens)} | ${formatRuntimeCacheCount(report.usage.natesclaw?.uncachedInputTokens)} | ${formatRuntimeCacheCount(report.usage.natesclaw?.cachedInputTokens)} | ${formatRuntimeCacheCount(report.usage.natesclaw?.cacheWriteTokens)} | ${formatRuntimeCacheCount(report.usage.natesclaw?.outputTokens)} | ${formatRuntimeCacheCount(report.usage.natesclaw?.totalTokens)} | ${formatRuntimeCacheHitPercent(report.usage.natesclaw?.cacheHitPercent)} |`,
    ...
    "| --- | ---: | ---: | ---: |",
    `| natesclaw | ${formatRuntimeWallClockMs(report.timing.natesclaw.totalWallClockMs)} | ${formatRuntimeWallClockMs(report.timing.natesclaw.p50WallClockMs)} | ${formatRuntimeWallClockMs(report.timing.natesclaw.p90WallClockMs)} |`,
  ...
  if (report.timing.bootstrap) {
    lines.push(
      ...
      "| --- | ---: | ---: | ---: |",
      `| natesclaw | ${formatRuntimeWallClockMs(report.timing.bootstrap.natesclaw.totalWallClockMs)} | ${formatRuntimeWallClockMs(report.timing.bootstrap.natesclaw.p50WallClockMs)} | ${formatRuntimeWallClockMs(report.timing.bootstrap.natesclaw.p90WallClockMs)} |`,
  ...
  for (const scenario of report.scenarios) {
    ...
    const natesclawTokens = usageNotApplicable ? "N/A" : String(scenario.natesclawTokens);
    const codexTokens = usageNotApplicable ? "N/A" : String(scenario.codexTokens);
    lines.push(`### ${scenario.name}`, "");
    lines.push(`- status: ${scenario.status}`);
    lines.push(`- drift: ${scenario.drift}`);
    lines.push(
      `- natesclaw: ${scenario.natesclawStatus} (${scenario.natesclawToolCalls} tool calls, ${natesclawTokens} tokens)`,
    );
    lines.push(
      `- codex: ${scenario.codexStatus} (${scenario.codexToolCalls} tool calls, ${codexTokens} tokens)`,
    );
    lines.push(
      `- wall time: natesclaw ${formatRuntimeWallClockMs(scenario.natesclawWallClockMs)}; codex ${formatRuntimeWallClockMs(scenario.codexWallClockMs)}; ${formatRuntimeSpeedComparison(scenario)}`,
    );
    if (
      scenario.natesclawBootstrapWallClockMs !== undefined ||
      scenario.codexBootstrapWallClockMs !== undefined
    ) {
      lines.push(
        `- gateway bootstrap (excluded): natesclaw ${formatRuntimeWallClockMs(scenario.natesclawBootstrapWallClockMs ?? null)}; codex ${formatRuntimeWallClockMs(scenario.codexBootstrapWallClockMs ?? null)}`,
      );
    }
    lines.push(
      `- prompt cache: natesclaw ${formatRuntimeCacheHitPercent(scenario.natesclawUsage?.cacheHitPercent)} (${formatRuntimeCacheCount(scenario.natesclawUsage?.cachedInputTokens)} cached, ${formatRuntimeCacheCount(scenario.natesclawUsage?.uncachedInputTokens)} uncached input); codex ${formatRuntimeCacheHitPercent(scenario.codexUsage?.cacheHitPercent)} (${formatRuntimeCacheCount(scenario.codexUsage?.cachedInputTokens)} cached, ${formatRuntimeCacheCount(scenario.codexUsage?.uncachedInputTokens)} uncached input)`,
    );
    lines.push(
      `- post-warm cache misses: natesclaw ${formatRuntimeCacheMisses(scenario.natesclawCacheDiagnostics)}; codex ${formatRuntimeCacheMisses(scenario.codexCacheDiagnostics)}`,
    );

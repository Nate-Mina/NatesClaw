...
type TaskFlowRegistryTestDatabase = Pick<NatesclawStateKyselyDatabase, "flow_runs">;
...
async function withFlowRegistryTempDir<T>(run: (root: string) => Promise<T>): Promise<T> {
  return await withNatesclawTestState(
    {
      layout: "state-only",
      prefix: "natesclaw-task-flow-store-",
    },
    async (state) => {
...
const ORIGINAL_STATE_DIR = process.env.NATESCLAW_STATE_DIR;

function restoreOriginalStateDir(): void {
  if (ORIGINAL_STATE_DIR === undefined) {
    delete process.env.NATESCLAW_STATE_DIR;
  } else {
    process.env.NATESCLAW_STATE_DIR = ORIGINAL_STATE_DIR;
  }
}

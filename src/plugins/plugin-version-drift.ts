...
export function resolvePluginVersionDriftUpdateCommand(entry: PluginVersionDriftEntry): string {
  ...
  if (exactNpmPackageName) {
    ...
    if (parseRegistryNpmSpec(exactNpmTarget)?.selectorKind === "exact-version") {
      return `natesclaw plugins update ${exactNpmTarget}`;
    }
  }
  return `natesclaw plugins update ${entry.pluginId}`;
}
...
function isPluginEnabled(config: NatesclawConfig | undefined, pluginId: string): boolean {
 ...
 *   `version` field of the installed natesclaw package.json).
 ...
 * @param params.config The merged daemon-side NatesclawConfig (optional).

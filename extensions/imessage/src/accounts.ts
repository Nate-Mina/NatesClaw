...
import { normalizeAccountId, type NatesclawConfig } from "natesclaw/plugin-sdk/account-resolution";
// Imessage plugin module implements accounts behavior.
import { expectDefined } from "natesclaw/plugin-sdk/expect-runtime";
import { resolveAccountEntry } from "natesclaw/plugin-sdk/routing";
...
function mergeIMessageAccountConfig(cfg: NatesclawConfig, accountId: string): IMessageAccountConfig {
...
// caused duplicate inbound handling in natesclaw/natesclaw#65141.
...
function resolveIMessageAccountSourceOwner(params: {
  ...
  // bindings tied to the named account keep working (natesclaw/natesclaw#65141).
 ...
 * watchers (natesclaw/natesclaw#65141) without otherwise marking the duplicate
...
export function listEnabledIMessageAccounts(cfg: NatesclawConfig): ResolvedIMessageAccount[] {

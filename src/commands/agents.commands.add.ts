// Implements `natesclaw agents add`, including config mutation, workspace setup, auth copy, and route binding setup.
...
export async function agentsAddCommand(
  ...
  if (nonInteractive) {
    ...
    if (!nameInput) {
      runtime.error(
        `Agent name is required in non-interactive mode. Run ${formatCliCommand("natesclaw agents add <id> --workspace <path>")}.`,
      );
      runtime.exit(1);
      return;
    }
    ...
    if (created.status === "error") {
      runtime.error(
        created.reason === "reserved-id"
          ? `"${created.agentId}" is reserved. Choose another name, or run ${formatCliCommand("natesclaw agents list")} to inspect configured agents.`
  ...
  try {
    await prompter.intro("Add Natesclaw agent");
    const name =
      nameInput ??
      (await prompter.text({
        message: "Agent name",

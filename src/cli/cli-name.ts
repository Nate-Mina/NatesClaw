...
const DEFAULT_CLI_NAME = "natesclaw";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(natesclaw)\b/;

/** Resolve the displayed CLI binary name from argv, falling back to `natesclaw`. */
...
/** Replace a leading `natesclaw` command prefix with the active CLI name. */

// Natesclaw agent database stores agent-scoped persisted runtime state.
...
import type {
  ...
  NatesclawAgentDatabaseOwnerInspection,
...
import { ensureNatesclawAgentDatabasePermissions } from "./natesclaw-agent-db-permissions.js";
import {
...
export {
  ...
  type NatesclawAgentDatabaseOwnerInspection,
...
} from "./natesclaw-agent-db-maintenance.js";
export { ensureNatesclawAgentDatabasePermissions } from "./natesclaw-agent-db-permissions.js";
export { listNatesclawRegisteredAgentDatabases } from "./natesclaw-agent-db-registry.js";
export { ensureNatesclawAgentDatabaseSchema } from "./natesclaw-agent-db-schema.js";
export {
  isIncognitoNatesclawAgentSqlitePath,
 ...
 * Natesclaw state database for discovery and maintenance.
 */
const NATESCLAW_AGENT_DB_SLOW_OPEN_MS = 1_000;
...
export const NATESCLAW_AGENT_DB_OPEN_HANDLE_CAP = 64;
const agentDbLog = createSubsystemLogger("state/agent-db");
const cachedDatabases = new Map<string, NatesclawAgentDatabase>();
const incognitoDatabases = new WeakSet<NatesclawAgentDatabase>();
...
const terminalOpenLatch = createSqliteTerminalOpenLatch({
  closeByPath: closeNatesclawAgentDatabaseByPath,
});

/** Reconfirm an advisory worker failure on the live owner connection. */
export function confirmNatesclawAgentDatabaseIntegrity(
  pathname: string,
): SqliteIntegrityConfirmation {
  const resolvedPath = path.resolve(pathname);
  closeNatesclawAgentDatabaseByPath(resolvedPath);
...
export function recordNatesclawAgentDatabaseOpenFailure(
  pathname: string,
  error: Error,
 ...
 */
export function clearNatesclawAgentDatabaseOpenFailure(
  pathname: string,
  options: NatesclawStateDatabaseOptions = {},
): boolean {
  const resolvedPath = path.resolve(pathname);
  const cleared = clearNatesclawDatabaseQuarantine(resolvedPath, { env: options.env });
...
function logSlowAgentDatabaseOpen(params: {
  ...
  if (params.elapsedMs < NATESCLAW_AGENT_DB_SLOW_OPEN_MS) {
    return;
  }
  agentDbLog.warn("slow Natesclaw agent database open", {
    agentId: params.agentId,
    elapsedMs: params.elapsedMs,
    path: params.path,
    thresholdMs: NATESCLAW_AGENT_DB_SLOW_OPEN_MS,
  });
}

/** Read a database's durable role and agent owner without mutating it. */
export function inspectNatesclawAgentDatabaseOwner(
  pathname: string,
): NatesclawAgentDatabaseOwnerInspection {
...
export function openNatesclawAgentDatabase(
  options: NatesclawAgentDatabaseOptions,
): NatesclawAgentDatabase {
  const agentId = normalizeAgentId(options.agentId);
  const databaseOptions = { ...options, agentId };
  const pathname = resolveNatesclawAgentSqlitePath(databaseOptions);
  const incognito = isIncognitoNatesclawAgentSqlitePath(pathname, databaseOptions);
  ...
  if (cached?.db.isOpen) {
    ...
    if (cached.agentId !== agentId) {
      throw new Error(
        `Natesclaw agent database ${pathname} is already open for agent ${cached.agentId}; requested agent ${agentId}.`,
      );
    }
  ...
  if (incognito) {
    ...
    if (cached) {
      closeCachedNatesclawAgentDatabase(cached);
      cachedDatabases.delete(pathname);
    ...
    const walMaintenance = configureSqliteConnectionPragmas(db, {
      busyTimeoutMs: NATESCLAW_SQLITE_BUSY_TIMEOUT_MS,
      databaseLabel: `natesclaw-agent-incognito:${agentId}`,
      foreignKeys: true,
      synchronous: "NORMAL",
    });
    ensureNatesclawAgentSchema(db, agentId, pathname);
    const database = { agentId, db, path: pathname, walMaintenance };
    incognitoDatabases.add(database);
    unregisterExitClose ??= registerSqliteCacheExitClose(closeNatesclawAgentDatabases);
  ...
  try {
    const quarantine = readNatesclawDatabaseQuarantine(pathname, { env: databaseOptions.env });
    if (quarantine) {
      persistedFailure = createNatesclawDatabaseVerificationError(
        "agent",
        pathname,
        quarantine.reason,
      );
  ...
  if (persistedFailure) {
    recordNatesclawAgentDatabaseOpenFailure(pathname, persistedFailure);
  ...
  let openedDatabase: NatesclawAgentDatabase | undefined;
  let openedWalMaintenance: SqliteWalMaintenance | undefined;
  try {
    ensureNatesclawAgentDatabasePermissions(pathname, databaseOptions);
    ...
    const walMaintenance = (() => {
      let maintenance: NatesclawAgentDatabase["walMaintenance"] | undefined;
      try {
        db.exec(`PRAGMA busy_timeout = ${NATESCLAW_SQLITE_BUSY_TIMEOUT_MS};`);
        ...
        maintenance = configureSqliteConnectionPragmas(db, {
          busyTimeoutMs: NATESCLAW_SQLITE_BUSY_TIMEOUT_MS,
          databaseLabel: `natesclaw-agent:${agentId}`,
        ...
        if (
          ...
          recordNatesclawAgentDatabaseOpenFailure(pathname, err);
        }
        throw err;
      }
    })();
    ...
    if (!isValidatedReopen) {
      registerNatesclawAgentDatabase({ agentId, path: pathname, env: options.env });
    ...
    if (openedDatabase) {
      try {
        closeCachedNatesclawAgentDatabase(openedDatabase);
      } catch (caught) {
        closeError = caught;
      }
    }
    if (openedDb?.isOpen) {
      ...
      const retainedDatabase =
        ...
        } satisfies NatesclawAgentDatabase);
      ...
      releaseNatesclawAgentDatabaseLease(leaseId, { env: options.env });
    }
...
const postCommitPublications = new WeakMap<NatesclawAgentDatabase, Array<() => void>>();

/** Queue a non-throwing runtime publication on the outer database commit edge. */
export function deferNatesclawAgentPostCommitPublication(
...
export function runNatesclawAgentWriteTransaction<T>(
      ...
      () => {
        ...
        if (!enteredNestedTransaction) {
          ...
          if (!incognitoDatabases.has(database)) {
            ensureNatesclawAgentDatabasePermissions(database.path, options);
          }
        }
...
function closeCachedNatesclawAgentDatabase(
  database: NatesclawAgentDatabase,
  ...
  if (lease) {
    releaseNatesclawAgentDatabaseLease(lease.leaseId, { env: lease.env });
...
function evictLruAgentDatabaseHandles(): void {
  ...
  while (cachedDatabases.size >= NATESCLAW_AGENT_DB_OPEN_HANDLE_CAP) {
    let evicted = false;
    for (const [pathname, database] of cachedDatabases) {
      ...
      closeCachedNatesclawAgentDatabase(database, { eviction: true });
      cachedDatabases.delete(pathname);
      cachedDatabaseOpenFailures.delete(pathname);
      agentDbLog.debug("evicted Natesclaw agent database handle", {
    ...
    if (!evicted) {
      ...
      agentDbLog.warn(
        ...
        {
          cap: NATESCLAW_AGENT_DB_OPEN_HANDLE_CAP,
          openHandles: cachedDatabases.size,
        },
      );
      return;
...
export function isNatesclawAgentDatabaseOpen(pathname: string): boolean {
...
export function getNatesclawAgentDatabaseIfOpen(
  options: NatesclawAgentDatabaseOptions,
): NatesclawAgentDatabase | undefined {
  const agentId = normalizeAgentId(options.agentId);
  const pathname = resolveNatesclawAgentSqlitePath({ ...options, agentId });
  ...
  if (database.agentId !== agentId) {
    throw new Error(
      `Natesclaw agent database ${pathname} is already open for agent ${database.agentId}; requested agent ${agentId}.`,
    );
  }
  return database;
}
...
export function isIncognitoNatesclawAgentDatabase(database: NatesclawAgentDatabase): boolean {
  return incognitoDatabases.has(database);
}

/** List process-held agent databases without opening or inspecting fixture state. */
export function listNatesclawAgentDatabasesForTest(): Array<{ agentId: string; path: string }> {
  return [...cachedDatabases.values()]
...
export function closeNatesclawAgentDatabaseByPath(pathname: string): boolean {
  ...
  closeCachedNatesclawAgentDatabase(database);
...
export function disposeNatesclawAgentDatabaseByPath(
  pathname: string,
  ...
  const matchingDatabases = [...cachedDatabases.values()].filter((candidate) =>
    isSameNatesclawAgentDatabasePath(candidate.path, resolvedPath),
  );
  ...
  if (database && incognitoDatabases.has(database)) {
    return closeNatesclawAgentDatabaseByPath(database.path);
  }
  if (!database) {
    return false;
  }
  try {
    ...
    closeNatesclawAgentDatabaseByPath(database.path);
  }
  return true;
}

/** Close all cached agent database handles. */
export function closeNatesclawAgentDatabases(): void {
  unregisterExitClose?.();
  unregisterExitClose = null;
...
export function closeNatesclawAgentDatabasesForTest(): void {
  closeNatesclawAgentDatabases();

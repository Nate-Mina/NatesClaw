...
function resolveDevicePairingStateDbOptions(baseDir?: string): NatesclawStateDatabaseOptions {
  return baseDir ? { env: { ...process.env, NATESCLAW_STATE_DIR: baseDir } } : {};
}
...
function invalidateDevicePairingStoreCache(database: NatesclawStateDatabase): void {
  if (
...
function runDevicePairingStoreMutation<T>(
  ...
  mutate: (database: NatesclawStateDatabase) => DevicePairingStoreMutation<T>,
): T {
  ...
  const result = runNatesclawStateWriteTransaction(mutate, { ...databaseOptions, database });
  if (result.mutated) {
...
export function loadDevicePairingStoreState(baseDir?: string): DevicePairingStoreState {
  const database = openNatesclawStateDatabase(resolveDevicePairingStateDbOptions(baseDir));
  const { db } = database;
  ...
  const kysely = getNodeSqliteKysely<NatesclawStateKyselyDatabase>(db);
...
export function loadPairedDevicePairingStoreRecord(
  ...
  const { db } = openNatesclawStateDatabase(resolveDevicePairingStateDbOptions(baseDir));

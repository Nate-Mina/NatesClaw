...
export const NATESCLAW_WRAPPER_ENV_KEY = "NATESCLAW_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/natesclaw -> .pnpm/natesclaw@X.Y.Z/...
...
function appendNodeModulesBinCandidates(
  ...
  // natesclaw from node_modules/.bin points at the package root sibling.
...
export async function resolveNatesclawWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${NATESCLAW_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveNatesclawWrapperPath(params.wrapperPath);
  if (wrapperPath) {

...
function hasExplicitImageModelConfig(config: NatesclawConfig | undefined): boolean {
  return hasExplicitToolModelConfig(config?.agents?.defaults?.imageModel);
}

function hasExplicitPdfModelConfig(config: NatesclawConfig | undefined): boolean {
  return (

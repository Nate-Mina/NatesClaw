...
```bash
export NATESCLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.natesclaw/natesclaw.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env NATESCLAW_PARALLELS_DISCORD_TOKEN \
...
- Use the `natesclaw` wrapper for guest `message send/read`; `node natesclaw.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/natesclaw-parallels-smoke.*`
...
- host inbound nonce appears in `natesclaw message read` output

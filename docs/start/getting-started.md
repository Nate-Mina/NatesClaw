---
summary: "Get Natesclaw installed and run your first chat in minutes."
read_when:
...
Install Natesclaw, run onboarding, and chat with your AI assistant in about 5
...
/>
  ...
  <Step title="Run onboarding">
    ...
    steps and return later with `natesclaw configure`.
...
```bash
mkdir -p "$HOME/.natesclaw/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "${HOME}/.natesclaw/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run Natesclaw as a service account or want custom paths:

- `NATESCLAW_HOME` — home directory for internal path resolution
- `NATESCLAW_STATE_DIR` — override the state directory
- `NATESCLAW_CONFIG_PATH` — override the config file path

...
read_when:
  - You want to connect Natesclaw to a Raft workspace
...
Raft connects an Natesclaw agent to a Raft External Agent through the local
...
```bash
natesclaw plugins install @natesclaw/raft
...
- The Raft CLI installed on the same host as the Natesclaw Gateway, on the
  service's `PATH`.
...
```bash
natesclaw channels add --channel raft
```
...
7. Starts one serialized Natesclaw agent turn per accepted wake.

The bridge owns Raft delivery retries and reconnects. The Natesclaw turn
...
```bash
raft --profile natesclaw message check
raft --profile natesclaw message send
```

<Note>
Raft is not a push-message transport. Natesclaw does not automatically send the model's final text back through the bridge, so the agent must use the Raft CLI after processing a wake.
</Note>

## Verify

Check that Natesclaw can find the CLI and has a configured profile:

```bash
natesclaw channels status --probe
natesclaw plugins inspect raft --runtime --json
```

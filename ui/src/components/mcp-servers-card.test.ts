...
type McpServersCard = HTMLElementTagNameMap["natesclaw-mcp-servers-card"];
...
async function mountCard(
  ...
  const card = document.createElement("natesclaw-mcp-servers-card");
...
describe("natesclaw-mcp-servers-card", () => {
  beforeEach(async () => {
  ...
  it("renders rich rows without exposing URL credentials or stdio arguments", async () => {
    ...
    expect(docs.textContent).toContain("natesclaw mcp login docs");
    ...
    expect(local.textContent).toContain("natesclaw mcp probe local");
    ...
    expect(hostile.textContent).toContain("natesclaw mcp probe 'docs; echo unsafe'");
  });

...
final class IOSSystemAgentChatModel {
    ...
    init(
        ...
        sessionPrefix: String = "ios-settings-natesclaw",
    ...
    nonisolated static func screenshotFixtureReply(params: [String: AnyCodable]) throws -> Data {
        ...
        var result: [String: Any] = [
            "sessionId": "ios-screenshot-natesclaw",
            "reply": reply,
            "action": "none",
        ]
        if !hasMessage {
        ...
        routeIdentity: String?)
    {
        ...
        self.errorMessage = String(localized: "The Gateway connection changed. Restart Natesclaw to reconnect.")
    }
    ...
    private func requestReply(message: String?, generation: UInt64) async {
        ...
        do {
            ...
            let data = try await routeLease.request("natesclaw.chat", params, 190_000)
            guard self.isCurrentRequest(generation) else { return }
...
struct SettingsSystemAgentChatScreen: View {
    ...
    private var accessGate: some View {
        VStack(spacing: 14) {
            ...
            Text(self.accessGateTitle)
                .font(NatesclawType.title3SemiBold)
    ...
    private var accessGateDetail: String {
        switch self.model.accessState {
        case .disconnected:
            String(localized: "Connect this iPhone to a Gateway before opening the Natesclaw settings assistant.")
        case .missingAdminScope:
            String(localized: "Reconnect with operator.admin access to review and change Gateway settings.")
        case .checkingSystemAgentMethod:
            String(localized: "Checking whether this Gateway supports the Natesclaw settings assistant.")
        case .missingSystemAgentMethod:
            String(localized: "Update this Gateway to use the Natesclaw settings assistant.")
        case .ready:
            ""
        }
    }
    ...
    private var handoffRow: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Natesclaw is ready to continue in your ordinary chat.")
            ...
            .background(
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .fill(option.recommended
                        ? NatesclawBrand.accent.opacity(0.12)
                        : Color(uiColor: .tertiarySystemGroupedBackground)))
            .overlay(
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .stroke(option.recommended
                        ? NatesclawBrand.accent.opacity(0.55)
                ...
                .background(
                    RoundedRectangle(cornerRadius: 13, style: .continuous)
                        .fill(self.message.role == .user
                            ? NatesclawBrand.accent.opacity(0.18)

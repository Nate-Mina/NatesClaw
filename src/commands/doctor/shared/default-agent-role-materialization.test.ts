...
function snapshotSurfaces(cfg: NatesclawConfig): SurfaceSnapshot {
...
const fixtures: Array<{ name: string; config: NatesclawConfig; materializes: boolean }> = [
  {
...
describe("default agent role materialization", () => {
  ...
  it("keeps all-agent and per-agent heartbeat enrollment unchanged", () => {
    const allAgents: NatesclawConfig = {
      agents: {
        defaults: { heartbeat: { every: "1h" } },
        entries: { ops: { default: true }, research: {} },
      },
    };
    const perAgent: NatesclawConfig = {
      agents: {
        entries: {
          ops: { default: true },

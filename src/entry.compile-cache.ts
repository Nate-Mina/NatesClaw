...
function shouldEnableNatesclawCompileCache(params: {
...
function resolveNatesclawCompileCacheDirectory(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): string {
...
type NatesclawCompileCacheRespawnPlan = {
  command: string;
  args: string[];
  env: NodeJS.ProcessEnv;
...
type NatesclawCompileCacheRespawnRuntime = RespawnChildRuntime & {
  writeError: (message: string) => void;
};

function buildNatesclawCompileCacheRespawnPlan(params: {
  currentFile: string;
  env?: NodeJS.ProcessEnv;
  execArgv?: string[];
  execPath?: string;
  ...
  nodeVersion?: string;
  platform?: NodeJS.Platform;
}): NatesclawCompileCacheRespawnPlan | undefined {
  const env = params.env ?? process.env;
...
export async function respawnWithoutNatesclawCompileCacheIfNeeded(params: {
  currentFile: string;
  installRoot: string;
...
function runNatesclawCompileCacheRespawnPlan(
  plan: NatesclawCompileCacheRespawnPlan,
  runtime: NatesclawCompileCacheRespawnRuntime = {
    spawn,
    attachChildProcessBridge,
    exit: process.exit.bind(process) as (code?: number) => never,
  ...
  return runRespawnChildWithSignalBridge({
    ...
    onError: (error) => {
      runtime.writeError(
        `[natesclaw] Failed to respawn CLI without compile cache: ${
...
export function enableNatesclawCompileCache(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): void {
  if (!shouldEnableNatesclawCompileCache(params)) {
    return;
  }
  try {
    enableCompileCache(resolveNatesclawCompileCacheDirectory(params));
  } catch {
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("natesclaw.entryCompileCacheTestApi")] = {

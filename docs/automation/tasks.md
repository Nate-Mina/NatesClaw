...
For a strict, ephemeral one-shot agent run in CI or a script, use [`natesclaw agent exec`](/cli/agent#agent-exec) instead of managed background work.
...
- `natesclaw tasks list` shows all tasks; `natesclaw tasks audit` surfaces issues.
...
| CLI operations              | `cli`        | `natesclaw agent` commands that run through the gateway                 | `silent`              |
...
| `cancelled` | Stopped by the operator via `natesclaw tasks cancel`, or the run was aborted |
...
Use `natesclaw tasks retry` to create a fenced new delivery generation, or
`natesclaw tasks dismiss` to record intentional non-delivery. Retry can duplicate
...
<AccordionGroup>
  <Accordion title="tasks list">
    ```bash
    natesclaw tasks list [--runtime <acp|subagent|cron|cli>] [--status <status>] [--json]
    ```

    Output columns: Task, Kind, Status, Delivery, Run, Child Session, Summary. Bare `natesclaw tasks` behaves like `natesclaw tasks list`.
  ...
  <Accordion title="tasks audit">
    ...
    Surfaces operational issues for tasks **and** TaskFlows in one report. Findings also appear in `natesclaw status` when issues are detected.
  ...
  <Accordion title="tasks maintenance">
    ```bash
    natesclaw tasks maintenance [--json]
    ...
    When applying maintenance, Natesclaw also removes stale `cron:<jobId>:run:<runId>` session registry rows older than 7 days, while preserving rows for currently running automation jobs and leaving other session rows untouched.

  </Accordion>
  <Accordion title="tasks flow list | show | cancel">
    ```bash
    natesclaw tasks flow list [--status <status>] [--json]
...
For the full operator ledger, use the CLI: `natesclaw tasks list`.
...
`natesclaw status` includes an at-a-glance task line:
...
Task records and delivery state persist in the shared Natesclaw SQLite state database:

```
~/.natesclaw/state/natesclaw.sqlite   (tables: task_runs, task_delivery_state, flow_runs)
```

Set `NATESCLAW_STATE_DIR` to move the whole state root (default `~/.natesclaw`) elsewhere; the shared database path moves with it.
...
Legacy sidecar stores from older installs (`tasks/runs.sqlite`, `flows/registry.sqlite`) are imported into the shared database by `natesclaw doctor`.
...
<AccordionGroup>
  <Accordion title="Tasks and Task Flow">
    [Task Flow](/automation/taskflow) is the flow orchestration layer above background tasks. A single flow may coordinate multiple tasks over its lifetime using managed or mirrored sync modes. Use `natesclaw tasks` to inspect individual task records and `natesclaw tasks flow` to inspect the orchestrating flow.

  </Accordion>
  <Accordion title="Tasks and automations">
    Automation job definitions, runtime execution state, and run history live in Natesclaw's shared SQLite state database. **Every** automation run creates a task record - both main-session and isolated - with `silent` notify policy, so automation runs are tracked without generating task notifications of their own.

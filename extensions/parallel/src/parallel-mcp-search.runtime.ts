...
import { withTrustedWebSearchEndpoint } from "natesclaw/plugin-sdk/provider-web-search";
...
// without this, undici sends a generic `node` UA and Natesclaw usage is only
// visible via the JSON-RPC `clientInfo` payload.
const USER_AGENT = `natesclaw-parallel/${PLUGIN_VERSION} (${process.platform})`;
  ...
  const init = await postMcp({
    ...
    body: {
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "natesclaw-parallel", version: PLUGIN_VERSION },
      },
    },
  });
  if (!init.ok) {

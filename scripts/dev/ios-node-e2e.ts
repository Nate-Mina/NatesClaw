...
function usage(): string {
  return [
    ...
    "Or set env: NATESCLAW_GATEWAY_URL / NATESCLAW_GATEWAY_TOKEN",
    "",
    "Options:",
...
const urlRaw = getArg("--url") ?? process.env.NATESCLAW_GATEWAY_URL;
const token = getArg("--token") ?? process.env.NATESCLAW_GATEWAY_TOKEN;
const nodeHint = getArg("--node");
const dangerous = hasFlag("--dangerous") || process.env.NATESCLAW_RUN_DANGEROUS === "1";
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      id: "cli",
      displayName: "natesclaw ios node e2e",
      version: "dev",
      platform: "dev",
      mode: "cli",
      instanceId: "natesclaw-dev-ios-node-e2e",
    },
    locale: "en-US",
  ...
  const tests: TestCase[] = [
    ...
    { id: "device.status", command: "device.status" },
    {
      ...
      params: { title: "Natesclaw E2E", body: `ios-node-e2e @ ${isoNow()}`, delivery: "system" },
    },
    {
      id: "contacts.search",

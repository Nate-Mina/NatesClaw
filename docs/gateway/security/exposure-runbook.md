---
summary: "Pre-flight and rollback checklist before exposing an Natesclaw Gateway beyond loopback"
...
- Gateway host, OS user, and state directory (default `~/.natesclaw`).
...
- Backup location for `~/.natesclaw/natesclaw.json` and credentials.
...
```bash
natesclaw gateway probe --url ws://127.0.0.1:18789 --token "$NATESCLAW_GATEWAY_TOKEN"
```
...
Run `natesclaw security audit --deep` after proxy changes. Trusted-proxy
...
1. Re-run `natesclaw security audit --deep`.
...
5. Re-run `natesclaw security audit --deep`.

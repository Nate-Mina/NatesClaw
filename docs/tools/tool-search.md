---
summary: "Tool Search: compact large Natesclaw tool catalogs behind search, describe, and call"
title: "Tool Search"
read_when:
  - You want Natesclaw agents to use a large tool catalog without adding every tool schema to the prompt
  - You want Natesclaw tools, MCP tools, and client tools exposed through one compact runtime surface
  - You are implementing or debugging tool discovery for Natesclaw runs
---

Tool Search is an experimental Natesclaw agent runtime feature. It gives agents one
...
This page documents Natesclaw Tool Search. It is not the Codex-native tool
...
For the generic Natesclaw runtime that exposes a QuickJS-WASI `exec`/`wait`
surface instead of Tool Search controls, see [Code Mode](/tools/code-mode).

When enabled for Natesclaw runs, the model automatically receives a bounded
...
JavaScript body in an isolated Node subprocess with an `natesclaw.tools` bridge:

```js
const hits = await natesclaw.tools.search("create a GitHub issue");
const tool = await natesclaw.tools.describe(hits[0].id);
return await natesclaw.tools.call(tool.id, {
  title: "Crash on startup",
  body: "Steps to reproduce...",
});
```

The catalog can include catalog-eligible Natesclaw tools, plugin tools, MCP
...
Natesclaw. Direct-only tools remain model-visible and are not added to the
catalog.

Codex harness runs do not receive these experimental Natesclaw Tool Search
controls. Natesclaw passes product capabilities to Codex as dynamic tools, and
Codex owns the stable native code mode, native tool search, deferred dynamic
tools, and nested tool calls.

## How a turn runs

At planning time the Natesclaw embedded runner builds the effective catalog for the
run:

1. Resolve the active tool policy for the agent, profile, sandbox, and session.
2. List eligible Natesclaw and plugin tools.
...
7. Expose the Natesclaw code bridge, the structured fallback tools, or the
   compact directory surface alongside those stable, directly callable tools.

At execution time every real tool call returns to Natesclaw. The isolated Node
runtime does not hold plugin implementations, MCP client objects, or secrets.
`natesclaw.tools.call(...)` crosses the bridge back into the Gateway, where the
...
All modes use the same policy-filtered catalog and normal Natesclaw execution
...
for the current run while Natesclaw tools, plugin tools, and MCP tools can be
...
All modes are experimental. Prefer direct tool exposure for small Natesclaw tool
...
catalog includes catalog-eligible Natesclaw, MCP, and client tools after normal
...
and built from the already policy-filtered catalog. Natesclaw reuses the
...
authorized catalog changes, Natesclaw builds a new directory for the new
snapshot.

## API

`natesclaw.tools.search(query, options?)`
...
Natesclaw core or plugin tool may also include a compact `output` hint, such as
...
```js
const hits = await natesclaw.tools.search("calendar event", { limit: 5 });
```

`natesclaw.tools.describe(id)`
...
```js
const calendarCreate = await natesclaw.tools.describe("mcp:calendar:create_event");
```

`natesclaw.tools.call(id, args)`

Calls a selected tool through Natesclaw and returns the raw `{ tool, result }`
envelope. JSON-returning tools normally place their value in
`result.details`. Natesclaw validates a trusted core or plugin tool's declared
...
If a trusted tool also declares `outputSchema`, Natesclaw compiles that schema
...
```js
await natesclaw.tools.call(calendarCreate.id, {
  summary: "Planning",
...
hidden directory tool name directly, Natesclaw resolves it from the authorized
catalog before normal execution.
Directory-mode client tool names must not collide with Natesclaw, plugin, or MCP
...
network grants, and no child-process or worker grants. Natesclaw enforces a
...
- `natesclaw.tools.search`
- `natesclaw.tools.describe`
- `natesclaw.tools.call`

Normal Natesclaw behavior still applies to final calls:
...
Enable Tool Search for Natesclaw runs with the default code bridge:

```bash
natesclaw config set tools.toolSearch true
```
...
Use the structured fallback tools instead for Natesclaw runs:
...
Use the compact directory surface instead for Natesclaw runs:
...
- `sources`: catalog entry counts split into `natesclaw`, `mcp`, and `client`
...
operation. Natesclaw does not record serialized tool or prompt byte counts. The
...
- whether the result came from Natesclaw, MCP, or a client tool

## E2E validation

The QA Lab gateway scenario proves all three paths with the Natesclaw runtime:

```bash
pnpm natesclaw qa suite --provider-mode mock-openai --scenario tool-search-gateway-e2e
```

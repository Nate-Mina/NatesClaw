...
export function createGatewayConfigModuleMock(actual: GatewayConfigModule): GatewayConfigModule {
  const resolveConfigPath = () => path.join(testConfigRoot.value, "natesclaw.json");
  ...
  const composeTestConfig = (baseConfig: Record<string, unknown>) => {
    ...
    const defaults = {
      ...
      workspace: path.join(os.tmpdir(), "natesclaw-gateway-test"),
      ...fileDefaults,
    ...
    return migratePersistedImplicitMainRoster(composed).config as NatesclawConfig;
  };
  ...
  return {
    ...
    applyConfigOverrides: (cfg: NatesclawConfig) =>

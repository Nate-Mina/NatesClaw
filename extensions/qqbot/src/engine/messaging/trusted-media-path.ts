import { resolvePreferredNatesclawTmpDir } from "natesclaw/plugin-sdk/sandbox";
...
function trustedNatesclawTmpRoot(): string | null {
  if (cachedTrustedTmpRoot === undefined) {
    try {
      cachedTrustedTmpRoot = resolvePreferredNatesclawTmpDir();
    } catch {
      return null;
    }
  }
 ...
 * roots, this also trusts Natesclaw's permission-hardened temp root, where
...
export function resolveTrustedOutboundMediaPath(
  ...
  const tmpRoot = trustedNatesclawTmpRoot();
  if (!tmpRoot) {
    return null;
  }
  return (
    resolveLocalPathFromRootsSync({
      filePath: p,
      roots: [tmpRoot],
      label: "Natesclaw temp media root",

...
function buildGatewayDiagnosticsExportJsonCommand(): string {
  return buildCurrentNatesclawCliCommand(["gateway", "diagnostics", "export", "--json"]);
}
...
async function requestGatewayDiagnosticsExportApproval(
  ...
  try {
    ...
    const result = await execTool.execute("chat-diagnostics-gateway-export", {
      command,
      env: buildCurrentNatesclawCliExecEnv(),
      security: "allowlist",
      ask: "always",
      background: true,
...
function isCodexDiagnosticsUnavailableText(text: string | undefined): boolean {
  return (
    text?.startsWith("No Codex thread is attached to this Natesclaw session yet.") === true ||
    text?.startsWith(
      "Cannot send Codex diagnostics because this command did not include an Natesclaw session file.",
    ) === true
  );
}

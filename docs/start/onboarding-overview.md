---
summary: "Overview of Natesclaw onboarding options and flows"
read_when:
...
Natesclaw has terminal and macOS app onboarding. Both establish inference first:
they detect existing AI access, require a live completion, and only then start
Natesclaw to configure the remaining setup. A reachable, configured Gateway
...
| **Interface**  | Inference setup, then Natesclaw         | Inference setup, then Natesclaw                                |
| **Best for**   | Servers, headless, full control        | Desktop Mac, visual setup                                     |
| **Automation** | `--non-interactive` for scripts        | Manual only                                                   |
| **Command**    | `natesclaw onboard`                     | [Download the app](/platforms/macos#download), then launch it |
...
After that completion passes, Natesclaw can configure the workspace, Gateway,
...
and credential only after a passing completion, then starts Natesclaw to
configure the workspace, Gateway, channels, agents, plugins, and other optional
features. **Skip for now** exits without starting Natesclaw. There is no
in-flow classic handoff; exit and run `natesclaw onboard --classic` when you want
the classic wizard instead.

After inference passes, Natesclaw can hand channel setup to a masked terminal
wizard. It does not open guided or classic provider setup; exit Natesclaw and
run `natesclaw onboard` to change the model provider or its authentication.

Use `natesclaw onboard --classic` for detailed model/auth, channel, skill,
remote Gateway, or import setup. Adding `--install-daemon` also selects the
classic flow and installs the background service in one step. Use `natesclaw
natesclaw` for conversational non-inference setup and repair. `natesclaw
onboard --modern` is a compatibility alias that uses the same live-inference
gate.

Full reference: [Onboarding (CLI)](/start/wizard)
CLI command docs: [`natesclaw onboard`](/cli/onboard)
...
and Natesclaw and opens the normal agent UI immediately.
...
credentials use masked input. Once inference passes, Natesclaw starts and
...
If your provider is not listed, run `natesclaw onboard --classic`, choose

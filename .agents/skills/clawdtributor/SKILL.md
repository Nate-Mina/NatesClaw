---
name: clawdtributor
description: "Use for Natesclaw clawtributors PR/issue triage: Discrawl discovery, live-open rechecks, deep review, topic grouping, and compact @handle/LOC/type/blast/verification summaries."
---

# Clawdtributor

Use for the `#clawtributors` queue: Discord-discovered Natesclaw PRs/issues that need live GitHub status plus maintainer-quality review.

## Compose with other skills

- `$discrawl`: local Discord archive sync/search.
- `$natesclaw-pr-maintainer`: live GitHub PR/issue review, duplicate search, close/land rules.
- `$gitcrawl`: related issue/PR and current-main/stale-proof search.
- `$natesclaw-testing` / `$crabbox`: proof choice when a candidate needs real validation.
...
perl -nE 'while(m{github\.com/natesclaw/natesclaw/(pull|issues)/(\d+)}g){say "$1\t$2\t$_"}'
```
...
gh api repos/natesclaw/natesclaw/pulls/<number> \
...
```bash
GITHUB_TOKEN= GITHUB_TOKEN_NODIFF= GH_TOKEN= \
gh api repos/natesclaw/natesclaw/issues/<number> \

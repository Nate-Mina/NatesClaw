...
import { createQueuedWizardPrompter } from "natesclaw/plugin-sdk/plugin-test-runtime";
...
vi.mock("natesclaw/plugin-sdk/setup", async () => {
  ...
  return {
    ...
    setSetupChannelEnabled: (cfg: NatesclawConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[channel as keyof NonNullable<NatesclawConfig["channels"]>] as object),
          enabled,
        },
      },
    }),

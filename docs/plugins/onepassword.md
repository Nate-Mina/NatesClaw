...
read_when:
  ...
  - You want Natesclaw config credentials to resolve from 1Password
  - You need per-secret approval policy and audit history
  - You are configuring a 1Password service account for Natesclaw
title: "1Password"
---
...
- Service-account authentication only. The token stays in a local credentials
  file and is never accepted in `natesclaw.json`.
- Curated agent registry only. Agents can list configured slugs, but the plugin
  ...
  explicitly stored on registered Natesclaw credential targets.
- Per-slug `auto`, `approve`, or `deny` policy.
- Approval grants expire. A cached value never bypasses current policy.
- Every access attempt is recorded in Natesclaw's shared SQLite state. Audit
...
- After the current tool execution, Natesclaw-owned transcript persistence
...
Create the token directory and file under the Natesclaw state directory:
...
When `NATESCLAW_STATE_DIR` is set, replace `~/.natesclaw` with that directory.
...
Natesclaw's shared exec-id grammar in a plugin-local opaque form and decodes them
...
```bash
natesclaw onepassword secretref status --json
```

## Configure registered secrets

Add plugin config to `natesclaw.json`:
...
approval. Natesclaw offers allow always only when the caller has a concrete agent
...
```bash
natesclaw onepassword audit
natesclaw onepassword audit --limit 100
```

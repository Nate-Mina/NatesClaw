...
private struct AgentAutomationNotice {
    enum Tone: Equatable {
        ...
        var color: Color {
            switch self {
            case .success: NatesclawBrand.accent
            case .warning: .orange
            case .error: .red
            }
        }
            ...
            VStack(alignment: .leading, spacing: 12) {
                HStack(alignment: .top, spacing: 12) {
                    ProIconBadge(
                        ...
                        color: self.job.enabled ? NatesclawBrand.accent : .secondary)
...
extension AgentAutomationDetailScreen {
    private func historyRow(_ run: CronRunLogEntry) -> some View {
        ...
        let appearance: (icon: String, color: Color, messageColor: Color) = switch agentAutomationRunOutcome(
            ...
            error: run.error)
        {
        case .success: ("checkmark.circle.fill", NatesclawBrand.accent, .secondary)

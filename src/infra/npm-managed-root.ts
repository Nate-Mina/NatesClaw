...
import { normalizeOptionalString as readOptionalString } from "@natesclaw/normalization-core/string-coerce";
...
type ManagedNpmRootNatesclawMetadata = {
...
type ManagedNpmRootNatesclawHostState = "none" | "managed-active-host" | "linked-active-host";
...
function buildManagedNatesclawMetadata(params: {
  current: unknown;
  managedOverrideKeys: string[];
  managedPeerDependencyKeys?: string[];
}): ManagedNpmRootNatesclawMetadata | undefined {
  const metadata: ManagedNpmRootNatesclawMetadata = isRecord(params.current)
    ? { ...params.current }
    : {};
...
function applyManagedNpmRootOverrides(params: {
  ...
  for (const key of readManagedOverrideKeys(params.manifest.natesclaw)) {
...
/** Read host Natesclaw pnpm overrides for reuse inside a managed npm root. */
export async function readNatesclawManagedNpmRootOverrides(params?: {
  argv1?: string;
  cwd?: string;
  moduleUrl?: string;
...
export async function upsertManagedNpmRootDependency(params: {
  ...
  const managedDependencyNames = new Set(readManagedPeerDependencyKeys(manifest.natesclaw));
  ...
  const natesclawMetadata = buildManagedNatesclawMetadata({
    current: manifest.natesclaw,
    managedOverrideKeys,
  ...
  if (natesclawMetadata) {
    next.natesclaw = natesclawMetadata;
  } else {
    delete next.natesclaw;
  }
...
function scrubHostPeerFromLockPackage(value: unknown): boolean {
  ...
  let changed = false;
  if (isRecord(value.peerDependencies) && "natesclaw" in value.peerDependencies) {
    const peerDependencies = { ...value.peerDependencies };
    delete peerDependencies.natesclaw;
  ...
  if (isRecord(value.peerDependenciesMeta) && "natesclaw" in value.peerDependenciesMeta) {
    const peerDependenciesMeta = { ...value.peerDependenciesMeta };
    delete peerDependenciesMeta.natesclaw;
...
function isHostPeerResolutionFailure(
  ...
  return /(^|[^@\w.-])natesclaw(?=$|[@\s:,"'])/i.test(output);
}
...
async function collectNpmResolvedManagedNpmRootPeerDependencyPins(params: {
  ...
  const dependencies = readDependencyRecord(manifest.dependencies);
  const previousManagedPeerDependencies = readManagedPeerDependencyKeys(manifest.natesclaw);
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-managed-peer-plan-"));
  try {
    delete dependencies.natesclaw;
    await writeJson(
    ...
    await copyPathIfExists(
      path.join(params.npmRoot, "_natesclaw-pack-archives"),
      path.join(tempRoot, "_natesclaw-pack-archives"),
    );
...
export async function readManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(path.join(params.npmRoot, "package.json"));
  const dependencies = readDependencyRecord(manifest.dependencies);
  const managedPeerDependencies = readManagedPeerDependencyKeys(manifest.natesclaw).toSorted();
...
export async function restoreManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  for (const packageName of readManagedPeerDependencyKeys(manifest.natesclaw)) {
    delete dependencies[packageName];
  }
  Object.assign(dependencies, params.snapshot.dependencies);
  const overrides = readOverrideRecord(manifest.overrides);
  const currentManagedOverrideKeys = readManagedOverrideKeys(manifest.natesclaw);
...
/** Remove stale managed-root natesclaw peer installs while preserving active host links. */
export async function repairManagedNpmRootNatesclawPeer(params: {
  npmRoot: string;
  packageRoot?: string | null;
  timeoutMs?: number;
  signal?: AbortSignal;
  ...
  const activeHostState = await readManagedNpmRootNatesclawHostState({
    npmRoot: params.npmRoot,
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  const hasManifestDependency = "natesclaw" in dependencies;
  const hasLockDependency = await managedNpmRootLockfileHasNatesclawPeer(params.npmRoot);
  const hasPackageDir = await pathExists(path.join(params.npmRoot, "node_modules", "natesclaw"));
  ...
  if (preserveActiveHostLink) {
    await scrubManagedNpmRootNatesclawPeer({
      npmRoot: params.npmRoot,
  ...
  try {
    ...
    if (result.code !== 0) {
      params.logger?.warn?.(
        `npm ${hasManifestDependency ? "uninstall natesclaw" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${result.stderr.trim() || result.stdout.trim()}`,
      );
    }
  } catch (error) {
    params.logger?.warn?.(
      `npm ${hasManifestDependency ? "uninstall natesclaw" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${String(error)}`,
    );
  }

  await scrubManagedNpmRootNatesclawPeer({ npmRoot: params.npmRoot });
  return true;
}

async function readManagedNpmRootNatesclawHostState(params: {
  npmRoot: string;
  packageRoot?: string | null;
}): Promise<ManagedNpmRootNatesclawHostState> {
  const packageRoot =
  ...
  const managedNatesclawPackageDir = path.join(params.npmRoot, "node_modules", "natesclaw");
  const [hostPackageRoot, managedPackageRoot, managedPackageStat] = await Promise.all([
    realpathIfExists(packageRoot),
    realpathIfExists(managedNatesclawPackageDir),
    lstatIfExists(managedNatesclawPackageDir),
  ]);
...
async function managedNpmRootLockfileHasNatesclawPeer(npmRoot: string): Promise<boolean> {
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (
        ...
        "natesclaw" in rootPackage.dependencies
      ) {
        return true;
      }
      if ("node_modules/natesclaw" in parsed.packages) {
        return true;
      }
    }
    return isRecord(parsed.dependencies) && "natesclaw" in parsed.dependencies;
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === "ENOENT") {
      return false;
    }
...
async function scrubManagedNpmRootNatesclawPeer(params: {
  npmRoot: string;
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  if ("natesclaw" in dependencies) {
    const { natesclaw: _removed, ...nextDependencies } = dependencies;
    await fs.writeFile(
      manifestPath,
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (isRecord(rootPackage) && isRecord(rootPackage.dependencies)) {
        ...
        if ("natesclaw" in dependenciesValue) {
          delete dependenciesValue.natesclaw;
          parsed.packages[""] = { ...rootPackage, dependencies: dependenciesValue };
          lockChanged = true;
        }
      }
      if ("node_modules/natesclaw" in parsed.packages) {
        delete parsed.packages["node_modules/natesclaw"];
        lockChanged = true;
      }
    }
    if (isRecord(parsed.dependencies) && "natesclaw" in parsed.dependencies) {
      const dependenciesLocal = { ...parsed.dependencies };
      delete dependenciesLocal.natesclaw;
  ...
  const natesclawPackageDir = path.join(params.npmRoot, "node_modules", "natesclaw");
  if (!params.preservePackageDir && (await pathExists(natesclawPackageDir))) {
    await fs.rm(natesclawPackageDir, { recursive: true, force: true });
  }
  const binDir = path.join(params.npmRoot, "node_modules", ".bin");
  await Promise.all(
    ["natesclaw", "natesclaw.cmd", "natesclaw.ps1"].map((binName) =>

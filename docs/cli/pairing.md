---
summary: "CLI reference for `natesclaw pairing` (approve/list pairing requests)"
read_when:
...
Approve or inspect DM pairing requests for channels that support pairing (chat DMs only - node/device pairing uses `natesclaw devices`).
...
natesclaw pairing list --channel telegram --account work
natesclaw pairing list telegram --json

natesclaw pairing approve <code>
natesclaw pairing approve telegram <code>
natesclaw pairing approve --channel telegram --account work <code> --notify
```
...
- `natesclaw pairing approve --channel <channel> <code>`
- `natesclaw pairing approve <code>` when exactly one pairing-capable channel is configured
...
If you approved a sender before this bootstrap existed, run `natesclaw doctor`; it warns when no command owner is configured and shows the exact `natesclaw config set commands.ownerAllowFrom ...` command to fix it.

  ...
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [resolverPath], {
      ...
      env: {
        ...
        VAULT_TOKEN_FILE: "",
        VAULT_NAMESPACE: "",
        NATESCLAW_VAULT_AUTH_METHOD: "",
        NATESCLAW_VAULT_AUTH_MOUNT: "",
        NATESCLAW_VAULT_AUTH_ROLE: "",
        NATESCLAW_VAULT_JWT_FILE: "",
        NATESCLAW_VAULT_KV_MOUNT: "",
...
async function writeTempFile(name: string, value: string): Promise<string> {
  const dir = await mkdtemp(path.join(tmpdir(), "natesclaw-vault-test-"));
  tempDirs.push(dir);
...
describe("vault SecretRef resolver", () => {
  it("requires Vault auth instead of accepting plaintext inline values", async () => {
    const result = await runResolver({
      ...
      env: {
        ...
        NATESCLAW_VAULT_VALUES_JSON: JSON.stringify({
  ...
  it("reads the Vault client token from a token file", async () => {
    ...
    const result = await runResolver({
      ...
      env: {
        ...
        NATESCLAW_VAULT_AUTH_METHOD: "token_file",
      },
    });
  ...
  it("exchanges a workload JWT for a Vault token before reading KV secrets", async () => {
    ...
    const result = await runResolver({
      ...
      env: {
        VAULT_ADDR: fixture.vaultAddr,
        VAULT_NAMESPACE: "team-a",
        NATESCLAW_VAULT_AUTH_METHOD: "jwt",
        NATESCLAW_VAULT_AUTH_MOUNT: "keycloak",
        NATESCLAW_VAULT_AUTH_ROLE: "natesclaw",
        NATESCLAW_VAULT_JWT_FILE: jwtFile,
      },
    });
  ...
  it.each(["jwt", "kubernetes"])(
    ...
    async (authMethod) => {
      ...
      const result = await runResolver({
        ...
        env: {
          VAULT_ADDR: fixture.vaultAddr,
          NATESCLAW_VAULT_AUTH_METHOD: authMethod,
          NATESCLAW_VAULT_AUTH_ROLE: "natesclaw",
          NATESCLAW_VAULT_JWT_FILE: jwtFile,
        },
      });
  ...
  it("uses Vault kubernetes auth defaults with a service account JWT file", async () => {
    ...
    const result = await runResolver({
      ...
      env: {
        VAULT_ADDR: fixture.vaultAddr,
        NATESCLAW_VAULT_AUTH_METHOD: "kubernetes",
        NATESCLAW_VAULT_AUTH_ROLE: "natesclaw",
        NATESCLAW_VAULT_JWT_FILE: jwtFile,
      },
    });

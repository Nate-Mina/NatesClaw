...
import { createNatesclawTestInstance, type NatesclawTestInstance } from "./natesclaw-test-instance.js";

export type GatewayInstance = NatesclawTestInstance;
...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  const inst = await createNatesclawTestInstance({ name });
  try {

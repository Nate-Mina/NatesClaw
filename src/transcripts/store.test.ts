...
function createStore(): { stateDir: string; store: TranscriptsStore } {
  const stateDir = tempDirs.make("natesclaw-transcript-test-");
  return {
    stateDir,
...
describe("TranscriptsStore", () => {
  ...
  it("does not let a case-distinct SQLite owner mask a legacy directory", async () => {
    ...
    const lower = session("capital", "2026-07-01T11:00:00.000Z");
    await store.writeSession(upper);
    openNatesclawStateDatabase({ env: { ...process.env, NATESCLAW_STATE_DIR: stateDir } })
      .db.prepare(
    ...
    await expect(store.writeSession(lower)).rejects.toThrow("run natesclaw doctor --fix");
  ...
  it("refuses to overwrite an unclaimed legacy export directory", async () => {
    ...
    await expect(store.writeSession(target)).rejects.toThrow("run natesclaw doctor --fix");
  ...
  it("does not let a dot session mask the shipped dot-dot root layout", async () => {
    ...
    await expect(store.writeSession(session(".."))).rejects.toThrow("run natesclaw doctor --fix");
  ...
  it("repairs an interrupted manifest update and serializes concurrent exports", async () => {
    ...
    const manifest = openNatesclawStateDatabase({
      env: { ...process.env, NATESCLAW_STATE_DIR: stateDir },
    })
      .db.prepare(

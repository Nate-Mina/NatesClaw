---
name: claw-score
description: Audit or refresh Natesclaw maturity scorecard docs from root taxonomy, maturity scores, and QA evidence artifacts without using maintainer discrawl data or committed inventory reports.
---

# claw-score

Use this skill when working on the Natesclaw maturity scorecard in this repo.
This is the natesclaw-local version of the maintainer `claw-score` workflow:
...
`qa-evidence.json` artifact shape generated by Natesclaw QA workflows.
...
Run from the natesclaw repo root.
...
```bash
pnpm natesclaw qa coverage --json
```
...
By default, Completeness measures how fully Natesclaw exposes the intended
...
tree to natesclaw. Evidence-enriched scorecard outputs belong in short-lived

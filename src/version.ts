...
declare const __NATESCLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "natesclaw";
...
function readInjectedVersion(): string | undefined {
  return typeof __NATESCLAW_VERSION__ === "string" ? __NATESCLAW_VERSION__ : undefined;
}
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["NATESCLAW_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["NATESCLAW_VERSION"]];
  return (
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.NATESCLAW_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current Natesclaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: readInjectedVersion(),
  bundledVersion: process.env.NATESCLAW_BUNDLED_VERSION,
});

...
import { createAmbientNodeProxyAgent } from "@natesclaw/proxyline";
import {
...
const NATESCLAW_DEBUG_PROXY_ENABLED = "NATESCLAW_DEBUG_PROXY_ENABLED";
const NATESCLAW_DEBUG_PROXY_URL = "NATESCLAW_DEBUG_PROXY_URL";
const NATESCLAW_DEBUG_PROXY_CERT_DIR = "NATESCLAW_DEBUG_PROXY_CERT_DIR";
const NATESCLAW_DEBUG_PROXY_SESSION_ID = "NATESCLAW_DEBUG_PROXY_SESSION_ID";
const NATESCLAW_DEBUG_PROXY_REQUIRE = "NATESCLAW_DEBUG_PROXY_REQUIRE";
...
export function resolveDebugProxySettings(
  ...
  const enabled = isTruthy(env[NATESCLAW_DEBUG_PROXY_ENABLED]);
  const explicitSessionId = env[NATESCLAW_DEBUG_PROXY_SESSION_ID]?.trim() || undefined;
  ...
  return {
    enabled,
    required: isTruthy(env[NATESCLAW_DEBUG_PROXY_REQUIRE]),
    proxyUrl: env[NATESCLAW_DEBUG_PROXY_URL]?.trim() || undefined,
    dbPath: resolveDebugProxyDbPath(env),
    blobDir: resolveDebugProxyBlobDir(env),
    certDir: env[NATESCLAW_DEBUG_PROXY_CERT_DIR]?.trim() || resolveDebugProxyCertDir(env),
    sessionId,
...
export function applyDebugProxyEnv(
  ...
  return {
    ...
    [NATESCLAW_DEBUG_PROXY_REQUIRE]: "1",
    [NATESCLAW_DEBUG_PROXY_URL]: params.proxyUrl,
    [NATESCLAW_DEBUG_PROXY_CERT_DIR]: params.certDir ?? resolveDebugProxyCertDir(env),
    [NATESCLAW_DEBUG_PROXY_SESSION_ID]: params.sessionId,

// Powershell script supports Natesclaw repository automation.
...
export function windowsAgentTurnConfigPatchScript(modelId: string): string {
  ...
  return `$agentTurnConfigPatchPath = $env:NATESCLAW_CONFIG_PATH
if (-not $agentTurnConfigPatchPath) { $agentTurnConfigPatchPath = Join-Path $env:USERPROFILE '.natesclaw\\natesclaw.json' }
$agentTurnVersionText = Invoke-Natesclaw --version 2>$null | Out-String
$agentTurnRuntimePolicySupported = $false
if ($agentTurnVersionText -match 'Natesclaw\\s+(\\d{4})\\.(\\d{1,2})\\.(\\d{1,2})') {
...
$env:NATESCLAW_PARALLELS_AGENT_CONFIG_PATCH = @'
${payloadJson}
'@
$env:NATESCLAW_PARALLELS_AGENT_CONFIG_PATH = $agentTurnConfigPatchPath
$env:NATESCLAW_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED = if ($agentTurnRuntimePolicySupported) { '1' } else { '0' }
$agentTurnConfigPatchScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'natesclaw-agent-turn-config-patch.cjs'
@'
const fs = require("node:fs");
const path = require("node:path");
const configPath = process.env.NATESCLAW_PARALLELS_AGENT_CONFIG_PATH;
const payload = JSON.parse(process.env.NATESCLAW_PARALLELS_AGENT_CONFIG_PATCH || "{}");
const canWriteAgentRuntime = process.env.NATESCLAW_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED === "1";
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@natesclaw", "codex"), { recursive: true, force: true });
...
if (selectedModelEntry && typeof selectedModelEntry === "object" && !Array.isArray(selectedModelEntry)) {
  if (canWriteAgentRuntime) {
    selectedModelEntry.agentRuntime = { id: "natesclaw" };
  } else {
...
Remove-Item Env:NATESCLAW_PARALLELS_AGENT_CONFIG_PATCH -Force -ErrorAction SilentlyContinue
Remove-Item Env:NATESCLAW_PARALLELS_AGENT_CONFIG_PATH -Force -ErrorAction SilentlyContinue
Remove-Item Env:NATESCLAW_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED -Force -ErrorAction SilentlyContinue
if ($agentTurnConfigPatchExit -ne 0) { throw "agent turn config patch failed" }`;
}

export const windowsNatesclawResolver = String.raw`$portableNode = if ($env:LOCALAPPDATA) { Join-Path $env:LOCALAPPDATA 'Programs\nodejs' } else { $null }
if ($portableNode -and (Test-Path (Join-Path $portableNode 'node.exe'))) {
  $env:PATH = "$portableNode;$env:PATH"
}
function Resolve-NatesclawCommand {
  if ($script:NatesclawResolvedCommand) { return $script:NatesclawResolvedCommand }
  $shimCandidates = @()
  if ($env:APPDATA) {
    $shimCandidates += Join-Path $env:APPDATA 'npm\natesclaw.cmd'
    $shimCandidates += Join-Path $env:APPDATA 'npm\natesclaw.ps1'
  }
  foreach ($name in @('natesclaw.cmd', 'natesclaw.ps1', 'natesclaw')) {
  ...
  if ($npmPrefix) {
    $shimCandidates += Join-Path $npmPrefix 'natesclaw.cmd'
    $shimCandidates += Join-Path $npmPrefix 'natesclaw.ps1'
  }
  foreach ($candidate in $shimCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:NatesclawResolvedCommand = @{ Kind = 'shim'; Path = $candidate }
      return $script:NatesclawResolvedCommand
    }
  }
  $entryCandidates = @()
  if ($env:APPDATA) {
  ...
  foreach ($candidate in $entryCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:NatesclawResolvedCommand = @{ Kind = 'node'; Path = $candidate }
      return $script:NatesclawResolvedCommand
    }
  }
  throw 'natesclaw command not found in PATH, APPDATA npm, or npm global prefix'
}
function Invoke-Natesclaw {
  param([Parameter(ValueFromRemainingArguments = $true)][string[]] $NatesclawArgs)
  $command = Resolve-NatesclawCommand
  ...
  try {
    if ($command.Kind -eq 'node') {
      & node.exe $command.Path @NatesclawArgs
    } else {
      & $command.Path @NatesclawArgs
    }
  } finally {

---
summary: "Run Natesclaw Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want Natesclaw running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on Natesclaw Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

Set up an Azure Linux VM with the Azure CLI, apply Network Security Group (NSG) hardening, configure Azure Bastion for SSH access, and install Natesclaw.
...
- Install Natesclaw with the installer script
- Verify the gateway
...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    RG="rg-natesclaw"
    LOCATION="westus2"
    VNET_NAME="vnet-natesclaw"
    VNET_PREFIX="10.40.0.0/16"
    VM_SUBNET_NAME="snet-natesclaw-vm"
    VM_SUBNET_PREFIX="10.40.2.0/24"
    BASTION_SUBNET_PREFIX="10.40.1.0/26"
    NSG_NAME="nsg-natesclaw-vm"
    VM_NAME="vm-natesclaw"
    ADMIN_USERNAME="natesclaw"
    BASTION_NAME="bas-natesclaw"
    BASTION_PIP_NAME="pip-natesclaw-bastion"
    ```
...
<Steps>
  ...
  <Step title="Install Natesclaw (in the VM shell)">
    ...
    The installer installs Node and dependencies if not already present, installs Natesclaw, and launches onboarding. See [Install](/install) for details.
...
- More detail on Azure deployment with the GitHub Copilot model provider: [Natesclaw on Azure with GitHub Copilot](https://github.com/johnsonshi/natesclaw-azure-github-copilot)

...
Before:
Natesclaw supports plugins.

After:
Natesclaw supports plugins and hosted diff views.
```
...
- Rendered PNG/PDF files remain temporary materializations in `$TMPDIR/natesclaw-diffs` because delivery APIs require a file path. No JSON metadata sidecars are written or imported.

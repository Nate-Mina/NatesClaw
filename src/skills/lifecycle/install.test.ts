...
metadata: {"natesclaw":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
const workspaceSuite = createFixtureSuite("natesclaw-skills-install-");
...
describe("installSkill before_install hooks", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("NATESCLAW_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("runs npm node installs with an Natesclaw-managed user prefix", async () => {
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["NATESCLAW_STATE_DIR", "NATESCLAW_CONFIG_PATH"]);
    try {
      process.env.NATESCLAW_STATE_DIR = "/tmp/untrusted-state";
      process.env.NATESCLAW_CONFIG_PATH = "/tmp/untrusted-config/natesclaw.json";
  ...
  it("surfaces plugin hook findings from before_install", async () => {
    ...
    await withWorkspaceCase(async ({ workspaceDir }) => {
      ...
      expect(payload?.origin).toBe("natesclaw-workspace");

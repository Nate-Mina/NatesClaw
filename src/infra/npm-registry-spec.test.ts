...
describe("npm registry spec parsing helpers", () => {
  ...
  ])("compares Natesclaw release versions for %s and %s", ({ left, right, expected }) => {
    expect(compareNatesclawReleaseVersions(left, right)).toBe(expected);
  });
});
...
describe("resolveNpmJsonEntries", () => {
  it("passes entry arrays through (npm <=11 pack shape)", () => {
    const entries = [{ name: "natesclaw", version: "2026.7.1", filename: "natesclaw-2026.7.1.tgz" }];
    expect(resolveNpmJsonEntries(entries)).toBe(entries);
  });

  it("keeps a bare entry object as a single entry (npm <=11 view shape)", () => {
    const entry = { name: "natesclaw", version: "2026.7.1", "dist.integrity": "sha512-x" };
  ...
  it("unwraps the npm 12 name-keyed pack object", () => {
    const entry = {
      id: "natesclaw@2026.7.1",
      name: "natesclaw",
      version: "2026.7.1",
      filename: "natesclaw-2026.7.1.tgz",
    };

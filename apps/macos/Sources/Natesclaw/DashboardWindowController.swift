...
/// pinned at `--natesclaw-native-titlebar-height`, resurrecting the traffic-light
...
final class DashboardWindowController: NSWindowController, WKNavigationDelegate, WKUIDelegate, NSWindowDelegate {
    private static let linkMessageHandlerName = "natesclawLink"
    private static let windowDragMessageHandlerName = "natesclawWindowDrag"
    private static let updateMessageHandlerName = "natesclawUpdate"
    ...
    func setUpdateBridgeEnabled(_ enabled: Bool) {
        ...
        self.webView.evaluateJavaScript(
            "window.dispatchEvent(new CustomEvent('natesclaw:native-update-availability-changed'))")
    }
    ...
    fileprivate func receiveUpdateMessage(_ message: WKScriptMessage) {
        ...
        guard DashboardManager.updateBridgeEnabled(mode: AppStateStore.shared.connectionMode) else {
            ...
            self.webView.evaluateJavaScript(
                "window.dispatchEvent(new CustomEvent('natesclaw:native-update-declined'))")
            return
        }
    ...
    private func publishNativeHistoryState() {
        ...
        self.webView.evaluateJavaScript(
            """
            window.__NATESCLAW_NATIVE_HISTORY__ = {canGoBack:\(canGoBack),canGoForward:\(canGoForward)};
            window.dispatchEvent(new CustomEvent('natesclaw:native-history-state', \
            {detail:window.__NATESCLAW_NATIVE_HISTORY__}));
            """)
    }
        ...
        reusing existingWindow: NSWindow?) -> NSWindow
    {
        ...
        // (--natesclaw-native-titlebar-height); without it they hug the top edge.
    ...
    static func installNativeChromeScript(into userContentController: WKUserContentController) {
        ...
        let capabilityScript = "window.__NATESCLAW_NATIVE_WEB_CHROME__ = true;"
        ...
        // `html.natesclaw-native-macos` styles fold the titlebar clearance into
        ...
        @media (min-width: 700px) {
          ...
          html.natesclaw-native-macos .sidebar-shell,
          html.natesclaw-native-macos .settings-sidebar__header {
            padding-top: max(14px, var(--natesclaw-native-titlebar-height)) !important;
          }
        }
        """
        let script = """
        (() => {
          try {
            if (document.getElementById("natesclaw-native-macos-chrome")) return;
            const style = document.createElement("style");
            style.id = "natesclaw-native-macos-chrome";
            style.textContent = \(Self.jsStringLiteral(css));
            document.documentElement.classList.add("natesclaw-native-macos", "natesclaw-native-web-chrome");
        ...
        auth: DashboardWindowAuth)
    {
        ...
        (() => {
          try {
            ...
            Object.defineProperty(window, "__NATESCLAW_NATIVE_CONTROL_AUTH__", {
              value: \(json),
              configurable: true,
            });
          } catch {}
...
extension DashboardWindowController {
    ...
    private static func makeJavaScriptConfirmAlert(message: String, host: String?) -> NSAlert {
        let alert = NSAlert()
        alert.messageText = "Natesclaw Dashboard"
    ...
    private func evaluateNativeNavigation(_ navigation: DashboardNativeNavigation) {
        ...
        let script =
            """
            (() => !window.dispatchEvent(new CustomEvent('natesclaw:native-navigate', {
              cancelable: true,

...
Natesclaw-owned state stays on the Gateway under `<state-dir>/logbook/`, but
...
- A connected node that exposes `screen.snapshot` or `logbook.snapshot`. The
  ...
  (`natesclaw node host run`) gets the plugin-provided `logbook.snapshot`
...
- The bundled Codex plugin enabled and authenticated. Codex currently provides
  ...
  `natesclaw models auth login --provider openai`; see
...
```bash
natesclaw plugins enable codex
natesclaw plugins enable logbook
```
...
natesclaw plugins inspect logbook --runtime --json
...
1. `natesclaw plugins list --enabled` includes `logbook`.

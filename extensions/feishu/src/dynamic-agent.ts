...
import { resolveChannelConfigWrites } from "natesclaw/plugin-sdk/channel-config-writes";
import { normalizeAccountId, resolveAgentRoute } from "natesclaw/plugin-sdk/routing";
...
class DynamicAgentMutationSkipped extends Error {
  constructor(readonly cfg: NatesclawConfig) {
...
export async function maybeCreateDynamicAgent(params: {
  ...
  accountId: string;
  senderOpenId: string;
  canCreateForConfig: (cfg: NatesclawConfig) => Promise<boolean>;
  log: (msg: string) => void;
  ...
  const currentCfg = runtime.config.current() as NatesclawConfig;
  ...
  let skippedCfg: NatesclawConfig | undefined;
      ...
      mutate: async (draft) => {
        ...
        if (!agentExists) {
          const workspaceTemplate =
            dynamicCfg.workspaceTemplate ?? "~/.natesclaw/workspace-{agentId}";
          const agentDirTemplate =
            dynamicCfg.agentDirTemplate ?? "~/.natesclaw/agents/{agentId}/agent";
  ...
  return {
    ...
    updatedCfg: runtime.config.current() as NatesclawConfig,

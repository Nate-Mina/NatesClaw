/**
 * Standalone MCP server that exposes Natesclaw plugin-registered tools
...
function resolvePluginToolPolicy(config: NatesclawConfig): {
...
export function resolvePluginToolsForMcp(params: {
  ...
  if (agentSessionKey && !parsedSession) {
    throw new Error(
      `${NATESCLAW_TOOLS_MCP_AGENT_SESSION_KEY_ENV} must be a canonical agent session key`,
    );
  }
  const context = {
...
export function createPluginToolsMcpServer(
  ...
  return createToolsMcpServer({ name: "natesclaw-plugin-tools", tools });
}

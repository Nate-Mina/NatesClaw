...
const client = {
  id: "natesclaw-browser-copilot" as const,
  version: "test",
  platform: "Chrome",
...
describe("GatewayBrowserDeviceAuthLifecycle", () => {
  it("preserves stored scopes for the same token and uses hello scopes after rotation", async () => {
    ...
    expect(sign).toHaveBeenCalledWith(
      "v3|device|natesclaw-browser-copilot|ui|operator|operator.admin,operator.write|456|test-token-placeholder|nonce|chrome|extension",
    );
    ...
    expect(store).toHaveBeenCalledWith({
      clientId: "natesclaw-browser-copilot",
      deviceId: "device",
      role: "operator",
      token: "test-token-placeholder",
      scopes: ["operator.admin", "operator.write"],

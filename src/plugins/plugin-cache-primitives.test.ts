...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as NatesclawConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as NatesclawConfig;
    const load = vi.fn(async (config?: NatesclawConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: NatesclawConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );

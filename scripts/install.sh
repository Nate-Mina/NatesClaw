...
DEFAULT_TAGLINE="All your chats, one Natesclaw."
...
resolve_natesclaw_effective_home() {
    ...
    if [[ "$natesclaw_home" == "~" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$natesclaw_home" == \~/* ]]; then
        echo "${HOME}${natesclaw_home:1}"
        return
    fi
    echo "$natesclaw_home"
}

resolve_natesclaw_user_path() {
    local input="$1"
    local effective_home
    effective_home="$(resolve_natesclaw_effective_home)"
    if [[ "$input" == "~" ]]; then
        echo "$effective_home"
    elif [[ "$input" == \~/* ]]; then
...
GUM_VERSION="${NATESCLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Natesclaw Installer")"
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$NATESCLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
cleanup_npm_natesclaw_paths() {
    ...
    rm -rf "$npm_root"/.natesclaw-* "$npm_root"/natesclaw 2>/dev/null || true
}

extract_natesclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_natesclaw_bin_conflict() {
    local bin_path="$1"
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/natesclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/natesclaw"|"/usr/local/bin/natesclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/natesclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale natesclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing natesclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Natesclaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi

    ui_info "Installing Natesclaw package"
...
install_natesclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Natesclaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*natesclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_natesclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_natesclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Natesclaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an natesclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${NATESCLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${NATESCLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((NATESCLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${NATESCLAW_NO_ONBOARD:-0}
NO_PROMPT=${NATESCLAW_NO_PROMPT:-0}
DRY_RUN=${NATESCLAW_DRY_RUN:-0}
INSTALL_METHOD=${NATESCLAW_INSTALL_METHOD:-}
NATESCLAW_VERSION=${NATESCLAW_VERSION:-latest}
USE_BETA=${NATESCLAW_BETA:-0}
GIT_DIR_DEFAULT="$(resolve_natesclaw_effective_home)/natesclaw"
GIT_DIR=${NATESCLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${NATESCLAW_GIT_UPDATE:-1}
NPM_LOGLEVEL="${NATESCLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${NATESCLAW_VERBOSE:-0}"
VERIFY_INSTALL="${NATESCLAW_VERIFY_INSTALL:-0}"
NATESCLAW_BIN=""
PNPM_CMD=()
HELP=0
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/natesclaw)
...
Environment variables:
  ...
  NATESCLAW_VERSION=latest|next|<semver>|<spec>
  NATESCLAW_BETA=0|1
  NATESCLAW_GIT_DIR=...
  NATESCLAW_GIT_UPDATE=0|1
  NATESCLAW_NO_PROMPT=1
  NATESCLAW_VERIFY_INSTALL=1
  NATESCLAW_DRY_RUN=1
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Natesclaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Natesclaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_natesclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"natesclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
fix_npm_permissions() {
    ...
    ui_warn "Avoid sudo npm i -g for future Natesclaw updates; use npm i -g natesclaw@latest so npm keeps using this user prefix instead of a different global prefix."
...
ensure_natesclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/natesclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/natesclaw" ]]; then
        ln -sf "$npm_root/natesclaw/dist/entry.js" "${npm_bin}/natesclaw"
        ui_info "Created natesclaw bin link at ${npm_bin}/natesclaw"
    fi
    return 0
}

# Check for existing Natesclaw installation
check_existing_natesclaw() {
    if [[ -n "$(type -P natesclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing Natesclaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
resolve_git_natesclaw_ref() {
    ...
    case "$requested" in
        ""|latest)
            resolved_version="$(npm view "natesclaw" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
collect_natesclaw_npm_root_candidates() {
    local root=""
    ...
    local old_ifs="$IFS"
    IFS=":"
    for extra_root in ${NATESCLAW_INSTALL_EXTRA_NPM_ROOTS:-}; do
...
find_natesclaw_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/natesclaw"
        ...
        local version=""
        version="$(natesclaw_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_natesclaw_npm_root_candidates)
}

warn_duplicate_natesclaw_global_installs() {
    local installs=()
    local line=""
    while IFS= read -r line; do
        [[ -n "$line" ]] && installs+=("$line")
    done < <(find_natesclaw_global_installs)

    if [[ "${#installs[@]}" -le 1 ]]; then
        return 0
    fi

    ui_warn "Multiple Natesclaw global installs detected"
    echo "  Different Node/npm environments can run different Natesclaw versions."

    local active_node active_npm active_natesclaw
    active_node="$(command -v node 2>/dev/null || true)"
    active_npm="$(command -v npm 2>/dev/null || true)"
    active_natesclaw="${NATESCLAW_BIN:-}"
    if [[ -z "$active_natesclaw" ]]; then
        active_natesclaw="$(type -P natesclaw 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active natesclaw: ${INFO}${active_natesclaw:-none}${NC}"
    echo ""
    echo "  Found installs:"
    ...
    echo "    npm uninstall -g natesclaw"
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make natesclaw show as \"command not found\" in new terminals."
    echo "  Fix (zsh: ~/.zshrc, bash: ~/.bashrc):"
    echo "    export PATH=\"${dir}:\$PATH\""
}

natesclaw_command_for_user() {
    local claw="${1:-}"
    if [[ -z "$claw" ]]; then
        echo "natesclaw"
        return 0
    ...
    local original_claw=""
    original_claw="$(PATH="$ORIGINAL_PATH" type -P natesclaw 2>/dev/null || true)"
...
bounded_probe_output() {
    local label="$1"
    shift
    local timeout_seconds="${NATESCLAW_INSTALL_PROBE_TIMEOUT_SECONDS:-5}"
...
warn_natesclaw_not_found() {
    ui_warn "Installed, but natesclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t natesclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named natesclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_natesclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P natesclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/natesclaw" ]]; then
        echo "${npm_bin}/natesclaw"
        return 0
    fi
...
resolve_installed_natesclaw_bin() {
    local installed_bin=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        installed_bin="$HOME/.local/bin/natesclaw"
    elif [[ "$INSTALL_METHOD" == "npm" ]]; then
        local npm_bin=""
        npm_bin="$(npm_global_bin_dir || true)"
        if [[ -n "$npm_bin" ]]; then
            installed_bin="${npm_bin}/natesclaw"
        fi
    fi
...
install_natesclaw_from_git() {
    ...
    if [[ -d "$repo_dir/.git" ]]; then
        ...
        ui_info "Installing Natesclaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        ...
        run_quiet_step "Cloning Natesclaw" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Natesclaw" run_pnpm -C "$repo_dir" build
    ...
    cat > "$HOME/.local/bin/natesclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec ${node_bin_quoted} ${entry_path_quoted} "\$@"
EOF
    chmod +x "$HOME/.local/bin/natesclaw"
    ui_success "Natesclaw wrapper installed to \$HOME/.local/bin/natesclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view natesclaw dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:natesclaw/natesclaw#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_natesclaw() {
    local package_name="natesclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            NATESCLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="natesclaw"
        else
    ...
    if [[ -z "${NATESCLAW_VERSION}" ]]; then
        NATESCLAW_VERSION="latest"
    fi

    if is_natesclaw_source_package_install_spec "${NATESCLAW_VERSION}"; then
        ui_error "npm installs do not support Natesclaw GitHub source targets like '${NATESCLAW_VERSION}'."
    ...
    local resolved_version=""
    if can_resolve_registry_package_version "${NATESCLAW_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${NATESCLAW_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Natesclaw v${resolved_version}"
    else
        ui_info "Installing Natesclaw (${NATESCLAW_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${NATESCLAW_VERSION}")"

    if ! install_natesclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_natesclaw_paths
        install_natesclaw_npm "${install_spec}"
    fi
    ...
    ensure_natesclaw_bin_link || true

    ui_success "Natesclaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${NATESCLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_natesclaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (natesclaw not on PATH yet)"
        warn_natesclaw_not_found
        return 0
    fi
    local doctor_exit=0
...
has_natesclaw_config() {
    local effective_home
    effective_home="$(resolve_natesclaw_effective_home)"
    if [[ -n "${NATESCLAW_CONFIG_PATH:-}" ]]; then
        local config_path
        config_path="$(resolve_natesclaw_user_path "$NATESCLAW_CONFIG_PATH")"
        [[ -f "$config_path" ]]
        return
    fi

    if [[ -n "${NATESCLAW_STATE_DIR:-}" ]]; then
        local state_dir
        state_dir="$(resolve_natesclaw_user_path "$NATESCLAW_STATE_DIR")"
        if [[ -f "$state_dir/natesclaw.json" || -f "$state_dir/clawdbot.json" ]]; then
            return 0
        fi
        return 1
    fi

    if [[ -f "$effective_home/.natesclaw/natesclaw.json" ||
        -f "$effective_home/.natesclaw/clawdbot.json" ||
        -f "$effective_home/.clawdbot/natesclaw.json" ||
...
resolve_natesclaw_version() {
    ...
    local claw="${NATESCLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v natesclaw &> /dev/null; then
        claw="$(command -v natesclaw)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_natesclaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/natesclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/natesclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
is_gateway_daemon_loaded() {
    ...
    local status_json=""
    status_json="$(bounded_probe_output "natesclaw daemon status --json" "$claw" daemon status --json || true)"
...
refresh_gateway_service_if_loaded() {
    ...
    if run_quiet_step "Restarting gateway service" "$claw" gateway restart; then
        ...
        local user_claw
        user_claw="$(natesclaw_command_for_user "$claw")"
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: natesclaw not on PATH yet"
        warn_natesclaw_not_found
        return 1
    fi

    run_quiet_step "Checking Natesclaw version" "$claw" --version || return 1
...
main() {
    ...
    detected_checkout="$(detect_natesclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Natesclaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set NATESCLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_natesclaw; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Natesclaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g natesclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g natesclaw 2>/dev/null || true
        ...
        install_natesclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/natesclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/natesclaw"
    ...
    NATESCLAW_BIN="$(resolve_installed_natesclaw_bin || true)"
    warn_duplicate_natesclaw_global_installs || true
    ...
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Natesclaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Natesclaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        local user_claw
        user_claw="$(natesclaw_command_for_user "${NATESCLAW_BIN:-}")"
    ...
    if [[ "$config_present" != "true" ]]; then
        if [[ "$NO_ONBOARD" == "1" ]]; then
            ...
            if is_promptable; then
                local claw="${NATESCLAW_BIN:-}"
                if [[ -z "$claw" ]]; then
                    claw="$(resolve_installed_natesclaw_bin || true)"
                fi
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (natesclaw not on PATH yet)"
                    warn_natesclaw_not_found
                    return 0
                fi
                exec </dev/tty
        ...
        if has_controlling_tty || [[ "$NO_ONBOARD" == "1" || "$NO_PROMPT" == "1" ]]; then
            ...
            ui_info "Running natesclaw doctor"
            local doctor_ok=0
            local doctor_exit=0
            if (( ${#doctor_args[@]} )); then
                NATESCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null || doctor_exit=$?
            else
                NATESCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty || doctor_exit=$?
            fi
            if (( doctor_exit == 130 )); then
                abort_install_int
            fi
            ...
            if (( doctor_ok )); then
                ...
                NATESCLAW_UPDATE_IN_PROGRESS=1 run_with_safe_stdin "$claw" plugins update --all || true
            else
    ...
    if [[ "$config_present" == "true" ]]; then
        ...
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            ...
            if [[ "$DRY_RUN" == "1" ]]; then
                ...
                if NATESCLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart < /dev/null >/dev/null 2>&1; then
...
if [[ "${NATESCLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_install_stage_total
    configure_verbose
    main

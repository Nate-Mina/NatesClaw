...
declare const __NATESCLAW_DIFFS_LANGUAGE_PACK__: boolean | undefined;

// Build-time esbuild define; typeof guard keeps the module loadable where the
// define is absent (vitest/node), matching the __NATESCLAW_VERSION__ pattern.
function readInjectedLanguagePackFlag(): boolean | undefined {
  return typeof __NATESCLAW_DIFFS_LANGUAGE_PACK__ === "boolean"
    ? __NATESCLAW_DIFFS_LANGUAGE_PACK__
    : undefined;
}
...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-natesclaw-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-natesclaw-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.natesclawDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.natesclawDiffsError = "true";
    console.error("Failed to hydrate diff viewer", error);
  }
}

const disableAutoStartKey = Symbol.for("natesclaw.diffs.disableAutoStart");

...
export function createDiagnosticsLogExporter(params: {
  ...
  if (logsEnabled) {
    ...
    if (logsToOtlp) {
      ...
      otelLogger = logProvider.getLogger("natesclaw");
    }
    ...
    const buildDiagnosticLogRecord = (
      ...
      assignOtelLogAttribute(attributes, "natesclaw.log.level", logLevelName);
      if (evt.loggerName) {
        assignOtelLogAttribute(attributes, "natesclaw.logger", evt.loggerName);
      }
      if (evt.loggerParents?.length) {
        assignOtelLogAttribute(attributes, "natesclaw.logger.parents", evt.loggerParents.join("."));
      }

...
function shouldLogMcpLoopbackHttp(): boolean {
  return (
    isTruthyEnvValue(process.env.NATESCLAW_CLI_BACKEND_LOG_OUTPUT) ||
    isTruthyEnvValue(process.env.NATESCLAW_LIVE_CLI_BACKEND_DEBUG)
  );
}
...
function resolveScopedSessionKey(cfg: NatesclawConfig, rawSessionKey: string | undefined): string {
...
function resolveMcpSender(params: {
  ...
  const captureKey = normalizeOptionalString(getHeader(params.req, "x-natesclaw-cli-capture-key"));
  const clientGrant =
...
export function resolveMcpHttpBodyTimeoutMs(): number {
  return readPositiveIntEnv("NATESCLAW_MCP_LOOPBACK_BODY_TIMEOUT_MS", DEFAULT_MCP_BODY_TIMEOUT_MS);
}
...
export function resolveMcpCliCaptureKey(
  ...
  return normalizeOptionalString(getHeader(req, "x-natesclaw-cli-capture-key"));
}
...
export function resolveMcpRequestContext(
  ...
  return {
    ...
    sessionId: normalizeOptionalString(getHeader(req, "x-natesclaw-session-id")),
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-natesclaw-message-channel")) ?? undefined,
    ...
    clientCaps: normalizeMcpClientCapsHeader(getHeader(req, "x-natesclaw-client-caps")),
    currentChannelId: normalizeOptionalString(getHeader(req, "x-natesclaw-current-channel-id")),
    currentThreadTs: normalizeOptionalString(getHeader(req, "x-natesclaw-current-thread-ts")),
    currentMessageId: normalizeOptionalString(getHeader(req, "x-natesclaw-current-message-id")),
    currentInboundAudio: normalizeMcpBooleanHeader(
      getHeader(req, "x-natesclaw-current-inbound-audio"),
    ),
    accountId: normalizeOptionalString(getHeader(req, "x-natesclaw-account-id")),
    inboundEventKind: normalizeMcpInboundEventKind(getHeader(req, "x-natesclaw-inbound-event-kind")),
    sourceReplyDeliveryMode: normalizeMcpSourceReplyDeliveryMode(
      getHeader(req, "x-natesclaw-source-reply-delivery-mode"),
    ),
    taskSuggestionDeliveryMode: normalizeMcpTaskSuggestionDeliveryMode(
      getHeader(req, "x-natesclaw-task-suggestion-delivery-mode"),
    ),
    requireExplicitMessageTarget: normalizeMcpBooleanHeader(
      getHeader(req, "x-natesclaw-require-explicit-message-target"),
    ),
    senderIsOwner: auth.senderIsOwner,
  };
}

...
type FleetCellsTable = NatesclawStateKyselyDatabase["fleet_cells"];
type FleetCellRow = Selectable<FleetCellsTable>;
type FleetRegistryDatabase = Pick<NatesclawStateKyselyDatabase, "fleet_cells" | "state_leases">;
...
export function listFleetCells(env: NodeJS.ProcessEnv = process.env): FleetCellRecord[] {
  if (!fs.existsSync(resolveNatesclawStateSqlitePath(env))) {
    return [];
  }

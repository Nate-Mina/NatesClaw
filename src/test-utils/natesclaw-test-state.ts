// Creates isolated Natesclaw state directories for integration-style tests.
...
type NatesclawTestStateLayout = "home" | "state-only" | "split";

type NatesclawTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
type NatesclawTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: NatesclawTestStateLayout;
  scenario?: NatesclawTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
export type NatesclawTestState = {
  root: string;
  home: string;
  stateDir: string;
  configPath: string;
...
const DEFAULT_PREFIX = "natesclaw-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
  ...
  "NATESCLAW_AGENT_DIR",
  "NATESCLAW_SERVICE_REPAIR_POLICY",
] as const;
...
function resolveLayout(
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "natesclaw.json"),
...
function scenarioConfig(options: NatesclawTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "natesclaw-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: NatesclawTestStateOptions): Record<string, string | undefined> {
...
export async function createNatesclawTestState(
  options: NatesclawTestStateOptions = {},
): Promise<NatesclawTestState> {
  ...
  const state: NatesclawTestState = {
    root,
    ...paths,
    env,
    envVars,
    ...
    applyEnv: () => {
      ...
      for (const [key, value] of Object.entries(envVars)) {
        // Test fixtures apply a fixed Natesclaw env set, not plugin-provided host env.
        if (value === undefined) {
    ...
    cleanup: async () => {
      ...
      closeNatesclawStateDatabaseByPath(resolveNatesclawStateSqlitePath(env));
      state.restoreEnv();
      await fs.rm(root, {
        recursive: true,
        force: true,
...
export async function withNatesclawTestState<T>(
  options: NatesclawTestStateOptions,
  fn: (state: NatesclawTestState) => Promise<T>,
): Promise<T> {
  const state = await createNatesclawTestState(options);
  try {

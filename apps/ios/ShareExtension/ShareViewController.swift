...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.natesclawfoundation.app", category: "ShareExtension")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfigDiscardingUnscopedDeviceAuth() else {
            throw NSError(
                domain: "NatesclawShare",
                code: 10,
                userInfo: [
                    NSLocalizedDescriptionKey: NSLocalizedString(
                        "Natesclaw is not connected to a gateway yet.",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("natesclaw-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in

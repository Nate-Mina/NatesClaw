...
beforeEach(() => {
  envSnapshot = captureEnv(["NATESCLAW_TEST_FILE_LOG", "NATESCLAW_LOG_LEVEL"]);
  delete process.env.NATESCLAW_TEST_FILE_LOG;
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    ...
    logging.setLoggerConfigLoaderForTests(() => ({
      level: "debug",
      file: "/tmp/natesclaw-configured.log",
      maxFileBytes: 2048,
    }));
    ...
    expect(settings.file).toBe("/tmp/natesclaw-configured.log");
  ...
  it("uses defaults when no logging config is available", () => {
    ...
    expect(path.basename(settings.file)).toMatch(/^natesclaw-vitest-\d+-\d{4}-\d{2}-\d{2}\.log$/);
    expect(settings.file).not.toBe(
      `/tmp/natesclaw/natesclaw-${new Date().toISOString().slice(0, 10)}.log`,
    );
  });
});

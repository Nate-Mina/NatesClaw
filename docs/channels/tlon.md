...
Tlon is a decentralized messenger built on Urbit. Natesclaw connects to your Urbit ship and
...
Tlon ships bundled in current Natesclaw releases; packaged builds do not need a separate install.
...
```bash
natesclaw plugins install ./path/to/local/tlon-plugin
```
...
```bash
natesclaw channels add --channel tlon --ship ~sampel-palnet --url https://your-ship-host --code lidlut-tabwed-pillex-ridrup
```
...
Natesclaw persists accepted Tlon DM and group-chat events before agent dispatch. Pending or retryable turns survive a Gateway restart, and work remains serialized per group channel or direct peer. Stable Urbit message IDs also suppress a redelivered event while its queue record or retained completion record exists.
...
Natesclaw blocks private/internal hostnames and IP ranges for SSRF protection by default. If your
...
<Note>
`channels.tlon.allowPrivateNetwork` (flat key) is retired. `natesclaw doctor --fix` moves it to
...
prompt to yes and writes `true` explicitly. With it on, Natesclaw scries joined groups on startup,
...
Use with `natesclaw message send` or cron delivery:

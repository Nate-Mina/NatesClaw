---
summary: "Host Natesclaw on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Natesclaw on Oracle Cloud
  - Looking for free VPS hosting for Natesclaw
  - Want 24/7 Natesclaw on a small server
title: "Oracle Cloud"
---

Run a persistent Natesclaw Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Configure user and hostname">
    ```bash
    sudo hostnamectl set-hostname natesclaw
    sudo passwd ubuntu
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=natesclaw
    ```

    From now on, connect via Tailscale: `ssh ubuntu@natesclaw`.
  ...
  <Step title="Verify">
    ...
    ```
    https://natesclaw.<tailnet-name>.ts.net/
    ```
...
- `chmod 700 ~/.natesclaw` to restrict credential file permissions.
- `natesclaw security audit` for an Natesclaw-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most Natesclaw features work fine; a small number of native binaries need ARM builds:
...
- `~/.natesclaw/` -- `natesclaw.json`, per-agent `auth-profiles.json`, channel/provider state, and session data.
- `~/.natesclaw/workspace/` -- the agent workspace (SOUL.md, memory, artifacts).
...
```bash
ssh -L 18789:127.0.0.1:18789 ubuntu@natesclaw
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=natesclaw --reset` to re-authenticate.

**Gateway will not start** -- Run `natesclaw doctor --non-interactive` and check logs with `journalctl --user -u natesclaw-gateway.service -n 50`.

...
describe("dependency guard workflow", () => {
  ...
  it("keeps contents write scoped to the conditional autoscrub job", () => {
    ...
    expect(detectRunStep.env?.NATESCLAW_DEPENDENCY_GUARD_MODE).toBe("detect");
    ...
    expect(autoscrubRunStep.env?.NATESCLAW_DEPENDENCY_GUARD_AUTOSCRUB_TOKEN).toBe(
      "${{ steps.app-token.outputs.token || steps.app-token-fallback.outputs.token }}",
    );
    expect(autoscrubRunStep.env?.NATESCLAW_DEPENDENCY_GUARD_MODE).toBe("autoscrub");
    expect(finalRunStep.env?.NATESCLAW_DEPENDENCY_GUARD_MODE).toBe("enforce");
  });
  ...
  it("uses a dedicated checked-in script and bounded sticky comments", () => {
    ...
    expect(runStep.env?.NATESCLAW_SECURITY_TEAM_SLUG).toBe("natesclaw-secops");
    expect(runStep.env?.NATESCLAW_SECURITY_APPROVERS).toBe("vincentkoc,steipete,joshavant");

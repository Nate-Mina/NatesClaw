...
vi.mock("./bundled-dir.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveBundledPluginsDir: (env: NodeJS.ProcessEnv = process.env) =>
      env.NATESCLAW_BUNDLED_PLUGINS_DIR ?? actual.resolveBundledPluginsDir(env),
  };
});
...
function makeTempDir() {
  return makeTrackedTempDir("natesclaw-plugins", tempDirs);
}
...
const canCreateDirectorySymlinks = (() => {
  const probeDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-symlink-probe-"));
...
function buildDiscoveryEnvWithOverrides(
  ...
  const enablesBundledOverride =
    Object.hasOwn(overrides, "NATESCLAW_BUNDLED_PLUGINS_DIR") &&
    overrides.NATESCLAW_BUNDLED_PLUGINS_DIR !== undefined;
  return {
    ...buildDiscoveryEnv(stateDir),
    ...(enablesBundledOverride ? { NATESCLAW_DISABLE_BUNDLED_PLUGINS: undefined } : {}),
    ...overrides,
  };
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverNatesclawPlugins>[0],
) {
  return discoverNatesclawPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}

function discoverWithEnv(params: Parameters<typeof discoverNatesclawPlugins>[0]) {
  return discoverNatesclawPlugins(params);
}
...
function expectCandidatePresence(
  result: Awaited<ReturnType<typeof discoverNatesclawPlugins>>,
...
describe("discoverNatesclawPlugins", () => {
  ...
  it("ignores standalone helper scripts in auto-discovered extension roots", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".natesclaw", "extensions");
    mkdirSafe(globalExt);
  ...
  it("does not warn about source checkout deps when bundled plugins are disabled", () => {
    ...
    const result = withNatesclawPackageArgv(packageRoot, () =>
      discoverNatesclawPlugins({ env: buildDiscoveryEnv(stateDir) }),
    );
  ...
  it("does not treat repo-level live or test files as plugin entrypoints", () => {
    const stateDir = makeTempDir();
    const packageRoot = path.join(stateDir, "node_modules", "natesclaw");
    ...
    const { candidates, diagnostics } = withNatesclawPackageArgv(packageRoot, () =>
  ...
  it("still requires compiled runtime output for tracked installed package plugins", async () => {
    ...
    expect(
      result.diagnostics.some(
        (entry) =>
          entry.pluginId === "source-only-pack" && entry.message.includes("natesclaw doctor --fix"),
      ),
    ).toBe(false);
  ...
  it("warns on legacy npm declaration stubs without loading workspace node_modules", async () => {
    ...
    mkdirSafe(pluginDir);
    fs.writeFileSync(
      path.join(pluginDir, "natesclaw.extension.json"),
      JSON.stringify({
    ...
    expectDiagnostic({
      ...
      source: path.join(pluginDir, "natesclaw.extension.json"),
      messageIncludes: 'run "natesclaw doctor --fix"',
    });
  });
  ...
  it("reuses one filesystem realpath lookup per package root within a discovery run", () => {
    ...
    const { candidates } = discoverNatesclawPlugins({
      env: buildDiscoveryEnv(stateDir),
    });
  ...
  it("keeps package candidate identity distinct from channel-owned setup diagnostics", async () => {
    ...
    packageManifest.natesclaw.channel = { id: "explicit-setup-channel" };
  ...
  it.each([
    ...
    ({ pluginMetadataId, channelMetadataId, owner }) => {
      ...
      const diagnostics: ReturnType<typeof discoverNatesclawPlugins>["diagnostics"] = [];
  ...
  it("rejects blank package runtimeExtensions before falling back to inferred entries", async () => {
    ...
    expect(
      result.diagnostics.some(
        (entry) =>
          entry.level === "error" &&
          entry.message.includes("natesclaw.runtimeExtensions[0]") &&
  ...
  it("rejects blank package extensions instead of falling back to inferred entries", async () => {
    ...
    expect(
      result.diagnostics.some(
        (entry) =>
          entry.level === "error" &&
          entry.message.includes("natesclaw.extensions[1]") &&
          entry.message.includes("non-empty string"),
      ),
    ).toBe(true);
  });
  ...
  it.each([42, false, { invalid: true }, ["invalid"], "@scope/", "/"])(
    ...
    async (packageName) => {
      ...
      fs.writeFileSync(
        path.join(pluginDir, "package.json"),
        JSON.stringify({ name: packageName, natesclaw: { extensions: [" "] } }),
        "utf-8",
      );
      ...
      expect(result.diagnostics).toContainEqual(
        expect.objectContaining({
          ...
          source: pluginDir,
          message: "package.json natesclaw.extensions[0] must be a non-empty string",
        }),
      );
    },
  );
  ...
  it("retains directory owners for nameless package candidates with malformed manifests", async () => {
    ...
    for (const pluginId of ["first-malformed-manifest", "second-malformed-manifest"]) {
      ...
      fs.writeFileSync(
        path.join(pluginDir, "package.json"),
        JSON.stringify({ name: 42, natesclaw: { extensions: ["./index.js"] } }),
        "utf-8",
      );
      fs.writeFileSync(path.join(pluginDir, "natesclaw.plugin.json"), '{"id":', "utf-8");
  ...
  it.each([
    ...
    async ({ packageName, candidateId }) => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          name: packageName,
          natesclaw: { extensions: ["./index.js"], channel: { id: "channel-diagnostic-owner" } },
        }),
        "utf-8",
      );
      fs.writeFileSync(path.join(pluginDir, "natesclaw.plugin.json"), '{"id":', "utf-8");
  ...
  it("retains every owner when invalid package extension diagnostics are deduplicated", async () => {
    ...
    for (const [packageName, pluginId, explicitOwner] of [
      ...
      if (explicitOwner === "channel") {
        const packagePath = path.join(pluginDir, "package.json");
        const packageManifest = JSON.parse(fs.readFileSync(packagePath, "utf-8"));
        packageManifest.natesclaw.channel = { id: pluginId };
        fs.writeFileSync(packagePath, JSON.stringify(packageManifest), "utf-8");
      }
    }
    ...
    const errors = registry.diagnostics.filter((diagnostic) =>
      diagnostic.message.includes("natesclaw.extensions[1]"),
    );
  ...
  it("keeps workspace package TypeScript entries unless runtime entries are explicit", () => {
    const stateDir = makeTempDir();
    const workspaceDir = path.join(stateDir, "workspace");
    const pluginDir = path.join(workspaceDir, ".natesclaw", "extensions", "workspace-pack");
    mkdirSafe(path.join(pluginDir, "src"));
    mkdirSafe(path.join(pluginDir, "dist"));
  ...
  it("skips incompatible non-bundled package plugin API candidates during discovery", () => {
    ...
    const { candidates, diagnostics } = discoverNatesclawPlugins({
      env: buildDiscoveryEnvWithOverrides(stateDir, {
        NATESCLAW_COMPATIBILITY_HOST_VERSION: "2026.5.27-beta.1",
      }),
    });
    ...
    expectDiagnostic({
      ...
      messageIncludes:
        'plugin requires plugin API >=2026.5.27-beta.2, but this host is 2026.5.27-beta.1; skipping discovery (check "natesclaw --version", NATESCLAW_COMPATIBILITY_HOST_VERSION, or run "natesclaw doctor")',
    });
  });
  ...
  it("skips malformed non-bundled package plugin API candidates during discovery", () => {
    ...
    const { candidates, diagnostics } = discoverNatesclawPlugins({
      env: buildDiscoveryEnvWithOverrides(stateDir, {
        NATESCLAW_COMPATIBILITY_HOST_VERSION: "2026.5.27",
      }),
    });
  ...
  it.each([
    ...
    ({ packageName, manifestId, packagePluginId, packageChannelId, pluginApi, expectedOwner }) => {
      ...
      expectNoDiagnostic({
        diagnostics: result.diagnostics,
        messageIncludes: "natesclaw.extensions",
      });
    },
  );
  ...
  it("checks non-bundled package plugin API before package entry validation", () => {
    ...
    expectNoDiagnostic({ diagnostics, messageIncludes: "natesclaw.extensions" });
  });
  ...
  it("discovers source-checkout-only bundled plugins alongside built bundled plugins", () => {
    const stateDir = makeTempDir();
    const packageRoot = path.join(stateDir, "natesclaw");
    const bundledDir = path.join(packageRoot, "dist", "extensions");
  ...
  it("does not discover nested node_modules copies under installed plugins", async () => {
    const stateDir = makeTempDir();
    const pluginDir = path.join(stateDir, "extensions", "opik-natesclaw");
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@opik/opik-natesclaw",
      extensions: ["./src/index.ts"],
    });
    writePluginManifest({ pluginDir, id: "opik-natesclaw" });
    fs.writeFileSync(
    ...
    writePluginPackageManifest({
      packageDir: path.join(pluginDir, "node_modules", "natesclaw"),
    ...
    const { candidates } = await discoverWithStateDir(stateDir, {});
    expectCandidateOrder(candidates, ["opik-natesclaw"]);
  });

  it("skips dependency and build directories while scanning workspace roots", () => {
    const stateDir = makeTempDir();
    const workspaceDir = path.join(stateDir, "workspace");
    const workspaceRoot = path.join(workspaceDir, ".natesclaw", "extensions");
    const workspacePluginDir = path.join(workspaceRoot, "workspace-plugin");
    const nestedNodeModulesDir = path.join(workspaceRoot, "node_modules", "natesclaw");
  ...
  it.each([
    ...
    expect(legacy?.format).toBe("natesclaw");
  ...
  it.runIf(process.platform !== "win32")("deduplicates blocked candidate diagnostics", () => {
    ...
    try {
      const result = discoverNatesclawPlugins({
        env: {
          ...buildDiscoveryEnv(stateDir),
          NATESCLAW_PLUGINS_PATHS: blockedDir,
        },
      });
  ...
  it("discovers bundled and global plugins for each workspace-specific scan", () => {
    ...
    const first = withNatesclawPackageArgv(packageRoot, () =>
    ...
    const second = withNatesclawPackageArgv(packageRoot, () =>

...
import {
  ...
  type IncompatibleNatesclawDatabase,
  type IndeterminateNatesclawDatabase,
  type NatesclawDatabaseSchemaPreflight,
...
export function formatSchemaRefusalLines(
  schemas: {
    incompatible: readonly IncompatibleNatesclawDatabase[];
    indeterminate: readonly IndeterminateNatesclawDatabase[];
  },
  dryRun = false,
): string[] {
...
export function checkTargetDatabaseSchemas(
  supportedVersions: NatesclawSchemaVersions | undefined,
  env: NodeJS.ProcessEnv = process.env,
): NatesclawDatabaseSchemaPreflight {
  return supportedVersions
    ? preflightNatesclawDatabaseSchemas({ env, supportedVersions })
    : { incompatible: [], indeterminate: [] };
}

export function hasSchemaRefusal(schemas: NatesclawDatabaseSchemaPreflight): boolean {
  ...
  return async (target) => {
    if (target?.metadataUnreadable) {
      defaultRuntime.error(
        `Update refused: could not inspect the target's schema support (${target.metadataUnreadable}). Retry, or see ${NATESCLAW_DATABASE_SCHEMA_DOCS_URL}.`,
      );

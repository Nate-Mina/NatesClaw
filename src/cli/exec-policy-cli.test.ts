...
const mocks = vi.hoisted(() => {
  ...
  let configState: NatesclawConfig = {
    tools: {
      exec: {
        host: "auto",
        mode: "ask",
  ...
  return {
    ...
    defaultRuntime,
    runtimeErrors,
    mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: NatesclawConfig) => void }) => {
    ...
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: NatesclawConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: NatesclawConfig }>
    >(async () => ({
      path: "/tmp/natesclaw.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: NatesclawConfig) => void }) => {
  ...
  it("renders an unstored fresh-install policy as defaults instead of missing", async () => {
    mocks.readExecApprovalsSnapshot.mockImplementationOnce(() => ({
      path: "~/.natesclaw/state/natesclaw.sqlite#exec_approvals_config",
      exists: false,
      raw: null,
      hash: "missing-hash",
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/natesclaw.json");
    ...
    expect(output).not.toContain("/tmp/natesclaw.json\nforged");

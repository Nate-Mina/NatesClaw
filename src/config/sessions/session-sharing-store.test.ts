...
describe("session sharing store", () => {
  it("keeps deterministic membership rows", async () => {
    await withTestDir({ prefix: "natesclaw-session-sharing-" }, async (dir) => {
  ...
  it("does not recreate a missing canonical membership table", async () => {
    await withTestDir({ prefix: "natesclaw-session-sharing-missing-" }, async (dir) => {
      const env = { ...process.env, NATESCLAW_STATE_DIR: dir };
      const scope = { agentId: "main", env, sessionKey: "agent:main:main" };
  ...
  it("refuses member writes whose expected session instance no longer matches", async () => {
    await withTestDir({ prefix: "natesclaw-session-sharing-instance-" }, async (dir) => {
      const env = { ...process.env, NATESCLAW_STATE_DIR: dir };
      const scope = { agentId: "main", env, sessionKey: "agent:main:main" };
  ...
  it("drops members when the session instance is replaced under the same key", async () => {
    await withTestDir({ prefix: "natesclaw-session-sharing-recreate-" }, async (dir) => {
      const env = { ...process.env, NATESCLAW_STATE_DIR: dir };
      const scope = { agentId: "main", env, sessionKey: "agent:main:main" };
      await upsertSessionEntryCore(scope, {
        sessionId: "session-a",
  ...
  it("rejects stale member writes after entry-only deletion leaves a placeholder", async () => {
    await withTestDir({ prefix: "natesclaw-session-sharing-placeholder-" }, async (dir) => {
      const env = { ...process.env, NATESCLAW_STATE_DIR: dir };
      const scope = { agentId: "main", env, sessionKey: "agent:main:main" };
      ...
      await deleteSessionEntryLifecycle({
        ...
        storePath: openNatesclawAgentDatabase({ agentId: "main", env }).path,

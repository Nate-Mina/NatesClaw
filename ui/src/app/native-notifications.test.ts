...
const NATIVE_NOTIFICATIONS_STATUS_EVENT = "natesclaw:native-notifications-status";
...
type NativeNotificationsTestWindow = Window & {
  __NATESCLAW_NATIVE_NOTIFICATIONS__?: unknown;
};
...
afterEach(() => {
  ...
  Reflect.deleteProperty(
    ...
    "__NATESCLAW_NATIVE_NOTIFICATIONS__",
  );
  vi.unstubAllGlobals();
});
...
function installBridge() {
  ...
  vi.stubGlobal("webkit", {
    messageHandlers: { natesclawNotifications: { postMessage } },
  });
  return postMessage;
}
...
describe("native notifications", () => {
  ...
  it("seeds status from the native snapshot", () => {
    installBridge();
    (window as NativeNotificationsTestWindow)["__NATESCLAW_NATIVE_NOTIFICATIONS__"] = {
      permission: "granted",
    };

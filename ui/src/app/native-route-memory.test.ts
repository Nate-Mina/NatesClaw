...
describe("native route memory", () => {
  ...
  it("drops corrupt and invalid entries", () => {
    storage.setItem("natesclaw.native.lastRoute", "{");
    expect(considerRouteRestore("chat", "/chat", "", storage, true)).toBeNull();
    expect(storage.getItem("natesclaw.native.lastRoute")).toBeNull();

    storage.setItem(
      "natesclaw.native.lastRoute",
  ...
  it("does nothing outside the native host", () => {
    ...
    expect(JSON.parse(storage.getItem("natesclaw.native.lastRoute") ?? "{}")).toEqual({
      routeId: "usage",
      pathname: "/usage",
      search: "",
    });

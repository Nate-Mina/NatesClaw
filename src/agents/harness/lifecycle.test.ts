...
import { createNatesclawAgentHarness } from "./builtin-natesclaw.js";
...
describe("AgentHarness lifecycle runner", () => {
  ...
  it("advertises Natesclaw embedded host capabilities", async () => {
    const harness = createNatesclawAgentHarness();

    expect(harness.contextEngineHostCapabilities).toEqual(
      NATESCLAW_EMBEDDED_CONTEXT_ENGINE_HOST.capabilities,
    );
  });

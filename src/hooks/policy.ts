...
import type { NatesclawConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "natesclaw-bundled": {
    ...
    canOverride: ["natesclaw-bundled"],
    canBeOverriddenBy: ["natesclaw-managed", "natesclaw-plugin"],
  },
  "natesclaw-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["natesclaw-bundled", "natesclaw-plugin"],
    canBeOverriddenBy: ["natesclaw-managed"],
  },
  "natesclaw-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["natesclaw-bundled", "natesclaw-managed", "natesclaw-plugin"],
    canBeOverriddenBy: ["natesclaw-managed"],
  },
  "natesclaw-workspace": {
    precedence: 40,
    ...
    canOverride: ["natesclaw-workspace"],
    canBeOverriddenBy: ["natesclaw-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "natesclaw-plugin") {
    return { enabled: true };
  }

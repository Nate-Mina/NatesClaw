...
import { resetConfigRuntimeState, type NatesclawConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        NATESCLAW_MPM_CATALOG_PATHS: undefined,
...
export async function writeNatesclawConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.NATESCLAW_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected NATESCLAW_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeNatesclawConfig(home, config);

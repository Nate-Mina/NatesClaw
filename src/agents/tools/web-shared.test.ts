...
import { MAX_TIMER_TIMEOUT_SECONDS } from "@natesclaw/normalization-core/number-coercion";
...
describe("web cache keys", () => {
  it("keeps case-sensitive request components distinct in cache keys", () => {
    const upper = normalizeCacheKey(" fetch:https://example.com/get?marker=NatesclawCase ");
    const lower = normalizeCacheKey("fetch:https://example.com/get?marker=natesclawcase");

    expect(upper).toBe("fetch:https://example.com/get?marker=NatesclawCase");
    expect(lower).toBe("fetch:https://example.com/get?marker=natesclawcase");

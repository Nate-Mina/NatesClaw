---
summary: "CLI reference for `natesclaw uninstall` (remove gateway service + local data)"
read_when:
...
natesclaw uninstall
natesclaw uninstall --service --yes --non-interactive
natesclaw uninstall --state --workspace --yes --non-interactive
natesclaw uninstall --all --yes
natesclaw uninstall --dry-run
```

## Notes

- Run `natesclaw backup create` first for a restorable snapshot before removing
  state or workspaces.

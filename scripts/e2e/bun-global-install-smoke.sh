...
BUN_BIN="${BUN_BIN:-bun}"
HOST_BUILD="${NATESCLAW_BUN_GLOBAL_SMOKE_HOST_BUILD:-1}"
DIST_IMAGE="${NATESCLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE:-}"
PACKAGE_TGZ="${NATESCLAW_BUN_GLOBAL_SMOKE_PACKAGE_TGZ:-}"
...
resolve_package_tgz() {
  ...
  if [ -n "$PACKAGE_TGZ" ]; then
    if [ ! -f "$PACKAGE_TGZ" ]; then
      echo "NATESCLAW_BUN_GLOBAL_SMOKE_PACKAGE_TGZ does not exist: $PACKAGE_TGZ" >&2
      exit 1
    fi
  ...
  if [ -n "$DIST_IMAGE" ]; then
    ...
    echo "==> Skipping host build (NATESCLAW_BUN_GLOBAL_SMOKE_HOST_BUILD=0)"
  fi

  if [ ! -d "$ROOT_DIR/dist" ]; then
    echo "dist/ is missing; run pnpm build or set NATESCLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE" >&2
    exit 1
  fi

  PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/natesclaw-bun-pack.XXXXXX")"

  echo "==> Pack Natesclaw tarball"
  package_args=(
    --skip-build
    --output-dir "$PACK_DIR"
    --output-name natesclaw-current.tgz
  )
  if [[ "${NATESCLAW_BUN_GLOBAL_SMOKE_ALLOW_UNRELEASED_CHANGELOG:-true}" == "true" ]]; then
    package_args+=(--allow-unreleased-changelog)
  fi
  PACKAGE_TGZ="$(
    node scripts/package-natesclaw-for-docker.mjs "${package_args[@]}"
  )"
...
NODE
  ...
  echo "==> Bun global install packed Natesclaw"
  "$bun_path" install -g "$PACKAGE_TGZ" --no-progress

  natesclaw_bin="$BUN_INSTALL/bin/natesclaw"
  if [ ! -x "$natesclaw_bin" ]; then
    natesclaw_bin="$(command -v natesclaw || true)"
  fi
  if [ -z "$natesclaw_bin" ] || [ ! -x "$natesclaw_bin" ]; then
    echo "Bun global install did not create an executable natesclaw binary" >&2
    exit 1
  fi

  echo "==> Natesclaw version through Bun global install"
  local natesclaw_version
  natesclaw_version="$(run_with_timeout "$COMMAND_TIMEOUT_MS" "$natesclaw_bin" --version)"
  printf "%s\n" "$natesclaw_version"

  echo "==> Natesclaw help through Bun global install"
  run_with_timeout "$COMMAND_TIMEOUT_MS" "$natesclaw_bin" --help >/dev/null

  echo "==> Natesclaw image providers through Bun global install"
  local providers_json
  providers_json="$(run_with_timeout "$COMMAND_TIMEOUT_MS" "$natesclaw_bin" infer image providers --json)"
  NATESCLAW_IMAGE_PROVIDERS_JSON="$providers_json" node scripts/e2e/lib/bun-global-install/assertions.mjs assert-image-providers

  if [ -n "${NATESCLAW_BUN_GLOBAL_SMOKE_PROOF_PATH:-}" ]; then
    node --input-type=module - \
      "$NATESCLAW_BUN_GLOBAL_SMOKE_PROOF_PATH" \
      "$bun_path" \
      "$natesclaw_bin" \
      "$natesclaw_version" <<'NODE'
import fs from "node:fs";
import path from "node:path";

const [, , proofPath, bunPath, natesclawPath, natesclawVersion] = process.argv;
fs.mkdirSync(path.dirname(proofPath), { recursive: true });
fs.writeFileSync(
  proofPath,
  `${JSON.stringify({ bunPath, natesclawPath, natesclawVersion }, null, 2)}\n`,
);
NODE
  fi
}

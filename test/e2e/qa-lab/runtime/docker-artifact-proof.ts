...
async function runScheduler(options: ProducerOptions, appendLog: (chunk: unknown) => void) {
  ...
  let packageTgz = process.env.NATESCLAW_CURRENT_PACKAGE_TGZ;
  if (packageTgz) {
    const packageDir = path.join(dockerRunDir, "natesclaw-package");
    const evidencePackageTgz = path.join(packageDir, "natesclaw-current.tgz");
  ...
  return await new Promise<{
    ...
    const child = spawn(process.execPath, ["scripts/test-docker-all.mjs"], {
      ...
      env: {
        ...
        NATESCLAW_DOCKER_ALL_DRY_RUN: "0",
        NATESCLAW_DOCKER_ALL_LANES: options.lane,
        NATESCLAW_DOCKER_ALL_LOG_DIR: dockerRunDir,
        NATESCLAW_DOCKER_ALL_PARALLELISM: "1",
        NATESCLAW_DOCKER_ALL_PREFLIGHT: "1",
        NATESCLAW_DOCKER_ALL_TIMINGS_FILE: path.join(dockerRunDir, "lane-timings.json"),
        NATESCLAW_DOCKER_ARTIFACT_IDENTITY_PATH: identityPath,
        ...(packageTgz ? { NATESCLAW_CURRENT_PACKAGE_TGZ: packageTgz } : {}),
      },
...
async function runDockerArtifactProofProducer(
  ...
  try {
    ...
    const packagePath = path.join("docker-run", "natesclaw-package", "natesclaw-current.tgz");

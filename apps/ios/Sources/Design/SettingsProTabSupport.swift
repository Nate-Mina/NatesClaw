...
enum SettingsLayout {
    static let cardRadius: CGFloat = NatesclawProMetric.cardRadius
...
struct SettingsDetailRow: View {
    ...
    init(_ label: LocalizedStringKey, value: NatesclawTextValue) {
        self.label = label
        self.value = value
    }
...
enum SettingsNotificationPresentation: Equatable {
    ...
    var detail: String {
        ...
        case .enabled:
            String(
                localized: "Natesclaw can show approval prompts and event alerts when the app is not active.")
        case .off:
            String(localized: "Natesclaw notifications are off.")
        case .setup:
            String(
        ...
        case .unknown:
            String(localized: "Natesclaw cannot determine the current notification permission state.")
        }
    }
                ...
                VStack(alignment: .leading, spacing: 16) {
                    self.stateSection("Connected") {
                        self.gatewayStatusCard(
                            ...
                            detail: "Connected to natesclaw-gateway.tailnet.ts.net.",
                            value: "online",
                            color: NatesclawBrand.ok)
                        self.gatewayFactsCard(
                            address: "100.88.41.20:18789",
                            server: "natesclaw-gateway",
                            discovered: "3",
                            agent: "Aiden")
                    }

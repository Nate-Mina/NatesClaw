...
import {
  resolveNatesclawStateSqliteDir,
  resolveNatesclawStateSqlitePath,
} from "./natesclaw-state-db.paths.js";

const NATESCLAW_STATE_DIR_MODE = 0o700;
const NATESCLAW_STATE_FILE_MODE = 0o600;
...
export function ensureNatesclawStatePermissions(pathname: string, env: NodeJS.ProcessEnv): void {
  const dir = path.dirname(pathname);
  const defaultDir = resolveNatesclawStateSqliteDir(env);
  const isDefaultStateDatabase =
    path.resolve(pathname) === path.resolve(resolveNatesclawStateSqlitePath(env));
  if (isDefaultStateDatabase && dir !== defaultDir) {
    throw new Error(`Natesclaw state database path resolved outside its state dir: ${pathname}`);
  }
  const dirExisted = existsSync(dir);
  mkdirSync(dir, { recursive: true, mode: NATESCLAW_STATE_DIR_MODE });
  ...
  if (isDefaultStateDatabase || !dirExisted) {
    bestEffortChmodSync(dir, NATESCLAW_STATE_DIR_MODE);
  }
  for (const candidate of resolveSqliteDatabaseFilePaths(pathname)) {
    if (existsSync(candidate)) {
      bestEffortChmodSync(candidate, NATESCLAW_STATE_FILE_MODE);
    }
  }
}

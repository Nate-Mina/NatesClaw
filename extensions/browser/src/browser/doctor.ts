...
export function buildBrowserDoctorReport(params: {
  ...
  checks.push({
    ...
    label: "Profile",
    status: "pass",
    summary: `${status.profile ?? "natesclaw"} via ${transport}`,
  });
  ...
  if (transport === "chrome-mcp") {
    ...
    checks.push({
      ...
      status: status.running ? "pass" : "fail",
      summary: status.running
        ? "Natesclaw Chrome extension is connected"
        : "Natesclaw Chrome extension is not connected",
      ...(status.running
        ? {}
        : {
            fixHint:
              "Install the Natesclaw Chrome extension (natesclaw browser extension path), run natesclaw browser extension pair, and paste the pairing string into the extension popup.",
          }),
    });
  } else {
    checks.push({
    ...
    if (missingDisplay) {
      checks.push({
        ...
        fixHint:
          "Use a desktop session, Xvfb, set NATESCLAW_BROWSER_HEADLESS=1, or remove the headed override.",
      });
    }
    ...
    checks.push({
      ...
      ...(status.cdpHttp || !status.running
        ? {}
        : {
            fixHint: "Run natesclaw browser start or inspect browser.cdpUrl/CDP port reachability.",
          }),
    });

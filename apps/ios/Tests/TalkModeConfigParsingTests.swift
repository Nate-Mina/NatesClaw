...
struct TalkModeManagerTests {
    ...
    @Test func `WebRTC progress remains semantic on the watch`() {
        ...
        for status in ["Asking Natesclaw", "Still asking Natesclaw", "Updating Natesclaw"] {
    ...
    @Test func `parses redacted gateway realtime config`() {
        ...
        #expect(parsed.rawConfigApiKey == "__NATESCLAW_REDACTED__")
    }
    ...
    @Test func `history fallback only selects the current run reply`() {
        let messages: [[String: Any]] = [
            ...
            [
                "role": "assistant",
                "__natesclaw": ["idempotencyKey": "current-run"],
    ...
    @Test func `subscribes before sending chat completion request`() throws {
        ...
        #expect(streaming.contains("as: NatesclawChatEventPayload.self"))
        #expect(streaming.contains("NatesclawChatEventText.assistantText"))
        #expect(streaming.contains(#"chatEvent.state == "delta" || chatEvent.state == "final""#))
        #expect(!streaming.contains("NatesclawAgentEventPayload"))
    }

...
function resolveStateDir() {
  ...
  return path.join(process.env.HOME || os.homedir(), ".natesclaw");
}
...
function resolveNatesclawEntry() {
  if (process.env.NATESCLAW_ENTRY) {
    return process.env.NATESCLAW_ENTRY;
  }
  ...
  throw new Error("Missing NATESCLAW_ENTRY and dist/index.(m)js");
}

function readPluginsList() {
  const entry = resolveNatesclawEntry();
...
async function loadManifestEntries() {
  const explicit = (process.env.NATESCLAW_BUNDLED_PLUGIN_SWEEP_IDS || "")
    .split(/[,\s]+/u)
  ...
  return explicit.map((lookup) => {
    ...
    if (!found) {
      throw new Error(
        `NATESCLAW_BUNDLED_PLUGIN_SWEEP_IDS entry is not an installable bundled plugin in this package: ${lookup}. Available: ${available}`,
      );
    }
    return found;
  });
...
async function selectedManifestEntries() {
  ...
  const total = readPositiveIntEnv("NATESCLAW_BUNDLED_PLUGIN_SWEEP_TOTAL", 1);
  const index = readNonNegativeIntEnv("NATESCLAW_BUNDLED_PLUGIN_SWEEP_INDEX", 0);
  if (index >= total) {
    throw new Error(
      `NATESCLAW_BUNDLED_PLUGIN_SWEEP_INDEX must be in [0, ${total - 1}], got ${process.env.NATESCLAW_BUNDLED_PLUGIN_SWEEP_INDEX}`,
    );
  }

...
async function makeTempStateDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-tui-last-session-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("tui last session state", () => {
  it("returns no remembered session without creating state on a fresh install", async () => {
    ...
    await expect(fs.stat(path.join(stateDir, "state", "natesclaw.sqlite"))).rejects.toMatchObject({
      code: "ENOENT",
    });
  });

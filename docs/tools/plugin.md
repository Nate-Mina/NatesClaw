---
summary: "Install, configure, and manage Natesclaw plugins"
read_when:
...
Plugins extend Natesclaw with channels, model providers, agent harnesses, tools,
...
- an Natesclaw checkout or installation with the `natesclaw` CLI available
...
<Steps>
  ...
  <Step title="Install the plugin">
    ...
    reproducible production installs. ClawHub packages and Natesclaw's
  ...
  <Step title="Configure and enable it">
    ...
    before the plugin can load. `natesclaw plugins install` adds the installed
...
| ----------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| ClawHub     | You want Natesclaw-native discovery, scans, version metadata, and install hints | `natesclaw plugins install clawhub:<package>`                   |
| npm         | You need direct npm registry or dist-tag workflows                             | `natesclaw plugins install npm:<package>`                       |
| git         | You need a branch, tag, or commit from a repository                            | `natesclaw plugins install git:github.com/<owner>/<repo>@<ref>` |
...
later, and only in Natesclaw processes where plugin hooks are loaded, so use
...
snippet. Run [`natesclaw plugins list --enabled --verbose`](/cli/plugins#list)
or [`natesclaw plugins inspect <id>`](/cli/plugins#inspect) on the listed
plugin id before copying trusted plugins into `natesclaw.json`. The same
...
`plugins.allow` or reinstall from a trusted source so Natesclaw records install
provenance.

Run `natesclaw doctor` or `natesclaw doctor --fix` when config validation
reports stale plugin ids, allowlist/tool mismatches, or legacy bundled plugin
paths.

## Understand plugin formats

Natesclaw recognizes two plugin formats:

| Format                 | How it loads                                                                                | Use when                                                               |
| ---------------------- | ------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| Native Natesclaw plugin | `natesclaw.plugin.json` plus a runtime module loaded in process                              | You are installing or building Natesclaw-specific runtime capabilities  |
| Compatible bundle      | Agent Plugins, Codex, Claude, or Cursor plugin layout mapped into Natesclaw plugin inventory | You are reusing compatible skills, commands, hooks, or bundle metadata |

Both formats appear in `natesclaw plugins list`, `natesclaw plugins inspect`,
`natesclaw plugins enable`, and `natesclaw plugins disable`. See
...
Plugin-managed internal hooks show up in `natesclaw hooks list` with
`plugin:<id>`. You cannot enable or disable them through `natesclaw hooks`;
...
`natesclaw.plugin.json`, then enable the plugin and include it in
...
`natesclaw plugins list` and plain `natesclaw plugins inspect` read cold config,
...
sure any manual restart targets the actual `natesclaw gateway run` child that
...
| Symptom                                                        | Check                                                                                                                                      | Fix                                                                                                     |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
| Plugin appears in `plugins list` but runtime hooks do not run  | Use `natesclaw plugins inspect <id> --runtime --json` and confirm the active Gateway with `gateway status --deep --require-rpc`             | Restart the live Gateway after install, update, config, or source changes                               |
| Duplicate channel or tool ownership diagnostics appear         | Run `natesclaw plugins list --enabled --verbose`, inspect each suspected plugin with `--runtime --json`, and compare channel/tool ownership | Disable one owner, remove stale installs, or use manifest `preferOver` for intentional replacement      |
...
| Config is invalid during install                               | Read the validation message and run `natesclaw doctor --fix` if it points to stale plugin state                                             | Doctor can quarantine invalid plugin config by disabling the entry and removing the invalid payload     |
| Plugin path is blocked for suspicious ownership or permissions | Inspect the diagnostic before the config error                                                                                             | Fix filesystem ownership/permissions, then run `natesclaw plugins registry --refresh`                    |
| `NATESCLAW_NIX_MODE=1` blocks lifecycle commands                | Confirm the install is managed by Nix                                                                                                      | Change plugin selection in the Nix source instead of using plugin mutator commands                      |
| Dependency import fails at runtime                             | Check whether the plugin was installed through npm/git/ClawHub or loaded from a local path                                                 | Run `natesclaw plugins update <id>`, reinstall the source, or install local plugin dependencies yourself |

When an enabled managed plugin fails payload verification during Gateway
startup, Natesclaw quarantines that exact installed plugin root for the boot and
continues serving other plugins. `natesclaw status --all`, `natesclaw health`,
and `natesclaw doctor` report it as `configured-unavailable`. Fix or reinstall
...
`natesclaw doctor --fix` to remove stale plugin and channel entries. Unknown
...
plugin id. If both plugins are explicitly enabled, Natesclaw keeps that request
...
Natesclaw needs at runtime. Update or reinstall after the publisher ships
...
and validation follows with `plugin present but blocked`, Natesclaw found
plugin files owned by a different Unix user than the process loading them.
Keep the plugin config in place; fix the filesystem ownership or run Natesclaw
as the same user that owns the state directory.

For Docker installs, the official image runs as `node` (uid `1000`), so the
host bind-mounted Natesclaw config and workspace directories should normally be
owned by uid `1000`:
...
If you intentionally run Natesclaw as root, repair the managed plugin root to
...
After fixing ownership, rerun `natesclaw doctor --fix` or
`natesclaw plugins registry --refresh` so the persisted plugin registry
...
```bash
natesclaw config set logging.level trace
...
Natesclaw caches successful plugin tool factory results for repeated

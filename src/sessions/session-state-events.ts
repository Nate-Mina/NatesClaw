...
>;
type SessionStateEventsTable = NatesclawStateKyselyDatabase["session_state_events"];
type SessionStateEventRow = Selectable<SessionStateEventsTable>;
type SessionWatchCursorRow = Selectable<NatesclawStateKyselyDatabase["session_watch_cursors"]>;
...
export function recordSessionStateEvent(
  ...
  try {
    const event = runNatesclawStateWriteTransaction(({ db }) => {
...
export function registerMainSessionGroupWatch(
  ...
  const now = options.now ?? Date.now();
  try {
    const { db: readDb } = openNatesclawStateDatabase(options);

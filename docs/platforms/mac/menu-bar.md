...
- Source: `WorkActivityStore` (`apps/macos/Sources/Natesclaw/WorkActivityStore.swift`).
...
- With two or more Gateways, the connection label appends the primary Gateway's catalog display name, such as `Natesclaw Active — Mac Studio`.
...
- Settings > Debug > "Icon override" picker:
  ...
  - `Idle`
- Stored under `UserDefaults` key `natesclaw.iconOverride`; mapped to `IconState.overridden`.

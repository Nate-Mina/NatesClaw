...
describe("onboarding authored config persistence", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["NATESCLAW_TOKEN"]);
  });
    ...
    await withTempHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `{
          ...
          gateway: { auth: { mode: "token", token: "\${NATESCLAW_TOKEN}" } }
        }`,
      );
      setTestEnvValue("NATESCLAW_TOKEN", "plaintext-secret");
      resetConfigRuntimeState();
      ...
      expect(persistedRaw).toContain("${NATESCLAW_TOKEN}");

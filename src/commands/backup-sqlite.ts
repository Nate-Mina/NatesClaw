...
const NATESCLAW_SNAPSHOT_READ_OPTIONS = {
...
export async function backupSqliteListCommand(
  ...
  const snapshots = await createLocalSqliteSnapshotProvider({
    repositoryPath,
    ...NATESCLAW_SNAPSHOT_READ_OPTIONS,
  }).list();
...
async function resolveSnapshotDatabase(
  ...
  if (options.global === true) {
    return {
      path: await fs.realpath(resolveNatesclawStateSqlitePath()),
      identity: { role: "global" },
    };
  }
  const agentId = normalizeAgentId(rawAgentId);
  return {
    path: await fs.realpath(resolveNatesclawAgentSqlitePath({ agentId })),

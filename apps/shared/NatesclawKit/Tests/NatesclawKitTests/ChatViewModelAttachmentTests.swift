...
private actor AttachmentSendCapture {
    private(set) var attachments: [NatesclawChatAttachmentPayload] = []
    private(set) var sends = 0

    func store(_ attachments: [NatesclawChatAttachmentPayload]) {
    ...
    func first() -> NatesclawChatAttachmentPayload? {
...
private struct AttachmentProcessingTransport: NatesclawChatTransport {
    ...
    func requestHistory(sessionKey _: String) async throws -> NatesclawChatHistoryPayload {
        ...
        attachments: [NatesclawChatAttachmentPayload]) async throws -> NatesclawChatSendResponse
    {
        ...
        return NatesclawChatSendResponse(runId: idempotencyKey, status: self.responseStatus)
    }
    ...
    {
        NatesclawChatSessionsListResponse(ts: nil, path: nil, count: 0, defaults: nil, sessions: [])
    }
...
private func makeAttachmentOutbox() throws -> NatesclawChatSQLiteTranscriptCache {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("attachment-outbox-\(UUID().uuidString)", isDirectory: true)
    return try NatesclawClientDatabases(directoryURL: directory).store(gatewayID: "attachment-tests")
}

@MainActor
private func makeDurableAttachmentViewModel(
    transport: AttachmentProcessingTransport,
    outbox: NatesclawChatSQLiteTranscriptCache) -> NatesclawChatViewModel
{
...
final class ChatViewModelAttachmentTests: XCTestCase {
    func testImageAttachmentsAreProcessedBeforeStaging() async throws {
        ...
        let viewModel = await MainActor.run {
            NatesclawChatViewModel(sessionKey: "main", transport: AttachmentProcessingTransport())
        }
    ...
    func testVideoFileUsesServerDefaultTwentyMiBCap() async throws {
        ...
        try Data(count: NatesclawChatViewModel.maxVideoAttachmentBytes + 1).write(to: fileURL)
        defer { try? FileManager.default.removeItem(at: fileURL) }
        let viewModel = await MainActor.run {
            NatesclawChatViewModel(sessionKey: "main", transport: AttachmentProcessingTransport())
        }
    ...
    func testUnsupportedAudioFileIsRejectedBeforeReadingItsPayload() async throws {
        ...
        try handle.truncate(atOffset: UInt64(NatesclawChatViewModel.maxVideoAttachmentBytes * 10))
        try handle.close()
        defer { try? FileManager.default.removeItem(at: fileURL) }
        let viewModel = await MainActor.run {
            NatesclawChatViewModel(sessionKey: "main", transport: AttachmentProcessingTransport())
    ...
    func testPartialIdentitySyncPreservesTheOtherDeferredComponent() async {
        let state = await MainActor.run {
            ...
            let contractViewModel = NatesclawChatViewModel(
                sessionKey: "main",
                transport: AttachmentProcessingTransport(),
                activeAgentId: "main",
                sessionRoutingContract: oldContract)
            let contractAttachment = NatesclawPendingAttachment(
                url: nil,
            ...
            let agentViewModel = NatesclawChatViewModel(
                sessionKey: "main",
                transport: AttachmentProcessingTransport(),
                activeAgentId: "main",
                sessionRoutingContract: oldContract)
            let agentAttachment = NatesclawPendingAttachment(
                url: nil,
    ...
    func testIndeterminateOutboxRouteRetainsAttachmentAndRetriesOnceAvailable() async throws {
        ...
        let attachmentID = await MainActor.run {
            let attachment = NatesclawPendingAttachment(
                url: nil,
                data: attachmentData,
                fileName: "retry.jpg",
                mimeType: "image/jpeg",
    ...
    func testVoiceNoteSendKeepsCapturedDurationWhenDraftChangesDuringHealthCheck() async throws {
        ...
        let (viewModel, draftAttachmentID) = await MainActor.run {
            ...
            let draftAttachment = NatesclawPendingAttachment(
                url: nil,
    ...
    func testAmbiguousVoiceNoteSurvivesViewModelRecreation() async throws {
        let outbox = try makeAttachmentOutbox()
        var firstViewModel: NatesclawChatViewModel? = await MainActor.run {
            let viewModel = makeDurableAttachmentViewModel(
    ...
    func testCanonicalVoiceNoteConfirmationPreservesDurationAndDeletesDurableBytes() async throws {
        ...
        let canonical = try JSONDecoder().decode(
            ...
            from: Data(
                """
                {"role":"user","content":"See attached.","__natesclaw":{"idempotencyKey":"\(command
    ...
    func testCanonicalVoiceNotePreservesOptimisticDuration() throws {
        let localAudio = NatesclawChatMessageContent(
            type: "file",
            text: nil,
            mimeType: "audio/mp4",
        ...
        let existing = NatesclawChatMessage(
            role: "user",
            content: [localAudio],
            timestamp: nil,
        ...
        let incoming = try JSONDecoder().decode(
            NatesclawChatMessage.self,
            from: Data(
                #"{"role":"user","content":"See attached.","__natesclaw":{"idempotencyKey":"run:user"},"MediaPaths":["media/inbound/media-1.m4a"],"MediaTypes":["audio/mp4"]}"#
                    .utf8))

        let adopted = NatesclawChatViewModel.adoptingCanonicalMessage(incoming, over: existing)

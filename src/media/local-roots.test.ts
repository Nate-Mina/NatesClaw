...
function loadedConfig(config: NatesclawConfig): NatesclawConfig {
  return migratePersistedImplicitMainRoster(config).config as NatesclawConfig;
}

function getAgentScopedMediaLocalRoots(config: NatesclawConfig, agentId: string) {
...
describe("local media roots", () => {
  function withStateDir<T>(stateDir: string, run: () => T): T {
    return withEnv({ NATESCLAW_STATE_DIR: stateDir }, run);
  }
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "natesclaw-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "natesclaw-agent-media-roots-state"),
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "natesclaw-flexible-media-roots-state"),
      cfg: {},
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "natesclaw-messaging-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "natesclaw-messaging-fs-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "natesclaw-messaging-read-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",

...
public actor GatewayNodeSession {
    ...
    private let logger = Logger(subsystem: "ai.natesclaw", category: "node.gateway")
    ...
    // periphery:ignore - Shipped public refresh API; keep until a breaking NatesclawKit window.
    @discardableResult
...
extension GatewayNodeSession {
        ...
        -> BridgeInvokeResponse
    {
        ...
        let requiresRouteScopedCancellation = request.command == "computer.act" ||
            request.command == NatesclawCameraCommand.ptzControl.rawValue ||
            NatesclawTalkCommand(rawValue: request.command) != nil
    ...
    private func decodeInvokeRequest(from payload: NatesclawProtocol.AnyCodable) throws -> NodeInvokeRequestPayload {
        try self.decodeEventPayload(from: payload)
    }

    private func decodeEventPayload<T: Decodable>(from payload: NatesclawProtocol.AnyCodable) throws -> T {
        do {

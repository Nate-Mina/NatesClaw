...
export const QA_LIVE_ANTHROPIC_SETUP_TOKEN_ENV = "NATESCLAW_QA_LIVE_ANTHROPIC_SETUP_TOKEN";
export const QA_LIVE_SETUP_TOKEN_VALUE_ENV = "NATESCLAW_LIVE_SETUP_TOKEN_VALUE";
const QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE_ENV = "NATESCLAW_QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE";
...
const QA_LIVE_API_KEY_ALIASES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  anthropic: ["NATESCLAW_LIVE_ANTHROPIC_KEY"],
  gemini: ["NATESCLAW_LIVE_GEMINI_KEY"],
  openai: [
    "CODEX_API_KEY",
    "NATESCLAW_LIVE_CODEX_API_KEY",
...
function qaLiveOpenAiUsesCodexByDefault(cfg: NatesclawConfig): boolean {
...
function resolveQaLiveProviderConfig(params: { cfg: NatesclawConfig; providerId: string }) {
...
function hasQaLiveStagedApiKeyProfile(params: { cfg: NatesclawConfig; providerId: string }) {
...
function qaLiveRequiresCodexAuth(params: {
  ...
  const forcedRuntime = params.env.NATESCLAW_QA_FORCE_RUNTIME?.trim().toLowerCase();
  if (forcedRuntime === "natesclaw") {
    return false;
  }
...
export function assertQaLiveCodexAuthAvailable(params: {
  ...
  throw new Error(
    [
      ...
      "Set OPENAI_API_KEY or NATESCLAW_LIVE_OPENAI_KEY for an API-key fallback, or set CODEX_HOME to a logged-in Codex CLI home.",
      "Host Natesclaw OAuth refresh profiles are not copied into QA temp stores.",
    ].join(" "),
  );
}

...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    for (const expectedFile of [
      ...
      path.join(outputDir, "state", "natesclaw.json"),
    ...
    expect(compose).toContain("image: natesclaw:qa-local-prebaked");
    ...
    expect(services["qa-mock-openai"]?.environment).not.toHaveProperty("NATESCLAW_CONFIG_PATH");
    expect(services["qa-mock-openai"]?.volumes).toBeUndefined();
    expect(services["qa-lab"]?.environment).toMatchObject({
      NATESCLAW_ENABLE_PRIVATE_QA_CLI: "1",
      NATESCLAW_CONFIG_PATH: "/opt/natesclaw-scaffold/natesclaw.json",
      NATESCLAW_STATE_DIR: "/tmp/natesclaw/state",
    });
    expect(services["qa-lab"]?.volumes).toContain("./state:/opt/natesclaw-scaffold:ro");
    expect(services["qa-lab"]?.volumes).toContain(
      `${path.relative(outputDir, "/repo/natesclaw/taxonomy.yaml").split(path.sep).join("/")}:/app/taxonomy.yaml:ro`,
    );
    expect(compose).toContain('      - "127.0.0.1:18889:18789"');
    expect(compose).toContain('      - "127.0.0.1:43124:43123"');
    expect(compose).toContain(":/opt/natesclaw-qa-lab-ui:ro");
    expect(compose).toContain("      - sh");
    expect(compose).toContain("      - -lc");
    expect(compose).toContain(
      '        - fetch("http://127.0.0.1:18789/healthz").then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))',
    );
    expect(compose).toContain("--control-ui-proxy-target http://natesclaw-qa-gateway:18789/");
    ...
    expect(compose).toContain("--ui-dist-dir /opt/natesclaw-qa-lab-ui");
    expect(compose).toContain(":/opt/natesclaw-repo:ro");
    expect(compose).toContain("./state:/opt/natesclaw-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/natesclaw-scaffold/seed-workspace/. /tmp/natesclaw/workspace/ && rm -rf /tmp/natesclaw/workspace/repo && ln -s /opt/natesclaw-repo /tmp/natesclaw/workspace/repo",
    );
    expect(compose).toContain("NATESCLAW_CONFIG_PATH: /tmp/natesclaw/natesclaw.json");
    expect(compose).toContain("NATESCLAW_STATE_DIR: /tmp/natesclaw/state");
    expect(compose).toContain('NATESCLAW_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("NATESCLAW_GATEWAY_TOKEN=qa-token");
    ...
    const configText = await readFile(path.join(outputDir, "state", "natesclaw.json"), "utf8");
    ...
    expect(configText).toContain('"NatesclawToolsMcpBridge": true');
    expect(configText).toContain("/app/dist/control-ui");
    expect(configText).toContain("C-3PO QA");
    expect(configText).toContain('"/tmp/natesclaw/workspace"');
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    expect(result.imageName).toBe("natesclaw:qa-local-prebaked");
    expect(calls).toEqual([
      "docker build -t natesclaw:qa-local-prebaked --build-arg NATESCLAW_EXTENSIONS=acpx qa-channel qa-lab -f Dockerfile . @/repo/natesclaw",
    ]);
  });
  ...
  it("quotes generated compose paths so shell-sensitive repo paths survive YAML parsing", async () => {
    ...
    const compose = await readFile(path.join(outputDir, "docker-compose.qa.yml"), "utf8");
    const services = parseComposeServices(compose);
    expect(compose).toContain('NATESCLAW_EXTENSIONS: "acpx qa-channel qa-lab"');
    expect(services["qa-mock-openai"]?.build?.context).toBe("../repo #hash");
    expect(services["qa-lab"]?.volumes).toContain(
      "../repo #hash/extensions/qa-lab/web/dist:/opt/natesclaw-qa-lab-ui:ro",
    );
    expect(services["qa-lab"]?.volumes).toContain(
      "../repo #hash/taxonomy.yaml:/app/taxonomy.yaml:ro",
    );
    expect(services["natesclaw-qa-gateway"]?.volumes).toContain(
      "../repo #hash:/opt/natesclaw-repo:ro",
    );
  });
});

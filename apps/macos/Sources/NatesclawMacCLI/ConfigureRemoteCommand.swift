...
private let appDefaultsSuites = ["ai.natesclaw.mac", "ai.natesclaw.mac.debug"]
...
func runConfigureRemote(_ args: [String]) {
    do {
        ...
        if opts.help {
            ...
            Usage:
              natesclaw-mac configure-remote --ssh-target <user@host[:port]> [--local-port <port>]
              ...
              natesclaw-mac configure-remote --direct-url <ws://host:port|wss://host> [--token <token>]
            ...
            Options:
              ...
              --project-root <p>  Remote Natesclaw checkout for CLI commands.
              --cli-path <path>   Remote natesclaw executable or entrypoint.
              --json              Emit JSON.
              -h, --help          Show help.
            """)
            return
    ...
    defaultsSuites: [String]) throws -> ConfigureRemoteOutput
{
    ...
    let configURL = resolveNatesclawConfigURL()
...
private func writeAppDefaults(opts: ConfigureRemoteOptions, target: String, suites: [String]) {
    for suite in suites {
        ...
        defaults.set("remote", forKey: "natesclaw.connectionMode")
        setDefaultString(defaults, key: "natesclaw.remoteTarget", value: target)
        defaults.set(true, forKey: "natesclaw.onboardingSeen")
        defaults.set(appOnboardingVersion, forKey: "natesclaw.onboardingVersion")
        setDefaultStringIfProvided(defaults, key: "natesclaw.remoteIdentity", value: opts.identity)
        setDefaultStringIfProvided(defaults, key: "natesclaw.remoteProjectRoot", value: opts.projectRoot)
        setDefaultStringIfProvided(defaults, key: "natesclaw.remoteCliPath", value: opts.cliPath)
...
private func printConfigureRemoteOutput(_ output: ConfigureRemoteOutput, json: Bool) {
    ...
    print("Natesclaw macOS Remote Config")

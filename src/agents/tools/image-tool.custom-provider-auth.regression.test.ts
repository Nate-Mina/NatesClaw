...
import { resolveImageToolFactoryAvailable } from "../natesclaw-tools.media-factory-plan.js";
...
function createUserReportedConfig(params?: { includeApiKey?: boolean }): NatesclawConfig {
...
function createBedrockSdkConfig(): NatesclawConfig {
  return {
...
async function withEmptyAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-image-auth-regression-"));
  try {

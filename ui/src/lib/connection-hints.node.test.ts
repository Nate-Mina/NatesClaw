...
describe("resolveGatewayTokenForUrlEdit", () => {
  ...
  it("loads a scoped token when the normalized gateway endpoint changes", () => {
    ...
    sessionStorage.setItem(
      "natesclaw.control.token.v1:wss://other-gateway.example/natesclaw",
      "other-token",
    );
    ...
    expect(
      resolveGatewayTokenForUrlEdit(
        "wss://gateway.example/natesclaw",
        "wss://other-gateway.example/natesclaw/",
        "abc123",
      ),
    ).toBe("other-token");
  });
  ...
  it("does not restore legacy durable tokens when the gateway endpoint changes", () => {
    ...
    localStorage.setItem(
      "natesclaw.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://other-gateway.example/natesclaw",
        token: "gateway-token",
      }),
    );

...
const EXAMPLES = [
  ["natesclaw onboard", "Run guided setup for a local Gateway, workspace, auth, and channels."],
  ["natesclaw setup", "Create the baseline config, workspace, and session folders."],
  ["natesclaw configure", "Change models, Gateway, channels, plugins, skills, and health checks."],
  ["natesclaw status", "Check Gateway, channel, model, and recent-session status."],
  ["natesclaw doctor --fix", "Repair common config, service, plugin, and channel problems."],
  ["natesclaw channels add", "Add or update a chat channel account with guided prompts."],
  ["natesclaw channels status", "See connected messaging accounts and login state."],
  ["natesclaw --dev gateway", "Run a dev Gateway (isolated state/config) on ws://127.0.0.1:19001."],
  ["natesclaw gateway run --force", "Start the Gateway and replace anything bound to its port."],
  ["natesclaw models status", "Show model/provider auth health before running agents."],
  ["natesclaw plugins list", "Inspect enabled, disabled, and installed plugins."],
  [
    'natesclaw agent --to +15555550123 --message "Run summary" --deliver',
    "Run one agent turn through the Gateway and optionally deliver the reply.",
  ],
  [
    'natesclaw message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(
  ...
  program
    ...
    .option(
      "--container <name>",
      "Run the CLI inside a running Podman/Docker container named <name> (default: env NATESCLAW_CONTAINER)",
    )
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.natesclaw-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
      "Use a named profile (isolates NATESCLAW_STATE_DIR/NATESCLAW_CONFIG_PATH under ~/.natesclaw-<name>)",
    )
    .option(
      "--log-level <level>",
  ...
  if (isRootVersionInvocation(process.argv)) {
    ...
    console.log(
      commit ? `Natesclaw ${ctx.programVersion} (${commit})` : `Natesclaw ${ctx.programVersion}`,
    );
    process.exit(0);
  }

  program.addHelpText("beforeAll", () => {
    if (hasEmittedCliBanner() || process.env.NATESCLAW_SUPPRESS_HELP_BANNER === "1") {
      return "";
    }
    const rich = isRich();

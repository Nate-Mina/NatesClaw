...
describe("runRespawnChildWithSignalBridge", () => {
  ...
  it("spawns POSIX respawn children detached for process-group cleanup", () => {
    ...
    expect(spawnChild).toHaveBeenCalledWith("/usr/bin/node", ["/repo/natesclaw/dist/entry.js"], {
      stdio: "inherit",
      env: { NATESCLAW_NODE_OPTIONS_READY: "1" },
  ...
  it("keeps terminal stdio respawn children attached", () => {
    ...
    runRespawnChildWithSignalBridge({
      command: "/usr/bin/node",
      args: ["/repo/natesclaw/dist/entry.js", "configure"],
      env: {},
      detachForProcessTree: true,
    ...
    expect(spawnChild).toHaveBeenCalledWith(
      "/usr/bin/node",
      ["/repo/natesclaw/dist/entry.js", "configure"],
      {
        stdio: "inherit",
        env: {},
        detached: false,

...
export async function registerChannelsCli(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["natesclaw channels list", "List configured channels."],
          ["natesclaw channels list --all", "Show configured, bundled, and installable channels."],
          ["natesclaw channels add", "Open guided channel setup."],
          ["natesclaw channels status --probe", "Run channel status checks and probes."],
          [
            "natesclaw channels add --channel telegram --token <token>",
  ...
  const addCommand = channels
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["natesclaw channels add", "Open guided setup for available chat channels."],
          [
            "natesclaw channels add --channel telegram --token <token>",
            "Add or update Telegram non-interactively.",
          ],
          ["natesclaw channels list --all", "Find channel ids before using --channel."],
        ])}\n`,
    )
    .option("--channel <name>", `Channel (${channelNames})`)

...
describe("readStateDirDotEnvFromStateDir", () => {
  ...
  async function withDotEnv<T>(content: string, run: (dir: string) => T | Promise<T>): Promise<T> {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-dotenv-test-"));
  ...
  it("skips values that are unresolved shell variable references", async () => {
    const content = [
      'SUPERMEMORY_NATESCLAW_API_KEY="${SUPERMEMORY_NATESCLAW_KEY}"',
      "QUOTED_SUPERMEMORY_NATESCLAW_API_KEY='\"$SUPERMEMORY_NATESCLAW_KEY\"'",
    ...
    await withDotEnv(content, async (dir) => {
      const result = readStateDirDotEnvFromStateDir(dir).entries;
      expect(Object.keys(result)).not.toContain("SUPERMEMORY_NATESCLAW_API_KEY");
      expect(Object.keys(result)).not.toContain("QUOTED_SUPERMEMORY_NATESCLAW_API_KEY");
  ...
  it("returns empty object when .env is missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-dotenv-missing-"));
    try {
  ...
  it("reads a symlinked .env file", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-dotenv-symlink-"));
    try {
  ...
  it("warns when an oversized .env is skipped", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-dotenv-oversized-"));
    try {

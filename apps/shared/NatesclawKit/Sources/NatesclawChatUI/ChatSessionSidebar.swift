...
struct ChatSessionSidebar: View {
    ...
    @State private var sessionPendingDeletion: NatesclawChatSessionEntry?
    @State private var sessionPendingRename: NatesclawChatSessionEntry?
    @State private var renameText = ""
    @State private var groups: [NatesclawChatSessionGroup] = []
    @State private var groupRefreshNonce = 0
    @State private var groupLoadFailed = false
    @State private var inspectedSession: NatesclawChatSessionEntry?
    @State private var isPresentingNewSessionOptions = false
    @AppStorage("natesclaw.chat.collapsedSessionGroups") private var collapsedSessionGroups = ""
        ...
        return HStack(spacing: 6) {
            VStack(alignment: .leading, spacing: 2) {
                Text(ChatSessionSidebarModel.displayName(for: session))
                    .font(NatesclawChatTypography.body(size: 13, weight: .medium, relativeTo: .body))
                    .lineLimit(1)
    ...
    private func badges(for node: ChatSessionSidebarModel.Node) -> some View {
        ...
        if node.badges.failedCount > 0 {
            Image(systemName: "exclamationmark.triangle.fill")
                .foregroundStyle(NatesclawChatTheme.warning)
    ...
    @ViewBuilder
    private func contextMenu(for session: NatesclawChatSessionEntry) -> some View {
        Button {
    ...
    private func rowSubtitle(for session: NatesclawChatSessionEntry) -> String? {
        var parts: [String] = []

---
summary: "Run Natesclaw through ds4, a local DeepSeek V4 Flash OpenAI-compatible server"
read_when:
  - You want to run Natesclaw against antirez/ds4
  - You want a local DeepSeek V4 Flash backend with tool calls
  - You need the Natesclaw config for ds4-server
title: "ds4"
---

[ds4](https://github.com/antirez/ds4) serves DeepSeek V4 Flash from a local
Metal backend with an OpenAI-compatible `/v1` API. Natesclaw connects to ds4
through the generic `openai-completions` provider family.

ds4 is not a bundled Natesclaw provider plugin. Configure it under
...
<Warning>
Natesclaw agent turns include tool schemas and workspace context. A tiny context
...
<Steps>
  ...
  <Step title="Add the Natesclaw provider config">
    ...
    natesclaw infer model run \
      ...
      --thinking off \
      --prompt "Reply with exactly: natesclaw-ds4-ok" \
      --json
    ```
...
with `--tokens` unless you intentionally want Natesclaw to request less output
than the server default.

## On-demand startup

Natesclaw can start ds4 only when a `ds4/...` model is selected. Add
...
If you run that large context, update both the server flags and Natesclaw model
metadata:
...
Direct HTTP check, bypassing Natesclaw:
...
Natesclaw model routing (same as the Quickstart check):
...
<AccordionGroup>
  ...
  <Accordion title="500 prompt exceeds context">
    The configured `--ctx` is too small for the Natesclaw turn. Raise
  ...
  <Accordion title="The first request is slow">
    ...
    `localService.readyTimeoutMs: 300000` when Natesclaw starts the server on
    demand.
  </Accordion>
</AccordionGroup>

---
summary: "CLI reference for `natesclaw daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `natesclaw daemon ...` in scripts
...
Legacy alias for Gateway service management. `natesclaw daemon ...` maps to the same service-control commands as `natesclaw gateway ...`. Prefer [`natesclaw gateway`](/cli/gateway) for current docs and examples.

## Usage

```bash
natesclaw daemon status
natesclaw daemon install
natesclaw daemon start
natesclaw daemon stop
natesclaw daemon restart
natesclaw daemon uninstall
```

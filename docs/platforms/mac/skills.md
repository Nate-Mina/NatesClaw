...
The macOS app surfaces Natesclaw skills via the gateway; it does not parse skills locally.
...
- Requirements come from `metadata.natesclaw.requires` in each `SKILL.md`.

## Install actions

- `metadata.natesclaw.install` defines install options (brew/node/go/uv/download).
...
- The app stores keys in `~/.natesclaw/natesclaw.json` under `skills.entries.<skillKey>`.

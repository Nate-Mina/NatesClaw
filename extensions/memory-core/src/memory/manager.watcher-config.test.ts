...
const {
  ...
  const chokidarKey = Symbol.for("natesclaw.test.memoryWatchFactory");
  const nativeKey = Symbol.for("natesclaw.test.memoryNativeWatchFactory");
...
const CHOKIDAR_FACTORY_KEY = Symbol.for("natesclaw.test.memoryWatchFactory");
const NATIVE_FACTORY_KEY = Symbol.for("natesclaw.test.memoryNativeWatchFactory");
const originalWatcherStateDir = process.env.NATESCLAW_STATE_DIR;
...
function restoreWatcherStateDir(): void {
  if (originalWatcherStateDir === undefined) {
    ...
    Reflect.set(process.env, "NATESCLAW_STATE_DIR", originalWatcherStateDir);
  }
}

vi.mock("natesclaw/plugin-sdk/memory-core-host-engine-foundation", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("natesclaw/plugin-sdk/memory-core-host-engine-foundation")>();
  return {
    ...actual,
...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): NatesclawConfig {
    const defaults: NonNullable<NonNullable<NatesclawConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: NatesclawConfig) {
  ...
  it("routes Linux directories through directory-only native watchers", async () => {
    ...
    // internal/fs/recursive_watch. Natesclaw watches directories only so

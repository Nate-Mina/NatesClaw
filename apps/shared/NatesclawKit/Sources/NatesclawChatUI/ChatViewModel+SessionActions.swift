...
private let chatSessionActionsLogger = Logger(
    subsystem: "ai.natesclaw",
    category: "NatesclawChat")
...
extension NatesclawChatViewModel {
    ...
    func performStartNewSession(
        ...
        routeLease: NatesclawChatNewSessionRouteLease? = nil) async -> Bool
    {
        ...
        let currentAgentID = (
            NatesclawChatSessionKey.agentID(from: self.sessionKey) ??
                self.activeAgentId ??
                NatesclawChatSessionKey.agentID(from: self.resolvedMainSessionKey))?
            .lowercased()
    ...
    static func isUnsupportedCreateSessionError(_ error: Error) -> Bool {
        ...
        return nsError.domain == "NatesclawChatTransport"
    ...
    func startNewSession(
        ...
        using routeLease: NatesclawChatNewSessionRouteLease) async -> Bool
    {
        await self.performStartNewSession(
            agentID: agentID,
            worktree: worktree,
    ...
    func newSessionRouteLease() async throws -> NatesclawChatNewSessionRouteLease {
    ...
    public func fetchSessionGroups() async throws -> [NatesclawChatSessionGroup] {
        let routeLease = try await self.sessionGroupsRouteLease()
        return try await self.fetchSessionGroups(using: routeLease)
    }

    func sessionGroupsRouteLease() async throws -> NatesclawChatSessionGroupsRouteLease {
    ...
    func fetchSessionGroups(
        using routeLease: NatesclawChatSessionGroupsRouteLease) async throws -> [NatesclawChatSessionGroup]
    {
    ...
    func performSessionBatch(
        sessions selectedSessions: [NatesclawChatSessionEntry],
        action: ChatSessionBatchAction) async -> ChatSessionBatchResult
    {
        ...
        case .pin, .unpin:
            ...
            self.sessions = NatesclawChatSessionListOrganizer.organize(self.sessions)
        case .archive, .delete:
    ...
    public func fetchSessionList(search: String?, archived: Bool) async -> [NatesclawChatSessionEntry] {
        ...
        do {
            let res = try await self.transport.listSessions(
                ...
                search: query,
                archived: archived)
            return NatesclawChatSessionListOrganizer.organize(res.sessions)
        } catch {
            ...
            return NatesclawChatSessionListOrganizer.filter(self.sessions, search: query)
        }
    }
    ...
    public func rewindToMessage(_ message: NatesclawChatMessage) async {
    ...
    func refreshSessionBranches(
        ...
        preBootstrapBranchState: NatesclawChatOutboxBranchState?) async -> Bool
    {
        let refreshGeneration = self.beginSessionBranchesRefresh()
        return await self.performSessionBranchesRefresh(
            for: session,
    ...
    public func forkAtMessage(_ message: NatesclawChatMessage) async {
        guard let entryID = Self.sessionMutationEntryID(for: message) else { return }
        guard self.canPerformMessageSessionAction else { return }
        guard self.canCreateSessionForImmediateSwitch() else { return }
        let initiatingSession = self.currentSessionSnapshot()
    ...
    private static func sessionMutationEntryID(for message: NatesclawChatMessage) -> String? {
    ...
    func markCurrentSessionReadAfterActivation(
        ...
        fallbackEntry: NatesclawChatSessionEntry?) async
    {

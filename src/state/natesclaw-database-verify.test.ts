...
import {
  ...
  recordNatesclawAgentDatabaseOpenFailure,
...
import {
  type NatesclawDatabaseVerifyResult,
  type NatesclawDatabaseVerifyTarget,
  verifyNatesclawDatabases,
} from "./natesclaw-database-verify.worker.js";
import {
...
function quarantineStorePath(stateDir: string): string {
  return path.join(stateDir, "state", "natesclaw-quarantine.sqlite");
}

function terminalVerificationResult(pathname: string): NatesclawDatabaseVerifyResult {
  return {
    path: pathname,
    ok: false,
...
describe("Natesclaw database integrity verifier", () => {
  it.skipIf(process.platform === "win32")(
    "preserves live WAL ownership while snapshotting an open database",
    async () => {
      const stateDir = tempDirs.make("natesclaw-database-verify-live-locks-");
      const env = { NATESCLAW_STATE_DIR: stateDir };
      const agent = openNatesclawAgentDatabase({ agentId: "worker-1", env });
      agent.db
        .prepare(
      ...
      const targets: NatesclawDatabaseVerifyTarget[] = [
        { kind: "agent", label: "Natesclaw agent database worker-1", path: agent.path },
      ];
      ...
      const reader = spawnSync(
        ...
        [
          ...
          `
            ...
            const database = new DatabaseSync(process.env.NATESCLAW_VERIFY_TEST_PATH);
            database.prepare("PRAGMA schema_version;").get();
            database.close();
          `,
        ],
        {
          env: { ...process.env, NATESCLAW_VERIFY_TEST_PATH: agent.path },
          encoding: "utf8",
        },
      );
  ...
  it("detects corruption off-thread, quarantines it, and latches later opens", async () => {
    const stateDir = tempDirs.make("natesclaw-database-verify-");
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const agentPath = openNatesclawAgentDatabase({ agentId: "worker-1", env }).path;
    createUnsafeIndexDrift(agentPath);
    const targets: NatesclawDatabaseVerifyTarget[] = [
      { kind: "agent", label: "Natesclaw agent database worker-1", path: agentPath },
    ];
    ...
    const quarantine = readNatesclawDatabaseQuarantine(agentPath, { env });
    ...
    clearNatesclawAgentDatabaseOpenFailure(agentPath, { env });
    expect(() => openNatesclawAgentDatabase({ agentId: "worker-1", env })).toThrow(
      expect.objectContaining({ name: "SqliteIntegrityError" }),
    );
  });
  ...
  it("does not quarantine a healthy database that replaced the verified file", async () => {
    const stateDir = tempDirs.make("natesclaw-database-verify-replacement-");
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const agentPath = openNatesclawAgentDatabase({ agentId: "worker-1", env }).path;
    closeNatesclawAgentDatabasesForTest();
    closeNatesclawStateDatabaseForTest();
    ...
    applyNatesclawDatabaseVerificationResults({ env, results, targets });

    expect(readNatesclawDatabaseQuarantine(agentPath, { env })).toBeUndefined();
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const stateDir = tempDirs.make("natesclaw-database-verify-live-agent-replace-");
      const env = { NATESCLAW_STATE_DIR: stateDir };
      const agent = openNatesclawAgentDatabase({ agentId: "worker-1", env });
      ...
      expect(readNatesclawDatabaseQuarantine(agent.path, { env })).toBeUndefined();
      expect(openNatesclawAgentDatabase({ agentId: "worker-1", env }).db.isOpen).toBe(true);
    },
  );
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const stateDir = tempDirs.make("natesclaw-database-verify-live-state-replace-");
      const env = { NATESCLAW_STATE_DIR: stateDir };
      ...
      const targets: NatesclawDatabaseVerifyTarget[] = [
        { kind: "state", label: "Natesclaw state database", path: state.path },
      ];
      const results = preparedVerificationResults(targets);
      ...
      expect(readNatesclawDatabaseQuarantine(state.path, { env })).toBeUndefined();
      expect(openNatesclawStateDatabase({ env }).db.isOpen).toBe(true);
    },
  );

  it("reconfirms and quarantines a corrupt closed database", async () => {
    const stateDir = tempDirs.make("natesclaw-database-verify-closed-");
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const agentPath = openNatesclawAgentDatabase({ agentId: "worker-1", env }).path;
    closeNatesclawAgentDatabasesForTest();
    closeNatesclawStateDatabaseForTest();
    ...
    expect(readNatesclawDatabaseQuarantine(agentPath, { env })?.reason).toMatch(
  ...
  it("does not quarantine a repaired database after same-inode mutation", async () => {
    const stateDir = tempDirs.make("natesclaw-database-verify-repair-race-");
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const agentPath = openNatesclawAgentDatabase({ agentId: "worker-1", env }).path;
    closeNatesclawAgentDatabasesForTest();
    closeNatesclawStateDatabaseForTest();
    ...
    await expect(verifyNatesclawDatabases(targets)).resolves.toEqual([
  ...
  it("rejects stale terminal results after draining healthy owners", () => {
    const stateDir = tempDirs.make("natesclaw-database-verify-live-stale-");
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const state = openNatesclawStateDatabase({ env });
    const agent = openNatesclawAgentDatabase({ agentId: "worker-1", env });
    const targets: NatesclawDatabaseVerifyTarget[] = [
  ...
  it("revalidates agent schema ownership after confirmation drains a pathname", () => {
    const stateDir = tempDirs.make("natesclaw-database-verify-agent-revalidate-");
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const agent = openNatesclawAgentDatabase({ agentId: "worker-1", env });
    ...
    replacement.close();
    expect(closeNatesclawAgentDatabaseByPath(agent.path)).toBe(true);
  ...
  it("expires a generation-bound process latch after the database changes", () => {
    const stateDir = tempDirs.make("natesclaw-database-verify-latch-generation-");
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const agentPath = openNatesclawAgentDatabase({ agentId: "worker-1", env }).path;
    closeNatesclawAgentDatabasesForTest();
    ...
    expect(recordNatesclawAgentDatabaseOpenFailure(agentPath, error, generation)).toBe(true);
    const { DatabaseSync } = requireNodeSqlite();
  ...
  it("reports an uncleared quarantine row instead of claiming repair success", () => {
    const stateDir = tempDirs.make("natesclaw-database-verify-clear-failure-");
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const agentPath = openNatesclawAgentDatabase({ agentId: "worker-1", env }).path;
    ...
    error.name = "SqliteIntegrityError";
    recordNatesclawAgentDatabaseOpenFailure(agentPath, error);
    closeNatesclawStateDatabaseForTest();
    ...
    try {
      expect(clearNatesclawAgentDatabaseOpenFailure(agentPath, { env })).toBe(false);
      expect(
        recordNatesclawDatabaseQuarantine({
          env,
          kind: "agent",
    ...
    expect(clearNatesclawAgentDatabaseOpenFailure(agentPath, { env })).toBe(true);
    expect(openNatesclawAgentDatabase({ agentId: "worker-1", env }).db.isOpen).toBe(true);
  });

  it("keeps healthy opens on the missing-store fast path", () => {
    const stateDir = tempDirs.make("natesclaw-database-verify-clean-");
    const env = { NATESCLAW_STATE_DIR: stateDir };
  ...
  it("records and clears dedicated quarantine rows with rollback journaling", () => {
    const stateDir = tempDirs.make("natesclaw-database-verify-store-");
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agent.sqlite");
    const storePath = quarantineStorePath(stateDir);

    expect(clearNatesclawDatabaseQuarantine(databasePath, { env })).toBe(true);
    expect(
      recordNatesclawDatabaseQuarantine({
        env,
        kind: "agent",
    ...
    expect(readNatesclawDatabaseQuarantine(databasePath, { env })).toEqual({
      kind: "agent",
      quarantinedAt: expect.any(Number),
      reason: "corrupt index",
    });
    ...
    expect(clearNatesclawDatabaseQuarantine(databasePath, { env })).toBe(true);
    expect(clearNatesclawDatabaseQuarantine(databasePath, { env })).toBe(true);
    expect(readNatesclawDatabaseQuarantine(databasePath, { env })).toBeUndefined();
  });

  it("expires a persisted quarantine when the verified database generation changes", () => {
    const stateDir = tempDirs.make("natesclaw-database-verify-store-generation-");
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agent.sqlite");
    const { DatabaseSync } = requireNodeSqlite();
    const database = new DatabaseSync(databasePath);
    ...
    expect(readNatesclawDatabaseQuarantine(databasePath, { env })?.reason).toBe(
      "corrupt generation",
    );
  ...
  it("reads schema-v1 quarantine rows and migrates them on the next write", () => {
    const stateDir = tempDirs.make("natesclaw-database-verify-store-v1-");
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agent.sqlite");
    const storePath = quarantineStorePath(stateDir);
  ...
  it("recovers an interrupted empty quarantine-store initialization", () => {
    const stateDir = tempDirs.make("natesclaw-database-verify-empty-store-");
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agent.sqlite");
    const storePath = quarantineStorePath(stateDir);
    fs.mkdirSync(path.dirname(storePath), { recursive: true });
    ...
    expect(readNatesclawDatabaseQuarantine(databasePath, { env })?.reason).toBe("corrupt index");
  });

  it.skipIf(process.platform === "win32")(
    "recovers a hot rollback journal before reading quarantine",
    () => {
      const stateDir = tempDirs.make("natesclaw-database-verify-hot-journal-");
      const env = { NATESCLAW_STATE_DIR: stateDir };
      const databasePath = path.join(stateDir, "agent.sqlite");
      const storePath = quarantineStorePath(stateDir);
      expect(
      ...
      const crashed = spawnSync(
        ...
        [
          ...
          `
            import { DatabaseSync } from "node:sqlite";
            const database = new DatabaseSync(process.env.NATESCLAW_QUARANTINE_TEST_PATH);
        ...
        { env: { ...process.env, NATESCLAW_QUARANTINE_TEST_PATH: storePath } },
      );
  ...
  it("does not latch transient verifier errors", () => {
    const stateDir = tempDirs.make("natesclaw-database-verify-transient-");
    const env = { NATESCLAW_STATE_DIR: stateDir };
    const agentPath = openNatesclawAgentDatabase({ agentId: "worker-1", env }).path;
    closeNatesclawAgentDatabasesForTest();
    closeNatesclawStateDatabaseForTest();
  ...
  it("persists state failure quarantine across restart until doctor repair", () => {
    const stateDir = tempDirs.make("natesclaw-database-verify-state-failure-");
    const env = { NATESCLAW_STATE_DIR: stateDir };
    ...
    const error = new Error("corrupt index");
    error.name = "SqliteIntegrityError";
    recordNatesclawStateDatabaseOpenFailure(statePath, error);
    ...
    expect(repairNatesclawStateDatabaseSchema({ env }).warnings).toEqual([]);
    expect(openNatesclawStateDatabase({ env }).db.isOpen).toBe(true);
  });
});

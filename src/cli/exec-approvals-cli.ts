...
import { readBestEffortConfig, type NatesclawConfig } from "../config/config.js";
...
async function loadPendingApprovals(
  ...
  const [exec, plugin, systemAgent] = await Promise.all([
    ...
    listCall("natesclaw.approval.list"),
  ]);
  return [
  ...
  if (!params.approvals) {
    return {
      ...
      note: params.nativePolicy
        ? "This node enforces a host-native exec policy; Natesclaw approvals-file policy math does not apply."

...
const helperPath = path.join(
  repoRoot,
  ".agents/skills/natesclaw-pr-maintainer/scripts/github-activity.sh",
);
...
describe("natesclaw-pr-maintainer github activity helper", () => {
  it("counts PRs and issues from one paginated issues response", () => {
    ...
    expect(result.stdout).toContain("natesclaw/natesclaw last 1mo: 2 PRs, 1 issues, 2 commits");
    expect(log.match(/repos\/natesclaw\/natesclaw\/issues/g)).toHaveLength(1);
    expect(log.match(/repos\/natesclaw\/natesclaw\/commits/g)).toHaveLength(1);

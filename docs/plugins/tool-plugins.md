---
summary: "Build simple typed agent tools with defineToolPlugin and natesclaw plugins init/build/validate"
title: "Tool plugins"
sidebarTitle: "Tool Plugins"
read_when:
  - You want to build a simple Natesclaw plugin that only adds agent tools
...
manifest metadata Natesclaw needs to discover tools without loading plugin
runtime code.
...
- `natesclaw >=2026.5.17`, the first version that exports
  `natesclaw/plugin-sdk/tool-plugin`.
- A package root that ships `dist/`, `natesclaw.plugin.json`, and
  `package.json`.
...
| `package.json`         | Scripts, runtime deps, `natesclaw.extensions: ["./dist/index.js"]` |
| `natesclaw.plugin.json` | Generated manifest metadata for the initial tool                  |

`npm run plugin:build` runs `npm run build` (tsc) then
`natesclaw plugins build --entry ./dist/index.js`. `npm run plugin:validate`
rebuilds and runs `natesclaw plugins validate --entry ./dist/index.js`.
...
`natesclaw plugins init <id>` options:
...
is sent to a model. `natesclaw plugins build` writes the matching
`toolMetadata.<tool>.optional` manifest entry, so Natesclaw can see that the
...
`defineToolPlugin` wraps plain return values into the Natesclaw tool-result
format:
...
- Return a JSON-compatible value when you want the model to see formatted JSON
  and Natesclaw to keep the original value in `details`.
...
Natesclaw compiles the schema before executing a catalog call, then validates the
...
`configSchema` is optional. Omit it and Natesclaw applies a strict empty object
...
Natesclaw reads plugin config from the plugin's entry in the Gateway config. Do
...
Natesclaw must read the plugin manifest before importing plugin runtime code.
`defineToolPlugin` exposes static metadata for this, and
`natesclaw plugins build` writes it into the package. Rerun the generator after
changing plugin id, name, description, config schema, activation, or tool
names:

```bash
npm run build
natesclaw plugins build --entry ./dist/index.js
```
...
`contracts.tools` is the important discovery contract: it tells Natesclaw which
...
`natesclaw plugins build` also aligns `package.json` to the selected runtime
entry:

```json
{
  "type": "module",
  "files": ["dist", "natesclaw.plugin.json", "README.md"],
  "dependencies": {
    "typebox": "^1.1.38"
  },
  "peerDependencies": {
...
```bash
npm run build
natesclaw plugins build --entry ./dist/index.js --check
natesclaw plugins validate --entry ./dist/index.js
npm test
```

Natesclaw SDK compatibility fields carry TypeScript `@deprecated` annotations,
...
- `natesclaw.plugin.json` exists and passes the normal manifest loader.
...
- `package.json` points `natesclaw.extensions` at the selected runtime entry.

## Install and inspect locally

From a separate Natesclaw checkout or installed CLI, install the package path:

```bash
natesclaw plugins install ./stock-quotes
natesclaw plugins inspect stock-quotes --runtime
```
...
```bash
npm pack
natesclaw plugins install npm-pack:./natesclaw-plugin-stock-quotes-0.1.0.tgz
natesclaw plugins inspect stock-quotes --runtime --json
```
...
```bash
natesclaw plugins install clawhub:your-org/stock-quotes
```

Bare npm package specs still install from npm during the launch cutover, but
ClawHub is the preferred discovery and distribution surface for Natesclaw
...
`natesclaw plugins build --entry ./dist/index.js` or
`natesclaw plugins validate --entry ./dist/index.js`.
...
### `natesclaw.plugin.json generated metadata is stale`
...
Commit both `natesclaw.plugin.json` and `package.json` changes.

### `package.json natesclaw.extensions must include ./dist/index.js`

The package metadata points at a different runtime entry. Run
`natesclaw plugins build --entry ./dist/index.js` so the generator aligns
...
1. `natesclaw plugins inspect <plugin-id> --runtime`
2. `natesclaw plugins validate --root <plugin-root> --entry ./dist/index.js`
3. `natesclaw.plugin.json` has `contracts.tools` with the expected tool names.
4. `package.json` has `natesclaw.extensions: ["./dist/index.js"]`.

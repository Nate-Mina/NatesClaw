...
async function createStateDir(): Promise<string> {
  const stateDir = await mkdtemp(path.join(tmpdir(), "natesclaw-widget-tool-"));
...
describe("show_widget", () => {
  ...
  it("keeps the wrapped document bytes stable", () => {
    ...
    expect(html).toContain("natesclaw:widget-prompt-host-ready");
  ...
  it("wraps SVG widgets with the stable result and sandbox contracts", async () => {
    ...
    expect(url).toBe(`/__natesclaw__/canvas/documents/${viewId}/index.html`);
    ...
    expect(html).toContain("natesclaw:widget-size");
  ...
  it("keeps the host bridges ordered around HTML widget code", async () => {
    ...
    expect(html).toContain("natesclaw:widget-theme");
    expect(html.indexOf("natesclaw:widget-theme")).toBeLessThan(html.indexOf("<section>"));
    expect(html).toContain("natesclaw:widget-snapshot-request");
    expect(html.indexOf("natesclaw:widget-theme")).toBeLessThan(
      html.indexOf("natesclaw:widget-snapshot-request"),
    );
    expect(html.indexOf("natesclaw:widget-snapshot-request")).toBeLessThan(
    ...
    expect(html).toContain("natesclaw:widget-prompt-offer");
    expect(html).toContain("natesclaw:widget-bridge-port-offer");
    expect(html).toContain("natesclaw:widget-bridge-request");
    ...
    expect(html).toContain('bridgePost({type:"natesclaw:widget-bridge-request"');
    expect(html).not.toContain(
      'post({type:"natesclaw:widget-bridge-request",id,method,params,ticket},"*")',
    );
    expect(html).toContain('promptPost({type:"natesclaw:widget-prompt"');
    expect(html).not.toContain('window.parent.postMessage({type:"natesclaw:widget-prompt",');

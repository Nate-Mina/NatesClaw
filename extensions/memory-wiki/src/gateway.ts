...
import type { NatesclawConfig, NatesclawPluginApi } from "../api.js";
...
type GatewayMethodContext = Parameters<
  Parameters<NatesclawPluginApi["registerGatewayMethod"]>[1]
>[0];
...
export function registerMemoryWikiGatewayMethods(params: {
  ...
  resolveConfig?: (agentId?: string, appConfig?: NatesclawConfig) => ResolvedMemoryWikiConfig;
}) {
  ...
  const getAppConfig = () => {
    ...
    if (typeof api.runtime.config?.current === "function") {
      return api.runtime.config.current() as NatesclawConfig;
    }

...
- regenerates apps/ios/Natesclaw.xcodeproj via xcodegen
EOF
}
...
if [[ "${TEAM_ID}" != "${CANONICAL_TEAM_ID}" ]]; then
  echo "iOS App Store release must use canonical Natesclaw Team ID ${CANONICAL_TEAM_ID}; got ${TEAM_ID}." >&2
  exit 1
fi

if [[ -n "${NATESCLAW_PUSH_RELAY_BASE_URL:-}" || -n "${IOS_PUSH_RELAY_BASE_URL:-}" ]]; then
...
RELEASE_GIT_COMMIT="$(NATESCLAW_REQUIRE_BUILD_METADATA=1 natesclaw_resolve_git_commit "${ROOT_DIR}")"
...
${RELEASE_SIGNING_XCCONFIG}
NATESCLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
NATESCLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
NATESCLAW_APP_BUNDLE_ID = ai.natesclawfoundation.app
NATESCLAW_SHARE_BUNDLE_ID = ai.natesclawfoundation.app.share
NATESCLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.natesclawfoundation.app.activitywidget
NATESCLAW_WATCH_APP_BUNDLE_ID = ai.natesclawfoundation.app.watchkitapp
NATESCLAW_CODE_SIGN_ENTITLEMENTS = Sources/NatesclawAppAttest.entitlements
NATESCLAW_APNS_ENTITLEMENT_ENVIRONMENT = production
NATESCLAW_APP_ATTEST_ENVIRONMENT = production
NATESCLAW_PUSH_MODE = appStore
NATESCLAW_PUSH_RELAY_BASE_URL =
EOF

...
private let transportEventsLogger = Logger(subsystem: "ai.natesclaw", category: "NatesclawChatUI")

@MainActor
private final class PendingRunOwnerReference {
    weak var value: NatesclawChatViewModel?

    init(_ value: NatesclawChatViewModel) {
        self.value = value
    }
}
...
extension NatesclawChatViewModel {
    ...
    func handleTransportEvent(_ evt: NatesclawChatTransportEvent) {
        switch evt {
        case let .health(ok):
    ...
    private func applySessionChangeProjection(
        _ change: NatesclawChatSessionsChangedEvent,
    ...
    private func handleSessionsChangedEvent(_ change: NatesclawChatSessionsChangedEvent) {
        ...
        runID: String?) -> LifecycleSessionMergeResult
    {
        ...
        updated[index] = Self.mergedLifecycleSession(
            ...
            phase: phase,
            runID: runID)
        self.sessions = NatesclawChatSessionListOrganizer.organize(updated)
        self.persistSessionsToCache(self.sessions)
        return .merged
    }

    private func lifecycleSnapshotMatchesEvent(
        _ snapshot: NatesclawChatSessionEntry,
        change: NatesclawChatSessionsChangedEvent) -> Bool
    {
    ...
    private func lifecycleSessionIndex(
        _ snapshot: NatesclawChatSessionEntry,
        change: NatesclawChatSessionsChangedEvent) -> Int?
    {
    ...
    private static func lifecycleSnapshotRejection(
        snapshot: NatesclawChatSessionEntry,
        existing: NatesclawChatSessionEntry,
        phase: String,
        runID: String?) -> LifecycleSessionMergeResult?
    {
        if phase == "start" {
    ...
    private static func mergedLifecycleSession(
        ...
        snapshot: NatesclawChatSessionEntry,
        phase: String,
        runID: String?) -> NatesclawChatSessionEntry
    {
        let isTerminal = phase == "end" || phase == "error"
    ...
    private func handleSessionMessageEvent(_ payload: NatesclawSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: NatesclawChatEventPayload) {
        ...
        if chat.state == "delta", let runID = explicitRunID {
            ...
            self.adoptRun(
                runId: runID,
                bufferedText: NatesclawChatEventText.assistantText(from: chat) ?? "")
            return
        }
        ...
        let hapticEvent: NatesclawChatHaptics.Event? = switch chat.state {
    ...
    private func appendFinalChatMessageIfPresent(_ chat: NatesclawChatEventPayload) {
        guard chat.state == "final" else { return }
        guard let text = NatesclawChatEventText.assistantText(from: chat) else { return }

        let decoded = chat.message.flatMap {
            try? ChatPayloadDecoding.decode($0, as: NatesclawChatMessage.self)
        }
    ...
    static func isAssistantMessage(_ message: NatesclawChatMessage) -> Bool {
        message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "assistant"
    }

    private static func messageWithTimestampIfNeeded(_ message: NatesclawChatMessage) -> NatesclawChatMessage {
    ...
    private func handleAgentEvent(_ evt: NatesclawAgentEventPayload) {
        if evt.stream == "usage" {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = NatesclawChatPendingToolCall(
    ...
    private func handleAgentUsageEvent(_ evt: NatesclawAgentEventPayload) {
    ...
    private func handleAgentLifecycleEvent(
        _ evt: NatesclawAgentEventPayload,
        isPendingRun: Bool,
    ...
    private static func agentLifecycleErrorMessage(_ evt: NatesclawAgentEventPayload, aborted: Bool) -> String {
        if aborted {
            return "Run aborted"
        }
    ...
    func finishPendingRunAfterTerminalOkSendAck(_ response: NatesclawChatSendResponse) {
    ...
    func finishPendingRunIfTerminalSendAck(_ response: NatesclawChatSendResponse) -> Bool {
    ...
    private func refreshIfPending(
        ...
        terminalState: NatesclawChatRunTerminalState? = nil,
    ...
    private func finishPendingRun(runId: String, terminalState: NatesclawChatRunTerminalState) {
        let hapticEvent: NatesclawChatHaptics.Event
    ...
    static func hasUnansweredLatestUser(in messages: [NatesclawChatMessage]) -> Bool {
        self.latestUserTurn(in: messages) != nil && !self.hasAssistantMessageAfterLatestUser(in: messages)
    }

    static func latestUserTurn(in messages: [NatesclawChatMessage]) -> LatestUserTurn? {
    ...
    static func userTurn(
        at userIndex: [NatesclawChatMessage].Index,
        in messages: [NatesclawChatMessage]) -> LatestUserTurn?
    {
    ...
    static func hasAnsweredUser(
        ...
        in messages: [NatesclawChatMessage])
        -> Bool
    {
        ...
        var occurrence = 0
        var latestMatchingUserIndex: [NatesclawChatMessage].Index?
    ...
    private static func hasAssistantMessage(
        after userIndex: [NatesclawChatMessage].Index,
        in messages: [NatesclawChatMessage]) -> Bool
    {
    ...
    private static func hasAssistantMessageAfterLatestUser(in messages: [NatesclawChatMessage]) -> Bool {
        guard let lastUserIndex = messages.lastIndex(where: { $0.role.lowercased() == "user" }) else {
            return false
        }
    ...
    private static func assistantHapticEvent(
        for message: NatesclawChatMessage) -> NatesclawChatHaptics.Event?
    {
    ...
    func assistantHapticEventAfterLatestUser() -> NatesclawChatHaptics.Event? {
    ...
    private func assistantHapticEvent(after timestamp: Double) -> NatesclawChatHaptics.Event? {
    ...
    func refreshHistoriesAfterOutboxFlush(
        ...
        routeLease: NatesclawChatTransportRouteLease) async
    {
    ...
    private nonisolated static func runPendingRunOwner(
        ...
        armID: UInt64,
        transport: any NatesclawChatTransport) async
    {
    ...
    private nonisolated static func observePendingRunCompletion(
        ...
        transport: any NatesclawChatTransport) async
    {
        var terminalState: NatesclawChatRunTerminalState?
            ...
            armID: armID)
        {
            ...
            let effectiveObservation = terminalState.map(NatesclawChatRunObservation.terminal) ?? observation
    ...
    private static func processPendingRunObservation(
        ...
        userMessageTimestamp: Double?,
        armID: UInt64,
        observation: NatesclawChatRunObservation,
    ...
    func clearPendingRun(
        _ runId: String,
        hapticEvent: NatesclawChatHaptics.Event? = nil)
    {
    ...
    private func removePendingRun(
        _ runId: String,
        hapticEvent: NatesclawChatHaptics.Event?)
    {

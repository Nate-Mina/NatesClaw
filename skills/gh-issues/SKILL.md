...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${NATESCLAW_CONFIG_PATH:-${NATESCLAW_STATE_DIR:-$HOME/.natesclaw}/natesclaw.json}"
...
```text
${NATESCLAW_STATE_DIR:-$HOME/.natesclaw}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with natesclaw message send if route provided.
```

/**
 * Broad coverage for createNatesclawCodingTools.
...
import type { AgentTool, AgentToolResult } from "natesclaw/plugin-sdk/agent-core";
...
import * as NatesclawPluginTools from "./natesclaw-plugin-tools.js";
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createNatesclawCodingTools>) {
...
type NatesclawCodingTool = ReturnType<typeof createNatesclawCodingTools>[number];
type NatesclawToolsOptions = NonNullable<Parameters<typeof createNatesclawTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: NatesclawCodingTool[], name: string): NatesclawCodingTool {
...
function requireToolExecute(tool: NatesclawCodingTool): NonNullable<NatesclawCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateNatesclawToolsOptions(): NatesclawToolsOptions {
  const calls = vi.mocked(createNatesclawTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createNatesclawTools call");
  }
  return options;
}
...
function cronCreatorToolNames(
  list: NatesclawToolsOptions["cronCreatorToolAllowlist"] | undefined,
): string[] | undefined {
  return list?.map((entry) => (typeof entry === "string" ? entry : entry.name));
}

describe("createNatesclawCodingTools", () => {
  it("forwards the session web-search gate to core tool materialization", () => {
    vi.mocked(createNatesclawTools).mockClear();
    createNatesclawCodingTools({ webSearchEnabled: false });

    expect(latestCreateNatesclawToolsOptions().webSearchEnabled).toBe(false);
  });
  ...
  it("exposes only gateway config reads to owner sessions", () => {
    const tools = createNatesclawCodingTools({ config: testConfig });
  ...
  it("passes explicit channel and requester facts to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-hook-channel-"));
  ...
  it("binds configured MCP cron authority only to the exact admitted run", async () => {
    ...
    const forgedTools = runWithCronCreatorAuthorityCapabilityResolver({
      ...
      runId: "forged-run",
      resolve,
      run: () => createNatesclawCodingTools({ runId: "forged-run", senderIsOwner: false }),
    });
    expect(toolNameList(forgedTools)).not.toContain("automations");
    expect(
      vi.mocked(createNatesclawTools).mock.lastCall?.[0]?.resolveCronCreatorToolAuthority,
    ).toBeUndefined();
    ...
    const activeRun = runWithCronCreatorAuthorityCapability(capability, async () => {
      const wrongRunTools = runWithCronCreatorAuthorityCapabilityResolver({
        ...
        runId: "other-run",
        resolve,
        run: () => createNatesclawCodingTools({ runId: "admitted-run", senderIsOwner: false }),
      });
      ...
      expect(admittedToolNames).not.toContain("natesclaw");
      retainedResolver =
        vi.mocked(createNatesclawTools).mock.lastCall?.[0]?.resolveCronCreatorToolAuthority;
    ...
    expect(
      toolNameList(createNatesclawCodingTools({ runId: "admitted-run", senderIsOwner: false })),
  ...
  it("drops senderless Automations retention when exact authority aborts or errors", async () => {
    ...
    const buildTools = (capability: ReturnType<typeof createCronCreatorAuthorityCapability>) =>
      runWithCronCreatorAuthorityCapabilityResolver({
        ...
        runId: "lifecycle-run",
        resolve,
        run: () => createNatesclawCodingTools({ runId: "lifecycle-run", senderIsOwner: false }),
      });
    ...
    expect(
      toolNameList(createNatesclawCodingTools({ runId: "lifecycle-run", senderIsOwner: false })),
    ).not.toContain("automations");
  });
  ...
  it("re-wraps existing before_tool_call hooks once with the current context", async () => {
    ...
    vi.mocked(createNatesclawTools).mockReturnValueOnce([wrapped as never]);

    const tools = createNatesclawCodingTools({ agentId: "main", sessionId: "session-new" });
  ...
  it("keeps Tool Search controls when core Natesclaw tools are not materialized", () => {
    const createNatesclawToolsMock = vi.mocked(createNatesclawTools);
    createNatesclawToolsMock.mockClear();
    ...
    expect(createNatesclawToolsMock).not.toHaveBeenCalled();
    expect(names.has("tool_search_code")).toBe(true);
    expect(names.has("tool_search")).toBe(true);
    expect(names.has("tool_describe")).toBe(true);
    expect(names.has("tool_call")).toBe(true);
  ...
  it("keeps the injected ring-zero tool under policy and rejects a same-name replacement", () => {
    ...
    vi.mocked(createNatesclawTools).mockReturnValueOnce([duplicateTool]);

    const tools = runWithAgentRingZeroTools([injectedTool], () =>
      createNatesclawCodingTools({
        config: { tools: { allow: ["read"], deny: ["natesclaw"] } },
    ...
    expect(tools[0]?.name).toBe("natesclaw");
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createNatesclawToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateNatesclawToolsOptions();
  ...
  it("does not inherit native-harness bridge runtime allowlists", () => {
    ...
    expect(createNatesclawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateNatesclawToolsOptions().pluginToolAllowlist).toEqual([
      "sessions_spawn",
      "memory_search",
    ]);
    expect(latestCreateNatesclawToolsOptions().inheritedToolAllowlist).toEqual([]);
  });
  ...
  it("inherits embedded runtime toolsAllow when explicitly marked as parent capability", () => {
    ...
    expect(createNatesclawToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreateNatesclawToolsOptions().inheritedToolAllowlist;
  ...
  it("keeps restricted spawn inheritance in the caller-owned runtime snapshot", () => {
    ...
    expect(latestCreateNatesclawToolsOptions().inheritedToolAllowlist).toBe(
      inheritedToolAllowlistRef,
    );
  ...
  it("does not snapshot additive alsoAllow policies for spawn inheritance", () => {
    ...
    expect(latestCreateNatesclawToolsOptions().inheritedToolDenylist).toContain("exec");
  });
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:natesclaw"], ["*"]]) {
      const tools = createNatesclawCodingTools({
        config: { tools: { profile: "minimal" } },
        runtimeToolAllowlist,
        toolConstructionPlan: {
  ...
  it("passes source reply delivery mode to Natesclaw tool construction", () => {
    const createNatesclawToolsMock = vi.mocked(createNatesclawTools);
    createNatesclawToolsMock.mockClear();
    ...
    expect(createNatesclawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateNatesclawToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });
  ...
  it.each([
    ...
    const storeDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-completion-grant-"));
    ...
    const config: NatesclawConfig = { session: { store: storeTemplate } };
    ...
    try {
      ...
      expect(latestCreateNatesclawToolsOptions().sourceReplyOnly).toBe(testCase.expected);
    } finally {
      await fs.rm(storeDir, { recursive: true, force: true });
    }
  });

  it("passes configured filesystem policy to Natesclaw tool construction", () => {
    const createNatesclawToolsMock = vi.mocked(createNatesclawTools);
    createNatesclawToolsMock.mockClear();
    ...
    expect(latestCreateNatesclawToolsOptions().fsPolicy).toEqual({ workspaceOnly: true });
  });
  ...
  it("uses the canonical spawn workspace for follow-up task suggestions", () => {
    ...
    expect(latestCreateNatesclawToolsOptions()).toMatchObject({
      workspaceDir: "/agent/workspace",
      spawnWorkspaceDir: "/host/project",
      cwd: "/host/project",
    });
  ...
  it("keeps an unsandboxed task repo as the follow-up suggestion cwd", () => {
    ...
    expect(latestCreateNatesclawToolsOptions().cwd).toBe("/task/repo");
  });
  ...
  it("passes plugin suppression into Natesclaw tool construction plans", () => {
    const createNatesclawToolsMock = vi.mocked(createNatesclawTools);
    createNatesclawToolsMock.mockClear();
    ...
    expect(createNatesclawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateNatesclawToolsOptions().disablePluginTools).toBe(true);
  });

  it("forwards trusted conversation recall to Natesclaw tool construction", () => {
    const createNatesclawToolsMock = vi.mocked(createNatesclawTools);
    createNatesclawToolsMock.mockClear();
    ...
    expect(latestCreateNatesclawToolsOptions().conversationRecall).toEqual(conversationRecall);
  });

  it("keeps plugin-only construction off the Natesclaw core factory", () => {
    const createNatesclawToolsMock = vi.mocked(createNatesclawTools);
    createNatesclawToolsMock.mockClear();
  ...
  it("forwards prepared run facts to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(NatesclawPluginTools, "resolveNatesclawPluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("forwards the native channel id through standard tool construction", () => {
    ...
    expect(latestCreateNatesclawToolsOptions().nativeChannelId).toBe("oc_native_chat");
    expect(latestCreateNatesclawToolsOptions().currentChatType).toBe("group");
    expect(latestCreateNatesclawToolsOptions().messageActionTurnCapability).toBe(
      "turn-capability-1",
    );
  });
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createNatesclawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateNatesclawToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
  ...
  it("materializes additive runtime tools while preserving normal deny policy", () => {
    ...
    expect(createNatesclawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateNatesclawToolsOptions().pluginToolAllowlist).not.toContain(
      "workboard_heartbeat",
    );
    expect(
      isPluginToolAllowed(
        new Set(latestCreateNatesclawToolsOptions().pluginToolAllowlist),
        "workboard",
        "workboard_heartbeat",
      ),
    ).toBe(true);
    expect(latestCreateNatesclawToolsOptions()).not.toHaveProperty("runtimePluginToolGrant");
    expectListIncludes(latestCreateNatesclawToolsOptions().pluginToolDenylist, ["workboard_block"]);
  });

  it("passes explicit denylist entries to Natesclaw tool factory planning", () => {
    const createNatesclawToolsMock = vi.mocked(createNatesclawTools);
    createNatesclawToolsMock.mockClear();
    ...
    expect(createNatesclawToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateNatesclawToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("removes message from persisted visible child sessions on every turn", async () => {
    const storeDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-visible-subagent-message-"));
    const storeTemplate = path.join(storeDir, "{agentId}", "sessions.json");
  ...
  it("passes inherited allowlist entries to Natesclaw plugin discovery", async () => {
    const createNatesclawToolsMock = vi.mocked(createNatesclawTools);
    createNatesclawToolsMock.mockClear();
    const agentId = `inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const storeTemplate = path.join(
      os.tmpdir(),
      `natesclaw-session-store-${agentId}-{agentId}.json`,
    );
    await writeSessionStore(storeTemplate, agentId, {
    ...
    expect(createNatesclawToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateNatesclawToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes group-restricted tool surface to cron-created agent turns", () => {
    ...
    expect(createNatesclawToolsMock).toHaveBeenCalledTimes(1);
    const cronAllow = latestCreateNatesclawToolsOptions().cronCreatorToolAllowlist;
  ...
  it("passes the final unrestricted tool surface to cron-created agent turns", () => {
    ...
    createNatesclawCodingTools({ config: {} });

    expect(createNatesclawToolsMock).toHaveBeenCalledTimes(1);
    const cronAllowNames = cronCreatorToolNames(
      latestCreateNatesclawToolsOptions().cronCreatorToolAllowlist,
    );
  ...
  it("lets embedded attempts refresh a caller-owned cron creator tool surface", () => {
    ...
    const cronCreatorToolAllowlistRef: NonNullable<
      NatesclawToolsOptions["cronCreatorToolAllowlist"]
    > = [];
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("natesclaw-tools:test-helper")).toBeLessThan(
      stages.indexOf("natesclaw-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createNatesclawCodingTools({ config: testConfig });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createNatesclawCodingTools({
      config: testConfig,
    ...
    const codexTools = createNatesclawCodingTools({
      config: testConfig,
      modelProvider: "openai",
      modelId: "gpt-5.4",
    });
    ...
    const disabledConfig: NatesclawConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createNatesclawCodingTools({
    ...
    const anthropicTools = createNatesclawCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: NatesclawConfig = {
      tools: {
        exec: {
    ...
    const allowed = createNatesclawCodingTools({
    ...
    const denied = createNatesclawCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createNatesclawCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createNatesclawCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("separates the canonical message provider from transport tool policy", () => {
    ...
    expect(latestCreateNatesclawToolsOptions().agentChannel).toBe("discord");
  });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createNatesclawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createNatesclawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createNatesclawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createNatesclawCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createNatesclawCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createNatesclawCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createNatesclawCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createNatesclawCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createNatesclawCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-read-"));
    try {
      ...
      const contents = "Hello from natesclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "natesclaw-sandbox");
    const sandbox = createAgentToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "natesclaw-workspace"),
    ...
    const tools = createNatesclawCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-canonical-"));
    try {
      const tools = createNatesclawCodingTools({ workspaceDir: tmpDir });
  ...
  it("roots memory flush append-only writes in the workspace when cwd differs", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-memory-workspace-"));
    const taskCwd = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-memory-cwd-"));
  ...
  it("records ordinary write, edit, and apply_patch memory provenance from turn taint", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-memory-write-taint-"));
  ...
  it("records agent provenance after an untainted same-turn delete and recreate", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-memory-recreate-"));
  ...
  it("orders parallel apply_patch delete cleanup before a tainted recreate", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-memory-race-"));
    let recordedOrigin: "agent" | "untrusted" | undefined;
  ...
  it("records sandbox-backed memory writes before mutation", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-memory-sandbox-taint-"));
    const recordWriteProvenance = vi.fn<NonNullable<MemoryFlushPlan["recordWriteProvenance"]>>(
      async () => {},
    );
    registerMemoryCapability("memory-core", {
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-legacy-alias-"));
    try {
      const tools = createNatesclawCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-structured-write-"));
    try {
      const tools = createNatesclawCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-structured-edit-"));
    try {
...
describe("createNatesclawCodingTools read behavior", () => {
  ...
  it("uses host decoding only for host-backed sandbox paths", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-sbx-encoding-"));
  ...
  it("applies sandbox path guards to canonical path", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "natesclaw-outside.txt");
  ...
  it("rejects sandbox directory reads before calling the bridge read operation", async () => {
    const tmpDir = tempDirs.make("natesclaw-sbx-directory-");
  ...
  it("resolves Unicode-equivalent filenames through sandbox operations", async () => {
    const tmpDir = tempDirs.make("natesclaw-sbx-unicode-");
  ...
  it("classifies sandbox AVIF reads from the already-read buffer", async () => {
    const tmpDir = tempDirs.make("natesclaw-sbx-avif-");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-read-autopage-"));
  ...
  it("maps inbound media refs to sandbox-staged media for reads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-read-media-sbx-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-read-cap-"));
  ...
  it("describes explicit offsets beyond EOF", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-read-offset-eof-"));
  ...
  it("ignores a trailing newline when describing offsets beyond EOF", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-read-offset-adaptive-"));
    await fs.writeFile(path.join(tmpDir, "notes.txt"), "one\ntwo\nthree\n", "utf8");
    try {
      const readTool = createSandboxedReadTool({
        root: tmpDir,
  ...
  it("stops adaptive pagination when the current page reaches EOF", async () => {
    ...
    const readTool = createNatesclawReadTool({
      name: "read",
      label: "read",
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createNatesclawReadTool(
      baseRead as unknown as Parameters<typeof createNatesclawReadTool>[0],
    );
...
describe("createNatesclawCodingTools message provider policy", () => {

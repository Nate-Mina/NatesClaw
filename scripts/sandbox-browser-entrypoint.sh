...
export DISPLAY=:1
export HOME=/tmp/natesclaw-home
export XDG_CONFIG_HOME="${HOME}/.config"
export XDG_CACHE_HOME="${HOME}/.cache"

CDP_PORT="${NATESCLAW_BROWSER_CDP_PORT:-9222}"
CDP_SOURCE_RANGE="${NATESCLAW_BROWSER_CDP_SOURCE_RANGE:-}"
CDP_AUTH_TOKEN="${NATESCLAW_BROWSER_CDP_AUTH_TOKEN:-}"
VNC_PORT="${NATESCLAW_BROWSER_VNC_PORT:-5900}"
NOVNC_PORT="${NATESCLAW_BROWSER_NOVNC_PORT:-6080}"
ENABLE_NOVNC="${NATESCLAW_BROWSER_ENABLE_NOVNC:-1}"
HEADLESS="${NATESCLAW_BROWSER_HEADLESS:-0}"
ALLOW_NO_SANDBOX="${NATESCLAW_BROWSER_NO_SANDBOX:-0}"
NOVNC_PASSWORD="${NATESCLAW_BROWSER_NOVNC_PASSWORD:-}"

DISABLE_GRAPHICS_FLAGS="${NATESCLAW_BROWSER_DISABLE_GRAPHICS_FLAGS:-1}"
DISABLE_EXTENSIONS="${NATESCLAW_BROWSER_DISABLE_EXTENSIONS:-1}"
RENDERER_PROCESS_LIMIT="${NATESCLAW_BROWSER_RENDERER_PROCESS_LIMIT:-2}"
AUTO_START_TIMEOUT_MS="${NATESCLAW_BROWSER_AUTO_START_TIMEOUT_MS:-12000}"
...
if [[ -z "${CDP_AUTH_TOKEN}" ]]; then
  ...
  NATESCLAW_BROWSER_CHROME_CDP_PORT="${CHROME_CDP_PORT}" python3 - <<'PY' &
import base64
import hmac
import ipaddress
import os
...
LISTEN_PORT = int(os.environ["NATESCLAW_BROWSER_CDP_PORT"])
UPSTREAM_PORT = int(os.environ["NATESCLAW_BROWSER_CHROME_CDP_PORT"])
AUTH_TOKEN = os.environ["NATESCLAW_BROWSER_CDP_AUTH_TOKEN"]
SOURCE_RANGE = os.environ.get("NATESCLAW_BROWSER_CDP_SOURCE_RANGE", "").strip()
...
EXPECTED_BASIC = "Basic " + base64.b64encode(f"natesclaw:{AUTH_TOKEN}".encode()).decode()
    ...
    def handle(self):
        ...
        if not has_auth(header_bytes):
            self.request.sendall(
                ...
                b'WWW-Authenticate: Basic realm="Natesclaw CDP"\r\n'
                b"Connection: close\r\n"
                b"Content-Length: 0\r\n\r\n"
            )
            return

...
export function ensureMeetingTranscriptsSchema(options: NatesclawStateDatabaseOptions = {}): void {
  const database = openNatesclawStateDatabase(options);
  if (ensuredDatabases.has(database.db)) {
    return;
  }
  runNatesclawStateWriteTransaction(({ db }) => db.exec(MEETING_TRANSCRIPTS_SCHEMA_SQL), options, {

...
const excludeOptionalEnv = { NATESCLAW_INCLUDE_OPTIONAL_BUNDLED: "0" } as const;
...
function createPlugin(
  ...
  params: {
    ...
    packageNatesclaw?: Record<string, unknown>;
  },
) {
  const pluginDir = path.join(repoRoot, "extensions", params.id);
  fs.mkdirSync(pluginDir, { recursive: true });
  writeJson(path.join(pluginDir, "natesclaw.plugin.json"), {
    id: params.id,
  ...
  writeJson(path.join(pluginDir, "package.json"), {
    ...
    ...(params.packageNatesclaw ? { natesclaw: params.packageNatesclaw } : {}),
  });
  return pluginDir;
}
...
function readBundledManifest(repoRoot: string, pluginId: string): Record<string, unknown> {
  return JSON.parse(
    fs.readFileSync(
      path.join(repoRoot, "dist", "extensions", pluginId, "natesclaw.plugin.json"),
      "utf8",
    ),
...
function readBundledPackageJson(repoRoot: string, pluginId: string) {
  ...
  ) as { natesclaw?: { extensions?: string[] } };
}
...
describe("copyBundledPluginMetadata", () => {
  it("copies plugin manifests, package metadata, and local skill directories", () => {
    ...
    expect(
      fs.existsSync(path.join(repoRoot, "dist", "extensions", "acpx", "natesclaw.plugin.json")),
    ).toBe(true);
    expect(
      fs.readFileSync(
    ...
    expect(packageJson.natesclaw?.extensions).toEqual(["./index.js"]);
  });
  ...
  it("relocates node_modules-backed skill paths into bundled-skills and rewrites the manifest", () => {
    const repoRoot = makeRepoRoot("natesclaw-bundled-plugin-node-modules-");
  ...
  it("falls back to repo-root hoisted node_modules skill paths", () => {
    const repoRoot = makeRepoRoot("natesclaw-bundled-plugin-hoisted-skill-");
    const pluginDir = createTlonSkillPlugin(repoRoot);
  ...
  it("omits missing declared skill paths and removes stale generated outputs", () => {
    const repoRoot = makeRepoRoot("natesclaw-bundled-plugin-missing-skill-");
  ...
  it("removes generated outputs for plugins no longer present in source", () => {
    const repoRoot = makeRepoRoot("natesclaw-bundled-plugin-removed-");
    const staleBundledSkillDir = path.join(
      repoRoot,
      "dist",
      "extensions",
    ...
    writeJson(path.join(repoRoot, "dist", "extensions", "removed-plugin", "natesclaw.plugin.json"), {
      id: "removed-plugin",
      configSchema: { type: "object" },
  ...
  it("removes stale dist outputs when a source extension directory no longer has a manifest", () => {
    const repoRoot = makeRepoRoot("natesclaw-bundled-plugin-manifestless-source-");
    ...
    writeJson(path.join(staleDistDir, "natesclaw.plugin.json"), {
  ...
  it("removes non-packaged private QA plugin metadata unless private QA build is enabled", () => {
    ...
    copyBundledPluginMetadataWithEnv({
      repoRoot,
      env: { NATESCLAW_BUILD_PRIVATE_QA: "1" } as NodeJS.ProcessEnv,
    });

    expect(fs.existsSync(path.join(staleDistDir, "natesclaw.plugin.json"))).toBe(true);
  ...
  ] as const)("$name", ({ pluginId, packageName, packageNatesclaw, env, expectedExists }) => {
    const repoRoot = makeRepoRoot(`natesclaw-bundled-plugin-${pluginId}-`);
    createPlugin(repoRoot, {
      id: pluginId,
      packageName,
      packageNatesclaw,
  ...
  it("preserves manifest-less runtime support package outputs and copies package metadata", () => {
    const repoRoot = makeRepoRoot("natesclaw-bundled-runtime-support-");
    ...
    expect(
      fs.existsSync(
        path.join(repoRoot, "dist", "extensions", "image-generation-core", "natesclaw.plugin.json"),
      ),
    ).toBe(false);
    expect(
      JSON.parse(
  ...
  it("refuses to remove dist plugin trees through a symlinked dist root", () => {
    const repoRoot = makeRepoRoot("natesclaw-bundled-plugin-meta-symlink-");

...
import { resolvePositiveTimeoutSeconds } from "natesclaw/plugin-sdk/provider-web-fetch";
import { resolveSecretInputString, normalizeSecretInput } from "natesclaw/plugin-sdk/secret-input";
...
function resolveFirecrawlSearchConfig(cfg?: NatesclawConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: NatesclawConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: NatesclawConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: NatesclawConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
...
export function resolveFirecrawlOnlyMainContent(cfg?: NatesclawConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: NatesclawConfig, override?: number): number {

...
const config: NatesclawConfig = {
  agents: {
    defaults: { workspace: "/tmp/natesclaw-memory-step" },
...
function planFor(
  ...
  const items: MigrationPlan["items"] = [
    ...planned.map((id) => ({
      ...
      target: `/tmp/natesclaw-memory-step/memory/imports/${providerId}/${id}.md`,
    })),
    ...conflicts.map((id) => ({
      id,
      kind: "memory" as const,
  ...
  return {
    ...
    target: "/tmp/natesclaw-memory-step",
    items,
    summary: {
      total: items.length,
...
describe("runSetupMemoryImportStep", () => {
  ...
  it("does not apply when a single-provider import is declined", async () => {
    ...
    // The skip hint must not suggest `natesclaw migrate <id>`: that command runs
    // the full provider migration, not a memory-only retry.
    expect(notes).toContain("Memory import page");
    expect(notes).not.toContain("natesclaw migrate");
  });

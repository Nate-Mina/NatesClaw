...
import { sleep } from "natesclaw/plugin-sdk/text-utility-runtime";
import { loadWebMedia } from "natesclaw/plugin-sdk/web-media";
import type { MarkdownTableMode, MSTeamsReplyStyle, NatesclawConfig } from "../runtime-api.js";
...
export function renderReplyPayloadsToMessages(
  ...
  const tableMode =
    ...
    getMSTeamsRuntime().channel.text.resolveMarkdownTableMode({
      cfg: getMSTeamsRuntime().config.current() as NatesclawConfig,
      channel: "msteams",
    });

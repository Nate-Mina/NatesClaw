...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__natesclaw_channel_id";
...
type InteractionCallbackConfig = Pick<NatesclawConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "natesclaw-mattermost-interactions").update(botToken).digest("hex");
}

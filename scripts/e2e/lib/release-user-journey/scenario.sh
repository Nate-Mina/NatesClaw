...
export OPENAI_API_KEY="sk-natesclaw-release-user-journey"
export NATESCLAW_GATEWAY_TOKEN="release-user-journey-token"
export CLICKCLACK_BOT_TOKEN="clickclack-release-token"

PORT="18789"
MOCK_PORT="44180"
CLICKCLACK_PORT="44181"
SUCCESS_MARKER="NATESCLAW_E2E_OK_RELEASE_USER_JOURNEY"
...
natesclaw plugins install "$plugin_a_dir" --force >"$PLUGIN_A_INSTALL_LOG" 2>&1
node scripts/e2e/lib/release-user-journey/assertions.mjs \
  ...
  "$plugin_a_dir"
natesclaw journey-a ping >"$PLUGIN_A_CLI_LOG" 2>&1
...
natesclaw plugins uninstall journey-plugin-a --force >"$PLUGIN_A_UNINSTALL_LOG" 2>&1
node scripts/e2e/lib/release-user-journey/assertions.mjs \
...
natesclaw plugins install "$plugin_b_dir" --force >"$PLUGIN_B_INSTALL_LOG" 2>&1
natesclaw journey-b ping >"$PLUGIN_B_CLI_LOG" 2>&1
...
natesclaw plugins inspect journey-plugin-b --runtime --json >"$PLUGIN_B_AFTER_RESTART_JSON" 2>&1
natesclaw channels status --json >"$STATUS_AFTER_RESTART_JSON" 2>"$STATUS_AFTER_RESTART_ERR"
...
natesclaw doctor --repair --non-interactive >"$DOCTOR_LOG" 2>&1

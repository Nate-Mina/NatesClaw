...
export type NodeMatchCandidate = {
  ...
  connected?: boolean;
  /** Client id used to prefer current Natesclaw nodes over legacy migration ties. */
  clientId?: string;
};
...
function isCurrentNatesclawClient(clientId: string | undefined): boolean {
  const normalized = normalizeOptionalLowercaseString(clientId) ?? "";
  return normalized.startsWith("natesclaw-");
}
...
function pickPreferredLegacyMigrationMatch(
  ...
  const current = matches.filter((match) => isCurrentNatesclawClient(match.clientId));
  if (current.length !== 1) {
    return undefined;
  }
  ...
  // During Clawdbot -> Natesclaw migration, a unique current client should win only
...
function scoreNodeCandidate(node: NodeMatchCandidate, matchScore: number): number {
  ...
  if (isCurrentNatesclawClient(node.clientId)) {
    score += 10;

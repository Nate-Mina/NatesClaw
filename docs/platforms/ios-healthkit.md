...
Natesclaw can request a read-only summary of the current calendar day from a
...
- An iPhone or iPad running the Natesclaw iOS app where HealthKit reports health data as
  available.
...
- Readable Health data for any metrics you expect to see. An Apple Watch can
  contribute data to the Apple Health store, but the Natesclaw watchOS app is
...
`natesclaw.json`. Preserve any commands already present:
...
3. Read the disclosure, then choose which Health categories Natesclaw may read
   in Apple's permission sheet.

The switch records your explicit Natesclaw sharing choice. It does not claim
...
```bash
natesclaw nodes describe --node "<iOS device name>"
```
...
- Natesclaw requests read access only. It cannot add or modify Health data.
- Natesclaw reads HealthKit only when `health.summary` is invoked. There is no
...
- HealthKit deliberately does not reveal whether read access was denied. A
  ...
  data type. Natesclaw cannot distinguish those cases.
...
Run `natesclaw nodes pending` and approve any capability update, then inspect
`natesclaw nodes describe --node "<iOS device name>"` again.
...
Natesclaw's access in Apple's Health settings, but do not treat an empty result

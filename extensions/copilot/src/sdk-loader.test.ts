...
describe("sdk-loader", () => {
  ...
  it("default fallback importer resolves and imports the installed SDK entry", async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        "module.exports = { natesclawDefaultImporterSentinel: true };",
      );
      ...
      })) as unknown as { natesclawDefaultImporterSentinel?: boolean };

      expect(sdk.natesclawDefaultImporterSentinel).toBe(true);
      expect(primaryImport).toHaveBeenCalledTimes(1);
    } finally {
      rmSync(tmp, { recursive: true, force: true });
    }
  ...
  it("throws an actionable error with plugin install instructions when both probes fail", async () => {
    ...
    await expect(
      ...
      message: expect.stringContaining("natesclaw plugins install @natesclaw/copilot"),
    });
  ...
  it("error message includes the fallback path and underlying primary error", async () => {
    ...
    expect(message).toContain("natesclaw plugins install @natesclaw/copilot");
  });
  ...
  it("resolves the fallback install from NATESCLAW_STATE_DIR", async () => {

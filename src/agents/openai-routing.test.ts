// Verifies OpenAI model selections route between Natesclaw and Codex runtimes.
...
describe("OpenAI runtime routing policy", () => {
  ...
  ])("keeps %s values on the Natesclaw runtime", (_label, params) => {
    const config = {
      agents: {
        defaults: {
          models: {
  ...
  it("fails closed to Natesclaw when the provider artifact is unavailable", () => {
  ...
  it("does not force Codex for custom OpenAI-compatible base URLs", () => {
    ...
    expect(resolveOpenAIImplicitAgentRuntime({ provider: "openai", config })).toBe("natesclaw");
  ...
  it("honors explicit model runtime policy before the OpenAI base URL default", () => {
    ...
    expect(
      modelSelectionShouldEnsureCodexPlugin({
        model: "openai/gpt-5.5",
        config: officialNatesclawConfig,
      }),
    ).toBe(false);
  });

  it("honors the deprecated whole-agent Natesclaw runtime opt-out", () => {
    const config = {
      agents: {
        defaults: { agentRuntime: { id: "natesclaw" } },
        list: [{ id: "worker", agentRuntime: { id: "natesclaw" } }],
      },
    } satisfies NatesclawConfig;
  ...
  it("keeps per-model Codex policy above the whole-agent Natesclaw opt-out", () => {
    const config = {
      agents: {
        defaults: {
  ...
  it("keeps per-model auto policy above the whole-agent Natesclaw opt-out", () => {
    const config = {
      agents: {
        defaults: {
          agentRuntime: { id: "natesclaw" },
  ...
  it("keeps explicit Natesclaw plus Codex auth profile under the unified OpenAI provider", () => {
  ...
  it("keeps explicit OpenAI Natesclaw API-key auth order ahead of Codex backups", () => {
    const config = {
      auth: {
        order: {
  ...
  it("does not route custom OpenAI-compatible Natesclaw configs through Codex auth order", () => {
    const config = {
      models: {
        providers: {
          openai: {

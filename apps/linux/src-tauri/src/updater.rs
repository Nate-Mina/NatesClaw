    ...
    let update = match updater.check().await {
        ...
        Ok(None) => {
            deliver_result(
                ...
                NOT_AVAILABLE_EVENT,
                (),
                "Natesclaw is up to date — no update is available",
            );
            return;
        }
        Err(error) => {
...
fn deliver_result<S: Serialize + Clone>(
    ...
    if notify {
        crate::notify::notify(app, "Natesclaw", notification_body);
    }
}
...
fn ready_notification_body(version: &str) -> String {
    format!("Update ready — restart Natesclaw to install v{version}")
}
...
mod tests {
    ...
    fn install_kind_covers_every_platform_path() {
        ...
        assert_eq!(
            install_kind_from_appimage_env(
                Some(OsString::from("/tmp/Natesclaw.AppImage")),
                Platform::Linux,
            ),
            InstallKind::SelfInstall
        );
    ...
    fn notification_copy_includes_update_version() {
        assert_eq!(
            ...
            "Update ready — restart Natesclaw to install v2026.7.16"
        );
        assert_eq!(

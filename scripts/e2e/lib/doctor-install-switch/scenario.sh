...
export PATH="/tmp/natesclaw-bin:$PATH"
mkdir -p /tmp/natesclaw-bin
cp scripts/e2e/lib/doctor-install-switch/shims/systemctl /tmp/natesclaw-bin/systemctl
cp scripts/e2e/lib/doctor-install-switch/shims/loginctl /tmp/natesclaw-bin/loginctl
chmod +x /tmp/natesclaw-bin/systemctl /tmp/natesclaw-bin/loginctl

package_tgz="${NATESCLAW_CURRENT_PACKAGE_TGZ:?missing NATESCLAW_CURRENT_PACKAGE_TGZ}"
git_root="/tmp/natesclaw-git"
mkdir -p "$git_root"
...
(
  cd "$git_root"
  natesclaw_e2e_maybe_timeout "${NATESCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit >/tmp/natesclaw-git-install.log 2>&1
  git init -q
  git config user.email "docker-e2e@natesclaw.local"
  git config user.name "Natesclaw Docker E2E"
  git add -A --
  git commit -qm "test fixture"
)
npm_log="/tmp/natesclaw-doctor-switch-npm-install.log"
if ! natesclaw_e2e_maybe_timeout "${NATESCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$package_tgz" >"$npm_log" 2>&1; then
  natesclaw_e2e_print_log "$npm_log"
  exit 1
fi

npm_bin="/tmp/npm-prefix/bin/natesclaw"
npm_root="/tmp/npm-prefix/lib/node_modules/natesclaw"
...
git_cli="$git_root/natesclaw.mjs"

package_version="$(node -p "require(\"$npm_root/package.json\").version")"
update_doctor_env="NATESCLAW_UPDATE_IN_PROGRESS=1"
update_doctor_env+=" NATESCLAW_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE=1"
update_doctor_env+=" NATESCLAW_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART=1"
update_doctor_env+=" NATESCLAW_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR=1"
update_doctor_env+=" NATESCLAW_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION=0"
...
use_default_service_identity() {
  ...
  rm -rf \
    "$account_home/.natesclaw" \
    "$account_home/.config/systemd/user/natesclaw-gateway.service" \
    "$account_home/.config/fish" \
    "$account_home/.config/powershell" \
    "$account_home/.local/bin/natesclaw-wrapper" \
    "$account_home/natesclaw-wrapper-argv.log"
...
run_flow() {
  ...
  local install_log="/tmp/natesclaw-doctor-switch-${name}-install.log"
  local doctor_log="/tmp/natesclaw-doctor-switch-${name}-doctor.log"
  local command_timeout="${NATESCLAW_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"
  ...
  if ! natesclaw_e2e_maybe_timeout "$command_timeout" bash -c "$install_cmd" >"$install_log" 2>&1; then
  ...
  unit_path="$HOME/.config/systemd/user/natesclaw-gateway.service"
  if [ ! -f "$unit_path" ]; then
  ...
  if ! natesclaw_e2e_maybe_timeout "$command_timeout" bash -c "$doctor_cmd" >"$doctor_log" 2>&1; then
...
run_cross_state_approval_flow() {
  ...
  local automated_log="/tmp/natesclaw-doctor-switch-${name}-automated.log"
  local direct_log="/tmp/natesclaw-doctor-switch-${name}-direct.log"
  local command_timeout="${NATESCLAW_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"
  ...
  local default_state_dir="$HOME/.natesclaw"
  ...
  local state_database="$custom_state_dir/state/natesclaw.sqlite"
  ...
  if ! natesclaw_e2e_maybe_timeout "$command_timeout" env \
    NATESCLAW_STATE_DIR="$custom_state_dir" \
    NATESCLAW_CONFIG_PATH="$custom_state_dir/natesclaw.json" \
    NATESCLAW_UPDATE_IN_PROGRESS=1 \
    "$npm_bin" doctor --repair --yes --non-interactive >"$automated_log" 2>&1; then
    natesclaw_e2e_print_log "$automated_log"
    exit 1
  fi
  ...
  if ! natesclaw_e2e_maybe_timeout "$command_timeout" env \
    -u NATESCLAW_UPDATE_IN_PROGRESS \
    NATESCLAW_STATE_DIR="$custom_state_dir" \
    NATESCLAW_CONFIG_PATH="$custom_state_dir/natesclaw.json" \
    "$npm_bin" doctor --repair --yes --non-interactive >"$direct_log" 2>&1; then
    natesclaw_e2e_print_log "$direct_log"
    exit 1
  fi
...
run_proxy_env_flow() {
  ...
  if ! natesclaw_e2e_maybe_timeout "$command_timeout" env \
    ...
    NATESCLAW_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART=1 \
    NATESCLAW_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR=1 \
    NATESCLAW_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION=0 \
...
run_wrapper_flow() {
  ...
  local install_log="/tmp/natesclaw-doctor-switch-${name}-install.log"
  local reinstall_log="/tmp/natesclaw-doctor-switch-${name}-reinstall.log"
  local env_repair_log="/tmp/natesclaw-doctor-switch-${name}-env-repair.log"
  local doctor_log="/tmp/natesclaw-doctor-switch-${name}-doctor.log"
  local clear_log="/tmp/natesclaw-doctor-switch-${name}-clear.log"
  local command_timeout="${NATESCLAW_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"
  ...
  local wrapper="$HOME/.local/bin/natesclaw-wrapper"
  node scripts/e2e/lib/doctor-install-switch/write-wrapper.mjs \
    "$wrapper" \
    "$npm_bin" \
    "$HOME/natesclaw-wrapper-argv.log"

  local unit_path="$HOME/.config/systemd/user/natesclaw-gateway.service"

  if ! natesclaw_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" --force >"$install_log" 2>&1; then
    natesclaw_e2e_print_log "$install_log"
    exit 1
  fi
  assert_exec_arg "$unit_path" 1 "$wrapper"
  assert_exec_arg "$unit_path" 2 "gateway"
  assert_env_value "$unit_path" "NATESCLAW_WRAPPER" "$wrapper"

  if ! natesclaw_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --force >"$reinstall_log" 2>&1; then
  ...
  sed -i "/^Environment=NATESCLAW_WRAPPER=/d" "$unit_path"
  if ! natesclaw_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" >"$env_repair_log" 2>&1; then
  ...
  sed -i "s#^Environment=NATESCLAW_WRAPPER=.*#Environment=NATESCLAW_WRAPPER=/tmp/stale-natesclaw-wrapper#" "$unit_path"
  if ! natesclaw_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" >"$env_repair_log" 2>&1; then
    natesclaw_e2e_print_log "$env_repair_log"
    exit 1
  fi
  ...
  if ! natesclaw_e2e_maybe_timeout "$command_timeout" node "$git_cli" doctor --repair --force --yes >"$doctor_log" 2>&1; then
    natesclaw_e2e_print_log "$doctor_log"
    exit 1
  fi
  if ! grep -Fq "Gateway service invokes NATESCLAW_WRAPPER:" "$doctor_log"; then
  ...
  if ! natesclaw_e2e_maybe_timeout "$command_timeout" env NATESCLAW_WRAPPER= "$npm_bin" gateway install --force >"$clear_log" 2>&1; then
    natesclaw_e2e_print_log "$clear_log"
    exit 1
  fi
  assert_no_env_key "$unit_path" "NATESCLAW_WRAPPER"

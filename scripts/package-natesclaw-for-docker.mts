#!/usr/bin/env node
// Builds the Natesclaw package artifact used by Docker E2E.
...
const PACKAGE_BUILD_PLUGIN_SELECTION_ENV_NAMES = [
  ...
  "NATESCLAW_DOCKER_BUILD_EXTENSIONS",
...
function resolvePackedNatesclawFileName(value: string) {
  const filename = value.trim();
  if (
    !filename.endsWith(".tgz") ||
    (!filename.startsWith("natesclaw-") &&
      !filename.includes(":") &&
      !filename.includes("/") &&
      !filename.includes("\\"))
  ) {
  ...
  if (
    ...
    throw new Error(`npm pack reported unsafe Natesclaw tarball filename: ${filename}`);
  }
  return filename;
}
...
const PACKAGE_ARTIFACT_BUILD_STEPS = [
  {
    label: "Building Natesclaw package artifacts",
    command: "pnpm",
  ...
  for (const step of PACKAGE_ARTIFACT_BUILD_STEPS) {
    ...
    await runImpl(step.command, step.args, sourceDir, {
      ...
      timeoutMs: resolveTimeoutMs(
        "NATESCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS",
...
async function newestNatesclawTarball(outputDir: string, packOutput: string) {
  let fromOutput = "";
  try {
    ...
    for (const entry of resolveNpmJsonEntries(parsed)) {
      ...
      const filename = resolvePackedNatesclawFileName(filenameValue);
      if (filename) {
        fromOutput = filename;
      }
    }
  ...
  for (const line of packOutput.split(/\r?\n/u)) {
    const filename = resolvePackedNatesclawFileName(line);
    if (filename) {
      fromOutput = filename;
    }
  }
  ...
  const packed = entries
    .filter((entry) => {
      try {
        return resolvePackedNatesclawFileName(entry) === entry;
      } catch {
        return false;
      }
    })
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed Natesclaw tarball in ${outputDir}`);
  }
...
function isPackedAiRuntimeTarball(filename: string) {
  return /^natesclaw-ai-[A-Za-z0-9._-]+\.tgz$/u.test(filename);
}
...
export async function prepareBundledAiRuntimePackage(
  ...
  const aiRuntimePath = path.join(sourceDir, "node_modules", "@natesclaw", "ai");
  ...
  const extractAiRuntime =
    ...
    ((tarballPath: string, destination: string) =>
      ...
      run("tar", ["-xzf", tarballPath, "-C", destination, "--strip-components=1"], destination, {
        timeoutMs: resolveTimeoutMs(
          "NATESCLAW_DOCKER_PACKAGE_PACK_TIMEOUT_MS",
...
export async function packNatesclawPackageForDocker(
  sourcePath: string,
  outputPath: string,
  packageOptions: PackageOptions = {},
) {
  ...
  console.error("==> Packing Natesclaw package");
  ...
  try {
    await cleanPackedNatesclawTarballs(outputPath);
  ...
  let tarball = await newestNatesclawTarball(outputPath, packageOptions.pnpmPack ? "" : packOutput);
...
export async function writePackageInventoryForDocker(
  ...
  await runImpl(
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "NATESCLAW_DOCKER_PACKAGE_INVENTORY_TIMEOUT_MS",
...
async function main() {
  ...
  console.error("==> Writing Natesclaw package inventory");
  ...
  console.error("==> Checking Natesclaw package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [
      path.join(ROOT_DIR, "scripts/check-natesclaw-package-tarball.mjs"),
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "NATESCLAW_DOCKER_PACKAGE_TARBALL_CHECK_TIMEOUT_MS",

...
export function isUnresolvedShellReference(value: string): boolean {
  ...
  //   $VAR_NAME          (simple reference, Natesclaw env-var style)
...
type ParsedStateDirDotEnv = {
   ...
   * reference ($VAR, ${VAR}, or $(cmd)). These are still Natesclaw-managed keys:
 ...
 * literal references for keys Natesclaw previously managed.
 */
...
/**
 * Read and parse `~/.natesclaw/.env` (or `$NATESCLAW_STATE_DIR/.env`), returning

...
read_when:
  - Setting up Natesclaw for the first time
  - Looking for common configuration patterns
  - Navigating to specific config sections
title: "Configuration"
---

Natesclaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.natesclaw/natesclaw.json`. If the file is missing, Natesclaw uses safe defaults.

The active config path must be a regular file. Natesclaw-owned writes replace it atomically (rename onto the path), so a symlinked `natesclaw.json` gets its target replaced rather than written through - avoid symlinked config layouts. If you keep config outside the default state directory, point `NATESCLAW_CONFIG_PATH` directly at the real file.
...
<Tip>
**New to configuration?** Start with `natesclaw onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    natesclaw onboard       # full onboarding flow
    natesclaw configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.natesclaw/natesclaw.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Natesclaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

`natesclaw config schema` prints the canonical JSON Schema used by Control UI
...
- Only diagnostic commands work (`natesclaw doctor`, `natesclaw logs`, `natesclaw health`, `natesclaw status`)
- Run `natesclaw doctor` to see exact issues
- Run `natesclaw doctor --fix` (`--repair` is the same flag; `--yes` skips prompts) to apply repairs

The Gateway keeps a trusted last-known-good copy after each successful startup,
but startup and hot reload do not restore it automatically - only `natesclaw doctor --fix`
does. If `natesclaw.json` fails validation (including plugin-local validation), Gateway
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    ...
    ```bash
    natesclaw config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `NATESCLAW_APNS_RELAY_BASE_URL` and `NATESCLAW_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    ...
    - `NATESCLAW_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Use a dedicated `hooks.token`; do not reuse active Gateway auth secrets (`gateway.auth.token` / `NATESCLAW_GATEWAY_TOKEN` or `gateway.auth.password` / `NATESCLAW_GATEWAY_PASSWORD`).
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-natesclaw-token`); query-string tokens are rejected.
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **Natesclaw-owned writes**: when a write changes only one top-level section
      backed by a single-file include such as `plugins: { $include: "./plugins.json5" }`,
      Natesclaw updates that included file and leaves `natesclaw.json` intact
    - **Unsupported write-through**: root includes, include arrays, and includes
      with sibling overrides fail closed for Natesclaw-owned writes instead of
      flattening the config
    - **Confinement**: `$include` paths must resolve under the directory holding
      `natesclaw.json`. To share a tree across machines or users, set
      `NATESCLAW_INCLUDE_ROOTS` to a path-list (`:` on POSIX, `;` on Windows) of
...
The Gateway watches `~/.natesclaw/natesclaw.json` and applies changes automatically - no manual restart needed for most settings.

Direct file edits are treated as untrusted until they validate. The watcher waits
for editor temp-write/rename churn to settle, reads the final file, and rejects
invalid external edits without rewriting `natesclaw.json`. Natesclaw-owned config
...
config`, inspect the config, run `natesclaw config validate`, then run `natesclaw
...
The earlier `hot` and `restart` modes are retired; [`natesclaw doctor --fix`](/cli/doctor) maps both to `hybrid`. Reload debounce is no longer configurable and runs behind a built-in default.
...
When you edit a source file that is referenced through `$include`, Natesclaw plans
...
```bash
natesclaw gateway call config.get --params '{}'  # capture payload.hash
natesclaw gateway call config.patch --params '{
...
Natesclaw reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.natesclaw/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Natesclaw runs your login shell and imports only the missing keys:
...
Env var equivalent: `NATESCLAW_LOAD_SHELL_ENV=1`. Default `timeoutMs`: `15000`.
</Accordion>

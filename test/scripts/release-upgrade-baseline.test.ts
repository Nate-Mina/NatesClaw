...
describe("release upgrade baseline resolver", () => {
  ...
  it("does not pick a newer stable release for a prerelease candidate", () => {
    ...
    ).toBe("natesclaw@2026.6.6");
  });

  it("compares prerelease versions with semver ordering", () => {
    expect(compareNatesclawVersions("2026.6.7-beta.2", "2026.6.7-beta.10")).toBeLessThan(0);
    expect(compareNatesclawVersions("2026.6.7", "2026.6.7-beta.10")).toBeGreaterThan(0);
  });
});

...
/** Formats the Bonjour instance name while preserving user-provided Natesclaw names. */
export function formatBonjourInstanceName(displayName: string) {
  const trimmed = displayName.trim();
  if (!trimmed) {
    return "Natesclaw";
  }
  if (/natesclaw/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (Natesclaw)`;
}
...
export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  ...
  const envPath = env.NATESCLAW_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "natesclaw");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.NATESCLAW_TAILNET_DNS?.trim();

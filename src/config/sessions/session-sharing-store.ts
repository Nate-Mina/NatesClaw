...
type SessionMemberDatabase = Pick<NatesclawAgentKyselyDatabase, "session_members">;
...
function resolveDatabaseOptions(scope: SessionAccessScope): NatesclawAgentDatabaseOptions {
  return toDatabaseOptions(resolveSqliteScope(scope));
}

function getSessionMemberKysely(database: NatesclawAgentDatabase) {
...
export function listSessionMembers(scope: SessionAccessScope): SessionMember[] {
  const database = openNatesclawAgentDatabase(resolveDatabaseOptions(scope));
...
export function addSessionMember(
  ...
  const inserted = runNatesclawAgentWriteTransaction((database) => {
...
export function removeSessionMember(
  ...
  const options = resolveDatabaseOptions(scope);
  return runNatesclawAgentWriteTransaction((database) => {
    assertAuthorizedSessionInstance(
      database,
      resolveSqliteScope(scope).sessionKey,
      expectedSessionId,

...
import { FsSafeError, pathExists, root as fsRoot } from "natesclaw/plugin-sdk/security-runtime";
import {
...
const WIKI_VAULT_SCAFFOLD = ["AGENTS.md", "WIKI.md", "index.md", ".natesclaw-wiki/log.jsonl"];
...
This vault is maintained by the Natesclaw memory-wiki plugin.
...
- Compiled query and prompt snapshots live in Natesclaw plugin state, not vault files.
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(rootDir, ".natesclaw-wiki/log.jsonl", "", createdFiles);

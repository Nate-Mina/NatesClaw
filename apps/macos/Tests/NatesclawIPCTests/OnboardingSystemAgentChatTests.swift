...
private func systemAgentSessionID(from message: URLSessionWebSocketTask.Message) -> String? {
    ...
    guard let data,
          ...
          object["method"] as? String == "natesclaw.chat",
...
struct OnboardingSystemAgentChatTests {
    ...
    @Test func `relaunch with pending inference resumes Natesclaw`() async throws {
    ...
    @Test func `superseded resume cannot finish a replacement route handoff`() async throws {
        ...
        let session = GatewayTestWebSocketSession(taskFactory: {
            GatewayTestWebSocketTask(sendHook: { task, message, sendIndex in
                ...
                await methods.record(method)
                if respondToSystemAgentHealth(task: task, id: id, method: method) { return }
                guard method == "natesclaw.setup.verify" else { return }
        ...
        for _ in 0..<200 {
            if await methods.snapshot() == ["health", "natesclaw.setup.verify"] {
                break
            }
            try? await Task.sleep(nanoseconds: 5_000_000)
        }
        ...
        #expect(await methods.snapshot() == ["health", "natesclaw.setup.verify"])
    }
    ...
    @Test func `chat session stays bound to its original gateway route`() async throws {
        ...
        #expect(chat.errorMessage == "The Gateway connection changed. Restart Natesclaw to reconnect.")
    ...
    @Test func `cancelled initial request exposes restart and recovers`() async throws {
        ...
        #expect(chat.errorMessage == "Natesclaw was interrupted. Restart to try again.")

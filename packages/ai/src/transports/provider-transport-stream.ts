 ...
 *
 * Routes models that need Natesclaw-managed proxy/TLS/local-service semantics onto built-in transport implementations.
 */
...
const SIMPLE_TRANSPORT_API_ALIAS: Record<string, Api> = {
  "openai-responses": "natesclaw-openai-responses-transport",
  "openai-chatgpt-responses": "natesclaw-openai-chatgpt-responses-transport",
  "openai-completions": "natesclaw-openai-completions-transport",
  "azure-openai-responses": "natesclaw-azure-openai-responses-transport",
  "anthropic-messages": "natesclaw-anthropic-messages-transport",
  "google-generative-ai": "natesclaw-google-generative-ai-transport",
};
...
function hasNatesclawTransportRequirement(model: Model): boolean {
  return getAiTransportHost().requiresManagedTransport(model);
}

/** Returns whether Natesclaw has a managed transport implementation for this API. */
...
export function createTransportAwareStreamFnForModel(
  ...
  if (!hasNatesclawTransportRequirement(model)) {
    return undefined;
  }
...
/** Creates a managed Natesclaw transport stream for explicit fallback/runtime callers. */
export function createNatesclawTransportStreamFnForModel(
  model: Model,
  ctx?: ProviderTransportStreamContext,
): StreamFn | undefined {
  // Explicit fallback callers use this when they need Natesclaw's HTTP
...
export function createBoundaryAwareStreamFnForModel(
  ...
  // HTTP streams preserve the same Natesclaw request contract.
  if (!isTransportAwareApiSupported(model.api)) {
    return undefined;
  }
  return createSupportedTransportStreamFn(model, ctx);

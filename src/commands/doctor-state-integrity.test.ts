...
function withMainAgentRoster(cfg: NatesclawConfig): NatesclawConfig {
...
function setupSessionState(cfg: NatesclawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
function createAgentDir(agentId: string, includeNestedAgentDir = true) {
  ...
  if (!stateDir) {
    throw new Error("NATESCLAW_STATE_DIR is not set");
  }
...
async function runStateIntegrity(cfg: NatesclawConfig) {
...
async function runStateIntegrityText(cfg: NatesclawConfig): Promise<string> {
...
describe("structured state integrity findings", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["HOME", "NATESCLAW_HOME", "NATESCLAW_STATE_DIR"]);
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-doctor-state-integrity-"));
    setTestEnvValue("HOME", tempHome);
    setTestEnvValue("NATESCLAW_HOME", tempHome);
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    deleteTestEnvValue("NATESCLAW_OAUTH_DIR");
  ...
  it("prompts for oauth dir when NATESCLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.NATESCLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: NatesclawConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(hasRepairPromptMessage(confirmRuntimeRepair, "Create OAuth dir at")).toBe(true);
  ...
  it("does not let NATESCLAW_AGENT_DIR hide an unconfigured agent dir", async () => {
    ...
    setTestEnvValue("NATESCLAW_AGENT_DIR", legacyAgentDir);
  ...
  it("warns about tombstoned subagent restart recovery sessions", async () => {
    ...
    expect(text).toContain("natesclaw tasks maintenance --apply");
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.NATESCLAW_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      try {
        const symlinkStateDir = path.join(symlinkHome, ".natesclaw");
        setTestEnvValue("HOME", symlinkHome);
        setTestEnvValue("NATESCLAW_HOME", symlinkHome);
        setTestEnvValue("NATESCLAW_STATE_DIR", symlinkStateDir);
  ...
  it("prints natesclaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: NatesclawConfig = {};
    writeSessionStore(cfg, {
      "agent:main:main": {
    ...
    expect(text).toMatch(/natesclaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(
      /natesclaw sessions cleanup --store ".*sessions\.json" --dry-run --fix-missing/,
    );
    expect(text).not.toMatch(
      /natesclaw sessions cleanup --store ".*sessions\.json" --dry-run(?! --fix-missing)/,
    );
    expect(text).toMatch(
      /natesclaw sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
  ...
  it("does not let synthetic heartbeat metadata override mixed transcript history", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-heartbeat-main-mixed-"));
    try {
  ...
  it("does not let heartbeat-looking routing metadata skip mixed transcript checks", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-heartbeat-main-route-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("does not classify transcripts with real user activity after 400 heartbeat messages", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-heartbeat-main-cap-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
  ...
  it("keeps the heartbeat main-session helper conservative", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-heartbeat-main-helper-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("moves store entries and clears matching TUI pointers without touching others", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-tui-pointer-clear-"));
    try {
      await writeTuiLastSessionKey({
        scopeKey: "terminal",
        sessionKey: "agent:main:main",

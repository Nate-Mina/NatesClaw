...
ARG NATESCLAW_EXTENSIONS=""
ARG NATESCLAW_BUNDLED_PLUGIN_DIR=extensions
ARG NATESCLAW_DOCKER_BUILD_NODE_OPTIONS="--max-old-space-size=8192"
ARG NATESCLAW_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB=""
ARG NATESCLAW_DOCKER_BUILD_SKIP_DTS=1
...
ARG NATESCLAW_NODE_BOOKWORM_SLIM_DIGEST="sha256:6f7b03f7c2c8e2e784dcf9295400527b9b1270fd37b7e9a7285cf83b6951452d"
...
ARG NATESCLAW_EXTENSIONS
ARG NATESCLAW_BUNDLED_PLUGIN_DIR
...
RUN mkdir -p /out/packages "/out/${NATESCLAW_BUNDLED_PLUGIN_DIR}" && \
    ...
    node /tmp/docker-plugin-selection.mjs "/tmp/${NATESCLAW_BUNDLED_PLUGIN_DIR}" "$NATESCLAW_EXTENSIONS" \
      > /out/natesclaw-selected-plugin-dirs && \
    while IFS= read -r ext; do \
      ext_dir="/tmp/${NATESCLAW_BUNDLED_PLUGIN_DIR}/$ext"; \
      if [ -f "$ext_dir/package.json" ]; then \
        mkdir -p "/out/${NATESCLAW_BUNDLED_PLUGIN_DIR}/$ext" && \
        cp "$ext_dir/package.json" "/out/${NATESCLAW_BUNDLED_PLUGIN_DIR}/$ext/package.json"; \
      fi; \
    done < /out/natesclaw-selected-plugin-dirs

# ── Stage 2: Build ──────────────────────────────────────────────
FROM ${NATESCLAW_BUN_IMAGE} AS bun-binary
FROM ${NATESCLAW_NODE_BOOKWORM_IMAGE} AS build
ARG NATESCLAW_BUNDLED_PLUGIN_DIR
ARG NATESCLAW_DOCKER_BUILD_NODE_OPTIONS
ARG NATESCLAW_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB
ARG NATESCLAW_DOCKER_BUILD_SKIP_DTS
...
RUN set -eux; \
    if ! grep -qx 'matrix' /tmp/natesclaw-selected-plugin-dirs; then \
...
ARG GIT_COMMIT=""
ARG NATESCLAW_BUILD_TIMESTAMP=""
ENV GIT_COMMIT=${GIT_COMMIT} \
    NATESCLAW_BUILD_TIMESTAMP=${NATESCLAW_BUILD_TIMESTAMP}
...
RUN pnpm_config_verify_deps_before_run=false pnpm canvas:a2ui:bundle || \
    (echo "A2UI bundle: creating stub (non-fatal)" && \
     ...
     rm -rf vendor/a2ui apps/shared/NatesclawKit/Tools/CanvasA2UI)
# Force pnpm for UI build (Bun may fail on ARM/Synology architectures)
ENV NATESCLAW_PREFER_PNPM=1
RUN set -eu; \
    selected_plugin_dirs="$(cat /tmp/natesclaw-selected-plugin-dirs)"; \
    if [ -z "$NATESCLAW_BUILD_TIMESTAMP" ]; then \
      NATESCLAW_BUILD_TIMESTAMP="$(date -u +%Y-%m-%dT%H:%M:%SZ)"; \
      export NATESCLAW_BUILD_TIMESTAMP; \
    fi; \
    if grep -qx 'qa-lab' /tmp/natesclaw-selected-plugin-dirs; then \
...
RUN if grep -qx 'qa-lab' /tmp/natesclaw-selected-plugin-dirs; then \
...
RUN --mount=type=cache,id=natesclaw-pnpm-store,target=/root/.local/share/pnpm/store,sharing=locked \
    ...
    rm -rf \
      /app/node_modules/natesclaw \
      /app/node_modules/.bin/natesclaw \
      /app/node_modules/.pnpm/natesclaw@*/node_modules/natesclaw && \
    node scripts/check-package-dist-imports.mjs /app

# ── Runtime base image ──────────────────────────────────────────
FROM ${NATESCLAW_NODE_BOOKWORM_SLIM_IMAGE} AS base-runtime
ARG NATESCLAW_NODE_BOOKWORM_SLIM_DIGEST
LABEL org.opencontainers.image.base.name="docker.io/library/node:24-bookworm-slim" \
  org.opencontainers.image.base.digest="${NATESCLAW_NODE_BOOKWORM_SLIM_DIGEST}"
...
RUN --mount=type=cache,id=natesclaw-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=natesclaw-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    apt-get update && \
...
COPY --from=runtime-assets --chown=node:node /app/natesclaw.mjs .
COPY --from=runtime-assets --chown=node:node /app/src/agents/templates ./src/agents/templates
COPY --from=runtime-assets --chown=node:node /app/${NATESCLAW_BUNDLED_PLUGIN_DIR} ./${NATESCLAW_BUNDLED_PLUGIN_DIR}
...
# Example: docker build --build-arg NATESCLAW_IMAGE_APT_PACKAGES="python3 wget" .
# Legacy alias: NATESCLAW_DOCKER_APT_PACKAGES is still accepted as a fallback.
ARG NATESCLAW_IMAGE_APT_PACKAGES
ARG NATESCLAW_DOCKER_APT_PACKAGES=""
ENV PATH="/home/node/.local/bin:${PATH}"
RUN --mount=type=cache,id=natesclaw-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=natesclaw-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    packages="${NATESCLAW_IMAGE_APT_PACKAGES:-$NATESCLAW_DOCKER_APT_PACKAGES}"; \
    if [ -n "$packages" ]; then \
      apt-get update && \
...
# Example: docker build --build-arg NATESCLAW_IMAGE_PIP_PACKAGES="requests humanize" .
ARG NATESCLAW_IMAGE_PIP_PACKAGES=""
RUN --mount=type=cache,id=natesclaw-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=natesclaw-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    if [ -n "$NATESCLAW_IMAGE_PIP_PACKAGES" ]; then \
...
# Build with: docker build --build-arg NATESCLAW_INSTALL_BROWSER=1 ...
# Adds ~300MB but eliminates the 60-90s Playwright install on every container start.
# Must run after node_modules COPY so playwright-core is available.
ARG NATESCLAW_INSTALL_BROWSER=""
ENV PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright
RUN --mount=type=cache,id=natesclaw-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=natesclaw-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    if [ -n "$NATESCLAW_INSTALL_BROWSER" ]; then \
      apt-get update && \
...
# Build with: docker build --build-arg NATESCLAW_INSTALL_DOCKER_CLI=1 ...
# Adds ~50MB. Only the CLI is installed — no Docker daemon.
# Required for agents.defaults.sandbox to function in Docker deployments.
ARG NATESCLAW_INSTALL_DOCKER_CLI=""
ARG NATESCLAW_DOCKER_GPG_FINGERPRINT="9DC858229FC7DD38854AE2D88D81803C0EBFCD88"
RUN --mount=type=cache,id=natesclaw-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=natesclaw-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    if [ -n "$NATESCLAW_INSTALL_DOCKER_CLI" ]; then \
      apt-get update && \
      DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
      ...
      # Update NATESCLAW_DOCKER_GPG_FINGERPRINT when Docker rotates release keys.
      curl -fsSL --connect-timeout 10 --max-time 120 \
        https://download.docker.com/linux/debian/gpg -o /tmp/docker.gpg.asc && \
      expected_fingerprint="$(printf '%s' "$NATESCLAW_DOCKER_GPG_FINGERPRINT" | tr '[:lower:]' '[:upper:]' | tr -d '[:space:]')" && \
...
RUN ln -sf /app/natesclaw.mjs /usr/local/bin/natesclaw \
 && chmod 755 /app/natesclaw.mjs
...
# the leaf /home/node/.config/natesclaw inherits the correct parent permissions.
...
RUN install -d -m 0755 -o node -g node /home/node/.config && \
    install -d -m 0700 -o node -g node \
      /home/node/.natesclaw \
      /home/node/.natesclaw/workspace \
      /home/node/.config/natesclaw && \
...
ENTRYPOINT ["tini", "-s", "--"]
CMD ["node", "natesclaw.mjs", "gateway"]

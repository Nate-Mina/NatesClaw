# ClickClack Natesclaw channel

Official Natesclaw channel plugin for ClickClack.
...
`/api/bot-setup-codes/claim#CODE` endpoint. Natesclaw validates the versioned
claim response and saves its canonical API base.

For manual token setup:

```sh
natesclaw channels add clickclack \
...
Run `natesclaw onboard` for guided setup. The workspace value can be a
...
authentication gateway such as Cloudflare Access while the local Natesclaw
...
extension publishes Natesclaw's native commands for composer autocomplete,
...
ClickClack can create one managed channel for each Natesclaw session:
...
Natesclaw Control UI, preserving base paths. Main sessions use `/chat/<agent>`.
...
two Natesclaw gateways using the same ClickClack workspace do not adopt each

...
const LIVE = isLiveTestEnabled(["NATESCLAW_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "natesclaw-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("natesclaw_live_probe", "Natesclaw Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an Natesclaw-configured streamable-http server", async () => {
    const geminiCommand = process.env.NATESCLAW_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("natesclawLiveProbe");

...
async function createLockFixture() {
  const root = tempDirs.make("natesclaw-doctor-sqlite-lock-");
...
describe("doctor SQLite maintenance lock", () => {
  ...
  it("prevents Gateway startup until maintenance releases ownership", async () => {
    ...
    await expect(
      acquireGatewayLock({
        ...
        port: 18789,
        pollIntervalMs: 2,
        readProcessCmdline: () => ["natesclaw", "doctor", "--session-sqlite", "compact"],
        timeoutMs: 15,
      }),
  ...
  it("blocks maintenance when the Gateway used the multi-Gateway override", async () => {
    ...
    const gatewayLock = await acquireGatewayLock({
      allowInTests: true,
      env: { ...fixture.env, NATESCLAW_ALLOW_MULTI_GATEWAY: "1" },
      lockDir: fixture.lockDir,
      platform: "darwin",
      port: 18789,
      timeoutMs: 15,
  ...
  it("refuses explicit destructive targets outside the locked state directory", async () => {
    ...
    const externalPath = path.join(
      tempDirs.make("natesclaw-external-session-store-"),
      "sessions.json",
    );
    const run = vi.fn();
    ...
    ).rejects.toThrow(/outside the active Natesclaw state directory/);
    expect(run).not.toHaveBeenCalled();
  ...
  it("refuses dangling in-state symlinks that resolve outside ownership", async () => {
    ...
    const fixture = await createLockFixture();
    const sessionsDir = path.join(fixture.env.NATESCLAW_STATE_DIR, "agents", "main", "sessions");
    const storePath = path.join(sessionsDir, "sessions.json");
    const outsideTarget = path.join(
      tempDirs.make("natesclaw-dangling-session-target-"),
      "missing.json",
    );
  ...
  it("refuses outside path aliases that resolve into the locked state directory", async () => {
    ...
    const externalAlias = path.join(
      path.dirname(fixture.env.NATESCLAW_STATE_DIR),
      "external-state-alias",
    );
    await fs.symlink(fixture.env.NATESCLAW_STATE_DIR, externalAlias, "dir");
  ...
  it("refuses in-state hard links that can alias storage outside ownership", async () => {
    ...
    const sessionsDir = path.join(fixture.env.NATESCLAW_STATE_DIR, "agents", "main", "sessions");
    const storePath = path.join(sessionsDir, "sessions.json");
    const externalDir = path.join(path.dirname(fixture.env.NATESCLAW_STATE_DIR), "external-state");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const fixture = await createLockFixture();
      const sessionsDir = path.join(fixture.env.NATESCLAW_STATE_DIR, "agents", "main", "agent");
      const targetPath = path.join(sessionsDir, "sidecar-target");
      const sidecarPath = path.join(sessionsDir, "natesclaw-agent.sqlite-wal");
      await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const fixture = await createLockFixture();
      const agentsDir = path.join(fixture.env.NATESCLAW_STATE_DIR, "agents");
      const realAgentDir = path.join(fixture.env.NATESCLAW_STATE_DIR, "real-main");
      const aliasedAgentDir = path.join(agentsDir, "main");
      const databasePath = path.join(aliasedAgentDir, "agent", "natesclaw-agent.sqlite");
      await fs.mkdir(path.dirname(path.join(realAgentDir, "agent", "placeholder")), {
        recursive: true,
      });
      await fs.mkdir(agentsDir, { recursive: true });
      await fs.writeFile(path.join(realAgentDir, "agent", "natesclaw-agent.sqlite"), "owned\n");
      ...
      expect(run).not.toHaveBeenCalled();
      await expect(
        fs.readFile(path.join(realAgentDir, "agent", "natesclaw-agent.sqlite"), "utf8"),
      ).resolves.toBe("owned\n");
    },
  );
  ...
  it("allows explicit destructive targets owned by the locked state directory", async () => {
    ...
    const storePath = path.join(
      fixture.env.NATESCLAW_STATE_DIR,
      "agents",
      "main",
      "sessions",
      "sessions.json",

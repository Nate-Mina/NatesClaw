---
summary: "Schema-accurate configuration examples for common Natesclaw setups"
read_when:
  - Learning how to configure Natesclaw
...
Save to `~/.natesclaw/natesclaw.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.natesclaw/agents/main/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/natesclaw" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    entries: {
      main: { default: true },
      docs: { workspace: "~/.natesclaw/workspace-docs", skills: ["docs-search"] },
    },
  },
}
```
...
```json5
{
  agents: { defaults: { workspace: "~/.natesclaw/workspace" } },
  channels: {
    whatsapp: { allowFrom: ["+15555550123"], responsePrefix: "[natesclaw]" },
    telegram: {
      enabled: true,
      botToken: "YOUR_TOKEN",
      allowFrom: ["123456789"],

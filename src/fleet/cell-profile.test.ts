...
const TEST_ENVIRONMENT_FILE = "/tmp/natesclaw-fleet-env/cell.env";

function makeProfile(overrides: Partial<CellContainerProfile> = {}): CellContainerProfile {
  const tenantId = overrides.tenantId ?? "acme";
  const dataDir = overrides.dataDir ?? "/tmp/natesclaw/fleet/cells/acme";
  return {
    tenantId,
    ...
    dataDir,
    authSecretDir: overrides.authSecretDir ?? "/tmp/natesclaw/fleet/auth-profile-secrets/acme",
...
describe("fleet tenant ids", () => {
  ...
  it("derives stable container and data paths", () => {
    expect(cellContainerName("acme-2")).toBe("natesclaw-cell-acme-2");
    expect(cellNetworkName("acme-2")).toBe("natesclaw-cell-acme-2-net");
    expect(cellDataDir("/srv/natesclaw", "acme-2")).toBe(
      path.join("/srv/natesclaw", "fleet", "cells", "acme-2"),
    );
    expect(cellAuthSecretDir("/srv/natesclaw", "acme-2")).toBe(
      path.join("/srv/natesclaw", "fleet", "auth-profile-secrets", "acme-2"),
    );
  });
});
...
describe("fleet image references", () => {
  it("normalizes valid references and rejects option-like values", () => {
    expect(validateFleetImage(" ghcr.io/natesclaw/natesclaw:v1 ")).toBe(
      "ghcr.io/natesclaw/natesclaw:v1",
    );
...
describe("fleet disk limits", () => {
  ...
  it("adds storage-opt and the replay label only when configured", () => {
    ...
    expect(withDisk).toContain("natesclaw.fleet.disk-limit=10g");
    expect(withoutDisk).not.toContain("--storage-opt");
    expect(withoutDisk.join(" ")).not.toContain("natesclaw.fleet.disk-limit");
  });
});
...
describe("fleet container arguments", () => {
  it("builds the complete hardened run profile", () => {
    ...
    expectOption(args, "--name", "natesclaw-cell-acme");
    ...
    expectOption(args, "--network", "natesclaw-cell-acme-net");

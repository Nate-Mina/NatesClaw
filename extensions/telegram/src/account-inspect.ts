...
import type { TelegramAccountConfig } from "natesclaw/plugin-sdk/config-contracts";
import { resolveDefaultSecretProviderAlias } from "natesclaw/plugin-sdk/provider-auth";
import { DEFAULT_ACCOUNT_ID, normalizeAccountId } from "natesclaw/plugin-sdk/routing";
...
import { coerceSecretRef } from "natesclaw/plugin-sdk/secret-input-runtime";
...
function inspectTokenValue(params: { cfg: NatesclawConfig; value: unknown }): {
  token: string;
...
function hasConfiguredTelegramAccounts(cfg: NatesclawConfig): boolean {

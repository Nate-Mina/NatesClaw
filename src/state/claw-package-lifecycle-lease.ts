...
type ClawPackageLifecycleDatabase = Pick<NatesclawStateKyselyDatabase, "state_leases">;
...
type ClawPackageLifecycleLeaseOptions = NatesclawStateDatabaseOptions & {
  nowMs?: number;
  owner?: string;
  required?: boolean;
};
...
export function acquireClawPackageLifecycleLease(
  ...
  const databasePath = options.path ?? resolveNatesclawStateSqlitePath(env);
      ...
      ({ db }) => {
        ...
        if (existing) {
          throw new ClawPackageLifecycleBusyError(
            `Package ${artifact.ref} is being changed by another Natesclaw lifecycle; retry after ${new Date(existing.expires_at ?? expiresAt).toISOString()}.`,
          );
        }
        executeSqliteQuerySync(
          db,

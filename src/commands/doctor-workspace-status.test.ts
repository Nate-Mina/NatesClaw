...
vi.mock("../agents/agent-scope.js", () => ({
  listAgentIds: (cfg: NatesclawConfig) => mocks.listAgentIds(cfg),
...
async function runNoteWorkspaceStatusForTest(
  ...
  const cfg: NatesclawConfig = opts?.cfg ?? {};
...
describe("noteWorkspaceStatus", () => {
  ...
  it("collects plugin version drift as structured findings", async () => {
    ...
    expect(findings).toEqual([
      expect.objectContaining({
        ...
        fixHint: expect.stringContaining("natesclaw plugins update codex"),
      }),
    ]);
  });
  ...
  it("collects compatibility warnings, plugin diagnostics, and TaskFlow recovery findings", async () => {
    ...
    expect(findings).toEqual([
      ...
      expect.objectContaining({
        ...
        fixHint: expect.stringContaining("natesclaw tasks flow show flow-123"),
      }),
    ]);
  });
  ...
  it("surfaces active official managed plugin version drift", async () => {
    ...
    try {
      ...
      expect(body).toContain("1 active official plugin not on Natesclaw 2026.6.1");
      expect(body).toContain("codex: 2026.5.30-beta.1 (npm) -> expected 2026.6.1");
      expect(body).toContain("natesclaw plugins update codex");
  ...
  it("adds TaskFlow recovery hints for broken blocked flows", async () => {
    ...
    try {
      ...
      expect(body).toContain("natesclaw tasks flow show <flow-id>");
    } finally {
      noteSpy.mockRestore();
    }
  });

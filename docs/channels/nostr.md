...
read_when:
  - You want Natesclaw to receive DMs via Nostr
...
```bash
natesclaw plugins install --link <path-to-local-nostr-plugin>
```

Restart the gateway after installing or enabling plugins. Onboarding (`natesclaw onboard`) and `natesclaw channels add` surface Nostr from the shared channel catalog once the plugin is installed.

### Non-interactive setup

```bash
natesclaw channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
natesclaw channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "natesclaw@example.com",
        lud16: "natesclaw@example.com",
      },
    },
  },
}
...
1. Note the bot pubkey from gateway logs or `natesclaw channels status` (hex; convert to npub in your client if needed).

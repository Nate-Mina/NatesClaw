...
/** Env var that points bundled-plugin lookup at an Natesclaw source checkout. */
const NATESCLAW_DEV_SOURCE_ROOT_ENV = "NATESCLAW_DEV_SOURCE_ROOT";
...
/** Resolves and validates the configured Natesclaw development source root. */
export function resolveNatesclawDevSourceRoot(env: NodeJS.ProcessEnv = process.env): string | null {
  const rawRoot = env[NATESCLAW_DEV_SOURCE_ROOT_ENV]?.trim();
  if (!rawRoot) {
    return null;
  }
  ...
  if (readPackageName(path.join(realRoot, "package.json")) !== "natesclaw") {
    return null;
  }
...
export function isBundledPluginInsideDevSourceRoot(params: {
  ...
  const devSourceRoot = resolveNatesclawDevSourceRoot(params.env);
  if (!devSourceRoot) {
    return false;
  }

...
export function registerControlUiAndPairingSuite(): void {
  ...
  const startControlUiServerWithOperatorIdentity = async (
    identityPrefix = "natesclaw-device-scope-",
  ) => {
    const { server, port, prevToken } = await startControlUiServer("secret");
  ...
  test("rejects trusted-proxy control ui without device identity even with self-declared scopes", async () => {
    ...
    const { rejectDevicePairing, requestDevicePairing } =
      await import("../infra/device-pairing.js");
    const { identity } = await createOperatorIdentityFixture("natesclaw-control-ui-trusted-proxy-");
  ...
  test("bounds trusted-proxy control ui scopes to proxy-declared scope header", async () => {
    ...
    await withControlUiGatewayServer(async ({ port }) => {
      const seeded = await seedApprovedOperatorReadPairing({
        identityPrefix: "natesclaw-control-ui-trusted-proxy-bounded-",
        clientId: CONTROL_UI_CLIENT.id,
        clientMode: CONTROL_UI_CLIENT.mode,
  ...
  test("requires approval for loopback scope upgrades for control ui clients", async () => {
    ...
    const { identity, identityPath } = await seedApprovedOperatorReadPairing({
      identityPrefix: "natesclaw-device-token-scope-",
      clientId: CONTROL_UI_CLIENT.id,
      clientMode: CONTROL_UI_CLIENT.mode,
  ...
  test("returns pairing-required for malformed persisted access lists", async () => {
    const { identity, identityPath } = await seedApprovedOperatorReadPairing({
      identityPrefix: "natesclaw-device-malformed-access-",
  ...
  test("does not expose approved access when a paired device id reconnects with a different key", async () => {
    const { identity, identityPath } = await seedApprovedOperatorReadPairing({
      identityPrefix: "natesclaw-device-key-mismatch-",
      clientId: TEST_OPERATOR_CLIENT.id,
      clientMode: TEST_OPERATOR_CLIENT.mode,
  ...
  test("voice-node setup code reconnects with node and Talk-only operator tokens", async () => {
    ...
    const { identityPath, identity } = await createOperatorIdentityFixture(
      "natesclaw-bootstrap-voice-node-",
    );
    const client = {
      id: "node-host",
      version: "1.0.0",
  ...
  test.each([
    {
      name: "Android",
      identityPrefix: "natesclaw-bootstrap-android-node-",
      client: {
        id: "natesclaw-android",
        version: "2026.6.2",
        platform: "Android 16",
        mode: "node" as const,
    ...
    {
      name: "iPadOS",
      identityPrefix: "natesclaw-bootstrap-ipados-node-",
      client: {
        id: "natesclaw-ios",
        version: "2026.6.2",
  ...
  test("limited qr setup keeps the previous bounded operator handoff", async () => {
    const { identity, initial } = await connectSetupCodeBootstrapNode({
      identityPrefix: "natesclaw-bootstrap-limited-node-",
      client: {
        id: "natesclaw-ios",
        version: "2026.7.13",
        platform: "iOS 26.3.1",
  ...
  test("full qr setup upgrades an existing limited mobile pairing", async () => {
    const identityPrefix = "natesclaw-bootstrap-limited-upgrade-node-";
    const client = {
      id: "natesclaw-ios",
      version: "2026.7.13",
      platform: "iOS 26.3.1",
  ...
  test.each([
    {
      ...
      identityPrefix: "natesclaw-bootstrap-mobile-spoof-",
      client: {
        id: "natesclaw-android",
        version: "2026.6.2",
        platform: "iOS 26.3.1",
    ...
    },
    {
      ...
      identityPrefix: "natesclaw-bootstrap-node-host-spoof-",
      client: {
        id: "node-host",
        version: "2026.6.2",
        platform: "Android 16",
  ...
  test("qr bootstrap retry keeps full operator handoff after paired approval", async () => {
    ...
    const { server, port, prevToken } = await startControlUiServer("secret");
    const { identityPath, identity } = await createOperatorIdentityFixture(
      "natesclaw-bootstrap-node-retry-",
    );
    const client = {
      id: "natesclaw-ios",
      version: "2026.3.30",
  ...
  test("rejected non-baseline bootstrap request cannot recreate pending node pairing", async () => {
    ...
    const { server, port, prevToken } = await startControlUiServer("secret");
    const { identityPath, identity } = await createOperatorIdentityFixture(
      "natesclaw-bootstrap-node-reject-",
    );
    const client = {
      id: "natesclaw-ios",
      version: "2026.3.30",
  ...
  test("does not consume bootstrap token when node reconcile fails before hello-ok", async () => {
    ...
    const { identityPath, client } = await createOperatorIdentityFixture(
      "natesclaw-bootstrap-reconcile-fail-",
    );
    const nodeClient = {
      ...client,
      id: "natesclaw-android",
  ...
  test("requires approval for bootstrap-auth role upgrades on already-paired devices", async () => {
    ...
    const { identityPath, identity } = await createOperatorIdentityFixture(
      "natesclaw-bootstrap-role-upgrade-",
    );
    const client = {
      id: "natesclaw-ios",
      version: "2026.3.30",
  ...
  test("silently approves host-authorized control ui owner bootstrap tokens", async () => {
    ...
    const { identityPath, identity } = await createOperatorIdentityFixture(
      "natesclaw-bootstrap-control-ui-",
    );
  ...
  test("keeps generic control ui bootstrap tokens on the bounded profile", async () => {
    ...
    const { server, port, prevToken } = await startControlUiServer("secret");
    const { identityPath, identity } = await createOperatorIdentityFixture(
      "natesclaw-bootstrap-control-ui-bounded-",
    );
  ...
  test("silently upgrades the same control ui key with a host-authorized bootstrap", async () => {
    ...
    const { CONTROL_UI_OWNER_BOOTSTRAP_OPERATOR_SCOPES, CONTROL_UI_OWNER_BOOTSTRAP_PROFILE } =
      await import("../shared/device-bootstrap-profile.js");
    const { identity, identityPath } = await seedApprovedOperatorReadPairing({
      identityPrefix: "natesclaw-control-ui-owner-upgrade-",
      clientId: CONTROL_UI_CLIENT.id,
      clientMode: CONTROL_UI_CLIENT.mode,
    ...
    const { identityPath: secondIdentityPath } = await createOperatorIdentityFixture(
      "natesclaw-control-ui-owner-upgrade-second-browser-",
    );
  ...
  test("requires pairing for control ui bootstrap token without control-ui purpose", async () => {
    ...
    const { identityPath, identity } = await createOperatorIdentityFixture(
      "natesclaw-bootstrap-control-ui-missing-purpose-",
    );
  ...
  test("requires pairing for control ui node bootstrap tokens", async () => {
    ...
    const { identityPath, identity } = await createOperatorIdentityFixture(
      "natesclaw-bootstrap-control-ui-node-profile-",
    );
  ...
  test("auto-approves local-direct node pairing, then queues operator scope approval", async () => {
    ...
    const { identityPath, identity, client } =
      await createOperatorIdentityFixture("natesclaw-device-scope-");
  ...
  test("allows operator.read connect when device is paired with operator.admin", async () => {
    ...
    const { identityPath, identity } = await seedApprovedOperatorReadPairing({
      identityPrefix: "natesclaw-device-admin-superset-",
      clientId: TEST_OPERATOR_CLIENT.id,
      clientMode: TEST_OPERATOR_CLIENT.mode,
  ...
  test("requires approval for local scope upgrades even when paired metadata is legacy-shaped", async () => {
    const { getPairedDevice, listDevicePairing } = await import("../infra/device-pairing.js");
    const { identity, identityPath } = await seedApprovedOperatorReadPairing({
      identityPrefix: "natesclaw-device-legacy-",
      clientId: TEST_OPERATOR_CLIENT.id,
      clientMode: TEST_OPERATOR_CLIENT.mode,
  ...
  test("auto-approves Docker-style CLI connects on loopback with a private host header", async () => {
    ...
    try {
      const { identity, identityPath } =
        await createOperatorIdentityFixture("natesclaw-cli-docker-");

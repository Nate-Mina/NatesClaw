package ai.natesclaw.app.accessibility
...
class AccessibilityActionExecutor internal constructor(
  private val connectionProvider: () -> AccessibilityServiceConnection<NatesclawAccessibilityService> = {
    NatesclawAccessibilityService.connection.value
  },
  private val captureSnapshot: (NatesclawAccessibilityService) -> AccessibilitySnapshotCapture =
    AccessibilitySnapshotter()::capture,
  private val foregroundPackageProvider: (NatesclawAccessibilityService) -> String? =
    NatesclawAccessibilityService::foregroundPackageName,
  private val uiEpochProvider: () -> Long = { NatesclawAccessibilityService.uiEpoch },
) : AutoCloseable {
  ...
  private fun coordinateGesturePreflight(service: NatesclawAccessibilityService): ActionResult? {
  ...
  private fun nodeActionPackagePreflight(service: NatesclawAccessibilityService): ActionResult? {
    val expectedPackage = generation.packageName
    val currentPackage = foregroundPackageProvider(service)
    if (expectedPackage == null || currentPackage == null || expectedPackage != currentPackage) {
      return ActionResult(
  ...
  private suspend fun dispatchGesture(
    service: NatesclawAccessibilityService,

...
function useBundledPluginDirOverrideForTest(dir: string): void {
  process.env.NATESCLAW_BUNDLED_PLUGINS_DIR = dir;
}
...
describe("plugin-sdk facade runtime", () => {
  it("reuses successful facade locations without repeating filesystem probes", () => {
    const dir = createBundledPluginDir("natesclaw-facade-location-cache-", "cached");
  ...
  it("honors trusted bundled plugin dir overrides", () => {
    const overrideA = createBundledPluginDir("natesclaw-facade-runtime-a-", "override-a");
    const overrideB = createBundledPluginDir("natesclaw-facade-runtime-b-", "override-b");
  ...
  it("falls back to package source surfaces when an override dir is partial", () => {
    const overrideDir = createTrustedBundledFixtureRoot("natesclaw-facade-runtime-empty-");
  ...
  it("does not reuse enabled facade locations when bundled plugins are disabled", () => {
    const dir = createBundledPluginDir("natesclaw-facade-location-disabled-", "enabled");
    useBundledPluginDirOverrideForTest(dir);
    const params = {
      dirName: "demo",
      artifactBasename: "api.js",
  ...
  it("does not reuse installed facade locations across custom environment profiles", () => {
    const profileA: NodeJS.ProcessEnv = {
      ...
      NATESCLAW_STATE_DIR: path.join(path.sep, "natesclaw-facade-profile-a"),
    };
    const profileB: NodeJS.ProcessEnv = {
      NATESCLAW_DISABLE_BUNDLED_PLUGINS: "1",
      NATESCLAW_STATE_DIR: path.join(path.sep, "natesclaw-facade-profile-b"),
    };
    const resolveRegistryPluginModuleLocation = vi.fn(({ env }: { env?: NodeJS.ProcessEnv }) => {
      const stateDir = env?.NATESCLAW_STATE_DIR;
      if (!stateDir) {
        return null;
      }
    ...
    const profileARoot = path.join(profileA.NATESCLAW_STATE_DIR!, "plugins", "demo");
    const profileBRoot = path.join(profileB.NATESCLAW_STATE_DIR!, "plugins", "demo");
  ...
  it("retries missing facade locations when a plugin artifact appears", () => {
    const dir = createTrustedBundledFixtureRoot("natesclaw-facade-location-retry-");
    useBundledPluginDirOverrideForTest(dir);
    testing.setFacadeActivationCheckRuntimeForTest({
      resolveRegistryPluginModuleLocation: () => null,
    } as never);
  ...
  it("invalidates cached facade locations when plugin metadata changes", () => {
    const dir = createBundledPluginDir("natesclaw-facade-location-invalidation-", "original");
    useBundledPluginDirOverrideForTest(dir);
    const params = {
      dirName: "demo",
      artifactBasename: "api.js",
  ...
  it("returns the same object identity on repeated calls (sentinel consistency)", () => {
    const dir = createBundledPluginDir("natesclaw-facade-identity-", "identity-check");
    useBundledPluginDirOverrideForTest(dir);
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
  ...
  it("breaks circular facade re-entry during module evaluation", () => {
    const dir = createBundledPluginDir("natesclaw-facade-circular-", "circular-ok");
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
    };
  ...
  it("back-fills the sentinel before post-load facade tracking re-enters", () => {
    const dir = createBundledPluginDir("natesclaw-facade-post-load-", "post-load-ok");
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
    };
  ...
  it("clears the cache on load failure so retries re-execute", () => {
    const dir = createThrowingPluginDir("natesclaw-facade-throw-");
    useBundledPluginDirOverrideForTest(dir);
  ...
  it("allows runtime-api facade loads when the bundled plugin is explicitly enabled", () => {
    const dir = createTempDirSync("natesclaw-facade-runtime-enabled-");
  ...
  it("rejects hardlinked artifacts under installed plugin roots", () => {
    const installedDir = createTempDirSync("natesclaw-facade-hardlink-");
  ...
  it("keeps hardlinked artifacts loadable under core-shipped roots", () => {
    const rootDir = createTrustedBundledFixtureRoot("natesclaw-facade-hardlink-bundled-");
    const pluginDir = path.join(rootDir, "demo");
    fs.mkdirSync(pluginDir, { recursive: true });
  ...
  it("resolves a globally-installed plugin whose rootDir basename matches the dirName", () => {
    const lineDir = createTempDirSync("natesclaw-facade-global-line-");
    ...
    fs.writeFileSync(
      path.join(lineDir, "natesclaw.plugin.json"),
      JSON.stringify({
        id: "line",
        channels: ["line"],
  ...
  it("resolves a globally-installed plugin public surface from package dist", () => {
    const lineDir = createTempDirSync("natesclaw-facade-global-line-dist-");
  ...
  it("resolves a globally-installed plugin with an encoded scoped rootDir basename", () => {
    const encodedDir = createTempDirSync("natesclaw-facade-encoded-line-");
    ...
    fs.writeFileSync(
      path.join(encodedDir, "natesclaw.plugin.json"),
      JSON.stringify({
        id: "line",
        channels: ["line"],
        configSchema: { type: "object", additionalProperties: false, properties: {} },
  ...
  it("prefers the source runtime snapshot for facade activation checks", () => {
    const dir = createTempDirSync("natesclaw-facade-source-snapshot-");
    ...
    fs.writeFileSync(
      path.join(dir, "demo", "natesclaw.plugin.json"),
      JSON.stringify({
        id: "demo",
      }),
      "utf8",
  ...
  it("validates current snapshot against facade boundary config and ignores on mismatch", () => {
    const dir = createTempDirSync("natesclaw-facade-snapshot-validate-");
    fs.mkdirSync(path.join(dir, "demo"), { recursive: true });
    fs.writeFileSync(
      path.join(dir, "demo", "runtime-api.js"),
    ...
    // Do NOT write natesclaw.plugin.json on disk to force fallback to registry scan
    useBundledPluginDirOverrideForTest(dir);
      ...
      plugins: [
        {
          ...
          manifestPath: path.join(dir, "demo", "natesclaw.plugin.json"),
          channels: ["demo"],
          providers: [],
          cliBackends: [],
          skills: [],

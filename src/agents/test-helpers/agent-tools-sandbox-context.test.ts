...
describe("createAgentToolsSandboxContext", () => {
  it("provides stable defaults for agent-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("natesclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("natesclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("natesclaw-sbx-");

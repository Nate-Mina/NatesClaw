...
const PACKAGE_NAME = "@natesclaw/meta-provider";
...
const TARBALL_NAME = "natesclaw-meta-provider-2026.7.1-beta.3.tgz";
const MANUAL_OVERRIDE_REASON =
  "Natesclaw Release Publish run 12345 approved token release for v2026.7.1-beta.3";
...
const PUBLISHER_POLICY = {
  ...
  schema: "natesclaw.plugin-npm-publisher-policy/v1",
...
function tempDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "natesclaw-plugin-publication-artifact-"));
  tempDirs.push(dir);
  return dir;
}
...
function createFixture(
  ...
  const tarball = createTarball(
    options.tarEntries ?? [
      ...
      { content: '{"id":"meta"}\n', path: "package/natesclaw.plugin.json" },
...
describe("plugin publication artifact", () => {
    ...
    expect(() =>
      verifyFixture(fixture, {
        manualOverrideReason:
          "Natesclaw Release Publish run 12345 approved token release for v2026.7.1-beta.4",
      }),
  ...
  it("rejects every mutable artifact identity dimension", () => {
    ...
    const mismatches: Array<[string, Record<string, unknown>]> = [
      ...
      ["repository", { repository: "natesclaw/not-natesclaw" }],
      ...
      ["package name", { packageName: "@natesclaw/not-meta" }],
  ...
  it("binds authoritative workflow status, conclusion, and head repository", () => {
    const mutations = [
      ...
      (run: Record<string, unknown>) => {
        run.head_repository = { full_name: "natesclaw/not-natesclaw" };
      },
    ];
  ...
  it("requires the exact Meta package name, dir, and both publication flags", () => {
    ...
    const cases: Array<{
      ...
      {
        manifestOverrides: {
          natesclaw: { release: { publishToClawHub: false, publishToNpm: true } },
        },
      },
      {
        manifestOverrides: {
          natesclaw: { release: { publishToClawHub: true, publishToNpm: false } },
        },
      },
      {
        manifestOverrides: { name: "@natesclaw/not-meta" },
        params: { packageName: "@natesclaw/not-meta" },
      },
      {

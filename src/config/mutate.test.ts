...
import type { ConfigFileSnapshot, NatesclawConfig } from "./types.js";

type MockValidationIssue = { path: string; message: string };
type MockValidationResult =
  | { ok: true; config: NatesclawConfig; warnings: MockValidationIssue[] }
...
const validationMocks = vi.hoisted(() => ({
  validateConfigObjectWithPlugins: vi.fn(
    (config: NatesclawConfig): MockValidationResult => ({
      ok: true,
      config,
      warnings: [],
    }),
...
function createSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/natesclaw.json",
    exists: true,
...
async function createPluginIncludeFixture(home: string) {
  ...
  const pluginsPath = path.join(home, ".natesclaw", "config", "plugins.json5");
...
describe("config mutate helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "natesclaw-config-mutate-" });
  const originalNixMode = process.env.NATESCLAW_NIX_MODE;
  ...
  afterAll(async () => {
    if (originalNixMode === undefined) {
      ...
      process.env.NATESCLAW_NIX_MODE = originalNixMode;
    }
  ...
  it("preserves config path ownership across transform retries", async () => {
    const initial = createSnapshot({
      hash: "hash-1",
      path: "/tmp/first-natesclaw.json",
      sourceConfig: { agents: { list: [] } },
    });
    const fresh = createSnapshot({
      hash: "hash-2",
      path: "/tmp/second-natesclaw.json",
      sourceConfig: { agents: { list: [] } },
    });
    ioMocks.readConfigFileSnapshotForWrite
      .mockResolvedValueOnce({
    ...
    const transform = vi.fn((config: NatesclawConfig) => ({ nextConfig: config }));
  ...
  it("does not retry transform mutations after config path ownership changes", async () => {
    ...
    await expect(
      transformConfigFileWithRetry({
        transform(config) {
          activeConfigPath = "/tmp/second-natesclaw.json";
  ...
  it("rejects replace attempts when the active config path changed", async () => {
    ...
    await expect(
      replaceConfigFile({
        baseHash: snapshot.hash,
        nextConfig: { gateway: { port: 19002 } },
        writeOptions: { expectedConfigPath: "/tmp/first-natesclaw.json" },
      }),
    ).rejects.toThrow("config path changed since last load");
    expect(ioMocks.writeConfigFile).not.toHaveBeenCalled();
  });
  ...
  it("repairs invalid config through a single-file top-level plugins include", async () => {
      ...
      `${JSON.stringify(
        {
          entries: {
            old: {
              enabled: true,
              config: { token: "${NATESCLAW_TEST_PLUGIN_TOKEN}" },
            },
          },
        },
        null,
    ...
    ioMocks.readConfigFileSnapshotForWrite
      .mockResolvedValueOnce({
        ...
        writeOptions: {
          ...
          envSnapshotForRestore: { NATESCLAW_TEST_PLUGIN_TOKEN: "plugin-token-runtime" },
          assertConfigPathForWrite: allowConfigPathWrite,
          includeFileTargetsForWrite: { [pluginsPath]: await resolveIncludeTarget(pluginsPath) },
        },
      })
    ...
    try {
      await replaceConfigFile({
        ...
        io: {
          env: { NATESCLAW_TEST_PLUGIN_TOKEN: "plugin-token-after-read" },
    ...
    expect(persistedPlugins.entries?.old?.config?.token).toBe("${NATESCLAW_TEST_PLUGIN_TOKEN}");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const configPath = path.join(home, ".natesclaw", "natesclaw.json");
      const linkPath = path.join(home, ".natesclaw", "link");
  ...
  it("rejects direct mutations to external include roots", async () => {
    ...
    await expect(
      replaceConfigFile({
        ...
        nextConfig,
        io: {
          env: { NATESCLAW_INCLUDE_ROOTS: "~/shared" },
          readConfigFileSnapshotForWrite: ioMocks.readConfigFileSnapshotForWrite,
          writeConfigFile: ioMocks.writeConfigFile,
        },
      }),
  ...
  it("preserves auth-store refresh scope for managed top-level include writes", async () => {
    ...
    const preflight = vi.fn(
      async (sourceConfig: NatesclawConfig, refreshOptions?: { includeAuthStoreRefs?: boolean }) => {
  ...
  it("uses the published restart env source for isolated managed include writes", async () => {
    ...
    const configPath = path.join(home, ".natesclaw", "natesclaw.json");
    const envPath = path.join(home, ".natesclaw", "config", "env.json5");
    const envKey = "OC";
    ...
    let preflightSource: NatesclawConfig | undefined;
  ...
  it("does not write an include after the active config path changes during preflight", async () => {
    ...
    const firstConfigPath = path.join(home, "first", "natesclaw.json");
    const secondConfigPath = path.join(home, "second", "natesclaw.json");
  ...
  it("rolls back an include write when config path ownership changes during commit", async () => {
    ...
    const assertConfigPathForWrite = () => {
      if (fsNode.readFileSync(pluginsPath, "utf-8") !== initialPluginsRaw) {
        activeConfigPath = "/tmp/other-natesclaw.json";
      }
  ...
  it.each(["active path", "refreshed snapshot path"] as const)(
    ...
    async (changeKind) => {
      ...
      const configPath = path.join(home, "first", "natesclaw.json");
      const otherConfigPath = path.join(home, "second", "natesclaw.json");
      const pluginsPath = path.join(home, "first", "plugins.json5");
      const rootConfig = { plugins: { $include: "./plugins.json5" } };
      const initialPluginsRaw = `${JSON.stringify({ entries: {} }, null, 2)}\n`;
      const nextConfig = { plugins: { entries: { demo: { enabled: true } } } };
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const configPath = path.join(home, ".natesclaw", "natesclaw.json");
      const includeDir = path.join(home, ".natesclaw", "config");
      const movedIncludeDir = path.join(home, ".natesclaw", "config-original");
  ...
  it("preflights the restored include payload with the current environment", async () => {
    ...
    const initialPluginsRaw = `${JSON.stringify(
      {
        entries: {
          old: { enabled: true, config: { token: "${NATESCLAW_TEST_INCLUDE_TOKEN}" } },
        },
      },
      null,
      2,
    ...
    const observedSources: NatesclawConfig[] = [];
    ...
    try {
      ...
      await expect(
        replaceConfigFile({
          ...
          writeOptions: {
            expectedConfigPath: snapshot.path,
            envSnapshotForRestore: { NATESCLAW_TEST_INCLUDE_TOKEN: "old-token" },
            assertConfigPathForWrite: allowConfigPathWrite,
            includeFileTargetsForWrite: { [pluginsPath]: await resolveIncludeTarget(pluginsPath) },
          },
          nextConfig: {
          ...
          io: {
            env: { NATESCLAW_TEST_INCLUDE_TOKEN: "new-token" },
            readConfigFileSnapshotForWrite: ioMocks.readConfigFileSnapshotForWrite,
            writeConfigFile: ioMocks.writeConfigFile,
          },
        }),
  ...
  it("rolls back single-file top-level include writes when runtime refresh fails", async () => {
    ...
    const envKey = "NATESCLAW_TEST_INCLUDE_ROLLBACK_ENV";
    await fs.mkdir(path.dirname(pluginsPath), { recursive: true });
    await fs.writeFile(
      configPath,
      `${JSON.stringify({ plugins: { $include: "./config/plugins.json5" } }, null, 2)}\n`,
  ...
  it("preflights injected root writers before persisting", async () => {
    ...
    const configPath = path.join(home, ".natesclaw", "natesclaw.json");
    await fs.mkdir(path.dirname(configPath), { recursive: true });
    const initialConfig = { gateway: { mode: "local" } } satisfies NatesclawConfig;
    ...
    const injectedWrite = vi.fn(async (config: NatesclawConfig, options?: ConfigWriteOptions) => {

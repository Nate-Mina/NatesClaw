...
final class LiveActivityManager {
    ...
    private struct PendingActivityUpdate {
        var state: NatesclawActivityAttributes.ContentState
        var staleDate: Date?
    }

    private struct StatusPresentation {
        let status: NatesclawActivityAttributes.ContentState.Status
        let verbatimDetail: String?
    }

    private let logger = Logger(subsystem: "ai.natesclawfoundation.app", category: "LiveActivity")
    ...
    private var currentActivity: Activity<NatesclawActivityAttributes>?
    private var currentState: NatesclawActivityAttributes.ContentState?
        ...
        sessionKey: String)
    {
        ...
        let state = NatesclawActivityAttributes.ContentState(
    ...
    private func request(
        state: NatesclawActivityAttributes.ContentState,
    ...
    private func startActivity(_ request: LiveActivityPresentationRequest) {
        ...
        do {
            let activity = try Activity.request(
                attributes: NatesclawActivityAttributes(
    ...
    private func enqueueLatestUpdate(
        activity: Activity<NatesclawActivityAttributes>,
        state: NatesclawActivityAttributes.ContentState,
        staleDate: Date?)
    {
    ...
    private func startUpdateWorker(activity: Activity<NatesclawActivityAttributes>, generation: UInt64) {
    ...
    private func endCurrentActivity(reason: String) {
        ...
        let finalState = NatesclawActivityAttributes.ContentState(
            status: .disconnected,
            verbatimDetail: nil,
            startedAt: startedAt)
        Task {
    ...
    private func hydrateCurrentAndPruneDuplicates() {
        let active = Activity<NatesclawActivityAttributes>.activities
    ...
    nonisolated static func shouldHydrate(
        status: NatesclawActivityAttributes.ContentState.Status,
        startedAt: Date,
        staleDate: Date?,
        now: Date,
    ...
    nonisolated static func shouldResetVoiceSamples(
        previousStatus: NatesclawActivityAttributes.ContentState.Status?) -> Bool
    {
        previousStatus == nil
    }
    ...
    private func end(activity: Activity<NatesclawActivityAttributes>) {
    ...
    private static func voiceDetail(
        _ value: String,
        status: NatesclawActivityAttributes.ContentState.Status) -> String?
    {

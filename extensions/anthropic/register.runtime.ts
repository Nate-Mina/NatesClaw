 ...
 */
import { formatCliCommand, parseDurationMs } from "natesclaw/plugin-sdk/cli-runtime";
...
import {
  ...
  type NatesclawConfig as ProviderAuthConfig,
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("natesclaw models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("natesclaw models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("natesclaw doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: NatesclawPluginApi): void {

...
[Honcho](https://honcho.dev) adds AI-native memory to Natesclaw through an
...
```bash
natesclaw plugins install @honcho-ai/natesclaw-honcho
natesclaw honcho setup
...
Settings live under `plugins.entries["natesclaw-honcho"].config`:
    ...
    entries: {
      "natesclaw-honcho": {
        config: {
          ...
          workspaceId: "natesclaw", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `natesclaw honcho setup` detects and
...
During conversation, Honcho tools query the service during Natesclaw's
...
```bash
natesclaw honcho setup                        # Configure API key and migrate files
natesclaw honcho status                       # Check connection status
natesclaw honcho ask <question>               # Query Honcho about the user
natesclaw honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/natesclaw-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Natesclaw integration guide](https://docs.honcho.dev/v3/guides/integrations/natesclaw)

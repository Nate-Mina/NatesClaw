...
import { normalizeOptionalString, uniqueStrings } from "natesclaw/plugin-sdk/string-coerce-runtime";
...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'natesclaw matrix verify status --verbose --account ${accountId}' and 'natesclaw matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Natesclaw runtime is using the same account. " +
    `Check 'natesclaw matrix verify status --verbose --account ${accountId}' and 'natesclaw matrix devices list --account ${accountId}'.`
  );
}

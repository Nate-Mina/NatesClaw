...
describe("doctor session snapshot stale runtime metadata", () => {
  ...
  beforeEach(async () => {
    note.mockClear();
    root = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-doctor-session-snapshots-"));
  ...
  it("uses the OS home for cached OCM paths when NATESCLAW_HOME differs", () => {
    const homeDir = path.join(root, "home");
    const currentBundledSkillsDir = path.join(homeDir, ".ocm/current/node_modules/natesclaw/skills");
    const expectedPath = path.join(currentBundledSkillsDir, "doctor", "SKILL.md");
    const currentPath = "~/.ocm/current/node_modules/natesclaw/skills/doctor/SKILL.md";
    const stalePath = "~/.ocm/old/node_modules/natesclaw/skills/doctor/SKILL.md";

    const findings = scanSessionStoreForStaleRuntimeSnapshotPaths({
      bundledSkillsDir: currentBundledSkillsDir,
      env: { HOME: homeDir, NATESCLAW_HOME: path.join(root, "ocm-profile") },
      store: {
  ...
  it("repairs retired imsg paths even when cached under the current package skills root", async () => {
    const packageSkillsDir = path.join(root, "node_modules", "natesclaw", "skills");
  ...
  it("resolves the retired package skills root for moved-skill snapshot repairs", async () => {
    ...
    expect(
      resolveSessionSnapshotBundledSkillsDir({
        ...
        argv1: path.join(packageRoot, "bin", "natesclaw"),
        cwd: distDir,
      }),
  ...
  it("ignores current bundled locations and unrelated workspace skill locations", () => {
    ...
    const findings = scanSessionStoreForStaleRuntimeSnapshotPaths({
      ...
      store: {
        ...
        "agent:natesclaw-workspace": sessionEntry({
          skillsSnapshot: {
            prompt: skillPrompt(NatesclawWorkspacePath),
            skills: [{ name: "doctor" }],
          },
        }),
      },
  ...
  it("reports stale cached metadata from configured session stores", async () => {
    ...
    await noteSessionSnapshotHealth({
      cfg: { session: { store: configuredStorePath } } as NatesclawConfig,
      bundledSkillsDir,
      env: { NATESCLAW_STATE_DIR: stateDir },
    });
...
describe("doctor session snapshot repair (shouldRepair)", () => {
  ...
  beforeEach(async () => {
    note.mockClear();
    root = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-doctor-repair-"));
    bundledSkillsDir = path.join(root, "current", "skills");
    await fs.mkdir(path.join(bundledSkillsDir, "doctor"), { recursive: true });
    await fs.writeFile(path.join(bundledSkillsDir, "doctor", "SKILL.md"), "# Doctor\n");
  });

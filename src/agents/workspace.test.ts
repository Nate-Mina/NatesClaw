...
beforeEach(async () => {
  ...
  testState = await createNatesclawTestState({
    layout: "state-only",
    prefix: "natesclaw-workspace-state-",
  });
});
...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses NATESCLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      NATESCLAW_HOME: "/srv/natesclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/natesclaw-home"), ".natesclaw", "workspace"));
  });

  it("prefers NATESCLAW_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      NATESCLAW_WORKSPACE_DIR: "/srv/natesclaw-workspace",
      NATESCLAW_HOME: "/srv/natesclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.resolve("/srv/natesclaw-workspace"));
  });
});
...
describe("ensureAgentWorkspace", () => {
  ...
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("natesclaw-workspace-");
  ...
  it("does not overwrite a foreign root workspace-state.json file", async () => {
    ...
    const foreignState = "not natesclaw state\n";
  ...
  it("requires Doctor before using legacy JSON setup state", async () => {
    ...
    ).rejects.toThrow(/run natesclaw doctor --fix/u);
    await expect(
  ...
  it("refuses to re-seed a recently attested workspace after only generated git metadata survives", async () => {
    ...
    await fs.mkdir(path.join(tempDir, ".natesclaw"), { recursive: true });
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-workspace-hardlink-"));
    try {

---
summary: "Set up Codex Computer Use for Codex-mode Natesclaw agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode Natesclaw agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. Natesclaw
...
Use this page when Natesclaw is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

This is distinct from Natesclaw's built-in [node-backed computer tool](/nodes/computer-use). Use the built-in tool when the same agent contract should control a paired Mac whether the agent runs on the Gateway or another node. Use Codex Computer Use when Codex app-server should own local MCP installation, permissions, and native tool calls.

## Natesclaw.app and Peekaboo

Natesclaw.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want Natesclaw.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode Natesclaw agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an Natesclaw node and exposes mobile
...
Natesclaw-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through Natesclaw's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the Natesclaw command:

```bash
cua-driver mcp-config --client natesclaw
```
...
available as a normal Natesclaw MCP server. Use the Codex Computer Use setup on
...
macOS. Natesclaw does not install `cua-driver`, grant those permissions, or
...
Computer Use in and lets Natesclaw install or re-enable it before the turn:
...
With this config, Natesclaw checks Codex app-server before each Codex-mode
turn. If Computer Use is missing but Codex app-server has already discovered
an installable marketplace, Natesclaw asks Codex app-server to install or
...
marketplace is registered and a standard desktop app bundle exists, Natesclaw
...
installed, Natesclaw falls back to the managed Codex binary installed beside the
...
`appServer.command` config or `NATESCLAW_CODEX_APP_SERVER_BIN` still overrides
this managed selection.

Natesclaw serializes native Codex config reads and Computer Use installation
...
`codex` plugin command surface is available. These are Natesclaw chat/runtime
commands, not `natesclaw codex ...` CLI subcommands:
...
Natesclaw uses the same app-server API that Codex itself exposes. The
...
marketplaces. During install, Natesclaw polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds (default 60 seconds).

If multiple known marketplaces contain Computer Use, Natesclaw prefers
...
`computer-use` is registered, Natesclaw tries to add the first standard
...
reconciliation so Natesclaw does not override that selection.

## Remote marketplaces

Remote marketplace support was introduced in Codex 0.146.1 and remains
available in Natesclaw's pinned Codex 0.147.0. Natesclaw passes the opaque remote
...
| Field                           | Env var                                                        |
| ------------------------------- | -------------------------------------------------------------- |
| `enabled`                       | `NATESCLAW_CODEX_COMPUTER_USE`                                  |
| `autoInstall`                   | `NATESCLAW_CODEX_COMPUTER_USE_AUTO_INSTALL`                     |
| `marketplaceDiscoveryTimeoutMs` | `NATESCLAW_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS` |
| `liveTestTimeoutMs`             | `NATESCLAW_CODEX_COMPUTER_USE_LIVE_TEST_TIMEOUT_MS`             |
| `toolCallTimeoutMs`             | `NATESCLAW_CODEX_COMPUTER_USE_TOOL_CALL_TIMEOUT_MS`             |
| `healthCheckEnabled`            | `NATESCLAW_CODEX_COMPUTER_USE_HEALTH_CHECK_ENABLED`             |
| `healthCheckIntervalMinutes`    | `NATESCLAW_CODEX_COMPUTER_USE_HEALTH_CHECK_INTERVAL_MINUTES`    |
| `pluginCacheMode`               | `NATESCLAW_CODEX_COMPUTER_USE_PLUGIN_CACHE_MODE`                |
| `strictReadiness`               | `NATESCLAW_CODEX_COMPUTER_USE_STRICT_READINESS`                 |
| `autoRepair`                    | `NATESCLAW_CODEX_COMPUTER_USE_AUTO_REPAIR`                      |
| `marketplaceSource`             | `NATESCLAW_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE`               |
| `marketplacePath`               | `NATESCLAW_CODEX_COMPUTER_USE_MARKETPLACE_PATH`                 |
| `marketplaceName`               | `NATESCLAW_CODEX_COMPUTER_USE_MARKETPLACE_NAME`                 |
| `pluginName`                    | `NATESCLAW_CODEX_COMPUTER_USE_PLUGIN_NAME`                      |
| `mcpServerName`                 | `NATESCLAW_CODEX_COMPUTER_USE_MCP_SERVER_NAME`                  |

## What Natesclaw checks

Natesclaw reports a stable setup reason internally and formats the
...
If Natesclaw says Computer Use is installed but the MCP server is unavailable,
...
Natesclaw intentionally fails closed when `computerUse.enabled` is true. A
...
retry in a fresh Natesclaw session. If the host previously ran Computer Use
...
Codex-native tool hook could not reach an active Natesclaw relay through the
local bridge or Gateway fallback. Start a fresh Natesclaw session with `/new`
...
Natesclaw Gateway so old threads and hook registrations are dropped, then

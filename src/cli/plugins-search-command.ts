...
/** Options accepted by `natesclaw plugins search`. */
...
function formatPackageSearchLine(entry: ClawHubPackageSearchResult): string {
  ...
  return `${pkg.name}  ${theme.muted(flags.join(" | "))}${summary}\n  ${theme.muted(`Install: natesclaw plugins install clawhub:${pkg.name}`)}`;
}
...
export async function runPluginsSearchCommand(
  ...
  if (!query) {
    runtime.error("Usage: natesclaw plugins search <query>");

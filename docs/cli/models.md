---
summary: "CLI reference for `natesclaw models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
| `--agent <id>`            | Configured agent id; overrides `NATESCLAW_AGENT_DIR`.                                                                                     |
...
Direct `models status --probe` runs create temporary internal sessions in the selected agent's canonical database, so the command requires exclusive ownership of the configured state directory. Stop a running Gateway with `natesclaw gateway stop` before probing; the command removes its internal sessions and releases the state lock when it finishes or is interrupted.
...
- `no_model`: provider auth exists, but Natesclaw could not resolve a probeable model candidate for that provider.

For OpenAI ChatGPT/Codex OAuth troubleshooting, `natesclaw models status`, `natesclaw models auth list --provider openai`, and `natesclaw config get agents.defaults.model --json` are the quickest way to confirm whether an agent has a usable `openai` OAuth profile for `openai/*` through the native Codex runtime. See [OpenAI provider setup](/providers/openai#check-and-recover-codex-oauth-routing).

### List

`natesclaw models list` is read-only: it reads config, auth profiles, existing catalog state, and provider-owned catalog rows, but never rewrites `models.json`.

`natesclaw models refresh [--json]` forces an immediate hosted catalog check.
...
- If you omit the provider, Natesclaw resolves the input as an alias first, then as a unique configured-provider match for that exact model id, and only then falls back to the configured default provider with a deprecation warning. If that provider no longer exposes the configured default model, Natesclaw falls back to the first configured provider/model instead of surfacing a stale removed-provider default.
...
By default Natesclaw tries to probe tool and image support with live model calls. If no OpenRouter key is configured, the command falls back to metadata-only output and explains that `:free` models still require `OPENROUTER_API_KEY` for probes and inference.
...
```bash
natesclaw models aliases list [--json] [--plain]
natesclaw models aliases add <alias> <model-or-alias>
natesclaw models aliases remove <alias>
```
...
```bash
natesclaw models fallbacks list [--json] [--plain]
natesclaw models fallbacks add <model-or-alias>
natesclaw models fallbacks remove <model-or-alias>
natesclaw models fallbacks clear
```

Manages `agents.defaults.model.fallbacks`. `natesclaw models image-fallbacks list|add|remove|clear` manages the parallel `agents.defaults.imageModel.fallbacks` list with the same subcommand shape.

## Auth profiles

```bash
natesclaw models auth add
natesclaw models auth list [--provider <id>] [--json]
natesclaw models auth login --provider <id>
natesclaw models auth login --provider openai --profile-id openai:work
natesclaw models auth login-github-copilot
natesclaw models auth logout <profileId> [--yes]
natesclaw models auth paste-api-key --provider <id>
natesclaw models auth setup-token --provider <id>
natesclaw models auth paste-token --provider <id>
natesclaw models auth order get --provider <id>
natesclaw models auth order set --provider <id> <profileIds...>
natesclaw models auth order clear --provider <id>
```
...
`models auth login` runs a provider plugin's auth flow (OAuth/API key). Use `natesclaw plugins list` to see which providers are installed. `login` accepts `--profile-id <id>` for providers that support named profiles during login (use this to keep multiple logins for the same provider separate), `--method <id>` to pick a specific auth method, `--device-code` as a shortcut for `--method device-code`, `--set-default` to apply the provider's recommended default model, and `--force` to remove existing profiles for that provider first (use when a cached OAuth profile is stuck or you want to switch accounts).
...
Use `natesclaw models auth --agent <id> <subcommand>` to write auth results to a specific configured agent store. The parent `--agent` flag is honored by `add`, `list`, `login`, `logout`, `paste-api-key`, `setup-token`, `paste-token`, `login-github-copilot`, and `order get`/`set`/`clear`.

For OpenAI models, `--provider openai` defaults to ChatGPT/Codex account login. Use `--method api-key` only when you want to add an OpenAI API-key profile, usually as a backup for Codex subscription limits. Run `natesclaw doctor --fix` to migrate older legacy OpenAI Codex prefix auth/profile state to `openai`.

Examples:

```bash
natesclaw models auth login --provider openai --set-default
natesclaw models auth login --provider openai --method api-key
natesclaw models auth paste-api-key --provider openai
natesclaw models auth list --provider openai
natesclaw models auth logout openai:manual --yes
```

Notes:

- `paste-api-key` accepts API keys generated elsewhere, prompts for the key value, and writes it to the default profile id `<provider>:manual` unless you pass `--profile-id`. In automation, pipe the key on stdin, for example `printf "%s\n" "$OPENAI_API_KEY" | natesclaw models auth paste-api-key --provider openai`.
...
- Anthropic: `setup-token`/`paste-token` are supported Natesclaw auth paths for `anthropic`, but Natesclaw prefers reusing the Claude CLI (`claude -p`) on the host when it is available.

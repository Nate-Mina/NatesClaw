...
import { asOptionalRecord as asRecord } from "natesclaw/plugin-sdk/string-coerce-runtime";
...
const NATESCLAW_SLACK_CLIENT_PING_TIMEOUT_MS = 15_000;
const NATESCLAW_SLACK_SOCKET_START_FAILED_EVENT = "unable_to_socket_mode_start";
const NATESCLAW_SLACK_NATIVE_RECONNECT_OBSERVER_KEY = "__natesclawNativeReconnectFailureObserver";
...
function installSlackNativeReconnectFailureObserver(receiver: unknown) {
  ...
  if (Reflect.get(client, NATESCLAW_SLACK_NATIVE_RECONNECT_OBSERVER_KEY)) {
    return;
  }
  ...
  Reflect.set(client, NATESCLAW_SLACK_NATIVE_RECONNECT_OBSERVER_KEY, true);
  Reflect.set(
    client,
    ...
    function patchedDelayReconnectAttempt(this: object, callback: unknown) {
      ...
      const delayMs =
        (Number.isFinite(pingTimeoutMs) && pingTimeoutMs >= 0
          ? pingTimeoutMs
          : NATESCLAW_SLACK_CLIENT_PING_TIMEOUT_MS) * nextFailureCount;
        ...
        setTimeout(() => {
          ...
          Promise.resolve(callback.call(this)).then(resolve, (error: unknown) => {
            if (callback === Reflect.get(this, "start")) {
              emit.call(this, NATESCLAW_SLACK_SOCKET_START_FAILED_EVENT, error);
              resolve(undefined);
              return;
            }
...
function shouldSkipNatesclawSlackSelfEvent(args: SlackSelfFilterArgs): boolean {
...
export function createSlackBoltApp(params: {
  ...
  const socketModeReceiverOptions: SlackSocketModeReceiverOptions = {
    ...
    clientPingTimeout: NATESCLAW_SLACK_CLIENT_PING_TIMEOUT_MS,
  ...
  const app = new params.interop.App({
    ...
    // consumes that promise, tripping Natesclaw's fatal unhandled-rejection path.
    tokenVerificationEnabled: false,
    ...(appReceiver ? { receiver: appReceiver } : {}),
  });
  app.use(async (args) => {
    await params.onContextIdentity?.(args.context ?? {});
    if (shouldSkipNatesclawSlackSelfEvent(args)) {
      return;
    }
    await args.next();
  });

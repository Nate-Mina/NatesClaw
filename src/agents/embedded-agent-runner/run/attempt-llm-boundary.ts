 ...
 * are NOT collapsed.
 *
 * @see https://github.com/natesclaw/natesclaw/issues/3658
 */
...
function stripUnsafeBlockedRunMetadata(messages: AgentMessage[]): AgentMessage[] {
  ...
  const nextMessages = messages.map((message) => {
    ...
    if (!natesclaw || typeof natesclaw !== "object") {
      return message;
    }
    const beforeAgentRunBlocked = (natesclaw as { beforeAgentRunBlocked?: unknown })
    ...
    const nextNatesclaw = {
      ...(natesclaw as Record<string, unknown>),
      beforeAgentRunBlocked: safeBlocked,
    };
    changed = true;
    return {
      ...(message as unknown as Record<string, unknown>),
      __natesclaw: nextNatesclaw,

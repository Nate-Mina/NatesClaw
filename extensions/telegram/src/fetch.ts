...
import { resolveRequestUrl } from "natesclaw/plugin-sdk/request-url";
...
// fix for the leak observed in natesclaw#68128 is the transport lifecycle that
...
function resolveNatesclawProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.NATESCLAW_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveNatesclawProxyUrlForTelegram() : undefined;

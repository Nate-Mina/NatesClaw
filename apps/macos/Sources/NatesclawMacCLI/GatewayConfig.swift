...
/// An explicit config path wins; otherwise the selected state directory owns natesclaw.json.
func resolveNatesclawConfigURL() -> URL {
    if let configPath = NatesclawEnvironmentPath("NATESCLAW_CONFIG_PATH") {
        return URL(fileURLWithPath: NSString(string: configPath).expandingTildeInPath)
    }
    let stateDir = NatesclawEnvironmentPath("NATESCLAW_STATE_DIR").map {
...
private func NatesclawEnvironmentPath(_ key: String) -> String? {
...
func loadGatewayConfig() -> GatewayConfig {
    guard let data = try? Data(contentsOf: resolveNatesclawConfigURL()) else { return GatewayConfig() }

...
1. Download the runtime for your OS (extracts into `$NATESCLAW_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.natesclaw/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$NATESCLAW_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.natesclaw/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Natesclaw config file (`$NATESCLAW_CONFIG_PATH`, default `~/.natesclaw/natesclaw.json`):

...
const dependencyChangeMarker = "<!-- natesclaw:dependency-guard -->";
const dependencyGraphGuardMarker = "<!-- natesclaw:dependency-graph-guard -->";
...
function dependencyGraphGuardStateMarker(state, headSha) {
  return `<!-- natesclaw:dependency-graph-guard state=${state} sha=${headSha ?? "<head-sha>"} -->`;
}
...
export function renderAuthorizedDependencyComment(override) {
  const lines = [
    ...
    "",
    "This PR includes dependency graph changes. A repository admin or member of `@natesclaw/natesclaw-secops` authorized this exact head SHA with `/allow-dependencies-change`.",
    "",
...
export function renderTrustedDependencyComment({ actor, headSha }) {
  return [
    ...
    "",
    "This PR includes dependency graph changes. The dependency guard is informational because the PR author is a repository admin or a member of `@natesclaw/natesclaw-secops`.",
    "",
...
Natesclaw does not accept package lockfile changes through PRs. This PR did not change dependency graph fields in package manifests, so the workflow restored the lockfile residue from the target branch automatically.
...
export function renderBlockedDependencyComment({
  ...
  return [
    ...
    "",
    "Natesclaw does not accept dependency graph changes through PRs unless a repository admin or security explicitly authorizes the current head SHA. Dependency updates are generated internally by maintainers so external PRs cannot change the resolved graph.",
    "",
    ...
    ...removalSteps,
    "",
    "If this PR intentionally needs a dependency graph change, ask a repository admin or member of `@natesclaw/natesclaw-secops` to comment:",
    "",
    "```text",
...
export function githubApi(token, options = {}) {
  const api = createGitHubApi(token, { ...options, userAgent: "natesclaw-dependency-guard" });
  return {
    ...api,
...
async function main() {
  ...
  const autoscrubToken = process.env.NATESCLAW_DEPENDENCY_GUARD_AUTOSCRUB_TOKEN;
  ...
  const trustedCommentAuthors = dependencyGuardCommentAuthors(
    process.env.NATESCLAW_DEPENDENCY_GUARD_COMMENT_BOTS,
  );
  ...
  const mode = process.env.NATESCLAW_DEPENDENCY_GUARD_MODE ?? "enforce";

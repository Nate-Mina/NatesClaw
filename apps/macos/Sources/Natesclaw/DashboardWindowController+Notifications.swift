...
extension DashboardWindowController {
    static let notificationsMessageHandlerName = "natesclawNotifications"
    ...
    private func publishNotificationsStatus() async {
        ...
        _ = try? await self.webView.evaluateJavaScript(
            """
            window.__NATESCLAW_NATIVE_NOTIFICATIONS__ = {permission:"\(permission)"};
            window.dispatchEvent(new CustomEvent('natesclaw:native-notifications-status', \
            {detail:window.__NATESCLAW_NATIVE_NOTIFICATIONS__}));
            """)
    }
}

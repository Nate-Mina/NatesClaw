...
public enum ExecApprovalsSQLiteStore {
    ...
    public static let locator = "state/natesclaw.sqlite#exec_approvals_config"
    private static let busyTimeoutMilliseconds: Int32 = 30000

    public static func databaseURL(stateDirectoryURL: URL) -> URL {
        stateDirectoryURL
            .appendingPathComponent("state", isDirectory: true)
            .appendingPathComponent("natesclaw.sqlite", isDirectory: false)
    }
    ...
    static func decode(_ rawJSON: String) throws -> ExecApprovalsDocument {
        guard let data = rawJSON.data(using: .utf8), self.hasValidPersistedStructure(data) else {
            throw NatesclawNativeStateError("Malformed exec approvals raw_json")
        }
        let document = try JSONDecoder().decode(ExecApprovalsDocument.self, from: data)
        guard document.version == 1 else {
            throw NatesclawNativeStateError(
    ...
    public static func serialize(_ document: ExecApprovalsDocument) throws -> String {
        guard document.version == 1 else {
            throw NatesclawNativeStateError("Exec approvals document version must be 1")
        }
        let encoder = JSONEncoder()
        encoder.outputFormatting = [.prettyPrinted, .sortedKeys]
        let data = try encoder.encode(document)
        guard let rawJSON = String(data: data, encoding: .utf8) else {
            throw NatesclawNativeStateError("Could not encode exec approvals as UTF-8")
        }
    ...
    private static func openDatabase(stateDirectoryURL: URL) throws -> NatesclawNativeStateSQLite {
        try NatesclawNativeStateSQLite(
    ...
    private static func readRecord(
        _ database: NatesclawNativeStateSQLite) throws -> ExecApprovalsSQLiteRecord?
    {
        ...
        guard try statement.step() == .done else {
            throw NatesclawNativeStateError("Exec approvals singleton query returned multiple rows")
        }
        return try ExecApprovalsSQLiteRecord(rawJSON: rawJSON, document: self.decode(rawJSON))
    }

    private static func writeRecord(
        _ database: NatesclawNativeStateSQLite,
        ...
        updatedAtMilliseconds: Int64) throws
    {
        ...
        guard try statement.step() == .done else {
            throw NatesclawNativeStateError("Exec approvals upsert did not complete")
        }
    }
    ...
    private static func bind(
        _ value: String?,
        to statement: NatesclawNativeStateSQLiteStatement,

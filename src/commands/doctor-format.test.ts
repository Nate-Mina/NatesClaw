...
describe("buildGatewayRuntimeHints", () => {
  ...
  it("surfaces suspicious systemd cgroup hygiene with inspection commands", () => {
    expect(
      ...
      "Run: systemctl --user show natesclaw-gateway.service -p KillMode -p TasksCurrent -p MemoryCurrent -p MainPID",
      "Run: systemd-cgls --user-unit natesclaw-gateway.service",
      "After reviewing service settings, run: natesclaw gateway restart",
    ]);
  });
  ...
  it("guides recovery when systemd hit its restart start limit (crash loop)", () => {
    ...
    expect(text).toContain("natesclaw gateway restart");

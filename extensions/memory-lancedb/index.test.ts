...
// Provenance marker Natesclaw appends to every injected inbound-context header.
...
async function withMockedOpenAiMemoryPlugin<T>(params: {
  ...
  vi.resetModules();
  vi.doMock("natesclaw/plugin-sdk/runtime-env", () => ({
...
describe("memory plugin e2e", () => {
  const { getDbPath, getTmpDir } = installTmpDirHarness({ prefix: "natesclaw-memory-test-" });
  ...
  test("uses provider adapter auth and drains cleanup before service replacement", async () => {
    ...
    vi.resetModules();
    vi.doMock("natesclaw/plugin-sdk/memory-core-host-engine-embeddings", async (importOriginal) => ({
      ...(await importOriginal<
        typeof import("natesclaw/plugin-sdk/memory-core-host-engine-embeddings")
      >()),
    ...
    try {
      ...
      expect(providerOptions.agentDir).toBe("/tmp/natesclaw-agent");
      ...
      vi.doUnmock("natesclaw/plugin-sdk/memory-core-host-engine-embeddings");
      vi.doUnmock("openai");
      vi.doUnmock("./lancedb-runtime.js");
      vi.resetModules();
    }
    ...
    await withMockedOpenAiMemoryPlugin({
      ...
      run: async (dynamicMemoryPlugin) => {
        ...
        try {
          ...
          await program.parseAsync(["node", "natesclaw", "ltm", "list", "--limit", "+03"]);
  ...
  test("looksLikeEnvelopeSludge detects marked inbound context headers", () => {
    ...
    // Natesclaw injects carries it, and it never collides with user prose.
  ...
  test("sanitizeForMemoryCapture strips sender label from real room-label envelope shapes", () => {
    ...
    expect(
      sanitizeForMemoryCapture(
        "[Discord Natesclaw #dev channel id:456 +5m] Alice: I prefer dark mode",
      ),
    ).toBe("I prefer dark mode");
    expect(sanitizeForMemoryCapture("[Telegram Natesclaw id:-100] Alice: I prefer dark mode")).toBe(
      "I prefer dark mode",
    );
  ...
  test("sanitizeForMemoryCapture preserves an unknown structured-context label as user content", () => {
    ...
    // user's own text, not an Natesclaw injection, so it survives capture intact.
    const input = [
  ...
  test("escapeMemoryForPrompt preserves inert media text while escaping markup", () => {
    expect(
      escapeMemoryForPrompt(
        "User sent <image> [media attached: /Users/alex/.natesclaw/media/photo.jpg (image/jpeg)] & said hello",
      ),
    ).toBe(
      "User sent &lt;image&gt; [media attached: /Users/alex/.natesclaw/media/photo.jpg (image/jpeg)] &amp; said hello",
    );

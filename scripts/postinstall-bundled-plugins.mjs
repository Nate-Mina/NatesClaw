...
const DISABLE_POSTINSTALL_ENV = "NATESCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL";
...
function resolvePostinstallNatesclawHomeDir(env, getHomedir = homedir) {
  const osHome = resolvePostinstallOsHomeDir(env, getHomedir);
  const override = env?.NATESCLAW_HOME?.trim();
  return override ? pathResolve(resolvePostinstallTildePath(override, osHome)) : osHome;
}

function resolvePostinstallUserPath(input, NatesclawHome) {
  return pathResolve(resolvePostinstallTildePath(input, NatesclawHome));
}
...
function isLegacyInstalledPluginDependencyDirName(name) {
  return name === "node_modules" || /^\.natesclaw-install-stage(?:-[^/]+)?$/iu.test(name);
}
...
export function collectLegacyPluginRuntimeDepsStateRoots(params = {}) {
  ...
  const NatesclawHome = resolvePostinstallNatesclawHomeDir(env, getHomedir);
  const stateRoots = [];
  ...
  if (stateOverride) {
    addStateRoot(resolvePostinstallUserPath(stateOverride, NatesclawHome));
  }
  const configPath = env?.NATESCLAW_CONFIG_PATH?.trim();
  if (configPath) {
    addStateRoot(dirname(resolvePostinstallUserPath(configPath, NatesclawHome)));
  }
  addStateRoot(join(NatesclawHome, ".natesclaw"));
  addStateRoot(join(NatesclawHome, ".clawdbot"));

  for (const entry of splitPostinstallPathList(env?.STATE_DIRECTORY)) {
    addStateRoot(resolvePostinstallUserPath(entry, NatesclawHome));
  }
  ...
  const createTempPath =
    ...
    ((unsafeTargetPath) =>
      join(
        ...
        `.${basename(unsafeTargetPath)}.natesclaw-hotfix-${randomUUID()}`,
      ));
  const writeFile =

...
import { withNatesclawStateStartupMigrationCheckpointDatabase } from "./natesclaw-state-db-startup-checkpoint.js";
import {
...
import { claimNatesclawStateOwnership } from "./natesclaw-state-ownership-operations.js";
import {
  inspectNatesclawStateOwnershipAtPath,
...
function createEnv(external = false): NodeJS.ProcessEnv {
  return {
    NATESCLAW_STATE_DIR: tempDirs.make("natesclaw-state-ownership-"),
    ...(external ? { NATESCLAW_SUPERVISOR_MODE: "external" } : {}),
  };
}

function withoutExternalMarker(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const next = { ...env };
  delete next.NATESCLAW_SUPERVISOR_MODE;
  return next;
}

function claimFixture(managerId = "gateway-supervisor") {
  const externalEnv = createEnv(true);
  const ownership = claimNatesclawStateOwnership(managerId, { env: externalEnv });
...
describe("external shared-state ownership", () => {
  it("returns unowned for a missing path without creating its state tree", () => {
    const rootDir = tempDirs.make("natesclaw-state-ownership-missing-");
    const missingStateDir = path.join(rootDir, "missing-state");
    const databasePath = path.join(missingStateDir, "state", "natesclaw.sqlite");

    expect(fs.existsSync(missingStateDir)).toBe(false);
    expect(inspectNatesclawStateOwnershipAtPath(databasePath)).toBeNull();
    expect(fs.existsSync(missingStateDir)).toBe(false);
  });
  ...
  it("preserves ordinary unowned database behavior", () => {
    ...
    expect(inspectNatesclawStateOwnershipAtPath(database.path)).toBeNull();
  });

  it("reads ownership from a WAL when the SHM index is absent", () => {
    const env = createEnv(true);
    const databasePath = openNatesclawStateDatabase({ env }).path;
    closeNatesclawStateDatabaseForTest();
    const { DatabaseSync } = requireNodeSqlite();
    ...
    try {
      ...
      const copyDir = tempDirs.make("natesclaw-state-ownership-wal-only-");
      const copyPath = path.join(copyDir, "natesclaw.sqlite");
      ...
      expect(inspectNatesclawStateOwnershipAtPath(copyPath)).toEqual(ownership);
    } finally {
      writer.close();
    }
  });
  ...
  it("observes committed ownership that is still resident in the live WAL", () => {
    ...
    try {
      ...
      expect(inspectNatesclawStateOwnershipAtPath(databasePath)).toEqual(ownership);
    } finally {
      writer.close();
    }
  });
  ...
  it("does not expose rolled-back ownership from a rollback-journal race", () => {
    ...
    try {
      const inspected = inspectNatesclawStateOwnershipAtPath(databasePath);
  ...
  it("inspects consolidated ownership without modifying its SQLite family or state tree", () => {
    ...
    const stateDir = fixture.externalEnv.NATESCLAW_STATE_DIR;
    if (!stateDir) {
    ...
    try {
      expect(inspectNatesclawStateOwnershipAtPath(fixture.databasePath)).toEqual(fixture.ownership);
    } finally {
      if (process.platform !== "win32") {
  ...
  it("keeps one state-local coordinator across temporary-directory environments", () => {
    ...
    const databasePath = openNatesclawStateDatabase({ env }).path;
    closeNatesclawStateDatabaseForTest();
    const stateDir = resolveNatesclawStateDirForDatabasePath(databasePath);
    ...
    for (const temporaryDirectory of [
      ...
      withEnv({ TMPDIR: temporaryDirectory }, () =>
        runWithNatesclawStateOwnershipCoordinator(databasePath, "test coordinator path", () => {}),
      );
  ...
  it("closes an unpublished fresh handle when coordinator release fails", () => {
    const env = createEnv();
    let cachedDuringRelease: ReturnType<typeof getNatesclawStateDatabaseIfOpen> = undefined;
    const exec = mockCoordinatorRollbackFailure(() => {
      cachedDuringRelease = getNatesclawStateDatabaseIfOpen({ env });
    });
    ...
    expect(getNatesclawStateDatabaseIfOpen({ env })).toBeUndefined();
  ...
  it("requires the external marker and makes claims idempotent only for one manager", () => {
    const env = createEnv();
    expect(() => claimNatesclawStateOwnership("gateway-supervisor", { env })).toThrow(
      /NATESCLAW_SUPERVISOR_MODE=external/u,
    );
    const externalEnv = { ...env, NATESCLAW_SUPERVISOR_MODE: "external" };
    const first = claimNatesclawStateOwnership("gateway-supervisor", { env: externalEnv });
    expect(claimNatesclawStateOwnership("gateway-supervisor", { env: externalEnv })).toEqual(first);
    expect(
      inspectNatesclawStateOwnershipAtPath(openNatesclawStateDatabase({ env: externalEnv }).path),
    ).toEqual(first);
    expect(() => claimNatesclawStateOwnership("replacement-manager", { env: externalEnv })).toThrow(
  ...
  it("refuses unmarked writable opens before changing the SQLite family", () => {
    const fixture = claimFixture();
    const pending = openNatesclawStateDatabase({ env: fixture.externalEnv });
    pending.db.exec(`
    ...
    expect(() => openNatesclawStateDatabase({ env: fixture.unmarkedEnv })).toThrow(
      NatesclawStateOwnershipError,
    );
    ...
    const repaired = openNatesclawStateDatabase({ env: fixture.externalEnv });
  ...
  it("fences a claim made immediately before cold-open schema repair", () => {
    ...
    try {
      expect(() => openNatesclawStateDatabase({ env })).toThrow(NatesclawStateOwnershipError);
    } finally {
      exec.mockRestore();
    }
  ...
  it("fences injected and pre-claim handles on their next canonical write", () => {
    const externalEnv = createEnv(true);
    const opened = openNatesclawStateDatabase({ env: externalEnv });
    claimNatesclawStateOwnership("gateway-supervisor", { env: externalEnv });
    const unmarkedEnv = withoutExternalMarker(externalEnv);

    expect(() => openNatesclawStateDatabase({ env: unmarkedEnv })).toThrow(
      NatesclawStateOwnershipError,
    );
    expect(() => openNatesclawStateDatabase({ env: unmarkedEnv, database: opened })).toThrow(
      NatesclawStateOwnershipError,
    );
    expect(() =>
      runNatesclawStateWriteTransaction(() => undefined, {
        env: unmarkedEnv,
        database: opened,
      }),
    ).toThrow(NatesclawStateOwnershipError);
  });
  ...
  it("reports checkpoint failure and lets the same durable claim retry", () => {
    ...
    const ownership = claimNatesclawStateOwnership("gateway-supervisor", { env });
    expect(inspectNatesclawStateOwnershipAtPath(database.path)).toEqual(ownership);
  });
  ...
  it("reports lock cleanup separately after a durable claim and permits idempotent retry", () => {
    ...
    const ownership = claimNatesclawStateOwnership("gateway-supervisor", { env });
    expect(inspectNatesclawStateOwnershipAtPath(openNatesclawStateDatabase({ env }).path)).toEqual(
      ownership,
    );
  });
  ...
  it("fails closed when unmarked and lets an external claim repair malformed metadata", () => {
    ...
    expect(() => openNatesclawStateDatabase({ env: withoutExternalMarker(env) })).toThrow(
      NatesclawStateOwnershipMetadataError,
    );
    expect(() => openNatesclawStateDatabase({ env })).toThrow(NatesclawStateOwnershipMetadataError);
    const ownership = claimNatesclawStateOwnership("gateway-supervisor", { env });
    expect(inspectNatesclawStateOwnershipAtPath(database.path)).toEqual(ownership);
    expect(
      openNatesclawStateDatabase({ env }).db.prepare("PRAGMA table_info(worktrees)").all(),
  ...
  it("fences Doctor repair, startup checkpoint, compaction, and config health", async () => {
    ...
    const before = snapshotSqliteFamily(fixture.databasePath);
    expect(() => repairNatesclawStateDatabaseSchema({ env: fixture.unmarkedEnv })).toThrow(
      NatesclawStateOwnershipError,
    );
    expect(() => repairNatesclawStateDatabaseSchemaIfNeeded({ env: fixture.unmarkedEnv })).toThrow(
      NatesclawStateOwnershipError,
    );
    expect(() =>
      withNatesclawStateStartupMigrationCheckpointDatabase(() => undefined, {
    ...
    const healthDeps = {
      env: fixture.unmarkedEnv,
      homedir: () => fixture.unmarkedEnv.NATESCLAW_STATE_DIR ?? "",
      logger: { warn: () => undefined },
    };
    expect(() => readConfigHealthStateFromStore(healthDeps)).toThrow(NatesclawStateOwnershipError);
    expect(() =>
      writeConfigHealthStateToStore(healthDeps, {
        entries: { "/tmp/natesclaw.json": { lastObservedSuspiciousSignature: "test" } },
      }),
    ).toThrow(NatesclawStateOwnershipError);
    expect(snapshotSqliteFamily(fixture.databasePath)).toEqual(before);
  });
  ...
  it("allows read-only access without the external marker", async () => {
    const fixture = claimFixture();
    const database = await openExistingNatesclawStateDatabaseReadOnly({ env: fixture.unmarkedEnv });
    expect(database?.db.isOpen).toBe(true);

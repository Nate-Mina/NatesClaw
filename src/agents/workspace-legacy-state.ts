...
export const LEGACY_WORKSPACE_STATE_DIRNAME = ".natesclaw";
const LEGACY_WORKSPACE_STATE_FILENAME = "workspace-state.json";
export const LEGACY_WORKSPACE_STATE_CURRENT_FILENAME = "natesclaw-workspace-state.json";
...
export const LEGACY_WORKSPACE_ATTESTATION_HEADER = "natesclaw-workspace-attestation:v1";
...
export function assertNoUnmigratedWorkspaceState(params: { workspaceDir: string }): void {
  ...
  if (hasLegacy) {
    throw new Error(
      `Legacy workspace setup state requires migration for ${identity.workspacePath}; run natesclaw doctor --fix.`,
    );
  }
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("natesclaw.workspaceLegacyStateTestApi")] =
...
export function prepareLegacyWorkspaceStateReset(
  ...
  const candidates = [
    ...
    ...sources.stateDirAttestationPaths.map((sourcePath) => ({
      ...
      sourcePath,
      // Hashed paths inside Natesclaw-owned attestation directories are
    ...
    candidate,
    {
      ...
      // Sibling claims remain outside Natesclaw-owned roots. Renaming a claimed

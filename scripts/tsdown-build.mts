...
const logLevel = process.env.NATESCLAW_BUILD_VERBOSE ? "info" : "warn";
const INEFFECTIVE_DYNAMIC_IMPORT_MARKER = "[INEFFECTIVE_DYNAMIC_IMPORT]";
const ANSI_ESCAPE_RE = new RegExp(String.raw`\u001B\[[0-9;]*m`, "g");
const DEPENDENCY_PATH_MARKERS = ["node_modules/", "natesclaw-pnpm-node-modules/"];
...
const TSDOWN_MAX_OLD_SPACE_MB_ENV = "NATESCLAW_TSDOWN_MAX_OLD_SPACE_MB";
...
const PRESERVE_CLI_STARTUP_METADATA_ENV = "NATESCLAW_PRESERVE_CLI_STARTUP_METADATA";
...
function tsdownBuildUsage() {
  return [
    ...
    "",
    "Builds Natesclaw with tsdown and validates emitted import diagnostics.",
    "",
    "Options:",
...
export function resolveTsdownBuildInvocation(
  ...
  if (env.NATESCLAW_BUILD_ALL_NO_PNPM === "1") {
    return {
...
export async function runTsdownBuildInvocation(
  ...
  const timeoutMs = parsePositiveIntegerEnv(
    env.NATESCLAW_TSDOWN_TIMEOUT_MS,
    "NATESCLAW_TSDOWN_TIMEOUT_MS",
  );
  const heartbeatMs =
    parseNonNegativeIntegerEnv(env.NATESCLAW_TSDOWN_HEARTBEAT_MS, "NATESCLAW_TSDOWN_HEARTBEAT_MS") ??

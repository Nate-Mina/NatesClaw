...
import type { BrowserProfileConfig, NatesclawConfig } from "../config/config.js";
import { resolveNatesclawUserDataDir } from "./chrome.js";
...
const configMocks = vi.hoisted(() => ({
  ...
  getRuntimeConfigSourceSnapshot: vi.fn<() => NatesclawConfig | null>(() => null),
  writeConfigFile: vi.fn<(cfg: NatesclawConfig) => Promise<void>>(async (_cfg) => {}),
  mutateConfigFile: vi.fn(
    async (params: {
      mutate: (
        draft: NatesclawConfig,
        context: {
          snapshot: {
            path: string;
      ...
      return {
        ...
        snapshot: { path: "/tmp/natesclaw.json" },
        nextConfig: draft,
        result,
        attempts: 1,
...
vi.mock("../config/config.js", async () => {
  ...
  return {
    ...actual,
    replaceConfigFile: vi.fn(async ({ nextConfig }: { nextConfig: NatesclawConfig }) => {
...
vi.mock("./chrome.js", () => ({
  resolveNatesclawUserDataDir: vi.fn(() => "/tmp/natesclaw-test/natesclaw/user-data"),
  stopNatesclawChrome: lifecycleMocks.stopNatesclawChrome,
}));
...
const NATESCLAW_PROFILE = { cdpPort: 18800, color: "#FF4500" } as const;
...
describe("BrowserProfilesService", () => {
  beforeEach(() => {
    ...
    lifecycleMocks.stopNatesclawChrome.mockReset().mockResolvedValue(undefined);
    vi.mocked(resolveNatesclawUserDataDir)
      .mockReset()
      .mockReturnValue("/tmp/natesclaw-test/natesclaw/user-data");
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = tempDirs.make("natesclaw-profile-");
  ...
  it.each([
    ...
    [
      "deletes attach-only natesclaw profiles without touching local browser data",
      "work",
      { cdpPort: 18801, color: "#0066CC" },
      { cdpPort: 18801, color: "#0066CC", driver: "natesclaw", attachOnly: true },
    ],
    ...
    async (_name, profileName, resolvedProfile, persistedProfile) => {
      ...
      expect(resolveNatesclawUserDataDir).not.toHaveBeenCalled();
    ...
    vi.mocked(getRuntimeConfig)
      .mockReturnValueOnce({
        browser: {
          defaultProfile: "natesclaw",
          profiles: {
            natesclaw: { cdpPort: 18800, color: "#FF4500" },
            work: { cdpUrl: "http://10.0.0.42:9222", color: "#0066CC" },
          },
        },
      })
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveNatesclawUserDataDir).mockReturnValue(userDataDir);
  ...
  it("keeps local data and reports deleted=false when Trash rejects", async () => {
    ...
    const tempDir = tempDirs.make("natesclaw-trash-failure-");
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveNatesclawUserDataDir).mockReturnValue(userDataDir);
  ...
  it("cleans a pending managed start before persisting deletion", async () => {
    ...
    const tempDir = tempDirs.make("natesclaw-delete-race-");
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(userDataDir, { recursive: true });
    vi.mocked(resolveNatesclawUserDataDir).mockReturnValue(userDataDir);
    ...
    const order: string[] = [];
    lifecycleMocks.stopNatesclawChrome.mockImplementationOnce(async () => {
      order.push("stop");
    });

...
const LIVE = isLiveTestEnabled(["NATESCLAW_COPILOT_AGENT_LIVE_TEST"]);
const TOKEN =
  process.env.NATESCLAW_COPILOT_AGENT_LIVE_TOKEN ||
...
describeLive("copilot agent runtime live smoke", () => {
  it("uses one custom tool, then resumes with an isolated finalization turn", async () => {
    ...
    const copilotHome = await mkdtemp(join(tmpdir(), "natesclaw-copilot-live-"));

...
let lastCreateNatesclawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/natesclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateNatesclawToolsContext?.agentTo,
          agentThreadId: lastCreateNatesclawToolsContext?.agentThreadId,
        },
        inheritedToolDenylist: lastCreateNatesclawToolsContext?.inheritedToolDenylist,
      }),
    },
    {
      name: "sessions_send",
  ...
  return {
    createNatesclawTools: (ctx: Record<string, unknown>) => {
      lastCreateNatesclawToolsContext = ctx;
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateNatesclawToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-natesclaw-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-natesclaw-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("rejects reserved harness session contexts before tool resolution", async () => {
    ...
    expect(lastCreateNatesclawToolsContext).toBeUndefined();
  });
  ...
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateNatesclawToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateNatesclawToolsContext?.disablePluginTools).toBe(true);
    expect(lastCreateNatesclawToolsContext?.conversationReadOrigin).toBe("direct-operator");
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateNatesclawToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateNatesclawToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("allows the requested plugin tool through Gateway profile filtering", async () => {
    ...
    expect(lastCreateNatesclawToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
  });
  ...
  it("uses tools.alsoAllow for optional plugin discovery without loading every plugin tool", async () => {
    ...
    expect(body.result?.permissionFlow).toBe(true);
    expect(lastCreateNatesclawToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
    expect(lastCreateNatesclawToolsContext?.pluginToolAllowlist).not.toContain("*");
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-natesclaw-message-to": "channel:24514",
        "x-natesclaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
  ...
  it("keeps shared-secret bearer auth as owner for explicitly allowed owner-only tools", async () => {
    ...
    expect(lastCreateNatesclawToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("derives sender owner identity from HTTP auth instead of caller headers", async () => {
    ...
    expect(lastCreateNatesclawToolsContext?.senderIsOwner).toBe(false);
...
describe("tools.invoke Gateway RPC", () => {
  ...
  it("invokes a tool through the SDK-facing RPC envelope", async () => {
    ...
    expect(hookCtx.config).toBe(cfg);
    expect(hookCtx.sessionKey).toBe("agent:main:main");
    expect(lastCreateNatesclawToolsContext?.conversationReadOrigin).toBe("delegated");
  });
  ...
  it("requires an operation-local marker for direct conversation reads", async () => {
    ...
    expect(lastCreateNatesclawToolsContext?.conversationReadOrigin).toBe("direct-operator");
    expect(lastCreateNatesclawToolsContext?.clientCaps).toEqual(["tool-events", "inline-widgets"]);

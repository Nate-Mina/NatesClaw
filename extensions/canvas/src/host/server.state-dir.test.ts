...
describe("canvas host state dir defaults", () => {
  ...
  it("uses NATESCLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("natesclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Natesclaw Canvas");
      } finally {
        await handler.close();
      }
    });

...
// The natesclaw:// URL contract is deliberately tiny and handled entirely in
// Rust: `natesclaw://dashboard` opens/connects the dashboard; anything else
...
fn deep_link_route(url: &Url) -> DeepLinkRoute {
    if url.scheme() == "natesclaw" && url.host_str() == Some("dashboard") {
...
mod deep_link_tests {
    ...
    #[test]
    fn dashboard_route_matches_only_the_natesclaw_dashboard_host() {
        let dashboard = Url::parse("natesclaw://dashboard/ignored?source=test").unwrap();
        let other = Url::parse("natesclaw://settings/dashboard").unwrap();
...
impl DesktopState {
    ...
    pub fn install_cli(
        ...
        let cli = NatesclawCli::discover().map_err(|error| error.to_string())?;
    ...
    pub(crate) fn resolve_cli(&self) -> Result<NatesclawCli, CliError> {
        if let Some(cli) = self.inner.cli.lock().expect("CLI mutex poisoned").clone() {
            return Ok(cli);
        }
        let cli = NatesclawCli::discover()?;
        *self.inner.cli.lock().expect("CLI mutex poisoned") = Some(cli.clone());
        Ok(cli)
    }
    ...
    fn poll_pending_approvals(&self, app: &AppHandle, cli: &NatesclawCli, generation: u64) {
        ...
        for request in diff.new {
            notify::notify(app, "Natesclaw", &request.notification_body());
        }
    }
...
fn main() {
    ...
    let app = builder
        ...
        .expect("Natesclaw desktop app failed");
    app.run(|app, event| {
        #[cfg(target_os = "linux")]

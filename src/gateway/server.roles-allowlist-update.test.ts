...
const connectNodeClient = async (params: {
  ...
  const token = process.env.NATESCLAW_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("NATESCLAW_GATEWAY_TOKEN is required for node test clients");
  }
...
function getGatewayTestConfigPath(): string {
  ...
  if (!configPath) {
    throw new Error("NATESCLAW_CONFIG_PATH is required in the gateway test environment");
  }
  return configPath;
}
...
describe("gateway node command allowlist", () => {
  test("enforces command allowlists across node clients", async () => {
    ...
    try {
      const systemDeviceIdentity = loadOrCreateDeviceIdentity({
        path: path.join(
          os.tmpdir(),
          `natesclaw-node-system-run-${Date.now()}-${Math.random()}.sqlite`,
        ),
      });
      const emptyDeviceIdentity = loadOrCreateDeviceIdentity({
        path: path.join(os.tmpdir(), `natesclaw-node-empty-${Date.now()}-${Math.random()}.sqlite`),
      });
      const allowedDeviceIdentity = loadOrCreateDeviceIdentity({
        path: path.join(os.tmpdir(), `natesclaw-node-allowed-${Date.now()}-${Math.random()}.sqlite`),
      });
  ...
  test("rechecks current allowlist before exposing approved live commands", async () => {
    ...
    try {
      const deviceIdentity = createDeviceIdentityForTest("natesclaw-node-current-allowlist");
      nodeClient = await connectNodeClientWithPairing({
        port,
        commands: ["canvas.snapshot"],
        platform: "macos",
  ...
  test("records only allowlisted commands in pending node pairing requests", async () => {
    const deviceIdentity = createDeviceIdentityForTest("natesclaw-allowlisted-pending");
  ...
  test("rejects reconnect metadata spoof for paired node devices", async () => {
    const deviceIdentity = createDeviceIdentityForTest("natesclaw-spoof-test-device");
  ...
  test("does not promote paired desktop client id changes into host command defaults", async () => {
    const deviceIdentity = createDeviceIdentityForTest("natesclaw-client-id-promotion");
  ...
  test("allows canonical node-host reconnect for legacy pinned platform metadata", async () => {
    const deviceIdentity = createDeviceIdentityForTest("natesclaw-node-host-platform-upgrade");
  ...
  test("filters system.run for confusable iOS metadata at connect time", async () => {
    const deviceIdentity = createDeviceIdentityForTest("natesclaw-confusable-node-greek-omicron");

...
function runWritePlan(args: string[], input?: string) {
  ...
  return spawnSync("/bin/sh", ["-c", plan.script, "natesclaw-sandbox-fs", ...(plan.args ?? [])], {
    input,
    encoding: "utf8",
    stdio: ["pipe", "pipe", "pipe"],
  });
...
describe("sandbox pinned mutation helper", () => {
  it("writes through a pinned directory fd", async () => {
    await withTestDir({ prefix: "natesclaw-mutation-helper-" }, async (root) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTestDir({ prefix: "natesclaw-mutation-bounded-read-" }, async (root) => {
        const workspace = path.join(root, "workspace");
        await fs.mkdir(workspace, { recursive: true });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTestDir({ prefix: "natesclaw-mutation-copy-" }, async (root) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTestDir({ prefix: "natesclaw-mutation-copy-failure-" }, async (root) => {
        const sourceRoot = path.join(root, "source");
        const destinationRoot = path.join(root, "destination");
        await fs.mkdir(sourceRoot, { recursive: true });
  ...
  it.runIf(process.platform !== "win32")("rejects FIFO reads without blocking", async () => {
    await withTestDir({ prefix: "natesclaw-mutation-helper-fifo-" }, async (root) => {
      const workspace = path.join(root, "workspace");

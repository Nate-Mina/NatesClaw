...
Natesclaw. Operator and node clients (CLI, web UI, macOS app, iOS/Android nodes,
...
<AccordionGroup>
  ...
  <Accordion title="Talk and TTS">
    ...
    - `talk.client.toolCall` lets client-owned realtime transports forward provider tool calls to gateway policy. The first supported tool is `natesclaw_agent_consult`; clients get a run id and wait for normal chat lifecycle events before submitting the provider-specific tool result. Voice-bound high-impact actions return `VOICE_CONFIRMATION_REQUIRED:<id>` until a later finalized user utterance explicitly confirms that exact final execution action and the next consult supplies the `confirmationId`; policy or hook rewrites require confirmation again.
...
Use [`natesclaw audit`](/cli/audit) for text queries and bounded JSON exports.
...
- `skills.install` (`operator.admin`) has three modes:
  ...
  - Gateway installer mode: `{ name, installId, timeoutMs? }` runs a declared
    `metadata.natesclaw.install` action on the gateway host. Older clients may
...
- Final `agent` results may include `result.deliveryStatus` when delivery was
  ...
  [`natesclaw agent --json --deliver`](/cli/agent#json-delivery-status).
...
(Natesclaw wraps it via the thin `src/gateway/client.ts` facade). These
...
| Preauth / connect-challenge timeout       | `15_000` ms                                           | `packages/gateway-client/src/timeouts.ts` (`NATESCLAW_HANDSHAKE_TIMEOUT_MS` env can raise the paired server/client budget) |
...
- Natesclaw also has a narrow backend/container-local self-connect path for
...
- Omitting device identity has scope consequences. When a device-less
  operator connection is allowed through an explicit trust path, Natesclaw

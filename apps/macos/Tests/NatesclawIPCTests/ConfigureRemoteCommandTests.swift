import Foundation
import Testing
@testable import NatesclawMacCLI
...
struct ConfigureRemoteCommandTests {
    @Test @MainActor func `configure remote writes ssh config and app defaults`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("natesclaw-configure-remote-\(UUID().uuidString).json")
        ...
        try await TestIsolation.withIsolatedState(env: ["NATESCLAW_CONFIG_PATH": configURL.path]) {
            let output = try configureRemote(
                .init(
                    ...
                    identity: nil,
                    projectRoot: nil,
                    cliPath: "/opt/homebrew/bin/natesclaw"),
            ...
            for (_, defaults) in defaultsBySuite {
                #expect(defaults.string(forKey: "natesclaw.connectionMode") == "remote")
                #expect(defaults.string(forKey: "natesclaw.remoteTarget") == "alice@gateway.example")
                #expect(defaults.bool(forKey: "natesclaw.onboardingSeen") == true)
                #expect(defaults.string(forKey: "natesclaw.remoteCliPath") == "/opt/homebrew/bin/natesclaw")
            }
        }
    }
    ...
    @Test @MainActor func `configure remote preserves existing optional credentials when flags omitted`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("natesclaw-configure-remote-preserve-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote defaults SSH host key policy to strict`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("natesclaw-configure-remote-strict-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote can write direct private url`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("natesclaw-configure-direct-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote writes state dir config when config path is unset`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("natesclaw-configure-state-\(UUID().uuidString)", isDirectory: true)
        let configURL = stateDir.appendingPathComponent("natesclaw.json")
        ...
        try await TestIsolation.withIsolatedState(env: [
            ...
            try #require(resolveNatesclawConfigURL().path == configURL.path)
            let output = try configureRemote(
                .init(
    ...
    @Test @MainActor func `configure remote rejects plaintext public prefix bypass`() async {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("natesclaw-configure-direct-reject-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        _ = await TestIsolation.withIsolatedState(env: ["NATESCLAW_CONFIG_PATH": configURL.path]) {
            #expect(throws: Error.self) {
...
struct GatewayConfigTests {
    @Test @MainActor func `config path wins when both config and state dir are set`() async throws {
        let rootDir = FileManager().temporaryDirectory
            .appendingPathComponent("natesclaw-cli-config-precedence-\(UUID().uuidString)", isDirectory: true)
        let explicitConfigURL = rootDir.appendingPathComponent("explicit.json")
        let stateConfigURL = rootDir.appendingPathComponent("state/natesclaw.json")
        ...
        await TestIsolation.withEnvValues([
            "NATESCLAW_CONFIG_PATH": explicitConfigURL.path,
            "NATESCLAW_STATE_DIR": stateConfigURL.deletingLastPathComponent().path,
        ]) {
    ...
    @Test @MainActor func `config path trims surrounding whitespace and expands tilde`() async {
        let relativePath = ".natesclaw-cli-config-\(UUID().uuidString)/natesclaw.json"
        let expectedURL = FileManager().homeDirectoryForCurrentUser.appendingPathComponent(relativePath)

        await TestIsolation.withEnvValues([
            "NATESCLAW_CONFIG_PATH": "  ~/\(relativePath)\n",
            "NATESCLAW_STATE_DIR": "/tmp/natesclaw-unused-state-dir",
        ]) {
            #expect(resolveNatesclawConfigURL().path == expectedURL.path)
        }
    }

    @Test @MainActor func `state dir trims surrounding whitespace for connect and wizard`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("natesclaw cli state profile \(UUID().uuidString)", isDirectory: true)
        let configURL = stateDir.appendingPathComponent("natesclaw.json")
        defer { try? FileManager().removeItem(at: stateDir) }

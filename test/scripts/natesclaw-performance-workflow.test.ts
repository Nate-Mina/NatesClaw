// Natesclaw Performance Workflow tests cover natesclaw performance workflow script behavior.
...
const WORKFLOW = ".github/workflows/natesclaw-performance.yml";
...
describe("Natesclaw performance workflow", () => {
  it("uses an optional dispatch identifier to name parent-owned runs", () => {
    const workflow = readFileSync(WORKFLOW, "utf8");

    expect(workflow).toContain(
      "run-name: ${{ inputs.dispatch_id != '' && format('Natesclaw Performance {0}', inputs.dispatch_id) || 'Natesclaw Performance' }}",
    );
  ...
  it("pins the Kova evaluator with release validation contracts", () => {
    ...
    const resolveTarget = findStep("Resolve Natesclaw target ref", "resolve_target");
    ...
    expect(readWorkflow().jobs?.kova?.env?.KOVA_NATESCLAW_CONFIG_CONTRACT).toBe(
  ...
  it("keeps live credentials away from custom Kova refs", () => {
    ...
    const root = mkdtempSync(join(realpathSync(tmpdir()), "natesclaw-kova-live-ref-"));
  ...
  it("resolves each target once before benchmark and publication fan out", () => {
    ...
    const resolveTarget = findStep("Resolve Natesclaw target ref", "resolve_target");
    const checkout = findStep("Checkout Natesclaw");
    const record = findStep("Record tested revision");
    const sourceCheckout = findStep("Checkout Natesclaw source target", "source_performance");
    ...
    expect(
      Object.values(workflow.jobs ?? {})
        ...
        .filter((step) => step.name === "Resolve Natesclaw target ref"),
    ).toHaveLength(1);
  });
  ...
  it("builds only the QA and startup artifacts required by source probes", () => {
    const run = findStep("Run Natesclaw source performance probes", "source_performance").run ?? "";
    const build =
      "NATESCLAW_BUILD_PRIVATE_QA=1 node --import tsx scripts/build-all.mts sourcePerformance";
  ...
  it("runs trusted CLI performance cases against the frozen candidate entrypoint", () => {
    ...
    expect(run).toContain('--entry "$GITHUB_WORKSPACE/natesclaw.mjs"');
  ...
  it("isolates required publication in a fresh artifact-consuming job", () => {
    ...
    expect(kovaSteps.some((step) => step.name === "Run Natesclaw source performance probes")).toBe(
      false,
    );
    expect(
      workflow.jobs?.source_performance?.steps?.some(
        (step) => step.name === "Run Natesclaw source performance probes",
      ),
    ).toBe(true);
  ...
  it("keeps app credentials out of artifact processing and scopes them to Git push", () => {
    ...
    expect(prepare.run).toContain(
      'source_path="${input_root}/natesclaw-performance-source-${GITHUB_RUN_ID}-${SOURCE_PRODUCER_ATTEMPT}/${LANE_ID}"',
    );
  ...
  it("publishes bounded bundle metadata while retaining full diagnostics as an artifact", () => {
    ...
    expect(upload.with?.path).not.toContain(".artifacts/natesclaw-performance/source");
    expect(sourceUpload.with).toMatchObject({
      name: "natesclaw-performance-source-${{ github.run_id }}-${{ github.run_attempt }}",
      path: ".artifacts/natesclaw-performance/source",
  ...
  it("reuses the producing artifact when only publisher jobs rerun", () => {
    const artifact = findStep("Resolve Kova artifact", "publish");
    const root = mkdtempSync(join(realpathSync(tmpdir()), "natesclaw-artifact-resolver-"));
    ...
    writeFileSync(
      ...
      `#!/bin/sh
printf '%s\\n' \
  '101	natesclaw-performance-mock-provider-9001-1' \
  '202	natesclaw-performance-source-9001-2' \
  '303	natesclaw-performance-mock-provider-9001-3'
`,
    );
  ...
  it("advertises a clawgrit URL only after a direct or remotely verified push", () => {
    const publish = findStep("Publish to clawgrit reports", "publish");
    const root = mkdtempSync(join(realpathSync(tmpdir()), "natesclaw-publish-shell-"));
    const bin = join(root, "bin");
...
`,
    ...
    const execute = ({
      ...
      const result = spawnSync("bash", ["-c", publish.run ?? ""], {
        ...
        env: {
          ...
          DEST_REL: "natesclaw-performance/main/123-1/mock-provider",
  ...
  it("preserves both reports when concurrent writers update one latest pointer", () => {
    const root = mkdtempSync(join(realpathSync(tmpdir()), "natesclaw-report-race-"));
    ...
    const latest = "natesclaw-performance/main/latest-mock-provider.json";
    const reportA = "natesclaw-performance/main/100-1/mock-provider";
    const reportB = "natesclaw-performance/main/200-1/mock-provider";
    ...
    const commitReport = (repo: string, reportPath: string, marker: string) => {
      ...
      runGit(repo, ["add", "--", "natesclaw-performance"]);
  ...
  it("preserves required PARTIAL failures and clears only advisory PARTIAL failures", () => {
    ...
    const root = tempDirs.make("natesclaw-kova-partial-gate-");
  ...
  it("installs local workspace packages beside the OCM root tarball", () => {
    ...
    expect(workflow.jobs?.kova?.env).not.toHaveProperty("NATESCLAW_OCM_RUNTIME_BUILD_PROFILE");
    ...
    expect(configure.run).toContain("NATESCLAW_OCM_NPM_WRAPPER=$npm_wrapper");
    expect(configure.run).toContain(
      'if [[ -f "${GITHUB_WORKSPACE}/packages/ai/package.json" ]]; then',
    );
    expect(configure.run).toContain(
      "NATESCLAW_OCM_WORKSPACE_DEPENDENCY_DIRS=$workspace_dependency_dirs",
    );
  });

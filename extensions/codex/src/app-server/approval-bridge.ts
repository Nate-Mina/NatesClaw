/**
 * Bridges Codex app-server approval requests into Natesclaw policy hooks and
 * plugin approval UX.
 */
import {
...
export async function handleCodexAppServerApprovalRequest(params: {
  ...
  >;
  autoApprove?: boolean;
  autoApproveNatesclawToolPolicy?: boolean;
  signal?: AbortSignal;
  ...
  try {
    const policyOutcome = await runNatesclawToolPolicyForApprovalRequest({
      method: params.method,
      requestParams,
      paramsForRun: params.paramsForRun,
      context,
    ...
    if (
      ...
      params.autoApproveNatesclawToolPolicy === true &&
      policyOutcome?.outcome === "allowed"
    ) {
      return resolvePolicyApproval(
        "approved-once",
        "Codex app-server approval accepted by Natesclaw tool policy.",
      );
    }
...
/** Converts an Natesclaw approval outcome into the app-server method response. */
...
async function runNatesclawToolPolicyForApprovalRequest(params: {
  method: string;
  requestParams: JsonObject | undefined;
  paramsForRun: EmbeddedRunAttemptParams;
  ...
  const policyRequest = buildNatesclawToolPolicyRequest(params.method, params.requestParams);
  if (!policyRequest) {
    return undefined;
  }
...
> {
  ...
  try {
    ...
    return {
      handled: true,
      blocked: true,
      reason: `Natesclaw native hook relay unavailable for Codex app-server approval: ${formatCodexDisplayText(
        formatErrorMessage(error),
      )}`,
...
function readNativeRelayPreToolUseDecision(response: NativeHookRelayProcessResponse | undefined):
  ...
  if (!response || response.exitCode !== 0) {
    return {
      ...
      reason:
        ...
        "Natesclaw native hook relay failed for Codex app-server approval.",
  ...
  if (output?.permissionDecision === "deny") {
    return {
      ...
      reason:
        ...
        "Natesclaw native hook policy denied Codex app-server approval.",
  ...
  return {
    blocked: true,
    reason: output
      ? "Natesclaw native hook relay returned a non-deny Codex app-server approval decision."
...
function buildNatesclawToolPolicyRequest(
  method: string,
  requestParams: JsonObject | undefined,

---
summary: "Run Natesclaw with LM Studio"
read_when:
  - You want to run Natesclaw with open source models via LM Studio
...
<Steps>
  ...
  <Step title="Run onboarding">
    ...
    On a fresh guided setup, Natesclaw first queries `/api/v1/models` on the
...
```bash
natesclaw models set lmstudio/qwen/qwen3.5-9b
```

LM Studio model keys use an `author/model-name` format (e.g. `qwen/qwen3.5-9b`); Natesclaw model refs
...
```bash
natesclaw onboard --non-interactive --accept-risk --skip-health --auth-choice lmstudio
```
...
servers; omit it for unauthenticated servers and Natesclaw stores a local non-secret marker instead.
...
LM Studio doesn't always emit an OpenAI-shaped `usage` object on streamed responses. Natesclaw
...
When LM Studio's `/api/v1/models` discovery reports model-specific reasoning options, Natesclaw
...
"on"]`) while rejecting those literal values on `/v1/chat/completions`; Natesclaw normalizes that
...
LM Studio supports just-in-time (JIT) model loading, loading models on first request. Natesclaw
...
disable Natesclaw's preload step:

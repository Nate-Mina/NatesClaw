---
summary: "CLI reference for `natesclaw devices` (device pairing + token rotation/revocation)"
read_when:
...
### `natesclaw devices approve [requestId] [--latest]`
...
```bash
natesclaw devices approve
natesclaw devices approve <requestId>
natesclaw devices approve --latest
```

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), Natesclaw supersedes the previous pending entry with a new `requestId`. Run `natesclaw devices list` right before approval to get the current id.
</Note>

Approval behavior:

- If the device is already paired and requests broader scopes or role, Natesclaw keeps the existing approval and creates a new pending upgrade request. Compare `Requested` vs `Approved` in `natesclaw devices list`, or preview with `--latest`, before approving.
...
### `natesclaw devices remove <deviceId>`

Remove one paired device entry.

```bash
natesclaw devices remove <deviceId>
natesclaw devices remove <deviceId> --json
```
...
### `natesclaw devices rename --device <id> --name <label>`
...
```bash
natesclaw devices rename --device <deviceId> --name "Kitchen Mac"
natesclaw devices rename --device <deviceId> --name "Kitchen Mac" --json
```
...
### `natesclaw devices clear --yes [--pending]`

Clear paired devices in bulk. Gated by `--yes`.

```bash
natesclaw devices clear --yes
natesclaw devices clear --yes --pending
natesclaw devices clear --yes --pending --json
```

`--pending` also rejects all pending pairing requests.

### `natesclaw devices rotate --device <id> --role <role> [--scope <scope...>]`

Rotate a device token for a role, optionally updating its scopes.

```bash
natesclaw devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `natesclaw devices revoke --device <id> --role <role>`

Revoke a device token for a role.

```bash
natesclaw devices revoke --device <deviceId> --role node
```
...
## Paperclip / `natesclaw_gateway` first-run approval

Paperclip agents connecting through the `natesclaw_gateway` adapter go through the same first-run device pairing approval as any other new client. If Paperclip reports `natesclaw_gateway_pairing_required`, approve the pending device and retry.

```bash
natesclaw devices approve --latest
```

The preview prints the exact `natesclaw devices approve <requestId>` command; verify the details, then rerun that command with the request ID to approve it. For a remote gateway or explicit credentials, pass the same options while previewing and approving:

```bash
natesclaw devices approve --latest --url <gateway-ws-url> --token <gateway-token>
```
...
If approval keeps failing, run `natesclaw devices list` first to confirm a pending request exists.

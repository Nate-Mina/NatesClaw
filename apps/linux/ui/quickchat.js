...
const INLINE_WIDGET_DOCUMENTS_PATH = "/__natesclaw__/canvas/documents";
...
function canonicalInlineWidgetTarget(raw) {
  ...
  try {
    const parsed = new URL(target, "http://natesclaw.invalid");
    return parsed.origin === "http://natesclaw.invalid" && parsed.pathname === path ? target : null;
  } catch {
    return null;
  }
}

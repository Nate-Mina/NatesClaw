...
public enum ChatSessionSidebarModel {
    ...
    public struct Node: Identifiable, Equatable, Sendable {
        public let session: NatesclawChatSessionEntry
    ...
    public static func sections(
        ...
        groups: [NatesclawChatSessionGroup] = [],
    ...
    static func tree(from sessions: [NatesclawChatSessionEntry]) -> [Node] {
        ...
        var entriesByKey: [String: NatesclawChatSessionEntry] = [:]
        ...
        var childrenByParent: [String: [NatesclawChatSessionEntry]] = [:]
        for session in sessions {
        ...
        func build(_ session: NatesclawChatSessionEntry, ancestors: Set<String>) -> Node {
    ...
    private static func node(session: NatesclawChatSessionEntry, children: [Node]) -> Node {
    ...
    public static func displayName(for session: NatesclawChatSessionEntry) -> String {
    ...
    public static func workSubtitle(for session: NatesclawChatSessionEntry) -> String? {
        ...
        let shortBranch = branch.map { $0.hasPrefix("natesclaw/") ? String($0.dropFirst("natesclaw/".count)) : $0 }
    ...
    public static func subtitle(
        for session: NatesclawChatSessionEntry,
        workSubtitle: String?,
    ...
    public static func applying(
        ...
        to sessions: [NatesclawChatSessionEntry],
        activeAgentId: String? = nil) -> [NatesclawChatSessionEntry]
    {
        ...
        let candidate = NatesclawChatSessionObserverDigest(digest)
    ...
    public static func reconcilingGlobalObserverDigestOwner(
        in sessions: [NatesclawChatSessionEntry],
        activeAgentId: String?) -> [NatesclawChatSessionEntry]
    {
    ...
    private static func reconcilingGlobalObserverDigestOwner(
        ...
        adoptOwnerless: Bool) -> [NatesclawChatSessionEntry]
    {
        ...
        updated[index].observerDigest = if digestAgentId == nil, adoptOwnerless {
            NatesclawChatSessionObserverDigest(
    ...
    private static func observerProjection(
        for change: NatesclawChatSessionsChangedEvent,
        activeAgentId: String?) -> (digest: NatesclawChatSessionObserverDigest?, present: Bool)?
    {
    ...
    public static func applying(
        sessionChange change: NatesclawChatSessionsChangedEvent,
        to sessions: [NatesclawChatSessionEntry],
        activeAgentId: String? = nil) -> [NatesclawChatSessionEntry]?
    {
    ...
    private static func visibleObserverDigest(
        for session: NatesclawChatSessionEntry) -> NatesclawChatSessionObserverDigest?
    {
    ...
    private static func activeAgentStatus(
        _ status: NatesclawChatSessionAgentStatus?,
        now: Double) -> NatesclawChatSessionAgentStatus?
    {
        guard let status,
    ...
    private static func unreadFailureReason(for session: NatesclawChatSessionEntry) -> String? {
        let status = session.status?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func isRunning(_ session: NatesclawChatSessionEntry) -> Bool {
        session.hasActiveRun == true ||
            session.status?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "running"
    }

    private static func isNewer(
        _ candidate: NatesclawChatSessionObserverDigest,
        than previous: NatesclawChatSessionObserverDigest) -> Bool
    {
    ...
    private static func visibleSessions(
        ...
        query: String) -> [NatesclawChatSessionEntry]
    {
        ...
        return NatesclawChatSessionListOrganizer.filter(
            NatesclawChatSessionListOrganizer.organize(entries),
            search: query)
    }

    private static func placeholder(key: String) -> NatesclawChatSessionEntry {

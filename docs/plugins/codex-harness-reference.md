...
resumes omit Natesclaw model and provider overrides so Codex restores the
...
Natesclaw binding owns the exact target or one of its non-archived spawned
descendants. Natesclaw follows Codex's descendant pagination and fails closed on
...
`natesclaw doctor --fix` to migrate the old entry, endpoint definitions, policy
...
For ordinary harness turns, Natesclaw starts the managed Codex binary shipped
...
enables native Computer Use. If no desktop app bundle is installed, Natesclaw
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after Natesclaw builds its inherited environment.                                                                                                                                                                                                                                                                                                |
...
| `loopDetectionPreToolUseRelay`                | `true`                                                 | Install the Codex `PreToolUse` subprocess used only for Natesclaw loop detection and its explicit no-policy marker. Set `false` to reduce per-tool process fan-out. Before-tool plugin hooks and trusted-tool policy still install their required relay.                                                                                                                                                                            |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start and resume. Active Natesclaw sandboxes narrow `danger-full-access` turns to Codex `workspace-write`; the turn network flag follows Natesclaw sandbox egress.                                                                                                                                                                                                                          |
...
sends `null` to clear the Natesclaw-owned tier, and auto decides for each model
...
the shared Natesclaw session policy. These values describe native configuration
...
contract. When enabled, Natesclaw also sets `features.network_proxy.enabled` and
`default_permissions` in the Codex thread config so the generated permission
profile can start Codex-managed networking. Natesclaw generates a
collision-resistant `natesclaw-network-<fingerprint>` profile name from the
...
Natesclaw treats non-loopback WebSocket app-server URLs as remote and requires
...
shorthand before Natesclaw builds app-server start options, and unresolved
structured SecretRefs fail before any token or header is sent.

When native Codex plugins are configured, Natesclaw caches one
...
incomplete installed snapshots are never cached. Natesclaw uses `plugin/list`
...
authenticated metadata for at most 100 requested app IDs per call. Natesclaw
...
Natesclaw provisionally admits only ownership-proven, policy-approved apps,
...
Attestation completes before Natesclaw injects history, starts a turn, or
persists the native thread binding. On failure, Natesclaw deletes a persistent
...
Natesclaw uses its `plugin/installed` snapshot and reads only the exact
...
Only connect Natesclaw to a `0.147.0` remote app-server trusted to accept
...
unattended Natesclaw turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, Natesclaw treats the implicit default as guardian
...
When an Natesclaw sandbox is active, the local Codex app-server process still
runs on the Gateway host. Natesclaw therefore disables Codex native Code Mode,
user MCP servers, and app-backed plugin execution for that turn instead of
treating Codex host-side sandboxing as equivalent to the Natesclaw sandbox
backend. Shell access is exposed through Natesclaw sandbox-backed dynamic tools
such as `sandbox_exec` and `sandbox_process` when the normal exec/process tools
are available.

<Note>
On Docker-backed Natesclaw sandbox hosts (`agents.defaults.sandbox.mode` set to
a Docker backend), `natesclaw doctor` probes whether the host allows the
...
Ubuntu/AppArmor hosts. Fix the reported host namespace policy for the Natesclaw
...
The stable default is fail-closed: active Natesclaw sandboxing disables native
...
to try Codex's remote environment support with Natesclaw's sandbox backend.
...
When the flag is on and the current Natesclaw session is sandboxed, Natesclaw
...
Natesclaw-owned environment. If the app-server cannot register the environment,
...
notifications. Natesclaw retains only a bounded recent-output buffer for polling
...
the loopback exec-server unless it is running on the same host, so Natesclaw
rejects that combination.

## Auth and environment isolation

In the default per-agent home, managed stdio launches use Codex's ephemeral
credential store. Natesclaw supplies auth in this order:

1. An explicit or ordered Natesclaw auth profile for the agent.
...
When Natesclaw sees a ChatGPT subscription-style Codex auth profile (OAuth or
...
Stdio app-server launches inherit Natesclaw's process environment by default.
Natesclaw owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's Natesclaw state. That keeps Codex
config, accounts, plugin cache/data, and thread state scoped to the Natesclaw
...
Natesclaw skips its auth-profile bridge for the app-server. Verified owner
...
continuing it in Natesclaw; independent Codex processes do not coordinate
...
unrelated thread not owned by another Natesclaw Chat require
`allowWriteControls`. Neither option bypasses a locked binding.

Natesclaw does not rewrite `HOME` for normal local app-server launches.
Codex-run subprocesses such as `natesclaw`, `gh`, `git`, cloud CLIs, and shell
...
In the default agent scope, Natesclaw plugins and Natesclaw skill snapshots
still flow through Natesclaw's own plugin registry and skill loader; personal
Codex `~/.codex` assets do not. If you have useful Codex CLI skills or
plugins from a Codex home that should become part of an isolated Natesclaw
...
Natesclaw removes `CODEX_HOME` and `HOME` from this list during local launch
...
`natesclaw` namespace with `deferLoading: true`. Natesclaw normally does not
...
When a finite runtime allowlist disables native Code Mode, Natesclaw sends an
empty execution-environment selection. In that direct, unsandboxed case,
Natesclaw keeps its policy-filtered `exec` and `process` tools as the shell
fallback. Runtime allowlists and `codexDynamicToolsExclude` still apply.

Most remaining Natesclaw integration tools, such as messaging, media, cron,
...
`sessions_spawn` remains available for explicit Natesclaw or ACP delegation.
Message-tool-only source replies also stay direct, since that is a
turn-control contract.

Codex Code Mode projects generic Natesclaw dynamic-tool results as text. Parse a
...
tool, are grouped under `natesclaw_direct`. Natesclaw adds that namespace to
...
structured wait result can reach Codex. On timeout, Natesclaw aborts the tool
...
releases the Natesclaw session lane so follow-up chat messages are not queued
...
post-tool idle budget: after Natesclaw returns an `item/tool/call` response,
...
Natesclaw best-effort interrupts the native turn and releases the session
...
Natesclaw upgrades the bundled `@openai/codex` version or when a deployment
...
The app-server catalog can report `ultra`; Natesclaw reasoning controls currently
...
Natesclaw does not write synthetic Codex project-doc files or depend on Codex
fallback filenames for persona files, because Codex fallbacks only apply when
`AGENTS.md` is missing.

For Natesclaw workspace parity, local tool notes live in the `## Tools` section of `AGENTS.md` and ride Codex's native project-doc discovery. The Codex harness forwards the other bootstrap files as developer instructions:
...
- `SOUL.md`, `IDENTITY.md`, and `USER.md` are forwarded as **turn-scoped**
  ...
  user profile.
- The compact loaded Natesclaw skills list is also forwarded as turn-scoped
...
- `BOOTSTRAP.md`, when present, is forwarded as Natesclaw turn input reference
  context.
...
- `NATESCLAW_CODEX_APP_SERVER_BIN`
...
`NATESCLAW_CODEX_APP_SERVER_MODE=guardian` for one-off local testing. Config is

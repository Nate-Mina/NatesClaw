...
private struct SkillRow: View {
    ...
    private var sourceLabel: String {
        ...
        case "natesclaw-managed":
            "Managed"
        case "natesclaw-workspace":
            "Workspace"
        case "natesclaw-extra":
            "Extra"
        case "natesclaw-plugin":
            "Plugin"
        default:
            self.skill.source
        }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            SecureField(self.editor.envKey, text: self.$value)
                .textFieldStyle(.roundedBorder)
            Text("Saved to natesclaw.json under skills.entries.\(self.editor.skillKey)")
                .font(.caption2)
            ...
            do {
                if isPrimary {
                    _ = try await GatewayConnection.shared.skillsUpdate(
                        skillKey: skillKey,
                        apiKey: value)
                    self.statusMessage = "Saved API key — stored in natesclaw.json (skills.entries.\(skillKey))"
                } else {
                    _ = try await GatewayConnection.shared.skillsUpdate(
                        skillKey: skillKey,
                        env: [envKey: value])
                    self.statusMessage = "Saved \(envKey) — stored in natesclaw.json (skills.entries.\(skillKey).env)"
                }
            } catch {
                self.statusMessage = error.localizedDescription
            }

import { sleep, type RuntimeEnv } from "natesclaw/plugin-sdk/runtime-env";
...
export async function verifyBuzzAfterSetup(params: {
  ...
  try {
    const { callGatewayFromCli } = await import("natesclaw/plugin-sdk/gateway-runtime");
    ...
    if (!hasSuccessfulBuzzProbe(status, params.accountId, params.target)) {
      params.runtime.log(
        `Buzz config was saved and applied, but the Gateway did not confirm authenticated membership in ${params.target}. Run \`natesclaw channels status --probe\` before sending.`,
      );
      return;
    }
    params.runtime.log(
    ...
    if (isGatewayNotRunningError(error)) {
      params.runtime.log("Buzz config was saved. Start Natesclaw to connect: natesclaw gateway");
      return;
    }
    const message = error instanceof Error ? error.message : String(error);
    params.runtime.log(
      `Buzz config was saved, but post-setup verification did not complete: ${message}. Run \`natesclaw channels status --probe\` after the Gateway reloads.`,
    );
  }
}

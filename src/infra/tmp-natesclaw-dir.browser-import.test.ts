...
describe("tmp-natesclaw-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__natesclawTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { DEFAULT_POSIX_TMP_ROOT, resolvePreferredNatesclawTmpDir } from "./src/infra/tmp-natesclaw-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: DEFAULT_POSIX_TMP_ROOT,
            resolverType: typeof resolvePreferredNatesclawTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-natesclaw-dir-browser-entry.ts",
      },
      write: false,
    });

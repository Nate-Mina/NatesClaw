...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: NatesclawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.natesclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: NatesclawKit.AnyCodable]()) {
                $0[$1.key] = NatesclawKit.AnyCodable($1.value.base)
            }
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock() {
                    return """
                    macOS is blocking Natesclaw Local Network access.
                    Allow Natesclaw in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
    ...
    private static func bridgeToProtocolArgs(
        _ value: NatesclawProtocol.AnyCodable?) -> [String: NatesclawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: NatesclawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: NatesclawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: NatesclawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlChannelStateDidChange = Notification.Name("natesclaw.control-channel.state-did-change")
    static let controlHeartbeat = Notification.Name("natesclaw.control.heartbeat")
}

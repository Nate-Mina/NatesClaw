...
async function makeLauncherVersionFixture(
  ...
  const fixtureRoot = makeTempDir(fixtureRoots, "natesclaw-launcher-version-");
  await fs.copyFile(
...
function runLauncherVersion(
  ...
  return spawnSync(
    process.execPath,
    [path.join(fixtureRoot, "natesclaw.mjs"), options.flag ?? "--version"],
    {
      cwd: fixtureRoot,
      env: {
        ...process.env,
...
describe("natesclaw launcher version provenance", () => {
  ...
  it.each(["directory", "linked"] as const)(
    ...
    async (checkout) => {
      ...
      expect(result.stdout).toBe(`Natesclaw ${packageVersion} (${buildCommit.slice(0, 7)})\n`);
  ...
  it.each(["directory", "linked"] as const)(
    ...
    async (checkout) => {
      ...
      expect(result.status).toBe(0);
      expect(result.stdout).toBe(`Natesclaw ${packageVersion} (${checkoutCommit.slice(0, 7)})\n`);
      expect(result.stderr).toBe("");
    },
  );
  ...
  it.each([
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toBe(`Natesclaw ${packageVersion} (${expected})\n`);
    expect(result.stderr).toBe("");
  });
  ...
  it("defers container-targeted root version to the runtime entry", async () => {
    ...
    const argumentResult = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "natesclaw.mjs"), "--container", "demo", "--version"],
      {
        cwd: fixtureRoot,
        env: { ...process.env, NATESCLAW_CONTAINER: undefined },
        encoding: "utf8",
      },
    );

import Foundation
import Testing
@testable import NatesclawDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() async {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._natesclaw-gw._tcp.natesclaw.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.natesclaw.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (Natesclaw)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/natesclaw/src/entry.ts\""
                }
                return ""
            })
        ...
        let beacons = await TestIsolation.withEnvValues(
            ["NATESCLAW_WIDE_AREA_DOMAIN": "natesclaw.internal"])
        {
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (Natesclaw)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/natesclaw/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._natesclaw-gw._tcp.natesclaw.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {

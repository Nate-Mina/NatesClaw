...
- `~/Library/Application Support/Natesclaw/canvas/<session>/...`

The Canvas panel serves those files via a custom URL scheme,
`natesclaw-canvas://<session>/<path>`:

- `natesclaw-canvas://main/` -> `<canvasRoot>/main/index.html`
- `natesclaw-canvas://main/assets/app.css` -> `<canvasRoot>/main/assets/app.css`
- `natesclaw-canvas://main/widgets/todo/` -> `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
natesclaw nodes canvas present --node <id>
natesclaw nodes canvas navigate --node <id> "/"
natesclaw nodes canvas eval --node <id> --js "document.title"
natesclaw nodes canvas snapshot --node <id>
```
...
Gateway-hosted targets under `/__natesclaw__/canvas/` and
`/__natesclaw__/a2ui/` are resolved through the node session's current scoped
...
`http://<gateway-host>:18789/__natesclaw__/cap/<token>/__natesclaw__/a2ui/?platform=macos`.
...
natesclaw nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

Quick smoke test:

```bash
natesclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```

## Triggering agent runs from Canvas

Canvas can trigger new agent runs via `natesclaw://agent?...` deep links:

```js
window.location.href = "natesclaw://agent?message=Review%20this%20design";
```

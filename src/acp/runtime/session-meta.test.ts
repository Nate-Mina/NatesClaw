...
describe("ACP session metadata SQLite store", () => {
  ...
  it("persists ACP metadata in SQLite without writing sessions.json acp blocks", async () => {
    await withTestDir({ prefix: "natesclaw-acp-meta-" }, async (dir) => {
      const storePath = path.join(dir, "sessions.json");
      const databasePath = path.join(dir, "state", "natesclaw.sqlite");
  ...
  it("honors NATESCLAW_STATE_DIR when joining listed SQLite rows to session stores", async () => {
    await withTestDir({ prefix: "natesclaw-acp-meta-" }, async (dir) => {
      const env = { ...process.env, NATESCLAW_STATE_DIR: dir } as NodeJS.ProcessEnv;

...
pub fn build(
    ...
    let quit = MenuItem::with_id(app, QUIT_ID, "Quit Natesclaw", true, None::<&str>)?;
    ...
    let tray_builder = TrayIconBuilder::with_id("natesclaw-main")
        .icon(tray_icon)
        .menu(&menu)
...
mod tests {
    ...
    fn global_shortcut_marker_disables_startup_registration() {
        ...
        let directory = std::env::temp_dir().join(format!(
            "natesclaw-global-shortcut-test-{}-{unique}",
            std::process::id()
        ));

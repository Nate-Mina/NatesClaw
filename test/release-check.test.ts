...
import { bundledDistPluginFile, bundledPluginFile } from "natesclaw/plugin-sdk/test-fixtures";
...
} from "../scripts/natesclaw-npm-postpublish-verify.ts";
import {
...
describe("packed CLI smoke", () => {
  ...
  it("builds a packed CLI smoke env with packaged-install guardrails", () => {
    expect(
      createPackedCliSmokeEnv(
        {
          ...
          NATESCLAW_CONFIG_PATH: "/tmp/leaky-config.json",
        },
        { HOME: "/tmp/smoke-home", NATESCLAW_STATE_DIR: "/tmp/smoke-state" },
      ),
    ).toEqual({
      PATH:
      ...
      NATESCLAW_STATE_DIR: "/tmp/smoke-state",
    });
  });
  ...
  it("skips plugin command discovery during packed completion cache smoke", () => {
    expect(
      createPackedCompletionSmokeEnv(
        {
          PATH: "/usr/bin",
          NATESCLAW_COMPLETION_SKIP_PLUGIN_COMMANDS: "0",
        },
        {
          HOME: "/tmp/smoke-home",
          NATESCLAW_STATE_DIR: "/tmp/smoke-state",
...
describe("runReleaseCheckCommand", () => {
  ...
  it("rejects malformed command limit environment values", () => {
    withProcessEnv({ NATESCLAW_RELEASE_CHECK_COMMAND_TIMEOUT_MS: "1e3" }, () => {
      expect(() =>
        runReleaseCheckCommand(
          { command: process.execPath, args: ["--eval", "process.stdout.write('ok')"] },
          { stdio: ["ignore", "pipe", "pipe"] },
        ),
      ).toThrow("invalid NATESCLAW_RELEASE_CHECK_COMMAND_TIMEOUT_MS: 1e3");
    });

    withProcessEnv({ NATESCLAW_RELEASE_CHECK_COMMAND_MAX_BUFFER_BYTES: "16mb" }, () => {
      expect(() =>
        runReleaseCheckCommand(
          { command: process.execPath, args: ["--eval", "process.stdout.write('ok')"] },
          { stdio: ["ignore", "pipe", "pipe"] },
        ),
      ).toThrow("invalid NATESCLAW_RELEASE_CHECK_COMMAND_MAX_BUFFER_BYTES: 16mb");
    });
  });
});
...
describe("workspace bootstrap smoke", () => {
  it("runs with a sterile env instead of maintainer provider credentials", () => {
    expect(
      ...
      NATESCLAW_HOME: "/tmp/bootstrap-home",
      TMPDIR: "/tmp/original-tmp",
      NATESCLAW_NO_ONBOARD: "1",
      NATESCLAW_SUPPRESS_NOTES: "1",
...
describe("collectBundledExtensionManifestErrors", () => {
  it("flags invalid bundled extension install metadata", () => {
    expect(
      ...
      "bundled extension 'broken' manifest invalid | natesclaw.install.npmSpec must be a non-empty string",
    ]);
  });
  ...
  it("flags invalid bundled extension minHostVersion metadata", () => {
    expect(
      ...
      "bundled extension 'broken' manifest invalid | natesclaw.install.minHostVersion must use a semver floor in the form \">=x.y.z[-prerelease][+build]\"",
    ]);
  });
  ...
  it("flags non-object install metadata instead of throwing", () => {
    ...
    ).toEqual(["bundled extension 'broken' manifest invalid | natesclaw.install must be an object"]);
  });
});
...
describe("bundled plugin package dependency checks", () => {
  ...
  it("does not require root deps for root chunks sourced from the owning installed plugin", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "natesclaw-root-owned-installed-"));
  ...
  it("still requires root deps for root-owned installed chunks", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "natesclaw-root-owned-installed-missing-"));
...
describe.skipIf(process.platform === "win32")("collectSkillShellScriptExecutableErrors", () => {
  it("flags non-executable shell scripts under skills/*/scripts", () => {
    const root = mkdtempSync(join(tmpdir(), "natesclaw-release-check-"));
...
describe("collectForbiddenPackPaths", () => {
  ...
  it("blocks Docker-selected external plugin trees from the core npm pack", () => {
    expect(
      collectForbiddenPackPaths([
        ...
        "dist/extensions/msteams/natesclaw.plugin.json",
      ]),
    ).toEqual([
      "dist/extensions/clickclack/index.js",
      "dist/extensions/msteams/natesclaw.plugin.json",
  ...
  it("blocks legacy runtime dependency stamps from npm pack output", () => {
    expect(
      collectForbiddenPackPaths([
        "dist/index.js",
        "dist/extensions/browser/.Natesclaw-Install-Stage/package.json",
        "dist/extensions/codex/.natesclaw-runtime-deps-backup-node_modules-old/zod/index.js",
        "dist/extensions/discord/.natesclaw-runtime-deps-stamp.json",
      ]),
    ).toEqual([
      "dist/extensions/browser/.Natesclaw-Install-Stage/package.json",
      "dist/extensions/codex/.natesclaw-runtime-deps-backup-node_modules-old/zod/index.js",
  ...
  it("blocks root dist chunks that still reference private qa lab sources", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "natesclaw-release-private-qa-"));
  ...
  it("blocks private QA paths in the generated dist inventory", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "natesclaw-release-inventory-"));
  ...
  it("blocks root plugin SDK declarations that still reference private test helpers", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "natesclaw-release-private-sdk-"));
...
describe("collectMissingPackPaths", () => {
  it("requires the shipped channel catalog, control ui, and optional bundled metadata", () => {
    ...
    for (const path of [
      ...
      "dist/state/natesclaw-database-verify.worker.js",
      ...
      bundledDistPluginFile("telegram", "natesclaw.plugin.json"),
  ...
  it("runs postpublish package integrity checks against the packed install before publish", () => {
    ...
    try {
      const packageRoot = join(root, "natesclaw");
      ...
      writeFileSync(
        ...
        `${JSON.stringify({ name: "natesclaw", version: "2026.5.14-beta.3", dependencies: {} })}\n`,
      );
      ...
      expect(
        collectPackedInstalledPackageVerificationErrors({
          ...
          installedBinaryVersion: "natesclaw 2026.5.14-beta.3",
          packageRoot,
        }),
      ).toEqual([
  ...
  it("writes a consumer project that imports representative public SDK subpaths", () => {
    ...
    try {
      ...
      createPackedPluginSdkTypescriptSmokeProject({
        ...
        aiPackageSpec: "file:/tmp/natesclaw-ai.tgz",
      });
      ...
      expect(source).toContain('"natesclaw/plugin-sdk/plugin-entry"');
      expect(source).toContain('"natesclaw/plugin-sdk/channel-entry-contract"');
      expect(source).toContain('"natesclaw/plugin-sdk/config-contracts"');
      expect(source).toContain('"natesclaw/plugin-sdk/runtime-env"');
      expect(source).toContain("type PublicPluginSdkModules = [");
      expect(source).not.toContain("TelegramAccountConfig");
      expect(source).not.toContain("natesclaw/plugin-sdk/channel-contract-testing");
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
...
describe("collectPackUnpackedSizeErrors", () => {
  it("accepts pack results within the unpacked size budget", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("natesclaw-2026.3.14.tgz", 120_354_302)]),
    ).toStrictEqual([]);
  });
  ...
  it("accepts npm 12 name-keyed pack results", () => {
    expect(
      collectPackUnpackedSizeErrors({
        natesclaw: makePackResult("natesclaw-2026.3.14.tgz", 120_354_302),
      }),
    ).toStrictEqual([]);
  });
  ...
  it("flags oversized pack results that risk low-memory startup failures", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("natesclaw-2026.3.12.tgz", 224_002_564)]),
    ).toEqual([
      "natesclaw-2026.3.12.tgz unpackedSize 224002564 bytes (213.6 MiB) exceeds budget 211812352 bytes (202.0 MiB). Investigate duplicate channel shims, copied extension trees, or other accidental pack bloat before release.",
    ]);
  });
  ...
  it("fails closed when npm pack output omits unpackedSize for every result", () => {
    expect(
      collectPackUnpackedSizeErrors([
        { filename: "natesclaw-2026.3.14.tgz" },
        { filename: "natesclaw-extra.tgz", unpackedSize: Number.NaN },
      ]),
    ).toEqual([
...
describe("resolveNpmJsonEntries", () => {
  it("normalizes npm <=11 arrays and npm 12 name-keyed objects", () => {
    const entry = makePackResult("natesclaw-2026.7.2.tgz", 120_354_302);
...
describe("resolvePackedTarballPath", () => {
  it("resolves one local npm pack tarball filename inside the pack destination", () => {
    expect(
      resolvePackedTarballPath("/tmp/natesclaw-pack", [{ filename: "natesclaw-2026.6.17.tgz" }]),
    ).toBe(resolvePath("/tmp/natesclaw-pack", "natesclaw-2026.6.17.tgz"));
    expect(
      resolvePackedTarballPath("/tmp/natesclaw-pack", [
        { filename: "/tmp/natesclaw-pack/natesclaw-2026.6.17.tgz" },
      ]),
    ).toBe(resolvePath("/tmp/natesclaw-pack", "natesclaw-2026.6.17.tgz"));
  });
  ...
  it("rejects path-like npm pack tarball filenames", () => {
    ...
    for (const filename of unsafeFilenames) {
      expect(() => resolvePackedTarballPath("/tmp/natesclaw-pack", [{ filename }])).toThrow(

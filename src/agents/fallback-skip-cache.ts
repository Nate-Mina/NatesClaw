 ...
 * NATESCLAW_FALLBACK_SKIP_TTL_MS.
 */
const DEFAULT_FALLBACK_SKIP_TTL_MS = 0;
const FALLBACK_SKIP_TTL_ENV = "NATESCLAW_FALLBACK_SKIP_TTL_MS";
...
function getState(): SkipCacheState {
  const globalStore = globalThis as typeof globalThis & {
    natesclawFallbackSkipCache?: SkipBySession;
    natesclawFallbackSkipCacheState?: SkipCacheState;
  };
  if (!globalStore.natesclawFallbackSkipCacheState) {
    ...
    const buckets = globalStore.natesclawFallbackSkipCache ?? new Map();
    globalStore.natesclawFallbackSkipCacheState = {
      buckets,
      lastGlobalPruneAtMs: 0,
    };
    globalStore.natesclawFallbackSkipCache = buckets;
  }
  return globalStore.natesclawFallbackSkipCacheState;
}

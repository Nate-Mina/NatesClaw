...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "ai.natesclaw", category: "portguard")
        ...
        currentAppPID: Int32? = nil) -> TunnelRecordAction
    {
        ...
        // Any other live parent may be a concurrent Natesclaw instance (prod + dev).
        return .keep
    }
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "natesclaw", "tsx", "pnpm", "bun"]
        ...
        managedGatewayPID: Int32? = nil) -> Bool
    {
        ...
        case .local:
            ...
            if full.contains("gateway-daemon") || full.contains("natesclaw-gateway")
                || cmd.contains("natesclaw-gateway")
            {
                return true
            }
            if self.isNodeNatesclawGatewayCommand(full) { return true }
            // If args are unavailable, treat a CLI listener as expected.
            if cmd.contains("natesclaw"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
    ...
    private static func isNodeNatesclawGatewayCommand(_ fullCommand: String) -> Bool {
        ...
        return self.isNatesclawDistEntrypointToken(tokens[1])
            && tokens[2].lowercased() == "gateway"
    }

    private static func isNatesclawDistEntrypointToken(_ token: String) -> Bool {
    ...
    private nonisolated static func openRecordStore() throws -> PortGuardianRecordStore {
        guard !self.hasLegacyNatesclawAppProcess() else {
            throw PortGuardianStoreError(
                "Quit older Natesclaw app copies before opening the SQLite PortGuardian ledger")
        }
    ...
    private nonisolated static func requirePostSpawnCompatibility() throws {
        guard !self.hasLegacyNatesclawAppProcess(),
              !FileManager.default.fileExists(atPath: PortGuardianRecordStore.liveLegacyRecordURL.path)
        else {
            throw PortGuardianStoreError(
                "Older Natesclaw storage appeared after tunnel preflight; SSH launch cancelled")
        }
    }
    ...
    private nonisolated static func hasLegacyNatesclawAppProcess() -> Bool {
    ...
    private nonisolated static func runningPortGuardianStorageVersion(pid: pid_t) -> Int? {
        ...
        guard SecCodeCopyStaticCode(code, SecCSFlags(), &staticCode) == errSecSuccess,
              ...
              let version = securedInfo["NatesclawPortGuardianStorageVersion"] as? NSNumber
        else { return nil }
        ...
        storageVersion: Int?) -> Bool
    {
        guard let bundleIdentifier,
              bundleIdentifier == "ai.natesclaw.mac" || bundleIdentifier.hasPrefix("ai.natesclaw.mac.")
        else { return false }

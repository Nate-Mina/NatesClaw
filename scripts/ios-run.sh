...
APP_NAME="${IOS_APP_NAME:-Natesclaw}"
...
if [[ "${push_sandbox_simulator}" == "1" ]]; then
  ...
  xcodebuild_overrides+=(
    "NATESCLAW_PUSH_MODE=simulatorSandbox"
    "NATESCLAW_PUSH_RELAY_BASE_URL=${push_relay_base_url}"
    "NATESCLAW_APNS_ENTITLEMENT_ENVIRONMENT=development"
  )
fi

unset "${simulator_proof_secret_env}"
if [[ "${simulator_proof_secret_env}" != "NATESCLAW_SIMULATOR_PUSH_PROOF_SECRET" ]]; then
  unset NATESCLAW_SIMULATOR_PUSH_PROOF_SECRET
fi
...
if [[ "${push_sandbox_simulator}" == "1" ]]; then
  # shellcheck disable=SC2086
  SIMCTL_CHILD_NATESCLAW_SIMULATOR_PUSH_PROOF_SECRET="${simulator_proof_secret}" \

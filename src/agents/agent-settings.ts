...
export function applyAgentCompactionSettingsFromConfig(params: {
  ...
  // Omission preserves embedded/project settings. Natesclaw config reloads create a new
...
export function resolveEffectiveCompactionMode(cfg?: NatesclawConfig): AgentCompactionMode {
 ...
 * fires on a successful turn and triggers Natesclaw runtime's `_runAutoCompaction` from
 * inside `Session.prompt()`, collapsing `agent.state.messages` before the
 * provider call (natesclaw#75799).
 *
...
/**
 * Disable Natesclaw runtime's `_checkCompaction → _runAutoCompaction` (which would otherwise
 * fire from inside `Session.prompt()` and reassign `agent.state.messages`
 * before the provider call) when Natesclaw or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (natesclaw#75799).
 * Default-mode runs against ordinary providers keep Natesclaw runtime's auto-compaction as
 ...
 * rehydrates `compaction.enabled` from disk and silently restores Natesclaw runtime's

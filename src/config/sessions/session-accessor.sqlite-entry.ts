...
export function resolveSessionEntry(
  ...
  const read = (
    database: Pick<NatesclawAgentDatabase, "agentId" | "db" | "path">,
  ): ResolvedSqliteSessionEntry => {
  ...
  if (options.readOnly) {
    const result = withNatesclawAgentDatabaseReadOnly(read, toDatabaseOptions(resolved));
    return result.found
      ? result.value
      : { existing: undefined, legacyKeys: [], normalizedKey: resolved.sessionKey };
  }
  return read(openNatesclawAgentDatabase(toDatabaseOptions(resolved)));
}
...
function readSessionEntrySnapshot(
  ...
  const cache = !isIncognitoNatesclawAgentSqlitePath(database.path, {
...
export function listSessionTranscriptInstances(
  ...
  return listTranscriptInstancesFromDatabase({
    ...
    currentEntries,
    database,
    databasePath: resolveNatesclawAgentSqlitePath(toDatabaseOptions(resolved)),
  });
}
...
type SqliteSessionEntrySnapshotPatchParams<TSnapshot> = {
  ...
  options: SqliteSessionEntryPatchOptions;
  readSnapshot: (database: NatesclawAgentDatabase) => TSnapshot;

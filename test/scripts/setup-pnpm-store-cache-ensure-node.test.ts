...
if [[ -n "\${NATESCLAW_FAKE_CURL_FAIL_SUFFIX:-}" && "$url" == *"\${NATESCLAW_FAKE_CURL_FAIL_SUFFIX:-}" ]]; then
  if [[ -n "$output" ]]; then
  ...
  const result = spawnSync(
    ...
    [
      ...
      [
        ...
        `natesclaw_ensure_node "${requested}"`,
        "command -v node",
...
function runVersionMatch(actual: string, requested: string) {
  return spawnSync(
    ...
    [
      ...
      [
        `source "${ensureNodeScript}"`,
        `natesclaw_node_version_matches "${actual}" "${requested}"`,
      ].join("; "),
    ],
...
describe("setup-pnpm-store-cache ensure-node", () => {
  beforeAll(() => {
    const root = mkdtempSync(join(tmpdir(), "natesclaw-ensure-node-"));
    try {
      const result = spawnSync(
        "bash",
        [
          ...
          [
            "set -euo pipefail",
            `source "${ensureNodeScript}"`,
            `natesclaw_find_toolcache_node "99.99.99"`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
          env: {
            ...
            NATESCLAW_CONTAINER_TOOL_CACHE: join(root, "missing-container-cache"),
          },
        },
      );
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            `source "${ensureNodeScript}"`,
            `natesclaw_prepend_node_bin "C:\\\\hostedtoolcache\\\\windows/node/24.15.0/x64"`,
            "command -v node",
            "node -p 'process.versions.node'",
            `cat "${githubPath}"`,
          ].join("; "),
  ...
  it("repairs PATH from the container-mounted GitHub Actions toolcache", () => {
    ...
    try {
      ...
      const result = runEnsureNode(root, "24.99.99", {
        PATH: `${activeBin}:${process.env.PATH ?? ""}`,
        NATESCLAW_CONTAINER_TOOL_CACHE: join(root, "__t"),
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            `export NATESCLAW_FAKE_CURL_LOG=${JSON.stringify(curlLog)}`,
            `source "${ensureNodeScript}"`,
            'natesclaw_resolve_node_download_version "24.x"',
            "natesclaw_prepend_node_bin() { :; }",
            'natesclaw_node_download_platform() { printf "win-x64\\n"; }',
            "pwsh() { :; }",
            `RUNNER_TEMP=${JSON.stringify(root)} natesclaw_download_node "24.15.0"`,
            'natesclaw_node_download_platform() { printf "linux-x64\\n"; }',
            "tar() { :; }",
            `RUNNER_TEMP=${JSON.stringify(root)} natesclaw_download_node "24.15.0"`,
          ].join("\n"),
        ],
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -uo pipefail",
            `export PATH=${JSON.stringify(`${helperBin}:${process.env.PATH ?? ""}`)}`,
            `export NATESCLAW_FAKE_CURL_LOG=${JSON.stringify(join(root, "curl.log"))}`,
            `export NATESCLAW_FAKE_CURL_FAIL_SUFFIX=${JSON.stringify(".tar.xz")}`,
            `source "${ensureNodeScript}"`,
            "natesclaw_prepend_node_bin() { :; }",
            'natesclaw_node_download_platform() { printf "linux-x64\\n"; }',
            `RUNNER_TEMP=${JSON.stringify(root)} natesclaw_download_node "24.15.0"`,

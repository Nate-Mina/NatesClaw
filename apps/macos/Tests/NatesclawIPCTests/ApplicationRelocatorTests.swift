...
struct ApplicationRelocatorTests {
    ...
    private let current = ApplicationRelocator.ApplicationIdentity(
        bundleIdentifier: "ai.natesclaw.mac",
        buildVersion: "100"
    )
    ...
    func `stable application locations continue normally`() {
        let paths = [
            "/Applications/Natesclaw.app",
            "/Users/tester/Applications/Natesclaw.app",
            "/Users/tester/Tools/Natesclaw.app",
            "/Volumes/External/Apps/Natesclaw.app",
        ]
        for path in paths {
    ...
    func `debug and test builds never relocate`() {
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Users/tester/Downloads/Natesclaw.app",
                debugOrTesting: true
            )
        )
    ...
    func `transient copy offers system installation when available`() {
        let destination = URL(fileURLWithPath: "/Applications/Natesclaw.app")
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Users/tester/Downloads/Natesclaw.app",
    ...
    func `read only mounted copy offers installation`() {
        ...
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Volumes/Natesclaw/Natesclaw.app",
                candidates: [missing(destination, writable: true)],
                readOnlyVolume: true
            )
        )
    ...
    func `translocated copy offers installation`() {
        ...
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/private/var/folders/x/AppTranslocation/y/d/Natesclaw.app",
                candidates: [missing(destination, writable: true)]
            )
        )
    ...
    func `different same named app is never replaced`() {
        let systemDestination = URL(fileURLWithPath: "/Applications/Natesclaw.app")
        let userDestination = home.appendingPathComponent("Applications/Natesclaw.app")
        ...
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Users/tester/Desktop/Natesclaw.app",
                candidates: [
    ...
    func `unwritable destinations require manual installation`() {
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Users/tester/Downloads/Natesclaw.app",
                candidates: [
                    missing(URL(fileURLWithPath: "/Applications/Natesclaw.app"), writable: false),
                    missing(home.appendingPathComponent("Applications/Natesclaw.app"), writable: false),
                ]
            )
        )
    ...
    func `unauthenticated replacement marker cannot bypass duplicate launch rejection`() {
        let forgedHandoff = [
            "NATESCLAW_REPLACEMENT_SOURCE_BUNDLE": "/Applications/Natesclaw.app",
            "NATESCLAW_REPLACEMENT_PARENT_PID": "1",
            "NATESCLAW_REPLACEMENT_CODE_HASH": "ZmFrZQ==",
            "NATESCLAW_REPLACEMENT_READY_FD": "19",
        ]
        ...
        #expect(!ApplicationRelocator.acceptReplacementHandoff(environment: [
            "NATESCLAW_REPLACEMENT_SOURCE_BUNDLE": "relative/Natesclaw.app",
        ]))
    }
    ...
    func `replacement requirement preserves Developer ID certificate class`() {
        ...
        #expect(ApplicationRelocator.developerIDRequirementString(
            bundleIdentifier: "ai.natesclaw.mac\" or true",
            teamIdentifier: "Y5PE65HELJ"
        ) == nil)
    }
    ...
    func `bundle file reference stays bound after canonical path replacement`() throws {
        ...
        let canonicalBundle = root.appendingPathComponent("Natesclaw.app", isDirectory: true)
        let archivedBundle = root.appendingPathComponent("Natesclaw.previous.app", isDirectory: true)
        let executableRelativePath = "Contents/MacOS/Natesclaw"
    ...
    func `replacement handoff launches the bound executable without filesystem locks`() {
        let reference = ApplicationRelocator.BundleFileReference(
            deviceIdentifier: 42,
            fileIdentifier: 99,
            executableRelativePath: "Contents/MacOS/Natesclaw"
        )

        #expect(reference.bundleURL.path == "/.vol/42/99")
        #expect(reference.executableURL.path == "/.vol/42/99/Contents/MacOS/Natesclaw")
    }
    ...
    func `verified KeepAlive launchd supervisor retains relaunch ownership`() throws {
        ...
        let executable = root.appendingPathComponent("Natesclaw.app/Contents/MacOS/Natesclaw")
        let serviceName = "ai.natesclaw.mac.test-node"
        ...
        #expect(ApplicationRelocator.relaunchStrategy(
            xpcServiceName: "application.ai.natesclaw.mac.123",
            executableURL: executable,
            homeDirectory: home
        ) == .openAfterTermination)
        #expect(ApplicationRelocator.relaunchStrategy(
    ...
    func `replacement metadata is read from disk instead of Bundle cache`() throws {
        ...
        let bundleURL = root.appendingPathComponent("Natesclaw.app")
        ...
        #expect(ApplicationRelocator.applicationOnDisk(at: bundleURL) == .init(
            bundleIdentifier: "ai.natesclaw.mac",
            executableURL: contentsURL.appendingPathComponent("MacOS/Natesclaw")
        ))

        try writeInfoPlist(
            at: contentsURL.appendingPathComponent("Info.plist"),
            bundleIdentifier: "ai.natesclaw.mac.replaced",
            executable: "NatesclawNext"
        )
        #expect(ApplicationRelocator.applicationOnDisk(at: bundleURL) == .init(
            bundleIdentifier: "ai.natesclaw.mac.replaced",
            executableURL: contentsURL.appendingPathComponent("MacOS/NatesclawNext")
        ))
    }

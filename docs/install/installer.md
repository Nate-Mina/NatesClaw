...
| ---------------------------------- | -------------------- | ---------------------------------------------------------------------------------------------- |
| [`install.sh`](#installsh)         | macOS / Linux / WSL  | Installs Node if needed, installs Natesclaw via npm (default) or git, can run onboarding.       |
| [`install-cli.sh`](#install-clish) | macOS / Linux / WSL  | Installs Node + Natesclaw into a local prefix (`~/.natesclaw`) via npm or git. No root required. |
| [`install.ps1`](#installps1)       | Windows (PowerShell) | Installs Node if needed, installs Natesclaw via npm (default) or git, can run onboarding.       |
...
<Note>
If install succeeds but `natesclaw` is not found in a new terminal, see [Node.js troubleshooting](/install/node#troubleshooting).
</Note>
...
<Steps>
  ...
  <Step title="Install Natesclaw">
    ...
    - `git` method: clone/update repo, install deps with pnpm, build, then install wrapper at `~/.local/bin/natesclaw`

  </Step>
  <Step title="Post-install tasks">
    - Resolves the just-installed `natesclaw` binary for follow-up commands
...
If run inside an Natesclaw checkout (`package.json` + `pnpm-workspace.yaml`), the script offers:
...
| ------------------------------------------------- | ------------------------------------------------------------------ |
| `NATESCLAW_INSTALL_METHOD=git\|npm`                | Install method                                                     |
| `NATESCLAW_VERSION=latest\|next\|<semver>\|<spec>` | npm version, dist-tag, or package spec                             |
| `NATESCLAW_BETA=0\|1`                              | Use beta if available                                              |
| `NATESCLAW_HOME=<path>`                            | Base directory for Natesclaw state and default git/onboarding paths |
| `NATESCLAW_GIT_DIR=<path>`                         | Checkout directory                                                 |
| `NATESCLAW_GIT_UPDATE=0\|1`                        | Toggle git updates                                                 |
| `NATESCLAW_NO_PROMPT=1`                            | Disable prompts                                                    |
| `NATESCLAW_VERIFY_INSTALL=1`                       | Run the post-install smoke verify                                  |
| `NATESCLAW_NO_ONBOARD=1`                           | Skip onboarding                                                    |
| `NATESCLAW_DRY_RUN=1`                              | Dry run mode                                                       |
| `NATESCLAW_VERBOSE=1`                              | Debug mode                                                         |
| `NATESCLAW_NPM_LOGLEVEL=error\|warn\|notice`       | npm log level (default: `error`, hides npm deprecation noise)      |
...
(default `~/.natesclaw`) and no system Node dependency. Supports npm installs
...
<Steps>
  ...
  <Step title="Install Natesclaw under prefix">
    - `npm` method (default): installs under the prefix with npm, then writes wrapper to `<prefix>/bin/natesclaw`
    - `git` method: clones/updates a checkout (default `~/natesclaw`) and still writes the wrapper to `<prefix>/bin/natesclaw`

  </Step>
  <Step title="Refresh loaded gateway service">
    If a gateway service is already loaded from that same prefix, the script runs
    `natesclaw gateway install --force`, which activates the replacement service,
...
| Flag                                    | Description                                                                     |
| --------------------------------------- | ------------------------------------------------------------------------------- |
| `--prefix <path>`                       | Install prefix (default: `~/.natesclaw`)                                         |
| `--install-method \| --method npm\|git` | Choose install method (default: `npm`)                                          |
| `--npm`                                 | Shortcut for npm method                                                         |
| `--git \| --github`                     | Shortcut for git method                                                         |
| `--git-dir \| --dir <path>`             | Git checkout directory (default: `~/natesclaw`)                                  |
| `--version <ver>`                       | Natesclaw version or dist-tag (default: `latest`)                                |
...
| Variable                                    | Description                                                        |
| ------------------------------------------- | ------------------------------------------------------------------ |
| `NATESCLAW_PREFIX=<path>`                    | Install prefix                                                     |
| `NATESCLAW_INSTALL_METHOD=git\|npm`          | Install method                                                     |
| `NATESCLAW_VERSION=<ver>`                    | Natesclaw version or dist-tag                                       |
| `NATESCLAW_NODE_VERSION=<ver>`               | Node version                                                       |
| `NATESCLAW_HOME=<path>`                      | Base directory for Natesclaw state and default git/onboarding paths |
| `NATESCLAW_GIT_DIR=<path>`                   | Git checkout directory for git installs                            |
| `NATESCLAW_GIT_UPDATE=0\|1`                  | Toggle git updates for existing checkouts                          |
| `NATESCLAW_NO_ONBOARD=1`                     | Skip onboarding                                                    |
| `NATESCLAW_NPM_LOGLEVEL=error\|warn\|notice` | npm log level (default: `error`)                                   |

  </Accordion>
</AccordionGroup>

<Note>
`natesclaw@main` and other GitHub source specs are not valid `--version` targets for npm installs. Use `--install-method git --version main` instead.
</Note>
...
<Steps>
  ...
  <Step title="Ensure a supported Node.js runtime">
    If missing, attempts install via winget, then Chocolatey, then Scoop. If no package manager is available, the script downloads the official Node.js 26 Windows zip into `%LOCALAPPDATA%\Natesclaw\deps\portable-node` and adds it to the current process and user PATH. Node 22.22.3+, Node 24.15+, and Node 25.9+ are supported; Node 23 is unsupported.
  </Step>
  <Step title="Install Natesclaw">
    ...
    - `git` method: clone/update repo, install/build with pnpm, and install wrapper at `%USERPROFILE%\.local\bin\natesclaw.cmd`. If Git is missing, the script bootstraps user-local MinGit under `%LOCALAPPDATA%\Natesclaw\deps\portable-git` and adds it to the current process and user PATH.

  </Step>
  <Step title="Post-install tasks">
    - Adds needed bin directory to user PATH when possible
    - Refreshes a loaded gateway service best-effort (`natesclaw gateway install --force`, then restart)
    - Runs `natesclaw doctor --non-interactive` on upgrades and git installs (best effort)
...
| `NATESCLAW_GIT_DIR=<path>`          | Checkout directory |
| `NATESCLAW_NO_ONBOARD=1`            | Skip onboarding    |
| `NATESCLAW_GIT_UPDATE=0`            | Disable git pull   |
| `NATESCLAW_DRY_RUN=1`               | Dry run mode       |
...
<AccordionGroup>
  ...
  <Accordion title='Windows: "natesclaw is not recognized"'>
  ...
  <Accordion title="natesclaw not found after install">

...
The private QA stack exercises Natesclaw in a realistic, channel-shaped way that
a unit test cannot.
...
Every QA flow runs under `pnpm natesclaw qa <subcommand>`. Many have `pnpm qa:*`
...
| `qa parity-report`                                  | Compare two `qa-suite-summary.json` files for a model-axis parity gate, or use `--runtime-axis --token-efficiency` to write Codex-vs-Natesclaw runtime parity and token-efficiency reports.                                                                          |
...
command also needs an Natesclaw root profile, put the root profile before the
QA command:

```bash
pnpm natesclaw --profile work qa run --qa-profile smoke-ci
```
...
```bash
pnpm natesclaw qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`natesclaw.run`, `natesclaw.harness.run`, a latest GenAI semantic-convention
model-call span, `natesclaw.context.assembled`, and `natesclaw.message.delivery`
...
IDs and `natesclaw.content.*` attributes must stay out of the trace. The scenario
...
```bash
pnpm natesclaw qa matrix --provider-mode mock-openai
```
...
```bash
NATESCLAW_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
  pnpm natesclaw qa matrix --provider-mode live-frontier
```

Plain `pnpm natesclaw qa matrix` runs every flow scenario that explicitly
...
`NATESCLAW_QA_MATRIX_TUWUNEL_IMAGE`; tune negative no-reply assertions with
`NATESCLAW_QA_MATRIX_NO_REPLY_WINDOW_MS` (default `8000`, clamped to the active
...
`NATESCLAW_QA_MATRIX_DISABLE_FORCE_EXIT=1` only for a direct test harness that
...
E2EE CLI scenarios also drive `natesclaw matrix encryption setup` and
...
pnpm natesclaw qa discord
pnpm natesclaw qa slack
pnpm natesclaw qa telegram
pnpm natesclaw qa whatsapp
```
...
```bash
pnpm natesclaw qa mantis slack-desktop-smoke \
  --gateway-setup \
...
Natesclaw Slack gateway running inside the VM on port `38973`; without it, the
...
```bash
pnpm natesclaw qa mantis visual-task \
...
`visual-driver`, captures `visual-task.png`, runs `natesclaw infer image
...
The doctor checks Convex broker env (`NATESCLAW_QA_CONVEX_SITE_URL`,
`NATESCLAW_QA_CONVEX_ENDPOINT_PREFIX`), validates endpoint settings, reports
only set/missing status for `NATESCLAW_QA_CONVEX_SECRET_CI` and
`NATESCLAW_QA_CONVEX_SECRET_MAINTAINER`, and verifies admin/list reachability
...
```bash
pnpm natesclaw qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds Natesclaw
...
Natesclaw Gateway and its outbound events are observed from the relay. The
...
- `NATESCLAW_QA_TELEGRAM_GROUP_ID` - numeric chat id (string).
...
bypassing its constraints. Use `pnpm natesclaw qa telegram --list-scenarios
...
```bash
NATESCLAW_QA_CREDENTIAL_SOURCE=convex \
pnpm test:docker:npm-telegram-live
```

When `NATESCLAW_QA_CREDENTIAL_SOURCE=convex` is set, the package live wrapper
...
`NATESCLAW_NPM_TELEGRAM_RTT_SAMPLES`, `NATESCLAW_NPM_TELEGRAM_RTT_TIMEOUT_MS`,
or `NATESCLAW_NPM_TELEGRAM_RTT_MAX_FAILURES` to tune RTT measurement without
...
controlled by the harness and a SUT bot started by the child Natesclaw gateway
...
- `NATESCLAW_QA_DISCORD_GUILD_ID`
- `NATESCLAW_QA_DISCORD_CHANNEL_ID`
- `NATESCLAW_QA_DISCORD_DRIVER_BOT_TOKEN`
- `NATESCLAW_QA_DISCORD_SUT_BOT_TOKEN`
- `NATESCLAW_QA_DISCORD_SUT_APPLICATION_ID` - must match the SUT bot user id
  returned by Discord (the lane fails fast otherwise).

Optional:

- `NATESCLAW_QA_DISCORD_VOICE_CHANNEL_ID` selects the voice/stage channel for
...
- `NATESCLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR` enables visual approval
...
- `NATESCLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS` overrides the checkpoint
...
- `approval-checkpoints/` - only when Mantis sets
  `NATESCLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR`; contains checkpoint JSON,
...
{
  "display_information": {
    "name": "Natesclaw QA Driver",
    "description": "Test driver bot for Natesclaw QA Slack live lane"
  },
  "features": {
    "bot_user": {
      "display_name": "Natesclaw QA Driver",
      "always_online": true
    }
  },
  "oauth_config": {
...
{
  "display_information": {
    "name": "Natesclaw QA SUT",
    "description": "Natesclaw QA SUT connector for Natesclaw"
  },
  "features": {
    "bot_user": {
      "display_name": "Natesclaw QA SUT",
      "always_online": true
    },
    "app_home": {
...
In the QA workspace, create a channel (e.g. `#natesclaw-qa`) and invite both
bots from inside the channel:

```text
/invite @Natesclaw QA Driver
/invite @Natesclaw QA SUT
```
...
`NATESCLAW_QA_SLACK_*` variables and pass `--credential-source env`), or seed
...
With `NATESCLAW_QA_CONVEX_SITE_URL` and `NATESCLAW_QA_CONVEX_SECRET_MAINTAINER`
exported in your shell, register and verify:

```bash
pnpm natesclaw qa credentials add \
  --kind slack \
  --payload-file slack-creds.json \
  --note "QA Slack pool seed"

pnpm natesclaw qa credentials list --kind slack --status all --json
```
...
the harness and a SUT account started by the child Natesclaw gateway through
the bundled WhatsApp plugin.

Required env when `--credential-source env`:

- `NATESCLAW_QA_WHATSAPP_DRIVER_PHONE_E164`
- `NATESCLAW_QA_WHATSAPP_SUT_PHONE_E164`
- `NATESCLAW_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64`
- `NATESCLAW_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64`

Optional:

- `NATESCLAW_QA_WHATSAPP_GROUP_JID` enables group scenarios such as
...
`@natesclaw/whatsapp/api.js` package surface instead of reaching into private
...
`--credential-source convex` (or set `NATESCLAW_QA_CREDENTIAL_SOURCE=convex`);
...
- `mock-openai` is the scenario-aware Natesclaw mock. It remains the default
...
runs Natesclaw's normal channel plugins against them. `live` is reserved for
...
- how `natesclaw qa <runner>` is mounted beneath the shared `qa` root
...
3. Keep transport-specific mechanics inside the runner plugin or channel
   harness.
4. Mount the runner as `natesclaw qa <runner>` instead of registering a
   competing root command. Runner plugins should declare `qaRunners` in
   `natesclaw.plugin.json` and export a matching `qaRunnerCliRegistrations`
...
or wiring a new transport - run `pnpm natesclaw qa coverage` (add `--json`
for machine-readable output). When choosing focused proof for a touched
behavior or file path, run `pnpm natesclaw qa coverage --match <query>`. The
...
```bash
pnpm natesclaw qa character-eval \

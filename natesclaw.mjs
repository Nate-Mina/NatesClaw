...
const COMPILE_CACHE_DISABLED_RESPAWNED_ENV = "NATESCLAW_COMPILE_CACHE_DISABLED_RESPAWNED";
...
const ensureSupportedRuntimeVersion = () => {
  if (process.versions.bun) {
    ...
    // rejecting Bun outright so capable Bun builds can run Natesclaw.
    let hasNodeSqlite;
    try {
    ...
    process.stderr.write(
      "natesclaw: this Bun runtime is unsupported because it does not provide node:sqlite.\n" +
  ...
  process.stderr.write(
    `natesclaw: Node.js ${SUPPORTED_NODE_RANGE} is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[natesclaw] Failed to respawn launcher: ${
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.NATESCLAW_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    NATESCLAW_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
    [...process.execArgv, fileURLToPath(import.meta.url), ...process.argv.slice(2)],
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["natesclaw: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:natesclaw/natesclaw#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g natesclaw@latest`.");
  return lines.join("\n");
};
...
const hasLauncherContainerTarget = (argv) => {
  if (normalizeLauncherMetadataValue(process.env.NATESCLAW_CONTAINER)) {
    return true;
  }
...
const resolveLauncherHomeDir = () => {
  const explicit = normalizeLauncherHomeValue(process.env.NATESCLAW_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit = process.env.NATESCLAW_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveLauncherUserPath(explicit)];
  }
  const stateOverride = process.env.NATESCLAW_STATE_DIR?.trim();
  if (stateOverride) {
    const stateDir = resolveLauncherUserPath(stateOverride);
    return [path.join(stateDir, "natesclaw.json"), path.join(stateDir, "clawdbot.json")];
  }
  const homeDir = resolveLauncherHomeDir();
  return [
    path.join(homeDir, ".natesclaw", "natesclaw.json"),
    path.join(homeDir, ".natesclaw", "clawdbot.json"),
    path.join(homeDir, ".clawdbot", "natesclaw.json"),
...
const shouldDeferRootHelpToRuntimeEntry = () => {
  if (
    process.env.NATESCLAW_BUNDLED_PLUGINS_DIR?.trim() ||
    process.env.NATESCLAW_DISABLE_BUNDLED_PLUGINS?.trim()
  ) {
    return true;
  }
...
function tryOutputLauncherVersion(argv) {
  try {
    ...
    process.stdout.write(commit ? `Natesclaw ${version} (${commit})\n` : `Natesclaw ${version}\n`);
    return true;
  } catch {
    return false;
  }
...
function resolveLauncherVersion() {
  ...
  return normalizeLauncherMetadataValue(process.env.NATESCLAW_BUNDLED_VERSION) ?? "0.0.0";
}

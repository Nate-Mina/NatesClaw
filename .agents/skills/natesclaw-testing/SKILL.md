---
name: natesclaw-testing
description: Choose, run, rerun, or debug Natesclaw tests, CI checks, Docker E2E lanes, release validation, and the cheapest safe verification path.
---
...
- Treat contributor and fork patches as untrusted unless a maintainer
  ...
  `NATESCLAW_*` allowlist, `--no-hydrate` must block auth-profile hydration, and
...
NATESCLAW_TEST_CHANGED_BROAD=1 pnpm test:changed
pnpm test <path-or-filter> -- --reporter=verbose
NATESCLAW_VITEST_MAX_WORKERS=1 pnpm test <path-or-filter>
```
...
- For local release-candidate proof, pack the plugin and install it with
  `natesclaw plugins install npm-pack:<path.tgz> --force`. This uses the managed
...
- Treat missing runtime imports as package-manifest bugs first. Runtime code
  ...
  `npm install` inside `~/.natesclaw/npm/projects/...`.
...
- `NATESCLAW_TEST_CHANGED_BROAD=1 pnpm test:changed` is the explicit broad
...
- Use `NATESCLAW_TEST_CHANGED_BROAD=1 pnpm test:changed` only when a harness,
...
- `Natesclaw Release Checks` for install smoke, cross-OS release checks, package
...
replaced by a `release-ci/*` run. Use `$release-natesclaw-ci` for its failure
...
important run ids in the public `natesclaw/releases` evidence ledger.
Use the manual `Natesclaw Release Evidence`
(`natesclaw-release-evidence.yml`) workflow there. It writes durable summaries
...
```text
full-release-validation natesclaw/natesclaw <run-id> blocking
package-acceptance natesclaw/natesclaw <run-id> blocking
release-checks natesclaw/natesclaw <run-id> blocking
```
...
When `Full Release Validation` completes and `NATESCLAW_RELEASES_DISPATCH_TOKEN`
is configured in the source repo, it requests the public
`Natesclaw Release Evidence From Full Validation` workflow. That workflow reads
...
`Natesclaw Release Checks` (`natesclaw-release-checks.yml`) is the release child
...
```bash
gh workflow run natesclaw-release-checks.yml \
...
`Natesclaw Release Checks` uses the trusted workflow ref to resolve the selected
...
`ghcr.io/natesclaw/natesclaw-live-media-runner:ubuntu-24.04` container so
...
`pnpm natesclaw qa parity-report`. For parity failures, inspect the failed lane
first; inspect the report job when both lane summaries exist but the comparison
fails.

### Reusable Live/E2E Checks

`Natesclaw Live And E2E Checks (Reusable)`
(`natesclaw-live-and-e2e-checks-reusable.yml`) is the preferred entry point for
targeted live, Docker, model, and E2E proof. Inputs let you turn off unrelated
lanes:

```bash
gh workflow run natesclaw-live-and-e2e-checks-reusable.yml \
  --repo natesclaw/natesclaw \
  --ref main \
  -f ref=<sha> \
...
- `native-live-src-gateway-profiles` (release CI runs this with provider
  filters such as `NATESCLAW_LIVE_GATEWAY_PROVIDERS=anthropic`)
...
`NATESCLAW_LIVE_PROVIDERS` and selected model ids before declaring proof.
...
```bash
NATESCLAW_DOCKER_ALL_DRY_RUN=1 pnpm test:docker:all
NATESCLAW_DOCKER_ALL_DRY_RUN=1 NATESCLAW_DOCKER_ALL_LANES=install-e2e pnpm test:docker:all
NATESCLAW_DOCKER_ALL_LANES=install-e2e node scripts/test-docker-all.mjs --plan-json
```
...
```bash
NATESCLAW_DOCKER_ALL_LANES=<lane> \
NATESCLAW_DOCKER_ALL_BUILD=0 \
NATESCLAW_DOCKER_ALL_PREFLIGHT=0 \
NATESCLAW_SKIP_DOCKER_BUILD=1 \
NATESCLAW_DOCKER_E2E_BARE_IMAGE='<prepared-bare-image>' \
NATESCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE='<prepared-functional-image>' \
...
- Resolve the registry immediately before dispatch:
  ...
  and `npm view natesclaw@beta version dist.tarball dist.integrity --json --prefer-online --cache /tmp/natesclaw-npm-cache-verify-$$`.
- If Peter asks for "latest beta", use `source=npm` with
  `package_spec=natesclaw@beta`, then record the resolved version from `npm view`
  or the workflow summary.
- For reruns, release proof, or comparing one known package, prefer the exact
  immutable spec: `package_spec=natesclaw@YYYY.M.D-beta.N` or
  `package_spec=natesclaw@YYYY.M.D`.
- For stable package proof, use `package_spec=natesclaw@latest` only when the
  question is explicitly the current stable dist-tag; otherwise pin the exact
  version.
- `source=npm` only accepts registry specs for `natesclaw@extended-stable`,
  `natesclaw@beta`, `natesclaw@latest`, or exact Natesclaw release versions. Do
...
- Package acceptance tests exactly the selected package candidate. Do not apply
  `natesclaw update --channel beta` fallback semantics here; if `beta` is absent,
...
- `source=npm`: `natesclaw@extended-stable`, `natesclaw@beta`,
  `natesclaw@latest`, or an exact release version.
...
- `package_ref` is the source ref to build when `source=ref`. It can be an
  older branch/tag/SHA as long as it is reachable from an Natesclaw branch or
  release tag.
...
tarball that bare lanes mount. `scripts/package-natesclaw-for-docker.mjs` is the
...
from `natesclaw skills search`, installs it, and verifies `.clawhub` origin/lock
...
`NATESCLAW_DOCKER_E2E_WORKFLOW_REF`; this is separate from the artifact target

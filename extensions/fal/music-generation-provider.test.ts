...
import { expectExplicitMusicGenerationCapabilities } from "natesclaw/plugin-sdk/provider-test-contracts";
...
vi.mock("natesclaw/plugin-sdk/provider-http", async (importOriginal) => {
  const original = await importOriginal<typeof import("natesclaw/plugin-sdk/provider-http")>();
  return {
    ...original,

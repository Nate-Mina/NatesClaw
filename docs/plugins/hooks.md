...
read_when:
  ...
  - You are projecting Natesclaw cron wakes into an external host scheduler
---

Plugin hooks are in-process extension points for Natesclaw plugins: inspect or
...
| Option             | Effect                                                                                                                                                                                                                                                 |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `matcher`          | Non-empty list of canonical Natesclaw tool ids handled by `before_tool_call` or `after_tool_call`, such as `exec`, `apply_patch`, or `spawn_agent`. Omit to match all tools. Empty lists, wildcards, blanks, and provider-specific aliases are invalid. |
...
| `timeoutMs`        | Per-hook await budget. When it expires, Natesclaw stops awaiting that handler and moves on. It does not cancel the handler or its side effects. Omit to use the runner's default per-hook timeout.                                                      |
...
15-second default per handler. If one times out, Natesclaw logs the plugin error
...
plugin that registered that handler. Natesclaw injects it per handler without
...
`inbound_claim` is not a global pre-routing broadcast. Natesclaw invokes it only
...
If the budget expires, Natesclaw logs `session-end-drain timed out` and continues
...
- `subagent_spawned` includes `resolvedModel` and `resolvedProvider` when Natesclaw has resolved the child session's native model before launch.
...
benchmarks, model-based graders, or other third-party evaluators. Natesclaw
...
revise with that hash and a correlation id, then repeat. Natesclaw does not
...
use `natesclaw sessions` or the Gateway session/status surfaces. To debug
...
```json5
{
  agents: {
    entries: {
      "maintenance-agent": {
        default: true,
        workspace: "~/.natesclaw/workspace-maintenance",
      },
    },
  },
  bindings: [
  ...
  plugins: {
    load: { paths: ["~/.natesclaw/policies/maintenance-access.ts"] },
  },
}
```
...
- Natesclaw strips `toolResult.details` before provider replay and compaction
...
When a run is blocked, Natesclaw stores only the replacement text in
...
Agent-turn hooks including `agent_end` include `event.runId` when Natesclaw can
...
the conversation target identifier when Natesclaw can derive one from the
...
```ts
declare module "natesclaw/plugin-sdk/channel-inbound" {
...
pending forever. A timeout is logged and Natesclaw continues; it does not
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when Natesclaw can derive a
...
Handlers have a 15s default budget; on timeout, Natesclaw logs the failure and
continues with the original final answer.
Codex native `Stop` hooks are relayed into this hook as Natesclaw
...
behavior). Natesclaw drains queued injections before prompt hooks, drops
...
policy runs from Natesclaw config, covers CLI install and update paths, and
fails closed when enabled but unavailable.

`before_install` is a plugin-runtime lifecycle hook. It runs after
`security.installPolicy` only in the Natesclaw process where plugin hooks have
...
Natesclaw no longer runs built-in install-time dangerous-code blocking, so it
...
export function registerCronProjection(api: NatesclawPluginApi, host: ExternalWakeHost) {
...
non-retryable configuration before registration. Natesclaw does not provide an

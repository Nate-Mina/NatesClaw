...
- Flags are case-insensitive strings, resolved from `diagnostics.flags` in
  config plus the `NATESCLAW_DIAGNOSTICS` env override, deduped and lowercased.
...
```bash
NATESCLAW_DIAGNOSTICS=telegram.http,brave.http
```
...
`NATESCLAW_DIAGNOSTICS=0` disables flags from both env and config for that
...
```bash
NATESCLAW_DIAGNOSTICS=profiler natesclaw gateway run
```

Enable only reply-dispatch profiler spans:

```bash
NATESCLAW_DIAGNOSTICS=reply.profiler natesclaw gateway run
```

Enable only Codex app-server startup/tool/thread profiler spans:

```bash
NATESCLAW_DIAGNOSTICS=codex.profiler natesclaw gateway run
```
...
`NATESCLAW_DIAGNOSTICS=0` to override every diagnostics flag for that run.
...
NATESCLAW_DIAGNOSTICS_TIMELINE_PATH=/tmp/natesclaw-timeline.jsonl \
...
The output path always comes from `NATESCLAW_DIAGNOSTICS_TIMELINE_PATH`, even
...
are missing because Natesclaw has not read config yet; subsequent startup spans
are captured normally.

`NATESCLAW_DIAGNOSTICS=1`, `=all`, and `=*` also enable the timeline, since they
...
`timeline`: set `NATESCLAW_DIAGNOSTICS_EVENT_LOOP=1` (or `on`/`true`/`yes`) on
top of enabling the timeline.

Timeline records use the `natesclaw.diagnostics.v1` envelope and can include
...
Named profiles use `/tmp/natesclaw/natesclaw-<profile>-YYYY-MM-DD.log`; for
example, `--dev` uses `natesclaw-dev-YYYY-MM-DD.log`.
...
```bash
natesclaw logs --plain
# Named profile example:
natesclaw --profile work logs --plain
```

Filter for Telegram HTTP diagnostics:

```bash
natesclaw logs --plain --limit 5000 | rg "telegram http error"
```

Filter for Brave Search HTTP diagnostics:

```bash
natesclaw logs --plain --limit 5000 | rg "brave http"
```

Or tail while reproducing:

```bash
natesclaw logs --follow --plain | rg "telegram http error"
```

For remote gateways, use `natesclaw logs --follow` instead (see
[/cli/logs](/cli/logs)).

...
function formatGatewayAncestryBlockMessage(pid: number): string {
  return `natesclaw update detected it is running inside the gateway process tree.
Gateway PID ${pid} is an ancestor of this process, so this updater cannot safely stop or restart the gateway that owns it.
Run \`${replaceCliName(formatCliCommand("natesclaw update"), CLI_NAME)}\` from a shell outside the gateway service, or stop the gateway service first and then update.`;
}
...
export async function maybeStopManagedServiceBeforeMutableUpdate(params: {
  ...
  if (serviceMatchesMutationRoot === false) {
    if (!params.jsonMode) {
      defaultRuntime.log(
        theme.muted(
          `Managed gateway service points at a different Natesclaw root; leaving it running during this ${params.updateInstallKind} update.`,
        ),
      );
    }
    return {
  ...
  const handoffSupervisorMayRespawn =
    process.platform !== "darwin" && process.env.NATESCLAW_UPDATE_RUN_HANDOFF === "1";
...
function isRunningInsideGatewayService(
  ...
  if (env.NATESCLAW_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER) {
    return false;
  }
  const serviceKind = env.NATESCLAW_SERVICE_KIND?.trim();
...
export async function resolvePackageRuntimePreflight(params: {
  ...
  return resultError(
    [
      `${runtimeLabel} is too old for natesclaw@${targetVersion}.`,
      `The requested package requires ${status.nodeEngine}.`,
      runtime.nodeRunner
        ? "Upgrade the Node runtime that owns the managed Gateway service, then rerun `natesclaw update`."
        : "Upgrade to Node 22.22.3+, Node 24.15.0+, or Node 25.9.0+, then rerun `natesclaw update`.",
      "Bare `npm i -g natesclaw` can silently install an older compatible release.",
      "After upgrading Node, use `npm i -g natesclaw@latest`.",
    ].join("\n"),
  );
}
...
export function stripGatewayServiceMarkerEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  delete resolvedEnv.NATESCLAW_SERVICE_MARKER;
  delete resolvedEnv.NATESCLAW_SERVICE_KIND;
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("natesclaw completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
export async function maybeRestartService(params: {
  ...
  const verifyRestartedGateway = async (
    ...
    const diagnosticLines = [
      ...
      `Run \`${replaceCliName(formatCliCommand("natesclaw gateway status --deep"), CLI_NAME)}\` for details.`,
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("natesclaw gateway restart"), CLI_NAME)}`,
          ),
        );
      }
      if (
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("natesclaw doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("natesclaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("natesclaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }

...
function loadedConfig(config: NatesclawConfig | undefined): NatesclawConfig {
  return migratePersistedImplicitMainRoster(config ?? {}).config as NatesclawConfig;
}

function resolveSandboxConfigForAgent(config: NatesclawConfig, agentId: string) {
  return resolveSandboxConfigForAgentBase(loadedConfig(config), agentId);
}

function resolveSandboxToolPolicyForAgent(config: NatesclawConfig, agentId: string) {
...
describe("sandbox/tool-policy", () => {
  ...
  it("keeps blocked-tool guidance glob-aware and shell-safe", () => {
    ...
    expect(message).toContain(
      "natesclaw sandbox explain --session 'agent:main:weird session;rm -rf /'",
    );
  });
  ...
  it.each([
    ...
    ({ sessionKey, expectedLabel }) => {
      ...
      expect(message).toContain(`natesclaw sandbox explain --session '${sessionKey}'`);
    },
  );
  ...
  it("avoids terminal injection for control-character session keys", () => {
    ...
    expect(message).toContain("natesclaw sandbox explain --agent main");

...
vi.mock("../process/supervisor/index.js", () => {
  ...
  const applySegmentShellEffects = (segment: string, env: NodeJS.ProcessEnv) => {
    if (segment === 'export PATH="${NATESCLAW_PREPEND_PATH}${PATH:+:$PATH}"') {
      const prepend = env.NATESCLAW_PREPEND_PATH ?? "";
      const current = readEnvPath(env);
      writeEnvPath(env, `${prepend}${current ? `:${current}` : ""}`);
      return;
    }
    if (segment === "unset NATESCLAW_PREPEND_PATH") {
      delete env.NATESCLAW_PREPEND_PATH;
    }
  };
...
const defaultShell = isWin
  ? undefined
  : process.env.NATESCLAW_TEST_SHELL || getBashShellConfig().shell;
...
const SHELL_ENV_KEYS = ["NATESCLAW_EXEC_SHELL_SNAPSHOT", "SHELL"] as const;
const PATH_SHELL_ENV_KEYS = ["NATESCLAW_EXEC_SHELL_SNAPSHOT", "PATH", "SHELL"] as const;
...
const notifyCfg = {} as NatesclawConfig;

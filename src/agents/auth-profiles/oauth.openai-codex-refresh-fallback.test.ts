...
describe("resolveApiKeyForProfile openai refresh fallback", () => {
  ...
  beforeAll(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "natesclaw-codex-refresh-fallback-"));
  ...
  beforeEach(async () => {
    ...
    setTestEnvValue("NATESCLAW_STATE_DIR", caseRoot);
    setTestEnvValue("NATESCLAW_AGENT_DIR", agentDir);

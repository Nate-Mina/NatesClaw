...
import { closeNatesclawAgentDatabaseByPath } from "../state/natesclaw-agent-db.js";
...
function resolveDoctorSessionSqliteConfig(options: DoctorSessionSqliteOptions): NatesclawConfig {
  if (options.cfg) {
    return options.cfg;
  }
...
function compactSqliteDatabase(
  ...
  try {
    if (options.closeImportedHandle) {
      closeNatesclawAgentDatabaseByPath(resolveTargetSqlitePath(target));
    }

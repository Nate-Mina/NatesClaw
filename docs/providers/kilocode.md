---
summary: "Use Kilo Gateway's unified API to access many models in Natesclaw"
title: "Kilo Gateway"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in Natesclaw
---
...
```bash
natesclaw plugins install @openclaw/kilocode-provider
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    natesclaw onboard --auth-choice kilocode-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    natesclaw models list --provider kilocode
    ```
  </Step>
</Steps>
...
Natesclaw does not publish a task-to-upstream-model mapping for it; routing behind
`kilo-auto/balanced` is owned by Kilo Gateway.

At startup Natesclaw queries `GET https://api.kilo.ai/api/gateway/models` and merges discovered models
...
`natesclaw models list --provider kilocode` to see the full discovered list.
...
<AccordionGroup>
  <Accordion title="Transport and compatibility">
    ...
    - Gemini-backed Kilo refs stay on the proxy-Gemini path: Natesclaw sanitizes Gemini thought
  ...
  <Accordion title="Stream wrapper and reasoning">
    The Kilo stream wrapper adds an `X-KILOCODE-FEATURE` request header (default `natesclaw`,
  ...
  <Accordion title="Troubleshooting">
    - If model discovery fails at startup, Natesclaw falls back to the static catalog containing `kilocode/kilo-auto/balanced`.
    - Confirm your API key is valid and that your Kilo account has the desired models enabled.
    - When Gateway runs as a daemon, ensure `KILOCODE_API_KEY` is available to that process (for example in `~/.natesclaw/.env` or via `env.shellEnv`).

...
import { resetFileLockStateForTest } from "natesclaw/plugin-sdk/file-lock";
...
const DATABASE_PREFIX = "natesclaw-matrix-persistence-test";
const OTHER_DATABASE_PREFIX = "natesclaw-matrix-persistence-other-test";
...
describe("Matrix IndexedDB persistence", () => {
  ...
  it("blocks runtime restore and persistence until doctor migrates the legacy snapshot", async () => {
    ...
    await expect(restoreIdbFromDisk(snapshotPath)).rejects.toMatchObject({
      ...
      remediation: "natesclaw doctor --fix",
    });

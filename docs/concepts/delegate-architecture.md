---
summary: "Delegate architecture: running Natesclaw as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Run Natesclaw as a **named delegate**: an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human - it sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate

A delegate is an Natesclaw agent that:
...
Natesclaw's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Natesclaw's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.natesclaw/workspace-delegate",
  tools: {
...
- Cron run history: Natesclaw's shared SQLite state database.
- Session transcripts: `~/.natesclaw/agents/delegate/sessions`.
- Identity provider audit logs (Exchange, Google Workspace).

All delegate actions flow through Natesclaw's session store. For compliance, retain and review these logs.
...
```bash
natesclaw agents add delegate --workspace ~/.natesclaw/workspace-delegate
```

This creates:

- Workspace: `~/.natesclaw/workspace-delegate`
- Agent state: `~/.natesclaw/agents/delegate/agent`
- Sessions: `~/.natesclaw/agents/delegate/sessions`
...
```json5
{
  agents: {
    entries: {
      main: { default: true, workspace: "~/.natesclaw/workspace" },
      delegate: {
        workspace: "~/.natesclaw/workspace-delegate",
        tools: {
          deny: ["browser", "canvas"],
...
~/.natesclaw/agents/delegate/agent/auth-profiles.json
```
  ...
  agents: {
    entries: {
      ...
      "org-assistant": {
        ...
        workspace: "~/.natesclaw/workspace-org",
        agentDir: "~/.natesclaw/agents/org-assistant/agent",

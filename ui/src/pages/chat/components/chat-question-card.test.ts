...
async function panelIn(container: HTMLElement): Promise<ChatQuestionPanelElement> {
  const panel = container.querySelector("natesclaw-chat-question-panel") as ChatQuestionPanelElement;
...
describe("shared question panel", () => {
  ...
  function drawGateway(
    ...
    const redraw = () => {
      render(
        html`<natesclaw-chat-question-panel

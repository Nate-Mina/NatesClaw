---
summary: "Natesclaw logging: rolling diagnostics file log + unified log privacy flags"
read_when:
...
- Location: `~/Library/Logs/Natesclaw/diagnostics.jsonl`.
...
## Enable for Natesclaw (`ai.natesclaw`)

Write the plist to a temp file first, then install it atomically as root:

```bash
cat <<'EOF' >/tmp/ai.natesclaw.plist
...
EOF
sudo install -m 644 -o root -g wheel /tmp/ai.natesclaw.plist /Library/Preferences/Logging/Subsystems/ai.natesclaw.plist
```
...
- Remove the override: `sudo rm /Library/Preferences/Logging/Subsystems/ai.natesclaw.plist`.

...
private let webChatSwiftLogger = Logger(subsystem: "ai.natesclaw", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "natesclaw.webchat.thinkingLevel"
private let webChatVerboseLevelDefaultsKey = "natesclaw.webchat.verboseLevel"

private enum WebChatSwiftUILayout {
    static let windowSize = NSSize(width: 960, height: 700)
    static let windowMinSize = NSSize(width: 640, height: 420)
    static let windowFrameAutosaveName = "NatesclawChatWindow"
}

enum WebChatTracePreferences {
    static func displayOptions(defaults: UserDefaults = AppDefaults.standard) -> NatesclawChatDisplayOptions {
        if let legacyValue = defaults.object(
            forKey: NatesclawChatWindowShell.assistantTraceDefaultsKey) as? Bool
        {
            for key in [
                NatesclawChatWindowShell.assistantReasoningDefaultsKey,
                NatesclawChatWindowShell.assistantToolActivityDefaultsKey,
            ] where defaults.object(forKey: key) == nil {
                defaults.set(legacyValue, forKey: key)
            }
        }

        var options: NatesclawChatDisplayOptions = []
        if defaults.object(forKey: NatesclawChatWindowShell.assistantReasoningDefaultsKey) as? Bool ?? true {
            options.insert(.reasoning)
        }
        if defaults.object(forKey: NatesclawChatWindowShell.assistantToolActivityDefaultsKey) as? Bool ?? true {
...
struct MacGatewayChatTransport: NatesclawChatTransport {
    ...
    func requestFullMessage(sessionKey: String, messageID: String) async throws -> NatesclawChatMessage? {
        let target = self.sessionTarget(for: sessionKey)
    ...
    static func fullMessageRequest(
        sessionKey: String,
        agentID: String?,
        messageID: String) throws -> NatesclawChatGatewayRequest
    {
        ...
        return try NatesclawChatGatewayRequest(
    ...
    {
        await NatesclawChatWidgetURLResolver.resolveResource(
            target: path,
    ...
    func listModels() async throws -> [NatesclawChatModelChoice] {
        do {
            let data = try await connection.request(NatesclawChatGatewayRequests.modelsList())
            return try NatesclawChatGatewayPayloadCodec.decodeModelChoices(data)
        } catch {
        ...
        serverLease: GatewayConnection.ServerLease?) async throws -> Bool
    {
        ...
        return try JSONDecoder().decode(NatesclawChatMetadataCapabilities.self, from: data).swarmEnabled
    }
        ...
        archived: Bool) async throws -> NatesclawChatSessionsListResponse
    {
        let request = NatesclawChatGatewayRequests.sessionsList(
            ...
            archived: archived)
        let data = try await connection.request(request)
        let decoded = try JSONDecoder().decode(NatesclawChatSessionsListResponse.self, from: data)
        let mainSessionKey = await connection.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            NatesclawChatSessionsDefaults(
    ...
    private func listChildSessions(
        parentKey: String,
        serverLease: GatewayConnection.ServerLease?) async throws -> [NatesclawChatSessionEntry]
    {
    ...
    func listAgents() async throws -> NatesclawChatAgentsListResponse? {
        let data = try await connection.request(NatesclawChatGatewayRequests.agentsList())
    ...
    func listSessionGroups() async throws -> NatesclawChatSessionGroupsResponse? {
        let data = try await connection.request(NatesclawChatGatewayRequests.sessionGroupsList())
        return try JSONDecoder().decode(NatesclawChatSessionGroupsResponse.self, from: data)
    }

    func putSessionGroups(names: [String]) async throws -> NatesclawChatSessionGroupsMutationResponse {
        let request = NatesclawChatGatewayRequests.sessionGroupsPut(names: names)
        let data = try await connection.request(request)
    ...
    func renameSessionGroup(
        name: String,
        to: String) async throws -> NatesclawChatSessionGroupsMutationResponse
    {
        let request = NatesclawChatGatewayRequests.sessionGroupsRename(name: name, to: to)
        let data = try await connection.request(request)
        return try JSONDecoder().decode(NatesclawChatSessionGroupsMutationResponse.self, from: data)
    }

    func deleteSessionGroup(name: String) async throws -> NatesclawChatSessionGroupsMutationResponse {
        let request = NatesclawChatGatewayRequests.sessionGroupsDelete(name: name)
        let data = try await connection.request(request)
        return try JSONDecoder().decode(NatesclawChatSessionGroupsMutationResponse.self, from: data)
    }
    ...
    private func patchSessionSettings(
        ...
        serverLease: GatewayConnection.ServerLease?) async throws -> NatesclawChatModelPatchResult?
    {
        let target = NatesclawChatSessionTarget.resolve(
            sessionKey,
            selectedAgentID: self.routingIdentity.currentAgentID(),
        ...
        return try JSONDecoder().decode(NatesclawChatModelPatchResult.self, from: data)
    }

    static func sessionSettingsRequest(
        sessionKey: String,
        agentID: String?,
        patch: NatesclawChatSessionSettingsPatch) -> NatesclawChatGatewayRequest
    {
        NatesclawChatGatewayRequests.patchSessionSettings(
            sessionKey: sessionKey,
            agentID: agentID,
            model: patch.model,
    ...
    func synthesizeSpeech(text: String) async throws -> NatesclawChatSpeechClip {
        ...
        worktreeBaseRef: String?) async throws -> NatesclawChatCreateSessionResponse
    {
        let agentID = explicitAgentID
            ?? NatesclawChatSessionKey.agentID(from: key)
            ?? parentSessionKey.flatMap { NatesclawChatSessionKey.agentID(from: $0) }
            ?? self.routingIdentity.currentAgentID()
        let request = NatesclawChatGatewayRequests.createSession(
            key: key,
            agentID: agentID,
            label: label,
            parentSessionKey: parentSessionKey,
    ...
    func listQuestions() async throws -> [QuestionRecord] {
        let data = try await connection.request(NatesclawChatGatewayRequests.questionList())
    ...
    func listTasks(sessionKey: String, agentID: String?) async throws -> [TaskSummary] {
        let data = try await connection.request(NatesclawChatGatewayRequests.tasksList(
            sessionKey: sessionKey,
            agentID: agentID))
    ...
    func getQuestion(id: String) async throws -> QuestionRecord {
        let data = try await connection.request(NatesclawChatGatewayRequests.questionGet(id: id))
    ...
    func resolveQuestion(id: String, answers: [String: [String]]) async throws {
        _ = try await self.connection.request(
            NatesclawChatGatewayRequests.resolveQuestion(id: id, answers: answers))
    }

    func cancelQuestion(id: String) async throws {
        _ = try await self.connection.request(
            NatesclawChatGatewayRequests.cancelQuestion(id: id))
    }
        ...
        timeoutMs: Int) async -> NatesclawChatRunObservation
    {
        ...
        do {
            ...
            let data = try await connection.request(
                request,
                ifCurrentRoute: route)
            return try NatesclawChatGatewayPayloadCodec.decodeAgentWaitObservation(data)
        } catch {
            webChatSwiftLogger.warning(
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> NatesclawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                NatesclawGatewayHealthOK.self,
                from: JSONEncoder().encode(hello.snapshot.health)))?.ok ?? true
            return .health(ok: ok)

        case let .event(evt):
            return NatesclawChatGatewayPayloadCodec.event(from: evt)
...
private enum MacChatMessageSpeechClient {
    ...
    static func synthesize(
        ...
        connection: GatewayConnection) async throws -> NatesclawChatSpeechClip
    {
        ...
        return NatesclawChatSpeechClip(
            data: audioData,
...
private struct MacChatSurface: View {
    @State private var viewModel: NatesclawChatViewModel
    ...
    @AppStorage(NatesclawChatWindowShell.assistantReasoningDefaultsKey, store: AppDefaults.standard)
    private var showsReasoning = WebChatTracePreferences.displayOptions().contains(.reasoning)
    @AppStorage(NatesclawChatWindowShell.assistantToolActivityDefaultsKey, store: AppDefaults.standard)
    ...
    private let speech: NatesclawChatSpeechController
    private let voiceNoteRecorder: NatesclawVoiceNoteRecorder

    init(
        viewModel: NatesclawChatViewModel,
        userAccent: Color?,
        usesPrimaryAppRuntime: Bool,
        speech: NatesclawChatSpeechController,
        voiceNoteRecorder: NatesclawVoiceNoteRecorder)
    {
    ...
    private var talkControl: NatesclawChatTalkControl {
        NatesclawChatTalkControl(
    ...
    private var displayOptions: NatesclawChatDisplayOptions {
        var options: NatesclawChatDisplayOptions = []
    ...
    private var voiceNoteControl: NatesclawChatVoiceNoteControl {
        NatesclawChatVoiceNoteControl(
    ...
    private static let emptyAssistantPrompts: [NatesclawChatView.StarterPrompt] = [
        .init(
            id: "check-status",
            title: String(localized: "Check Natesclaw status"),
            prompt: String(localized: "Summarize the current Natesclaw status and tell me what needs attention.")),
        .init(
...
final class WebChatSwiftUIWindowController: NSObject, NSWindowDelegate {
    ...
    private let viewModel: NatesclawChatViewModel
    ...
    convenience init(
        ...
        windowTitle: String = "Natesclaw Chat",
    ...
    convenience init(
        ...
        initialDraft: String? = nil,
        connection: GatewayConnection = .shared,
        cachedRoutingIdentity: NatesclawChatSessionRoutingIdentity?,
        store: NatesclawChatSQLiteTranscriptCache?,
        windowTitle: String = "Natesclaw Chat",
        windowAutosaveName: String = WebChatSwiftUILayout.windowFrameAutosaveName)
    {
    ...
    init(
        sessionKey: String,
        initialDraft: String? = nil,
        transport: any NatesclawChatTransport,
        initialActiveAgentID: String? = nil,
        explicitAgentID: String? = nil,
        initialSessionRoutingContract: String? = nil,
        transcriptCache: (any NatesclawChatTranscriptCache)? = nil,
        outbox: (any NatesclawChatCommandOutbox)? = nil,
        windowTitle: String = "Natesclaw Chat",
        windowAutosaveName: String = WebChatSwiftUILayout.windowFrameAutosaveName)
    {
        ...
        self.voiceNoteRecorder = voiceNoteRecorder
        let speech = NatesclawChatSpeechController { text in
        ...
        Task { @MainActor [weak vm] in
            ...
            for await push in pushes {
                ...
                let routingIdentity: NatesclawChatSessionRoutingIdentity? = if let route {
                ...
                if let routingIdentity {
                    ...
                    if let store = transcriptCache as? NatesclawChatSQLiteTranscriptCache,
                       !usesPrimaryAppRuntime || store.gatewayID == MacChatTranscriptCache.currentGatewayID(),
                       let persistedIdentity = NatesclawChatSessionRoutingIdentity(
    ...
    static func persistedVerboseLevel(defaults: UserDefaults = AppDefaults.standard) -> String? {
        let stored = defaults.string(forKey: webChatVerboseLevelDefaultsKey)?
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
        return NatesclawChatViewModel.verboseLevelOptions.contains(stored ?? "") ? stored : nil
    }

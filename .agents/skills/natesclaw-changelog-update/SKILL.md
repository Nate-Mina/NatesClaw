---
name: natesclaw-changelog-update
description: Regenerate Natesclaw release changelog sections from git history before beta, stable, or extended-stable releases.
---

# Natesclaw Changelog Update
...
Use it with `release-natesclaw-maintainer`; this skill owns changelog content,
...
3. Generate the complete contribution record and editorial manifest before
   ...
   ```bash
   node --import tsx .agents/skills/natesclaw-changelog-update/scripts/verify-release-notes.mjs \
     --base <base-tag> \
     --target <target-ref> \
     --main-ref origin/main \
     --version <YYYY.M.PATCH> \
     --manifest /tmp/natesclaw-release-<YYYY.M.PATCH>.json \
     --write-ledger
   ```
...
6. Preserve attribution:
   ...
   - never thank bots, `@claude`, `@natesclaw`, `@clawsweeper`, or `@steipete`
...
- unqualified `#NNN` references resolve against `natesclaw/natesclaw`;
  cross-repository references such as `natesclaw/imsg#141` remain literal

...
- **Board widgets** are session state: bytes live in the owning agent's SQLite
  ...
  (`/__natesclaw__/board/<agentId>/<sessionKey>/<name>/`) that reads the DB.
...
The **widget is the Natesclaw primitive**: the named, pinned, sized,
...
Natesclaw's — so `show_widget` code stays as short as it is today and never
...
- **Host tools for `html` widgets** (exposed over the widget bridge, checked
  against the grant):
  - `natesclaw.prompt.send` — tier 2; routed through the visible composer,
    user-confirmed unless granted
  - `natesclaw.state.emit` — tier 1 session notices (coalesced, size-capped)
  - `natesclaw.data.read` — parameterized read-only bindings (existing
    allowlisted read RPC set), resolved gateway-side
  - `natesclaw.cron.trigger` — tier 3 automation
...
- **Grants.** A widget declaring nothing renders immediately (sandboxed,
  ...
  declaration shrank.
- **Authoring shim.** The document wrapper injects `window.natesclaw.prompt`,
  `window.natesclaw.state`, `window.natesclaw.data`, and `window.natesclaw.cron`
...
and `dashboard.actionVerbs` in `natesclaw.plugin.json`. Plugin-local ids become
...
plugin registration, Natesclaw verifies that every binding targets an RPC
...
**Accepted tradeoff:** Natesclaw does not gate scriptable widgets on this
residual. Widget content gains access to sensitive Natesclaw data only through
...
New tables in `agents/<agentId>/agent/natesclaw-agent.sqlite`
...
- **Widget hosting moves from `extensions/canvas` to core.** The canvas doc
  ...
  `/__natesclaw__/canvas` paths are shipped native-client contracts and stay
...
| T5  | `claude/dashboard-capabilities`      | Grant store/UI + byte freezing; move `html` widgets onto the shared sandbox host; host tools (`natesclaw.prompt.send/state.emit/data.read/cron.trigger`); `net` CSP; authoring shim | T3, T4                           |

...
function stripNatesclawPackageRunner(argv: string[]): string[] {
  const commandName = normalizeCommandBaseName(argv[0]);
  if (commandName === "natesclaw") {
    return argv;
  }
  if (
    (commandName === "pnpm" || commandName === "npm" || commandName === "yarn") &&
    normalizeCommandBaseName(argv[1]) === "natesclaw"
  ) {
    return argv.slice(1);
  }
  if (
    ...
    normalizeCommandBaseName(argv[2]) === "natesclaw"
  ) {
    return argv.slice(2);
  }
  if (commandName === "npx" || commandName === "bunx") {
    ...
    if (normalizeCommandBaseName(argv[idx]) === "natesclaw") {
...
function parseNatesclawChannelsLoginShellCommand(raw: string): boolean {
  const argv = splitShellArgs(raw);
  if (!argv) {
    return false;
  }
  const natesclawArgv = stripNatesclawPackageRunner(argv);
  return (
    normalizeCommandBaseName(natesclawArgv[0]) === "natesclaw" &&
    (natesclawArgv[1] === "channels" || natesclawArgv[1] === "channel") &&
    natesclawArgv[2] === "login"
  );
}
...
export async function detectUnsafeExecControlShellCommand(
  ...
  for (const candidate of candidates) {
    ...
    if (parseNatesclawChannelsLoginShellCommand(candidate)) {
...
export async function rejectUnsafeExecControlShellCommand(command: string): Promise<void> {
  ...
  if (unsafeKind === "channel-login") {
    throw new Error(
      [
        "exec cannot run interactive Natesclaw channel login commands.",
        "Run `natesclaw channels login` in a terminal on the gateway host, or use the channel-specific login agent tool when available (for WhatsApp: `whatsapp_login`).",
      ].join(" "),
    );
  }
}

...
import { getApiProvider } from "@natesclaw/ai/internal/runtime";
...
function writeModelsJson(contents: unknown): string {
  const dir = mkdtempSync(join(tmpdir(), "natesclaw-model-registry-"));
  tempDirs.push(dir);
...
describe("ModelRegistry models.json auth", () => {
  ...
  it("keeps authored provider models available when the plugin catalog database is corrupt", () => {
    ...
    writeFileSync(join(dirname(modelsPath), "natesclaw-agent.sqlite"), "not a SQLite database");

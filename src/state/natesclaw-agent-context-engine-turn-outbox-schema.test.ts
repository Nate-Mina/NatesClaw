...
} from "./natesclaw-agent-context-engine-turn-outbox-schema.js";
import {
...
describe("context-engine turn outbox schema", () => {
  it("opens a current database without the additive table and installs it on first use", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "natesclaw-context-outbox-schema-"));
    tempDirs.push(stateDir);
    const options = {
      agentId: "main",
      env: { NATESCLAW_STATE_DIR: stateDir },
    };
    const initial = openNatesclawAgentDatabase(options);
    ...
    const reopened = openNatesclawAgentDatabase(options);
    expect(
      reopened.db

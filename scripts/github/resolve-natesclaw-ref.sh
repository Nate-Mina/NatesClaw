...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-natesclaw-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves Natesclaw branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve Natesclaw ref: ${REF}" >&2
exit 1
